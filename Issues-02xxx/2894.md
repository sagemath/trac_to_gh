# Issue 2894: [posititve] notebook -- cache elements to improve the speed of get_cell()

archive/issues_002894.json:
```json
{
    "assignees": [
        "https://github.com/boothby"
    ],
    "body": "The following code takes .004ms/call, which is about 5 times faster than the existing get_cell():\n\n```\nvar cell_element_cache = [];\nfunction get_cell2(id) {\n    var v = cell_element[id];\n    if(v == undefined)\n        v = cell_element[id] = get_cell(id)\n    return v;\n}\n```\n\nIt follows that we should update get_cell to the 5-times faster version, since get_cell is called  quite frequently in the notebook code.\n\nas tested with\n\n```\nvar t0;\nvar e;\nvar n = cell_id_list[cell_id_list.length-1];\nvar N = 100000.;\nt0 = time_now();\nfor(i=0;i<N;i++)\n   e = get_cell(n);\nt1 = time_now();\nalert((t1-t0)/N);\n```\n\nComponent: **notebook**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/2894_\n\n",
    "closed_at": "2008-04-12T12:44:38Z",
    "created_at": "2008-04-12T07:51:42Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "[posititve] notebook -- cache elements to improve the speed of get_cell()",
    "type": "issue",
    "updated_at": "2008-04-12T12:51:23Z",
    "url": "https://github.com/sagemath/sage/issues/2894",
    "user": "https://github.com/boothby"
}
```
The following code takes .004ms/call, which is about 5 times faster than the existing get_cell():

```
var cell_element_cache = [];
function get_cell2(id) {
    var v = cell_element[id];
    if(v == undefined)
        v = cell_element[id] = get_cell(id)
    return v;
}
```

It follows that we should update get_cell to the 5-times faster version, since get_cell is called  quite frequently in the notebook code.

as tested with

```
var t0;
var e;
var n = cell_id_list[cell_id_list.length-1];
var N = 100000.;
t0 = time_now();
for(i=0;i<N;i++)
   e = get_cell(n);
t1 = time_now();
alert((t1-t0)/N);
```

Component: **notebook**

_Issue created by migration from https://trac.sagemath.org/ticket/2894_





---

archive/issue_events_030711.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2008-04-12T07:51:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2894",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20porting%3A%20cygwin",
    "label_color": "000080",
    "label_name": "c: porting: cygwin",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2894#event-30711"
}
```



---

archive/issue_events_030712.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2008-04-12T07:51:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2894",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2894#event-30712"
}
```



---

archive/issue_events_030713.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2008-04-12T07:51:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2894",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2894#event-30713"
}
```



---

archive/issue_events_030714.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-12T07:51:42Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/2894",
    "subject": "https://github.com/boothby",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2894#event-30714"
}
```



---

archive/issue_events_030715.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2008-04-12T07:52:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2894",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2894#event-30715"
}
```



---

archive/issue_events_030716.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2008-04-12T07:52:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2894",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2894#event-30716"
}
```



---

archive/issue_events_030717.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2008-04-12T07:52:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2894",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20porting%3A%20cygwin",
    "label_color": "000080",
    "label_name": "c: porting: cygwin",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2894#event-30717"
}
```



---

archive/issue_events_030718.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2008-04-12T07:52:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2894",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "000080",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2894#event-30718"
}
```



---

archive/issue_events_030719.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-12T07:52:06Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/2894",
    "subject": "https://github.com/boothby",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2894#event-30719"
}
```



---

archive/issue_events_030720.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2008-04-12T07:52:06Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/2894",
    "subject": "https://github.com/boothby",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2894#event-30720"
}
```



---

archive/issue_events_030721.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2008-04-12T07:52:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2894",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2894#event-30721"
}
```



---

archive/issue_events_030722.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2008-04-12T07:52:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2894",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2894#event-30722"
}
```



---

archive/issue_comments_015126.json:
```json
{
    "body": "Attachment: **[2894-notebookperformance.patch.gz](https://github.com/sagemath/sage/files/ticket2894/2894-notebookperformance.patch.gz)**",
    "created_at": "2008-04-12T07:55:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2894",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2894#issuecomment-15126",
    "user": "https://github.com/boothby"
}
```

Attachment: **[2894-notebookperformance.patch.gz](https://github.com/sagemath/sage/files/ticket2894/2894-notebookperformance.patch.gz)**



---

archive/issue_events_030723.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-12T12:44:38Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2894",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2894#event-30723"
}
```



---

archive/issue_comments_015127.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nHi Tom,\n\nthis patch is identical to #2887 where William posted your code except for the added comment by you. Since #2887 has been credited to you I will merge the extra comment and close this as fixed.\n\nThought?\n\nCheers,\n\nMichael",
    "created_at": "2008-04-12T12:44:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2894",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2894#issuecomment-15127",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Hi Tom,

this patch is identical to #2887 where William posted your code except for the added comment by you. Since #2887 has been credited to you I will merge the extra comment and close this as fixed.

Thought?

Cheers,

Michael



---

archive/issue_events_030724.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-12T12:44:38Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/2894",
    "title_is": "[posititve] notebook -- cache elements to improve the speed of get_cell()",
    "title_was": "notebook -- cache elements to improve the speed of get_cell()",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2894#event-30724"
}
```



---

archive/issue_events_030725.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-12T12:44:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2894",
    "milestone_number": null,
    "milestone_title": "sage-3.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2894#event-30725"
}
```



---

archive/issue_comments_015128.json:
```json
{
    "body": "Since #2887 was merged only merge the extra comment from the previous patch",
    "created_at": "2008-04-12T12:50:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2894",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2894#issuecomment-15128",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

Since #2887 was merged only merge the extra comment from the previous patch



---

archive/issue_comments_015129.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nAttachment: **[trac_2887-comment-from-2894.patch.gz](https://github.com/sagemath/sage/files/ticket2894/trac_2887-comment-from-2894.patch.gz)**\n\nMerged trac_2887-comment-from-2894.patch in Sage 3.0.alpha4",
    "created_at": "2008-04-12T12:51:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2894",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2894#issuecomment-15129",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Attachment: **[trac_2887-comment-from-2894.patch.gz](https://github.com/sagemath/sage/files/ticket2894/trac_2887-comment-from-2894.patch.gz)**

Merged trac_2887-comment-from-2894.patch in Sage 3.0.alpha4
