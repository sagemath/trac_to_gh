# Issue 2295: build cache check fails on paths containing symlinks

archive/issues_002295.json:
```json
{
    "assignees": [
        "https://github.com/burcin"
    ],
    "body": "My SAGE_ROOT contains a symlinked component, upgrading from 2.10.2.alpha0 to 2.10.2 failed with the error message in this thread:\n\nhttp://groups.google.com/group/sage-support/browse_thread/thread/d8ee3de015fbf7be\n\nOnly the filename listed was different.\n\nThis is caused by the module_path function in setup.py, assuming os.path.abspath starts with SAGE_ROOT, which is not the case when path contains a symlink. Attached patch fixes this issue.\n\n\n\nComponent: **misc**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/2295_\n\n",
    "closed_at": "2008-02-24T20:45:35Z",
    "created_at": "2008-02-24T20:00:54Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.10.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "build cache check fails on paths containing symlinks",
    "type": "issue",
    "updated_at": "2008-02-24T20:45:35Z",
    "url": "https://github.com/sagemath/sage/issues/2295",
    "user": "https://github.com/burcin"
}
```
My SAGE_ROOT contains a symlinked component, upgrading from 2.10.2.alpha0 to 2.10.2 failed with the error message in this thread:

http://groups.google.com/group/sage-support/browse_thread/thread/d8ee3de015fbf7be

Only the filename listed was different.

This is caused by the module_path function in setup.py, assuming os.path.abspath starts with SAGE_ROOT, which is not the case when path contains a symlink. Attached patch fixes this issue.



Component: **misc**

_Issue created by migration from https://trac.sagemath.org/ticket/2295_





---

archive/issue_events_023536.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2008-02-24T20:00:54Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2295",
    "milestone_number": null,
    "milestone_title": "sage-2.10.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2295#event-23536"
}
```



---

archive/issue_events_023537.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2008-02-24T20:00:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2295",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2295#event-23537"
}
```



---

archive/issue_events_023538.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2008-02-24T20:00:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2295",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2295#event-23538"
}
```



---

archive/issue_events_023539.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2008-02-24T20:00:54Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/2295",
    "subject": "https://github.com/burcin",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2295#event-23539"
}
```



---

archive/issue_events_023540.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2008-02-24T20:00:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2295",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2295#event-23540"
}
```



---

archive/issue_comments_011325.json:
```json
{
    "body": "Attachment: **[build_cache_symlink.patch.gz](https://github.com/sagemath/sage/files/ticket2295/build_cache_symlink.patch.gz)**\n\nfix build cache problem when sage_root has symlinks",
    "created_at": "2008-02-24T20:02:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2295",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2295#issuecomment-11325",
    "user": "https://github.com/burcin"
}
```

Attachment: **[build_cache_symlink.patch.gz](https://github.com/sagemath/sage/files/ticket2295/build_cache_symlink.patch.gz)**

fix build cache problem when sage_root has symlinks



---

archive/issue_comments_011326.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nI tested the patch with a non-symlinked $SAGE_ROOT and it keeps working. The problem has come up before and it is good that it has been finally fixed. Positive review.\n\nCheers,\n\nMichael",
    "created_at": "2008-02-24T20:44:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2295",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2295#issuecomment-11326",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:1" align="right">comment:1</div>

I tested the patch with a non-symlinked $SAGE_ROOT and it keeps working. The problem has come up before and it is good that it has been finally fixed. Positive review.

Cheers,

Michael



---

archive/issue_events_023541.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-02-24T20:45:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2295",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2295#event-23541"
}
```



---

archive/issue_comments_011327.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nMerged in Sage 2.10.3.alpha0",
    "created_at": "2008-02-24T20:45:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2295",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2295#issuecomment-11327",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Merged in Sage 2.10.3.alpha0



---

archive/issue_events_023542.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-02-24T20:45:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2295",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2295#event-23542"
}
```



---

archive/issue_events_023543.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-02-24T20:45:35Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2295",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2295#event-23543"
}
```
