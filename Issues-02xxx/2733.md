# Issue 2733: PARI in Debian has the mathnf bug

archive/issues_002733.json:
```json
{
    "assignees": [
        "https://github.com/timabbott"
    ],
    "body": "Is this bug important enough to bother Bill Allombert (the maintainer of PARI in Debian) to upgrade PARI in Debian?\n\nsage -t  devel/sage-main/sage/matrix/tests.py               **********************************************************************\nFile \"tests.py\", line 55:\n    sage: a.mathnf(1)[1][1,] == gp('[4, 2, 1, 0, 3, 1, 1, 0, 1, 1, 2, 2, 3, 3, 0, 0, 1, 3]')\nExpected:\n    True\nGot:\n    False\n**********************************************************************\n\n\nComponent: **debian-package**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/2733_\n\n",
    "closed_at": "2008-03-30T09:50:43Z",
    "created_at": "2008-03-30T05:14:16Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20distribution",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "PARI in Debian has the mathnf bug",
    "type": "issue",
    "updated_at": "2008-03-30T09:50:43Z",
    "url": "https://github.com/sagemath/sage/issues/2733",
    "user": "https://github.com/timabbott"
}
```
Is this bug important enough to bother Bill Allombert (the maintainer of PARI in Debian) to upgrade PARI in Debian?

sage -t  devel/sage-main/sage/matrix/tests.py               **********************************************************************
File "tests.py", line 55:
    sage: a.mathnf(1)[1][1,] == gp('[4, 2, 1, 0, 3, 1, 1, 0, 1, 1, 2, 2, 3, 3, 0, 0, 1, 3]')
Expected:
    True
Got:
    False
**********************************************************************


Component: **debian-package**

_Issue created by migration from https://trac.sagemath.org/ticket/2733_





---

archive/issue_events_028933.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-03-30T05:14:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2733",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20distribution",
    "label_color": "000080",
    "label_name": "c: distribution",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2733#event-28933"
}
```



---

archive/issue_events_028934.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-03-30T05:14:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2733",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2733#event-28934"
}
```



---

archive/issue_events_028935.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-03-30T05:14:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2733",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2733#event-28935"
}
```



---

archive/issue_events_028936.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-03-30T05:14:16Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/2733",
    "subject": "https://github.com/timabbott",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2733#event-28936"
}
```



---

archive/issue_events_028937.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-30T09:50:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2733",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2733#event-28937"
}
```



---

archive/issue_comments_014250.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nThis is not \"Sage Specific\": This is a packaging bug in Debian's pari.deb and should be filed as a bug report at the Debian bug tracker. See http://wiki.sagemath.org/TracGuidelines for the rules that up to now weren't written down.\n\nCheers,\n\nMichael",
    "created_at": "2008-03-30T09:50:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2733",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2733#issuecomment-14250",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:1" align="right">comment:1</div>

This is not "Sage Specific": This is a packaging bug in Debian's pari.deb and should be filed as a bug report at the Debian bug tracker. See http://wiki.sagemath.org/TracGuidelines for the rules that up to now weren't written down.

Cheers,

Michael



---

archive/issue_events_028938.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-30T09:50:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2733",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2733#event-28938"
}
```



---

archive/issue_events_028939.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-30T09:50:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2733",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "a6a6a6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2733#event-28939"
}
```
