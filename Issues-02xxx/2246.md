# Issue 2246: sage-2.10.2.alpha2: special.py -- fix it -- several bugs etc

archive/issues_002246.json:
```json
{
    "assignees": [],
    "body": "The attached patches fix const.tex and fix numerous small issues with special.py that would lead to subtle and not so subtle bugs and mistakes. \n\nI also fixed a problem with const.tex caused by Cremona's #1946.\n\n\n\nAssignee: **@williamstein**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/2246_\n\n",
    "closed_at": "2008-02-21T17:02:18Z",
    "created_at": "2008-02-21T16:17:48Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20calculus",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.10.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sage-2.10.2.alpha2: special.py -- fix it -- several bugs etc",
    "type": "issue",
    "updated_at": "2008-02-21T17:21:50Z",
    "url": "https://github.com/sagemath/sage/issues/2246",
    "user": "https://github.com/williamstein"
}
```
The attached patches fix const.tex and fix numerous small issues with special.py that would lead to subtle and not so subtle bugs and mistakes. 

I also fixed a problem with const.tex caused by Cremona's #1946.



Assignee: **@williamstein**

_Issue created by migration from https://trac.sagemath.org/ticket/2246_





---

archive/issue_events_019890.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-02-21T16:17:48Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2246",
    "milestone_number": null,
    "milestone_title": "sage-2.10.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2246#event-19890"
}
```



---

archive/issue_events_019891.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-02-21T16:17:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2246",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20calculus",
    "label_color": "0000ff",
    "label_name": "component: calculus",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2246#event-19891"
}
```



---

archive/issue_events_019892.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-02-21T16:17:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2246",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2246#event-19892"
}
```



---

archive/issue_events_019893.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-02-21T16:17:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2246",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2246#event-19893"
}
```



---

archive/issue_comments_011674.json:
```json
{
    "body": "Attachment: **[doc-2246.patch.gz](https://github.com/sagemath/sage/files/ticket2246/doc-2246.patch.gz)**",
    "created_at": "2008-02-21T16:22:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2246#issuecomment-11674",
    "user": "https://github.com/williamstein"
}
```

Attachment: **[doc-2246.patch.gz](https://github.com/sagemath/sage/files/ticket2246/doc-2246.patch.gz)**



---

archive/issue_comments_011675.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[sage-2246.patch.gz](https://github.com/sagemath/sage/files/ticket2246/sage-2246.patch.gz)**\n\nBoth patches look good, the doctest failures are fixed. Positive review.\n\nCheers,\n\nMichael",
    "created_at": "2008-02-21T17:01:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2246#issuecomment-11675",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[sage-2246.patch.gz](https://github.com/sagemath/sage/files/ticket2246/sage-2246.patch.gz)**

Both patches look good, the doctest failures are fixed. Positive review.

Cheers,

Michael



---

archive/issue_comments_011676.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nMerged in Sage 2.10.2.rc0",
    "created_at": "2008-02-21T17:02:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2246#issuecomment-11676",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:2'>Comment 2:</a>
Merged in Sage 2.10.2.rc0



---

archive/issue_events_019894.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-02-21T17:02:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2246",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2246#event-19894"
}
```



---

archive/issue_comments_011677.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nI forgot the change the summary, so I am doing now.\n\nCheers,\n\nMichael",
    "created_at": "2008-02-21T17:21:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2246#issuecomment-11677",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:3'>Comment 3:</a>
I forgot the change the summary, so I am doing now.

Cheers,

Michael



---

archive/issue_events_019895.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-02-21T17:21:50Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/2246",
    "title_is": "sage-2.10.2.alpha2: special.py -- fix it -- several bugs etc",
    "title_was": "[needs quick review] sage-2.10.2.alpha2: special.py -- fix it -- several bugs etc",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2246#event-19895"
}
```



---

archive/issue_events_019896.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-02-21T17:21:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2246",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2246#event-19896"
}
```
