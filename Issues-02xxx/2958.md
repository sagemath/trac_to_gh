# Issue 2958: [with patch; positive review] twist.py -- doctest failures in sage-3.0.alpha6 on some test machines

archive/issues_002958.json:
```json
{
    "body": "\n\nAssignee: boothby\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/2958\n\n",
    "closed_at": "2008-04-19T23:10:59Z",
    "created_at": "2008-04-19T21:19:08Z",
    "labels": [
        "component: notebook",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0",
    "title": "[with patch; positive review] twist.py -- doctest failures in sage-3.0.alpha6 on some test machines",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2958",
    "user": "https://github.com/williamstein"
}
```


Assignee: boothby

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/2958





---

archive/issue_comments_021399.json:
```json
{
    "body": "Attachment [sage-2958.patch](tarball://root/attachments/some-uuid/ticket2958/sage-2958.patch) by @williamstein created at 2008-04-19 21:56:35",
    "created_at": "2008-04-19T21:56:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2958#issuecomment-21399",
    "user": "https://github.com/williamstein"
}
```

Attachment [sage-2958.patch](tarball://root/attachments/some-uuid/ticket2958/sage-2958.patch) by @williamstein created at 2008-04-19 21:56:35



---

archive/issue_events_010510.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2958",
    "rename": {
        "from": "twist.py -- doctest failures in sage-3.0.alpha6 on some test machines",
        "to": "[with patch; needs review] twist.py -- doctest failures in sage-3.0.alpha6 on some test machines"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2958#event-10510"
}
```



---

archive/issue_comments_021400.json:
```json
{
    "body": "<a id='comment:2'></a>\nPatch looks good to me. I will do some more doctesting before merging this. \n\nCheers,\n\nMichael",
    "created_at": "2008-04-19T22:10:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2958#issuecomment-21400",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Patch looks good to me. I will do some more doctesting before merging this. 

Cheers,

Michael



---

archive/issue_events_010511.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2958",
    "rename": {
        "from": "[with patch; needs review] twist.py -- doctest failures in sage-3.0.alpha6 on some test machines",
        "to": "[with patch; positive review] twist.py -- doctest failures in sage-3.0.alpha6 on some test machines"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2958#event-10511"
}
```



---

archive/issue_comments_021401.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.0.rc0",
    "created_at": "2008-04-19T23:10:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2958#issuecomment-21401",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.0.rc0



---

archive/issue_events_010512.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-19T23:10:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2958",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2958#event-10512"
}
```



---

archive/issue_comments_021402.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-04-19T23:10:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2958#issuecomment-21402",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed
