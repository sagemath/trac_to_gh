# Issue 2568: [with patch, not ready for review] dokchitser port, work in progress

archive/issues_002568.json:
```json
{
    "body": "snapshot of work in progress of the port of the Dokchitser L-series calculator\n\nAssignee: @williamstein\n\nKeywords: editor_wstein\n\nResolution: duplicate\n\nIssue created by migration from https://trac.sagemath.org/ticket/2568\n\n",
    "closed_at": "2008-11-09T06:55:45Z",
    "created_at": "2008-03-17T08:59:17Z",
    "labels": [
        "component: number theory"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "[with patch, not ready for review] dokchitser port, work in progress",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2568",
    "user": "https://github.com/jbalakrishnan"
}
```
snapshot of work in progress of the port of the Dokchitser L-series calculator

Assignee: @williamstein

Keywords: editor_wstein

Resolution: duplicate

Issue created by migration from https://trac.sagemath.org/ticket/2568





---

archive/issue_events_009055.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2568",
    "rename": {
        "from": "dokchitser port, work in progress",
        "to": "[with patch, not ready for review?] dokchitser port, work in progress"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2568#event-9055"
}
```



---

archive/issue_comments_018394.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [l.py](tarball://root/attachments/some-uuid/ticket2568/l.py) by mabshoff created at 2008-04-20 06:24:57",
    "created_at": "2008-04-20T06:24:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2568",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2568#issuecomment-18394",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>
Attachment [l.py](tarball://root/attachments/some-uuid/ticket2568/l.py) by mabshoff created at 2008-04-20 06:24:57



---

archive/issue_events_009056.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2568",
    "rename": {
        "from": "[with patch, not ready for review?] dokchitser port, work in progress",
        "to": "[with patch, not ready for review] dokchitser port, work in progress"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2568#event-9056"
}
```



---

archive/issue_comments_018395.json:
```json
{
    "body": "Changing keywords from \"\" to \"editor_wstein\".",
    "created_at": "2008-06-20T04:47:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2568",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2568#issuecomment-18395",
    "user": "https://github.com/craigcitro"
}
```

Changing keywords from "" to "editor_wstein".



---

archive/issue_events_009057.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-11-09T06:55:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2568",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2568#event-9057"
}
```



---

archive/issue_comments_018396.json:
```json
{
    "body": "Resolution: duplicate",
    "created_at": "2008-11-09T06:55:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2568",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2568#issuecomment-18396",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: duplicate



---

archive/issue_events_009058.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-11-09T06:55:45Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/2568",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2568#event-9058"
}
```



---

archive/issue_comments_018397.json:
```json
{
    "body": "<a id='comment:3'></a>\nVarious people at SD 11 are working on an improved version of this code, so I am closing this as a dupe. The current code is at #4475.\n\nCheers,\n\nMichael",
    "created_at": "2008-11-09T06:55:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2568",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2568#issuecomment-18397",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Various people at SD 11 are working on an improved version of this code, so I am closing this as a dupe. The current code is at #4475.

Cheers,

Michael
