# Issue 2293: [with patch at #2292, positive review] word_problem error in AbelianGroup

archive/issues_002293.json:
```json
{
    "assignees": [
        "https://github.com/wdjoyner"
    ],
    "body": "\n```\nsage: sage: A.<a,b,c,d,e> = AbelianGroup(5,[4, 5, 5, 7, 8])\nsage: wp = word_problem([a,b,c,d,e],a); wp\n[[a, 1]]\n```\nis okay but all these are wrong:\n\n```\nsage: wp = word_problem([a,b,c,d,e],b); wp\n[[a, 1]]\nsage: wp = word_problem([a,b,c,d,e],c); wp\n[[a, 1]]\nsage: wp = word_problem([a,b,c,d,e],d); wp\n[[a, 1]]\nsage: wp = word_problem([a,b,c,d,e],e); wp\n[[a, 1]]\n```\n\n\nComponent: **group theory**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/2293_\n\n",
    "closed_at": "2008-02-27T23:10:18Z",
    "created_at": "2008-02-24T15:33:04Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20group%20theory",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.10.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "[with patch at #2292, positive review] word_problem error in AbelianGroup",
    "type": "issue",
    "updated_at": "2008-02-27T23:10:18Z",
    "url": "https://github.com/sagemath/sage/issues/2293",
    "user": "https://github.com/wdjoyner"
}
```

```
sage: sage: A.<a,b,c,d,e> = AbelianGroup(5,[4, 5, 5, 7, 8])
sage: wp = word_problem([a,b,c,d,e],a); wp
[[a, 1]]
```
is okay but all these are wrong:

```
sage: wp = word_problem([a,b,c,d,e],b); wp
[[a, 1]]
sage: wp = word_problem([a,b,c,d,e],c); wp
[[a, 1]]
sage: wp = word_problem([a,b,c,d,e],d); wp
[[a, 1]]
sage: wp = word_problem([a,b,c,d,e],e); wp
[[a, 1]]
```


Component: **group theory**

_Issue created by migration from https://trac.sagemath.org/ticket/2293_





---

archive/issue_events_023516.json:
```json
{
    "actor": "https://github.com/wdjoyner",
    "created_at": "2008-02-24T15:33:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2293",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20group%20theory",
    "label_color": "0000ff",
    "label_name": "c: group theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2293#event-23516"
}
```



---

archive/issue_events_023517.json:
```json
{
    "actor": "https://github.com/wdjoyner",
    "created_at": "2008-02-24T15:33:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2293",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2293#event-23517"
}
```



---

archive/issue_events_023518.json:
```json
{
    "actor": "https://github.com/wdjoyner",
    "created_at": "2008-02-24T15:33:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2293",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2293#event-23518"
}
```



---

archive/issue_events_023519.json:
```json
{
    "actor": "https://github.com/wdjoyner",
    "created_at": "2008-02-24T15:33:04Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/2293",
    "subject": "https://github.com/wdjoyner",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2293#event-23519"
}
```



---

archive/issue_comments_011318.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nPlease see\nhttps://github.com/sagemath/sage/issues/2292\nfor the patch.",
    "created_at": "2008-02-24T18:41:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2293",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2293#issuecomment-11318",
    "user": "https://github.com/wdjoyner"
}
```

<div id="comment:1" align="right">comment:1</div>

Please see
https://github.com/sagemath/sage/issues/2292
for the patch.



---

archive/issue_events_023520.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-02-24T18:48:00Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/2293",
    "title_is": "[with patch at #2292] word_problem error in AbelianGroup",
    "title_was": "word_problem error in AbelianGroup",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2293#event-23520"
}
```



---

archive/issue_events_023521.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-02-24T19:08:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2293",
    "milestone_number": null,
    "milestone_title": "sage-2.10.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2293#event-23521"
}
```



---

archive/issue_events_023522.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-02-27T22:20:45Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/2293",
    "title_is": "[with patch at #2292, positive review] word_problem error in AbelianGroup",
    "title_was": "[with patch at #2292] word_problem error in AbelianGroup",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2293#event-23522"
}
```



---

archive/issue_comments_011319.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nThe patch for #2292 fixes this.",
    "created_at": "2008-02-27T22:20:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2293",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2293#issuecomment-11319",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:4" align="right">comment:4</div>

The patch for #2292 fixes this.



---

archive/issue_comments_011320.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nFixed due to the patch at #2292 merged in Sage 2.10.3.rc0",
    "created_at": "2008-02-27T23:10:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2293",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2293#issuecomment-11320",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:5" align="right">comment:5</div>

Fixed due to the patch at #2292 merged in Sage 2.10.3.rc0



---

archive/issue_events_023523.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-02-27T23:10:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2293",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2293#event-23523"
}
```
