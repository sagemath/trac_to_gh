# Issue 2571: [with additional patch] problem with copy() on sage.rings.integer_mod.IntegerMod_gmp

archive/issues_002571.json:
```json
{
    "assignees": [],
    "body": "John Cremona:\n\n```\nsage: a=[Mod(2,next_prime(2^n)) for n in range(28,35)]\nsage: [type(x) for x in a]\n\n[<type 'sage.rings.integer_mod.IntegerMod_int64'>,\n <type 'sage.rings.integer_mod.IntegerMod_int64'>,\n <type 'sage.rings.integer_mod.IntegerMod_int64'>,\n <type 'sage.rings.integer_mod.IntegerMod_gmp'>,\n <type 'sage.rings.integer_mod.IntegerMod_gmp'>,\n <type 'sage.rings.integer_mod.IntegerMod_gmp'>,\n <type 'sage.rings.integer_mod.IntegerMod_gmp'>]\n\nsage: [copy(x) for x in a]\n[2, 2, 2, None, None, None, None]\n\nsage: [deepcopy(x) for x in a]\n[2, 2, 2, 2, 2, 2, 2]\n```\n\nAssignee: **@JohnCremona**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/2571_\n\n",
    "closed_at": "2008-03-19T00:36:59Z",
    "created_at": "2008-03-17T12:55:24Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.11",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "[with additional patch] problem with copy() on sage.rings.integer_mod.IntegerMod_gmp",
    "type": "issue",
    "updated_at": "2008-03-19T00:36:59Z",
    "url": "https://github.com/sagemath/sage/issues/2571",
    "user": "https://github.com/dfdeshom"
}
```
John Cremona:

```
sage: a=[Mod(2,next_prime(2^n)) for n in range(28,35)]
sage: [type(x) for x in a]

[<type 'sage.rings.integer_mod.IntegerMod_int64'>,
 <type 'sage.rings.integer_mod.IntegerMod_int64'>,
 <type 'sage.rings.integer_mod.IntegerMod_int64'>,
 <type 'sage.rings.integer_mod.IntegerMod_gmp'>,
 <type 'sage.rings.integer_mod.IntegerMod_gmp'>,
 <type 'sage.rings.integer_mod.IntegerMod_gmp'>,
 <type 'sage.rings.integer_mod.IntegerMod_gmp'>]

sage: [copy(x) for x in a]
[2, 2, 2, None, None, None, None]

sage: [deepcopy(x) for x in a]
[2, 2, 2, 2, 2, 2, 2]
```

Assignee: **@JohnCremona**

_Issue created by migration from https://trac.sagemath.org/ticket/2571_





---

archive/issue_events_023350.json:
```json
{
    "actor": "https://github.com/dfdeshom",
    "created_at": "2008-03-17T12:55:24Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2571",
    "milestone_number": null,
    "milestone_title": "sage-2.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2571#event-23350"
}
```



---

archive/issue_events_023351.json:
```json
{
    "actor": "https://github.com/dfdeshom",
    "created_at": "2008-03-17T12:55:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2571",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "component: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2571#event-23351"
}
```



---

archive/issue_events_023352.json:
```json
{
    "actor": "https://github.com/dfdeshom",
    "created_at": "2008-03-17T12:55:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2571",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2571#event-23352"
}
```



---

archive/issue_events_023353.json:
```json
{
    "actor": "https://github.com/dfdeshom",
    "created_at": "2008-03-17T12:55:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2571",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2571#event-23353"
}
```



---

archive/issue_comments_013925.json:
```json
{
    "body": "Attachment: **[8950.patch.gz](https://github.com/sagemath/sage/files/ticket2571/8950.patch.gz)**",
    "created_at": "2008-03-17T14:24:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2571",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2571#issuecomment-13925",
    "user": "https://github.com/JohnCremona"
}
```

Attachment: **[8950.patch.gz](https://github.com/sagemath/sage/files/ticket2571/8950.patch.gz)**



---

archive/issue_comments_013926.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nTo fix this I added a return statement to the `__copy__()` function of IntegerMod_gmp.  Patch attached (based on 2.10.4.rc0)",
    "created_at": "2008-03-17T14:27:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2571",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2571#issuecomment-13926",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:1'>Comment 1:</a>
To fix this I added a return statement to the `__copy__()` function of IntegerMod_gmp.  Patch attached (based on 2.10.4.rc0)



---

archive/issue_events_023354.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2008-03-17T14:27:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2571",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2571#event-23354"
}
```



---

archive/issue_events_023355.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2008-03-17T14:27:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2571",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2571#event-23355"
}
```



---

archive/issue_comments_013927.json:
```json
{
    "body": "Assignee: **@JohnCremona**",
    "created_at": "2008-03-17T14:27:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2571",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2571#issuecomment-13927",
    "user": "https://github.com/JohnCremona"
}
```

Assignee: **@JohnCremona**



---

archive/issue_comments_013928.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nWhile you're editing that function, would you mind adding a doctest or 2 to it? That would help with the overall 3.0 goal of >50% function doctest coverage",
    "created_at": "2008-03-17T14:34:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2571",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2571#issuecomment-13928",
    "user": "https://github.com/dfdeshom"
}
```

<a id='comment:2'>Comment 2:</a>
While you're editing that function, would you mind adding a doctest or 2 to it? That would help with the overall 3.0 goal of >50% function doctest coverage



---

archive/issue_comments_013929.json:
```json
{
    "body": "apply after 8950.patch",
    "created_at": "2008-03-17T16:56:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2571",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2571#issuecomment-13929",
    "user": "https://github.com/JohnCremona"
}
```

apply after 8950.patch



---

archive/issue_events_023356.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2008-03-17T16:56:39Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/2571",
    "title_is": "[with additional patch] problem with copy() on sage.rings.integer_mod.IntegerMod_gmp",
    "title_was": "problem with copy() on sage.rings.integer_mod.IntegerMod_gmp",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2571#event-23356"
}
```



---

archive/issue_events_023357.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2008-03-17T16:56:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2571",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2571#event-23357"
}
```



---

archive/issue_comments_013930.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nAttachment: **[8951.patch.gz](https://github.com/sagemath/sage/files/ticket2571/8951.patch.gz)**\n\nAs suggested, a few doctests have been added, in the second patch.",
    "created_at": "2008-03-17T16:56:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2571",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2571#issuecomment-13930",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:3'>Comment 3:</a>
Attachment: **[8951.patch.gz](https://github.com/sagemath/sage/files/ticket2571/8951.patch.gz)**

As suggested, a few doctests have been added, in the second patch.



---

archive/issue_events_023358.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-17T20:19:21Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2571",
    "milestone_number": null,
    "milestone_title": "sage-2.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2571#event-23358"
}
```



---

archive/issue_comments_013931.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nThese patches fix the bug and add and improve documentation bits.  I've tested everything I could see that could go wrong (including the copy and sqrt/square_root change) and all looks good to me.\n\nThis is a positive review to both patches!",
    "created_at": "2008-03-18T15:39:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2571",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2571#issuecomment-13931",
    "user": "https://github.com/sagetrac-jbmohler"
}
```

<a id='comment:5'>Comment 5:</a>
These patches fix the bug and add and improve documentation bits.  I've tested everything I could see that could go wrong (including the copy and sqrt/square_root change) and all looks good to me.

This is a positive review to both patches!



---

archive/issue_events_023359.json:
```json
{
    "actor": "https://github.com/sagetrac-jbmohler",
    "created_at": "2008-03-18T15:39:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2571",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2571#event-23359"
}
```



---

archive/issue_events_023360.json:
```json
{
    "actor": "https://github.com/sagetrac-jbmohler",
    "created_at": "2008-03-18T15:39:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2571",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2571#event-23360"
}
```



---

archive/issue_events_023361.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-19T00:36:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2571",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2571#event-23361"
}
```



---

archive/issue_events_023362.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-19T00:36:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2571",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2571#event-23362"
}
```



---

archive/issue_comments_013932.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nMerged both patches in Sage 2.11.alpha0",
    "created_at": "2008-03-19T00:36:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2571",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2571#issuecomment-13932",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:6'>Comment 6:</a>
Merged both patches in Sage 2.11.alpha0
