# Issue 2330: modforms -- implement computation of weight 1 forms in Sage

archive/issues_002330.json:
```json
{
    "body": "Kevin Buzzard and I intend to do this ASAP based on algorithms he designed in 2002.\n\nIf you see this and are interested in helping, send me an email (wstein`@`gmail.com).\n\nAssignee: @craigcitro\n\nBranch/Commit: 42291700f1c422209abf7862e87f0c65466ab9ec\n\nReviewer: David Roe\n\nAuthor: David Loeffler\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/2330\n\n",
    "closed_at": "2017-12-11T01:04:08Z",
    "created_at": "2008-02-27T11:02:54Z",
    "labels": [
        "component: modular forms"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.2",
    "title": "modforms -- implement computation of weight 1 forms in Sage",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2330",
    "user": "https://github.com/williamstein"
}
```
Kevin Buzzard and I intend to do this ASAP based on algorithms he designed in 2002.

If you see this and are interested in helping, send me an email (wstein`@`gmail.com).

Assignee: @craigcitro

Branch/Commit: 42291700f1c422209abf7862e87f0c65466ab9ec

Reviewer: David Roe

Author: David Loeffler

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/2330





---

archive/issue_comments_016808.json:
```json
{
    "body": "Changing type from defect to enhancement.",
    "created_at": "2008-02-27T11:03:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2330#issuecomment-16808",
    "user": "https://github.com/williamstein"
}
```

Changing type from defect to enhancement.



---

archive/issue_comments_016809.json:
```json
{
    "body": "Changing assignee from @williamstein to @craigcitro.",
    "created_at": "2008-04-13T01:56:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2330#issuecomment-16809",
    "user": "https://github.com/aghitza"
}
```

Changing assignee from @williamstein to @craigcitro.



---

archive/issue_comments_016810.json:
```json
{
    "body": "Changing component from number theory to modular forms.",
    "created_at": "2008-04-13T01:56:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2330#issuecomment-16810",
    "user": "https://github.com/aghitza"
}
```

Changing component from number theory to modular forms.



---

archive/issue_events_005489.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/2330",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2330#event-5489"
}
```



---

archive/issue_events_005490.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/2330",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2330#event-5490"
}
```



---

archive/issue_events_005491.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/2330",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2330#event-5491"
}
```



---

archive/issue_events_005492.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/2330",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2330#event-5492"
}
```



---

archive/issue_events_005493.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/2330",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2330#event-5493"
}
```



---

archive/issue_events_005494.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/2330",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2330#event-5494"
}
```



---

archive/issue_events_005495.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/2330",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2330#event-5495"
}
```



---

archive/issue_comments_016811.json:
```json
{
    "body": "Changing upstream from \"\" to \"N/A\"",
    "created_at": "2017-02-06T18:20:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2330#issuecomment-16811",
    "user": "https://github.com/roed314"
}
```

Changing upstream from "" to "N/A"



---

archive/issue_comments_016812.json:
```json
{
    "body": "<a id='comment:7'></a>\nPresumably this would be based on http://wwwf.imperial.ac.uk/~buzzard/maths/research/papers/wt1.pdf",
    "created_at": "2017-02-06T18:20:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2330#issuecomment-16812",
    "user": "https://github.com/roed314"
}
```

<a id='comment:7'></a>
Presumably this would be based on http://wwwf.imperial.ac.uk/~buzzard/maths/research/papers/wt1.pdf



---

archive/issue_comments_016813.json:
```json
{
    "body": "Changing branch from \"\" to \"public/2330-weight1\"",
    "created_at": "2017-11-08T02:49:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2330#issuecomment-16813",
    "user": "https://github.com/loefflerd"
}
```

Changing branch from "" to "public/2330-weight1"



---

archive/issue_comments_016814.json:
```json
{
    "body": "<a id='comment:9'></a>\nNew commits:\n|                                                                                                                                          |                                                            |\n|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------|\n|[84310cd](https://git.sagemath.org/sage.git/commit?id=84310cd1580d3d7b4b59ba916da4c71868ffd221)|`Implementation of Schaeffer's algorithm for weight 1 forms`|",
    "created_at": "2017-11-08T02:52:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2330#issuecomment-16814",
    "user": "https://github.com/loefflerd"
}
```

<a id='comment:9'></a>
New commits:
|                                                                                                                                          |                                                            |
|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------|
|[84310cd](https://git.sagemath.org/sage.git/commit?id=84310cd1580d3d7b4b59ba916da4c71868ffd221)|`Implementation of Schaeffer's algorithm for weight 1 forms`|



---

archive/issue_comments_016815.json:
```json
{
    "body": "Changing author from \"\" to \"David Loeffler\"",
    "created_at": "2017-11-08T02:52:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2330#issuecomment-16815",
    "user": "https://github.com/loefflerd"
}
```

Changing author from "" to "David Loeffler"



---

archive/issue_comments_016816.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2017-11-08T02:52:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2330#issuecomment-16816",
    "user": "https://github.com/loefflerd"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_016817.json:
```json
{
    "body": "Changing commit from \"\" to \"84310cd1580d3d7b4b59ba916da4c71868ffd221\"",
    "created_at": "2017-11-08T02:52:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2330#issuecomment-16817",
    "user": "https://github.com/loefflerd"
}
```

Changing commit from "" to "84310cd1580d3d7b4b59ba916da4c71868ffd221"



---

archive/issue_comments_016818.json:
```json
{
    "body": "Changing reviewer from \"\" to \"David Roe\"",
    "created_at": "2017-11-08T07:59:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2330#issuecomment-16818",
    "user": "https://github.com/roed314"
}
```

Changing reviewer from "" to "David Roe"



---

archive/issue_comments_016819.json:
```json
{
    "body": "<a id='comment:10'></a>\nIt's awesome to have weight 1 forms!\n\nOverall, the code looks good.  A few issues to be addressed:\n\n* There's a plugin failure complaining about two lines with non-ASCII characters\n* The definition of `R` in the docstring of `modular_ratio_space` is off by one compared to the usage of `R` in the code.",
    "created_at": "2017-11-08T07:59:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2330#issuecomment-16819",
    "user": "https://github.com/roed314"
}
```

<a id='comment:10'></a>
It's awesome to have weight 1 forms!

Overall, the code looks good.  A few issues to be addressed:

* There's a plugin failure complaining about two lines with non-ASCII characters
* The definition of `R` in the docstring of `modular_ratio_space` is off by one compared to the usage of `R` in the code.



---

archive/issue_comments_016820.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2017-11-08T07:59:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2330#issuecomment-16820",
    "user": "https://github.com/roed314"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_016821.json:
```json
{
    "body": "<a id='comment:11'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                        |\n|-------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------|\n|[4229170](https://git.sagemath.org/sage.git/commit/?id=42291700f1c422209abf7862e87f0c65466ab9ec)|`Trac 2330: fixes requested by reviewer`|",
    "created_at": "2017-11-08T12:00:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2330#issuecomment-16821",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:11'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                                        |
|-------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------|
|[4229170](https://git.sagemath.org/sage.git/commit/?id=42291700f1c422209abf7862e87f0c65466ab9ec)|`Trac 2330: fixes requested by reviewer`|



---

archive/issue_comments_016822.json:
```json
{
    "body": "Changing commit from \"84310cd1580d3d7b4b59ba916da4c71868ffd221\" to \"42291700f1c422209abf7862e87f0c65466ab9ec\"",
    "created_at": "2017-11-08T12:00:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2330#issuecomment-16822",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "84310cd1580d3d7b4b59ba916da4c71868ffd221" to "42291700f1c422209abf7862e87f0c65466ab9ec"



---

archive/issue_comments_016823.json:
```json
{
    "body": "<a id='comment:12'></a>\nThis should address those two issues.\n\n---\nNew commits:\n|                                                                                                                                          |                                        |\n|------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------|\n|[4229170](https://git.sagemath.org/sage.git/commit?id=42291700f1c422209abf7862e87f0c65466ab9ec)|`Trac 2330: fixes requested by reviewer`|",
    "created_at": "2017-11-08T12:01:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2330#issuecomment-16823",
    "user": "https://github.com/loefflerd"
}
```

<a id='comment:12'></a>
This should address those two issues.

---
New commits:
|                                                                                                                                          |                                        |
|------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------|
|[4229170](https://git.sagemath.org/sage.git/commit?id=42291700f1c422209abf7862e87f0c65466ab9ec)|`Trac 2330: fixes requested by reviewer`|



---

archive/issue_comments_016824.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2017-11-08T12:01:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2330#issuecomment-16824",
    "user": "https://github.com/loefflerd"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_016825.json:
```json
{
    "body": "<a id='comment:13'></a>\nI'm a bit puzzled that four out of the five patchbot reports for the first version of the code are reporting test failures, all of which are nothing to do with the ticket as far as I can see -- is there something wrong with the patchbot? Most of them seem to be timeout errors.",
    "created_at": "2017-11-08T12:29:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2330#issuecomment-16825",
    "user": "https://github.com/loefflerd"
}
```

<a id='comment:13'></a>
I'm a bit puzzled that four out of the five patchbot reports for the first version of the code are reporting test failures, all of which are nothing to do with the ticket as far as I can see -- is there something wrong with the patchbot? Most of them seem to be timeout errors.



---

archive/issue_comments_016826.json:
```json
{
    "body": "<a id='comment:14'></a>\nI'm not sure what's wrong with the other patchbots, but I've seen some of these errors in other places.  It's certainly not a problem with your code.\n\nLooks good!",
    "created_at": "2017-11-08T18:23:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2330#issuecomment-16826",
    "user": "https://github.com/roed314"
}
```

<a id='comment:14'></a>
I'm not sure what's wrong with the other patchbots, but I've seen some of these errors in other places.  It's certainly not a problem with your code.

Looks good!



---

archive/issue_comments_016827.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2017-11-08T18:23:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2330#issuecomment-16827",
    "user": "https://github.com/roed314"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_events_005496.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-11-30T14:49:19Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/2330",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2330#event-5496"
}
```



---

archive/issue_events_005497.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-11-30T14:49:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/2330",
    "milestone": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2330#event-5497"
}
```



---

archive/issue_comments_016828.json:
```json
{
    "body": "Changing branch from \"public/2330-weight1\" to \"42291700f1c422209abf7862e87f0c65466ab9ec\"",
    "created_at": "2017-12-11T01:04:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2330#issuecomment-16828",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "public/2330-weight1" to "42291700f1c422209abf7862e87f0c65466ab9ec"



---

archive/issue_events_005498.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-12-11T01:04:08Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2330",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2330#event-5498"
}
```



---

archive/issue_comments_016829.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2017-12-11T01:04:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2330#issuecomment-16829",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed
