# Issue 2330: modforms -- implement computation of weight 1 forms in Sage

archive/issues_002330.json:
```json
{
    "assignees": [],
    "body": "Kevin Buzzard and I intend to do this ASAP based on algorithms he designed in 2002.\n\nIf you see this and are interested in helping, send me an email (wstein@gmail.com).\n\nAssignee: **@craigcitro**\n\nBranch/Commit: **[4229170](https://github.com/sagemath/sagetrac-mirror/commit/42291700f1c422209abf7862e87f0c65466ab9ec)**\n\nReviewer: **David Roe**\n\nAuthor: **David Loeffler**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/2330_\n\n",
    "closed_at": "2017-12-11T01:04:08Z",
    "created_at": "2008-02-27T11:02:54Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20modular%20forms",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "modforms -- implement computation of weight 1 forms in Sage",
    "type": "issue",
    "updated_at": "2017-12-11T01:04:08Z",
    "url": "https://github.com/sagemath/sage/issues/2330",
    "user": "https://github.com/williamstein"
}
```
Kevin Buzzard and I intend to do this ASAP based on algorithms he designed in 2002.

If you see this and are interested in helping, send me an email (wstein@gmail.com).

Assignee: **@craigcitro**

Branch/Commit: **[4229170](https://github.com/sagemath/sagetrac-mirror/commit/42291700f1c422209abf7862e87f0c65466ab9ec)**

Reviewer: **David Roe**

Author: **David Loeffler**

_Issue created by migration from https://trac.sagemath.org/ticket/2330_





---

archive/issue_events_021083.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-02-27T11:02:54Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2330#event-21083"
}
```



---

archive/issue_events_021084.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-02-27T11:02:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20theory",
    "label_color": "0000ff",
    "label_name": "component: number theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2330#event-21084"
}
```



---

archive/issue_events_021085.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-02-27T11:02:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2330#event-21085"
}
```



---

archive/issue_events_021086.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-02-27T11:02:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2330#event-21086"
}
```



---

archive/issue_events_021087.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-02-27T11:03:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2330#event-21087"
}
```



---

archive/issue_events_021088.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-02-27T11:03:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2330#event-21088"
}
```



---

archive/issue_comments_012247.json:
```json
{
    "body": "Changed assignee from **@williamstein** to **@craigcitro**",
    "created_at": "2008-04-13T01:56:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2330#issuecomment-12247",
    "user": "https://github.com/aghitza"
}
```

Changed assignee from **@williamstein** to **@craigcitro**



---

archive/issue_events_021089.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2008-04-13T01:56:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20theory",
    "label_color": "0000ff",
    "label_name": "component: number theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2330#event-21089"
}
```



---

archive/issue_events_021090.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2008-04-13T01:56:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20modular%20forms",
    "label_color": "0000ff",
    "label_name": "component: modular forms",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2330#event-21090"
}
```



---

archive/issue_events_021091.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2330#event-21091"
}
```



---

archive/issue_events_021092.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2330#event-21092"
}
```



---

archive/issue_events_021093.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2330#event-21093"
}
```



---

archive/issue_events_021094.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2330#event-21094"
}
```



---

archive/issue_events_021095.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2330#event-21095"
}
```



---

archive/issue_events_021096.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2330#event-21096"
}
```



---

archive/issue_events_021097.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2330#event-21097"
}
```



---

archive/issue_events_021098.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2330#event-21098"
}
```



---

archive/issue_comments_012248.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nPresumably this would be based on http://wwwf.imperial.ac.uk/~buzzard/maths/research/papers/wt1.pdf",
    "created_at": "2017-02-06T18:20:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2330#issuecomment-12248",
    "user": "https://github.com/roed314"
}
```

<a id='comment:7'>Comment 7:</a>
Presumably this would be based on http://wwwf.imperial.ac.uk/~buzzard/maths/research/papers/wt1.pdf



---

archive/issue_comments_012249.json:
```json
{
    "body": "Branch: **[public/2330-weight1](https://github.com/sagemath/sagetrac-mirror/tree/public/2330-weight1)**",
    "created_at": "2017-11-08T02:49:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2330#issuecomment-12249",
    "user": "https://github.com/loefflerd"
}
```

Branch: **[public/2330-weight1](https://github.com/sagemath/sagetrac-mirror/tree/public/2330-weight1)**



---

archive/issue_comments_012250.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/84310cd1580d3d7b4b59ba916da4c71868ffd221\">84310cd</a></td><td><code>Implementation of Schaeffer's algorithm for weight 1 forms</code></td></tr></table>\n",
    "created_at": "2017-11-08T02:52:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2330#issuecomment-12250",
    "user": "https://github.com/loefflerd"
}
```

<a id='comment:9'>Comment 9:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/84310cd1580d3d7b4b59ba916da4c71868ffd221">84310cd</a></td><td><code>Implementation of Schaeffer's algorithm for weight 1 forms</code></td></tr></table>




---

archive/issue_comments_012251.json:
```json
{
    "body": "Author: **David Loeffler**",
    "created_at": "2017-11-08T02:52:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2330#issuecomment-12251",
    "user": "https://github.com/loefflerd"
}
```

Author: **David Loeffler**



---

archive/issue_events_021099.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2017-11-08T02:52:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2330#event-21099"
}
```



---

archive/issue_comments_012252.json:
```json
{
    "body": "Commit: **[84310cd](https://github.com/sagemath/sagetrac-mirror/commit/84310cd1580d3d7b4b59ba916da4c71868ffd221)**",
    "created_at": "2017-11-08T02:52:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2330#issuecomment-12252",
    "user": "https://github.com/loefflerd"
}
```

Commit: **[84310cd](https://github.com/sagemath/sagetrac-mirror/commit/84310cd1580d3d7b4b59ba916da4c71868ffd221)**



---

archive/issue_comments_012253.json:
```json
{
    "body": "Reviewer: **David Roe**",
    "created_at": "2017-11-08T07:59:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2330#issuecomment-12253",
    "user": "https://github.com/roed314"
}
```

Reviewer: **David Roe**



---

archive/issue_comments_012254.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nIt's awesome to have weight 1 forms!\n\nOverall, the code looks good.  A few issues to be addressed:\n\n* There's a plugin failure complaining about two lines with non-ASCII characters\n* The definition of `R` in the docstring of `modular_ratio_space` is off by one compared to the usage of `R` in the code.",
    "created_at": "2017-11-08T07:59:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2330#issuecomment-12254",
    "user": "https://github.com/roed314"
}
```

<a id='comment:10'>Comment 10:</a>
It's awesome to have weight 1 forms!

Overall, the code looks good.  A few issues to be addressed:

* There's a plugin failure complaining about two lines with non-ASCII characters
* The definition of `R` in the docstring of `modular_ratio_space` is off by one compared to the usage of `R` in the code.



---

archive/issue_events_021100.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-11-08T07:59:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2330#event-21100"
}
```



---

archive/issue_events_021101.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-11-08T07:59:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2330#event-21101"
}
```



---

archive/issue_comments_012255.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/42291700f1c422209abf7862e87f0c65466ab9ec\">4229170</a></td><td><code>Trac 2330: fixes requested by reviewer</code></td></tr></table>\n",
    "created_at": "2017-11-08T12:00:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2330#issuecomment-12255",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:11'>Comment 11:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/42291700f1c422209abf7862e87f0c65466ab9ec">4229170</a></td><td><code>Trac 2330: fixes requested by reviewer</code></td></tr></table>




---

archive/issue_comments_012256.json:
```json
{
    "body": "Changed commit from **[84310cd](https://github.com/sagemath/sagetrac-mirror/commit/84310cd1580d3d7b4b59ba916da4c71868ffd221)** to **[4229170](https://github.com/sagemath/sagetrac-mirror/commit/42291700f1c422209abf7862e87f0c65466ab9ec)**",
    "created_at": "2017-11-08T12:00:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2330#issuecomment-12256",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[84310cd](https://github.com/sagemath/sagetrac-mirror/commit/84310cd1580d3d7b4b59ba916da4c71868ffd221)** to **[4229170](https://github.com/sagemath/sagetrac-mirror/commit/42291700f1c422209abf7862e87f0c65466ab9ec)**



---

archive/issue_comments_012257.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nThis should address those two issues.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/42291700f1c422209abf7862e87f0c65466ab9ec\">4229170</a></td><td><code>Trac 2330: fixes requested by reviewer</code></td></tr></table>\n",
    "created_at": "2017-11-08T12:01:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2330#issuecomment-12257",
    "user": "https://github.com/loefflerd"
}
```

<a id='comment:12'>Comment 12:</a>
This should address those two issues.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/42291700f1c422209abf7862e87f0c65466ab9ec">4229170</a></td><td><code>Trac 2330: fixes requested by reviewer</code></td></tr></table>




---

archive/issue_events_021102.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2017-11-08T12:01:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2330#event-21102"
}
```



---

archive/issue_events_021103.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2017-11-08T12:01:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2330#event-21103"
}
```



---

archive/issue_comments_012258.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nI'm a bit puzzled that four out of the five patchbot reports for the first version of the code are reporting test failures, all of which are nothing to do with the ticket as far as I can see -- is there something wrong with the patchbot? Most of them seem to be timeout errors.",
    "created_at": "2017-11-08T12:29:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2330#issuecomment-12258",
    "user": "https://github.com/loefflerd"
}
```

<a id='comment:13'>Comment 13:</a>
I'm a bit puzzled that four out of the five patchbot reports for the first version of the code are reporting test failures, all of which are nothing to do with the ticket as far as I can see -- is there something wrong with the patchbot? Most of them seem to be timeout errors.



---

archive/issue_comments_012259.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nI'm not sure what's wrong with the other patchbots, but I've seen some of these errors in other places.  It's certainly not a problem with your code.\n\nLooks good!",
    "created_at": "2017-11-08T18:23:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2330#issuecomment-12259",
    "user": "https://github.com/roed314"
}
```

<a id='comment:14'>Comment 14:</a>
I'm not sure what's wrong with the other patchbots, but I've seen some of these errors in other places.  It's certainly not a problem with your code.

Looks good!



---

archive/issue_events_021104.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-11-08T18:23:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2330#event-21104"
}
```



---

archive/issue_events_021105.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2017-11-08T18:23:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2330#event-21105"
}
```



---

archive/issue_events_021106.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-11-30T14:49:19Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2330#event-21106"
}
```



---

archive/issue_events_021107.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-11-30T14:49:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2330#event-21107"
}
```



---

archive/issue_comments_012260.json:
```json
{
    "body": "Changed branch from **[public/2330-weight1](https://github.com/sagemath/sagetrac-mirror/tree/public/2330-weight1)** to **[4229170](https://github.com/sagemath/sagetrac-mirror/commit/42291700f1c422209abf7862e87f0c65466ab9ec)**",
    "created_at": "2017-12-11T01:04:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2330#issuecomment-12260",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/2330-weight1](https://github.com/sagemath/sagetrac-mirror/tree/public/2330-weight1)** to **[4229170](https://github.com/sagemath/sagetrac-mirror/commit/42291700f1c422209abf7862e87f0c65466ab9ec)**



---

archive/issue_events_021108.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-12-11T01:04:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2330#event-21108"
}
```



---

archive/issue_events_021109.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "035aed0c312441c395e5ee81f9e07bac33df7b06",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-12-11T01:04:08Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2330",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2330#event-21109"
}
```
