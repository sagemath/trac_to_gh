# Issue 2471: [with patch; with positive review] doctest failures in graph.py and factorization.py (caused by architecture dependence)

archive/issues_002471.json:
```json
{
    "body": "\n\nAssignee: @mwhansen\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/2471\n\n",
    "closed_at": "2008-03-11T15:37:11Z",
    "created_at": "2008-03-11T08:10:25Z",
    "labels": [
        "component: combinatorics",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.10.3",
    "title": "[with patch; with positive review] doctest failures in graph.py and factorization.py (caused by architecture dependence)",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2471",
    "user": "https://github.com/williamstein"
}
```


Assignee: @mwhansen

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/2471





---

archive/issue_comments_017489.json:
```json
{
    "body": "Attachment [sage-2471.patch](tarball://root/attachments/some-uuid/ticket2471/sage-2471.patch) by @williamstein created at 2008-03-11 08:11:21",
    "created_at": "2008-03-11T08:11:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2471#issuecomment-17489",
    "user": "https://github.com/williamstein"
}
```

Attachment [sage-2471.patch](tarball://root/attachments/some-uuid/ticket2471/sage-2471.patch) by @williamstein created at 2008-03-11 08:11:21



---

archive/issue_comments_017490.json:
```json
{
    "body": "<a id='comment:1'></a>\nLooks good and fixes the doctest failures on my machine.",
    "created_at": "2008-03-11T13:22:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2471#issuecomment-17490",
    "user": "https://github.com/aghitza"
}
```

<a id='comment:1'></a>
Looks good and fixes the doctest failures on my machine.



---

archive/issue_events_008690.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2471",
    "rename": {
        "from": "[with patch; needs review] doctest failures in graph.py and factorization.py (caused by architecture dependence)",
        "to": "[with patch; with positive review] doctest failures in graph.py and factorization.py (caused by architecture dependence)"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2471#event-8690"
}
```



---

archive/issue_events_008691.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-03-11T15:37:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2471",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2471#event-8691"
}
```



---

archive/issue_comments_017491.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-03-11T15:37:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2471#issuecomment-17491",
    "user": "https://github.com/williamstein"
}
```

Resolution: fixed



---

archive/issue_comments_017492.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 2.10.3.final",
    "created_at": "2008-03-11T20:31:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2471#issuecomment-17492",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 2.10.3.final
