# Issue 2837: [witch patch, positive review] use twisted-8.0.1's blockingCallFromThread

archive/issues_002837.json:
```json
{
    "body": "This patch makes use of the official blockingCallFromThread method in twisted.internet.threads instead of the one supplied by dsage.\n\nAssignee: @yqiang\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/2837\n\n",
    "closed_at": "2008-04-07T01:23:49Z",
    "created_at": "2008-04-07T00:14:16Z",
    "labels": [
        "component: dsage",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0",
    "title": "[witch patch, positive review] use twisted-8.0.1's blockingCallFromThread",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2837",
    "user": "https://github.com/yqiang"
}
```
This patch makes use of the official blockingCallFromThread method in twisted.internet.threads instead of the one supplied by dsage.

Assignee: @yqiang

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/2837





---

archive/issue_comments_020391.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [blockingcall_api_change.patch](tarball://root/attachments/some-uuid/ticket2837/blockingcall_api_change.patch) by @mwhansen created at 2008-04-07 01:05:03\n\nApplies and passes tests on 3.0.alpha1 + new twisted spkg.",
    "created_at": "2008-04-07T01:05:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2837",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2837#issuecomment-20391",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'></a>
Attachment [blockingcall_api_change.patch](tarball://root/attachments/some-uuid/ticket2837/blockingcall_api_change.patch) by @mwhansen created at 2008-04-07 01:05:03

Applies and passes tests on 3.0.alpha1 + new twisted spkg.



---

archive/issue_events_010017.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2837",
    "rename": {
        "from": "[witch patch, needs review] use twisted-8.0.1's blockingCallFromThread",
        "to": "[witch patch, positive review] use twisted-8.0.1's blockingCallFromThread"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2837#event-10017"
}
```



---

archive/issue_comments_020392.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-04-07T01:23:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2837",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2837#issuecomment-20392",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_010018.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-07T01:23:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2837",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2837#event-10018"
}
```



---

archive/issue_comments_020393.json:
```json
{
    "body": "<a id='comment:2'></a>\nMerged in Sage 3.0.alpha2",
    "created_at": "2008-04-07T01:23:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2837",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2837#issuecomment-20393",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Merged in Sage 3.0.alpha2
