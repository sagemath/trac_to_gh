# Issue 2439: [with patch, positive review] ZZ.random_element() crashes Sage with probability 2^-31

archive/issues_002439.json:
```json
{
    "body": "ZZ.random_element() does an integer divide by zero once every 2<sup>31</sup> calls.\n\nI'll make a patch as soon as my rc3 build finishes.\n\nAssignee: cwitty\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/2439\n\n",
    "closed_at": "2008-03-09T19:04:00Z",
    "created_at": "2008-03-09T17:40:22Z",
    "labels": [
        "component: basic arithmetic",
        "critical",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.10.3",
    "title": "[with patch, positive review] ZZ.random_element() crashes Sage with probability 2^-31",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2439",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```
ZZ.random_element() does an integer divide by zero once every 2<sup>31</sup> calls.

I'll make a patch as soon as my rc3 build finishes.

Assignee: cwitty

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/2439





---

archive/issue_comments_017253.json:
```json
{
    "body": "Attachment [trac2439.patch](tarball://root/attachments/some-uuid/ticket2439/trac2439.patch) by cwitty created at 2008-03-09 18:42:43",
    "created_at": "2008-03-09T18:42:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2439#issuecomment-17253",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```

Attachment [trac2439.patch](tarball://root/attachments/some-uuid/ticket2439/trac2439.patch) by cwitty created at 2008-03-09 18:42:43



---

archive/issue_comments_017254.json:
```json
{
    "body": "<a id='comment:1'></a>\nThe attached patch fixes the crash, fixes other crashes in related code, and deletes some dead code.",
    "created_at": "2008-03-09T18:45:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2439#issuecomment-17254",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```

<a id='comment:1'></a>
The attached patch fixes the crash, fixes other crashes in related code, and deletes some dead code.



---

archive/issue_events_008551.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2439",
    "rename": {
        "from": "ZZ.random_element() crashes Sage with probability 2^-31",
        "to": "[with patch, needs review] ZZ.random_element() crashes Sage with probability 2^-31"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2439#event-8551"
}
```



---

archive/issue_comments_017255.json:
```json
{
    "body": "<a id='comment:2'></a>\nPatch looks good to me. Good that cwitty found a very unlikely bug to hit :)",
    "created_at": "2008-03-09T19:01:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2439#issuecomment-17255",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Patch looks good to me. Good that cwitty found a very unlikely bug to hit :)



---

archive/issue_events_008552.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2439",
    "rename": {
        "from": "[with patch, needs review] ZZ.random_element() crashes Sage with probability 2^-31",
        "to": "[with patch, positive review] ZZ.random_element() crashes Sage with probability 2^-31"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2439#event-8552"
}
```



---

archive/issue_comments_017256.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 2.10.3.rc4",
    "created_at": "2008-03-09T19:03:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2439#issuecomment-17256",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 2.10.3.rc4



---

archive/issue_comments_017257.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-03-09T19:04:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2439#issuecomment-17257",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_008553.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-03-09T19:04:00Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2439",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2439#event-8553"
}
```



---

archive/issue_comments_017258.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 2.10.3.rc4 - and this time I am closing the ticket, too.",
    "created_at": "2008-03-09T19:04:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2439#issuecomment-17258",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 2.10.3.rc4 - and this time I am closing the ticket, too.
