# Issue 2237: plot options for turning off randomness in sampling x data and turning off adaptive plotting

archive/issues_002237.json:
```json
{
    "body": "In doing animations like in http://groups.google.com/group/sage-support/browse_thread/thread/8e2edf8165e9ded2\n\nthe adaptive plotting and the randomness added to the x-sample points is very annoying, creating a wiggling graph in the animation.  We ought to have options to plot to turn these things off so that we can get the same graph over and over again.\n\nAssignee: @williamstein\n\nResolution: duplicate\n\nIssue created by migration from https://trac.sagemath.org/ticket/2237\n\n",
    "closed_at": "2008-02-21T06:58:11Z",
    "created_at": "2008-02-20T22:56:44Z",
    "labels": [
        "component: graphics"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.10.2",
    "title": "plot options for turning off randomness in sampling x data and turning off adaptive plotting",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2237",
    "user": "https://github.com/jasongrout"
}
```
In doing animations like in http://groups.google.com/group/sage-support/browse_thread/thread/8e2edf8165e9ded2

the adaptive plotting and the randomness added to the x-sample points is very annoying, creating a wiggling graph in the animation.  We ought to have options to plot to turn these things off so that we can get the same graph over and over again.

Assignee: @williamstein

Resolution: duplicate

Issue created by migration from https://trac.sagemath.org/ticket/2237





---

archive/issue_comments_015996.json:
```json
{
    "body": "<a id='comment:1'></a>Another idea would be for animate to use the same random offsets in each plot in the animation.",
    "created_at": "2008-02-21T02:19:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2237",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2237#issuecomment-15996",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```

<a id='comment:1'></a>Another idea would be for animate to use the same random offsets in each plot in the animation.



---

archive/issue_comments_015997.json:
```json
{
    "body": "<a id='comment:2'></a>See trac #2244 that implements an option to turn off randomness in plotting.\n\nThis ticket should likely be marked as a dupe.",
    "created_at": "2008-02-21T06:58:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2237",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2237#issuecomment-15997",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:2'></a>See trac #2244 that implements an option to turn off randomness in plotting.

This ticket should likely be marked as a dupe.



---

archive/issue_events_005320.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-02-21T06:58:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2237",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2237#event-5320"
}
```



---

archive/issue_comments_015998.json:
```json
{
    "body": "Resolution: duplicate",
    "created_at": "2008-02-21T06:58:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2237",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2237#issuecomment-15998",
    "user": "https://github.com/williamstein"
}
```

Resolution: duplicate
