# Issue 2630: Python API for sage-ptest

archive/issues_002630.json:
```json
{
    "body": "From #311, \n  a Python API so testing is integrated into the SAGE shell (and into the notebook, although this is not particularly powerful at this time)\nThis ought to be easy to do now that sage-ptest is written in python.\n\n**Assignee:** @garyfurnish\n\n**Reviewer:** David Roe\n\nIssue created by migration from https://trac.sagemath.org/ticket/2630\n\n",
    "closed_at": "2013-03-15T13:01:18Z",
    "created_at": "2008-03-21T16:02:10Z",
    "labels": [
        "component: doctest coverage",
        "minor",
        "enhancement",
        "duplicate"
    ],
    "title": "Python API for sage-ptest",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2630",
    "user": "https://github.com/garyfurnish"
}
```
From #311, 
  a Python API so testing is integrated into the SAGE shell (and into the notebook, although this is not particularly powerful at this time)
This ought to be easy to do now that sage-ptest is written in python.

**Assignee:** @garyfurnish

**Reviewer:** David Roe

Issue created by migration from https://trac.sagemath.org/ticket/2630





---

archive/issue_comments_014344.json:
```json
{
    "body": "<a id='comment:1'></a>\nThis is resolved by #12415.",
    "created_at": "2013-03-14T20:36:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2630",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2630#issuecomment-14344",
    "user": "https://github.com/roed314"
}
```

<a id='comment:1'></a>
This is resolved by #12415.



---

archive/issue_events_013645.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2013-03-14T20:36:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/2630",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2630#event-13645"
}
```



---

archive/issue_events_013646.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2013-03-14T20:36:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/2630",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2630#event-13646"
}
```



---

archive/issue_events_013647.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2013-03-14T20:36:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/2630",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2630#event-13647"
}
```



---

archive/issue_comments_014345.json:
```json
{
    "body": "**Reviewer:** David Roe",
    "created_at": "2013-03-15T13:01:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2630",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2630#issuecomment-14345",
    "user": "https://github.com/jdemeyer"
}
```

**Reviewer:** David Roe



---

archive/issue_events_013648.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-03-15T13:01:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/2630",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2630#event-13648"
}
```



---

archive/issue_events_013649.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-03-15T13:01:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/2630",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2630#event-13649"
}
```



---

archive/issue_events_013650.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-03-15T13:01:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2630",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2630#event-13650"
}
```
