# Issue 2104: missing dependency: lcalc on mpfr

archive/issues_002104.json:
```json
{
    "body": "lcalc seems to link against mpfr, but doesn't depend on it in the spkg/standard/deps.\n\n**Assignee:** mabshoff\n\nIssue created by migration from https://trac.sagemath.org/ticket/2104\n\n",
    "closed_at": "2008-02-10T01:46:33Z",
    "created_at": "2008-02-08T08:27:26Z",
    "labels": [
        "component: packages: standard",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.10.2",
    "title": "missing dependency: lcalc on mpfr",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2104",
    "user": "https://github.com/timabbott"
}
```
lcalc seems to link against mpfr, but doesn't depend on it in the spkg/standard/deps.

**Assignee:** mabshoff

Issue created by migration from https://trac.sagemath.org/ticket/2104





---

archive/attachments_001760.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "deps.patch",
    "asset_url": "tarball://root/attachments/some-uuid/ticket2104/deps.patch",
    "created_at": "2008-02-08T22:24:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2104",
    "type": "attachment",
    "url": "https://github.com/assets/some-id/some-uuid.patch",
    "user": "https://github.com/timabbott"
}
```



---

archive/issue_comments_010920.json:
```json
{
    "body": "",
    "created_at": "2008-02-08T22:24:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2104#issuecomment-10920",
    "user": "https://github.com/timabbott"
}
```





---

archive/issue_events_010438.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-02-08T22:24:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/2104",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2104#event-10438"
}
```



---

archive/issue_events_010439.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-02-08T23:40:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/2104",
    "milestone": "sage-2.10.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2104#event-10439"
}
```



---

archive/issue_events_010440.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-02-10T01:44:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/2104",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2104#event-10440"
}
```



---

archive/issue_events_010441.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-02-10T01:44:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/2104",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2104#event-10441"
}
```



---

archive/issue_comments_010921.json:
```json
{
    "body": "<a id='comment:3'></a>\nPatch looks good to me and fixes a real bug that we just don't hit due to luck. \n\nCheers,\n\nMichael",
    "created_at": "2008-02-10T01:44:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2104#issuecomment-10921",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Patch looks good to me and fixes a real bug that we just don't hit due to luck. 

Cheers,

Michael



---

archive/issue_events_010442.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-02-10T01:46:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/2104",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2104#event-10442"
}
```



---

archive/issue_events_010443.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-02-10T01:46:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2104",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2104#event-10443"
}
```



---

archive/issue_comments_010922.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 2.10.2.alpha0",
    "created_at": "2008-02-10T01:46:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2104#issuecomment-10922",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 2.10.2.alpha0
