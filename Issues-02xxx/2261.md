# Issue 2261: [with patch, positive review]setup.py: Don't add emtpy Debian directory to scripts section

archive/issues_002261.json:
```json
{
    "body": "The directory entry causes the sage lib to fail building on a minimal Debian install. Go figure.\n\nCheers,\n\nMichaek\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/2261\n\n",
    "closed_at": "2008-02-22T18:44:28Z",
    "created_at": "2008-02-22T18:04:05Z",
    "labels": [
        "component: debian-package",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.10.2",
    "title": "[with patch, positive review]setup.py: Don't add emtpy Debian directory to scripts section",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2261",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
The directory entry causes the sage lib to fail building on a minimal Debian install. Go figure.

Cheers,

Michaek

Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/2261





---

archive/issue_comments_015672.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-02-22T18:04:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2261",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2261#issuecomment-15672",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing status from new to assigned.



---

archive/issue_events_007794.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2261",
    "rename": {
        "from": "setup.py: Don't add emtpy Debian directory to scripts section",
        "to": "[with patch, needs trivial review]setup.py: Don't add emtpy Debian directory to scripts section"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2261#event-7794"
}
```



---

archive/issue_comments_015673.json:
```json
{
    "body": "Attachment [Sage-2.10.2.rc0-trac_2261.patch](tarball://root/attachments/some-uuid/ticket2261/Sage-2.10.2.rc0-trac_2261.patch) by mabshoff created at 2008-02-22 18:30:45",
    "created_at": "2008-02-22T18:30:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2261",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2261#issuecomment-15673",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Attachment [Sage-2.10.2.rc0-trac_2261.patch](tarball://root/attachments/some-uuid/ticket2261/Sage-2.10.2.rc0-trac_2261.patch) by mabshoff created at 2008-02-22 18:30:45



---

archive/issue_events_007795.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2261",
    "rename": {
        "from": "[with patch, needs trivial review]setup.py: Don't add emtpy Debian directory to scripts section",
        "to": "[with patch, positive review]setup.py: Don't add emtpy Debian directory to scripts section"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2261#event-7795"
}
```



---

archive/issue_comments_015674.json:
```json
{
    "body": "<a id='comment:3'></a>\nthis looks like it follows William's request exactly.",
    "created_at": "2008-02-22T18:39:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2261",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2261#issuecomment-15674",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:3'></a>
this looks like it follows William's request exactly.



---

archive/issue_comments_015675.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 2.10.2.final",
    "created_at": "2008-02-22T18:44:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2261",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2261#issuecomment-15675",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 2.10.2.final



---

archive/issue_comments_015676.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-02-22T18:44:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2261",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2261#issuecomment-15676",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_007796.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-02-22T18:44:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2261",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2261#event-7796"
}
```
