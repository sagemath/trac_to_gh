# Issue 2863: [with patch, positive review] Integer() does not specify that numbers beginning with 0 and 0x are treated specially

archive/issues_002863.json:
```json
{
    "body": "The Integer() function interprets numbers and strings beginning with 0 (respectively, 0x) as octal (respectively hexadecimal) numbers. The docstring does not reflect this. Attached patch fixes this.\n\nAssignee: somebody\n\nKeywords: integer octal hexadecimal\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/2863\n\n",
    "closed_at": "2008-04-09T08:52:12Z",
    "created_at": "2008-04-09T08:45:49Z",
    "labels": [
        "component: basic arithmetic",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0",
    "title": "[with patch, positive review] Integer() does not specify that numbers beginning with 0 and 0x are treated specially",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2863",
    "user": "https://github.com/dandrake"
}
```
The Integer() function interprets numbers and strings beginning with 0 (respectively, 0x) as octal (respectively hexadecimal) numbers. The docstring does not reflect this. Attached patch fixes this.

Assignee: somebody

Keywords: integer octal hexadecimal

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/2863





---

archive/issue_comments_020569.json:
```json
{
    "body": "Attachment [integer-doc.patch](tarball://root/attachments/some-uuid/ticket2863/integer-doc.patch) by @dandrake created at 2008-04-09 08:46:12",
    "created_at": "2008-04-09T08:46:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2863#issuecomment-20569",
    "user": "https://github.com/dandrake"
}
```

Attachment [integer-doc.patch](tarball://root/attachments/some-uuid/ticket2863/integer-doc.patch) by @dandrake created at 2008-04-09 08:46:12



---

archive/issue_events_010111.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2863",
    "rename": {
        "from": "Integer() does not specify that numbers beginning with 0 and 0x are treated specially",
        "to": "[with patch, needs review] Integer() does not specify that numbers beginning with 0 and 0x are treated specially"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2863#event-10111"
}
```



---

archive/issue_events_010112.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2863",
    "rename": {
        "from": "[with patch, needs review] Integer() does not specify that numbers beginning with 0 and 0x are treated specially",
        "to": "[with patch, positive review] Integer() does not specify that numbers beginning with 0 and 0x are treated specially"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2863#event-10112"
}
```



---

archive/issue_comments_020570.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.0.alpha3",
    "created_at": "2008-04-09T08:52:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2863#issuecomment-20570",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.0.alpha3



---

archive/issue_comments_020571.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-04-09T08:52:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2863",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2863#issuecomment-20571",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_010113.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-09T08:52:12Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2863",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2863#event-10113"
}
```
