# Issue 2374: [with patch, positive review] sage-ptest must run in $SAGE_ROOT

archive/issues_002374.json:
```json
{
    "body": "sage-ptest only works from $SAGE_ROOT\n\nAssignee: @garyfurnish\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/2374\n\n",
    "closed_at": "2008-03-10T07:20:47Z",
    "created_at": "2008-03-03T15:59:56Z",
    "labels": [
        "component: packages: standard",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.10.3",
    "title": "[with patch, positive review] sage-ptest must run in $SAGE_ROOT",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2374",
    "user": "https://github.com/garyfurnish"
}
```
sage-ptest only works from $SAGE_ROOT

Assignee: @garyfurnish

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/2374





---

archive/issue_comments_016761.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-03-03T16:00:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2374#issuecomment-16761",
    "user": "https://github.com/garyfurnish"
}
```

Changing status from new to assigned.



---

archive/issue_comments_016762.json:
```json
{
    "body": "Attachment [trac_2374.patch](tarball://root/attachments/some-uuid/ticket2374/trac_2374.patch) by @garyfurnish created at 2008-03-10 07:12:13",
    "created_at": "2008-03-10T07:12:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2374#issuecomment-16762",
    "user": "https://github.com/garyfurnish"
}
```

Attachment [trac_2374.patch](tarball://root/attachments/some-uuid/ticket2374/trac_2374.patch) by @garyfurnish created at 2008-03-10 07:12:13



---

archive/issue_events_008294.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2374",
    "rename": {
        "from": "sage-ptest must run in $SAGE_ROOT",
        "to": "[with patch, needs review] sage-ptest must run in $SAGE_ROOT"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2374#event-8294"
}
```



---

archive/issue_comments_016763.json:
```json
{
    "body": "<a id='comment:3'></a>\nThis allows you to test a file (as opposed to a directory) and contains a ui fix for some cases in which file tested is in a higher directory.",
    "created_at": "2008-03-10T07:17:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2374#issuecomment-16763",
    "user": "https://github.com/garyfurnish"
}
```

<a id='comment:3'></a>
This allows you to test a file (as opposed to a directory) and contains a ui fix for some cases in which file tested is in a higher directory.



---

archive/issue_comments_016764.json:
```json
{
    "body": "<a id='comment:4'></a>\nPatch looks good to me.\n\nCheers,\n\nMichael",
    "created_at": "2008-03-10T07:19:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2374#issuecomment-16764",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Patch looks good to me.

Cheers,

Michael



---

archive/issue_events_008295.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2374",
    "rename": {
        "from": "[with patch, needs review] sage-ptest must run in $SAGE_ROOT",
        "to": "[with patch, positive review] sage-ptest must run in $SAGE_ROOT"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2374#event-8295"
}
```



---

archive/issue_comments_016765.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-03-10T07:20:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2374#issuecomment-16765",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_016766.json:
```json
{
    "body": "<a id='comment:5'></a>\nMerged in Sage 2.10.3.rc4",
    "created_at": "2008-03-10T07:20:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2374#issuecomment-16766",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:5'></a>
Merged in Sage 2.10.3.rc4



---

archive/issue_events_008296.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-03-10T07:20:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2374",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2374#event-8296"
}
```
