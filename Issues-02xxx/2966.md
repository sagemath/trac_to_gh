# Issue 2966: [with patch; positive review] Debian Singular spkg upgrade

archive/issues_002966.json:
```json
{
    "body": "Since many of the SAGE modifications to Singular were fixed in the new upstream, the Debian packaging code needs some patches removed.  Patch attached.\n\nAssignee: @timabbott\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/2966\n\n",
    "closed_at": "2008-04-20T04:24:34Z",
    "created_at": "2008-04-20T04:05:36Z",
    "labels": [
        "component: debian-package",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0",
    "title": "[with patch; positive review] Debian Singular spkg upgrade",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2966",
    "user": "https://github.com/timabbott"
}
```
Since many of the SAGE modifications to Singular were fixed in the new upstream, the Debian packaging code needs some patches removed.  Patch attached.

Assignee: @timabbott

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/2966





---

archive/issue_comments_021443.json:
```json
{
    "body": "Attachment [singular-patches.patch](tarball://root/attachments/some-uuid/ticket2966/singular-patches.patch) by @timabbott created at 2008-04-20 04:05:44",
    "created_at": "2008-04-20T04:05:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2966",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2966#issuecomment-21443",
    "user": "https://github.com/timabbott"
}
```

Attachment [singular-patches.patch](tarball://root/attachments/some-uuid/ticket2966/singular-patches.patch) by @timabbott created at 2008-04-20 04:05:44



---

archive/issue_events_010542.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2966",
    "rename": {
        "from": "Debian Singular spkg upgrade",
        "to": "[with patch; needs review] Debian Singular spkg upgrade"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2966#event-10542"
}
```



---

archive/issue_events_010543.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2966",
    "rename": {
        "from": "[with patch; needs review] Debian Singular spkg upgrade",
        "to": "[with patch; positive review] Debian Singular spkg upgrade"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2966#event-10543"
}
```



---

archive/issue_comments_021444.json:
```json
{
    "body": "<a id='comment:2'></a>\nPatch looks good to me. Positive review.\n\nCheers,\n\nMichael",
    "created_at": "2008-04-20T04:24:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2966",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2966#issuecomment-21444",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Patch looks good to me. Positive review.

Cheers,

Michael



---

archive/issue_comments_021445.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.0.rc0",
    "created_at": "2008-04-20T04:24:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2966",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2966#issuecomment-21445",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.0.rc0



---

archive/issue_comments_021446.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-04-20T04:24:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2966",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2966#issuecomment-21446",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_010544.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-20T04:24:34Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2966",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2966#event-10544"
}
```



---

archive/issue_events_010545.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-20T04:32:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/2966",
    "milestone": "sage-3.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2966#event-10545"
}
```
