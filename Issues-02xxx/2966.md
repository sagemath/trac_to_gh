# Issue 2966: Debian Singular spkg upgrade

archive/issues_002966.json:
```json
{
    "body": "Since many of the SAGE modifications to Singular were fixed in the new upstream, the Debian packaging code needs some patches removed.  Patch attached.\n\n**Assignee:** @timabbott\n\nIssue created by migration from https://trac.sagemath.org/ticket/2966\n\n",
    "closed_at": "2008-04-20T04:24:34Z",
    "created_at": "2008-04-20T04:05:36Z",
    "labels": [
        "component: debian-package",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0",
    "title": "Debian Singular spkg upgrade",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2966",
    "user": "https://github.com/timabbott"
}
```
Since many of the SAGE modifications to Singular were fixed in the new upstream, the Debian packaging code needs some patches removed.  Patch attached.

**Assignee:** @timabbott

Issue created by migration from https://trac.sagemath.org/ticket/2966





---

archive/attachments_002823.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "singular-patches.patch",
    "asset_url": "tarball://root/attachments/some-uuid/ticket2966/singular-patches.patch",
    "created_at": "2008-04-20T04:05:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2966",
    "type": "attachment",
    "url": "https://github.com/assets/some-id/some-uuid.patch",
    "user": "https://github.com/timabbott"
}
```



---

archive/issue_comments_016297.json:
```json
{
    "body": "",
    "created_at": "2008-04-20T04:05:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2966",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2966#issuecomment-16297",
    "user": "https://github.com/timabbott"
}
```





---

archive/issue_events_015638.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-04-20T04:07:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/2966",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2966#event-15638"
}
```



---

archive/issue_events_015639.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-20T04:24:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/2966",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2966#event-15639"
}
```



---

archive/issue_events_015640.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-20T04:24:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/2966",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2966#event-15640"
}
```



---

archive/issue_comments_016298.json:
```json
{
    "body": "<a id='comment:2'></a>\nPatch looks good to me. Positive review.\n\nCheers,\n\nMichael",
    "created_at": "2008-04-20T04:24:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2966",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2966#issuecomment-16298",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Patch looks good to me. Positive review.

Cheers,

Michael



---

archive/issue_comments_016299.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.0.rc0",
    "created_at": "2008-04-20T04:24:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2966",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2966#issuecomment-16299",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.0.rc0



---

archive/issue_events_015641.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-20T04:24:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/2966",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2966#event-15641"
}
```



---

archive/issue_events_015642.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-20T04:24:34Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2966",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2966#event-15642"
}
```



---

archive/issue_events_015643.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-20T04:32:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/2966",
    "milestone": "sage-3.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2966#event-15643"
}
```



---

archive/issue_events_015644.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-20T04:32:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/2966",
    "milestone": "sage-3.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2966#event-15644"
}
```
