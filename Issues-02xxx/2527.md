# Issue 2527: get doctests for congroup.py and congroup_element.py up to 100%

archive/issues_002527.json:
```json
{
    "assignees": [],
    "body": "Start:\n\n```\nteragon:modular was$ sage -coverage congroup.py congroup_element.py \nSCORE congroup.py: 20% (14 of 68)\nSCORE congroup_element.py: 0% (0 of 10)\n```\n\n\n**Assignee:** @craigcitro\n\nIssue created by migration from https://trac.sagemath.org/ticket/2527\n\n",
    "closed_at": "2008-08-12T23:46:47Z",
    "created_at": "2008-03-15T02:40:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20modular%20forms",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "get doctests for congroup.py and congroup_element.py up to 100%",
    "type": "issue",
    "updated_at": "2008-08-12T23:46:47Z",
    "url": "https://github.com/sagemath/sage/issues/2527",
    "user": "https://github.com/williamstein"
}
```
Start:

```
teragon:modular was$ sage -coverage congroup.py congroup_element.py 
SCORE congroup.py: 20% (14 of 68)
SCORE congroup_element.py: 0% (0 of 10)
```


**Assignee:** @craigcitro

Issue created by migration from https://trac.sagemath.org/ticket/2527





---

archive/issue_events_021178.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-03-15T02:40:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2527",
    "milestone_number": null,
    "milestone_title": "sage-3.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2527#event-21178"
}
```



---

archive/issue_events_021179.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-03-15T02:40:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2527",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20theory",
    "label_color": "08517b",
    "label_name": "component: number theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2527#event-21179"
}
```



---

archive/issue_events_021180.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-03-15T02:40:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2527",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "008080",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2527#event-21180"
}
```



---

archive/issue_events_021181.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2008-04-23T02:36:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2527",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20theory",
    "label_color": "08517b",
    "label_name": "component: number theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2527#event-21181"
}
```



---

archive/issue_events_021182.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2008-04-23T02:36:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2527",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20modular%20forms",
    "label_color": "08517b",
    "label_name": "component: modular forms",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2527#event-21182"
}
```



---

archive/issue_comments_013645.json:
```json
{
    "body": "**Changing assignee** from @williamstein to @craigcitro.",
    "created_at": "2008-04-23T02:36:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2527",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2527#issuecomment-13645",
    "user": "https://github.com/aghitza"
}
```

**Changing assignee** from @williamstein to @craigcitro.



---

archive/issue_events_021183.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-08-12T23:46:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/2527",
    "milestone_number": null,
    "milestone_title": "sage-3.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2527#event-21183"
}
```



---

archive/issue_events_021184.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-08-12T23:46:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2527",
    "milestone_number": null,
    "milestone_title": "sage-3.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2527#event-21184"
}
```



---

archive/issue_comments_013646.json:
```json
{
    "body": "<a id='comment:2'>**Comment 2:**</a>\nThis was achieved in Sage 3.1.alpha2 via #1220.\n\nCheers,\n\nMichael",
    "created_at": "2008-08-12T23:46:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2527",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2527#issuecomment-13646",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:2'>**Comment 2:**</a>
This was achieved in Sage 3.1.alpha2 via #1220.

Cheers,

Michael



---

archive/issue_events_021185.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-08-12T23:46:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2527",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2527#event-21185"
}
```
