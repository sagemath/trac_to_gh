# Issue 2607: find_minimum_on_interval() uses the wrong scipy function

archive/issues_002607.json:
```json
{
    "assignees": [],
    "body": "This was reported by Dean Moore on sage-support.  Consider:\n\n```\nsage: f(x) = -x*sin(x^2)\nsage: f.find_minimum_on_interval(-2.5, 2)\n(-1.3076194129914434, 1.35521114057)\nsage: f.find_minimum_on_interval(-2.5, -1)\n(-2.1827697846777219, -2.19450274985)\n```\n\nSo find_minimum_on_interval() returns a local minimum as opposed to the global one.  (The same issue applies to find_maximum_on_interval.)  This is due to the fact that the function wraps scipy.optimize.fminbound, which is only a local optimizer (Carl Witty pointed this out).  We should instead use one of the global optimizers, i.e. scipy.optimize.anneal or scipy.optimize.brute.\n\n**Apply:**\n* [attachment: trac2607.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607.patch.gz)\n* [attachment: trac2607-whitespace.2.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607-whitespace.2.patch.gz)\n* [attachment: trac_2607_renaming.3.patch](https://github.com/sagemath/sage/files/ticket2607/trac_2607_renaming.3.patch.gz)\n* [attachment: trac2607-doctest-and-spacing.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607-doctest-and-spacing.patch.gz)\n* [attachment: trac_2607_update_deprecation.patch](https://github.com/sagemath/sage/files/ticket2607/trac_2607_update_deprecation.patch.gz)\n\nDepends on #13109\n\nAssignee: **@jicama**\n\nCC:  @robert-marik @kcrisman\n\nKeywords: **sd31, sd40.5**\n\nReviewer: **Karl-Dieter Crisman, Mike Hansen, Andrey Novoseltsev**\n\nAuthor: **Dan Drake, Andrey Novoseltsev, Andrzej Giniewicz, Volker Braun**\n\nMerged: **sage-5.3.beta0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/2607_\n\n",
    "closed_at": "2012-08-01T12:08:41Z",
    "created_at": "2008-03-19T22:17:25Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20calculus",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "find_minimum_on_interval() uses the wrong scipy function",
    "type": "issue",
    "updated_at": "2012-08-01T12:08:41Z",
    "url": "https://github.com/sagemath/sage/issues/2607",
    "user": "https://github.com/aghitza"
}
```
This was reported by Dean Moore on sage-support.  Consider:

```
sage: f(x) = -x*sin(x^2)
sage: f.find_minimum_on_interval(-2.5, 2)
(-1.3076194129914434, 1.35521114057)
sage: f.find_minimum_on_interval(-2.5, -1)
(-2.1827697846777219, -2.19450274985)
```

So find_minimum_on_interval() returns a local minimum as opposed to the global one.  (The same issue applies to find_maximum_on_interval.)  This is due to the fact that the function wraps scipy.optimize.fminbound, which is only a local optimizer (Carl Witty pointed this out).  We should instead use one of the global optimizers, i.e. scipy.optimize.anneal or scipy.optimize.brute.

**Apply:**
* [attachment: trac2607.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607.patch.gz)
* [attachment: trac2607-whitespace.2.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607-whitespace.2.patch.gz)
* [attachment: trac_2607_renaming.3.patch](https://github.com/sagemath/sage/files/ticket2607/trac_2607_renaming.3.patch.gz)
* [attachment: trac2607-doctest-and-spacing.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607-doctest-and-spacing.patch.gz)
* [attachment: trac_2607_update_deprecation.patch](https://github.com/sagemath/sage/files/ticket2607/trac_2607_update_deprecation.patch.gz)

Depends on #13109

Assignee: **@jicama**

CC:  @robert-marik @kcrisman

Keywords: **sd31, sd40.5**

Reviewer: **Karl-Dieter Crisman, Mike Hansen, Andrey Novoseltsev**

Author: **Dan Drake, Andrey Novoseltsev, Andrzej Giniewicz, Volker Braun**

Merged: **sage-5.3.beta0**

_Issue created by migration from https://trac.sagemath.org/ticket/2607_





---

archive/issue_events_024131.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2008-03-19T22:17:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "milestone_number": null,
    "milestone_title": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24131"
}
```



---

archive/issue_events_024132.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2008-03-19T22:17:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20calculus",
    "label_color": "0000ff",
    "label_name": "component: calculus",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24132"
}
```



---

archive/issue_events_024133.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2008-03-19T22:17:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24133"
}
```



---

archive/issue_events_024134.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2008-03-19T22:17:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24134"
}
```



---

archive/issue_events_024135.json:
```json
{
    "actor": "https://github.com/jicama",
    "created_at": "2008-08-31T14:30:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20calculus",
    "label_color": "0000ff",
    "label_name": "component: calculus",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24135"
}
```



---

archive/issue_events_024136.json:
```json
{
    "actor": "https://github.com/jicama",
    "created_at": "2008-08-31T14:30:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20numerical",
    "label_color": "0000ff",
    "label_name": "component: numerical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24136"
}
```



---

archive/issue_events_024137.json:
```json
{
    "actor": "https://github.com/jicama",
    "created_at": "2008-08-31T15:08:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20numerical",
    "label_color": "0000ff",
    "label_name": "component: numerical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24137"
}
```



---

archive/issue_events_024138.json:
```json
{
    "actor": "https://github.com/jicama",
    "created_at": "2008-08-31T15:08:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20calculus",
    "label_color": "0000ff",
    "label_name": "component: calculus",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24138"
}
```



---

archive/issue_comments_014126.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nIt seems that there are two global optimizers in scipy: scipy.optimize.anneal and scipy.optimize.brute. Does anybody more experienced in numerics know, which one is better for including into Sage?",
    "created_at": "2010-08-25T21:24:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14126",
    "user": "https://github.com/robert-marik"
}
```

<div id="comment:3" align="right">Comment 3</div>

It seems that there are two global optimizers in scipy: scipy.optimize.anneal and scipy.optimize.brute. Does anybody more experienced in numerics know, which one is better for including into Sage?



---

archive/issue_comments_014127.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nRobert, that sounds like a great question for either sage-devel or the scipy list.",
    "created_at": "2010-08-25T22:28:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14127",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:5" align="right">Comment 5</div>

Robert, that sounds like a great question for either sage-devel or the scipy list.



---

archive/issue_comments_014128.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nThis [Scipy tutorial page](http://docs.scipy.org/doc/scipy/reference/tutorial/optimize.html) should be relevant.  I will try to resolve this soon.",
    "created_at": "2011-06-15T21:56:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14128",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:7" align="right">Comment 7</div>

This [Scipy tutorial page](http://docs.scipy.org/doc/scipy/reference/tutorial/optimize.html) should be relevant.  I will try to resolve this soon.



---

archive/issue_comments_014129.json:
```json
{
    "body": "Changed keywords from none to **sd31**",
    "created_at": "2011-06-15T21:56:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14129",
    "user": "https://github.com/kcrisman"
}
```

Changed keywords from none to **sd31**



---

archive/issue_comments_014130.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\n#5960 was a dup.   Here are the examples from there.\n\n```\nsage: h(x) =  -sin(x) - 2*sin(2*x)\nsage: h.find_minimum_on_interval(0, 2*pi)\n(-1.3271810224585345, 3.8298351449342838)\nBut there is another local minimum at h(0.8666760871050464) = -2.73581510406\n\n\nsage: find_minimum_on_interval(x*(x-1)*(x+1), -2, 2)\n(-0.38490017945975047, 0.57735026913115706)\nThe minimum on this interval is the endpoint h(-2) = 6.\n\n\nsage: find_minimum_on_interval((x-2)*(x-1)*x*(x+1) - x, -2, 2)\n(-0.43749999999999994, -0.49999999973911674)\n\nbut\nsage: find_minimum_on_interval((x-2)*(x-1)*x*(x+1) - x, 0, 2)\n(-2.6642135623730949, 1.7071067879138031)\n```",
    "created_at": "2011-06-16T00:07:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14130",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:8" align="right">Comment 8</div>

#5960 was a dup.   Here are the examples from there.

```
sage: h(x) =  -sin(x) - 2*sin(2*x)
sage: h.find_minimum_on_interval(0, 2*pi)
(-1.3271810224585345, 3.8298351449342838)
But there is another local minimum at h(0.8666760871050464) = -2.73581510406


sage: find_minimum_on_interval(x*(x-1)*(x+1), -2, 2)
(-0.38490017945975047, 0.57735026913115706)
The minimum on this interval is the endpoint h(-2) = 6.


sage: find_minimum_on_interval((x-2)*(x-1)*x*(x+1) - x, -2, 2)
(-0.43749999999999994, -0.49999999973911674)

but
sage: find_minimum_on_interval((x-2)*(x-1)*x*(x+1) - x, 0, 2)
(-2.6642135623730949, 1.7071067879138031)
```



---

archive/issue_comments_014131.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nThe `brent` algorithm will also not work.\n\n```\nTriple (a,b,c) where (a<b<c) and func(b) < func(a),func(c). If bracket consists of two numbers (a,c) then they are assumed to be a starting interval for a downhill bracket search (see bracket); it doesn\u2019t always mean that the obtained solution will satisfy a<=x<=c.\n```\nWhich is not the same as constrained minimization, for us.",
    "created_at": "2011-06-16T00:12:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14131",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:9" align="right">Comment 9</div>

The `brent` algorithm will also not work.

```
Triple (a,b,c) where (a<b<c) and func(b) < func(a),func(c). If bracket consists of two numbers (a,c) then they are assumed to be a starting interval for a downhill bracket search (see bracket); it doesn’t always mean that the obtained solution will satisfy a<=x<=c.
```
Which is not the same as constrained minimization, for us.



---

archive/issue_comments_014132.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nIt seems like finding a local minimum might be the best you can hope for with a general function. Wouldn't finding an absolute minimum (on an interval) be intractable unless you can exploit some special structure of the function?",
    "created_at": "2011-06-16T00:18:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14132",
    "user": "https://github.com/jicama"
}
```

<div id="comment:10" align="right">Comment 10</div>

It seems like finding a local minimum might be the best you can hope for with a general function. Wouldn't finding an absolute minimum (on an interval) be intractable unless you can exploit some special structure of the function?



---

archive/issue_comments_014133.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nAnswering the question first...\n\nSure, but it would be nice if we at least got the 'right' answer for 'easy' functions.  That's all I'm looking for here, not things like finding a minimum on a set of measure zero...\n\n---\n\n\nI think we can fix Brent to use this.  Compare:\n\n```\nsage: optimize.fminbound(h._fast_float_(x),0,6,full_output=True)\n(3.8298366870225147, -1.327181022449951, 0, 10)\nsage: optimize.fminbound(h._fast_float_(x),0,3,full_output=True)\n(0.86667541098916612, -2.7358151040622416, 0, 9)\nsage: optimize.brent(h._fast_float_(x),brack=(0,6),full_output=True)\n(0.86667608708813437, -2.73581510406422, 11, 12)\n```\nThis shows that brent does give the 'right' answer in this case.   So when does it give a 'wrong' answer?\n\n```\nsage: j(x) = sin(x)\nsage: optimize.brent(j._fast_float_(x),brack=(0,6),full_output=True)\n(10.995574367047061, -0.99999999999999689, 10, 11)\nsage: 3.5*pi.n()\n10.9955742875643\n```\nWell, of course - there IS no calculus-style minimum of sin between 0 and pi!  Only a minimum relative to the interval itself.  Interesting that it goes all the way to 7/2*pi, rather than 3/2*pi, but oh well!\n\nSo the fix is to switch to Brent, and then if it gives an answer outside the interval, pick the 'lower' endpoint.  This would need lots of testing with well-behaved functions to make sure they actually work correctly.",
    "created_at": "2011-06-16T00:26:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14133",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:11" align="right">Comment 11</div>

Answering the question first...

Sure, but it would be nice if we at least got the 'right' answer for 'easy' functions.  That's all I'm looking for here, not things like finding a minimum on a set of measure zero...

---


I think we can fix Brent to use this.  Compare:

```
sage: optimize.fminbound(h._fast_float_(x),0,6,full_output=True)
(3.8298366870225147, -1.327181022449951, 0, 10)
sage: optimize.fminbound(h._fast_float_(x),0,3,full_output=True)
(0.86667541098916612, -2.7358151040622416, 0, 9)
sage: optimize.brent(h._fast_float_(x),brack=(0,6),full_output=True)
(0.86667608708813437, -2.73581510406422, 11, 12)
```
This shows that brent does give the 'right' answer in this case.   So when does it give a 'wrong' answer?

```
sage: j(x) = sin(x)
sage: optimize.brent(j._fast_float_(x),brack=(0,6),full_output=True)
(10.995574367047061, -0.99999999999999689, 10, 11)
sage: 3.5*pi.n()
10.9955742875643
```
Well, of course - there IS no calculus-style minimum of sin between 0 and pi!  Only a minimum relative to the interval itself.  Interesting that it goes all the way to 7/2*pi, rather than 3/2*pi, but oh well!

So the fix is to switch to Brent, and then if it gives an answer outside the interval, pick the 'lower' endpoint.  This would need lots of testing with well-behaved functions to make sure they actually work correctly.



---

archive/issue_comments_014134.json:
```json
{
    "body": "Changed assignee from **@williamstein** to **@jicama**",
    "created_at": "2011-06-16T00:58:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14134",
    "user": "https://github.com/jicama"
}
```

Changed assignee from **@williamstein** to **@jicama**



---

archive/issue_comments_014135.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nBut how will you explain to users which functions are 'easy', and when they should expect to get the 'right' answer? I think it's better design to just change the contract of this function to admit that it is only looking for local minima.",
    "created_at": "2011-06-16T00:58:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14135",
    "user": "https://github.com/jicama"
}
```

<div id="comment:12" align="right">Comment 12</div>

But how will you explain to users which functions are 'easy', and when they should expect to get the 'right' answer? I think it's better design to just change the contract of this function to admit that it is only looking for local minima.



---

archive/issue_comments_014136.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nIt doesn't matter.  Or, at worst, we add some documentation to clarify that.  \n\nThe reason it doesn't matter is that this is still *better* than the other.  Unless you can produce some (natural) examples where optimize.brent does the same.  \n\nThe Scipy documentation is not 100% clear on what is done, and it's conceivable they are the same.  It's certainly possible that in fact using optimize.brent in the way I'm suggesting would be just as 'bad' as the previous one, or even essentially equivalent.  But it would be nice to have an explicit example of this before we resort to that.",
    "created_at": "2011-06-16T01:06:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14136",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:13" align="right">Comment 13</div>

It doesn't matter.  Or, at worst, we add some documentation to clarify that.  

The reason it doesn't matter is that this is still *better* than the other.  Unless you can produce some (natural) examples where optimize.brent does the same.  

The Scipy documentation is not 100% clear on what is done, and it's conceivable they are the same.  It's certainly possible that in fact using optimize.brent in the way I'm suggesting would be just as 'bad' as the previous one, or even essentially equivalent.  But it would be nice to have an explicit example of this before we resort to that.



---

archive/issue_comments_014137.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nHere's another example where `brent` does better.\n\n```\nsage: j(x) = sin(x^8)-.1*x\nsage: optimize.brent(j._fast_float_(x),brack=(0,2),full_output=True)\n(2.0000389609484905, -1.2000038913452364, 22, 23)\nsage: optimize.fminbound(j._fast_float_(x),0,2,full_output=True)\n(1.5288339777087034, -1.152883200877608, 0, 16)\n```\nOf course, this does cause problems for my supposed algorithm to then go back to an endpoint, since it's *just* outside of it...",
    "created_at": "2011-06-16T01:10:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14137",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:14" align="right">Comment 14</div>

Here's another example where `brent` does better.

```
sage: j(x) = sin(x^8)-.1*x
sage: optimize.brent(j._fast_float_(x),brack=(0,2),full_output=True)
(2.0000389609484905, -1.2000038913452364, 22, 23)
sage: optimize.fminbound(j._fast_float_(x),0,2,full_output=True)
(1.5288339777087034, -1.152883200877608, 0, 16)
```
Of course, this does cause problems for my supposed algorithm to then go back to an endpoint, since it's *just* outside of it...



---

archive/issue_comments_014138.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">Comment 15</div>\n\nI just ran into this bug with the following input:\n\n```\n   find_maximum_on_interval(fast_float(8*e^(-x)*sin(x) - 1, x), 0, 8)\n    (1.5791755355586754, 0.78539817769603915)\n\n    find_maximum_on_interval(fast_float(8*e^(-x)*sin(x) - 1, x), 0, 9)\n    (-0.9951835373923219, 7.0685835435476418)\n```\n...and was truly surprised that `find_maximum_on_interval` is not monotonic (in the sense that a bigger interval should always give a (weakly) bigger maximum)!\n\nAt the VERY LEAST, we should fix the documentation to specify that this finds *local* extrema, and perhaps change the name of the function, too, since it does *not* always find the actual maximum value on the interval!\n\nNote that one strange workaround is to simply plot the function; something like:\n\n```\ndef find_maximum_on_interval(f, a, b):\n    return plot(f, a, b).ymax()\n```\nseems like it would be pretty effective, despite being inelegant and crude!",
    "created_at": "2012-04-23T01:57:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14138",
    "user": "https://github.com/dandrake"
}
```

<div id="comment:15" align="right">Comment 15</div>

I just ran into this bug with the following input:

```
   find_maximum_on_interval(fast_float(8*e^(-x)*sin(x) - 1, x), 0, 8)
    (1.5791755355586754, 0.78539817769603915)

    find_maximum_on_interval(fast_float(8*e^(-x)*sin(x) - 1, x), 0, 9)
    (-0.9951835373923219, 7.0685835435476418)
```
...and was truly surprised that `find_maximum_on_interval` is not monotonic (in the sense that a bigger interval should always give a (weakly) bigger maximum)!

At the VERY LEAST, we should fix the documentation to specify that this finds *local* extrema, and perhaps change the name of the function, too, since it does *not* always find the actual maximum value on the interval!

Note that one strange workaround is to simply plot the function; something like:

```
def find_maximum_on_interval(f, a, b):
    return plot(f, a, b).ymax()
```
seems like it would be pretty effective, despite being inelegant and crude!



---

archive/issue_comments_014139.json:
```json
{
    "body": "Author: **Dan Drake**",
    "created_at": "2012-05-25T00:47:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14139",
    "user": "https://github.com/dandrake"
}
```

Author: **Dan Drake**



---

archive/issue_comments_014140.json:
```json
{
    "body": "Changed keywords from **sd31** to **sd31, sd40.5**",
    "created_at": "2012-05-25T00:47:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14140",
    "user": "https://github.com/dandrake"
}
```

Changed keywords from **sd31** to **sd31, sd40.5**



---

archive/issue_comments_014141.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">Comment 16</div>\n\nPatch up, please review. I have just changed the documentation and added some suggestions for a workaround using `plot(...).ymin()`. I think we should at least merge something like this right away and worry about fixing the algorithm later.",
    "created_at": "2012-05-25T00:47:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14141",
    "user": "https://github.com/dandrake"
}
```

<div id="comment:16" align="right">Comment 16</div>

Patch up, please review. I have just changed the documentation and added some suggestions for a workaround using `plot(...).ymin()`. I think we should at least merge something like this right away and worry about fixing the algorithm later.



---

archive/issue_events_024139.json:
```json
{
    "actor": "https://github.com/dandrake",
    "created_at": "2012-05-25T00:47:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24139"
}
```



---

archive/issue_comments_014142.json:
```json
{
    "body": "<div id=\"comment:17\" align=\"right\">Comment 17</div>\n\nSuggestions in person about how to further enhance the messages in documentation.  Thank you so much for doing this - don't forget to open a followup ticket.",
    "created_at": "2012-05-25T00:55:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14142",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:17" align="right">Comment 17</div>

Suggestions in person about how to further enhance the messages in documentation.  Thank you so much for doing this - don't forget to open a followup ticket.



---

archive/issue_comments_014143.json:
```json
{
    "body": "Reviewer: **Karl-Dieter Crisman**",
    "created_at": "2012-05-25T00:55:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14143",
    "user": "https://github.com/kcrisman"
}
```

Reviewer: **Karl-Dieter Crisman**



---

archive/issue_events_024140.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-05-25T00:55:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24140"
}
```



---

archive/issue_events_024141.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-05-25T00:55:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24141"
}
```



---

archive/issue_comments_014144.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">Comment 18</div>\n\nI fixed the versions of this function in `symbolic.expression` to specify that they call the `numerical.optimize` version. Please check the documentation; I looked at the bare html, but am working remotely and haven't viewed the result.",
    "created_at": "2012-05-25T02:42:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14144",
    "user": "https://github.com/dandrake"
}
```

<div id="comment:18" align="right">Comment 18</div>

I fixed the versions of this function in `symbolic.expression` to specify that they call the `numerical.optimize` version. Please check the documentation; I looked at the bare html, but am working remotely and haven't viewed the result.



---

archive/issue_events_024142.json:
```json
{
    "actor": "https://github.com/dandrake",
    "created_at": "2012-05-25T02:42:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24142"
}
```



---

archive/issue_events_024143.json:
```json
{
    "actor": "https://github.com/dandrake",
    "created_at": "2012-05-25T02:42:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24143"
}
```



---

archive/issue_comments_014145.json:
```json
{
    "body": "<div id=\"comment:19\" align=\"right\">Comment 19</div>\n\nThe whitespace patch removes trailing whitespace (and tabs! **TABS**!) from the relevant source files.",
    "created_at": "2012-05-25T03:34:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14145",
    "user": "https://github.com/dandrake"
}
```

<div id="comment:19" align="right">Comment 19</div>

The whitespace patch removes trailing whitespace (and tabs! **TABS**!) from the relevant source files.



---

archive/issue_comments_014146.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -10,3 +10,4 @@\n \n So find_minimum_on_interval() returns a local minimum as opposed to the global one.  (The same issue applies to find_maximum_on_interval.)  This is due to the fact that the function wraps scipy.optimize.fminbound, which is only a local optimizer (Carl Witty pointed this out).  We should instead use one of the global optimizers, i.e. scipy.optimize.anneal or scipy.optimize.brute.\n \n+Apply trac2607.patch and trac2607-whitespace.patch\n``````\n",
    "created_at": "2012-05-25T03:36:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14146",
    "user": "https://github.com/dandrake"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -10,3 +10,4 @@
 
 So find_minimum_on_interval() returns a local minimum as opposed to the global one.  (The same issue applies to find_maximum_on_interval.)  This is due to the fact that the function wraps scipy.optimize.fminbound, which is only a local optimizer (Carl Witty pointed this out).  We should instead use one of the global optimizers, i.e. scipy.optimize.anneal or scipy.optimize.brute.
 
+Apply trac2607.patch and trac2607-whitespace.patch
``````




---

archive/issue_comments_014147.json:
```json
{
    "body": "<div id=\"comment:21\" align=\"right\">Comment 21</div>\n\nNeeds work for sentence that doesn't end and `:trac:` thing...",
    "created_at": "2012-05-25T03:50:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14147",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:21" align="right">Comment 21</div>

Needs work for sentence that doesn't end and `:trac:` thing...



---

archive/issue_comments_014148.json:
```json
{
    "body": "apply to Sage library",
    "created_at": "2012-05-25T05:34:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14148",
    "user": "https://github.com/dandrake"
}
```

apply to Sage library



---

archive/issue_comments_014149.json:
```json
{
    "body": "<div id=\"comment:22\" align=\"right\">Comment 22</div>\n\nAttachment: **[trac2607.patch.gz](https://github.com/sagemath/sage/files/ticket2607/trac2607.patch.gz)**\n\nAdd a \"...\" to fix doctest on OS X.",
    "created_at": "2012-05-25T05:35:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14149",
    "user": "https://github.com/dandrake"
}
```

<div id="comment:22" align="right">Comment 22</div>

Attachment: **[trac2607.patch.gz](https://github.com/sagemath/sage/files/ticket2607/trac2607.patch.gz)**

Add a "..." to fix doctest on OS X.



---

archive/issue_comments_014150.json:
```json
{
    "body": "<div id=\"comment:23\" align=\"right\">Comment 23</div>\n\nshould `trac2607.2.patch` be removed?\n\nPaul",
    "created_at": "2012-05-25T07:46:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14150",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:23" align="right">Comment 23</div>

should `trac2607.2.patch` be removed?

Paul



---

archive/issue_comments_014151.json:
```json
{
    "body": "<div id=\"comment:24\" align=\"right\">Comment 24</div>\n\nafter applying both patches on Sage 5.0, I still get:\n\n```\nsage: f(x) = -x*sin(x^2)                 \nsage: f.find_minimum_on_interval(-2.5, 2)\n(-1.3076194129914434, 1.3552111405712108)\n```\nDid I something wrong?\n\nPaul",
    "created_at": "2012-05-25T08:03:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14151",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:24" align="right">Comment 24</div>

after applying both patches on Sage 5.0, I still get:

```
sage: f(x) = -x*sin(x^2)                 
sage: f.find_minimum_on_interval(-2.5, 2)
(-1.3076194129914434, 1.3552111405712108)
```
Did I something wrong?

Paul



---

archive/issue_comments_014152.json:
```json
{
    "body": "<div id=\"comment:25\" align=\"right\">Comment 25</div>\n\nThe patch doesn't change the behaviour, it only warns the user that it only returns some local minimum.",
    "created_at": "2012-05-25T13:15:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14152",
    "user": "https://github.com/sagetrac-dsm"
}
```

<div id="comment:25" align="right">Comment 25</div>

The patch doesn't change the behaviour, it only warns the user that it only returns some local minimum.



---

archive/issue_comments_014153.json:
```json
{
    "body": "<div id=\"comment:26\" align=\"right\">Comment 26</div>\n\nReplying to [@sagetrac-dsm](#comment%3A25):\n> The patch doesn't change the behaviour, it only warns the user that it only returns some local minimum.\n\nthen I don't get any warning (see comment [comment:24]).\n\nPaul",
    "created_at": "2012-05-25T13:23:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14153",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:26" align="right">Comment 26</div>

Replying to [@sagetrac-dsm](#comment%3A25):
> The patch doesn't change the behaviour, it only warns the user that it only returns some local minimum.

then I don't get any warning (see comment [comment:24]).

Paul



---

archive/issue_comments_014154.json:
```json
{
    "body": "<div id=\"comment:27\" align=\"right\">Comment 27</div>\n\n@sagetrac-dsm: Correct, and we HAVE to open a new ticket to get something better eventually.  But that turns out to be hard with the current tools.\n\nHmm, maybe Paul is suggesting we should use the new \"system warning that you won't get what you think/mathematically correct result\" in Sage whose name I forget?  Paul, the warning is just in the documentation, not the function itself.\n\nI also have a tiny change to this so that the documentation looks better coming up.",
    "created_at": "2012-05-25T13:26:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14154",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:27" align="right">Comment 27</div>

@sagetrac-dsm: Correct, and we HAVE to open a new ticket to get something better eventually.  But that turns out to be hard with the current tools.

Hmm, maybe Paul is suggesting we should use the new "system warning that you won't get what you think/mathematically correct result" in Sage whose name I forget?  Paul, the warning is just in the documentation, not the function itself.

I also have a tiny change to this so that the documentation looks better coming up.



---

archive/issue_comments_014155.json:
```json
{
    "body": "<div id=\"comment:28\" align=\"right\">Comment 28</div>\n\nActually, if we need to do more along the lines of warnings, I'll just put it here.  The point is the tilde, as opposed to having the big long thing show up in the doc (while still telling people that it's in another place, which I imagine was the point of not having the tilde before).\n\n```\n-Uses :func:`sage.numerical.optimize.find_minimum_on_interval`\n+Uses the :func:`~sage.numerical.optimize.find_minimum_on_interval`\n+function in the numerical optimization module of Sage.\n```",
    "created_at": "2012-05-25T13:29:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14155",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:28" align="right">Comment 28</div>

Actually, if we need to do more along the lines of warnings, I'll just put it here.  The point is the tilde, as opposed to having the big long thing show up in the doc (while still telling people that it's in another place, which I imagine was the point of not having the tilde before).

```
-Uses :func:`sage.numerical.optimize.find_minimum_on_interval`
+Uses the :func:`~sage.numerical.optimize.find_minimum_on_interval`
+function in the numerical optimization module of Sage.
```



---

archive/issue_comments_014156.json:
```json
{
    "body": "<div id=\"comment:29\" align=\"right\">Comment 29</div>\n\nKarl-Dieter, you mean the \"stopgap\" mechanism?\n\nPaul",
    "created_at": "2012-05-25T14:10:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14156",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:29" align="right">Comment 29</div>

Karl-Dieter, you mean the "stopgap" mechanism?

Paul



---

archive/issue_comments_014157.json:
```json
{
    "body": "<div id=\"comment:30\" align=\"right\">Comment 30</div>\n\nI think so.  I've not yet used it, so I don't know if it would be appropriate here.",
    "created_at": "2012-05-25T14:22:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14157",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:30" align="right">Comment 30</div>

I think so.  I've not yet used it, so I don't know if it would be appropriate here.



---

archive/issue_comments_014158.json:
```json
{
    "body": "<div id=\"comment:31\" align=\"right\">Comment 31</div>\n\nReplying to [@zimmermann6](#comment%3A23):\n> should `trac2607.2.patch` be removed?\n\nYes. Or simply ignored.\n\nReplying to [@zimmermann6](#comment%3A26):\n> then I don't get any warning\n\nThe warning is in the documentation, which isn't the best...but it's better than right now, where it's impossible to figure out without reading a lot of code. There will, of course, be a followup ticket that actually fixes the functionality.",
    "created_at": "2012-05-25T15:42:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14158",
    "user": "https://github.com/dandrake"
}
```

<div id="comment:31" align="right">Comment 31</div>

Replying to [@zimmermann6](#comment%3A23):
> should `trac2607.2.patch` be removed?

Yes. Or simply ignored.

Replying to [@zimmermann6](#comment%3A26):
> then I don't get any warning

The warning is in the documentation, which isn't the best...but it's better than right now, where it's impossible to figure out without reading a lot of code. There will, of course, be a followup ticket that actually fixes the functionality.



---

archive/issue_comments_014159.json:
```json
{
    "body": "<div id=\"comment:32\" align=\"right\">Comment 32</div>\n\nI'm not in favour of giving a positive review, since the proposed patch does not solve the problem described in the description of that ticket.\n\nPaul",
    "created_at": "2012-05-25T18:14:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14159",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:32" align="right">Comment 32</div>

I'm not in favour of giving a positive review, since the proposed patch does not solve the problem described in the description of that ticket.

Paul



---

archive/issue_comments_014160.json:
```json
{
    "body": "<div id=\"comment:33\" align=\"right\">Comment 33</div>\n\nReplying to [@zimmermann6](#comment%3A32):\n> I'm not in favour of giving a positive review, since the proposed patch does not solve the problem described in the description of that ticket.\n\nFair enough. However, no one is offerring a fix to the code; it looks like we are going to change the documentation, and then later fix the actual function. This will require two tickets. We could use this ticket for the documentation and a new ticket for the code, or vice versa. We can change the description of this ticket if we want.\n\nThe exact way we do this doesn't seem very important to me, as long as we fix the documentation and later fix the code. If you would like to make a new ticket and move [attachment: trac2607.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607.patch.gz) and [attachment: trac2607-whitespace.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607-whitespace.patch.gz) over, that's fine.",
    "created_at": "2012-05-25T22:09:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14160",
    "user": "https://github.com/dandrake"
}
```

<div id="comment:33" align="right">Comment 33</div>

Replying to [@zimmermann6](#comment%3A32):
> I'm not in favour of giving a positive review, since the proposed patch does not solve the problem described in the description of that ticket.

Fair enough. However, no one is offerring a fix to the code; it looks like we are going to change the documentation, and then later fix the actual function. This will require two tickets. We could use this ticket for the documentation and a new ticket for the code, or vice versa. We can change the description of this ticket if we want.

The exact way we do this doesn't seem very important to me, as long as we fix the documentation and later fix the code. If you would like to make a new ticket and move [attachment: trac2607.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607.patch.gz) and [attachment: trac2607-whitespace.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607-whitespace.patch.gz) over, that's fine.



---

archive/issue_comments_014161.json:
```json
{
    "body": "<div id=\"comment:34\" align=\"right\">Comment 34</div>\n\nIt seems reasonable to me to fix the docs now, add `stopgap`, and open a new ticket to address the global / local optimization issue; especially given how old this ticket is. In this case the added documentation that refers to \"See :trac:`2607`\" should be updated to point to the new ticket.",
    "created_at": "2012-05-26T02:56:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14161",
    "user": "https://github.com/benjaminfjones"
}
```

<div id="comment:34" align="right">Comment 34</div>

It seems reasonable to me to fix the docs now, add `stopgap`, and open a new ticket to address the global / local optimization issue; especially given how old this ticket is. In this case the added documentation that refers to "See :trac:`2607`" should be updated to point to the new ticket.



---

archive/issue_comments_014162.json:
```json
{
    "body": "<div id=\"comment:35\" align=\"right\">Comment 35</div>\n\nHere's a relevant sage-support thread, showing some other problems with these functions: https://groups.google.com/forum/#!topic/sage-support/KCjW5QlB_sA",
    "created_at": "2012-05-26T17:37:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14162",
    "user": "https://github.com/dandrake"
}
```

<div id="comment:35" align="right">Comment 35</div>

Here's a relevant sage-support thread, showing some other problems with these functions: https://groups.google.com/forum/#!topic/sage-support/KCjW5QlB_sA



---

archive/issue_comments_014163.json:
```json
{
    "body": "<div id=\"comment:36\" align=\"right\">Comment 36</div>\n\npatchbot: apply trac2607.patch trac2607-whitespace.patch",
    "created_at": "2012-05-26T19:54:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14163",
    "user": "https://github.com/kini"
}
```

<div id="comment:36" align="right">Comment 36</div>

patchbot: apply trac2607.patch trac2607-whitespace.patch



---

archive/issue_events_024144.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2012-05-27T05:07:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24144"
}
```



---

archive/issue_events_024145.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2012-05-27T05:07:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24145"
}
```



---

archive/issue_comments_014164.json:
```json
{
    "body": "<div id=\"comment:37\" align=\"right\">Comment 37</div>\n\nI talked to Dan and plan to write the following patch:\n* rename this functions to clearly mark that they compute local extrema;\n* keep old names with deprecation warning and explanation of their behaviour.\n\nWill do it on top of the current patches, so no changes please!",
    "created_at": "2012-05-27T05:07:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14164",
    "user": "https://github.com/novoselt"
}
```

<div id="comment:37" align="right">Comment 37</div>

I talked to Dan and plan to write the following patch:
* rename this functions to clearly mark that they compute local extrema;
* keep old names with deprecation warning and explanation of their behaviour.

Will do it on top of the current patches, so no changes please!



---

archive/issue_comments_014165.json:
```json
{
    "body": "Work Issues: **deprecation**",
    "created_at": "2012-05-27T05:07:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14165",
    "user": "https://github.com/novoselt"
}
```

Work Issues: **deprecation**



---

archive/issue_comments_014166.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -10,4 +10,7 @@\n \n So find_minimum_on_interval() returns a local minimum as opposed to the global one.  (The same issue applies to find_maximum_on_interval.)  This is due to the fact that the function wraps scipy.optimize.fminbound, which is only a local optimizer (Carl Witty pointed this out).  We should instead use one of the global optimizers, i.e. scipy.optimize.anneal or scipy.optimize.brute.\n \n-Apply trac2607.patch and trac2607-whitespace.patch\n+**Apply:**\n+* [attachment: trac2607.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607.patch.gz)\n+* [attachment: trac2607-whitespace.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607-whitespace.patch.gz)\n+* [attachment: trac_2607_renaming.patch](https://github.com/sagemath/sage/files/ticket2607/trac_2607_renaming.patch.gz)\n``````\n",
    "created_at": "2012-05-27T05:40:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14166",
    "user": "https://github.com/novoselt"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -10,4 +10,7 @@
 
 So find_minimum_on_interval() returns a local minimum as opposed to the global one.  (The same issue applies to find_maximum_on_interval.)  This is due to the fact that the function wraps scipy.optimize.fminbound, which is only a local optimizer (Carl Witty pointed this out).  We should instead use one of the global optimizers, i.e. scipy.optimize.anneal or scipy.optimize.brute.
 
-Apply trac2607.patch and trac2607-whitespace.patch
+**Apply:**
+* [attachment: trac2607.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607.patch.gz)
+* [attachment: trac2607-whitespace.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607-whitespace.patch.gz)
+* [attachment: trac_2607_renaming.patch](https://github.com/sagemath/sage/files/ticket2607/trac_2607_renaming.patch.gz)
``````




---

archive/issue_events_024146.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2012-05-27T05:40:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24146"
}
```



---

archive/issue_events_024147.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2012-05-27T05:40:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24147"
}
```



---

archive/issue_comments_014167.json:
```json
{
    "body": "Changed author from **Dan Drake** to **Dan Drake, Andrey Novoseltsev**",
    "created_at": "2012-05-27T05:40:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14167",
    "user": "https://github.com/novoselt"
}
```

Changed author from **Dan Drake** to **Dan Drake, Andrey Novoseltsev**



---

archive/issue_comments_014168.json:
```json
{
    "body": "Changed work issues from **deprecation** to none",
    "created_at": "2012-05-27T05:40:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14168",
    "user": "https://github.com/novoselt"
}
```

Changed work issues from **deprecation** to none



---

archive/issue_comments_014169.json:
```json
{
    "body": "<div id=\"comment:38\" align=\"right\">Comment 38</div>\n\nApply trac2607.patch trac2607-whitespace.patch trac_2607_renaming.patch",
    "created_at": "2012-05-27T05:40:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14169",
    "user": "https://github.com/novoselt"
}
```

<div id="comment:38" align="right">Comment 38</div>

Apply trac2607.patch trac2607-whitespace.patch trac_2607_renaming.patch



---

archive/issue_comments_014170.json:
```json
{
    "body": "<div id=\"comment:39\" align=\"right\">Comment 39</div>\n\nAccording to the patchbot, these patches introduce some trailing whitespace :-)  Otherwise, it looks good to me.",
    "created_at": "2012-05-28T19:41:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14170",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:39" align="right">Comment 39</div>

According to the patchbot, these patches introduce some trailing whitespace :-)  Otherwise, it looks good to me.



---

archive/issue_comments_014171.json:
```json
{
    "body": "<div id=\"comment:40\" align=\"right\">Comment 40</div>\n\nGiven how many whitespaces were removed, I think these are fine to go...",
    "created_at": "2012-05-28T19:44:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14171",
    "user": "https://github.com/novoselt"
}
```

<div id="comment:40" align="right">Comment 40</div>

Given how many whitespaces were removed, I think these are fine to go...



---

archive/issue_comments_014172.json:
```json
{
    "body": "<div id=\"comment:41\" align=\"right\">Comment 41</div>\n\nOkay, sounds good to me.  Everything looks good.",
    "created_at": "2012-05-28T21:10:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14172",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:41" align="right">Comment 41</div>

Okay, sounds good to me.  Everything looks good.



---

archive/issue_events_024148.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2012-05-28T21:10:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24148"
}
```



---

archive/issue_events_024149.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2012-05-28T21:10:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24149"
}
```



---

archive/issue_comments_014173.json:
```json
{
    "body": "Changed reviewer from **Karl-Dieter Crisman** to **Karl-Dieter Crisman, Mike Hansen**",
    "created_at": "2012-05-28T21:10:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14173",
    "user": "https://github.com/mwhansen"
}
```

Changed reviewer from **Karl-Dieter Crisman** to **Karl-Dieter Crisman, Mike Hansen**



---

archive/issue_events_024150.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-01T09:38:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24150"
}
```



---

archive/issue_events_024151.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-01T09:38:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24151"
}
```



---

archive/issue_comments_014174.json:
```json
{
    "body": "<div id=\"comment:42\" align=\"right\">Comment 42</div>\n\nWhy [attachment: trac2607-whitespace.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607-whitespace.patch.gz)?  These kind of patches are very annoying for rebasing.  Removing whitespace *from code which you change* is fine (and even encouraged!), simply removing whitespace all over the place is bad.",
    "created_at": "2012-06-01T09:38:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14174",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:42" align="right">Comment 42</div>

Why [attachment: trac2607-whitespace.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607-whitespace.patch.gz)?  These kind of patches are very annoying for rebasing.  Removing whitespace *from code which you change* is fine (and even encouraged!), simply removing whitespace all over the place is bad.



---

archive/issue_comments_014175.json:
```json
{
    "body": "<div id=\"comment:43\" align=\"right\">Comment 43</div>\n\nHi Jeroen, this patch removes some surprising TABs and I already rebased another patch on top of this whitespace removal, so maybe we can leave it... (Although in general I agree that meddling with whitespaces only complicates life and perhaps patchbot plugin was not such a great idea.)",
    "created_at": "2012-06-01T21:24:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14175",
    "user": "https://github.com/novoselt"
}
```

<div id="comment:43" align="right">Comment 43</div>

Hi Jeroen, this patch removes some surprising TABs and I already rebased another patch on top of this whitespace removal, so maybe we can leave it... (Although in general I agree that meddling with whitespaces only complicates life and perhaps patchbot plugin was not such a great idea.)



---

archive/issue_comments_014176.json:
```json
{
    "body": "<div id=\"comment:44\" align=\"right\">Comment 44</div>\n\nWhat's wrong with the patchbot plugin? It only checks whether you added new trailing whitespace. I'm pretty sure everyone agrees you should at least try to avoid doing that.",
    "created_at": "2012-06-01T22:22:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14176",
    "user": "https://github.com/kini"
}
```

<div id="comment:44" align="right">Comment 44</div>

What's wrong with the patchbot plugin? It only checks whether you added new trailing whitespace. I'm pretty sure everyone agrees you should at least try to avoid doing that.



---

archive/issue_comments_014177.json:
```json
{
    "body": "<div id=\"comment:45\" align=\"right\">Comment 45</div>\n\nReplying to [@kini](#comment%3A44):\n> What's wrong with the patchbot plugin? It only checks whether you added new trailing whitespace. I'm pretty sure everyone agrees you should at least try to avoid doing that.\n\nThe problem is that some people get too eager to remove whitespace or insist on patchbot not showing any whitespace mistakes while others don't want to work on new patches if whitespaces are the only issue, and I cannot blame them. I am pretty sure that nobody adds whitespaces on purpose, but having some is not such a problem. Perhaps limiting the overall line length would be better. By the way - will the switch to git help somehow with whitespaces and patches that have conflicts because of them only?",
    "created_at": "2012-06-01T22:37:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14177",
    "user": "https://github.com/novoselt"
}
```

<div id="comment:45" align="right">Comment 45</div>

Replying to [@kini](#comment%3A44):
> What's wrong with the patchbot plugin? It only checks whether you added new trailing whitespace. I'm pretty sure everyone agrees you should at least try to avoid doing that.

The problem is that some people get too eager to remove whitespace or insist on patchbot not showing any whitespace mistakes while others don't want to work on new patches if whitespaces are the only issue, and I cannot blame them. I am pretty sure that nobody adds whitespaces on purpose, but having some is not such a problem. Perhaps limiting the overall line length would be better. By the way - will the switch to git help somehow with whitespaces and patches that have conflicts because of them only?



---

archive/issue_comments_014178.json:
```json
{
    "body": "Dependencies: **#12950**",
    "created_at": "2012-06-01T22:51:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14178",
    "user": "https://github.com/jdemeyer"
}
```

Dependencies: **#12950**



---

archive/issue_comments_014179.json:
```json
{
    "body": "<div id=\"comment:46\" align=\"right\">Comment 46</div>\n\nIn any case, this should be rebased to #12950.",
    "created_at": "2012-06-01T22:51:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14179",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:46" align="right">Comment 46</div>

In any case, this should be rebased to #12950.



---

archive/issue_events_024152.json:
```json
{
    "actor": "https://github.com/dandrake",
    "created_at": "2012-06-07T04:55:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24152"
}
```



---

archive/issue_events_024153.json:
```json
{
    "actor": "https://github.com/dandrake",
    "created_at": "2012-06-07T04:55:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24153"
}
```



---

archive/issue_comments_014180.json:
```json
{
    "body": "<div id=\"comment:47\" align=\"right\">Comment 47</div>\n\nThe new whitespace patch now only affects the relevant functions and removes a couple tabs from `expression.pyx`. It's far less invasive. All three relevant patches apply to 5.1.beta2.",
    "created_at": "2012-06-07T04:55:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14180",
    "user": "https://github.com/dandrake"
}
```

<div id="comment:47" align="right">Comment 47</div>

The new whitespace patch now only affects the relevant functions and removes a couple tabs from `expression.pyx`. It's far less invasive. All three relevant patches apply to 5.1.beta2.



---

archive/issue_comments_014181.json:
```json
{
    "body": "Changed dependencies from **#12950** to **sage-5.1.beta2**",
    "created_at": "2012-06-07T11:25:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14181",
    "user": "https://github.com/novoselt"
}
```

Changed dependencies from **#12950** to **sage-5.1.beta2**



---

archive/issue_events_024154.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2012-06-07T20:38:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24154"
}
```



---

archive/issue_events_024155.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2012-06-07T20:38:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24155"
}
```



---

archive/issue_comments_014182.json:
```json
{
    "body": "<div id=\"comment:50\" align=\"right\">Comment 50</div>\n\nIf in this ticket find_maximum_on_interval is renamed to find_local_maximum_on_interval, why not use the change to name it just find_local_maximum and keep it consistent with find_root? After all they both work same:\n\n```\nfind_root(f, a, b)\nfind_local_maximum(f, a, b)\n```\n\nfind_local_maximum_on_interval is 31 characters long, I think that such long names become hard to type. I know, it works on interval, but find_root also looks for root on [a,b] and isn't called find_root_on_interval.",
    "created_at": "2012-06-09T09:05:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14182",
    "user": "https://github.com/sagetrac-aginiewicz"
}
```

<div id="comment:50" align="right">Comment 50</div>

If in this ticket find_maximum_on_interval is renamed to find_local_maximum_on_interval, why not use the change to name it just find_local_maximum and keep it consistent with find_root? After all they both work same:

```
find_root(f, a, b)
find_local_maximum(f, a, b)
```

find_local_maximum_on_interval is 31 characters long, I think that such long names become hard to type. I know, it works on interval, but find_root also looks for root on [a,b] and isn't called find_root_on_interval.



---

archive/issue_comments_014183.json:
```json
{
    "body": "<div id=\"comment:51\" align=\"right\">Comment 51</div>\n\nMakes sense to me, new patch is attached. Please review!",
    "created_at": "2012-06-09T18:43:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14183",
    "user": "https://github.com/novoselt"
}
```

<div id="comment:51" align="right">Comment 51</div>

Makes sense to me, new patch is attached. Please review!



---

archive/issue_comments_014184.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -13,4 +13,4 @@\n **Apply:**\n * [attachment: trac2607.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607.patch.gz)\n * [attachment: trac2607-whitespace.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607-whitespace.patch.gz)\n-* [attachment: trac_2607_renaming.patch](https://github.com/sagemath/sage/files/ticket2607/trac_2607_renaming.patch.gz)\n+* [attachment: trac_2607_renaming.2.patch](https://github.com/sagemath/sage/files/ticket2607/trac_2607_renaming.2.patch.gz)\n``````\n",
    "created_at": "2012-06-09T18:43:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14184",
    "user": "https://github.com/novoselt"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -13,4 +13,4 @@
 **Apply:**
 * [attachment: trac2607.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607.patch.gz)
 * [attachment: trac2607-whitespace.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607-whitespace.patch.gz)
-* [attachment: trac_2607_renaming.patch](https://github.com/sagemath/sage/files/ticket2607/trac_2607_renaming.patch.gz)
+* [attachment: trac_2607_renaming.2.patch](https://github.com/sagemath/sage/files/ticket2607/trac_2607_renaming.2.patch.gz)
``````




---

archive/issue_events_024156.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2012-06-09T18:43:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24156"
}
```



---

archive/issue_events_024157.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2012-06-09T18:43:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24157"
}
```



---

archive/issue_events_024158.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2012-06-09T18:43:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24158"
}
```



---

archive/issue_events_024159.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2012-06-09T18:43:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24159"
}
```



---

archive/issue_events_024160.json:
```json
{
    "actor": "https://github.com/sagetrac-aginiewicz",
    "created_at": "2012-06-11T10:29:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24160"
}
```



---

archive/issue_events_024161.json:
```json
{
    "actor": "https://github.com/sagetrac-aginiewicz",
    "created_at": "2012-06-11T10:29:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24161"
}
```



---

archive/issue_comments_014185.json:
```json
{
    "body": "<div id=\"comment:54\" align=\"right\">Comment 54</div>\n\nI'd say it looks good now, applies cleanly, all tests passed, and new name is way easier to remember.",
    "created_at": "2012-06-11T10:29:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14185",
    "user": "https://github.com/sagetrac-aginiewicz"
}
```

<div id="comment:54" align="right">Comment 54</div>

I'd say it looks good now, applies cleanly, all tests passed, and new name is way easier to remember.



---

archive/issue_comments_014186.json:
```json
{
    "body": "<div id=\"comment:55\" align=\"right\">Comment 55</div>\n\n[attachment: trac2607-whitespace.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607-whitespace.patch.gz) needs a proper commit message.",
    "created_at": "2012-06-18T07:50:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14186",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:55" align="right">Comment 55</div>

[attachment: trac2607-whitespace.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607-whitespace.patch.gz) needs a proper commit message.



---

archive/issue_comments_014187.json:
```json
{
    "body": "<div id=\"comment:56\" align=\"right\">Comment 56</div>\n\nMore importantly, this fails on OS X 10.4 ppc with numerical noise:\n\n```\nsage -t  --long -force_lib devel/sage/sage/numerical/optimize.py\n**********************************************************************\nFile \"/Users/buildbot/build/sage/moufang-1/moufang_full/build/sage-5.1.beta5/devel/sage-main/sage/numerical/optimize.py\", line 135:\n    sage: find_local_maximum(fast_float(8*e^(-x)*sin(x) - 1, x), 0, 8)\nExpected:\n    (1.5791755355586754, 0.78539817769603...)\nGot:\n    (1.5791755355586754, 0.7853981777050254)\nGLPK Simplex Optimizer, v4.44\n6 rows, 3 columns, 8 non-zeros\nPreprocessing...\n2 rows, 2 columns, 4 non-zeros\nScaling...\n A: min|aij| =  2.400e+01  max|aij| =  5.000e+01  ratio =  2.083e+00\nGM: min|aij| =  8.128e-01  max|aij| =  1.230e+00  ratio =  1.514e+00\nEQ: min|aij| =  6.606e-01  max|aij| =  1.000e+00  ratio =  1.514e+00\nConstructing initial basis...\nSize of triangular part = 2\n*     0: obj =  -5.100000000e+01  infeas =  0.000e+00 (0)\n*     1: obj =  -5.225000000e+01  infeas =  0.000e+00 (0)\nOPTIMAL SOLUTION FOUND\n**********************************************************************\n```",
    "created_at": "2012-06-18T07:51:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14187",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:56" align="right">Comment 56</div>

More importantly, this fails on OS X 10.4 ppc with numerical noise:

```
sage -t  --long -force_lib devel/sage/sage/numerical/optimize.py
**********************************************************************
File "/Users/buildbot/build/sage/moufang-1/moufang_full/build/sage-5.1.beta5/devel/sage-main/sage/numerical/optimize.py", line 135:
    sage: find_local_maximum(fast_float(8*e^(-x)*sin(x) - 1, x), 0, 8)
Expected:
    (1.5791755355586754, 0.78539817769603...)
Got:
    (1.5791755355586754, 0.7853981777050254)
GLPK Simplex Optimizer, v4.44
6 rows, 3 columns, 8 non-zeros
Preprocessing...
2 rows, 2 columns, 4 non-zeros
Scaling...
 A: min|aij| =  2.400e+01  max|aij| =  5.000e+01  ratio =  2.083e+00
GM: min|aij| =  8.128e-01  max|aij| =  1.230e+00  ratio =  1.514e+00
EQ: min|aij| =  6.606e-01  max|aij| =  1.000e+00  ratio =  1.514e+00
Constructing initial basis...
Size of triangular part = 2
*     0: obj =  -5.100000000e+01  infeas =  0.000e+00 (0)
*     1: obj =  -5.225000000e+01  infeas =  0.000e+00 (0)
OPTIMAL SOLUTION FOUND
**********************************************************************
```



---

archive/issue_events_024162.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-18T07:58:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24162"
}
```



---

archive/issue_events_024163.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-18T07:58:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24163"
}
```



---

archive/issue_comments_014188.json:
```json
{
    "body": "patch trac2607-whitespace.patch with commit message",
    "created_at": "2012-06-24T10:07:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14188",
    "user": "https://github.com/sagetrac-aginiewicz"
}
```

patch trac2607-whitespace.patch with commit message



---

archive/issue_comments_014189.json:
```json
{
    "body": "Attachment: **[trac2607-whitespace.2.patch.gz](https://github.com/sagemath/sage/files/ticket2607/trac2607-whitespace.2.patch.gz)**\n\npatch trac_2607_renaming.2.patch rebased on top of trac2607-whitespace.2.patch",
    "created_at": "2012-06-24T10:08:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14189",
    "user": "https://github.com/sagetrac-aginiewicz"
}
```

Attachment: **[trac2607-whitespace.2.patch.gz](https://github.com/sagemath/sage/files/ticket2607/trac2607-whitespace.2.patch.gz)**

patch trac_2607_renaming.2.patch rebased on top of trac2607-whitespace.2.patch



---

archive/issue_comments_014190.json:
```json
{
    "body": "Attachment: **[trac_2607_renaming.3.patch.gz](https://github.com/sagemath/sage/files/ticket2607/trac_2607_renaming.3.patch.gz)**\n\ndoctest and spacing cleanup",
    "created_at": "2012-06-24T10:08:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14190",
    "user": "https://github.com/sagetrac-aginiewicz"
}
```

Attachment: **[trac_2607_renaming.3.patch.gz](https://github.com/sagemath/sage/files/ticket2607/trac_2607_renaming.3.patch.gz)**

doctest and spacing cleanup



---

archive/issue_events_024164.json:
```json
{
    "actor": "https://github.com/sagetrac-aginiewicz",
    "created_at": "2012-06-24T10:13:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24164"
}
```



---

archive/issue_events_024165.json:
```json
{
    "actor": "https://github.com/sagetrac-aginiewicz",
    "created_at": "2012-06-24T10:13:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24165"
}
```



---

archive/issue_comments_014191.json:
```json
{
    "body": "<div id=\"comment:58\" align=\"right\">Comment 58</div>\n\nAttachment: **[trac2607-doctest-and-spacing.patch.gz](https://github.com/sagemath/sage/files/ticket2607/trac2607-doctest-and-spacing.patch.gz)**\n\nI added the commit message to whitespace patch and rebased renaming patch based on it (keeping original authors/dates, I did not wanted to claim their work and it was minor rebase only to accommodate the commit message taken from comment by Dan).\n\nI also uploaded trac2607-doctest-and-spacing.patch, which adjusts doctest for numerical noise, fixes spacing to be more consistent (3 places, spaces between arguments - i.e. \"0,5\" looks very close to \"0.5\" instead of \"0, 5\") and changed \"f.find_local_minimum(\" to \"find_local_minimum(f\" - because after all this test occurs in definition of find_local_minimum function, not method.",
    "created_at": "2012-06-24T10:13:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14191",
    "user": "https://github.com/sagetrac-aginiewicz"
}
```

<div id="comment:58" align="right">Comment 58</div>

Attachment: **[trac2607-doctest-and-spacing.patch.gz](https://github.com/sagemath/sage/files/ticket2607/trac2607-doctest-and-spacing.patch.gz)**

I added the commit message to whitespace patch and rebased renaming patch based on it (keeping original authors/dates, I did not wanted to claim their work and it was minor rebase only to accommodate the commit message taken from comment by Dan).

I also uploaded trac2607-doctest-and-spacing.patch, which adjusts doctest for numerical noise, fixes spacing to be more consistent (3 places, spaces between arguments - i.e. "0,5" looks very close to "0.5" instead of "0, 5") and changed "f.find_local_minimum(" to "find_local_minimum(f" - because after all this test occurs in definition of find_local_minimum function, not method.



---

archive/issue_comments_014192.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -12,5 +12,6 @@\n \n **Apply:**\n * [attachment: trac2607.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607.patch.gz)\n-* [attachment: trac2607-whitespace.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607-whitespace.patch.gz)\n-* [attachment: trac_2607_renaming.2.patch](https://github.com/sagemath/sage/files/ticket2607/trac_2607_renaming.2.patch.gz)\n+* [attachment: trac2607-whitespace.2.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607-whitespace.2.patch.gz)\n+* [attachment: trac_2607_renaming.3.patch](https://github.com/sagemath/sage/files/ticket2607/trac_2607_renaming.3.patch.gz)\n+* [attachment: trac2607-doctest-and-spacing.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607-doctest-and-spacing.patch.gz)\n``````\n",
    "created_at": "2012-06-24T10:13:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14192",
    "user": "https://github.com/sagetrac-aginiewicz"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -12,5 +12,6 @@
 
 **Apply:**
 * [attachment: trac2607.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607.patch.gz)
-* [attachment: trac2607-whitespace.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607-whitespace.patch.gz)
-* [attachment: trac_2607_renaming.2.patch](https://github.com/sagemath/sage/files/ticket2607/trac_2607_renaming.2.patch.gz)
+* [attachment: trac2607-whitespace.2.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607-whitespace.2.patch.gz)
+* [attachment: trac_2607_renaming.3.patch](https://github.com/sagemath/sage/files/ticket2607/trac_2607_renaming.3.patch.gz)
+* [attachment: trac2607-doctest-and-spacing.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607-doctest-and-spacing.patch.gz)
``````




---

archive/issue_comments_014193.json:
```json
{
    "body": "Changed reviewer from **Karl-Dieter Crisman, Mike Hansen** to **Karl-Dieter Crisman, Mike Hansen, Andrey Novoseltsev**",
    "created_at": "2012-06-25T12:41:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14193",
    "user": "https://github.com/novoselt"
}
```

Changed reviewer from **Karl-Dieter Crisman, Mike Hansen** to **Karl-Dieter Crisman, Mike Hansen, Andrey Novoseltsev**



---

archive/issue_events_024166.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2012-06-25T12:41:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24166"
}
```



---

archive/issue_events_024167.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2012-06-25T12:41:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24167"
}
```



---

archive/issue_comments_014194.json:
```json
{
    "body": "Changed author from **Dan Drake, Andrey Novoseltsev** to **Dan Drake, Andrey Novoseltsev, Andrzej Giniewicz**",
    "created_at": "2012-06-25T12:41:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14194",
    "user": "https://github.com/novoselt"
}
```

Changed author from **Dan Drake, Andrey Novoseltsev** to **Dan Drake, Andrey Novoseltsev, Andrzej Giniewicz**



---

archive/issue_comments_014195.json:
```json
{
    "body": "<div id=\"comment:59\" align=\"right\">Comment 59</div>\n\nLooks good and all tests pass.",
    "created_at": "2012-06-25T12:41:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14195",
    "user": "https://github.com/novoselt"
}
```

<div id="comment:59" align="right">Comment 59</div>

Looks good and all tests pass.



---

archive/issue_events_024168.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-25T15:34:35Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "milestone_number": null,
    "milestone_title": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24168"
}
```



---

archive/issue_events_024169.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-25T15:34:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "milestone_number": null,
    "milestone_title": "sage-5.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24169"
}
```



---

archive/issue_comments_014196.json:
```json
{
    "body": "Changed dependencies from **sage-5.1.beta2** to **#13109**",
    "created_at": "2012-06-30T18:26:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14196",
    "user": "https://github.com/vbraun"
}
```

Changed dependencies from **sage-5.1.beta2** to **#13109**



---

archive/issue_comments_014197.json:
```json
{
    "body": "<div id=\"comment:61\" align=\"right\">Comment 61</div>\n\nAdded patch to switch the deprecation to the new syntax.",
    "created_at": "2012-06-30T18:26:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14197",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:61" align="right">Comment 61</div>

Added patch to switch the deprecation to the new syntax.



---

archive/issue_comments_014198.json:
```json
{
    "body": "Updated patch",
    "created_at": "2012-06-30T18:28:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14198",
    "user": "https://github.com/vbraun"
}
```

Updated patch



---

archive/issue_comments_014199.json:
```json
{
    "body": "<div id=\"comment:62\" align=\"right\">Comment 62</div>\n\nAttachment: **[trac_2607_update_deprecation.patch.gz](https://github.com/sagemath/sage/files/ticket2607/trac_2607_update_deprecation.patch.gz)**",
    "created_at": "2012-06-30T18:31:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14199",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:62" align="right">Comment 62</div>

Attachment: **[trac_2607_update_deprecation.patch.gz](https://github.com/sagemath/sage/files/ticket2607/trac_2607_update_deprecation.patch.gz)**



---

archive/issue_events_024170.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-06-30T18:31:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24170"
}
```



---

archive/issue_events_024171.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-06-30T18:31:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24171"
}
```



---

archive/issue_comments_014200.json:
```json
{
    "body": "Changed author from **Dan Drake, Andrey Novoseltsev, Andrzej Giniewicz** to **Dan Drake, Andrey Novoseltsev, Andrzej Giniewicz, Volker Braun**",
    "created_at": "2012-06-30T18:31:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14200",
    "user": "https://github.com/vbraun"
}
```

Changed author from **Dan Drake, Andrey Novoseltsev, Andrzej Giniewicz** to **Dan Drake, Andrey Novoseltsev, Andrzej Giniewicz, Volker Braun**



---

archive/issue_comments_014201.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -15,3 +15,4 @@\n * [attachment: trac2607-whitespace.2.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607-whitespace.2.patch.gz)\n * [attachment: trac_2607_renaming.3.patch](https://github.com/sagemath/sage/files/ticket2607/trac_2607_renaming.3.patch.gz)\n * [attachment: trac2607-doctest-and-spacing.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607-doctest-and-spacing.patch.gz)\n+* [attachment: trac_2607_update_deprecation.patch](https://github.com/sagemath/sage/files/ticket2607/trac_2607_update_deprecation.patch.gz)\n``````\n",
    "created_at": "2012-06-30T18:31:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14201",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -15,3 +15,4 @@
 * [attachment: trac2607-whitespace.2.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607-whitespace.2.patch.gz)
 * [attachment: trac_2607_renaming.3.patch](https://github.com/sagemath/sage/files/ticket2607/trac_2607_renaming.3.patch.gz)
 * [attachment: trac2607-doctest-and-spacing.patch](https://github.com/sagemath/sage/files/ticket2607/trac2607-doctest-and-spacing.patch.gz)
+* [attachment: trac_2607_update_deprecation.patch](https://github.com/sagemath/sage/files/ticket2607/trac_2607_update_deprecation.patch.gz)
``````




---

archive/issue_events_024172.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-06-30T18:31:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24172"
}
```



---

archive/issue_events_024173.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-06-30T18:31:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24173"
}
```



---

archive/issue_comments_014202.json:
```json
{
    "body": "<div id=\"comment:64\" align=\"right\">Comment 64</div>\n\nDeprecation changes look good to me.",
    "created_at": "2012-06-30T19:21:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14202",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:64" align="right">Comment 64</div>

Deprecation changes look good to me.



---

archive/issue_events_024174.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-06-30T19:21:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24174"
}
```



---

archive/issue_events_024175.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-06-30T19:21:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24175"
}
```



---

archive/issue_comments_014203.json:
```json
{
    "body": "<div id=\"comment:65\" align=\"right\">Comment 65</div>\n\n(and all tests still pass)",
    "created_at": "2012-06-30T19:43:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14203",
    "user": "https://github.com/sagetrac-aginiewicz"
}
```

<div id="comment:65" align="right">Comment 65</div>

(and all tests still pass)



---

archive/issue_events_024176.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-01T18:51:32Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "milestone_number": null,
    "milestone_title": "sage-5.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24176"
}
```



---

archive/issue_events_024177.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-01T18:51:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/pending",
    "label_color": "008080",
    "label_name": "pending",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24177"
}
```



---

archive/issue_events_024178.json:
```json
{
    "actor": "https://github.com/sagetrac-aginiewicz",
    "created_at": "2012-07-08T12:19:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "milestone_number": null,
    "milestone_title": "sage-5.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24178"
}
```



---

archive/issue_events_024179.json:
```json
{
    "actor": "https://github.com/sagetrac-aginiewicz",
    "created_at": "2012-07-08T12:19:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/pending",
    "label_color": "008080",
    "label_name": "pending",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24179"
}
```



---

archive/issue_comments_014204.json:
```json
{
    "body": "<div id=\"comment:67\" align=\"right\">Comment 67</div>\n\nAs I understand that ticket moved to sage-pending together with #13109 because it depends on it. But #13109 is back in 5.2 with positive review, so let's move this one back too.",
    "created_at": "2012-07-08T12:19:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14204",
    "user": "https://github.com/sagetrac-aginiewicz"
}
```

<div id="comment:67" align="right">Comment 67</div>

As I understand that ticket moved to sage-pending together with #13109 because it depends on it. But #13109 is back in 5.2 with positive review, so let's move this one back too.



---

archive/issue_events_024180.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-16T16:41:54Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "milestone_number": null,
    "milestone_title": "sage-5.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24180"
}
```



---

archive/issue_events_024181.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-16T16:41:54Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "milestone_number": null,
    "milestone_title": "sage-5.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24181"
}
```



---

archive/issue_events_024182.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-08-01T12:08:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24182"
}
```



---

archive/issue_events_024183.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-08-01T12:08:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2607#event-24183"
}
```



---

archive/issue_comments_014205.json:
```json
{
    "body": "Merged: **sage-5.3.beta0**",
    "created_at": "2012-08-01T12:08:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2607",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2607#issuecomment-14205",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.3.beta0**
