# Issue 2886: change an error message

archive/issues_002886.json:
```json
{
    "assignees": [
        "https://github.com/mwhansen"
    ],
    "body": "When you crash Sage you get this error message:\n\n```\n**********************************************************************\n\nOops, SAGE crashed. We do our best to make it stable, but...\n\nA crash report was automatically generated with the following information:\n  - A verbatim copy of the crash traceback.\n  - A copy of your input history during this session.\n  - Data on your current SAGE configuration.\n\nIt was left in the file named:\n'/Users/was/.sage/ipython/SAGE_crash_report.txt'\nIf you can email this file to the developers, the information in it will help\nthem in understanding and correcting the problem.\n\nYou can mail it to: William Stein at wstein@gmail.com\nwith the subject 'SAGE Crash Report'.\n\nIf you want to do it now, the following command will work (under Unix):\nmail -s 'SAGE Crash Report' wstein@gmail.com < /Users/was/.sage/ipython/SAGE_crash_report.txt\n\nTo ensure accurate tracking of this issue, please file a report about it at:\nhttp://www.sagemath.org:9002/sage_trac\n\n```\n\nIssues: \n  (1) People need accounts to file a report.  It might be much better to tell people to report to sage-support.\n\n  (2) the url for trac is now trac.sagemath.org/sage_trac\n\n--\n\nI think doing (2) would be fine to fix this ticket.\n\nWilliam\n\nComponent: **user interface**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/2886_\n\n",
    "closed_at": "2008-04-15T20:01:17Z",
    "created_at": "2008-04-12T00:09:35Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "change an error message",
    "type": "issue",
    "updated_at": "2015-09-06T15:50:33Z",
    "url": "https://github.com/sagemath/sage/issues/2886",
    "user": "https://github.com/williamstein"
}
```
When you crash Sage you get this error message:

```
**********************************************************************

Oops, SAGE crashed. We do our best to make it stable, but...

A crash report was automatically generated with the following information:
  - A verbatim copy of the crash traceback.
  - A copy of your input history during this session.
  - Data on your current SAGE configuration.

It was left in the file named:
'/Users/was/.sage/ipython/SAGE_crash_report.txt'
If you can email this file to the developers, the information in it will help
them in understanding and correcting the problem.

You can mail it to: William Stein at wstein@gmail.com
with the subject 'SAGE Crash Report'.

If you want to do it now, the following command will work (under Unix):
mail -s 'SAGE Crash Report' wstein@gmail.com < /Users/was/.sage/ipython/SAGE_crash_report.txt

To ensure accurate tracking of this issue, please file a report about it at:
http://www.sagemath.org:9002/sage_trac

```

Issues: 
  (1) People need accounts to file a report.  It might be much better to tell people to report to sage-support.

  (2) the url for trac is now trac.sagemath.org/sage_trac

--

I think doing (2) would be fine to fix this ticket.

William

Component: **user interface**

_Issue created by migration from https://trac.sagemath.org/ticket/2886_





---

archive/issue_events_030610.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-04-12T00:09:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2886",
    "milestone_number": null,
    "milestone_title": "sage-3.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2886#event-30610"
}
```



---

archive/issue_events_030611.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-04-12T00:09:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2886",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "000080",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2886#event-30611"
}
```



---

archive/issue_events_030612.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-04-12T00:09:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2886",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2886#event-30612"
}
```



---

archive/issue_events_030613.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-04-12T00:09:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2886",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2886#event-30613"
}
```



---

archive/issue_events_030614.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-04-12T00:09:35Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/2886",
    "subject": "https://github.com/williamstein",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2886#event-30614"
}
```



---

archive/issue_comments_015074.json:
```json
{
    "body": "Attachment: **[2886.patch.gz](https://github.com/sagemath/sage/files/ticket2886/2886.patch.gz)**",
    "created_at": "2008-04-12T07:45:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2886",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2886#issuecomment-15074",
    "user": "https://github.com/mwhansen"
}
```

Attachment: **[2886.patch.gz](https://github.com/sagemath/sage/files/ticket2886/2886.patch.gz)**



---

archive/issue_events_030615.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-04-12T07:45:54Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/2886",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2886#event-30615"
}
```



---

archive/issue_events_030616.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-04-12T07:45:54Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/2886",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2886#event-30616"
}
```



---

archive/issue_events_030617.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-04-12T07:45:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2886",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2886#event-30617"
}
```



---

archive/issue_comments_015075.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nThe patch applies to hg_scripts.",
    "created_at": "2008-04-12T07:45:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2886",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2886#issuecomment-15075",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:1" align="right">comment:1</div>

The patch applies to hg_scripts.



---

archive/issue_events_030618.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2008-04-15T16:36:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2886",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2886#event-30618"
}
```



---

archive/issue_events_030619.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2008-04-15T16:36:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2886",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2886#event-30619"
}
```



---

archive/issue_comments_015076.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nFine by me.",
    "created_at": "2008-04-15T16:36:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2886",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2886#issuecomment-15076",
    "user": "https://github.com/ncalexan"
}
```

<div id="comment:2" align="right">comment:2</div>

Fine by me.



---

archive/issue_comments_015077.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged in Sage 3.0.alpha6",
    "created_at": "2008-04-15T20:01:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2886",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2886#issuecomment-15077",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged in Sage 3.0.alpha6



---

archive/issue_events_030620.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-15T20:01:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2886",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2886#event-30620"
}
```



---

archive/issue_events_030621.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-15T20:01:17Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2886",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2886#event-30621"
}
```



---

archive/issue_comments_015078.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -11,7 +11,7 @@\n   - Data on your current SAGE configuration.\n \n It was left in the file named:\n-\t'/Users/was/.sage/ipython/SAGE_crash_report.txt'\n+'/Users/was/.sage/ipython/SAGE_crash_report.txt'\n If you can email this file to the developers, the information in it will help\n them in understanding and correcting the problem.\n \n``````\n",
    "created_at": "2015-09-06T15:50:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2886",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2886#issuecomment-15078",
    "user": "https://github.com/fchapoton"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -11,7 +11,7 @@
   - Data on your current SAGE configuration.
 
 It was left in the file named:
-	'/Users/was/.sage/ipython/SAGE_crash_report.txt'
+'/Users/was/.sage/ipython/SAGE_crash_report.txt'
 If you can email this file to the developers, the information in it will help
 them in understanding and correcting the problem.
 
``````

