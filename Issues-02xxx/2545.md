# Issue 2545: FractionFieldElement lacks derivative method

archive/issues_002545.json:
```json
{
    "body": "Attached patch adds a `derivative` method to `FractionFieldElement`s, and fixes a bug in the `_derivative` method of `Polynomial_rational_dense`.\n\nSo these now work:\n\n```\nsage: R = ZZ['x']\nsage: S = R.fraction_field(); x = S.gen()\nsage: R(1).derivative(R(x))\n0\n\nsage: F = FractionField(PolynomialRing(RationalField(),'x,y'))\nsage: x,y = F.gens()\nsage: (1/(x+y)).derivative(x,y)\n2/(x^3 + 3*x^2*y + 3*x*y^2 + y^3)\n```\n\n\n\n\n**Assignee:** @burcin\n\n**Resolution:** fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/2545\n\n",
    "closed_at": "2008-03-18T00:06:36Z",
    "created_at": "2008-03-16T12:35:22Z",
    "labels": [
        "component: basic arithmetic"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.11",
    "title": "FractionFieldElement lacks derivative method",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2545",
    "user": "https://github.com/burcin"
}
```
Attached patch adds a `derivative` method to `FractionFieldElement`s, and fixes a bug in the `_derivative` method of `Polynomial_rational_dense`.

So these now work:

```
sage: R = ZZ['x']
sage: S = R.fraction_field(); x = S.gen()
sage: R(1).derivative(R(x))
0

sage: F = FractionField(PolynomialRing(RationalField(),'x,y'))
sage: x,y = F.gens()
sage: (1/(x+y)).derivative(x,y)
2/(x^3 + 3*x^2*y + 3*x*y^2 + y^3)
```




**Assignee:** @burcin

**Resolution:** fixed

Issue created by migration from https://trac.sagemath.org/ticket/2545





---

archive/issue_comments_019335.json:
```json
{
    "body": "derivative method for FractionFieldElement",
    "created_at": "2008-03-16T12:35:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2545",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2545#issuecomment-19335",
    "user": "https://github.com/burcin"
}
```

derivative method for FractionFieldElement



---

archive/attachments_002332.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "sage_fraction_field_derivative.patch",
    "asset_url": "tarball://root/attachments/some-uuid/ticket2545/sage_fraction_field_derivative.patch",
    "created_at": "2008-03-16T14:34:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2545",
    "type": "attachment",
    "url": "https://github.com/assets/some-id/some-uuid.patch",
    "user": "https://github.com/aghitza"
}
```



---

archive/issue_comments_019336.json:
```json
{
    "body": "<a id='comment:1'></a>\nThis appears to work as advertised.  I have one request: the docstrings for derivative() and _derivative() refer to \"the derivative of this polynomial\", which is bad since these elements are (most of the time) not polynomials.  This should be replaced with \"rational function\" or something similar.",
    "created_at": "2008-03-16T14:34:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2545",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2545#issuecomment-19336",
    "user": "https://github.com/aghitza"
}
```

<a id='comment:1'></a>
This appears to work as advertised.  I have one request: the docstrings for derivative() and _derivative() refer to "the derivative of this polynomial", which is bad since these elements are (most of the time) not polynomials.  This should be replaced with "rational function" or something similar.



---

archive/issue_events_007574.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2545",
    "rename": {
        "from": "FractionFieldElement lacks derivative method",
        "to": "[positive review pending minor change] FractionFieldElement lacks derivative method"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2545#event-7574"
}
```



---

archive/issue_comments_019337.json:
```json
{
    "body": "new patch with requested doc changes",
    "created_at": "2008-03-16T14:48:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2545",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2545#issuecomment-19337",
    "user": "https://github.com/burcin"
}
```

new patch with requested doc changes



---

archive/attachments_002333.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "2545-sage_fraction_field_derivative-1.patch",
    "asset_url": "tarball://root/attachments/some-uuid/ticket2545/2545-sage_fraction_field_derivative-1.patch",
    "created_at": "2008-03-16T14:51:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2545",
    "type": "attachment",
    "url": "https://github.com/assets/some-id/some-uuid.patch",
    "user": "https://github.com/burcin"
}
```



---

archive/issue_comments_019338.json:
```json
{
    "body": "<a id='comment:2'></a>\nYou're right, I copied the text from the docstring for polynomials, and forgot to change it. :)\n\nattachment:2545-sage_fraction_field_derivative-1.patch replaces \"polynomial\" with \"rational function\" as suggested.",
    "created_at": "2008-03-16T14:51:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2545",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2545#issuecomment-19338",
    "user": "https://github.com/burcin"
}
```

<a id='comment:2'></a>
You're right, I copied the text from the docstring for polynomials, and forgot to change it. :)

attachment:2545-sage_fraction_field_derivative-1.patch replaces "polynomial" with "rational function" as suggested.



---

archive/issue_comments_019339.json:
```json
{
    "body": "<a id='comment:3'></a>\nCool.  I'm satisfied.",
    "created_at": "2008-03-16T15:14:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2545",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2545#issuecomment-19339",
    "user": "https://github.com/aghitza"
}
```

<a id='comment:3'></a>
Cool.  I'm satisfied.



---

archive/issue_events_007575.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2545",
    "rename": {
        "from": "[positive review pending minor change] FractionFieldElement lacks derivative method",
        "to": "FractionFieldElement lacks derivative method"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2545#event-7575"
}
```



---

archive/issue_comments_019340.json:
```json
{
    "body": "**Changing status** from new to positive_review.",
    "created_at": "2008-03-16T15:14:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2545",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2545#issuecomment-19340",
    "user": "https://github.com/aghitza"
}
```

**Changing status** from new to positive_review.



---

archive/issue_comments_019341.json:
```json
{
    "body": "**Resolution:** fixed",
    "created_at": "2008-03-18T00:06:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2545",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2545#issuecomment-19341",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

**Resolution:** fixed



---

archive/issue_comments_019342.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 2.11.alpha0",
    "created_at": "2008-03-18T00:06:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2545",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2545#issuecomment-19342",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 2.11.alpha0



---

archive/issue_events_007576.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-03-18T00:06:36Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2545",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2545#event-7576"
}
```
