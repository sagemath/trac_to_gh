# Issue 2067: tutorial 5.3 misformatting

archive/issues_002067.json:
```json
{
    "assignees": [],
    "body": "In Section 5.3 of the tutorial, the following string is misformatted:\n\n\"bash ./factor20062*17*59bash \"\n\n**Assignee:** mvngu\n\nIssue created by migration from https://trac.sagemath.org/ticket/2067\n\n",
    "closed_at": "2008-09-19T22:10:52Z",
    "created_at": "2008-02-05T22:39:43Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/trivial",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.1.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "tutorial 5.3 misformatting",
    "type": "issue",
    "updated_at": "2008-09-19T22:10:52Z",
    "url": "https://github.com/sagemath/sage/issues/2067",
    "user": "https://github.com/sagetrac-ljpk"
}
```
In Section 5.3 of the tutorial, the following string is misformatted:

"bash ./factor20062*17*59bash "

**Assignee:** mvngu

Issue created by migration from https://trac.sagemath.org/ticket/2067





---

archive/issue_events_016564.json:
```json
{
    "actor": "https://github.com/sagetrac-ljpk",
    "created_at": "2008-02-05T22:39:43Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2067",
    "milestone_number": null,
    "milestone_title": "sage-3.1.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2067#event-16564"
}
```



---

archive/issue_events_016565.json:
```json
{
    "actor": "https://github.com/sagetrac-ljpk",
    "created_at": "2008-02-05T22:39:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2067",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "08517b",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2067#event-16565"
}
```



---

archive/issue_events_016566.json:
```json
{
    "actor": "https://github.com/sagetrac-ljpk",
    "created_at": "2008-02-05T22:39:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2067",
    "label": "https://github.com/sagemath/sage/labels/trivial",
    "label_color": "ff0000",
    "label_name": "trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2067#event-16566"
}
```



---

archive/issue_events_016567.json:
```json
{
    "actor": "https://github.com/sagetrac-ljpk",
    "created_at": "2008-02-05T22:39:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2067",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "008080",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2067#event-16567"
}
```



---

archive/issue_events_016568.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-02-06T16:56:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2067",
    "milestone_number": null,
    "milestone_title": "sage-2.10.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2067#event-16568"
}
```



---

archive/issue_comments_010403.json:
```json
{
    "body": "<a id='comment:2'></a>\nMmh, is this still a problem? I am not even sure which section to check.\n\nCheers,\n\nMichael",
    "created_at": "2008-09-14T13:36:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2067",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2067#issuecomment-10403",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:2'></a>
Mmh, is this still a problem? I am not even sure which section to check.

Cheers,

Michael



---

archive/issue_comments_010404.json:
```json
{
    "body": "Replying to [ticket:2067 ljpk]:\n> In Section 5.3 of the tutorial, the following string is misformatted:\n> \n> \"bash ./factor20062*17*59bash \"\n\n\nBy \"tutorial\", I assume this refers to the official tutorial on Sage. Then \"Section 5.3\" of that tutorial can be found online [here](http://www.sagemath.org/doc/tut/node55.html), which shows this\n\n```\nbash $ ./factor 2006\n2 * 17 * 59\n```\nThe same snippet can also be found within the [PDF version](http://www.sagemath.org/doc-pdf/tut.pdf) of the official tutorial, on page 76. In both the online and PDF versions, I don't see any misformatting at all.\n\nRegards,\n\nMinh Van Nguyen",
    "created_at": "2008-09-19T20:41:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2067",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2067#issuecomment-10404",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Replying to [ticket:2067 ljpk]:
> In Section 5.3 of the tutorial, the following string is misformatted:
> 
> "bash ./factor20062*17*59bash "


By "tutorial", I assume this refers to the official tutorial on Sage. Then "Section 5.3" of that tutorial can be found online [here](http://www.sagemath.org/doc/tut/node55.html), which shows this

```
bash $ ./factor 2006
2 * 17 * 59
```
The same snippet can also be found within the [PDF version](http://www.sagemath.org/doc-pdf/tut.pdf) of the official tutorial, on page 76. In both the online and PDF versions, I don't see any misformatting at all.

Regards,

Minh Van Nguyen



---

archive/issue_comments_010405.json:
```json
{
    "body": "**Assignee:** mvngu",
    "created_at": "2008-09-19T21:08:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2067",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2067#issuecomment-10405",
    "user": "https://github.com/sagetrac-mvngu"
}
```

**Assignee:** mvngu



---

archive/issue_comments_010406.json:
```json
{
    "body": "<a id='comment:5'></a>\nYeah, looks fixed to me. Closed. Thanks for tracking this down.\n\nCheers,\n\nMichael",
    "created_at": "2008-09-19T22:10:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2067",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2067#issuecomment-10406",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:5'></a>
Yeah, looks fixed to me. Closed. Thanks for tracking this down.

Cheers,

Michael



---

archive/issue_events_016569.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-09-19T22:10:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2067",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2067#event-16569"
}
```
