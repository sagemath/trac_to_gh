# Issue 2067: tutorial 5.3 misformatting

archive/issues_002067.json:
```json
{
    "body": "In Section 5.3 of the tutorial, the following string is misformatted:\n\n\"bash ./factor20062*17*59bash \"\n\nAssignee: mvngu\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/2067\n\n",
    "closed_at": "2008-09-19T22:10:52Z",
    "created_at": "2008-02-05T22:39:43Z",
    "labels": [
        "component: documentation",
        "trivial",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.1.3",
    "title": "tutorial 5.3 misformatting",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2067",
    "user": "https://trac.sagemath.org/admin/accounts/users/ljpk"
}
```
In Section 5.3 of the tutorial, the following string is misformatted:

"bash ./factor20062*17*59bash "

Assignee: mvngu

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/2067





---

archive/issue_events_004953.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-02-06T16:56:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/2067",
    "milestone": "sage-2.10.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2067#event-4953"
}
```



---

archive/issue_comments_013350.json:
```json
{
    "body": "<a id='comment:2'></a>Mmh, is this still a problem? I am not even sure which section to check.\n\nCheers,\n\nMichael",
    "created_at": "2008-09-14T13:36:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2067",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2067#issuecomment-13350",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>Mmh, is this still a problem? I am not even sure which section to check.

Cheers,

Michael



---

archive/issue_comments_013351.json:
```json
{
    "body": "Replying to [ticket:2067 ljpk]:\n> In Section 5.3 of the tutorial, the following string is misformatted:\n> \n> \"bash ./factor20062*17*59bash \"\n\n\n\nBy \"tutorial\", I assume this refers to the official tutorial on Sage. Then \"Section 5.3\" of that tutorial can be found online [here](http://www.sagemath.org/doc/tut/node55.html), which shows this\n\n```\nbash $ ./factor 2006\n2 * 17 * 59\n```\nThe same snippet can also be found within the [PDF version](http://www.sagemath.org/doc-pdf/tut.pdf) of the official tutorial, on page 76. In both the online and PDF versions, I don't see any misformatting at all.\n\nRegards,\n\nMinh Van Nguyen",
    "created_at": "2008-09-19T20:41:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2067",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2067#issuecomment-13351",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Replying to [ticket:2067 ljpk]:
> In Section 5.3 of the tutorial, the following string is misformatted:
> 
> "bash ./factor20062*17*59bash "



By "tutorial", I assume this refers to the official tutorial on Sage. Then "Section 5.3" of that tutorial can be found online [here](http://www.sagemath.org/doc/tut/node55.html), which shows this

```
bash $ ./factor 2006
2 * 17 * 59
```
The same snippet can also be found within the [PDF version](http://www.sagemath.org/doc-pdf/tut.pdf) of the official tutorial, on page 76. In both the online and PDF versions, I don't see any misformatting at all.

Regards,

Minh Van Nguyen



---

archive/issue_comments_013352.json:
```json
{
    "body": "Changing assignee from tba to mvngu.",
    "created_at": "2008-09-19T21:08:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2067",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2067#issuecomment-13352",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Changing assignee from tba to mvngu.



---

archive/issue_comments_013353.json:
```json
{
    "body": "<a id='comment:5'></a>Yeah, looks fixed to me. Closed. Thanks for tracking this down.\n\nCheers,\n\nMichael",
    "created_at": "2008-09-19T22:10:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2067",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2067#issuecomment-13353",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:5'></a>Yeah, looks fixed to me. Closed. Thanks for tracking this down.

Cheers,

Michael



---

archive/issue_comments_013354.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-09-19T22:10:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2067",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2067#issuecomment-13354",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_004954.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-09-19T22:10:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2067",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2067#event-4954"
}
```
