# Issue 2018: [with spkg] crap -- scipy sandbox has a bunch of .svn directories.  Delete them

archive/issues_002018.json:
```json
{
    "body": "\n```\nsage-2.10.1.rc3/spkg/standard/scipy_sandbox-20071020.p1/delaunay/.svn\nsage-2.10.1.rc3/spkg/standard/scipy_sandbox-20071020.p1/delaunay/tests/.svn\nsage-2.10.1.rc3/spkg/standard/scipy_sandbox-20071020.p1/arpack/tests/.svn\nsage-2.10.1.rc3/spkg/standard/scipy_sandbox-20071020.p1/arpack/ARPACK/.svn\nsage-2.10.1.rc3/spkg/standard/scipy_sandbox-20071020.p1/arpack/ARPACK/UTIL/.svn\nsage-2.10.1.rc3/spkg/standard/scipy_sandbox-20071020.p1/arpack/ARPACK/SRC/.svn\nsage-2.10.1.rc3/spkg/standard/scipy_sandbox-20071020.p1/arpack/ARPACK/LAPACK/.svn\n```\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/2018\n\n",
    "closed_at": "2008-02-01T02:02:29Z",
    "created_at": "2008-01-31T23:29:23Z",
    "labels": [
        "component: packages: standard",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.10.1",
    "title": "[with spkg] crap -- scipy sandbox has a bunch of .svn directories.  Delete them",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2018",
    "user": "https://github.com/williamstein"
}
```

```
sage-2.10.1.rc3/spkg/standard/scipy_sandbox-20071020.p1/delaunay/.svn
sage-2.10.1.rc3/spkg/standard/scipy_sandbox-20071020.p1/delaunay/tests/.svn
sage-2.10.1.rc3/spkg/standard/scipy_sandbox-20071020.p1/arpack/tests/.svn
sage-2.10.1.rc3/spkg/standard/scipy_sandbox-20071020.p1/arpack/ARPACK/.svn
sage-2.10.1.rc3/spkg/standard/scipy_sandbox-20071020.p1/arpack/ARPACK/UTIL/.svn
sage-2.10.1.rc3/spkg/standard/scipy_sandbox-20071020.p1/arpack/ARPACK/SRC/.svn
sage-2.10.1.rc3/spkg/standard/scipy_sandbox-20071020.p1/arpack/ARPACK/LAPACK/.svn
```

Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/2018





---

archive/issue_comments_013584.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-02-01T01:20:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2018",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2018#issuecomment-13584",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing status from new to assigned.



---

archive/issue_comments_013585.json:
```json
{
    "body": "<a id='comment:1'></a>\nI created new spkgs with crap removed:\n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-2.10.1/rc4/scipy-20071020-0.6.p3.spkg\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-2.10.1/rc4/scipy_sandbox-20071020.p2.spkg\n\nBoth spkgs now have a proper mercurial repo.\n\nCheers,\n\nMichael",
    "created_at": "2008-02-01T01:20:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2018",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2018#issuecomment-13585",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>
I created new spkgs with crap removed:

http://sage.math.washington.edu/home/mabshoff/release-cycles-2.10.1/rc4/scipy-20071020-0.6.p3.spkg
http://sage.math.washington.edu/home/mabshoff/release-cycles-2.10.1/rc4/scipy_sandbox-20071020.p2.spkg

Both spkgs now have a proper mercurial repo.

Cheers,

Michael



---

archive/issue_events_006898.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2018",
    "rename": {
        "from": "crap -- scipy sandbox has a bunch of .svn directories.  Delete them",
        "to": "[with spkg] crap -- scipy sandbox has a bunch of .svn directories.  Delete them"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2018#event-6898"
}
```



---

archive/issue_comments_013586.json:
```json
{
    "body": "<a id='comment:2'></a>\nMerged in Sage 2.10.1.rc4",
    "created_at": "2008-02-01T02:02:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2018",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2018#issuecomment-13586",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Merged in Sage 2.10.1.rc4



---

archive/issue_events_006899.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-02-01T02:02:29Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2018",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2018#event-6899"
}
```



---

archive/issue_comments_013587.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-02-01T02:02:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2018",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2018#issuecomment-13587",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed
