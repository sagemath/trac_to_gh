# Issue 2505: [with patch, positive review] Sage has no Wronskian function

archive/issues_002505.json:
```json
{
    "body": "The Wronskian is a basic tool in differential equations. It's used to see if a set of functions is linearly independent. The attached patch adds such a function to calculus.py.\n\n\n\nAssignee: @williamstein\n\nKeywords: wronskian, calculus\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/2505\n\n",
    "closed_at": "2008-03-16T00:04:03Z",
    "created_at": "2008-03-13T07:58:20Z",
    "labels": [
        "component: calculus",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.10.4",
    "title": "[with patch, positive review] Sage has no Wronskian function",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2505",
    "user": "https://github.com/dandrake"
}
```
The Wronskian is a basic tool in differential equations. It's used to see if a set of functions is linearly independent. The attached patch adds such a function to calculus.py.



Assignee: @williamstein

Keywords: wronskian, calculus

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/2505





---

archive/issue_comments_017389.json:
```json
{
    "body": "Changing assignee from mabshoff to @williamstein.",
    "created_at": "2008-03-13T08:06:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2505",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2505#issuecomment-17389",
    "user": "https://github.com/dandrake"
}
```

Changing assignee from mabshoff to @williamstein.



---

archive/issue_comments_017390.json:
```json
{
    "body": "Changing component from Cygwin to calculus.",
    "created_at": "2008-03-13T08:06:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2505",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2505#issuecomment-17390",
    "user": "https://github.com/dandrake"
}
```

Changing component from Cygwin to calculus.



---

archive/issue_comments_017391.json:
```json
{
    "body": "Attachment [wronskian.patch](tarball://root/attachments/some-uuid/ticket2505/wronskian.patch) by @dandrake created at 2008-03-13 09:24:13",
    "created_at": "2008-03-13T09:24:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2505",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2505#issuecomment-17391",
    "user": "https://github.com/dandrake"
}
```

Attachment [wronskian.patch](tarball://root/attachments/some-uuid/ticket2505/wronskian.patch) by @dandrake created at 2008-03-13 09:24:13



---

archive/issue_comments_017392.json:
```json
{
    "body": "<a id='comment:2'></a>New version of patch; puts the function into a new calculus/functions.py file, as suggested by gfurnish on IRC. Patch is against 2.10.3.",
    "created_at": "2008-03-13T09:26:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2505",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2505#issuecomment-17392",
    "user": "https://github.com/dandrake"
}
```

<a id='comment:2'></a>New version of patch; puts the function into a new calculus/functions.py file, as suggested by gfurnish on IRC. Patch is against 2.10.3.



---

archive/issue_comments_017393.json:
```json
{
    "body": "<a id='comment:3'></a>Applies to 2.10.4.alpha0 and works for me.",
    "created_at": "2008-03-15T21:28:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2505",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2505#issuecomment-17393",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:3'></a>Applies to 2.10.4.alpha0 and works for me.



---

archive/issue_comments_017394.json:
```json
{
    "body": "<a id='comment:4'></a>Merged in Sage 2.10.4.rc0",
    "created_at": "2008-03-16T00:04:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2505",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2505#issuecomment-17394",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>Merged in Sage 2.10.4.rc0



---

archive/issue_events_005887.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-03-16T00:04:03Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2505",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2505#event-5887"
}
```



---

archive/issue_comments_017395.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-03-16T00:04:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2505",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2505#issuecomment-17395",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed
