# Issue 2533: [with patch, positive review] add "-p" flag to $CP for make install

archive/issues_002533.json:
```json
{
    "body": "\n```\nOn Saturday 15 March 2008, Paul Zimmermann wrote:\n>        Hi,\n\n> I wonder why sage -br takes so much time after a fresh build from source\n> and make install. Normally, since everything was just compiled, it should\n> have nothing to do. I guess the reason lies in:\n\n>    bash-3.00$ make install DESTDIR=/usr/local/sage-2.10.3 -n\n>    ...\n>    cp -rv * /usr/local/sage-2.10.3/sage/\n>    ...\n\n> where 'cp' does not preserve the dates of the files, and thus the correct\n> dependencies are lost. Maybe \"mv * /usr/local/sage-2.10.3/sage/\" would\n> solve that problem?\n\nOr use\n    cp -prv * ...\nthe -p option preserve timestamps.\n\nBill\n```\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/2533\n\n",
    "closed_at": "2008-03-15T22:26:04Z",
    "created_at": "2008-03-15T22:15:35Z",
    "labels": [
        "component: build",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.10.4",
    "title": "[with patch, positive review] add \"-p\" flag to $CP for make install",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2533",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

```
On Saturday 15 March 2008, Paul Zimmermann wrote:
>        Hi,

> I wonder why sage -br takes so much time after a fresh build from source
> and make install. Normally, since everything was just compiled, it should
> have nothing to do. I guess the reason lies in:

>    bash-3.00$ make install DESTDIR=/usr/local/sage-2.10.3 -n
>    ...
>    cp -rv * /usr/local/sage-2.10.3/sage/
>    ...

> where 'cp' does not preserve the dates of the files, and thus the correct
> dependencies are lost. Maybe "mv * /usr/local/sage-2.10.3/sage/" would
> solve that problem?

Or use
    cp -prv * ...
the -p option preserve timestamps.

Bill
```

Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/2533





---

archive/issue_comments_018080.json:
```json
{
    "body": "Attachment [trac_2533.patch](tarball://root/attachments/some-uuid/ticket2533/trac_2533.patch) by mabshoff created at 2008-03-15 22:19:16",
    "created_at": "2008-03-15T22:19:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2533",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2533#issuecomment-18080",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Attachment [trac_2533.patch](tarball://root/attachments/some-uuid/ticket2533/trac_2533.patch) by mabshoff created at 2008-03-15 22:19:16



---

archive/issue_events_008932.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2533",
    "rename": {
        "from": "add \"-p\" flag to $CP for make install",
        "to": "[with patch, needs trivial review] add \"-p\" flag to $CP for make install"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2533#event-8932"
}
```



---

archive/issue_comments_018081.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-03-15T22:19:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2533",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2533#issuecomment-18081",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing status from new to assigned.



---

archive/issue_events_008933.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2533",
    "rename": {
        "from": "[with patch, needs trivial review] add \"-p\" flag to $CP for make install",
        "to": "[with patch, positive review] add \"-p\" flag to $CP for make install"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2533#event-8933"
}
```



---

archive/issue_comments_018082.json:
```json
{
    "body": "<a id='comment:2'></a>\nLooks good to me.",
    "created_at": "2008-03-15T22:22:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2533",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2533#issuecomment-18082",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:2'></a>
Looks good to me.



---

archive/issue_comments_018083.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 2.10.4.rc0",
    "created_at": "2008-03-15T22:26:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2533",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2533#issuecomment-18083",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 2.10.4.rc0



---

archive/issue_events_008934.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-03-15T22:26:04Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2533",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2533#event-8934"
}
```



---

archive/issue_comments_018084.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-03-15T22:26:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2533",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2533#issuecomment-18084",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed
