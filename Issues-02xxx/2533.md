# Issue 2533: add "-p" flag to $CP for make install

archive/issues_002533.json:
```json
{
    "assignees": [],
    "body": "\n```\nOn Saturday 15 March 2008, Paul Zimmermann wrote:\n>        Hi,\n\n> I wonder why sage -br takes so much time after a fresh build from source\n> and make install. Normally, since everything was just compiled, it should\n> have nothing to do. I guess the reason lies in:\n\n>    bash-3.00$ make install DESTDIR=/usr/local/sage-2.10.3 -n\n>    ...\n>    cp -rv * /usr/local/sage-2.10.3/sage/\n>    ...\n\n> where 'cp' does not preserve the dates of the files, and thus the correct\n> dependencies are lost. Maybe \"mv * /usr/local/sage-2.10.3/sage/\" would\n> solve that problem?\n\nOr use\n    cp -prv * ...\nthe -p option preserve timestamps.\n\nBill\n```\n\nAssignee: **mabshoff**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/2533_\n\n",
    "closed_at": "2008-03-15T22:26:04Z",
    "created_at": "2008-03-15T22:15:35Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.10.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "add \"-p\" flag to $CP for make install",
    "type": "issue",
    "updated_at": "2008-03-15T22:26:04Z",
    "url": "https://github.com/sagemath/sage/issues/2533",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

```
On Saturday 15 March 2008, Paul Zimmermann wrote:
>        Hi,

> I wonder why sage -br takes so much time after a fresh build from source
> and make install. Normally, since everything was just compiled, it should
> have nothing to do. I guess the reason lies in:

>    bash-3.00$ make install DESTDIR=/usr/local/sage-2.10.3 -n
>    ...
>    cp -rv * /usr/local/sage-2.10.3/sage/
>    ...

> where 'cp' does not preserve the dates of the files, and thus the correct
> dependencies are lost. Maybe "mv * /usr/local/sage-2.10.3/sage/" would
> solve that problem?

Or use
    cp -prv * ...
the -p option preserve timestamps.

Bill
```

Assignee: **mabshoff**

_Issue created by migration from https://trac.sagemath.org/ticket/2533_





---

archive/issue_events_022952.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-15T22:15:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2533",
    "milestone_number": null,
    "milestone_title": "sage-2.10.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2533#event-22952"
}
```



---

archive/issue_events_022953.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-15T22:15:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2533",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2533#event-22953"
}
```



---

archive/issue_events_022954.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-15T22:15:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2533",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2533#event-22954"
}
```



---

archive/issue_events_022955.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-15T22:15:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2533",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2533#event-22955"
}
```



---

archive/issue_comments_013675.json:
```json
{
    "body": "Attachment: **[trac_2533.patch.gz](https://github.com/sagemath/sage/files/ticket2533/trac_2533.patch.gz)**",
    "created_at": "2008-03-15T22:19:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2533",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2533#issuecomment-13675",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

Attachment: **[trac_2533.patch.gz](https://github.com/sagemath/sage/files/ticket2533/trac_2533.patch.gz)**



---

archive/issue_events_022956.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-15T22:19:45Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/2533",
    "title_is": "[needs trivial review] add \"-p\" flag to $CP for make install",
    "title_was": "add \"-p\" flag to $CP for make install",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2533#event-22956"
}
```



---

archive/issue_events_022957.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-03-15T22:22:13Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/2533",
    "title_is": "add \"-p\" flag to $CP for make install",
    "title_was": "[needs trivial review] add \"-p\" flag to $CP for make install",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2533#event-22957"
}
```



---

archive/issue_events_022958.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-03-15T22:22:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2533",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2533#event-22958"
}
```



---

archive/issue_comments_013676.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nLooks good to me.",
    "created_at": "2008-03-15T22:22:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2533",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2533#issuecomment-13676",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:2'>Comment 2:</a>
Looks good to me.



---

archive/issue_comments_013677.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nMerged in Sage 2.10.4.rc0",
    "created_at": "2008-03-15T22:26:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2533",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2533#issuecomment-13677",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:3'>Comment 3:</a>
Merged in Sage 2.10.4.rc0



---

archive/issue_events_022959.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-15T22:26:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2533",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2533#event-22959"
}
```



---

archive/issue_events_022960.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-15T22:26:04Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2533",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2533#event-22960"
}
```
