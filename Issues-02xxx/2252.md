# Issue 2252: [with patch, positive review] sage 2.10.2.rc0: rings/number_field/number_field_ideal.py failure

archive/issues_002252.json:
```json
{
    "body": "\n```\nsage -t -long devel/sage-main/sage/rings/number_field/number_field_ideal.py\n**********************************************************************\nFile \"number_field_ideal.py\", line 868:\n    sage: I.prime_factors()\nExpected:\n    [Fractional ideal (w)]\nGot:\n    [Fractional ideal (-w)]\n**********************************************************************\n1 items had failures:\n   1 of   3 in __main__.example_32\n***Test Failed*** 1 failures.\nFor whitespace errors, see the file .doctest_number_field_ideal.py\n         [6.6 s]\nexit code: 256\n```\n\nAssignee: @craigcitro\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/2252\n\n",
    "closed_at": "2008-02-22T00:26:16Z",
    "created_at": "2008-02-21T19:33:49Z",
    "labels": [
        "component: doctest coverage",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.10.2",
    "title": "[with patch, positive review] sage 2.10.2.rc0: rings/number_field/number_field_ideal.py failure",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2252",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

```
sage -t -long devel/sage-main/sage/rings/number_field/number_field_ideal.py
**********************************************************************
File "number_field_ideal.py", line 868:
    sage: I.prime_factors()
Expected:
    [Fractional ideal (w)]
Got:
    [Fractional ideal (-w)]
**********************************************************************
1 items had failures:
   1 of   3 in __main__.example_32
***Test Failed*** 1 failures.
For whitespace errors, see the file .doctest_number_field_ideal.py
         [6.6 s]
exit code: 256
```

Assignee: @craigcitro

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/2252





---

archive/issue_comments_015620.json:
```json
{
    "body": "Attachment [trac-2252.patch](tarball://root/attachments/some-uuid/ticket2252/trac-2252.patch) by @craigcitro created at 2008-02-21 23:57:57",
    "created_at": "2008-02-21T23:57:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2252",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2252#issuecomment-15620",
    "user": "https://github.com/craigcitro"
}
```

Attachment [trac-2252.patch](tarball://root/attachments/some-uuid/ticket2252/trac-2252.patch) by @craigcitro created at 2008-02-21 23:57:57



---

archive/issue_comments_015621.json:
```json
{
    "body": "<a id='comment:1'></a>\nThe answer is still mathematically correct, and it's what sage is producing, so there's nothing \"wrong\" with it. However, there's something slightly unsavory (for me) about this -- tracing through the code, Pari doesn't seem to be producing the minus sign. I'm not sure where it's coming from.",
    "created_at": "2008-02-22T00:05:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2252",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2252#issuecomment-15621",
    "user": "https://github.com/craigcitro"
}
```

<a id='comment:1'></a>
The answer is still mathematically correct, and it's what sage is producing, so there's nothing "wrong" with it. However, there's something slightly unsavory (for me) about this -- tracing through the code, Pari doesn't seem to be producing the minus sign. I'm not sure where it's coming from.



---

archive/issue_comments_015622.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-02-22T00:05:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2252",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2252#issuecomment-15622",
    "user": "https://github.com/craigcitro"
}
```

Changing status from new to assigned.



---

archive/issue_events_007770.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2252",
    "rename": {
        "from": "sage 2.10.2.rc0: rings/number_field/number_field_ideal.py failure",
        "to": "[with patch, needs quick review] sage 2.10.2.rc0: rings/number_field/number_field_ideal.py failure"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2252#event-7770"
}
```



---

archive/issue_events_007771.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2252",
    "rename": {
        "from": "[with patch, needs quick review] sage 2.10.2.rc0: rings/number_field/number_field_ideal.py failure",
        "to": "[with patch, positive review] sage 2.10.2.rc0: rings/number_field/number_field_ideal.py failure"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2252#event-7771"
}
```



---

archive/issue_comments_015623.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-02-22T00:26:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2252",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2252#issuecomment-15623",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_007772.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-02-22T00:26:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2252",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2252#event-7772"
}
```



---

archive/issue_comments_015624.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 2.10.2.rc0",
    "created_at": "2008-02-22T00:26:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2252",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2252#issuecomment-15624",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 2.10.2.rc0
