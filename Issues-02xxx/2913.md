# Issue 2913: notebook -- deleting a running cell results in a bug

archive/issues_002913.json:
```json
{
    "body": "\n```\nToday when I was using the notebook I got this:\n\nhttp://skitch.com/yqiang/jqwb/notebook-bug\n\nIs this a known bug? It happened when I deleted a cell while it was\nstill running and trying to produce output.\n\nCheers,\nYi\n```\n\n**Assignee:** boothby\n\nIssue created by migration from https://trac.sagemath.org/ticket/2913\n\n",
    "closed_at": "2008-04-14T04:58:26Z",
    "created_at": "2008-04-14T03:27:27Z",
    "labels": [
        "component: notebook",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0",
    "title": "notebook -- deleting a running cell results in a bug",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/2913",
    "user": "https://github.com/williamstein"
}
```

```
Today when I was using the notebook I got this:

http://skitch.com/yqiang/jqwb/notebook-bug

Is this a known bug? It happened when I deleted a cell while it was
still running and trying to produce output.

Cheers,
Yi
```

**Assignee:** boothby

Issue created by migration from https://trac.sagemath.org/ticket/2913





---

archive/attachments_002767.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "sage-2913.patch",
    "asset_url": "tarball://root/attachments/ticket2913/sage-2913.patch",
    "created_at": "2008-04-14T03:29:44Z",
    "issue": "https://github.com/sagemath/sage/issues/2913",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket2913/sage-2913.patch",
    "user": "https://github.com/williamstein"
}
```



---

archive/issue_comments_015984.json:
```json
{
    "body": "Attachment [sage-2913.patch](https://github.com/sagemath/sage/files/ticket2913/sage-2913.patch) by @williamstein created at 2008-04-14 03:29:44",
    "created_at": "2008-04-14T03:29:44Z",
    "issue": "https://github.com/sagemath/sage/issues/2913",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2913#issuecomment-15984",
    "user": "https://github.com/williamstein"
}
```

Attachment [sage-2913.patch](https://github.com/sagemath/sage/files/ticket2913/sage-2913.patch) by @williamstein created at 2008-04-14 03:29:44



---

archive/issue_comments_015985.json:
```json
{
    "body": "<a id='comment:1'></a>\nPatch applied cleanly against alpha3 and fixes my original issue, good job!",
    "created_at": "2008-04-14T04:51:25Z",
    "issue": "https://github.com/sagemath/sage/issues/2913",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2913#issuecomment-15985",
    "user": "https://github.com/yqiang"
}
```

<a id='comment:1'></a>
Patch applied cleanly against alpha3 and fixes my original issue, good job!



---

archive/issue_events_015304.json:
```json
{
    "actor": "https://github.com/yqiang",
    "created_at": "2008-04-14T04:51:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2913",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2913#event-15304"
}
```



---

archive/issue_events_015305.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-14T04:58:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2913",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2913#event-15305"
}
```



---

archive/issue_events_015306.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-14T04:58:26Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2913",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2913#event-15306"
}
```



---

archive/issue_comments_015986.json:
```json
{
    "body": "<a id='comment:2'></a>\nMerged in Sage 3.0.alpha5",
    "created_at": "2008-04-14T04:58:26Z",
    "issue": "https://github.com/sagemath/sage/issues/2913",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2913#issuecomment-15986",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Merged in Sage 3.0.alpha5
