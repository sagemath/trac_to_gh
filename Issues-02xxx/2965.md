# Issue 2965: extcode spkg build failure on Debian

archive/issues_002965.json:
```json
{
    "assignees": [
        "https://github.com/timabbott"
    ],
    "body": "Now that there's a dist/debian directory in the extcode spkg, my Debian scripts try to build extcode as a separate package.  This doesn't work; I've attached a trivial patch to make this not happen.\n\nComponent: **debian-package**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/2965_\n\n",
    "closed_at": "2008-04-20T04:17:21Z",
    "created_at": "2008-04-20T04:02:32Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20distribution",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "extcode spkg build failure on Debian",
    "type": "issue",
    "updated_at": "2008-04-20T04:32:23Z",
    "url": "https://github.com/sagemath/sage/issues/2965",
    "user": "https://github.com/timabbott"
}
```
Now that there's a dist/debian directory in the extcode spkg, my Debian scripts try to build extcode as a separate package.  This doesn't work; I've attached a trivial patch to make this not happen.

Component: **debian-package**

_Issue created by migration from https://trac.sagemath.org/ticket/2965_





---

archive/issue_events_031581.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-04-20T04:02:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2965",
    "milestone_number": null,
    "milestone_title": "sage-3.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2965#event-31581"
}
```



---

archive/issue_events_031582.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-04-20T04:02:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2965",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20distribution",
    "label_color": "000080",
    "label_name": "c: distribution",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2965#event-31582"
}
```



---

archive/issue_events_031583.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-04-20T04:02:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2965",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2965#event-31583"
}
```



---

archive/issue_events_031584.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-04-20T04:02:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2965",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2965#event-31584"
}
```



---

archive/issue_events_031585.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-04-20T04:02:32Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/2965",
    "subject": "https://github.com/timabbott",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2965#event-31585"
}
```



---

archive/issue_comments_015551.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[extcode-debian.patch.gz](https://github.com/sagemath/sage/files/ticket2965/extcode-debian.patch.gz)**",
    "created_at": "2008-04-20T04:07:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2965#issuecomment-15551",
    "user": "https://github.com/timabbott"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[extcode-debian.patch.gz](https://github.com/sagemath/sage/files/ticket2965/extcode-debian.patch.gz)**



---

archive/issue_events_031586.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-04-20T04:07:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2965",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2965#event-31586"
}
```



---

archive/issue_comments_015552.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nPatch looks good to me. Positive review.\n\nCheers,\n\nMichael",
    "created_at": "2008-04-20T04:17:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2965#issuecomment-15552",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Patch looks good to me. Positive review.

Cheers,

Michael



---

archive/issue_comments_015553.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged in Sage 3.0.rc0",
    "created_at": "2008-04-20T04:17:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2965#issuecomment-15553",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged in Sage 3.0.rc0



---

archive/issue_events_031587.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-20T04:17:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2965",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2965#event-31587"
}
```



---

archive/issue_events_031588.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-20T04:17:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2965",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2965#event-31588"
}
```



---

archive/issue_events_031589.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-20T04:32:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/2965",
    "milestone_number": null,
    "milestone_title": "sage-3.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2965#event-31589"
}
```



---

archive/issue_events_031590.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-20T04:32:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2965",
    "milestone_number": null,
    "milestone_title": "sage-3.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2965#event-31590"
}
```
