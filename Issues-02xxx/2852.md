# Issue 2852: ctrl-enter broken in firefox/linux

archive/issues_002852.json:
```json
{
    "body": "\n\n**Assignee:** boothby\n\nIssue created by migration from https://trac.sagemath.org/ticket/2852\n\n",
    "closed_at": "2008-04-08T15:33:45Z",
    "created_at": "2008-04-08T05:35:26Z",
    "labels": [
        "component: notebook",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0",
    "title": "ctrl-enter broken in firefox/linux",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/2852",
    "user": "https://trac.sagemath.org/admin/accounts/users/boothby"
}
```


**Assignee:** boothby

Issue created by migration from https://trac.sagemath.org/ticket/2852





---

archive/issue_events_014863.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/boothby",
    "created_at": "2008-04-08T05:40:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2852",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2852#event-14863"
}
```



---

archive/attachments_002694.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "2852-keyfixes.patch",
    "asset_url": "tarball://root/attachments/ticket2852/2852-keyfixes.patch",
    "created_at": "2008-04-08T05:40:36Z",
    "issue": "https://github.com/sagemath/sage/issues/2852",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket2852/2852-keyfixes.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/boothby"
}
```



---

archive/issue_comments_015570.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [2852-keyfixes.patch](https://github.com/sagemath/sage/files/ticket2852/2852-keyfixes.patch) by boothby created at 2008-04-08 05:40:36\n\nThe attached should be approached with distrust.  It makes a very low-level change to how key events are handled.  Test all keycodes in all browsers before and after applying this patch.",
    "created_at": "2008-04-08T05:40:36Z",
    "issue": "https://github.com/sagemath/sage/issues/2852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2852#issuecomment-15570",
    "user": "https://trac.sagemath.org/admin/accounts/users/boothby"
}
```

<a id='comment:1'></a>
Attachment [2852-keyfixes.patch](https://github.com/sagemath/sage/files/ticket2852/2852-keyfixes.patch) by boothby created at 2008-04-08 05:40:36

The attached should be approached with distrust.  It makes a very low-level change to how key events are handled.  Test all keycodes in all browsers before and after applying this patch.



---

archive/issue_comments_015571.json:
```json
{
    "body": "<a id='comment:2'></a>\nIt looks good to me.  I'm going to test the heck out of the notebook before 3.0 is released anyways, so I say we apply this. \n\nNOTE: I updated a comment that should be updated because of this patch, but that will go in a subsequent \"fix of a few small bugs\" notebook patch I'll post shortly.",
    "created_at": "2008-04-08T13:27:32Z",
    "issue": "https://github.com/sagemath/sage/issues/2852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2852#issuecomment-15571",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:2'></a>
It looks good to me.  I'm going to test the heck out of the notebook before 3.0 is released anyways, so I say we apply this. 

NOTE: I updated a comment that should be updated because of this patch, but that will go in a subsequent "fix of a few small bugs" notebook patch I'll post shortly.



---

archive/issue_events_014864.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-04-08T13:27:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2852",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2852#event-14864"
}
```



---

archive/issue_events_014865.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-04-08T13:27:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2852",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2852#event-14865"
}
```



---

archive/issue_events_014866.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-08T15:33:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2852",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2852#event-14866"
}
```



---

archive/issue_events_014867.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-08T15:33:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2852",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2852#event-14867"
}
```



---

archive/issue_comments_015572.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.0.alpha3",
    "created_at": "2008-04-08T15:33:45Z",
    "issue": "https://github.com/sagemath/sage/issues/2852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2852#issuecomment-15572",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.0.alpha3
