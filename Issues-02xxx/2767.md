...
<a id='comment:1'></a>`ncalexan` and `wstein` looked at this for a few minutes, and discovered that the root cause was that my "fix" for #1083 really only worked in the case that the absolute extension was of degree 2. (Sloppy testing on my part.) So, I'm fixing this.
...
```json
{
    "body": "<a id='comment:2'></a>Does this handle fields with unusual defining polynomials, such as F = L.extension(2*x - L.gen())?  I don't think it will.\n\nNick",
...
<a id='comment:2'></a>Does this handle fields with unusual defining polynomials, such as F = L.extension(2*x - L.gen())?  I don't think it will.
...
<a id='comment:3'></a>After discussion on IRC, reviewed positive:
...
```json
{
    "body": "<a id='comment:4'></a>Doctests pass for me.\n\nCheers,\n\nMichael",
...
<a id='comment:4'></a>Doctests pass for me.
...
```json
{
    "body": "<a id='comment:5'></a>Merged in Sage 3.0.alpha1",
...
<a id='comment:5'></a>Merged in Sage 3.0.alpha1
