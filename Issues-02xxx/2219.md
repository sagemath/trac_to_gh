# Issue 2219: [with patch, positive review] docs for root_field should say the polynomial needs to be irreducible

archive/issues_002219.json:
```json
{
    "body": "\n```\nroot_field()\n-- whose docstring does not say that self should be irreducible,\nthough in fact it must. \n```\n\nSee http://groups.google.com/group/sage-devel/browse_thread/thread/32fe12de12d5f6a5/c91753b5e65fe7b9#c91753b5e65fe7b9\n\nAssignee: @williamstein\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/2219\n\n",
    "closed_at": "2008-04-15T00:31:55Z",
    "created_at": "2008-02-20T03:51:37Z",
    "labels": [
        "component: number theory",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0",
    "title": "[with patch, positive review] docs for root_field should say the polynomial needs to be irreducible",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2219",
    "user": "https://github.com/jasongrout"
}
```

```
root_field()
-- whose docstring does not say that self should be irreducible,
though in fact it must. 
```

See http://groups.google.com/group/sage-devel/browse_thread/thread/32fe12de12d5f6a5/c91753b5e65fe7b9#c91753b5e65fe7b9

Assignee: @williamstein

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/2219





---

archive/issue_comments_015393.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [2219-root_field.patch](tarball://root/attachments/some-uuid/ticket2219/2219-root_field.patch) by @aghitza created at 2008-04-13 03:03:53\n\nSee the attached patch.",
    "created_at": "2008-04-13T03:03:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2219",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2219#issuecomment-15393",
    "user": "https://github.com/aghitza"
}
```

<a id='comment:1'></a>
Attachment [2219-root_field.patch](tarball://root/attachments/some-uuid/ticket2219/2219-root_field.patch) by @aghitza created at 2008-04-13 03:03:53

See the attached patch.



---

archive/issue_events_007674.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2219",
    "rename": {
        "from": "docs for root_field should say the polynomial needs to be irreducible",
        "to": "[with patch, needs review] docs for root_field should say the polynomial needs to be irreducible"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2219#event-7674"
}
```



---

archive/issue_comments_015394.json:
```json
{
    "body": "<a id='comment:2'></a>\nLooks good to me.",
    "created_at": "2008-04-14T23:08:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2219",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2219#issuecomment-15394",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:2'></a>
Looks good to me.



---

archive/issue_events_007675.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2219",
    "rename": {
        "from": "[with patch, needs review] docs for root_field should say the polynomial needs to be irreducible",
        "to": "[with patch, positive review] docs for root_field should say the polynomial needs to be irreducible"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2219#event-7675"
}
```



---

archive/issue_comments_015395.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.0.alpha5",
    "created_at": "2008-04-15T00:31:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2219",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2219#issuecomment-15395",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.0.alpha5



---

archive/issue_comments_015396.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-04-15T00:31:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2219",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2219#issuecomment-15396",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_007676.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-15T00:31:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2219",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2219#event-7676"
}
```
