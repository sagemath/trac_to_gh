# Issue 2404: [with patch, positive review] subs_expr claims to take a dictionary, but doesn't

archive/issues_002404.json:
```json
{
    "body": "The docstring for subs_expr (in calculus.py) says:\n\n> Given a dictionary of key:value pairs, substitute all occurences of key for value in self.\n\n\n...but the function does not accept a dictionary:\n\n```\ndef subs_expr(self, *equations):\n```\n\nIt'll take an arbitrary number of regular parameters (which must be SymbolicEquations), but not a dictionary.\n\nI tried to modify the function to something like the following, but couldn't get it to work. Someone familiar with this code should have no problem implementing it correctly:\n\n```\ndef subs_expr(self, *equations, **equationsdict):\n  for x in equations:\n    if not isinstance(x, SymbolicEquation):\n      raise TypeError, \"each expression must be an equation\"\n\n  R = self.parent()\n  v = ','.join(['%s=%s'%(x.lhs()._maxima_init_(), x.rhs()._maxima_init_()) \\\n                    for x in equations])\n  v = v + ','.join(['%s=%s' % (key._maxima_init_(), \\\n    equationsdict[key]._maxima_init_()) for key in equationsdict.keys()]\n  return R(self._maxima_().subst(v))\n```\n\n\n\nAssignee: @williamstein\n\nKeywords: calculus, substitution, subs_expr\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/2404\n\n",
    "closed_at": "2009-01-28T15:17:27Z",
    "created_at": "2008-03-06T09:49:43Z",
    "labels": [
        "component: calculus",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.3",
    "title": "[with patch, positive review] subs_expr claims to take a dictionary, but doesn't",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2404",
    "user": "https://github.com/dandrake"
}
```
The docstring for subs_expr (in calculus.py) says:

> Given a dictionary of key:value pairs, substitute all occurences of key for value in self.


...but the function does not accept a dictionary:

```
def subs_expr(self, *equations):
```

It'll take an arbitrary number of regular parameters (which must be SymbolicEquations), but not a dictionary.

I tried to modify the function to something like the following, but couldn't get it to work. Someone familiar with this code should have no problem implementing it correctly:

```
def subs_expr(self, *equations, **equationsdict):
  for x in equations:
    if not isinstance(x, SymbolicEquation):
      raise TypeError, "each expression must be an equation"

  R = self.parent()
  v = ','.join(['%s=%s'%(x.lhs()._maxima_init_(), x.rhs()._maxima_init_()) \
                    for x in equations])
  v = v + ','.join(['%s=%s' % (key._maxima_init_(), \
    equationsdict[key]._maxima_init_()) for key in equationsdict.keys()]
  return R(self._maxima_().subst(v))
```



Assignee: @williamstein

Keywords: calculus, substitution, subs_expr

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/2404





---

archive/issue_events_008407.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-03-06T12:31:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/2404",
    "milestone": "sage-2.10.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2404#event-8407"
}
```



---

archive/issue_comments_016976.json:
```json
{
    "body": "<a id='comment:2'></a>\nAttachment [trac_2404.patch](tarball://root/attachments/some-uuid/ticket2404/trac_2404.patch) by @aghitza created at 2009-01-23 19:03:16\n\nThe attached patch fixes subs_expr to take a dictionary, adds an appropriate doctest, and clarifies the docstring a bit.",
    "created_at": "2009-01-23T19:03:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2404#issuecomment-16976",
    "user": "https://github.com/aghitza"
}
```

<a id='comment:2'></a>
Attachment [trac_2404.patch](tarball://root/attachments/some-uuid/ticket2404/trac_2404.patch) by @aghitza created at 2009-01-23 19:03:16

The attached patch fixes subs_expr to take a dictionary, adds an appropriate doctest, and clarifies the docstring a bit.



---

archive/issue_events_008408.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2404",
    "rename": {
        "from": "subs_expr claims to take a dictionary, but doesn't",
        "to": "[with patch, needs review] subs_expr claims to take a dictionary, but doesn't"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2404#event-8408"
}
```



---

archive/issue_events_008409.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2404",
    "rename": {
        "from": "[with patch, needs review] subs_expr claims to take a dictionary, but doesn't",
        "to": "[with patch, positive review] subs_expr claims to take a dictionary, but doesn't"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2404#event-8409"
}
```



---

archive/issue_comments_016977.json:
```json
{
    "body": "<a id='comment:3'></a>\nLooks good to me.",
    "created_at": "2009-01-24T03:07:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2404#issuecomment-16977",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:3'></a>
Looks good to me.



---

archive/issue_comments_016978.json:
```json
{
    "body": "<a id='comment:4'></a>\nNote that the following in the patch needs to be changed\n\n```\nThe following test shows that \\#4364 is indeed fixed.\n```\nI did so in the patch I applied.\n\nCheers,\n\nMichael",
    "created_at": "2009-01-28T14:41:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2404#issuecomment-16978",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Note that the following in the patch needs to be changed

```
The following test shows that \#4364 is indeed fixed.
```
I did so in the patch I applied.

Cheers,

Michael



---

archive/issue_comments_016979.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-01-28T15:17:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2404#issuecomment-16979",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_008410.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-01-28T15:17:27Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2404",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2404#event-8410"
}
```



---

archive/issue_events_008411.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-01-28T15:17:27Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/2404",
    "milestone": "sage-2.10.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2404#event-8411"
}
```



---

archive/issue_events_008412.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-01-28T15:17:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/2404",
    "milestone": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2404#event-8412"
}
```



---

archive/issue_comments_016980.json:
```json
{
    "body": "<a id='comment:5'></a>\nMerged in Sage 3.3.alpha3.\n\nCheers,\n\nMichael",
    "created_at": "2009-01-28T15:17:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2404#issuecomment-16980",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:5'></a>
Merged in Sage 3.3.alpha3.

Cheers,

Michael
