# Issue 2914: [with patch, with positive review] integers are immutable but set_str breaks that -- having this function is a *major* bug.

archive/issues_002914.json:
```json
{
    "body": "This is *terrible*\n\n```\nsage: n = 10\nsage: n.set_str('15')\nsage: n\n15\n```\n\nThe set_str function must be made __ or removed.\n\nLook at what evil can occur:\n\n```\nsage: a = matrix(ZZ,2,[1,2,3,4]); d = a.det(); d\n-2\nsage: d.set_str('10')\nsage: a.det()\n10\n```\n\nAssignee: somebody\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/2914\n\n",
    "closed_at": "2008-04-14T17:04:27Z",
    "created_at": "2008-04-14T04:01:26Z",
    "labels": [
        "component: basic arithmetic",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0",
    "title": "[with patch, with positive review] integers are immutable but set_str breaks that -- having this function is a *major* bug.",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2914",
    "user": "https://github.com/williamstein"
}
```
This is *terrible*

```
sage: n = 10
sage: n.set_str('15')
sage: n
15
```

The set_str function must be made __ or removed.

Look at what evil can occur:

```
sage: a = matrix(ZZ,2,[1,2,3,4]); d = a.det(); d
-2
sage: d.set_str('10')
sage: a.det()
10
```

Assignee: somebody

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/2914





---

archive/issue_comments_021039.json:
```json
{
    "body": "Changing assignee from mabshoff to somebody.",
    "created_at": "2008-04-14T04:13:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2914",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2914#issuecomment-21039",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing assignee from mabshoff to somebody.



---

archive/issue_events_010335.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2914",
    "rename": {
        "from": "integers are immutable but set_str breaks that -- having this function is a *major* bug.",
        "to": "[with patch; needs review] integers are immutable but set_str breaks that -- having this function is a *major* bug."
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2914#event-10335"
}
```



---

archive/issue_comments_021040.json:
```json
{
    "body": "<a id='comment:2'></a>\nAttachment [sage-2914.patch](tarball://root/attachments/some-uuid/ticket2914/sage-2914.patch) by @williamstein created at 2008-04-14 04:42:04",
    "created_at": "2008-04-14T04:42:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2914",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2914#issuecomment-21040",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:2'></a>
Attachment [sage-2914.patch](tarball://root/attachments/some-uuid/ticket2914/sage-2914.patch) by @williamstein created at 2008-04-14 04:42:04



---

archive/issue_events_010336.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2914",
    "rename": {
        "from": "[with patch; needs review] integers are immutable but set_str breaks that -- having this function is a *major* bug.",
        "to": "[with patch, with mostly positive review] integers are immutable but set_str breaks that -- having this function is a *major* bug."
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2914#event-10336"
}
```



---

archive/issue_comments_021041.json:
```json
{
    "body": "<a id='comment:3'></a>\nThis looks good and should be applied.\n\nIn general I prefer single underscores to double -- strange things happen with double when inheritance comes in.  But in this situation, perhaps double is justified?\n\nIn the docstrings for the rational fixes, integer needs to be replaced by rational in a few places.",
    "created_at": "2008-04-14T06:02:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2914",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2914#issuecomment-21041",
    "user": "https://github.com/ncalexan"
}
```

<a id='comment:3'></a>
This looks good and should be applied.

In general I prefer single underscores to double -- strange things happen with double when inheritance comes in.  But in this situation, perhaps double is justified?

In the docstrings for the rational fixes, integer needs to be replaced by rational in a few places.



---

archive/issue_comments_021042.json:
```json
{
    "body": "<a id='comment:4'></a>\nThe fact that this is not touched *anywhere* else in the Sage codebase (or that there's a mistake) suggests this could just be deleted.  Why not just nuke the functionality, or make it only available from Cython?",
    "created_at": "2008-04-14T06:03:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2914",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2914#issuecomment-21042",
    "user": "https://github.com/ncalexan"
}
```

<a id='comment:4'></a>
The fact that this is not touched *anywhere* else in the Sage codebase (or that there's a mistake) suggests this could just be deleted.  Why not just nuke the functionality, or make it only available from Cython?



---

archive/issue_comments_021043.json:
```json
{
    "body": "<a id='comment:5'></a>\nI've attached a new patch that just nukes the functionality, as Nick suggested. \nThis should be applied *instead* of the previously posted patch.\n\n -- William",
    "created_at": "2008-04-14T14:47:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2914",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2914#issuecomment-21043",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:5'></a>
I've attached a new patch that just nukes the functionality, as Nick suggested. 
This should be applied *instead* of the previously posted patch.

 -- William



---

archive/issue_comments_021044.json:
```json
{
    "body": "apply this instead of previous version",
    "created_at": "2008-04-14T14:48:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2914",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2914#issuecomment-21044",
    "user": "https://github.com/williamstein"
}
```

apply this instead of previous version



---

archive/issue_events_010337.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2914",
    "rename": {
        "from": "[with patch, with mostly positive review] integers are immutable but set_str breaks that -- having this function is a *major* bug.",
        "to": "[with patch,  needs review] integers are immutable but set_str breaks that -- having this function is a *major* bug."
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2914#event-10337"
}
```



---

archive/issue_comments_021045.json:
```json
{
    "body": "<a id='comment:6'></a>\nAttachment [sage-2914_nuke_em.patch](tarball://root/attachments/some-uuid/ticket2914/sage-2914_nuke_em.patch) by @williamstein created at 2008-04-14 14:49:07",
    "created_at": "2008-04-14T14:49:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2914",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2914#issuecomment-21045",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:6'></a>
Attachment [sage-2914_nuke_em.patch](tarball://root/attachments/some-uuid/ticket2914/sage-2914_nuke_em.patch) by @williamstein created at 2008-04-14 14:49:07



---

archive/issue_comments_021046.json:
```json
{
    "body": "<a id='comment:7'></a>\nI think this is better.",
    "created_at": "2008-04-14T15:37:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2914",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2914#issuecomment-21046",
    "user": "https://github.com/ncalexan"
}
```

<a id='comment:7'></a>
I think this is better.



---

archive/issue_events_010338.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2914",
    "rename": {
        "from": "[with patch,  needs review] integers are immutable but set_str breaks that -- having this function is a *major* bug.",
        "to": "[with patch, with positive review] integers are immutable but set_str breaks that -- having this function is a *major* bug."
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2914#event-10338"
}
```



---

archive/issue_comments_021047.json:
```json
{
    "body": "<a id='comment:8'></a>\nMerged sage-2914_nuke_em.patch in Sage 3.0.alpha5",
    "created_at": "2008-04-14T17:04:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2914",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2914#issuecomment-21047",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:8'></a>
Merged sage-2914_nuke_em.patch in Sage 3.0.alpha5



---

archive/issue_events_010339.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-14T17:04:27Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2914",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2914#event-10339"
}
```



---

archive/issue_comments_021048.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-04-14T17:04:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2914",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2914#issuecomment-21048",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed
