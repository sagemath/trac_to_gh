# Issue 2236: [with patch, positive review] plot randomizes the endpoints of the interval and causes wiggling in the graph

archive/issues_002236.json:
```json
{
    "body": "\np=plot(x, (x,-1,1))\np[0][0] == -1\np[0][-1] == 1\n\nThey will almost always return false before the patch.  After the patch, the two statements should return True always.\n\nAssignee: @williamstein\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/2236\n\n",
    "closed_at": "2008-02-20T23:05:01Z",
    "created_at": "2008-02-20T22:39:42Z",
    "labels": [
        "component: graphics",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.10.2",
    "title": "[with patch, positive review] plot randomizes the endpoints of the interval and causes wiggling in the graph",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2236",
    "user": "https://github.com/jasongrout"
}
```

p=plot(x, (x,-1,1))
p[0][0] == -1
p[0][-1] == 1

They will almost always return false before the patch.  After the patch, the two statements should return True always.

Assignee: @williamstein

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/2236





---

archive/issue_comments_015511.json:
```json
{
    "body": "<a id='comment:1'></a>\nI agree with the suggestion to *not* randomize the endpoints.  That's bad.",
    "created_at": "2008-02-20T22:43:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2236",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2236#issuecomment-15511",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'></a>
I agree with the suggestion to *not* randomize the endpoints.  That's bad.



---

archive/issue_comments_015512.json:
```json
{
    "body": "Attachment [plot_fix_endpoints.patch](tarball://root/attachments/some-uuid/ticket2236/plot_fix_endpoints.patch) by @jasongrout created at 2008-02-20 22:51:08",
    "created_at": "2008-02-20T22:51:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2236",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2236#issuecomment-15512",
    "user": "https://github.com/jasongrout"
}
```

Attachment [plot_fix_endpoints.patch](tarball://root/attachments/some-uuid/ticket2236/plot_fix_endpoints.patch) by @jasongrout created at 2008-02-20 22:51:08



---

archive/issue_events_007720.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2236",
    "rename": {
        "from": "plot randomizes the endpoints of the interval and causes wiggling in the graph",
        "to": "[with patch, needs review] plot randomizes the endpoints of the interval and causes wiggling in the graph"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2236#event-7720"
}
```



---

archive/issue_comments_015513.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 2.10.2.alpha2",
    "created_at": "2008-02-20T23:05:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2236",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2236#issuecomment-15513",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 2.10.2.alpha2



---

archive/issue_events_007721.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-02-20T23:05:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2236",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2236#event-7721"
}
```



---

archive/issue_comments_015514.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-02-20T23:05:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2236",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2236#issuecomment-15514",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_015515.json:
```json
{
    "body": "<a id='comment:4'></a>\nFor the record, on IRC:\n\n[16:55] <wstein> #2236 -- positive review.",
    "created_at": "2008-02-20T23:05:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2236",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2236#issuecomment-15515",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:4'></a>
For the record, on IRC:

[16:55] <wstein> #2236 -- positive review.



---

archive/issue_events_007722.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2236",
    "rename": {
        "from": "[with patch, needs review] plot randomizes the endpoints of the interval and causes wiggling in the graph",
        "to": "[with patch, positive review] plot randomizes the endpoints of the interval and causes wiggling in the graph"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2236#event-7722"
}
```
