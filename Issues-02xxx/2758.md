# Issue 2758: iml and linbox updates for Debian gfortran transition

archive/issues_002758.json:
```json
{
    "assignees": [],
    "body": "Debian switched from g77 to gfortran while I wasn't looking; these patches should update the Debian packages for iml and linbox to deal with the transition.\n\nAssignee: **@timabbott**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/2758_\n\n",
    "closed_at": "2008-04-01T20:51:00Z",
    "created_at": "2008-04-01T18:52:01Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20debian-package",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "iml and linbox updates for Debian gfortran transition",
    "type": "issue",
    "updated_at": "2008-04-01T20:51:00Z",
    "url": "https://github.com/sagemath/sage/issues/2758",
    "user": "https://github.com/timabbott"
}
```
Debian switched from g77 to gfortran while I wasn't looking; these patches should update the Debian packages for iml and linbox to deal with the transition.

Assignee: **@timabbott**

_Issue created by migration from https://trac.sagemath.org/ticket/2758_





---

archive/issue_events_025271.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-04-01T18:52:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2758",
    "milestone_number": null,
    "milestone_title": "sage-3.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2758#event-25271"
}
```



---

archive/issue_events_025272.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-04-01T18:52:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2758",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20debian-package",
    "label_color": "0000ff",
    "label_name": "component: debian-package",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2758#event-25272"
}
```



---

archive/issue_events_025273.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-04-01T18:52:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2758",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2758#event-25273"
}
```



---

archive/issue_events_025274.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-04-01T18:52:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2758",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2758#event-25274"
}
```



---

archive/issue_events_025275.json:
```json
{
    "actor": "https://github.com/timabbott",
    "created_at": "2008-04-01T18:52:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2758",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2758#event-25275"
}
```



---

archive/issue_comments_015089.json:
```json
{
    "body": "Attachment: **[iml-gfortran.patch.gz](https://github.com/sagemath/sage/files/ticket2758/iml-gfortran.patch.gz)**",
    "created_at": "2008-04-01T18:52:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2758",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2758#issuecomment-15089",
    "user": "https://github.com/timabbott"
}
```

Attachment: **[iml-gfortran.patch.gz](https://github.com/sagemath/sage/files/ticket2758/iml-gfortran.patch.gz)**



---

archive/issue_comments_015090.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[linbox-gfortran.patch.gz](https://github.com/sagemath/sage/files/ticket2758/linbox-gfortran.patch.gz)**\n\nBoth patches look good to me. Positive review. The linbox patch has been merged in linbox-1.1.5rc2.p7, the iml patch has been merged in iml-1.0.1.p10.\n\nCheers,\n\nMichael",
    "created_at": "2008-04-01T20:50:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2758",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2758#issuecomment-15090",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[linbox-gfortran.patch.gz](https://github.com/sagemath/sage/files/ticket2758/linbox-gfortran.patch.gz)**

Both patches look good to me. Positive review. The linbox patch has been merged in linbox-1.1.5rc2.p7, the iml patch has been merged in iml-1.0.1.p10.

Cheers,

Michael



---

archive/issue_events_025276.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-01T20:51:00Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2758",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2758#event-25276"
}
```



---

archive/issue_comments_015091.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nMerged in Sage 3.0.alpha0",
    "created_at": "2008-04-01T20:51:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2758",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2758#issuecomment-15091",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:2'>Comment 2:</a>
Merged in Sage 3.0.alpha0
