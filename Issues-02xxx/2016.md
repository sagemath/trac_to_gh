# Issue 2016: crap -- there is a precompiled object file in the SAGE_ROOT/examples directory

archive/issues_002016.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "\n```\nsage-2.10.1.rc3/spkg/standard/examples-2.10.1.rc3/gsl/examples/math.o\n```\n\nComponent: **packages: standard**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/2016_\n\n",
    "closed_at": "2008-02-01T00:59:34Z",
    "created_at": "2008-01-31T23:27:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.10.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "crap -- there is a precompiled object file in the SAGE_ROOT/examples directory",
    "type": "issue",
    "updated_at": "2008-02-01T00:59:34Z",
    "url": "https://github.com/sagemath/sage/issues/2016",
    "user": "https://github.com/williamstein"
}
```

```
sage-2.10.1.rc3/spkg/standard/examples-2.10.1.rc3/gsl/examples/math.o
```

Component: **packages: standard**

_Issue created by migration from https://trac.sagemath.org/ticket/2016_





---

archive/issue_events_020285.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-01-31T23:27:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2016",
    "milestone_number": null,
    "milestone_title": "sage-2.10.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2016#event-20285"
}
```



---

archive/issue_events_020286.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-01-31T23:27:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2016",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2016#event-20286"
}
```



---

archive/issue_events_020287.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-01-31T23:27:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2016",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2016#event-20287"
}
```



---

archive/issue_events_020288.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-01-31T23:27:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2016",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2016#event-20288"
}
```



---

archive/issue_events_020289.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-01-31T23:27:28Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/2016",
    "subject": "https://github.com/williamstein",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2016#event-20289"
}
```



---

archive/issue_comments_009564.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nFixed in Sage 2.10.1rc4. No point in posting a spkg since it will be created vis sdist.",
    "created_at": "2008-02-01T00:59:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2016",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2016#issuecomment-9564",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:1" align="right">comment:1</div>

Fixed in Sage 2.10.1rc4. No point in posting a spkg since it will be created vis sdist.



---

archive/issue_events_020290.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-02-01T00:59:34Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2016",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2016#event-20290"
}
```
