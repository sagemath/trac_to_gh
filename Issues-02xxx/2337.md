# Issue 2337: [with patch, positive review] fix typo in save_session

archive/issues_002337.json:
```json
{
    "body": "\n```\nsage: save_session?\n\n[stuff deleted]\n\n       Save all variables that can be saved wto the given filename.\nThe\n\n^\n\n******\nChange \"wto\" to \"to\".\n\nKate\n```\n\nAssignee: @williamstein\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/2337\n\n",
    "closed_at": "2008-02-27T23:37:42Z",
    "created_at": "2008-02-27T20:35:44Z",
    "labels": [
        "component: user interface",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.10.3",
    "title": "[with patch, positive review] fix typo in save_session",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2337",
    "user": "https://github.com/williamstein"
}
```

```
sage: save_session?

[stuff deleted]

       Save all variables that can be saved wto the given filename.
The

^

******
Change "wto" to "to".

Kate
```

Assignee: @williamstein

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/2337





---

archive/issue_events_008106.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2337",
    "rename": {
        "from": "[with patch; needs review] fix typo in save_session",
        "to": "[with patch, positive review] fix typo in save_session"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2337#event-8106"
}
```



---

archive/issue_comments_016368.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [sage-2337.patch](tarball://root/attachments/some-uuid/ticket2337/sage-2337.patch) by @mwhansen created at 2008-02-27 21:59:05\n\nWorks for me on 2.10.3.alpha0.",
    "created_at": "2008-02-27T21:59:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2337",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2337#issuecomment-16368",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'></a>
Attachment [sage-2337.patch](tarball://root/attachments/some-uuid/ticket2337/sage-2337.patch) by @mwhansen created at 2008-02-27 21:59:05

Works for me on 2.10.3.alpha0.



---

archive/issue_comments_016369.json:
```json
{
    "body": "<a id='comment:2'></a>\nMerged in Sage 2.10.3.rc0",
    "created_at": "2008-02-27T23:37:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2337",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2337#issuecomment-16369",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Merged in Sage 2.10.3.rc0



---

archive/issue_comments_016370.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-02-27T23:37:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2337",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2337#issuecomment-16370",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_008107.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-02-27T23:37:42Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2337",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2337#event-8107"
}
```
