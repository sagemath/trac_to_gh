# Issue 2158: [with patch, with positive review] Fix some typos in matrix/matrix2.pyx

archive/issues_002158.json:
```json
{
    "body": "The patch fixes a few typos in matrix2.pyx\n\nThe patch will be attached shortly\n\nAssignee: tba\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/2158\n\n",
    "closed_at": "2008-02-14T17:07:57Z",
    "created_at": "2008-02-14T16:56:02Z",
    "labels": [
        "component: documentation",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.10.2",
    "title": "[with patch, with positive review] Fix some typos in matrix/matrix2.pyx",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2158",
    "user": "https://github.com/jaapspies"
}
```
The patch fixes a few typos in matrix2.pyx

The patch will be attached shortly

Assignee: tba

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/2158





---

archive/issue_comments_014835.json:
```json
{
    "body": "Attachment [matrix2_typos.patch](tarball://root/attachments/some-uuid/ticket2158/matrix2_typos.patch) by @jaapspies created at 2008-02-14 16:57:51",
    "created_at": "2008-02-14T16:57:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2158",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2158#issuecomment-14835",
    "user": "https://github.com/jaapspies"
}
```

Attachment [matrix2_typos.patch](tarball://root/attachments/some-uuid/ticket2158/matrix2_typos.patch) by @jaapspies created at 2008-02-14 16:57:51



---

archive/issue_events_007428.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2158",
    "rename": {
        "from": "Fix some typos in matrix/matrix2.pyx",
        "to": "[with patch] Fix some typos in matrix/matrix2.pyx"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2158#event-7428"
}
```



---

archive/issue_comments_014836.json:
```json
{
    "body": "<a id='comment:2'></a>\nPatch looks good to me.\n\nCheers,\n\nMichael",
    "created_at": "2008-02-14T17:04:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2158",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2158#issuecomment-14836",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Patch looks good to me.

Cheers,

Michael



---

archive/issue_events_007429.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2158",
    "rename": {
        "from": "[with patch] Fix some typos in matrix/matrix2.pyx",
        "to": "[with patch, with positive review] Fix some typos in matrix/matrix2.pyx"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2158#event-7429"
}
```



---

archive/issue_comments_014837.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 2.10.2.alpha0",
    "created_at": "2008-02-14T17:07:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2158",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2158#issuecomment-14837",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 2.10.2.alpha0



---

archive/issue_comments_014838.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-02-14T17:07:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2158",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2158#issuecomment-14838",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_007430.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-02-14T17:07:57Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2158",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2158#event-7430"
}
```
