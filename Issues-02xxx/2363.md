# Issue 2363: Integer.pyx mpz structure fix

archive/issues_002363.json:
```json
{
    "assignees": [
        "https://github.com/garyfurnish"
    ],
    "body": "See 1337. This patch fixes a wrong ordering in a mpz structure used by the integer.pyx memory manager. \n\n\nComponent: **memleak**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/2363_\n\n",
    "closed_at": "2008-03-01T22:39:52Z",
    "created_at": "2008-03-01T20:25:50Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/memleak",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.10.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Integer.pyx mpz structure fix",
    "type": "issue",
    "updated_at": "2008-03-01T22:39:52Z",
    "url": "https://github.com/sagemath/sage/issues/2363",
    "user": "https://github.com/garyfurnish"
}
```
See 1337. This patch fixes a wrong ordering in a mpz structure used by the integer.pyx memory manager. 


Component: **memleak**

_Issue created by migration from https://trac.sagemath.org/ticket/2363_





---

archive/issue_events_024468.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-03-01T20:25:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2363",
    "label": "https://github.com/sagemath/sage/labels/memleak",
    "label_color": "696969",
    "label_name": "memleak",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2363#event-24468"
}
```



---

archive/issue_events_024469.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-03-01T20:25:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2363",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2363#event-24469"
}
```



---

archive/issue_events_024470.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-03-01T20:25:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2363",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2363#event-24470"
}
```



---

archive/issue_events_024471.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-01T20:25:50Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/2363",
    "subject": "https://github.com/garyfurnish",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2363#event-24471"
}
```



---

archive/issue_events_024472.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-03-01T20:25:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2363",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2363#event-24472"
}
```



---

archive/issue_comments_011915.json:
```json
{
    "body": "Attachment: **[integer.pyx.patch.gz](https://github.com/sagemath/sage/files/ticket2363/integer.pyx.patch.gz)**",
    "created_at": "2008-03-01T20:26:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2363",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2363#issuecomment-11915",
    "user": "https://github.com/garyfurnish"
}
```

Attachment: **[integer.pyx.patch.gz](https://github.com/sagemath/sage/files/ticket2363/integer.pyx.patch.gz)**



---

archive/issue_events_024473.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-03-01T20:27:05Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/2363",
    "title_is": "Integer.pyx mpz structure fix",
    "title_was": "No title",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2363#event-24473"
}
```



---

archive/issue_events_024474.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-03-01T20:27:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2363",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2363#event-24474"
}
```



---

archive/issue_events_024475.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-01T20:27:05Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/2363",
    "subject": "https://github.com/garyfurnish",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2363#event-24475"
}
```



---

archive/issue_events_024476.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-03-01T20:27:05Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/2363",
    "subject": "https://github.com/garyfurnish",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2363#event-24476"
}
```



---

archive/issue_events_024477.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-03-01T20:27:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2363",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2363#event-24477"
}
```



---

archive/issue_events_024478.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-03-01T20:27:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2363",
    "milestone_number": null,
    "milestone_title": "sage-2.10.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2363#event-24478"
}
```



---

archive/issue_comments_011916.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nRobert gave this a positive at #1337, so I am moving that review over here.\n\nCheers,\n\nMichael",
    "created_at": "2008-03-01T22:30:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2363",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2363#issuecomment-11916",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Robert gave this a positive at #1337, so I am moving that review over here.

Cheers,

Michael



---

archive/issue_events_024479.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-01T22:30:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2363",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2363#event-24479"
}
```



---

archive/issue_events_024480.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-01T22:39:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2363",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2363#event-24480"
}
```



---

archive/issue_events_024481.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-01T22:39:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2363",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2363#event-24481"
}
```



---

archive/issue_comments_011917.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nMerged in Sage 2.10.3.rc1",
    "created_at": "2008-03-01T22:39:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2363",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2363#issuecomment-11917",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

Merged in Sage 2.10.3.rc1
