# Issue 2534: Sage does not handle Symmetrica's large integers

archive/issues_002534.json:
```json
{
    "assignees": [
        "https://github.com/mwhansen"
    ],
    "body": "This causes problems when working with larger partitions.  For example,\n\n```\nsage: s = SFASchur(QQ)\nsage: a = s([8,8])\nsage: a.itensor(a)\n```\ngives the wrong results.\n\nComponent: **interfaces**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/2534_\n\n",
    "closed_at": "2008-03-16T05:23:43Z",
    "created_at": "2008-03-15T23:36:24Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20interfaces",
        "https://github.com/sagemath/sage/labels/p%3A%202%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.10.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Sage does not handle Symmetrica's large integers",
    "type": "issue",
    "updated_at": "2008-03-16T05:23:43Z",
    "url": "https://github.com/sagemath/sage/issues/2534",
    "user": "https://github.com/mwhansen"
}
```
This causes problems when working with larger partitions.  For example,

```
sage: s = SFASchur(QQ)
sage: a = s([8,8])
sage: a.itensor(a)
```
gives the wrong results.

Component: **interfaces**

_Issue created by migration from https://trac.sagemath.org/ticket/2534_





---

archive/issue_events_026524.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-03-15T23:36:24Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2534",
    "milestone_number": null,
    "milestone_title": "sage-2.10.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2534#event-26524"
}
```



---

archive/issue_events_026525.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-03-15T23:36:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2534",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20interfaces",
    "label_color": "0000ff",
    "label_name": "c: interfaces",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2534#event-26525"
}
```



---

archive/issue_events_026526.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-03-15T23:36:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2534",
    "label": "https://github.com/sagemath/sage/labels/p%3A%202%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p: 2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2534#event-26526"
}
```



---

archive/issue_events_026527.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-03-15T23:36:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2534",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2534#event-26527"
}
```



---

archive/issue_events_026528.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-03-15T23:36:24Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/2534",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2534#event-26528"
}
```



---

archive/issue_events_026529.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-03-15T23:36:45Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/2534",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2534#event-26529"
}
```



---

archive/issue_events_026530.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-03-15T23:36:45Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/2534",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2534#event-26530"
}
```



---

archive/issue_comments_013001.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nI have a fix for this, but cannot get the Cython generated code to compile.",
    "created_at": "2008-03-15T23:36:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2534#issuecomment-13001",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:1" align="right">comment:1</div>

I have a fix for this, but cannot get the Cython generated code to compile.



---

archive/issue_comments_013002.json:
```json
{
    "body": "Attachment: **[2534.patch.gz](https://github.com/sagemath/sage/files/ticket2534/2534.patch.gz)**",
    "created_at": "2008-03-16T03:32:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2534#issuecomment-13002",
    "user": "https://github.com/mwhansen"
}
```

Attachment: **[2534.patch.gz](https://github.com/sagemath/sage/files/ticket2534/2534.patch.gz)**



---

archive/issue_events_026531.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-03-16T03:33:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2534",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2534#event-26531"
}
```



---

archive/issue_comments_013003.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nAttachment: **[2534.2.patch.gz](https://github.com/sagemath/sage/files/ticket2534/2534.2.patch.gz)**\n\nOnly apply 2534.2.patch which was made against 2.10.4.alpha0.",
    "created_at": "2008-03-16T04:23:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2534#issuecomment-13003",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:3" align="right">comment:3</div>

Attachment: **[2534.2.patch.gz](https://github.com/sagemath/sage/files/ticket2534/2534.2.patch.gz)**

Only apply 2534.2.patch which was made against 2.10.4.alpha0.



---

archive/issue_comments_013004.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nPatch looks good to me, testall shows no problem.",
    "created_at": "2008-03-16T05:07:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2534#issuecomment-13004",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

Patch looks good to me, testall shows no problem.



---

archive/issue_events_026532.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-16T05:07:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2534",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2534#event-26532"
}
```



---

archive/issue_events_026533.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-16T05:07:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2534",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2534#event-26533"
}
```



---

archive/issue_comments_013005.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nMerged in Sage 2.10.4.rc0 - I will valgrind this once rc0 is out.",
    "created_at": "2008-03-16T05:23:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2534#issuecomment-13005",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:5" align="right">comment:5</div>

Merged in Sage 2.10.4.rc0 - I will valgrind this once rc0 is out.



---

archive/issue_events_026534.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-16T05:23:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2534",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2534#event-26534"
}
```



---

archive/issue_events_026535.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-16T05:23:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2534",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2534#event-26535"
}
```
