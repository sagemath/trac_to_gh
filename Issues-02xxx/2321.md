# Issue 2321: remove dsage_server.py

archive/issues_002321.json:
```json
{
    "assignees": [],
    "body": "dsage switched from using dsage_server.py to using a .tac located in .sage/dsage. When we roll out the next version of sage (2.10.3) we need to remove SAGE_ROOT/local/bin/dsage_server.py.\n\nAssignee: **@williamstein**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/2321_\n\n",
    "closed_at": "2008-04-14T00:04:25Z",
    "created_at": "2008-02-26T17:47:20Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20dsage",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "remove dsage_server.py",
    "type": "issue",
    "updated_at": "2008-04-14T00:04:25Z",
    "url": "https://github.com/sagemath/sage/issues/2321",
    "user": "https://github.com/yqiang"
}
```
dsage switched from using dsage_server.py to using a .tac located in .sage/dsage. When we roll out the next version of sage (2.10.3) we need to remove SAGE_ROOT/local/bin/dsage_server.py.

Assignee: **@williamstein**

_Issue created by migration from https://trac.sagemath.org/ticket/2321_





---

archive/issue_events_020592.json:
```json
{
    "actor": "https://github.com/yqiang",
    "created_at": "2008-02-26T17:47:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2321",
    "milestone_number": null,
    "milestone_title": "sage-3.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2321#event-20592"
}
```



---

archive/issue_events_020593.json:
```json
{
    "actor": "https://github.com/yqiang",
    "created_at": "2008-02-26T17:47:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2321",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20dsage",
    "label_color": "0000ff",
    "label_name": "component: dsage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2321#event-20593"
}
```



---

archive/issue_events_020594.json:
```json
{
    "actor": "https://github.com/yqiang",
    "created_at": "2008-02-26T17:47:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2321",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2321#event-20594"
}
```



---

archive/issue_events_020595.json:
```json
{
    "actor": "https://github.com/yqiang",
    "created_at": "2008-02-26T17:47:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2321",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2321#event-20595"
}
```



---

archive/issue_comments_012112.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nYi: Does this depend on any patches still getting merged for 2.10.3 from your end or can I nuke the file now? A patch for this is obviously trivial.\n\nCheers,\n\nMichael",
    "created_at": "2008-02-26T17:58:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2321#issuecomment-12112",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:1'>Comment 1:</a>
Yi: Does this depend on any patches still getting merged for 2.10.3 from your end or can I nuke the file now? A patch for this is obviously trivial.

Cheers,

Michael



---

archive/issue_comments_012113.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nYes, we should not nuke dsage_server.py until we've successfully merged in the huge dsage changes for 2.10.3.",
    "created_at": "2008-02-26T18:09:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2321#issuecomment-12113",
    "user": "https://github.com/yqiang"
}
```

<a id='comment:2'>Comment 2:</a>
Yes, we should not nuke dsage_server.py until we've successfully merged in the huge dsage changes for 2.10.3.



---

archive/issue_comments_012114.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nThe file has been removed, probably in 2.11. Ergo it is fixed.\n\nCheers,\n\nMichael",
    "created_at": "2008-04-14T00:04:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2321#issuecomment-12114",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:3'>Comment 3:</a>
The file has been removed, probably in 2.11. Ergo it is fixed.

Cheers,

Michael



---

archive/issue_events_020596.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-14T00:04:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2321",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2321#event-20596"
}
```
