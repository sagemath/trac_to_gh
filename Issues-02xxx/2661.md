# Issue 2661: [with patch, positive review] sage cannot compute floor / ceil of log(8)/log(2)

archive/issues_002661.json:
```json
{
    "body": "This happens because the interval remains centered around an integer.\n\nAssignee: @williamstein\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/2661\n\n",
    "closed_at": "2008-03-25T04:48:35Z",
    "created_at": "2008-03-24T20:13:01Z",
    "labels": [
        "component: calculus",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.11",
    "title": "[with patch, positive review] sage cannot compute floor / ceil of log(8)/log(2)",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2661",
    "user": "https://github.com/mwhansen"
}
```
This happens because the interval remains centered around an integer.

Assignee: @williamstein

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/2661





---

archive/issue_events_009447.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2661",
    "rename": {
        "from": "sage cannot compute floor / ceil of log(8)/log(2)",
        "to": "[with patch, needs review] sage cannot compute floor / ceil of log(8)/log(2)"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2661#event-9447"
}
```



---

archive/issue_comments_019201.json:
```json
{
    "body": "<a id='comment:2'></a>\nSome concerns about this were raised on IRC.  It seems that the real problem is telling if log(8)/log(2) is an integer.  The (log(8)/log(2)).simplify_log() function tells us that this is 3.",
    "created_at": "2008-03-24T21:12:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2661",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2661#issuecomment-19201",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:2'></a>
Some concerns about this were raised on IRC.  It seems that the real problem is telling if log(8)/log(2) is an integer.  The (log(8)/log(2)).simplify_log() function tells us that this is 3.



---

archive/issue_events_009448.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2661",
    "rename": {
        "from": "[with patch, needs review] sage cannot compute floor / ceil of log(8)/log(2)",
        "to": "[with patch, positive review] sage cannot compute floor / ceil of log(8)/log(2)"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2661#event-9448"
}
```



---

archive/issue_comments_019202.json:
```json
{
    "body": "<a id='comment:3'></a>\nAttachment [2661.patch](tarball://root/attachments/some-uuid/ticket2661/2661.patch) by @jasongrout created at 2008-03-24 21:26:49\n\nlooks good to me.",
    "created_at": "2008-03-24T21:26:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2661",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2661#issuecomment-19202",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:3'></a>
Attachment [2661.patch](tarball://root/attachments/some-uuid/ticket2661/2661.patch) by @jasongrout created at 2008-03-24 21:26:49

looks good to me.



---

archive/issue_comments_019203.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-03-25T04:48:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2661",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2661#issuecomment-19203",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_019204.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 2.11.alpha2",
    "created_at": "2008-03-25T04:48:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2661",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2661#issuecomment-19204",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 2.11.alpha2



---

archive/issue_events_009449.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-03-25T04:48:35Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2661",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2661#event-9449"
}
```
