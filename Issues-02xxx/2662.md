# Issue 2662: support Mathematica options for simplify_full function

archive/issues_002662.json:
```json
{
    "body": "Create a simplify_full function that (somewhat intelligently?) applies a battery simplifications to try to get a function to be simpler.  \n\nIt would be nice to support some of the options that FullSimplify in Mma has (see http://reference.wolfram.com/mathematica/ref/FullSimplify.html?q=fullsimplify&lang=en ):\n\n* user can pass in a \"complexity function\" which determines how simple an expression is\n* user can pass in a time limit for the simplification\n* user can pass in a list of things that won't be simplified\n* etc.\n\n\nAssignee: @garyfurnish\n\nStatus: new\n\nIssue created by migration from https://trac.sagemath.org/ticket/2662\n\n",
    "created_at": "2008-03-24T21:18:35Z",
    "labels": [
        "component: calculus"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-wishlist",
    "title": "support Mathematica options for simplify_full function",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2662",
    "user": "https://github.com/jasongrout"
}
```
Create a simplify_full function that (somewhat intelligently?) applies a battery simplifications to try to get a function to be simpler.  

It would be nice to support some of the options that FullSimplify in Mma has (see http://reference.wolfram.com/mathematica/ref/FullSimplify.html?q=fullsimplify&lang=en ):

* user can pass in a "complexity function" which determines how simple an expression is
* user can pass in a time limit for the simplification
* user can pass in a list of things that won't be simplified
* etc.


Assignee: @garyfurnish

Status: new

Issue created by migration from https://trac.sagemath.org/ticket/2662





---

archive/issue_comments_019764.json:
```json
{
    "body": "<a id='comment:1'></a>mhansen created a simplify_full function in #2661.  So how about this ticket is the enhancement ticket for putting the Mma options that we can into full_simplify.",
    "created_at": "2008-03-24T21:27:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2662#issuecomment-19764",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:1'></a>mhansen created a simplify_full function in #2661.  So how about this ticket is the enhancement ticket for putting the Mma options that we can into full_simplify.



---

archive/issue_comments_019765.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-03-25T00:39:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2662#issuecomment-19765",
    "user": "https://github.com/garyfurnish"
}
```

Changing status from new to assigned.



---

archive/issue_comments_019766.json:
```json
{
    "body": "Changing assignee from @williamstein to @garyfurnish.",
    "created_at": "2008-03-25T00:39:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2662#issuecomment-19766",
    "user": "https://github.com/garyfurnish"
}
```

Changing assignee from @williamstein to @garyfurnish.



---

archive/issue_comments_019767.json:
```json
{
    "body": "<a id='comment:3'></a>This definately *has* to be handled better in new symbolics then it is currently.",
    "created_at": "2008-03-25T00:39:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2662#issuecomment-19767",
    "user": "https://github.com/garyfurnish"
}
```

<a id='comment:3'></a>This definately *has* to be handled better in new symbolics then it is currently.



---

archive/issue_comments_019768.json:
```json
{
    "body": "Changing upstream from \"\" to \"N/A\"",
    "created_at": "2009-12-30T04:01:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2662#issuecomment-19768",
    "user": "https://github.com/kcrisman"
}
```

Changing upstream from "" to "N/A"



---

archive/issue_comments_019769.json:
```json
{
    "body": "<a id='comment:4'></a>Note also there are several other open tickets about the various simplify functions, with no resolution yet on what our \"real\" options should be.",
    "created_at": "2009-12-30T04:01:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2662#issuecomment-19769",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:4'></a>Note also there are several other open tickets about the various simplify functions, with no resolution yet on what our "real" options should be.



---

archive/issue_events_006228.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-05-11T18:15:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/2662",
    "milestone": "sage-wishlist",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2662#event-6228"
}
```



---

archive/issue_comments_019770.json:
```json
{
    "body": "<a id='comment:6'></a>#13099 is related.",
    "created_at": "2012-12-04T16:57:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2662#issuecomment-19770",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:6'></a>#13099 is related.
