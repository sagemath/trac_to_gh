# Issue 2688: Kuratowski subgraph isolator for planarity checking

archive/issues_002688.json:
```json
{
    "assignees": [],
    "body": "\n\nAssignee: **@rlmill**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/2688_\n\n",
    "closed_at": "2008-03-29T21:54:43Z",
    "created_at": "2008-03-27T20:54:59Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.11",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Kuratowski subgraph isolator for planarity checking",
    "type": "issue",
    "updated_at": "2008-03-29T21:54:43Z",
    "url": "https://github.com/sagemath/sage/issues/2688",
    "user": "https://github.com/rlmill"
}
```


Assignee: **@rlmill**

_Issue created by migration from https://trac.sagemath.org/ticket/2688_





---

archive/issue_events_024601.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-03-27T20:54:59Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2688",
    "milestone_number": null,
    "milestone_title": "sage-2.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2688#event-24601"
}
```



---

archive/issue_events_024602.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-03-27T20:54:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2688",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2688#event-24602"
}
```



---

archive/issue_events_024603.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-03-27T20:54:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2688",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2688#event-24603"
}
```



---

archive/issue_events_024604.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-03-27T20:54:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2688",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2688#event-24604"
}
```



---

archive/issue_comments_014717.json:
```json
{
    "body": "Attachment: **[2688-kuratowski-isolator.patch.gz](https://github.com/sagemath/sage/files/ticket2688/2688-kuratowski-isolator.patch.gz)**",
    "created_at": "2008-03-29T05:56:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2688#issuecomment-14717",
    "user": "https://github.com/sagetrac-ekirkman"
}
```

Attachment: **[2688-kuratowski-isolator.patch.gz](https://github.com/sagemath/sage/files/ticket2688/2688-kuratowski-isolator.patch.gz)**



---

archive/issue_events_024605.json:
```json
{
    "actor": "https://github.com/sagetrac-ekirkman",
    "created_at": "2008-03-29T05:57:46Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/2688",
    "title_is": "[needs easy review] Kuratowski subgraph isolator for planarity checking",
    "title_was": "Kuratowski subgraph isolator for planarity checking",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2688#event-24605"
}
```



---

archive/issue_comments_014718.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nPasses all tests after applying cleanly to 2.11.alpha1. I'll give it a try on alpha2 once it finishes building.",
    "created_at": "2008-03-29T17:21:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2688#issuecomment-14718",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:2'>Comment 2:</a>
Passes all tests after applying cleanly to 2.11.alpha1. I'll give it a try on alpha2 once it finishes building.



---

archive/issue_events_024606.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-03-29T17:21:49Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/2688",
    "title_is": "Kuratowski subgraph isolator for planarity checking",
    "title_was": "[needs easy review] Kuratowski subgraph isolator for planarity checking",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2688#event-24606"
}
```



---

archive/issue_events_024607.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-03-29T17:21:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2688",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2688#event-24607"
}
```



---

archive/issue_events_024608.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-29T18:28:24Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/2688",
    "title_is": "[needs doctest fix] Kuratowski subgraph isolator for planarity checking",
    "title_was": "Kuratowski subgraph isolator for planarity checking",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2688#event-24608"
}
```



---

archive/issue_comments_014719.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nWell, against my 2.11.rc0 build I got the following doctest failure:\n\n```\nsage -t  devel/sage-main/sage/graphs/graph.py\n**********************************************************************\nFile \"graph.py\", line 1695:\n    sage: cube.is_circular_planar()\nExpected:\n    False\nGot:\n    (False, Graph on 9 vertices)\n**********************************************************************\n```\nIt seems easy enough to fix. Care to update the patch?\n\nCheers,\n\nMichael",
    "created_at": "2008-03-29T18:28:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2688#issuecomment-14719",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:3'>Comment 3:</a>
Well, against my 2.11.rc0 build I got the following doctest failure:

```
sage -t  devel/sage-main/sage/graphs/graph.py
**********************************************************************
File "graph.py", line 1695:
    sage: cube.is_circular_planar()
Expected:
    False
Got:
    (False, Graph on 9 vertices)
**********************************************************************
```
It seems easy enough to fix. Care to update the patch?

Cheers,

Michael



---

archive/issue_events_024609.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-03-29T20:45:35Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/2688",
    "title_is": "Kuratowski subgraph isolator for planarity checking",
    "title_was": "[needs doctest fix] Kuratowski subgraph isolator for planarity checking",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2688#event-24609"
}
```



---

archive/issue_comments_014720.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nAttachment: **[2688-fix.patch.gz](https://github.com/sagemath/sage/files/ticket2688/2688-fix.patch.gz)**",
    "created_at": "2008-03-29T20:45:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2688#issuecomment-14720",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:4'>Comment 4:</a>
Attachment: **[2688-fix.patch.gz](https://github.com/sagemath/sage/files/ticket2688/2688-fix.patch.gz)**



---

archive/issue_comments_014721.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nMerged 2688-kuratowski-isolator.patch and 2688-fix.patch in Sage 2.11.rc0",
    "created_at": "2008-03-29T21:54:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2688#issuecomment-14721",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:5'>Comment 5:</a>
Merged 2688-kuratowski-isolator.patch and 2688-fix.patch in Sage 2.11.rc0



---

archive/issue_events_024610.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-29T21:54:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2688",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2688#event-24610"
}
```



---

archive/issue_events_024611.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-29T21:54:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2688",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2688#event-24611"
}
```
