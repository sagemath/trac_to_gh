# Issue 2492: [with patch, with positive review] Reference Manual: 18.2 Functions that compute some of the sequences in Sloane's tables

archive/issues_002492.json:
```json
{
    "body": "There seems to be a lot of duplication now coverage is at 100%\n\nMaybe the Examples in __init__ and friends should be suppressed?\n\nJaap\n\n\n\nAssignee: @mwhansen\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/2492\n\n",
    "closed_at": "2008-03-16T23:10:48Z",
    "created_at": "2008-03-12T14:21:34Z",
    "labels": [
        "component: documentation",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.10.4",
    "title": "[with patch, with positive review] Reference Manual: 18.2 Functions that compute some of the sequences in Sloane's tables",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2492",
    "user": "https://github.com/jaapspies"
}
```
There seems to be a lot of duplication now coverage is at 100%

Maybe the Examples in __init__ and friends should be suppressed?

Jaap



Assignee: @mwhansen

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/2492





---

archive/issue_comments_017645.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-03-16T21:02:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2492",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2492#issuecomment-17645",
    "user": "https://github.com/mwhansen"
}
```

Changing status from new to assigned.



---

archive/issue_comments_017646.json:
```json
{
    "body": "Changing assignee from tba to @mwhansen.",
    "created_at": "2008-03-16T21:02:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2492",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2492#issuecomment-17646",
    "user": "https://github.com/mwhansen"
}
```

Changing assignee from tba to @mwhansen.



---

archive/issue_events_008780.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2492",
    "rename": {
        "from": "Reference Manual: 18.2 Functions that compute some of the sequences in Sloane's tables",
        "to": "[with patch, needs review] Reference Manual: 18.2 Functions that compute some of the sequences in Sloane's tables"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2492#event-8780"
}
```



---

archive/issue_comments_017647.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [2492.patch](tarball://root/attachments/some-uuid/ticket2492/2492.patch) by @mwhansen created at 2008-03-16 21:02:24\n\nI've attached a patch which should be applied to hg_doc.",
    "created_at": "2008-03-16T21:02:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2492",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2492#issuecomment-17647",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'></a>
Attachment [2492.patch](tarball://root/attachments/some-uuid/ticket2492/2492.patch) by @mwhansen created at 2008-03-16 21:02:24

I've attached a patch which should be applied to hg_doc.



---

archive/issue_comments_017648.json:
```json
{
    "body": "<a id='comment:2'></a>\nPatch looks good to me and should be merged into 2.10.4.final.\n\nCheers,\n\nMichael",
    "created_at": "2008-03-16T21:20:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2492",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2492#issuecomment-17648",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Patch looks good to me and should be merged into 2.10.4.final.

Cheers,

Michael



---

archive/issue_events_008781.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2492",
    "rename": {
        "from": "[with patch, needs review] Reference Manual: 18.2 Functions that compute some of the sequences in Sloane's tables",
        "to": "[with patch, with positive review] Reference Manual: 18.2 Functions that compute some of the sequences in Sloane's tables"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2492#event-8781"
}
```



---

archive/issue_events_008782.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-03-16T23:10:48Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2492",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2492#event-8782"
}
```



---

archive/issue_comments_017649.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 2.10.4.final",
    "created_at": "2008-03-16T23:10:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2492",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2492#issuecomment-17649",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 2.10.4.final



---

archive/issue_comments_017650.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-03-16T23:10:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2492",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2492#issuecomment-17650",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed
