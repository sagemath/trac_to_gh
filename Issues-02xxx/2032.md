# Issue 2032: [with patch; with positive review] bug in bundle script for sage hg repo

archive/issues_002032.json:
```json
{
    "body": "\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/2032\n\n",
    "closed_at": "2008-02-04T05:02:57Z",
    "created_at": "2008-02-02T10:14:19Z",
    "labels": [
        "component: packages: standard",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.10.2",
    "title": "[with patch; with positive review] bug in bundle script for sage hg repo",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2032",
    "user": "https://github.com/williamstein"
}
```


Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/2032





---

archive/issue_comments_013720.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [trac-2032.patch](tarball://root/attachments/some-uuid/ticket2032/trac-2032.patch) by @williamstein created at 2008-02-02 10:15:11",
    "created_at": "2008-02-02T10:15:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2032",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2032#issuecomment-13720",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'></a>
Attachment [trac-2032.patch](tarball://root/attachments/some-uuid/ticket2032/trac-2032.patch) by @williamstein created at 2008-02-02 10:15:11



---

archive/issue_events_006945.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2032",
    "rename": {
        "from": "bug in bundle script for sage hg repo",
        "to": "[with patch; needs review] bug in bundle script for sage hg repo"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2032#event-6945"
}
```



---

archive/issue_comments_013721.json:
```json
{
    "body": "<a id='comment:2'></a>\nPatch looks good to me.\n\nCheers,\n\nMichael",
    "created_at": "2008-02-03T16:34:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2032",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2032#issuecomment-13721",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Patch looks good to me.

Cheers,

Michael



---

archive/issue_events_006946.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2032",
    "rename": {
        "from": "[with patch; needs review] bug in bundle script for sage hg repo",
        "to": "[with patch; with positive review] bug in bundle script for sage hg repo"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2032#event-6946"
}
```



---

archive/issue_comments_013722.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 2.10.2.alpha0",
    "created_at": "2008-02-04T05:02:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2032",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2032#issuecomment-13722",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 2.10.2.alpha0



---

archive/issue_comments_013723.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-02-04T05:02:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2032",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2032#issuecomment-13723",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_006947.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-02-04T05:02:57Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2032",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2032#event-6947"
}
```
