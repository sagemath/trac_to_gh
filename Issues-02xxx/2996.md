# Issue 2996: lite webpage broken under Konqueror

archive/issues_002996.json:
```json
{
    "assignees": [],
    "body": "With KDE 3.5.9, the background color of the window defaults to white, not purple.  \n\nAssignee: **@williamstein**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/2996_\n\n",
    "closed_at": "2008-08-31T05:19:27Z",
    "created_at": "2008-04-22T05:49:13Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20website/wiki",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.1.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "lite webpage broken under Konqueror",
    "type": "issue",
    "updated_at": "2008-08-31T05:22:36Z",
    "url": "https://github.com/sagemath/sage/issues/2996",
    "user": "https://github.com/garyfurnish"
}
```
With KDE 3.5.9, the background color of the window defaults to white, not purple.  

Assignee: **@williamstein**

_Issue created by migration from https://trac.sagemath.org/ticket/2996_





---

archive/issue_events_028158.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-04-22T05:49:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2996",
    "milestone_number": null,
    "milestone_title": "sage-3.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2996#event-28158"
}
```



---

archive/issue_events_028159.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-04-22T05:49:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2996",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20website/wiki",
    "label_color": "0075ca",
    "label_name": "component: website/wiki",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2996#event-28159"
}
```



---

archive/issue_events_028160.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-04-22T05:49:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2996",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2996#event-28160"
}
```



---

archive/issue_events_028161.json:
```json
{
    "actor": "https://github.com/garyfurnish",
    "created_at": "2008-04-22T05:49:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2996",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2996#event-28161"
}
```



---

archive/issue_comments_016424.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nLooks identical to Firefox to me. I vote for invalid or fixed.",
    "created_at": "2008-08-18T17:30:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2996",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2996#issuecomment-16424",
    "user": "https://github.com/malb"
}
```

<a id='comment:1'>Comment 1:</a>
Looks identical to Firefox to me. I vote for invalid or fixed.



---

archive/issue_comments_016425.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nFixed it is. If someone can reproduce this please let us know which konqueror version is broken.\n\nCheers,\n\nMichael",
    "created_at": "2008-08-31T05:19:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2996",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2996#issuecomment-16425",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:2'>Comment 2:</a>
Fixed it is. If someone can reproduce this please let us know which konqueror version is broken.

Cheers,

Michael



---

archive/issue_events_028162.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-08-31T05:19:27Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2996",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2996#event-28162"
}
```



---

archive/issue_comments_016426.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nMike Hansen just confirmed that the color issue is fixed with Konqueror 3.5.9.\n\nCheers,\n\nMichael",
    "created_at": "2008-08-31T05:22:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2996",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2996#issuecomment-16426",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:3'>Comment 3:</a>
Mike Hansen just confirmed that the color issue is fixed with Konqueror 3.5.9.

Cheers,

Michael
