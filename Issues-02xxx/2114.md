# Issue 2114: Get gf2x version 1.1 into Sage!

archive/issues_002114.json:
```json
{
    "assignees": [],
    "body": "Check out http://wwwmaths.anu.edu.au/~brent/gf2x.html\n\nIt's:\n* by very well respected people\n* GPL'd (v2 or later)\n* Small pure C code\n* and Paul Z. says: \n\n```\nfor your information, on http://wwwmaths.anu.edu.au/~brent/gf2x.html you will\nfind an implementation up to 5 times faster than NTL's GF2X (for degree 2^20).\n```\n\nLatest 1.1 version is at http://gf2x.gforge.inria.fr/\n\nUse **spkgs** at:\n* [http://boxen.math.washington.edu/home/jdemeyer/spkg/gf2x-1.1.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/gf2x-1.1.spkg) ([spkg diff](https://github.com/sagemath/sage/files/ticket2114/gf2x-1.1.diff.gz))\n* [http://boxen.math.washington.edu/home/jdemeyer/spkg/ntl-5.5.2.p1.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/ntl-5.5.2.p1.spkg)\n\n**Apply** to Sage's root:\n* [attachment: trac_2114-gf2x.patch](https://github.com/sagemath/sage/files/ticket2114/trac_2114-gf2x.patch.gz)\n\n\nDepends on #12447\n\nCC:  @zimmermann6 @malb @jpflori @jdemeyer\n\nKeywords: **spkg gf2x**\n\nReviewer: **Jeroen Demeyer**\n\nAuthor: **Jean-Pierre Flori**\n\nMerged: **sage-5.11.beta1**\n\nComponent: **packages: standard**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/2114_\n\n",
    "closed_at": "2013-06-10T08:41:43Z",
    "created_at": "2008-02-08T15:12:20Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.11",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Get gf2x version 1.1 into Sage!",
    "type": "issue",
    "updated_at": "2013-06-10T08:41:43Z",
    "url": "https://github.com/sagemath/sage/issues/2114",
    "user": "https://github.com/williamstein"
}
```
Check out http://wwwmaths.anu.edu.au/~brent/gf2x.html

It's:
* by very well respected people
* GPL'd (v2 or later)
* Small pure C code
* and Paul Z. says: 

```
for your information, on http://wwwmaths.anu.edu.au/~brent/gf2x.html you will
find an implementation up to 5 times faster than NTL's GF2X (for degree 2^20).
```

Latest 1.1 version is at http://gf2x.gforge.inria.fr/

Use **spkgs** at:
* [http://boxen.math.washington.edu/home/jdemeyer/spkg/gf2x-1.1.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/gf2x-1.1.spkg) ([spkg diff](https://github.com/sagemath/sage/files/ticket2114/gf2x-1.1.diff.gz))
* [http://boxen.math.washington.edu/home/jdemeyer/spkg/ntl-5.5.2.p1.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/ntl-5.5.2.p1.spkg)

**Apply** to Sage's root:
* [attachment: trac_2114-gf2x.patch](https://github.com/sagemath/sage/files/ticket2114/trac_2114-gf2x.patch.gz)


Depends on #12447

CC:  @zimmermann6 @malb @jpflori @jdemeyer

Keywords: **spkg gf2x**

Reviewer: **Jeroen Demeyer**

Author: **Jean-Pierre Flori**

Merged: **sage-5.11.beta1**

Component: **packages: standard**

_Issue created by migration from https://trac.sagemath.org/ticket/2114_





---

archive/issue_events_021510.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-02-08T15:12:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "milestone_number": null,
    "milestone_title": "sage-5.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21510"
}
```



---

archive/issue_events_021511.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-02-08T15:12:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "c: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21511"
}
```



---

archive/issue_events_021512.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-02-08T15:12:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21512"
}
```



---

archive/issue_events_021513.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-02-08T15:12:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21513"
}
```



---

archive/issue_comments_010168.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\n\n```\nEmmanuel Thom\u00e9 to me, Paul, Pierrick\n\t\nshow details 2:53 PM (5 hours ago) [gf2x-20070214.tar.gz]\n\t\n\t\nReply\n\t\n\t\n\tfrom\tEmmanuel Thom\u00e9 <Emmanuel.Thome@normalesup.org>\nto\twstein@gmail.com,\ncc\tPaul Zimmermann <Paul.Zimmermann@loria.fr>,\nPierrick Gaudry <gaudry@lix.polytechnique.fr>,\ndate\tThu, Feb 14, 2008 at 2:53 PM\nsubject\tgf2x package\n\t\nhide details 2:53 PM (5 hours ago) [gf2x-20070214.tar.gz]\n\t\n\t\nReply\n\t\n\t\nOn Thu, Feb 14, 2008 at 06:12:34PM +0100, Paul Zimmermann wrote:\n> https://github.com/sagemath/sage/issues/2114\n\nFor your information, you might find the attached version more\npackageable (it can live outside NTL, for instance). Yet, the build\nprocess is still tricky: Auto tuning and so on. Depending on your\npreferred practices for sage, your preferred option could be to\nstatically save the thresholds file per-machine.\n\nRegards,\n\nE.\n```\n\nThe \"attached version\" is here:\n   http://sage.math.washington.edu/home/was/tmp/gf2x-20070214.tar.gz",
    "created_at": "2008-02-15T04:42:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10168",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:1" align="right">comment:1</div>


```
Emmanuel Thomé to me, Paul, Pierrick
	
show details 2:53 PM (5 hours ago) [gf2x-20070214.tar.gz]
	
	
Reply
	
	
	from	Emmanuel Thomé <Emmanuel.Thome@normalesup.org>
to	wstein@gmail.com,
cc	Paul Zimmermann <Paul.Zimmermann@loria.fr>,
Pierrick Gaudry <gaudry@lix.polytechnique.fr>,
date	Thu, Feb 14, 2008 at 2:53 PM
subject	gf2x package
	
hide details 2:53 PM (5 hours ago) [gf2x-20070214.tar.gz]
	
	
Reply
	
	
On Thu, Feb 14, 2008 at 06:12:34PM +0100, Paul Zimmermann wrote:
> https://github.com/sagemath/sage/issues/2114

For your information, you might find the attached version more
packageable (it can live outside NTL, for instance). Yet, the build
process is still tricky: Auto tuning and so on. Depending on your
preferred practices for sage, your preferred option could be to
statically save the thresholds file per-machine.

Regards,

E.
```

The "attached version" is here:
   http://sage.math.washington.edu/home/was/tmp/gf2x-20070214.tar.gz



---

archive/issue_comments_010169.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nA new version (0.3.1) is available from http://wwwmaths.anu.edu.au/~brent/gf2x.html.",
    "created_at": "2008-10-18T11:43:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10169",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:2" align="right">comment:2</div>

A new version (0.3.1) is available from http://wwwmaths.anu.edu.au/~brent/gf2x.html.



---

archive/issue_comments_010170.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nGF2X has now its own development page: http://gf2x.gforge.inria.fr/.\nIt should be easier to incorporate into Sage now, but maybe it would be better\nto include it through NTL, since since NTL 5.5, NTL can be configured to use GF2X as\nauxiliary package. In such a way, Sage will benefit from GF2X for all computations with NTL.",
    "created_at": "2009-11-27T10:13:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10170",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:4" align="right">comment:4</div>

GF2X has now its own development page: http://gf2x.gforge.inria.fr/.
It should be easier to incorporate into Sage now, but maybe it would be better
to include it through NTL, since since NTL 5.5, NTL can be configured to use GF2X as
auxiliary package. In such a way, Sage will benefit from GF2X for all computations with NTL.



---

archive/issue_comments_010171.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nNote that since ntl-5.5, NTL can now be configured to use GF2X instead of its own routines.\nThis is probably the easiest way to get GF2X into Sage.",
    "created_at": "2010-02-25T09:06:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10171",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:5" align="right">comment:5</div>

Note that since ntl-5.5, NTL can now be configured to use GF2X instead of its own routines.
This is probably the easiest way to get GF2X into Sage.



---

archive/issue_comments_010172.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nPaul, I'm finally packaging this, only planning to build NTL on top of it, no \"native\" interface.\nIs tuning still highly recommended? Because it really takes a long time.\nI was thinking of providing an option (GF2X_TUNE=yes/no, any better idea?) to enable/disable it, not sure what the default should be.\nThe README suggests it would be yes by default.",
    "created_at": "2013-06-04T14:10:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10172",
    "user": "https://github.com/jpflori"
}
```

<div id="comment:7" align="right">comment:7</div>

Paul, I'm finally packaging this, only planning to build NTL on top of it, no "native" interface.
Is tuning still highly recommended? Because it really takes a long time.
I was thinking of providing an option (GF2X_TUNE=yes/no, any better idea?) to enable/disable it, not sure what the default should be.
The README suggests it would be yes by default.



---

archive/issue_comments_010173.json:
```json
{
    "body": "Changed keywords from none to **spkg gf2x**",
    "created_at": "2013-06-04T15:01:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10173",
    "user": "https://github.com/jpflori"
}
```

Changed keywords from none to **spkg gf2x**



---

archive/issue_comments_010174.json:
```json
{
    "body": "Author: **Jean-Pierre Flori**",
    "created_at": "2013-06-04T15:01:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10174",
    "user": "https://github.com/jpflori"
}
```

Author: **Jean-Pierre Flori**



---

archive/issue_comments_010175.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -42,4 +42,9 @@\n find an implementation up to 5 times faster than NTL's GF2X (for degree 2^20).\n ```\n \n+Latest 1.1 version is at http://gf2x.gforge.inria.fr/\n \n+Use spkgs at:\n+* http://boxen.math.washington.edu/home/jpflori/spkg/gf2x-1.1.spkg\n+* http://boxen.math.washington.edu/home/jpflori/spkg/ntl-5.5.2.p1.spkg\n+\n``````\n",
    "created_at": "2013-06-04T15:01:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10175",
    "user": "https://github.com/jpflori"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -42,4 +42,9 @@
 find an implementation up to 5 times faster than NTL's GF2X (for degree 2^20).
 ```
 
+Latest 1.1 version is at http://gf2x.gforge.inria.fr/
 
+Use spkgs at:
+* http://boxen.math.washington.edu/home/jpflori/spkg/gf2x-1.1.spkg
+* http://boxen.math.washington.edu/home/jpflori/spkg/ntl-5.5.2.p1.spkg
+
``````




---

archive/issue_events_021514.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-06-04T15:01:38Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "title_is": "Get gf2x version 1.1 into Sage!",
    "title_was": "get gf2x into Sage!",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21514"
}
```



---

archive/issue_comments_010176.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nUpped spkgs, with the GF2X_TUNE option off by default (took one hour on a quite recent Xeon (with only one thread, but I did not test whether tuning gets parallelized)).\nSet it to yes to perform tuning.\n\nI did not check this actually speeds up NTL, anyone wanting to benchmark the new NTL spkg against the old one please go ahead.\n\nAs NTL is a standard spkg, not sure what the way to go is.\nJeroen please decide what to do.",
    "created_at": "2013-06-04T15:01:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10176",
    "user": "https://github.com/jpflori"
}
```

<div id="comment:8" align="right">comment:8</div>

Upped spkgs, with the GF2X_TUNE option off by default (took one hour on a quite recent Xeon (with only one thread, but I did not test whether tuning gets parallelized)).
Set it to yes to perform tuning.

I did not check this actually speeds up NTL, anyone wanting to benchmark the new NTL spkg against the old one please go ahead.

As NTL is a standard spkg, not sure what the way to go is.
Jeroen please decide what to do.



---

archive/issue_events_021515.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-06-04T15:01:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21515"
}
```



---

archive/issue_comments_010177.json:
```json
{
    "body": "Spkg diff, for review only.",
    "created_at": "2013-06-04T15:02:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10177",
    "user": "https://github.com/jpflori"
}
```

Spkg diff, for review only.



---

archive/issue_events_021516.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-04T20:08:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21516"
}
```



---

archive/issue_events_021517.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-04T20:08:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21517"
}
```



---

archive/issue_comments_010178.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nAttachment: **[ntl-5.5.2.p1.diff.gz](https://github.com/sagemath/sage/files/ticket2114/ntl-5.5.2.p1.diff.gz)**\n\nReplying to [@jpflori](#comment:8):\n> Jeroen please decide what to do.\n\nWhy should I?  *You* should decide if you want this:\na. as standard package (which requires a discussion on `sage-devel` and changes to `spkg/standard/deps`)\nb. as optional package (but then NTL needs to work both with and without `gf2x`)\nc. not at all",
    "created_at": "2013-06-04T20:08:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10178",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:9" align="right">comment:9</div>

Attachment: **[ntl-5.5.2.p1.diff.gz](https://github.com/sagemath/sage/files/ticket2114/ntl-5.5.2.p1.diff.gz)**

Replying to [@jpflori](#comment:8):
> Jeroen please decide what to do.

Why should I?  *You* should decide if you want this:
a. as standard package (which requires a discussion on `sage-devel` and changes to `spkg/standard/deps`)
b. as optional package (but then NTL needs to work both with and without `gf2x`)
c. not at all



---

archive/issue_comments_010179.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">comment:10</div>\n\nI say we want this as a standard package and would prefer to avoid an optional stage (just imagine we patch ntl inbetween.\nBut I thought we needed optional before standard for all packages, so in this case this would be complicated.\nSo I'll post on sage-devel.",
    "created_at": "2013-06-04T22:11:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10179",
    "user": "https://github.com/jpflori"
}
```

<div id="comment:10" align="right">comment:10</div>

I say we want this as a standard package and would prefer to avoid an optional stage (just imagine we patch ntl inbetween.
But I thought we needed optional before standard for all packages, so in this case this would be complicated.
So I'll post on sage-devel.



---

archive/issue_comments_010180.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">comment:11</div>\n\nSo does it actually speed up things, considering that we disable tuning? If not then forget about it. If yes then I'd be happy to see it included.",
    "created_at": "2013-06-04T23:18:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10180",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:11" align="right">comment:11</div>

So does it actually speed up things, considering that we disable tuning? If not then forget about it. If yes then I'd be happy to see it included.



---

archive/issue_comments_010181.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">comment:12</div>\n\nJust for hint, my experience playing with FLINT, using gf2x for GF(2) polynomials instead of the basic implementation inside FLINT using long gave an incredible speedup.\nAnd from the maybe 4 years ago comments in the ticket description, it should also seepdup NTL.\nI just don't have the time or the motivation to benchmark products of GF(2**n) elements (once you don't use Givaro) right now to prove it is beneficial.",
    "created_at": "2013-06-04T23:26:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10181",
    "user": "https://github.com/jpflori"
}
```

<div id="comment:12" align="right">comment:12</div>

Just for hint, my experience playing with FLINT, using gf2x for GF(2) polynomials instead of the basic implementation inside FLINT using long gave an incredible speedup.
And from the maybe 4 years ago comments in the ticket description, it should also seepdup NTL.
I just don't have the time or the motivation to benchmark products of GF(2**n) elements (once you don't use Givaro) right now to prove it is beneficial.



---

archive/issue_comments_010182.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">comment:13</div>\n\nReplying to [@jpflori](#comment:7):\n> Paul, I'm finally packaging this, only planning to build NTL on top of it, no \"native\" interface.\n> Is tuning still highly recommended? Because it really takes a long time.\n> I was thinking of providing an option (GF2X_TUNE=yes/no, any better idea?) to enable/disable it, not sure what the default should be.\n> The README suggests it would be yes by default.\n\nJean-Pierre,\n\nmake tune-lowlevel is still highly recommended, since it will choose the best routine up to 9 words.\nIt takes a little more than one minute on my 4-year-old laptop (including recompiling the library).\n\nThen you can do say `make tune-toom TOOM_TUNING_LIMIT=100` to choose the best algorithm up to say\n100 words (i.e., degree < 6400 on a 64-bit computer). It should take a few minutes only.\n\nPaul",
    "created_at": "2013-06-05T08:21:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10182",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:13" align="right">comment:13</div>

Replying to [@jpflori](#comment:7):
> Paul, I'm finally packaging this, only planning to build NTL on top of it, no "native" interface.
> Is tuning still highly recommended? Because it really takes a long time.
> I was thinking of providing an option (GF2X_TUNE=yes/no, any better idea?) to enable/disable it, not sure what the default should be.
> The README suggests it would be yes by default.

Jean-Pierre,

make tune-lowlevel is still highly recommended, since it will choose the best routine up to 9 words.
It takes a little more than one minute on my 4-year-old laptop (including recompiling the library).

Then you can do say `make tune-toom TOOM_TUNING_LIMIT=100` to choose the best algorithm up to say
100 words (i.e., degree < 6400 on a 64-bit computer). It should take a few minutes only.

Paul



---

archive/issue_comments_010183.json:
```json
{
    "body": "Work Issues: **tuning**",
    "created_at": "2013-06-05T09:17:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10183",
    "user": "https://github.com/jpflori"
}
```

Work Issues: **tuning**



---

archive/issue_comments_010184.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">comment:14</div>\n\nOk, I'll do that.\nThanks for the suggestion.",
    "created_at": "2013-06-05T09:17:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10184",
    "user": "https://github.com/jpflori"
}
```

<div id="comment:14" align="right">comment:14</div>

Ok, I'll do that.
Thanks for the suggestion.



---

archive/issue_events_021518.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-06-05T09:20:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21518"
}
```



---

archive/issue_events_021519.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-06-05T09:20:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21519"
}
```



---

archive/issue_comments_010185.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">comment:15</div>\n\nReplying to [@jpflori](#comment:12):\n> Just for hint, my experience playing with FLINT, using gf2x for GF(2) polynomials instead of the basic implementation inside FLINT using long gave an incredible speedup.\n> And from the maybe 4 years ago comments in the ticket description, it should also seepdup NTL.\n> I just don't have the time or the motivation to benchmark products of GF(2**n) elements (once you don't use Givaro) right now to prove it is beneficial.\n\nOn my pc, multiplying two random elements of GF(2**10000) goes from 103us in Sage 5.9 with old NTL to 21.4us in 5.10.rc0 with NTL+gf2x.",
    "created_at": "2013-06-05T09:20:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10185",
    "user": "https://github.com/jpflori"
}
```

<div id="comment:15" align="right">comment:15</div>

Replying to [@jpflori](#comment:12):
> Just for hint, my experience playing with FLINT, using gf2x for GF(2) polynomials instead of the basic implementation inside FLINT using long gave an incredible speedup.
> And from the maybe 4 years ago comments in the ticket description, it should also seepdup NTL.
> I just don't have the time or the motivation to benchmark products of GF(2**n) elements (once you don't use Givaro) right now to prove it is beneficial.

On my pc, multiplying two random elements of GF(2**10000) goes from 103us in Sage 5.9 with old NTL to 21.4us in 5.10.rc0 with NTL+gf2x.



---

archive/issue_events_021520.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-06-05T09:20:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21520"
}
```



---

archive/issue_events_021521.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-06-05T09:20:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21521"
}
```



---

archive/issue_comments_010186.json:
```json
{
    "body": "<div id=\"comment:17\" align=\"right\">comment:17</div>\n\nNew spkg up and diff updated.\nPleas ignore the broken tuning.diff I posted.",
    "created_at": "2013-06-05T09:38:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10186",
    "user": "https://github.com/jpflori"
}
```

<div id="comment:17" align="right">comment:17</div>

New spkg up and diff updated.
Pleas ignore the broken tuning.diff I posted.



---

archive/issue_comments_010187.json:
```json
{
    "body": "Changed work issues from **tuning** to none",
    "created_at": "2013-06-05T09:39:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10187",
    "user": "https://github.com/jpflori"
}
```

Changed work issues from **tuning** to none



---

archive/issue_events_021522.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-06-05T09:39:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21522"
}
```



---

archive/issue_events_021523.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-06-05T09:39:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21523"
}
```



---

archive/issue_events_021524.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-06-05T09:41:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21524"
}
```



---

archive/issue_events_021525.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-06-05T09:41:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21525"
}
```



---

archive/issue_comments_010188.json:
```json
{
    "body": "<div id=\"comment:19\" align=\"right\">comment:19</div>\n\nWrong spelling...",
    "created_at": "2013-06-05T09:41:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10188",
    "user": "https://github.com/jpflori"
}
```

<div id="comment:19" align="right">comment:19</div>

Wrong spelling...



---

archive/issue_comments_010189.json:
```json
{
    "body": "Work Issues: **tuning**",
    "created_at": "2013-06-05T09:41:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10189",
    "user": "https://github.com/jpflori"
}
```

Work Issues: **tuning**



---

archive/issue_comments_010190.json:
```json
{
    "body": "Changed work issues from **tuning** to none",
    "created_at": "2013-06-05T09:44:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10190",
    "user": "https://github.com/jpflori"
}
```

Changed work issues from **tuning** to none



---

archive/issue_events_021526.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-06-05T09:44:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21526"
}
```



---

archive/issue_events_021527.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-06-05T09:44:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21527"
}
```



---

archive/issue_comments_010191.json:
```json
{
    "body": "<div id=\"comment:20\" align=\"right\">comment:20</div>\n\nShould be ok now.",
    "created_at": "2013-06-05T09:44:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10191",
    "user": "https://github.com/jpflori"
}
```

<div id="comment:20" align="right">comment:20</div>

Should be ok now.



---

archive/issue_events_021528.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-05T10:35:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21528"
}
```



---

archive/issue_events_021529.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-05T10:35:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21529"
}
```



---

archive/issue_comments_010192.json:
```json
{
    "body": "<div id=\"comment:21\" align=\"right\">comment:21</div>\n\nThis needs a patch to `spkg/standard/deps` and `spkg/install`.",
    "created_at": "2013-06-05T10:35:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10192",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:21" align="right">comment:21</div>

This needs a patch to `spkg/standard/deps` and `spkg/install`.



---

archive/issue_comments_010193.json:
```json
{
    "body": "<div id=\"comment:22\" align=\"right\">comment:22</div>\n\nwhile trying gf2x-1.1 on my laptop, I hit a bug due to the fact that the tuning gave\n`GF2X_MUL_TOOM4_ALWAYS_THRESHOLD=16`, whereas a value at least 30 is required.\nThe following patch will avoid this problem (or detect it earlier). It is against the development version but should be easily adapted to 1.1:\n\n```\n--- toom.c      (revision 148)\n+++ toom.c      (working copy)\n@@ -1,6 +1,6 @@\n /* This file is part of the gf2x library.\n \n-   Copyright 2007, 2008, 2009\n+   Copyright 2007, 2008, 2009, 2013\n    Richard Brent, Pierrick Gaudry, Emmanuel Thome', Paul Zimmermann\n \n    This program is free software; you can redistribute it and/or modify it\n@@ -53,6 +53,9 @@\n     if (n < GF2X_MUL_TOOMW_THRESHOLD)\n        return GF2X_SELECT_KARA;                // KarMul\n \n+#if GF2X_MUL_TOOM4_ALWAYS_THRESHOLD < 30\n+#error \"GF2X_MUL_TOOM4_ALWAYS_THRESHOLD must be >= 30\"\n+#endif\n     if (n >= GF2X_MUL_TOOM4_ALWAYS_THRESHOLD)\n        return GF2X_SELECT_TC4;         // Toom4Mul\n```",
    "created_at": "2013-06-05T12:11:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10193",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:22" align="right">comment:22</div>

while trying gf2x-1.1 on my laptop, I hit a bug due to the fact that the tuning gave
`GF2X_MUL_TOOM4_ALWAYS_THRESHOLD=16`, whereas a value at least 30 is required.
The following patch will avoid this problem (or detect it earlier). It is against the development version but should be easily adapted to 1.1:

```
--- toom.c      (revision 148)
+++ toom.c      (working copy)
@@ -1,6 +1,6 @@
 /* This file is part of the gf2x library.
 
-   Copyright 2007, 2008, 2009
+   Copyright 2007, 2008, 2009, 2013
    Richard Brent, Pierrick Gaudry, Emmanuel Thome', Paul Zimmermann
 
    This program is free software; you can redistribute it and/or modify it
@@ -53,6 +53,9 @@
     if (n < GF2X_MUL_TOOMW_THRESHOLD)
        return GF2X_SELECT_KARA;                // KarMul
 
+#if GF2X_MUL_TOOM4_ALWAYS_THRESHOLD < 30
+#error "GF2X_MUL_TOOM4_ALWAYS_THRESHOLD must be >= 30"
+#endif
     if (n >= GF2X_MUL_TOOM4_ALWAYS_THRESHOLD)
        return GF2X_SELECT_TC4;         // Toom4Mul
```



---

archive/issue_comments_010194.json:
```json
{
    "body": "<div id=\"comment:23\" align=\"right\">comment:23</div>\n\noh, I understand what happened now. I did `make tune-toom TOOM_TUNING_LIMIT=16` which sets automatically `GF2X_MUL_TOOM4_ALWAYS_THRESHOLD` to 16. When running `make tune-toom TOOM_TUNING_LIMIT=nnn`, one should always have nnn at least 30.\n\nPaul",
    "created_at": "2013-06-05T12:16:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10194",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:23" align="right">comment:23</div>

oh, I understand what happened now. I did `make tune-toom TOOM_TUNING_LIMIT=16` which sets automatically `GF2X_MUL_TOOM4_ALWAYS_THRESHOLD` to 16. When running `make tune-toom TOOM_TUNING_LIMIT=nnn`, one should always have nnn at least 30.

Paul



---

archive/issue_comments_010195.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -48,3 +48,6 @@\n * http://boxen.math.washington.edu/home/jpflori/spkg/gf2x-1.1.spkg\n * http://boxen.math.washington.edu/home/jpflori/spkg/ntl-5.5.2.p1.spkg\n \n+Apply to Sage's root:\n+* [attachment: trac_2114-gf2x.patch](https://github.com/sagemath/sage/files/ticket2114/trac_2114-gf2x.patch.gz)\n+\n``````\n",
    "created_at": "2013-06-05T12:37:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10195",
    "user": "https://github.com/jpflori"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -48,3 +48,6 @@
 * http://boxen.math.washington.edu/home/jpflori/spkg/gf2x-1.1.spkg
 * http://boxen.math.washington.edu/home/jpflori/spkg/ntl-5.5.2.p1.spkg
 
+Apply to Sage's root:
+* [attachment: trac_2114-gf2x.patch](https://github.com/sagemath/sage/files/ticket2114/trac_2114-gf2x.patch.gz)
+
``````




---

archive/issue_events_021530.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-06-05T12:37:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21530"
}
```



---

archive/issue_events_021531.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-06-05T12:37:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21531"
}
```



---

archive/issue_comments_010196.json:
```json
{
    "body": "<div id=\"comment:24\" align=\"right\">comment:24</div>\n\nI've used 100 as a limit so it should be ok.",
    "created_at": "2013-06-05T12:37:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10196",
    "user": "https://github.com/jpflori"
}
```

<div id="comment:24" align="right">comment:24</div>

I've used 100 as a limit so it should be ok.



---

archive/issue_comments_010197.json:
```json
{
    "body": "<div id=\"comment:25\" align=\"right\">comment:25</div>\n\nWaaaaah!  Just created (or rather finished) an NTL 5.5.2.p1 spkg (fixing hardcoded `make`) at #14692... 8-/",
    "created_at": "2013-06-05T21:08:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10197",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:25" align="right">comment:25</div>

Waaaaah!  Just created (or rather finished) an NTL 5.5.2.p1 spkg (fixing hardcoded `make`) at #14692... 8-/



---

archive/issue_comments_010198.json:
```json
{
    "body": "<div id=\"comment:26\" align=\"right\">comment:26</div>\n\nReplying to [@nexttime](#comment:25):\n> Waaaaah!  Just created (or rather finished) an NTL 5.5.2.p1 spkg (fixing hardcoded `make`) at #14692... 8-/\n\nOk, I've rebased *your* `.p1` on mine:\n\nhttp://boxen.math.washington.edu/home/leif/Sage/spkgs/ntl-5.5.2.p2.spkg\n\n```diff\ndiff --git a/.hgtags b/.hgtags\n--- a/.hgtags\n+++ b/.hgtags\n@@ -1,3 +1,4 @@\n 73d22601a79e226c590bb93cc842391e9e8f8d11 ntl-5.5.2\n 5cf2d2f43b4d9cf1fc3cf8e9bb54efc58ccf2b4f ntl-5.5.2.p0\n c7af41e56a64bdef778ee579beda9d54943105fe ntl-5.5.2.p1\n+4984ca9298a7310c9378ca8cd391dc8a1ba85d9f ntl-5.5.2.p2\ndiff --git a/SPKG.txt b/SPKG.txt\n--- a/SPKG.txt\n+++ b/SPKG.txt\n@@ -18,6 +18,7 @@\n \n == Dependencies ==\n  * gmp\n+ * gf2x\n \n == Special Update/Build Instructions ==\n  * We need to modfiy new.h to accomodate Singular\n@@ -34,6 +35,9 @@\n \n == Changelog ==\n \n+=== ntl-5.5.2.p2 (Jean-Pierre Flori, June 5th 2013) ===\n+ * #2114: Build NTL with gf2x support.\n+\n === ntl-5.5.2.p1 (Leif Leonhardy, June 5th 2013) ===\n  * #14692: Patch upstream to use `$(MAKE)` (instead of `make`) in the generated\n    Makefile (and in two scripts called from the Makefile which in turn invoke\ndiff --git a/spkg-install b/spkg-install\n--- a/spkg-install\n+++ b/spkg-install\n@@ -86,7 +86,7 @@\n         CC=\"$CC\" CFLAGS=\"$CFLAGS $SHAREDFLAGS\" \\\n         CXX=\"$CXX\" CXXFLAGS=\"$CXXFLAGS $SHAREDFLAGS\" \\\n         LDFLAGS=\"$LDFLAGS\" LIBTOOL_LINK_FLAGS=\"$LIBTOOL_LINK_FLAGS\" \\\n-        NTL_GMP_LIP=on NTL_STD_CXX=on \\\n+        NTL_GMP_LIP=on NTL_GF2X_LIB=on NTL_STD_CXX=on \\\n         LIBTOOL=../../libtool/libtool\n \n     if [ $? -ne 0 ]; then\n```",
    "created_at": "2013-06-05T21:35:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10198",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:26" align="right">comment:26</div>

Replying to [@nexttime](#comment:25):
> Waaaaah!  Just created (or rather finished) an NTL 5.5.2.p1 spkg (fixing hardcoded `make`) at #14692... 8-/

Ok, I've rebased *your* `.p1` on mine:

http://boxen.math.washington.edu/home/leif/Sage/spkgs/ntl-5.5.2.p2.spkg

```diff
diff --git a/.hgtags b/.hgtags
--- a/.hgtags
+++ b/.hgtags
@@ -1,3 +1,4 @@
 73d22601a79e226c590bb93cc842391e9e8f8d11 ntl-5.5.2
 5cf2d2f43b4d9cf1fc3cf8e9bb54efc58ccf2b4f ntl-5.5.2.p0
 c7af41e56a64bdef778ee579beda9d54943105fe ntl-5.5.2.p1
+4984ca9298a7310c9378ca8cd391dc8a1ba85d9f ntl-5.5.2.p2
diff --git a/SPKG.txt b/SPKG.txt
--- a/SPKG.txt
+++ b/SPKG.txt
@@ -18,6 +18,7 @@
 
 == Dependencies ==
  * gmp
+ * gf2x
 
 == Special Update/Build Instructions ==
  * We need to modfiy new.h to accomodate Singular
@@ -34,6 +35,9 @@
 
 == Changelog ==
 
+=== ntl-5.5.2.p2 (Jean-Pierre Flori, June 5th 2013) ===
+ * #2114: Build NTL with gf2x support.
+
 === ntl-5.5.2.p1 (Leif Leonhardy, June 5th 2013) ===
  * #14692: Patch upstream to use `$(MAKE)` (instead of `make`) in the generated
    Makefile (and in two scripts called from the Makefile which in turn invoke
diff --git a/spkg-install b/spkg-install
--- a/spkg-install
+++ b/spkg-install
@@ -86,7 +86,7 @@
         CC="$CC" CFLAGS="$CFLAGS $SHAREDFLAGS" \
         CXX="$CXX" CXXFLAGS="$CXXFLAGS $SHAREDFLAGS" \
         LDFLAGS="$LDFLAGS" LIBTOOL_LINK_FLAGS="$LIBTOOL_LINK_FLAGS" \
-        NTL_GMP_LIP=on NTL_STD_CXX=on \
+        NTL_GMP_LIP=on NTL_GF2X_LIB=on NTL_STD_CXX=on \
         LIBTOOL=../../libtool/libtool
 
     if [ $? -ne 0 ]; then
```



---

archive/issue_comments_010199.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -46,7 +46,12 @@\n \n Use spkgs at:\n * http://boxen.math.washington.edu/home/jpflori/spkg/gf2x-1.1.spkg\n-* http://boxen.math.washington.edu/home/jpflori/spkg/ntl-5.5.2.p1.spkg\n+\n+* http://boxen.math.washington.edu/home/jpflori/spkg/ntl-5.5.2.p1.spkg \n+\n+  **or** \n+\n+  http://boxen.math.washington.edu/home/leif/Sage/spkgs/ntl-5.5.2.p2.spkg (rebased on #14692)\n \n Apply to Sage's root:\n * [attachment: trac_2114-gf2x.patch](https://github.com/sagemath/sage/files/ticket2114/trac_2114-gf2x.patch.gz)\n``````\n",
    "created_at": "2013-06-05T21:38:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10199",
    "user": "https://github.com/nexttime"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -46,7 +46,12 @@
 
 Use spkgs at:
 * http://boxen.math.washington.edu/home/jpflori/spkg/gf2x-1.1.spkg
-* http://boxen.math.washington.edu/home/jpflori/spkg/ntl-5.5.2.p1.spkg
+
+* http://boxen.math.washington.edu/home/jpflori/spkg/ntl-5.5.2.p1.spkg 
+
+  **or** 
+
+  http://boxen.math.washington.edu/home/leif/Sage/spkgs/ntl-5.5.2.p2.spkg (rebased on #14692)
 
 Apply to Sage's root:
 * [attachment: trac_2114-gf2x.patch](https://github.com/sagemath/sage/files/ticket2114/trac_2114-gf2x.patch.gz)
``````




---

archive/issue_comments_010200.json:
```json
{
    "body": "<div id=\"comment:28\" align=\"right\">comment:28</div>\n\nW.r.t. gf2x:\n\nThe following doesn't work (`-O0` never takes effect):\n\n```sh\n\nif [ \"$SAGE64\" = \"yes\" ]; then\n    echo \"Building a 64-bit version of gf2x.\"\n    ABI=\"64\"; export ABI\n    CFLAGS=\"-O2 -g -m64 $CFLAGS\"; export CFLAGS\nelse\n    CFLAGS=\"-O2 -g $CFLAGS\"; export CFLAGS\nfi\n\nif [ \"$SAGE_DEBUG\" = \"yes\" ]; then\n    echo \"Building a debug version of gf2x.\"\n    CFLAGS=\"-O0 -g $CFLAGS\"; export CFLAGS\nfi\n```\n\n`$SAGE_LOCAL` should be quoted in the `rm` commands in `spkg-install`.",
    "created_at": "2013-06-05T22:00:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10200",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:28" align="right">comment:28</div>

W.r.t. gf2x:

The following doesn't work (`-O0` never takes effect):

```sh

if [ "$SAGE64" = "yes" ]; then
    echo "Building a 64-bit version of gf2x."
    ABI="64"; export ABI
    CFLAGS="-O2 -g -m64 $CFLAGS"; export CFLAGS
else
    CFLAGS="-O2 -g $CFLAGS"; export CFLAGS
fi

if [ "$SAGE_DEBUG" = "yes" ]; then
    echo "Building a debug version of gf2x."
    CFLAGS="-O0 -g $CFLAGS"; export CFLAGS
fi
```

`$SAGE_LOCAL` should be quoted in the `rm` commands in `spkg-install`.



---

archive/issue_comments_010201.json:
```json
{
    "body": "<div id=\"comment:29\" align=\"right\">comment:29</div>\n\nMinor thing:  \"Patching gf2x.\" gets printed even when no patch gets applied.",
    "created_at": "2013-06-05T22:05:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10201",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:29" align="right">comment:29</div>

Minor thing:  "Patching gf2x." gets printed even when no patch gets applied.



---

archive/issue_comments_010202.json:
```json
{
    "body": "<div id=\"comment:30\" align=\"right\">comment:30</div>\n\nP.S.:  For other spkgs, we (currently) use `SAGE_TUNE_<package name>[={yes,no}]` (where `<package name>` can be both upper or lower case).  And the new variable should probably get documented in `devel/sage/doc/en/installation/source.rst` (section \"Environment variables\").\n\n`GF2X_TUNE=full` really takes ages, and uses **a lot** of memory (so far up to 2 GB resident / 2.5 GB virtual).",
    "created_at": "2013-06-06T01:06:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10202",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:30" align="right">comment:30</div>

P.S.:  For other spkgs, we (currently) use `SAGE_TUNE_<package name>[={yes,no}]` (where `<package name>` can be both upper or lower case).  And the new variable should probably get documented in `devel/sage/doc/en/installation/source.rst` (section "Environment variables").

`GF2X_TUNE=full` really takes ages, and uses **a lot** of memory (so far up to 2 GB resident / 2.5 GB virtual).



---

archive/issue_comments_010203.json:
```json
{
    "body": "<div id=\"comment:31\" align=\"right\">comment:31</div>\n\n\n```\ngf2x.c: In function 'gf2x_mul_pool_init':\ngf2x.c:82:24: warning: argument to 'sizeof' in 'memset' call is the same expression as the destination; did you mean to dereference it? [-Wsizeof-pointer-memaccess]\n     memset(p, 0, sizeof(p));\n                        ^\n```\n\n? :-)",
    "created_at": "2013-06-06T04:14:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10203",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:31" align="right">comment:31</div>


```
gf2x.c: In function 'gf2x_mul_pool_init':
gf2x.c:82:24: warning: argument to 'sizeof' in 'memset' call is the same expression as the destination; did you mean to dereference it? [-Wsizeof-pointer-memaccess]
     memset(p, 0, sizeof(p));
                        ^
```

? :-)



---

archive/issue_comments_010204.json:
```json
{
    "body": "<div id=\"comment:32\" align=\"right\">comment:32</div>\n\nReplying to [@nexttime](#comment:31):\n> \n> ```\n> gf2x.c: In function 'gf2x_mul_pool_init':\n> gf2x.c:82:24: warning: argument to 'sizeof' in 'memset' call is the same expression as the destination; did you mean to dereference it? [-Wsizeof-pointer-memaccess]\n>      memset(p, 0, sizeof(p));\n>                         ^\n> ```\n\nHarmless, but warning could be avoided by using `sizeof(gf2x_mul_pool_t)` or `sizeof(p[0])` instead in\n\n```c\nvoid gf2x_mul_pool_init(gf2x_mul_pool_t p)\n{\n    memset(p, 0, sizeof(p));\n}\n```",
    "created_at": "2013-06-06T04:38:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10204",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:32" align="right">comment:32</div>

Replying to [@nexttime](#comment:31):
> 
> ```
> gf2x.c: In function 'gf2x_mul_pool_init':
> gf2x.c:82:24: warning: argument to 'sizeof' in 'memset' call is the same expression as the destination; did you mean to dereference it? [-Wsizeof-pointer-memaccess]
>      memset(p, 0, sizeof(p));
>                         ^
> ```

Harmless, but warning could be avoided by using `sizeof(gf2x_mul_pool_t)` or `sizeof(p[0])` instead in

```c
void gf2x_mul_pool_init(gf2x_mul_pool_t p)
{
    memset(p, 0, sizeof(p));
}
```



---

archive/issue_comments_010205.json:
```json
{
    "body": "<div id=\"comment:33\" align=\"right\">comment:33</div>\n\nOtherwise (i.e., modulo mentioned issues) looks ok (and apparently works; Linux x86 and x86_64, GCC 4.4.3 / 4.7.2 / 4.8.0, with `SAGE_CHECK=yes`, *some* doctests run).\n\n\n\n\nBuilding with `GF2X_TUNE=full` finally took more than four and a half hours, admittedly on a not-so-fast (but otherwise almost idle, dual-core) machine...",
    "created_at": "2013-06-06T04:49:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10205",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:33" align="right">comment:33</div>

Otherwise (i.e., modulo mentioned issues) looks ok (and apparently works; Linux x86 and x86_64, GCC 4.4.3 / 4.7.2 / 4.8.0, with `SAGE_CHECK=yes`, *some* doctests run).




Building with `GF2X_TUNE=full` finally took more than four and a half hours, admittedly on a not-so-fast (but otherwise almost idle, dual-core) machine...



---

archive/issue_comments_010206.json:
```json
{
    "body": "<div id=\"comment:34\" align=\"right\">comment:34</div>\n\nI've fixed the warning reported in comment [comment:31] upstream, thanks.\n\nPaul",
    "created_at": "2013-06-06T06:46:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10206",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:34" align="right">comment:34</div>

I've fixed the warning reported in comment [comment:31] upstream, thanks.

Paul



---

archive/issue_comments_010207.json:
```json
{
    "body": "<div id=\"comment:35\" align=\"right\">comment:35</div>\n\nThanks leif, I'll take all these sensible remarks into account.",
    "created_at": "2013-06-06T09:18:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10207",
    "user": "https://github.com/jpflori"
}
```

<div id="comment:35" align="right">comment:35</div>

Thanks leif, I'll take all these sensible remarks into account.



---

archive/issue_comments_010208.json:
```json
{
    "body": "<div id=\"comment:36\" align=\"right\">comment:36</div>\n\nAlso `$SAGE_ROOT/COPYING.txt` should be updated (keeping in mind #12447).",
    "created_at": "2013-06-06T09:19:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10208",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:36" align="right">comment:36</div>

Also `$SAGE_ROOT/COPYING.txt` should be updated (keeping in mind #12447).



---

archive/issue_events_021532.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-06T09:20:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21532"
}
```



---

archive/issue_events_021533.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-06T09:20:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21533"
}
```



---

archive/issue_comments_010209.json:
```json
{
    "body": "Dependencies: **#12447**",
    "created_at": "2013-06-06T09:23:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10209",
    "user": "https://github.com/jdemeyer"
}
```

Dependencies: **#12447**



---

archive/issue_comments_010210.json:
```json
{
    "body": "<div id=\"comment:39\" align=\"right\">comment:39</div>\n\nI've not found doc for the other tune variables in the doc, so I'll postpone adding the doc until we've listed all that has to be documented.\n\nI'm aware that we always print the patching message, it saves a few lines of script...",
    "created_at": "2013-06-06T09:46:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10210",
    "user": "https://github.com/jpflori"
}
```

<div id="comment:39" align="right">comment:39</div>

I've not found doc for the other tune variables in the doc, so I'll postpone adding the doc until we've listed all that has to be documented.

I'm aware that we always print the patching message, it saves a few lines of script...



---

archive/issue_comments_010211.json:
```json
{
    "body": "Attachment: **[trac_2114-gf2x.patch.gz](https://github.com/sagemath/sage/files/ticket2114/trac_2114-gf2x.patch.gz)**",
    "created_at": "2013-06-06T09:52:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10211",
    "user": "https://github.com/jpflori"
}
```

Attachment: **[trac_2114-gf2x.patch.gz](https://github.com/sagemath/sage/files/ticket2114/trac_2114-gf2x.patch.gz)**



---

archive/issue_events_021534.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-06-06T09:52:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21534"
}
```



---

archive/issue_events_021535.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-06-06T09:52:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21535"
}
```



---

archive/issue_comments_010212.json:
```json
{
    "body": "<div id=\"comment:41\" align=\"right\">comment:41</div>\n\nIf you prefer, I can include Paul's patch so that we will actually be patching gf2x.",
    "created_at": "2013-06-06T09:53:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10212",
    "user": "https://github.com/jpflori"
}
```

<div id="comment:41" align="right">comment:41</div>

If you prefer, I can include Paul's patch so that we will actually be patching gf2x.



---

archive/issue_comments_010213.json:
```json
{
    "body": "<div id=\"comment:42\" align=\"right\">comment:42</div>\n\nmy patch (from comment [comment:22]) is not needed if you do `tune-toom TOOM_TUNING_LIMIT=nnn` with nnn at least 30.\n\nPaul",
    "created_at": "2013-06-06T09:58:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10213",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:42" align="right">comment:42</div>

my patch (from comment [comment:22]) is not needed if you do `tune-toom TOOM_TUNING_LIMIT=nnn` with nnn at least 30.

Paul



---

archive/issue_comments_010214.json:
```json
{
    "body": "<div id=\"comment:43\" align=\"right\">comment:43</div>\n\nI meant leif's patch in fact, the one from comment https://github.com/sagemath/sage/issues/2114#comment:32 suppressing a warning.",
    "created_at": "2013-06-06T10:02:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10214",
    "user": "https://github.com/jpflori"
}
```

<div id="comment:43" align="right">comment:43</div>

I meant leif's patch in fact, the one from comment https://github.com/sagemath/sage/issues/2114#comment:32 suppressing a warning.



---

archive/issue_comments_010215.json:
```json
{
    "body": "<div id=\"comment:44\" align=\"right\">comment:44</div>\n\nok, here is the change I did upstream:\n\n```\n--- gf2x.c\t(revision 150)\n+++ gf2x.c\t(working copy)\n@@ -79,7 +79,7 @@\n \n void gf2x_mul_pool_init(gf2x_mul_pool_t p)\n {\n-    memset(p, 0, sizeof(p));\n+    memset(p, 0, sizeof(gf2x_mul_pool_t));\n }\n \n void gf2x_mul_pool_clear(gf2x_mul_pool_t p)\n```\nPaul",
    "created_at": "2013-06-06T10:08:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10215",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:44" align="right">comment:44</div>

ok, here is the change I did upstream:

```
--- gf2x.c	(revision 150)
+++ gf2x.c	(working copy)
@@ -79,7 +79,7 @@
 
 void gf2x_mul_pool_init(gf2x_mul_pool_t p)
 {
-    memset(p, 0, sizeof(p));
+    memset(p, 0, sizeof(gf2x_mul_pool_t));
 }
 
 void gf2x_mul_pool_clear(gf2x_mul_pool_t p)
```
Paul



---

archive/issue_comments_010216.json:
```json
{
    "body": "<div id=\"comment:45\" align=\"right\">comment:45</div>\n\nThanks Paul.\nIf leif rants, I'll include it.\n\nBut IMHO we can wait for the gf2x 1.2 release to get this warning suppressed.\nI think that getting gf2x in Sage quickly is a more important matter.",
    "created_at": "2013-06-06T10:10:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10216",
    "user": "https://github.com/jpflori"
}
```

<div id="comment:45" align="right">comment:45</div>

Thanks Paul.
If leif rants, I'll include it.

But IMHO we can wait for the gf2x 1.2 release to get this warning suppressed.
I think that getting gf2x in Sage quickly is a more important matter.



---

archive/issue_comments_010217.json:
```json
{
    "body": "<div id=\"comment:46\" align=\"right\">comment:46</div>\n\nReplying to [@jpflori](#comment:45):\n> Thanks Paul.\n> If leif rants, I'll include it.\n\nThe \"rant\" addressed upstream, and (as expected) reached it... ;-)\n\nI don't care whether you include a patch here.",
    "created_at": "2013-06-06T14:47:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10217",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:46" align="right">comment:46</div>

Replying to [@jpflori](#comment:45):
> Thanks Paul.
> If leif rants, I'll include it.

The "rant" addressed upstream, and (as expected) reached it... ;-)

I don't care whether you include a patch here.



---

archive/issue_comments_010218.json:
```json
{
    "body": "<div id=\"comment:47\" align=\"right\">comment:47</div>\n\nP.S.:  There are other warnings where the return value of `fgets()` (IIRC) gets discarded / ignored, in tuning-related code.  (But in those cases it's IMHO more obvious to the user that the warnings can safely be ignored.)",
    "created_at": "2013-06-06T14:50:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10218",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:47" align="right">comment:47</div>

P.S.:  There are other warnings where the return value of `fgets()` (IIRC) gets discarded / ignored, in tuning-related code.  (But in those cases it's IMHO more obvious to the user that the warnings can safely be ignored.)



---

archive/issue_comments_010219.json:
```json
{
    "body": "<div id=\"comment:48\" align=\"right\">comment:48</div>\n\nReplying to [@jpflori](#comment:39):\n> I've not found doc for the other tune variables in the doc, so I'll postpone adding the doc until we've listed all that has to be documented.\n\nHmmm...  If you at least create a meta-ticket to record what has to be documented...\n\n\n\n\n> I'm aware that we always print the patching message, it saves a few lines of script...\n\nI'm using either `ls ../patches/*.patch >/dev/null 2>&1 && ...`, or simply *\"Applying patches to upstream (if any) ...\"*",
    "created_at": "2013-06-06T14:56:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10219",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:48" align="right">comment:48</div>

Replying to [@jpflori](#comment:39):
> I've not found doc for the other tune variables in the doc, so I'll postpone adding the doc until we've listed all that has to be documented.

Hmmm...  If you at least create a meta-ticket to record what has to be documented...




> I'm aware that we always print the patching message, it saves a few lines of script...

I'm using either `ls ../patches/*.patch >/dev/null 2>&1 && ...`, or simply *"Applying patches to upstream (if any) ..."*



---

archive/issue_comments_010220.json:
```json
{
    "body": "<div id=\"comment:49\" align=\"right\">comment:49</div>\n\nBtw., `ptestlong` passed with Sage 5.10.rc0 on Linux x86_64.",
    "created_at": "2013-06-06T14:57:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10220",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:49" align="right">comment:49</div>

Btw., `ptestlong` passed with Sage 5.10.rc0 on Linux x86_64.



---

archive/issue_comments_010221.json:
```json
{
    "body": "<div id=\"comment:50\" align=\"right\">comment:50</div>\n\nReplying to [@nexttime](#comment:48):\n> Replying to [@jpflori](#comment:39):\n> > I've not found doc for the other tune variables in the doc, so I'll postpone adding the doc until we've listed all that has to be documented.\n\n> \n> Hmmm...  If you at least create a meta-ticket to record what has to be documented...\n\nDone, see #14698 (you're cc'ed there anyway).\n> \n> \n\n> \n> > I'm aware that we always print the patching message, it saves a few lines of script...\n\n> \n> I'm using either `ls ../patches/*.patch >/dev/null 2>&1 && ...`, or simply *\"Applying patches to upstream (if any) ...\"*\n\nOk, it's true it's simple enough, I'll change that.",
    "created_at": "2013-06-06T15:02:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10221",
    "user": "https://github.com/jpflori"
}
```

<div id="comment:50" align="right">comment:50</div>

Replying to [@nexttime](#comment:48):
> Replying to [@jpflori](#comment:39):
> > I've not found doc for the other tune variables in the doc, so I'll postpone adding the doc until we've listed all that has to be documented.

> 
> Hmmm...  If you at least create a meta-ticket to record what has to be documented...

Done, see #14698 (you're cc'ed there anyway).
> 
> 

> 
> > I'm aware that we always print the patching message, it saves a few lines of script...

> 
> I'm using either `ls ../patches/*.patch >/dev/null 2>&1 && ...`, or simply *"Applying patches to upstream (if any) ..."*

Ok, it's true it's simple enough, I'll change that.



---

archive/issue_events_021536.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-06-06T15:02:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21536"
}
```



---

archive/issue_events_021537.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-06-06T15:02:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21537"
}
```



---

archive/issue_comments_010222.json:
```json
{
    "body": "<div id=\"comment:51\" align=\"right\">comment:51</div>\n\nDone.\n\nNow the NTL spkg should be rebased when #14692 is done.",
    "created_at": "2013-06-06T15:10:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10222",
    "user": "https://github.com/jpflori"
}
```

<div id="comment:51" align="right">comment:51</div>

Done.

Now the NTL spkg should be rebased when #14692 is done.



---

archive/issue_comments_010223.json:
```json
{
    "body": "Work Issues: **rebase ntl spkg**",
    "created_at": "2013-06-06T15:10:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10223",
    "user": "https://github.com/jpflori"
}
```

Work Issues: **rebase ntl spkg**



---

archive/issue_comments_010224.json:
```json
{
    "body": "<div id=\"comment:52\" align=\"right\">comment:52</div>\n\nOr the converse.",
    "created_at": "2013-06-06T15:11:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10224",
    "user": "https://github.com/jpflori"
}
```

<div id="comment:52" align="right">comment:52</div>

Or the converse.



---

archive/issue_comments_010225.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -47,11 +47,7 @@\n Use spkgs at:\n * http://boxen.math.washington.edu/home/jpflori/spkg/gf2x-1.1.spkg\n \n-* http://boxen.math.washington.edu/home/jpflori/spkg/ntl-5.5.2.p1.spkg \n-\n-  **or** \n-\n-  http://boxen.math.washington.edu/home/leif/Sage/spkgs/ntl-5.5.2.p2.spkg (rebased on #14692)\n+* http://boxen.math.washington.edu/home/jpflori/spkg/ntl-5.5.2.p1.spkg\n \n Apply to Sage's root:\n * [attachment: trac_2114-gf2x.patch](https://github.com/sagemath/sage/files/ticket2114/trac_2114-gf2x.patch.gz)\n``````\n",
    "created_at": "2013-06-06T15:24:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10225",
    "user": "https://github.com/jpflori"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -47,11 +47,7 @@
 Use spkgs at:
 * http://boxen.math.washington.edu/home/jpflori/spkg/gf2x-1.1.spkg
 
-* http://boxen.math.washington.edu/home/jpflori/spkg/ntl-5.5.2.p1.spkg 
-
-  **or** 
-
-  http://boxen.math.washington.edu/home/leif/Sage/spkgs/ntl-5.5.2.p2.spkg (rebased on #14692)
+* http://boxen.math.washington.edu/home/jpflori/spkg/ntl-5.5.2.p1.spkg
 
 Apply to Sage's root:
 * [attachment: trac_2114-gf2x.patch](https://github.com/sagemath/sage/files/ticket2114/trac_2114-gf2x.patch.gz)
``````




---

archive/issue_comments_010226.json:
```json
{
    "body": "Changed work issues from **rebase ntl spkg** to none",
    "created_at": "2013-06-06T15:24:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10226",
    "user": "https://github.com/jpflori"
}
```

Changed work issues from **rebase ntl spkg** to none



---

archive/issue_comments_010227.json:
```json
{
    "body": "<div id=\"comment:53\" align=\"right\">comment:53</div>\n\nAs #14692 needs more work, let's base it on top of this ticket rather than the other way around.",
    "created_at": "2013-06-06T15:24:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10227",
    "user": "https://github.com/jpflori"
}
```

<div id="comment:53" align="right">comment:53</div>

As #14692 needs more work, let's base it on top of this ticket rather than the other way around.



---

archive/issue_events_021538.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-06-06T15:24:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21538"
}
```



---

archive/issue_events_021539.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-06-06T15:24:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21539"
}
```



---

archive/issue_comments_010228.json:
```json
{
    "body": "<div id=\"comment:54\" align=\"right\">comment:54</div>\n\nReplying to [@nexttime](#comment:47):\n> P.S.:  There are other warnings where the return value of `fgets()` (IIRC) gets discarded / ignored, in tuning-related code.  (But in those cases it's IMHO more obvious to the user that the warnings can safely be ignored.) \n\nLeif, thanks, I've fixed a few upstream. It is always nice to get feedback when a package is used in Sage!\n\nPaul",
    "created_at": "2013-06-06T17:02:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10228",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:54" align="right">comment:54</div>

Replying to [@nexttime](#comment:47):
> P.S.:  There are other warnings where the return value of `fgets()` (IIRC) gets discarded / ignored, in tuning-related code.  (But in those cases it's IMHO more obvious to the user that the warnings can safely be ignored.) 

Leif, thanks, I've fixed a few upstream. It is always nice to get feedback when a package is used in Sage!

Paul



---

archive/issue_comments_010229.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -44,10 +44,9 @@\n \n Latest 1.1 version is at http://gf2x.gforge.inria.fr/\n \n-Use spkgs at:\n-* http://boxen.math.washington.edu/home/jpflori/spkg/gf2x-1.1.spkg\n-\n-* http://boxen.math.washington.edu/home/jpflori/spkg/ntl-5.5.2.p1.spkg\n+Use **spkgs** at:\n+* [http://boxen.math.washington.edu/home/jdemeyer/spkg/gf2x-1.1.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/gf2x-1.1.spkg) ([spkg diff](https://github.com/sagemath/sage/files/ticket2114/gf2x-1.1.diff.gz))\n+* [http://boxen.math.washington.edu/home/jpflori/spkg/ntl-5.5.2.p1.spkg](http://boxen.math.washington.edu/home/jpflori/spkg/ntl-5.5.2.p1.spkg)\n \n Apply to Sage's root:\n * [attachment: trac_2114-gf2x.patch](https://github.com/sagemath/sage/files/ticket2114/trac_2114-gf2x.patch.gz)\n``````\n",
    "created_at": "2013-06-08T11:01:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10229",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -44,10 +44,9 @@
 
 Latest 1.1 version is at http://gf2x.gforge.inria.fr/
 
-Use spkgs at:
-* http://boxen.math.washington.edu/home/jpflori/spkg/gf2x-1.1.spkg
-
-* http://boxen.math.washington.edu/home/jpflori/spkg/ntl-5.5.2.p1.spkg
+Use **spkgs** at:
+* [http://boxen.math.washington.edu/home/jdemeyer/spkg/gf2x-1.1.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/gf2x-1.1.spkg) ([spkg diff](https://github.com/sagemath/sage/files/ticket2114/gf2x-1.1.diff.gz))
+* [http://boxen.math.washington.edu/home/jpflori/spkg/ntl-5.5.2.p1.spkg](http://boxen.math.washington.edu/home/jpflori/spkg/ntl-5.5.2.p1.spkg)
 
 Apply to Sage's root:
 * [attachment: trac_2114-gf2x.patch](https://github.com/sagemath/sage/files/ticket2114/trac_2114-gf2x.patch.gz)
``````




---

archive/issue_comments_010230.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -3,38 +3,7 @@\n It's:\n * by very well respected people\n * GPL'd (v2 or later)\n-* Small pure C code:\n-\n-```\n-dhcp46-72:gf2x-0.1 was$ ls\n-BestToom.c          README              ToomSpace.c         gen_bb_mul_code.c   mul-tc3w.c          mul2t.c             tune1               tuneup.c\n-COPYING             TC.h                cantor              mul-tc3.c           mul-tc4.c           mulfft-bit.c        tunefft.c\n-HalfGCD.c           Toom.c              factor.c            mul-tc3u.c          mul.c               patch-wrt-ntl-5.3.1 tunetoom.c\n-dhcp46-72:gf2x-0.1 was$ usage\n-4\tBestToom.c\n-4\tREADME\n-4\tTC.h\n-4\tToom.c\n-4\tToomSpace.c\n-4\tpatch-wrt-ntl-5.3.1\n-4\ttune1\n-4\ttuneup.c\n-8\tgen_bb_mul_code.c\n-8\tmul2t.c\n-8\ttunefft.c\n-12\tmul-tc3.c\n-12\tmul-tc3u.c\n-12\tmul-tc3w.c\n-12\tmul-tc4.c\n-12\ttunetoom.c\n-16\tHalfGCD.c\n-16\tmul.c\n-20\tCOPYING\n-28\tmulfft-bit.c\n-40\tfactor.c\n-132\tcantor\n-368\ttotal\n-```\n+* Small pure C code\n * and Paul Z. says: \n \n ```\n@@ -48,6 +17,6 @@\n * [http://boxen.math.washington.edu/home/jdemeyer/spkg/gf2x-1.1.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/gf2x-1.1.spkg) ([spkg diff](https://github.com/sagemath/sage/files/ticket2114/gf2x-1.1.diff.gz))\n * [http://boxen.math.washington.edu/home/jpflori/spkg/ntl-5.5.2.p1.spkg](http://boxen.math.washington.edu/home/jpflori/spkg/ntl-5.5.2.p1.spkg)\n \n-Apply to Sage's root:\n+**Apply** to Sage's root:\n * [attachment: trac_2114-gf2x.patch](https://github.com/sagemath/sage/files/ticket2114/trac_2114-gf2x.patch.gz)\n \n``````\n",
    "created_at": "2013-06-08T11:02:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10230",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -3,38 +3,7 @@
 It's:
 * by very well respected people
 * GPL'd (v2 or later)
-* Small pure C code:
-
-```
-dhcp46-72:gf2x-0.1 was$ ls
-BestToom.c          README              ToomSpace.c         gen_bb_mul_code.c   mul-tc3w.c          mul2t.c             tune1               tuneup.c
-COPYING             TC.h                cantor              mul-tc3.c           mul-tc4.c           mulfft-bit.c        tunefft.c
-HalfGCD.c           Toom.c              factor.c            mul-tc3u.c          mul.c               patch-wrt-ntl-5.3.1 tunetoom.c
-dhcp46-72:gf2x-0.1 was$ usage
-4	BestToom.c
-4	README
-4	TC.h
-4	Toom.c
-4	ToomSpace.c
-4	patch-wrt-ntl-5.3.1
-4	tune1
-4	tuneup.c
-8	gen_bb_mul_code.c
-8	mul2t.c
-8	tunefft.c
-12	mul-tc3.c
-12	mul-tc3u.c
-12	mul-tc3w.c
-12	mul-tc4.c
-12	tunetoom.c
-16	HalfGCD.c
-16	mul.c
-20	COPYING
-28	mulfft-bit.c
-40	factor.c
-132	cantor
-368	total
-```
+* Small pure C code
 * and Paul Z. says: 
 
 ```
@@ -48,6 +17,6 @@
 * [http://boxen.math.washington.edu/home/jdemeyer/spkg/gf2x-1.1.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/gf2x-1.1.spkg) ([spkg diff](https://github.com/sagemath/sage/files/ticket2114/gf2x-1.1.diff.gz))
 * [http://boxen.math.washington.edu/home/jpflori/spkg/ntl-5.5.2.p1.spkg](http://boxen.math.washington.edu/home/jpflori/spkg/ntl-5.5.2.p1.spkg)
 
-Apply to Sage's root:
+**Apply** to Sage's root:
 * [attachment: trac_2114-gf2x.patch](https://github.com/sagemath/sage/files/ticket2114/trac_2114-gf2x.patch.gz)
 
``````




---

archive/issue_comments_010231.json:
```json
{
    "body": "<div id=\"comment:57\" align=\"right\">comment:57</div>\n\nI made a few small changes to `gf2x` to make `spkg-install` more like most other standard packages. I also re-downloaded the upstream source, as timestamps got messed up somehow.",
    "created_at": "2013-06-08T11:04:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10231",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:57" align="right">comment:57</div>

I made a few small changes to `gf2x` to make `spkg-install` more like most other standard packages. I also re-downloaded the upstream source, as timestamps got messed up somehow.



---

archive/issue_comments_010232.json:
```json
{
    "body": "<div id=\"comment:58\" align=\"right\">comment:58</div>\n\nFor NTL, I added the `hg tag`.",
    "created_at": "2013-06-08T11:06:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10232",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:58" align="right">comment:58</div>

For NTL, I added the `hg tag`.



---

archive/issue_comments_010233.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -15,7 +15,7 @@\n \n Use **spkgs** at:\n * [http://boxen.math.washington.edu/home/jdemeyer/spkg/gf2x-1.1.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/gf2x-1.1.spkg) ([spkg diff](https://github.com/sagemath/sage/files/ticket2114/gf2x-1.1.diff.gz))\n-* [http://boxen.math.washington.edu/home/jpflori/spkg/ntl-5.5.2.p1.spkg](http://boxen.math.washington.edu/home/jpflori/spkg/ntl-5.5.2.p1.spkg)\n+* [http://boxen.math.washington.edu/home/jdemeyer/spkg/ntl-5.5.2.p1.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/ntl-5.5.2.p1.spkg)\n \n **Apply** to Sage's root:\n * [attachment: trac_2114-gf2x.patch](https://github.com/sagemath/sage/files/ticket2114/trac_2114-gf2x.patch.gz)\n``````\n",
    "created_at": "2013-06-08T11:06:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10233",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -15,7 +15,7 @@
 
 Use **spkgs** at:
 * [http://boxen.math.washington.edu/home/jdemeyer/spkg/gf2x-1.1.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/gf2x-1.1.spkg) ([spkg diff](https://github.com/sagemath/sage/files/ticket2114/gf2x-1.1.diff.gz))
-* [http://boxen.math.washington.edu/home/jpflori/spkg/ntl-5.5.2.p1.spkg](http://boxen.math.washington.edu/home/jpflori/spkg/ntl-5.5.2.p1.spkg)
+* [http://boxen.math.washington.edu/home/jdemeyer/spkg/ntl-5.5.2.p1.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/ntl-5.5.2.p1.spkg)
 
 **Apply** to Sage's root:
 * [attachment: trac_2114-gf2x.patch](https://github.com/sagemath/sage/files/ticket2114/trac_2114-gf2x.patch.gz)
``````




---

archive/issue_events_021540.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-08T11:07:00Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "milestone_number": null,
    "milestone_title": "sage-5.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21540"
}
```



---

archive/issue_events_021541.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-08T11:07:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21541"
}
```



---

archive/issue_comments_010234.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2013-06-08T11:07:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10234",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_021542.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-08T11:07:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "c: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21542"
}
```



---

archive/issue_events_021543.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-08T11:07:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21543"
}
```



---

archive/issue_comments_010235.json:
```json
{
    "body": "<div id=\"comment:61\" align=\"right\">comment:61</div>\n\nReplying to [@jdemeyer](#comment:57):\n> I made a few small changes to `gf2x` to make `spkg-install` more like most other standard packages. I also re-downloaded the upstream source, as timestamps got messed up somehow.\n\nYou didn't fix \"gf2x is a C/C+ software package\" though... ;-)\n\nCould you attach a diff of **your** changes?\n\nI don't want to re-review everything.  Retesting is odd enough.",
    "created_at": "2013-06-08T11:09:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10235",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:61" align="right">comment:61</div>

Replying to [@jdemeyer](#comment:57):
> I made a few small changes to `gf2x` to make `spkg-install` more like most other standard packages. I also re-downloaded the upstream source, as timestamps got messed up somehow.

You didn't fix "gf2x is a C/C+ software package" though... ;-)

Could you attach a diff of **your** changes?

I don't want to re-review everything.  Retesting is odd enough.



---

archive/issue_comments_010236.json:
```json
{
    "body": "Spkg diff, for review only.",
    "created_at": "2013-06-08T11:28:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10236",
    "user": "https://github.com/jdemeyer"
}
```

Spkg diff, for review only.



---

archive/issue_comments_010237.json:
```json
{
    "body": "Attachment: **[gf2x-1.1.diff.gz](https://github.com/sagemath/sage/files/ticket2114/gf2x-1.1.diff.gz)**",
    "created_at": "2013-06-08T11:28:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10237",
    "user": "https://github.com/jdemeyer"
}
```

Attachment: **[gf2x-1.1.diff.gz](https://github.com/sagemath/sage/files/ticket2114/gf2x-1.1.diff.gz)**



---

archive/issue_comments_010238.json:
```json
{
    "body": "<div id=\"comment:62\" align=\"right\">comment:62</div>\n\nAttachment: **[gf2x-1.1-jdemeyer.diff.gz](https://github.com/sagemath/sage/files/ticket2114/gf2x-1.1-jdemeyer.diff.gz)**\n\nReplying to [@nexttime](#comment:61):\n> You didn't fix \"gf2x is a C/C+ software package\" though... ;-)\n\nFixed now.\n\n> Could you attach a diff of **your** changes?\n\n[attachment: gf2x-1.1-jdemeyer.diff](https://github.com/sagemath/sage/files/ticket2114/gf2x-1.1-jdemeyer.diff.gz)",
    "created_at": "2013-06-08T11:29:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10238",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:62" align="right">comment:62</div>

Attachment: **[gf2x-1.1-jdemeyer.diff.gz](https://github.com/sagemath/sage/files/ticket2114/gf2x-1.1-jdemeyer.diff.gz)**

Replying to [@nexttime](#comment:61):
> You didn't fix "gf2x is a C/C+ software package" though... ;-)

Fixed now.

> Could you attach a diff of **your** changes?

[attachment: gf2x-1.1-jdemeyer.diff](https://github.com/sagemath/sage/files/ticket2114/gf2x-1.1-jdemeyer.diff.gz)



---

archive/issue_comments_010239.json:
```json
{
    "body": "<div id=\"comment:63\" align=\"right\">comment:63</div>\n\nReplying to [@jdemeyer](#comment:62):\n> Replying to [@nexttime](#comment:61):\n> > You didn't fix \"gf2x is a C/C+ software package\" though... ;-)\n\n> Fixed now.\n> \n> > Could you attach a diff of **your** changes?\n\n> [attachment: gf2x-1.1-jdemeyer.diff](https://github.com/sagemath/sage/files/ticket2114/gf2x-1.1-jdemeyer.diff.gz)\n\nI would have dropped the \"C++\"; if gf2x was (partially) implemented in C++, we'd have to set / change `CXXFLAGS` as well.\n\nI'd probably also use `$CFLAG64` instead of hardcoding `-m64`.",
    "created_at": "2013-06-08T12:04:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10239",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:63" align="right">comment:63</div>

Replying to [@jdemeyer](#comment:62):
> Replying to [@nexttime](#comment:61):
> > You didn't fix "gf2x is a C/C+ software package" though... ;-)

> Fixed now.
> 
> > Could you attach a diff of **your** changes?

> [attachment: gf2x-1.1-jdemeyer.diff](https://github.com/sagemath/sage/files/ticket2114/gf2x-1.1-jdemeyer.diff.gz)

I would have dropped the "C++"; if gf2x was (partially) implemented in C++, we'd have to set / change `CXXFLAGS` as well.

I'd probably also use `$CFLAG64` instead of hardcoding `-m64`.



---

archive/issue_comments_010240.json:
```json
{
    "body": "<div id=\"comment:64\" align=\"right\">comment:64</div>\n\nDid I mention it also builds with `clang`? ;-)",
    "created_at": "2013-06-08T12:06:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10240",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:64" align="right">comment:64</div>

Did I mention it also builds with `clang`? ;-)



---

archive/issue_comments_010241.json:
```json
{
    "body": "<div id=\"comment:65\" align=\"right\">comment:65</div>\n\nReplying to [@nexttime](#comment:63):\n> I would have dropped the \"C++\"\n\nThe text was taken verbatim from [upstream](http://gf2x.gforge.inria.fr/).",
    "created_at": "2013-06-08T12:09:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10241",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:65" align="right">comment:65</div>

Replying to [@nexttime](#comment:63):
> I would have dropped the "C++"

The text was taken verbatim from [upstream](http://gf2x.gforge.inria.fr/).



---

archive/issue_comments_010242.json:
```json
{
    "body": "<div id=\"comment:66\" align=\"right\">comment:66</div>\n\nReplying to [@jdemeyer](#comment:65):\n> Replying to [@nexttime](#comment:63):\n> > I would have dropped the \"C++\"\n\n> The text was taken verbatim from [upstream](http://gf2x.gforge.inria.fr/).\n\nI think it's covered by the GPL as well.",
    "created_at": "2013-06-08T12:13:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10242",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:66" align="right">comment:66</div>

Replying to [@jdemeyer](#comment:65):
> Replying to [@nexttime](#comment:63):
> > I would have dropped the "C++"

> The text was taken verbatim from [upstream](http://gf2x.gforge.inria.fr/).

I think it's covered by the GPL as well.



---

archive/issue_comments_010243.json:
```json
{
    "body": "<div id=\"comment:67\" align=\"right\">comment:67</div>\n\nthe C++ part of gf2x is only in the \"apps\" subdirectory, which contains binaries to be linked with NTL\n(we had to use C++ here since NTL is under C++) to check for primitive trinomials. I guess Sage does not use that part, thus indeed only C code is used within Sage.\n\nPaul",
    "created_at": "2013-06-08T12:46:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10243",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:67" align="right">comment:67</div>

the C++ part of gf2x is only in the "apps" subdirectory, which contains binaries to be linked with NTL
(we had to use C++ here since NTL is under C++) to check for primitive trinomials. I guess Sage does not use that part, thus indeed only C code is used within Sage.

Paul



---

archive/issue_comments_010244.json:
```json
{
    "body": "<div id=\"comment:68\" align=\"right\">comment:68</div>\n\nReplying to [@zimmermann6](#comment:67):\n> the C++ part of gf2x is only in the \"apps\" subdirectory, which contains binaries to be linked with NTL\n> (we had to use C++ here since NTL is under C++) to check for primitive trinomials.\n> \n\n> \n> I guess Sage does not use that part, thus indeed only C code is used within Sage.\n\nWe can't build those apps (at least not immediately), since we decided to let NTL depend on gf2x.",
    "created_at": "2013-06-08T13:17:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10244",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:68" align="right">comment:68</div>

Replying to [@zimmermann6](#comment:67):
> the C++ part of gf2x is only in the "apps" subdirectory, which contains binaries to be linked with NTL
> (we had to use C++ here since NTL is under C++) to check for primitive trinomials.
> 

> 
> I guess Sage does not use that part, thus indeed only C code is used within Sage.

We can't build those apps (at least not immediately), since we decided to let NTL depend on gf2x.



---

archive/issue_comments_010245.json:
```json
{
    "body": "<div id=\"comment:69\" align=\"right\">comment:69</div>\n\nPositive review to everything except my changes ([attachment: gf2x-1.1-jdemeyer.diff](https://github.com/sagemath/sage/files/ticket2114/gf2x-1.1-jdemeyer.diff.gz)).",
    "created_at": "2013-06-08T18:53:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10245",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:69" align="right">comment:69</div>

Positive review to everything except my changes ([attachment: gf2x-1.1-jdemeyer.diff](https://github.com/sagemath/sage/files/ticket2114/gf2x-1.1-jdemeyer.diff.gz)).



---

archive/issue_comments_010246.json:
```json
{
    "body": "<div id=\"comment:70\" align=\"right\">comment:70</div>\n\nI'm ok with them.",
    "created_at": "2013-06-08T18:56:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10246",
    "user": "https://github.com/jpflori"
}
```

<div id="comment:70" align="right">comment:70</div>

I'm ok with them.



---

archive/issue_events_021544.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-06-08T18:56:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21544"
}
```



---

archive/issue_events_021545.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-06-08T18:56:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21545"
}
```



---

archive/issue_comments_010247.json:
```json
{
    "body": "<div id=\"comment:71\" align=\"right\">comment:71</div>\n\nFWIW, I was going to give the previous spkgs positive review right when Jeroen started to change both, so I don't insist on fixing the hardcoded `-m64`.",
    "created_at": "2013-06-08T19:02:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10247",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:71" align="right">comment:71</div>

FWIW, I was going to give the previous spkgs positive review right when Jeroen started to change both, so I don't insist on fixing the hardcoded `-m64`.



---

archive/issue_comments_010248.json:
```json
{
    "body": "Merged: **sage-5.11.beta1**",
    "created_at": "2013-06-10T08:41:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2114#issuecomment-10248",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.11.beta1**



---

archive/issue_events_021546.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-10T08:41:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21546"
}
```



---

archive/issue_events_021547.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-10T08:41:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2114",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2114#event-21547"
}
```
