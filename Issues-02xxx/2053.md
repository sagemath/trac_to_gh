# Issue 2053: creating symbolic matrices is insanely slow

archive/issues_002053.json:
```json
{
    "assignees": [],
    "body": "On the fastest modern hardware we have:\n\n```\nsage: time m = matrix(SR, 20, [1..20^2])\nCPU times: user 0.34 s, sys: 0.12 s, total: 0.45 s\nWall time: 1.05\n```\nwhich is frickin' slow.  And the time isn't just in coercion, since\n\n```\nsage: time v = [SR(a) for a in [1..20^2]]\nCPU times: user 0.01 s, sys: 0.00 s, total: 0.01 s\nWall time: 0.01\n```\n\nAssignee: **@williamstein**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/2053_\n\n",
    "closed_at": "2008-02-07T10:10:31Z",
    "created_at": "2008-02-05T14:28:48Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20calculus",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.10.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "creating symbolic matrices is insanely slow",
    "type": "issue",
    "updated_at": "2008-02-07T10:10:31Z",
    "url": "https://github.com/sagemath/sage/issues/2053",
    "user": "https://github.com/williamstein"
}
```
On the fastest modern hardware we have:

```
sage: time m = matrix(SR, 20, [1..20^2])
CPU times: user 0.34 s, sys: 0.12 s, total: 0.45 s
Wall time: 1.05
```
which is frickin' slow.  And the time isn't just in coercion, since

```
sage: time v = [SR(a) for a in [1..20^2]]
CPU times: user 0.01 s, sys: 0.00 s, total: 0.01 s
Wall time: 0.01
```

Assignee: **@williamstein**

_Issue created by migration from https://trac.sagemath.org/ticket/2053_





---

archive/issue_events_017853.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-02-05T14:28:48Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2053",
    "milestone_number": null,
    "milestone_title": "sage-2.10.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2053#event-17853"
}
```



---

archive/issue_events_017854.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-02-05T14:28:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2053",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20calculus",
    "label_color": "0000ff",
    "label_name": "component: calculus",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2053#event-17854"
}
```



---

archive/issue_events_017855.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-02-05T14:28:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2053",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2053#event-17855"
}
```



---

archive/issue_events_017856.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-02-05T14:28:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2053",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2053#event-17856"
}
```



---

archive/issue_events_017857.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-02-05T14:47:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2053",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2053#event-17857"
}
```



---

archive/issue_comments_010333.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nThe attached patch fixes this problem.  Now the following works and speeds\nup the above benchmark by a huge amount:\n\n```\nsage: time m = matrix(SR, 20, [1..20^2])\nCPU times: user 0.02 s, sys: 0.00 s, total: 0.02 s\nWall time: 0.07\nsage: time m = matrix(SR, 50, [1..50^2])\nCPU times: user 0.07 s, sys: 0.02 s, total: 0.09 s\nWall time: 0.46\nsage: time m = matrix(SR, 100, [1..100^2])\nCPU times: user 0.26 s, sys: 0.02 s, total: 0.27 s\nWall time: 0.49\n```\n\nEven a 500x500 matrix is possible, which used to be out\nof the question. \n\n```\nsage: time m = matrix(SR, 500, [1..500^2])\nCPU times: user 6.79 s, sys: 0.39 s, total: 7.17 s\nWall time: 13.32\n```",
    "created_at": "2008-02-05T14:47:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2053",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2053#issuecomment-10333",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'>Comment 1:</a>
The attached patch fixes this problem.  Now the following works and speeds
up the above benchmark by a huge amount:

```
sage: time m = matrix(SR, 20, [1..20^2])
CPU times: user 0.02 s, sys: 0.00 s, total: 0.02 s
Wall time: 0.07
sage: time m = matrix(SR, 50, [1..50^2])
CPU times: user 0.07 s, sys: 0.02 s, total: 0.09 s
Wall time: 0.46
sage: time m = matrix(SR, 100, [1..100^2])
CPU times: user 0.26 s, sys: 0.02 s, total: 0.27 s
Wall time: 0.49
```

Even a 500x500 matrix is possible, which used to be out
of the question. 

```
sage: time m = matrix(SR, 500, [1..500^2])
CPU times: user 6.79 s, sys: 0.39 s, total: 7.17 s
Wall time: 13.32
```



---

archive/issue_comments_010334.json:
```json
{
    "body": "Attachment: **[trac-2053-symbolic_matrices_slow.patch.gz](https://github.com/sagemath/sage/files/ticket2053/trac-2053-symbolic_matrices_slow.patch.gz)**",
    "created_at": "2008-02-05T14:48:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2053",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2053#issuecomment-10334",
    "user": "https://github.com/williamstein"
}
```

Attachment: **[trac-2053-symbolic_matrices_slow.patch.gz](https://github.com/sagemath/sage/files/ticket2053/trac-2053-symbolic_matrices_slow.patch.gz)**



---

archive/issue_events_017858.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-02-06T04:08:33Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/2053",
    "title_is": "[negative review] creating symbolic matrices is insanely slow",
    "title_was": "creating symbolic matrices is insanely slow",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2053#event-17858"
}
```



---

archive/issue_comments_010335.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nI cannot apply this due to a failed hunk with the docstring in set_from_list.",
    "created_at": "2008-02-06T04:08:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2053",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2053#issuecomment-10335",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:2'>Comment 2:</a>
I cannot apply this due to a failed hunk with the docstring in set_from_list.



---

archive/issue_comments_010336.json:
```json
{
    "body": "Attachment: **[trac-2053-fix_symbolic.patch.gz](https://github.com/sagemath/sage/files/ticket2053/trac-2053-fix_symbolic.patch.gz)**\n\napply this *INSTEAD*!",
    "created_at": "2008-02-06T19:12:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2053",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2053#issuecomment-10336",
    "user": "https://github.com/williamstein"
}
```

Attachment: **[trac-2053-fix_symbolic.patch.gz](https://github.com/sagemath/sage/files/ticket2053/trac-2053-fix_symbolic.patch.gz)**

apply this *INSTEAD*!



---

archive/issue_comments_010337.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nLooks good to me.",
    "created_at": "2008-02-07T07:59:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2053",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2053#issuecomment-10337",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:3'>Comment 3:</a>
Looks good to me.



---

archive/issue_events_017859.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-02-07T07:59:47Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/2053",
    "title_is": "creating symbolic matrices is insanely slow",
    "title_was": "[negative review] creating symbolic matrices is insanely slow",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2053#event-17859"
}
```



---

archive/issue_events_017860.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-02-07T07:59:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2053",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2053#event-17860"
}
```



---

archive/issue_events_017861.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-02-07T07:59:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2053",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2053#event-17861"
}
```



---

archive/issue_events_017862.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-02-07T10:10:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2053",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2053#event-17862"
}
```



---

archive/issue_events_017863.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-02-07T10:10:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2053",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2053#event-17863"
}
```



---

archive/issue_comments_010338.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nMerged the second patch only in Sage 2.10.2.alpha2",
    "created_at": "2008-02-07T10:10:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2053",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2053#issuecomment-10338",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:4'>Comment 4:</a>
Merged the second patch only in Sage 2.10.2.alpha2
