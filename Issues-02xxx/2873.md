# Issue 2873: [with patch and spkg] Customize JMOL menu

archive/issues_002873.json:
```json
{
    "body": "It would be nice to shorten the menu quite a bit and pick the options that make sense to us.\n\nSee http://chemapps.stolaf.edu/jmol/docs/examples-11/new4.htm point 13 for how to do this.\n\n\n\n**Assignee:** @jasongrout\n\nIssue created by migration from https://trac.sagemath.org/ticket/2873\n\n",
    "closed_at": "2009-02-14T14:51:03Z",
    "created_at": "2008-04-10T22:42:45Z",
    "labels": [
        "component: graphics",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "title": "[with patch and spkg] Customize JMOL menu",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/2873",
    "user": "https://github.com/jasongrout"
}
```
It would be nice to shorten the menu quite a bit and pick the options that make sense to us.

See http://chemapps.stolaf.edu/jmol/docs/examples-11/new4.htm point 13 for how to do this.



**Assignee:** @jasongrout

Issue created by migration from https://trac.sagemath.org/ticket/2873





---

archive/issue_comments_015694.json:
```json
{
    "body": "<a id='comment:1'></a>\nSee also the very nice writeup at http://wiki.jmol.org:81/index.php/Custom_Menus",
    "created_at": "2009-02-13T21:10:04Z",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2873#issuecomment-15694",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:1'></a>
See also the very nice writeup at http://wiki.jmol.org:81/index.php/Custom_Menus



---

archive/issue_comments_015695.json:
```json
{
    "body": "<a id='comment:2'></a>\nWhoever fixes this, please make sure that #1777 is taken care of (which can be fixed by this ticket, I believe).",
    "created_at": "2009-02-13T21:11:12Z",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2873#issuecomment-15695",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:2'></a>
Whoever fixes this, please make sure that #1777 is taken care of (which can be fixed by this ticket, I believe).



---

archive/attachments_002726.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "jmol-custom-menu.patch",
    "asset_url": "tarball://root/attachments/ticket2873/jmol-custom-menu.patch",
    "created_at": "2009-02-14T09:48:17Z",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket2873/jmol-custom-menu.patch",
    "user": "https://github.com/jasongrout"
}
```



---

archive/issue_comments_015696.json:
```json
{
    "body": "Attachment [jmol-custom-menu.patch](https://github.com/sagemath/sage/files/ticket2873/jmol-custom-menu.patch) by @jasongrout created at 2009-02-14 09:48:17",
    "created_at": "2009-02-14T09:48:17Z",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2873#issuecomment-15696",
    "user": "https://github.com/jasongrout"
}
```

Attachment [jmol-custom-menu.patch](https://github.com/sagemath/sage/files/ticket2873/jmol-custom-menu.patch) by @jasongrout created at 2009-02-14 09:48:17



---

archive/issue_comments_015697.json:
```json
{
    "body": "<a id='comment:3'></a>\nWe need both a patch and a new spkg (which contains the custom menu file).  The spkg is up at http://sage.math.washington.edu/home/jason/jmol-11.6.16.spkg (I took the opportunity to update to the latest jmol stable version).",
    "created_at": "2009-02-14T09:49:23Z",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2873#issuecomment-15697",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:3'></a>
We need both a patch and a new spkg (which contains the custom menu file).  The spkg is up at http://sage.math.washington.edu/home/jason/jmol-11.6.16.spkg (I took the opportunity to update to the latest jmol stable version).



---

archive/issue_events_014996.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-02-14T09:49:23Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "rename": {
        "from": "Customize JMOL menu",
        "to": "[with patch and spkg] Customize JMOL menu"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2873#event-14996"
}
```



---

archive/issue_events_014997.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-02-14T09:49:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2873#event-14997"
}
```



---

archive/issue_comments_015698.json:
```json
{
    "body": "<a id='comment:4'></a>\nThis patch and spkg take care of #1777 too.",
    "created_at": "2009-02-14T10:01:53Z",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2873#issuecomment-15698",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:4'></a>
This patch and spkg take care of #1777 too.



---

archive/issue_comments_015699.json:
```json
{
    "body": "**Changing assignee** from @williamstein to @jasongrout.",
    "created_at": "2009-02-14T10:04:12Z",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2873#issuecomment-15699",
    "user": "https://github.com/jasongrout"
}
```

**Changing assignee** from @williamstein to @jasongrout.



---

archive/issue_events_014998.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-02-14T10:04:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2873#event-14998"
}
```



---

archive/issue_events_014999.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans",
    "created_at": "2009-02-14T11:13:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2873#event-14999"
}
```



---

archive/issue_events_015000.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-02-14T13:57:09Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "milestone": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2873#event-15000"
}
```



---

archive/issue_events_015001.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-02-14T13:57:09Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "milestone": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2873#event-15001"
}
```



---

archive/issue_comments_015700.json:
```json
{
    "body": "<a id='comment:7'></a>\nThe spkg is **not** based in the latest jmol.spkg in tree and is missing fixes to spkg-install. Bad Jason :)\n\nTimothy: **always** check the repo and verify that it is based on the previous spkg.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-14T13:57:09Z",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2873#issuecomment-15700",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:7'></a>
The spkg is **not** based in the latest jmol.spkg in tree and is missing fixes to spkg-install. Bad Jason :)

Timothy: **always** check the repo and verify that it is based on the previous spkg.

Cheers,

Michael



---

archive/issue_comments_015701.json:
```json
{
    "body": "<a id='comment:8'></a>\nI will put up a new jmol spkg at #5271.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-14T14:31:14Z",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2873#issuecomment-15701",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:8'></a>
I will put up a new jmol spkg at #5271.

Cheers,

Michael



---

archive/issue_events_015002.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-02-14T14:51:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2873#event-15002"
}
```



---

archive/issue_events_015003.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-02-14T14:51:03Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2873#event-15003"
}
```



---

archive/issue_comments_015702.json:
```json
{
    "body": "<a id='comment:9'></a>\nMerged jmol-custom-menu.patch in Sage 3.3.rc1. \n\nNote that the spkg above was **not** merged, but the one at #5271.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-14T14:51:03Z",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2873#issuecomment-15702",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:9'></a>
Merged jmol-custom-menu.patch in Sage 3.3.rc1. 

Note that the spkg above was **not** merged, but the one at #5271.

Cheers,

Michael
