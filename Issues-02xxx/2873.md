# Issue 2873: [with patch and spkg] Customize JMOL menu

archive/issues_002873.json:
```json
{
    "assignees": [],
    "body": "It would be nice to shorten the menu quite a bit and pick the options that make sense to us.\n\nSee http://chemapps.stolaf.edu/jmol/docs/examples-11/new4.htm point 13 for how to do this.\n\n\n\n**Assignee:** @jasongrout\n\nIssue created by migration from https://trac.sagemath.org/ticket/2873\n\n",
    "closed_at": "2009-02-14T14:51:03Z",
    "created_at": "2008-04-10T22:42:45Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graphics",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "[with patch and spkg] Customize JMOL menu",
    "type": "issue",
    "updated_at": "2009-02-14T14:51:03Z",
    "url": "https://github.com/sagemath/sage/issues/2873",
    "user": "https://github.com/jasongrout"
}
```
It would be nice to shorten the menu quite a bit and pick the options that make sense to us.

See http://chemapps.stolaf.edu/jmol/docs/examples-11/new4.htm point 13 for how to do this.



**Assignee:** @jasongrout

Issue created by migration from https://trac.sagemath.org/ticket/2873





---

archive/issue_events_024459.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-04-10T22:42:45Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2873#event-24459"
}
```



---

archive/issue_events_024460.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-04-10T22:42:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graphics",
    "label_color": "08517b",
    "label_name": "component: graphics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2873#event-24460"
}
```



---

archive/issue_events_024461.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-04-10T22:42:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "008080",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2873#event-24461"
}
```



---

archive/issue_comments_015702.json:
```json
{
    "body": "<a id='comment:1'></a>\nSee also the very nice writeup at http://wiki.jmol.org:81/index.php/Custom_Menus",
    "created_at": "2009-02-13T21:10:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2873#issuecomment-15702",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:1'></a>
See also the very nice writeup at http://wiki.jmol.org:81/index.php/Custom_Menus



---

archive/issue_comments_015703.json:
```json
{
    "body": "<a id='comment:2'></a>\nWhoever fixes this, please make sure that #1777 is taken care of (which can be fixed by this ticket, I believe).",
    "created_at": "2009-02-13T21:11:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2873#issuecomment-15703",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:2'></a>
Whoever fixes this, please make sure that #1777 is taken care of (which can be fixed by this ticket, I believe).



---

archive/issue_comments_015704.json:
```json
{
    "body": "**Attachment:** [jmol-custom-menu.patch.gz](https://github.com/sagemath/sage/files/ticket2873/jmol-custom-menu.patch.gz)",
    "created_at": "2009-02-14T09:48:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2873#issuecomment-15704",
    "user": "https://github.com/jasongrout"
}
```

**Attachment:** [jmol-custom-menu.patch.gz](https://github.com/sagemath/sage/files/ticket2873/jmol-custom-menu.patch.gz)



---

archive/issue_comments_015705.json:
```json
{
    "body": "<a id='comment:3'></a>\nWe need both a patch and a new spkg (which contains the custom menu file).  The spkg is up at http://sage.math.washington.edu/home/jason/jmol-11.6.16.spkg (I took the opportunity to update to the latest jmol stable version).",
    "created_at": "2009-02-14T09:49:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2873#issuecomment-15705",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:3'></a>
We need both a patch and a new spkg (which contains the custom menu file).  The spkg is up at http://sage.math.washington.edu/home/jason/jmol-11.6.16.spkg (I took the opportunity to update to the latest jmol stable version).



---

archive/issue_events_024462.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-02-14T09:49:23Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "rename": {
        "from": "Customize JMOL menu",
        "to": "[with patch and spkg] Customize JMOL menu"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2873#event-24462"
}
```



---

archive/issue_events_024463.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-02-14T09:49:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "008080",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2873#event-24463"
}
```



---

archive/issue_comments_015706.json:
```json
{
    "body": "<a id='comment:4'></a>\nThis patch and spkg take care of #1777 too.",
    "created_at": "2009-02-14T10:01:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2873#issuecomment-15706",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:4'></a>
This patch and spkg take care of #1777 too.



---

archive/issue_comments_015707.json:
```json
{
    "body": "**Changing assignee** from @williamstein to @jasongrout.",
    "created_at": "2009-02-14T10:04:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2873#issuecomment-15707",
    "user": "https://github.com/jasongrout"
}
```

**Changing assignee** from @williamstein to @jasongrout.



---

archive/issue_events_024464.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-02-14T10:04:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "008080",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2873#event-24464"
}
```



---

archive/issue_events_024465.json:
```json
{
    "actor": "https://github.com/sagetrac-TimothyClemans",
    "created_at": "2009-02-14T11:13:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "008080",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2873#event-24465"
}
```



---

archive/issue_events_024466.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-14T13:57:09Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2873#event-24466"
}
```



---

archive/issue_events_024467.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-14T13:57:09Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2873#event-24467"
}
```



---

archive/issue_comments_015708.json:
```json
{
    "body": "<a id='comment:7'></a>\nThe spkg is **not** based in the latest jmol.spkg in tree and is missing fixes to spkg-install. Bad Jason :)\n\nTimothy: **always** check the repo and verify that it is based on the previous spkg.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-14T13:57:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2873#issuecomment-15708",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:7'></a>
The spkg is **not** based in the latest jmol.spkg in tree and is missing fixes to spkg-install. Bad Jason :)

Timothy: **always** check the repo and verify that it is based on the previous spkg.

Cheers,

Michael



---

archive/issue_comments_015709.json:
```json
{
    "body": "<a id='comment:8'></a>\nI will put up a new jmol spkg at #5271.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-14T14:31:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2873#issuecomment-15709",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:8'></a>
I will put up a new jmol spkg at #5271.

Cheers,

Michael



---

archive/issue_events_024468.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-14T14:51:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "008080",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2873#event-24468"
}
```



---

archive/issue_events_024469.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-14T14:51:03Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2873#event-24469"
}
```



---

archive/issue_comments_015710.json:
```json
{
    "body": "<a id='comment:9'></a>\nMerged jmol-custom-menu.patch in Sage 3.3.rc1. \n\nNote that the spkg above was **not** merged, but the one at #5271.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-14T14:51:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2873#issuecomment-15710",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:9'></a>
Merged jmol-custom-menu.patch in Sage 3.3.rc1. 

Note that the spkg above was **not** merged, but the one at #5271.

Cheers,

Michael
