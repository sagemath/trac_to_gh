# Issue 2921: [with spkg, positive review] gcc 4.3: Fix flintqs.spkg build

archive/issues_002921.json:
```json
{
    "body": "Flintqs currently doesn't build with gcc 4.3. The following simple patch fixes that.\n\nCheers,\n\nMichael\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/2921\n\n",
    "closed_at": "2008-04-14T21:56:45Z",
    "created_at": "2008-04-14T21:27:46Z",
    "labels": [
        "component: packages: standard",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0",
    "title": "[with spkg, positive review] gcc 4.3: Fix flintqs.spkg build",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2921",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
Flintqs currently doesn't build with gcc 4.3. The following simple patch fixes that.

Cheers,

Michael

Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/2921





---

archive/issue_comments_021078.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-04-14T21:30:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2921",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2921#issuecomment-21078",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing status from new to assigned.



---

archive/issue_events_010357.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2921",
    "rename": {
        "from": "gcc 4.3: Fix flintqs.spkg build",
        "to": "[with spkg, needs review] gcc 4.3: Fix flintqs.spkg build"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2921#event-10357"
}
```



---

archive/issue_comments_021079.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [flintqs-gcc-4.3-fix.patch](tarball://root/attachments/some-uuid/ticket2921/flintqs-gcc-4.3-fix.patch) by mabshoff created at 2008-04-14 21:30:22\n\nThe updated spkg is at\n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-3.0/alpha5/flintqs-20070817.p3.spkg\n\nCheers,\n\nMichael",
    "created_at": "2008-04-14T21:30:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2921",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2921#issuecomment-21079",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>
Attachment [flintqs-gcc-4.3-fix.patch](tarball://root/attachments/some-uuid/ticket2921/flintqs-gcc-4.3-fix.patch) by mabshoff created at 2008-04-14 21:30:22

The updated spkg is at

http://sage.math.washington.edu/home/mabshoff/release-cycles-3.0/alpha5/flintqs-20070817.p3.spkg

Cheers,

Michael



---

archive/issue_comments_021080.json:
```json
{
    "body": "<a id='comment:2'></a>\nLooks good to me. Also builds fine on gcc 4.1.2.",
    "created_at": "2008-04-14T21:48:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2921",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2921#issuecomment-21080",
    "user": "https://github.com/wjp"
}
```

<a id='comment:2'></a>
Looks good to me. Also builds fine on gcc 4.1.2.



---

archive/issue_events_010358.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2921",
    "rename": {
        "from": "[with spkg, needs review] gcc 4.3: Fix flintqs.spkg build",
        "to": "[with spkg, positive review] gcc 4.3: Fix flintqs.spkg build"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2921#event-10358"
}
```



---

archive/issue_comments_021081.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.0.alpha5",
    "created_at": "2008-04-14T21:56:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2921",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2921#issuecomment-21081",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.0.alpha5



---

archive/issue_events_010359.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-14T21:56:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2921",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2921#event-10359"
}
```



---

archive/issue_comments_021082.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-04-14T21:56:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2921",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2921#issuecomment-21082",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed
