# Issue 2552: [with patch, positive review] fix problems in the combinat reference manual caused by #2432

archive/issues_002552.json:
```json
{
    "body": "\n\nAssignee: tba\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/2552\n\n",
    "closed_at": "2008-03-16T23:16:05Z",
    "created_at": "2008-03-16T21:12:21Z",
    "labels": [
        "component: documentation",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.10.4",
    "title": "[with patch, positive review] fix problems in the combinat reference manual caused by #2432",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2552",
    "user": "https://github.com/mwhansen"
}
```


Assignee: tba

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/2552





---

archive/issue_comments_018248.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [combinat-ref.patch](tarball://root/attachments/some-uuid/ticket2552/combinat-ref.patch) by @mwhansen created at 2008-03-16 21:15:56",
    "created_at": "2008-03-16T21:15:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2552",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2552#issuecomment-18248",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'></a>
Attachment [combinat-ref.patch](tarball://root/attachments/some-uuid/ticket2552/combinat-ref.patch) by @mwhansen created at 2008-03-16 21:15:56



---

archive/issue_events_009006.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2552",
    "rename": {
        "from": "fix problems in the combinat reference manual caused by #2432",
        "to": "[with patch, needs review] fix problems in the combinat reference manual caused by #2432"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2552#event-9006"
}
```



---

archive/issue_events_009007.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2552",
    "rename": {
        "from": "[with patch, needs review] fix problems in the combinat reference manual caused by #2432",
        "to": "[with patch, positive review] fix problems in the combinat reference manual caused by #2432"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2552#event-9007"
}
```



---

archive/issue_comments_018249.json:
```json
{
    "body": "<a id='comment:2'></a>\nPatch looks good to me and will be merged in Sage 2.10.4.final\n\nCheers,\n\nMichael",
    "created_at": "2008-03-16T21:30:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2552",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2552#issuecomment-18249",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Patch looks good to me and will be merged in Sage 2.10.4.final

Cheers,

Michael



---

archive/issue_events_009008.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-03-16T23:16:05Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2552",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2552#event-9008"
}
```



---

archive/issue_comments_018250.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-03-16T23:16:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2552",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2552#issuecomment-18250",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_018251.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 2.10.4.final",
    "created_at": "2008-03-16T23:16:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2552",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2552#issuecomment-18251",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 2.10.4.final
