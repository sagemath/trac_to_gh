# Issue 2880: [With revised patch] Special code for elliptic curve cardinality for j=0 and j=1728

archive/issues_002880.json:
```json
{
    "assignees": [
        "https://github.com/williamstein"
    ],
    "body": "When the new code for point counting on elliptic curves over arbitrary finite fields was implemented, I left handling the special cases j=0 and j=1728 for a rainy day.  These cases were handled in not too bad a way, but as there are special formulas for these cases it was always going to be a good idea to implement them.\n\nNot having any reference which does everything needed here (especially for the really exceptional cases where the characteristic is 2 or 3 and j=0=1728) I worked it all out from scratch, and here is the result.\n\nThere are copious comments and doctests.  I will write up the full justification in due course.  In the meantime I hope we can merge this patch (based on 3.0.alpha1) quite soon!\n\nKeywords: **elliptic curves**\n\nComponent: **algebraic geometry**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/2880_\n\n",
    "closed_at": "2008-04-13T16:03:31Z",
    "created_at": "2008-04-11T20:07:20Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20algebraic%20geometry",
        "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "[With revised patch] Special code for elliptic curve cardinality for j=0 and j=1728",
    "type": "issue",
    "updated_at": "2008-04-14T13:25:42Z",
    "url": "https://github.com/sagemath/sage/issues/2880",
    "user": "https://github.com/JohnCremona"
}
```
When the new code for point counting on elliptic curves over arbitrary finite fields was implemented, I left handling the special cases j=0 and j=1728 for a rainy day.  These cases were handled in not too bad a way, but as there are special formulas for these cases it was always going to be a good idea to implement them.

Not having any reference which does everything needed here (especially for the really exceptional cases where the characteristic is 2 or 3 and j=0=1728) I worked it all out from scratch, and here is the result.

There are copious comments and doctests.  I will write up the full justification in due course.  In the meantime I hope we can merge this patch (based on 3.0.alpha1) quite soon!

Keywords: **elliptic curves**

Component: **algebraic geometry**

_Issue created by migration from https://trac.sagemath.org/ticket/2880_





---

archive/issue_events_030540.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2008-04-11T20:07:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2880",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20algebraic%20geometry",
    "label_color": "0000ff",
    "label_name": "c: algebraic geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2880#event-30540"
}
```



---

archive/issue_events_030541.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2008-04-11T20:07:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2880",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2880#event-30541"
}
```



---

archive/issue_events_030542.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2008-04-11T20:07:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2880",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2880#event-30542"
}
```



---

archive/issue_events_030543.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-04-11T20:07:20Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/2880",
    "subject": "https://github.com/JohnCremona",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2880#event-30543"
}
```



---

archive/issue_comments_015045.json:
```json
{
    "body": "Attachment: **[9384.patch.gz](https://github.com/sagemath/sage/files/ticket2880/9384.patch.gz)**",
    "created_at": "2008-04-11T20:07:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2880",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2880#issuecomment-15045",
    "user": "https://github.com/JohnCremona"
}
```

Attachment: **[9384.patch.gz](https://github.com/sagemath/sage/files/ticket2880/9384.patch.gz)**



---

archive/issue_events_030544.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2008-04-11T20:10:03Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/2880",
    "title_is": "[With patch] Special code for elliptic curve cardinality for j=0 and j=1728",
    "title_was": "Special code for elliptic curve cardinality for j=0 and j=1728",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2880#event-30544"
}
```



---

archive/issue_events_030545.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2008-04-11T20:10:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2880",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2880#event-30545"
}
```



---

archive/issue_comments_015046.json:
```json
{
    "body": "Changed keywords from none to **elliptic curves**",
    "created_at": "2008-04-11T20:10:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2880",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2880#issuecomment-15046",
    "user": "https://github.com/JohnCremona"
}
```

Changed keywords from none to **elliptic curves**



---

archive/issue_comments_015047.json:
```json
{
    "body": "Attachment: **[9467.patch.gz](https://github.com/sagemath/sage/files/ticket2880/9467.patch.gz)**\n\nreplaces previous patch; applies to 3.0.alpha3",
    "created_at": "2008-04-11T21:17:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2880",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2880#issuecomment-15047",
    "user": "https://github.com/JohnCremona"
}
```

Attachment: **[9467.patch.gz](https://github.com/sagemath/sage/files/ticket2880/9467.patch.gz)**

replaces previous patch; applies to 3.0.alpha3



---

archive/issue_comments_015048.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThe original patch was based on 3.0.alpha1 and did not apply to alpha3.  The new one does apply ok to alpha3.",
    "created_at": "2008-04-11T21:18:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2880",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2880#issuecomment-15048",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:2" align="right">comment:2</div>

The original patch was based on 3.0.alpha1 and did not apply to alpha3.  The new one does apply ok to alpha3.



---

archive/issue_events_030546.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2008-04-11T21:18:20Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/2880",
    "title_is": "[With revised patch] Special code for elliptic curve cardinality for j=0 and j=1728",
    "title_was": "[With patch] Special code for elliptic curve cardinality for j=0 and j=1728",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2880#event-30546"
}
```



---

archive/issue_events_030547.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-11T22:49:44Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2880",
    "milestone_number": null,
    "milestone_title": "sage-3.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2880#event-30547"
}
```



---

archive/issue_comments_015049.json:
```json
{
    "body": "Attachment: **[9468.patch.gz](https://github.com/sagemath/sage/files/ticket2880/9468.patch.gz)**\n\nApply after preceding main patch",
    "created_at": "2008-04-12T19:12:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2880",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2880#issuecomment-15049",
    "user": "https://github.com/JohnCremona"
}
```

Attachment: **[9468.patch.gz](https://github.com/sagemath/sage/files/ticket2880/9468.patch.gz)**

Apply after preceding main patch



---

archive/issue_comments_015050.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nTwo small changes to be applied after the main patch:\n\n* In abelian_group() when j=0 or 1728 call cardinality() first, now that it is very fast, as that speeds up the group computation\n\n* In abelian_group(), a small adjustment to speed up the linear_relation() finding for the second generator.",
    "created_at": "2008-04-12T19:12:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2880",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2880#issuecomment-15050",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:4" align="right">comment:4</div>

Two small changes to be applied after the main patch:

* In abelian_group() when j=0 or 1728 call cardinality() first, now that it is very fast, as that speeds up the group computation

* In abelian_group(), a small adjustment to speed up the linear_relation() finding for the second generator.



---

archive/issue_comments_015051.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nPatches look good.",
    "created_at": "2008-04-13T15:11:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2880",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2880#issuecomment-15051",
    "user": "https://github.com/aghitza"
}
```

<div id="comment:5" align="right">comment:5</div>

Patches look good.



---

archive/issue_events_030548.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2008-04-13T15:11:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2880",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2880#event-30548"
}
```



---

archive/issue_events_030549.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2008-04-13T15:11:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2880",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2880#event-30549"
}
```



---

archive/issue_events_030550.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-13T16:03:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2880",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2880#event-30550"
}
```



---

archive/issue_events_030551.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-13T16:03:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2880",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2880#event-30551"
}
```



---

archive/issue_comments_015052.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nMerged 9467.patch and 9468.patch in Sage 3.0.alpha5",
    "created_at": "2008-04-13T16:03:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2880",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2880#issuecomment-15052",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:6" align="right">comment:6</div>

Merged 9467.patch and 9468.patch in Sage 3.0.alpha5



---

archive/issue_comments_015053.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nReplying to [@aghitza](#comment:5):\n> Patches look good.\n\nThanks!  and sorry for the factor_integer() problem which these caused.  John",
    "created_at": "2008-04-14T13:25:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2880",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2880#issuecomment-15053",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:7" align="right">comment:7</div>

Replying to [@aghitza](#comment:5):
> Patches look good.

Thanks!  and sorry for the factor_integer() problem which these caused.  John
