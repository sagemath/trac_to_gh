# Issue 2294: RealDoubleElement _interface_init_ is very poor

archive/issues_002294.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-cwitty"
    ],
    "body": "We see here that _interface_init_() on RDF loses the last few digits of its value, by truncation.\n\n```\nsage: RR(RDF(sin(1)))\n0.841470984807897\nsage: RR(RDF(sin(1))._interface_init_())\n0.841470984808000\n```\n\nI should have a patch for this very soon.\n\n\nComponent: **interfaces**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/2294_\n\n",
    "closed_at": "2008-02-24T21:28:30Z",
    "created_at": "2008-02-24T19:18:35Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20interfaces",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.10.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "RealDoubleElement _interface_init_ is very poor",
    "type": "issue",
    "updated_at": "2008-02-24T21:28:30Z",
    "url": "https://github.com/sagemath/sage/issues/2294",
    "user": "https://github.com/sagetrac-cwitty"
}
```
We see here that _interface_init_() on RDF loses the last few digits of its value, by truncation.

```
sage: RR(RDF(sin(1)))
0.841470984807897
sage: RR(RDF(sin(1))._interface_init_())
0.841470984808000
```

I should have a patch for this very soon.


Component: **interfaces**

_Issue created by migration from https://trac.sagemath.org/ticket/2294_





---

archive/issue_events_023524.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2008-02-24T19:18:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2294",
    "milestone_number": null,
    "milestone_title": "sage-2.10.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2294#event-23524"
}
```



---

archive/issue_events_023525.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2008-02-24T19:18:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2294",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20interfaces",
    "label_color": "0000ff",
    "label_name": "c: interfaces",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2294#event-23525"
}
```



---

archive/issue_events_023526.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2008-02-24T19:18:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2294",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2294#event-23526"
}
```



---

archive/issue_events_023527.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2008-02-24T19:18:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2294",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2294#event-23527"
}
```



---

archive/issue_events_023528.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-02-24T19:18:35Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/2294",
    "subject": "https://github.com/sagetrac-cwitty",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2294#event-23528"
}
```



---

archive/issue_events_023529.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2008-02-24T21:09:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2294",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2294#event-23529"
}
```



---

archive/issue_events_023530.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-02-24T21:09:54Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/2294",
    "subject": "https://github.com/sagetrac-cwitty",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2294#event-23530"
}
```



---

archive/issue_events_023531.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2008-02-24T21:09:54Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/2294",
    "subject": "https://github.com/sagetrac-cwitty",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2294#event-23531"
}
```



---

archive/issue_comments_011321.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[fp-interface-init.patch.gz](https://github.com/sagemath/sage/files/ticket2294/fp-interface-init.patch.gz)**",
    "created_at": "2008-02-24T21:09:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2294",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2294#issuecomment-11321",
    "user": "https://github.com/sagetrac-cwitty"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[fp-interface-init.patch.gz](https://github.com/sagemath/sage/files/ticket2294/fp-interface-init.patch.gz)**



---

archive/issue_events_023532.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-02-24T21:19:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2294",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2294#event-23532"
}
```



---

archive/issue_events_023533.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-02-24T21:19:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2294",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2294#event-23533"
}
```



---

archive/issue_comments_011322.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nREFEREE REPORT\n\n\nWow, this is a really important patch to apply ASAP.  \n\nThere is an English typo in a parenthetical remark:\n\n```\n \t105\t        computer algebra system.  (This the default function used for\n```",
    "created_at": "2008-02-24T21:19:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2294",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2294#issuecomment-11322",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:2" align="right">comment:2</div>

REFEREE REPORT


Wow, this is a really important patch to apply ASAP.  

There is an English typo in a parenthetical remark:

```
 	105	        computer algebra system.  (This the default function used for
```



---

archive/issue_events_023534.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-02-24T21:28:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2294",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2294#event-23534"
}
```



---

archive/issue_events_023535.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-02-24T21:28:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2294",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2294#event-23535"
}
```



---

archive/issue_comments_011323.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged in Sage 2.10.3.alpha0",
    "created_at": "2008-02-24T21:28:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2294",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2294#issuecomment-11323",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged in Sage 2.10.3.alpha0



---

archive/issue_comments_011324.json:
```json
{
    "body": "Attachment: **[trac_2294_fix_small_grammatical_issues.patch.gz](https://github.com/sagemath/sage/files/ticket2294/trac_2294_fix_small_grammatical_issues.patch.gz)**\n\nThe patch corrects some small issue and has already been merged.",
    "created_at": "2008-02-24T22:34:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2294",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2294#issuecomment-11324",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

Attachment: **[trac_2294_fix_small_grammatical_issues.patch.gz](https://github.com/sagemath/sage/files/ticket2294/trac_2294_fix_small_grammatical_issues.patch.gz)**

The patch corrects some small issue and has already been merged.
