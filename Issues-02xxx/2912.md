# Issue 2912: [with spkg, positive review] M4RI should be built with -O3

archive/issues_002912.json:
```json
{
    "body": "* it is fairly straight forward C so it shouldn't break under `-O3`\n* it makes a noticeable speed difference. To echelonise a random 10<sup>4</sup> x 10<sup>4</sup> matrix takes ~ 8 seconds with `-O2` and ~ 6 seconds with `-O3`.\n\nAssignee: @williamstein\n\nKeywords: speed, build system\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/2912\n\n",
    "closed_at": "2008-04-13T23:42:25Z",
    "created_at": "2008-04-13T21:05:11Z",
    "labels": [
        "component: linear algebra"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0",
    "title": "[with spkg, positive review] M4RI should be built with -O3",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2912",
    "user": "https://github.com/malb"
}
```
* it is fairly straight forward C so it shouldn't break under `-O3`
* it makes a noticeable speed difference. To echelonise a random 10<sup>4</sup> x 10<sup>4</sup> matrix takes ~ 8 seconds with `-O2` and ~ 6 seconds with `-O3`.

Assignee: @williamstein

Keywords: speed, build system

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/2912





---

archive/issue_events_010329.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2912",
    "rename": {
        "from": "M4RI should be built with -O3",
        "to": "[with spkg, needs review] M4RI should be built with -O3"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2912#event-10329"
}
```



---

archive/issue_comments_021031.json:
```json
{
    "body": "<a id='comment:1'></a>\nNew SPKG up at:\n\n   http://sage.math.washington.edu/home/malb/libm4ri-20071224.p2.spkg",
    "created_at": "2008-04-13T21:55:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2912#issuecomment-21031",
    "user": "https://github.com/malb"
}
```

<a id='comment:1'></a>
New SPKG up at:

   http://sage.math.washington.edu/home/malb/libm4ri-20071224.p2.spkg



---

archive/issue_comments_021032.json:
```json
{
    "body": "<a id='comment:2'></a>\nSpkg looks good to me. Changes are minimal. We still need an SPKG.txt, but that can be done down the road. Positive review.\n\nCheers,\n\nMichael",
    "created_at": "2008-04-13T23:41:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2912#issuecomment-21032",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Spkg looks good to me. Changes are minimal. We still need an SPKG.txt, but that can be done down the road. Positive review.

Cheers,

Michael



---

archive/issue_events_010330.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2912",
    "rename": {
        "from": "[with spkg, needs review] M4RI should be built with -O3",
        "to": "[with spkg, positive review] M4RI should be built with -O3"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2912#event-10330"
}
```



---

archive/issue_events_010331.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-13T23:41:50Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/2912",
    "milestone": "sage-3.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2912#event-10331"
}
```



---

archive/issue_comments_021033.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-04-13T23:42:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2912#issuecomment-21033",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_010332.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-13T23:42:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2912",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2912#event-10332"
}
```



---

archive/issue_comments_021034.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.0.alpha5",
    "created_at": "2008-04-13T23:42:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2912#issuecomment-21034",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.0.alpha5
