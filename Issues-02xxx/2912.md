# Issue 2912: M4RI should be built with -O3

archive/issues_002912.json:
```json
{
    "body": "* it is fairly straight forward C so it shouldn't break under `-O3`\n* it makes a noticeable speed difference. To echelonise a random 10<sup>4</sup> x 10<sup>4</sup> matrix takes ~ 8 seconds with `-O2` and ~ 6 seconds with `-O3`.\n\n**Assignee:** @williamstein\n\n**Keywords:** speed, build system\n\nIssue created by migration from https://trac.sagemath.org/ticket/2912\n\n",
    "closed_at": "2008-04-13T23:42:25Z",
    "created_at": "2008-04-13T21:05:11Z",
    "labels": [
        "component: linear algebra",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0",
    "title": "M4RI should be built with -O3",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2912",
    "user": "https://github.com/malb"
}
```
* it is fairly straight forward C so it shouldn't break under `-O3`
* it makes a noticeable speed difference. To echelonise a random 10<sup>4</sup> x 10<sup>4</sup> matrix takes ~ 8 seconds with `-O2` and ~ 6 seconds with `-O3`.

**Assignee:** @williamstein

**Keywords:** speed, build system

Issue created by migration from https://trac.sagemath.org/ticket/2912





---

archive/issue_events_015668.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2008-04-13T21:55:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/2912",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2912#event-15668"
}
```



---

archive/issue_comments_016308.json:
```json
{
    "body": "<a id='comment:1'></a>\nNew SPKG up at:\n\n   http://sage.math.washington.edu/home/malb/libm4ri-20071224.p2.spkg",
    "created_at": "2008-04-13T21:55:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2912#issuecomment-16308",
    "user": "https://github.com/malb"
}
```

<a id='comment:1'></a>
New SPKG up at:

   http://sage.math.washington.edu/home/malb/libm4ri-20071224.p2.spkg



---

archive/issue_comments_016309.json:
```json
{
    "body": "<a id='comment:2'></a>\nSpkg looks good to me. Changes are minimal. We still need an SPKG.txt, but that can be done down the road. Positive review.\n\nCheers,\n\nMichael",
    "created_at": "2008-04-13T23:41:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2912#issuecomment-16309",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Spkg looks good to me. Changes are minimal. We still need an SPKG.txt, but that can be done down the road. Positive review.

Cheers,

Michael



---

archive/issue_events_015669.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-13T23:41:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/2912",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2912#event-15669"
}
```



---

archive/issue_events_015670.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-13T23:41:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/2912",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2912#event-15670"
}
```



---

archive/issue_events_015671.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-13T23:41:50Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/2912",
    "milestone": "sage-3.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2912#event-15671"
}
```



---

archive/issue_events_015672.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-13T23:41:50Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/2912",
    "milestone": "sage-3.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2912#event-15672"
}
```



---

archive/issue_events_015673.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-13T23:42:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/2912",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2912#event-15673"
}
```



---

archive/issue_events_015674.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-13T23:42:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2912",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2912#event-15674"
}
```



---

archive/issue_comments_016310.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.0.alpha5",
    "created_at": "2008-04-13T23:42:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2912#issuecomment-16310",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.0.alpha5
