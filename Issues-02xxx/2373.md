# Issue 2373: relicense the javascript part of the sage notebook bsd

archive/issues_002373.json:
```json
{
    "assignees": [],
    "body": "\n```\nHi guys,\nYou have my permission to change the licenses to BSD - all changes are completely fine with me.\n-Alex\n```\n\n```\nboothby@u.washington.edu to me, Alex\n\t\nYou have my permission to change the license on anything I've contributed to the notebook, to whatever suits you.\n```\n\n**Assignee:** boothby\n\nIssue created by migration from https://trac.sagemath.org/ticket/2373\n\n",
    "closed_at": "2008-03-16T19:01:23Z",
    "created_at": "2008-03-03T07:36:25Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20notebook",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.10.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "relicense the javascript part of the sage notebook bsd",
    "type": "issue",
    "updated_at": "2008-03-16T21:07:01Z",
    "url": "https://github.com/sagemath/sage/issues/2373",
    "user": "https://github.com/williamstein"
}
```

```
Hi guys,
You have my permission to change the licenses to BSD - all changes are completely fine with me.
-Alex
```

```
boothby@u.washington.edu to me, Alex
	
You have my permission to change the license on anything I've contributed to the notebook, to whatever suits you.
```

**Assignee:** boothby

Issue created by migration from https://trac.sagemath.org/ticket/2373





---

archive/issue_events_019638.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-03-03T07:36:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2373",
    "milestone_number": null,
    "milestone_title": "sage-2.10.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2373#event-19638"
}
```



---

archive/issue_events_019639.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-03-03T07:36:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2373",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20notebook",
    "label_color": "08517b",
    "label_name": "component: notebook",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2373#event-19639"
}
```



---

archive/issue_events_019640.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-03-03T07:36:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2373",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "008080",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2373#event-19640"
}
```



---

archive/issue_events_019641.json:
```json
{
    "actor": "https://github.com/sagetrac-boothby",
    "created_at": "2008-03-16T19:01:23Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2373",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2373#event-19641"
}
```



---

archive/issue_comments_012609.json:
```json
{
    "body": "<a id='comment:1'></a>\nFixed in 2.10.3.",
    "created_at": "2008-03-16T19:01:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2373",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2373#issuecomment-12609",
    "user": "https://github.com/sagetrac-boothby"
}
```

<a id='comment:1'></a>
Fixed in 2.10.3.



---

archive/issue_comments_012610.json:
```json
{
    "body": "<a id='comment:2'></a>\nWell, as far as I know this patch hasn't been posted or merged. Unless it is part of the interactive patchset. That seems to be the case, i.e. notebook/js.py  states:\n\n```\n###########################################################################\n#       Copyright (C) 2006 William Stein <wstein@gmail.com>\n#                     2006 Tom Boothby <boothby@u.washington.edu>\n#\n#   Released under the *modified* BSD license.\n#     Tom wrote in email to me at wstein@gmail.com on March 2, 2008: \"You have my permission\n#     to change the license on anything I've contributed to the notebook, to whatever suits you.\"\n#\n###########################################################################\n```\n\nTom, if you close any tickets please give a reference to the ticket that solved the issue.\n\nCheers,\n\nMichael",
    "created_at": "2008-03-16T20:03:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2373",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2373#issuecomment-12610",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:2'></a>
Well, as far as I know this patch hasn't been posted or merged. Unless it is part of the interactive patchset. That seems to be the case, i.e. notebook/js.py  states:

```
###########################################################################
#       Copyright (C) 2006 William Stein <wstein@gmail.com>
#                     2006 Tom Boothby <boothby@u.washington.edu>
#
#   Released under the *modified* BSD license.
#     Tom wrote in email to me at wstein@gmail.com on March 2, 2008: "You have my permission
#     to change the license on anything I've contributed to the notebook, to whatever suits you."
#
###########################################################################
```

Tom, if you close any tickets please give a reference to the ticket that solved the issue.

Cheers,

Michael



---

archive/issue_events_019642.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-16T21:07:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/2373",
    "milestone_number": null,
    "milestone_title": "sage-2.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2373#event-19642"
}
```



---

archive/issue_events_019643.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-16T21:07:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2373",
    "milestone_number": null,
    "milestone_title": "sage-2.10.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2373#event-19643"
}
```
