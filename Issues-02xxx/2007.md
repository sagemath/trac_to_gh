# Issue 2007: crap -- the R spkg contains precompiled java files

archive/issues_002007.json:
```json
{
    "body": "```\nsage-2.10.1.rc3/spkg/standard/r-2.6.1.p14/src/tools/getsp.class\nsage-2.10.1.rc3/spkg/standard/r-2.6.1.p14/src/share/java/getsp.class\nsage-2.10.1.rc3/spkg/standard/r-2.6.1.p14/src/doc/html/search/Value.class\nsage-2.10.1.rc3/spkg/standard/r-2.6.1.p14/src/doc/html/search/Tracer.class\nsage-2.10.1.rc3/spkg/standard/r-2.6.1.p14/src/doc/html/search/SearchEngine.class\nsage-2.10.1.rc3/spkg/standard/r-2.6.1.p14/src/doc/html/search/IndexTable.class\nsage-2.10.1.rc3/spkg/standard/r-2.6.1.p14/src/doc/html/search/IndexStream.class\nsage-2.10.1.rc3/spkg/standard/r-2.6.1.p14/src/doc/html/search/IndexEntry.class\n```\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/2007\n\n",
    "closed_at": "2008-07-19T13:17:44Z",
    "created_at": "2008-01-31T23:17:21Z",
    "labels": [
        "component: packages: standard",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0.5",
    "title": "crap -- the R spkg contains precompiled java files",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2007",
    "user": "https://github.com/williamstein"
}
```
```
sage-2.10.1.rc3/spkg/standard/r-2.6.1.p14/src/tools/getsp.class
sage-2.10.1.rc3/spkg/standard/r-2.6.1.p14/src/share/java/getsp.class
sage-2.10.1.rc3/spkg/standard/r-2.6.1.p14/src/doc/html/search/Value.class
sage-2.10.1.rc3/spkg/standard/r-2.6.1.p14/src/doc/html/search/Tracer.class
sage-2.10.1.rc3/spkg/standard/r-2.6.1.p14/src/doc/html/search/SearchEngine.class
sage-2.10.1.rc3/spkg/standard/r-2.6.1.p14/src/doc/html/search/IndexTable.class
sage-2.10.1.rc3/spkg/standard/r-2.6.1.p14/src/doc/html/search/IndexStream.class
sage-2.10.1.rc3/spkg/standard/r-2.6.1.p14/src/doc/html/search/IndexEntry.class
```

Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/2007





---

archive/issue_comments_012951.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-01-31T23:32:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2007",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2007#issuecomment-12951",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing status from new to assigned.



---

archive/issue_comments_012952.json:
```json
{
    "body": "<a id='comment:2'></a>This must be fixed!!",
    "created_at": "2008-03-12T05:00:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2007",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2007#issuecomment-12952",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:2'></a>This must be fixed!!



---

archive/issue_comments_012953.json:
```json
{
    "body": "Changing priority from major to blocker.",
    "created_at": "2008-03-12T05:00:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2007",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2007#issuecomment-12953",
    "user": "https://github.com/williamstein"
}
```

Changing priority from major to blocker.



---

archive/issue_events_004838.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-03-12T05:00:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/2007",
    "milestone": "sage-2.10.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2007#event-4838"
}
```



---

archive/issue_comments_012954.json:
```json
{
    "body": "<a id='comment:3'></a>William did remove the offending Java files in Sage 3.0.5.\n\nCheers,\n\nMichael",
    "created_at": "2008-07-19T13:17:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2007",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2007#issuecomment-12954",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>William did remove the offending Java files in Sage 3.0.5.

Cheers,

Michael



---

archive/issue_events_004839.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-07-19T13:17:44Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2007",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2007#event-4839"
}
```



---

archive/issue_comments_012955.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-07-19T13:17:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2007",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2007#issuecomment-12955",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_004840.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-07-19T13:17:44Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/2007",
    "milestone": "sage-2.10.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2007#event-4840"
}
```



---

archive/issue_events_004841.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-07-19T13:17:44Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/2007",
    "milestone": "sage-3.0.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2007#event-4841"
}
```
