# Issue 2508: Problem converting a Laurent Series from pari to Sage

archive/issues_002508.json:
```json
{
    "body": "It seems that PARI laurent series can only be converted to Sage LaurentSeries if the valuation is >= 0.\n\nSimple example:\n\n```\nsage: R = LaurentSeriesRing(QQ, 'q')\nsage: R(pari('1/x'))\n0                       # Very wrong\nsage: R(pari('1/x + O(x^20)'))\nTraceback (most recent call last)\n[...]\nPariError:  (5)\n```\n\nExample from John Cremona:\n\n```\nsage: E = EllipticCurve('11a1')\nsage: R = LaurentSeriesRing(RationalField(),'q')\nsage: XY = E.pari_mincurve().elltaniyama()\nsage: [R(XY[0]),R(XY[1])]\n```\n\n**Assignee:** @jdemeyer\n\n**CC:**  @jdemeyer\n\n**Keywords:** Laurent series, pari\n\n**Author:** Jeroen Demeyer\n\n**Reviewer:** John Cremona\n\n**Merged:** sage-4.5.3.alpha0\n\nIssue created by migration from https://trac.sagemath.org/ticket/2508\n\n",
    "closed_at": "2010-08-09T09:50:56Z",
    "created_at": "2008-03-13T18:35:51Z",
    "labels": [
        "component: interfaces",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.5.3",
    "title": "Problem converting a Laurent Series from pari to Sage",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/2508",
    "user": "https://github.com/JohnCremona"
}
```
It seems that PARI laurent series can only be converted to Sage LaurentSeries if the valuation is >= 0.

Simple example:

```
sage: R = LaurentSeriesRing(QQ, 'q')
sage: R(pari('1/x'))
0                       # Very wrong
sage: R(pari('1/x + O(x^20)'))
Traceback (most recent call last)
[...]
PariError:  (5)
```

Example from John Cremona:

```
sage: E = EllipticCurve('11a1')
sage: R = LaurentSeriesRing(RationalField(),'q')
sage: XY = E.pari_mincurve().elltaniyama()
sage: [R(XY[0]),R(XY[1])]
```

**Assignee:** @jdemeyer

**CC:**  @jdemeyer

**Keywords:** Laurent series, pari

**Author:** Jeroen Demeyer

**Reviewer:** John Cremona

**Merged:** sage-4.5.3.alpha0

Issue created by migration from https://trac.sagemath.org/ticket/2508





---

archive/issue_events_012800.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-03-13T20:39:54Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/2508",
    "rename": {
        "from": "Probkem converting a Laurent Series from pari to Sage",
        "to": "Problem converting a Laurent Series from pari to Sage"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2508#event-12800"
}
```



---

archive/issue_comments_013401.json:
```json
{
    "body": "<a id='comment:2'></a>\nI cannot replicate John's example (maybe it has already been fixed?):\n\n```\nsage: X = E=EllipticCurve('389a1').modular_parametrization()[0]\nsage: X\nq^-2 + 2*q^-1 + 4 + 7*q + 13*q^2 + 18*q^3 + 31*q^4 + 49*q^5 + 74*q^6 + 111*q^7 + 173*q^8 + 251*q^9 + 379*q^10 + 560*q^11 + 824*q^12 + 1199*q^13 + 1773*q^14 + 2365*q^15 + 3463*q^16 + 4508*q^17 + O(q^18)\nsage: type(X)\n<type 'sage.rings.laurent_series_ring_element.LaurentSeries'>\nsage: version()\n'SAGE Version 2.10.4, Release Date: 2008-03-17'\n```\n\nNotice how modular_parametrization() now returns a Sage LaurentSeries instead of a pari object.",
    "created_at": "2008-03-23T23:41:08Z",
    "issue": "https://github.com/sagemath/sage/issues/2508",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2508#issuecomment-13401",
    "user": "https://github.com/aghitza"
}
```

<a id='comment:2'></a>
I cannot replicate John's example (maybe it has already been fixed?):

```
sage: X = E=EllipticCurve('389a1').modular_parametrization()[0]
sage: X
q^-2 + 2*q^-1 + 4 + 7*q + 13*q^2 + 18*q^3 + 31*q^4 + 49*q^5 + 74*q^6 + 111*q^7 + 173*q^8 + 251*q^9 + 379*q^10 + 560*q^11 + 824*q^12 + 1199*q^13 + 1773*q^14 + 2365*q^15 + 3463*q^16 + 4508*q^17 + O(q^18)
sage: type(X)
<type 'sage.rings.laurent_series_ring_element.LaurentSeries'>
sage: version()
'SAGE Version 2.10.4, Release Date: 2008-03-17'
```

Notice how modular_parametrization() now returns a Sage LaurentSeries instead of a pari object.



---

archive/issue_events_012801.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2008-03-23T23:41:08Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/2508",
    "rename": {
        "from": "Problem converting a Laurent Series from pari to Sage",
        "to": "[already fixed?] Problem converting a Laurent Series from pari to Sage"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2508#event-12801"
}
```



---

archive/issue_events_012802.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2008-03-24T11:17:33Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/2508",
    "rename": {
        "from": "[already fixed?] Problem converting a Laurent Series from pari to Sage",
        "to": "Problem converting a Laurent Series from pari to Sage"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2508#event-12802"
}
```



---

archive/issue_comments_013402.json:
```json
{
    "body": "<a id='comment:3'></a>\nAlex,\n\nThat's right, I changed the code to return a proper Laurent series:\n\n```\n       R = LaurentSeriesRing(RationalField(),'q')\n        XY = self.pari_mincurve().elltaniyama()\n        return [1/R(1/XY[0]),1/R(1/XY[1])]\n```\n\nBut look at what I had to do to coerce the pari Laurent series into the Sage ring R!  X and Y have negative valuation;  I could only get the coercions to work with things of positive valuation, so had to insert two spurious inversions  (for each of X and Y)\n\nThe following shoud work but does not:\n\n```\nsage: E=EllipticCurve('11a1')\nsage: R = LaurentSeriesRing(RationalField(),'q')\nsage: XY = E.pari_mincurve().elltaniyama()\nsage: [R(XY[0]),R(XY[1])]\n```",
    "created_at": "2008-03-24T11:17:33Z",
    "issue": "https://github.com/sagemath/sage/issues/2508",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2508#issuecomment-13402",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:3'></a>
Alex,

That's right, I changed the code to return a proper Laurent series:

```
       R = LaurentSeriesRing(RationalField(),'q')
        XY = self.pari_mincurve().elltaniyama()
        return [1/R(1/XY[0]),1/R(1/XY[1])]
```

But look at what I had to do to coerce the pari Laurent series into the Sage ring R!  X and Y have negative valuation;  I could only get the coercions to work with things of positive valuation, so had to insert two spurious inversions  (for each of X and Y)

The following shoud work but does not:

```
sage: E=EllipticCurve('11a1')
sage: R = LaurentSeriesRing(RationalField(),'q')
sage: XY = E.pari_mincurve().elltaniyama()
sage: [R(XY[0]),R(XY[1])]
```



---

archive/issue_events_012803.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-08-01T15:29:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2508",
    "label": "minor",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2508#event-12803"
}
```



---

archive/issue_comments_013403.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,47 +1,22 @@\n-The elliptic curve function modular_parametrization() returns a list of two pari objects which are Laurent Series in x (of degrees -2, -3).  I wanted to convert these into proper Sage Laurent Series, but that does not work.  However if I invert these (so they have positive degree, i.e. are power series) then I can coerce them into the Laurent series ring, and then invert again!\n+It seems that PARI laurent series can only be converted to Sage LaurentSeries if the valuation is >= 0.\n+\n+Simple example:\n \n ```\n-sage: X = E=EllipticCurve('389a1').modular_parametrization()[0]\n-sage: type(X)\n-<type 'sage.libs.pari.gen.gen'>\n-sage: X\n-x^-2 + 2*x^-1 + 4 + 7*x + 13*x^2 + 18*x^3 + 31*x^4 + 49*x^5 + 74*x^6 + 111*x^7 + 173*x^8 + 251*x^9 + 379*x^10 + 560*x^11 + 824*x^12 + 1199*x^13 + 1773*x^14 + O(x^15)\n-sage: R=LaurentSeriesRing(QQ,'q')\n-sage: R(X)\n----------------------------------------------------------------------------\n-<class 'sage.libs.pari.gen.PariError'>    Traceback (most recent call last)\n-\n-/home/jec/<ipython console> in <module>()\n-\n-/home/jec/sage-2.10.3/local/lib/python2.5/site-packages/sage/rings/laurent_series_ring.py in __call__(self, x, n)\n-    182             return self.gen()**n * x\n-    183         else:\n---> 184             return laurent_series_ring_element.LaurentSeries(self, x, n)\n-    185\n-    186     def _coerce_impl(self, x):\n-\n-/home/jec/laurent_series_ring_element.pyx in sage.rings.laurent_series_ring_element.LaurentSeries.__init__()\n-\n-/home/jec/sage-2.10.3/local/lib/python2.5/site-packages/sage/rings/power_series_ring.py in __call__(self, f, prec, check)\n-    324             v = sage_eval(f.Eltseq())\n-    325             return self(v) * (self.gen(0)**f.Valuation())\n---> 326         return self.__power_series_class(self, f, prec, check=check)\n-    327\n-    328     def construction(self):\n-\n-/home/jec/power_series_poly.pyx in sage.rings.power_series_poly.PowerSeries_poly.__init__()\n-\n-/home/jec/sage-2.10.3/local/lib/python2.5/site-packages/sage/rings/polynomial/polynomial_ring.py in __call__(self, x, check, is_gen, construct, absprec)\n-    237         elif isinstance(x, pari_gen):\n-    238             if x.type() != 't_POL':\n---> 239                 x = x.Polrev()\n-    240\n-    241         C = self.__polynomial_class\n-\n-/home/jec/gen.pyx in sage.libs.pari.gen._pari_trap()\n-\n-<class 'sage.libs.pari.gen.PariError'>:  (8)\n-sage: 1/R(1/X)\n-q^-2 + 2*q^-1 + 4 + 7*q + 13*q^2 + 18*q^3 + 31*q^4 + 49*q^5 + 74*q^6 + 111*q^7 + 173*q^8 + 251*q^9 + 379*q^10 + 560*q^11 + 824*q^12 + 1199*q^13 + 1773*q^14 + 2365*q^15 + 3463*q^16 + 4508*q^17 + O(q^18)\n+sage: R = LaurentSeriesRing(QQ, 'q')\n+sage: R(pari('1/x'))\n+0                       # Very wrong\n+sage: R(pari('1/x + O(x^20)'))\n+Traceback (most recent call last)\n+[...]\n+PariError:  (5)\n ```\n \n+Example from John Cremona:\n+\n+```\n+sage: E = EllipticCurve('11a1')\n+sage: R = LaurentSeriesRing(RationalField(),'q')\n+sage: XY = E.pari_mincurve().elltaniyama()\n+sage: [R(XY[0]),R(XY[1])]\n+```\n``````\n",
    "created_at": "2010-08-01T15:29:35Z",
    "issue": "https://github.com/sagemath/sage/issues/2508",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2508#issuecomment-13403",
    "user": "https://github.com/jdemeyer"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,47 +1,22 @@
-The elliptic curve function modular_parametrization() returns a list of two pari objects which are Laurent Series in x (of degrees -2, -3).  I wanted to convert these into proper Sage Laurent Series, but that does not work.  However if I invert these (so they have positive degree, i.e. are power series) then I can coerce them into the Laurent series ring, and then invert again!
+It seems that PARI laurent series can only be converted to Sage LaurentSeries if the valuation is >= 0.
+
+Simple example:
 
 ```
-sage: X = E=EllipticCurve('389a1').modular_parametrization()[0]
-sage: type(X)
-<type 'sage.libs.pari.gen.gen'>
-sage: X
-x^-2 + 2*x^-1 + 4 + 7*x + 13*x^2 + 18*x^3 + 31*x^4 + 49*x^5 + 74*x^6 + 111*x^7 + 173*x^8 + 251*x^9 + 379*x^10 + 560*x^11 + 824*x^12 + 1199*x^13 + 1773*x^14 + O(x^15)
-sage: R=LaurentSeriesRing(QQ,'q')
-sage: R(X)
----------------------------------------------------------------------------
-<class 'sage.libs.pari.gen.PariError'>    Traceback (most recent call last)
-
-/home/jec/<ipython console> in <module>()
-
-/home/jec/sage-2.10.3/local/lib/python2.5/site-packages/sage/rings/laurent_series_ring.py in __call__(self, x, n)
-    182             return self.gen()**n * x
-    183         else:
---> 184             return laurent_series_ring_element.LaurentSeries(self, x, n)
-    185
-    186     def _coerce_impl(self, x):
-
-/home/jec/laurent_series_ring_element.pyx in sage.rings.laurent_series_ring_element.LaurentSeries.__init__()
-
-/home/jec/sage-2.10.3/local/lib/python2.5/site-packages/sage/rings/power_series_ring.py in __call__(self, f, prec, check)
-    324             v = sage_eval(f.Eltseq())
-    325             return self(v) * (self.gen(0)**f.Valuation())
---> 326         return self.__power_series_class(self, f, prec, check=check)
-    327
-    328     def construction(self):
-
-/home/jec/power_series_poly.pyx in sage.rings.power_series_poly.PowerSeries_poly.__init__()
-
-/home/jec/sage-2.10.3/local/lib/python2.5/site-packages/sage/rings/polynomial/polynomial_ring.py in __call__(self, x, check, is_gen, construct, absprec)
-    237         elif isinstance(x, pari_gen):
-    238             if x.type() != 't_POL':
---> 239                 x = x.Polrev()
-    240
-    241         C = self.__polynomial_class
-
-/home/jec/gen.pyx in sage.libs.pari.gen._pari_trap()
-
-<class 'sage.libs.pari.gen.PariError'>:  (8)
-sage: 1/R(1/X)
-q^-2 + 2*q^-1 + 4 + 7*q + 13*q^2 + 18*q^3 + 31*q^4 + 49*q^5 + 74*q^6 + 111*q^7 + 173*q^8 + 251*q^9 + 379*q^10 + 560*q^11 + 824*q^12 + 1199*q^13 + 1773*q^14 + 2365*q^15 + 3463*q^16 + 4508*q^17 + O(q^18)
+sage: R = LaurentSeriesRing(QQ, 'q')
+sage: R(pari('1/x'))
+0                       # Very wrong
+sage: R(pari('1/x + O(x^20)'))
+Traceback (most recent call last)
+[...]
+PariError:  (5)
 ```
 
+Example from John Cremona:
+
+```
+sage: E = EllipticCurve('11a1')
+sage: R = LaurentSeriesRing(RationalField(),'q')
+sage: XY = E.pari_mincurve().elltaniyama()
+sage: [R(XY[0]),R(XY[1])]
+```
``````




---

archive/issue_events_012804.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-08-01T15:29:35Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/2508",
    "milestone": "sage-4.5.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2508#event-12804"
}
```



---

archive/issue_events_012805.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-08-01T15:29:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2508",
    "milestone": "sage-4.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2508#event-12805"
}
```



---

archive/issue_comments_013404.json:
```json
{
    "body": "**Changing assignee** from @williamstein to @jdemeyer.",
    "created_at": "2010-08-02T20:14:37Z",
    "issue": "https://github.com/sagemath/sage/issues/2508",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2508#issuecomment-13404",
    "user": "https://github.com/jdemeyer"
}
```

**Changing assignee** from @williamstein to @jdemeyer.



---

archive/issue_comments_013405.json:
```json
{
    "body": "**Author:** Jeroen Demeyer",
    "created_at": "2010-08-02T20:14:37Z",
    "issue": "https://github.com/sagemath/sage/issues/2508",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2508#issuecomment-13405",
    "user": "https://github.com/jdemeyer"
}
```

**Author:** Jeroen Demeyer



---

archive/issue_comments_013406.json:
```json
{
    "body": "<a id='comment:6'></a>\nNote also that the answer wrongly gets more terms:\n\n```\nsage: E = EllipticCurve('11a1')\nsage: R = LaurentSeriesRing(RationalField(),'q')\nsage: XY = E.pari_mincurve().elltaniyama()\nsage: XY[0]\nx^-2 + 2*x^-1 + 4 + 5*x + 8*x^2 + x^3 + 7*x^4 - 11*x^5 + 10*x^6 - 12*x^7 - 18*x^8 - 22*x^9 + 26*x^10 - 11*x^11 + 41*x^12 + 44*x^13 - 15*x^14 + O(x^15)\nsage: E.modular_parametrization().power_series()[0]\nq^-2 + 2*q^-1 + 4 + 5*q + 8*q^2 + q^3 + 7*q^4 - 11*q^5 + 10*q^6 - 12*q^7 - 18*q^8 - 22*q^9 + 26*q^10 - 11*q^11 + 41*q^12 + 44*q^13 - 15*q^14 + 19746*q^15 + 51565*q^16 + 150132*q^17 + O(q^18)\n```",
    "created_at": "2010-08-02T20:35:23Z",
    "issue": "https://github.com/sagemath/sage/issues/2508",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2508#issuecomment-13406",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:6'></a>
Note also that the answer wrongly gets more terms:

```
sage: E = EllipticCurve('11a1')
sage: R = LaurentSeriesRing(RationalField(),'q')
sage: XY = E.pari_mincurve().elltaniyama()
sage: XY[0]
x^-2 + 2*x^-1 + 4 + 5*x + 8*x^2 + x^3 + 7*x^4 - 11*x^5 + 10*x^6 - 12*x^7 - 18*x^8 - 22*x^9 + 26*x^10 - 11*x^11 + 41*x^12 + 44*x^13 - 15*x^14 + O(x^15)
sage: E.modular_parametrization().power_series()[0]
q^-2 + 2*q^-1 + 4 + 5*q + 8*q^2 + q^3 + 7*q^4 - 11*q^5 + 10*q^6 - 12*q^7 - 18*q^8 - 22*q^9 + 26*q^10 - 11*q^11 + 41*q^12 + 44*q^13 - 15*q^14 + 19746*q^15 + 51565*q^16 + 150132*q^17 + O(q^18)
```



---

archive/issue_events_012806.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-08-03T07:13:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2508",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2508#event-12806"
}
```



---

archive/issue_events_012807.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-08-03T07:13:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/2508",
    "milestone": "sage-4.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2508#event-12807"
}
```



---

archive/issue_events_012808.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-08-03T07:13:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2508",
    "milestone": "sage-4.5.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2508#event-12808"
}
```



---

archive/issue_comments_013407.json:
```json
{
    "body": "<a id='comment:8'></a>\nPatch looks very good, and applies fine to 4.5.2.rc0.  Testing now..",
    "created_at": "2010-08-04T01:30:17Z",
    "issue": "https://github.com/sagemath/sage/issues/2508",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2508#issuecomment-13407",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:8'></a>
Patch looks very good, and applies fine to 4.5.2.rc0.  Testing now..



---

archive/issue_events_012809.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2010-08-04T01:48:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2508",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2508#event-12809"
}
```



---

archive/issue_events_012810.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2010-08-04T01:48:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2508",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2508#event-12810"
}
```



---

archive/issue_comments_013408.json:
```json
{
    "body": "<a id='comment:9'></a>\n... and all passed (entire Sage library but without -long).",
    "created_at": "2010-08-04T01:48:20Z",
    "issue": "https://github.com/sagemath/sage/issues/2508",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2508#issuecomment-13408",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:9'></a>
... and all passed (entire Sage library but without -long).



---

archive/issue_events_012811.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-08-07T05:03:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2508",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2508#event-12811"
}
```



---

archive/issue_events_012812.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-08-07T05:03:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2508",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2508#event-12812"
}
```



---

archive/issue_comments_013409.json:
```json
{
    "body": "**Reviewer:** John Cremona",
    "created_at": "2010-08-07T05:03:37Z",
    "issue": "https://github.com/sagemath/sage/issues/2508",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2508#issuecomment-13409",
    "user": "https://github.com/qed777"
}
```

**Reviewer:** John Cremona



---

archive/issue_comments_013410.json:
```json
{
    "body": "<a id='comment:10'></a>\nCould someone update the patch with a more descriptive commit string?",
    "created_at": "2010-08-07T05:03:37Z",
    "issue": "https://github.com/sagemath/sage/issues/2508",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2508#issuecomment-13410",
    "user": "https://github.com/qed777"
}
```

<a id='comment:10'></a>
Could someone update the patch with a more descriptive commit string?



---

archive/issue_events_012813.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-08-09T08:31:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2508",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2508#event-12813"
}
```



---

archive/issue_events_012814.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-08-09T08:31:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2508",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2508#event-12814"
}
```



---

archive/attachments_002269.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "2508.patch",
    "asset_url": "tarball://root/attachments/ticket2508/2508.patch",
    "created_at": "2010-08-09T08:31:23Z",
    "issue": "https://github.com/sagemath/sage/issues/2508",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket2508/2508.patch",
    "user": "https://github.com/qed777"
}
```



---

archive/issue_comments_013411.json:
```json
{
    "body": "<a id='comment:11'></a>\nAttachment [2508.patch](https://github.com/sagemath/sage/files/ticket2508/2508.patch) by @qed777 created at 2010-08-09 08:31:23\n\nThanks, Jeroen!",
    "created_at": "2010-08-09T08:31:23Z",
    "issue": "https://github.com/sagemath/sage/issues/2508",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2508#issuecomment-13411",
    "user": "https://github.com/qed777"
}
```

<a id='comment:11'></a>
Attachment [2508.patch](https://github.com/sagemath/sage/files/ticket2508/2508.patch) by @qed777 created at 2010-08-09 08:31:23

Thanks, Jeroen!



---

archive/issue_comments_013412.json:
```json
{
    "body": "**Merged:** sage-4.5.3.alpha0",
    "created_at": "2010-08-09T09:50:56Z",
    "issue": "https://github.com/sagemath/sage/issues/2508",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2508#issuecomment-13412",
    "user": "https://github.com/qed777"
}
```

**Merged:** sage-4.5.3.alpha0



---

archive/issue_events_012815.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-08-09T09:50:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2508",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2508#event-12815"
}
```



---

archive/issue_events_012816.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-08-09T09:50:56Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2508",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2508#event-12816"
}
```
