# Issue 2774: conversion from PolyBoRi to Singular

archive/issues_002774.json:
```json
{
    "assignees": [
        "https://github.com/malb"
    ],
    "body": "\n```\nsage: B.<x,y> = BooleanPolynomialRing(2)\nsage: B._singular_()\n//   characteristic : 2\n//   number of vars : 2\n//        block   1 : ordering lp\n//                  : names    x y\n//        block   2 : ordering C\n// quotient ring from ideal\n_[1]=x2+x\n_[2]=y2+y\n```\n\nCC:  @burcin\n\nKeywords: **polybori**\n\nComponent: **commutative algebra**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/2774_\n\n",
    "closed_at": "2008-04-04T22:15:05Z",
    "created_at": "2008-04-02T16:09:06Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20commutative%20algebra",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "conversion from PolyBoRi to Singular",
    "type": "issue",
    "updated_at": "2008-04-04T22:15:05Z",
    "url": "https://github.com/sagemath/sage/issues/2774",
    "user": "https://github.com/malb"
}
```

```
sage: B.<x,y> = BooleanPolynomialRing(2)
sage: B._singular_()
//   characteristic : 2
//   number of vars : 2
//        block   1 : ordering lp
//                  : names    x y
//        block   2 : ordering C
// quotient ring from ideal
_[1]=x2+x
_[2]=y2+y
```

CC:  @burcin

Keywords: **polybori**

Component: **commutative algebra**

_Issue created by migration from https://trac.sagemath.org/ticket/2774_





---

archive/issue_events_029337.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2008-04-02T16:09:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2774",
    "milestone_number": null,
    "milestone_title": "sage-3.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2774#event-29337"
}
```



---

archive/issue_events_029338.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2008-04-02T16:09:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2774",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20commutative%20algebra",
    "label_color": "0000ff",
    "label_name": "c: commutative algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2774#event-29338"
}
```



---

archive/issue_events_029339.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2008-04-02T16:09:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2774",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2774#event-29339"
}
```



---

archive/issue_events_029340.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2008-04-02T16:09:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2774",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2774#event-29340"
}
```



---

archive/issue_events_029341.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2008-04-02T16:09:06Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/2774",
    "subject": "https://github.com/malb",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2774#event-29341"
}
```



---

archive/issue_events_029342.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2008-04-02T16:09:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2774",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2774#event-29342"
}
```



---

archive/issue_comments_014448.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[polybori_to_singular.patch.gz](https://github.com/sagemath/sage/files/ticket2774/polybori_to_singular.patch.gz)**\n\nLooks good to me.",
    "created_at": "2008-04-04T21:32:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2774",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2774#issuecomment-14448",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[polybori_to_singular.patch.gz](https://github.com/sagemath/sage/files/ticket2774/polybori_to_singular.patch.gz)**

Looks good to me.



---

archive/issue_events_029343.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-04-04T21:32:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2774",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2774#event-29343"
}
```



---

archive/issue_comments_014449.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nMerged in Sage 3.0.alpha1",
    "created_at": "2008-04-04T22:15:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2774",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2774#issuecomment-14449",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Merged in Sage 3.0.alpha1



---

archive/issue_events_029344.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-04T22:15:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2774",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2774#event-29344"
}
```



---

archive/issue_events_029345.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-04T22:15:05Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2774",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2774#event-29345"
}
```
