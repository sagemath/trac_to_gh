# Issue 2446: algebras module lacks many docstrings and tests (score 15.7%)

archive/issues_002446.json:
```json
{
    "assignees": [],
    "body": "Most files in sage/algebras have little or no docstrings and almost no doctests.  This patch makes a start on remedying this, but there is a lot more to be done.\n\n\n**Assignee:** @malb\n\n**Keywords:** algebras modules\n\nIssue created by migration from https://trac.sagemath.org/ticket/2446\n\n",
    "closed_at": "2008-03-09T21:30:26Z",
    "created_at": "2008-03-09T21:22:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "algebras module lacks many docstrings and tests (score 15.7%)",
    "type": "issue",
    "updated_at": "2008-03-09T21:30:26Z",
    "url": "https://github.com/sagemath/sage/issues/2446",
    "user": "https://github.com/JohnCremona"
}
```
Most files in sage/algebras have little or no docstrings and almost no doctests.  This patch makes a start on remedying this, but there is a lot more to be done.


**Assignee:** @malb

**Keywords:** algebras modules

Issue created by migration from https://trac.sagemath.org/ticket/2446





---

archive/issue_events_020336.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2008-03-09T21:22:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2446",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
    "label_color": "08517b",
    "label_name": "component: commutative algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2446#event-20336"
}
```



---

archive/issue_events_020337.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2008-03-09T21:22:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2446",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "008080",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2446#event-20337"
}
```



---

archive/issue_events_020338.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2008-03-09T21:22:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2446",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "008080",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2446#event-20338"
}
```



---

archive/issue_comments_013033.json:
```json
{
    "body": "<a id='comment:1'>**Comment 1:**</a>\nThis duplicates #2445 and can be deleted.  Sorry.",
    "created_at": "2008-03-09T21:23:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2446",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2446#issuecomment-13033",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:1'>**Comment 1:**</a>
This duplicates #2445 and can be deleted.  Sorry.



---

archive/issue_events_020339.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-09T21:30:26Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2446",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2446#event-20339"
}
```



---

archive/issue_events_020340.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-09T21:30:26Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/2446",
    "milestone_number": null,
    "milestone_title": "sage-3.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2446#event-20340"
}
```



---

archive/issue_events_020341.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-09T21:30:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2446",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "008080",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2446#event-20341"
}
```
