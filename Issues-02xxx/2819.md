# Issue 2819: edge_style option for directed graphs

archive/issues_002819.json:
```json
{
    "assignees": [
        "https://github.com/rlmill"
    ],
    "body": "This adds an edge_style option for passing options to the arrow() command that draws edges of directed graphs.  It is a (small) step towards #2817.  In order for more to be done, we have to write graph-drawing (really, vertex and edge-drawing) code, since now we just pass everything off to networkx.\n\n\nComponent: **graph theory**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/2819_\n\n",
    "closed_at": "2008-04-06T05:06:06Z",
    "created_at": "2008-04-06T03:46:46Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "edge_style option for directed graphs",
    "type": "issue",
    "updated_at": "2008-04-06T05:06:06Z",
    "url": "https://github.com/sagemath/sage/issues/2819",
    "user": "https://github.com/jasongrout"
}
```
This adds an edge_style option for passing options to the arrow() command that draws edges of directed graphs.  It is a (small) step towards #2817.  In order for more to be done, we have to write graph-drawing (really, vertex and edge-drawing) code, since now we just pass everything off to networkx.


Component: **graph theory**

_Issue created by migration from https://trac.sagemath.org/ticket/2819_





---

archive/issue_events_029841.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-04-06T03:46:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2819",
    "milestone_number": null,
    "milestone_title": "sage-3.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2819#event-29841"
}
```



---

archive/issue_events_029842.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-04-06T03:46:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2819",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "c: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2819#event-29842"
}
```



---

archive/issue_events_029843.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-04-06T03:46:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2819",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2819#event-29843"
}
```



---

archive/issue_events_029844.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-04-06T03:46:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2819",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2819#event-29844"
}
```



---

archive/issue_events_029845.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-04-06T03:46:46Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/2819",
    "subject": "https://github.com/jasongrout",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2819#event-29845"
}
```



---

archive/issue_comments_014670.json:
```json
{
    "body": "Attachment: **[trac-2819-edge_style.patch.gz](https://github.com/sagemath/sage/files/ticket2819/trac-2819-edge_style.patch.gz)**",
    "created_at": "2008-04-06T03:47:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2819",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2819#issuecomment-14670",
    "user": "https://github.com/jasongrout"
}
```

Attachment: **[trac-2819-edge_style.patch.gz](https://github.com/sagemath/sage/files/ticket2819/trac-2819-edge_style.patch.gz)**



---

archive/issue_events_029846.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2008-04-06T03:48:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2819",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2819#event-29846"
}
```



---

archive/issue_comments_014671.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nLooks good to me.",
    "created_at": "2008-04-06T04:50:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2819",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2819#issuecomment-14671",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:2" align="right">comment:2</div>

Looks good to me.



---

archive/issue_events_029847.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-04-06T04:50:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2819",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2819#event-29847"
}
```



---

archive/issue_events_029848.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-04-06T04:50:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2819",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2819#event-29848"
}
```



---

archive/issue_comments_014672.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged in Sage 3.0.alpha2",
    "created_at": "2008-04-06T05:06:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2819",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2819#issuecomment-14672",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged in Sage 3.0.alpha2



---

archive/issue_events_029849.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-06T05:06:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2819",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2819#event-29849"
}
```



---

archive/issue_events_029850.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-06T05:06:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2819",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2819#event-29850"
}
```
