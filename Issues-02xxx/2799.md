# Issue 2799: [with patch, positive review ]matrix's __getitem__ doesn't respect slice object's step attribute

archive/issues_002799.json:
```json
{
    "body": "Ex: ` A[:,0:4:2]` should return the 0th and 2nd column of A, if possible:\n\n```\nage: A = matrix(ZZ,3,4, [3, 2, -5, 0, 1, -1, 1, -4, 1, 0, 1, -3]); A\n[ 3  2 -5  0]\n[ 1 -1  1 -4]\n[ 1  0  1 -3]\n\nsage: A[:,0:4:2]\n[ 3  2 -5  0]\n[ 1 -1  1 -4]\n[ 1  0  1 -3]\n\n```\n\nAssignee: @dfdeshom\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/2799\n\n",
    "closed_at": "2008-04-05T16:56:53Z",
    "created_at": "2008-04-04T19:09:31Z",
    "labels": [
        "component: linear algebra",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0",
    "title": "[with patch, positive review ]matrix's __getitem__ doesn't respect slice object's step attribute",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2799",
    "user": "https://github.com/dfdeshom"
}
```
Ex: ` A[:,0:4:2]` should return the 0th and 2nd column of A, if possible:

```
age: A = matrix(ZZ,3,4, [3, 2, -5, 0, 1, -1, 1, -4, 1, 0, 1, -3]); A
[ 3  2 -5  0]
[ 1 -1  1 -4]
[ 1  0  1 -3]

sage: A[:,0:4:2]
[ 3  2 -5  0]
[ 1 -1  1 -4]
[ 1  0  1 -3]

```

Assignee: @dfdeshom

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/2799





---

archive/issue_comments_020135.json:
```json
{
    "body": "Attachment [2799.patch](tarball://root/attachments/some-uuid/ticket2799/2799.patch) by @dfdeshom created at 2008-04-04 22:02:48",
    "created_at": "2008-04-04T22:02:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2799#issuecomment-20135",
    "user": "https://github.com/dfdeshom"
}
```

Attachment [2799.patch](tarball://root/attachments/some-uuid/ticket2799/2799.patch) by @dfdeshom created at 2008-04-04 22:02:48



---

archive/issue_comments_020136.json:
```json
{
    "body": "<a id='comment:1'></a>\nI've added a patch which should allow people to do more flexible things such as \n`A[2:-1:-1,3:1:-1] `.",
    "created_at": "2008-04-04T22:05:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2799#issuecomment-20136",
    "user": "https://github.com/dfdeshom"
}
```

<a id='comment:1'></a>
I've added a patch which should allow people to do more flexible things such as 
`A[2:-1:-1,3:1:-1] `.



---

archive/issue_events_009893.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2799",
    "rename": {
        "from": "matrix's __getitem__ doesn't respect slice object's step attribute",
        "to": "[with patch, needs review ]matrix's __getitem__ doesn't respect slice object's step attribute"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2799#event-9893"
}
```



---

archive/issue_events_009894.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/2799",
    "rename": {
        "from": "[with patch, needs review ]matrix's __getitem__ doesn't respect slice object's step attribute",
        "to": "[with patch, positive review ]matrix's __getitem__ doesn't respect slice object's step attribute"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2799#event-9894"
}
```



---

archive/issue_comments_020137.json:
```json
{
    "body": "<a id='comment:2'></a>\nLooks good to me.",
    "created_at": "2008-04-05T10:36:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2799#issuecomment-20137",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:2'></a>
Looks good to me.



---

archive/issue_comments_020138.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-04-05T16:56:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2799#issuecomment-20138",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_020139.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.0.alpha2",
    "created_at": "2008-04-05T16:56:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2799#issuecomment-20139",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.0.alpha2



---

archive/issue_events_009895.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-04-05T16:56:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2799",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2799#event-9895"
}
```
