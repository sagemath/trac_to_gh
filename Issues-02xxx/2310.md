# Issue 2310: bug attaching files in files that are attached

archive/issues_002310.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-cwitty"
    ],
    "body": "I found this bug when testing #1964.  To replicate, create a file a.sage and put \n\n```\nattach b.sage\n```\nin it.  This fails, but {{{attach \"b.sage\"} works.  This was a problem before this patch, so it is NOT the fault of this patch.  \n\nComponent: **misc**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/2310_\n\n",
    "closed_at": "2008-03-05T05:33:19Z",
    "created_at": "2008-02-26T04:15:27Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.10.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "bug attaching files in files that are attached",
    "type": "issue",
    "updated_at": "2008-03-05T05:33:19Z",
    "url": "https://github.com/sagemath/sage/issues/2310",
    "user": "https://github.com/williamstein"
}
```
I found this bug when testing #1964.  To replicate, create a file a.sage and put 

```
attach b.sage
```
in it.  This fails, but {{{attach "b.sage"} works.  This was a problem before this patch, so it is NOT the fault of this patch.  

Component: **misc**

_Issue created by migration from https://trac.sagemath.org/ticket/2310_





---

archive/issue_events_023720.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-02-26T04:15:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/2310",
    "milestone_number": null,
    "milestone_title": "sage-2.10.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2310#event-23720"
}
```



---

archive/issue_events_023721.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-02-26T04:15:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2310",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2310#event-23721"
}
```



---

archive/issue_events_023722.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-02-26T04:15:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2310",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2310#event-23722"
}
```



---

archive/issue_events_023723.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2008-02-26T04:15:27Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/2310",
    "subject": "https://github.com/williamstein",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2310#event-23723"
}
```



---

archive/issue_events_023724.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-02-26T04:15:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2310",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2310#event-23724"
}
```



---

archive/issue_comments_011444.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[sage-2310.patch.gz](https://github.com/sagemath/sage/files/ticket2310/sage-2310.patch.gz)**\n\nThis likely depends on #1964.",
    "created_at": "2008-02-26T04:17:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2310#issuecomment-11444",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[sage-2310.patch.gz](https://github.com/sagemath/sage/files/ticket2310/sage-2310.patch.gz)**

This likely depends on #1964.



---

archive/issue_comments_011445.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nNOTE: This could likely be done better after #1964 is done by using the method split_file_names(line): in interpreter.py?",
    "created_at": "2008-02-26T04:35:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2310#issuecomment-11445",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:2" align="right">comment:2</div>

NOTE: This could likely be done better after #1964 is done by using the method split_file_names(line): in interpreter.py?



---

archive/issue_comments_011446.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nRegarding my comment about split_file_names -- yes it could be done that way, but split_file_names is a VERY complicated function whereas _strip_quotes which is included in this patch is very simple and easy to use/understand. So I'm not convinced by my previous suggestion.",
    "created_at": "2008-02-26T04:39:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2310#issuecomment-11446",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:3" align="right">comment:3</div>

Regarding my comment about split_file_names -- yes it could be done that way, but split_file_names is a VERY complicated function whereas _strip_quotes which is included in this patch is very simple and easy to use/understand. So I'm not convinced by my previous suggestion.



---

archive/issue_events_023725.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-03-05T00:40:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/2310",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2310#event-23725"
}
```



---

archive/issue_comments_011447.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nWorks for me in 2.10.3.rc1",
    "created_at": "2008-03-05T00:40:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2310#issuecomment-11447",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:4" align="right">comment:4</div>

Works for me in 2.10.3.rc1



---

archive/issue_events_023726.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-05T05:33:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/2310",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2310#event-23726"
}
```



---

archive/issue_events_023727.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-03-05T05:33:19Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/2310",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/2310#event-23727"
}
```



---

archive/issue_comments_011448.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nMerged in Sage 2.10.3.rc2",
    "created_at": "2008-03-05T05:33:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/2310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/2310#issuecomment-11448",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:5" align="right">comment:5</div>

Merged in Sage 2.10.3.rc2
