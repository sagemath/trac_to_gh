...
<a id='comment:1'></a>In fact, naively, I would have expected that if
`F(arg)`
returns successfully then
`F(arg, check=False)`
will as well. To me this looks like `check=False` does more than turn off checks on whether input is valid -- it changes what valid input is!
...
<a id='comment:2'></a>Replying to [ticket:8197 cremona]:
...
<a id='comment:3'></a>You are quite right -- the code run when check=True converts the input into a sequence.
