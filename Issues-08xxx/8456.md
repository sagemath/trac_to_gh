# Issue 8456: lazy import improvements

archive/issues_008456.json:
```json
{
    "assignees": [],
    "body": "1. Cythonize for speed.\n2. Insert original object into namespace on resolution\n3. Support import *\n\nRelated: #7502, #8254.\n\nApply only\n1. [attachment: 8456-lazy-import-cython-rebase-4.6.patch.gz](https://github.com/sagemath/sage/files/ticket8456/8456-lazy-import-cython-rebase-4.6.patch.gz)\n2. [attachment: 8456-lazy-import-enhancements-4.6.patch.gz](https://github.com/sagemath/sage/files/ticket8456/8456-lazy-import-enhancements-4.6.patch.gz)\n3. [attachment: 8456-lazy-import-documentation.patch.gz](https://github.com/sagemath/sage/files/ticket8456/8456-lazy-import-documentation.patch.gz)\n\nCC:  @rishikesha @jasongrout @rlmill\n\nAuthor: **Robert Bradshaw**\n\nReviewer: **Luis Felipe Tabera Alonso**\n\nMerged: **sage-4.6.2.alpha3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8456_\n\n",
    "closed_at": "2011-01-26T22:26:28Z",
    "created_at": "2010-03-06T08:52:13Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.6.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "lazy import improvements",
    "type": "issue",
    "updated_at": "2011-01-26T22:26:28Z",
    "url": "https://github.com/sagemath/sage/issues/8456",
    "user": "https://github.com/robertwb"
}
```
1. Cythonize for speed.
2. Insert original object into namespace on resolution
3. Support import *

Related: #7502, #8254.

Apply only
1. [attachment: 8456-lazy-import-cython-rebase-4.6.patch.gz](https://github.com/sagemath/sage/files/ticket8456/8456-lazy-import-cython-rebase-4.6.patch.gz)
2. [attachment: 8456-lazy-import-enhancements-4.6.patch.gz](https://github.com/sagemath/sage/files/ticket8456/8456-lazy-import-enhancements-4.6.patch.gz)
3. [attachment: 8456-lazy-import-documentation.patch.gz](https://github.com/sagemath/sage/files/ticket8456/8456-lazy-import-documentation.patch.gz)

CC:  @rishikesha @jasongrout @rlmill

Author: **Robert Bradshaw**

Reviewer: **Luis Felipe Tabera Alonso**

Merged: **sage-4.6.2.alpha3**

_Issue created by migration from https://trac.sagemath.org/ticket/8456_





---

archive/issue_events_093593.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-03-06T08:52:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "milestone_number": null,
    "milestone_title": "sage-4.6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8456#event-93593"
}
```



---

archive/issue_events_093594.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-03-06T08:52:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8456#event-93594"
}
```



---

archive/issue_events_093595.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-03-06T08:52:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8456#event-93595"
}
```



---

archive/issue_comments_068905.json:
```json
{
    "body": "Attachment: **[8456-lazy-import-cython.patch.gz](https://github.com/sagemath/sage/files/ticket8456/8456-lazy-import-cython.patch.gz)**\n\nAttachment: **[8456-lazy-import-enhancements.patch.gz](https://github.com/sagemath/sage/files/ticket8456/8456-lazy-import-enhancements.patch.gz)**",
    "created_at": "2010-03-06T09:37:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68905",
    "user": "https://github.com/robertwb"
}
```

Attachment: **[8456-lazy-import-cython.patch.gz](https://github.com/sagemath/sage/files/ticket8456/8456-lazy-import-cython.patch.gz)**

Attachment: **[8456-lazy-import-enhancements.patch.gz](https://github.com/sagemath/sage/files/ticket8456/8456-lazy-import-enhancements.patch.gz)**



---

archive/issue_events_093596.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-03-06T09:39:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8456#event-93596"
}
```



---

archive/issue_comments_068906.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nI split it up into two patches for easier refereeing. The first simply moves lazy_import.py to lazy_import.pyx.",
    "created_at": "2010-03-06T09:39:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68906",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:1'>Comment 1:</a>
I split it up into two patches for easier refereeing. The first simply moves lazy_import.py to lazy_import.pyx.



---

archive/issue_comments_068907.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nI started with importing the first patch only. I tried to lazy-import ZZ, but it did not properly work:\n\n```\nsage: from sage.misc.lazy_import import lazy_import\nsage: lazy_import('sage.rings.all', 'ZZ')\nsage: type(ZZ)\n<type 'sage.rings.integer_ring.IntegerRing_class'>\n```\nAccording to the doc tests, the result should be different.\n\nI then imported the second patch. When starting sage, I got\n\n```\n---------------------------------------------------------------------------\nOSError                                   Traceback (most recent call last)\n\n/home/king/SAGE/sage-4.3.1/local/lib/python2.6/site-packages/IPython/ipmaker.pyc in force_import(modname)\n     64         reload(sys.modules[modname])\n     65     else:\n---> 66         __import__(modname)\n     67\n     68\n\n/home/king/SAGE/sage-4.3.1/local/bin/ipy_profile_sage.py in <module>()\n      5     preparser(True)\n      6\n----> 7     import sage.all_cmdline\n      8     sage.all_cmdline._init_cmdline(globals())\n      9\n\n/home/king/SAGE/sage-4.3.1/local/lib/python2.6/site-packages/sage/all_cmdline.py in <module>()\n     12 try:\n     13\n---> 14     from sage.all import *\n     15     from sage.calculus.predefined import x\n     16     preparser(on=True)\n\n/home/king/SAGE/sage-4.3.1/local/lib/python2.6/site-packages/sage/all.py in <module>()\n    323 # Cache the contents of star imports.\n\n    324 import sage.misc.lazy_import\n--> 325 sage.misc.lazy_import.save_cache_file()\n    326\n    327\n\n/home/king/SAGE/sage-4.3.1/local/lib/python2.6/site-packages/sage/misc/lazy_import.so in sage.misc.lazy_import.save_cache_file (sage/misc/lazy_import.c:1977)()\n\nOSError: [Errno 18] Invalid cross-device link\nError importing ipy_profile_sage - perhaps you should run %upgrade?\nWARNING: Loading of ipy_profile_sage failed.\n```\nNo idea what this means. But I think it is \"needs work\".\n\nCheers, Simon",
    "created_at": "2010-03-19T10:48:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68907",
    "user": "https://github.com/simon-king-jena"
}
```

<a id='comment:2'>Comment 2:</a>
I started with importing the first patch only. I tried to lazy-import ZZ, but it did not properly work:

```
sage: from sage.misc.lazy_import import lazy_import
sage: lazy_import('sage.rings.all', 'ZZ')
sage: type(ZZ)
<type 'sage.rings.integer_ring.IntegerRing_class'>
```
According to the doc tests, the result should be different.

I then imported the second patch. When starting sage, I got

```
---------------------------------------------------------------------------
OSError                                   Traceback (most recent call last)

/home/king/SAGE/sage-4.3.1/local/lib/python2.6/site-packages/IPython/ipmaker.pyc in force_import(modname)
     64         reload(sys.modules[modname])
     65     else:
---> 66         __import__(modname)
     67
     68

/home/king/SAGE/sage-4.3.1/local/bin/ipy_profile_sage.py in <module>()
      5     preparser(True)
      6
----> 7     import sage.all_cmdline
      8     sage.all_cmdline._init_cmdline(globals())
      9

/home/king/SAGE/sage-4.3.1/local/lib/python2.6/site-packages/sage/all_cmdline.py in <module>()
     12 try:
     13
---> 14     from sage.all import *
     15     from sage.calculus.predefined import x
     16     preparser(on=True)

/home/king/SAGE/sage-4.3.1/local/lib/python2.6/site-packages/sage/all.py in <module>()
    323 # Cache the contents of star imports.

    324 import sage.misc.lazy_import
--> 325 sage.misc.lazy_import.save_cache_file()
    326
    327

/home/king/SAGE/sage-4.3.1/local/lib/python2.6/site-packages/sage/misc/lazy_import.so in sage.misc.lazy_import.save_cache_file (sage/misc/lazy_import.c:1977)()

OSError: [Errno 18] Invalid cross-device link
Error importing ipy_profile_sage - perhaps you should run %upgrade?
WARNING: Loading of ipy_profile_sage failed.
```
No idea what this means. But I think it is "needs work".

Cheers, Simon



---

archive/issue_events_093597.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2010-03-19T10:48:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8456#event-93597"
}
```



---

archive/issue_events_093598.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2010-03-19T10:48:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8456#event-93598"
}
```



---

archive/issue_comments_068908.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nPS:\n\nWhen quitting sage after the unsuccessful start, I got\n\n```\nsage: quit\n---------------------------------------------------------------------------\nTypeError                                 Traceback (most recent call last)\n\n/home/king/SAGE/sage-4.3.1/local/bin/<ipython console> in <module>()\n\n/home/king/SAGE/sage-4.3.1/local/lib/python2.6/site-packages/IPython/iplib.pyc in ipmagic(self, arg_s)\n    951         else:\n    952             magic_args = self.var_expand(magic_args,1)\n--> 953             return fn(magic_args)\n    954\n    955     def ipalias(self,arg_s):\n\n/home/king/SAGE/sage-4.3.1/local/lib/python2.6/site-packages/IPython/Magic.pyc in magic_quit(self, parameter_s)\n   2478         \"\"\"Exit IPython, confirming if configured to do so (like %exit)\"\"\"\n   2479\n-> 2480         self.shell.exit()\n   2481\n   2482     def magic_Exit(self, parameter_s=''):\n\n/home/king/SAGE/sage-4.3.1/local/lib/python2.6/site-packages/sage/all.py in _quit_sage_(self)\n    292         self.exit_now = True\n    293     if self.exit_now:\n--> 294         quit_sage()\n    295         self.exit_now = True\n    296\n\nTypeError: 'NoneType' object is not callable\n```\nEven worse, hg_sage does not work. So, it is more difficult now to repair my sage installation...",
    "created_at": "2010-03-19T10:50:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68908",
    "user": "https://github.com/simon-king-jena"
}
```

<a id='comment:3'>Comment 3:</a>
PS:

When quitting sage after the unsuccessful start, I got

```
sage: quit
---------------------------------------------------------------------------
TypeError                                 Traceback (most recent call last)

/home/king/SAGE/sage-4.3.1/local/bin/<ipython console> in <module>()

/home/king/SAGE/sage-4.3.1/local/lib/python2.6/site-packages/IPython/iplib.pyc in ipmagic(self, arg_s)
    951         else:
    952             magic_args = self.var_expand(magic_args,1)
--> 953             return fn(magic_args)
    954
    955     def ipalias(self,arg_s):

/home/king/SAGE/sage-4.3.1/local/lib/python2.6/site-packages/IPython/Magic.pyc in magic_quit(self, parameter_s)
   2478         """Exit IPython, confirming if configured to do so (like %exit)"""
   2479
-> 2480         self.shell.exit()
   2481
   2482     def magic_Exit(self, parameter_s=''):

/home/king/SAGE/sage-4.3.1/local/lib/python2.6/site-packages/sage/all.py in _quit_sage_(self)
    292         self.exit_now = True
    293     if self.exit_now:
--> 294         quit_sage()
    295         self.exit_now = True
    296

TypeError: 'NoneType' object is not callable
```
Even worse, hg_sage does not work. So, it is more difficult now to repair my sage installation...



---

archive/issue_comments_068909.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nHmm... is this on top of an alpha? Worked fine for me. \n\nAs for backing out, you can do \"sage -hg rollback\"",
    "created_at": "2010-03-19T17:31:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68909",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:4'>Comment 4:</a>
Hmm... is this on top of an alpha? Worked fine for me. 

As for backing out, you can do "sage -hg rollback"



---

archive/issue_comments_068910.json:
```json
{
    "body": "Attachment: **[8456-lazy-import-fixes.patch.gz](https://github.com/sagemath/sage/files/ticket8456/8456-lazy-import-fixes.patch.gz)**",
    "created_at": "2010-06-22T20:45:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68910",
    "user": "https://github.com/robertwb"
}
```

Attachment: **[8456-lazy-import-fixes.patch.gz](https://github.com/sagemath/sage/files/ticket8456/8456-lazy-import-fixes.patch.gz)**



---

archive/issue_events_093599.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-06-22T20:46:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8456#event-93599"
}
```



---

archive/issue_events_093600.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-06-22T20:46:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8456#event-93600"
}
```



---

archive/issue_comments_068911.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nOK, the problem was I was using os.rename rather than, say, shutil.move, which caused errors when the DOT_SAGE file was not on the same filesystem as temp files. Glad you caught that, ready for another review.",
    "created_at": "2010-06-22T20:46:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68911",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:5'>Comment 5:</a>
OK, the problem was I was using os.rename rather than, say, shutil.move, which caused errors when the DOT_SAGE file was not on the same filesystem as temp files. Glad you caught that, ready for another review.



---

archive/issue_comments_068912.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nSimon, could you look at this again, when it's convenient?  I can try to review this, but I'm not sure that I'm sufficiently familiar with Cython.\n\nFor me, all three patches apply cleanly to 4.5.3.alpha1 (using a Mercurial queue).",
    "created_at": "2010-08-20T01:26:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68912",
    "user": "https://github.com/qed777"
}
```

<a id='comment:6'>Comment 6:</a>
Simon, could you look at this again, when it's convenient?  I can try to review this, but I'm not sure that I'm sufficiently familiar with Cython.

For me, all three patches apply cleanly to 4.5.3.alpha1 (using a Mercurial queue).



---

archive/issue_comments_068913.json:
```json
{
    "body": "Author: **Robert Bradshaw**",
    "created_at": "2010-08-20T01:26:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68913",
    "user": "https://github.com/qed777"
}
```

Author: **Robert Bradshaw**



---

archive/issue_comments_068914.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,5 @@\n-1) Cythonize for speed.\n-2) Insert original object into namespace on resolution\n-3) Support import *\n+1. Cythonize for speed.\n+2. Insert original object into namespace on resolution\n+3. Support import *\n+\n+Related: #7502, #8254.\n``````\n",
    "created_at": "2010-09-07T09:53:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68914",
    "user": "https://github.com/qed777"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,5 @@
-1) Cythonize for speed.
-2) Insert original object into namespace on resolution
-3) Support import *
+1. Cythonize for speed.
+2. Insert original object into namespace on resolution
+3. Support import *
+
+Related: #7502, #8254.
``````




---

archive/issue_comments_068915.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nLots of people want to see a faster Sage startup; is anyone willing to review this?",
    "created_at": "2010-10-20T05:27:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68915",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:10'>Comment 10:</a>
Lots of people want to see a faster Sage startup; is anyone willing to review this?



---

archive/issue_comments_068916.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nrebase 8456-lazy-import-cython.patch to sage 4.6\n\npatches apply and doctest pass. The code looks ok, but I do not know (yet) cython nor python introspection to give a positive review.",
    "created_at": "2010-11-04T12:33:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68916",
    "user": "https://github.com/lftabera"
}
```

<a id='comment:11'>Comment 11:</a>
rebase 8456-lazy-import-cython.patch to sage 4.6

patches apply and doctest pass. The code looks ok, but I do not know (yet) cython nor python introspection to give a positive review.



---

archive/issue_events_093601.json:
```json
{
    "actor": "https://github.com/lftabera",
    "created_at": "2010-11-04T15:01:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8456#event-93601"
}
```



---

archive/issue_events_093602.json:
```json
{
    "actor": "https://github.com/lftabera",
    "created_at": "2010-11-04T15:01:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8456#event-93602"
}
```



---

archive/issue_comments_068917.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nAs a user, I would be puzzled if I found at the begining of the session:\n\n```\nsage: InfinityRing\n<sage.misc.lazy_import.LazyImport object at 0x7f51dca0e290>\nsage: InfinityRing()\nZero\nsage: InfinityRing\nThe Infinity Ring\n```\n\nSo I think that LazyImport should have a `__repr__` method that inserts the real object in the namespace.\n\nThe problem is that once you start with this kind of things you do not know where to stop...\n\n```\nsage: infinity in InfinityRing\nTypeError: argument of type 'sage.misc.lazy_import.LazyImport' is not iterable\n```\n\nSo, what about `__repr__`, `__contains__`, `_latex_`, `__cmp__` ? Is it worth to add them to the class? Should this be in another ticket?",
    "created_at": "2010-11-04T15:01:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68917",
    "user": "https://github.com/lftabera"
}
```

<a id='comment:12'>Comment 12:</a>
As a user, I would be puzzled if I found at the begining of the session:

```
sage: InfinityRing
<sage.misc.lazy_import.LazyImport object at 0x7f51dca0e290>
sage: InfinityRing()
Zero
sage: InfinityRing
The Infinity Ring
```

So I think that LazyImport should have a `__repr__` method that inserts the real object in the namespace.

The problem is that once you start with this kind of things you do not know where to stop...

```
sage: infinity in InfinityRing
TypeError: argument of type 'sage.misc.lazy_import.LazyImport' is not iterable
```

So, what about `__repr__`, `__contains__`, `_latex_`, `__cmp__` ? Is it worth to add them to the class? Should this be in another ticket?



---

archive/issue_events_093603.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-11-04T17:57:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8456#event-93603"
}
```



---

archive/issue_events_093604.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-11-04T17:57:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8456#event-93604"
}
```



---

archive/issue_comments_068918.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nHmm... it looks like I need to manually add the typeslot methods, as they don't get forwarded with getattr. Thanks for catching this, I'll post another patch.",
    "created_at": "2010-11-04T17:57:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68918",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:13'>Comment 13:</a>
Hmm... it looks like I need to manually add the typeslot methods, as they don't get forwarded with getattr. Thanks for catching this, I'll post another patch.



---

archive/issue_comments_068919.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nApply only 8456-lazy-import-cython-rebase-4.6.patch and 8456-lazy-import-enhancements-4.6.patch. This fixes the issue with `__repr__` and all the other special methods.",
    "created_at": "2010-11-07T06:13:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68919",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:14'>Comment 14:</a>
Apply only 8456-lazy-import-cython-rebase-4.6.patch and 8456-lazy-import-enhancements-4.6.patch. This fixes the issue with `__repr__` and all the other special methods.



---

archive/issue_events_093605.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-11-07T06:13:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8456#event-93605"
}
```



---

archive/issue_events_093606.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-11-07T06:13:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8456#event-93606"
}
```



---

archive/issue_comments_068920.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -3,3 +3,7 @@\n 3. Support import *\n \n Related: #7502, #8254.\n+\n+Apply only\n+1. [attachment: 8456-lazy-import-cython-rebase-4.6.patch.gz](https://github.com/sagemath/sage/files/ticket8456/8456-lazy-import-cython-rebase-4.6.patch.gz)\n+2. [attachment: 8456-lazy-import-enhancements-4.6.patch.gz](https://github.com/sagemath/sage/files/ticket8456/8456-lazy-import-enhancements-4.6.patch.gz)\n``````\n",
    "created_at": "2010-11-26T11:00:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68920",
    "user": "https://github.com/rlmill"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -3,3 +3,7 @@
 3. Support import *
 
 Related: #7502, #8254.
+
+Apply only
+1. [attachment: 8456-lazy-import-cython-rebase-4.6.patch.gz](https://github.com/sagemath/sage/files/ticket8456/8456-lazy-import-cython-rebase-4.6.patch.gz)
+2. [attachment: 8456-lazy-import-enhancements-4.6.patch.gz](https://github.com/sagemath/sage/files/ticket8456/8456-lazy-import-enhancements-4.6.patch.gz)
``````




---

archive/issue_comments_068921.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nReplying to [@robertwb](#comment%3A14):\n> Apply only 8456-lazy-import-cython-rebase-4.6.patch and 8456-lazy-import-enhancements-4.6.patch. This fixes the issue with `__repr__` and all the other special methods. \n\nAre you sure that's right?\n\n```\nrlmill@fermat:~/sage-4.6.1.alpha2/devel/sage-main$ hg qpush\napplying 8456-lazy-import-cython-rebase-4.6.patch\nunable to find 'sage/misc/lazy_import.pyx' for patching\n8 out of 8 hunks FAILED -- saving rejects to file sage/misc/lazy_import.pyx.rej\npatch failed, unable to continue (try -v)\npatch failed, rejects left in working dir\nerrors during apply, please fix and refresh 8456-lazy-import-cython-rebase-4.6.patch\n```",
    "created_at": "2010-11-26T13:29:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68921",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:16'>Comment 16:</a>
Replying to [@robertwb](#comment%3A14):
> Apply only 8456-lazy-import-cython-rebase-4.6.patch and 8456-lazy-import-enhancements-4.6.patch. This fixes the issue with `__repr__` and all the other special methods. 

Are you sure that's right?

```
rlmill@fermat:~/sage-4.6.1.alpha2/devel/sage-main$ hg qpush
applying 8456-lazy-import-cython-rebase-4.6.patch
unable to find 'sage/misc/lazy_import.pyx' for patching
8 out of 8 hunks FAILED -- saving rejects to file sage/misc/lazy_import.pyx.rej
patch failed, unable to continue (try -v)
patch failed, rejects left in working dir
errors during apply, please fix and refresh 8456-lazy-import-cython-rebase-4.6.patch
```



---

archive/issue_events_093607.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-11-26T13:29:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8456#event-93607"
}
```



---

archive/issue_events_093608.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-11-26T13:29:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8456#event-93608"
}
```



---

archive/issue_events_093609.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-11-26T16:45:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8456#event-93609"
}
```



---

archive/issue_events_093610.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-11-26T16:45:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8456#event-93610"
}
```



---

archive/issue_comments_068922.json:
```json
{
    "body": "<a id='comment:17'>Comment 17:</a>\nSorry, I missed the dependency... :/",
    "created_at": "2010-11-26T16:45:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68922",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:17'>Comment 17:</a>
Sorry, I missed the dependency... :/



---

archive/issue_comments_068923.json:
```json
{
    "body": "Attachment: **[8456-lazy-import-cython-rebase-4.6.patch.gz](https://github.com/sagemath/sage/files/ticket8456/8456-lazy-import-cython-rebase-4.6.patch.gz)**\n\napply instead of 8456-lazy-import-cython.patch",
    "created_at": "2010-12-02T06:31:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68923",
    "user": "https://github.com/robertwb"
}
```

Attachment: **[8456-lazy-import-cython-rebase-4.6.patch.gz](https://github.com/sagemath/sage/files/ticket8456/8456-lazy-import-cython-rebase-4.6.patch.gz)**

apply instead of 8456-lazy-import-cython.patch



---

archive/issue_comments_068924.json:
```json
{
    "body": "<a id='comment:18'>Comment 18:</a>\nThe code looks good. Is there a reason why `_cmp_` does not have a test?",
    "created_at": "2010-12-04T17:00:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68924",
    "user": "https://github.com/lftabera"
}
```

<a id='comment:18'>Comment 18:</a>
The code looks good. Is there a reason why `_cmp_` does not have a test?



---

archive/issue_comments_068925.json:
```json
{
    "body": "<a id='comment:19'>Comment 19:</a>\n{{__cmp__}} does have a test. \n\nThe buildbot needs to know to apply 8456-lazy-import-cython-rebase-4.6.patch and 8456-lazy-import-enhancements-4.6.patch",
    "created_at": "2010-12-04T19:08:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68925",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:19'>Comment 19:</a>
{{__cmp__}} does have a test. 

The buildbot needs to know to apply 8456-lazy-import-cython-rebase-4.6.patch and 8456-lazy-import-enhancements-4.6.patch



---

archive/issue_comments_068926.json:
```json
{
    "body": "<a id='comment:20'>Comment 20:</a>\nI may be misunderstanding, since ther is also a `__richcmp__` method and `__cmp__` method is not used, but I see the following code:\n\n```\n    def __cmp__(left, right):\n        \"\"\"\n        TESTS::\n        \n            sage: lazy_import('sage.all', 'ZZ'); lazy_ZZ = ZZ\n        \"\"\"\n        return binop(cmp, left, right)\n```",
    "created_at": "2010-12-05T12:35:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68926",
    "user": "https://github.com/lftabera"
}
```

<a id='comment:20'>Comment 20:</a>
I may be misunderstanding, since ther is also a `__richcmp__` method and `__cmp__` method is not used, but I see the following code:

```
    def __cmp__(left, right):
        """
        TESTS::
        
            sage: lazy_import('sage.all', 'ZZ'); lazy_ZZ = ZZ
        """
        return binop(cmp, left, right)
```



---

archive/issue_comments_068927.json:
```json
{
    "body": "<a id='comment:21'>Comment 21:</a>\nNo, you're absolutely right, that (half) doctest is an oversight on my part.",
    "created_at": "2010-12-05T12:44:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68927",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:21'>Comment 21:</a>
No, you're absolutely right, that (half) doctest is an oversight on my part.



---

archive/issue_comments_068928.json:
```json
{
    "body": "Attachment: **[8456-lazy-import-enhancements-4.6.patch.gz](https://github.com/sagemath/sage/files/ticket8456/8456-lazy-import-enhancements-4.6.patch.gz)**",
    "created_at": "2010-12-07T10:21:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68928",
    "user": "https://github.com/robertwb"
}
```

Attachment: **[8456-lazy-import-enhancements-4.6.patch.gz](https://github.com/sagemath/sage/files/ticket8456/8456-lazy-import-enhancements-4.6.patch.gz)**



---

archive/issue_comments_068929.json:
```json
{
    "body": "<a id='comment:22'>Comment 22:</a>\nI've updated the patch.",
    "created_at": "2010-12-07T10:22:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68929",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:22'>Comment 22:</a>
I've updated the patch.



---

archive/issue_events_093611.json:
```json
{
    "actor": "https://github.com/lftabera",
    "created_at": "2010-12-16T20:53:33Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "milestone_number": null,
    "milestone_title": "sage-4.6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8456#event-93611"
}
```



---

archive/issue_events_093612.json:
```json
{
    "actor": "https://github.com/lftabera",
    "created_at": "2010-12-16T20:53:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "milestone_number": null,
    "milestone_title": "sage-4.6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8456#event-93612"
}
```



---

archive/issue_comments_068930.json:
```json
{
    "body": "Reviewer: **Luis Felipe Tabera**",
    "created_at": "2010-12-16T20:53:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68930",
    "user": "https://github.com/lftabera"
}
```

Reviewer: **Luis Felipe Tabera**



---

archive/issue_comments_068931.json:
```json
{
    "body": "<a id='comment:23'>Comment 23:</a>\nThe code looks right so I give it a positive review.\n\nHowever, I upload a patch to add some minor documentation explaining the parameters of lazy_import. \nRobert, could you please review and check that I did not make a mistake?",
    "created_at": "2010-12-16T20:53:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68931",
    "user": "https://github.com/lftabera"
}
```

<a id='comment:23'>Comment 23:</a>
The code looks right so I give it a positive review.

However, I upload a patch to add some minor documentation explaining the parameters of lazy_import. 
Robert, could you please review and check that I did not make a mistake?



---

archive/issue_comments_068932.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -7,3 +7,4 @@\n Apply only\n 1. [attachment: 8456-lazy-import-cython-rebase-4.6.patch.gz](https://github.com/sagemath/sage/files/ticket8456/8456-lazy-import-cython-rebase-4.6.patch.gz)\n 2. [attachment: 8456-lazy-import-enhancements-4.6.patch.gz](https://github.com/sagemath/sage/files/ticket8456/8456-lazy-import-enhancements-4.6.patch.gz)\n+3. [attachment: 8456-lazy-import-documentation.patch.gz](https://github.com/sagemath/sage/files/ticket8456/8456-lazy-import-documentation.patch.gz)\n``````\n",
    "created_at": "2010-12-16T20:55:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68932",
    "user": "https://github.com/lftabera"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -7,3 +7,4 @@
 Apply only
 1. [attachment: 8456-lazy-import-cython-rebase-4.6.patch.gz](https://github.com/sagemath/sage/files/ticket8456/8456-lazy-import-cython-rebase-4.6.patch.gz)
 2. [attachment: 8456-lazy-import-enhancements-4.6.patch.gz](https://github.com/sagemath/sage/files/ticket8456/8456-lazy-import-enhancements-4.6.patch.gz)
+3. [attachment: 8456-lazy-import-documentation.patch.gz](https://github.com/sagemath/sage/files/ticket8456/8456-lazy-import-documentation.patch.gz)
``````




---

archive/issue_comments_068933.json:
```json
{
    "body": "<a id='comment:25'>Comment 25:</a>\nYour documentation is correct, thanks.",
    "created_at": "2010-12-17T22:39:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68933",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:25'>Comment 25:</a>
Your documentation is correct, thanks.



---

archive/issue_events_093613.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-12-17T22:39:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8456#event-93613"
}
```



---

archive/issue_events_093614.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-12-17T22:39:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8456#event-93614"
}
```



---

archive/issue_comments_068934.json:
```json
{
    "body": "Changed reviewer from **Luis Felipe Tabera** to **Luis Felipe Tabera Alonso**",
    "created_at": "2011-01-12T10:43:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68934",
    "user": "https://github.com/lftabera"
}
```

Changed reviewer from **Luis Felipe Tabera** to **Luis Felipe Tabera Alonso**



---

archive/issue_comments_068935.json:
```json
{
    "body": "<a id='comment:27'>Comment 27:</a>\nPlease check Sphinx syntax\n\n```\ndocstring of sage.misc.lazy_import.get_star_imports:1: (WARNING/2) Inline emphasis start-string without end-string.\n```",
    "created_at": "2011-01-24T16:45:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68935",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:27'>Comment 27:</a>
Please check Sphinx syntax

```
docstring of sage.misc.lazy_import.get_star_imports:1: (WARNING/2) Inline emphasis start-string without end-string.
```



---

archive/issue_events_093615.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-24T16:45:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8456#event-93615"
}
```



---

archive/issue_events_093616.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-24T16:45:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8456#event-93616"
}
```



---

archive/issue_events_093617.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2011-01-25T07:00:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8456#event-93617"
}
```



---

archive/issue_events_093618.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2011-01-25T07:00:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8456#event-93618"
}
```



---

archive/issue_comments_068936.json:
```json
{
    "body": "<a id='comment:28'>Comment 28:</a>\nAttachment: **[8456-lazy-import-documentation.patch.gz](https://github.com/sagemath/sage/files/ticket8456/8456-lazy-import-documentation.patch.gz)**\n\nI've refreshed the documentation patch to escape this asterisk.",
    "created_at": "2011-01-25T07:00:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68936",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:28'>Comment 28:</a>
Attachment: **[8456-lazy-import-documentation.patch.gz](https://github.com/sagemath/sage/files/ticket8456/8456-lazy-import-documentation.patch.gz)**

I've refreshed the documentation patch to escape this asterisk.



---

archive/issue_events_093619.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2011-01-25T18:24:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8456#event-93619"
}
```



---

archive/issue_events_093620.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2011-01-25T18:24:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8456#event-93620"
}
```



---

archive/issue_comments_068937.json:
```json
{
    "body": "<a id='comment:29'>Comment 29:</a>\nI'm re-instating the positive review, as adding the asterisk escape was a superficial change.",
    "created_at": "2011-01-25T18:24:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68937",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:29'>Comment 29:</a>
I'm re-instating the positive review, as adding the asterisk escape was a superficial change.



---

archive/issue_comments_068938.json:
```json
{
    "body": "Merged: **sage-4.6.2.alpha3**",
    "created_at": "2011-01-26T22:26:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8456#issuecomment-68938",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.6.2.alpha3**



---

archive/issue_events_093621.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-26T22:26:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8456#event-93621"
}
```



---

archive/issue_events_093622.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-26T22:26:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8456",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8456#event-93622"
}
```
