...
```json
{
    "body": "<a id='comment:1'></a>I split it up into two patches for easier refereeing. The first simply moves lazy_import.py to lazy_import.pyx.",
    "created_at": "2010-03-06T09:39:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8456",
    "type": "issue_comment",
...
<a id='comment:1'></a>I split it up into two patches for easier refereeing. The first simply moves lazy_import.py to lazy_import.pyx.
...
<a id='comment:2'></a>I started with importing the first patch only. I tried to lazy-import ZZ, but it did not properly work:
...
```json
{
    "body": "<a id='comment:4'></a>Hmm... is this on top of an alpha? Worked fine for me. \n\nAs for backing out, you can do \"sage -hg rollback\"",
...
<a id='comment:4'></a>Hmm... is this on top of an alpha? Worked fine for me. 
...
```json
{
    "body": "<a id='comment:5'></a>OK, the problem was I was using os.rename rather than, say, shutil.move, which caused errors when the DOT_SAGE file was not on the same filesystem as temp files. Glad you caught that, ready for another review.",
    "created_at": "2010-06-22T20:46:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8456",
    "type": "issue_comment",
...
<a id='comment:5'></a>OK, the problem was I was using os.rename rather than, say, shutil.move, which caused errors when the DOT_SAGE file was not on the same filesystem as temp files. Glad you caught that, ready for another review.
...
```json
{
    "body": "<a id='comment:6'></a>Simon, could you look at this again, when it's convenient?  I can try to review this, but I'm not sure that I'm sufficiently familiar with Cython.\n\nFor me, all three patches apply cleanly to 4.5.3.alpha1 (using a Mercurial queue).",
...
<a id='comment:6'></a>Simon, could you look at this again, when it's convenient?  I can try to review this, but I'm not sure that I'm sufficiently familiar with Cython.
...
```json
{
    "body": "<a id='comment:10'></a>Lots of people want to see a faster Sage startup; is anyone willing to review this?",
...
<a id='comment:10'></a>Lots of people want to see a faster Sage startup; is anyone willing to review this?
...
<a id='comment:11'></a>rebase 8456-lazy-import-cython.patch to sage 4.6
...
<a id='comment:12'></a>As a user, I would be puzzled if I found at the begining of the session:
...
```json
{
    "body": "<a id='comment:13'></a>Hmm... it looks like I need to manually add the typeslot methods, as they don't get forwarded with getattr. Thanks for catching this, I'll post another patch.",
    "created_at": "2010-11-04T17:57:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8456",
    "type": "issue_comment",
...
<a id='comment:13'></a>Hmm... it looks like I need to manually add the typeslot methods, as they don't get forwarded with getattr. Thanks for catching this, I'll post another patch.
...
```json
{
    "body": "<a id='comment:14'></a>Apply only 8456-lazy-import-cython-rebase-4.6.patch and 8456-lazy-import-enhancements-4.6.patch. This fixes the issue with `__repr__` and all the other special methods.",
...
<a id='comment:14'></a>Apply only 8456-lazy-import-cython-rebase-4.6.patch and 8456-lazy-import-enhancements-4.6.patch. This fixes the issue with `__repr__` and all the other special methods.
...
<a id='comment:16'></a>Replying to [robertwb](#comment%3A14):
> Apply only 8456-lazy-import-cython-rebase-4.6.patch and 8456-lazy-import-enhancements-4.6.patch. This fixes the issue with `__repr__` and all the other special methods. 
...
```json
{
    "body": "<a id='comment:17'></a>Sorry, I missed the dependency... :/",
    "created_at": "2010-11-26T16:45:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8456",
    "type": "issue_comment",
...
<a id='comment:17'></a>Sorry, I missed the dependency... :/
...
```json
{
    "body": "<a id='comment:18'></a>The code looks good. Is there a reason why _cmp_ does not have a test?",
...
<a id='comment:18'></a>The code looks good. Is there a reason why _cmp_ does not have a test?
...
<a id='comment:19'></a>{{__cmp__}} does have a test. 
...
```json
{
    "body": "<a id='comment:20'></a>I may be misunderstanding, since ther is also a __richcmp__ method and __cmp__ method is not used, but I see the following code:\n\n```\n    def __cmp__(left, right):\n        \"\"\"\n        TESTS::\n        \n            sage: lazy_import('sage.all', 'ZZ'); lazy_ZZ = ZZ\n        \"\"\"\n        return binop(cmp, left, right)\n```",
...
<a id='comment:20'></a>I may be misunderstanding, since ther is also a __richcmp__ method and __cmp__ method is not used, but I see the following code:
...
```json
{
    "body": "<a id='comment:21'></a>No, you're absolutely right, that (half) doctest is an oversight on my part.",
...
<a id='comment:21'></a>No, you're absolutely right, that (half) doctest is an oversight on my part.
...
```json
{
    "body": "<a id='comment:22'></a>I've updated the patch.",
...
<a id='comment:22'></a>I've updated the patch.
...
<a id='comment:23'></a>The code looks right so I give it a positive review.
...
```json
{
    "body": "<a id='comment:25'></a>Your documentation is correct, thanks.",
...
<a id='comment:25'></a>Your documentation is correct, thanks.
...
<a id='comment:27'></a>Please check Sphinx syntax
...
```json
{
    "body": "<a id='comment:28'></a>Attachment [8456-lazy-import-documentation.patch](tarball://root/attachments/some-uuid/ticket8456/8456-lazy-import-documentation.patch) by @robertwb created at 2011-01-25 07:00:59\n\nI've refreshed the documentation patch to escape this asterisk.",
    "created_at": "2011-01-25T07:00:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8456",
    "type": "issue_comment",
...
<a id='comment:28'></a>Attachment [8456-lazy-import-documentation.patch](tarball://root/attachments/some-uuid/ticket8456/8456-lazy-import-documentation.patch) by @robertwb created at 2011-01-25 07:00:59
...
```json
{
    "body": "<a id='comment:29'></a>I'm re-instating the positive review, as adding the asterisk escape was a superficial change.",
    "created_at": "2011-01-25T18:24:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8456",
    "type": "issue_comment",
...
<a id='comment:29'></a>I'm re-instating the positive review, as adding the asterisk escape was a superficial change.
