# Issue 8662: Iterated Coleman integration

archive/issues_008662.json:
```json
{
    "assignees": [
        "https://github.com/williamstein"
    ],
    "body": "This extends current functionality for Coleman integrals on hyperelliptic curves; in particular, this allows one to compute iterated integrals between non-Weierstrass points on hyperelliptic curves and iterated integrals with one Weierstrass endpoint on elliptic curves.\n\nCC:  @kedlaya\n\nAuthor: **jen**\n\nComponent: **number theory**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8662_\n\n",
    "created_at": "2010-04-08T22:53:46Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20number%20theory",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Iterated Coleman integration",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/8662",
    "user": "https://github.com/jbalakrishnan"
}
```
This extends current functionality for Coleman integrals on hyperelliptic curves; in particular, this allows one to compute iterated integrals between non-Weierstrass points on hyperelliptic curves and iterated integrals with one Weierstrass endpoint on elliptic curves.

CC:  @kedlaya

Author: **jen**

Component: **number theory**

_Issue created by migration from https://trac.sagemath.org/ticket/8662_





---

archive/issue_events_109634.json:
```json
{
    "actor": "https://github.com/jbalakrishnan",
    "created_at": "2010-04-08T22:53:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8662",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8662#event-109634"
}
```



---

archive/issue_events_109635.json:
```json
{
    "actor": "https://github.com/jbalakrishnan",
    "created_at": "2010-04-08T22:53:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8662",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20number%20theory",
    "label_color": "0000ff",
    "label_name": "c: number theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8662#event-109635"
}
```



---

archive/issue_events_109636.json:
```json
{
    "actor": "https://github.com/jbalakrishnan",
    "created_at": "2010-04-08T22:53:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8662",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8662#event-109636"
}
```



---

archive/issue_events_109637.json:
```json
{
    "actor": "https://github.com/jbalakrishnan",
    "created_at": "2010-04-08T22:53:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8662",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8662#event-109637"
}
```



---

archive/issue_events_109638.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-04-08T22:53:46Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/8662",
    "subject": "https://github.com/jbalakrishnan",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8662#event-109638"
}
```



---

archive/issue_comments_069562.json:
```json
{
    "body": "Attachment: **[13544.2.patch.gz](https://github.com/sagemath/sage/files/ticket8662/13544.2.patch.gz)**\n\nAttachment: **[13544.patch.gz](https://github.com/sagemath/sage/files/ticket8662/13544.patch.gz)**",
    "created_at": "2010-04-09T00:55:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8662#issuecomment-69562",
    "user": "https://github.com/jbalakrishnan"
}
```

Attachment: **[13544.2.patch.gz](https://github.com/sagemath/sage/files/ticket8662/13544.2.patch.gz)**

Attachment: **[13544.patch.gz](https://github.com/sagemath/sage/files/ticket8662/13544.patch.gz)**



---

archive/issue_comments_069563.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nHi Jen,\nAre you still trying to get this into Sage?  It's unclear what the status of this ticket is....",
    "created_at": "2012-09-17T05:38:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8662#issuecomment-69563",
    "user": "https://github.com/roed314"
}
```

<div id="comment:2" align="right">comment:2</div>

Hi Jen,
Are you still trying to get this into Sage?  It's unclear what the status of this ticket is....



---

archive/issue_comments_069564.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nHi David, Perhaps we can close it for now?",
    "created_at": "2012-09-17T15:19:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8662#issuecomment-69564",
    "user": "https://github.com/jbalakrishnan"
}
```

<div id="comment:3" align="right">comment:3</div>

Hi David, Perhaps we can close it for now?



---

archive/issue_events_109639.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8662",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8662#event-109639"
}
```



---

archive/issue_events_109640.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8662",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8662#event-109640"
}
```



---

archive/issue_events_109641.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8662",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8662#event-109641"
}
```



---

archive/issue_events_109642.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8662",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8662#event-109642"
}
```



---

archive/issue_events_109643.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8662",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8662#event-109643"
}
```



---

archive/issue_events_109644.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8662",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8662#event-109644"
}
```



---

archive/issue_events_109645.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8662",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8662#event-109645"
}
```



---

archive/issue_events_109646.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8662",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8662#event-109646"
}
```



---

archive/issue_comments_069565.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nPing. Status of this?",
    "created_at": "2016-08-19T22:30:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8662#issuecomment-69565",
    "user": "https://github.com/kedlaya"
}
```

<div id="comment:8" align="right">comment:8</div>

Ping. Status of this?



---

archive/issue_events_109647.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8662",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8662#event-109647"
}
```
