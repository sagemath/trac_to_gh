# Issue 8672: SCIP support

archive/issues_008672.json:
```json
{
    "assignees": [],
    "body": "Based upon Harald Schilly's SPKG for SCIP, here is a patch to enable the use of this solver through the usual interface for LP.\n\nRequires #8172 (though none of GLPK, CBC or CPLEX needs to be installed)\n\nNathann\n\nAssignee: **@jasongrout, @sagetrac-jkantor**\n\nCC:  @malb @haraldschilly @sagetrac-r-gaia-cs\n\nReviewer: **Vincent Delecroix**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8672_\n\n",
    "closed_at": "2016-06-12T12:02:30Z",
    "created_at": "2010-04-11T12:32:06Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20programming",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "SCIP support",
    "type": "issue",
    "updated_at": "2016-06-12T12:02:30Z",
    "url": "https://github.com/sagemath/sage/issues/8672",
    "user": "https://github.com/nathanncohen"
}
```
Based upon Harald Schilly's SPKG for SCIP, here is a patch to enable the use of this solver through the usual interface for LP.

Requires #8172 (though none of GLPK, CBC or CPLEX needs to be installed)

Nathann

Assignee: **@jasongrout, @sagetrac-jkantor**

CC:  @malb @haraldschilly @sagetrac-r-gaia-cs

Reviewer: **Vincent Delecroix**

_Issue created by migration from https://trac.sagemath.org/ticket/8672_





---

archive/issue_events_099663.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-04-11T12:32:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8672#event-99663"
}
```



---

archive/issue_events_099664.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-04-11T12:32:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20numerical",
    "label_color": "0000ff",
    "label_name": "component: numerical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8672#event-99664"
}
```



---

archive/issue_events_099665.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-04-11T12:32:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8672#event-99665"
}
```



---

archive/issue_events_099666.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-04-11T12:32:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8672#event-99666"
}
```



---

archive/issue_events_099667.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-04-11T12:33:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8672#event-99667"
}
```



---

archive/issue_comments_071614.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,5 @@\n Based upon Harald Schilly's SPKG for SCIP, here is a patch to enable the use of this solver through the usual interface for LP.\n \n+Requires #8172 (though none of GLPK, CBC or CPLEX needs to be installed)\n+\n Nathann\n``````\n",
    "created_at": "2010-04-11T12:33:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8672#issuecomment-71614",
    "user": "https://github.com/nathanncohen"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,5 @@
 Based upon Harald Schilly's SPKG for SCIP, here is a patch to enable the use of this solver through the usual interface for LP.
 
+Requires #8172 (though none of GLPK, CBC or CPLEX needs to be installed)
+
 Nathann
``````




---

archive/issue_comments_071615.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nwow, thank you!! just one thing, is it possible to enable verbose output if enabled? i don't know which parameter you have to set in Sage's MIP interface, but it is  scip.solver(quiet=False) in scip.",
    "created_at": "2010-04-11T12:44:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8672#issuecomment-71615",
    "user": "https://github.com/haraldschilly"
}
```

<div id="comment:2" align="right">Comment 2</div>

wow, thank you!! just one thing, is it possible to enable verbose output if enabled? i don't know which parameter you have to set in Sage's MIP interface, but it is  scip.solver(quiet=False) in scip.



---

archive/issue_comments_071616.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nFixed ! The corresponding argument in sage is log=# in the solve method. 0 for no output, and integers to have progressdively more (only GLPk has different levels of output though, the others only understand 0/1 or none at all )\n\nNathann",
    "created_at": "2010-04-11T12:54:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8672#issuecomment-71616",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:3" align="right">Comment 3</div>

Fixed ! The corresponding argument in sage is log=# in the solve method. 0 for no output, and integers to have progressdively more (only GLPk has different levels of output though, the others only understand 0/1 or none at all )

Nathann



---

archive/issue_comments_071617.json:
```json
{
    "body": "Attachment: **[trac_8672.patch.gz](https://github.com/sagemath/sage/files/ticket8672/trac_8672.patch.gz)**",
    "created_at": "2010-04-11T12:55:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8672#issuecomment-71617",
    "user": "https://github.com/nathanncohen"
}
```

Attachment: **[trac_8672.patch.gz](https://github.com/sagemath/sage/files/ticket8672/trac_8672.patch.gz)**



---

archive/issue_events_099668.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2010-06-02T21:30:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8672#event-99668"
}
```



---

archive/issue_events_099669.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2010-06-02T21:30:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8672#event-99669"
}
```



---

archive/issue_comments_071618.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nHi Nathann,\n\ntwo small things:\n\n* `solve_scip` has no documentation/doctests \n* \"the default solver is used (COIN if available, GLPK otherwise)\" seems to be inaccurate since you also try SCIP whether it is available",
    "created_at": "2010-06-02T21:30:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8672#issuecomment-71618",
    "user": "https://github.com/malb"
}
```

<div id="comment:4" align="right">Comment 4</div>

Hi Nathann,

two small things:

* `solve_scip` has no documentation/doctests 
* "the default solver is used (COIN if available, GLPK otherwise)" seems to be inaccurate since you also try SCIP whether it is available



---

archive/issue_events_099670.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-09-06T11:13:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20numerical",
    "label_color": "0000ff",
    "label_name": "component: numerical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8672#event-99670"
}
```



---

archive/issue_events_099671.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-09-06T11:13:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20programming",
    "label_color": "0000ff",
    "label_name": "component: linear programming",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8672#event-99671"
}
```



---

archive/issue_events_099672.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8672#event-99672"
}
```



---

archive/issue_events_099673.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8672#event-99673"
}
```



---

archive/issue_events_099674.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8672#event-99674"
}
```



---

archive/issue_events_099675.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8672#event-99675"
}
```



---

archive/issue_events_099676.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8672#event-99676"
}
```



---

archive/issue_events_099677.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8672#event-99677"
}
```



---

archive/issue_events_099678.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8672#event-99678"
}
```



---

archive/issue_events_099679.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8672#event-99679"
}
```



---

archive/issue_events_099680.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2016-03-30T20:43:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8672#event-99680"
}
```



---

archive/issue_events_099681.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2016-03-30T20:43:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8672#event-99681"
}
```



---

archive/issue_events_099682.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2016-03-30T20:43:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8672#event-99682"
}
```



---

archive/issue_comments_071619.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nDuplicate of #10879, so I'm marking this one as \"needs_review\" so it can be closed -- hoping this is the correct procedure.",
    "created_at": "2016-03-30T20:43:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8672#issuecomment-71619",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:11" align="right">Comment 11</div>

Duplicate of #10879, so I'm marking this one as "needs_review" so it can be closed -- hoping this is the correct procedure.



---

archive/issue_events_099683.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2016-03-30T20:43:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8672#event-99683"
}
```



---

archive/issue_events_099684.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2016-03-30T20:43:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8672#event-99684"
}
```



---

archive/issue_comments_071620.json:
```json
{
    "body": "Reviewer: **Vincent Delecroix**",
    "created_at": "2016-04-07T20:13:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8672#issuecomment-71620",
    "user": "https://github.com/videlec"
}
```

Reviewer: **Vincent Delecroix**



---

archive/issue_events_099685.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2016-04-07T20:13:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8672#event-99685"
}
```



---

archive/issue_events_099686.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2016-04-07T20:13:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8672#event-99686"
}
```



---

archive/issue_events_099687.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-06-12T12:02:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8672#event-99687"
}
```



---

archive/issue_events_099688.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-06-12T12:02:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8672",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8672#event-99688"
}
```
