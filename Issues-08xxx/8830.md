# Issue 8830: polybori-0.6.4.spkg causes crashes on exit

archive/issues_008830.json:
```json
{
    "assignees": [],
    "body": "Per the discussion on #8192, the polybori spkg there caused crashes and valgrind errors on exit.\n\nThe new spkg to fix this created by Alexander Dreyer and Minh Van Nguyen is at:\n\nhttp://sage.math.washington.edu/home/mvngu/spkg/standard/polybori/polybori-0.6.4.p0.spkg\n\nI tested it, and it gets rid of the valgrind errors and crashes.\n\nAuthor: **Alexander Dreyer, Minh Van Nguyen**\n\nReviewer: **Willem Jan Palenstijn**\n\nMerged: **sage-4.4.1.alpha3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8830_\n\n",
    "closed_at": "2010-05-01T05:02:58Z",
    "created_at": "2010-04-30T12:53:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.4.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "polybori-0.6.4.spkg causes crashes on exit",
    "type": "issue",
    "updated_at": "2010-08-19T20:34:19Z",
    "url": "https://github.com/sagemath/sage/issues/8830",
    "user": "https://github.com/wjp"
}
```
Per the discussion on #8192, the polybori spkg there caused crashes and valgrind errors on exit.

The new spkg to fix this created by Alexander Dreyer and Minh Van Nguyen is at:

http://sage.math.washington.edu/home/mvngu/spkg/standard/polybori/polybori-0.6.4.p0.spkg

I tested it, and it gets rid of the valgrind errors and crashes.

Author: **Alexander Dreyer, Minh Van Nguyen**

Reviewer: **Willem Jan Palenstijn**

Merged: **sage-4.4.1.alpha3**

_Issue created by migration from https://trac.sagemath.org/ticket/8830_





---

archive/issue_events_101948.json:
```json
{
    "actor": "https://github.com/wjp",
    "created_at": "2010-04-30T12:53:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8830",
    "milestone_number": null,
    "milestone_title": "sage-4.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8830#event-101948"
}
```



---

archive/issue_events_101949.json:
```json
{
    "actor": "https://github.com/wjp",
    "created_at": "2010-04-30T12:53:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8830",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8830#event-101949"
}
```



---

archive/issue_events_101950.json:
```json
{
    "actor": "https://github.com/wjp",
    "created_at": "2010-04-30T12:53:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8830",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8830#event-101950"
}
```



---

archive/issue_events_101951.json:
```json
{
    "actor": "https://github.com/wjp",
    "created_at": "2010-04-30T12:53:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8830",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8830#event-101951"
}
```



---

archive/issue_events_101952.json:
```json
{
    "actor": "https://github.com/wjp",
    "created_at": "2010-04-30T12:53:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8830",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8830#event-101952"
}
```



---

archive/issue_events_101953.json:
```json
{
    "actor": "https://github.com/wjp",
    "created_at": "2010-04-30T12:53:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8830",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8830#event-101953"
}
```



---

archive/issue_events_101954.json:
```json
{
    "actor": "https://github.com/wjp",
    "created_at": "2010-04-30T12:53:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8830",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8830#event-101954"
}
```



---

archive/issue_comments_074039.json:
```json
{
    "body": "Changed author from **Alexander Dreyer, Minh Nguyen** to **Alexander Dreyer, Minh Van Nguyen**",
    "created_at": "2010-04-30T19:58:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8830",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8830#issuecomment-74039",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Changed author from **Alexander Dreyer, Minh Nguyen** to **Alexander Dreyer, Minh Van Nguyen**



---

archive/issue_comments_074040.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,6 +1,6 @@\n Per the discussion on #8192, the polybori spkg there caused crashes and valgrind errors on exit.\n \n-The new spkg to fix this created by Alexander Dreyer and Minh Nguyen is at:\n+The new spkg to fix this created by Alexander Dreyer and Minh Van Nguyen is at:\n \n http://sage.math.washington.edu/home/mvngu/spkg/standard/polybori/polybori-0.6.4.p0.spkg\n \n``````\n",
    "created_at": "2010-04-30T20:22:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8830",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8830#issuecomment-74040",
    "user": "https://github.com/wjp"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,6 +1,6 @@
 Per the discussion on #8192, the polybori spkg there caused crashes and valgrind errors on exit.
 
-The new spkg to fix this created by Alexander Dreyer and Minh Nguyen is at:
+The new spkg to fix this created by Alexander Dreyer and Minh Van Nguyen is at:
 
 http://sage.math.washington.edu/home/mvngu/spkg/standard/polybori/polybori-0.6.4.p0.spkg
 
``````




---

archive/issue_comments_074041.json:
```json
{
    "body": "Merged: **4.4.1.alpha3**",
    "created_at": "2010-05-01T05:02:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8830",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8830#issuecomment-74041",
    "user": "https://github.com/williamstein"
}
```

Merged: **4.4.1.alpha3**



---

archive/issue_events_101955.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-05-01T05:02:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8830",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8830#event-101955"
}
```



---

archive/issue_events_101956.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-05-01T05:02:58Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8830",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8830#event-101956"
}
```



---

archive/issue_comments_074042.json:
```json
{
    "body": "Changed merged from **4.4.1.alpha3** to **sage-4.4.1.alpha3**",
    "created_at": "2010-05-03T01:12:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8830",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8830#issuecomment-74042",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Changed merged from **4.4.1.alpha3** to **sage-4.4.1.alpha3**



---

archive/issue_comments_074043.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\n#9768 should fix the dynamic libraries finally.",
    "created_at": "2010-08-19T20:34:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8830",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8830#issuecomment-74043",
    "user": "https://github.com/alexanderdreyer"
}
```

<a id='comment:7'>Comment 7:</a>
#9768 should fix the dynamic libraries finally.
