# Issue 8358: improved documentation of number_field.py

archive/issues_008358.json:
```json
{
    "assignees": [
        "https://github.com/loefflerd"
    ],
    "body": "There is an example in number_field.py which says it factors Gaussian integers however units are not considered, which might confuse the\nuser. The attached patch documents that feature. See also #7545.\n\nCC:  @williamstein @categorie\n\nKeywords: **Gaussian integers**\n\nComponent: **number fields**\n\nAuthor: **Paul Zimmermann**\n\nReviewer: **Christian Wulthrich**\n\nMerged: **sage-4.3.4.alpha1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8358_\n\n",
    "closed_at": "2010-03-06T09:18:59Z",
    "created_at": "2010-02-25T08:22:30Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20number%20fields",
        "https://github.com/sagemath/sage/labels/p%3A%205%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "improved documentation of number_field.py",
    "type": "issue",
    "updated_at": "2010-03-06T09:18:59Z",
    "url": "https://github.com/sagemath/sage/issues/8358",
    "user": "https://github.com/zimmermann6"
}
```
There is an example in number_field.py which says it factors Gaussian integers however units are not considered, which might confuse the
user. The attached patch documents that feature. See also #7545.

CC:  @williamstein @categorie

Keywords: **Gaussian integers**

Component: **number fields**

Author: **Paul Zimmermann**

Reviewer: **Christian Wulthrich**

Merged: **sage-4.3.4.alpha1**

_Issue created by migration from https://trac.sagemath.org/ticket/8358_





---

archive/issue_events_104555.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2010-02-25T08:22:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8358",
    "milestone_number": null,
    "milestone_title": "sage-4.3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8358#event-104555"
}
```



---

archive/issue_events_104556.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2010-02-25T08:22:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8358",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20number%20fields",
    "label_color": "0000ff",
    "label_name": "c: number fields",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8358#event-104556"
}
```



---

archive/issue_events_104557.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2010-02-25T08:22:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8358",
    "label": "https://github.com/sagemath/sage/labels/p%3A%205%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p: 5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8358#event-104557"
}
```



---

archive/issue_events_104558.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2010-02-25T08:22:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8358",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8358#event-104558"
}
```



---

archive/issue_events_104559.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2010-02-25T08:22:30Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/8358",
    "subject": "https://github.com/zimmermann6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8358#event-104559"
}
```



---

archive/issue_comments_065777.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,2 +1,2 @@\n There is an example in number_field.py which says it factors Gaussian integers however units are not considered, which might confuse the\n-user.\n+user. The attached patch documents that feature. See also #7545.\n``````\n",
    "created_at": "2010-02-25T08:25:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8358",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8358#issuecomment-65777",
    "user": "https://github.com/zimmermann6"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,2 +1,2 @@
 There is an example in number_field.py which says it factors Gaussian integers however units are not considered, which might confuse the
-user.
+user. The attached patch documents that feature. See also #7545.
``````




---

archive/issue_comments_065778.json:
```json
{
    "body": "Changed keywords from none to **Gaussian integers**",
    "created_at": "2010-02-25T08:25:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8358",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8358#issuecomment-65778",
    "user": "https://github.com/zimmermann6"
}
```

Changed keywords from none to **Gaussian integers**



---

archive/issue_comments_065779.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_8358.patch.gz](https://github.com/sagemath/sage/files/ticket8358/trac_8358.patch.gz)**",
    "created_at": "2010-02-25T08:25:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8358",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8358#issuecomment-65779",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_8358.patch.gz](https://github.com/sagemath/sage/files/ticket8358/trac_8358.patch.gz)**



---

archive/issue_events_104560.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2010-02-25T08:25:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8358",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8358#event-104560"
}
```



---

archive/issue_events_104561.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2010-02-25T08:25:49Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/8358",
    "subject": "https://github.com/zimmermann6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8358#event-104561"
}
```



---

archive/issue_events_104562.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2010-02-25T08:25:49Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/8358",
    "subject": "https://github.com/zimmermann6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8358#event-104562"
}
```



---

archive/issue_comments_065780.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nIf anybody wants to work on it, look at the discussion on sage-nt:\nhttp://groups.google.com/group/sage-nt/browse_thread/thread/4bf6d83aece75c80\nand also #7545.\n\nPaul",
    "created_at": "2010-02-26T09:40:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8358",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8358#issuecomment-65780",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:3" align="right">comment:3</div>

If anybody wants to work on it, look at the discussion on sage-nt:
http://groups.google.com/group/sage-nt/browse_thread/thread/4bf6d83aece75c80
and also #7545.

Paul



---

archive/issue_events_104563.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2010-02-26T09:40:58Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/8358",
    "subject": "https://github.com/zimmermann6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8358#event-104563"
}
```



---

archive/issue_events_104564.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2010-02-26T09:40:58Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/8358",
    "subject": "https://github.com/zimmermann6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8358#event-104564"
}
```



---

archive/issue_events_104565.json:
```json
{
    "actor": "https://github.com/categorie",
    "created_at": "2010-03-04T17:30:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8358",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8358#event-104565"
}
```



---

archive/issue_events_104566.json:
```json
{
    "actor": "https://github.com/categorie",
    "created_at": "2010-03-04T17:30:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8358",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8358#event-104566"
}
```



---

archive/issue_comments_065781.json:
```json
{
    "body": "Reviewer: **Christian Wulthrich**",
    "created_at": "2010-03-06T09:18:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8358",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8358#issuecomment-65781",
    "user": "https://github.com/mwhansen"
}
```

Reviewer: **Christian Wulthrich**



---

archive/issue_events_104567.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-03-06T09:18:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8358",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8358#event-104567"
}
```



---

archive/issue_events_104568.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-03-06T09:18:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8358",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8358#event-104568"
}
```



---

archive/issue_comments_065782.json:
```json
{
    "body": "Merged: **sage-4.3.4.alpha1**",
    "created_at": "2010-03-06T09:18:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8358",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8358#issuecomment-65782",
    "user": "https://github.com/mwhansen"
}
```

Merged: **sage-4.3.4.alpha1**
