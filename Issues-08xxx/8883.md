# Issue 8883: Implement morphism.register_as_conversion()

archive/issues_008883.json:
```json
{
    "body": "\n\n**Assignee:** @robertwb\n\n**Reviewer:** Vincent Delecroix\n\nIssue created by migration from https://trac.sagemath.org/ticket/8883\n\n",
    "closed_at": "2015-04-26T01:46:21Z",
    "created_at": "2010-05-05T03:11:50Z",
    "labels": [
        "component: coercion",
        "enhancement",
        "invalid"
    ],
    "title": "Implement morphism.register_as_conversion()",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/8883",
    "user": "https://github.com/nthiery"
}
```


**Assignee:** @robertwb

**Reviewer:** Vincent Delecroix

Issue created by migration from https://trac.sagemath.org/ticket/8883





---

archive/issue_comments_074451.json:
```json
{
    "body": "<a id='comment:1'></a>\ndone in #6588",
    "created_at": "2015-04-14T11:25:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8883#issuecomment-74451",
    "user": "https://github.com/mezzarobba"
}
```

<a id='comment:1'></a>
done in #6588



---

archive/issue_events_063894.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2015-04-14T11:25:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/8883",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8883#event-63894"
}
```



---

archive/issue_comments_074452.json:
```json
{
    "body": "**Reviewer:** Vincent Delecroix",
    "created_at": "2015-04-24T21:37:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8883",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8883#issuecomment-74452",
    "user": "https://github.com/videlec"
}
```

**Reviewer:** Vincent Delecroix



---

archive/issue_events_063895.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-04-24T21:37:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/8883",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8883#event-63895"
}
```



---

archive/issue_events_063896.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-04-24T21:37:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/8883",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8883#event-63896"
}
```



---

archive/issue_events_063897.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-26T01:46:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/8883",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8883#event-63897"
}
```



---

archive/issue_events_063898.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-26T01:46:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/8883",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8883#event-63898"
}
```
