# Issue 8442: Lie Methods and Related Combinatorics (tutorial)

archive/issues_008442.json:
```json
{
    "assignees": [],
    "body": "This patch adds a tutorial about Lie Methods and Related Combinatorics to Sage.\n\nA copy of the html documentation built with dvipng may be found at:\n\nhttp://match.stanford.edu/bump/thematic_tutorials/lie.html\n\nor built with jsmath:\n\nhttp://match.stanford.edu/bump/thematic_tutorials-js/lie.html\n\nNote: only the second js version is up to date. I will update the other one later.\n\nApply:\n\n1. [attachment: trac_8442-rebased-4.6.1.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-rebased-4.6.1.patch.gz)\n2. [attachment: trac_8442-png-files.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-png-files.patch.gz)\n3. [attachment: trac_8442-ref-spacing.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-ref-spacing.patch.gz)\n4. [attachment: 8442_manifest.patch](https://github.com/sagemath/sage/files/ticket8442/8442_manifest.patch.gz)\n\nThen build the doc with\n\n```\nsage -docbuild thematic_tutorials html\n```\n\nadd the `-j` flag to use jsmath.\n\nSee also #8470.\n\nAssignee: **@dwbump**\n\nCC:  @sagetrac-sage-combinat @nilesjohnson\n\nAuthor: **Daniel Bump**\n\nReviewer: **Minh Van Nguyen, Mark Jordan, Dmitrii Pasechnik**\n\nMerged: **sage-4.6.2.alpha3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8442_\n\n",
    "closed_at": "2011-01-28T08:48:05Z",
    "created_at": "2010-03-05T03:08:27Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.6.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Lie Methods and Related Combinatorics (tutorial)",
    "type": "issue",
    "updated_at": "2011-01-28T08:48:05Z",
    "url": "https://github.com/sagemath/sage/issues/8442",
    "user": "https://github.com/dwbump"
}
```
This patch adds a tutorial about Lie Methods and Related Combinatorics to Sage.

A copy of the html documentation built with dvipng may be found at:

http://match.stanford.edu/bump/thematic_tutorials/lie.html

or built with jsmath:

http://match.stanford.edu/bump/thematic_tutorials-js/lie.html

Note: only the second js version is up to date. I will update the other one later.

Apply:

1. [attachment: trac_8442-rebased-4.6.1.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-rebased-4.6.1.patch.gz)
2. [attachment: trac_8442-png-files.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-png-files.patch.gz)
3. [attachment: trac_8442-ref-spacing.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-ref-spacing.patch.gz)
4. [attachment: 8442_manifest.patch](https://github.com/sagemath/sage/files/ticket8442/8442_manifest.patch.gz)

Then build the doc with

```
sage -docbuild thematic_tutorials html
```

add the `-j` flag to use jsmath.

See also #8470.

Assignee: **@dwbump**

CC:  @sagetrac-sage-combinat @nilesjohnson

Author: **Daniel Bump**

Reviewer: **Minh Van Nguyen, Mark Jordan, Dmitrii Pasechnik**

Merged: **sage-4.6.2.alpha3**

_Issue created by migration from https://trac.sagemath.org/ticket/8442_





---

archive/issue_events_096288.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-03-05T03:08:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "milestone_number": null,
    "milestone_title": "sage-4.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96288"
}
```



---

archive/issue_events_096289.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-03-05T03:08:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96289"
}
```



---

archive/issue_events_096290.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-03-05T03:08:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96290"
}
```



---

archive/issue_events_096291.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-03-05T03:08:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96291"
}
```



---

archive/issue_comments_068636.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nAttachment: **[wcf1.png](https://github.com/sagemath/sage/files/ticket8442/wcf1.png)**\n\nThis patch adds a tutorial about Lie Methods and Related Combinatorics to Sage.\n\nApply the patch and also copy the file wcf1.png into $SAGE_ROOT/devel/sage-queue/doc/en/lie/. Then you can build the documentation with the commands:\n\nsage -docbuild lie html\n\nsage -docbuild lie pdf\n\nThere is supposed to be a chapter on Crystals but that is not written yet.",
    "created_at": "2010-03-05T03:16:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68636",
    "user": "https://github.com/dwbump"
}
```

<div id="comment:1" align="right">Comment 1</div>

Attachment: **[wcf1.png](https://github.com/sagemath/sage/files/ticket8442/wcf1.png)**

This patch adds a tutorial about Lie Methods and Related Combinatorics to Sage.

Apply the patch and also copy the file wcf1.png into $SAGE_ROOT/devel/sage-queue/doc/en/lie/. Then you can build the documentation with the commands:

sage -docbuild lie html

sage -docbuild lie pdf

There is supposed to be a chapter on Crystals but that is not written yet.



---

archive/issue_comments_068637.json:
```json
{
    "body": "Work Issues: **Crystal chapter not written yet**",
    "created_at": "2010-03-05T03:16:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68637",
    "user": "https://github.com/dwbump"
}
```

Work Issues: **Crystal chapter not written yet**



---

archive/issue_comments_068638.json:
```json
{
    "body": "Changed assignee from **@sagetrac-mvngu** to **@dwbump**",
    "created_at": "2010-03-05T03:16:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68638",
    "user": "https://github.com/dwbump"
}
```

Changed assignee from **@sagetrac-mvngu** to **@dwbump**



---

archive/issue_comments_068639.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,12 @@\n \n+This patch adds a tutorial about Lie Methods and Related Combinatorics to Sage.\n+\n+Apply the patch and also copy the file wcf1.png into $SAGE_ROOT/devel/sage-queue/doc/en/lie/. Then you can build the documentation with the commands:\n+\n+sage -docbuild lie html\n+\n+sage -docbuild lie pdf\n+\n+There is supposed to be a chapter on Crystals but that is not written yet.\n+\n+Depends on #8411 and #8414 since it documents the behavior of Sage after these patches.\n``````\n",
    "created_at": "2010-03-05T10:41:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68639",
    "user": "https://github.com/dwbump"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,12 @@
 
+This patch adds a tutorial about Lie Methods and Related Combinatorics to Sage.
+
+Apply the patch and also copy the file wcf1.png into $SAGE_ROOT/devel/sage-queue/doc/en/lie/. Then you can build the documentation with the commands:
+
+sage -docbuild lie html
+
+sage -docbuild lie pdf
+
+There is supposed to be a chapter on Crystals but that is not written yet.
+
+Depends on #8411 and #8414 since it documents the behavior of Sage after these patches.
``````




---

archive/issue_comments_068640.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,7 +1,6 @@\n-\n This patch adds a tutorial about Lie Methods and Related Combinatorics to Sage.\n \n-Apply the patch and also copy the file wcf1.png into $SAGE_ROOT/devel/sage-queue/doc/en/lie/. Then you can build the documentation with the commands:\n+Apply the patch and also copy the file wcf1.png into $SAGE_ROOT/devel/sage/doc/en/lie/. Then you can build the documentation with the commands:\n \n sage -docbuild lie html\n \n``````\n",
    "created_at": "2010-03-05T10:48:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68640",
    "user": "https://github.com/dwbump"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,7 +1,6 @@
-
 This patch adds a tutorial about Lie Methods and Related Combinatorics to Sage.
 
-Apply the patch and also copy the file wcf1.png into $SAGE_ROOT/devel/sage-queue/doc/en/lie/. Then you can build the documentation with the commands:
+Apply the patch and also copy the file wcf1.png into $SAGE_ROOT/devel/sage/doc/en/lie/. Then you can build the documentation with the commands:
 
 sage -docbuild lie html
 
``````




---

archive/issue_comments_068641.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -9,3 +9,7 @@\n There is supposed to be a chapter on Crystals but that is not written yet.\n \n Depends on #8411 and #8414 since it documents the behavior of Sage after these patches.\n+\n+\n+\n+**Notes:** The current ticket needs to be coordinated with #8470.\n``````\n",
    "created_at": "2010-03-07T02:42:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68641",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -9,3 +9,7 @@
 There is supposed to be a chapter on Crystals but that is not written yet.
 
 Depends on #8411 and #8414 since it documents the behavior of Sage after these patches.
+
+
+
+**Notes:** The current ticket needs to be coordinated with #8470.
``````




---

archive/issue_comments_068642.json:
```json
{
    "body": "Attachment: **[a2rho.png](https://github.com/sagemath/sage/files/ticket8442/a2rho.png)**",
    "created_at": "2010-03-09T04:27:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68642",
    "user": "https://github.com/dwbump"
}
```

Attachment: **[a2rho.png](https://github.com/sagemath/sage/files/ticket8442/a2rho.png)**



---

archive/issue_comments_068643.json:
```json
{
    "body": "Attachment: **[standard1.png](https://github.com/sagemath/sage/files/ticket8442/standard1.png)**",
    "created_at": "2010-03-09T04:28:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68643",
    "user": "https://github.com/dwbump"
}
```

Attachment: **[standard1.png](https://github.com/sagemath/sage/files/ticket8442/standard1.png)**



---

archive/issue_comments_068644.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -6,10 +6,16 @@\n \n sage -docbuild lie pdf\n \n-There is supposed to be a chapter on Crystals but that is not written yet.\n-\n Depends on #8411 and #8414 since it documents the behavior of Sage after these patches.\n \n \n \n **Notes:** The current ticket needs to be coordinated with #8470.\n+\n+A copy of the html documentation may be found at:\n+\n+http://sporadic.stanford.edu/bump/trac_8442/\n+\n+I think an html link should be added from \n+`$SAGE_ROOT/devel/sage/doc/output/html/en/index.html`. I\n+did not add such a link in the patch. See #8470.\n``````\n",
    "created_at": "2010-03-09T04:38:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68644",
    "user": "https://github.com/dwbump"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -6,10 +6,16 @@
 
 sage -docbuild lie pdf
 
-There is supposed to be a chapter on Crystals but that is not written yet.
-
 Depends on #8411 and #8414 since it documents the behavior of Sage after these patches.
 
 
 
 **Notes:** The current ticket needs to be coordinated with #8470.
+
+A copy of the html documentation may be found at:
+
+http://sporadic.stanford.edu/bump/trac_8442/
+
+I think an html link should be added from 
+`$SAGE_ROOT/devel/sage/doc/output/html/en/index.html`. I
+did not add such a link in the patch. See #8470.
``````




---

archive/issue_comments_068645.json:
```json
{
    "body": "Changed work issues from **Crystal chapter not written yet** to none",
    "created_at": "2010-03-09T04:38:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68645",
    "user": "https://github.com/dwbump"
}
```

Changed work issues from **Crystal chapter not written yet** to none



---

archive/issue_events_096292.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-03-09T04:38:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96292"
}
```



---

archive/issue_comments_068646.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,6 +1,6 @@\n This patch adds a tutorial about Lie Methods and Related Combinatorics to Sage.\n \n-Apply the patch and also copy the file wcf1.png into $SAGE_ROOT/devel/sage/doc/en/lie/. Then you can build the documentation with the commands:\n+Apply the patch and also copy the files wcf1.png, a2rho.png and standard1.png into $SAGE_ROOT/devel/sage/doc/en/lie/. Then you can build the documentation with the commands:\n \n sage -docbuild lie html\n \n``````\n",
    "created_at": "2010-03-09T15:01:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68646",
    "user": "https://github.com/dwbump"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,6 +1,6 @@
 This patch adds a tutorial about Lie Methods and Related Combinatorics to Sage.
 
-Apply the patch and also copy the file wcf1.png into $SAGE_ROOT/devel/sage/doc/en/lie/. Then you can build the documentation with the commands:
+Apply the patch and also copy the files wcf1.png, a2rho.png and standard1.png into $SAGE_ROOT/devel/sage/doc/en/lie/. Then you can build the documentation with the commands:
 
 sage -docbuild lie html
 
``````




---

archive/issue_comments_068647.json:
```json
{
    "body": "Lie methods and related combinatorics tutorial",
    "created_at": "2010-03-11T18:15:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68647",
    "user": "https://github.com/dwbump"
}
```

Lie methods and related combinatorics tutorial



---

archive/issue_comments_068648.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nAttachment: **[trac_8442_lie_documentation.patch.gz](https://github.com/sagemath/sage/files/ticket8442/trac_8442_lie_documentation.patch.gz)**\n\nRevised to take into account changes in #8414.",
    "created_at": "2010-03-11T18:17:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68648",
    "user": "https://github.com/dwbump"
}
```

<div id="comment:7" align="right">Comment 7</div>

Attachment: **[trac_8442_lie_documentation.patch.gz](https://github.com/sagemath/sage/files/ticket8442/trac_8442_lie_documentation.patch.gz)**

Revised to take into account changes in #8414.



---

archive/issue_events_096293.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-03-12T22:50:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96293"
}
```



---

archive/issue_events_096294.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-03-12T22:50:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96294"
}
```



---

archive/issue_comments_068649.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nI'm rebasing [trac_8442_lie_documentation.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442_lie_documentation.patch.gz) on top of the prerequisites at #8470.",
    "created_at": "2010-03-12T22:50:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68649",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:8" align="right">Comment 8</div>

I'm rebasing [trac_8442_lie_documentation.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442_lie_documentation.patch.gz) on top of the prerequisites at #8470.



---

archive/issue_comments_068650.json:
```json
{
    "body": "Attachment: **[trac_8442-lie-rebased.patch.gz](https://github.com/sagemath/sage/files/ticket8442/trac_8442-lie-rebased.patch.gz)**\n\nrebased Lie tutorial",
    "created_at": "2010-03-13T07:28:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68650",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Attachment: **[trac_8442-lie-rebased.patch.gz](https://github.com/sagemath/sage/files/ticket8442/trac_8442-lie-rebased.patch.gz)**

rebased Lie tutorial



---

archive/issue_comments_068651.json:
```json
{
    "body": "Attachment: **[trac_8442-reviewer.patch.gz](https://github.com/sagemath/sage/files/ticket8442/trac_8442-reviewer.patch.gz)**\n\nreviewer patch",
    "created_at": "2010-03-13T07:28:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68651",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Attachment: **[trac_8442-reviewer.patch.gz](https://github.com/sagemath/sage/files/ticket8442/trac_8442-reviewer.patch.gz)**

reviewer patch



---

archive/issue_comments_068652.json:
```json
{
    "body": "Reviewer: **Minh Van Nguyen**",
    "created_at": "2010-03-13T07:39:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68652",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Reviewer: **Minh Van Nguyen**



---

archive/issue_comments_068653.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -19,3 +19,5 @@\n I think an html link should be added from \n `$SAGE_ROOT/devel/sage/doc/output/html/en/index.html`. I\n did not add such a link in the patch. See #8470.\n+\n+**Prerequisites:** #8468\n``````\n",
    "created_at": "2010-03-13T07:39:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68653",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -19,3 +19,5 @@
 I think an html link should be added from 
 `$SAGE_ROOT/devel/sage/doc/output/html/en/index.html`. I
 did not add such a link in the patch. See #8470.
+
+**Prerequisites:** #8468
``````




---

archive/issue_events_096295.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-03-13T07:39:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96295"
}
```



---

archive/issue_events_096296.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-03-13T07:39:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96296"
}
```



---

archive/issue_comments_068654.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nI have rebased Dan's patch; see the result at [trac_8442-lie-rebased.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-lie-rebased.patch.gz). I have also attached a reviewer patch that makes substantial clean-ups in terms of style and consistency. Changes include:\n\n1. Remove the use of \":math:\". That is not necessary, as the Sage docbuilder can handle typeset mathematics without that syntax. Also, the resulting text is easier to read without heaps of \":math:\" scattered throughout a ReST document.\n2. Some stylistic clean-ups.\n3. Make the formatting consistent.\n4. Using the syntax \".. MATH::\" to center math expressions.\n\nBoth Dan's rebased patch and my reviewer patch need reviewing. This ticket now depends on #8468.",
    "created_at": "2010-03-13T07:39:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68654",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:9" align="right">Comment 9</div>

I have rebased Dan's patch; see the result at [trac_8442-lie-rebased.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-lie-rebased.patch.gz). I have also attached a reviewer patch that makes substantial clean-ups in terms of style and consistency. Changes include:

1. Remove the use of ":math:". That is not necessary, as the Sage docbuilder can handle typeset mathematics without that syntax. Also, the resulting text is easier to read without heaps of ":math:" scattered throughout a ReST document.
2. Some stylistic clean-ups.
3. Make the formatting consistent.
4. Using the syntax ".. MATH::" to center math expressions.

Both Dan's rebased patch and my reviewer patch need reviewing. This ticket now depends on #8468.



---

archive/issue_comments_068655.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nLooks very good after the rebased patch and the reviewer's patch.\n\nThe three .png files must be in `doc/en/thematic_tutorials/static/` .\n\nThe patch will require minor revision if #8414 is not merged. (There\nis also a dependence on #8411 but less important.) If this proves\nnecessary I will provide a revision. It is a matter of changing \"space\"\nback to \"lattice\" in a few places.\n\nI give a positive review to the rebased and reviewer's patch. I am not\nchanging the patch status since I am the author of the original patch.",
    "created_at": "2010-03-13T18:45:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68655",
    "user": "https://github.com/dwbump"
}
```

<div id="comment:10" align="right">Comment 10</div>

Looks very good after the rebased patch and the reviewer's patch.

The three .png files must be in `doc/en/thematic_tutorials/static/` .

The patch will require minor revision if #8414 is not merged. (There
is also a dependence on #8411 but less important.) If this proves
necessary I will provide a revision. It is a matter of changing "space"
back to "lattice" in a few places.

I give a positive review to the rebased and reviewer's patch. I am not
changing the patch status since I am the author of the original patch.



---

archive/issue_comments_068656.json:
```json
{
    "body": "Attachment: **[trac_8442-weyl_groups-revision.patch.gz](https://github.com/sagemath/sage/files/ticket8442/trac_8442-weyl_groups-revision.patch.gz)**\n\nAdditional patch taking into account changes in #8414",
    "created_at": "2010-03-24T22:15:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68656",
    "user": "https://github.com/dwbump"
}
```

Attachment: **[trac_8442-weyl_groups-revision.patch.gz](https://github.com/sagemath/sage/files/ticket8442/trac_8442-weyl_groups-revision.patch.gz)**

Additional patch taking into account changes in #8414



---

archive/issue_comments_068657.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -16,8 +16,14 @@\n \n http://sporadic.stanford.edu/bump/trac_8442/\n \n-I think an html link should be added from \n-`$SAGE_ROOT/devel/sage/doc/output/html/en/index.html`. I\n-did not add such a link in the patch. See #8470.\n+There are three patches to merge:\n+\n+```\n+ trac_8442-lie-rebased.patch\n+ trac_8442-reviewer.patch\n+ trac_8442-weyl_groups-revision.patch\n+```\n+\n+In addition, the three .png files must be where they get properly linked.\n \n **Prerequisites:** #8468\n``````\n",
    "created_at": "2010-03-24T22:50:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68657",
    "user": "https://github.com/dwbump"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -16,8 +16,14 @@
 
 http://sporadic.stanford.edu/bump/trac_8442/
 
-I think an html link should be added from 
-`$SAGE_ROOT/devel/sage/doc/output/html/en/index.html`. I
-did not add such a link in the patch. See #8470.
+There are three patches to merge:
+
+```
+ trac_8442-lie-rebased.patch
+ trac_8442-reviewer.patch
+ trac_8442-weyl_groups-revision.patch
+```
+
+In addition, the three .png files must be where they get properly linked.
 
 **Prerequisites:** #8468
``````




---

archive/issue_comments_068658.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nDue to changes in #8414, there is an additional small patch called trac_8442-weyl_groups-revision.patch.",
    "created_at": "2010-03-24T22:50:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68658",
    "user": "https://github.com/dwbump"
}
```

<div id="comment:11" align="right">Comment 11</div>

Due to changes in #8414, there is an additional small patch called trac_8442-weyl_groups-revision.patch.



---

archive/issue_comments_068659.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nLooks good! I tested it fine using the three latest patches and the three PNG files. Pictures were inserted properly and looked fine.\n~Mark",
    "created_at": "2010-05-05T19:49:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68659",
    "user": "https://github.com/sagetrac-mjordan7"
}
```

<div id="comment:12" align="right">Comment 12</div>

Looks good! I tested it fine using the three latest patches and the three PNG files. Pictures were inserted properly and looked fine.
~Mark



---

archive/issue_events_096297.json:
```json
{
    "actor": "https://github.com/sagetrac-mjordan7",
    "created_at": "2010-05-14T19:50:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96297"
}
```



---

archive/issue_events_096298.json:
```json
{
    "actor": "https://github.com/sagetrac-mjordan7",
    "created_at": "2010-05-14T19:50:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96298"
}
```



---

archive/issue_comments_068660.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nTested with Sage Version 4.4.2.alpha0, Release Date: 2010-05-08 and index.rst under thematic tutorials fails to apply now.\n~Mark",
    "created_at": "2010-05-14T19:50:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68660",
    "user": "https://github.com/sagetrac-mjordan7"
}
```

<div id="comment:13" align="right">Comment 13</div>

Tested with Sage Version 4.4.2.alpha0, Release Date: 2010-05-08 and index.rst under thematic tutorials fails to apply now.
~Mark



---

archive/issue_comments_068661.json:
```json
{
    "body": "Attachment: **[trac_8442-lie-rebased-to-4.4.2.alpha0.gz](https://github.com/sagemath/sage/files/ticket8442/trac_8442-lie-rebased-to-4.4.2.alpha0.gz)**",
    "created_at": "2010-05-15T03:55:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68661",
    "user": "https://github.com/dwbump"
}
```

Attachment: **[trac_8442-lie-rebased-to-4.4.2.alpha0.gz](https://github.com/sagemath/sage/files/ticket8442/trac_8442-lie-rebased-to-4.4.2.alpha0.gz)**



---

archive/issue_comments_068662.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nI have uploaded the patch trac_8442-lie-rebased-to-4.4.2.alpha0.\n\nThis contains the three patches:\n\n```\ntrac_8442-lie-rebased.patch \ntrac_8442-reviewer.patch-reviewer patch\ntrac_8442-weyl_groups-revision.patch\n```\n\nThe purpose of this patch is simply to address mjordan's comment that the patch does not apply cleanly. In order to accomplish a clean this the patch\nadds files doc/en/thematic_tutorials/index.rst and bibliography.rst.\nThese files presume that three other thematic tutorials will also be included in sage - otherwise they will produce broken links. \n\nThe release manager may prefer to work with the original three patches.\n\nNote that in addition to the patch three .png files must be included.",
    "created_at": "2010-05-15T04:05:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68662",
    "user": "https://github.com/dwbump"
}
```

<div id="comment:14" align="right">Comment 14</div>

I have uploaded the patch trac_8442-lie-rebased-to-4.4.2.alpha0.

This contains the three patches:

```
trac_8442-lie-rebased.patch 
trac_8442-reviewer.patch-reviewer patch
trac_8442-weyl_groups-revision.patch
```

The purpose of this patch is simply to address mjordan's comment that the patch does not apply cleanly. In order to accomplish a clean this the patch
adds files doc/en/thematic_tutorials/index.rst and bibliography.rst.
These files presume that three other thematic tutorials will also be included in sage - otherwise they will produce broken links. 

The release manager may prefer to work with the original three patches.

Note that in addition to the patch three .png files must be included.



---

archive/issue_events_096299.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-05-15T04:05:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96299"
}
```



---

archive/issue_events_096300.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-05-15T04:05:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96300"
}
```



---

archive/issue_comments_068663.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -24,6 +24,15 @@\n  trac_8442-weyl_groups-revision.patch\n ```\n \n+The patches do not apply to sage-4.4.2.alpha0 because two files \n+in doc/en/thematic_tutorials are missing: index.rst and bibliography.rst.\n+An alternative patch\n+\n+```\n+trac_8442-lie-rebased-to-4.4.2.alpha0\n+```\n+\n+includes these files, and applies cleanly to sage-4.4.2.alpha0. Otherwise it is\n+identical to the union of the three patches.\n+\n In addition, the three .png files must be where they get properly linked.\n-\n-**Prerequisites:** #8468\n``````\n",
    "created_at": "2010-05-15T04:10:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68663",
    "user": "https://github.com/dwbump"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -24,6 +24,15 @@
  trac_8442-weyl_groups-revision.patch
 ```
 
+The patches do not apply to sage-4.4.2.alpha0 because two files 
+in doc/en/thematic_tutorials are missing: index.rst and bibliography.rst.
+An alternative patch
+
+```
+trac_8442-lie-rebased-to-4.4.2.alpha0
+```
+
+includes these files, and applies cleanly to sage-4.4.2.alpha0. Otherwise it is
+identical to the union of the three patches.
+
 In addition, the three .png files must be where they get properly linked.
-
-**Prerequisites:** #8468
``````




---

archive/issue_comments_068664.json:
```json
{
    "body": "Attachment: **[trac_8442-config.patch.gz](https://github.com/sagemath/sage/files/ticket8442/trac_8442-config.patch.gz)**",
    "created_at": "2010-05-15T06:59:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68664",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Attachment: **[trac_8442-config.patch.gz](https://github.com/sagemath/sage/files/ticket8442/trac_8442-config.patch.gz)**



---

archive/issue_comments_068665.json:
```json
{
    "body": "Attachment: **[trac_8442-lie-rebased-4.4.2.rc0.patch.gz](https://github.com/sagemath/sage/files/ticket8442/trac_8442-lie-rebased-4.4.2.rc0.patch.gz)**",
    "created_at": "2010-05-15T06:59:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68665",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Attachment: **[trac_8442-lie-rebased-4.4.2.rc0.patch.gz](https://github.com/sagemath/sage/files/ticket8442/trac_8442-lie-rebased-4.4.2.rc0.patch.gz)**



---

archive/issue_comments_068666.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -36,3 +36,10 @@\n identical to the union of the three patches.\n \n In addition, the three .png files must be where they get properly linked.\n+\n+**Apply:**\n+\n+1. #8464\n+2. [trac_8442-config.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-config.patch.gz)\n+3. [trac_8442-lie-rebased-4.4.2.rc0.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-lie-rebased-4.4.2.rc0.patch.gz)\n+4. Copy [wcf1.png](https://github.com/sagemath/sage/files/ticket8442/wcf1.png), [a2rho.png](https://github.com/sagemath/sage/files/ticket8442/a2rho.png), and [standard1.png](https://github.com/sagemath/sage/files/ticket8442/standard1.png) to the directory `doc/en/thematic_tutorials/static/`.\n``````\n",
    "created_at": "2010-05-15T07:11:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68666",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -36,3 +36,10 @@
 identical to the union of the three patches.
 
 In addition, the three .png files must be where they get properly linked.
+
+**Apply:**
+
+1. #8464
+2. [trac_8442-config.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-config.patch.gz)
+3. [trac_8442-lie-rebased-4.4.2.rc0.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-lie-rebased-4.4.2.rc0.patch.gz)
+4. Copy [wcf1.png](https://github.com/sagemath/sage/files/ticket8442/wcf1.png), [a2rho.png](https://github.com/sagemath/sage/files/ticket8442/a2rho.png), and [standard1.png](https://github.com/sagemath/sage/files/ticket8442/standard1.png) to the directory `doc/en/thematic_tutorials/static/`.
``````




---

archive/issue_comments_068667.json:
```json
{
    "body": "Changed reviewer from **Minh Van Nguyen** to **Minh Van Nguyen, Mark Jordan**",
    "created_at": "2010-05-15T07:11:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68667",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Changed reviewer from **Minh Van Nguyen** to **Minh Van Nguyen, Mark Jordan**



---

archive/issue_comments_068668.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">Comment 16</div>\n\nIn addition to applying cleanly, the thematic tutorials must also show up on the standard documentation and be linked from there. I have uploaded the patch [trac_8442-config.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-config.patch.gz) in order to configure the documentation build system to link to the thematic tutorials. The patch [trac_8442-lie-rebased-4.4.2.rc0.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-lie-rebased-4.4.2.rc0.patch.gz) is a rebase of Dan's patch against Sage 4.4.2.rc0. This rebase also configures the doc build system to build the Lie tutorial. The situation now is that anyone but myself can sign off on the two latest patches I uploaded. That is, anyone but myself need to only review those two patches. See the ticket description for instruction on how to apply patches on this ticket.",
    "created_at": "2010-05-15T07:11:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68668",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:16" align="right">Comment 16</div>

In addition to applying cleanly, the thematic tutorials must also show up on the standard documentation and be linked from there. I have uploaded the patch [trac_8442-config.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-config.patch.gz) in order to configure the documentation build system to link to the thematic tutorials. The patch [trac_8442-lie-rebased-4.4.2.rc0.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-lie-rebased-4.4.2.rc0.patch.gz) is a rebase of Dan's patch against Sage 4.4.2.rc0. This rebase also configures the doc build system to build the Lie tutorial. The situation now is that anyone but myself can sign off on the two latest patches I uploaded. That is, anyone but myself need to only review those two patches. See the ticket description for instruction on how to apply patches on this ticket.



---

archive/issue_comments_068669.json:
```json
{
    "body": "<div id=\"comment:17\" align=\"right\">Comment 17</div>\n\nThis is a very impressive contribution to the documentation and will be a great example for the thematic section.  Applied #8464, the two patches, and the added the three PNG's.  Everything applied and built cleanly, once I caught the proper location for the three PNG files.\n\nHowever, running doctests produced 22 errors across several of the files.  On 4.4.2.rc0\n\n```\nsage -t doc/en/thematic_tutorials/lie\n```\n\nyields\n\n```\nThe following tests failed:\n\n\n        sage -t  \"devel/sage-main/doc/en/thematic_tutorials/lie/weyl_groups.rst\"\n        sage -t  \"devel/sage-main/doc/en/thematic_tutorials/lie/weight_ring.rst\"\n        sage -t  \"devel/sage-main/doc/en/thematic_tutorials/lie/crystals.rst\"\n        sage -t  \"devel/sage-main/doc/en/thematic_tutorials/lie/weyl_character_ring.rst\" # Exception from doctest framework\n        sage -t  \"devel/sage-main/doc/en/thematic_tutorials/lie/branching_rules.rst\"\n```\n\nI did not investigate the source of the failures very carefully, Many of them were of the \"name 'foo' not defined\" variety rather than mis-matched outputs.  Hopefully this isn't a false alarm.\n\nI'd be happy to give this another look after somebody investigates/fixes the doctests.\n\nRob",
    "created_at": "2010-05-24T03:16:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68669",
    "user": "https://github.com/rbeezer"
}
```

<div id="comment:17" align="right">Comment 17</div>

This is a very impressive contribution to the documentation and will be a great example for the thematic section.  Applied #8464, the two patches, and the added the three PNG's.  Everything applied and built cleanly, once I caught the proper location for the three PNG files.

However, running doctests produced 22 errors across several of the files.  On 4.4.2.rc0

```
sage -t doc/en/thematic_tutorials/lie
```

yields

```
The following tests failed:


        sage -t  "devel/sage-main/doc/en/thematic_tutorials/lie/weyl_groups.rst"
        sage -t  "devel/sage-main/doc/en/thematic_tutorials/lie/weight_ring.rst"
        sage -t  "devel/sage-main/doc/en/thematic_tutorials/lie/crystals.rst"
        sage -t  "devel/sage-main/doc/en/thematic_tutorials/lie/weyl_character_ring.rst" # Exception from doctest framework
        sage -t  "devel/sage-main/doc/en/thematic_tutorials/lie/branching_rules.rst"
```

I did not investigate the source of the failures very carefully, Many of them were of the "name 'foo' not defined" variety rather than mis-matched outputs.  Hopefully this isn't a false alarm.

I'd be happy to give this another look after somebody investigates/fixes the doctests.

Rob



---

archive/issue_events_096301.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2010-05-24T03:16:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96301"
}
```



---

archive/issue_events_096302.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2010-05-24T03:16:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96302"
}
```



---

archive/issue_comments_068670.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">Comment 18</div>\n\nIn addition to the doctest failures, here are some other issues:\n\nThe documentation page that you get by clicking \"help\" in the notebook, then \"fast static versions ...\", does not look good with the new section on thematic tutorials: I don't think the three-column layout looks good.\n\nThe command `\\mathfrak` is not recognized by jsMath, so if you make the documentation with the `--jsmath` flag, it has a lot of errors.\n\nAlso, I would suggest creating a directory `doc/en/thematic_tutorials/media/` (just like in the `reference` directory, and we put pictures there.   Wherever the pictures go, they need to be included in SAGE_ROOT/devel/sage/MANIFEST.in so they get included in the Sage library spkg -- you need a line like\n\n```\nrecursive-include doc/en/thematic_tutorials/media *\n```\n\nBy the way, you can include the png files in the patch by using the command\n\n```\n$ hg add (path to file)/standard1.png\n$ hg commit ...\n$ hg export -g ...\n```\n(See [https://github.com/sagemath/sage/issues/9074#comment:4](https://github.com/sagemath/sage/issues/9074#comment:4) for a similar explanation.)",
    "created_at": "2010-06-23T17:40:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68670",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:18" align="right">Comment 18</div>

In addition to the doctest failures, here are some other issues:

The documentation page that you get by clicking "help" in the notebook, then "fast static versions ...", does not look good with the new section on thematic tutorials: I don't think the three-column layout looks good.

The command `\mathfrak` is not recognized by jsMath, so if you make the documentation with the `--jsmath` flag, it has a lot of errors.

Also, I would suggest creating a directory `doc/en/thematic_tutorials/media/` (just like in the `reference` directory, and we put pictures there.   Wherever the pictures go, they need to be included in SAGE_ROOT/devel/sage/MANIFEST.in so they get included in the Sage library spkg -- you need a line like

```
recursive-include doc/en/thematic_tutorials/media *
```

By the way, you can include the png files in the patch by using the command

```
$ hg add (path to file)/standard1.png
$ hg commit ...
$ hg export -g ...
```
(See [https://github.com/sagemath/sage/issues/9074#comment:4](https://github.com/sagemath/sage/issues/9074#comment:4) for a similar explanation.)



---

archive/issue_comments_068671.json:
```json
{
    "body": "<div id=\"comment:19\" align=\"right\">Comment 19</div>\n\nIf #8465 gets merged, then trac_8442-config.patch will not be needed.",
    "created_at": "2010-07-28T01:36:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68671",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:19" align="right">Comment 19</div>

If #8465 gets merged, then trac_8442-config.patch will not be needed.



---

archive/issue_comments_068672.json:
```json
{
    "body": "Attachment: **[trac_8442-lie-rebased-4.5.2.patch.gz](https://github.com/sagemath/sage/files/ticket8442/trac_8442-lie-rebased-4.5.2.patch.gz)**",
    "created_at": "2010-08-22T15:53:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68672",
    "user": "https://github.com/dwbump"
}
```

Attachment: **[trac_8442-lie-rebased-4.5.2.patch.gz](https://github.com/sagemath/sage/files/ticket8442/trac_8442-lie-rebased-4.5.2.patch.gz)**



---

archive/issue_comments_068673.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -43,3 +43,5 @@\n 2. [trac_8442-config.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-config.patch.gz)\n 3. [trac_8442-lie-rebased-4.4.2.rc0.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-lie-rebased-4.4.2.rc0.patch.gz)\n 4. Copy [wcf1.png](https://github.com/sagemath/sage/files/ticket8442/wcf1.png), [a2rho.png](https://github.com/sagemath/sage/files/ticket8442/a2rho.png), and [standard1.png](https://github.com/sagemath/sage/files/ticket8442/standard1.png) to the directory `doc/en/thematic_tutorials/static/`.\n+\n+\n``````\n",
    "created_at": "2010-08-22T16:04:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68673",
    "user": "https://github.com/dwbump"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -43,3 +43,5 @@
 2. [trac_8442-config.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-config.patch.gz)
 3. [trac_8442-lie-rebased-4.4.2.rc0.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-lie-rebased-4.4.2.rc0.patch.gz)
 4. Copy [wcf1.png](https://github.com/sagemath/sage/files/ticket8442/wcf1.png), [a2rho.png](https://github.com/sagemath/sage/files/ticket8442/a2rho.png), and [standard1.png](https://github.com/sagemath/sage/files/ticket8442/standard1.png) to the directory `doc/en/thematic_tutorials/static/`.
+
+
``````




---

archive/issue_comments_068674.json:
```json
{
    "body": "<div id=\"comment:20\" align=\"right\">Comment 20</div>\n\nAttachment: **[trac_8442-lie-rebased-4.5.2.2.patch.gz](https://github.com/sagemath/sage/files/ticket8442/trac_8442-lie-rebased-4.5.2.2.patch.gz)**\n\nI have added two revised patches:\n\n```\ntrac_8442-lie-rebased-4.5.2.patch\ntrac_8442-png-files.patch\n```\n\nBy mistake I added the first patch twice.\n\nThese address most of the problems found by rbeezer and jhpalmieri.\n\n* Now passes `sage -t`\n\n* New `doc/en/thematic_tutorials/media/*png`\n\n* `doc/en/thematic_tutorials/*png` added to MANIFEST.in\n\n* `.png` files are in a patch.\n\nThe one matter I did not try to address is the problem with jsMath. Is there a jsMath way to get fraktur fonts?",
    "created_at": "2010-08-22T16:04:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68674",
    "user": "https://github.com/dwbump"
}
```

<div id="comment:20" align="right">Comment 20</div>

Attachment: **[trac_8442-lie-rebased-4.5.2.2.patch.gz](https://github.com/sagemath/sage/files/ticket8442/trac_8442-lie-rebased-4.5.2.2.patch.gz)**

I have added two revised patches:

```
trac_8442-lie-rebased-4.5.2.patch
trac_8442-png-files.patch
```

By mistake I added the first patch twice.

These address most of the problems found by rbeezer and jhpalmieri.

* Now passes `sage -t`

* New `doc/en/thematic_tutorials/media/*png`

* `doc/en/thematic_tutorials/*png` added to MANIFEST.in

* `.png` files are in a patch.

The one matter I did not try to address is the problem with jsMath. Is there a jsMath way to get fraktur fonts?



---

archive/issue_events_096303.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-08-22T16:11:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96303"
}
```



---

archive/issue_events_096304.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-08-22T16:11:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96304"
}
```



---

archive/issue_comments_068675.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -44,4 +44,14 @@\n 3. [trac_8442-lie-rebased-4.4.2.rc0.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-lie-rebased-4.4.2.rc0.patch.gz)\n 4. Copy [wcf1.png](https://github.com/sagemath/sage/files/ticket8442/wcf1.png), [a2rho.png](https://github.com/sagemath/sage/files/ticket8442/a2rho.png), and [standard1.png](https://github.com/sagemath/sage/files/ticket8442/standard1.png) to the directory `doc/en/thematic_tutorials/static/`.\n \n+**Revised instructions. Apply:**\n \n+```\n+ trac_8442-lie-rebased-4.5.2.patch\n+ trac_8442-png-files.patch\n+```\n+\n+\n+\n+\n+\n``````\n",
    "created_at": "2010-08-22T16:11:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68675",
    "user": "https://github.com/dwbump"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -44,4 +44,14 @@
 3. [trac_8442-lie-rebased-4.4.2.rc0.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-lie-rebased-4.4.2.rc0.patch.gz)
 4. Copy [wcf1.png](https://github.com/sagemath/sage/files/ticket8442/wcf1.png), [a2rho.png](https://github.com/sagemath/sage/files/ticket8442/a2rho.png), and [standard1.png](https://github.com/sagemath/sage/files/ticket8442/standard1.png) to the directory `doc/en/thematic_tutorials/static/`.
 
+**Revised instructions. Apply:**
 
+```
+ trac_8442-lie-rebased-4.5.2.patch
+ trac_8442-png-files.patch
+```
+
+
+
+
+
``````




---

archive/issue_comments_068676.json:
```json
{
    "body": "<div id=\"comment:22\" align=\"right\">Comment 22</div>\n\nReplying to [@dwbump](#comment%3A20):\n> The one matter I did not try to address is the problem with jsMath. Is there a jsMath way to get fraktur fonts?\n\nMathJAX is slated to replace jsMath.  It has been released, but I can't guess at how much work it will be to get it into the notebook.\nhttps://github.com/sagemath/sage/issues/9774\n\nIt would appear from the discussion below that Fraktur fonts will be possible (maybe standard) then.\nhttp://meta.mathoverflow.net/discussion/473/help-test-mathjax/\n\nIt seems Fraktur is available now for jsMath, but maybe we don't ship that with the notebook?\nhttp://www.math.union.edu/~dpvc/jsMath/download/extra-fonts/welcome.html\n\nI'll look closer at reviewing this again when I've got a bit more time.\n\nRob",
    "created_at": "2010-08-22T20:19:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68676",
    "user": "https://github.com/rbeezer"
}
```

<div id="comment:22" align="right">Comment 22</div>

Replying to [@dwbump](#comment%3A20):
> The one matter I did not try to address is the problem with jsMath. Is there a jsMath way to get fraktur fonts?

MathJAX is slated to replace jsMath.  It has been released, but I can't guess at how much work it will be to get it into the notebook.
https://github.com/sagemath/sage/issues/9774

It would appear from the discussion below that Fraktur fonts will be possible (maybe standard) then.
http://meta.mathoverflow.net/discussion/473/help-test-mathjax/

It seems Fraktur is available now for jsMath, but maybe we don't ship that with the notebook?
http://www.math.union.edu/~dpvc/jsMath/download/extra-fonts/welcome.html

I'll look closer at reviewing this again when I've got a bit more time.

Rob



---

archive/issue_events_096305.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-09-04T19:10:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96305"
}
```



---

archive/issue_events_096306.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-09-04T19:10:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96306"
}
```



---

archive/issue_comments_068677.json:
```json
{
    "body": "<div id=\"comment:23\" align=\"right\">Comment 23</div>\n\nI just discovered that after the patch this still does *not* pass sage -t.\n\nMaybe I uploaded the wrong patch.\n\nI am changing the status back to needs_work.\n\nDan",
    "created_at": "2010-09-04T19:10:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68677",
    "user": "https://github.com/dwbump"
}
```

<div id="comment:23" align="right">Comment 23</div>

I just discovered that after the patch this still does *not* pass sage -t.

Maybe I uploaded the wrong patch.

I am changing the status back to needs_work.

Dan



---

archive/issue_comments_068678.json:
```json
{
    "body": "<div id=\"comment:24\" align=\"right\">Comment 24</div>\n\nEvidently I made a mercurial mistake and uploaded the wrong patch. (I probably forgot to commit before exporting.)\n\nFurthermore, the fraktur problem is serious enough that it needs to be fixed. Unfortunately frakture jsmath fonts are available but not installed by default. Perhaps the best short-term option is to find another font.\n\nFinally, I can make use of the information here:\n\nhttp://groups.google.com/group/sage-devel/msg/086441965eb4f20b\n\nto improve the document. This addresses the cause of many of the failures.\nI am leaving the status needs_work for the time being.",
    "created_at": "2010-09-05T13:19:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68678",
    "user": "https://github.com/dwbump"
}
```

<div id="comment:24" align="right">Comment 24</div>

Evidently I made a mercurial mistake and uploaded the wrong patch. (I probably forgot to commit before exporting.)

Furthermore, the fraktur problem is serious enough that it needs to be fixed. Unfortunately frakture jsmath fonts are available but not installed by default. Perhaps the best short-term option is to find another font.

Finally, I can make use of the information here:

http://groups.google.com/group/sage-devel/msg/086441965eb4f20b

to improve the document. This addresses the cause of many of the failures.
I am leaving the status needs_work for the time being.



---

archive/issue_comments_068679.json:
```json
{
    "body": "<div id=\"comment:25\" align=\"right\">Comment 25</div>\n\nI am posting corrected files that pass `sage -t` with sage-4.5.3.\n\nRegarding the --jsmath issue, the \\mathfrak problem is not the end of the story. There are further problems. Matrices of size > 2 look awful with jsmath, and in the section crystals.rst there are some tableaux. I tried two different methods of creating these in tex. One failed since jsmath doesn't know \\raisebox, and the other (based on arrays) failed since jsmath doesn't know \\hline.\n\nI therefore feel it is better not to try revising the files to work with jsmath. I recommend building them without jsmath and can revisit the issue after the switch to mathjax. The built documentation (with dvipng) is here:\n\nhttp://match.stanford.edu/bump/thematic_tutorials/lie.html\n\nI am changing the status back to needs_review.",
    "created_at": "2010-09-09T00:14:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68679",
    "user": "https://github.com/dwbump"
}
```

<div id="comment:25" align="right">Comment 25</div>

I am posting corrected files that pass `sage -t` with sage-4.5.3.

Regarding the --jsmath issue, the \mathfrak problem is not the end of the story. There are further problems. Matrices of size > 2 look awful with jsmath, and in the section crystals.rst there are some tableaux. I tried two different methods of creating these in tex. One failed since jsmath doesn't know \raisebox, and the other (based on arrays) failed since jsmath doesn't know \hline.

I therefore feel it is better not to try revising the files to work with jsmath. I recommend building them without jsmath and can revisit the issue after the switch to mathjax. The built documentation (with dvipng) is here:

http://match.stanford.edu/bump/thematic_tutorials/lie.html

I am changing the status back to needs_review.



---

archive/issue_events_096307.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-09-09T00:14:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96307"
}
```



---

archive/issue_events_096308.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-09-09T00:14:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96308"
}
```



---

archive/issue_comments_068680.json:
```json
{
    "body": "<div id=\"comment:26\" align=\"right\">Comment 26</div>\n\nReplying to [@dwbump](#comment%3A25):\n> Matrices of size > 2 look awful with jsmath, and in the section crystals.rst there are some tableaux. I tried two different methods of creating these in tex. One failed since jsmath doesn't know \\raisebox, and the other (based on arrays) failed since jsmath doesn't know \\hline.\n\nHi Dan,\n\nI've had good luck with jsMath if I use \\begin{tabular}\\end{tabular} and then wrap individual entries with dollar signs.  Then the \\hline's seem to workout OK.  The PDF doesn't really seem to suffer too much from this abuse.\n\nI don't know if MathJAX or jsMath will ever be too happy about a \\raisebox - the notion of a precise length doesn't always translate from printed page to web page very well.\n\nAnyway, I didn't even look at your tableau, but you might experiment with a single one using the tabular environment and decide from there.\n\nRob",
    "created_at": "2010-09-09T00:34:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68680",
    "user": "https://github.com/rbeezer"
}
```

<div id="comment:26" align="right">Comment 26</div>

Replying to [@dwbump](#comment%3A25):
> Matrices of size > 2 look awful with jsmath, and in the section crystals.rst there are some tableaux. I tried two different methods of creating these in tex. One failed since jsmath doesn't know \raisebox, and the other (based on arrays) failed since jsmath doesn't know \hline.

Hi Dan,

I've had good luck with jsMath if I use \begin{tabular}\end{tabular} and then wrap individual entries with dollar signs.  Then the \hline's seem to workout OK.  The PDF doesn't really seem to suffer too much from this abuse.

I don't know if MathJAX or jsMath will ever be too happy about a \raisebox - the notion of a precise length doesn't always translate from printed page to web page very well.

Anyway, I didn't even look at your tableau, but you might experiment with a single one using the tabular environment and decide from there.

Rob



---

archive/issue_comments_068681.json:
```json
{
    "body": "<div id=\"comment:27\" align=\"right\">Comment 27</div>\n\n> I've had good luck with jsMath if I use \\begin{tabular}\\end{tabular} and then wrap individual entries with dollar signs. Then the \\hline's seem to workout OK. The PDF doesn't really seem to suffer too much from this abuse.\n\nCan you give an example?\n\nMy hope is that the patch can be reviewed without making jsmath a prerequisite. The doc looks good with dvipng, but not so good with jsmath. I could substitute another font for fraktur (I tried mathbf), and I could try again with your suggestion for making tableaux, and I could try to overlook the fact that the large matrices really awful with mathjs, but given that the dvipng doc looks fine, I'd rather leave it as it is and revisit the issue when mathjax rolls in.\n\nDan",
    "created_at": "2010-09-09T04:07:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68681",
    "user": "https://github.com/dwbump"
}
```

<div id="comment:27" align="right">Comment 27</div>

> I've had good luck with jsMath if I use \begin{tabular}\end{tabular} and then wrap individual entries with dollar signs. Then the \hline's seem to workout OK. The PDF doesn't really seem to suffer too much from this abuse.

Can you give an example?

My hope is that the patch can be reviewed without making jsmath a prerequisite. The doc looks good with dvipng, but not so good with jsmath. I could substitute another font for fraktur (I tried mathbf), and I could try again with your suggestion for making tableaux, and I could try to overlook the fact that the large matrices really awful with mathjs, but given that the dvipng doc looks fine, I'd rather leave it as it is and revisit the issue when mathjax rolls in.

Dan



---

archive/issue_comments_068682.json:
```json
{
    "body": "<div id=\"comment:28\" align=\"right\">Comment 28</div>\n\nI almost always use the `--jsmath` option for docbuilding because it's faster than not using it.  I would guess that some other people do the same.  We really try to avoid warnings when building the docs, so I don't know if this should get a positive review if it doesn't build right with jsmath, especially since there is an easy fix: use something other than mathfrak.\n\nWait, here's an idea: we can add a jsMath macro (as [described here](http://www.math.union.edu/~dpvc/jsMath/authors/macros.html)) to define `\\mathfrak` in jsMath to be boldface, for example.  Since this is defined in jsMath, it doesn't override the ordinary definition if you're not using the jsmath option.  This is easy to do: we just patch the configuration file for the thematic tutorials document.  I'm attaching a patch.",
    "created_at": "2010-09-09T04:37:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68682",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:28" align="right">Comment 28</div>

I almost always use the `--jsmath` option for docbuilding because it's faster than not using it.  I would guess that some other people do the same.  We really try to avoid warnings when building the docs, so I don't know if this should get a positive review if it doesn't build right with jsmath, especially since there is an easy fix: use something other than mathfrak.

Wait, here's an idea: we can add a jsMath macro (as [described here](http://www.math.union.edu/~dpvc/jsMath/authors/macros.html)) to define `\mathfrak` in jsMath to be boldface, for example.  Since this is defined in jsMath, it doesn't override the ordinary definition if you're not using the jsmath option.  This is easy to do: we just patch the configuration file for the thematic tutorials document.  I'm attaching a patch.



---

archive/issue_comments_068683.json:
```json
{
    "body": "Attachment: **[trac_8442-mathfrak.patch.gz](https://github.com/sagemath/sage/files/ticket8442/trac_8442-mathfrak.patch.gz)**\n\navoid jsmath errors about mathfrak",
    "created_at": "2010-09-09T04:41:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68683",
    "user": "https://github.com/jhpalmieri"
}
```

Attachment: **[trac_8442-mathfrak.patch.gz](https://github.com/sagemath/sage/files/ticket8442/trac_8442-mathfrak.patch.gz)**

avoid jsmath errors about mathfrak



---

archive/issue_comments_068684.json:
```json
{
    "body": "<div id=\"comment:29\" align=\"right\">Comment 29</div>\n\nI have revised the patch. The resulting patch produces a tutorial that looks OK with both dvipng and jsmath. This addressed three problems.\n\n(1) For \\mathfrak, it includes trac_8442-mathfrak.patch (thanks!)\n\n(2) The problems with matrices turned out to be only on a particular machine. On this machine, every browser I've tried (firefox, seamonkey and konqueror) makes jsmath matrices look very broken. But the same html looks OK on other machines.\n\n(3) For the tableaux, I hand-made png images for all of these. I also did some other rewriting of the section on crystals.\n\nThanks to jhpalmieri and rbeezer for feedback.\n\nDan",
    "created_at": "2010-09-09T20:24:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68684",
    "user": "https://github.com/dwbump"
}
```

<div id="comment:29" align="right">Comment 29</div>

I have revised the patch. The resulting patch produces a tutorial that looks OK with both dvipng and jsmath. This addressed three problems.

(1) For \mathfrak, it includes trac_8442-mathfrak.patch (thanks!)

(2) The problems with matrices turned out to be only on a particular machine. On this machine, every browser I've tried (firefox, seamonkey and konqueror) makes jsmath matrices look very broken. But the same html looks OK on other machines.

(3) For the tableaux, I hand-made png images for all of these. I also did some other rewriting of the section on crystals.

Thanks to jhpalmieri and rbeezer for feedback.

Dan



---

archive/issue_comments_068685.json:
```json
{
    "body": "<div id=\"comment:30\" align=\"right\">Comment 30</div>\n\nHi Dan,\n\nI think I was a bit too cavalier.  I've been converting a lot of Latex to jsMath (using the tex4ht converter) and switching to tabular solves lots of gotchas, but I don't think I can make that approach work directly.  In other words, no - I don't have an example, despite trying quite a few things last night.  I hope I didn't send you on too wild of a goose chase.\n\nOn the plus side, `MathJax` looks good - I've been fiddling with it (outside of the notebook).\n\nI think PNGs are a good compromise.  I've been helping Tom Judson with his open source textbook and he's redone the diagrams in tikz.  There's code in the CVS version of PGF that will \"externalize\" the graphics.  So the PDF has \"native\" diagrams, and the web page versions suck in the externalized PNG graphics versions as images.  Real similar to your workaround here.\n\nRob",
    "created_at": "2010-09-09T21:01:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68685",
    "user": "https://github.com/rbeezer"
}
```

<div id="comment:30" align="right">Comment 30</div>

Hi Dan,

I think I was a bit too cavalier.  I've been converting a lot of Latex to jsMath (using the tex4ht converter) and switching to tabular solves lots of gotchas, but I don't think I can make that approach work directly.  In other words, no - I don't have an example, despite trying quite a few things last night.  I hope I didn't send you on too wild of a goose chase.

On the plus side, `MathJax` looks good - I've been fiddling with it (outside of the notebook).

I think PNGs are a good compromise.  I've been helping Tom Judson with his open source textbook and he's redone the diagrams in tikz.  There's code in the CVS version of PGF that will "externalize" the graphics.  So the PDF has "native" diagrams, and the web page versions suck in the externalized PNG graphics versions as images.  Real similar to your workaround here.

Rob



---

archive/issue_comments_068686.json:
```json
{
    "body": "<div id=\"comment:31\" align=\"right\">Comment 31</div>\n\nI reposted the two relevant patches, trac_8442-png-files.patch and trac_8442-rebased-4.5.3.patch.\n\nI made some further changes to crystals.rst and I found that one of the png files had been obsoleted by the revisions, hence removed it from the distributed files.\n\nDan",
    "created_at": "2010-09-10T00:34:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68686",
    "user": "https://github.com/dwbump"
}
```

<div id="comment:31" align="right">Comment 31</div>

I reposted the two relevant patches, trac_8442-png-files.patch and trac_8442-rebased-4.5.3.patch.

I made some further changes to crystals.rst and I found that one of the png files had been obsoleted by the revisions, hence removed it from the distributed files.

Dan



---

archive/issue_comments_068687.json:
```json
{
    "body": "Thematic tutorial: Lie methods and related combinatorics",
    "created_at": "2010-09-10T23:17:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68687",
    "user": "https://github.com/dwbump"
}
```

Thematic tutorial: Lie methods and related combinatorics



---

archive/issue_comments_068688.json:
```json
{
    "body": "<div id=\"comment:32\" align=\"right\">Comment 32</div>\n\nAttachment: **[trac_8442-rebased-4.5.3.patch.gz](https://github.com/sagemath/sage/files/ticket8442/trac_8442-rebased-4.5.3.patch.gz)**\n\nI uploaded a new version with minor polishing here and there.",
    "created_at": "2010-09-10T23:18:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68688",
    "user": "https://github.com/dwbump"
}
```

<div id="comment:32" align="right">Comment 32</div>

Attachment: **[trac_8442-rebased-4.5.3.patch.gz](https://github.com/sagemath/sage/files/ticket8442/trac_8442-rebased-4.5.3.patch.gz)**

I uploaded a new version with minor polishing here and there.



---

archive/issue_comments_068689.json:
```json
{
    "body": "<div id=\"comment:33\" align=\"right\">Comment 33</div>\n\nYou may find versions of this tutorial made with both dvipng and jsmath here and here:\n\nhttp://match.stanford.edu/bump/thematic_tutorials/lie.html\nhttp://match.stanford.edu/bump/thematic_tutorials-js/lie.html",
    "created_at": "2010-09-10T23:22:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68689",
    "user": "https://github.com/dwbump"
}
```

<div id="comment:33" align="right">Comment 33</div>

You may find versions of this tutorial made with both dvipng and jsmath here and here:

http://match.stanford.edu/bump/thematic_tutorials/lie.html
http://match.stanford.edu/bump/thematic_tutorials-js/lie.html



---

archive/issue_comments_068690.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,57 +1,29 @@\n This patch adds a tutorial about Lie Methods and Related Combinatorics to Sage.\n \n-Apply the patch and also copy the files wcf1.png, a2rho.png and standard1.png into $SAGE_ROOT/devel/sage/doc/en/lie/. Then you can build the documentation with the commands:\n+The current ticket needs to be coordinated with #8470.\n \n-sage -docbuild lie html\n+A copy of the html documentation built with dvipng may be found at:\n \n-sage -docbuild lie pdf\n+http://match.stanford.edu/bump/thematic_tutorials/lie.html\n \n-Depends on #8411 and #8414 since it documents the behavior of Sage after these patches.\n+or built with jsmath:\n+\n+http://match.stanford.edu/bump/thematic_tutorials-js/lie.html\n+\n+Apply:'''\n+\n+```\n+ trac_8442-rebased-4.5.3.patch\n+ trac_8442-png-files.patch\n+```\n+\n+Then build the doc with\n+\n+```\n+sage -docbuild thematic_tutorials html\n+```\n+\n+add the `-j` flag to use jsmath.\n \n \n \n-**Notes:** The current ticket needs to be coordinated with #8470.\n-\n-A copy of the html documentation may be found at:\n-\n-http://sporadic.stanford.edu/bump/trac_8442/\n-\n-There are three patches to merge:\n-\n-```\n- trac_8442-lie-rebased.patch\n- trac_8442-reviewer.patch\n- trac_8442-weyl_groups-revision.patch\n-```\n-\n-The patches do not apply to sage-4.4.2.alpha0 because two files \n-in doc/en/thematic_tutorials are missing: index.rst and bibliography.rst.\n-An alternative patch\n-\n-```\n-trac_8442-lie-rebased-to-4.4.2.alpha0\n-```\n-\n-includes these files, and applies cleanly to sage-4.4.2.alpha0. Otherwise it is\n-identical to the union of the three patches.\n-\n-In addition, the three .png files must be where they get properly linked.\n-\n-**Apply:**\n-\n-1. #8464\n-2. [trac_8442-config.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-config.patch.gz)\n-3. [trac_8442-lie-rebased-4.4.2.rc0.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-lie-rebased-4.4.2.rc0.patch.gz)\n-4. Copy [wcf1.png](https://github.com/sagemath/sage/files/ticket8442/wcf1.png), [a2rho.png](https://github.com/sagemath/sage/files/ticket8442/a2rho.png), and [standard1.png](https://github.com/sagemath/sage/files/ticket8442/standard1.png) to the directory `doc/en/thematic_tutorials/static/`.\n-\n-**Revised instructions. Apply:**\n-\n-```\n- trac_8442-lie-rebased-4.5.2.patch\n- trac_8442-png-files.patch\n-```\n-\n-\n-\n-\n-\n``````\n",
    "created_at": "2010-09-10T23:30:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68690",
    "user": "https://github.com/dwbump"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,57 +1,29 @@
 This patch adds a tutorial about Lie Methods and Related Combinatorics to Sage.
 
-Apply the patch and also copy the files wcf1.png, a2rho.png and standard1.png into $SAGE_ROOT/devel/sage/doc/en/lie/. Then you can build the documentation with the commands:
+The current ticket needs to be coordinated with #8470.
 
-sage -docbuild lie html
+A copy of the html documentation built with dvipng may be found at:
 
-sage -docbuild lie pdf
+http://match.stanford.edu/bump/thematic_tutorials/lie.html
 
-Depends on #8411 and #8414 since it documents the behavior of Sage after these patches.
+or built with jsmath:
+
+http://match.stanford.edu/bump/thematic_tutorials-js/lie.html
+
+Apply:'''
+
+```
+ trac_8442-rebased-4.5.3.patch
+ trac_8442-png-files.patch
+```
+
+Then build the doc with
+
+```
+sage -docbuild thematic_tutorials html
+```
+
+add the `-j` flag to use jsmath.
 
 
 
-**Notes:** The current ticket needs to be coordinated with #8470.
-
-A copy of the html documentation may be found at:
-
-http://sporadic.stanford.edu/bump/trac_8442/
-
-There are three patches to merge:
-
-```
- trac_8442-lie-rebased.patch
- trac_8442-reviewer.patch
- trac_8442-weyl_groups-revision.patch
-```
-
-The patches do not apply to sage-4.4.2.alpha0 because two files 
-in doc/en/thematic_tutorials are missing: index.rst and bibliography.rst.
-An alternative patch
-
-```
-trac_8442-lie-rebased-to-4.4.2.alpha0
-```
-
-includes these files, and applies cleanly to sage-4.4.2.alpha0. Otherwise it is
-identical to the union of the three patches.
-
-In addition, the three .png files must be where they get properly linked.
-
-**Apply:**
-
-1. #8464
-2. [trac_8442-config.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-config.patch.gz)
-3. [trac_8442-lie-rebased-4.4.2.rc0.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-lie-rebased-4.4.2.rc0.patch.gz)
-4. Copy [wcf1.png](https://github.com/sagemath/sage/files/ticket8442/wcf1.png), [a2rho.png](https://github.com/sagemath/sage/files/ticket8442/a2rho.png), and [standard1.png](https://github.com/sagemath/sage/files/ticket8442/standard1.png) to the directory `doc/en/thematic_tutorials/static/`.
-
-**Revised instructions. Apply:**
-
-```
- trac_8442-lie-rebased-4.5.2.patch
- trac_8442-png-files.patch
-```
-
-
-
-
-
``````




---

archive/issue_comments_068691.json:
```json
{
    "body": "Thematic Tutorial: Lie methods and related combinatorics",
    "created_at": "2010-09-26T17:40:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68691",
    "user": "https://github.com/dwbump"
}
```

Thematic Tutorial: Lie methods and related combinatorics



---

archive/issue_comments_068692.json:
```json
{
    "body": "<div id=\"comment:35\" align=\"right\">Comment 35</div>\n\nAttachment: **[trac_8442-rebased-4.6.patch.gz](https://github.com/sagemath/sage/files/ticket8442/trac_8442-rebased-4.6.patch.gz)**\n\nWith sage-4.6.alpha1 the patch needed rebasing. I did this, then made a couple of other changes which I think are improvements. I removed the material about intertwining operators, which seems too specialized for a document like this.  I will make available this elsewhere. I added a new section called Integration showing how to compute some integrals over Lie groups.\n\nI updated the links at:\n\nhttp://match.stanford.edu/bump/thematic_tutorials/lie.html  \n\nhttp://match.stanford.edu/bump/thematic_tutorials-js/lie.html",
    "created_at": "2010-09-26T17:52:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68692",
    "user": "https://github.com/dwbump"
}
```

<div id="comment:35" align="right">Comment 35</div>

Attachment: **[trac_8442-rebased-4.6.patch.gz](https://github.com/sagemath/sage/files/ticket8442/trac_8442-rebased-4.6.patch.gz)**

With sage-4.6.alpha1 the patch needed rebasing. I did this, then made a couple of other changes which I think are improvements. I removed the material about intertwining operators, which seems too specialized for a document like this.  I will make available this elsewhere. I added a new section called Integration showing how to compute some integrals over Lie groups.

I updated the links at:

http://match.stanford.edu/bump/thematic_tutorials/lie.html  

http://match.stanford.edu/bump/thematic_tutorials-js/lie.html



---

archive/issue_comments_068693.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -13,7 +13,7 @@\n Apply:'''\n \n ```\n- trac_8442-rebased-4.5.3.patch\n+ trac_8442-rebased-4.6.patch\n  trac_8442-png-files.patch\n ```\n \n``````\n",
    "created_at": "2010-09-26T17:52:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68693",
    "user": "https://github.com/dwbump"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -13,7 +13,7 @@
 Apply:'''
 
 ```
- trac_8442-rebased-4.5.3.patch
+ trac_8442-rebased-4.6.patch
  trac_8442-png-files.patch
 ```
 
``````




---

archive/issue_events_096309.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-10-29T02:17:04Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "milestone_number": null,
    "milestone_title": "sage-4.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96309"
}
```



---

archive/issue_events_096310.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-10-29T02:17:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "milestone_number": null,
    "milestone_title": "sage-4.6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96310"
}
```



---

archive/issue_comments_068694.json:
```json
{
    "body": "<div id=\"comment:37\" align=\"right\">Comment 37</div>\n\nThis looks like an amazing document to include with Sage.  Thanks for working on it.\n\nFirst, all doctests pass for me.\n\nI get three warnings when building the docs, two of which anyone can fix:\n\n```\n/Applications/sage/devel/sage/doc/en/thematic_tutorials/lie/weyl_character_ring.rst:397: (WARNING/2) Title underline too short.\n\nIntegration\n-------\n/Applications/sage/devel/sage/doc/en/thematic_tutorials/lie/weyl_character_ring.rst:397: (WARNING/2) Title underline too short.\n\nIntegration\n-------\n```\nThe third one I think Dan has to do:\n\n```\n/Applications/sage/devel/sage/doc/en/thematic_tutorials/lie/weyl_groups.rst:: WARNING: citation not found: BumpNakasuji2010\n```\n\nA few questions and comments about the text: first, the style in the Sage documentation is to use `\\Bold{C`} for the complex numbers, and similarly for R and Q.  This is more of an issue for docstrings for methods, functions, etc., but you might consider changing it.\n\nIn introduction.rst, \n\n```\nFor example, we could take `G = SU(n)`,\n`\\mathfrak{g} = \\mathfrak{sl}(n, \\mathbb{R})`,\n`\\mathfrak{g}_{\\mathbb{C}} = \\mathfrak{sl}(n, \\mathbb{C})` and\n`G = SL(n, \\mathbb{C})`.\n```\nShould the last \"G\" be \"G_{\\mathbb{C}}\" (or \"G_{\\Bold{C}}\")?\n\nIn lie_basics.rst, on line 141: `However Parabolic subgroups do not exist for compact Lie groups.`  Don't capitalize \"Parabolic\".\n\nOn line 150: \n\n```\nSuch a type is implemented in Sage as a pair ``[`X`,r]``\n```  \nI would use plain quotes around the X, not backquotes: <code>\\`\\`['X',r]\\`\\`</code>\n\nA few lines later:\n\n```\nThe exceptional types are::\n\n    ['G',2],  ['F',4],  ['E',6], ['E',7] or ['E',8].\n```\nThe indented block is verbatim text, so spacing matters.  The inconsistency between double-spacing and single-spacing looks odd to me, and I would just put single spaces after all of the commas.\n\nweyl_character_ring.rst, line 239, <code>with Cartan type \\`['B',r]\\`</code>: I would use double back quotes here instead of single ones.",
    "created_at": "2010-11-02T20:49:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68694",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:37" align="right">Comment 37</div>

This looks like an amazing document to include with Sage.  Thanks for working on it.

First, all doctests pass for me.

I get three warnings when building the docs, two of which anyone can fix:

```
/Applications/sage/devel/sage/doc/en/thematic_tutorials/lie/weyl_character_ring.rst:397: (WARNING/2) Title underline too short.

Integration
-------
/Applications/sage/devel/sage/doc/en/thematic_tutorials/lie/weyl_character_ring.rst:397: (WARNING/2) Title underline too short.

Integration
-------
```
The third one I think Dan has to do:

```
/Applications/sage/devel/sage/doc/en/thematic_tutorials/lie/weyl_groups.rst:: WARNING: citation not found: BumpNakasuji2010
```

A few questions and comments about the text: first, the style in the Sage documentation is to use `\Bold{C`} for the complex numbers, and similarly for R and Q.  This is more of an issue for docstrings for methods, functions, etc., but you might consider changing it.

In introduction.rst, 

```
For example, we could take `G = SU(n)`,
`\mathfrak{g} = \mathfrak{sl}(n, \mathbb{R})`,
`\mathfrak{g}_{\mathbb{C}} = \mathfrak{sl}(n, \mathbb{C})` and
`G = SL(n, \mathbb{C})`.
```
Should the last "G" be "G_{\mathbb{C}}" (or "G_{\Bold{C}}")?

In lie_basics.rst, on line 141: `However Parabolic subgroups do not exist for compact Lie groups.`  Don't capitalize "Parabolic".

On line 150: 

```
Such a type is implemented in Sage as a pair ``[`X`,r]``
```  
I would use plain quotes around the X, not backquotes: <code>\`\`['X',r]\`\`</code>

A few lines later:

```
The exceptional types are::

    ['G',2],  ['F',4],  ['E',6], ['E',7] or ['E',8].
```
The indented block is verbatim text, so spacing matters.  The inconsistency between double-spacing and single-spacing looks odd to me, and I would just put single spaces after all of the commas.

weyl_character_ring.rst, line 239, <code>with Cartan type \`['B',r]\`</code>: I would use double back quotes here instead of single ones.



---

archive/issue_events_096311.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-11-02T20:49:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96311"
}
```



---

archive/issue_events_096312.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-11-02T20:49:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96312"
}
```



---

archive/issue_comments_068695.json:
```json
{
    "body": "#8442: Thematic tutorial on Lie methods and related combinatorics",
    "created_at": "2010-11-02T23:57:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68695",
    "user": "https://github.com/dwbump"
}
```

#8442: Thematic tutorial on Lie methods and related combinatorics



---

archive/issue_comments_068696.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -10,10 +10,12 @@\n \n http://match.stanford.edu/bump/thematic_tutorials-js/lie.html\n \n+Note: only the second js version is up to date. I will update the other one later.\n+\n Apply:'''\n \n ```\n- trac_8442-rebased-4.6.patch\n+ trac_8442-rebased-4.6.1.patch\n  trac_8442-png-files.patch\n ```\n \n``````\n",
    "created_at": "2010-11-03T00:00:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68696",
    "user": "https://github.com/dwbump"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -10,10 +10,12 @@
 
 http://match.stanford.edu/bump/thematic_tutorials-js/lie.html
 
+Note: only the second js version is up to date. I will update the other one later.
+
 Apply:'''
 
 ```
- trac_8442-rebased-4.6.patch
+ trac_8442-rebased-4.6.1.patch
  trac_8442-png-files.patch
 ```
 
``````




---

archive/issue_comments_068697.json:
```json
{
    "body": "<div id=\"comment:38\" align=\"right\">Comment 38</div>\n\nAttachment: **[trac_8442-rebased-4.6.1.patch.gz](https://github.com/sagemath/sage/files/ticket8442/trac_8442-rebased-4.6.1.patch.gz)**",
    "created_at": "2010-11-03T00:00:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68697",
    "user": "https://github.com/dwbump"
}
```

<div id="comment:38" align="right">Comment 38</div>

Attachment: **[trac_8442-rebased-4.6.1.patch.gz](https://github.com/sagemath/sage/files/ticket8442/trac_8442-rebased-4.6.1.patch.gz)**



---

archive/issue_events_096313.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-11-03T00:00:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96313"
}
```



---

archive/issue_events_096314.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-11-03T00:00:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96314"
}
```



---

archive/issue_comments_068698.json:
```json
{
    "body": "<div id=\"comment:39\" align=\"right\">Comment 39</div>\n\nAll jhpalmieri's comments are good. I have updated the patch and reposted it as trac_8442-rebased-4.6.1.patch.",
    "created_at": "2010-11-03T00:02:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68698",
    "user": "https://github.com/dwbump"
}
```

<div id="comment:39" align="right">Comment 39</div>

All jhpalmieri's comments are good. I have updated the patch and reposted it as trac_8442-rebased-4.6.1.patch.



---

archive/issue_comments_068699.json:
```json
{
    "body": "<div id=\"comment:40\" align=\"right\">Comment 40</div>\n\nI'm attaching a patch to fix the spacing in one table in lie_basics.rst.  All tests pass, and with the extra patch, the docs build in both html and pdf formats.\n\nI'm not a Lie algebra expert, nor an expert in Sage's implementation of Lie algebras.  Can someone else take a look at this and (I hope) positively review it soon)?  It would be nice to get this merged.",
    "created_at": "2010-11-06T22:11:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68699",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:40" align="right">Comment 40</div>

I'm attaching a patch to fix the spacing in one table in lie_basics.rst.  All tests pass, and with the extra patch, the docs build in both html and pdf formats.

I'm not a Lie algebra expert, nor an expert in Sage's implementation of Lie algebras.  Can someone else take a look at this and (I hope) positively review it soon)?  It would be nice to get this merged.



---

archive/issue_comments_068700.json:
```json
{
    "body": "apply on top of other patches",
    "created_at": "2010-11-06T22:11:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68700",
    "user": "https://github.com/jhpalmieri"
}
```

apply on top of other patches



---

archive/issue_comments_068701.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -17,6 +17,7 @@\n ```\n  trac_8442-rebased-4.6.1.patch\n  trac_8442-png-files.patch\n+ trac_8442-ref-spacing.patch\n ```\n \n Then build the doc with\n``````\n",
    "created_at": "2010-11-06T22:12:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68701",
    "user": "https://github.com/jhpalmieri"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -17,6 +17,7 @@
 ```
  trac_8442-rebased-4.6.1.patch
  trac_8442-png-files.patch
+ trac_8442-ref-spacing.patch
 ```
 
 Then build the doc with
``````




---

archive/issue_comments_068702.json:
```json
{
    "body": "<div id=\"comment:41\" align=\"right\">Comment 41</div>\n\nAttachment: **[trac_8442-ref-spacing.patch.gz](https://github.com/sagemath/sage/files/ticket8442/trac_8442-ref-spacing.patch.gz)**",
    "created_at": "2010-11-06T22:12:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68702",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:41" align="right">Comment 41</div>

Attachment: **[trac_8442-ref-spacing.patch.gz](https://github.com/sagemath/sage/files/ticket8442/trac_8442-ref-spacing.patch.gz)**



---

archive/issue_comments_068703.json:
```json
{
    "body": "<div id=\"comment:42\" align=\"right\">Comment 42</div>\n\nIt appears that the buildbot is trying to apply all 17 patches.\n\nhttp://sage.math.washington.edu:21100/ticket/8442/\n\nMost of these are superceded, though only admin can actually remove them. As far as I know the three patches\n\n```\n trac_8442-rebased-4.6.1.patch\n trac_8442-png-files.patch\n trac_8442-ref-spacing.patch\n```\n\nshould apply cleanly.",
    "created_at": "2010-12-09T13:48:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68703",
    "user": "https://github.com/dwbump"
}
```

<div id="comment:42" align="right">Comment 42</div>

It appears that the buildbot is trying to apply all 17 patches.

http://sage.math.washington.edu:21100/ticket/8442/

Most of these are superceded, though only admin can actually remove them. As far as I know the three patches

```
 trac_8442-rebased-4.6.1.patch
 trac_8442-png-files.patch
 trac_8442-ref-spacing.patch
```

should apply cleanly.



---

archive/issue_comments_068704.json:
```json
{
    "body": "<div id=\"comment:43\" align=\"right\">Comment 43</div>\n\nTo the buildbot: apply\n\n- trac_8442-rebased-4.6.1.patch\n- trac_8442-png-files.patch\n- trac_8442-ref-spacing.patch",
    "created_at": "2010-12-15T18:45:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68704",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:43" align="right">Comment 43</div>

To the buildbot: apply

- trac_8442-rebased-4.6.1.patch
- trac_8442-png-files.patch
- trac_8442-ref-spacing.patch



---

archive/issue_events_096315.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2010-12-16T08:23:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96315"
}
```



---

archive/issue_events_096316.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2010-12-16T08:23:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96316"
}
```



---

archive/issue_comments_068705.json:
```json
{
    "body": "<div id=\"comment:44\" align=\"right\">Comment 44</div>\n\nI'd like to give it a positive review, but I still might like to \nknow more about few related things:\n\nOne is about a way to connecting Lie functionality in GAP to the one in Sage. Anything on this?\n\nIt would also be good if anything is said regarding the optional Sage package lie (by Marc van Leeuween). Is it right that basically anything doable in lie can be done in Sage?\nIn particular, lie can compute decompositions of, say, a tensor product of two representations into irreducibles. It's not clear to me whether one can do this in Sage (without lie).\n\nDmitrii",
    "created_at": "2010-12-16T08:23:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68705",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:44" align="right">Comment 44</div>

I'd like to give it a positive review, but I still might like to 
know more about few related things:

One is about a way to connecting Lie functionality in GAP to the one in Sage. Anything on this?

It would also be good if anything is said regarding the optional Sage package lie (by Marc van Leeuween). Is it right that basically anything doable in lie can be done in Sage?
In particular, lie can compute decompositions of, say, a tensor product of two representations into irreducibles. It's not clear to me whether one can do this in Sage (without lie).

Dmitrii



---

archive/issue_comments_068706.json:
```json
{
    "body": "<div id=\"comment:45\" align=\"right\">Comment 45</div>\n\n> One is about a way to connecting Lie functionality in GAP to the one in Sage. Anything on this?\n\nOne issue with GAP on Sage is that the interface needs a lot of work. There is a lot of power in GAP that can't be accessed from Sage because of this.\n\nThe Lie theory in Sage is mostly written from scratch. But here is an example where GAP is\ninvolved in the background. We have a class for WeylGroups in weyl_group.py. This\nclass inherits from MatrixGroup_gens which in turn inherits from MatrixGroup_gap. So\nGAP is involved in Weyl Groups.\n\n> It would also be good if anything is said regarding the optional Sage package lie (by Marc van Leeuween). Is it right that basically anything doable in lie can be done in Sage? In particular, lie can compute decompositions of, say, a tensor product of two representations into irreducibles. It's not clear to me whether one can do this in Sage (without lie).\n\nI am not sure whether everything that is doable with LiE is doable with Sage but I do think that anything that is needed from LiE is either in Sage already or (if needed) should be reimplemented. What is in Sage is a pretty complete toolkit for finite-dimensional representations of Lie groups. Decomposing a tensor product into irreducibles is just the multiplication in the WeylCharacterRing. This is addressed in the tutorial. See:\n\nhttp://match.stanford.edu/bump/thematic_tutorials-js/lie/weyl_character_ring.html#tensor-products-of-representations\n\nHave a look also at the branching rules.\n\nhttp://match.stanford.edu/bump/thematic_tutorials-js/lie/branching_rules.html\n\nLiE has some functionality for working with Kazhdan-Lusztig polynomials, but that is in Sage, as fast as LiE (though not as fast as Coxeter3). LiE has alternate methods of computing Weyl Characters including use of Demazure characters. Some version of the Demazure character is in the crystal code, but it would also be easy and perhaps useful to add a method to the WeightRing. But it is not urgently needed. Sage uses the Freudenthal multiplicity formula to compute the character.",
    "created_at": "2010-12-16T13:31:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68706",
    "user": "https://github.com/dwbump"
}
```

<div id="comment:45" align="right">Comment 45</div>

> One is about a way to connecting Lie functionality in GAP to the one in Sage. Anything on this?

One issue with GAP on Sage is that the interface needs a lot of work. There is a lot of power in GAP that can't be accessed from Sage because of this.

The Lie theory in Sage is mostly written from scratch. But here is an example where GAP is
involved in the background. We have a class for WeylGroups in weyl_group.py. This
class inherits from MatrixGroup_gens which in turn inherits from MatrixGroup_gap. So
GAP is involved in Weyl Groups.

> It would also be good if anything is said regarding the optional Sage package lie (by Marc van Leeuween). Is it right that basically anything doable in lie can be done in Sage? In particular, lie can compute decompositions of, say, a tensor product of two representations into irreducibles. It's not clear to me whether one can do this in Sage (without lie).

I am not sure whether everything that is doable with LiE is doable with Sage but I do think that anything that is needed from LiE is either in Sage already or (if needed) should be reimplemented. What is in Sage is a pretty complete toolkit for finite-dimensional representations of Lie groups. Decomposing a tensor product into irreducibles is just the multiplication in the WeylCharacterRing. This is addressed in the tutorial. See:

http://match.stanford.edu/bump/thematic_tutorials-js/lie/weyl_character_ring.html#tensor-products-of-representations

Have a look also at the branching rules.

http://match.stanford.edu/bump/thematic_tutorials-js/lie/branching_rules.html

LiE has some functionality for working with Kazhdan-Lusztig polynomials, but that is in Sage, as fast as LiE (though not as fast as Coxeter3). LiE has alternate methods of computing Weyl Characters including use of Demazure characters. Some version of the Demazure character is in the crystal code, but it would also be easy and perhaps useful to add a method to the WeightRing. But it is not urgently needed. Sage uses the Freudenthal multiplicity formula to compute the character.



---

archive/issue_comments_068707.json:
```json
{
    "body": "<div id=\"comment:46\" align=\"right\">Comment 46</div>\n\nReplying to [@dwbump](#comment%3A45):\n\n\n> > It would also be good if anything is said regarding the optional Sage package lie (by Marc van Leeuween). Is it right that basically anything doable in lie can be done in Sage? In particular, lie can compute decompositions of, say, a tensor product of two representations into irreducibles. It's not clear to me whether one can do this in Sage (without lie).\n\n> \n> I am not sure whether everything that is doable with LiE is doable with Sage but I do think that anything that is needed from LiE is either in Sage already or (if needed) should be reimplemented. What is in Sage is a pretty complete toolkit for finite-dimensional representations of Lie groups. Decomposing a tensor product into irreducibles is just the multiplication in the WeylCharacterRing. This is addressed in the tutorial. See:\n> \n> http://match.stanford.edu/bump/thematic_tutorials-js/lie/weyl_character_ring.html#tensor-products-of-representations\n\n\nOne particular thing I was able to do using Lie was to compute things in classical invariant theory, such as the dimension of the space of invariants of degree k\nof the m-ary form of degree d (for fixed k,m,d). Basically, that meant computing certain symmetric power of certain representation of GL_m (or SL_m), and finding out whether there was a 1-dimensional sub-representation.\n\nIs this doable in Sage?\nThanks!",
    "created_at": "2010-12-16T14:10:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68707",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:46" align="right">Comment 46</div>

Replying to [@dwbump](#comment%3A45):


> > It would also be good if anything is said regarding the optional Sage package lie (by Marc van Leeuween). Is it right that basically anything doable in lie can be done in Sage? In particular, lie can compute decompositions of, say, a tensor product of two representations into irreducibles. It's not clear to me whether one can do this in Sage (without lie).

> 
> I am not sure whether everything that is doable with LiE is doable with Sage but I do think that anything that is needed from LiE is either in Sage already or (if needed) should be reimplemented. What is in Sage is a pretty complete toolkit for finite-dimensional representations of Lie groups. Decomposing a tensor product into irreducibles is just the multiplication in the WeylCharacterRing. This is addressed in the tutorial. See:
> 
> http://match.stanford.edu/bump/thematic_tutorials-js/lie/weyl_character_ring.html#tensor-products-of-representations


One particular thing I was able to do using Lie was to compute things in classical invariant theory, such as the dimension of the space of invariants of degree k
of the m-ary form of degree d (for fixed k,m,d). Basically, that meant computing certain symmetric power of certain representation of GL_m (or SL_m), and finding out whether there was a 1-dimensional sub-representation.

Is this doable in Sage?
Thanks!



---

archive/issue_comments_068708.json:
```json
{
    "body": "<div id=\"comment:48\" align=\"right\">Comment 48</div>\n\n> Basically, that meant computing certain symmetric power of certain representation of GL_m (or SL_m), and finding out whether there was a 1-dimensional sub-representation. \n\nFor symmetric (or exterior) square you can use the frobenius_schur_indicator method. For higher symmetric powers, here is a way. Suppose we want to compute the symmetric 5-th power of the 8-dimensional adjoint representation of SL(3). The relevant groups are SL(3) and SL(8), so we need Weyl Character rings A2 and A7.\n\n```\nsage: A2=WeylCharacterRing(\"A2\",style=\"coroots\")\nsage: A7=WeylCharacterRing(\"A7\",style=\"coroots\",cache=\"true\")\nsage: s = A7.fundamental_weights()[1]\nsage: A7(5*s)\nA7(5,0,0,0,0,0,0)\nsage: A7(5*s)\nA7(5,0,0,0,0,0,0)\nsage: A7(5*s).degree()\n792\n```\n\nThis is the symmetric 5-th power of the standard representation of SL(8), which we want to branch down to SL(3) along the adjoint representation, which is a homomorphism SL(3) --> SL(8). So we create the adjoint representation, then branch the symmetric 5-th power representation of SL(8) down to SL(3).\n\n```\nsage: ad=A2(1,1); ad.degree()\n8\nsage: A7(5*s).branch(A2,rule=branching_rule_from_plethysm(ad,\"A7\"))\nA2(0,0) + 2*A2(1,1) + A2(0,3) + A2(3,0) + 2*A2(2,2) + A2(1,4) + A2(4,1) + 2*A2(3,3) + A2(2,5) + A2(5,2) + A2(4,4) + A2(5,5)\n```\n\nThere is your decomposition into irreducibles. You can see that there is a copy of the trivial representation.",
    "created_at": "2010-12-16T16:21:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68708",
    "user": "https://github.com/dwbump"
}
```

<div id="comment:48" align="right">Comment 48</div>

> Basically, that meant computing certain symmetric power of certain representation of GL_m (or SL_m), and finding out whether there was a 1-dimensional sub-representation. 

For symmetric (or exterior) square you can use the frobenius_schur_indicator method. For higher symmetric powers, here is a way. Suppose we want to compute the symmetric 5-th power of the 8-dimensional adjoint representation of SL(3). The relevant groups are SL(3) and SL(8), so we need Weyl Character rings A2 and A7.

```
sage: A2=WeylCharacterRing("A2",style="coroots")
sage: A7=WeylCharacterRing("A7",style="coroots",cache="true")
sage: s = A7.fundamental_weights()[1]
sage: A7(5*s)
A7(5,0,0,0,0,0,0)
sage: A7(5*s)
A7(5,0,0,0,0,0,0)
sage: A7(5*s).degree()
792
```

This is the symmetric 5-th power of the standard representation of SL(8), which we want to branch down to SL(3) along the adjoint representation, which is a homomorphism SL(3) --> SL(8). So we create the adjoint representation, then branch the symmetric 5-th power representation of SL(8) down to SL(3).

```
sage: ad=A2(1,1); ad.degree()
8
sage: A7(5*s).branch(A2,rule=branching_rule_from_plethysm(ad,"A7"))
A2(0,0) + 2*A2(1,1) + A2(0,3) + A2(3,0) + 2*A2(2,2) + A2(1,4) + A2(4,1) + 2*A2(3,3) + A2(2,5) + A2(5,2) + A2(4,4) + A2(5,5)
```

There is your decomposition into irreducibles. You can see that there is a copy of the trivial representation.



---

archive/issue_events_096317.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-12-16T16:31:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96317"
}
```



---

archive/issue_events_096318.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-12-16T16:31:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96318"
}
```



---

archive/issue_events_096319.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2010-12-18T03:21:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96319"
}
```



---

archive/issue_events_096320.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2010-12-18T03:21:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96320"
}
```



---

archive/issue_comments_068709.json:
```json
{
    "body": "<div id=\"comment:50\" align=\"right\">Comment 50</div>\n\nReplying to [@dwbump](#comment%3A49):\nVery nice! Positive review. \n(IMHO it would be nice to have your last example included in the tutorial, but I'd leave it to you to decide whether to do this.)\n\nDmitrii",
    "created_at": "2010-12-18T03:21:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68709",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:50" align="right">Comment 50</div>

Replying to [@dwbump](#comment%3A49):
Very nice! Positive review. 
(IMHO it would be nice to have your last example included in the tutorial, but I'd leave it to you to decide whether to do this.)

Dmitrii



---

archive/issue_comments_068710.json:
```json
{
    "body": "<div id=\"comment:51\" align=\"right\">Comment 51</div>\n\n> Replying to bump: Very nice! Positive review. (IMHO it would be nice to have your last example included in the tutorial, but I'd leave it to you to decide whether to do this.)\n\nI will make a patch for this. Perhaps it should be a separate ticket so as not to further delay this one.",
    "created_at": "2010-12-18T13:22:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68710",
    "user": "https://github.com/dwbump"
}
```

<div id="comment:51" align="right">Comment 51</div>

> Replying to bump: Very nice! Positive review. (IMHO it would be nice to have your last example included in the tutorial, but I'd leave it to you to decide whether to do this.)

I will make a patch for this. Perhaps it should be a separate ticket so as not to further delay this one.



---

archive/issue_events_096321.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-12-19T12:52:26Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "milestone_number": null,
    "milestone_title": "sage-4.6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96321"
}
```



---

archive/issue_events_096322.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2010-12-19T12:52:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "milestone_number": null,
    "milestone_title": "sage-4.6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96322"
}
```



---

archive/issue_comments_068711.json:
```json
{
    "body": "Changed reviewer from **Minh Van Nguyen, Mark Jordan** to **Minh Van Nguyen, Mark Jordan, Dmitrii Pasechnik**",
    "created_at": "2010-12-19T16:11:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68711",
    "user": "https://github.com/dimpase"
}
```

Changed reviewer from **Minh Van Nguyen, Mark Jordan** to **Minh Van Nguyen, Mark Jordan, Dmitrii Pasechnik**



---

archive/issue_comments_068712.json:
```json
{
    "body": "<div id=\"comment:54\" align=\"right\">Comment 54</div>\n\nI just noticed that the trac_8442-png-files containing the .png files is not\nthe right patch!\n\nThe files are in the wrong directory (should be doc/en/thematic_tutorials/media)\nand some of the images are missing. I am puzzled as to how this happened since\neverything was built and tested.\n\nI am changing the status back to needs_work until I can fix this.",
    "created_at": "2010-12-21T01:15:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68712",
    "user": "https://github.com/dwbump"
}
```

<div id="comment:54" align="right">Comment 54</div>

I just noticed that the trac_8442-png-files containing the .png files is not
the right patch!

The files are in the wrong directory (should be doc/en/thematic_tutorials/media)
and some of the images are missing. I am puzzled as to how this happened since
everything was built and tested.

I am changing the status back to needs_work until I can fix this.



---

archive/issue_events_096323.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-12-21T01:15:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96323"
}
```



---

archive/issue_events_096324.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-12-21T01:15:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96324"
}
```



---

archive/issue_comments_068713.json:
```json
{
    "body": "<div id=\"comment:55\" align=\"right\">Comment 55</div>\n\nI replaced the file trac_8442-png-files.patch, and am restoring the status to positive_review.\n\nThe correct file is 52266 bytes.",
    "created_at": "2010-12-21T01:26:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68713",
    "user": "https://github.com/dwbump"
}
```

<div id="comment:55" align="right">Comment 55</div>

I replaced the file trac_8442-png-files.patch, and am restoring the status to positive_review.

The correct file is 52266 bytes.



---

archive/issue_events_096325.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-12-21T01:26:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96325"
}
```



---

archive/issue_events_096326.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2010-12-21T01:26:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96326"
}
```



---

archive/issue_comments_068714.json:
```json
{
    "body": "<div id=\"comment:56\" align=\"right\">Comment 56</div>\n\nSetting milestone to sage-feature because of #8470.",
    "created_at": "2011-01-13T05:46:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68714",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:56" align="right">Comment 56</div>

Setting milestone to sage-feature because of #8470.



---

archive/issue_events_096327.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-13T05:46:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "milestone_number": null,
    "milestone_title": "sage-4.6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96327"
}
```



---

archive/issue_events_096328.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-13T05:46:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/feature",
    "label_color": "008080",
    "label_name": "feature",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96328"
}
```



---

archive/issue_comments_068715.json:
```json
{
    "body": "<div id=\"comment:57\" align=\"right\">Comment 57</div>\n\n> Setting milestone to sage-feature because of #8470.\n\nAt some point Minh Nguyen added the words \"needs to be coordinated with #8470\" to the description. However #8470 has not been touched for 8 months. It appears to me that #8470 is an orphaned ticket.\n\nI propose that the milestone for this patch should be changed back to 4.6.2.",
    "created_at": "2011-01-13T13:59:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68715",
    "user": "https://github.com/dwbump"
}
```

<div id="comment:57" align="right">Comment 57</div>

> Setting milestone to sage-feature because of #8470.

At some point Minh Nguyen added the words "needs to be coordinated with #8470" to the description. However #8470 has not been touched for 8 months. It appears to me that #8470 is an orphaned ticket.

I propose that the milestone for this patch should be changed back to 4.6.2.



---

archive/issue_comments_068716.json:
```json
{
    "body": "<div id=\"comment:58\" align=\"right\">Comment 58</div>\n\nReplying to [@dwbump](#comment%3A57):\n> > Setting milestone to sage-feature because of #8470.\n\n> \n> At some point Minh Nguyen added the words \"needs to be coordinated with #8470\" to the description. However #8470 has not been touched for 8 months. It appears to me that #8470 is an orphaned ticket.\n> \n> I propose that the milestone for this patch should be changed back to 4.6.2.\n> \n> \n\nI second this. #8470 is a meta-ticket meant for coordinating documentation. Closing the current ticket is a prereq to closing it. So, please close the current ticket, which in no ways depends on #8470.",
    "created_at": "2011-01-13T17:08:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68716",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:58" align="right">Comment 58</div>

Replying to [@dwbump](#comment%3A57):
> > Setting milestone to sage-feature because of #8470.

> 
> At some point Minh Nguyen added the words "needs to be coordinated with #8470" to the description. However #8470 has not been touched for 8 months. It appears to me that #8470 is an orphaned ticket.
> 
> I propose that the milestone for this patch should be changed back to 4.6.2.
> 
> 

I second this. #8470 is a meta-ticket meant for coordinating documentation. Closing the current ticket is a prereq to closing it. So, please close the current ticket, which in no ways depends on #8470.



---

archive/issue_events_096329.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2011-01-13T17:17:07Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "milestone_number": null,
    "milestone_title": "sage-4.6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96329"
}
```



---

archive/issue_events_096330.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2011-01-13T17:17:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/feature",
    "label_color": "008080",
    "label_name": "feature",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96330"
}
```



---

archive/issue_comments_068717.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,6 +1,4 @@\n This patch adds a tutorial about Lie Methods and Related Combinatorics to Sage.\n-\n-The current ticket needs to be coordinated with #8470.\n \n A copy of the html documentation built with dvipng may be found at:\n \n@@ -28,5 +26,4 @@\n \n add the `-j` flag to use jsmath.\n \n-\n-\n+See also #8470.\n``````\n",
    "created_at": "2011-01-13T18:42:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68717",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,6 +1,4 @@
 This patch adds a tutorial about Lie Methods and Related Combinatorics to Sage.
-
-The current ticket needs to be coordinated with #8470.
 
 A copy of the html documentation built with dvipng may be found at:
 
@@ -28,5 +26,4 @@
 
 add the `-j` flag to use jsmath.
 
-
-
+See also #8470.
``````




---

archive/issue_comments_068718.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -10,13 +10,11 @@\n \n Note: only the second js version is up to date. I will update the other one later.\n \n-Apply:'''\n+Apply:\n \n-```\n- trac_8442-rebased-4.6.1.patch\n- trac_8442-png-files.patch\n- trac_8442-ref-spacing.patch\n-```\n+1. [attachment: trac_8442-rebased-4.6.1.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-rebased-4.6.1.patch.gz)\n+2. [attachment: trac_8442-png-files.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-png-files.patch.gz)\n+3. [attachment: trac_8442-ref-spacing.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-ref-spacing.patch.gz)\n \n Then build the doc with\n \n``````\n",
    "created_at": "2011-01-22T19:47:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68718",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -10,13 +10,11 @@
 
 Note: only the second js version is up to date. I will update the other one later.
 
-Apply:'''
+Apply:
 
-```
- trac_8442-rebased-4.6.1.patch
- trac_8442-png-files.patch
- trac_8442-ref-spacing.patch
-```
+1. [attachment: trac_8442-rebased-4.6.1.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-rebased-4.6.1.patch.gz)
+2. [attachment: trac_8442-png-files.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-png-files.patch.gz)
+3. [attachment: trac_8442-ref-spacing.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-ref-spacing.patch.gz)
 
 Then build the doc with
 
``````




---

archive/issue_comments_068719.json:
```json
{
    "body": "<div id=\"comment:62\" align=\"right\">Comment 62</div>\n\nPlease add ticket number to commit message of [attachment: trac_8442-png-files.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-png-files.patch.gz)",
    "created_at": "2011-01-22T20:18:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68719",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:62" align="right">Comment 62</div>

Please add ticket number to commit message of [attachment: trac_8442-png-files.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-png-files.patch.gz)



---

archive/issue_events_096331.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-22T20:18:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96331"
}
```



---

archive/issue_events_096332.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-22T20:18:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96332"
}
```



---

archive/issue_comments_068720.json:
```json
{
    "body": "Attachment: **[trac_8442-png-files.patch.gz](https://github.com/sagemath/sage/files/ticket8442/trac_8442-png-files.patch.gz)**\n\n8442: png files",
    "created_at": "2011-01-23T19:21:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68720",
    "user": "https://github.com/dwbump"
}
```

Attachment: **[trac_8442-png-files.patch.gz](https://github.com/sagemath/sage/files/ticket8442/trac_8442-png-files.patch.gz)**

8442: png files



---

archive/issue_events_096333.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2011-01-23T19:25:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96333"
}
```



---

archive/issue_events_096334.json:
```json
{
    "actor": "https://github.com/dwbump",
    "created_at": "2011-01-23T19:25:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96334"
}
```



---

archive/issue_comments_068721.json:
```json
{
    "body": "<div id=\"comment:63\" align=\"right\">Comment 63</div>\n\n> Please add ticket number to commit message of trac_8442-png-files.patch\n\nDone.",
    "created_at": "2011-01-23T19:25:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68721",
    "user": "https://github.com/dwbump"
}
```

<div id="comment:63" align="right">Comment 63</div>

> Please add ticket number to commit message of trac_8442-png-files.patch

Done.



---

archive/issue_comments_068722.json:
```json
{
    "body": "Attachment: **[8442_manifest.patch.gz](https://github.com/sagemath/sage/files/ticket8442/8442_manifest.patch.gz)**",
    "created_at": "2011-01-27T13:57:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68722",
    "user": "https://github.com/jdemeyer"
}
```

Attachment: **[8442_manifest.patch.gz](https://github.com/sagemath/sage/files/ticket8442/8442_manifest.patch.gz)**



---

archive/issue_events_096335.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-27T13:58:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96335"
}
```



---

archive/issue_events_096336.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-27T13:58:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96336"
}
```



---

archive/issue_comments_068723.json:
```json
{
    "body": "<div id=\"comment:64\" align=\"right\">Comment 64</div>\n\nAdded [attachment: 8442_manifest.patch](https://github.com/sagemath/sage/files/ticket8442/8442_manifest.patch.gz), still needs testing.",
    "created_at": "2011-01-27T13:58:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68723",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:64" align="right">Comment 64</div>

Added [attachment: 8442_manifest.patch](https://github.com/sagemath/sage/files/ticket8442/8442_manifest.patch.gz), still needs testing.



---

archive/issue_comments_068724.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -15,6 +15,7 @@\n 1. [attachment: trac_8442-rebased-4.6.1.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-rebased-4.6.1.patch.gz)\n 2. [attachment: trac_8442-png-files.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-png-files.patch.gz)\n 3. [attachment: trac_8442-ref-spacing.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-ref-spacing.patch.gz)\n+4. [attachment: 8442_manifest.patch](https://github.com/sagemath/sage/files/ticket8442/8442_manifest.patch.gz)\n \n Then build the doc with\n \n``````\n",
    "created_at": "2011-01-27T15:04:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68724",
    "user": "https://github.com/dimpase"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -15,6 +15,7 @@
 1. [attachment: trac_8442-rebased-4.6.1.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-rebased-4.6.1.patch.gz)
 2. [attachment: trac_8442-png-files.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-png-files.patch.gz)
 3. [attachment: trac_8442-ref-spacing.patch](https://github.com/sagemath/sage/files/ticket8442/trac_8442-ref-spacing.patch.gz)
+4. [attachment: 8442_manifest.patch](https://github.com/sagemath/sage/files/ticket8442/8442_manifest.patch.gz)
 
 Then build the doc with
 
``````




---

archive/issue_events_096337.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2011-01-27T15:04:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96337"
}
```



---

archive/issue_events_096338.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2011-01-27T15:04:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96338"
}
```



---

archive/issue_comments_068725.json:
```json
{
    "body": "<div id=\"comment:65\" align=\"right\">Comment 65</div>\n\nReplying to [@jdemeyer](#comment%3A64):\n> Added [attachment: 8442_manifest.patch](https://github.com/sagemath/sage/files/ticket8442/8442_manifest.patch.gz), still needs testing.\n\njust tested, it works. I also edited the description to mention this patch too.",
    "created_at": "2011-01-27T15:04:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68725",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:65" align="right">Comment 65</div>

Replying to [@jdemeyer](#comment%3A64):
> Added [attachment: 8442_manifest.patch](https://github.com/sagemath/sage/files/ticket8442/8442_manifest.patch.gz), still needs testing.

just tested, it works. I also edited the description to mention this patch too.



---

archive/issue_events_096339.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2011-01-27T15:05:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96339"
}
```



---

archive/issue_events_096340.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2011-01-27T15:05:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96340"
}
```



---

archive/issue_events_096341.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-28T08:48:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96341"
}
```



---

archive/issue_events_096342.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-28T08:48:05Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8442#event-96342"
}
```



---

archive/issue_comments_068726.json:
```json
{
    "body": "Merged: **sage-4.6.2.alpha3**",
    "created_at": "2011-01-28T08:48:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8442#issuecomment-68726",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.6.2.alpha3**
