# Issue 8094: shortcuts properties for matrix transpose, complex conjugate, conjugate transpose, and inverse

archive/issues_008094.json:
```json
{
    "assignees": [],
    "body": "The aim is to enhance the `/sage/matrix/matrix2.pyx` file with cython's properties to call `self.transpose()` and other methods more quickly. A shortcut like` self.T` is resolved to `self.transpose()`. Shortcuts should be similar to other environments like `numpy`.  Shortcuts added are:\n\n* m.T -> m.transpose()\n* m.I -> m.inverse()\n* m.C -> m.conjugate()\n* m.H -> m.conjugate().transpose()\n\nNotes: [python data model](http://docs.python.org/reference/datamodel.html#customizing-attribute-access), [Cython's properties](http://docs.cython.org/src/userguide/extension_types.html#properties)\n\n---\n\n**Apply**:\n\n1. [attachment: 8094-shortcut-matrix-transpose-v2-rebased_on_4.7.2.alpha3.patch.gz](https://github.com/sagemath/sage/files/ticket8094/8094-shortcut-matrix-transpose-v2-rebased_on_4.7.2.alpha3.patch.gz)\n2. [attachment: trac-8094_matrix_shortcut_docs_reviewer2.patch.gz](https://github.com/sagemath/sage/files/ticket8094/trac-8094_matrix_shortcut_docs_reviewer2.patch.gz)\n3. [attachment: trac_8094-matrix-properties-more-documentation-rebased_on_4.7.2.alpha3.patch.gz](https://github.com/sagemath/sage/files/ticket8094/trac_8094-matrix-properties-more-documentation-rebased_on_4.7.2.alpha3.patch.gz)\n\n\n\n\nAssignee: **@williamstein**\n\nCC:  @robertwb @rbeezer\n\nKeywords: **sd32**\n\nAuthor: **Harald Schilly, Jason Grout, Martin Raum**\n\nReviewer: **Rob Beezer, Martin Raum**\n\nMerged: **sage-4.7.2.alpha3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8094_\n\n",
    "closed_at": "2011-09-18T09:37:06Z",
    "created_at": "2010-01-27T13:00:23Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "shortcuts properties for matrix transpose, complex conjugate, conjugate transpose, and inverse",
    "type": "issue",
    "updated_at": "2011-09-18T09:37:06Z",
    "url": "https://github.com/sagemath/sage/issues/8094",
    "user": "https://github.com/haraldschilly"
}
```
The aim is to enhance the `/sage/matrix/matrix2.pyx` file with cython's properties to call `self.transpose()` and other methods more quickly. A shortcut like` self.T` is resolved to `self.transpose()`. Shortcuts should be similar to other environments like `numpy`.  Shortcuts added are:

* m.T -> m.transpose()
* m.I -> m.inverse()
* m.C -> m.conjugate()
* m.H -> m.conjugate().transpose()

Notes: [python data model](http://docs.python.org/reference/datamodel.html#customizing-attribute-access), [Cython's properties](http://docs.cython.org/src/userguide/extension_types.html#properties)

---

**Apply**:

1. [attachment: 8094-shortcut-matrix-transpose-v2-rebased_on_4.7.2.alpha3.patch.gz](https://github.com/sagemath/sage/files/ticket8094/8094-shortcut-matrix-transpose-v2-rebased_on_4.7.2.alpha3.patch.gz)
2. [attachment: trac-8094_matrix_shortcut_docs_reviewer2.patch.gz](https://github.com/sagemath/sage/files/ticket8094/trac-8094_matrix_shortcut_docs_reviewer2.patch.gz)
3. [attachment: trac_8094-matrix-properties-more-documentation-rebased_on_4.7.2.alpha3.patch.gz](https://github.com/sagemath/sage/files/ticket8094/trac_8094-matrix-properties-more-documentation-rebased_on_4.7.2.alpha3.patch.gz)




Assignee: **@williamstein**

CC:  @robertwb @rbeezer

Keywords: **sd32**

Author: **Harald Schilly, Jason Grout, Martin Raum**

Reviewer: **Rob Beezer, Martin Raum**

Merged: **sage-4.7.2.alpha3**

_Issue created by migration from https://trac.sagemath.org/ticket/8094_





---

archive/issue_events_088775.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2010-01-27T13:00:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "milestone_number": null,
    "milestone_title": "sage-4.7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8094#event-88775"
}
```



---

archive/issue_events_088776.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2010-01-27T13:00:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "component: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8094#event-88776"
}
```



---

archive/issue_events_088777.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2010-01-27T13:00:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8094#event-88777"
}
```



---

archive/issue_events_088778.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2010-01-27T13:00:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8094#event-88778"
}
```



---

archive/issue_events_088779.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2010-01-27T15:39:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8094#event-88779"
}
```



---

archive/issue_comments_063931.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nThis first patch adds .T for all kinds of matrices in matrix_dense.pyx and some docstrings.",
    "created_at": "2010-01-27T15:39:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63931",
    "user": "https://github.com/haraldschilly"
}
```

<a id='comment:1'>Comment 1:</a>
This first patch adds .T for all kinds of matrices in matrix_dense.pyx and some docstrings.



---

archive/issue_comments_063932.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\n`.T` has more tests and I think it works for all of them now.\n\nattachment *.2.patch should be deleted or ignored.",
    "created_at": "2010-01-27T18:09:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63932",
    "user": "https://github.com/haraldschilly"
}
```

<a id='comment:2'>Comment 2:</a>
`.T` has more tests and I think it works for all of them now.

attachment *.2.patch should be deleted or ignored.



---

archive/issue_comments_063933.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nI deleted the .2.patch.",
    "created_at": "2010-01-28T06:42:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63933",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:3'>Comment 3:</a>
I deleted the .2.patch.



---

archive/issue_comments_063934.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nSince these are cython files, why don't you use the Cython property functionality to implement this?\n\nhttp://docs.cython.org/docs/extension_types.html#properties\n\nRobert, is there an efficiency gain or some reason why one would prefer the Cython property statement to what is in this patch?",
    "created_at": "2010-01-28T06:44:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63934",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:4'>Comment 4:</a>
Since these are cython files, why don't you use the Cython property functionality to implement this?

http://docs.cython.org/docs/extension_types.html#properties

Robert, is there an efficiency gain or some reason why one would prefer the Cython property statement to what is in this patch?



---

archive/issue_comments_063935.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,6 +1,5 @@\n-The aim is to enhance `/sage/matrix/matrix*.pyx` files with a `__getattr__` mechanism to call `self.transpose()` and others more quickly. The attribute lookup mechanism provides a shortcut like `self.T` that is resolved to `self.transpose()`. \n-Shortcuts should be similar to other environments like `numpy`.\n+The aim is to enhance the `/sage/matrix/matrix2.pyx` file witha`__getattr__`mechanismcython's properties to call `self.transpose()` and other methods more quickly. A shortcut like` self.T`\u00a0is resolved to\u00a0`self.transpose()`.\u00a0Shortcuts should be similar to other environments like `numpy`.\n \n-Additionally, `__setattr__` and `__delattr__` have to be implemented to avoid modification and deletion of these attributes.\n+Additionally, `property __set__` and `__del__` have to be implemented to avoid modification and deletion of these attributes.\n \n-Notes: [python data model](http://docs.python.org/reference/datamodel.html#customizing-attribute-access). \n+Notes: [python data model](http://docs.python.org/reference/datamodel.html#customizing-attribute-access), [Cython's properties](http://docs.cython.org/docs/extension_types.html#properties)\n``````\n",
    "created_at": "2010-01-28T09:37:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63935",
    "user": "https://github.com/haraldschilly"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,6 +1,5 @@
-The aim is to enhance `/sage/matrix/matrix*.pyx` files with a `__getattr__` mechanism to call `self.transpose()` and others more quickly. The attribute lookup mechanism provides a shortcut like `self.T` that is resolved to `self.transpose()`. 
-Shortcuts should be similar to other environments like `numpy`.
+The aim is to enhance the `/sage/matrix/matrix2.pyx` file witha`__getattr__`mechanismcython's properties to call `self.transpose()` and other methods more quickly. A shortcut like` self.T` is resolved to `self.transpose()`. Shortcuts should be similar to other environments like `numpy`.
 
-Additionally, `__setattr__` and `__delattr__` have to be implemented to avoid modification and deletion of these attributes.
+Additionally, `property __set__` and `__del__` have to be implemented to avoid modification and deletion of these attributes.
 
-Notes: [python data model](http://docs.python.org/reference/datamodel.html#customizing-attribute-access). 
+Notes: [python data model](http://docs.python.org/reference/datamodel.html#customizing-attribute-access), [Cython's properties](http://docs.cython.org/docs/extension_types.html#properties)
``````




---

archive/issue_comments_063936.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nI thought that properties don't work in cython files, but yesterday I tried them (the decorator didn't work, but just the statement is fine) and they do. Thank's for the reference! I'll use them. The advantage is that then you have these shortcuts in autocomplete. It's also better to define them once and for all in matrix2.pyx - I think that's the best place?",
    "created_at": "2010-01-28T09:37:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63936",
    "user": "https://github.com/haraldschilly"
}
```

<a id='comment:5'>Comment 5:</a>
I thought that properties don't work in cython files, but yesterday I tried them (the decorator didn't work, but just the statement is fine) and they do. Thank's for the reference! I'll use them. The advantage is that then you have these shortcuts in autocomplete. It's also better to define them once and for all in matrix2.pyx - I think that's the best place?



---

archive/issue_comments_063937.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-The aim is to enhance the `/sage/matrix/matrix2.pyx` file witha`__getattr__`mechanismcython's properties to call `self.transpose()` and other methods more quickly. A shortcut like` self.T`\u00a0is resolved to\u00a0`self.transpose()`.\u00a0Shortcuts should be similar to other environments like `numpy`.\n+The aim is to enhance the `/sage/matrix/matrix2.pyx` file with\u00a0cython's properties to call `self.transpose()` and other methods more quickly. A shortcut like` self.T` is resolved to `self.transpose()`. Shortcuts should be similar to other environments like `numpy`.\n \n Additionally, `property __set__` and `__del__` have to be implemented to avoid modification and deletion of these attributes.\n \n``````\n",
    "created_at": "2010-01-28T09:37:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63937",
    "user": "https://github.com/haraldschilly"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-The aim is to enhance the `/sage/matrix/matrix2.pyx` file witha`__getattr__`mechanismcython's properties to call `self.transpose()` and other methods more quickly. A shortcut like` self.T` is resolved to `self.transpose()`. Shortcuts should be similar to other environments like `numpy`.
+The aim is to enhance the `/sage/matrix/matrix2.pyx` file with cython's properties to call `self.transpose()` and other methods more quickly. A shortcut like` self.T` is resolved to `self.transpose()`. Shortcuts should be similar to other environments like `numpy`.
 
 Additionally, `property __set__` and `__del__` have to be implemented to avoid modification and deletion of these attributes.
 
``````




---

archive/issue_comments_063938.json:
```json
{
    "body": "doin' it the cython way",
    "created_at": "2010-03-22T17:40:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63938",
    "user": "https://github.com/haraldschilly"
}
```

doin' it the cython way



---

archive/issue_comments_063939.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,4 +2,4 @@\n \n Additionally, `property __set__` and `__del__` have to be implemented to avoid modification and deletion of these attributes.\n \n-Notes: [python data model](http://docs.python.org/reference/datamodel.html#customizing-attribute-access), [Cython's properties](http://docs.cython.org/docs/extension_types.html#properties)\n+Notes: [python data model](http://docs.python.org/reference/datamodel.html#customizing-attribute-access), [Cython's properties](http://docs.cython.org/src/userguide/extension_types.html#properties)\n``````\n",
    "created_at": "2010-03-22T17:41:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63939",
    "user": "https://github.com/haraldschilly"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,4 +2,4 @@
 
 Additionally, `property __set__` and `__del__` have to be implemented to avoid modification and deletion of these attributes.
 
-Notes: [python data model](http://docs.python.org/reference/datamodel.html#customizing-attribute-access), [Cython's properties](http://docs.cython.org/docs/extension_types.html#properties)
+Notes: [python data model](http://docs.python.org/reference/datamodel.html#customizing-attribute-access), [Cython's properties](http://docs.cython.org/src/userguide/extension_types.html#properties)
``````




---

archive/issue_comments_063940.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nAttachment: **[8094-shortcut-matrix-transpose.patch.gz](https://github.com/sagemath/sage/files/ticket8094/8094-shortcut-matrix-transpose.patch.gz)**",
    "created_at": "2010-03-22T17:41:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63940",
    "user": "https://github.com/haraldschilly"
}
```

<a id='comment:7'>Comment 7:</a>
Attachment: **[8094-shortcut-matrix-transpose.patch.gz](https://github.com/sagemath/sage/files/ticket8094/8094-shortcut-matrix-transpose.patch.gz)**



---

archive/issue_comments_063941.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nschilly: is this patch still needs_work, or should it be needs_review now?",
    "created_at": "2010-12-18T19:57:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63941",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:8'>Comment 8:</a>
schilly: is this patch still needs_work, or should it be needs_review now?



---

archive/issue_events_088780.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-12-18T19:57:03Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "title_is": "shortcuts properties for matrix transpose, complex conjugate, conjugate transpose, and inverse",
    "title_was": "shortcuts for matrix transpose, complex conjugate, ...",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8094#event-88780"
}
```



---

archive/issue_comments_063942.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\njason: as far as i remember, the problem was that it was interfering with another matrix property (i.e. a doctest was failing and I didn't really understand why). so, the idea as it is coded works, but it's not ready for review.",
    "created_at": "2010-12-18T20:00:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63942",
    "user": "https://github.com/haraldschilly"
}
```

<a id='comment:9'>Comment 9:</a>
jason: as far as i remember, the problem was that it was interfering with another matrix property (i.e. a doctest was failing and I didn't really understand why). so, the idea as it is coded works, but it's not ready for review.



---

archive/issue_comments_063943.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,5 +1,8 @@\n-The aim is to enhance the `/sage/matrix/matrix2.pyx` file with\u00a0cython's properties to call `self.transpose()` and other methods more quickly. A shortcut like` self.T` is resolved to `self.transpose()`. Shortcuts should be similar to other environments like `numpy`.\n+The aim is to enhance the `/sage/matrix/matrix2.pyx` file with cython's properties to call `self.transpose()` and other methods more quickly. A shortcut like` self.T` is resolved to `self.transpose()`. Shortcuts should be similar to other environments like `numpy`.  Shortcuts added are:\n \n-Additionally, `property __set__` and `__del__` have to be implemented to avoid modification and deletion of these attributes.\n+* m.T -> m.transpose()\n+* m.I -> m.inverse()\n+* m.C -> m.conjugate()\n+* m.H -> m.conjugate().transpose()\n \n Notes: [python data model](http://docs.python.org/reference/datamodel.html#customizing-attribute-access), [Cython's properties](http://docs.cython.org/src/userguide/extension_types.html#properties)\n``````\n",
    "created_at": "2010-12-18T20:01:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63943",
    "user": "https://github.com/jasongrout"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,5 +1,8 @@
-The aim is to enhance the `/sage/matrix/matrix2.pyx` file with cython's properties to call `self.transpose()` and other methods more quickly. A shortcut like` self.T` is resolved to `self.transpose()`. Shortcuts should be similar to other environments like `numpy`.
+The aim is to enhance the `/sage/matrix/matrix2.pyx` file with cython's properties to call `self.transpose()` and other methods more quickly. A shortcut like` self.T` is resolved to `self.transpose()`. Shortcuts should be similar to other environments like `numpy`.  Shortcuts added are:
 
-Additionally, `property __set__` and `__del__` have to be implemented to avoid modification and deletion of these attributes.
+* m.T -> m.transpose()
+* m.I -> m.inverse()
+* m.C -> m.conjugate()
+* m.H -> m.conjugate().transpose()
 
 Notes: [python data model](http://docs.python.org/reference/datamodel.html#customizing-attribute-access), [Cython's properties](http://docs.cython.org/src/userguide/extension_types.html#properties)
``````




---

archive/issue_comments_063944.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nI see one doctest in matrix2.pyx failing, but that's because the expected output was wrong (it's the test for M.H in the conjugate() method).",
    "created_at": "2010-12-18T20:04:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63944",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:11'>Comment 11:</a>
I see one doctest in matrix2.pyx failing, but that's because the expected output was wrong (it's the test for M.H in the conjugate() method).



---

archive/issue_comments_063945.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nah well, time has passed since i looked into it, but it was a doctest failure somewhere else. probably just another one that has to be modified ... or it isn't an issue any more ;)",
    "created_at": "2010-12-18T20:08:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63945",
    "user": "https://github.com/haraldschilly"
}
```

<a id='comment:12'>Comment 12:</a>
ah well, time has passed since i looked into it, but it was a doctest failure somewhere else. probably just another one that has to be modified ... or it isn't an issue any more ;)



---

archive/issue_comments_063946.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nI'm uploading a reviewer patch and giving a positive review momentarily :).",
    "created_at": "2010-12-18T20:12:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63946",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:13'>Comment 13:</a>
I'm uploading a reviewer patch and giving a positive review momentarily :).



---

archive/issue_comments_063947.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nwoohoo \\o/",
    "created_at": "2010-12-18T20:16:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63947",
    "user": "https://github.com/haraldschilly"
}
```

<a id='comment:14'>Comment 14:</a>
woohoo \o/



---

archive/issue_comments_063948.json:
```json
{
    "body": "apply on top of previous patches",
    "created_at": "2010-12-18T20:22:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63948",
    "user": "https://github.com/jasongrout"
}
```

apply on top of previous patches



---

archive/issue_comments_063949.json:
```json
{
    "body": "<a id='comment:15'>Comment 15:</a>\nAttachment: **[8094_matrix_shortcut_docs_reviewer.patch.gz](https://github.com/sagemath/sage/files/ticket8094/8094_matrix_shortcut_docs_reviewer.patch.gz)**\n\nOkay, maybe my changes should be reviewed (since they fix some of the doc issues).  \n\nDoctests pass in matrix/*.py[x] with these patches applied, and Harald's code looks good, so positive review for his patch (given that my doctest patch is applied).  \n\nSomeone should verify my doctest changes are good.",
    "created_at": "2010-12-18T20:25:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63949",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:15'>Comment 15:</a>
Attachment: **[8094_matrix_shortcut_docs_reviewer.patch.gz](https://github.com/sagemath/sage/files/ticket8094/8094_matrix_shortcut_docs_reviewer.patch.gz)**

Okay, maybe my changes should be reviewed (since they fix some of the doc issues).  

Doctests pass in matrix/*.py[x] with these patches applied, and Harald's code looks good, so positive review for his patch (given that my doctest patch is applied).  

Someone should verify my doctest changes are good.



---

archive/issue_events_088781.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-12-18T20:25:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8094#event-88781"
}
```



---

archive/issue_events_088782.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-12-18T20:25:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8094#event-88782"
}
```



---

archive/issue_events_088783.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-12-18T20:25:35Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "milestone_number": null,
    "milestone_title": "sage-4.6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8094#event-88783"
}
```



---

archive/issue_events_088784.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-12-18T20:25:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "milestone_number": null,
    "milestone_title": "sage-4.6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8094#event-88784"
}
```



---

archive/issue_comments_063950.json:
```json
{
    "body": "<a id='comment:17'>Comment 17:</a>\nJason and Harald,\n\nThanks for putting this together.  I'd like to finish up getting this reviewed.  But two questions:\n\n1.  It seems like there is evidence of the shortcuts in each of the associated docstrings for the methods, but maybe it isn't always obvious.  Like  `A.I` seems to be buried along with the `~` version and has no real explanation that it is a property.  I find students get very confused with method calls with empty parentheses, and a property will just add to that, so I think it would be better to have some explanation.\n\n2.  #10471  implements `conjugate_transpose()` until we reclaim `adjoint()`.  So that would be the place to add in the shortcut.  Should we add that here since #10471 has a positive review, and maybe change the definition?\n\nI can do the above on a reviewer patch in the next day or two if necessary.\n\nRob",
    "created_at": "2010-12-19T19:08:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63950",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:17'>Comment 17:</a>
Jason and Harald,

Thanks for putting this together.  I'd like to finish up getting this reviewed.  But two questions:

1.  It seems like there is evidence of the shortcuts in each of the associated docstrings for the methods, but maybe it isn't always obvious.  Like  `A.I` seems to be buried along with the `~` version and has no real explanation that it is a property.  I find students get very confused with method calls with empty parentheses, and a property will just add to that, so I think it would be better to have some explanation.

2.  #10471  implements `conjugate_transpose()` until we reclaim `adjoint()`.  So that would be the place to add in the shortcut.  Should we add that here since #10471 has a positive review, and maybe change the definition?

I can do the above on a reviewer patch in the next day or two if necessary.

Rob



---

archive/issue_events_088785.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2010-12-19T19:08:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8094#event-88785"
}
```



---

archive/issue_events_088786.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2010-12-19T19:08:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8094#event-88786"
}
```



---

archive/issue_comments_063951.json:
```json
{
    "body": "Attachment: **[trac-8094_matrix_shortcut_docs_reviewer2.patch.gz](https://github.com/sagemath/sage/files/ticket8094/trac-8094_matrix_shortcut_docs_reviewer2.patch.gz)**",
    "created_at": "2011-08-05T17:24:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63951",
    "user": "https://github.com/sagetrac-mraum"
}
```

Attachment: **[trac-8094_matrix_shortcut_docs_reviewer2.patch.gz](https://github.com/sagemath/sage/files/ticket8094/trac-8094_matrix_shortcut_docs_reviewer2.patch.gz)**



---

archive/issue_events_088787.json:
```json
{
    "actor": "https://github.com/sagetrac-mraum",
    "created_at": "2011-08-05T17:28:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8094#event-88787"
}
```



---

archive/issue_events_088788.json:
```json
{
    "actor": "https://github.com/sagetrac-mraum",
    "created_at": "2011-08-05T17:28:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8094#event-88788"
}
```



---

archive/issue_comments_063952.json:
```json
{
    "body": "<a id='comment:18'>Comment 18:</a>\nI added a further example in the documentation of conjugate_transpose.\n\nI am not sure whether 1. can be addressed at all. Typing m.T? I get the documentation of m. I would suggest to finish this anyway. It is a shame that this patch hasn't been merged for almost 2 years, and one doesn't need to tell students that they type m.T.",
    "created_at": "2011-08-05T17:28:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63952",
    "user": "https://github.com/sagetrac-mraum"
}
```

<a id='comment:18'>Comment 18:</a>
I added a further example in the documentation of conjugate_transpose.

I am not sure whether 1. can be addressed at all. Typing m.T? I get the documentation of m. I would suggest to finish this anyway. It is a shame that this patch hasn't been merged for almost 2 years, and one doesn't need to tell students that they type m.T.



---

archive/issue_comments_063953.json:
```json
{
    "body": "Attachment: **[trac_8094-matrix-properties-more-documentation.patch.gz](https://github.com/sagemath/sage/files/ticket8094/trac_8094-matrix-properties-more-documentation.patch.gz)**",
    "created_at": "2011-08-22T21:55:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63953",
    "user": "https://github.com/rbeezer"
}
```

Attachment: **[trac_8094-matrix-properties-more-documentation.patch.gz](https://github.com/sagemath/sage/files/ticket8094/trac_8094-matrix-properties-more-documentation.patch.gz)**



---

archive/issue_comments_063954.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -6,3 +6,12 @@\n * m.H -> m.conjugate().transpose()\n \n Notes: [python data model](http://docs.python.org/reference/datamodel.html#customizing-attribute-access), [Cython's properties](http://docs.cython.org/src/userguide/extension_types.html#properties)\n+\n+**Apply**:\n+\n+1. [attachment: 8094-shortcut-matrix-transpose.patch.gz](https://github.com/sagemath/sage/files/ticket8094/8094-shortcut-matrix-transpose.patch.gz)\n+2. [attachment: trac-8094_matrix_shortcut_docs_reviewer2.patch.gz](https://github.com/sagemath/sage/files/ticket8094/trac-8094_matrix_shortcut_docs_reviewer2.patch.gz)\n+3. [attachment: trac_8094-matrix-properties-more-documentation.patch.gz](https://github.com/sagemath/sage/files/ticket8094/trac_8094-matrix-properties-more-documentation.patch.gz)\n+\n+\n+\n``````\n",
    "created_at": "2011-08-22T22:00:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63954",
    "user": "https://github.com/rbeezer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -6,3 +6,12 @@
 * m.H -> m.conjugate().transpose()
 
 Notes: [python data model](http://docs.python.org/reference/datamodel.html#customizing-attribute-access), [Cython's properties](http://docs.cython.org/src/userguide/extension_types.html#properties)
+
+**Apply**:
+
+1. [attachment: 8094-shortcut-matrix-transpose.patch.gz](https://github.com/sagemath/sage/files/ticket8094/8094-shortcut-matrix-transpose.patch.gz)
+2. [attachment: trac-8094_matrix_shortcut_docs_reviewer2.patch.gz](https://github.com/sagemath/sage/files/ticket8094/trac-8094_matrix_shortcut_docs_reviewer2.patch.gz)
+3. [attachment: trac_8094-matrix-properties-more-documentation.patch.gz](https://github.com/sagemath/sage/files/ticket8094/trac_8094-matrix-properties-more-documentation.patch.gz)
+
+
+
``````




---

archive/issue_comments_063955.json:
```json
{
    "body": "Author: **Harald Schilly, Jason Grout, Martin Raum**",
    "created_at": "2011-08-22T22:00:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63955",
    "user": "https://github.com/rbeezer"
}
```

Author: **Harald Schilly, Jason Grout, Martin Raum**



---

archive/issue_comments_063956.json:
```json
{
    "body": "Reviewer: **Rob Beezer**",
    "created_at": "2011-08-22T22:00:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63956",
    "user": "https://github.com/rbeezer"
}
```

Reviewer: **Rob Beezer**



---

archive/issue_comments_063957.json:
```json
{
    "body": "<a id='comment:19'>Comment 19:</a>\nThese are some of the earliest entries in the reference manual for `sage/matrix/matrix2.pyx`, so I think the documentation should include some examples, especially since the syntax is different (no parentheses).  However, INPUT and OUTPUT blocks feel like make-work, so I have not included that.\n\nI can give a positive review to everything up to my latest patch.  If somebody would review my (documentation-only) changes, this could be done.\n\nRob",
    "created_at": "2011-08-22T22:00:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63957",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:19'>Comment 19:</a>
These are some of the earliest entries in the reference manual for `sage/matrix/matrix2.pyx`, so I think the documentation should include some examples, especially since the syntax is different (no parentheses).  However, INPUT and OUTPUT blocks feel like make-work, so I have not included that.

I can give a positive review to everything up to my latest patch.  If somebody would review my (documentation-only) changes, this could be done.

Rob



---

archive/issue_comments_063958.json:
```json
{
    "body": "<a id='comment:20'>Comment 20:</a>\nGood idea to add this documentation. Everything works fine.",
    "created_at": "2011-08-24T08:08:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63958",
    "user": "https://github.com/sagetrac-mraum"
}
```

<a id='comment:20'>Comment 20:</a>
Good idea to add this documentation. Everything works fine.



---

archive/issue_events_088789.json:
```json
{
    "actor": "https://github.com/sagetrac-mraum",
    "created_at": "2011-08-24T08:08:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8094#event-88789"
}
```



---

archive/issue_events_088790.json:
```json
{
    "actor": "https://github.com/sagetrac-mraum",
    "created_at": "2011-08-24T08:08:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8094#event-88790"
}
```



---

archive/issue_comments_063959.json:
```json
{
    "body": "<a id='comment:21'>Comment 21:</a>\nMaybe on another ticket, but can I ask for one more enhancement on this?  Can we make the following work:\n\n```\nmatrix(\"\"\"\n[1 2]\n[3 4]\n\"\"\")\n\n(i.e., any \"]\" or \"[\" next to row delimiters are stripped).  That would mean that I could cut and paste output from Sage back into Sage to make a matrix.",
    "created_at": "2011-08-24T13:11:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63959",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:21'>Comment 21:</a>
Maybe on another ticket, but can I ask for one more enhancement on this?  Can we make the following work:

```
matrix("""
[1 2]
[3 4]
""")

(i.e., any "]" or "[" next to row delimiters are stripped).  That would mean that I could cut and paste output from Sage back into Sage to make a matrix.



---

archive/issue_comments_063960.json:
```json
{
    "body": "<a id='comment:22'>Comment 22:</a>\nArgh.   Wrong ticket; please ignore my comment!",
    "created_at": "2011-08-24T13:12:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63960",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:22'>Comment 22:</a>
Argh.   Wrong ticket; please ignore my comment!



---

archive/issue_comments_063961.json:
```json
{
    "body": "<a id='comment:23'>Comment 23:</a>\nReplying to [@jasongrout](#comment%3A22):\n> Argh.   Wrong ticket; please ignore my comment!\n\nWhew!  ;-)",
    "created_at": "2011-08-24T15:09:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63961",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:23'>Comment 23:</a>
Replying to [@jasongrout](#comment%3A22):
> Argh.   Wrong ticket; please ignore my comment!

Whew!  ;-)



---

archive/issue_comments_063962.json:
```json
{
    "body": "Changed reviewer from **Rob Beezer** to **Rob Beezer, Martin Raum**",
    "created_at": "2011-08-24T15:09:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63962",
    "user": "https://github.com/rbeezer"
}
```

Changed reviewer from **Rob Beezer** to **Rob Beezer, Martin Raum**



---

archive/issue_comments_063963.json:
```json
{
    "body": "Changed keywords from **** to **sd32**",
    "created_at": "2011-08-25T18:58:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63963",
    "user": "https://github.com/rbeezer"
}
```

Changed keywords from **** to **sd32**



---

archive/issue_events_088791.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2011-09-12T15:18:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8094#event-88791"
}
```



---

archive/issue_events_088792.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2011-09-12T15:18:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8094#event-88792"
}
```



---

archive/issue_comments_063964.json:
```json
{
    "body": "<a id='comment:26'>Comment 26:</a>\n[attachment: 8094-shortcut-matrix-transpose.patch.gz](https://github.com/sagemath/sage/files/ticket8094/8094-shortcut-matrix-transpose.patch.gz) is not a Mercurial changeset.\n\n```\nabort: no username supplied (see \"hg help config\")\n```\nSorry, Harald. ;)",
    "created_at": "2011-09-12T15:18:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63964",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:26'>Comment 26:</a>
[attachment: 8094-shortcut-matrix-transpose.patch.gz](https://github.com/sagemath/sage/files/ticket8094/8094-shortcut-matrix-transpose.patch.gz) is not a Mercurial changeset.

```
abort: no username supplied (see "hg help config")
```
Sorry, Harald. ;)



---

archive/issue_comments_063965.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -7,6 +7,8 @@\n \n Notes: [python data model](http://docs.python.org/reference/datamodel.html#customizing-attribute-access), [Cython's properties](http://docs.cython.org/src/userguide/extension_types.html#properties)\n \n+---\n+\n **Apply**:\n \n 1. [attachment: 8094-shortcut-matrix-transpose.patch.gz](https://github.com/sagemath/sage/files/ticket8094/8094-shortcut-matrix-transpose.patch.gz)\n``````\n",
    "created_at": "2011-09-12T15:18:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63965",
    "user": "https://github.com/nexttime"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -7,6 +7,8 @@
 
 Notes: [python data model](http://docs.python.org/reference/datamodel.html#customizing-attribute-access), [Cython's properties](http://docs.cython.org/src/userguide/extension_types.html#properties)
 
+---
+
 **Apply**:
 
 1. [attachment: 8094-shortcut-matrix-transpose.patch.gz](https://github.com/sagemath/sage/files/ticket8094/8094-shortcut-matrix-transpose.patch.gz)
``````




---

archive/issue_comments_063966.json:
```json
{
    "body": "<a id='comment:27'>Comment 27:</a>\nAttachment: **[8094-shortcut-matrix-transpose-v2.patch.gz](https://github.com/sagemath/sage/files/ticket8094/8094-shortcut-matrix-transpose-v2.patch.gz)**\n\nI did some maintenance on the main patch, naming it \"v2\".  Has Harald's user info and is now a proper hg patch.  I hope.  \n\nShould now be ready to go, since I did not change any code, so should not need a review.",
    "created_at": "2011-09-18T04:15:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63966",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:27'>Comment 27:</a>
Attachment: **[8094-shortcut-matrix-transpose-v2.patch.gz](https://github.com/sagemath/sage/files/ticket8094/8094-shortcut-matrix-transpose-v2.patch.gz)**

I did some maintenance on the main patch, naming it "v2".  Has Harald's user info and is now a proper hg patch.  I hope.  

Should now be ready to go, since I did not change any code, so should not need a review.



---

archive/issue_events_088793.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-09-18T04:15:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8094#event-88793"
}
```



---

archive/issue_events_088794.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-09-18T04:15:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8094#event-88794"
}
```



---

archive/issue_comments_063967.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -11,7 +11,7 @@\n \n **Apply**:\n \n-1. [attachment: 8094-shortcut-matrix-transpose.patch.gz](https://github.com/sagemath/sage/files/ticket8094/8094-shortcut-matrix-transpose.patch.gz)\n+1. [attachment: 8094-shortcut-matrix-transpose-v2.patch.gz](https://github.com/sagemath/sage/files/ticket8094/8094-shortcut-matrix-transpose-v2.patch.gz)\n 2. [attachment: trac-8094_matrix_shortcut_docs_reviewer2.patch.gz](https://github.com/sagemath/sage/files/ticket8094/trac-8094_matrix_shortcut_docs_reviewer2.patch.gz)\n 3. [attachment: trac_8094-matrix-properties-more-documentation.patch.gz](https://github.com/sagemath/sage/files/ticket8094/trac_8094-matrix-properties-more-documentation.patch.gz)\n \n``````\n",
    "created_at": "2011-09-18T04:15:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63967",
    "user": "https://github.com/rbeezer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -11,7 +11,7 @@
 
 **Apply**:
 
-1. [attachment: 8094-shortcut-matrix-transpose.patch.gz](https://github.com/sagemath/sage/files/ticket8094/8094-shortcut-matrix-transpose.patch.gz)
+1. [attachment: 8094-shortcut-matrix-transpose-v2.patch.gz](https://github.com/sagemath/sage/files/ticket8094/8094-shortcut-matrix-transpose-v2.patch.gz)
 2. [attachment: trac-8094_matrix_shortcut_docs_reviewer2.patch.gz](https://github.com/sagemath/sage/files/ticket8094/trac-8094_matrix_shortcut_docs_reviewer2.patch.gz)
 3. [attachment: trac_8094-matrix-properties-more-documentation.patch.gz](https://github.com/sagemath/sage/files/ticket8094/trac_8094-matrix-properties-more-documentation.patch.gz)
 
``````




---

archive/issue_comments_063968.json:
```json
{
    "body": "<a id='comment:28'>Comment 28:</a>\nReplying to [@rbeezer](#comment%3A27):\n> I did some maintenance on the main patch, naming it \"v2\".  Has Harald's user info and is now a proper hg patch.  I hope.\n\nYep, sorry. I would have done it myself but failed to recall Harald's e-mail address that moment; then later was simply too busy to search which ticket it was...\n\n\n\n\n> Should now be ready to go, since I did not change any code, so should not need a review.\n\nThink so. Only meta data (including line numbers) differ.\n\nAs I had other trouble with the release, I think I can still include it.",
    "created_at": "2011-09-18T04:32:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63968",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:28'>Comment 28:</a>
Replying to [@rbeezer](#comment%3A27):
> I did some maintenance on the main patch, naming it "v2".  Has Harald's user info and is now a proper hg patch.  I hope.

Yep, sorry. I would have done it myself but failed to recall Harald's e-mail address that moment; then later was simply too busy to search which ticket it was...




> Should now be ready to go, since I did not change any code, so should not need a review.

Think so. Only meta data (including line numbers) differ.

As I had other trouble with the release, I think I can still include it.



---

archive/issue_comments_063969.json:
```json
{
    "body": "Same patch rebased on (prerelease) Sage 4.7.2.alpha3 because of fuzz 2.",
    "created_at": "2011-09-18T07:01:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63969",
    "user": "https://github.com/nexttime"
}
```

Same patch rebased on (prerelease) Sage 4.7.2.alpha3 because of fuzz 2.



---

archive/issue_comments_063970.json:
```json
{
    "body": "Attachment: **[8094-shortcut-matrix-transpose-v2-rebased_on_4.7.2.alpha3.patch.gz](https://github.com/sagemath/sage/files/ticket8094/8094-shortcut-matrix-transpose-v2-rebased_on_4.7.2.alpha3.patch.gz)**\n\nAttachment: **[trac_8094-matrix-properties-more-documentation-rebased_on_4.7.2.alpha3.patch.gz](https://github.com/sagemath/sage/files/ticket8094/trac_8094-matrix-properties-more-documentation-rebased_on_4.7.2.alpha3.patch.gz)**\n\nSame patch rebased on (prerelease) Sage 4.7.2.alpha3 because of fuzz 2.",
    "created_at": "2011-09-18T07:01:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63970",
    "user": "https://github.com/nexttime"
}
```

Attachment: **[8094-shortcut-matrix-transpose-v2-rebased_on_4.7.2.alpha3.patch.gz](https://github.com/sagemath/sage/files/ticket8094/8094-shortcut-matrix-transpose-v2-rebased_on_4.7.2.alpha3.patch.gz)**

Attachment: **[trac_8094-matrix-properties-more-documentation-rebased_on_4.7.2.alpha3.patch.gz](https://github.com/sagemath/sage/files/ticket8094/trac_8094-matrix-properties-more-documentation-rebased_on_4.7.2.alpha3.patch.gz)**

Same patch rebased on (prerelease) Sage 4.7.2.alpha3 because of fuzz 2.



---

archive/issue_comments_063971.json:
```json
{
    "body": "<a id='comment:29'>Comment 29:</a>\nAttached rebased versions of two patches because of fuzz 2.",
    "created_at": "2011-09-18T07:08:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63971",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:29'>Comment 29:</a>
Attached rebased versions of two patches because of fuzz 2.



---

archive/issue_comments_063972.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -11,9 +11,9 @@\n \n **Apply**:\n \n-1. [attachment: 8094-shortcut-matrix-transpose-v2.patch.gz](https://github.com/sagemath/sage/files/ticket8094/8094-shortcut-matrix-transpose-v2.patch.gz)\n+1. [attachment: 8094-shortcut-matrix-transpose-v2-rebased_on_4.7.2.alpha3.patch.gz](https://github.com/sagemath/sage/files/ticket8094/8094-shortcut-matrix-transpose-v2-rebased_on_4.7.2.alpha3.patch.gz)\n 2. [attachment: trac-8094_matrix_shortcut_docs_reviewer2.patch.gz](https://github.com/sagemath/sage/files/ticket8094/trac-8094_matrix_shortcut_docs_reviewer2.patch.gz)\n-3. [attachment: trac_8094-matrix-properties-more-documentation.patch.gz](https://github.com/sagemath/sage/files/ticket8094/trac_8094-matrix-properties-more-documentation.patch.gz)\n+3. [attachment: trac_8094-matrix-properties-more-documentation-rebased_on_4.7.2.alpha3.patch.gz](https://github.com/sagemath/sage/files/ticket8094/trac_8094-matrix-properties-more-documentation-rebased_on_4.7.2.alpha3.patch.gz)\n \n \n \n``````\n",
    "created_at": "2011-09-18T07:08:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63972",
    "user": "https://github.com/nexttime"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -11,9 +11,9 @@
 
 **Apply**:
 
-1. [attachment: 8094-shortcut-matrix-transpose-v2.patch.gz](https://github.com/sagemath/sage/files/ticket8094/8094-shortcut-matrix-transpose-v2.patch.gz)
+1. [attachment: 8094-shortcut-matrix-transpose-v2-rebased_on_4.7.2.alpha3.patch.gz](https://github.com/sagemath/sage/files/ticket8094/8094-shortcut-matrix-transpose-v2-rebased_on_4.7.2.alpha3.patch.gz)
 2. [attachment: trac-8094_matrix_shortcut_docs_reviewer2.patch.gz](https://github.com/sagemath/sage/files/ticket8094/trac-8094_matrix_shortcut_docs_reviewer2.patch.gz)
-3. [attachment: trac_8094-matrix-properties-more-documentation.patch.gz](https://github.com/sagemath/sage/files/ticket8094/trac_8094-matrix-properties-more-documentation.patch.gz)
+3. [attachment: trac_8094-matrix-properties-more-documentation-rebased_on_4.7.2.alpha3.patch.gz](https://github.com/sagemath/sage/files/ticket8094/trac_8094-matrix-properties-more-documentation-rebased_on_4.7.2.alpha3.patch.gz)
 
 
 
``````




---

archive/issue_events_088795.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2011-09-18T09:37:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8094#event-88795"
}
```



---

archive/issue_events_088796.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2011-09-18T09:37:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8094#event-88796"
}
```



---

archive/issue_comments_063973.json:
```json
{
    "body": "Merged: **sage-4.7.2.alpha3**",
    "created_at": "2011-09-18T09:37:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8094#issuecomment-63973",
    "user": "https://github.com/nexttime"
}
```

Merged: **sage-4.7.2.alpha3**
