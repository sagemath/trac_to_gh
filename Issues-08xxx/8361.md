# Issue 8361: Add an __iter__ method for IntegerVectors

archive/issues_008361.json:
```json
{
    "body": "Add an iterator for IntegerVectors\n\n\nOld proposition :\nUsing improvement of SearchForset and the breath_first_search_iterator inside it, implement an iterator over all integers vectors.\n\ndepend on #8288\n\n**Assignee:** sage-combinat\n\n**Keywords:** iterator, integer, vector\n\n**Status:** needs_work\n\nIssue created by migration from https://trac.sagemath.org/ticket/8361\n\n",
    "created_at": "2010-02-25T13:18:32Z",
    "labels": [
        "component: combinatorics",
        "minor",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.4",
    "title": "Add an __iter__ method for IntegerVectors",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/8361",
    "user": "https://trac.sagemath.org/admin/accounts/users/nborie"
}
```
Add an iterator for IntegerVectors


Old proposition :
Using improvement of SearchForset and the breath_first_search_iterator inside it, implement an iterator over all integers vectors.

depend on #8288

**Assignee:** sage-combinat

**Keywords:** iterator, integer, vector

**Status:** needs_work

Issue created by migration from https://trac.sagemath.org/ticket/8361





---

archive/attachments_010376.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "iter_method_for_integer_vectors-nb.patch",
    "asset_url": "tarball://root/attachments/some-uuid/ticket8361/iter_method_for_integer_vectors-nb.patch",
    "created_at": "2010-02-25T13:18:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8361",
    "type": "attachment",
    "url": "https://github.com/assets/some-id/some-uuid.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/nborie"
}
```



---

archive/issue_comments_086440.json:
```json
{
    "body": "",
    "created_at": "2010-02-25T13:18:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8361",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8361#issuecomment-86440",
    "user": "https://trac.sagemath.org/admin/accounts/users/nborie"
}
```





---

archive/issue_comments_086441.json:
```json
{
    "body": "**Changing status** from new to needs_review.",
    "created_at": "2010-02-25T13:19:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8361",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8361#issuecomment-86441",
    "user": "https://trac.sagemath.org/admin/accounts/users/nborie"
}
```

**Changing status** from new to needs_review.



---

archive/issue_comments_086442.json:
```json
{
    "body": "<a id='comment:2'></a>\nI have started a discussion about this patch in sage-devel: http://groups.google.com/group/sage-devel/browse_thread/thread/26d80fb37461ac19#\n\nI think that accepting this enhancement should be deferred until that discussion is concluded.",
    "created_at": "2010-08-02T10:11:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8361",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8361#issuecomment-86442",
    "user": "https://github.com/johanrosenkilde"
}
```

<a id='comment:2'></a>
I have started a discussion about this patch in sage-devel: http://groups.google.com/group/sage-devel/browse_thread/thread/26d80fb37461ac19#

I think that accepting this enhancement should be deferred until that discussion is concluded.



---

archive/issue_comments_086443.json:
```json
{
    "body": "**Changing status** from needs_review to needs_work.",
    "created_at": "2010-08-02T10:11:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8361",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8361#issuecomment-86443",
    "user": "https://github.com/johanrosenkilde"
}
```

**Changing status** from needs_review to needs_work.



---

archive/issue_comments_086444.json:
```json
{
    "body": "**Changing author** from \"Nicolas Borie\" to \"\".",
    "created_at": "2010-08-02T12:13:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8361",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8361#issuecomment-86444",
    "user": "https://trac.sagemath.org/admin/accounts/users/nborie"
}
```

**Changing author** from "Nicolas Borie" to "".



---

archive/issue_comments_086445.json:
```json
{
    "body": "<a id='comment:3'></a>\nI should have put the need work myself...\n\nI opened it a long time ago but in fact, the main reason was try to play with the SearchForest feature. As I have no mathematical specific argument or idea, anyone would make a better work than my old proposition.\n\nThis is clearly free to assign. Thanks for your comments and the start of a discussion.",
    "created_at": "2010-08-02T12:13:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8361",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8361#issuecomment-86445",
    "user": "https://trac.sagemath.org/admin/accounts/users/nborie"
}
```

<a id='comment:3'></a>
I should have put the need work myself...

I opened it a long time ago but in fact, the main reason was try to play with the SearchForest feature. As I have no mathematical specific argument or idea, anyone would make a better work than my old proposition.

This is clearly free to assign. Thanks for your comments and the start of a discussion.



---

archive/issue_comments_086446.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,3 +1,7 @@\n+Add an iterator for IntegerVectors\n+\n+\n+Old proposition :\n Using improvement of SearchForset and the breath_first_search_iterator inside it, implement an iterator over all integers vectors.\n \n depend on #8288\n``````\n",
    "created_at": "2010-08-02T12:13:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8361",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8361#issuecomment-86446",
    "user": "https://trac.sagemath.org/admin/accounts/users/nborie"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,3 +1,7 @@
+Add an iterator for IntegerVectors
+
+
+Old proposition :
 Using improvement of SearchForset and the breath_first_search_iterator inside it, implement an iterator over all integers vectors.
 
 depend on #8288
``````




---

archive/issue_events_024438.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/8361",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8361#event-24438"
}
```



---

archive/issue_events_024439.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/8361",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8361#event-24439"
}
```



---

archive/issue_events_024440.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/8361",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8361#event-24440"
}
```



---

archive/issue_events_024441.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/8361",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8361#event-24441"
}
```



---

archive/issue_events_024442.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/8361",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8361#event-24442"
}
```



---

archive/issue_events_024443.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/8361",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8361#event-24443"
}
```



---

archive/issue_events_024444.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/8361",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8361#event-24444"
}
```
