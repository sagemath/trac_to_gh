# Issue 8361: Add an __iter__ method for IntegerVectors

archive/issues_008361.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-sage-combinat"
    ],
    "body": "Add an iterator for IntegerVectors\n\n\nOld proposition :\nUsing improvement of SearchForset and the breath_first_search_iterator inside it, implement an iterator over all integers vectors.\n\ndepend on #8288\n\nKeywords: **iterator, integer, vector**\n\nComponent: **combinatorics**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8361_\n\n",
    "created_at": "2010-02-25T13:18:32Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/needs%20work"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add an __iter__ method for IntegerVectors",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/8361",
    "user": "https://github.com/sagetrac-nborie"
}
```
Add an iterator for IntegerVectors


Old proposition :
Using improvement of SearchForset and the breath_first_search_iterator inside it, implement an iterator over all integers vectors.

depend on #8288

Keywords: **iterator, integer, vector**

Component: **combinatorics**

_Issue created by migration from https://trac.sagemath.org/ticket/8361_





---

archive/issue_events_104591.json:
```json
{
    "actor": "https://github.com/sagetrac-nborie",
    "created_at": "2010-02-25T13:18:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8361",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8361#event-104591"
}
```



---

archive/issue_events_104592.json:
```json
{
    "actor": "https://github.com/sagetrac-nborie",
    "created_at": "2010-02-25T13:18:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8361",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "c: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8361#event-104592"
}
```



---

archive/issue_events_104593.json:
```json
{
    "actor": "https://github.com/sagetrac-nborie",
    "created_at": "2010-02-25T13:18:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8361",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8361#event-104593"
}
```



---

archive/issue_events_104594.json:
```json
{
    "actor": "https://github.com/sagetrac-nborie",
    "created_at": "2010-02-25T13:18:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8361",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8361#event-104594"
}
```



---

archive/issue_events_104595.json:
```json
{
    "actor": "https://github.com/sagetrac-sage-combinat",
    "created_at": "2010-02-25T13:18:32Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/8361",
    "subject": "https://github.com/sagetrac-nborie",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8361#event-104595"
}
```



---

archive/issue_events_104596.json:
```json
{
    "actor": "https://github.com/sagetrac-nborie",
    "created_at": "2010-02-25T13:19:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8361",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8361#event-104596"
}
```



---

archive/issue_comments_065819.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[iter_method_for_integer_vectors-nb.patch.gz](https://github.com/sagemath/sage/files/ticket8361/iter_method_for_integer_vectors-nb.patch.gz)**",
    "created_at": "2010-02-25T13:19:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8361",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8361#issuecomment-65819",
    "user": "https://github.com/sagetrac-nborie"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[iter_method_for_integer_vectors-nb.patch.gz](https://github.com/sagemath/sage/files/ticket8361/iter_method_for_integer_vectors-nb.patch.gz)**



---

archive/issue_events_104597.json:
```json
{
    "actor": "https://github.com/johanrosenkilde",
    "created_at": "2010-08-02T10:11:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8361",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8361#event-104597"
}
```



---

archive/issue_events_104598.json:
```json
{
    "actor": "https://github.com/johanrosenkilde",
    "created_at": "2010-08-02T10:11:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8361",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8361#event-104598"
}
```



---

archive/issue_comments_065820.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nI have started a discussion about this patch in sage-devel: http://groups.google.com/group/sage-devel/browse_thread/thread/26d80fb37461ac19#\n\nI think that accepting this enhancement should be deferred until that discussion is concluded.",
    "created_at": "2010-08-02T10:11:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8361",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8361#issuecomment-65820",
    "user": "https://github.com/johanrosenkilde"
}
```

<div id="comment:2" align="right">comment:2</div>

I have started a discussion about this patch in sage-devel: http://groups.google.com/group/sage-devel/browse_thread/thread/26d80fb37461ac19#

I think that accepting this enhancement should be deferred until that discussion is concluded.



---

archive/issue_comments_065821.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,7 @@\n+Add an iterator for IntegerVectors\n+\n+\n+Old proposition :\n Using improvement of SearchForset and the breath_first_search_iterator inside it, implement an iterator over all integers vectors.\n \n depend on #8288\n``````\n",
    "created_at": "2010-08-02T12:13:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8361",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8361#issuecomment-65821",
    "user": "https://github.com/sagetrac-nborie"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,7 @@
+Add an iterator for IntegerVectors
+
+
+Old proposition :
 Using improvement of SearchForset and the breath_first_search_iterator inside it, implement an iterator over all integers vectors.
 
 depend on #8288
``````




---

archive/issue_comments_065822.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nI should have put the need work myself...\n\nI opened it a long time ago but in fact, the main reason was try to play with the SearchForest feature. As I have no mathematical specific argument or idea, anyone would make a better work than my old proposition.\n\nThis is clearly free to assign. Thanks for your comments and the start of a discussion.",
    "created_at": "2010-08-02T12:13:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8361",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8361#issuecomment-65822",
    "user": "https://github.com/sagetrac-nborie"
}
```

<div id="comment:3" align="right">comment:3</div>

I should have put the need work myself...

I opened it a long time ago but in fact, the main reason was try to play with the SearchForest feature. As I have no mathematical specific argument or idea, anyone would make a better work than my old proposition.

This is clearly free to assign. Thanks for your comments and the start of a discussion.



---

archive/issue_comments_065823.json:
```json
{
    "body": "Changed author from **Nicolas Borie** to none",
    "created_at": "2010-08-02T12:13:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8361",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8361#issuecomment-65823",
    "user": "https://github.com/sagetrac-nborie"
}
```

Changed author from **Nicolas Borie** to none



---

archive/issue_events_104599.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8361",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8361#event-104599"
}
```



---

archive/issue_events_104600.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8361",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8361#event-104600"
}
```



---

archive/issue_events_104601.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8361",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8361#event-104601"
}
```



---

archive/issue_events_104602.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8361",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8361#event-104602"
}
```



---

archive/issue_events_104603.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8361",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8361#event-104603"
}
```



---

archive/issue_events_104604.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8361",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8361#event-104604"
}
```



---

archive/issue_events_104605.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8361",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8361#event-104605"
}
```



---

archive/issue_events_104606.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8361",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8361#event-104606"
}
```



---

archive/issue_events_104607.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8361",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8361#event-104607"
}
```
