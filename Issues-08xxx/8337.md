# Issue 8337: factorization of multivariate polynomials is terribly slow

archive/issues_008337.json:
```json
{
    "assignees": [],
    "body": "From http://groups.google.com/group/sage-support/browse_thread/thread/72fbc6d6f5a7d746#, with sage 4.3.3:\n\n```\nsage: var('E1, E2, E4, E5, E10, E20'); \nsage: var( 'q' ); \nsage: t=(E20^16*E5^8*q^4*E2^24 + (-E20^16*E5^8*q^4*E4^8*E1^16 + (-E10^24 + E20^8*E5^16)*E4^16*E1^8)) \nsage: factor(t)\n```\ndoes not answer in reasonable time (a few seconds).\n\nMaple 13 answers in less than a second (and says the polynomial\nis irreducible).\n\nAssignee: **@malb**\n\nReviewer: **Paul Zimmermann**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8337_\n\n",
    "closed_at": "2013-08-30T08:43:54Z",
    "created_at": "2010-02-23T18:18:55Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/invalid",
        "https://github.com/sagemath/sage/labels/performance",
        "https://github.com/sagemath/sage/labels/worksforme"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "factorization of multivariate polynomials is terribly slow",
    "type": "issue",
    "updated_at": "2013-08-30T08:43:54Z",
    "url": "https://github.com/sagemath/sage/issues/8337",
    "user": "https://github.com/zimmermann6"
}
```
From http://groups.google.com/group/sage-support/browse_thread/thread/72fbc6d6f5a7d746#, with sage 4.3.3:

```
sage: var('E1, E2, E4, E5, E10, E20'); 
sage: var( 'q' ); 
sage: t=(E20^16*E5^8*q^4*E2^24 + (-E20^16*E5^8*q^4*E4^8*E1^16 + (-E10^24 + E20^8*E5^16)*E4^16*E1^8)) 
sage: factor(t)
```
does not answer in reasonable time (a few seconds).

Maple 13 answers in less than a second (and says the polynomial
is irreducible).

Assignee: **@malb**

Reviewer: **Paul Zimmermann**

_Issue created by migration from https://trac.sagemath.org/ticket/8337_





---

archive/issue_events_094849.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2010-02-23T18:18:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8337",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8337#event-94849"
}
```



---

archive/issue_events_094850.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2010-02-23T18:18:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8337",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
    "label_color": "0000ff",
    "label_name": "component: commutative algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8337#event-94850"
}
```



---

archive/issue_events_094851.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2010-02-23T18:18:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8337",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8337#event-94851"
}
```



---

archive/issue_events_094852.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2010-02-23T18:18:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8337",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8337#event-94852"
}
```



---

archive/issue_events_094853.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8337",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8337#event-94853"
}
```



---

archive/issue_events_094854.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8337",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8337#event-94854"
}
```



---

archive/issue_events_094855.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2013-08-23T15:02:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8337",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8337#event-94855"
}
```



---

archive/issue_comments_067296.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nseems to be fixed in Sage 5.11:\n\n```\nsage: %time factor(t)\nCPU times: user 0.05 s, sys: 0.00 s, total: 0.05 s\nWall time: 0.09 s\nE2^24*E20^16*E5^8*q^4 - E1^16*E20^16*E4^8*E5^8*q^4 - E1^8*E10^24*E4^16 + E1^8*E20^8*E4^16*E5^16\n```\nShould I add a doctest for that? If yes, how to check the computation is fast enough?\n\nPaul",
    "created_at": "2013-08-23T15:02:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8337",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8337#issuecomment-67296",
    "user": "https://github.com/zimmermann6"
}
```

<a id='comment:2'>Comment 2:</a>
seems to be fixed in Sage 5.11:

```
sage: %time factor(t)
CPU times: user 0.05 s, sys: 0.00 s, total: 0.05 s
Wall time: 0.09 s
E2^24*E20^16*E5^8*q^4 - E1^16*E20^16*E4^8*E5^8*q^4 - E1^8*E10^24*E4^16 + E1^8*E20^8*E4^16*E5^16
```
Should I add a doctest for that? If yes, how to check the computation is fast enough?

Paul



---

archive/issue_events_094856.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-24T08:01:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8337",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
    "label_color": "0000ff",
    "label_name": "component: commutative algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8337#event-94856"
}
```



---

archive/issue_events_094857.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-24T08:01:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8337",
    "label": "https://github.com/sagemath/sage/labels/performance",
    "label_color": "696969",
    "label_name": "performance",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8337#event-94857"
}
```



---

archive/issue_comments_067297.json:
```json
{
    "body": "Reviewer: **Paul Zimmermann**",
    "created_at": "2013-08-24T08:01:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8337",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8337#issuecomment-67297",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Paul Zimmermann**



---

archive/issue_comments_067298.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nThere currently isn't really a way in Sage to test timings. There has been a lot of thought about this, but it's hard to do right.\n\nSo the most reasonable thing to do is to close this ticket.",
    "created_at": "2013-08-24T08:01:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8337",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8337#issuecomment-67298",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:3'>Comment 3:</a>
There currently isn't really a way in Sage to test timings. There has been a lot of thought about this, but it's hard to do right.

So the most reasonable thing to do is to close this ticket.



---

archive/issue_events_094858.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-24T08:01:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8337",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8337#event-94858"
}
```



---

archive/issue_events_094859.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-24T08:01:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8337",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8337#event-94859"
}
```



---

archive/issue_comments_067299.json:
```json
{
    "body": "Changed author from **Paul Zimmermann** to none",
    "created_at": "2013-08-24T08:01:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8337",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8337#issuecomment-67299",
    "user": "https://github.com/jdemeyer"
}
```

Changed author from **Paul Zimmermann** to none



---

archive/issue_events_094860.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-24T08:01:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8337",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8337#event-94860"
}
```



---

archive/issue_events_094861.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-24T08:01:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8337",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8337#event-94861"
}
```



---

archive/issue_events_094862.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-24T08:01:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8337",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8337#event-94862"
}
```



---

archive/issue_comments_067300.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\n> So the most reasonable thing to do is to close this ticket. \n\nok, fine to me.\n\nPaul",
    "created_at": "2013-08-24T08:09:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8337",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8337#issuecomment-67300",
    "user": "https://github.com/zimmermann6"
}
```

<a id='comment:4'>Comment 4:</a>
> So the most reasonable thing to do is to close this ticket. 

ok, fine to me.

Paul



---

archive/issue_events_094863.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-30T08:43:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8337",
    "label": "https://github.com/sagemath/sage/labels/worksforme",
    "label_color": "a6a6a6",
    "label_name": "worksforme",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8337#event-94863"
}
```



---

archive/issue_events_094864.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-30T08:43:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8337",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8337#event-94864"
}
```



---

archive/issue_events_094865.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-30T08:43:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8337",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8337#event-94865"
}
```
