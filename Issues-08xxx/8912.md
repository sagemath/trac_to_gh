# Issue 8912: documentation error in pseudo_order for finite monoid element

archive/issues_008912.json:
```json
{
    "assignees": [
        "https://github.com/saliola"
    ],
    "body": "the word self is latexed in the first line of the documentation string, but it shouldn't be.\n\n```\nm = FiniteMonoids().example()\nx = m.an_element()\nx.pseudo_order??\n```\n\na patch is coming soon\n\nKeywords: **documentation**\n\nComponent: **documentation**\n\nAuthor: **Franco Saliola**\n\nReviewer: **Minh Van Nguyen**\n\nMerged: **sage-4.4.2.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8912_\n\n",
    "closed_at": "2010-05-09T04:16:42Z",
    "created_at": "2010-05-07T14:56:05Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%205%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.4.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "documentation error in pseudo_order for finite monoid element",
    "type": "issue",
    "updated_at": "2010-05-09T04:16:42Z",
    "url": "https://github.com/sagemath/sage/issues/8912",
    "user": "https://github.com/saliola"
}
```
the word self is latexed in the first line of the documentation string, but it shouldn't be.

```
m = FiniteMonoids().example()
x = m.an_element()
x.pseudo_order??
```

a patch is coming soon

Keywords: **documentation**

Component: **documentation**

Author: **Franco Saliola**

Reviewer: **Minh Van Nguyen**

Merged: **sage-4.4.2.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/8912_





---

archive/issue_events_113056.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2010-05-07T14:56:05Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8912",
    "milestone_number": null,
    "milestone_title": "sage-4.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8912#event-113056"
}
```



---

archive/issue_events_113057.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2010-05-07T14:56:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8912",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8912#event-113057"
}
```



---

archive/issue_events_113058.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2010-05-07T14:56:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8912",
    "label": "https://github.com/sagemath/sage/labels/p%3A%205%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p: 5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8912#event-113058"
}
```



---

archive/issue_events_113059.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2010-05-07T14:56:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8912",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8912#event-113059"
}
```



---

archive/issue_events_113060.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2010-05-07T14:56:05Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/8912",
    "subject": "https://github.com/saliola",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8912#event-113060"
}
```



---

archive/issue_comments_073120.json:
```json
{
    "body": "Attachment: **[trac8912.patch.gz](https://github.com/sagemath/sage/files/ticket8912/trac8912.patch.gz)**",
    "created_at": "2010-05-07T15:28:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8912#issuecomment-73120",
    "user": "https://github.com/saliola"
}
```

Attachment: **[trac8912.patch.gz](https://github.com/sagemath/sage/files/ticket8912/trac8912.patch.gz)**



---

archive/issue_events_113061.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2010-05-07T15:30:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8912",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8912#event-113061"
}
```



---

archive/issue_comments_073121.json:
```json
{
    "body": "Author: **Franco Saliola**",
    "created_at": "2010-05-07T15:30:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8912#issuecomment-73121",
    "user": "https://github.com/saliola"
}
```

Author: **Franco Saliola**



---

archive/issue_events_113062.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-05-08T23:53:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8912",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8912#event-113062"
}
```



---

archive/issue_events_113063.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-05-08T23:53:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8912",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8912#event-113063"
}
```



---

archive/issue_comments_073122.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nLooks good to me.",
    "created_at": "2010-05-08T23:53:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8912#issuecomment-73122",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:3" align="right">comment:3</div>

Looks good to me.



---

archive/issue_comments_073123.json:
```json
{
    "body": "Reviewer: **Minh Van Nguyen**",
    "created_at": "2010-05-08T23:53:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8912#issuecomment-73123",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Reviewer: **Minh Van Nguyen**



---

archive/issue_events_113064.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-05-09T04:16:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8912",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8912#event-113064"
}
```



---

archive/issue_events_113065.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-05-09T04:16:42Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8912",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8912#event-113065"
}
```



---

archive/issue_comments_073124.json:
```json
{
    "body": "Merged: **sage-4.4.2.alpha0**",
    "created_at": "2010-05-09T04:16:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8912#issuecomment-73124",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Merged: **sage-4.4.2.alpha0**
