# Issue 8621: New functions in lcalc wrapper

archive/issues_008621.json:
```json
{
    "assignees": [],
    "body": "I have added access to two new functions in lcalc wrapper.\n\ncompute_rank and hardy_z_function. This patch depends on #5396.\n\n---\nApply:\n\n* [attachment: trac8621.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621.patch.gz)\n* [attachment: trac8621_review_rebase.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621_review_rebase.patch.gz)\n\nAssignee: **@williamstein**\n\nCC:  bober @JohnCremona @craigcitro\n\nKeywords: **lcalc**\n\nBranch/Commit: **[u/chapoton/8621](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/8621) @ [1c387d6](https://github.com/sagemath/sagetrac-mirror/commit/1c387d6cb01f6a378fd53fe0af1217a15211e1cb)**\n\nReviewer: **Jonathan Bober**\n\nAuthor: **Rishikesh**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8621_\n\n",
    "closed_at": "2014-02-03T22:59:12Z",
    "created_at": "2010-03-29T00:36:00Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20number%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "New functions in lcalc wrapper",
    "type": "issue",
    "updated_at": "2014-02-03T22:59:12Z",
    "url": "https://github.com/sagemath/sage/issues/8621",
    "user": "https://github.com/rishikesha"
}
```
I have added access to two new functions in lcalc wrapper.

compute_rank and hardy_z_function. This patch depends on #5396.

---
Apply:

* [attachment: trac8621.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621.patch.gz)
* [attachment: trac8621_review_rebase.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621_review_rebase.patch.gz)

Assignee: **@williamstein**

CC:  bober @JohnCremona @craigcitro

Keywords: **lcalc**

Branch/Commit: **[u/chapoton/8621](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/8621) @ [1c387d6](https://github.com/sagemath/sagetrac-mirror/commit/1c387d6cb01f6a378fd53fe0af1217a15211e1cb)**

Reviewer: **Jonathan Bober**

Author: **Rishikesh**

_Issue created by migration from https://trac.sagemath.org/ticket/8621_





---

archive/issue_events_098936.json:
```json
{
    "actor": "https://github.com/rishikesha",
    "created_at": "2010-03-29T00:36:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8621#event-98936"
}
```



---

archive/issue_events_098937.json:
```json
{
    "actor": "https://github.com/rishikesha",
    "created_at": "2010-03-29T00:36:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20theory",
    "label_color": "0000ff",
    "label_name": "component: number theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8621#event-98937"
}
```



---

archive/issue_events_098938.json:
```json
{
    "actor": "https://github.com/rishikesha",
    "created_at": "2010-03-29T00:36:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8621#event-98938"
}
```



---

archive/issue_events_098939.json:
```json
{
    "actor": "https://github.com/rishikesha",
    "created_at": "2010-03-29T00:36:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8621#event-98939"
}
```



---

archive/issue_events_098940.json:
```json
{
    "actor": "https://github.com/rishikesha",
    "created_at": "2010-03-29T00:36:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8621#event-98940"
}
```



---

archive/issue_events_098941.json:
```json
{
    "actor": "https://github.com/rishikesha",
    "created_at": "2010-03-29T00:36:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8621#event-98941"
}
```



---

archive/issue_comments_070856.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n I have added access to two new functions in lcalc wrapper.\n \n-compute_rank and hardy_z_function\n+compute_rank and hardy_z_function. This patch depends on #5396\n``````\n",
    "created_at": "2010-03-29T00:44:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70856",
    "user": "https://github.com/rishikesha"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
 I have added access to two new functions in lcalc wrapper.
 
-compute_rank and hardy_z_function
+compute_rank and hardy_z_function. This patch depends on #5396
``````




---

archive/issue_comments_070857.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nAttachment: **[trac_8621.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac_8621.patch.gz)**\n\nI have added two new functions\n\n```\nsage: from sage.libs.lcalc.lcalc_Lfunction import (Lfunction_from_character, Lfunction_from_elliptic_curve)\nsage: chi=DirichletGroup(123)[31]\nsage: L1=Lfunction_from_character(chi)\nsage: L1.hardy_z_function(.5+5*I)\n-0.462453973892362 - 7.93526871565814e-15*I\nsage: L1.compute_rank()\n0\nsage: L2=Lfunction_from_elliptic_curve(EllipticCurve('37a'))\nsage: L2.compute_rank()\n1\nsage: L2.hardy_z_function(.5+6*I)\n-2.17184689048993 - 1.76053169785863e-15*I\nsage: L2.hardy_z_function(-.5+6*I)\n4.17981266933977 + 36.9688966864015*I\n```\n\nAlthough for elliptic curves, other programs will be better for analytic rank, I have added this because here it works for any L function",
    "created_at": "2010-03-29T00:44:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70857",
    "user": "https://github.com/rishikesha"
}
```

<a id='comment:2'>Comment 2:</a>
Attachment: **[trac_8621.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac_8621.patch.gz)**

I have added two new functions

```
sage: from sage.libs.lcalc.lcalc_Lfunction import (Lfunction_from_character, Lfunction_from_elliptic_curve)
sage: chi=DirichletGroup(123)[31]
sage: L1=Lfunction_from_character(chi)
sage: L1.hardy_z_function(.5+5*I)
-0.462453973892362 - 7.93526871565814e-15*I
sage: L1.compute_rank()
0
sage: L2=Lfunction_from_elliptic_curve(EllipticCurve('37a'))
sage: L2.compute_rank()
1
sage: L2.hardy_z_function(.5+6*I)
-2.17184689048993 - 1.76053169785863e-15*I
sage: L2.hardy_z_function(-.5+6*I)
4.17981266933977 + 36.9688966864015*I
```

Although for elliptic curves, other programs will be better for analytic rank, I have added this because here it works for any L function



---

archive/issue_comments_070858.json:
```json
{
    "body": "Attachment: **[trac_8621.2.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac_8621.2.patch.gz)**\n\nrebase of rishi's patch",
    "created_at": "2011-09-06T11:05:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70858",
    "user": "https://github.com/sagetrac-bober"
}
```

Attachment: **[trac_8621.2.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac_8621.2.patch.gz)**

rebase of rishi's patch



---

archive/issue_events_098942.json:
```json
{
    "actor": "https://github.com/sagetrac-bober",
    "created_at": "2011-09-07T19:08:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8621#event-98942"
}
```



---

archive/issue_comments_070859.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nI don't love everything about the way this patch or the lcalc wrapper do things, and maybe a better or at least newer wrapper is on the way, but this patch does improve on the functionality available in sage and doesn't break things.",
    "created_at": "2011-09-07T19:08:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70859",
    "user": "https://github.com/sagetrac-bober"
}
```

<a id='comment:4'>Comment 4:</a>
I don't love everything about the way this patch or the lcalc wrapper do things, and maybe a better or at least newer wrapper is on the way, but this patch does improve on the functionality available in sage and doesn't break things.



---

archive/issue_events_098943.json:
```json
{
    "actor": "https://github.com/sagetrac-bober",
    "created_at": "2011-09-07T19:08:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8621#event-98943"
}
```



---

archive/issue_events_098944.json:
```json
{
    "actor": "https://github.com/sagetrac-bober",
    "created_at": "2011-09-07T19:08:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8621#event-98944"
}
```



---

archive/issue_comments_070860.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,8 @@\n I have added access to two new functions in lcalc wrapper.\n \n-compute_rank and hardy_z_function. This patch depends on #5396\n+compute_rank and hardy_z_function. This patch depends on #5396.\n+\n+---\n+\n+Apply [attachment: trac_8621.2.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac_8621.2.patch.gz) to the Sage library.\n+\n``````\n",
    "created_at": "2011-09-09T01:15:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70860",
    "user": "https://github.com/nexttime"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,8 @@
 I have added access to two new functions in lcalc wrapper.
 
-compute_rank and hardy_z_function. This patch depends on #5396
+compute_rank and hardy_z_function. This patch depends on #5396.
+
+---
+
+Apply [attachment: trac_8621.2.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac_8621.2.patch.gz) to the Sage library.
+
``````




---

archive/issue_comments_070861.json:
```json
{
    "body": "Author: **Rishikesh**",
    "created_at": "2011-09-09T01:15:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70861",
    "user": "https://github.com/nexttime"
}
```

Author: **Rishikesh**



---

archive/issue_comments_070862.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nJonathan, you can add yourself to [http://trac.sagemath.org/sage_trac/wiki#AccountNamesMappedtoRealNames](http://trac.sagemath.org/sage_trac/wiki#AccountNamesMappedtoRealNames).",
    "created_at": "2011-09-09T01:15:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70862",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:6'>Comment 6:</a>
Jonathan, you can add yourself to [http://trac.sagemath.org/sage_trac/wiki#AccountNamesMappedtoRealNames](http://trac.sagemath.org/sage_trac/wiki#AccountNamesMappedtoRealNames).



---

archive/issue_comments_070863.json:
```json
{
    "body": "Reviewer: **Jonathan Bober**",
    "created_at": "2011-09-09T01:15:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70863",
    "user": "https://github.com/nexttime"
}
```

Reviewer: **Jonathan Bober**



---

archive/issue_comments_070864.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nI do get doctest errors due to numerical noise.\n\nThis is on a Core2 Penryn (Ubuntu 10.04.3 x86_64):\n\n```\n**********************************************************************\nFile \".../sage/libs/lcalc/lcalc_Lfunction.pyx\", line 181:\n    sage: L.hardy_z_function(.2+.4*I)\nExpected:\n    0.2166144222685... - 0.004081871278504...*I\nGot:\n    0.216614422268554 - 0.00408187127850235*I\n**********************************************************************\nFile \".../sage/libs/lcalc/lcalc_Lfunction.pyx\", line 191:\n    sage: L.hardy_z_function(.5+2.1*I)\nExpected:\n    -0.0064317917686980...\nGot:\n    -0.00643179176869426 - 3.93820653320273e-19*I\n**********************************************************************\n1 items had failures:\n   2 of  17 in __main__.example_4\n***Test Failed*** 2 failures.\n```\n\nI've seen almost the same on redhawk, an Opteron 8439 SE:\n\n```\n**********************************************************************\nFile \".../sage/libs/lcalc/lcalc_Lfunction.pyx\", line 181:\n    sage: L.hardy_z_function(.2+.4*I)\nExpected:\n    0.2166144222685... - 0.004081871278504...*I\nGot:\n    0.216614422268554 - 0.00408187127850235*I\n**********************************************************************\nFile \".../sage/libs/lcalc/lcalc_Lfunction.pyx\", line 191:\n    sage: L.hardy_z_function(.5+2.1*I)\nExpected:\n    -0.0064317917686980...\nGot:\n    -0.00643179176869423 - 3.93820653320271e-19*I\n**********************************************************************\n1 items had failures:\n   2 of  17 in __main__.example_4\n***Test Failed*** 2 failures.\n```",
    "created_at": "2011-09-15T12:15:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70864",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:7'>Comment 7:</a>
I do get doctest errors due to numerical noise.

This is on a Core2 Penryn (Ubuntu 10.04.3 x86_64):

```
**********************************************************************
File ".../sage/libs/lcalc/lcalc_Lfunction.pyx", line 181:
    sage: L.hardy_z_function(.2+.4*I)
Expected:
    0.2166144222685... - 0.004081871278504...*I
Got:
    0.216614422268554 - 0.00408187127850235*I
**********************************************************************
File ".../sage/libs/lcalc/lcalc_Lfunction.pyx", line 191:
    sage: L.hardy_z_function(.5+2.1*I)
Expected:
    -0.0064317917686980...
Got:
    -0.00643179176869426 - 3.93820653320273e-19*I
**********************************************************************
1 items had failures:
   2 of  17 in __main__.example_4
***Test Failed*** 2 failures.
```

I've seen almost the same on redhawk, an Opteron 8439 SE:

```
**********************************************************************
File ".../sage/libs/lcalc/lcalc_Lfunction.pyx", line 181:
    sage: L.hardy_z_function(.2+.4*I)
Expected:
    0.2166144222685... - 0.004081871278504...*I
Got:
    0.216614422268554 - 0.00408187127850235*I
**********************************************************************
File ".../sage/libs/lcalc/lcalc_Lfunction.pyx", line 191:
    sage: L.hardy_z_function(.5+2.1*I)
Expected:
    -0.0064317917686980...
Got:
    -0.00643179176869423 - 3.93820653320271e-19*I
**********************************************************************
1 items had failures:
   2 of  17 in __main__.example_4
***Test Failed*** 2 failures.
```



---

archive/issue_events_098945.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2011-09-15T12:15:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8621#event-98945"
}
```



---

archive/issue_events_098946.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2011-09-15T12:15:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8621#event-98946"
}
```



---

archive/issue_comments_070865.json:
```json
{
    "body": "fixing the numerical noise",
    "created_at": "2011-09-15T22:23:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70865",
    "user": "https://github.com/kiwifb"
}
```

fixing the numerical noise



---

archive/issue_events_098947.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2011-09-15T22:25:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8621#event-98947"
}
```



---

archive/issue_events_098948.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2011-09-15T22:25:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8621#event-98948"
}
```



---

archive/issue_comments_070866.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nAttachment: **[trac_8621-review.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac_8621-review.patch.gz)**\n\nI am attaching a patch to deal with this.",
    "created_at": "2011-09-15T22:25:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70866",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:8'>Comment 8:</a>
Attachment: **[trac_8621-review.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac_8621-review.patch.gz)**

I am attaching a patch to deal with this.



---

archive/issue_comments_070867.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -4,5 +4,5 @@\n \n ---\n \n-Apply [attachment: trac_8621.2.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac_8621.2.patch.gz) to the Sage library.\n+Apply [ and [attachment: trac_8621-review.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac_8621-review.patch.gz)(https://github.com/sagemath/sage/files/ticket8621/d056703d8570b46eedd4b9bebad1e158.gz) (in that order) to the Sage library.\n \n``````\n",
    "created_at": "2011-09-15T22:25:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70867",
    "user": "https://github.com/kiwifb"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -4,5 +4,5 @@
 
 ---
 
-Apply [attachment: trac_8621.2.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac_8621.2.patch.gz) to the Sage library.
+Apply [ and [attachment: trac_8621-review.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac_8621-review.patch.gz)(https://github.com/sagemath/sage/files/ticket8621/d056703d8570b46eedd4b9bebad1e158.gz) (in that order) to the Sage library.
 
``````




---

archive/issue_comments_070868.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nThe documentation formatting can be improved, see [http://sagemath.org/doc/developer/conventions.html#documentation-strings](http://sagemath.org/doc/developer/conventions.html#documentation-strings)\n\nThere should be an example for the \"rotate\" option.\n\nAlso, you should probably change the `EllipticCurve.analytic_rank()` function to use the `compute_rank` library call if `algoritm=rubenstein`.",
    "created_at": "2011-10-15T13:03:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70868",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:9'>Comment 9:</a>
The documentation formatting can be improved, see [http://sagemath.org/doc/developer/conventions.html#documentation-strings](http://sagemath.org/doc/developer/conventions.html#documentation-strings)

There should be an example for the "rotate" option.

Also, you should probably change the `EllipticCurve.analytic_rank()` function to use the `compute_rank` library call if `algoritm=rubenstein`.



---

archive/issue_events_098949.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-10-15T13:03:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8621#event-98949"
}
```



---

archive/issue_events_098950.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-10-15T13:03:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8621#event-98950"
}
```



---

archive/issue_comments_070869.json:
```json
{
    "body": "Attachment: **[trac8621.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621.patch.gz)**",
    "created_at": "2012-04-15T20:00:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70869",
    "user": "https://github.com/rishikesha"
}
```

Attachment: **[trac8621.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621.patch.gz)**



---

archive/issue_comments_070870.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nI have attached a patch which corrects some horrible typos, and uses the standard definition of hardy Z function. This builds on patches which came earlier. Only the last patch is needed.",
    "created_at": "2012-04-15T20:02:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70870",
    "user": "https://github.com/rishikesha"
}
```

<a id='comment:10'>Comment 10:</a>
I have attached a patch which corrects some horrible typos, and uses the standard definition of hardy Z function. This builds on patches which came earlier. Only the last patch is needed.



---

archive/issue_comments_070871.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -4,5 +4,4 @@\n \n ---\n \n-Apply [ and [attachment: trac_8621-review.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac_8621-review.patch.gz)(https://github.com/sagemath/sage/files/ticket8621/d056703d8570b46eedd4b9bebad1e158.gz) (in that order) to the Sage library.\n-\n+Apply [attachment: trac8621.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621.patch.gz)  \n``````\n",
    "created_at": "2012-04-15T20:04:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70871",
    "user": "https://github.com/rishikesha"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -4,5 +4,4 @@
 
 ---
 
-Apply [ and [attachment: trac_8621-review.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac_8621-review.patch.gz)(https://github.com/sagemath/sage/files/ticket8621/d056703d8570b46eedd4b9bebad1e158.gz) (in that order) to the Sage library.
-
+Apply [attachment: trac8621.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621.patch.gz)  
``````




---

archive/issue_events_098951.json:
```json
{
    "actor": "https://github.com/rishikesha",
    "created_at": "2012-04-17T11:12:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8621#event-98951"
}
```



---

archive/issue_events_098952.json:
```json
{
    "actor": "https://github.com/rishikesha",
    "created_at": "2012-04-17T11:12:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8621#event-98952"
}
```



---

archive/issue_comments_070872.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nTo address Joroen's comments:\n\nEvery example in hardy_z_function is an example for rotate option.\n\nThe anaytic rank computation using Dokchitser's program is superior to lcalc's.",
    "created_at": "2012-04-17T11:12:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70872",
    "user": "https://github.com/rishikesha"
}
```

<a id='comment:12'>Comment 12:</a>
To address Joroen's comments:

Every example in hardy_z_function is an example for rotate option.

The anaytic rank computation using Dokchitser's program is superior to lcalc's.



---

archive/issue_comments_070873.json:
```json
{
    "body": "Attachment: **[trac8621_review.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621_review.patch.gz)**",
    "created_at": "2012-05-29T04:32:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70873",
    "user": "https://github.com/sagetrac-bober"
}
```

Attachment: **[trac8621_review.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621_review.patch.gz)**



---

archive/issue_comments_070874.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -4,4 +4,5 @@\n \n ---\n \n-Apply [attachment: trac8621.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621.patch.gz)  \n+Apply [attachment: trac8621.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621.patch.gz)\n+Apply [attachment: trac8621_review.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621_review.patch.gz)\n``````\n",
    "created_at": "2012-05-29T04:33:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70874",
    "user": "https://github.com/sagetrac-bober"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -4,4 +4,5 @@
 
 ---
 
-Apply [attachment: trac8621.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621.patch.gz)  
+Apply [attachment: trac8621.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621.patch.gz)
+Apply [attachment: trac8621_review.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621_review.patch.gz)
``````




---

archive/issue_comments_070875.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -3,6 +3,7 @@\n compute_rank and hardy_z_function. This patch depends on #5396.\n \n ---\n+Apply:\n \n-Apply [attachment: trac8621.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621.patch.gz)\n-Apply [attachment: trac8621_review.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621_review.patch.gz)\n+* [attachment: trac8621.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621.patch.gz)\n+* [attachment: trac8621_review.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621_review.patch.gz)\n``````\n",
    "created_at": "2012-05-29T04:35:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70875",
    "user": "https://github.com/sagetrac-bober"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -3,6 +3,7 @@
 compute_rank and hardy_z_function. This patch depends on #5396.
 
 ---
+Apply:
 
-Apply [attachment: trac8621.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621.patch.gz)
-Apply [attachment: trac8621_review.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621_review.patch.gz)
+* [attachment: trac8621.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621.patch.gz)
+* [attachment: trac8621_review.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621_review.patch.gz)
``````




---

archive/issue_comments_070876.json:
```json
{
    "body": "<a id='comment:15'>Comment 15:</a>\nReplying to [bober](#comment%3A14):\n\nDear patchbot,\n\nApply: [[attachment: trac8621_review.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621_review.patch.gz)(https://github.com/sagemath/sage/files/ticket8621/71c94dedb1afeb9d510123057b83264f.gz)",
    "created_at": "2012-05-29T04:38:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70876",
    "user": "https://github.com/sagetrac-bober"
}
```

<a id='comment:15'>Comment 15:</a>
Replying to [bober](#comment%3A14):

Dear patchbot,

Apply: [[attachment: trac8621_review.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621_review.patch.gz)(https://github.com/sagemath/sage/files/ticket8621/71c94dedb1afeb9d510123057b83264f.gz)



---

archive/issue_comments_070877.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nPlease fill in your real name as Author.",
    "created_at": "2012-07-27T20:42:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70877",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:16'>Comment 16:</a>
Please fill in your real name as Author.



---

archive/issue_comments_070878.json:
```json
{
    "body": "<a id='comment:17'>Comment 17:</a>\nReplying to [@jdemeyer](#comment%3A16):\n> Please fill in your real name as Author.\n\nIs that for me or Jonathan? My real name is indeed \"Rishikesh\". Jonathan should also be added as an author since he really improved the documentation more than I did.",
    "created_at": "2012-07-27T21:25:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70878",
    "user": "https://github.com/rishikesha"
}
```

<a id='comment:17'>Comment 17:</a>
Replying to [@jdemeyer](#comment%3A16):
> Please fill in your real name as Author.

Is that for me or Jonathan? My real name is indeed "Rishikesh". Jonathan should also be added as an author since he really improved the documentation more than I did.



---

archive/issue_comments_070879.json:
```json
{
    "body": "<a id='comment:18'>Comment 18:</a>\nReplying to [@rishikesha](#comment%3A17):\n> Is that for me or Jonathan? My real name is indeed \"Rishikesh\".\n\nReally?  So it seems that I am \"culturally challenged\" in thinking that names should always consist of at least two parts: first name and last name.  Just for curiosity: where do you come from?",
    "created_at": "2012-10-05T19:38:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70879",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:18'>Comment 18:</a>
Replying to [@rishikesha](#comment%3A17):
> Is that for me or Jonathan? My real name is indeed "Rishikesh".

Really?  So it seems that I am "culturally challenged" in thinking that names should always consist of at least two parts: first name and last name.  Just for curiosity: where do you come from?



---

archive/issue_comments_070880.json:
```json
{
    "body": "<a id='comment:19'>Comment 19:</a>\nIn any case: Jonathan and Rishikesh, could you review each other's patches?  These patches almost got merged and it would be a pity to waste the work.",
    "created_at": "2012-10-05T19:41:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70880",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:19'>Comment 19:</a>
In any case: Jonathan and Rishikesh, could you review each other's patches?  These patches almost got merged and it would be a pity to waste the work.



---

archive/issue_comments_070881.json:
```json
{
    "body": "replace previous (rebased for 5.11.rc1)",
    "created_at": "2013-08-11T16:17:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70881",
    "user": "https://github.com/JohnCremona"
}
```

replace previous (rebased for 5.11.rc1)



---

archive/issue_comments_070882.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -6,4 +6,4 @@\n Apply:\n \n * [attachment: trac8621.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621.patch.gz)\n-* [attachment: trac8621_review.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621_review.patch.gz)\n+* [attachment: trac8621_review_rebase.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621_review_rebase.patch.gz)\n``````\n",
    "created_at": "2013-08-11T16:18:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70882",
    "user": "https://github.com/JohnCremona"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -6,4 +6,4 @@
 Apply:
 
 * [attachment: trac8621.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621.patch.gz)
-* [attachment: trac8621_review.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621_review.patch.gz)
+* [attachment: trac8621_review_rebase.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621_review_rebase.patch.gz)
``````




---

archive/issue_comments_070883.json:
```json
{
    "body": "<a id='comment:20'>Comment 20:</a>\nAttachment: **[trac8621_review_rebase.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621_review_rebase.patch.gz)**\n\nI replaced the review patch with one which applies to 5.11.rc1.  The only change was in the index entry to the reference manual, which has since been rearranged.",
    "created_at": "2013-08-11T16:18:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70883",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:20'>Comment 20:</a>
Attachment: **[trac8621_review_rebase.patch.gz](https://github.com/sagemath/sage/files/ticket8621/trac8621_review_rebase.patch.gz)**

I replaced the review patch with one which applies to 5.11.rc1.  The only change was in the index entry to the reference manual, which has since been rearranged.



---

archive/issue_events_098953.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8621#event-98953"
}
```



---

archive/issue_events_098954.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8621#event-98954"
}
```



---

archive/issue_comments_070884.json:
```json
{
    "body": "<a id='comment:22'>Comment 22:</a>\napply trac8621.patch trac8621_review_rebase.patch",
    "created_at": "2013-08-21T09:32:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70884",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:22'>Comment 22:</a>
apply trac8621.patch trac8621_review_rebase.patch



---

archive/issue_comments_070885.json:
```json
{
    "body": "<a id='comment:23'>Comment 23:</a>\nPing ? This ticket looks good to go, can somebody confirm ?",
    "created_at": "2013-10-23T18:30:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70885",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:23'>Comment 23:</a>
Ping ? This ticket looks good to go, can somebody confirm ?



---

archive/issue_comments_070886.json:
```json
{
    "body": "<a id='comment:24'>Comment 24:</a>\nHello ! What about this one ?",
    "created_at": "2013-11-25T20:02:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70886",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:24'>Comment 24:</a>
Hello ! What about this one ?



---

archive/issue_comments_070887.json:
```json
{
    "body": "<a id='comment:25'>Comment 25:</a>\nReplying to [@fchapoton](#comment%3A24):\n> Hello ! What about this one ? \n\nI will have a look at it again and get back by Thursday.",
    "created_at": "2013-11-26T06:04:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70887",
    "user": "https://github.com/rishikesha"
}
```

<a id='comment:25'>Comment 25:</a>
Replying to [@fchapoton](#comment%3A24):
> Hello ! What about this one ? 

I will have a look at it again and get back by Thursday.



---

archive/issue_comments_070888.json:
```json
{
    "body": "<a id='comment:26'>Comment 26:</a>\nFor what it's worth: all ok on the buildbots...",
    "created_at": "2013-12-05T08:03:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70888",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:26'>Comment 26:</a>
For what it's worth: all ok on the buildbots...



---

archive/issue_comments_070889.json:
```json
{
    "body": "Changed keywords from **** to **lcalc**",
    "created_at": "2014-01-06T15:49:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70889",
    "user": "https://github.com/fchapoton"
}
```

Changed keywords from **** to **lcalc**



---

archive/issue_comments_070890.json:
```json
{
    "body": "<a id='comment:28'>Comment 28:</a>\nI have made a git branch. Needs review.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6b6c0e19702a225db1393dd8fdef641cad348da5\">6b6c0e1</a></td><td><code>This is modified to redefine hardy z function. corrected some typos</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5d0c7f60b7ff2f90dc9b53bb19dd399c67b5f143\">5d0c7f6</a></td><td><code>Review patch on top of Rishi's wrapper changes to significantly improve the</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1c387d6cb01f6a378fd53fe0af1217a15211e1cb\">1c387d6</a></td><td><code>trac #8621 just minor details</code></td></tr></table>\n",
    "created_at": "2014-01-12T20:20:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70890",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:28'>Comment 28:</a>
I have made a git branch. Needs review.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6b6c0e19702a225db1393dd8fdef641cad348da5">6b6c0e1</a></td><td><code>This is modified to redefine hardy z function. corrected some typos</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5d0c7f60b7ff2f90dc9b53bb19dd399c67b5f143">5d0c7f6</a></td><td><code>Review patch on top of Rishi's wrapper changes to significantly improve the</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1c387d6cb01f6a378fd53fe0af1217a15211e1cb">1c387d6</a></td><td><code>trac #8621 just minor details</code></td></tr></table>




---

archive/issue_comments_070891.json:
```json
{
    "body": "Commit: **[1c387d6](https://github.com/sagemath/sagetrac-mirror/commit/1c387d6cb01f6a378fd53fe0af1217a15211e1cb)**",
    "created_at": "2014-01-12T20:20:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70891",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[1c387d6](https://github.com/sagemath/sagetrac-mirror/commit/1c387d6cb01f6a378fd53fe0af1217a15211e1cb)**



---

archive/issue_comments_070892.json:
```json
{
    "body": "Branch: **[u/chapoton/8621](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/8621)**",
    "created_at": "2014-01-12T20:20:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70892",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/8621](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/8621)**



---

archive/issue_events_098955.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2014-01-17T10:42:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8621#event-98955"
}
```



---

archive/issue_events_098956.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2014-01-17T10:42:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8621#event-98956"
}
```



---

archive/issue_comments_070893.json:
```json
{
    "body": "<a id='comment:29'>Comment 29:</a>\nPositive review.  I have tested this by itself and also checked that when this branch is used to run a copy of the lmfdb website (www.lmfdb.org) the Z-plots work.  Note that the latches on which thie review branch was based have been running on the lmfdb website for more than a year already.\n\nPlease can we have this in Sage-6.1!  It will help lmfdb development a lot.",
    "created_at": "2014-01-17T10:42:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8621#issuecomment-70893",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:29'>Comment 29:</a>
Positive review.  I have tested this by itself and also checked that when this branch is used to run a copy of the lmfdb website (www.lmfdb.org) the Z-plots work.  Note that the latches on which thie review branch was based have been running on the lmfdb website for more than a year already.

Please can we have this in Sage-6.1!  It will help lmfdb development a lot.



---

archive/issue_events_098957.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8621#event-98957"
}
```



---

archive/issue_events_098958.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8621#event-98958"
}
```



---

archive/issue_events_098959.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-02-03T22:59:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8621#event-98959"
}
```



---

archive/issue_events_098960.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "fb09d3ebf0c2998cb31e5031fac1629431ce22f0",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-02-03T22:59:12Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8621",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8621#event-98960"
}
```
