# Issue 8060: New f2c-20070816.p2.spkg now works with Open Solaris x64 as 64 bit

archive/issues_008060.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-drkirkby"
    ],
    "body": "Made SAGE64=\"yes\" also work on Opensolaris.\n\n[http://boxen.math.washington.edu/home/jsp/ports/f2c-20070816.p2.spkg](http://boxen.math.washington.edu/home/jsp/ports/f2c-20070816.p2.spkg)\n\nJaap\n\n\n\nComponent: **porting**\n\nAuthor: **Jaap Spies**\n\nReviewer: **David Kirkby**\n\nMerged: **sage-4.3.3.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8060_\n\n",
    "closed_at": "2010-02-11T15:16:23Z",
    "created_at": "2010-01-25T19:57:56Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20porting",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "New f2c-20070816.p2.spkg now works with Open Solaris x64 as 64 bit",
    "type": "issue",
    "updated_at": "2010-02-11T15:16:23Z",
    "url": "https://github.com/sagemath/sage/issues/8060",
    "user": "https://github.com/jaapspies"
}
```
Made SAGE64="yes" also work on Opensolaris.

[http://boxen.math.washington.edu/home/jsp/ports/f2c-20070816.p2.spkg](http://boxen.math.washington.edu/home/jsp/ports/f2c-20070816.p2.spkg)

Jaap



Component: **porting**

Author: **Jaap Spies**

Reviewer: **David Kirkby**

Merged: **sage-4.3.3.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/8060_





---

archive/issue_events_100219.json:
```json
{
    "actor": "https://github.com/jaapspies",
    "created_at": "2010-01-25T19:57:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8060",
    "milestone_number": null,
    "milestone_title": "sage-4.3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8060#event-100219"
}
```



---

archive/issue_events_100220.json:
```json
{
    "actor": "https://github.com/jaapspies",
    "created_at": "2010-01-25T19:57:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8060",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20porting",
    "label_color": "000080",
    "label_name": "c: porting",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8060#event-100220"
}
```



---

archive/issue_events_100221.json:
```json
{
    "actor": "https://github.com/jaapspies",
    "created_at": "2010-01-25T19:57:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8060",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8060#event-100221"
}
```



---

archive/issue_events_100222.json:
```json
{
    "actor": "https://github.com/jaapspies",
    "created_at": "2010-01-25T19:57:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8060",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8060#event-100222"
}
```



---

archive/issue_events_100223.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-01-25T19:57:56Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/8060",
    "subject": "https://github.com/jaapspies",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8060#event-100223"
}
```



---

archive/issue_events_100224.json:
```json
{
    "actor": "https://github.com/jaapspies",
    "created_at": "2010-01-25T19:58:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8060",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8060#event-100224"
}
```



---

archive/issue_comments_062033.json:
```json
{
    "body": "Attachment: **[f2c-20070816.p2.patch.gz](https://github.com/sagemath/sage/files/ticket8060/f2c-20070816.p2.patch.gz)**",
    "created_at": "2010-01-26T17:33:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8060",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8060#issuecomment-62033",
    "user": "https://github.com/jaapspies"
}
```

Attachment: **[f2c-20070816.p2.patch.gz](https://github.com/sagemath/sage/files/ticket8060/f2c-20070816.p2.patch.gz)**



---

archive/issue_comments_062034.json:
```json
{
    "body": "Reviewer: **David Kirkby**",
    "created_at": "2010-01-27T13:49:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8060",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8060#issuecomment-62034",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

Reviewer: **David Kirkby**



---

archive/issue_events_100225.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-01-27T13:49:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8060",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8060#event-100225"
}
```



---

archive/issue_events_100226.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-01-27T13:49:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8060",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8060#event-100226"
}
```



---

archive/issue_comments_062035.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nI changed the title, to put x64 rather than 64-bit, as there is Open Solaris running on SPARC systems, and we have no idea if this fixes issues on SPARC or not, unless we test it. You probably do not have SPARC hardware, and I don't have Open Solaris running on SPARC, so we can't actually be sure. I suspect it will work on SPARC too, but by putting x64, we make it clear it works on x64, and is untested on SPARC.",
    "created_at": "2010-01-27T13:49:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8060",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8060#issuecomment-62035",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

<div id="comment:2" align="right">comment:2</div>

I changed the title, to put x64 rather than 64-bit, as there is Open Solaris running on SPARC systems, and we have no idea if this fixes issues on SPARC or not, unless we test it. You probably do not have SPARC hardware, and I don't have Open Solaris running on SPARC, so we can't actually be sure. I suspect it will work on SPARC too, but by putting x64, we make it clear it works on x64, and is untested on SPARC.



---

archive/issue_events_100227.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-01-27T13:49:34Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/8060",
    "title_is": "New f2c-20070816.p2.spkg now works with Open Solaris x64 as 64 bit",
    "title_was": "New f2c-20070816.p2.spkg now works with Open Solaris 64 bit",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8060#event-100227"
}
```



---

archive/issue_events_100228.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-02-11T15:16:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8060",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8060#event-100228"
}
```



---

archive/issue_events_100229.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-02-11T15:16:23Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8060",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8060#event-100229"
}
```



---

archive/issue_comments_062036.json:
```json
{
    "body": "Merged: **sage-4.3.3.alpha0**",
    "created_at": "2010-02-11T15:16:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8060",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8060#issuecomment-62036",
    "user": "https://github.com/qed777"
}
```

Merged: **sage-4.3.3.alpha0**
