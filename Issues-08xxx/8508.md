# Issue 8508: max is broken on symbolic input

archive/issues_008508.json:
```json
{
    "assignees": [
        "https://github.com/burcin"
    ],
    "body": "cf http://groups.google.com/group/sage-support/msg/55dafb49058a55c6\n\n```\nsage: var('y');\nsage: max(x,y)\nx\nsage: max(y,x)\ny\n```\nWe expect both to give `max(x,y)` of course.\n\nComponent: **calculus**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8508_\n\n",
    "closed_at": "2010-03-12T18:07:20Z",
    "created_at": "2010-03-12T17:19:56Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20calculus",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "max is broken on symbolic input",
    "type": "issue",
    "updated_at": "2010-03-12T22:10:08Z",
    "url": "https://github.com/sagemath/sage/issues/8508",
    "user": "https://github.com/zimmermann6"
}
```
cf http://groups.google.com/group/sage-support/msg/55dafb49058a55c6

```
sage: var('y');
sage: max(x,y)
x
sage: max(y,x)
y
```
We expect both to give `max(x,y)` of course.

Component: **calculus**

_Issue created by migration from https://trac.sagemath.org/ticket/8508_





---

archive/issue_events_106913.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2010-03-12T17:19:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8508",
    "milestone_number": null,
    "milestone_title": "sage-4.3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8508#event-106913"
}
```



---

archive/issue_events_106914.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2010-03-12T17:19:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8508",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20calculus",
    "label_color": "0000ff",
    "label_name": "c: calculus",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8508#event-106914"
}
```



---

archive/issue_events_106915.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2010-03-12T17:19:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8508",
    "label": "https://github.com/sagemath/sage/labels/p%3A%202%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p: 2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8508#event-106915"
}
```



---

archive/issue_events_106916.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2010-03-12T17:19:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8508",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8508#event-106916"
}
```



---

archive/issue_events_106917.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2010-03-12T17:19:56Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/8508",
    "subject": "https://github.com/zimmermann6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8508#event-106917"
}
```



---

archive/issue_events_106918.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2010-03-12T18:07:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8508",
    "label": "https://github.com/sagemath/sage/labels/p%3A%202%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p: 2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8508#event-106918"
}
```



---

archive/issue_events_106919.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2010-03-12T18:07:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8508",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8508#event-106919"
}
```



---

archive/issue_events_106920.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2010-03-12T18:07:20Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8508",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8508#event-106920"
}
```



---

archive/issue_comments_067860.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nThis is a duplicate of #6949.",
    "created_at": "2010-03-12T18:07:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8508",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8508#issuecomment-67860",
    "user": "https://github.com/burcin"
}
```

<div id="comment:1" align="right">comment:1</div>

This is a duplicate of #6949.



---

archive/issue_events_106921.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-03-12T22:10:08Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8508",
    "milestone_number": null,
    "milestone_title": "sage-4.3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8508#event-106921"
}
```
