# Issue 8064: New iml-1.0.1.p12.spkg works with Open Solaris 64 bit

archive/issues_008064.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-drkirkby"
    ],
    "body": "Make iml-1.0.1.p12.spkg install on Open Solaris 64 bit\n\nThe spkg is here:\n[http://boxen.math.washington.edu/home/jsp/ports/iml-1.0.1.p12.spkg](http://boxen.math.washington.edu/home/jsp/ports/iml-1.0.1.p12.spkg)\n\nJaap\n\n\n\nCC:  @sagetrac-drkirkby @williamstein @malb\n\nReviewer: **David Kirkby**\n\nAuthor: **Jaap Spies**\n\nMerged: **sage-4.3.3.alpha0**\n\nComponent: **porting**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8064_\n\n",
    "closed_at": "2010-02-11T15:17:11Z",
    "created_at": "2010-01-25T22:19:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20porting",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "New iml-1.0.1.p12.spkg works with Open Solaris 64 bit",
    "type": "issue",
    "updated_at": "2023-01-08T09:04:11Z",
    "url": "https://github.com/sagemath/sage/issues/8064",
    "user": "https://github.com/jaapspies"
}
```
Make iml-1.0.1.p12.spkg install on Open Solaris 64 bit

The spkg is here:
[http://boxen.math.washington.edu/home/jsp/ports/iml-1.0.1.p12.spkg](http://boxen.math.washington.edu/home/jsp/ports/iml-1.0.1.p12.spkg)

Jaap



CC:  @sagetrac-drkirkby @williamstein @malb

Reviewer: **David Kirkby**

Author: **Jaap Spies**

Merged: **sage-4.3.3.alpha0**

Component: **porting**

_Issue created by migration from https://trac.sagemath.org/ticket/8064_





---

archive/issue_events_100265.json:
```json
{
    "actor": "https://github.com/jaapspies",
    "created_at": "2010-01-25T22:19:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8064",
    "milestone_number": null,
    "milestone_title": "sage-4.3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8064#event-100265"
}
```



---

archive/issue_events_100266.json:
```json
{
    "actor": "https://github.com/jaapspies",
    "created_at": "2010-01-25T22:19:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8064",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20porting",
    "label_color": "000080",
    "label_name": "c: porting",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8064#event-100266"
}
```



---

archive/issue_events_100267.json:
```json
{
    "actor": "https://github.com/jaapspies",
    "created_at": "2010-01-25T22:19:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8064",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8064#event-100267"
}
```



---

archive/issue_events_100268.json:
```json
{
    "actor": "https://github.com/jaapspies",
    "created_at": "2010-01-25T22:19:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8064",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8064#event-100268"
}
```



---

archive/issue_events_100269.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-01-25T22:19:26Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/8064",
    "subject": "https://github.com/jaapspies",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8064#event-100269"
}
```



---

archive/issue_events_100270.json:
```json
{
    "actor": "https://github.com/jaapspies",
    "created_at": "2010-01-25T22:20:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8064",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8064#event-100270"
}
```



---

archive/issue_comments_062052.json:
```json
{
    "body": "Attachment: **[iml-1.0.1.p12.patch.gz](https://github.com/sagemath/sage/files/ticket8064/iml-1.0.1.p12.patch.gz)**",
    "created_at": "2010-01-26T18:25:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8064",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8064#issuecomment-62052",
    "user": "https://github.com/jaapspies"
}
```

Attachment: **[iml-1.0.1.p12.patch.gz](https://github.com/sagemath/sage/files/ticket8064/iml-1.0.1.p12.patch.gz)**



---

archive/issue_events_100271.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-01-27T13:26:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8064",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8064#event-100271"
}
```



---

archive/issue_events_100272.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-01-27T13:26:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8064",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8064#event-100272"
}
```



---

archive/issue_comments_062053.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThere's not a lot of point writing in 'SPKG.txt' that 'updating SPKG.txt'! \n\nBut I can see you have removed  Michael Abshoff as as maintainer, which is the correct thing to do. Ideally you would have wrote 'Removing Michael Abshoff as a maintainer as agree on ticket #7738\n\nIt would also be desirable if you created a ticket for the defect before fixing it. Then in SPKG.txt you can put the ticket number you are fixing. \n\nBut these are very minor points, to bear in mind for the future",
    "created_at": "2010-01-27T13:26:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8064",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8064#issuecomment-62053",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

<div id="comment:2" align="right">comment:2</div>

There's not a lot of point writing in 'SPKG.txt' that 'updating SPKG.txt'! 

But I can see you have removed  Michael Abshoff as as maintainer, which is the correct thing to do. Ideally you would have wrote 'Removing Michael Abshoff as a maintainer as agree on ticket #7738

It would also be desirable if you created a ticket for the defect before fixing it. Then in SPKG.txt you can put the ticket number you are fixing. 

But these are very minor points, to bear in mind for the future



---

archive/issue_comments_062054.json:
```json
{
    "body": "Reviewer: **David Kirkby**",
    "created_at": "2010-01-27T13:26:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8064",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8064#issuecomment-62054",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

Reviewer: **David Kirkby**



---

archive/issue_comments_062055.json:
```json
{
    "body": "Merged: **sage-4.3.3.alpha0**",
    "created_at": "2010-02-11T15:17:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8064",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8064#issuecomment-62055",
    "user": "https://github.com/qed777"
}
```

Merged: **sage-4.3.3.alpha0**



---

archive/issue_events_100273.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-02-11T15:17:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8064",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8064#event-100273"
}
```



---

archive/issue_events_100274.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-02-11T15:17:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8064",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8064#event-100274"
}
```
