# Issue 8561: Implement PicklableFunction(interactive_function)

archive/issues_008561.json:
```json
{
    "body": "Extend sage.misc.fpickle.pyx with a PicklableFunction class wrapping\ninteractively defined (simple) functions to make them picklable:\n\n```\n    sage: f = lambda x: x^2\n    sage: loads(dumps(f))\n    ------------------------------------------------------------\n    Traceback (most recent call last):\n      File \"<ipython console>\", line 1, in <module>\n      File \"sage_object.pyx\", line 792, in sage.structure.sage_object.dumps (sage/structure/sage_object.c:8357)\n    PicklingError: Can't pickle <type 'function'>: attribute lookup __builtin__.function failed\n\n    sage: f = PicklableFunction(f)\n    sage: f(3)\n    9\n    sage: f == loads(dumps(f))\n    True\n```\n\n\nAssignee: @nthiery\n\nCC:  sage-combinat\n\nKeywords: Pickling, lambda, interactively defined functions\n\nStatus: new\n\nIssue created by migration from https://trac.sagemath.org/ticket/8561\n\n",
    "created_at": "2010-03-19T15:52:59Z",
    "labels": [
        "component: pickling"
    ],
    "title": "Implement PicklableFunction(interactive_function)",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/8561",
    "user": "https://github.com/nthiery"
}
```
Extend sage.misc.fpickle.pyx with a PicklableFunction class wrapping
interactively defined (simple) functions to make them picklable:

```
    sage: f = lambda x: x^2
    sage: loads(dumps(f))
    ------------------------------------------------------------
    Traceback (most recent call last):
      File "<ipython console>", line 1, in <module>
      File "sage_object.pyx", line 792, in sage.structure.sage_object.dumps (sage/structure/sage_object.c:8357)
    PicklingError: Can't pickle <type 'function'>: attribute lookup __builtin__.function failed

    sage: f = PicklableFunction(f)
    sage: f(3)
    9
    sage: f == loads(dumps(f))
    True
```


Assignee: @nthiery

CC:  sage-combinat

Keywords: Pickling, lambda, interactively defined functions

Status: new

Issue created by migration from https://trac.sagemath.org/ticket/8561





---

archive/issue_comments_080344.json:
```json
{
    "body": "Changing assignee from @williamstein to @nthiery.",
    "created_at": "2010-03-19T15:53:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8561#issuecomment-80344",
    "user": "https://github.com/nthiery"
}
```

Changing assignee from @williamstein to @nthiery.



---

archive/issue_comments_080345.json:
```json
{
    "body": "<a id='comment:2'></a>See #11845 for some code in this direction and for warnings why this should never be allowed in \"standard\" pickles.",
    "created_at": "2013-01-09T23:45:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8561#issuecomment-80345",
    "user": "https://github.com/nbruin"
}
```

<a id='comment:2'></a>See #11845 for some code in this direction and for warnings why this should never be allowed in "standard" pickles.
