# Issue 8670: Provide projections methods for word paths

archive/issues_008670.json:
```json
{
    "body": "Nice mathematical objects can be obtained when projecting appropriately a discrete path (Rauzy fractals for instance).\n\nThis patch introduces 3 projection functions for word path. It also adds 2 functions to `WordMorphism` and 2 matrix rotation functions to `sage/matrix/constructor.py`.\n\nThe first 1000 points of the Rauzy fractal :\n\n```\n    sage: s = WordMorphism('1->12,2->13,3->1')\n    sage: D = s.fixed_point('1')\n    sage: v = s.pisot_vector()\n    sage: P = WordPaths('123',[(1,0,0),(0,1,0),(0,0,1)])\n    sage: w = P(D[:1000])\n    sage: w.projected_plot(v)\n```\nSee more examples in doctests.\n\n\n**Assignee:** @seblabbe\n\n**CC:**  abmasse @videlec @robertwb tjolivet tmonteil\n\n**Author:** S\u00e9bastien Labb\u00e9\n\n**Reviewer:** Franco Saliola, Timo Jolivet\n\n**Merged:** sage-4.7.alpha2\n\nIssue created by migration from https://trac.sagemath.org/ticket/8670\n\n",
    "closed_at": "2011-03-17T19:22:34Z",
    "created_at": "2010-04-11T12:10:06Z",
    "labels": [
        "component: combinatorics",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7",
    "title": "Provide projections methods for word paths",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/8670",
    "user": "https://github.com/seblabbe"
}
```
Nice mathematical objects can be obtained when projecting appropriately a discrete path (Rauzy fractals for instance).

This patch introduces 3 projection functions for word path. It also adds 2 functions to `WordMorphism` and 2 matrix rotation functions to `sage/matrix/constructor.py`.

The first 1000 points of the Rauzy fractal :

```
    sage: s = WordMorphism('1->12,2->13,3->1')
    sage: D = s.fixed_point('1')
    sage: v = s.pisot_vector()
    sage: P = WordPaths('123',[(1,0,0),(0,1,0),(0,0,1)])
    sage: w = P(D[:1000])
    sage: w.projected_plot(v)
```
See more examples in doctests.


**Assignee:** @seblabbe

**CC:**  abmasse @videlec @robertwb tjolivet tmonteil

**Author:** Sébastien Labbé

**Reviewer:** Franco Saliola, Timo Jolivet

**Merged:** sage-4.7.alpha2

Issue created by migration from https://trac.sagemath.org/ticket/8670





---

archive/issue_events_061921.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2010-04-11T13:08:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8670#event-61921"
}
```



---

archive/issue_events_061922.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2010-04-13T11:44:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8670#event-61922"
}
```



---

archive/issue_events_061923.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2010-04-13T11:44:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8670#event-61923"
}
```



---

archive/issue_comments_071577.json:
```json
{
    "body": "<a id='comment:2'></a>\nThere are some limit case problems :\n\n```\nsage: from sage.plot.plot3d.transform import *\nsage: rotate_vector_on_axis((1,0,0), 0)\nTraceback (most recent call last):\n...\nZeroDivisionError: Rational division by zero\n```\n\nWill post a new patch soon.",
    "created_at": "2010-04-13T11:44:16Z",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8670#issuecomment-71577",
    "user": "https://github.com/seblabbe"
}
```

<a id='comment:2'></a>
There are some limit case problems :

```
sage: from sage.plot.plot3d.transform import *
sage: rotate_vector_on_axis((1,0,0), 0)
Traceback (most recent call last):
...
ZeroDivisionError: Rational division by zero
```

Will post a new patch soon.



---

archive/issue_comments_071578.json:
```json
{
    "body": "<a id='comment:3'></a>\nI fixed the above problem in the updated patch. Needs review!\n\nSince I am adding two functions to the file `sage/plot/plot3d/transform.pyx`, I am also adding Robert Bradshaw in cc of this ticket. He might have some comments to share.",
    "created_at": "2010-04-18T19:39:15Z",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8670#issuecomment-71578",
    "user": "https://github.com/seblabbe"
}
```

<a id='comment:3'></a>
I fixed the above problem in the updated patch. Needs review!

Since I am adding two functions to the file `sage/plot/plot3d/transform.pyx`, I am also adding Robert Bradshaw in cc of this ticket. He might have some comments to share.



---

archive/issue_events_061924.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2010-04-18T19:39:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8670#event-61924"
}
```



---

archive/issue_events_061925.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2010-04-18T19:39:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8670#event-61925"
}
```



---

archive/issue_comments_071579.json:
```json
{
    "body": "Does not depend on any known patch. Applies on 4.3.4.",
    "created_at": "2010-04-27T15:58:48Z",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8670#issuecomment-71579",
    "user": "https://github.com/seblabbe"
}
```

Does not depend on any known patch. Applies on 4.3.4.



---

archive/attachments_011238.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_8670-word-path-projection-sl.patch",
    "asset_url": "tarball://root/attachments/ticket8670/trac_8670-word-path-projection-sl.patch",
    "created_at": "2010-06-23T16:04:26Z",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket8670/trac_8670-word-path-projection-sl.patch",
    "user": "https://github.com/saliola"
}
```



---

archive/issue_comments_071580.json:
```json
{
    "body": "<a id='comment:4'></a>\n**Attachment:** [trac_8670-word-path-projection-sl.patch](https://github.com/sagemath/sage/files/ticket8670/trac_8670-word-path-projection-sl.patch)\n\n1. Your definition of Pisot eigenvector is ambiguous.\n\n```\nReturns the left eigenvector of the incidence matrix associated\nto the largest eigenvalue (in absolute value).\n```\nIt is possible that there are multiple eigenvalues with the same absolute value:\n\n```\nsage: mu = WordMorphism('a->c,b->c,c->ab')\nsage: m = matrix(mu); m\n[0 0 1]\n[0 0 1]\n[1 1 0]\nsage: m.eigenvalues()\n[0, -1.414213562373095?, 1.414213562373095?]\n```\nIt is not clear which eigenvector gets returned here.\n\nAlso, there may be more than one eigenvector associated to your \"maximal\" eigenvalue, and your method only returns one eigenvector:\n\n```\nsage: mu = WordMorphism('a->a,b->b,c->abc')\nsage: mu.pisot_eigenvector_left()\n(1, -1, 0)\nsage: m = matrix(mu); m\n[1 0 1]\n[0 1 1]\n[0 0 1]\nsage: m.eigenspaces_left()\n[\n(1, Vector space of degree 3 and dimension 2 over Rational Field\nUser basis matrix:\n[ 1 -1  0]\n[ 0  0  1])\n]\n```\n\n2. The method `directive_vector` should include a definition of what the directive vector is.\n\n3. In your functions `rotate_ith_to_zero` and `rotate_vector_on_axis`, you first construct the matrix and then coerce it into the ring specified by the user. Why is this preferable to doing the computations directly in the ring? What about making this an option `compute_in_ring=False`? I think all you need to do is add the following line at the beginning:\n\n```\nif compute_in_ring is True:\n    v = vector(ring, v)\n```",
    "created_at": "2010-06-23T16:04:26Z",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8670#issuecomment-71580",
    "user": "https://github.com/saliola"
}
```

<a id='comment:4'></a>
**Attachment:** [trac_8670-word-path-projection-sl.patch](https://github.com/sagemath/sage/files/ticket8670/trac_8670-word-path-projection-sl.patch)

1. Your definition of Pisot eigenvector is ambiguous.

```
Returns the left eigenvector of the incidence matrix associated
to the largest eigenvalue (in absolute value).
```
It is possible that there are multiple eigenvalues with the same absolute value:

```
sage: mu = WordMorphism('a->c,b->c,c->ab')
sage: m = matrix(mu); m
[0 0 1]
[0 0 1]
[1 1 0]
sage: m.eigenvalues()
[0, -1.414213562373095?, 1.414213562373095?]
```
It is not clear which eigenvector gets returned here.

Also, there may be more than one eigenvector associated to your "maximal" eigenvalue, and your method only returns one eigenvector:

```
sage: mu = WordMorphism('a->a,b->b,c->abc')
sage: mu.pisot_eigenvector_left()
(1, -1, 0)
sage: m = matrix(mu); m
[1 0 1]
[0 1 1]
[0 0 1]
sage: m.eigenspaces_left()
[
(1, Vector space of degree 3 and dimension 2 over Rational Field
User basis matrix:
[ 1 -1  0]
[ 0  0  1])
]
```

2. The method `directive_vector` should include a definition of what the directive vector is.

3. In your functions `rotate_ith_to_zero` and `rotate_vector_on_axis`, you first construct the matrix and then coerce it into the ring specified by the user. Why is this preferable to doing the computations directly in the ring? What about making this an option `compute_in_ring=False`? I think all you need to do is add the following line at the beginning:

```
if compute_in_ring is True:
    v = vector(ring, v)
```



---

archive/issue_events_061926.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2010-06-23T16:04:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8670#event-61926"
}
```



---

archive/issue_events_061927.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2010-06-23T16:04:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8670#event-61927"
}
```



---

archive/issue_events_061928.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2010-10-15T21:00:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8670#event-61928"
}
```



---

archive/issue_events_061929.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2010-10-15T21:00:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8670#event-61929"
}
```



---

archive/issue_comments_071581.json:
```json
{
    "body": "<a id='comment:5'></a>\nNeeds review again!",
    "created_at": "2010-10-15T21:00:21Z",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8670#issuecomment-71581",
    "user": "https://github.com/seblabbe"
}
```

<a id='comment:5'></a>
Needs review again!



---

archive/attachments_011239.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_8670-review-sl.patch",
    "asset_url": "tarball://root/attachments/ticket8670/trac_8670-review-sl.patch",
    "created_at": "2010-10-15T22:05:42Z",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket8670/trac_8670-review-sl.patch",
    "user": "https://github.com/seblabbe"
}
```



---

archive/issue_comments_071582.json:
```json
{
    "body": "**Attachment:** [trac_8670-review-sl.patch](https://github.com/sagemath/sage/files/ticket8670/trac_8670-review-sl.patch)\n\nApplies over the precedent patch",
    "created_at": "2010-10-15T22:05:42Z",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8670#issuecomment-71582",
    "user": "https://github.com/seblabbe"
}
```

**Attachment:** [trac_8670-review-sl.patch](https://github.com/sagemath/sage/files/ticket8670/trac_8670-review-sl.patch)

Applies over the precedent patch



---

archive/issue_events_061930.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/tjolivet",
    "created_at": "2011-01-28T17:56:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8670#event-61930"
}
```



---

archive/issue_events_061931.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/tjolivet",
    "created_at": "2011-01-28T17:56:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8670#event-61931"
}
```



---

archive/issue_comments_071583.json:
```json
{
    "body": "<a id='comment:8'></a>\nHi,\n\nHere are a few remarks:\n\n(1) There is a doctest failure:\n\n  `File \"/home/timo/sage-4.6.1/devel/sage-trac_8670/sage/plot/plot3d/transform.pyx\", line 325:    sage: rotate_vector_on_axis(v, 0, ring=RealField(10)) * vExpected:    (5.5, 0.0020, 0.0020, 0.00)Got:    (5.5, 0.00098, 0.00098, 0.00)`\n\nThis should be fixed by using \"`...`\", and could be done for the other occurrences of printed floats.\n\n(2) I think that the rotations matrices should be in a file in sage/matrix/. I remember having looked for something like `rotate_arbitrary`, but I didn't find it because it was in plot3d/ and not in matrix/.\n\n(3) The documentation for `rotate_vector_on_axis` could be a little bit clearer (the description).\n\nOtherwise this is a nice and useful patch.",
    "created_at": "2011-01-28T17:56:02Z",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8670#issuecomment-71583",
    "user": "https://trac.sagemath.org/admin/accounts/users/tjolivet"
}
```

<a id='comment:8'></a>
Hi,

Here are a few remarks:

(1) There is a doctest failure:

  `File "/home/timo/sage-4.6.1/devel/sage-trac_8670/sage/plot/plot3d/transform.pyx", line 325:    sage: rotate_vector_on_axis(v, 0, ring=RealField(10)) * vExpected:    (5.5, 0.0020, 0.0020, 0.00)Got:    (5.5, 0.00098, 0.00098, 0.00)`

This should be fixed by using "`...`", and could be done for the other occurrences of printed floats.

(2) I think that the rotations matrices should be in a file in sage/matrix/. I remember having looked for something like `rotate_arbitrary`, but I didn't find it because it was in plot3d/ and not in matrix/.

(3) The documentation for `rotate_vector_on_axis` could be a little bit clearer (the description).

Otherwise this is a nice and useful patch.



---

archive/issue_comments_071584.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -12,7 +12,5 @@\n     sage: w = P(D[:1000])\n     sage: w.projected_plot(v)\n ```\n-\n See more examples in doctests.\n \n-\n``````\n",
    "created_at": "2011-01-28T17:56:02Z",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8670#issuecomment-71584",
    "user": "https://trac.sagemath.org/admin/accounts/users/tjolivet"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -12,7 +12,5 @@
     sage: w = P(D[:1000])
     sage: w.projected_plot(v)
 ```
-
 See more examples in doctests.
 
-
``````




---

archive/attachments_011240.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_8670_second_corrections-sl.patch",
    "asset_url": "tarball://root/attachments/ticket8670/trac_8670_second_corrections-sl.patch",
    "created_at": "2011-01-29T13:24:26Z",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket8670/trac_8670_second_corrections-sl.patch",
    "user": "https://github.com/seblabbe"
}
```



---

archive/issue_comments_071585.json:
```json
{
    "body": "**Attachment:** [trac_8670_second_corrections-sl.patch](https://github.com/sagemath/sage/files/ticket8670/trac_8670_second_corrections-sl.patch)\n\nApplies over the precedent 2 patches",
    "created_at": "2011-01-29T13:24:26Z",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8670#issuecomment-71585",
    "user": "https://github.com/seblabbe"
}
```

**Attachment:** [trac_8670_second_corrections-sl.patch](https://github.com/sagemath/sage/files/ticket8670/trac_8670_second_corrections-sl.patch)

Applies over the precedent 2 patches



---

archive/issue_comments_071586.json:
```json
{
    "body": "<a id='comment:9'></a>\nThanks for the comments. I moved the two rotation matrix constructor to the file `sage/matrix/constructor.py`. I improved the docstrings and fixed the doctest failure (I was having the same).\n\nNeeds review.",
    "created_at": "2011-01-29T13:28:21Z",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8670#issuecomment-71586",
    "user": "https://github.com/seblabbe"
}
```

<a id='comment:9'></a>
Thanks for the comments. I moved the two rotation matrix constructor to the file `sage/matrix/constructor.py`. I improved the docstrings and fixed the doctest failure (I was having the same).

Needs review.



---

archive/issue_events_061932.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2011-01-29T13:28:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8670#event-61932"
}
```



---

archive/issue_events_061933.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2011-01-29T13:28:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8670#event-61933"
}
```



---

archive/issue_comments_071587.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,6 +1,6 @@\n Nice mathematical objects can be obtained when projecting appropriately a discrete path (Rauzy fractals for instance).\n \n-This patch introduces 3 projection functions for word path. It also adds 1 function to `WordMorphism` and 2 matrix rotation functions to `sage/plot/plot3d/transforms.pyx`.\n+This patch introduces 3 projection functions for word path. It also adds 2 functions to `WordMorphism` and 2 matrix rotation functions to `sage/matrix/constructor.py`.\n \n The first 1000 points of the Rauzy fractal :\n \n``````\n",
    "created_at": "2011-01-29T13:30:05Z",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8670#issuecomment-71587",
    "user": "https://github.com/seblabbe"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,6 +1,6 @@
 Nice mathematical objects can be obtained when projecting appropriately a discrete path (Rauzy fractals for instance).
 
-This patch introduces 3 projection functions for word path. It also adds 1 function to `WordMorphism` and 2 matrix rotation functions to `sage/plot/plot3d/transforms.pyx`.
+This patch introduces 3 projection functions for word path. It also adds 2 functions to `WordMorphism` and 2 matrix rotation functions to `sage/matrix/constructor.py`.
 
 The first 1000 points of the Rauzy fractal :
 
``````




---

archive/attachments_011241.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_8670_folded-sl.patch",
    "asset_url": "tarball://root/attachments/ticket8670/trac_8670_folded-sl.patch",
    "created_at": "2011-01-30T00:42:40Z",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket8670/trac_8670_folded-sl.patch",
    "user": "https://github.com/seblabbe"
}
```



---

archive/issue_comments_071588.json:
```json
{
    "body": "**Attachment:** [trac_8670_folded-sl.patch](https://github.com/sagemath/sage/files/ticket8670/trac_8670_folded-sl.patch)\n\nApply only this one.",
    "created_at": "2011-01-30T00:42:40Z",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8670#issuecomment-71588",
    "user": "https://github.com/seblabbe"
}
```

**Attachment:** [trac_8670_folded-sl.patch](https://github.com/sagemath/sage/files/ticket8670/trac_8670_folded-sl.patch)

Apply only this one.



---

archive/issue_comments_071589.json:
```json
{
    "body": "<a id='comment:11'></a>\nFor the patchbot :\n\nApply trac_8670_folded-sl.patch",
    "created_at": "2011-01-30T00:43:23Z",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8670#issuecomment-71589",
    "user": "https://github.com/seblabbe"
}
```

<a id='comment:11'></a>
For the patchbot :

Apply trac_8670_folded-sl.patch



---

archive/issue_events_061934.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/tjolivet",
    "created_at": "2011-02-07T12:36:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8670#event-61934"
}
```



---

archive/issue_events_061935.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/tjolivet",
    "created_at": "2011-02-07T12:36:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8670#event-61935"
}
```



---

archive/issue_comments_071590.json:
```json
{
    "body": "**Reviewer:** Franco Saliola, Timo Jolivet",
    "created_at": "2011-02-07T12:36:27Z",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8670#issuecomment-71590",
    "user": "https://trac.sagemath.org/admin/accounts/users/tjolivet"
}
```

**Reviewer:** Franco Saliola, Timo Jolivet



---

archive/issue_comments_071591.json:
```json
{
    "body": "**Author:** S\u00e9bastien Labb\u00e9",
    "created_at": "2011-02-07T12:36:27Z",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8670#issuecomment-71591",
    "user": "https://trac.sagemath.org/admin/accounts/users/tjolivet"
}
```

**Author:** Sébastien Labbé



---

archive/issue_events_061936.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-02-07T20:20:28Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "milestone": "sage-4.6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8670#event-61936"
}
```



---

archive/issue_events_061937.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-02-07T20:20:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "milestone": "sage-4.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8670#event-61937"
}
```



---

archive/issue_comments_071592.json:
```json
{
    "body": "**Merged:** sage-4.7.alpha2",
    "created_at": "2011-03-17T19:22:34Z",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8670#issuecomment-71592",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-4.7.alpha2



---

archive/issue_events_061938.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-17T19:22:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8670#event-61938"
}
```



---

archive/issue_events_061939.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-17T19:22:34Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8670",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8670#event-61939"
}
```
