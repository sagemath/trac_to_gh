# Issue 8639: Bug when defining a MIPVariable's  type

archive/issues_008639.json:
```json
{
    "assignees": [],
    "body": "A missing '_' in a variable's name :-p\n\nNathann\n\nAssignee: **@wdjoyner**\n\nAuthor: **Nathann Cohen**\n\nReviewer: **David Joyner**\n\nMerged: **sage-4.4.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8639_\n\n",
    "closed_at": "2010-04-16T18:55:52Z",
    "created_at": "2010-04-01T14:06:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20numerical",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Bug when defining a MIPVariable's  type",
    "type": "issue",
    "updated_at": "2010-04-16T18:55:52Z",
    "url": "https://github.com/sagemath/sage/issues/8639",
    "user": "https://github.com/nathanncohen"
}
```
A missing '_' in a variable's name :-p

Nathann

Assignee: **@wdjoyner**

Author: **Nathann Cohen**

Reviewer: **David Joyner**

Merged: **sage-4.4.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/8639_





---

archive/issue_events_099166.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-04-01T14:06:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8639",
    "milestone_number": null,
    "milestone_title": "sage-4.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8639#event-99166"
}
```



---

archive/issue_events_099167.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-04-01T14:06:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8639",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20numerical",
    "label_color": "0000ff",
    "label_name": "component: numerical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8639#event-99167"
}
```



---

archive/issue_events_099168.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-04-01T14:06:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8639",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8639#event-99168"
}
```



---

archive/issue_events_099169.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-04-01T14:06:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8639",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8639#event-99169"
}
```



---

archive/issue_comments_071063.json:
```json
{
    "body": "Attachment: **[trac_8639.patch.gz](https://github.com/sagemath/sage/files/ticket8639/trac_8639.patch.gz)**",
    "created_at": "2010-04-01T14:07:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8639",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8639#issuecomment-71063",
    "user": "https://github.com/nathanncohen"
}
```

Attachment: **[trac_8639.patch.gz](https://github.com/sagemath/sage/files/ticket8639/trac_8639.patch.gz)**



---

archive/issue_comments_071064.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n-The variable's name was changed.... :-p\n+A missing '_' in a variable's name :-p\n \n Nathann\n``````\n",
    "created_at": "2010-04-01T14:07:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8639",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8639#issuecomment-71064",
    "user": "https://github.com/nathanncohen"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
-The variable's name was changed.... :-p
+A missing '_' in a variable's name :-p
 
 Nathann
``````




---

archive/issue_events_099170.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-04-01T14:07:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8639",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8639#event-99170"
}
```



---

archive/issue_comments_071065.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nThis seems to pass all tests on  sage 4.3.5 on a 10.6.2 mac (the only failure was that of the fresh copy of 4.3.5).",
    "created_at": "2010-04-02T23:01:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8639",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8639#issuecomment-71065",
    "user": "https://github.com/wdjoyner"
}
```

<a id='comment:2'>Comment 2:</a>
This seems to pass all tests on  sage 4.3.5 on a 10.6.2 mac (the only failure was that of the fresh copy of 4.3.5).



---

archive/issue_comments_071066.json:
```json
{
    "body": "Changed assignee from **@jasongrout, jkantor** to **@wdjoyner**",
    "created_at": "2010-04-02T23:01:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8639",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8639#issuecomment-71066",
    "user": "https://github.com/wdjoyner"
}
```

Changed assignee from **@jasongrout, jkantor** to **@wdjoyner**



---

archive/issue_comments_071067.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nSeems like a very very minor change. As far as I can seen, positive review.",
    "created_at": "2010-04-02T23:02:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8639",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8639#issuecomment-71067",
    "user": "https://github.com/wdjoyner"
}
```

<a id='comment:3'>Comment 3:</a>
Seems like a very very minor change. As far as I can seen, positive review.



---

archive/issue_events_099171.json:
```json
{
    "actor": "https://github.com/wdjoyner",
    "created_at": "2010-04-02T23:02:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8639",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8639#event-99171"
}
```



---

archive/issue_events_099172.json:
```json
{
    "actor": "https://github.com/wdjoyner",
    "created_at": "2010-04-02T23:02:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8639",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8639#event-99172"
}
```



---

archive/issue_comments_071068.json:
```json
{
    "body": "Merged: **sage-4.4.alpha0**",
    "created_at": "2010-04-16T18:55:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8639",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8639#issuecomment-71068",
    "user": "https://github.com/jhpalmieri"
}
```

Merged: **sage-4.4.alpha0**



---

archive/issue_comments_071069.json:
```json
{
    "body": "Author: **Nathann Cohen**",
    "created_at": "2010-04-16T18:55:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8639",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8639#issuecomment-71069",
    "user": "https://github.com/jhpalmieri"
}
```

Author: **Nathann Cohen**



---

archive/issue_comments_071070.json:
```json
{
    "body": "Reviewer: **David Joyner**",
    "created_at": "2010-04-16T18:55:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8639",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8639#issuecomment-71070",
    "user": "https://github.com/jhpalmieri"
}
```

Reviewer: **David Joyner**



---

archive/issue_comments_071071.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nMerged \"trac_8639.patch\" in 4.4.alpha0.",
    "created_at": "2010-04-16T18:55:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8639",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8639#issuecomment-71071",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:4'>Comment 4:</a>
Merged "trac_8639.patch" in 4.4.alpha0.



---

archive/issue_events_099173.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-04-16T18:55:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8639",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8639#event-99173"
}
```



---

archive/issue_events_099174.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-04-16T18:55:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8639",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8639#event-99174"
}
```
