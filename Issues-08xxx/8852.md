# Issue 8852: minor doctest fix in fast_eval.pyx for Cygwin

archive/issues_008852.json:
```json
{
    "assignees": [],
    "body": "\n\nAuthor: **Mike Hansen**\n\nReviewer: **William Stein**\n\nMerged: **sage-4.4.3.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8852_\n\n",
    "closed_at": "2010-05-25T02:19:11Z",
    "created_at": "2010-05-03T12:27:37Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.4.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "minor doctest fix in fast_eval.pyx for Cygwin",
    "type": "issue",
    "updated_at": "2010-05-25T15:59:13Z",
    "url": "https://github.com/sagemath/sage/issues/8852",
    "user": "https://github.com/mwhansen"
}
```


Author: **Mike Hansen**

Reviewer: **William Stein**

Merged: **sage-4.4.3.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/8852_





---

archive/issue_events_098946.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-05-03T12:27:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8852",
    "milestone_number": null,
    "milestone_title": "sage-4.4.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8852#event-98946"
}
```



---

archive/issue_events_098947.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-05-03T12:27:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8852",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
    "label_color": "000080",
    "label_name": "component: porting: cygwin",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8852#event-98947"
}
```



---

archive/issue_events_098948.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-05-03T12:27:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8852",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8852#event-98948"
}
```



---

archive/issue_events_098949.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-05-03T12:27:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8852",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8852#event-98949"
}
```



---

archive/issue_comments_074200.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[trac_8852-fix_fast_eval.patch.gz](https://github.com/sagemath/sage/files/ticket8852/trac_8852-fix_fast_eval.patch.gz)**",
    "created_at": "2010-05-03T13:17:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8852#issuecomment-74200",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[trac_8852-fix_fast_eval.patch.gz](https://github.com/sagemath/sage/files/ticket8852/trac_8852-fix_fast_eval.patch.gz)**



---

archive/issue_events_098950.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-05-03T13:17:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8852",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8852#event-98950"
}
```



---

archive/issue_events_098951.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-05-25T02:19:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8852",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8852#event-98951"
}
```



---

archive/issue_events_098952.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-05-25T02:19:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8852",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8852#event-98952"
}
```



---

archive/issue_events_098953.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-05-25T02:19:11Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8852",
    "milestone_number": null,
    "milestone_title": "sage-4.4.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8852#event-98953"
}
```



---

archive/issue_comments_074201.json:
```json
{
    "body": "Reviewer: **wstein**",
    "created_at": "2010-05-25T02:19:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8852#issuecomment-74201",
    "user": "https://github.com/williamstein"
}
```

Reviewer: **wstein**



---

archive/issue_comments_074202.json:
```json
{
    "body": "Merged: **4.4.3.alpha0**",
    "created_at": "2010-05-25T02:19:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8852#issuecomment-74202",
    "user": "https://github.com/williamstein"
}
```

Merged: **4.4.3.alpha0**



---

archive/issue_comments_074203.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nThis seems reasonably harmless. It's the result of getting 2.0000...0004 on cygwin.",
    "created_at": "2010-05-25T02:19:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8852#issuecomment-74203",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:2'>Comment 2:</a>
This seems reasonably harmless. It's the result of getting 2.0000...0004 on cygwin.



---

archive/issue_comments_074204.json:
```json
{
    "body": "Changed merged from **4.4.3.alpha0** to **sage-4.4.3.alpha0**",
    "created_at": "2010-05-25T15:59:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8852#issuecomment-74204",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Changed merged from **4.4.3.alpha0** to **sage-4.4.3.alpha0**



---

archive/issue_comments_074205.json:
```json
{
    "body": "Changed reviewer from **wstein** to **William Stein**",
    "created_at": "2010-05-25T15:59:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8852",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8852#issuecomment-74205",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Changed reviewer from **wstein** to **William Stein**
