# Issue 8256: The documentation of Misc is a mess

archive/issues_008256.json:
```json
{
    "body": "The documentation of the misc directory is a mess. We should sort and organize the file `misc.rst`.\n\n**Assignee:** mvngu\n\n**Keywords:** misc documentation\n\n**Reviewer:** Jeroen Demeyer\n\nIssue created by migration from https://trac.sagemath.org/ticket/8256\n\n",
    "closed_at": "2015-07-17T20:06:10Z",
    "created_at": "2010-02-13T16:51:49Z",
    "labels": [
        "component: documentation",
        "bug",
        "invalid"
    ],
    "title": "The documentation of Misc is a mess",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/8256",
    "user": "https://github.com/hivert"
}
```
The documentation of the misc directory is a mess. We should sort and organize the file `misc.rst`.

**Assignee:** mvngu

**Keywords:** misc documentation

**Reviewer:** Jeroen Demeyer

Issue created by migration from https://trac.sagemath.org/ticket/8256





---

archive/issue_events_057808.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-02-13T17:07:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/8256",
    "milestone": "sage-4.3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8256#event-57808"
}
```



---

archive/issue_comments_065926.json:
```json
{
    "body": "<a id='comment:2'></a>\nThis is ... really vague.  If someone doesn't put something a little more concrete here, maybe we should close it - not because the doc is better, of course.",
    "created_at": "2013-01-25T15:40:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8256",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8256#issuecomment-65926",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:2'></a>
This is ... really vague.  If someone doesn't put something a little more concrete here, maybe we should close it - not because the doc is better, of course.



---

archive/issue_events_057809.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/8256",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8256#event-57809"
}
```



---

archive/issue_events_057810.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/8256",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8256#event-57810"
}
```



---

archive/issue_events_057811.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/8256",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8256#event-57811"
}
```



---

archive/issue_events_057812.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/8256",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8256#event-57812"
}
```



---

archive/issue_events_057813.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/8256",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8256#event-57813"
}
```



---

archive/issue_events_057814.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/8256",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8256#event-57814"
}
```



---

archive/issue_events_057815.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/8256",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8256#event-57815"
}
```



---

archive/issue_events_057816.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/8256",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8256#event-57816"
}
```



---

archive/issue_comments_065927.json:
```json
{
    "body": "<a id='comment:7'></a>\nDuplicate of #17932.",
    "created_at": "2015-07-07T14:18:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8256",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8256#issuecomment-65927",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:7'></a>
Duplicate of #17932.



---

archive/issue_comments_065928.json:
```json
{
    "body": "**Reviewer:** Jeroen Demeyer",
    "created_at": "2015-07-07T14:18:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8256",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8256#issuecomment-65928",
    "user": "https://github.com/jdemeyer"
}
```

**Reviewer:** Jeroen Demeyer



---

archive/issue_events_057817.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-07-07T14:18:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/8256",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8256#event-57817"
}
```



---

archive/issue_events_057818.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-07-07T14:18:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/8256",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8256#event-57818"
}
```



---

archive/issue_events_057819.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-07-07T14:19:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/8256",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8256#event-57819"
}
```



---

archive/issue_events_057820.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-07-07T14:19:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/8256",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8256#event-57820"
}
```



---

archive/issue_events_057821.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-07-17T20:06:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/8256",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8256#event-57821"
}
```



---

archive/issue_events_057822.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-07-17T20:06:10Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/8256",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8256#event-57822"
}
```
