# Issue 8673: No KeyErrror raised when it should for FiniteWord_callable

archive/issues_008673.json:
```json
{
    "assignees": [
        "https://github.com/seblabbe"
    ],
    "body": "\n```\nLe 10-02-23, Paul Zimmermann a \u00e9crit :\n\nsage: def f(n):\n   return n^2\n\nsage: w = Word(f,length=23)\n\nsage: w[24]\n576\n\nPaul\n```\n\nCC:  @sagetrac-abmasse\n\nComponent: **combinatorics**\n\nAuthor: **S\u00e9bastien Labb\u00e9**\n\nReviewer: **Alexandre Blondin Mass\u00e9**\n\nMerged: **sage-4.7.alpha3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8673_\n\n",
    "closed_at": "2011-03-25T12:31:06Z",
    "created_at": "2010-04-11T14:00:57Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "No KeyErrror raised when it should for FiniteWord_callable",
    "type": "issue",
    "updated_at": "2011-03-25T12:31:06Z",
    "url": "https://github.com/sagemath/sage/issues/8673",
    "user": "https://github.com/seblabbe"
}
```

```
Le 10-02-23, Paul Zimmermann a écrit :

sage: def f(n):
   return n^2

sage: w = Word(f,length=23)

sage: w[24]
576

Paul
```

CC:  @sagetrac-abmasse

Component: **combinatorics**

Author: **Sébastien Labbé**

Reviewer: **Alexandre Blondin Massé**

Merged: **sage-4.7.alpha3**

_Issue created by migration from https://trac.sagemath.org/ticket/8673_





---

archive/issue_events_109457.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2010-04-11T14:00:57Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8673",
    "milestone_number": null,
    "milestone_title": "sage-4.6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8673#event-109457"
}
```



---

archive/issue_events_109458.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2010-04-11T14:00:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8673",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "c: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8673#event-109458"
}
```



---

archive/issue_events_109459.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2010-04-11T14:00:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8673",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8673#event-109459"
}
```



---

archive/issue_events_109460.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2010-04-11T14:00:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8673",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8673#event-109460"
}
```



---

archive/issue_events_109461.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2010-04-11T14:00:57Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/8673",
    "subject": "https://github.com/seblabbe",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8673#event-109461"
}
```



---

archive/issue_comments_069843.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_8673_out_of_range_index-sl.patch.gz](https://github.com/sagemath/sage/files/ticket8673/trac_8673_out_of_range_index-sl.patch.gz)**",
    "created_at": "2011-02-18T19:57:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8673#issuecomment-69843",
    "user": "https://github.com/seblabbe"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_8673_out_of_range_index-sl.patch.gz](https://github.com/sagemath/sage/files/ticket8673/trac_8673_out_of_range_index-sl.patch.gz)**



---

archive/issue_events_109462.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2011-02-18T19:57:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8673",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8673#event-109462"
}
```



---

archive/issue_comments_069844.json:
```json
{
    "body": "Reviewer: **Alexandre Blondin Mass\u00e9**",
    "created_at": "2011-02-18T20:38:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8673#issuecomment-69844",
    "user": "https://github.com/sagetrac-abmasse"
}
```

Reviewer: **Alexandre Blondin Massé**



---

archive/issue_comments_069845.json:
```json
{
    "body": "Author: **S\u00e9bastien Labb\u00e9**",
    "created_at": "2011-02-18T20:38:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8673#issuecomment-69845",
    "user": "https://github.com/sagetrac-abmasse"
}
```

Author: **Sébastien Labbé**



---

archive/issue_events_109463.json:
```json
{
    "actor": "https://github.com/sagetrac-abmasse",
    "created_at": "2011-02-18T20:38:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8673",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8673#event-109463"
}
```



---

archive/issue_events_109464.json:
```json
{
    "actor": "https://github.com/sagetrac-abmasse",
    "created_at": "2011-02-18T20:38:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8673",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8673#event-109464"
}
```



---

archive/issue_comments_069846.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nPretty straight-forward. All tests pass! Positive review.",
    "created_at": "2011-02-18T20:38:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8673#issuecomment-69846",
    "user": "https://github.com/sagetrac-abmasse"
}
```

<div id="comment:2" align="right">comment:2</div>

Pretty straight-forward. All tests pass! Positive review.



---

archive/issue_events_109465.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-02-19T11:32:40Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8673",
    "milestone_number": null,
    "milestone_title": "sage-4.6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8673#event-109465"
}
```



---

archive/issue_events_109466.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-02-19T11:32:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8673",
    "milestone_number": null,
    "milestone_title": "sage-4.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8673#event-109466"
}
```



---

archive/issue_comments_069847.json:
```json
{
    "body": "Merged: **sage-4.7.alpha3**",
    "created_at": "2011-03-25T12:31:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8673#issuecomment-69847",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.7.alpha3**



---

archive/issue_events_109467.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-25T12:31:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8673",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8673#event-109467"
}
```



---

archive/issue_events_109468.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-25T12:31:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8673",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8673#event-109468"
}
```
