# Issue 8891: sage -t doesn't accept . as current directory

archive/issues_008891.json:
```json
{
    "assignees": [],
    "body": "\n\nAuthor: **Mike Hansen**\n\nReviewer: **Mitesh Patel**\n\nMerged: **sage-4.5.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8891_\n\n",
    "closed_at": "2010-06-25T15:42:54Z",
    "created_at": "2010-05-05T16:31:01Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/doctest%20coverage",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sage -t doesn't accept . as current directory",
    "type": "issue",
    "updated_at": "2010-06-25T15:42:54Z",
    "url": "https://github.com/sagemath/sage/issues/8891",
    "user": "https://github.com/sagetrac-wjlaffin"
}
```


Author: **Mike Hansen**

Reviewer: **Mitesh Patel**

Merged: **sage-4.5.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/8891_





---

archive/issue_events_099403.json:
```json
{
    "actor": "https://github.com/sagetrac-wjlaffin",
    "created_at": "2010-05-05T16:31:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8891",
    "milestone_number": null,
    "milestone_title": "sage-4.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8891#event-99403"
}
```



---

archive/issue_events_099404.json:
```json
{
    "actor": "https://github.com/sagetrac-wjlaffin",
    "created_at": "2010-05-05T16:31:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8891",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8891#event-99404"
}
```



---

archive/issue_events_099405.json:
```json
{
    "actor": "https://github.com/sagetrac-wjlaffin",
    "created_at": "2010-05-05T16:31:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8891",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8891#event-99405"
}
```



---

archive/issue_events_099406.json:
```json
{
    "actor": "https://github.com/sagetrac-wjlaffin",
    "created_at": "2010-05-05T16:31:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8891",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8891#event-99406"
}
```



---

archive/issue_comments_074586.json:
```json
{
    "body": "Author: **Mike Hansen**",
    "created_at": "2010-05-05T16:48:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8891",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8891#issuecomment-74586",
    "user": "https://github.com/mwhansen"
}
```

Author: **Mike Hansen**



---

archive/issue_comments_074587.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[trac_8891.patch.gz](https://github.com/sagemath/sage/files/ticket8891/trac_8891.patch.gz)**\n\nIt was explicitly disabled long ago (before Trac), but I don't see any reason why it should be now.",
    "created_at": "2010-05-05T16:48:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8891",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8891#issuecomment-74587",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[trac_8891.patch.gz](https://github.com/sagemath/sage/files/ticket8891/trac_8891.patch.gz)**

It was explicitly disabled long ago (before Trac), but I don't see any reason why it should be now.



---

archive/issue_events_099407.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-05-05T16:48:11Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8891",
    "milestone_number": null,
    "milestone_title": "sage-4.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8891#event-99407"
}
```



---

archive/issue_events_099408.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-05-05T16:48:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8891",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8891#event-99408"
}
```



---

archive/issue_comments_074588.json:
```json
{
    "body": "Reviewer: **Mitesh Patel**",
    "created_at": "2010-06-12T08:56:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8891",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8891#issuecomment-74588",
    "user": "https://github.com/qed777"
}
```

Reviewer: **Mitesh Patel**



---

archive/issue_events_099409.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-06-12T08:56:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8891",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8891#event-99409"
}
```



---

archive/issue_events_099410.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-06-12T08:56:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8891",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8891#event-99410"
}
```



---

archive/issue_comments_074589.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nWithout the patch, I get, e.g.,\n\n```sh\nmpatel@sage monoids$ sage -t .\n \n----------------------------------------------------------------------\nAll tests passed!\nTotal time for all tests: 0.0 seconds\nmpatel@sage monoids$ \n```\nWith the patch, I get\n\n```sh\nmpatel@sage monoids$ sage -t .\nsage -t  \"./string_monoid_element.py\"                       \n         [2.1 s]\n[...]\n```",
    "created_at": "2010-06-12T08:56:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8891",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8891#issuecomment-74589",
    "user": "https://github.com/qed777"
}
```

<a id='comment:2'>Comment 2:</a>
Without the patch, I get, e.g.,

```sh
mpatel@sage monoids$ sage -t .
 
----------------------------------------------------------------------
All tests passed!
Total time for all tests: 0.0 seconds
mpatel@sage monoids$ 
```
With the patch, I get

```sh
mpatel@sage monoids$ sage -t .
sage -t  "./string_monoid_element.py"                       
         [2.1 s]
[...]
```



---

archive/issue_comments_074590.json:
```json
{
    "body": "Merged: **sage-4.5.alpha0**",
    "created_at": "2010-06-25T15:42:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8891",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8891#issuecomment-74590",
    "user": "https://github.com/rlmill"
}
```

Merged: **sage-4.5.alpha0**



---

archive/issue_events_099411.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-06-25T15:42:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8891",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8891#event-99411"
}
```



---

archive/issue_events_099412.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-06-25T15:42:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8891",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8891#event-99412"
}
```
