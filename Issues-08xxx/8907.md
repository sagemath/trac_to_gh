# Issue 8907: build Python with --enabled-shared

archive/issues_008907.json:
```json
{
    "assignees": [],
    "body": "This is needed by #8542 .  There is an spkg at http://sage.math.washington.edu/home/mhansen/python-2.6.4.p8.spkg but it requires lots of testing.  Also, the changes are not committed to the repository.\n\nAssignee: **@aghitza**\n\nAuthor: **Mike Hansen**\n\nReviewer: **William Stein**\n\nMerged: **sage-4.4.3.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8907_\n\n",
    "closed_at": "2010-05-26T00:44:22Z",
    "created_at": "2010-05-06T17:10:55Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.4.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "build Python with --enabled-shared",
    "type": "issue",
    "updated_at": "2010-05-27T04:34:48Z",
    "url": "https://github.com/sagemath/sage/issues/8907",
    "user": "https://github.com/mwhansen"
}
```
This is needed by #8542 .  There is an spkg at http://sage.math.washington.edu/home/mhansen/python-2.6.4.p8.spkg but it requires lots of testing.  Also, the changes are not committed to the repository.

Assignee: **@aghitza**

Author: **Mike Hansen**

Reviewer: **William Stein**

Merged: **sage-4.4.3.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/8907_





---

archive/issue_events_102915.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-05-06T17:10:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8907#event-102915"
}
```



---

archive/issue_events_102916.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-05-06T17:10:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8907#event-102916"
}
```



---

archive/issue_events_102917.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-05-06T17:10:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8907#event-102917"
}
```



---

archive/issue_events_102918.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-05-06T17:11:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "milestone_number": null,
    "milestone_title": "sage-4.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8907#event-102918"
}
```



---

archive/issue_comments_074856.json:
```json
{
    "body": "Author: **Mike Hansen**",
    "created_at": "2010-05-06T17:11:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8907#issuecomment-74856",
    "user": "https://github.com/mwhansen"
}
```

Author: **Mike Hansen**



---

archive/issue_events_102919.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-05-06T17:11:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8907#event-102919"
}
```



---

archive/issue_events_102920.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-05-06T17:11:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
    "label_color": "000080",
    "label_name": "component: porting: cygwin",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8907#event-102920"
}
```



---

archive/issue_events_102921.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-05-06T17:11:26Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "title_is": "build Python with --enabled-shared",
    "title_was": "build Python as a shared library",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8907#event-102921"
}
```



---

archive/issue_events_102922.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-05-06T17:11:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8907#event-102922"
}
```



---

archive/issue_comments_074857.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nCheck on OS X, we don't need --enable-shared.  I'm check on t2 now.  The spkg will have to be updated accordingly.",
    "created_at": "2010-05-06T17:57:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8907#issuecomment-74857",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:2" align="right">Comment 2</div>

Check on OS X, we don't need --enable-shared.  I'm check on t2 now.  The spkg will have to be updated accordingly.



---

archive/issue_comments_074858.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\n--enabled-shared fails on t2 because of http://bugs.python.org/issue1628484 .  We need to apply the patch there.",
    "created_at": "2010-05-25T05:47:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8907#issuecomment-74858",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:3" align="right">Comment 3</div>

--enabled-shared fails on t2 because of http://bugs.python.org/issue1628484 .  We need to apply the patch there.



---

archive/issue_events_102923.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-05-26T00:44:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8907#event-102923"
}
```



---

archive/issue_events_102924.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-05-26T00:44:22Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8907#event-102924"
}
```



---

archive/issue_comments_074859.json:
```json
{
    "body": "Merged: **4.4.3.alpha0**",
    "created_at": "2010-05-26T00:44:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8907#issuecomment-74859",
    "user": "https://github.com/williamstein"
}
```

Merged: **4.4.3.alpha0**



---

archive/issue_comments_074860.json:
```json
{
    "body": "Reviewer: **wstein**",
    "created_at": "2010-05-26T00:44:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8907#issuecomment-74860",
    "user": "https://github.com/williamstein"
}
```

Reviewer: **wstein**



---

archive/issue_comments_074861.json:
```json
{
    "body": "Changed merged from **4.4.3.alpha0** to **sage-4.4.3.alpha0**",
    "created_at": "2010-05-27T04:34:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8907#issuecomment-74861",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Changed merged from **4.4.3.alpha0** to **sage-4.4.3.alpha0**



---

archive/issue_comments_074862.json:
```json
{
    "body": "Changed reviewer from **wstein** to **William Stein**",
    "created_at": "2010-05-27T04:34:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8907",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8907#issuecomment-74862",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Changed reviewer from **wstein** to **William Stein**
