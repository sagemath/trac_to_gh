# Issue 8760: change sage notebook to use HTML5 doctype, and also change the email part of registration form to actually use something from html5

archive/issues_008760.json:
```json
{
    "body": "\n\n**Assignee:** @jasongrout, @williamstein\n\n**Author:** William Stein\n\n**Reviewer:** Alex Leone\n\n**Merged:** SageNB 0.8.1\n\nIssue created by migration from https://trac.sagemath.org/ticket/8760\n\n",
    "closed_at": "2010-07-11T05:57:55Z",
    "created_at": "2010-04-25T00:07:45Z",
    "labels": [
        "component: notebook",
        "minor",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.5",
    "title": "change sage notebook to use HTML5 doctype, and also change the email part of registration form to actually use something from html5",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/8760",
    "user": "https://github.com/williamstein"
}
```


**Assignee:** @jasongrout, @williamstein

**Author:** William Stein

**Reviewer:** Alex Leone

**Merged:** SageNB 0.8.1

Issue created by migration from https://trac.sagemath.org/ticket/8760





---

archive/issue_events_062808.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-04-25T00:09:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8760",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8760#event-62808"
}
```



---

archive/attachments_010967.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_8760-sagenb.patch",
    "asset_url": "tarball://root/attachments/ticket8760/trac_8760-sagenb.patch",
    "created_at": "2010-04-25T00:09:03Z",
    "issue": "https://github.com/sagemath/sage/issues/8760",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket8760/trac_8760-sagenb.patch",
    "user": "https://github.com/williamstein"
}
```



---

archive/issue_comments_072910.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [trac_8760-sagenb.patch](https://github.com/sagemath/sage/files/ticket8760/trac_8760-sagenb.patch) by @williamstein created at 2010-04-25 00:09:03",
    "created_at": "2010-04-25T00:09:03Z",
    "issue": "https://github.com/sagemath/sage/issues/8760",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8760#issuecomment-72910",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'></a>
Attachment [trac_8760-sagenb.patch](https://github.com/sagemath/sage/files/ticket8760/trac_8760-sagenb.patch) by @williamstein created at 2010-04-25 00:09:03



---

archive/issue_events_062809.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/acleone",
    "created_at": "2010-04-25T00:36:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8760",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8760#event-62809"
}
```



---

archive/issue_events_062810.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/acleone",
    "created_at": "2010-04-25T00:36:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8760",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8760#event-62810"
}
```



---

archive/issue_comments_072911.json:
```json
{
    "body": "<a id='comment:2'></a>\nhttp://stackoverflow.com/questions/5629/any-reason-not-to-start-using-the-html-5-doctype\n\nSeems to work in IE6, so LGTM.",
    "created_at": "2010-04-25T00:36:32Z",
    "issue": "https://github.com/sagemath/sage/issues/8760",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8760#issuecomment-72911",
    "user": "https://trac.sagemath.org/admin/accounts/users/acleone"
}
```

<a id='comment:2'></a>
http://stackoverflow.com/questions/5629/any-reason-not-to-start-using-the-html-5-doctype

Seems to work in IE6, so LGTM.



---

archive/issue_comments_072912.json:
```json
{
    "body": "**Reviewer:** Alex Leone",
    "created_at": "2010-04-25T00:36:43Z",
    "issue": "https://github.com/sagemath/sage/issues/8760",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8760#issuecomment-72912",
    "user": "https://trac.sagemath.org/admin/accounts/users/acleone"
}
```

**Reviewer:** Alex Leone



---

archive/issue_comments_072913.json:
```json
{
    "body": "**Author:** William Stein",
    "created_at": "2010-06-29T16:15:01Z",
    "issue": "https://github.com/sagemath/sage/issues/8760",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8760#issuecomment-72913",
    "user": "https://github.com/loefflerd"
}
```

**Author:** William Stein



---

archive/issue_events_062811.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2010-07-11T05:57:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8760",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8760#event-62811"
}
```



---

archive/issue_events_062812.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2010-07-11T05:57:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8760",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8760#event-62812"
}
```



---

archive/issue_comments_072914.json:
```json
{
    "body": "**Merged:** SageNB 0.8.1",
    "created_at": "2010-07-11T05:57:55Z",
    "issue": "https://github.com/sagemath/sage/issues/8760",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8760#issuecomment-72914",
    "user": "https://github.com/TimDumol"
}
```

**Merged:** SageNB 0.8.1
