# Issue 8460: doctest failure sagenb-0.7.5.1-py2.6.egg/sagenb/misc/sageinspect.py

archive/issues_008460.json:
```json
{
    "assignees": [
        "https://github.com/williamstein"
    ],
    "body": "I get this failure on Solaris 10, on SPARC using a modified version of the 4.3.4.alpha0 sources. (All modifications were necessary to get Sage to build). \n\n```\n**********************************************************************\nFile \"/export/home/drkirkby/32/sage-4.3.4.alpha0/local/lib/python2.6/site-packages/sagenb-0.7.5.1-py2.6.egg/sagenb/misc/sageinspect.py\", line 562:\n    sage: sage_getsourcelines(matrix, True)[1]\nExpected:\n    33\nGot:\n    34\n**********************************************************************\n```\n\n\nI'm guessing a bit about the category for his. I'll this if I have chosen the wrong category. \n\nComponent: **notebook**\n\nWork Issues: **Dup of #8430, Fixed at #8324**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8460_\n\n",
    "closed_at": "2010-03-09T06:41:20Z",
    "created_at": "2010-03-06T22:13:05Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "doctest failure sagenb-0.7.5.1-py2.6.egg/sagenb/misc/sageinspect.py",
    "type": "issue",
    "updated_at": "2010-03-09T06:41:20Z",
    "url": "https://github.com/sagemath/sage/issues/8460",
    "user": "https://github.com/sagetrac-drkirkby"
}
```
I get this failure on Solaris 10, on SPARC using a modified version of the 4.3.4.alpha0 sources. (All modifications were necessary to get Sage to build). 

```
**********************************************************************
File "/export/home/drkirkby/32/sage-4.3.4.alpha0/local/lib/python2.6/site-packages/sagenb-0.7.5.1-py2.6.egg/sagenb/misc/sageinspect.py", line 562:
    sage: sage_getsourcelines(matrix, True)[1]
Expected:
    33
Got:
    34
**********************************************************************
```


I'm guessing a bit about the category for his. I'll this if I have chosen the wrong category. 

Component: **notebook**

Work Issues: **Dup of #8430, Fixed at #8324**

_Issue created by migration from https://trac.sagemath.org/ticket/8460_





---

archive/issue_events_106148.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-03-06T22:13:05Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8460",
    "milestone_number": null,
    "milestone_title": "sage-4.3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8460#event-106148"
}
```



---

archive/issue_events_106149.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-03-06T22:13:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8460",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "000080",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8460#event-106149"
}
```



---

archive/issue_events_106150.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-03-06T22:13:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8460",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8460#event-106150"
}
```



---

archive/issue_events_106151.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-03-06T22:13:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8460",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8460#event-106151"
}
```



---

archive/issue_events_106152.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-03-06T22:13:05Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/8460",
    "subject": "https://github.com/sagetrac-drkirkby",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8460#event-106152"
}
```



---

archive/issue_comments_067233.json:
```json
{
    "body": "Work Issues: **Dup of #8430, Fixed at #8324**",
    "created_at": "2010-03-09T06:41:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8460#issuecomment-67233",
    "user": "https://github.com/qed777"
}
```

Work Issues: **Dup of #8430, Fixed at #8324**



---

archive/issue_comments_067234.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nThis happens to be part of #8430 and the fix from #8324 is part of #8435.",
    "created_at": "2010-03-09T06:41:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8460#issuecomment-67234",
    "user": "https://github.com/qed777"
}
```

<div id="comment:1" align="right">comment:1</div>

This happens to be part of #8430 and the fix from #8324 is part of #8435.



---

archive/issue_events_106153.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-03-09T06:41:20Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8460",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8460#event-106153"
}
```



---

archive/issue_events_106154.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-03-09T06:41:20Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8460",
    "milestone_number": null,
    "milestone_title": "sage-4.3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8460#event-106154"
}
```



---

archive/issue_events_106155.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-03-09T06:41:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8460",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8460#event-106155"
}
```



---

archive/issue_events_106156.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-03-09T06:41:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8460",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8460#event-106156"
}
```
