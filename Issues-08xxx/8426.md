# Issue 8426: polynomial * constant does not work if constant is a numpy type

archive/issues_008426.json:
```json
{
    "body": "This should work:\n\n```\nimport numpy\nR.<x>=RR[]\nx*numpy.float32('23.0')\n```\n\nInstead, I get:\n\n```\n---------------------------------------------------------------------------\nTypeError                                 Traceback (most recent call last)\n\n/home/jason/tmp/<ipython console> in <module>()\n\n/home/jason/sage/local/lib/python2.6/site-packages/sage/structure/element.so in sage.structure.element.RingElement.__mul__ (sage/structure/element.c:11337)()\n\n/home/jason/sage/local/lib/python2.6/site-packages/sage/structure/coerce.so in sage.structure.coerce.CoercionModel_cache_maps.bin_op (sage/structure/coerce.c:6994)()\n\nTypeError: unsupported operand parent(s) for '*': 'Univariate Polynomial Ring in x over Real Field with 53 bits of precision' and '<type 'numpy.float32'>'\n```\n\nNote that this does work:\n\n```\nsage: numpy.float32('23.0')*x\n23.0000000000000*x\n```\n\n\n**Assignee:** @aghitza\n\n**CC:**  @williamstein @robertwb\n\n**Reviewer:** Jeroen Demeyer\n\nIssue created by migration from https://trac.sagemath.org/ticket/8426\n\n",
    "closed_at": "2015-04-23T14:52:09Z",
    "created_at": "2010-03-03T05:17:18Z",
    "labels": [
        "component: algebra",
        "bug",
        "duplicate"
    ],
    "title": "polynomial * constant does not work if constant is a numpy type",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/8426",
    "user": "https://github.com/jasongrout"
}
```
This should work:

```
import numpy
R.<x>=RR[]
x*numpy.float32('23.0')
```

Instead, I get:

```
---------------------------------------------------------------------------
TypeError                                 Traceback (most recent call last)

/home/jason/tmp/<ipython console> in <module>()

/home/jason/sage/local/lib/python2.6/site-packages/sage/structure/element.so in sage.structure.element.RingElement.__mul__ (sage/structure/element.c:11337)()

/home/jason/sage/local/lib/python2.6/site-packages/sage/structure/coerce.so in sage.structure.coerce.CoercionModel_cache_maps.bin_op (sage/structure/coerce.c:6994)()

TypeError: unsupported operand parent(s) for '*': 'Univariate Polynomial Ring in x over Real Field with 53 bits of precision' and '<type 'numpy.float32'>'
```

Note that this does work:

```
sage: numpy.float32('23.0')*x
23.0000000000000*x
```


**Assignee:** @aghitza

**CC:**  @williamstein @robertwb

**Reviewer:** Jeroen Demeyer

Issue created by migration from https://trac.sagemath.org/ticket/8426





---

archive/issue_events_060792.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/8426",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8426#event-60792"
}
```



---

archive/issue_events_060793.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/8426",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8426#event-60793"
}
```



---

archive/issue_events_060794.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/8426",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8426#event-60794"
}
```



---

archive/issue_events_060795.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/8426",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8426#event-60795"
}
```



---

archive/issue_comments_069952.json:
```json
{
    "body": "<a id='comment:3'></a>\nNow works, but see #15695.",
    "created_at": "2014-03-15T18:55:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8426",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8426#issuecomment-69952",
    "user": "https://github.com/mezzarobba"
}
```

<a id='comment:3'></a>
Now works, but see #15695.



---

archive/issue_events_060796.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/8426",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8426#event-60796"
}
```



---

archive/issue_events_060797.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/8426",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8426#event-60797"
}
```



---

archive/issue_events_060798.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/8426",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8426#event-60798"
}
```



---

archive/issue_events_060799.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/8426",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8426#event-60799"
}
```



---

archive/issue_events_060800.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-03-28T11:57:50Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/8426",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8426#event-60800"
}
```



---

archive/issue_events_060801.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-03-28T11:57:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/8426",
    "label": "duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8426#event-60801"
}
```



---

archive/issue_comments_069953.json:
```json
{
    "body": "<a id='comment:6'></a>\nHello,\n\nI propose to close this as duplicates because of #18076 that fixes it. With the branch applied I got\n\n```\nsage: import numpy\nsage: R.<x> = RR[]\nsage: x * numpy.float32('23.0')\n23.0000000000000*x\n```\nand even the pushout works\n\n```\nsage: R.<x> = ZZ[]\nsage: x * numpy.float32('23.0')\n23.0*x\nsage: parent(_)\nUnivariate Polynomial Ring in x over Real Double Field\n```\n\nVincent",
    "created_at": "2015-03-28T11:57:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8426",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8426#issuecomment-69953",
    "user": "https://github.com/videlec"
}
```

<a id='comment:6'></a>
Hello,

I propose to close this as duplicates because of #18076 that fixes it. With the branch applied I got

```
sage: import numpy
sage: R.<x> = RR[]
sage: x * numpy.float32('23.0')
23.0000000000000*x
```
and even the pushout works

```
sage: R.<x> = ZZ[]
sage: x * numpy.float32('23.0')
23.0*x
sage: parent(_)
Univariate Polynomial Ring in x over Real Double Field
```

Vincent



---

archive/issue_events_060802.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-03-28T11:57:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/8426",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8426#event-60802"
}
```



---

archive/issue_events_060803.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-23T09:40:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/8426",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8426#event-60803"
}
```



---

archive/issue_events_060804.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-23T09:40:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/8426",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8426#event-60804"
}
```



---

archive/issue_comments_069954.json:
```json
{
    "body": "**Reviewer:** Jeroen Demeyer",
    "created_at": "2015-04-23T09:40:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8426",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8426#issuecomment-69954",
    "user": "https://github.com/jdemeyer"
}
```

**Reviewer:** Jeroen Demeyer



---

archive/issue_events_060805.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-23T14:52:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/8426",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8426#event-60805"
}
```



---

archive/issue_events_060806.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-23T14:52:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/8426",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8426#event-60806"
}
```



---

archive/issue_events_060807.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-23T14:52:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/8426",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8426#event-60807"
}
```
