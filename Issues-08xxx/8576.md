# Issue 8576: Categories for QQ, CC, RR and friends

archive/issues_008576.json:
```json
{
    "assignees": [],
    "body": "After this patch, QQ,ZZ,... inherit properly from categories: \n\n```\nsage: QQ.category()\nCategory of fields\nsage: TestSuite(QQ).run()\n```\n\nThis patch also documents the following effect discovered by TestSuite:\n\n```\n    sage: CDF = ComplexDoubleField()\n    sage: x = CDF.an_element()\n    sage: x\n    1.0*I\n    sage: x*x, x**2, x*x == x**2\n    (-1.0, -1.0 + 1.22460635382e-16*I, False)\n```\nThis effect won't be touched by this patch. Should anyone consider this as a bug, please open a new ticket.\n\nAssignee: **@aghitza**\n\nCC:  @sagetrac-sage-combinat\n\nKeywords: **categories, real fields**\n\nAuthor: **Nicolas M. Thi\u00e9ry**\n\nReviewer: **John Cremona**\n\nMerged: **sage-4.4.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8576_\n\n",
    "closed_at": "2010-04-16T18:49:06Z",
    "created_at": "2010-03-22T10:16:15Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Categories for QQ, CC, RR and friends",
    "type": "issue",
    "updated_at": "2010-04-16T18:49:06Z",
    "url": "https://github.com/sagemath/sage/issues/8576",
    "user": "https://github.com/nthiery"
}
```
After this patch, QQ,ZZ,... inherit properly from categories: 

```
sage: QQ.category()
Category of fields
sage: TestSuite(QQ).run()
```

This patch also documents the following effect discovered by TestSuite:

```
    sage: CDF = ComplexDoubleField()
    sage: x = CDF.an_element()
    sage: x
    1.0*I
    sage: x*x, x**2, x*x == x**2
    (-1.0, -1.0 + 1.22460635382e-16*I, False)
```
This effect won't be touched by this patch. Should anyone consider this as a bug, please open a new ticket.

Assignee: **@aghitza**

CC:  @sagetrac-sage-combinat

Keywords: **categories, real fields**

Author: **Nicolas M. Thiéry**

Reviewer: **John Cremona**

Merged: **sage-4.4.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/8576_





---

archive/issue_events_098339.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2010-03-22T10:16:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8576",
    "milestone_number": null,
    "milestone_title": "sage-4.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8576#event-98339"
}
```



---

archive/issue_events_098340.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2010-03-22T10:16:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8576",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8576#event-98340"
}
```



---

archive/issue_events_098341.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2010-03-22T10:16:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8576",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8576#event-98341"
}
```



---

archive/issue_events_098342.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2010-03-22T10:16:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8576",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8576#event-98342"
}
```



---

archive/issue_comments_070427.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nAttachment: **[trac_8576-category-QQ-RR-CC-nt.patch.gz](https://github.com/sagemath/sage/files/ticket8576/trac_8576-category-QQ-RR-CC-nt.patch.gz)**\n\nAll test passed for me.",
    "created_at": "2010-03-22T22:25:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8576",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8576#issuecomment-70427",
    "user": "https://github.com/nthiery"
}
```

<div id="comment:1" align="right">Comment 1</div>

Attachment: **[trac_8576-category-QQ-RR-CC-nt.patch.gz](https://github.com/sagemath/sage/files/ticket8576/trac_8576-category-QQ-RR-CC-nt.patch.gz)**

All test passed for me.



---

archive/issue_events_098343.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2010-03-22T22:25:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8576",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8576#event-98343"
}
```



---

archive/issue_comments_070428.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nApplies fine to 4.3.5 and all tests pass.",
    "created_at": "2010-04-05T14:09:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8576",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8576#issuecomment-70428",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:2" align="right">Comment 2</div>

Applies fine to 4.3.5 and all tests pass.



---

archive/issue_events_098344.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2010-04-05T14:09:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8576",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8576#event-98344"
}
```



---

archive/issue_events_098345.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2010-04-05T14:09:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8576",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8576#event-98345"
}
```



---

archive/issue_comments_070429.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nThanks for the review!",
    "created_at": "2010-04-06T05:56:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8576",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8576#issuecomment-70429",
    "user": "https://github.com/nthiery"
}
```

<div id="comment:3" align="right">Comment 3</div>

Thanks for the review!



---

archive/issue_comments_070430.json:
```json
{
    "body": "Reviewer: **John Cremona**",
    "created_at": "2010-04-06T05:56:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8576",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8576#issuecomment-70430",
    "user": "https://github.com/nthiery"
}
```

Reviewer: **John Cremona**



---

archive/issue_comments_070431.json:
```json
{
    "body": "Merged: **sage-4.4.alpha0**",
    "created_at": "2010-04-16T18:49:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8576",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8576#issuecomment-70431",
    "user": "https://github.com/jhpalmieri"
}
```

Merged: **sage-4.4.alpha0**



---

archive/issue_events_098346.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-04-16T18:49:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8576",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8576#event-98346"
}
```



---

archive/issue_events_098347.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-04-16T18:49:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8576",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8576#event-98347"
}
```



---

archive/issue_comments_070432.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nMerged \"trac_8576-category-QQ-RR-CC-nt.patch\" in 4.4.alpha0",
    "created_at": "2010-04-16T18:49:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8576",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8576#issuecomment-70432",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:4" align="right">Comment 4</div>

Merged "trac_8576-category-QQ-RR-CC-nt.patch" in 4.4.alpha0
