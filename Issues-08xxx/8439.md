# Issue 8439: maxima interface hangs gcl and/or clisp backends

archive/issues_008439.json:
```json
{
    "body": "This is a possible problem when using an external maxima, that uses a backend other then ecl.\n\nclisp has some peculiar ways to figure out there is an associated pty, that is created by pty.py from pexpect interface.\n\ngcl hangs due to an \"unexcaped\" ecl specific command at the start of maxima interface; this took quite some time to find, as I was thinking it was a clisp like related issue, and tried to correct the problem in gcl and/or sage-maxima.lisp.\n\nWith the attached patch, going to be used in mandriva rpm, all currently maxima backends works in sage. (When installing sagemath, unless using --auto-select, the installation process will ask what package that provides maxima-backend the user wants, and the options are sbcl, clisp, gcl and ecl).\n\n[The first chunk of the patch is not applicable to sage, as I use a modified clisp and maxima, that checks the --disable-readline option]\n\n\nAssignee: @williamstein\n\nCC:  @orlitzky @fchapoton\n\nResolution: invalid\n\nIssue created by migration from https://trac.sagemath.org/ticket/8439\n\n",
    "closed_at": "2020-06-28T19:29:53Z",
    "created_at": "2010-03-04T22:37:40Z",
    "labels": [
        "component: interfaces",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "maxima interface hangs gcl and/or clisp backends",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/8439",
    "user": "https://trac.sagemath.org/admin/accounts/users/pcpa"
}
```
This is a possible problem when using an external maxima, that uses a backend other then ecl.

clisp has some peculiar ways to figure out there is an associated pty, that is created by pty.py from pexpect interface.

gcl hangs due to an "unexcaped" ecl specific command at the start of maxima interface; this took quite some time to find, as I was thinking it was a clisp like related issue, and tried to correct the problem in gcl and/or sage-maxima.lisp.

With the attached patch, going to be used in mandriva rpm, all currently maxima backends works in sage. (When installing sagemath, unless using --auto-select, the installation process will ask what package that provides maxima-backend the user wants, and the options are sbcl, clisp, gcl and ecl).

[The first chunk of the patch is not applicable to sage, as I use a modified clisp and maxima, that checks the --disable-readline option]


Assignee: @williamstein

CC:  @orlitzky @fchapoton

Resolution: invalid

Issue created by migration from https://trac.sagemath.org/ticket/8439





---

archive/issue_comments_089155.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -12,6 +12,9 @@\n \n With the attached patch, going to be used in mandriva rpm, all currently maxima backends works in sage. (When installing sagemath, unless using --auto-select, the installation process will ask what package that provides maxima-backend the user wants, and the options are sbcl, clisp, gcl and ecl).\n \n+[The first chunk of the patch is not applicable to sage, as I use a modified clisp and maxima, that checks the --disable-readline option]\n+\n+\n Comment: 1\n \n Priority: major\n``````\n",
    "created_at": "2010-03-04T22:40:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8439#issuecomment-89155",
    "user": "https://trac.sagemath.org/admin/accounts/users/pcpa"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -12,6 +12,9 @@
 
 With the attached patch, going to be used in mandriva rpm, all currently maxima backends works in sage. (When installing sagemath, unless using --auto-select, the installation process will ask what package that provides maxima-backend the user wants, and the options are sbcl, clisp, gcl and ecl).
 
+[The first chunk of the patch is not applicable to sage, as I use a modified clisp and maxima, that checks the --disable-readline option]
+
+
 Comment: 1
 
 Priority: major
``````




---

archive/issue_comments_089156.json:
```json
{
    "body": "<a id='comment:2'></a>I asked in http://lists.gnu.org/archive/html/gcl-devel/2010-03/msg00000.html about some possible solution for the issue of gcl backend hanging, but now I believe I found a better workaround, so that, maxima gcl backend has the same behavior as other lisps.\n\nThe problem is that adapting the solution used for clisp in `$SAGE_LOCAL/bin/sage-maxima.lisp `was not enough, as for some reason, the predicate `(output-stream-p *standard-input*) `was still true in the maxima parsing error message code.\n\nThe new patch should correct the issue, or at least, now it doesn't hang in the doctect\n\n`sage: maxima.eval('sage0: x == x;')`",
    "created_at": "2010-03-17T20:24:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8439#issuecomment-89156",
    "user": "https://trac.sagemath.org/admin/accounts/users/pcpa"
}
```

<a id='comment:2'></a>I asked in http://lists.gnu.org/archive/html/gcl-devel/2010-03/msg00000.html about some possible solution for the issue of gcl backend hanging, but now I believe I found a better workaround, so that, maxima gcl backend has the same behavior as other lisps.

The problem is that adapting the solution used for clisp in `$SAGE_LOCAL/bin/sage-maxima.lisp `was not enough, as for some reason, the predicate `(output-stream-p *standard-input*) `was still true in the maxima parsing error message code.

The new patch should correct the issue, or at least, now it doesn't hang in the doctect

`sage: maxima.eval('sage0: x == x;')`



---

archive/issue_comments_089157.json:
```json
{
    "body": "sage-4.3.3-maxima.patch",
    "created_at": "2010-03-17T20:26:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8439#issuecomment-89157",
    "user": "https://trac.sagemath.org/admin/accounts/users/pcpa"
}
```

sage-4.3.3-maxima.patch



---

archive/issue_comments_089158.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2012-01-16T02:55:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8439#issuecomment-89158",
    "user": "https://github.com/orlitzky"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_089159.json:
```json
{
    "body": "<a id='comment:3'></a>Attachment [sage-4.3.3-maxima.patch](tarball://root/attachments/some-uuid/ticket8439/sage-4.3.3-maxima.patch) by @orlitzky created at 2012-01-16 02:55:39",
    "created_at": "2012-01-16T02:55:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8439#issuecomment-89159",
    "user": "https://github.com/orlitzky"
}
```

<a id='comment:3'></a>Attachment [sage-4.3.3-maxima.patch](tarball://root/attachments/some-uuid/ticket8439/sage-4.3.3-maxima.patch) by @orlitzky created at 2012-01-16 02:55:39



---

archive/issue_comments_089160.json:
```json
{
    "body": "Changing status from needs_review to needs_info.",
    "created_at": "2012-01-28T02:34:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8439#issuecomment-89160",
    "user": "https://github.com/kcrisman"
}
```

Changing status from needs_review to needs_info.



---

archive/issue_comments_089161.json:
```json
{
    "body": "<a id='comment:4'></a>Umm... is this even valid anymore?  Putting 'needs info'.  \n\nI certainly can't believe that this would apply properly any more... and it doesn't.",
    "created_at": "2012-01-28T02:34:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8439#issuecomment-89161",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:4'></a>Umm... is this even valid anymore?  Putting 'needs info'.  

I certainly can't believe that this would apply properly any more... and it doesn't.



---

archive/issue_comments_089162.json:
```json
{
    "body": "<a id='comment:5'></a>Actually, I took a look at this when I was looking for tickets that might be fixed by maxima-5.24. Right now on my machine, it hangs:\n\n```\n$ sage\n----------------------------------------------------------------------\n----------------------------------------------------------------------\n**********************************************************************\n*                                                                    *\n* Warning: this is a prerelease version, and it may be unstable.     *\n*                                                                    *\n**********************************************************************\nLoading Sage library. Current Mercurial branch is: devel\nsage: maxima.eval('1+1;;')\n```\n| Sage Version 5.0.beta1, Release Date: 2012-01-22                   |\n| Type notebook() for the GUI, and license() for information.        |\n(I've waited a good minute.)",
    "created_at": "2012-01-28T02:57:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8439#issuecomment-89162",
    "user": "https://github.com/orlitzky"
}
```

<a id='comment:5'></a>Actually, I took a look at this when I was looking for tickets that might be fixed by maxima-5.24. Right now on my machine, it hangs:

```
$ sage
----------------------------------------------------------------------
----------------------------------------------------------------------
**********************************************************************
*                                                                    *
* Warning: this is a prerelease version, and it may be unstable.     *
*                                                                    *
**********************************************************************
Loading Sage library. Current Mercurial branch is: devel
sage: maxima.eval('1+1;;')
```
| Sage Version 5.0.beta1, Release Date: 2012-01-22                   |
| Type notebook() for the GUI, and license() for information.        |
(I've waited a good minute.)



---

archive/issue_comments_089163.json:
```json
{
    "body": "<a id='comment:6'></a>Here's what happens in Maxima.\n\n```\n\n(%i4) 1+1;;\n\n(%o4) 2\n\nstdin:76667:incorrect syntax: Premature termination of input at ;.\n;\n ^\n```\nI'm just going to make a ticket for that - it's #12372.\n\nThis is still a somewhat weird ticket.  I didn't even know we allowed other Maxima backends officially.",
    "created_at": "2012-01-28T03:04:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8439#issuecomment-89163",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:6'></a>Here's what happens in Maxima.

```

(%i4) 1+1;;

(%o4) 2

stdin:76667:incorrect syntax: Premature termination of input at ;.
;
 ^
```
I'm just going to make a ticket for that - it's #12372.

This is still a somewhat weird ticket.  I didn't even know we allowed other Maxima backends officially.



---

archive/issue_comments_089164.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,12 @@\n+This is a possible problem when using an external maxima, that uses a backend other then ecl.\n+\n+clisp has some peculiar ways to figure out there is an associated pty, that is created by pty.py from pexpect interface.\n+\n+gcl hangs due to an \"unexcaped\" ecl specific command at the start of maxima interface; this took quite some time to find, as I was thinking it was a clisp like related issue, and tried to correct the problem in gcl and/or sage-maxima.lisp.\n+\n+With the attached patch, going to be used in mandriva rpm, all currently maxima backends works in sage. (When installing sagemath, unless using --auto-select, the installation process will ask what package that provides maxima-backend the user wants, and the options are sbcl, clisp, gcl and ecl).\n+\n+[The first chunk of the patch is not applicable to sage, as I use a modified clisp and maxima, that checks the --disable-readline option]\n \n \n Comment: 1\n``````\n",
    "created_at": "2012-01-28T03:04:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8439#issuecomment-89164",
    "user": "https://github.com/kcrisman"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,12 @@
+This is a possible problem when using an external maxima, that uses a backend other then ecl.
+
+clisp has some peculiar ways to figure out there is an associated pty, that is created by pty.py from pexpect interface.
+
+gcl hangs due to an "unexcaped" ecl specific command at the start of maxima interface; this took quite some time to find, as I was thinking it was a clisp like related issue, and tried to correct the problem in gcl and/or sage-maxima.lisp.
+
+With the attached patch, going to be used in mandriva rpm, all currently maxima backends works in sage. (When installing sagemath, unless using --auto-select, the installation process will ask what package that provides maxima-backend the user wants, and the options are sbcl, clisp, gcl and ecl).
+
+[The first chunk of the patch is not applicable to sage, as I use a modified clisp and maxima, that checks the --disable-readline option]
 
 
 Comment: 1
``````




---

archive/issue_comments_089165.json:
```json
{
    "body": "Changing priority from major to minor.",
    "created_at": "2012-01-28T03:04:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8439#issuecomment-89165",
    "user": "https://github.com/kcrisman"
}
```

Changing priority from major to minor.



---

archive/issue_comments_089166.json:
```json
{
    "body": "<a id='comment:8'></a>I do not remember the exact reason I used the \"1+1;;\" example. Maybe as a hint to show that the interface was very fragile.\n\nI did the initial work to make maxima clisp backend functional because at the time I worked in the first sagemath package it was the lisp used by sage.\n\nI just tested, and all backends (clisp, gcl, ecl, and sbcl) appear functional in my sagemath package.\n\nThe patch used is still sage-4.3.3-maxima.patch (renamed/rediffed) and the related maxima patch is http://svn.mandriva.com/viewvc/packages/cooker/maxima/current/SOURCES/maxima-5.23.0-clisp-noreadline.patch?view=markup that should need a somewhat recent clisp, that understands the -disable-readline option (it was added upstream due to my bug report about this same issue).\n\nIn the case of my sagemath rpm package, for quite some time it requires maxima-runtime-ecl, so, only experienced users should usually install other backends in normal conditions due to never being asked to choose one, at least when installing sagemath.",
    "created_at": "2012-01-28T04:51:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8439#issuecomment-89166",
    "user": "https://trac.sagemath.org/admin/accounts/users/pcpa"
}
```

<a id='comment:8'></a>I do not remember the exact reason I used the "1+1;;" example. Maybe as a hint to show that the interface was very fragile.

I did the initial work to make maxima clisp backend functional because at the time I worked in the first sagemath package it was the lisp used by sage.

I just tested, and all backends (clisp, gcl, ecl, and sbcl) appear functional in my sagemath package.

The patch used is still sage-4.3.3-maxima.patch (renamed/rediffed) and the related maxima patch is http://svn.mandriva.com/viewvc/packages/cooker/maxima/current/SOURCES/maxima-5.23.0-clisp-noreadline.patch?view=markup that should need a somewhat recent clisp, that understands the -disable-readline option (it was added upstream due to my bug report about this same issue).

In the case of my sagemath rpm package, for quite some time it requires maxima-runtime-ecl, so, only experienced users should usually install other backends in normal conditions due to never being asked to choose one, at least when installing sagemath.



---

archive/issue_comments_089167.json:
```json
{
    "body": "<a id='comment:9'></a>This is very interesting.  Is there any documentation about this backend business?  I'm not as familiar with Mandriva as I ought to be - is this something like the Gentoo idea, where Sage is used with system versions of the programs?",
    "created_at": "2012-01-28T05:05:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8439#issuecomment-89167",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:9'></a>This is very interesting.  Is there any documentation about this backend business?  I'm not as familiar with Mandriva as I ought to be - is this something like the Gentoo idea, where Sage is used with system versions of the programs?



---

archive/issue_comments_089168.json:
```json
{
    "body": "<a id='comment:10'></a>Yes. It is a rpm package installing a sagemath that uses (mostly) system packages. The first working package (at least notebook and most interfaces) I had was for sage 3.2.3. But update after update, it got only good when updating to sage 4.0.1.\n\nAbout maxima backends, I think the proper term is maxima runtime. Just the lisps enabled at compile time. Example:\n\n```\n$ maxima --list-avail\nAvailable versions:\nversion 5.24.0, lisp ecl\nversion 5.24.0, lisp clisp\nversion 5.24.0, lisp sbcl\n\n```",
    "created_at": "2012-01-28T05:50:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8439#issuecomment-89168",
    "user": "https://trac.sagemath.org/admin/accounts/users/pcpa"
}
```

<a id='comment:10'></a>Yes. It is a rpm package installing a sagemath that uses (mostly) system packages. The first working package (at least notebook and most interfaces) I had was for sage 3.2.3. But update after update, it got only good when updating to sage 4.0.1.

About maxima backends, I think the proper term is maxima runtime. Just the lisps enabled at compile time. Example:

```
$ maxima --list-avail
Available versions:
version 5.24.0, lisp ecl
version 5.24.0, lisp clisp
version 5.24.0, lisp sbcl

```



---

archive/issue_comments_089169.json:
```json
{
    "body": "Changing status from needs_info to needs_review.",
    "created_at": "2020-06-28T19:22:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8439#issuecomment-89169",
    "user": "https://github.com/mkoeppe"
}
```

Changing status from needs_info to needs_review.



---

archive/issue_comments_089170.json:
```json
{
    "body": "<a id='comment:11'></a>This ticket is outdated and should be closed.",
    "created_at": "2020-06-28T19:22:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8439#issuecomment-89170",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:11'></a>This ticket is outdated and should be closed.



---

archive/issue_events_020237.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-28T19:22:09Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/8439",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8439#event-20237"
}
```



---

archive/issue_comments_089171.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2020-06-28T19:29:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8439#issuecomment-89171",
    "user": "https://github.com/fchapoton"
}
```

Resolution: invalid



---

archive/issue_events_020238.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-06-28T19:29:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/8439",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8439#event-20238"
}
```
