...
<a id='comment:1'></a>The basic functionality can be obtained from matplotlib by using `matrix_plot`:
...
<a id='comment:9'></a>I've just opened a PR here https://github.com/sagemath/sage/pull/109
...
<a id='comment:11'></a>Replying to [Dave Morris](#comment%3A1):
> The basic functionality can be obtained from matplotlib by using `matrix_plot`:
> 
> ```
> sage: def color_table(OT, cmap):
...
<a id='comment:12'></a>Replying to [gh-Bruno-TT](#comment%3A9):
> I've just opened a PR here https://github.com/sagemath/sage/pull/109
> 
> Haven't contributed to sage before so please let me know if I've done anything wrong / need to do anything else!
...
```json
{
    "body": "<a id='comment:17'></a>Replying to [Dima Pasechnik](#comment%3A15):\n> Thanks. \n> Could you add `EXAMPLES:` section in the docstring to give a clue how to use it?\n\nDone",
...
<a id='comment:17'></a>Replying to [Dima Pasechnik](#comment%3A15):
> Thanks. 
> Could you add `EXAMPLES:` section in the docstring to give a clue how to use it?
...
<a id='comment:20'></a>while for magmas with a left and right identity the 1st column and the 1st row can be
served as lists of elements to multiply, this no longer is the case for more general objects. An option to have an extra row (and/or column) listing the elements might be useful then (and useful regardless, in fact).
...
<a id='comment:21'></a>in the HTML docs, I see weird
...
<a id='comment:24'></a>Replying to [Dima Pasechnik](#comment%3A21):
> in the HTML docs, I see weird
> 
> ```
> color_table(element_names=True, cmap=<matplotlib.colors.LinearSegmentedColormap object at 0x7fc24f7e10a0>, **options)
...
<a id='comment:25'></a>fix for plot is easy; all you need to call `sphinx_plot(foo)` is that `foo.plot()` can be called.
(my branch is rebased over the latest beta (always good), and the last commit does this fix)
...
<a id='comment:26'></a>Sorry, I should have told you a quick way to rebuild only this part of docs:
...
```json
{
    "body": "<a id='comment:30'></a>OK, looks good enough now. I took out the documentation hack - now it looks not necessary, in fact (no idea why)",
    "created_at": "2022-12-19T10:48:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8598",
    "type": "issue_comment",
...
<a id='comment:30'></a>OK, looks good enough now. I took out the documentation hack - now it looks not necessary, in fact (no idea why)
...
```json
{
    "body": "<a id='comment:32'></a>OK, fine, thanks.",
...
<a id='comment:32'></a>OK, fine, thanks.
