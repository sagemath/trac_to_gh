# Issue 8593: Add Lehmer code of Permutation as an infinite enumerated set

archive/issues_008593.json:
```json
{
    "assignees": [],
    "body": "The goal of this ticket is to implement Lehmer_codes as a parent (InfiniteEnumeratedSets()). One of the goal is to use this features to index Schubert evaluation points and Schubert polynomials.\n\nThere is also an iterator over all codes, and two methods for elements\nis_dominant() (easy Schubert) and is_anti_dominant() (symmetric Schubert).\n\nAssignee: **@sagetrac-nborie**\n\nCC:  @sagetrac-sage-combinat\n\nKeywords: **code, permutation**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8593_\n\n",
    "created_at": "2010-03-23T23:37:58Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/needs%20work"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add Lehmer code of Permutation as an infinite enumerated set",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/8593",
    "user": "https://github.com/sagetrac-nborie"
}
```
The goal of this ticket is to implement Lehmer_codes as a parent (InfiniteEnumeratedSets()). One of the goal is to use this features to index Schubert evaluation points and Schubert polynomials.

There is also an iterator over all codes, and two methods for elements
is_dominant() (easy Schubert) and is_anti_dominant() (symmetric Schubert).

Assignee: **@sagetrac-nborie**

CC:  @sagetrac-sage-combinat

Keywords: **code, permutation**

_Issue created by migration from https://trac.sagemath.org/ticket/8593_





---

archive/issue_events_098560.json:
```json
{
    "actor": "https://github.com/sagetrac-nborie",
    "created_at": "2010-03-23T23:37:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8593",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8593#event-98560"
}
```



---

archive/issue_events_098561.json:
```json
{
    "actor": "https://github.com/sagetrac-nborie",
    "created_at": "2010-03-23T23:37:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8593",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8593#event-98561"
}
```



---

archive/issue_events_098562.json:
```json
{
    "actor": "https://github.com/sagetrac-nborie",
    "created_at": "2010-03-23T23:37:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8593",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8593#event-98562"
}
```



---

archive/issue_events_098563.json:
```json
{
    "actor": "https://github.com/sagetrac-nborie",
    "created_at": "2010-03-23T23:37:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8593",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8593#event-98563"
}
```



---

archive/issue_comments_070545.json:
```json
{
    "body": "Attachment: **[trac_8593_lehmer_code_schubert_step_2-nb.patch.gz](https://github.com/sagemath/sage/files/ticket8593/trac_8593_lehmer_code_schubert_step_2-nb.patch.gz)**",
    "created_at": "2010-03-23T23:49:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8593",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8593#issuecomment-70545",
    "user": "https://github.com/sagetrac-nborie"
}
```

Attachment: **[trac_8593_lehmer_code_schubert_step_2-nb.patch.gz](https://github.com/sagemath/sage/files/ticket8593/trac_8593_lehmer_code_schubert_step_2-nb.patch.gz)**



---

archive/issue_events_098564.json:
```json
{
    "actor": "https://github.com/sagetrac-nborie",
    "created_at": "2010-03-23T23:50:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8593",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8593#event-98564"
}
```



---

archive/issue_events_098565.json:
```json
{
    "actor": "https://github.com/sagetrac-nborie",
    "created_at": "2010-03-24T20:38:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8593",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8593#event-98565"
}
```



---

archive/issue_events_098566.json:
```json
{
    "actor": "https://github.com/sagetrac-nborie",
    "created_at": "2010-03-24T20:38:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8593",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8593#event-98566"
}
```



---

archive/issue_comments_070546.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nHi Nicolas,\n\nOne quick remark. Since they are many different interesting code for permutations. I'd rather call this file permutations codes and maybe make an abstract class for those. For all the other code, the bijection is different. If you are interested you can have a look at\n\n    Multivariate generalizations of the Foata-Schutzenberger equidistribition (with F. Hivert  and J.-C. Novelli), in  Fourth Colloquium on Mathematics and Computer Science: Algorithms, Trees, Combinatorics and Probabilities, DMTCS Proceedings, 2006",
    "created_at": "2010-04-10T09:28:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8593",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8593#issuecomment-70546",
    "user": "https://github.com/hivert"
}
```

<div id="comment:3" align="right">Comment 3</div>

Hi Nicolas,

One quick remark. Since they are many different interesting code for permutations. I'd rather call this file permutations codes and maybe make an abstract class for those. For all the other code, the bijection is different. If you are interested you can have a look at

    Multivariate generalizations of the Foata-Schutzenberger equidistribition (with F. Hivert  and J.-C. Novelli), in  Fourth Colloquium on Mathematics and Computer Science: Algorithms, Trees, Combinatorics and Probabilities, DMTCS Proceedings, 2006



---

archive/issue_events_098567.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8593",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8593#event-98567"
}
```



---

archive/issue_events_098568.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8593",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8593#event-98568"
}
```



---

archive/issue_events_098569.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8593",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8593#event-98569"
}
```



---

archive/issue_events_098570.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8593",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8593#event-98570"
}
```



---

archive/issue_events_098571.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8593",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8593#event-98571"
}
```



---

archive/issue_events_098572.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8593",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8593#event-98572"
}
```



---

archive/issue_events_098573.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8593",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8593#event-98573"
}
```



---

archive/issue_events_098574.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8593",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8593#event-98574"
}
```



---

archive/issue_events_098575.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8593",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8593#event-98575"
}
```
