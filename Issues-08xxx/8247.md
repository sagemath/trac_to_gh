# Issue 8247: Remove package GLPK 4.9

archive/issues_008247.json:
```json
{
    "assignees": [],
    "body": "Experimental package glpk 4.9 should be removed\n\n* We have a more recent version of GLPK (v 4.38)\n* No one seems to know who is the package maintainer of 4.9\n* there does not seem to be any reason for having 2 different GLPK packages as 4.9 only contains glpk and no custom code... \n\nNathann\n\n**CC:**  mvngu\n\nIssue created by migration from https://trac.sagemath.org/ticket/8247\n\n",
    "closed_at": "2010-02-14T16:40:43Z",
    "created_at": "2010-02-12T08:41:56Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
        "https://github.com/sagemath/sage/labels/minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Remove package GLPK 4.9",
    "type": "issue",
    "updated_at": "2010-02-14T16:40:43Z",
    "url": "https://github.com/sagemath/sage/issues/8247",
    "user": "https://github.com/nathanncohen"
}
```
Experimental package glpk 4.9 should be removed

* We have a more recent version of GLPK (v 4.38)
* No one seems to know who is the package maintainer of 4.9
* there does not seem to be any reason for having 2 different GLPK packages as 4.9 only contains glpk and no custom code... 

Nathann

**CC:**  mvngu

Issue created by migration from https://trac.sagemath.org/ticket/8247





---

archive/issue_events_086067.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-02-12T08:41:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8247",
    "milestone_number": null,
    "milestone_title": "sage-4.3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8247#event-86067"
}
```



---

archive/issue_events_086068.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-02-12T08:41:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8247",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
    "label_color": "08517b",
    "label_name": "component: packages: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8247#event-86068"
}
```



---

archive/issue_events_086069.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-02-12T08:41:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8247",
    "label": "https://github.com/sagemath/sage/labels/minor",
    "label_color": "ff0000",
    "label_name": "minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8247#event-86069"
}
```



---

archive/issue_events_086070.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-02-12T08:41:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8247",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "008080",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8247#event-86070"
}
```



---

archive/issue_events_086071.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-02-14T16:40:43Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/8247",
    "rename": {
        "from": "Remove package GLPK 3.9",
        "to": "Remove package GLPK 4.9"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8247#event-86071"
}
```



---

archive/issue_comments_065840.json:
```json
{
    "body": "<a id='comment:1'>**Comment 1:**</a>\nFixed by removing `glpk-4.9.spkg` from the [experimental packages](http://www.sagemath.org/packages/experimental/) repository.",
    "created_at": "2010-02-14T16:40:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8247",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8247#issuecomment-65840",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<a id='comment:1'>**Comment 1:**</a>
Fixed by removing `glpk-4.9.spkg` from the [experimental packages](http://www.sagemath.org/packages/experimental/) repository.



---

archive/issue_events_086072.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-02-14T16:40:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8247",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8247#event-86072"
}
```
