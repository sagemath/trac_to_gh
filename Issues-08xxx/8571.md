# Issue 8571: Fix the documentation of abstract methods.

archive/issues_008571.json:
```json
{
    "assignees": [],
    "body": "Currently, the documentation of abstract methods has the following annoying problem, it doesn't appear when asked with \"?\" from a parent.\n\n```\nsage: P = Parent(category=CoxeterGroups())\nsage: P.index_set?\nObject `P.index_set` not found.\nsage:                                \n``` \nYou have to write:\n\n```\nsage: CG = CoxeterGroups()\nsage: CG.ParentMethods.index_set?\nType:           AbstractMethod\nBase Class:     <class 'sage.misc.abstract_method.AbstractMethod'>\nString Form:    <abstract method index_set at 0x219a320>\nNamespace:      Interactive\nFile:           /usr/local/sage/sage/local/lib/python2.6/site-packages/sage/misc/abstract_method.py\nDefinition:     CG.ParentMethods.index_set(self)\nDocstring:\n       Returns the index set of (the simple reflections of) ``self``, as a\n       list (or iterable).\n\nClass Docstring:\n       Constructor for abstract methods\n[...]\n```\nThis should be fixed.\n\n\nAssignee: **@sagetrac-mvngu**\n\nCC:  @nthiery\n\nKeywords: **abstract methods**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8571_\n\n",
    "created_at": "2010-03-21T18:47:03Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix the documentation of abstract methods.",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/8571",
    "user": "https://github.com/hivert"
}
```
Currently, the documentation of abstract methods has the following annoying problem, it doesn't appear when asked with "?" from a parent.

```
sage: P = Parent(category=CoxeterGroups())
sage: P.index_set?
Object `P.index_set` not found.
sage:                                
``` 
You have to write:

```
sage: CG = CoxeterGroups()
sage: CG.ParentMethods.index_set?
Type:           AbstractMethod
Base Class:     <class 'sage.misc.abstract_method.AbstractMethod'>
String Form:    <abstract method index_set at 0x219a320>
Namespace:      Interactive
File:           /usr/local/sage/sage/local/lib/python2.6/site-packages/sage/misc/abstract_method.py
Definition:     CG.ParentMethods.index_set(self)
Docstring:
       Returns the index set of (the simple reflections of) ``self``, as a
       list (or iterable).

Class Docstring:
       Constructor for abstract methods
[...]
```
This should be fixed.


Assignee: **@sagetrac-mvngu**

CC:  @nthiery

Keywords: **abstract methods**

_Issue created by migration from https://trac.sagemath.org/ticket/8571_





---

archive/issue_events_098265.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2010-03-21T18:47:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8571",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8571#event-98265"
}
```



---

archive/issue_events_098266.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2010-03-21T18:47:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8571",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8571#event-98266"
}
```



---

archive/issue_events_098267.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2010-03-21T18:47:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8571",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8571#event-98267"
}
```



---

archive/issue_events_098268.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2010-03-21T18:47:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8571",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8571#event-98268"
}
```



---

archive/issue_comments_070371.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,29 @@\n-Currently, the documentation of abstract methods has two annoying problems:\n-- it doesn't appear when asked with \"?\" \n-- nothing says that they are abstract method in the doc.\n+Currently, the documentation of abstract methods has the following annoying problem, it doesn't appear when asked with \"?\" from a parent.\n+\n+```\n+sage: P = Parent(category=CoxeterGroups())\n+sage: P.index_set?\n+Object `P.index_set` not found.\n+sage:                                \n+``` \n+You have to write:\n+\n+```\n+sage: CG = CoxeterGroups()\n+sage: CG.ParentMethods.index_set?\n+Type:           AbstractMethod\n+Base Class:     <class 'sage.misc.abstract_method.AbstractMethod'>\n+String Form:    <abstract method index_set at 0x219a320>\n+Namespace:      Interactive\n+File:           /usr/local/sage/sage/local/lib/python2.6/site-packages/sage/misc/abstract_method.py\n+Definition:     CG.ParentMethods.index_set(self)\n+Docstring:\n+       Returns the index set of (the simple reflections of) ``self``, as a\n+       list (or iterable).\n+\n+Class Docstring:\n+       Constructor for abstract methods\n+[...]\n+```\n This should be fixed.\n+\n``````\n",
    "created_at": "2010-03-21T19:04:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8571",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8571#issuecomment-70371",
    "user": "https://github.com/hivert"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,29 @@
-Currently, the documentation of abstract methods has two annoying problems:
-- it doesn't appear when asked with "?" 
-- nothing says that they are abstract method in the doc.
+Currently, the documentation of abstract methods has the following annoying problem, it doesn't appear when asked with "?" from a parent.
+
+```
+sage: P = Parent(category=CoxeterGroups())
+sage: P.index_set?
+Object `P.index_set` not found.
+sage:                                
+``` 
+You have to write:
+
+```
+sage: CG = CoxeterGroups()
+sage: CG.ParentMethods.index_set?
+Type:           AbstractMethod
+Base Class:     <class 'sage.misc.abstract_method.AbstractMethod'>
+String Form:    <abstract method index_set at 0x219a320>
+Namespace:      Interactive
+File:           /usr/local/sage/sage/local/lib/python2.6/site-packages/sage/misc/abstract_method.py
+Definition:     CG.ParentMethods.index_set(self)
+Docstring:
+       Returns the index set of (the simple reflections of) ``self``, as a
+       list (or iterable).
+
+Class Docstring:
+       Constructor for abstract methods
+[...]
+```
 This should be fixed.
+
``````




---

archive/issue_comments_070372.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nWe should also think about if/how to merge with Python's new abstractmethods:\n\nhttp://docs.python.org/library/abc.html",
    "created_at": "2010-04-17T10:19:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8571",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8571#issuecomment-70372",
    "user": "https://github.com/nthiery"
}
```

<div id="comment:2" align="right">Comment 2</div>

We should also think about if/how to merge with Python's new abstractmethods:

http://docs.python.org/library/abc.html



---

archive/issue_events_098269.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8571",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8571#event-98269"
}
```



---

archive/issue_events_098270.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8571",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8571#event-98270"
}
```



---

archive/issue_events_098271.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8571",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8571#event-98271"
}
```



---

archive/issue_events_098272.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8571",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8571#event-98272"
}
```



---

archive/issue_events_098273.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8571",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8571#event-98273"
}
```



---

archive/issue_events_098274.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8571",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8571#event-98274"
}
```



---

archive/issue_events_098275.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8571",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8571#event-98275"
}
```



---

archive/issue_events_098276.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8571",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8571#event-98276"
}
```



---

archive/issue_events_098277.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8571",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8571#event-98277"
}
```
