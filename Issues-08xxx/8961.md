# Issue 8961: zope.testbrowser is included in sagenb but is no longer required

archive/issues_008961.json:
```json
{
    "body": "`zope.testbrowser` is no longer needed by sagenb but is still listed as a requirement.\n\nThis also fixes the problem regarding the online access during install.\n\n**Assignee:** @jasongrout, @williamstein\n\n**CC:**  @qed777 @nexttime\n\n**Author:** Tim Dumol\n\n**Merged:** SageNB 0.8.1\n\nIssue created by migration from https://trac.sagemath.org/ticket/8961\n\n",
    "closed_at": "2010-07-11T06:06:27Z",
    "created_at": "2010-05-14T11:27:14Z",
    "labels": [
        "component: notebook",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.5",
    "title": "zope.testbrowser is included in sagenb but is no longer required",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/8961",
    "user": "https://github.com/TimDumol"
}
```
`zope.testbrowser` is no longer needed by sagenb but is still listed as a requirement.

This also fixes the problem regarding the online access during install.

**Assignee:** @jasongrout, @williamstein

**CC:**  @qed777 @nexttime

**Author:** Tim Dumol

**Merged:** SageNB 0.8.1

Issue created by migration from https://trac.sagemath.org/ticket/8961





---

archive/issue_comments_075341.json:
```json
{
    "body": "Removes zope.testbrowser as a dependency.",
    "created_at": "2010-07-06T13:21:45Z",
    "issue": "https://github.com/sagemath/sage/issues/8961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8961#issuecomment-75341",
    "user": "https://github.com/TimDumol"
}
```

Removes zope.testbrowser as a dependency.



---

archive/issue_events_064648.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2010-07-06T13:23:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8961",
    "milestone": "sage-4.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8961#event-64648"
}
```



---

archive/issue_events_064649.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2010-07-06T13:23:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8961",
    "label": "minor",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8961#event-64649"
}
```



---

archive/issue_events_064650.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2010-07-06T13:23:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8961",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8961#event-64650"
}
```



---

archive/issue_comments_075342.json:
```json
{
    "body": "**Author:** Tim Dumol",
    "created_at": "2010-07-06T13:23:04Z",
    "issue": "https://github.com/sagemath/sage/issues/8961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8961#issuecomment-75342",
    "user": "https://github.com/TimDumol"
}
```

**Author:** Tim Dumol



---

archive/attachments_011219.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_8961-remove-zope.testbrowser.patch",
    "asset_url": "tarball://root/attachments/ticket8961/trac_8961-remove-zope.testbrowser.patch",
    "created_at": "2010-07-06T13:23:04Z",
    "issue": "https://github.com/sagemath/sage/issues/8961",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket8961/trac_8961-remove-zope.testbrowser.patch",
    "user": "https://github.com/TimDumol"
}
```



---

archive/issue_comments_075343.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [trac_8961-remove-zope.testbrowser.patch](https://github.com/sagemath/sage/files/ticket8961/trac_8961-remove-zope.testbrowser.patch) by @TimDumol created at 2010-07-06 13:23:04\n\nNow only Twisted (and its zope.interface, which is included in the Twisted spkg) is required by SageNB.",
    "created_at": "2010-07-06T13:23:04Z",
    "issue": "https://github.com/sagemath/sage/issues/8961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8961#issuecomment-75343",
    "user": "https://github.com/TimDumol"
}
```

<a id='comment:1'></a>
Attachment [trac_8961-remove-zope.testbrowser.patch](https://github.com/sagemath/sage/files/ticket8961/trac_8961-remove-zope.testbrowser.patch) by @TimDumol created at 2010-07-06 13:23:04

Now only Twisted (and its zope.interface, which is included in the Twisted spkg) is required by SageNB.



---

archive/issue_comments_075344.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n `zope.testbrowser` is no longer needed by sagenb but is still listed as a requirement.\n+\n+This also fixes the problem regarding the online access during install.\n``````\n",
    "created_at": "2010-07-06T13:23:04Z",
    "issue": "https://github.com/sagemath/sage/issues/8961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8961#issuecomment-75344",
    "user": "https://github.com/TimDumol"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1 +1,3 @@
 `zope.testbrowser` is no longer needed by sagenb but is still listed as a requirement.
+
+This also fixes the problem regarding the online access during install.
``````




---

archive/issue_comments_075345.json:
```json
{
    "body": "Removes an extra line.",
    "created_at": "2010-07-06T16:36:48Z",
    "issue": "https://github.com/sagemath/sage/issues/8961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8961#issuecomment-75345",
    "user": "https://github.com/TimDumol"
}
```

Removes an extra line.



---

archive/attachments_011220.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_8961-remove-zope.testbrowser.2.patch",
    "asset_url": "tarball://root/attachments/ticket8961/trac_8961-remove-zope.testbrowser.2.patch",
    "created_at": "2010-07-06T17:09:19Z",
    "issue": "https://github.com/sagemath/sage/issues/8961",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket8961/trac_8961-remove-zope.testbrowser.2.patch",
    "user": "https://github.com/TimDumol"
}
```



---

archive/issue_comments_075346.json:
```json
{
    "body": "<a id='comment:3'></a>\nAttachment [trac_8961-remove-zope.testbrowser.2.patch](https://github.com/sagemath/sage/files/ticket8961/trac_8961-remove-zope.testbrowser.2.patch) by @TimDumol created at 2010-07-06 17:09:19\n\nThe package is at http://sage.math.washington.edu/home/timdumol/sagenb-0.8.1.spkg, and is the tentative package for #9430.",
    "created_at": "2010-07-06T17:09:19Z",
    "issue": "https://github.com/sagemath/sage/issues/8961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8961#issuecomment-75346",
    "user": "https://github.com/TimDumol"
}
```

<a id='comment:3'></a>
Attachment [trac_8961-remove-zope.testbrowser.2.patch](https://github.com/sagemath/sage/files/ticket8961/trac_8961-remove-zope.testbrowser.2.patch) by @TimDumol created at 2010-07-06 17:09:19

The package is at http://sage.math.washington.edu/home/timdumol/sagenb-0.8.1.spkg, and is the tentative package for #9430.



---

archive/issue_events_064651.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2010-07-11T06:06:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8961",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8961#event-64651"
}
```



---

archive/issue_events_064652.json:
```json
{
    "actor": "https://github.com/TimDumol",
    "created_at": "2010-07-11T06:06:27Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8961",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8961#event-64652"
}
```



---

archive/issue_comments_075347.json:
```json
{
    "body": "**Merged:** SageNB 0.8.1",
    "created_at": "2010-07-11T06:06:27Z",
    "issue": "https://github.com/sagemath/sage/issues/8961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8961#issuecomment-75347",
    "user": "https://github.com/TimDumol"
}
```

**Merged:** SageNB 0.8.1
