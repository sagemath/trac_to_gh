# Issue 8043: #7544 breaks #7355: "sage -i foo" does not automatically find the version number

archive/issues_008043.json:
```json
{
    "assignees": [],
    "body": "As detailed at [#7544 comment:4](https://github.com/sagemath/sage/issues/7544#comment:4), the functionality introduced in #7355 was broken by #7544.\n\nAuthor: **John Palmieri**\n\nReviewer: **Dan Drake**\n\nMerged: **sage-4.4.alpha2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8043_\n\n",
    "closed_at": "2010-04-23T19:10:20Z",
    "created_at": "2010-01-23T05:54:41Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "#7544 breaks #7355: \"sage -i foo\" does not automatically find the version number",
    "type": "issue",
    "updated_at": "2010-04-23T19:10:20Z",
    "url": "https://github.com/sagemath/sage/issues/8043",
    "user": "https://github.com/dandrake"
}
```
As detailed at [#7544 comment:4](https://github.com/sagemath/sage/issues/7544#comment:4), the functionality introduced in #7355 was broken by #7544.

Author: **John Palmieri**

Reviewer: **Dan Drake**

Merged: **sage-4.4.alpha2**

_Issue created by migration from https://trac.sagemath.org/ticket/8043_





---

archive/issue_events_090974.json:
```json
{
    "actor": "https://github.com/dandrake",
    "created_at": "2010-01-23T05:54:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8043",
    "milestone_number": null,
    "milestone_title": "sage-4.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8043#event-90974"
}
```



---

archive/issue_events_090975.json:
```json
{
    "actor": "https://github.com/dandrake",
    "created_at": "2010-01-23T05:54:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8043",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8043#event-90975"
}
```



---

archive/issue_events_090976.json:
```json
{
    "actor": "https://github.com/dandrake",
    "created_at": "2010-01-23T05:54:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8043",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8043#event-90976"
}
```



---

archive/issue_events_090977.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-04-10T18:58:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8043",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8043#event-90977"
}
```



---

archive/issue_comments_063333.json:
```json
{
    "body": "Author: **John Palmieri**",
    "created_at": "2010-04-10T18:58:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8043#issuecomment-63333",
    "user": "https://github.com/jhpalmieri"
}
```

Author: **John Palmieri**



---

archive/issue_comments_063334.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nHere's a patch.  See #8669 for a somewhat related ticket.",
    "created_at": "2010-04-10T18:58:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8043#issuecomment-63334",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:1" align="right">Comment 1</div>

Here's a patch.  See #8669 for a somewhat related ticket.



---

archive/issue_comments_063335.json:
```json
{
    "body": "Attachment: **[trac_8043-spkg-no-version.patch.gz](https://github.com/sagemath/sage/files/ticket8043/trac_8043-spkg-no-version.patch.gz)**\n\nscripts repo",
    "created_at": "2010-04-10T18:58:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8043#issuecomment-63335",
    "user": "https://github.com/jhpalmieri"
}
```

Attachment: **[trac_8043-spkg-no-version.patch.gz](https://github.com/sagemath/sage/files/ticket8043/trac_8043-spkg-no-version.patch.gz)**

scripts repo



---

archive/issue_events_090978.json:
```json
{
    "actor": "https://github.com/dandrake",
    "created_at": "2010-04-12T06:28:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8043",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8043#event-90978"
}
```



---

archive/issue_events_090979.json:
```json
{
    "actor": "https://github.com/dandrake",
    "created_at": "2010-04-12T06:28:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8043",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8043#event-90979"
}
```



---

archive/issue_comments_063336.json:
```json
{
    "body": "Reviewer: **Dan Drake**",
    "created_at": "2010-04-12T06:28:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8043#issuecomment-63336",
    "user": "https://github.com/dandrake"
}
```

Reviewer: **Dan Drake**



---

archive/issue_comments_063337.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nThis fixes the problem, and does so in a simple, POSIX-compliant way. Positive review.",
    "created_at": "2010-04-12T06:28:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8043#issuecomment-63337",
    "user": "https://github.com/dandrake"
}
```

<div id="comment:2" align="right">Comment 2</div>

This fixes the problem, and does so in a simple, POSIX-compliant way. Positive review.



---

archive/issue_comments_063338.json:
```json
{
    "body": "Merged: **sage-4.4.alpha2**",
    "created_at": "2010-04-23T19:10:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8043#issuecomment-63338",
    "user": "https://github.com/jhpalmieri"
}
```

Merged: **sage-4.4.alpha2**



---

archive/issue_comments_063339.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nMerged in 4.4.alpha2.",
    "created_at": "2010-04-23T19:10:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8043",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8043#issuecomment-63339",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:3" align="right">Comment 3</div>

Merged in 4.4.alpha2.



---

archive/issue_events_090980.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-04-23T19:10:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8043",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8043#event-90980"
}
```



---

archive/issue_events_090981.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-04-23T19:10:20Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8043",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8043#event-90981"
}
```
