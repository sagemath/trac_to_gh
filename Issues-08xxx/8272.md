# Issue 8272: R's spkg-install tries to disable iconv, but it is *essential* for R.

archive/issues_008272.json:
```json
{
    "assignees": [],
    "body": "R has long since wanted iconv support, although there was an option to disable it. That option has now been removed, and R **must** have iconv. However, there is junk left over in R's spkg-install which tries to disable iconv, and reports it is doing so. \n\n```\nDisabling libiconv support on Solaris\n```\n\nIt likewise tries to do that same on OS X. \n\nAssignee: **amhou**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8272_\n\n",
    "closed_at": "2010-07-19T07:39:59Z",
    "created_at": "2010-02-15T11:17:11Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20statistics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "R's spkg-install tries to disable iconv, but it is *essential* for R.",
    "type": "issue",
    "updated_at": "2010-07-19T07:39:59Z",
    "url": "https://github.com/sagemath/sage/issues/8272",
    "user": "https://github.com/sagetrac-drkirkby"
}
```
R has long since wanted iconv support, although there was an option to disable it. That option has now been removed, and R **must** have iconv. However, there is junk left over in R's spkg-install which tries to disable iconv, and reports it is doing so. 

```
Disabling libiconv support on Solaris
```

It likewise tries to do that same on OS X. 

Assignee: **amhou**

_Issue created by migration from https://trac.sagemath.org/ticket/8272_





---

archive/issue_events_093887.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-02-15T11:17:11Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8272",
    "milestone_number": null,
    "milestone_title": "sage-4.5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8272#event-93887"
}
```



---

archive/issue_events_093888.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-02-15T11:17:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8272",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20statistics",
    "label_color": "0000ff",
    "label_name": "component: statistics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8272#event-93888"
}
```



---

archive/issue_events_093889.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-02-15T11:17:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8272",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8272#event-93889"
}
```



---

archive/issue_events_093890.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-02-15T11:17:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8272",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8272#event-93890"
}
```



---

archive/issue_comments_066108.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\n#8285 addresses this issue, along with several other issues.",
    "created_at": "2010-02-17T01:02:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8272",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8272#issuecomment-66108",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

<a id='comment:1'>Comment 1:</a>
#8285 addresses this issue, along with several other issues.



---

archive/issue_comments_066109.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nThis has long been resolved, so can be closed.",
    "created_at": "2010-07-19T07:39:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8272",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8272#issuecomment-66109",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

<a id='comment:2'>Comment 2:</a>
This has long been resolved, so can be closed.



---

archive/issue_events_093891.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-07-19T07:39:59Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8272",
    "milestone_number": null,
    "milestone_title": "sage-4.5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8272#event-93891"
}
```



---

archive/issue_events_093892.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-07-19T07:39:59Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8272",
    "milestone_number": null,
    "milestone_title": "sage-4.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8272#event-93892"
}
```



---

archive/issue_events_093893.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-07-19T07:39:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8272",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8272#event-93893"
}
```
