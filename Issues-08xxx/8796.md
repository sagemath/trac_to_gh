# Issue 8796: clean up documentation of logic/propcalc.py

archive/issues_008796.json:
```json
{
    "body": "As the subject says. This needs to be coordinated with #8797.\n\n**Assignee:** mvngu\n\n**Author:** Minh Van Nguyen\n\n**Reviewer:** Nathann Cohen\n\n**Merged:** sage-4.4.4.alpha0\n\nIssue created by migration from https://trac.sagemath.org/ticket/8796\n\n",
    "closed_at": "2010-06-06T08:39:10Z",
    "created_at": "2010-04-28T06:53:13Z",
    "labels": [
        "component: documentation",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.4.4",
    "title": "clean up documentation of logic/propcalc.py",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/8796",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```
As the subject says. This needs to be coordinated with #8797.

**Assignee:** mvngu

**Author:** Minh Van Nguyen

**Reviewer:** Nathann Cohen

**Merged:** sage-4.4.4.alpha0

Issue created by migration from https://trac.sagemath.org/ticket/8796





---

archive/issue_comments_073353.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-As the subject says.\n+As the subject says. This needs to be coordinated with #8797.\n``````\n",
    "created_at": "2010-04-28T07:01:03Z",
    "issue": "https://github.com/sagemath/sage/issues/8796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8796#issuecomment-73353",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1 +1 @@
-As the subject says.
+As the subject says. This needs to be coordinated with #8797.
``````




---

archive/issue_comments_073354.json:
```json
{
    "body": "**Author:** Minh Van Nguyen",
    "created_at": "2010-05-03T02:50:50Z",
    "issue": "https://github.com/sagemath/sage/issues/8796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8796#issuecomment-73354",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Author:** Minh Van Nguyen



---

archive/issue_events_063147.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-05-03T02:50:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8796",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8796#event-63147"
}
```



---

archive/attachments_011006.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_8796-propcalc-clean-ups.patch",
    "asset_url": "tarball://root/attachments/ticket8796/trac_8796-propcalc-clean-ups.patch",
    "created_at": "2010-05-03T02:50:50Z",
    "issue": "https://github.com/sagemath/sage/issues/8796",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket8796/trac_8796-propcalc-clean-ups.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```



---

archive/issue_comments_073355.json:
```json
{
    "body": "<a id='comment:2'></a>\nAttachment [trac_8796-propcalc-clean-ups.patch](https://github.com/sagemath/sage/files/ticket8796/trac_8796-propcalc-clean-ups.patch) by mvngu created at 2010-05-03 02:50:50\n\nChanges in the patch include:\n\n* Add `sage/logic/propcalc.py` to the reference manual.\n* Remove tab characters.\n* Clean-ups in accordance with [PEP 008](http://www.python.org/dev/peps/pep-0008/).",
    "created_at": "2010-05-03T02:50:50Z",
    "issue": "https://github.com/sagemath/sage/issues/8796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8796#issuecomment-73355",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:2'></a>
Attachment [trac_8796-propcalc-clean-ups.patch](https://github.com/sagemath/sage/files/ticket8796/trac_8796-propcalc-clean-ups.patch) by mvngu created at 2010-05-03 02:50:50

Changes in the patch include:

* Add `sage/logic/propcalc.py` to the reference manual.
* Remove tab characters.
* Clean-ups in accordance with [PEP 008](http://www.python.org/dev/peps/pep-0008/).



---

archive/issue_events_063148.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-05-19T20:08:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8796",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8796#event-63148"
}
```



---

archive/issue_events_063149.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-05-19T20:08:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8796",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8796#event-63149"
}
```



---

archive/issue_comments_073356.json:
```json
{
    "body": "<a id='comment:3'></a>\nNice patch. Nothing to complain about :-)\n\nNathann",
    "created_at": "2010-05-19T20:08:57Z",
    "issue": "https://github.com/sagemath/sage/issues/8796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8796#issuecomment-73356",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:3'></a>
Nice patch. Nothing to complain about :-)

Nathann



---

archive/issue_comments_073357.json:
```json
{
    "body": "<a id='comment:4'></a>\n*\"Parentheses may be used to explicitly **show** order of operation.\"*\n\nI usually use brackets to **express** (i.e. force) [desired/intended] \"order of operation\", too. ;-)\n\n*\"... variables that consist of a leading letter and trailing underscores and alphanumerics\"* sounds a bit strange.\n\nI'd say *\"... whose names can be made up of alphanumerics and underscores but must start with a letter\"* or something like that.\n\n[Haven't applied the patch yet.]",
    "created_at": "2010-05-21T03:56:38Z",
    "issue": "https://github.com/sagemath/sage/issues/8796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8796#issuecomment-73357",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:4'></a>
*"Parentheses may be used to explicitly **show** order of operation."*

I usually use brackets to **express** (i.e. force) [desired/intended] "order of operation", too. ;-)

*"... variables that consist of a leading letter and trailing underscores and alphanumerics"* sounds a bit strange.

I'd say *"... whose names can be made up of alphanumerics and underscores but must start with a letter"* or something like that.

[Haven't applied the patch yet.]



---

archive/issue_comments_073358.json:
```json
{
    "body": "**Merged:** sage-4.4.4.alpha0",
    "created_at": "2010-06-06T08:39:10Z",
    "issue": "https://github.com/sagemath/sage/issues/8796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8796#issuecomment-73358",
    "user": "https://github.com/mwhansen"
}
```

**Merged:** sage-4.4.4.alpha0



---

archive/issue_comments_073359.json:
```json
{
    "body": "**Reviewer:** Nathann Cohen",
    "created_at": "2010-06-06T08:39:10Z",
    "issue": "https://github.com/sagemath/sage/issues/8796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8796#issuecomment-73359",
    "user": "https://github.com/mwhansen"
}
```

**Reviewer:** Nathann Cohen



---

archive/issue_events_063150.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-06-06T08:39:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8796",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8796#event-63150"
}
```



---

archive/issue_events_063151.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-06-06T08:39:10Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8796",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8796#event-63151"
}
```
