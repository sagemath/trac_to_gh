# Issue 8355: Fix hsv_to_rgb to take all 3 arguments

archive/issues_008355.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: hue(.5,.5,.5)\nTraceback (click to the left of this block for traceback)\n...\nTypeError: can't multiply sequence by non-int of type 'float'\n```\n\nAssignee: **@williamstein**\n\nCC:  @kcrisman\n\nAuthor: **Aly Deines, Robert Bradshaw**\n\nReviewer: **Willem Jan Palenstijn**\n\nMerged: **sage-4.6.2.alpha1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8355_\n\n",
    "closed_at": "2011-01-19T22:19:30Z",
    "created_at": "2010-02-25T00:25:00Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graphics",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/good%20first%20issue"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.6.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix hsv_to_rgb to take all 3 arguments",
    "type": "issue",
    "updated_at": "2011-01-19T22:19:30Z",
    "url": "https://github.com/sagemath/sage/issues/8355",
    "user": "https://github.com/sagetrac-boothby"
}
```

```
sage: hue(.5,.5,.5)
Traceback (click to the left of this block for traceback)
...
TypeError: can't multiply sequence by non-int of type 'float'
```

Assignee: **@williamstein**

CC:  @kcrisman

Author: **Aly Deines, Robert Bradshaw**

Reviewer: **Willem Jan Palenstijn**

Merged: **sage-4.6.2.alpha1**

_Issue created by migration from https://trac.sagemath.org/ticket/8355_





---

archive/issue_events_092195.json:
```json
{
    "actor": "https://github.com/sagetrac-boothby",
    "created_at": "2010-02-25T00:25:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "milestone_number": null,
    "milestone_title": "sage-4.6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8355#event-92195"
}
```



---

archive/issue_events_092196.json:
```json
{
    "actor": "https://github.com/sagetrac-boothby",
    "created_at": "2010-02-25T00:25:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graphics",
    "label_color": "0000ff",
    "label_name": "component: graphics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8355#event-92196"
}
```



---

archive/issue_events_092197.json:
```json
{
    "actor": "https://github.com/sagetrac-boothby",
    "created_at": "2010-02-25T00:25:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8355#event-92197"
}
```



---

archive/issue_events_092198.json:
```json
{
    "actor": "https://github.com/sagetrac-boothby",
    "created_at": "2010-02-25T00:25:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8355#event-92198"
}
```



---

archive/issue_comments_067450.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[8355-hue.patch.gz](https://github.com/sagemath/sage/files/ticket8355/8355-hue.patch.gz)**",
    "created_at": "2010-04-02T00:31:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8355#issuecomment-67450",
    "user": "https://github.com/sagetrac-boothby"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[8355-hue.patch.gz](https://github.com/sagemath/sage/files/ticket8355/8355-hue.patch.gz)**



---

archive/issue_events_092199.json:
```json
{
    "actor": "https://github.com/sagetrac-boothby",
    "created_at": "2010-04-02T00:31:57Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "milestone_number": null,
    "milestone_title": "sage-4.3.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8355#event-92199"
}
```



---

archive/issue_events_092200.json:
```json
{
    "actor": "https://github.com/sagetrac-boothby",
    "created_at": "2010-04-02T00:31:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8355#event-92200"
}
```



---

archive/issue_comments_067451.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nWorks for me.",
    "created_at": "2010-04-02T00:35:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8355#issuecomment-67451",
    "user": "https://github.com/sagetrac-boothby"
}
```

<a id='comment:2'>Comment 2:</a>
Works for me.



---

archive/issue_events_092201.json:
```json
{
    "actor": "https://github.com/sagetrac-boothby",
    "created_at": "2010-04-02T00:35:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8355#event-92201"
}
```



---

archive/issue_events_092202.json:
```json
{
    "actor": "https://github.com/sagetrac-boothby",
    "created_at": "2010-04-02T00:35:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8355#event-92202"
}
```



---

archive/issue_comments_067452.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nThis doesn't apply cleanly to me against 4.3.5.  Please rebase.",
    "created_at": "2010-04-15T22:16:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8355#issuecomment-67452",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:3'>Comment 3:</a>
This doesn't apply cleanly to me against 4.3.5.  Please rebase.



---

archive/issue_comments_067453.json:
```json
{
    "body": "Reviewer: **Tom Boothby**",
    "created_at": "2010-04-15T22:16:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8355#issuecomment-67453",
    "user": "https://github.com/jhpalmieri"
}
```

Reviewer: **Tom Boothby**



---

archive/issue_events_092203.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-04-15T22:16:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8355#event-92203"
}
```



---

archive/issue_events_092204.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-04-15T22:16:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8355#event-92204"
}
```



---

archive/issue_comments_067454.json:
```json
{
    "body": "Author: **Robert Bradshaw**",
    "created_at": "2010-04-15T22:16:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8355#issuecomment-67454",
    "user": "https://github.com/jhpalmieri"
}
```

Author: **Robert Bradshaw**



---

archive/issue_events_092205.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-04-15T22:16:21Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "milestone_number": null,
    "milestone_title": "sage-4.3.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8355#event-92205"
}
```



---

archive/issue_events_092206.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-04-15T22:16:21Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "milestone_number": null,
    "milestone_title": "sage-4.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8355#event-92206"
}
```



---

archive/issue_events_092207.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-05-26T15:41:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "label": "https://github.com/sagemath/sage/labels/good%20first%20issue",
    "label_color": "7057ff",
    "label_name": "good first issue",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8355#event-92207"
}
```



---

archive/issue_events_092208.json:
```json
{
    "actor": "https://github.com/sagetrac-ryan",
    "created_at": "2011-01-09T00:03:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8355#event-92208"
}
```



---

archive/issue_events_092209.json:
```json
{
    "actor": "https://github.com/sagetrac-ryan",
    "created_at": "2011-01-09T00:03:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8355#event-92209"
}
```



---

archive/issue_comments_067455.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nI think this has been fixed already.  Works in sage-4.6.0",
    "created_at": "2011-01-09T00:03:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8355#issuecomment-67455",
    "user": "https://github.com/sagetrac-ryan"
}
```

<a id='comment:7'>Comment 7:</a>
I think this has been fixed already.  Works in sage-4.6.0



---

archive/issue_comments_067456.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nConfirmed.  But should we add a patch that confirms this, as in the previous patch?  This is because we now use the Python version of this, afaict.",
    "created_at": "2011-01-09T02:56:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8355#issuecomment-67456",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:8'>Comment 8:</a>
Confirmed.  But should we add a patch that confirms this, as in the previous patch?  This is because we now use the Python version of this, afaict.



---

archive/issue_comments_067457.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nI've created a patch that confirms\n\n```\nsage: hue(.5,.5,.5)\n(0.25, 0.5, 0.5)\n```\nin the doctest of hue and applies to sage-4.6.1.rc1.",
    "created_at": "2011-01-09T22:41:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8355#issuecomment-67457",
    "user": "https://github.com/adeines"
}
```

<a id='comment:9'>Comment 9:</a>
I've created a patch that confirms

```
sage: hue(.5,.5,.5)
(0.25, 0.5, 0.5)
```
in the doctest of hue and applies to sage-4.6.1.rc1.



---

archive/issue_comments_067458.json:
```json
{
    "body": "hue doctest which applies to sage-4.6.1.rc1",
    "created_at": "2011-01-09T22:42:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8355#issuecomment-67458",
    "user": "https://github.com/adeines"
}
```

hue doctest which applies to sage-4.6.1.rc1



---

archive/issue_comments_067459.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nAttachment: **[8355-hue.2.patch.gz](https://github.com/sagemath/sage/files/ticket8355/8355-hue.2.patch.gz)**\n\nLooks good to me.",
    "created_at": "2011-01-09T23:15:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8355#issuecomment-67459",
    "user": "https://github.com/wjp"
}
```

<a id='comment:10'>Comment 10:</a>
Attachment: **[8355-hue.2.patch.gz](https://github.com/sagemath/sage/files/ticket8355/8355-hue.2.patch.gz)**

Looks good to me.



---

archive/issue_events_092210.json:
```json
{
    "actor": "https://github.com/wjp",
    "created_at": "2011-01-09T23:15:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8355#event-92210"
}
```



---

archive/issue_events_092211.json:
```json
{
    "actor": "https://github.com/wjp",
    "created_at": "2011-01-09T23:15:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8355#event-92211"
}
```



---

archive/issue_events_092212.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-10T02:17:16Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "milestone_number": null,
    "milestone_title": "sage-4.6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8355#event-92212"
}
```



---

archive/issue_events_092213.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-10T02:17:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "milestone_number": null,
    "milestone_title": "sage-4.6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8355#event-92213"
}
```



---

archive/issue_comments_067460.json:
```json
{
    "body": "Changed reviewer from **Tom Boothby** to **Willem Jan Palenstijn**",
    "created_at": "2011-01-10T13:58:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8355#issuecomment-67460",
    "user": "https://github.com/kcrisman"
}
```

Changed reviewer from **Tom Boothby** to **Willem Jan Palenstijn**



---

archive/issue_comments_067461.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nIf these spellings of the author and reviewer aren't right, please correct them.",
    "created_at": "2011-01-10T13:58:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8355#issuecomment-67461",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:12'>Comment 12:</a>
If these spellings of the author and reviewer aren't right, please correct them.



---

archive/issue_comments_067462.json:
```json
{
    "body": "Changed author from **Robert Bradshaw** to **Aly Deines**",
    "created_at": "2011-01-10T13:58:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8355#issuecomment-67462",
    "user": "https://github.com/kcrisman"
}
```

Changed author from **Robert Bradshaw** to **Aly Deines**



---

archive/issue_comments_067463.json:
```json
{
    "body": "Changed author from **Aly Deines** to **Aly Deines, Robert Bradshaw**",
    "created_at": "2011-01-11T17:12:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8355#issuecomment-67463",
    "user": "https://github.com/adeines"
}
```

Changed author from **Aly Deines** to **Aly Deines, Robert Bradshaw**



---

archive/issue_comments_067464.json:
```json
{
    "body": "Changed reviewer from **Willem Jan Palenstijn** to **Tom Boothby, Willem Jan Palenstijn**",
    "created_at": "2011-01-11T17:37:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8355#issuecomment-67464",
    "user": "https://github.com/wjp"
}
```

Changed reviewer from **Willem Jan Palenstijn** to **Tom Boothby, Willem Jan Palenstijn**



---

archive/issue_comments_067465.json:
```json
{
    "body": "<a id='comment:15'>Comment 15:</a>\nWell, up to you guys.  It just seemed to me that since there wasn't anything left to patch, maybe the older ones weren't relevant - but you can give credit where you feel it's due!",
    "created_at": "2011-01-11T17:42:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8355#issuecomment-67465",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:15'>Comment 15:</a>
Well, up to you guys.  It just seemed to me that since there wasn't anything left to patch, maybe the older ones weren't relevant - but you can give credit where you feel it's due!



---

archive/issue_events_092214.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-17T20:52:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8355#event-92214"
}
```



---

archive/issue_events_092215.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-17T20:52:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8355#event-92215"
}
```



---

archive/issue_comments_067466.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nPlease add the **correct** ticket number to the commit message :-)",
    "created_at": "2011-01-17T20:52:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8355#issuecomment-67466",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:16'>Comment 16:</a>
Please add the **correct** ticket number to the commit message :-)



---

archive/issue_comments_067467.json:
```json
{
    "body": "Use only this patch",
    "created_at": "2011-01-17T21:00:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8355#issuecomment-67467",
    "user": "https://github.com/kcrisman"
}
```

Use only this patch



---

archive/issue_comments_067468.json:
```json
{
    "body": "<a id='comment:17'>Comment 17:</a>\nAttachment: **[8355-hue.2.2.patch.gz](https://github.com/sagemath/sage/files/ticket8355/8355-hue.2.2.patch.gz)**\n\nThis should fix this.  Apply only 8355-hue.2.2.patch",
    "created_at": "2011-01-17T21:01:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8355#issuecomment-67468",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:17'>Comment 17:</a>
Attachment: **[8355-hue.2.2.patch.gz](https://github.com/sagemath/sage/files/ticket8355/8355-hue.2.2.patch.gz)**

This should fix this.  Apply only 8355-hue.2.2.patch



---

archive/issue_events_092216.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-01-17T21:01:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8355#event-92216"
}
```



---

archive/issue_events_092217.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-01-17T21:01:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8355#event-92217"
}
```



---

archive/issue_events_092218.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-01-17T21:01:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8355#event-92218"
}
```



---

archive/issue_events_092219.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-01-17T21:01:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8355#event-92219"
}
```



---

archive/issue_comments_067469.json:
```json
{
    "body": "<a id='comment:19'>Comment 19:</a>\nI didn't have anything to do with the review of the patch going in... I think it's weird to get credit for this.",
    "created_at": "2011-01-17T21:17:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8355#issuecomment-67469",
    "user": "https://github.com/sagetrac-boothby"
}
```

<a id='comment:19'>Comment 19:</a>
I didn't have anything to do with the review of the patch going in... I think it's weird to get credit for this.



---

archive/issue_comments_067470.json:
```json
{
    "body": "Changed reviewer from **Tom Boothby, Willem Jan Palenstijn** to **Willem Jan Palenstijn**",
    "created_at": "2011-01-17T21:17:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8355#issuecomment-67470",
    "user": "https://github.com/sagetrac-boothby"
}
```

Changed reviewer from **Tom Boothby, Willem Jan Palenstijn** to **Willem Jan Palenstijn**



---

archive/issue_comments_067471.json:
```json
{
    "body": "Merged: **sage-4.6.2.alpha1**",
    "created_at": "2011-01-19T22:19:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8355#issuecomment-67471",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.6.2.alpha1**



---

archive/issue_events_092220.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-19T22:19:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8355#event-92220"
}
```



---

archive/issue_events_092221.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-01-19T22:19:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8355",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8355#event-92221"
}
```
