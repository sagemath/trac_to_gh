# Issue 8179: configure Mercurial to ignore two binaries by cddlib

archive/issues_008179.json:
```json
{
    "body": "The package cddlib-094f.p?.spkg places two new binaries under the directory $SAGE_LOCAL/bin, in particular:\n\n```\n[mvngu@mod bin]$ pwd\n/scratch/mvngu/release/sage-4.3.2.alpha1/local/bin\n[mvngu@mod bin]$ hg st\n? cdd_both_reps\n? cdd_both_reps_gmp\n```\nThese are required by the rewritten polyhedra package at #7109. A patch to $SAGE_LOCAL/bin/.hgignore is attached. This ticket is a prerequisite for #8115. \n\n**CC:**  @vbraun mhampton\n\n**Author:** Volker Braun\n\n**Reviewer:** Minh Van Nguyen\n\n**Merged:** sage-4.3.2.rc0\n\nIssue created by migration from https://trac.sagemath.org/ticket/8179\n\n",
    "closed_at": "2010-02-03T22:12:02Z",
    "created_at": "2010-02-03T19:41:59Z",
    "labels": [
        "component: misc",
        "critical",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3.2",
    "title": "configure Mercurial to ignore two binaries by cddlib",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/8179",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```
The package cddlib-094f.p?.spkg places two new binaries under the directory $SAGE_LOCAL/bin, in particular:

```
[mvngu@mod bin]$ pwd
/scratch/mvngu/release/sage-4.3.2.alpha1/local/bin
[mvngu@mod bin]$ hg st
? cdd_both_reps
? cdd_both_reps_gmp
```
These are required by the rewritten polyhedra package at #7109. A patch to $SAGE_LOCAL/bin/.hgignore is attached. This ticket is a prerequisite for #8115. 

**CC:**  @vbraun mhampton

**Author:** Volker Braun

**Reviewer:** Minh Van Nguyen

**Merged:** sage-4.3.2.rc0

Issue created by migration from https://trac.sagemath.org/ticket/8179





---

archive/issue_comments_064985.json:
```json
{
    "body": "apply to script repository",
    "created_at": "2010-02-03T19:43:14Z",
    "issue": "https://github.com/sagemath/sage/issues/8179",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8179#issuecomment-64985",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

apply to script repository



---

archive/issue_comments_064986.json:
```json
{
    "body": "**Author:** Volker Braun",
    "created_at": "2010-02-03T19:45:00Z",
    "issue": "https://github.com/sagemath/sage/issues/8179",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8179#issuecomment-64986",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Author:** Volker Braun



---

archive/issue_events_057135.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-02-03T19:45:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8179",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8179#event-57135"
}
```



---

archive/attachments_010093.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_8179-cddlib-local-bin-hgignore.patch",
    "asset_url": "tarball://root/attachments/ticket8179/trac_8179-cddlib-local-bin-hgignore.patch",
    "created_at": "2010-02-03T19:45:00Z",
    "issue": "https://github.com/sagemath/sage/issues/8179",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket8179/trac_8179-cddlib-local-bin-hgignore.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```



---

archive/issue_comments_064987.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [trac_8179-cddlib-local-bin-hgignore.patch](https://github.com/sagemath/sage/files/ticket8179/trac_8179-cddlib-local-bin-hgignore.patch) by mvngu created at 2010-02-03 19:45:00",
    "created_at": "2010-02-03T19:45:00Z",
    "issue": "https://github.com/sagemath/sage/issues/8179",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8179#issuecomment-64987",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:1'></a>
Attachment [trac_8179-cddlib-local-bin-hgignore.patch](https://github.com/sagemath/sage/files/ticket8179/trac_8179-cddlib-local-bin-hgignore.patch) by mvngu created at 2010-02-03 19:45:00



---

archive/issue_comments_064988.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -7,4 +7,4 @@\n ? cdd_both_reps\n ? cdd_both_reps_gmp\n ```\n-These are required by the rewritten polyhedra package at #7109. A patch to $SAGE_LOCAL/bin/.hgignore is attached.\n+These are required by the rewritten polyhedra package at #7109. A patch to $SAGE_LOCAL/bin/.hgignore is attached. This ticket is a prerequisite for #8115. \n``````\n",
    "created_at": "2010-02-03T19:45:00Z",
    "issue": "https://github.com/sagemath/sage/issues/8179",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8179#issuecomment-64988",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -7,4 +7,4 @@
 ? cdd_both_reps
 ? cdd_both_reps_gmp
 ```
-These are required by the rewritten polyhedra package at #7109. A patch to $SAGE_LOCAL/bin/.hgignore is attached.
+These are required by the rewritten polyhedra package at #7109. A patch to $SAGE_LOCAL/bin/.hgignore is attached. This ticket is a prerequisite for #8115. 
``````




---

archive/issue_comments_064989.json:
```json
{
    "body": "<a id='comment:3'></a>\nLooks good to me.",
    "created_at": "2010-02-03T22:11:00Z",
    "issue": "https://github.com/sagemath/sage/issues/8179",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8179#issuecomment-64989",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:3'></a>
Looks good to me.



---

archive/issue_events_057136.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-02-03T22:11:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8179",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8179#event-57136"
}
```



---

archive/issue_events_057137.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-02-03T22:11:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8179",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8179#event-57137"
}
```



---

archive/issue_comments_064990.json:
```json
{
    "body": "**Reviewer:** Minh Van Nguyen",
    "created_at": "2010-02-03T22:11:00Z",
    "issue": "https://github.com/sagemath/sage/issues/8179",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8179#issuecomment-64990",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Reviewer:** Minh Van Nguyen



---

archive/issue_comments_064991.json:
```json
{
    "body": "**Merged:** sage-4.3.2.rc0",
    "created_at": "2010-02-03T22:12:02Z",
    "issue": "https://github.com/sagemath/sage/issues/8179",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8179#issuecomment-64991",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Merged:** sage-4.3.2.rc0



---

archive/issue_events_057138.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-02-03T22:12:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8179",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8179#event-57138"
}
```



---

archive/issue_events_057139.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-02-03T22:12:02Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8179",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8179#event-57139"
}
```



---

archive/issue_comments_064992.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged [trac_8179-cddlib-local-bin-hgignore.patch](https://github.com/sagemath/sage/files/ticket8179/trac_8179-cddlib-local-bin-hgignore.patch). vbraun --- please remember to put your username and the ticket number on your patches. I have committed the above patch in your name.",
    "created_at": "2010-02-03T22:12:02Z",
    "issue": "https://github.com/sagemath/sage/issues/8179",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8179#issuecomment-64992",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:4'></a>
Merged [trac_8179-cddlib-local-bin-hgignore.patch](https://github.com/sagemath/sage/files/ticket8179/trac_8179-cddlib-local-bin-hgignore.patch). vbraun --- please remember to put your username and the ticket number on your patches. I have committed the above patch in your name.
