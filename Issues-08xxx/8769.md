...
```json
{
    "body": "<a id='comment:1'></a>Machines where this can be easily replicated (on skynet):\n\n* flavius\n\n* taurus",
...
<a id='comment:1'></a>Machines where this can be easily replicated (on skynet):
...
```json
{
    "body": "<a id='comment:2'></a>* This turned out to be a bug in the system-wide LD_LIBRARY_PATH on flavius. \n\n  * On taurus there was also an LD_LIBRARY_PATH mistake.\n\nSo not a sage issue; case closed.",
...
<a id='comment:2'></a>* This turned out to be a bug in the system-wide LD_LIBRARY_PATH on flavius. 
