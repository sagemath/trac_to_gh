# Issue 8057: New boehm-gc-7.1.p3.spkg works with Open Solaris x64 as 64-bit

archive/issues_008057.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-drkirkby"
    ],
    "body": "I made spkg-install work with SunOS with SAGE64=\"yes\"\n\nThe new spkg can be found here:\n[http://boxen.math.washington.edu/home/jsp/ports/boehm_gc-7.1.p3.spkg](http://boxen.math.washington.edu/home/jsp/ports/boehm_gc-7.1.p3.spkg)\n\nJaap\n\n\n\nComponent: **porting**\n\nAuthor: **Jaap Spies**\n\nReviewer: **David Kirkby**\n\nMerged: **sage-4.3.2.alpha1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8057_\n\n",
    "closed_at": "2010-02-01T00:41:22Z",
    "created_at": "2010-01-25T18:19:56Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20porting",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "New boehm-gc-7.1.p3.spkg works with Open Solaris x64 as 64-bit",
    "type": "issue",
    "updated_at": "2010-02-01T00:41:22Z",
    "url": "https://github.com/sagemath/sage/issues/8057",
    "user": "https://github.com/jaapspies"
}
```
I made spkg-install work with SunOS with SAGE64="yes"

The new spkg can be found here:
[http://boxen.math.washington.edu/home/jsp/ports/boehm_gc-7.1.p3.spkg](http://boxen.math.washington.edu/home/jsp/ports/boehm_gc-7.1.p3.spkg)

Jaap



Component: **porting**

Author: **Jaap Spies**

Reviewer: **David Kirkby**

Merged: **sage-4.3.2.alpha1**

_Issue created by migration from https://trac.sagemath.org/ticket/8057_





---

archive/issue_events_100164.json:
```json
{
    "actor": "https://github.com/jaapspies",
    "created_at": "2010-01-25T18:19:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8057",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20porting",
    "label_color": "000080",
    "label_name": "c: porting",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8057#event-100164"
}
```



---

archive/issue_events_100165.json:
```json
{
    "actor": "https://github.com/jaapspies",
    "created_at": "2010-01-25T18:19:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8057",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8057#event-100165"
}
```



---

archive/issue_events_100166.json:
```json
{
    "actor": "https://github.com/jaapspies",
    "created_at": "2010-01-25T18:19:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8057",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8057#event-100166"
}
```



---

archive/issue_events_100167.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-01-25T18:19:56Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/8057",
    "subject": "https://github.com/jaapspies",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8057#event-100167"
}
```



---

archive/issue_events_100168.json:
```json
{
    "actor": "https://github.com/jaapspies",
    "created_at": "2010-01-25T18:20:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8057",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8057#event-100168"
}
```



---

archive/issue_comments_061881.json:
```json
{
    "body": "Attachment: **[boehm_gc-7.1.p3.patch.gz](https://github.com/sagemath/sage/files/ticket8057/boehm_gc-7.1.p3.patch.gz)**",
    "created_at": "2010-01-26T17:24:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8057#issuecomment-61881",
    "user": "https://github.com/jaapspies"
}
```

Attachment: **[boehm_gc-7.1.p3.patch.gz](https://github.com/sagemath/sage/files/ticket8057/boehm_gc-7.1.p3.patch.gz)**



---

archive/issue_events_100169.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-01-27T14:01:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8057",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8057#event-100169"
}
```



---

archive/issue_events_100170.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-01-27T14:01:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8057",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8057#event-100170"
}
```



---

archive/issue_comments_061882.json:
```json
{
    "body": "Reviewer: **David Kirkby**",
    "created_at": "2010-01-27T14:01:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8057#issuecomment-61882",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

Reviewer: **David Kirkby**



---

archive/issue_comments_061883.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThat's fine. My usual comment it would be better to create a trac ticket for the defect first, then the ticket number can be put in SPKG.txt.",
    "created_at": "2010-01-27T14:01:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8057#issuecomment-61883",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

<div id="comment:2" align="right">comment:2</div>

That's fine. My usual comment it would be better to create a trac ticket for the defect first, then the ticket number can be put in SPKG.txt.



---

archive/issue_events_100171.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2010-01-27T14:01:36Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/8057",
    "title_is": "New boehm-gc-7.1.p3.spkg works with Open Solaris x64 as 64-bit",
    "title_was": "New boehm-gc-7.1.p3.spkg works with Open Solaris 64 bit",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8057#event-100171"
}
```



---

archive/issue_events_100172.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-02-01T00:41:22Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8057",
    "milestone_number": null,
    "milestone_title": "sage-4.3.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8057#event-100172"
}
```



---

archive/issue_comments_061884.json:
```json
{
    "body": "Merged: **sage-4.3.2.alpha1**",
    "created_at": "2010-02-01T00:41:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8057#issuecomment-61884",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Merged: **sage-4.3.2.alpha1**



---

archive/issue_comments_061885.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged [boehm_gc-7.1.p3.spkg](http://boxen.math.washington.edu/home/jsp/ports/boehm_gc-7.1.p3.spkg) in the standard spkg repository.",
    "created_at": "2010-02-01T00:41:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8057#issuecomment-61885",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged [boehm_gc-7.1.p3.spkg](http://boxen.math.washington.edu/home/jsp/ports/boehm_gc-7.1.p3.spkg) in the standard spkg repository.



---

archive/issue_events_100173.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-02-01T00:41:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8057",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8057#event-100173"
}
```



---

archive/issue_events_100174.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-02-01T00:41:22Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8057",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8057#event-100174"
}
```
