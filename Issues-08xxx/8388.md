# Issue 8388: pickle the paths of Rauzy diagrams

archive/issues_008388.json:
```json
{
    "assignees": [],
    "body": "There is a pickle error with the nested class RauzyDiagram.Path in sage.combinat.iet.template\n\n```\nsage: p = iet.Permutation('a b c','c b a')\nsage: r = p.rauzy_diagram()\nsage: g = r.path(p, 't', 'b')\nsage: dumps(g)\nPicklingError Traceback(most recent call last)\n...\nPicklingError: Can't pickle <class 'sage.combinat.iet.labelled.Path'>: attribute lookup sage.combinat.iet.labelled.Path failed\n```\n\nA `__metaclass__` must be defined for RauzyDiagram.\n\nThe ticket depends on #8386 which moves the module for interval exchange transformations from sage.combinat.iet to sage.dynamics.interval_exchanges\n\nApply only [attachment: trac_8388_pickling_path.v2.patch.gz](https://github.com/sagemath/sage/files/ticket8388/trac_8388_pickling_path.v2.patch.gz)\n\nDepends on #8386\n\nAssignee: **@videlec**\n\nCC:  sage-combinat tmonteil\n\nKeywords: **pickle, nested class**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton, Nathann Cohen**\n\nAuthor: **Vincent Delecroix**\n\nMerged: **sage-5.13.beta0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8388_\n\n",
    "closed_at": "2013-10-07T06:49:17Z",
    "created_at": "2010-02-27T16:55:51Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.13",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "pickle the paths of Rauzy diagrams",
    "type": "issue",
    "updated_at": "2013-10-07T06:49:17Z",
    "url": "https://github.com/sagemath/sage/issues/8388",
    "user": "https://github.com/videlec"
}
```
There is a pickle error with the nested class RauzyDiagram.Path in sage.combinat.iet.template

```
sage: p = iet.Permutation('a b c','c b a')
sage: r = p.rauzy_diagram()
sage: g = r.path(p, 't', 'b')
sage: dumps(g)
PicklingError Traceback(most recent call last)
...
PicklingError: Can't pickle <class 'sage.combinat.iet.labelled.Path'>: attribute lookup sage.combinat.iet.labelled.Path failed
```

A `__metaclass__` must be defined for RauzyDiagram.

The ticket depends on #8386 which moves the module for interval exchange transformations from sage.combinat.iet to sage.dynamics.interval_exchanges

Apply only [attachment: trac_8388_pickling_path.v2.patch.gz](https://github.com/sagemath/sage/files/ticket8388/trac_8388_pickling_path.v2.patch.gz)

Depends on #8386

Assignee: **@videlec**

CC:  sage-combinat tmonteil

Keywords: **pickle, nested class**

Reviewer: **Frédéric Chapoton, Nathann Cohen**

Author: **Vincent Delecroix**

Merged: **sage-5.13.beta0**

_Issue created by migration from https://trac.sagemath.org/ticket/8388_





---

archive/issue_events_092663.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2010-02-27T16:55:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "milestone_number": null,
    "milestone_title": "sage-5.13",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8388#event-92663"
}
```



---

archive/issue_events_092664.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2010-02-27T16:55:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8388#event-92664"
}
```



---

archive/issue_events_092665.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2010-02-27T16:55:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8388#event-92665"
}
```



---

archive/issue_events_092666.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2010-02-27T16:55:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8388#event-92666"
}
```



---

archive/issue_comments_067905.json:
```json
{
    "body": "Changed keywords from **pickle of a nested class** to **pickle, nested class**",
    "created_at": "2010-02-27T17:00:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8388#issuecomment-67905",
    "user": "https://github.com/videlec"
}
```

Changed keywords from **pickle of a nested class** to **pickle, nested class**



---

archive/issue_comments_067906.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nReplying to [@videlec](#comment%3A1):\nDo you want it to be reviewed (you didn't check need review). I'm volunteering to review it as soon as #8386 is reviewed.",
    "created_at": "2010-02-27T18:31:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8388#issuecomment-67906",
    "user": "https://github.com/hivert"
}
```

<a id='comment:2'>Comment 2:</a>
Replying to [@videlec](#comment%3A1):
Do you want it to be reviewed (you didn't check need review). I'm volunteering to review it as soon as #8386 is reviewed.



---

archive/issue_events_092667.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2010-02-27T22:28:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8388#event-92667"
}
```



---

archive/issue_events_092668.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2010-02-27T22:28:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8388#event-92668"
}
```



---

archive/issue_events_092669.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2010-02-27T22:28:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8388#event-92669"
}
```



---

archive/issue_comments_067907.json:
```json
{
    "body": "Dependencies: **#8386**",
    "created_at": "2011-06-11T20:29:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8388#issuecomment-67907",
    "user": "https://github.com/fchapoton"
}
```

Dependencies: **#8386**



---

archive/issue_comments_067908.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nplease add a commit message to your patch (using sage -hg qrefresh -e) starting with #8388 (to make the bot more happy)",
    "created_at": "2011-06-11T20:29:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8388#issuecomment-67908",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:5'>Comment 5:</a>
please add a commit message to your patch (using sage -hg qrefresh -e) starting with #8388 (to make the bot more happy)



---

archive/issue_comments_067909.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nDoes it really depends on #8386 ? The buildbot was happier before I added this dependency.",
    "created_at": "2011-06-11T21:08:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8388#issuecomment-67909",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:6'>Comment 6:</a>
Does it really depends on #8386 ? The buildbot was happier before I added this dependency.



---

archive/issue_comments_067910.json:
```json
{
    "body": "Changed dependencies from **#8386** to none",
    "created_at": "2011-06-15T10:20:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8388#issuecomment-67910",
    "user": "https://github.com/fchapoton"
}
```

Changed dependencies from **#8386** to none



---

archive/issue_comments_067911.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nHellooo !!!!\n\nThis ticket can be set to positive review after two changes :\n\n* That it be rebased against a recent version of Sage. The patch applies with a hunk right now\n* That the commit message be changed so as to contain the ticket's number, or the release manager will have to shout `:-D` \n\nNathann",
    "created_at": "2011-10-01T15:10:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8388#issuecomment-67911",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:8'>Comment 8:</a>
Hellooo !!!!

This ticket can be set to positive review after two changes :

* That it be rebased against a recent version of Sage. The patch applies with a hunk right now
* That the commit message be changed so as to contain the ticket's number, or the release manager will have to shout `:-D` 

Nathann



---

archive/issue_events_092670.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2011-10-01T15:10:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8388#event-92670"
}
```



---

archive/issue_events_092671.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2011-10-01T15:10:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8388#event-92671"
}
```



---

archive/issue_comments_067912.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nOh, and let me just add that I was glad to review this ticket : I had to learn what a metaclass was. Great explanation there :\n\nhttp://stackoverflow.com/questions/100003/what-is-a-metaclass-in-python\n\nNathann",
    "created_at": "2011-10-01T15:11:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8388#issuecomment-67912",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:9'>Comment 9:</a>
Oh, and let me just add that I was glad to review this ticket : I had to learn what a metaclass was. Great explanation there :

http://stackoverflow.com/questions/100003/what-is-a-metaclass-in-python

Nathann



---

archive/issue_comments_067913.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -11,3 +11,5 @@\n ```\n \n A `__metaclass__` must be defined for RauzyDiagram.\n+\n+The ticket depends on #8386 which moves the module for interval exchange transformations from sage.combinat.iet to sage.dynamics.interval_exchanges\n``````\n",
    "created_at": "2012-03-13T00:45:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8388#issuecomment-67913",
    "user": "https://github.com/videlec"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -11,3 +11,5 @@
 ```
 
 A `__metaclass__` must be defined for RauzyDiagram.
+
+The ticket depends on #8386 which moves the module for interval exchange transformations from sage.combinat.iet to sage.dynamics.interval_exchanges
``````




---

archive/issue_comments_067914.json:
```json
{
    "body": "Dependencies: **#8386**",
    "created_at": "2012-03-13T00:45:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8388#issuecomment-67914",
    "user": "https://github.com/videlec"
}
```

Dependencies: **#8386**



---

archive/issue_comments_067915.json:
```json
{
    "body": "Attachment: **[trac_8388_pickling_path.patch.gz](https://github.com/sagemath/sage/files/ticket8388/trac_8388_pickling_path.patch.gz)**\n\nDepends on ticket 8386.",
    "created_at": "2012-03-13T00:56:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8388#issuecomment-67915",
    "user": "https://github.com/videlec"
}
```

Attachment: **[trac_8388_pickling_path.patch.gz](https://github.com/sagemath/sage/files/ticket8388/trac_8388_pickling_path.patch.gz)**

Depends on ticket 8386.



---

archive/issue_comments_067916.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nThe test error was due to 8386.",
    "created_at": "2012-04-29T19:44:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8388#issuecomment-67916",
    "user": "https://github.com/videlec"
}
```

<a id='comment:11'>Comment 11:</a>
The test error was due to 8386.



---

archive/issue_events_092672.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2012-04-29T19:44:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8388#event-92672"
}
```



---

archive/issue_events_092673.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2012-04-29T19:44:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8388#event-92673"
}
```



---

archive/issue_comments_067917.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nPlease fill in your real name as Author.",
    "created_at": "2012-07-27T20:42:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8388#issuecomment-67917",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:12'>Comment 12:</a>
Please fill in your real name as Author.



---

archive/issue_comments_067918.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -13,3 +13,5 @@\n A `__metaclass__` must be defined for RauzyDiagram.\n \n The ticket depends on #8386 which moves the module for interval exchange transformations from sage.combinat.iet to sage.dynamics.interval_exchanges\n+\n+Apply [attachment: trac_8388_pickling_path.patch.gz](https://github.com/sagemath/sage/files/ticket8388/trac_8388_pickling_path.patch.gz)\n``````\n",
    "created_at": "2012-08-29T19:42:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8388#issuecomment-67918",
    "user": "https://github.com/fchapoton"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -13,3 +13,5 @@
 A `__metaclass__` must be defined for RauzyDiagram.
 
 The ticket depends on #8386 which moves the module for interval exchange transformations from sage.combinat.iet to sage.dynamics.interval_exchanges
+
+Apply [attachment: trac_8388_pickling_path.patch.gz](https://github.com/sagemath/sage/files/ticket8388/trac_8388_pickling_path.patch.gz)
``````




---

archive/issue_comments_067919.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\napply trac_8388_pickling_path.patch",
    "created_at": "2012-08-29T19:42:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8388#issuecomment-67919",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:13'>Comment 13:</a>
apply trac_8388_pickling_path.patch



---

archive/issue_comments_067920.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\napply trac_8388_pickling_path.v2.patch",
    "created_at": "2012-08-29T19:52:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8388#issuecomment-67920",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:14'>Comment 14:</a>
apply trac_8388_pickling_path.v2.patch



---

archive/issue_comments_067921.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -14,4 +14,4 @@\n \n The ticket depends on #8386 which moves the module for interval exchange transformations from sage.combinat.iet to sage.dynamics.interval_exchanges\n \n-Apply [attachment: trac_8388_pickling_path.patch.gz](https://github.com/sagemath/sage/files/ticket8388/trac_8388_pickling_path.patch.gz)\n+Apply only [attachment: trac_8388_pickling_path.v2.patch.gz](https://github.com/sagemath/sage/files/ticket8388/trac_8388_pickling_path.v2.patch.gz)\n``````\n",
    "created_at": "2012-08-29T19:52:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8388#issuecomment-67921",
    "user": "https://github.com/fchapoton"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -14,4 +14,4 @@
 
 The ticket depends on #8386 which moves the module for interval exchange transformations from sage.combinat.iet to sage.dynamics.interval_exchanges
 
-Apply [attachment: trac_8388_pickling_path.patch.gz](https://github.com/sagemath/sage/files/ticket8388/trac_8388_pickling_path.patch.gz)
+Apply only [attachment: trac_8388_pickling_path.v2.patch.gz](https://github.com/sagemath/sage/files/ticket8388/trac_8388_pickling_path.v2.patch.gz)
``````




---

archive/issue_comments_067922.json:
```json
{
    "body": "<a id='comment:15'>Comment 15:</a>\napply trac_8388_pickling_path.v2.patch",
    "created_at": "2013-05-23T20:15:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8388#issuecomment-67922",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:15'>Comment 15:</a>
apply trac_8388_pickling_path.v2.patch



---

archive/issue_events_092674.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8388#event-92674"
}
```



---

archive/issue_events_092675.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8388#event-92675"
}
```



---

archive/issue_comments_067923.json:
```json
{
    "body": "Attachment: **[trac_8388_pickling_path.v2.patch.gz](https://github.com/sagemath/sage/files/ticket8388/trac_8388_pickling_path.v2.patch.gz)**",
    "created_at": "2013-08-29T11:11:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8388#issuecomment-67923",
    "user": "https://github.com/fchapoton"
}
```

Attachment: **[trac_8388_pickling_path.v2.patch.gz](https://github.com/sagemath/sage/files/ticket8388/trac_8388_pickling_path.v2.patch.gz)**



---

archive/issue_comments_067924.json:
```json
{
    "body": "<a id='comment:17'>Comment 17:</a>\napply trac_8388_pickling_path.v2.patch",
    "created_at": "2013-08-29T11:12:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8388#issuecomment-67924",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:17'>Comment 17:</a>
apply trac_8388_pickling_path.v2.patch



---

archive/issue_comments_067925.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton, Nathann Cohen**",
    "created_at": "2013-08-29T11:13:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8388#issuecomment-67925",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton, Nathann Cohen**



---

archive/issue_comments_067926.json:
```json
{
    "body": "Changed author from **vdelecroix** to **Vincent Delecroix**",
    "created_at": "2013-08-29T11:13:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8388#issuecomment-67926",
    "user": "https://github.com/fchapoton"
}
```

Changed author from **vdelecroix** to **Vincent Delecroix**



---

archive/issue_comments_067927.json:
```json
{
    "body": "<a id='comment:19'>Comment 19:</a>\nok, good enough for me",
    "created_at": "2013-09-15T14:52:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8388#issuecomment-67927",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:19'>Comment 19:</a>
ok, good enough for me



---

archive/issue_events_092676.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-09-15T14:52:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8388#event-92676"
}
```



---

archive/issue_events_092677.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-09-15T14:52:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8388#event-92677"
}
```



---

archive/issue_events_092678.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-01T07:19:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8388#event-92678"
}
```



---

archive/issue_events_092679.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-01T07:19:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "milestone_number": null,
    "milestone_title": "sage-5.13",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8388#event-92679"
}
```



---

archive/issue_comments_067928.json:
```json
{
    "body": "Merged: **sage-5.13.beta0**",
    "created_at": "2013-10-07T06:49:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8388#issuecomment-67928",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.13.beta0**



---

archive/issue_events_092680.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-07T06:49:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8388#event-92680"
}
```



---

archive/issue_events_092681.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-07T06:49:17Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8388",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8388#event-92681"
}
```
