# Issue 8910: Have CombinatorialClass inherits from Parent

archive/issues_008910.json:
```json
{
    "body": "This is the first step of the cleanup of the combinatorial classes see\n  http://trac.sagemath.org/sage_trac/wiki/SageCombinatRoadMap\n\nDepend on #8881\n\nApply the two patches:\n\n- [trac_8910-combinatorial_class_parent-fh.patch](http://trac.sagemath.org/sage_trac/attachment/ticket/8910/trac_8910-combinatorial_class_parent-fh.patch)\n- [trac_8910-subsets_an_element-fh.patch](http://trac.sagemath.org/sage_trac/attachment/ticket/8910/trac_8910-subsets_an_element-fh.patch)\n\n\nAssignee: @hivert\n\nCC:  sage-combinat\n\nKeywords: CombinatorialClass Parent\n\nResolution: fixed\n\nAuthor: Florent Hivert\n\nReviewer: Nicolas M. Thi\u00e9ry\n\nMerged: sage-4.4.4.alpha0\n\nIssue created by migration from https://trac.sagemath.org/ticket/8910\n\n",
    "closed_at": "2010-06-05T22:21:59Z",
    "created_at": "2010-05-07T12:02:11Z",
    "labels": [
        "component: combinatorics",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.4.4",
    "title": "Have CombinatorialClass inherits from Parent",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/8910",
    "user": "https://github.com/hivert"
}
```
This is the first step of the cleanup of the combinatorial classes see
  http://trac.sagemath.org/sage_trac/wiki/SageCombinatRoadMap

Depend on #8881

Apply the two patches:

- [trac_8910-combinatorial_class_parent-fh.patch](http://trac.sagemath.org/sage_trac/attachment/ticket/8910/trac_8910-combinatorial_class_parent-fh.patch)
- [trac_8910-subsets_an_element-fh.patch](http://trac.sagemath.org/sage_trac/attachment/ticket/8910/trac_8910-subsets_an_element-fh.patch)


Assignee: @hivert

CC:  sage-combinat

Keywords: CombinatorialClass Parent

Resolution: fixed

Author: Florent Hivert

Reviewer: Nicolas M. Thiéry

Merged: sage-4.4.4.alpha0

Issue created by migration from https://trac.sagemath.org/ticket/8910





---

archive/issue_comments_097408.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,5 +1,7 @@\n This is the first step of the cleanup of the combinatorial classes see \n   http://trac.sagemath.org/sage_trac/wiki/SageCombinatRoadMap\n+\n+Patch in progress and review on sage-combinat queue. \n \n Author: Florent Hivert\n \n``````\n",
    "created_at": "2010-05-12T17:47:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8910#issuecomment-97408",
    "user": "https://github.com/hivert"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,5 +1,7 @@
 This is the first step of the cleanup of the combinatorial classes see 
   http://trac.sagemath.org/sage_trac/wiki/SageCombinatRoadMap
+
+Patch in progress and review on sage-combinat queue. 
 
 Author: Florent Hivert
 
``````




---

archive/issue_comments_097409.json:
```json
{
    "body": "Changing status from new to needs_work.",
    "created_at": "2010-05-12T17:47:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8910#issuecomment-97409",
    "user": "https://github.com/hivert"
}
```

Changing status from new to needs_work.



---

archive/issue_comments_097410.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Nicolas M. Thi\u00e9ry\"",
    "created_at": "2010-05-12T17:47:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8910#issuecomment-97410",
    "user": "https://github.com/hivert"
}
```

Changing reviewer from "" to "Nicolas M. Thiéry"



---

archive/issue_comments_097411.json:
```json
{
    "body": "<a id='comment:2'></a>Attachment [trac_8910-combinatorial_class_parent-fh.patch](tarball://root/attachments/some-uuid/ticket8910/trac_8910-combinatorial_class_parent-fh.patch) by @hivert created at 2010-05-31 15:37:21\n\nThe patch has been reviewed by Nicolas on the patch queue. I folded the review patches:\n\n```\n$ hg qgoto trac_8910-subsets_an_element-fh.patch\n$ hg qfold trac_8910-subsets_an_element-review-nt.patch\n$ hg qfold trac_8910-subsets_an_element-review-review-fh.patch\n```\nAnd Nicolas said by e-mail\n\n```\n> D\u00e8s que j'ai ton feu vert, je folde tout, et met la positive review sur trac.\n\nFeu vert!\n```\nMeaning: (FH) as soon as I have your green light I fold the whole bunch, and set positive review on trac. (Answer from Nicolas) Green Light.\n\nTherefore I allowed myself to put the positive review.",
    "created_at": "2010-05-31T15:37:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8910#issuecomment-97411",
    "user": "https://github.com/hivert"
}
```

<a id='comment:2'></a>Attachment [trac_8910-combinatorial_class_parent-fh.patch](tarball://root/attachments/some-uuid/ticket8910/trac_8910-combinatorial_class_parent-fh.patch) by @hivert created at 2010-05-31 15:37:21

The patch has been reviewed by Nicolas on the patch queue. I folded the review patches:

```
$ hg qgoto trac_8910-subsets_an_element-fh.patch
$ hg qfold trac_8910-subsets_an_element-review-nt.patch
$ hg qfold trac_8910-subsets_an_element-review-review-fh.patch
```
And Nicolas said by e-mail

```
> Dès que j'ai ton feu vert, je folde tout, et met la positive review sur trac.

Feu vert!
```
Meaning: (FH) as soon as I have your green light I fold the whole bunch, and set positive review on trac. (Answer from Nicolas) Green Light.

Therefore I allowed myself to put the positive review.



---

archive/issue_comments_097412.json:
```json
{
    "body": "Changing keywords from \"\" to \"CombinatorialClass Parent\".",
    "created_at": "2010-05-31T15:37:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8910#issuecomment-97412",
    "user": "https://github.com/hivert"
}
```

Changing keywords from "" to "CombinatorialClass Parent".



---

archive/issue_comments_097413.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2010-05-31T15:37:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8910#issuecomment-97413",
    "user": "https://github.com/hivert"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_097414.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2010-05-31T15:38:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8910#issuecomment-97414",
    "user": "https://github.com/hivert"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_097415.json:
```json
{
    "body": "<a id='comment:3'></a>> Meaning: (FH) as soon as I have your green light I fold the whole bunch, and set positive review on trac. (Answer from Nicolas) Green Light.\n\n\n> Therefore I allowed myself to put the positive review.\n\n\nDone",
    "created_at": "2010-05-31T15:38:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8910#issuecomment-97415",
    "user": "https://github.com/hivert"
}
```

<a id='comment:3'></a>> Meaning: (FH) as soon as I have your green light I fold the whole bunch, and set positive review on trac. (Answer from Nicolas) Green Light.


> Therefore I allowed myself to put the positive review.


Done



---

archive/issue_comments_097416.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,9 @@\n+This is the first step of the cleanup of the combinatorial classes see \n+  http://trac.sagemath.org/sage_trac/wiki/SageCombinatRoadMap\n \n+Patch in progress and review on sage-combinat queue. \n+\n+Depend on #8881\n \n Author: Florent Hivert\n \n``````\n",
    "created_at": "2010-05-31T15:40:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8910#issuecomment-97416",
    "user": "https://github.com/hivert"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,9 @@
+This is the first step of the cleanup of the combinatorial classes see 
+  http://trac.sagemath.org/sage_trac/wiki/SageCombinatRoadMap
 
+Patch in progress and review on sage-combinat queue. 
+
+Depend on #8881
 
 Author: Florent Hivert
 
``````




---

archive/issue_comments_097417.json:
```json
{
    "body": "<a id='comment:4'></a>I forgot to tell: depend on #8881 (review in progress. Should be done soon).",
    "created_at": "2010-05-31T15:40:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8910#issuecomment-97417",
    "user": "https://github.com/hivert"
}
```

<a id='comment:4'></a>I forgot to tell: depend on #8881 (review in progress. Should be done soon).



---

archive/issue_comments_097418.json:
```json
{
    "body": "Attachment [trac_8910-subsets_an_element-fh.patch](tarball://root/attachments/some-uuid/ticket8910/trac_8910-subsets_an_element-fh.patch) by @hivert created at 2010-05-31 15:44:42",
    "created_at": "2010-05-31T15:44:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8910#issuecomment-97418",
    "user": "https://github.com/hivert"
}
```

Attachment [trac_8910-subsets_an_element-fh.patch](tarball://root/attachments/some-uuid/ticket8910/trac_8910-subsets_an_element-fh.patch) by @hivert created at 2010-05-31 15:44:42



---

archive/issue_comments_097419.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,12 @@\n+This is the first step of the cleanup of the combinatorial classes see\n+  http://trac.sagemath.org/sage_trac/wiki/SageCombinatRoadMap\n+\n+Depend on #8881\n+\n+Apply the two patches:\n+\n+- [trac_8910-combinatorial_class_parent-fh.patch](http://trac.sagemath.org/sage_trac/attachment/ticket/8910/trac_8910-combinatorial_class_parent-fh.patch)\n+- [trac_8910-subsets_an_element-fh.patch](http://trac.sagemath.org/sage_trac/attachment/ticket/8910/trac_8910-subsets_an_element-fh.patch)\n \n \n Author: Florent Hivert\n``````\n",
    "created_at": "2010-05-31T15:50:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8910#issuecomment-97419",
    "user": "https://github.com/hivert"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,12 @@
+This is the first step of the cleanup of the combinatorial classes see
+  http://trac.sagemath.org/sage_trac/wiki/SageCombinatRoadMap
+
+Depend on #8881
+
+Apply the two patches:
+
+- [trac_8910-combinatorial_class_parent-fh.patch](http://trac.sagemath.org/sage_trac/attachment/ticket/8910/trac_8910-combinatorial_class_parent-fh.patch)
+- [trac_8910-subsets_an_element-fh.patch](http://trac.sagemath.org/sage_trac/attachment/ticket/8910/trac_8910-subsets_an_element-fh.patch)
 
 
 Author: Florent Hivert
``````




---

archive/issue_comments_097420.json:
```json
{
    "body": "<a id='comment:6'></a>Note that this depends on #8902.",
    "created_at": "2010-06-05T22:19:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8910#issuecomment-97420",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:6'></a>Note that this depends on #8902.



---

archive/issue_comments_097421.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2010-06-05T22:21:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8910#issuecomment-97421",
    "user": "https://github.com/mwhansen"
}
```

Resolution: fixed



---

archive/issue_comments_097422.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-4.4.4.alpha0\"",
    "created_at": "2010-06-05T22:21:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8910#issuecomment-97422",
    "user": "https://github.com/mwhansen"
}
```

Changing merged from "" to "sage-4.4.4.alpha0"



---

archive/issue_events_021768.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-06-05T22:21:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/8910",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8910#event-21768"
}
```
