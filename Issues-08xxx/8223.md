# Issue 8223: tab completion broken for many parent objects

archive/issues_008223.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: P.<x,y> = QQ[]\nsage: dir(P)\nBOOM!\n```\n\nThis is caused by Nick Thiery's patch from trac #7921 which very badly defines a `__dir__` method for parents:\n\n```\nchangeset:   13746:6ad207aca83f\nuser:        Nicolas M. Thiery <nthiery@users.sf.net>\ndate:        Fri Jan 22 22:53:29 2010 +0100          \nsummary:     #7921: Categories for extension types via __getattr___\n```\n\nThe problem was reported at this [sage-devel](http://groups.google.com/group/sage-devel/browse_frm/thread/b559a8a6abf228ed) thread.\n\nAssignee: **@nthiery**\n\nCC:  @novoselt\n\nAuthor: **Nicolas M. Thi\u00e9ry**\n\nReviewer: **Andrey Novoseltsev**\n\nMerged: **sage-4.3.3.alpha1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8223_\n\n",
    "closed_at": "2010-02-17T20:41:35Z",
    "created_at": "2010-02-09T20:34:02Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "tab completion broken for many parent objects",
    "type": "issue",
    "updated_at": "2010-02-17T20:41:35Z",
    "url": "https://github.com/sagemath/sage/issues/8223",
    "user": "https://github.com/williamstein"
}
```

```
sage: P.<x,y> = QQ[]
sage: dir(P)
BOOM!
```

This is caused by Nick Thiery's patch from trac #7921 which very badly defines a `__dir__` method for parents:

```
changeset:   13746:6ad207aca83f
user:        Nicolas M. Thiery <nthiery@users.sf.net>
date:        Fri Jan 22 22:53:29 2010 +0100          
summary:     #7921: Categories for extension types via __getattr___
```

The problem was reported at this [sage-devel](http://groups.google.com/group/sage-devel/browse_frm/thread/b559a8a6abf228ed) thread.

Assignee: **@nthiery**

CC:  @novoselt

Author: **Nicolas M. Thiéry**

Reviewer: **Andrey Novoseltsev**

Merged: **sage-4.3.3.alpha1**

_Issue created by migration from https://trac.sagemath.org/ticket/8223_





---

archive/issue_events_090425.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-02-09T20:34:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8223",
    "milestone_number": null,
    "milestone_title": "sage-4.3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8223#event-90425"
}
```



---

archive/issue_events_090426.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-02-09T20:34:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8223",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8223#event-90426"
}
```



---

archive/issue_events_090427.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-02-09T20:34:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8223",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8223#event-90427"
}
```



---

archive/issue_comments_065509.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,17 @@\n \n ```\n+sage: P.<x,y> = QQ[]\n+sage: dir(P)\n+BOOM!\n+```\n \n+This is caused by Nick Thiery's patch from trac #7921 which very badly defines a `__dir__` method for parents:\n+\n+```\n+changeset:   13746:6ad207aca83f\n+user:        Nicolas M. Thiery <nthiery@users.sf.net>\n+date:        Fri Jan 22 22:53:29 2010 +0100          \n+summary:     #7921: Categories for extension types via __getattr___\n+```\n+\n+\n``````\n",
    "created_at": "2010-02-09T20:35:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8223",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8223#issuecomment-65509",
    "user": "https://github.com/williamstein"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,17 @@
 
 ```
+sage: P.<x,y> = QQ[]
+sage: dir(P)
+BOOM!
+```
 
+This is caused by Nick Thiery's patch from trac #7921 which very badly defines a `__dir__` method for parents:
+
+```
+changeset:   13746:6ad207aca83f
+user:        Nicolas M. Thiery <nthiery@users.sf.net>
+date:        Fri Jan 22 22:53:29 2010 +0100          
+summary:     #7921: Categories for extension types via __getattr___
+```
+
+
``````




---

archive/issue_comments_065510.json:
```json
{
    "body": "Author: **Nicolas M. Thi\u00e9ry**",
    "created_at": "2010-02-09T20:54:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8223",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8223#issuecomment-65510",
    "user": "https://github.com/nthiery"
}
```

Author: **Nicolas M. Thiéry**



---

archive/issue_comments_065511.json:
```json
{
    "body": "Assignee: **@nthiery**",
    "created_at": "2010-02-09T20:54:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8223",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8223#issuecomment-65511",
    "user": "https://github.com/nthiery"
}
```

Assignee: **@nthiery**



---

archive/issue_comments_065512.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nAttachment: **[trac_8223-fix_dir-nt.patch.gz](https://github.com/sagemath/sage/files/ticket8223/trac_8223-fix_dir-nt.patch.gz)**\n\nThe attached patch should fix the issue. That being said, I would love to see a more robust implementation of ``sage.structure.parent.dir_with_other_class``.",
    "created_at": "2010-02-09T20:56:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8223",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8223#issuecomment-65512",
    "user": "https://github.com/nthiery"
}
```

<a id='comment:3'>Comment 3:</a>
Attachment: **[trac_8223-fix_dir-nt.patch.gz](https://github.com/sagemath/sage/files/ticket8223/trac_8223-fix_dir-nt.patch.gz)**

The attached patch should fix the issue. That being said, I would love to see a more robust implementation of ``sage.structure.parent.dir_with_other_class``.



---

archive/issue_comments_065513.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -14,4 +14,4 @@\n summary:     #7921: Categories for extension types via __getattr___\n ```\n \n-\n+The problem was reported at this [sage-devel](http://groups.google.com/group/sage-devel/browse_frm/thread/b559a8a6abf228ed) thread.\n``````\n",
    "created_at": "2010-02-10T04:39:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8223",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8223#issuecomment-65513",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -14,4 +14,4 @@
 summary:     #7921: Categories for extension types via __getattr___
 ```
 
-
+The problem was reported at this [sage-devel](http://groups.google.com/group/sage-devel/browse_frm/thread/b559a8a6abf228ed) thread.
``````




---

archive/issue_comments_065514.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\non #sage-devel\n\n```\n00:39 < logix> fwiw, for me the patch in #8223 makes k.[TAB] work again (where e.g. k.<a>=GF(8) )\n```",
    "created_at": "2010-02-10T08:55:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8223",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8223#issuecomment-65514",
    "user": "https://github.com/haraldschilly"
}
```

<a id='comment:5'>Comment 5:</a>
on #sage-devel

```
00:39 < logix> fwiw, for me the patch in #8223 makes k.[TAB] work again (where e.g. k.<a>=GF(8) )
```



---

archive/issue_events_090428.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2010-02-15T22:49:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8223",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8223#event-90428"
}
```



---

archive/issue_comments_065515.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nLooks good to me.",
    "created_at": "2010-02-15T23:07:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8223",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8223#issuecomment-65515",
    "user": "https://github.com/novoselt"
}
```

<a id='comment:8'>Comment 8:</a>
Looks good to me.



---

archive/issue_events_090429.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2010-02-15T23:07:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8223",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8223#event-90429"
}
```



---

archive/issue_events_090430.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2010-02-15T23:07:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8223",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8223#event-90430"
}
```



---

archive/issue_comments_065516.json:
```json
{
    "body": "Reviewer: **Andrey Novoseltsev**",
    "created_at": "2010-02-15T23:07:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8223",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8223#issuecomment-65516",
    "user": "https://github.com/novoselt"
}
```

Reviewer: **Andrey Novoseltsev**



---

archive/issue_events_090431.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-02-17T20:41:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8223",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8223#event-90431"
}
```



---

archive/issue_events_090432.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-02-17T20:41:35Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8223",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8223#event-90432"
}
```



---

archive/issue_comments_065517.json:
```json
{
    "body": "Merged: **sage-4.3.3.alpha1**",
    "created_at": "2010-02-17T20:41:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8223",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8223#issuecomment-65517",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Merged: **sage-4.3.3.alpha1**



---

archive/issue_comments_065518.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nMerged [trac_8223-fix_dir-nt.patch](https://github.com/sagemath/sage/files/ticket8223/trac_8223-fix_dir-nt.patch.gz) with a sensible commit message containing the ticket number.",
    "created_at": "2010-02-17T20:41:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8223",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8223#issuecomment-65518",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<a id='comment:9'>Comment 9:</a>
Merged [trac_8223-fix_dir-nt.patch](https://github.com/sagemath/sage/files/ticket8223/trac_8223-fix_dir-nt.patch.gz) with a sensible commit message containing the ticket number.
