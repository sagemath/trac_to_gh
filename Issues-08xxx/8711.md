# Issue 8711: fix warnings when building the ref manual (4.4.alpha0)

archive/issues_008711.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mvngu"
    ],
    "body": "Here is a patch to fix the warnings...\n\nComponent: **documentation**\n\nAuthor: **John Palmieri**\n\nReviewer: **Minh Van Nguyen**\n\nMerged: **sage-4.4.alpha1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8711_\n\n",
    "closed_at": "2010-04-19T05:18:58Z",
    "created_at": "2010-04-18T04:45:17Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fix warnings when building the ref manual (4.4.alpha0)",
    "type": "issue",
    "updated_at": "2010-04-19T05:18:58Z",
    "url": "https://github.com/sagemath/sage/issues/8711",
    "user": "https://github.com/jhpalmieri"
}
```
Here is a patch to fix the warnings...

Component: **documentation**

Author: **John Palmieri**

Reviewer: **Minh Van Nguyen**

Merged: **sage-4.4.alpha1**

_Issue created by migration from https://trac.sagemath.org/ticket/8711_





---

archive/issue_events_110018.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-04-18T04:45:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8711",
    "milestone_number": null,
    "milestone_title": "sage-4.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8711#event-110018"
}
```



---

archive/issue_events_110019.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-04-18T04:45:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8711",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8711#event-110019"
}
```



---

archive/issue_events_110020.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-04-18T04:45:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8711",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8711#event-110020"
}
```



---

archive/issue_events_110021.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-04-18T04:45:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8711",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8711#event-110021"
}
```



---

archive/issue_events_110022.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-04-18T04:45:17Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/8711",
    "subject": "https://github.com/jhpalmieri",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8711#event-110022"
}
```



---

archive/issue_events_110023.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-04-18T07:06:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8711",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8711#event-110023"
}
```



---

archive/issue_comments_070461.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_8711-reference44alpha0.patch.gz](https://github.com/sagemath/sage/files/ticket8711/trac_8711-reference44alpha0.patch.gz)**",
    "created_at": "2010-04-18T07:06:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8711",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8711#issuecomment-70461",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_8711-reference44alpha0.patch.gz](https://github.com/sagemath/sage/files/ticket8711/trac_8711-reference44alpha0.patch.gz)**



---

archive/issue_events_110024.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-04-18T07:07:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8711",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8711#event-110024"
}
```



---

archive/issue_events_110025.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-04-18T07:07:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8711",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8711#event-110025"
}
```



---

archive/issue_comments_070462.json:
```json
{
    "body": "Reviewer: **Minh Van Nguyen**",
    "created_at": "2010-04-18T07:07:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8711",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8711#issuecomment-70462",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Reviewer: **Minh Van Nguyen**



---

archive/issue_comments_070463.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nBuilding the reference manual of Sage 4.4.alpha0 results in the following warnings:\n\n```\n/dev/shm/mvngu/sandbox/sage-4.4.alpha0-8711-ref/local/lib/python2.6/site-packages/sage/graphs/generic_graph.py:docstring of sage.graphs.generic_graph.GenericGraph.add_cycle:5: (ERROR/3) Unexpected indentation.\n/dev/shm/mvngu/sandbox/sage-4.4.alpha0-8711-ref/local/lib/python2.6/site-packages/sage/graphs/generic_graph.py:docstring of sage.graphs.generic_graph.GenericGraph.vertex_cover:26: (WARNING/2) Block quote ends without a blank line; unexpected unindent.\n/dev/shm/mvngu/sandbox/sage-4.4.alpha0-8711-ref/local/lib/python2.6/site-packages/sage/symbolic/units.py:docstring of sage.symbolic.units.unitdocs:15: (WARNING/2) Block quote ends without a blank line; unexpected unindent.\n/dev/shm/mvngu/sandbox/sage-4.4.alpha0-8711-ref/local/lib/python2.6/site-packages/sage/symbolic/units.py:docstring of sage.symbolic.units.unitdocs:18: (WARNING/2) Definition list ends without a blank line; unexpected unindent.\n```\nThe attached patch resolves those warnings as claimed.",
    "created_at": "2010-04-18T07:07:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8711",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8711#issuecomment-70463",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:2" align="right">comment:2</div>

Building the reference manual of Sage 4.4.alpha0 results in the following warnings:

```
/dev/shm/mvngu/sandbox/sage-4.4.alpha0-8711-ref/local/lib/python2.6/site-packages/sage/graphs/generic_graph.py:docstring of sage.graphs.generic_graph.GenericGraph.add_cycle:5: (ERROR/3) Unexpected indentation.
/dev/shm/mvngu/sandbox/sage-4.4.alpha0-8711-ref/local/lib/python2.6/site-packages/sage/graphs/generic_graph.py:docstring of sage.graphs.generic_graph.GenericGraph.vertex_cover:26: (WARNING/2) Block quote ends without a blank line; unexpected unindent.
/dev/shm/mvngu/sandbox/sage-4.4.alpha0-8711-ref/local/lib/python2.6/site-packages/sage/symbolic/units.py:docstring of sage.symbolic.units.unitdocs:15: (WARNING/2) Block quote ends without a blank line; unexpected unindent.
/dev/shm/mvngu/sandbox/sage-4.4.alpha0-8711-ref/local/lib/python2.6/site-packages/sage/symbolic/units.py:docstring of sage.symbolic.units.unitdocs:18: (WARNING/2) Definition list ends without a blank line; unexpected unindent.
```
The attached patch resolves those warnings as claimed.



---

archive/issue_events_110026.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-04-19T05:18:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8711",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8711#event-110026"
}
```



---

archive/issue_events_110027.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2010-04-19T05:18:58Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8711",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8711#event-110027"
}
```



---

archive/issue_comments_070464.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged \"trac_8711-reference44alpha0.patch\" into 4.4.alpha1.",
    "created_at": "2010-04-19T05:18:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8711",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8711#issuecomment-70464",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged "trac_8711-reference44alpha0.patch" into 4.4.alpha1.



---

archive/issue_comments_070465.json:
```json
{
    "body": "Merged: **sage-4.4.alpha1**",
    "created_at": "2010-04-19T05:18:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8711",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8711#issuecomment-70465",
    "user": "https://github.com/jhpalmieri"
}
```

Merged: **sage-4.4.alpha1**
