# Issue 8893: disjoint routed paths

archive/issues_008893.json:
```json
{
    "assignees": [],
    "body": "This patch implements the method disjoint_routed_path (k-DRP) which solves the following problem :\n\ngiven a list of pairs s_i, t_i, find in a given graph a set of vertex-disjoints paths P_i such that P_i goes from s_i to t_i\n\nRequires #8870\n\nAssignee: **@jasongrout, @nathanncohen, @rlmill**\n\nAuthor: **Nathann Cohen**\n\nReviewer: **Robert Miller**\n\nMerged: **sage-4.5.alpha1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/8893_\n\n",
    "closed_at": "2010-06-29T16:47:15Z",
    "created_at": "2010-05-05T18:21:32Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "disjoint routed paths",
    "type": "issue",
    "updated_at": "2010-06-29T16:47:15Z",
    "url": "https://github.com/sagemath/sage/issues/8893",
    "user": "https://github.com/nathanncohen"
}
```
This patch implements the method disjoint_routed_path (k-DRP) which solves the following problem :

given a list of pairs s_i, t_i, find in a given graph a set of vertex-disjoints paths P_i such that P_i goes from s_i to t_i

Requires #8870

Assignee: **@jasongrout, @nathanncohen, @rlmill**

Author: **Nathann Cohen**

Reviewer: **Robert Miller**

Merged: **sage-4.5.alpha1**

_Issue created by migration from https://trac.sagemath.org/ticket/8893_





---

archive/issue_events_102652.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-05-05T18:21:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/8893",
    "milestone_number": null,
    "milestone_title": "sage-4.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8893#event-102652"
}
```



---

archive/issue_events_102653.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-05-05T18:21:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8893",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8893#event-102653"
}
```



---

archive/issue_events_102654.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-05-05T18:21:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8893",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8893#event-102654"
}
```



---

archive/issue_events_102655.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-05-05T18:21:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8893",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8893#event-102655"
}
```



---

archive/issue_comments_074609.json:
```json
{
    "body": "Attachment: **[trac_8893.patch.gz](https://github.com/sagemath/sage/files/ticket8893/trac_8893.patch.gz)**",
    "created_at": "2010-05-08T22:24:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8893",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8893#issuecomment-74609",
    "user": "https://github.com/nathanncohen"
}
```

Attachment: **[trac_8893.patch.gz](https://github.com/sagemath/sage/files/ticket8893/trac_8893.patch.gz)**



---

archive/issue_events_102656.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-05-08T22:28:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8893",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8893#event-102656"
}
```



---

archive/issue_comments_074610.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,5 @@\n+This patch implements the method disjoint_routed_path (k-DRP) which solves the following problem :\n+\n+given a list of pairs s_i, t_i, find in a given graph a set of vertex-disjoints paths P_i such that P_i goes from s_i to t_i\n+\n Using #8870\n``````\n",
    "created_at": "2010-05-08T22:28:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8893",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8893#issuecomment-74610",
    "user": "https://github.com/nathanncohen"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,5 @@
+This patch implements the method disjoint_routed_path (k-DRP) which solves the following problem :
+
+given a list of pairs s_i, t_i, find in a given graph a set of vertex-disjoints paths P_i such that P_i goes from s_i to t_i
+
 Using #8870
``````




---

archive/issue_comments_074611.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,4 +2,4 @@\n \n given a list of pairs s_i, t_i, find in a given graph a set of vertex-disjoints paths P_i such that P_i goes from s_i to t_i\n \n-Using #8870\n+Requires #8870\n``````\n",
    "created_at": "2010-05-08T22:29:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8893",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8893#issuecomment-74611",
    "user": "https://github.com/nathanncohen"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,4 +2,4 @@
 
 given a list of pairs s_i, t_i, find in a given graph a set of vertex-disjoints paths P_i such that P_i goes from s_i to t_i
 
-Using #8870
+Requires #8870
``````




---

archive/issue_events_102657.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-05-08T22:29:07Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/8893",
    "title_is": "disjoint routed paths",
    "title_was": "disjoint rooted paths",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8893#event-102657"
}
```



---

archive/issue_events_102658.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-05-20T20:05:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8893",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8893#event-102658"
}
```



---

archive/issue_events_102659.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2010-05-20T20:05:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8893",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8893#event-102659"
}
```



---

archive/issue_comments_074612.json:
```json
{
    "body": "Author: **Nathann Cohen**",
    "created_at": "2010-06-23T04:18:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8893",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8893#issuecomment-74612",
    "user": "https://github.com/rlmill"
}
```

Author: **Nathann Cohen**



---

archive/issue_events_102660.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-06-23T04:18:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8893",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8893#event-102660"
}
```



---

archive/issue_events_102661.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-06-23T04:18:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/8893",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8893#event-102661"
}
```



---

archive/issue_comments_074613.json:
```json
{
    "body": "Reviewer: **Robert Miller**",
    "created_at": "2010-06-23T04:18:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8893",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8893#issuecomment-74613",
    "user": "https://github.com/rlmill"
}
```

Reviewer: **Robert Miller**



---

archive/issue_comments_074614.json:
```json
{
    "body": "Merged: **sage-4.5.alpha1**",
    "created_at": "2010-06-29T16:47:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/8893",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/8893#issuecomment-74614",
    "user": "https://github.com/rlmill"
}
```

Merged: **sage-4.5.alpha1**



---

archive/issue_events_102662.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-06-29T16:47:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/8893",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8893#event-102662"
}
```



---

archive/issue_events_102663.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-06-29T16:47:15Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/8893",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/8893#event-102663"
}
```
