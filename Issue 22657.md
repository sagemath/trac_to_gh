# Issue 22657: Symbolic expression.is_exact()

archive/issues_022657.json:
```json
{
    "body": "CC:  tmonteil\n\nExpressions can contain inexact elements which makes themselves inexact. This status could be made queriable e.g. to prevent integration attempts.\n\nIssue created by migration from https://trac.sagemath.org/ticket/22894\n\n",
    "created_at": "2017-04-28T08:07:33Z",
    "labels": [
        "component: symbolics"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.0",
    "title": "Symbolic expression.is_exact()",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/22657",
    "user": "https://github.com/rwst"
}
```
CC:  tmonteil

Expressions can contain inexact elements which makes themselves inexact. This status could be made queriable e.g. to prevent integration attempts.

Issue created by migration from https://trac.sagemath.org/ticket/22894





---

archive/issue_comments_315679.json:
```json
{
    "body": "how do you approach this problem? since numeric coefficients are cast into SR which is inexact, one has: \n\n```\nsage: f1 = x - 1\nsage: [op.base_ring().is_exact() for op in f1.operands()]\n[False, False]\n\nsage: f2 = x - 1.0\nsage: [op.base_ring().is_exact() for op in f2.operands()]\n[False, False]\n```",
    "created_at": "2017-05-07T12:08:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22657",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22657#issuecomment-315679",
    "user": "https://github.com/mforets"
}
```

how do you approach this problem? since numeric coefficients are cast into SR which is inexact, one has: 

```
sage: f1 = x - 1
sage: [op.base_ring().is_exact() for op in f1.operands()]
[False, False]

sage: f2 = x - 1.0
sage: [op.base_ring().is_exact() for op in f2.operands()]
[False, False]
```



---

archive/issue_comments_315680.json:
```json
{
    "body": "Apparently ring exactness can be tainted if it can possibly contain inexact elements. So we would not check the exactness of symbols/functions etc. which leaves those Python objects that are wrapped by SR (into `Expression`) and called \"numeric\". We can get the pure object via `Expression.pyobject()`, e.g.\n\n```\nsage: f1 = x - 1\nsage: [op.is_numeric() and not op.pyobject().base_ring().is_exact() for op in f1\n....: .operands()]\n[False, False]\nsage: f2 = x - 1.0\nsage: [op.is_numeric() and not op.pyobject().base_ring().is_exact() for op in f2\n....: .operands()]\n[False, True]\n```\nBut maybe not every of these rings has an `is_exact` member?",
    "created_at": "2017-05-08T05:48:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22657",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22657#issuecomment-315680",
    "user": "https://github.com/rwst"
}
```

Apparently ring exactness can be tainted if it can possibly contain inexact elements. So we would not check the exactness of symbols/functions etc. which leaves those Python objects that are wrapped by SR (into `Expression`) and called "numeric". We can get the pure object via `Expression.pyobject()`, e.g.

```
sage: f1 = x - 1
sage: [op.is_numeric() and not op.pyobject().base_ring().is_exact() for op in f1
....: .operands()]
[False, False]
sage: f2 = x - 1.0
sage: [op.is_numeric() and not op.pyobject().base_ring().is_exact() for op in f2
....: .operands()]
[False, True]
```
But maybe not every of these rings has an `is_exact` member?



---

archive/issue_comments_315681.json:
```json
{
    "body": "`@`rws : nice. and one can consider more complicated expressions (fractions, etc) by recursively applying the `operands()` method to each piece (until you get []).\n\n> But maybe not every of these rings has an is_exact member? \n\n\nI know the basic ones: (QQ, QQbar, ZZ) have `is_exact=True` while (RDF, RR, CC) give `is_exact=False`.. But Thierry has a comprehensive list so I'm CC'ing him!",
    "created_at": "2017-05-08T17:37:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22657",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22657#issuecomment-315681",
    "user": "https://github.com/mforets"
}
```

`@`rws : nice. and one can consider more complicated expressions (fractions, etc) by recursively applying the `operands()` method to each piece (until you get []).

> But maybe not every of these rings has an is_exact member? 


I know the basic ones: (QQ, QQbar, ZZ) have `is_exact=True` while (RDF, RR, CC) give `is_exact=False`.. But Thierry has a comprehensive list so I'm CC'ing him!



---

archive/issue_comments_315682.json:
```json
{
    "body": "Regarding real and complex numbers only, basically, numerical representations are inexact:\n\n```\nRealDoubleField()\nRealField(prec)\nRealIntervalField(prec)\nRealBallField(prec)\nComplexDoubleField()\nComplexField(prec)\nComplexIntervalField(prec)\nComplexBallField(prec)\n```\n\nAlgebraic representations are exact:\n\n```\nAlgebraicRealField()\nNumberField(poly)\nQuadraticField(n)\nRationalField()\nAlgebraicField()\nUniversalCyclotomicField()\nCyclotomicField(n)\n```\n\nSymbolic ring is a mixture, so it is inexact.\n\nThe main problem are `RealLazyField()` and `ComplexLazyField()` which claim to be exact, but they are not:\n\n```\nsage: a = RLF(0.1)\nsage: a\n0.10000000000000001?\nsage: a._value\n0.100000000000000\nsage: a._value.parent()\nReal Field with 53 bits of precision\nsage: RLF.is_exact()\nTrue\n```\n\nPerhaps should we fix this.",
    "created_at": "2017-05-08T18:36:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22657",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22657#issuecomment-315682",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```

Regarding real and complex numbers only, basically, numerical representations are inexact:

```
RealDoubleField()
RealField(prec)
RealIntervalField(prec)
RealBallField(prec)
ComplexDoubleField()
ComplexField(prec)
ComplexIntervalField(prec)
ComplexBallField(prec)
```

Algebraic representations are exact:

```
AlgebraicRealField()
NumberField(poly)
QuadraticField(n)
RationalField()
AlgebraicField()
UniversalCyclotomicField()
CyclotomicField(n)
```

Symbolic ring is a mixture, so it is inexact.

The main problem are `RealLazyField()` and `ComplexLazyField()` which claim to be exact, but they are not:

```
sage: a = RLF(0.1)
sage: a
0.10000000000000001?
sage: a._value
0.100000000000000
sage: a._value.parent()
Real Field with 53 bits of precision
sage: RLF.is_exact()
True
```

Perhaps should we fix this.



---

archive/issue_comments_315683.json:
```json
{
    "body": "See #22960.",
    "created_at": "2017-05-08T18:51:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22657",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22657#issuecomment-315683",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```

See #22960.



---

archive/issue_comments_315684.json:
```json
{
    "body": "`@`Thierry: that is indeed a *comprehensive* list :0",
    "created_at": "2017-05-25T19:57:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22657",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22657#issuecomment-315684",
    "user": "https://github.com/mforets"
}
```

`@`Thierry: that is indeed a *comprehensive* list :0



---

archive/issue_comments_315685.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2017-05-25T19:57:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22657",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22657#issuecomment-315685",
    "user": "https://github.com/mforets"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_315686.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2017-05-25T20:01:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22657",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22657#issuecomment-315686",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_315687.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2017-06-05T05:53:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22657",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22657#issuecomment-315687",
    "user": "https://github.com/rwst"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_315688.json:
```json
{
    "body": "LGTM.",
    "created_at": "2017-06-05T05:53:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22657",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22657#issuecomment-315688",
    "user": "https://github.com/rwst"
}
```

LGTM.



---

archive/issue_events_059438.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-06-07T20:13:24Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/22657",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/22657#event-59438"
}
```



---

archive/issue_comments_315689.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2017-06-07T20:13:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22657",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22657#issuecomment-315689",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed
