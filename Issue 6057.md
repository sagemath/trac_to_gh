# Issue 6057: Add a file Solaris.txt / SOLARIS.txt

archive/issues_006057.json:
```json
{
    "body": "Assignee: mabshoff\n\nIt would be useful if a file was added in the top-level directory with the current state of the Solaris port. i.e. what the known issues are with Solaris, any tips, workarounds etc. \n\nIssue created by migration from https://trac.sagemath.org/ticket/6057\n\n",
    "created_at": "2009-05-17T08:45:26Z",
    "labels": [
        "porting: Solaris",
        "major",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "Add a file Solaris.txt / SOLARIS.txt",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/6057",
    "user": "drkirkby"
}
```
Assignee: mabshoff

It would be useful if a file was added in the top-level directory with the current state of the Solaris port. i.e. what the known issues are with Solaris, any tips, workarounds etc. 

Issue created by migration from https://trac.sagemath.org/ticket/6057





---

archive/issue_comments_048230.json:
```json
{
    "body": "Resolution: wontfix",
    "created_at": "2009-05-17T09:20:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6057#issuecomment-48230",
    "user": "mabshoff"
}
```

Resolution: wontfix



---

archive/issue_comments_048231.json:
```json
{
    "body": "No, we have README.txt and the wiki, no need to add anything.\n\nCheers,\n\nMichael",
    "created_at": "2009-05-17T09:20:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6057#issuecomment-48231",
    "user": "mabshoff"
}
```

No, we have README.txt and the wiki, no need to add anything.

Cheers,

Michael
