# Issue 9569: Expose/Update methods using NetworkX1-1

archive/issues_009569.json:
```json
{
    "body": "Assignee: jason, ncohen, rlm\n\nCC:  bedwards @rlmill\n\nNetworkX is being updated to 1.1 on #9567\n\nSeveral new methods are available, and other should be updated.\n\nNathann\n\nIssue created by migration from https://trac.sagemath.org/ticket/9569\n\n",
    "created_at": "2010-07-22T01:27:02Z",
    "labels": [
        "component: graph theory"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "Expose/Update methods using NetworkX1-1",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/9569",
    "user": "https://github.com/nathanncohen"
}
```
Assignee: jason, ncohen, rlm

CC:  bedwards @rlmill

NetworkX is being updated to 1.1 on #9567

Several new methods are available, and other should be updated.

Nathann

Issue created by migration from https://trac.sagemath.org/ticket/9569





---

archive/issue_comments_092262.json:
```json
{
    "body": "We are now using NetowrkX 1.2 (#9567), and this ticket has been adressed in many others since... Should be closed as duplicate/wontfix !\n\nNathann",
    "created_at": "2010-09-12T12:24:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9569",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9569#issuecomment-92262",
    "user": "https://github.com/nathanncohen"
}
```

We are now using NetowrkX 1.2 (#9567), and this ticket has been adressed in many others since... Should be closed as duplicate/wontfix !

Nathann



---

archive/issue_comments_092263.json:
```json
{
    "body": "Resolution: duplicate",
    "created_at": "2010-09-12T19:19:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9569",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9569#issuecomment-92263",
    "user": "https://github.com/qed777"
}
```

Resolution: duplicate



---

archive/issue_events_009716.json:
```json
{
    "actor": "@qed777",
    "created_at": "2010-09-12T19:19:10Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/9569",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/9569#event-9716"
}
```
