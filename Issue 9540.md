...
```json
{
    "body": "The conversion from `QQ` to `ZZ` actually bypasses the `_integer_` method: instead, it uses `sage.rational.Q_to_Z` (discovered as the canonical section of the coercion in the reverse direction). No such fast conversion is implemented for quadratic number field elements.",
...
```json
{
    "body": "Conversely, the conversion from the integers to the Gaussian integers, also is somewhat slow, see #34408.",
