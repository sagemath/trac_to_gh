# Issue 125: maxima doctest freezes

archive/issues_000125.json:
```json
{
    "body": "Assignee: @williamstein\n\nOn a clean vanilla sage 1.4 install (running on sage.math), the doctest for `devel/sage-main/sage/interfaces/maxima.py` just freezes. SAGE never seems to pick up the timeout. I hit ctrl-c, and the doctests proceed normally after that.\n\n\nIssue created by migration from https://trac.sagemath.org/ticket/125\n\n",
    "created_at": "2006-10-11T00:12:51Z",
    "labels": [
        "component: interfaces",
        "bug"
    ],
    "title": "maxima doctest freezes",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/125",
    "user": "https://trac.sagemath.org/admin/accounts/users/dmharvey"
}
```
Assignee: @williamstein

On a clean vanilla sage 1.4 install (running on sage.math), the doctest for `devel/sage-main/sage/interfaces/maxima.py` just freezes. SAGE never seems to pick up the timeout. I hit ctrl-c, and the doctests proceed normally after that.


Issue created by migration from https://trac.sagemath.org/ticket/125





---

archive/issue_comments_000566.json:
```json
{
    "body": "This is a very annoying bug that only happens on 64-bit linux.  I will have to carefully step through the problem.  It shouldn't be hard to fix though.",
    "created_at": "2006-10-12T19:18:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/125",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/125#issuecomment-566",
    "user": "https://github.com/williamstein"
}
```

This is a very annoying bug that only happens on 64-bit linux.  I will have to carefully step through the problem.  It shouldn't be hard to fix though.



---

archive/issue_comments_000567.json:
```json
{
    "body": "fixed for a while...",
    "created_at": "2007-01-12T22:58:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/125",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/125#issuecomment-567",
    "user": "https://github.com/williamstein"
}
```

fixed for a while...



---

archive/issue_events_000131.json:
```json
{
    "actor": "@williamstein",
    "created_at": "2007-01-12T22:58:57Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/125",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/125#event-131"
}
```



---

archive/issue_comments_000568.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2007-01-12T22:58:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/125",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/125#issuecomment-568",
    "user": "https://github.com/williamstein"
}
```

Resolution: fixed
