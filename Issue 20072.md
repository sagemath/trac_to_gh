# Issue 20072: Expose PARI code for Frobenius matrix on hyperelliptic curves

archive/issues_020072.json:
```json
{
    "body": "CC:  @tornaria @edgarcosta\n\nKeywords: hyperelliptic curve, matrix of Frobenius, sd87\n\nPARI provides a method `hyperellpadicfrobenius` for computing the matrix of Frobenius on p-adic cohomology of a hyperelliptic curve over a prime field, and another method `nfhyperellpadicfrobenius` for doing so over a nonprime field. We don't currently provide any alternative for the latter, so I propose to expose the PARI code as a method of `HyperellipticCurve`. See #18916 for more discussion.\n\nIssue created by migration from https://trac.sagemath.org/ticket/20309\n\n",
    "created_at": "2016-03-27T20:28:18Z",
    "labels": [
        "component: algebraic geometry"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.8",
    "title": "Expose PARI code for Frobenius matrix on hyperelliptic curves",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/20072",
    "user": "https://github.com/kedlaya"
}
```
CC:  @tornaria @edgarcosta

Keywords: hyperelliptic curve, matrix of Frobenius, sd87

PARI provides a method `hyperellpadicfrobenius` for computing the matrix of Frobenius on p-adic cohomology of a hyperelliptic curve over a prime field, and another method `nfhyperellpadicfrobenius` for doing so over a nonprime field. We don't currently provide any alternative for the latter, so I propose to expose the PARI code as a method of `HyperellipticCurve`. See #18916 for more discussion.

Issue created by migration from https://trac.sagemath.org/ticket/20309





---

archive/issue_comments_276137.json:
```json
{
    "body": "Do you know why GP does not have `nfhyperellpadicfrobenius`? If PARI/GP doesn't expose it, I'm doubting whether Sage should.",
    "created_at": "2016-03-29T09:49:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20072#issuecomment-276137",
    "user": "https://github.com/jdemeyer"
}
```

Do you know why GP does not have `nfhyperellpadicfrobenius`? If PARI/GP doesn't expose it, I'm doubting whether Sage should.



---

archive/issue_comments_276138.json:
```json
{
    "body": "According to an email from Bill Allombert:\n\n```\nMostly, the reason I did not make nfhyperellpadicfrobenius() \npublic in the first place was that I was not sure what to write in\nthe documentation that would tell users what it is good for and how\nto use it.\n```\nHowever, there do exist some use cases where it is relevant; I'll try to put together a coherent post describing these.",
    "created_at": "2016-03-29T22:52:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20072#issuecomment-276138",
    "user": "https://github.com/kedlaya"
}
```

According to an email from Bill Allombert:

```
Mostly, the reason I did not make nfhyperellpadicfrobenius() 
public in the first place was that I was not sure what to write in
the documentation that would tell users what it is good for and how
to use it.
```
However, there do exist some use cases where it is relevant; I'll try to put together a coherent post describing these.



---

archive/issue_comments_276139.json:
```json
{
    "body": "Replying to [comment:2 kedlaya]:\n> According to an email from Bill Allombert:\n> \n> ```\n> Mostly, the reason I did not make nfhyperellpadicfrobenius() \n> public in the first place was that I was not sure what to write in\n> the documentation that would tell users what it is good for and how\n> to use it.\n> ```\n> However, there do exist some use cases where it is relevant; I'll try to put together a coherent post describing these.\n\nOn top of what you said here: http://trac.sagemath.org/ticket/18916#comment:16\na use case for getting the matrix rather than the charpoly is to compute number of points on the curve faster by using less precision than need for the charpoly.\nThat's already implemented that way in Sage.",
    "created_at": "2016-04-05T16:39:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20072#issuecomment-276139",
    "user": "https://trac.sagemath.org/admin/accounts/users/jpflori"
}
```

Replying to [comment:2 kedlaya]:
> According to an email from Bill Allombert:
> 
> ```
> Mostly, the reason I did not make nfhyperellpadicfrobenius() 
> public in the first place was that I was not sure what to write in
> the documentation that would tell users what it is good for and how
> to use it.
> ```
> However, there do exist some use cases where it is relevant; I'll try to put together a coherent post describing these.

On top of what you said here: http://trac.sagemath.org/ticket/18916#comment:16
a use case for getting the matrix rather than the charpoly is to compute number of points on the curve faster by using less precision than need for the charpoly.
That's already implemented that way in Sage.



---

archive/issue_comments_276140.json:
```json
{
    "body": "Changing keywords from \"hyperelliptic curve, matrix of Frobenius\" to \"hyperelliptic curve, matrix of Frobenius, sd87\".",
    "created_at": "2017-07-17T17:42:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20072#issuecomment-276140",
    "user": "https://github.com/roed314"
}
```

Changing keywords from "hyperelliptic curve, matrix of Frobenius" to "hyperelliptic curve, matrix of Frobenius, sd87".



---

archive/issue_comments_276141.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2017-07-20T00:40:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20072#issuecomment-276141",
    "user": "https://github.com/mmasdeu"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_276142.json:
```json
{
    "body": "The code seems to work. The documentation does not build but we believe that it's not our fault (it seems to complain about plot3d, which this patch does not touch).",
    "created_at": "2017-07-20T00:40:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20072#issuecomment-276142",
    "user": "https://github.com/mmasdeu"
}
```

The code seems to work. The documentation does not build but we believe that it's not our fault (it seems to complain about plot3d, which this patch does not touch).



---

archive/issue_comments_276143.json:
```json
{
    "body": "New commits:",
    "created_at": "2017-07-20T00:44:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20072#issuecomment-276143",
    "user": "https://github.com/mmasdeu"
}
```

New commits:



---

archive/issue_comments_276144.json:
```json
{
    "body": "What is the point of your function `hyperellpadicfrobenius`!? It is already exposed in cypari\n\n```\nsage: x = polygen(QQ)\nsage: pari(4*x^5 - 92*x^3 + 72*x^2 + 160*x).hyperellpadicfrobenius(13, 20)\n[3*13^2 + 6*13^3 + 3*13^4 + 8*13^5 + 12*13^6 + 7*13^7 + 4*13^8 + 2*13^9 + 10*13^10 + 10*13^11 + 4*13^12 + 12*13^13 + 11*13^14 + 11*13^15 + 4*13^16 + 8*13^17 + 11*13^18 + 9*13^19 + O(13^20), 11*13 + 7*13^2 + 8*13^3 + 12*13^5 + 8*13^6 + 12*13^7 + 3*13^8 + 2*13^9 + 5*13^10 + 2*13^12 + 9*13^13 + 10*13^14 + 8*13^15 + 8*13^16 + 3*13^17 + 6*13^18 + 7*13^19 + O(13^20), 2 + 6*13 + 11*13^2 + 8*13^3 + 13^4 + 9*13^5 + 9*13^6 + 11*13^7 + 8*13^8 + 9*13^9 + 5*13^10 + 4*13^11 + 10*13^12 + 5*13^13 + 12*13^14 + 11*13^15 + 3*13^16 + 13^17 + 6*13^18 + 11*13^19 + O(13^20), 10 + 10*13^2 + 3*13^3 + 8*13^4 + 13^5 + 4*13^7 + 6*13^8 + 5*13^9 + 9*13^10 + 7*13^11 + 6*13^12 + 13^13 + 2*13^14 + 9*13^15 + 8*13^16 + 7*13^17 + 4*13^18 + O(13^20); 5*13 + 3*13^2 + 3*13^3 + 12*13^4 + 13^5 + 12*13^6 + 12*13^7 + 2*13^8 + 10*13^9 + 3*13^10 + 10*13^11 + 5*13^12 + 3*13^13 + 13^14 + 12*13^15 + 3*13^16 + 13^18 + 12*13^19 + O(13^20), 4*13^2 + 3*13^3 + 11*13^4 + 6*13^5 + 2*13^6 + 8*13^7 + 11*13^8 + 6*13^9 + 8*13^10 + 11*13^11 + 8*13^12 + 6*13^13 + 6*13^14 + 4*13^15 + 10*13^17 + 8*13^18 + 10*13^19 + O(13^20), 8 + 12*13 + 7*13^2 + 13^3 + 4*13^4 + 2*13^5 + 4*13^6 + 10*13^7 + 2*13^8 + 4*13^9 + 7*13^10 + 7*13^11 + 12*13^12 + 5*13^13 + 10*13^14 + 9*13^15 + 13^16 + 8*13^17 + 2*13^18 + 10*13^19 + O(13^20), 6*13 + 9*13^2 + 9*13^4 + 10*13^5 + 4*13^6 + 12*13^7 + 10*13^9 + 5*13^10 + 7*13^11 + 4*13^12 + 10*13^13 + 8*13^14 + 2*13^15 + 2*13^16 + 7*13^17 + 10*13^18 + 5*13^19 + O(13^20); 9*13 + 7*13^2 + 11*13^3 + 10*13^4 + 4*13^5 + 9*13^6 + 13^7 + 3*13^8 + 10*13^9 + 4*13^10 + 7*13^12 + 8*13^13 + 7*13^14 + 4*13^15 + 9*13^16 + 10*13^17 + 8*13^18 + 3*13^19 + O(13^20), 4*13 + 6*13^2 + 3*13^3 + 11*13^4 + 5*13^5 + 7*13^6 + 6*13^7 + 9*13^8 + 10*13^9 + 13^10 + 9*13^11 + 5*13^12 + 10*13^13 + 11*13^14 + 6*13^15 + 7*13^16 + 7*13^17 + 9*13^18 + 11*13^19 + O(13^20), 4 + 6*13 + 3*13^2 + 8*13^3 + 8*13^4 + 9*13^6 + 11*13^7 + 4*13^8 + 8*13^9 + 6*13^10 + 2*13^11 + 6*13^12 + 8*13^13 + 6*13^14 + 13^15 + 11*13^16 + 9*13^17 + 4*13^18 + 13^19 + O(13^20), 10 + 9*13 + 9*13^2 + 8*13^3 + 10*13^4 + 6*13^5 + 12*13^7 + 8*13^8 + 11*13^9 + 10*13^10 + 10*13^11 + 11*13^12 + 9*13^13 + 11*13^14 + 12*13^15 + 7*13^16 + 9*13^17 + 5*13^18 + 12*13^19 + O(13^20); 11*13 + 10*13^2 + 11*13^3 + 4*13^4 + 11*13^5 + 9*13^6 + 8*13^7 + 9*13^8 + 3*13^9 + 7*13^10 + 12*13^11 + 8*13^12 + 4*13^13 + 13^14 + 13^15 + 11*13^16 + 7*13^17 + 5*13^18 + 12*13^19 + O(13^20), 11*13 + 11*13^3 + 5*13^4 + 12*13^5 + 13^6 + 13^7 + 12*13^8 + 5*13^9 + 12*13^10 + 7*13^11 + 10*13^12 + 5*13^13 + 5*13^14 + 9*13^15 + 7*13^16 + 8*13^17 + 10*13^18 + 13^19 + O(13^20), 11 + 6*13 + 7*13^2 + 9*13^3 + 13^4 + 5*13^5 + 12*13^6 + 13^7 + 10*13^8 + 12*13^9 + 2*13^10 + 13^11 + 8*13^13 + 8*13^14 + 11*13^15 + 5*13^16 + 9*13^17 + O(13^20), 5 + 6*13 + 2*13^2 + 8*13^3 + 2*13^4 + 10*13^5 + 13^6 + 11*13^7 + 4*13^8 + 8*13^9 + 13^11 + 6*13^12 + 11*13^13 + 8*13^15 + 9*13^16 + 10*13^17 + 4*13^19 + O(13^20)]\n```\n\nThough, `nfhyperellpadicfrobenius` is not... any idea why this function is not available in GP?",
    "created_at": "2017-07-20T15:13:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20072#issuecomment-276144",
    "user": "https://github.com/videlec"
}
```

What is the point of your function `hyperellpadicfrobenius`!? It is already exposed in cypari

```
sage: x = polygen(QQ)
sage: pari(4*x^5 - 92*x^3 + 72*x^2 + 160*x).hyperellpadicfrobenius(13, 20)
[3*13^2 + 6*13^3 + 3*13^4 + 8*13^5 + 12*13^6 + 7*13^7 + 4*13^8 + 2*13^9 + 10*13^10 + 10*13^11 + 4*13^12 + 12*13^13 + 11*13^14 + 11*13^15 + 4*13^16 + 8*13^17 + 11*13^18 + 9*13^19 + O(13^20), 11*13 + 7*13^2 + 8*13^3 + 12*13^5 + 8*13^6 + 12*13^7 + 3*13^8 + 2*13^9 + 5*13^10 + 2*13^12 + 9*13^13 + 10*13^14 + 8*13^15 + 8*13^16 + 3*13^17 + 6*13^18 + 7*13^19 + O(13^20), 2 + 6*13 + 11*13^2 + 8*13^3 + 13^4 + 9*13^5 + 9*13^6 + 11*13^7 + 8*13^8 + 9*13^9 + 5*13^10 + 4*13^11 + 10*13^12 + 5*13^13 + 12*13^14 + 11*13^15 + 3*13^16 + 13^17 + 6*13^18 + 11*13^19 + O(13^20), 10 + 10*13^2 + 3*13^3 + 8*13^4 + 13^5 + 4*13^7 + 6*13^8 + 5*13^9 + 9*13^10 + 7*13^11 + 6*13^12 + 13^13 + 2*13^14 + 9*13^15 + 8*13^16 + 7*13^17 + 4*13^18 + O(13^20); 5*13 + 3*13^2 + 3*13^3 + 12*13^4 + 13^5 + 12*13^6 + 12*13^7 + 2*13^8 + 10*13^9 + 3*13^10 + 10*13^11 + 5*13^12 + 3*13^13 + 13^14 + 12*13^15 + 3*13^16 + 13^18 + 12*13^19 + O(13^20), 4*13^2 + 3*13^3 + 11*13^4 + 6*13^5 + 2*13^6 + 8*13^7 + 11*13^8 + 6*13^9 + 8*13^10 + 11*13^11 + 8*13^12 + 6*13^13 + 6*13^14 + 4*13^15 + 10*13^17 + 8*13^18 + 10*13^19 + O(13^20), 8 + 12*13 + 7*13^2 + 13^3 + 4*13^4 + 2*13^5 + 4*13^6 + 10*13^7 + 2*13^8 + 4*13^9 + 7*13^10 + 7*13^11 + 12*13^12 + 5*13^13 + 10*13^14 + 9*13^15 + 13^16 + 8*13^17 + 2*13^18 + 10*13^19 + O(13^20), 6*13 + 9*13^2 + 9*13^4 + 10*13^5 + 4*13^6 + 12*13^7 + 10*13^9 + 5*13^10 + 7*13^11 + 4*13^12 + 10*13^13 + 8*13^14 + 2*13^15 + 2*13^16 + 7*13^17 + 10*13^18 + 5*13^19 + O(13^20); 9*13 + 7*13^2 + 11*13^3 + 10*13^4 + 4*13^5 + 9*13^6 + 13^7 + 3*13^8 + 10*13^9 + 4*13^10 + 7*13^12 + 8*13^13 + 7*13^14 + 4*13^15 + 9*13^16 + 10*13^17 + 8*13^18 + 3*13^19 + O(13^20), 4*13 + 6*13^2 + 3*13^3 + 11*13^4 + 5*13^5 + 7*13^6 + 6*13^7 + 9*13^8 + 10*13^9 + 13^10 + 9*13^11 + 5*13^12 + 10*13^13 + 11*13^14 + 6*13^15 + 7*13^16 + 7*13^17 + 9*13^18 + 11*13^19 + O(13^20), 4 + 6*13 + 3*13^2 + 8*13^3 + 8*13^4 + 9*13^6 + 11*13^7 + 4*13^8 + 8*13^9 + 6*13^10 + 2*13^11 + 6*13^12 + 8*13^13 + 6*13^14 + 13^15 + 11*13^16 + 9*13^17 + 4*13^18 + 13^19 + O(13^20), 10 + 9*13 + 9*13^2 + 8*13^3 + 10*13^4 + 6*13^5 + 12*13^7 + 8*13^8 + 11*13^9 + 10*13^10 + 10*13^11 + 11*13^12 + 9*13^13 + 11*13^14 + 12*13^15 + 7*13^16 + 9*13^17 + 5*13^18 + 12*13^19 + O(13^20); 11*13 + 10*13^2 + 11*13^3 + 4*13^4 + 11*13^5 + 9*13^6 + 8*13^7 + 9*13^8 + 3*13^9 + 7*13^10 + 12*13^11 + 8*13^12 + 4*13^13 + 13^14 + 13^15 + 11*13^16 + 7*13^17 + 5*13^18 + 12*13^19 + O(13^20), 11*13 + 11*13^3 + 5*13^4 + 12*13^5 + 13^6 + 13^7 + 12*13^8 + 5*13^9 + 12*13^10 + 7*13^11 + 10*13^12 + 5*13^13 + 5*13^14 + 9*13^15 + 7*13^16 + 8*13^17 + 10*13^18 + 13^19 + O(13^20), 11 + 6*13 + 7*13^2 + 9*13^3 + 13^4 + 5*13^5 + 12*13^6 + 13^7 + 10*13^8 + 12*13^9 + 2*13^10 + 13^11 + 8*13^13 + 8*13^14 + 11*13^15 + 5*13^16 + 9*13^17 + O(13^20), 5 + 6*13 + 2*13^2 + 8*13^3 + 2*13^4 + 10*13^5 + 13^6 + 11*13^7 + 4*13^8 + 8*13^9 + 13^11 + 6*13^12 + 11*13^13 + 8*13^15 + 9*13^16 + 10*13^17 + 4*13^19 + O(13^20)]
```

Though, `nfhyperellpadicfrobenius` is not... any idea why this function is not available in GP?



---

archive/issue_comments_276145.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2017-07-20T15:13:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20072#issuecomment-276145",
    "user": "https://github.com/videlec"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_276146.json:
```json
{
    "body": "Replying to [comment:9 vdelecroix]:\n> Though, `nfhyperellpadicfrobenius` is not... any idea why this function is not available in GP?\n\n\nOh I discovered [comment:3 comment:3]! It would be much better to provide a patch to PARI/GP for that purpose. In the mean time, you can provide a pull request to [cypari2](https://github.com/defeo/cypari2) (as was done by Rob Harron in [PR16](https://github.com/defeo/cypari2/pull/16)).",
    "created_at": "2017-07-20T15:17:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20072#issuecomment-276146",
    "user": "https://github.com/videlec"
}
```

Replying to [comment:9 vdelecroix]:
> Though, `nfhyperellpadicfrobenius` is not... any idea why this function is not available in GP?


Oh I discovered [comment:3 comment:3]! It would be much better to provide a patch to PARI/GP for that purpose. In the mean time, you can provide a pull request to [cypari2](https://github.com/defeo/cypari2) (as was done by Rob Harron in [PR16](https://github.com/defeo/cypari2/pull/16)).



---

archive/issue_comments_276147.json:
```json
{
    "body": "I have a made a rebased, squashed, and refreshed branch.\n\n---\nNew commits:",
    "created_at": "2019-06-06T19:26:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20072#issuecomment-276147",
    "user": "https://github.com/fchapoton"
}
```

I have a made a rebased, squashed, and refreshed branch.

---
New commits:



---

archive/issue_events_055101.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-06-06T19:26:50Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "milestone": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20072#event-55101"
}
```



---

archive/issue_comments_276148.json:
```json
{
    "body": "Tickets still needing working or clarification should be moved to the next release milestone at the soonest (please feel free to revert if you think the ticket is close to being resolved).",
    "created_at": "2019-06-14T14:50:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20072#issuecomment-276148",
    "user": "https://github.com/embray"
}
```

Tickets still needing working or clarification should be moved to the next release milestone at the soonest (please feel free to revert if you think the ticket is close to being resolved).



---

archive/issue_events_055102.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-06-14T14:50:27Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "milestone": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20072#event-55102"
}
```



---

archive/issue_events_055103.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-06-14T14:50:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "milestone": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20072#event-55103"
}
```



---

archive/issue_comments_276149.json:
```json
{
    "body": "I just heard from the Pari team that there will be change in the next release that affects this ticket: the functionality of `nfhyperellpadicfrobenius` will be exposed by modifying the call syntax of `hyperellpadicfrobenius`. Once that happens, we should revisit the issue.",
    "created_at": "2019-12-17T01:34:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20072#issuecomment-276149",
    "user": "https://github.com/kedlaya"
}
```

I just heard from the Pari team that there will be change in the next release that affects this ticket: the functionality of `nfhyperellpadicfrobenius` will be exposed by modifying the call syntax of `hyperellpadicfrobenius`. Once that happens, we should revisit the issue.



---

archive/issue_events_055104.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-30T14:48:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "milestone": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20072#event-55104"
}
```



---

archive/issue_events_055105.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-30T14:48:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "milestone": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20072#event-55105"
}
```



---

archive/issue_comments_276150.json:
```json
{
    "body": "Ticket retargeted after milestone closed",
    "created_at": "2019-12-30T14:48:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20072#issuecomment-276150",
    "user": "https://github.com/embray"
}
```

Ticket retargeted after milestone closed



---

archive/issue_events_055106.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "milestone": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20072#event-55106"
}
```



---

archive/issue_events_055107.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "milestone": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20072#event-55107"
}
```



---

archive/issue_comments_276151.json:
```json
{
    "body": "Batch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.",
    "created_at": "2020-04-14T19:41:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20072#issuecomment-276151",
    "user": "https://github.com/mkoeppe"
}
```

Batch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.



---

archive/issue_events_055108.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-05T19:21:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "milestone": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20072#event-55108"
}
```



---

archive/issue_events_055109.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-05T19:21:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20072#event-55109"
}
```



---

archive/issue_events_055110.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20072#event-55110"
}
```



---

archive/issue_events_055111.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20072#event-55111"
}
```



---

archive/issue_comments_276152.json:
```json
{
    "body": "Setting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-02-13T20:51:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20072#issuecomment-276152",
    "user": "https://github.com/mkoeppe"
}
```

Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_comments_276153.json:
```json
{
    "body": "Setting a new milestone for this ticket based on a cursory review.",
    "created_at": "2021-07-19T00:44:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20072#issuecomment-276153",
    "user": "https://github.com/mkoeppe"
}
```

Setting a new milestone for this ticket based on a cursory review.



---

archive/issue_events_055112.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T00:44:56Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20072#event-55112"
}
```



---

archive/issue_events_055113.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T00:44:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20072#event-55113"
}
```



---

archive/issue_events_055114.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:24:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20072#event-55114"
}
```



---

archive/issue_events_055115.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:24:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20072#event-55115"
}
```



---

archive/issue_events_055116.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-02T20:27:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20072#event-55116"
}
```



---

archive/issue_events_055117.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-02T20:27:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20072#event-55117"
}
```



---

archive/issue_events_055118.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20072#event-55118"
}
```



---

archive/issue_events_055119.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20072",
    "milestone": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20072#event-55119"
}
```
