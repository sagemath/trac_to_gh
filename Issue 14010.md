...
Original creation time: 2013-03-02 10:26:55
...
Comment by SimonKing created at 2013-03-02 10:56:47
...
- One could try to make some important basic methods of Homset fast, by inheritance from `WithEqualityById` that I introduced in #14054. It would be desirable that two homsets are equal if and only if they are identical if and only if both domain, codomain and category are identical. 
  - Unfortunately, the old parts of sage (about elliptic curves and stuff) rely on the assumption that we do _not_ have unique parents and do _not_ have unique homsets. This would be another can of worms.
...
Comment by SimonKing created at 2013-03-02 11:05:51
...
- Getting a homset out of the cache could still be faster. The time-limiting factor is that one needs to find an appropriate category, if it is not explicitly passed as an argument. But since the cache is weak anyway, couldn't we simply cache the same homset _twice_, namely as `Hom(X,Y)` and as `Hom(X,Y,C)`? This would give a considerable speed-up.
- The hash is _much_ too slow. Ideally, one would simply inherit from `WithEqualityById`, but that's non-trivial, as I have pointed out above.
...
Comment by SimonKing created at 2013-03-02 14:06:37
...
I think it would only be significant if it was 95% confidence. Nevertheless, it is a _decrease_ of the startup time!


---

Comment by nthiery created at 2013-03-03 18:33:22
...
Comment by SimonKing created at 2013-03-03 18:52:09
...
- In the coercion framework, the domain of a map _must_ be identical (and not just equal) to the parents of elements-to-be-coerced. Hence, if X' is equal to X, then we need that Hom(X,Y) is not identical with Hom(X',Y). But a weak cached function would compare by equality, and this would give rise to errors in those parts of Sage that still work with non-unique parents. `TripleDict` does compare the keys by identity.
...
Comment by SimonKing created at 2013-03-03 19:52:20
...
Comment by SimonKing created at 2013-03-03 20:33:50
...
Comment by SimonKing created at 2013-03-03 20:35:36
...
Comment by nbruin created at 2013-03-03 21:12:13
...
Comment by SimonKing created at 2013-03-03 22:01:39
...
They _are_ an issue, if the equality-by-id paradigma is used in some parts and isn't used in other parts of the coercion framework. They are not an issue, if the coercion framework consistently uses equality-by-id even on non-unique parents, as I am now explaining.
...
This is why I think that non-unique parents are not an issue in the coercion framework---but it is essential to be consistent, i.e., _inside_ of the coercion framework one should consistently use comparison by identity even for non-unique parents (as in the P1-versus-P2 example above).
...
In fact, this would be a problem. We can not simply use `CachedRepresentation` here, because this uses comparison-by-== (and, as I tried to explain, _in the coercion model_ we want comparison-by-id even for non-unique parents).

But couldn't we make Homset have the `ClasscallMetaclass`, inherit from `EqualityById` _and_ use the current cache logic from Hom in its `__classcall__` method? The advantage would be that in some parts of Sage Homsets are created without calling the Hom function. Putting the Hom-logic into a `__classcall__`, we would have Homsets cached in this case, too!
...
Inside of the coercion framework, I think _nothing_ prevents us from comparing non-unique parents by identity---except old code. In that way, we have freedom to choose non-trivial coercions between non-identical copies of a parent.


---

Comment by SimonKing created at 2013-03-03 22:14:01
...
Comment by SimonKing created at 2013-03-03 22:28:14
...
Comment by nbruin created at 2013-03-04 02:58:41
...
Good catch. However, the comparison-by-== happens on _construction parameters_. Hence, if someone implements some equal-but-not-identical rings `R` and `S` then we'll have
...
That's a serious problem. If you're implementing something that can be used in further (functorial) (cached) parent constructions that are expected to have coercions they _have_ to be `EqualityById`. Otherwise you get the mess as above.

I doubt things like elliptic curves disappear into such cached parent constructions, which was why I thought the problem might not be _that_ bad.

> But couldn't we make Homset have the `ClasscallMetaclass`, inherit from `EqualityById` _and_ use the current cache logic from Hom in its `__classcall__` method?

Homsets are important in coercion _and_ have proper mathematical meaning. If you make them consider their arguments by identity, you're basically telling people "you can use `==` as an equivalence relation on your class to some extent, but as far as maps are concerned, `id` is what really matters". I'm fine with that, but you are restricting people's ability to define and use `==` as they see fit.
...
Comment by SimonKing created at 2013-03-11 18:26:09
...
Comment by SimonKing created at 2013-03-14 11:37:20
...
Comment by SimonKing created at 2013-03-14 12:35:53
...
Comment by nbruin created at 2013-03-14 16:10:42
...
Comment by SimonKing created at 2013-03-14 16:35:47
...
Anyway, that's why I like to keep the cached method _and_ self._domain and self._codomain around.
...
* If you try to _directly_ call `Homset` to create an instance, you would get an error, because you can't easily assign attributes to a cdef class. Hence, I work with the implicit assumption that all homsets are actually instances of _non-cdef_ classes inheriting from `Homset`.
...
Comment by nbruin created at 2013-04-04 18:14:12
...
I think they _should_ be cdef attributes because that would allow for way faster access in cdef-type-declared cython code than `property` access would.
...
Comment by nbruin created at 2013-04-04 18:30:39
...
Comment by tscrim created at 2013-04-04 18:41:42
...
Comment by SimonKing created at 2013-04-04 21:39:33
...
Properties have a getter, a setter and a deleter. These three methods _are_ provided by cached methods: set_cache is the setter, calling it is the getter, and clear_cache is the deleter.
...
But properties still need to access the storage space. The storage space of a `CachedMethodCallerNoArgs` is very fast, since you won't even need a dictionary lookup (since it does _not_ use `__cached_methods` for storing the value, in contrast to cached methods _with_ arguments!).
...
Comment by SimonKing created at 2013-04-04 21:51:20
...
Comment by nbruin created at 2013-04-04 23:08:54
...
Comment by nbruin created at 2013-04-05 00:07:51
...
Comment by nbruin created at 2013-04-05 04:29:06
...
Comment by SimonKing created at 2013-04-05 06:51:39
...
I did not say that it is not competitive. I said that a `CachedMethodCallerNoArgs` _is_ a property, since it has a setter and a getter, with the only difference that for accessing it you need to do A.C() and not just A.C.
...
Comment by SimonKing created at 2013-04-05 07:00:13
...
Comment by nbruin created at 2013-04-05 07:32:12

Replying to [comment:27 SimonKing]:
> I said that a `CachedMethodCallerNoArgs` _is_ a property, since it has a setter and a getter
...
Comment by nbruin created at 2013-04-09 16:52:52
...
Comment by SimonKing created at 2013-05-17 08:17:22
...
Comment by SimonKing created at 2013-05-17 08:31:57
...
Comment by SimonKing created at 2013-05-17 08:48:44
...
Comment by SimonKing created at 2013-05-17 08:57:50

I don't know why, but it seems that putting something into `__cached_methods` does _not_ mean that it will be available as an attribute. But this used to be the case! What is happening here?


---

Comment by SimonKing created at 2013-05-17 09:05:03
...
Comment by SimonKing created at 2013-05-17 09:10:43
...
Comment by SimonKing created at 2013-05-17 09:15:39
...
Comment by SimonKing created at 2013-05-17 10:18:49
...
Comment by SimonKing created at 2013-05-17 10:23:54
...
Comment by nbruin created at 2013-05-18 03:46:43
...
Comment by SimonKing created at 2013-05-18 07:32:59
...
Comment by SimonKing created at 2013-05-18 16:10:38
...
Comment by SimonKing created at 2013-05-18 16:21:43
...
Comment by SimonKing created at 2013-05-18 16:30:25
...
Comment by SimonKing created at 2013-05-18 16:49:33
...
Comment by nbruin created at 2013-05-18 18:05:44
...
Comment by nbruin created at 2013-05-18 19:21:12
...
Comment by nbruin created at 2013-05-19 05:51:00
...
Comment by SimonKing created at 2013-05-19 11:23:00
...
So, the choice of an implementation of the non-data descriptor will _only_ matter if there is no attribute assignment available (let's call it "case 2"). And if I am not mistaken, your code will be  very slow in case 2, and certainly not faster than a `@`lazy_attribute.
...
Comment by nbruin created at 2013-05-19 18:07:50
...
I guess our discussion got side-tracked a little bit:
 ...
 - what we should really have a `cdef public` because then one and the same location is fast from C _and_ python writes the python access code for us. But for that we have inheritance problems at the moment.
...
Comment by nbruin created at 2013-05-19 21:18:51
...
Comment by nbruin created at 2013-05-23 15:04:54
...
Comment by SimonKing created at 2013-05-23 15:27:01
...
Note that I tested to cythonize lazy_attribute _and_ to cimport Parent. It failed (circular import or so). So, we come back to the first question: Where shall we put the code? I think it makes sense to cdefine `instance` being Parent, but this would be impossible in `sage/misc/lazy_attribute.pyx`.
...
Comment by nbruin created at 2013-05-23 15:42:14
...
> Note that I tested to cythonize lazy_attribute _and_ to cimport Parent. It failed (circular import or so).
...
Comment by SimonKing created at 2013-05-23 15:54:30
...
Comment by SimonKing created at 2013-05-23 15:56:16
...
Comment by SimonKing created at 2013-05-23 17:19:31
...
Comment by nbruin created at 2013-05-23 17:26:17
...
Comment by SimonKing created at 2013-05-23 17:32:33
...
Comment by nthiery created at 2013-05-23 19:10:02
...
Comment by SimonKing created at 2013-05-23 19:47:15
...
Comment by nthiery created at 2013-05-23 21:10:40
...
Comment by nbruin created at 2013-05-25 20:25:46
...
Comment by SimonKing created at 2015-07-14 07:26:10
...
Comment by SimonKing created at 2015-07-14 07:33:40
...
Comment by SimonKing created at 2015-07-14 07:55:53
...
Comment by SimonKing created at 2015-07-14 09:24:22
