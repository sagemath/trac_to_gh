# Issue 12049: matrix_integer_dense.pyx: 'algorithn' instead of 'algorithm'

archive/issues_011877.json:
```json
{
    "assignees": [],
    "body": "Looking through the code of ``matrix_integer_dense.pyx`` I found **``algorithn``** instead of **``algorithm``** in line 2689, which is wrong.\n\nApply [attachment: trac_12049_algorithn_algorithm.2.patch.](https://github.com/sagemath/sage/files/ticket12049/trac_12049_algorithn_algorithm.2.patch..gz)\n\nAssignee: **@jasongrout, @williamstein**\n\nAuthor: **Daniel Krenn**\n\nReviewer: **Burcin Erocal**\n\nMerged: **sage-4.8.alpha3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12049_\n\n",
    "closed_at": "2011-11-23T13:25:55Z",
    "created_at": "2011-11-17T14:51:01Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "matrix_integer_dense.pyx: 'algorithn' instead of 'algorithm'",
    "type": "issue",
    "updated_at": "2011-11-23T13:25:55Z",
    "url": "https://github.com/sagemath/sage/issues/12049",
    "user": "https://github.com/dkrenn"
}
```
Looking through the code of ``matrix_integer_dense.pyx`` I found **``algorithn``** instead of **``algorithm``** in line 2689, which is wrong.

Apply [attachment: trac_12049_algorithn_algorithm.2.patch.](https://github.com/sagemath/sage/files/ticket12049/trac_12049_algorithn_algorithm.2.patch..gz)

Assignee: **@jasongrout, @williamstein**

Author: **Daniel Krenn**

Reviewer: **Burcin Erocal**

Merged: **sage-4.8.alpha3**

_Issue created by migration from https://trac.sagemath.org/ticket/12049_





---

archive/issue_events_149102.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2011-11-17T14:51:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12049",
    "milestone_number": null,
    "milestone_title": "sage-4.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12049#event-149102"
}
```



---

archive/issue_events_149103.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2011-11-17T14:51:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12049",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "component: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12049#event-149103"
}
```



---

archive/issue_events_149104.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2011-11-17T14:51:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12049",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12049#event-149104"
}
```



---

archive/issue_events_149105.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2011-11-17T14:51:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12049",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12049#event-149105"
}
```



---

archive/issue_comments_130461.json:
```json
{
    "body": "Attachment: **[trac_12049_algorithn_algorithm.patch.gz](https://github.com/sagemath/sage/files/ticket12049/trac_12049_algorithn_algorithm.patch.gz)**",
    "created_at": "2011-11-17T14:53:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12049",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12049#issuecomment-130461",
    "user": "https://github.com/dkrenn"
}
```

Attachment: **[trac_12049_algorithn_algorithm.patch.gz](https://github.com/sagemath/sage/files/ticket12049/trac_12049_algorithn_algorithm.patch.gz)**



---

archive/issue_events_149106.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2011-11-17T14:53:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12049",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12049#event-149106"
}
```



---

archive/issue_comments_130462.json:
```json
{
    "body": "Author: **Daniel Krenn**",
    "created_at": "2011-11-17T14:53:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12049",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12049#issuecomment-130462",
    "user": "https://github.com/dkrenn"
}
```

Author: **Daniel Krenn**



---

archive/issue_comments_130463.json:
```json
{
    "body": "Reviewer: **Burcin Erocal**",
    "created_at": "2011-11-17T15:16:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12049",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12049#issuecomment-130463",
    "user": "https://github.com/burcin"
}
```

Reviewer: **Burcin Erocal**



---

archive/issue_comments_130464.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNice catch!\n\nIt's a simple fix, but we should add a doctest to make sure this doesn't come up again. It would be better to add tests for all different options for `algorithm`, but can you at least add a doctest for this case?",
    "created_at": "2011-11-17T15:16:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12049",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12049#issuecomment-130464",
    "user": "https://github.com/burcin"
}
```

<div id="comment:2" align="right">Comment 2</div>

Nice catch!

It's a simple fix, but we should add a doctest to make sure this doesn't come up again. It would be better to add tests for all different options for `algorithm`, but can you at least add a doctest for this case?



---

archive/issue_comments_130465.json:
```json
{
    "body": "Work Issues: **needs doctest**",
    "created_at": "2011-11-17T15:16:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12049",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12049#issuecomment-130465",
    "user": "https://github.com/burcin"
}
```

Work Issues: **needs doctest**



---

archive/issue_events_149107.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2011-11-17T15:16:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12049",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12049#event-149107"
}
```



---

archive/issue_events_149108.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2011-11-17T15:16:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12049",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12049#event-149108"
}
```



---

archive/issue_comments_130466.json:
```json
{
    "body": "replaces previous patch",
    "created_at": "2011-11-17T18:33:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12049",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12049#issuecomment-130466",
    "user": "https://github.com/dkrenn"
}
```

replaces previous patch



---

archive/issue_comments_130467.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nAttachment: **[trac_12049_algorithn_algorithm.2.patch.gz](https://github.com/sagemath/sage/files/ticket12049/trac_12049_algorithn_algorithm.2.patch.gz)**",
    "created_at": "2011-11-17T18:35:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12049",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12049#issuecomment-130467",
    "user": "https://github.com/dkrenn"
}
```

<div id="comment:3" align="right">Comment 3</div>

Attachment: **[trac_12049_algorithn_algorithm.2.patch.gz](https://github.com/sagemath/sage/files/ticket12049/trac_12049_algorithn_algorithm.2.patch.gz)**



---

archive/issue_comments_130468.json:
```json
{
    "body": "Changed work issues from **needs doctest** to none",
    "created_at": "2011-11-17T18:35:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12049",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12049#issuecomment-130468",
    "user": "https://github.com/dkrenn"
}
```

Changed work issues from **needs doctest** to none



---

archive/issue_events_149109.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2011-11-17T18:35:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12049",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12049#event-149109"
}
```



---

archive/issue_events_149110.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2011-11-17T18:35:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12049",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12049#event-149110"
}
```



---

archive/issue_events_149111.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2011-11-17T19:01:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12049",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12049#event-149111"
}
```



---

archive/issue_events_149112.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2011-11-17T19:01:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12049",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12049#event-149112"
}
```



---

archive/issue_comments_130469.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThanks! The new patch looks good to me. I didn't run doctests on the whole library, but I don't expect any problems. Let's see what the patchbot says.\n\nApply [attachment: trac_12049_algorithn_algorithm.2.patch.](https://github.com/sagemath/sage/files/ticket12049/trac_12049_algorithn_algorithm.2.patch..gz)",
    "created_at": "2011-11-17T19:01:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12049",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12049#issuecomment-130469",
    "user": "https://github.com/burcin"
}
```

<div id="comment:4" align="right">Comment 4</div>

Thanks! The new patch looks good to me. I didn't run doctests on the whole library, but I don't expect any problems. Let's see what the patchbot says.

Apply [attachment: trac_12049_algorithn_algorithm.2.patch.](https://github.com/sagemath/sage/files/ticket12049/trac_12049_algorithn_algorithm.2.patch..gz)



---

archive/issue_comments_130470.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n Looking through the code of ``matrix_integer_dense.pyx`` I found **``algorithn``** instead of **``algorithm``** in line 2689, which is wrong.\n+\n+Apply [attachment: trac_12049_algorithn_algorithm.2.patch.](https://github.com/sagemath/sage/files/ticket12049/trac_12049_algorithn_algorithm.2.patch..gz)\n``````\n",
    "created_at": "2011-11-17T19:01:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12049",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12049#issuecomment-130470",
    "user": "https://github.com/burcin"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
 Looking through the code of ``matrix_integer_dense.pyx`` I found **``algorithn``** instead of **``algorithm``** in line 2689, which is wrong.
+
+Apply [attachment: trac_12049_algorithn_algorithm.2.patch.](https://github.com/sagemath/sage/files/ticket12049/trac_12049_algorithn_algorithm.2.patch..gz)
``````




---

archive/issue_events_149113.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-23T13:25:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12049",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12049#event-149113"
}
```



---

archive/issue_events_149114.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-23T13:25:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12049",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12049#event-149114"
}
```



---

archive/issue_comments_130471.json:
```json
{
    "body": "Merged: **sage-4.8.alpha3**",
    "created_at": "2011-11-23T13:25:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12049",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12049#issuecomment-130471",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.8.alpha3**
