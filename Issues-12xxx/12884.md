...
<a id='comment:1'></a>Okay, a patch is ready. I don't see any way of removing the constraints efficiently, shy of trying to changing the constraints to a list. I'm not sure we lose that much in terms of efficiency in searching for a new constraint, but removing the ith constraint is quick and easy. Maybe you have a better way of removing the constraints?
...
```json
{
    "body": "<a id='comment:3'></a>> Well, what about taking the ith constraint, then normalize it (as you do before adding it to the set), then pop the normalized constraint out of the set ? We could have a _normalize_constraint method in the main class, which would be called before adding or removing a constraint, couldn't we ?\n\n\nI thought about that, but would floating-point comparisons be a problem?\n\n> And could you please add a doctest ? Otherwise I am pretty sure we could forget again this duplication mechanism next time we work on constraints `:-)`\n\n\nWe already have a doctest for the duplication mechanism. But, sure, I can add a doctest for this.",
...
<a id='comment:3'></a>> Well, what about taking the ith constraint, then normalize it (as you do before adding it to the set), then pop the normalized constraint out of the set ? We could have a _normalize_constraint method in the main class, which would be called before adding or removing a constraint, couldn't we ?
...
```json
{
    "body": "<a id='comment:4'></a>BTW, I will either work on this at home (where I have beta13 on two different machines!) or wait until 5.0 comes out. I don't want to download & build yet another beta at the office.",
...
<a id='comment:4'></a>BTW, I will either work on this at home (where I have beta13 on two different machines!) or wait until 5.0 comes out. I don't want to download & build yet another beta at the office.
...
```json
{
    "body": "<a id='comment:7'></a>+1 to experience against hasty thinking. Let them be lists `:-)`\n\nNathann",
...
<a id='comment:7'></a>+1 to experience against hasty thinking. Let them be lists `:-)`
...
<a id='comment:11'></a>Attachment [trac_12884.patch](tarball://root/attachments/some-uuid/ticket12884/trac_12884.patch) by @johnperry-math created at 2012-05-25 22:12:07
...
```json
{
    "body": "<a id='comment:13'></a>Please fill in your real name in the Author / Reviewer fields.",
...
<a id='comment:13'></a>Please fill in your real name in the Author / Reviewer fields.
