# Issue 12102: Make bzip2 a standard (instead of base) package

archive/issues_011930.json:
```json
{
    "body": "Now that we have support for gzipped Sage packages (#12602), we can easily make `bzip2` a standard package instead of a base package.  This will further the goal of decreasing the number of base packages (ideally to zero), see also #12631.\n\n**spkg**: [http://boxen.math.washington.edu/home/jdemeyer/spkg/bzip2-1.0.6.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/bzip2-1.0.6.spkg) (bzip2 compressed, to be converted to gzip when merged)\n\n**apply** [attachment:12102_bzip2_standard.patch] to `SAGE_ROOT`.\n\n**delete** the untracked file `spkg/base/bzip2-1.0.5.tar.gz`.\n\nAssignee: tbd\n\nReviewer: John Palmieri\n\nAuthor: Jeroen Demeyer\n\nMerged: sage-5.0.beta11\n\nDependencies: #12479, #10492, #12602\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/12102\n\n",
    "closed_at": "2012-03-28T10:04:20Z",
    "created_at": "2011-11-30T11:11:37Z",
    "labels": [
        "component: packages: standard"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-5.0",
    "title": "Make bzip2 a standard (instead of base) package",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/12102",
    "user": "https://github.com/jdemeyer"
}
```
Now that we have support for gzipped Sage packages (#12602), we can easily make `bzip2` a standard package instead of a base package.  This will further the goal of decreasing the number of base packages (ideally to zero), see also #12631.

**spkg**: [http://boxen.math.washington.edu/home/jdemeyer/spkg/bzip2-1.0.6.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/bzip2-1.0.6.spkg) (bzip2 compressed, to be converted to gzip when merged)

**apply** [attachment:12102_bzip2_standard.patch] to `SAGE_ROOT`.

**delete** the untracked file `spkg/base/bzip2-1.0.5.tar.gz`.

Assignee: tbd

Reviewer: John Palmieri

Author: Jeroen Demeyer

Merged: sage-5.0.beta11

Dependencies: #12479, #10492, #12602

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/12102





---

archive/issue_comments_168031.json:
```json
{
    "body": "Changing dependencies from \"\" to \"#12602\"",
    "created_at": "2012-03-05T14:12:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12102",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12102#issuecomment-168031",
    "user": "https://github.com/jdemeyer"
}
```

Changing dependencies from "" to "#12602"



---

archive/issue_comments_168032.json:
```json
{
    "body": "Changing author from \"\" to \"Jeroen Demeyer\"",
    "created_at": "2012-03-05T14:55:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12102",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12102#issuecomment-168032",
    "user": "https://github.com/jdemeyer"
}
```

Changing author from "" to "Jeroen Demeyer"



---

archive/issue_comments_168033.json:
```json
{
    "body": "Changing dependencies from \"#12602\" to \"#12479, #10492, #12602\"",
    "created_at": "2012-03-05T14:55:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12102",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12102#issuecomment-168033",
    "user": "https://github.com/jdemeyer"
}
```

Changing dependencies from "#12602" to "#12479, #10492, #12602"



---

archive/issue_comments_168034.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n-If we add support for gzipped files to `sage-spkg`, we can easily make `bzip2` a standard package instead of a base package.  This fits well with the theme of #11073.\n+If we add support for gzipped files to `sage-spkg`, we can easily make `bzip2` a standard package instead of a base package.\n+\n+**spkg**: [http://boxen.math.washington.edu/home/jdemeyer/spkg/bzip2-1.0.6.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/bzip2-1.0.6.spkg)\n``````\n",
    "created_at": "2012-03-05T15:04:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12102",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12102#issuecomment-168034",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
-If we add support for gzipped files to `sage-spkg`, we can easily make `bzip2` a standard package instead of a base package.  This fits well with the theme of #11073.
+If we add support for gzipped files to `sage-spkg`, we can easily make `bzip2` a standard package instead of a base package.
+
+**spkg**: [http://boxen.math.washington.edu/home/jdemeyer/spkg/bzip2-1.0.6.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/bzip2-1.0.6.spkg)
``````




---

archive/issue_comments_168035.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n If we add support for gzipped files to `sage-spkg`, we can easily make `bzip2` a standard package instead of a base package.\n \n-**spkg**: [http://boxen.math.washington.edu/home/jdemeyer/spkg/bzip2-1.0.6.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/bzip2-1.0.6.spkg)\n+**spkg**: [http://boxen.math.washington.edu/home/jdemeyer/spkg/bzip2-1.0.6.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/bzip2-1.0.6.spkg) (gzip compressed)\n``````\n",
    "created_at": "2012-03-05T15:05:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12102",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12102#issuecomment-168035",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
 If we add support for gzipped files to `sage-spkg`, we can easily make `bzip2` a standard package instead of a base package.
 
-**spkg**: [http://boxen.math.washington.edu/home/jdemeyer/spkg/bzip2-1.0.6.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/bzip2-1.0.6.spkg)
+**spkg**: [http://boxen.math.washington.edu/home/jdemeyer/spkg/bzip2-1.0.6.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/bzip2-1.0.6.spkg) (gzip compressed)
``````




---

archive/issue_comments_168036.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,5 @@\n If we add support for gzipped files to `sage-spkg`, we can easily make `bzip2` a standard package instead of a base package.\n \n **spkg**: [http://boxen.math.washington.edu/home/jdemeyer/spkg/bzip2-1.0.6.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/bzip2-1.0.6.spkg) (gzip compressed)\n+\n+**Release Manager**: delete the untracked file `spkg/base/bzip2-1.0.5.tar.gz`.\n``````\n",
    "created_at": "2012-03-05T16:01:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12102",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12102#issuecomment-168036",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,5 @@
 If we add support for gzipped files to `sage-spkg`, we can easily make `bzip2` a standard package instead of a base package.
 
 **spkg**: [http://boxen.math.washington.edu/home/jdemeyer/spkg/bzip2-1.0.6.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/bzip2-1.0.6.spkg) (gzip compressed)
+
+**Release Manager**: delete the untracked file `spkg/base/bzip2-1.0.5.tar.gz`.
``````




---

archive/issue_comments_168037.json:
```json
{
    "body": "<a id='comment:6'></a>Attachment [12102_bzip2_standard.patch](tarball://root/attachments/some-uuid/ticket12102/12102_bzip2_standard.patch) by @jdemeyer created at 2012-03-23 14:26:23",
    "created_at": "2012-03-23T14:26:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12102",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12102#issuecomment-168037",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:6'></a>Attachment [12102_bzip2_standard.patch](tarball://root/attachments/some-uuid/ticket12102/12102_bzip2_standard.patch) by @jdemeyer created at 2012-03-23 14:26:23



---

archive/issue_comments_168038.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,5 +1,7 @@\n-If we add support for gzipped files to `sage-spkg`, we can easily make `bzip2` a standard package instead of a base package.\n+Now that we have support for gzipped Sage packages (#12602), we can easily make `bzip2` a standard package instead of a base package.  This will further the goal of decreasing the number of base packages (ideally to zero), see also #12631.\n \n **spkg**: [http://boxen.math.washington.edu/home/jdemeyer/spkg/bzip2-1.0.6.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/bzip2-1.0.6.spkg) (gzip compressed)\n \n-**Release Manager**: delete the untracked file `spkg/base/bzip2-1.0.5.tar.gz`.\n+**apply** [attachment:12102_bzip2_standard.patch] to `SAGE_ROOT`.\n+\n+**delete** the untracked file `spkg/base/bzip2-1.0.5.tar.gz`.\n``````\n",
    "created_at": "2012-03-23T14:26:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12102",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12102#issuecomment-168038",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,5 +1,7 @@
-If we add support for gzipped files to `sage-spkg`, we can easily make `bzip2` a standard package instead of a base package.
+Now that we have support for gzipped Sage packages (#12602), we can easily make `bzip2` a standard package instead of a base package.  This will further the goal of decreasing the number of base packages (ideally to zero), see also #12631.
 
 **spkg**: [http://boxen.math.washington.edu/home/jdemeyer/spkg/bzip2-1.0.6.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/bzip2-1.0.6.spkg) (gzip compressed)
 
-**Release Manager**: delete the untracked file `spkg/base/bzip2-1.0.5.tar.gz`.
+**apply** [attachment:12102_bzip2_standard.patch] to `SAGE_ROOT`.
+
+**delete** the untracked file `spkg/base/bzip2-1.0.5.tar.gz`.
``````




---

archive/issue_comments_168039.json:
```json
{
    "body": "<a id='comment:7'></a>Is this ready for review?",
    "created_at": "2012-03-23T15:32:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12102",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12102#issuecomment-168039",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:7'></a>Is this ready for review?



---

archive/issue_comments_168040.json:
```json
{
    "body": "<a id='comment:8'></a>If you want, it's ready for review.  But to be honest, I haven't really tested it.",
    "created_at": "2012-03-23T15:37:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12102",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12102#issuecomment-168040",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:8'></a>If you want, it's ready for review.  But to be honest, I haven't really tested it.



---

archive/issue_comments_168041.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2012-03-25T09:08:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12102",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12102#issuecomment-168041",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_168042.json:
```json
{
    "body": "<a id='comment:10'></a>I see that the old package compiles with commands like this:\n\n```\ngcc -fPIC -c blocksort.c\n```\nwhile the new one compiles with\n\n```\ngcc -fPIC -O2 -g  -c blocksort.c\n```\n`-O2` seems like a fine idea, but do we need `-g`? I don't know much about debugging: since this is a pretty well-established package, there aren't like to be bugs in it, so can we omit `-g`, or would that mess up debugging for other parts of Sage? Should we add a test for `SAGE_DEBUG` so that we at least have the option to turn it off? I notice that the new `libbz2.a` is more than twice the size of the old one, at least on sage.math.",
    "created_at": "2012-03-26T21:55:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12102",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12102#issuecomment-168042",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:10'></a>I see that the old package compiles with commands like this:

```
gcc -fPIC -c blocksort.c
```
while the new one compiles with

```
gcc -fPIC -O2 -g  -c blocksort.c
```
`-O2` seems like a fine idea, but do we need `-g`? I don't know much about debugging: since this is a pretty well-established package, there aren't like to be bugs in it, so can we omit `-g`, or would that mess up debugging for other parts of Sage? Should we add a test for `SAGE_DEBUG` so that we at least have the option to turn it off? I notice that the new `libbz2.a` is more than twice the size of the old one, at least on sage.math.



---

archive/issue_comments_168043.json:
```json
{
    "body": "<a id='comment:11'></a>Replying to [jhpalmieri](#comment%3A10):\n> `-O2` seems like a fine idea, but do we need `-g`?\n\nI thought it was \"standard practice\" to compile all Sage packages using `-g` (regardless of `SAGE_DEBUG`).  With GCC, files only get larger when compiled with debug information, but the code is exactly the same.  So programs should run equally fast with or without debugging information (except for the I/O effects of having larger files).",
    "created_at": "2012-03-27T06:47:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12102",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12102#issuecomment-168043",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:11'></a>Replying to [jhpalmieri](#comment%3A10):
> `-O2` seems like a fine idea, but do we need `-g`?

I thought it was "standard practice" to compile all Sage packages using `-g` (regardless of `SAGE_DEBUG`).  With GCC, files only get larger when compiled with debug information, but the code is exactly the same.  So programs should run equally fast with or without debugging information (except for the I/O effects of having larger files).



---

archive/issue_comments_168044.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2012-03-27T17:15:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12102",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12102#issuecomment-168044",
    "user": "https://github.com/jhpalmieri"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_168045.json:
```json
{
    "body": "Changing reviewer from \"\" to \"John Palmieri\"",
    "created_at": "2012-03-27T17:15:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12102",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12102#issuecomment-168045",
    "user": "https://github.com/jhpalmieri"
}
```

Changing reviewer from "" to "John Palmieri"



---

archive/issue_comments_168046.json:
```json
{
    "body": "<a id='comment:12'></a>It looks to me as though `SAGE_DEBUG` is used somewhat inconsistently. Since William (among others) is concerned about the size of the Sage distribution, I think at some point we should consider not using `-g`. Maybe don't use it for `bdists`? Anyway, this is not something that should be decided here, and I'm happy with the changes in this ticket.",
    "created_at": "2012-03-27T17:15:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12102",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12102#issuecomment-168046",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:12'></a>It looks to me as though `SAGE_DEBUG` is used somewhat inconsistently. Since William (among others) is concerned about the size of the Sage distribution, I think at some point we should consider not using `-g`. Maybe don't use it for `bdists`? Anyway, this is not something that should be decided here, and I'm happy with the changes in this ticket.



---

archive/issue_comments_168047.json:
```json
{
    "body": "<a id='comment:13'></a>Let me mention that I also tested upgrading from sage-4.5 and sage-4.8, no problems.",
    "created_at": "2012-03-27T20:17:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12102",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12102#issuecomment-168047",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:13'></a>Let me mention that I also tested upgrading from sage-4.5 and sage-4.8, no problems.



---

archive/issue_comments_168048.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,6 +1,6 @@\n Now that we have support for gzipped Sage packages (#12602), we can easily make `bzip2` a standard package instead of a base package.  This will further the goal of decreasing the number of base packages (ideally to zero), see also #12631.\n \n-**spkg**: [http://boxen.math.washington.edu/home/jdemeyer/spkg/bzip2-1.0.6.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/bzip2-1.0.6.spkg) (gzip compressed)\n+**spkg**: [http://boxen.math.washington.edu/home/jdemeyer/spkg/bzip2-1.0.6.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/bzip2-1.0.6.spkg) (bzip2 compressed, to be converted to gzip when merged)\n \n **apply** [attachment:12102_bzip2_standard.patch] to `SAGE_ROOT`.\n \n``````\n",
    "created_at": "2012-03-28T08:14:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12102",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12102#issuecomment-168048",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,6 +1,6 @@
 Now that we have support for gzipped Sage packages (#12602), we can easily make `bzip2` a standard package instead of a base package.  This will further the goal of decreasing the number of base packages (ideally to zero), see also #12631.
 
-**spkg**: [http://boxen.math.washington.edu/home/jdemeyer/spkg/bzip2-1.0.6.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/bzip2-1.0.6.spkg) (gzip compressed)
+**spkg**: [http://boxen.math.washington.edu/home/jdemeyer/spkg/bzip2-1.0.6.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/bzip2-1.0.6.spkg) (bzip2 compressed, to be converted to gzip when merged)
 
 **apply** [attachment:12102_bzip2_standard.patch] to `SAGE_ROOT`.
 
``````




---

archive/issue_comments_168049.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-5.0.beta11\"",
    "created_at": "2012-03-28T10:04:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12102",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12102#issuecomment-168049",
    "user": "https://github.com/jdemeyer"
}
```

Changing merged from "" to "sage-5.0.beta11"



---

archive/issue_events_032151.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-03-28T10:04:20Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/12102",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12102#event-32151"
}
```



---

archive/issue_comments_168050.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2012-03-28T10:04:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12102",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12102#issuecomment-168050",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: fixed



---

archive/issue_comments_168051.json:
```json
{
    "body": "<a id='comment:16'></a>By the way, the file `spkg/base/README.txt` should be modified to reflect this change. Is the directory `spkg/base` going to disappear altogether soon? If so, we don't have to worry about this.",
    "created_at": "2012-04-03T18:51:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12102",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12102#issuecomment-168051",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:16'></a>By the way, the file `spkg/base/README.txt` should be modified to reflect this change. Is the directory `spkg/base` going to disappear altogether soon? If so, we don't have to worry about this.



---

archive/issue_comments_168052.json:
```json
{
    "body": "<a id='comment:17'></a>Replying to [jhpalmieri](#comment%3A16):\n> By the way, the file `spkg/base/README.txt` should be modified to reflect this change.\n\nGoing to do this in #12631.\n\n> Is the directory `spkg/base` going to disappear altogether soon?\n\nProbably not.  Eventually, I would like prereq to become a top-level `./configure` for Sage, but I haven't thought nearly enough about this to see how that would work.",
    "created_at": "2012-04-04T07:05:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12102",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12102#issuecomment-168052",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:17'></a>Replying to [jhpalmieri](#comment%3A16):
> By the way, the file `spkg/base/README.txt` should be modified to reflect this change.

Going to do this in #12631.

> Is the directory `spkg/base` going to disappear altogether soon?

Probably not.  Eventually, I would like prereq to become a top-level `./configure` for Sage, but I haven't thought nearly enough about this to see how that would work.
