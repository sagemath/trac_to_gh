...
```json
{
    "body": "While I was at it, I tried to upgrade to gmp 5.0.4. Unfortunately, it complains that it can not find bdivmod.c in the path -- the old spkg contains that file in src/mpn/generic, but the original gmp 5.0.4 sources do no contain bdivmod.c in src/mpn/generic. Too bad.",
...
```json
{
    "body": "See modified ticket description: It was easy to make gmp 5.0.4 build and pass all tests. Simply one has to drop all patches of the old gmp spkg.",
...
```json
{
    "body": "A related ticket: #12661 (optional `CLooG` spkg).\n\n`CLooG` needs gmp to build.",
...
```json
{
    "body": "Building the new gmp spkg here on Mac OS X (with gcc 4.6.3) with SAGE_CHECK=yes succeeds and passess all tests!",
...
```json
{
    "body": "GMP 5.0.5 is out since a while...",
...
```json
{
    "body": "gmp 5.1.x is out and transition to git is needed if we want to keep gmp in sage.",
...
```json
{
    "body": "Yup, also note that FLINT >= 2.4 can now be built on top of GMP.\nIIRC it's the only spkg which explicitely depended on MPIR.",
...
```json
{
    "body": "Also, mpir was not updated since Nov 2012. However gmp isn't even in optional now, so a new ticket would have to be opened to include it.",
...
```json
{
    "body": "Problems with farey_symbol.pyx which uses additional c++ source files (farey.cpp and sl2z.cpp) but cython wants to compile them with CC and that fails when GMP is used underneath.\nNot sure why it succeeded on top of MPIR.",
...
```json
{
    "body": "I got the first one in sage/rings/integer.pyx and knew it was from gmp vs mpir. I also have sage/tests/book_stein_modform.py but I didn't know it was originating from gmp vs mpir. I don't have the second failure in integer.pyx it may be because I am currently only at gmp 5.1.2.",
...
```json
{
    "body": "Actually I got the second one in integer.pyx. It wasn't present when I last made a record of doctests failures in sage-on-gentoo for sage 6.0, but it is in 6.2.beta2.",
...
```json
{
    "body": "Any reason why this is still stuck at \"need work\"? I can give this a positive review, fixing the doctests to work with gmp is probably more work than it is worth. Especially the top one in integer.pyx.\n\nOpinions?",
...
```json
{
    "body": "The only thing I can see right now that prevent sage to build with gmp out of the box is the inclusion of mpir.h instead of gmp.h in memory.c in clib.",
...
```json
{
    "body": "And you should also prevent Sage from building mpir I guess.",
...
```json
{
    "body": "Ambitious I see. Of course I have total control in sage-on-gentoo but I would appreciate just having to switch --with-gmp or something.",
...
```json
{
    "body": "Ok so everything is in fact already here.",
    "created_at": "2014-03-17T09:54:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12661",
    "type": "issue_comment",
...
```json
{
    "body": "I don't know that the renaming of sage_mpir_* to sage_gmp_* is necessary. In fact I am not even sure it needs to be named gmp or mpir at all. Aside from that it is all ok and conform to what we do in sage-on-gentoo. \nIt also would cross a patch and a sed from my ebuilds at least.",
    "created_at": "2014-03-17T10:04:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12661",
    "type": "issue_comment",
