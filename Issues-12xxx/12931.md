# Issue 12931: evaluation of coefficients of polynomials

archive/issues_012759.json:
```json
{
    "body": "It seems that substitution inside coefficients of a polynomial forgets to change the dominant term :\n\n```\nsage: rng=PolynomialRing(QQ,'q')\nsage: q=rng.gen()\nsage: x=PolynomialRing(rng,'x').gen()\nsage: po=q+(1+q)*x+(1+q+q**2)*x**2+(2+q)*x**3\nsage: po(q=1)\n(q + 2)*x^3 + 3*x^2 + 2*x + 1\n```\n\nSame behaviour in 4.8 and 5.0rc0\n\nFixed by #3979.\n\n**Assignee:** @aghitza\n\n**Keywords:** polynomials\n\n**Reviewer:** Travis Scrimshaw\n\nIssue created by migration from https://trac.sagemath.org/ticket/12931\n\n",
    "closed_at": "2012-12-21T22:45:16Z",
    "created_at": "2012-05-09T18:47:54Z",
    "labels": [
        "component: algebra",
        "bug",
        "duplicate"
    ],
    "title": "evaluation of coefficients of polynomials",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/12931",
    "user": "https://github.com/fchapoton"
}
```
It seems that substitution inside coefficients of a polynomial forgets to change the dominant term :

```
sage: rng=PolynomialRing(QQ,'q')
sage: q=rng.gen()
sage: x=PolynomialRing(rng,'x').gen()
sage: po=q+(1+q)*x+(1+q+q**2)*x**2+(2+q)*x**3
sage: po(q=1)
(q + 2)*x^3 + 3*x^2 + 2*x + 1
```

Same behaviour in 4.8 and 5.0rc0

Fixed by #3979.

**Assignee:** @aghitza

**Keywords:** polynomials

**Reviewer:** Travis Scrimshaw

Issue created by migration from https://trac.sagemath.org/ticket/12931





---

archive/issue_comments_150329.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -9,3 +9,4 @@\n (q + 2)*x^3 + 3*x^2 + 2*x + 1\n ```\n \n+Same behaviour in 4.8 and 5.0rc0\n``````\n",
    "created_at": "2012-05-09T18:49:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12931",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12931#issuecomment-150329",
    "user": "https://github.com/fchapoton"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -9,3 +9,4 @@
 (q + 2)*x^3 + 3*x^2 + 2*x + 1
 ```
 
+Same behaviour in 4.8 and 5.0rc0
``````




---

archive/issue_comments_150330.json:
```json
{
    "body": "<a id='comment:2'></a>\nThis is corrected in the latest patch for #3979; see the changes to `sage/rings/polynomial/polynomial_element.pyx`.  A new patch which should allow that ticket to be closed will be posted soon.",
    "created_at": "2012-05-10T07:36:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12931",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12931#issuecomment-150330",
    "user": "https://trac.sagemath.org/admin/accounts/users/fwclarke"
}
```

<a id='comment:2'></a>
This is corrected in the latest patch for #3979; see the changes to `sage/rings/polynomial/polynomial_element.pyx`.  A new patch which should allow that ticket to be closed will be posted soon.



---

archive/issue_comments_150331.json:
```json
{
    "body": "<a id='comment:3'></a>\nReplying to [fwclarke](#comment%3A2):\n> A new patch [...] will be posted soon.\n\n\nIt has been now.",
    "created_at": "2012-05-13T10:02:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12931",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12931#issuecomment-150331",
    "user": "https://trac.sagemath.org/admin/accounts/users/fwclarke"
}
```

<a id='comment:3'></a>
Replying to [fwclarke](#comment%3A2):
> A new patch [...] will be posted soon.


It has been now.



---

archive/issue_events_110697.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2012-11-14T15:21:35Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/12931",
    "milestone": "sage-5.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12931#event-110697"
}
```



---

archive/issue_events_110698.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2012-11-14T15:21:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/12931",
    "label": "duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12931#event-110698"
}
```



---

archive/issue_events_110699.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2012-11-14T15:21:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/12931",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12931#event-110699"
}
```



---

archive/issue_comments_150332.json:
```json
{
    "body": "**Reviewer:** Travis Scrimshaw",
    "created_at": "2012-11-17T06:13:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12931",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12931#issuecomment-150332",
    "user": "https://github.com/tscrim"
}
```

**Reviewer:** Travis Scrimshaw



---

archive/issue_events_110700.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2012-11-17T06:13:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/12931",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12931#event-110700"
}
```



---

archive/issue_events_110701.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2012-11-17T06:13:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/12931",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12931#event-110701"
}
```



---

archive/issue_events_110702.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-12-21T22:45:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/12931",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12931#event-110702"
}
```



---

archive/issue_events_110703.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-12-21T22:45:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/12931",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12931#event-110703"
}
```



---

archive/issue_comments_150333.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -10,3 +10,5 @@\n ```\n \n Same behaviour in 4.8 and 5.0rc0\n+\n+Fixed by #3979.\n``````\n",
    "created_at": "2012-12-21T22:45:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12931",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12931#issuecomment-150333",
    "user": "https://github.com/jdemeyer"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -10,3 +10,5 @@
 ```
 
 Same behaviour in 4.8 and 5.0rc0
+
+Fixed by #3979.
``````




---

archive/issue_events_110704.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-12-21T22:45:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/12931",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12931#event-110704"
}
```
