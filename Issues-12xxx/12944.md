# Issue 12944: Inconcistencies in the documentation of traveling_salesman_problem

archive/issues_012772.json:
```json
{
    "assignees": [],
    "body": "Just two minor things. See the discussion at the bottom of #10497.\n\nThese modifications were initially made in #12942, which has been updated\n\nNathann\n\nDepends on #12942\n\nAssignee: **@jasongrout, @nathanncohen, @rlmill**\n\nCC:  @kini\n\nReviewer: **Keshav Kini**\n\nAuthor: **Nathann Cohen**\n\nMerged: **sage-5.1.beta1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12944_\n\n",
    "closed_at": "2012-05-23T21:38:08Z",
    "created_at": "2012-05-13T19:37:24Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Inconcistencies in the documentation of traveling_salesman_problem",
    "type": "issue",
    "updated_at": "2012-05-23T21:38:08Z",
    "url": "https://github.com/sagemath/sage/issues/12944",
    "user": "https://github.com/nathanncohen"
}
```
Just two minor things. See the discussion at the bottom of #10497.

These modifications were initially made in #12942, which has been updated

Nathann

Depends on #12942

Assignee: **@jasongrout, @nathanncohen, @rlmill**

CC:  @kini

Reviewer: **Keshav Kini**

Author: **Nathann Cohen**

Merged: **sage-5.1.beta1**

_Issue created by migration from https://trac.sagemath.org/ticket/12944_





---

archive/issue_events_163954.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2012-05-13T19:37:24Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12944",
    "milestone_number": null,
    "milestone_title": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12944#event-163954"
}
```



---

archive/issue_events_163955.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2012-05-13T19:37:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12944",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12944#event-163955"
}
```



---

archive/issue_events_163956.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2012-05-13T19:37:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12944",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12944#event-163956"
}
```



---

archive/issue_events_163957.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2012-05-13T19:37:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12944",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12944#event-163957"
}
```



---

archive/issue_events_163958.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2012-05-13T19:38:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12944",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12944#event-163958"
}
```



---

archive/issue_events_163959.json:
```json
{
    "actor": "https://github.com/kini",
    "created_at": "2012-05-13T20:21:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12944",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12944#event-163959"
}
```



---

archive/issue_events_163960.json:
```json
{
    "actor": "https://github.com/kini",
    "created_at": "2012-05-13T20:21:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12944",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12944#event-163960"
}
```



---

archive/issue_comments_148963.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nThis patch breaks the doctests of the function! So we need to edit those doctests. But...\n\nYou said on #10497:\n> Though if I remember correctly the french lines have been removed recently in an unrelated patch by David Coudert `:-D`\n\nWell, this patch is at least not merged in 5.0.rc1, since the French lines in the docstring are still there - if I am to edit the doctests, I should know where that patch is, so I don't conflict with it. But because the patch is unrelated, I can't search for it - do you remember which patch it is? (Here is an example of why one should keep unrelated changes out of patches! :P)",
    "created_at": "2012-05-13T20:21:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12944",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12944#issuecomment-148963",
    "user": "https://github.com/kini"
}
```

<div id="comment:2" align="right">Comment 2</div>

This patch breaks the doctests of the function! So we need to edit those doctests. But...

You said on #10497:
> Though if I remember correctly the french lines have been removed recently in an unrelated patch by David Coudert `:-D`

Well, this patch is at least not merged in 5.0.rc1, since the French lines in the docstring are still there - if I am to edit the doctests, I should know where that patch is, so I don't conflict with it. But because the patch is unrelated, I can't search for it - do you remember which patch it is? (Here is an example of why one should keep unrelated changes out of patches! :P)



---

archive/issue_comments_148964.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\n> This patch breaks the doctests of the function! So we need to edit those doctests. But...\n\nOh, right !\n\n> Well, this patch is at least not merged in 5.0.rc1, since the French lines in the docstring are still there\n\nThese changes have been made in #12743 (thank you David for reminding me ! `:-D`), and will be merged into Sage-5.1-beta0.\n\nPatch updated !\n\nNathann",
    "created_at": "2012-05-14T08:54:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12944",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12944#issuecomment-148964",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:3" align="right">Comment 3</div>

> This patch breaks the doctests of the function! So we need to edit those doctests. But...

Oh, right !

> Well, this patch is at least not merged in 5.0.rc1, since the French lines in the docstring are still there

These changes have been made in #12743 (thank you David for reminding me ! `:-D`), and will be merged into Sage-5.1-beta0.

Patch updated !

Nathann



---

archive/issue_events_163961.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2012-05-14T08:54:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12944",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12944#event-163961"
}
```



---

archive/issue_events_163962.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2012-05-14T08:54:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12944",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12944#event-163962"
}
```



---

archive/issue_comments_148965.json:
```json
{
    "body": "Attachment: **[trac_12944.patch.gz](https://github.com/sagemath/sage/files/ticket12944/trac_12944.patch.gz)**",
    "created_at": "2012-05-14T08:54:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12944",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12944#issuecomment-148965",
    "user": "https://github.com/nathanncohen"
}
```

Attachment: **[trac_12944.patch.gz](https://github.com/sagemath/sage/files/ticket12944/trac_12944.patch.gz)**



---

archive/issue_events_163963.json:
```json
{
    "actor": "https://github.com/kini",
    "created_at": "2012-05-14T09:48:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12944",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12944#event-163963"
}
```



---

archive/issue_events_163964.json:
```json
{
    "actor": "https://github.com/kini",
    "created_at": "2012-05-14T09:48:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12944",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12944#event-163964"
}
```



---

archive/issue_comments_148966.json:
```json
{
    "body": "Reviewer: **Keshav Kini**",
    "created_at": "2012-05-14T09:48:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12944",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12944#issuecomment-148966",
    "user": "https://github.com/kini"
}
```

Reviewer: **Keshav Kini**



---

archive/issue_comments_148967.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nOh, good, it doesn't conflict with these lines at all :) Passes full doctests as well. Positive review!",
    "created_at": "2012-05-14T09:48:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12944",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12944#issuecomment-148967",
    "user": "https://github.com/kini"
}
```

<div id="comment:4" align="right">Comment 4</div>

Oh, good, it doesn't conflict with these lines at all :) Passes full doctests as well. Positive review!



---

archive/issue_comments_148968.json:
```json
{
    "body": "Author: **Nathann Cohen**",
    "created_at": "2012-05-14T09:48:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12944",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12944#issuecomment-148968",
    "user": "https://github.com/kini"
}
```

Author: **Nathann Cohen**



---

archive/issue_events_163965.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-23T21:38:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12944",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12944#event-163965"
}
```



---

archive/issue_events_163966.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-23T21:38:08Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12944",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12944#event-163966"
}
```



---

archive/issue_comments_148969.json:
```json
{
    "body": "Merged: **sage-5.1.beta1**",
    "created_at": "2012-05-23T21:38:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12944",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12944#issuecomment-148969",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.1.beta1**
