# Issue 12799: Fix PolyBoRi's dependencies in `module_list.py`

archive/issues_012627.json:
```json
{
    "body": "Actually only the `pbori` module needs to get rebuilt; while touching `$SAGE_LOCAL/include/polybori.h` doesn't help, touching `$SAGE_LOCAL/include/polybori/*` does (afterwards running `./sage -b`).\n\nThe better alternative is to fix the dependency list in `module_list.py`\n\n### Current patch\n* **Apply** [attachment:polybori_depends.3.patch]\n\nAssignee: @alexanderdreyer\n\nCC:  @nexttime\n\nUpstream: None of the above - read trac for reasoning.\n\nReviewer: Leif Leonhardy\n\nAuthor: Alexander Dreyer\n\nMerged: sage-5.0.beta13\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/12799\n\n",
    "closed_at": "2012-04-07T15:09:47Z",
    "created_at": "2012-04-03T14:14:41Z",
    "labels": [
        "component: build"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-5.0",
    "title": "Fix PolyBoRi's dependencies in `module_list.py`",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/12799",
    "user": "https://github.com/alexanderdreyer"
}
```
Actually only the `pbori` module needs to get rebuilt; while touching `$SAGE_LOCAL/include/polybori.h` doesn't help, touching `$SAGE_LOCAL/include/polybori/*` does (afterwards running `./sage -b`).

The better alternative is to fix the dependency list in `module_list.py`

### Current patch
* **Apply** [attachment:polybori_depends.3.patch]

Assignee: @alexanderdreyer

CC:  @nexttime

Upstream: None of the above - read trac for reasoning.

Reviewer: Leif Leonhardy

Author: Alexander Dreyer

Merged: sage-5.0.beta13

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/12799





---

archive/issue_comments_185905.json:
```json
{
    "body": "Attachment [polybori_depends.patch](tarball://root/attachments/some-uuid/ticket12799/polybori_depends.patch) by @alexanderdreyer created at 2012-04-03 14:15:43\n\nPatch for the sage library (devel/sage)",
    "created_at": "2012-04-03T14:15:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12799#issuecomment-185905",
    "user": "https://github.com/alexanderdreyer"
}
```

Attachment [polybori_depends.patch](tarball://root/attachments/some-uuid/ticket12799/polybori_depends.patch) by @alexanderdreyer created at 2012-04-03 14:15:43

Patch for the sage library (devel/sage)



---

archive/issue_comments_185906.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,4 +2,4 @@\n The better alternative is to fix the dependency list in `module_list.py`\n \n ### Current patch\n-* **Patch** \n+* **Apply** [attachment:polybori_depends.patch]\n``````\n",
    "created_at": "2012-04-03T14:16:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12799#issuecomment-185906",
    "user": "https://github.com/alexanderdreyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,4 +2,4 @@
 The better alternative is to fix the dependency list in `module_list.py`
 
 ### Current patch
-* **Patch** 
+* **Apply** [attachment:polybori_depends.patch]
``````




---

archive/issue_comments_185907.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2012-04-03T14:16:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12799#issuecomment-185907",
    "user": "https://github.com/alexanderdreyer"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_185908.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,5 @@\n-Actually only the pbori module needs to get rebuilt; while touching $SAGE_LOCAL/include/polybori.h doesn't help, touching $SAGE_LOCAL/include/polybori/* does (afterwards running ./sage -b).\n+Actually only the `pbori` module needs to get rebuilt; while touching `$SAGE_LOCAL/include/polybori.h` doesn't help, touching `$SAGE_LOCAL/include/polybori/*` does (afterwards running `./sage -b`).\n+\n The better alternative is to fix the dependency list in `module_list.py`\n \n ### Current patch\n``````\n",
    "created_at": "2012-04-03T20:40:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12799#issuecomment-185908",
    "user": "https://github.com/nexttime"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,5 @@
-Actually only the pbori module needs to get rebuilt; while touching $SAGE_LOCAL/include/polybori.h doesn't help, touching $SAGE_LOCAL/include/polybori/* does (afterwards running ./sage -b).
+Actually only the `pbori` module needs to get rebuilt; while touching `$SAGE_LOCAL/include/polybori.h` doesn't help, touching `$SAGE_LOCAL/include/polybori/*` does (afterwards running `./sage -b`).
+
 The better alternative is to fix the dependency list in `module_list.py`
 
 ### Current patch
``````




---

archive/issue_comments_185909.json:
```json
{
    "body": "Changing author from \"AlexanderDreyer\" to \"Alexander Dreyer\"",
    "created_at": "2012-04-03T20:40:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12799#issuecomment-185909",
    "user": "https://github.com/nexttime"
}
```

Changing author from "AlexanderDreyer" to "Alexander Dreyer"



---

archive/issue_comments_185910.json:
```json
{
    "body": "<a id='comment:2'></a>As mentioned on #12655, I'd rather make the extension module depend on just `$SAGE_INC/polybori.h` and/or its libraries, `$SAGE_LOCAL/lib/{polybori,polybori_groebner}.so`, and `touch` the former from `spkg-install` at least in case you don't add the libraries.\n\nAFAIK Cython is smart enough to notice when (at least *directly*) included headers are newer, but this doesn't help if their timestamps are preserved during PolyBoRi's installation.\n\nAdding *all of* PolyBoRi's headers *explicitly* seems a bit overkill to me.\n\n[Also note that we have both `$SAGE_INC/polybori.h` and `$SAGE_INC/polybori/polybori.h`, which are not the same files.]",
    "created_at": "2012-04-03T20:40:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12799#issuecomment-185910",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:2'></a>As mentioned on #12655, I'd rather make the extension module depend on just `$SAGE_INC/polybori.h` and/or its libraries, `$SAGE_LOCAL/lib/{polybori,polybori_groebner}.so`, and `touch` the former from `spkg-install` at least in case you don't add the libraries.

AFAIK Cython is smart enough to notice when (at least *directly*) included headers are newer, but this doesn't help if their timestamps are preserved during PolyBoRi's installation.

Adding *all of* PolyBoRi's headers *explicitly* seems a bit overkill to me.

[Also note that we have both `$SAGE_INC/polybori.h` and `$SAGE_INC/polybori/polybori.h`, which are not the same files.]



---

archive/issue_comments_185911.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Leif Leonhardy\"",
    "created_at": "2012-04-03T20:40:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12799#issuecomment-185911",
    "user": "https://github.com/nexttime"
}
```

Changing reviewer from "" to "Leif Leonhardy"



---

archive/issue_comments_185912.json:
```json
{
    "body": "<a id='comment:3'></a>Replying to [comment:2 leif]:\n> As mentioned on #12655, I'd rather make the extension module depend on just `$SAGE_INC/polybori.h` and/or its libraries, `$SAGE_LOCAL/lib/{polybori,polybori_groebner}.so`, and `touch` the former from `spkg-install` at least in case you don't add the libraries.\n\nThe timestamp of `include/polybori/config.h` also varies, this would also help here.\n\n> AFAIK Cython is smart enough to notice when (at least *directly*) included headers are newer, but this doesn't help if their timestamps are preserved during PolyBoRi's installation.\n> \n> Adding *all of* PolyBoRi's headers *explicitly* seems a bit overkill to me.\n> \n> [Also note that we have both `$SAGE_INC/polybori.h` and `$SAGE_INC/polybori/polybori.h`, which are not the same files.]\n\nThe first header is just for convenience.",
    "created_at": "2012-04-03T20:52:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12799#issuecomment-185912",
    "user": "https://github.com/alexanderdreyer"
}
```

<a id='comment:3'></a>Replying to [comment:2 leif]:
> As mentioned on #12655, I'd rather make the extension module depend on just `$SAGE_INC/polybori.h` and/or its libraries, `$SAGE_LOCAL/lib/{polybori,polybori_groebner}.so`, and `touch` the former from `spkg-install` at least in case you don't add the libraries.

The timestamp of `include/polybori/config.h` also varies, this would also help here.

> AFAIK Cython is smart enough to notice when (at least *directly*) included headers are newer, but this doesn't help if their timestamps are preserved during PolyBoRi's installation.
> 
> Adding *all of* PolyBoRi's headers *explicitly* seems a bit overkill to me.
> 
> [Also note that we have both `$SAGE_INC/polybori.h` and `$SAGE_INC/polybori/polybori.h`, which are not the same files.]

The first header is just for convenience.



---

archive/issue_comments_185913.json:
```json
{
    "body": "<a id='comment:4'></a>Replying to [comment:3 AlexanderDreyer]:\n> The timestamp of `include/polybori/config.h` also varies, this would also help here.\n\n\nThen I'd add just that... (cross-posting)",
    "created_at": "2012-04-03T20:58:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12799#issuecomment-185913",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:4'></a>Replying to [comment:3 AlexanderDreyer]:
> The timestamp of `include/polybori/config.h` also varies, this would also help here.


Then I'd add just that... (cross-posting)



---

archive/issue_comments_185914.json:
```json
{
    "body": "<a id='comment:5'></a>Replying to [comment:4 leif]:\n> Replying to [comment:3 AlexanderDreyer]:\n> > The timestamp of `include/polybori/config.h` also varies, this would also help here.\n\n> \n> Then I'd add just that... (cross-posting)\n\nOk, let's try this one:\n[attachment:polybori_depends.2.patch]",
    "created_at": "2012-04-03T21:13:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12799#issuecomment-185914",
    "user": "https://github.com/alexanderdreyer"
}
```

<a id='comment:5'></a>Replying to [comment:4 leif]:
> Replying to [comment:3 AlexanderDreyer]:
> > The timestamp of `include/polybori/config.h` also varies, this would also help here.

> 
> Then I'd add just that... (cross-posting)

Ok, let's try this one:
[attachment:polybori_depends.2.patch]



---

archive/issue_comments_185915.json:
```json
{
    "body": "Attachment [polybori_depends.2.patch](tarball://root/attachments/some-uuid/ticket12799/polybori_depends.2.patch) by @alexanderdreyer created at 2012-04-03 21:15:37\n\nPatch for the sage library (devel/sage)",
    "created_at": "2012-04-03T21:15:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12799#issuecomment-185915",
    "user": "https://github.com/alexanderdreyer"
}
```

Attachment [polybori_depends.2.patch](tarball://root/attachments/some-uuid/ticket12799/polybori_depends.2.patch) by @alexanderdreyer created at 2012-04-03 21:15:37

Patch for the sage library (devel/sage)



---

archive/issue_comments_185916.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -3,4 +3,4 @@\n The better alternative is to fix the dependency list in `module_list.py`\n \n ### Current patch\n-* **Apply** [attachment:polybori_depends.patch]\n+* **Apply** [attachment:polybori_depends.2.patch]\n``````\n",
    "created_at": "2012-04-03T21:15:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12799#issuecomment-185916",
    "user": "https://github.com/alexanderdreyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -3,4 +3,4 @@
 The better alternative is to fix the dependency list in `module_list.py`
 
 ### Current patch
-* **Apply** [attachment:polybori_depends.patch]
+* **Apply** [attachment:polybori_depends.2.patch]
``````




---

archive/issue_comments_185917.json:
```json
{
    "body": "<a id='comment:7'></a>Replying to [comment:5 AlexanderDreyer]:\n> Ok, let's try this one:\n> [attachment:polybori_depends.2.patch]\n\n\nLooks better.",
    "created_at": "2012-04-03T21:57:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12799#issuecomment-185917",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:7'></a>Replying to [comment:5 AlexanderDreyer]:
> Ok, let's try this one:
> [attachment:polybori_depends.2.patch]


Looks better.



---

archive/issue_comments_185918.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2012-04-03T22:46:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12799#issuecomment-185918",
    "user": "https://github.com/nexttime"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_185919.json:
```json
{
    "body": "Changing component from algebra to build.",
    "created_at": "2012-04-03T22:46:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12799#issuecomment-185919",
    "user": "https://github.com/nexttime"
}
```

Changing component from algebra to build.



---

archive/issue_comments_185920.json:
```json
{
    "body": "<a id='comment:8'></a>Applies to Sage 5.0.beta11 with three lines offset, but I don't mind.",
    "created_at": "2012-04-03T22:46:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12799#issuecomment-185920",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:8'></a>Applies to Sage 5.0.beta11 with three lines offset, but I don't mind.



---

archive/issue_comments_185921.json:
```json
{
    "body": "Attachment [polybori_depends.3.patch](tarball://root/attachments/some-uuid/ticket12799/polybori_depends.3.patch) by @alexanderdreyer created at 2012-04-04 07:42:51\n\npatch rebased to sage-5.0 beta11",
    "created_at": "2012-04-04T07:42:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12799#issuecomment-185921",
    "user": "https://github.com/alexanderdreyer"
}
```

Attachment [polybori_depends.3.patch](tarball://root/attachments/some-uuid/ticket12799/polybori_depends.3.patch) by @alexanderdreyer created at 2012-04-04 07:42:51

patch rebased to sage-5.0 beta11



---

archive/issue_comments_185922.json:
```json
{
    "body": "<a id='comment:9'></a>Replying to [comment:8 leif]:\n> Applies to Sage 5.0.beta11 with three lines offset, but I don't mind.\n\nThanks! I rebased the patch accordingly.",
    "created_at": "2012-04-04T07:44:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12799#issuecomment-185922",
    "user": "https://github.com/alexanderdreyer"
}
```

<a id='comment:9'></a>Replying to [comment:8 leif]:
> Applies to Sage 5.0.beta11 with three lines offset, but I don't mind.

Thanks! I rebased the patch accordingly.



---

archive/issue_comments_185923.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -3,4 +3,4 @@\n The better alternative is to fix the dependency list in `module_list.py`\n \n ### Current patch\n-* **Apply** [attachment:polybori_depends.2.patch]\n+* **Apply** [attachment:polybori_depends.3.patch]\n``````\n",
    "created_at": "2012-04-04T07:44:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12799#issuecomment-185923",
    "user": "https://github.com/alexanderdreyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -3,4 +3,4 @@
 The better alternative is to fix the dependency list in `module_list.py`
 
 ### Current patch
-* **Apply** [attachment:polybori_depends.2.patch]
+* **Apply** [attachment:polybori_depends.3.patch]
``````




---

archive/issue_comments_185924.json:
```json
{
    "body": "<a id='comment:10'></a>Replying to [comment:8 leif]:\n> Applies to Sage 5.0.beta11 with three lines offset, but I don't mind.\n\n\nI consider an offset in a patch to be totally harmless; fuzz 1 should be rebased but I can usually live with; fuzz 2 must be rebased.",
    "created_at": "2012-04-04T08:52:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12799#issuecomment-185924",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:10'></a>Replying to [comment:8 leif]:
> Applies to Sage 5.0.beta11 with three lines offset, but I don't mind.


I consider an offset in a patch to be totally harmless; fuzz 1 should be rebased but I can usually live with; fuzz 2 must be rebased.



---

archive/issue_comments_185925.json:
```json
{
    "body": "<a id='comment:11'></a>Replying to [comment:10 jdemeyer]:\n> Replying to [comment:8 leif]:\n> > Applies to Sage 5.0.beta11 with three lines offset, but I don't mind.\n\n> \n> I consider an offset in a patch to be totally harmless; fuzz 1 should be rebased but I can usually live with; fuzz 2 must be rebased.\n\n\nYes, I should perhaps have said that... ;-)",
    "created_at": "2012-04-04T19:30:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12799#issuecomment-185925",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:11'></a>Replying to [comment:10 jdemeyer]:
> Replying to [comment:8 leif]:
> > Applies to Sage 5.0.beta11 with three lines offset, but I don't mind.

> 
> I consider an offset in a patch to be totally harmless; fuzz 1 should be rebased but I can usually live with; fuzz 2 must be rebased.


Yes, I should perhaps have said that... ;-)



---

archive/issue_events_034812.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-04-07T15:09:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/12799",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12799#event-34812"
}
```



---

archive/issue_comments_185926.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-5.0.beta13\"",
    "created_at": "2012-04-07T15:09:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12799#issuecomment-185926",
    "user": "https://github.com/jdemeyer"
}
```

Changing merged from "" to "sage-5.0.beta13"



---

archive/issue_comments_185927.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2012-04-07T15:09:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12799#issuecomment-185927",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: fixed
