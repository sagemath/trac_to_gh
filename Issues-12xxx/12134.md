# Issue 12134: is_planar(set_pos=True) doesn't work with small graphs

archive/issues_011962.json:
```json
{
    "body": "```\nsage: graphs.PathGraph(2).is_planar(set_pos=True)\nTraceback (click to the left of this block for traceback)\n...\nNotImplementedError: _triangulate() only accepts graphs with more than 2\nvertices as input.\n```\n\nIt is interesting that _triangulate raises NotImplementedError. The usual definition of a triangulation says something like \"maximal planar graph\", so it seems like PathGraph(2) is already triangulated, but if you ask [Mathematica](http://mathworld.wolfram.com/TriangulatedGraph.html) or [OEIS](http://oeis.org/A000109), they don't accept this: according to this source there aren't any triangulated graphs on two vertices, so _triangulate should scream \"This can't be done\" and is_planar should avoid use _triangulate in this scenario, because there is no doubt that PathGraph(2) is planar.\n\nThe error is only present when set_pos=True. Also, notice that\n\n```\nsage: graphs.PathGraph(1).is_planar(set_pos=True)\nTrue\n```\n\nApply:\n* [attachment:trac_12134_typo_fix.patch]\n* [attachment:trac_12134_is_planar_special_cases.patch]\n* [attachment:trac_12134_reviewer.patch]\n\nAssignee: jason, ncohen, rlm\n\nCC:  @jasongrout\n\nResolution: fixed\n\nAuthor: Luk\u00e1\u0161 L\u00e1nsk\u00fd\n\nReviewer: Nathann Cohen\n\nMerged: sage-5.0.beta2\n\nIssue created by migration from https://trac.sagemath.org/ticket/12134\n\n",
    "closed_at": "2012-02-02T12:53:39Z",
    "created_at": "2011-12-09T19:16:36Z",
    "labels": [
        "component: graph theory",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-5.0",
    "title": "is_planar(set_pos=True) doesn't work with small graphs",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/12134",
    "user": "https://trac.sagemath.org/admin/accounts/users/brunellus"
}
```
```
sage: graphs.PathGraph(2).is_planar(set_pos=True)
Traceback (click to the left of this block for traceback)
...
NotImplementedError: _triangulate() only accepts graphs with more than 2
vertices as input.
```

It is interesting that _triangulate raises NotImplementedError. The usual definition of a triangulation says something like "maximal planar graph", so it seems like PathGraph(2) is already triangulated, but if you ask [Mathematica](http://mathworld.wolfram.com/TriangulatedGraph.html) or [OEIS](http://oeis.org/A000109), they don't accept this: according to this source there aren't any triangulated graphs on two vertices, so _triangulate should scream "This can't be done" and is_planar should avoid use _triangulate in this scenario, because there is no doubt that PathGraph(2) is planar.

The error is only present when set_pos=True. Also, notice that

```
sage: graphs.PathGraph(1).is_planar(set_pos=True)
True
```

Apply:
* [attachment:trac_12134_typo_fix.patch]
* [attachment:trac_12134_is_planar_special_cases.patch]
* [attachment:trac_12134_reviewer.patch]

Assignee: jason, ncohen, rlm

CC:  @jasongrout

Resolution: fixed

Author: Lukáš Lánský

Reviewer: Nathann Cohen

Merged: sage-5.0.beta2

Issue created by migration from https://trac.sagemath.org/ticket/12134





---

archive/issue_comments_169023.json:
```json
{
    "body": "<a id='comment:1'></a>\n```\nsage: graphs.PathGraph(3).is_planar(set_pos=True)\nTraceback (most recent call last):\n  File \"<stdin>\", line 1, in <module>\n  File \"_sage_input_30.py\", line 10, in <module>\n    exec compile(u'open(\"___code___.py\",\"w\").write(\"# -*- coding: utf-8 -*-\\\\n\" + _support_.preparse_worksheet_cell(base64.b64decode(\"Z3JhcGhzLlBhdGhHcmFwaCgzKS5pc19wbGFuYXIoc2V0X3Bvcz1UcnVlKQ==\"),globals())+\"\\\\n\"); execfile(os.path.abspath(\"___code___.py\"))\n  File \"\", line 1, in <module>\n    \n  File \"/tmp/tmpcidEK0/___code___.py\", line 3, in <module>\n    exec compile(u'graphs.PathGraph(_sage_const_3 ).is_planar(set_pos=True)\n  File \"\", line 1, in <module>\n    \n  File \"/sagenb/sage_install/sage-4.7.2/local/lib/python2.6/site-packages/sage/graphs/generic_graph.py\", line 2572, in is_planar\n    planar = is_planar(G,kuratowski=kuratowski,set_pos=set_pos,set_embedding=set_embedding)\n  File \"planarity.pyx\", line 143, in sage.graphs.planarity.is_planar (sage/graphs/planarity.c:1574)\n  File \"/sagenb/sage_install/sage-4.7.2/local/lib/python2.6/site-packages/sage/graphs/generic_graph.py\", line 2760, in set_planar_positions\n    self.layout(layout = \"planar\", save_pos = True, test = test, **layout_options)\n  File \"/sagenb/sage_install/sage-4.7.2/local/lib/python2.6/site-packages/sage/graphs/generic_graph.py\", line 12753, in layout\n    pos = getattr(self, \"layout_%s\"%layout)(dim = dim, **options)\n  File \"/sagenb/sage_install/sage-4.7.2/local/lib/python2.6/site-packages/sage/graphs/generic_graph.py\", line 2859, in layout_planar\n    extra_edges = _triangulate( G, G._embedding)\n  File \"/sagenb/sage_install/sage-4.7.2/local/lib/python2.6/site-packages/sage/graphs/schnyder.py\", line 67, in _triangulate\n    if len(g.neighbors(vertex_list[0]) == 2):          # figure our which of the vertices already has two neighbors\nTypeError: object of type 'bool' has no len()\n```\n\nUfff... For longer paths it seems fine. This error is also dependent on set_pos=True.",
    "created_at": "2011-12-09T19:23:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12134",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12134#issuecomment-169023",
    "user": "https://trac.sagemath.org/admin/accounts/users/brunellus"
}
```

<a id='comment:1'></a>
```
sage: graphs.PathGraph(3).is_planar(set_pos=True)
Traceback (most recent call last):
  File "<stdin>", line 1, in <module>
  File "_sage_input_30.py", line 10, in <module>
    exec compile(u'open("___code___.py","w").write("# -*- coding: utf-8 -*-\\n" + _support_.preparse_worksheet_cell(base64.b64decode("Z3JhcGhzLlBhdGhHcmFwaCgzKS5pc19wbGFuYXIoc2V0X3Bvcz1UcnVlKQ=="),globals())+"\\n"); execfile(os.path.abspath("___code___.py"))
  File "", line 1, in <module>
    
  File "/tmp/tmpcidEK0/___code___.py", line 3, in <module>
    exec compile(u'graphs.PathGraph(_sage_const_3 ).is_planar(set_pos=True)
  File "", line 1, in <module>
    
  File "/sagenb/sage_install/sage-4.7.2/local/lib/python2.6/site-packages/sage/graphs/generic_graph.py", line 2572, in is_planar
    planar = is_planar(G,kuratowski=kuratowski,set_pos=set_pos,set_embedding=set_embedding)
  File "planarity.pyx", line 143, in sage.graphs.planarity.is_planar (sage/graphs/planarity.c:1574)
  File "/sagenb/sage_install/sage-4.7.2/local/lib/python2.6/site-packages/sage/graphs/generic_graph.py", line 2760, in set_planar_positions
    self.layout(layout = "planar", save_pos = True, test = test, **layout_options)
  File "/sagenb/sage_install/sage-4.7.2/local/lib/python2.6/site-packages/sage/graphs/generic_graph.py", line 12753, in layout
    pos = getattr(self, "layout_%s"%layout)(dim = dim, **options)
  File "/sagenb/sage_install/sage-4.7.2/local/lib/python2.6/site-packages/sage/graphs/generic_graph.py", line 2859, in layout_planar
    extra_edges = _triangulate( G, G._embedding)
  File "/sagenb/sage_install/sage-4.7.2/local/lib/python2.6/site-packages/sage/graphs/schnyder.py", line 67, in _triangulate
    if len(g.neighbors(vertex_list[0]) == 2):          # figure our which of the vertices already has two neighbors
TypeError: object of type 'bool' has no len()
```

Ufff... For longer paths it seems fine. This error is also dependent on set_pos=True.



---

archive/issue_comments_169024.json:
```json
{
    "body": "<a id='comment:2'></a>(cc me)",
    "created_at": "2011-12-09T23:51:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12134",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12134#issuecomment-169024",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:2'></a>(cc me)



---

archive/issue_comments_169025.json:
```json
{
    "body": "Attachment [trac_12134_typo_fix.patch](tarball://root/attachments/some-uuid/ticket12134/trac_12134_typo_fix.patch) by brunellus created at 2011-12-22 15:12:24",
    "created_at": "2011-12-22T15:12:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12134",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12134#issuecomment-169025",
    "user": "https://trac.sagemath.org/admin/accounts/users/brunellus"
}
```

Attachment [trac_12134_typo_fix.patch](tarball://root/attachments/some-uuid/ticket12134/trac_12134_typo_fix.patch) by brunellus created at 2011-12-22 15:12:24



---

archive/issue_comments_169026.json:
```json
{
    "body": "<a id='comment:4'></a>The patch resolves P_3 error -- it was merely a typo. The main problem is independent of this and I'll try to fix it later this week.",
    "created_at": "2011-12-22T15:15:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12134",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12134#issuecomment-169026",
    "user": "https://trac.sagemath.org/admin/accounts/users/brunellus"
}
```

<a id='comment:4'></a>The patch resolves P_3 error -- it was merely a typo. The main problem is independent of this and I'll try to fix it later this week.



---

archive/issue_comments_169027.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2011-12-31T03:30:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12134",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12134#issuecomment-169027",
    "user": "https://trac.sagemath.org/admin/accounts/users/brunellus"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_169028.json:
```json
{
    "body": "<a id='comment:5'></a>Attachment [trac_12134_is_planar_special_cases.patch](tarball://root/attachments/some-uuid/ticket12134/trac_12134_is_planar_special_cases.patch) by brunellus created at 2011-12-31 03:30:30\n\nOK, that should be it. Please apply both patches. This should also solve #10139.",
    "created_at": "2011-12-31T03:30:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12134",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12134#issuecomment-169028",
    "user": "https://trac.sagemath.org/admin/accounts/users/brunellus"
}
```

<a id='comment:5'></a>Attachment [trac_12134_is_planar_special_cases.patch](tarball://root/attachments/some-uuid/ticket12134/trac_12134_is_planar_special_cases.patch) by brunellus created at 2011-12-31 03:30:30

OK, that should be it. Please apply both patches. This should also solve #10139.



---

archive/issue_comments_169029.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Nathann Cohen\"",
    "created_at": "2011-12-31T09:43:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12134",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12134#issuecomment-169029",
    "user": "https://github.com/nathanncohen"
}
```

Changing reviewer from "" to "Nathann Cohen"



---

archive/issue_comments_169030.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -14,3 +14,8 @@\n sage: graphs.PathGraph(1).is_planar(set_pos=True)\n True\n ```\n+\n+Apply:\n+* [attachment:trac_12134_typo_fix.patch]\n+* [attachment:trac_12134_is_planar_special_cases.patch]\n+* [attachment:trac_12134_reviewer.patch]\n``````\n",
    "created_at": "2011-12-31T09:43:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12134",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12134#issuecomment-169030",
    "user": "https://github.com/nathanncohen"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -14,3 +14,8 @@
 sage: graphs.PathGraph(1).is_planar(set_pos=True)
 True
 ```
+
+Apply:
+* [attachment:trac_12134_typo_fix.patch]
+* [attachment:trac_12134_is_planar_special_cases.patch]
+* [attachment:trac_12134_reviewer.patch]
``````




---

archive/issue_comments_169031.json:
```json
{
    "body": "<a id='comment:6'></a>Helloooooooo !!\n\nA good patch, with a several things to fix: \n\n* There must always be a double semicolumn \"::\" before any Sage code in the doctests, so that it appears as code in the documentation.\n* The first doctest you add in planarity.pyx is written on one *very long* linem and is not that easy to read `^^;`. We have ways to write doctests on several lines when we need it, though ! You have many such examples in files like graph.py (hint: look for the string \"sage: for\" to see how it is done !). Sadly, this also means that the \"long\" flags have to be copied on each line.\n* There is no need to test connectedness at this moment :\n  {{{\nif len(g) == 2 and g.is_connected():\n      }}}\n      There are two graphs on 2 vertices, and if the graph is not connected it has been detected just previously by g.size() == 0.\n* The .vertices() method does a lot of unnecessary stuff (for instance is *sorts* the vertices before returning them), so it hurts a bit when I see \n  {{{\n{ g.vertices()[0]: [g.vertices()[1]], g.vertices()[1]: [g.vertices()[0]] }\n      }}}\n\nI added a reviewer's patch to this ticket that does precisely that, so if you can agree with its changes you can set this ticket to positively reviewed ! `:-)`\n\nThank you for your patch !\n\nNathann",
    "created_at": "2011-12-31T09:43:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12134",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12134#issuecomment-169031",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:6'></a>Helloooooooo !!

A good patch, with a several things to fix: 

* There must always be a double semicolumn "::" before any Sage code in the doctests, so that it appears as code in the documentation.
* The first doctest you add in planarity.pyx is written on one *very long* linem and is not that easy to read `^^;`. We have ways to write doctests on several lines when we need it, though ! You have many such examples in files like graph.py (hint: look for the string "sage: for" to see how it is done !). Sadly, this also means that the "long" flags have to be copied on each line.
* There is no need to test connectedness at this moment :
  {{{
if len(g) == 2 and g.is_connected():
      }}}
      There are two graphs on 2 vertices, and if the graph is not connected it has been detected just previously by g.size() == 0.
* The .vertices() method does a lot of unnecessary stuff (for instance is *sorts* the vertices before returning them), so it hurts a bit when I see 
  {{{
{ g.vertices()[0]: [g.vertices()[1]], g.vertices()[1]: [g.vertices()[0]] }
      }}}

I added a reviewer's patch to this ticket that does precisely that, so if you can agree with its changes you can set this ticket to positively reviewed ! `:-)`

Thank you for your patch !

Nathann



---

archive/issue_comments_169032.json:
```json
{
    "body": "<a id='comment:7'></a>Arg... I forgot about those `%#$)`@`#(*%&&` loops on edges. Sorry about my remark on your second call to ``is_connected``, I updated my patch so that it sticks to your version on this point... Anyway, it's only a  graph on 2 nodes `:-p`\n\nNathann",
    "created_at": "2011-12-31T09:51:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12134",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12134#issuecomment-169032",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:7'></a>Arg... I forgot about those `%#$)`@`#(*%&&` loops on edges. Sorry about my remark on your second call to ``is_connected``, I updated my patch so that it sticks to your version on this point... Anyway, it's only a  graph on 2 nodes `:-p`

Nathann



---

archive/issue_comments_169033.json:
```json
{
    "body": "<a id='comment:8'></a>Thanks for these remarks. I'll look into the other patches I sent and correct similar errors. :-)\n\nIf we suppose that the function planarity.is_planar is only accessed by generic_graph.is_planar, then there shouldn't be any loops at this moment, because the generic_graph function makes a graph simple first. I'm not sure if that's true, but I must admit I didn't wrote the code with this in mind -- I just didn't notice that disconected case was handled by previous change. :-)\n\nI plan to work with this more. For example, it seems to me that teaching _triangulate how to handle disconnected cases shouldn't be very hard, as allowing embedding of multigraphs. I guess this can be usefull for #6236.",
    "created_at": "2011-12-31T15:08:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12134",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12134#issuecomment-169033",
    "user": "https://trac.sagemath.org/admin/accounts/users/brunellus"
}
```

<a id='comment:8'></a>Thanks for these remarks. I'll look into the other patches I sent and correct similar errors. :-)

If we suppose that the function planarity.is_planar is only accessed by generic_graph.is_planar, then there shouldn't be any loops at this moment, because the generic_graph function makes a graph simple first. I'm not sure if that's true, but I must admit I didn't wrote the code with this in mind -- I just didn't notice that disconected case was handled by previous change. :-)

I plan to work with this more. For example, it seems to me that teaching _triangulate how to handle disconnected cases shouldn't be very hard, as allowing embedding of multigraphs. I guess this can be usefull for #6236.



---

archive/issue_comments_169034.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2011-12-31T15:08:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12134",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12134#issuecomment-169034",
    "user": "https://trac.sagemath.org/admin/accounts/users/brunellus"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_169035.json:
```json
{
    "body": "<a id='comment:9'></a>> If we suppose that the function planarity.is_planar is only accessed by generic_graph.is_planar, then there shouldn't be any loops at this moment, because the generic_graph function makes a graph simple first. I'm not sure if that's true, but I must admit I didn't wrote the code with this in mind -- I just didn't notice that disconected case was handled by previous change. :-)\n\n\nHmmmm... Well, anyway that's not such a horrible problem :-)`\n\n> I plan to work with this more. For example, it seems to me that teaching _triangulate how to handle disconnected cases shouldn't be very hard, as allowing embedding of multigraphs. I guess this can be usefull for #6236.\n\n\n+1 for the idea, and +1 for your relationship with Sage's source code. The lad knows a lot of stuff I do not know myself, but its education is still lacking on many points.\n\nBy the way, you should add your name to the \"Authors\" field of the ticket, lest the guy above (the release manager) .set the ticket back to \"needs_work\"\n\nNathann",
    "created_at": "2011-12-31T15:15:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12134",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12134#issuecomment-169035",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:9'></a>> If we suppose that the function planarity.is_planar is only accessed by generic_graph.is_planar, then there shouldn't be any loops at this moment, because the generic_graph function makes a graph simple first. I'm not sure if that's true, but I must admit I didn't wrote the code with this in mind -- I just didn't notice that disconected case was handled by previous change. :-)


Hmmmm... Well, anyway that's not such a horrible problem :-)`

> I plan to work with this more. For example, it seems to me that teaching _triangulate how to handle disconnected cases shouldn't be very hard, as allowing embedding of multigraphs. I guess this can be usefull for #6236.


+1 for the idea, and +1 for your relationship with Sage's source code. The lad knows a lot of stuff I do not know myself, but its education is still lacking on many points.

By the way, you should add your name to the "Authors" field of the ticket, lest the guy above (the release manager) .set the ticket back to "needs_work"

Nathann



---

archive/issue_events_032265.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-12-31T16:25:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/12134",
    "milestone": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12134#event-32265"
}
```



---

archive/issue_comments_169036.json:
```json
{
    "body": "<a id='comment:11'></a>My God... I remember when \"milestone 5.0\" meant \"whishlist\" `:-D`\n\nNathann",
    "created_at": "2011-12-31T16:30:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12134",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12134#issuecomment-169036",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:11'></a>My God... I remember when "milestone 5.0" meant "whishlist" `:-D`

Nathann



---

archive/issue_comments_169037.json:
```json
{
    "body": "Changing author from \"\" to \"Luk\u00e1\u0161 L\u00e1nsk\u00fd\"",
    "created_at": "2012-01-01T15:49:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12134",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12134#issuecomment-169037",
    "user": "https://trac.sagemath.org/admin/accounts/users/brunellus"
}
```

Changing author from "" to "Lukáš Lánský"



---

archive/issue_comments_169038.json:
```json
{
    "body": "Changing status from positive_review to needs_work.",
    "created_at": "2012-01-10T07:15:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12134",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12134#issuecomment-169038",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from positive_review to needs_work.



---

archive/issue_comments_169039.json:
```json
{
    "body": "<a id='comment:13'></a>I get a doctest error:\n\n```\nsage -t  -force_lib devel/sage/sage/graphs/planarity.pyx\n**********************************************************************\nFile \"/mnt/usb1/scratch/jdemeyer/merger/sage-5.0.beta1/devel/sage-main/sage/graphs/planarity.pyx\", line 68:\n    sage: for i,g in enumerate(atlas_graphs):\n          if (not g.is_connected() or i==Integer(0)):\n              continue\n          try:\n              _ = g.is_planar(set_embedding=True, set_pos=True)\n          except:\n              print \"There is something wrong here !\"\n              break\nException raised:\n    Traceback (most recent call last):\n      File \"/mnt/usb1/scratch/jdemeyer/merger/sage-5.0.beta1/local/bin/ncadoctest.py\", line 1231, in run_one_test\n        self.run_one_example(test, example, filename, compileflags)\n      File \"/mnt/usb1/scratch/jdemeyer/merger/sage-5.0.beta1/local/bin/sagedoctest.py\", line 38, in run_one_example\n        OrigDocTestRunner.run_one_example(self, test, example, filename, compileflags)\n      File \"/mnt/usb1/scratch/jdemeyer/merger/sage-5.0.beta1/local/bin/ncadoctest.py\", line 1172, in run_one_example\n        compileflags, 1) in test.globs\n      File \"<doctest __main__.example_1[6]>\", line 1, in <module>\n        for i,g in enumerate(atlas_graphs):###line 68:\n    sage: for i,g in enumerate(atlas_graphs):\n    NameError: name 'atlas_graphs' is not defined\n**********************************************************************\n```",
    "created_at": "2012-01-10T07:15:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12134",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12134#issuecomment-169039",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:13'></a>I get a doctest error:

```
sage -t  -force_lib devel/sage/sage/graphs/planarity.pyx
**********************************************************************
File "/mnt/usb1/scratch/jdemeyer/merger/sage-5.0.beta1/devel/sage-main/sage/graphs/planarity.pyx", line 68:
    sage: for i,g in enumerate(atlas_graphs):
          if (not g.is_connected() or i==Integer(0)):
              continue
          try:
              _ = g.is_planar(set_embedding=True, set_pos=True)
          except:
              print "There is something wrong here !"
              break
Exception raised:
    Traceback (most recent call last):
      File "/mnt/usb1/scratch/jdemeyer/merger/sage-5.0.beta1/local/bin/ncadoctest.py", line 1231, in run_one_test
        self.run_one_example(test, example, filename, compileflags)
      File "/mnt/usb1/scratch/jdemeyer/merger/sage-5.0.beta1/local/bin/sagedoctest.py", line 38, in run_one_example
        OrigDocTestRunner.run_one_example(self, test, example, filename, compileflags)
      File "/mnt/usb1/scratch/jdemeyer/merger/sage-5.0.beta1/local/bin/ncadoctest.py", line 1172, in run_one_example
        compileflags, 1) in test.globs
      File "<doctest __main__.example_1[6]>", line 1, in <module>
        for i,g in enumerate(atlas_graphs):###line 68:
    sage: for i,g in enumerate(atlas_graphs):
    NameError: name 'atlas_graphs' is not defined
**********************************************************************
```



---

archive/issue_comments_169040.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2012-01-10T07:42:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12134",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12134#issuecomment-169040",
    "user": "https://github.com/nathanncohen"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_169041.json:
```json
{
    "body": "<a id='comment:14'></a>Attachment [trac_12134_reviewer.patch](tarball://root/attachments/some-uuid/ticket12134/trac_12134_reviewer.patch) by @nathanncohen created at 2012-01-10 07:42:46\n\nArggggggg.... Patch updated `:-/`\n\nThe missing variable was declared with a \"# long time\" flag, so the doctest was running with the -long flag and we missed it `O_o`\n\nI set the whole thing to \"long\" in the end.\n\nLukasz can you give it another look ? \n\nNathann",
    "created_at": "2012-01-10T07:42:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12134",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12134#issuecomment-169041",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:14'></a>Attachment [trac_12134_reviewer.patch](tarball://root/attachments/some-uuid/ticket12134/trac_12134_reviewer.patch) by @nathanncohen created at 2012-01-10 07:42:46

Arggggggg.... Patch updated `:-/`

The missing variable was declared with a "# long time" flag, so the doctest was running with the -long flag and we missed it `O_o`

I set the whole thing to "long" in the end.

Lukasz can you give it another look ? 

Nathann



---

archive/issue_comments_169042.json:
```json
{
    "body": "<a id='comment:15'></a>Sorry, I should have noticed this. :-( Now it seems OK and tests show no error.",
    "created_at": "2012-01-11T17:32:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12134",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12134#issuecomment-169042",
    "user": "https://trac.sagemath.org/admin/accounts/users/brunellus"
}
```

<a id='comment:15'></a>Sorry, I should have noticed this. :-( Now it seems OK and tests show no error.



---

archive/issue_comments_169043.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2012-01-13T10:15:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12134",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12134#issuecomment-169043",
    "user": "https://trac.sagemath.org/admin/accounts/users/brunellus"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_169044.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2012-02-02T12:53:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12134",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12134#issuecomment-169044",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: fixed



---

archive/issue_events_032266.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-02T12:53:39Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/12134",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12134#event-32266"
}
```



---

archive/issue_comments_169045.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-5.0.beta2\"",
    "created_at": "2012-02-02T12:53:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12134",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12134#issuecomment-169045",
    "user": "https://github.com/jdemeyer"
}
```

Changing merged from "" to "sage-5.0.beta2"
