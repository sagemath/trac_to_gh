...
```json
{
    "body": "That's unexpected, It could be a cvxopt problem rather than a scipy problem. I may be able to check on Monday when I go back to work for real if there are more problems with cvxopt.",
...
```json
{
    "body": "Yes I can confirm that with 5.1beta5 (and cvxopt-1.1.5 from #13160). Will look at the code.",
...
```json
{
    "body": "I cannot see it going in scal[DOUBLE] on my x86_64 box either. I will have to dig it a bit more to find how to crack what is happening there.",
...
```json
{
    "body": "Finally found out it is supposed to be a direct call to blas dscal_ but the declaration are all wrong for that I think. I may have to copy the declaration from base.c and see if it works.",
...
```json
{
    "body": "I have no clue why the call to dscal_ either doesn't happen or return the wrong thing.",
...
```json
{
    "body": "maybe this is due to the fact that the power7 is big endian, and some code is not endian-safe?\n\nPaul",
...
```json
{
    "body": "I am pinging upstream to see if they have a clue on where things go wrong.",
...
```json
{
    "body": "Very similar results if I use openblas instead of atlas. There are some slight differences after 100 iterations - possibly a test of iterative precision of a sort. But still fails to converge.",
...
```json
{
    "body": "I am currently in contact to figure out where things go wrong. I doesn't quite fit any of the given options.",
...
```json
{
    "body": "I have an unstable release to test to see if it fixes the problem.",
...
```json
{
    "body": "Francois, who is \"upstream\" here? Scipy?\n\nPaul",
...
```json
{
    "body": "No cvxopt: [http://abel.ee.ucla.edu/cvxopt/](http://abel.ee.ucla.edu/cvxopt/) in particular Lieven Vandenberghe has been in contact with me regularly to find the problem.",
...
```json
{
    "body": "The beta provided by upstream fix the bug. I need to polish it a little bit before even thinking of posting it here. I have to rebase the solaris patch and fix spkg-check for the changes in 1.1.6 but the doctests pass and one test upstream devised to find the problem now passes too.",
...
```json
{
    "body": "Upstream has released 1.1.6 with power7 fix included. spkg when I have time or someone else has it.",
...
```json
{
    "body": "I'm including the fix from #10508.",
    "created_at": "2013-05-15T08:40:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12832",
    "type": "issue_comment",
...
```json
{
    "body": "Attachment [cvxopt-1.1.6.p0.diff](tarball://root/attachments/some-uuid/ticket12832/cvxopt-1.1.6.p0.diff) by @zimmermann6 created at 2013-05-15 08:59:35\n\nI'll review that ticket on silius. Now downloading Sage 5.9 source...\n\nPaul",
...
```json
{
    "body": "Isn't that silly, building stuff manually so we don't have to trouble the buildbot? I thought skynet is there to serve us, not the other way 'round ;-)",
...
```json
{
    "body": "Volker, what is wrong in testing a patch with the latest Sage release?\n\nPaul",
...
```json
{
    "body": "Does spkg-check works? I thought it would need work when I checked the rc sent to me by upstream.",
...
```json
{
    "body": "Also from my point of view it works on power7, it needs checking against #10508 and other platforms.",
...
```json
{
    "body": "OK there may have been special testing code in the rc I was sent. spkg-check definitely didn't work with it. I had to run various tests manually after install.",
...
```json
{
    "body": "Works with #10508 and fixes the power7 issue. So I guess its good to go.",
...
```json
{
    "body": "As reported on sage-release by Steven Trogdon, this fails to build if `SAGE_SPKG_INSTALL_DOCS=yes`, because the (pre-built) HTML docs have moved from `src/doc/build/html/` to `src/doc/html/`.",
...
```json
{
    "body": "P.P.S.:  Perhaps also `s|$SAGE_LOCAL/share|$SAGE_SHARE|`.\n\nAnd add error checks w.r.t. doc installation.",
...
```json
{
    "body": "Follow-up is #14645, new spkg on the way...",
