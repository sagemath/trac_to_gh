# Issue 12500: sagetex: fix self-tests on OpenSolaris

archive/issues_012328.json:
```json
{
    "assignees": [],
    "body": "On OpenSolaris, sagetex's self-tests fail.  New spkg:\n\n[http://sage.math.washington.edu/home/palmieri/SPKG/sagetex-2.3.3.p2.spkg](http://sage.math.washington.edu/home/palmieri/SPKG/sagetex-2.3.3.p2.spkg)\n\n\nCC:  @dandrake\n\nAuthor: **John Palmieri**\n\nReviewer: **Jeroen Demeyer**\n\nMerged: **sage-5.0.beta4**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12500_\n\n",
    "closed_at": "2012-02-12T09:33:31Z",
    "created_at": "2012-02-11T23:54:21Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sagetex: fix self-tests on OpenSolaris",
    "type": "issue",
    "updated_at": "2012-02-13T02:41:30Z",
    "url": "https://github.com/sagemath/sage/issues/12500",
    "user": "https://github.com/jhpalmieri"
}
```
On OpenSolaris, sagetex's self-tests fail.  New spkg:

[http://sage.math.washington.edu/home/palmieri/SPKG/sagetex-2.3.3.p2.spkg](http://sage.math.washington.edu/home/palmieri/SPKG/sagetex-2.3.3.p2.spkg)


CC:  @dandrake

Author: **John Palmieri**

Reviewer: **Jeroen Demeyer**

Merged: **sage-5.0.beta4**

_Issue created by migration from https://trac.sagemath.org/ticket/12500_





---

archive/issue_events_156510.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-02-11T23:54:21Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12500",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12500#event-156510"
}
```



---

archive/issue_events_156511.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-02-11T23:54:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12500",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12500#event-156511"
}
```



---

archive/issue_events_156512.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-02-11T23:54:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12500",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12500#event-156512"
}
```



---

archive/issue_events_156513.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-02-11T23:54:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12500",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12500#event-156513"
}
```



---

archive/issue_comments_140450.json:
```json
{
    "body": "patch for sagetex spkg; for reference only",
    "created_at": "2012-02-11T23:56:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12500",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12500#issuecomment-140450",
    "user": "https://github.com/jhpalmieri"
}
```

patch for sagetex spkg; for reference only



---

archive/issue_comments_140451.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nAttachment: **[trac_12500-sagetex.patch.gz](https://github.com/sagemath/sage/files/ticket12500/trac_12500-sagetex.patch.gz)**",
    "created_at": "2012-02-11T23:57:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12500",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12500#issuecomment-140451",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:1" align="right">Comment 1</div>

Attachment: **[trac_12500-sagetex.patch.gz](https://github.com/sagemath/sage/files/ticket12500/trac_12500-sagetex.patch.gz)**



---

archive/issue_events_156514.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-02-11T23:57:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12500",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12500#event-156514"
}
```



---

archive/issue_comments_140452.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2012-02-12T09:33:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12500",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12500#issuecomment-140452",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_156515.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-12T09:33:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12500",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12500#event-156515"
}
```



---

archive/issue_events_156516.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-12T09:33:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12500",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12500#event-156516"
}
```



---

archive/issue_comments_140453.json:
```json
{
    "body": "Merged: **sage-5.0.beta4**",
    "created_at": "2012-02-12T09:33:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12500",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12500#issuecomment-140453",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.0.beta4**



---

archive/issue_comments_140454.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nThanks for noticing this. I've pulled your changes into my spkg repo.",
    "created_at": "2012-02-13T02:41:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12500",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12500#issuecomment-140454",
    "user": "https://github.com/dandrake"
}
```

<div id="comment:3" align="right">Comment 3</div>

Thanks for noticing this. I've pulled your changes into my spkg repo.
