# Issue 12820: OS X Lion: use "clang" as default C compiler

archive/issues_012648.json:
```json
{
    "body": "William Stein reports this problem while building the GCC spkg:\n\n```\nchecking for x86_64-apple-darwin11.3.0-gcc...\n/Users/wstein/sage/install/sage-5.0.beta12-gcc/spkg/build/gcc-4.6.3/gcc-build/./gcc/xgcc\n-B/Users/wstein/sage/install/sage-5.0.beta12-gcc/spkg/build/gcc-4.6.3/gcc-build/./gcc/\n-B/Users/wstein/sage/install/sage-5.0.beta12-gcc/local/x86_64-apple-darwin11.3.0/bin/\n-B/Users/wstein/sage/install/sage-5.0.beta12-gcc/local/x86_64-apple-darwin11.3.0/lib/\n-isystem /Users/wstein/sage/install/sage-5.0.beta12-gcc/local/x86_64-apple-darwin11.3.0/include\n-isystem /Users/wstein/sage/install/sage-5.0.beta12-gcc/local/x86_64-apple-darwin11.3.0/sys-include\nchecking for suffix of object files... configure: error: in\n`/Users/wstein/sage/install/sage-5.0.beta12-gcc/spkg/build/gcc-4.6.3/gcc-build/x86_64-apple-darwin11.3.0/libgcc':\nconfigure: error: cannot compute suffix of object files: cannot compile\nSee `config.log' for more details.\nmake[4]: *** [configure-stage1-target-libgcc] Error 1\nmake[3]: *** [stage1-bubble] Error 2\nmake[2]: *** [all] Error 2\n```\n\nThe problem seems to be an outdated XCode version which has a broken GCC.  Building with clang instead succeeded.  Therefore, I propose to set \"CC=clang\" by default on OS X 10.7 or later in sage-env.\n\nAssignee: GeorgSWeber\n\nReviewer: Leif Leonhardy\n\nAuthor: Jeroen Demeyer\n\nMerged: sage-5.0.beta14\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/12820\n\n",
    "closed_at": "2012-04-19T06:40:33Z",
    "created_at": "2012-04-09T13:27:00Z",
    "labels": [
        "component: build",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-5.0",
    "title": "OS X Lion: use \"clang\" as default C compiler",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/12820",
    "user": "https://github.com/jdemeyer"
}
```
William Stein reports this problem while building the GCC spkg:

```
checking for x86_64-apple-darwin11.3.0-gcc...
/Users/wstein/sage/install/sage-5.0.beta12-gcc/spkg/build/gcc-4.6.3/gcc-build/./gcc/xgcc
-B/Users/wstein/sage/install/sage-5.0.beta12-gcc/spkg/build/gcc-4.6.3/gcc-build/./gcc/
-B/Users/wstein/sage/install/sage-5.0.beta12-gcc/local/x86_64-apple-darwin11.3.0/bin/
-B/Users/wstein/sage/install/sage-5.0.beta12-gcc/local/x86_64-apple-darwin11.3.0/lib/
-isystem /Users/wstein/sage/install/sage-5.0.beta12-gcc/local/x86_64-apple-darwin11.3.0/include
-isystem /Users/wstein/sage/install/sage-5.0.beta12-gcc/local/x86_64-apple-darwin11.3.0/sys-include
checking for suffix of object files... configure: error: in
`/Users/wstein/sage/install/sage-5.0.beta12-gcc/spkg/build/gcc-4.6.3/gcc-build/x86_64-apple-darwin11.3.0/libgcc':
configure: error: cannot compute suffix of object files: cannot compile
See `config.log' for more details.
make[4]: *** [configure-stage1-target-libgcc] Error 1
make[3]: *** [stage1-bubble] Error 2
make[2]: *** [all] Error 2
```

The problem seems to be an outdated XCode version which has a broken GCC.  Building with clang instead succeeded.  Therefore, I propose to set "CC=clang" by default on OS X 10.7 or later in sage-env.

Assignee: GeorgSWeber

Reviewer: Leif Leonhardy

Author: Jeroen Demeyer

Merged: sage-5.0.beta14

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/12820





---

archive/issue_comments_186470.json:
```json
{
    "body": "Changing type from PLEASE CHANGE to defect.",
    "created_at": "2012-04-09T13:27:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12820#issuecomment-186470",
    "user": "https://github.com/jdemeyer"
}
```

Changing type from PLEASE CHANGE to defect.



---

archive/issue_comments_186471.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2012-04-09T14:35:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12820#issuecomment-186471",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_186472.json:
```json
{
    "body": "<a id='comment:3'></a>Shouldn't that currently only be done if we really (decide to) build the GCC spkg, i.e., in `spkg/install` (rather than `sage-env`)?\n\n(Sage currently wouldn't fully build with Clang [3.x].)\n\nAlso, are all possible versions of XCode's Clang capable of building GCC?",
    "created_at": "2012-04-13T15:42:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12820#issuecomment-186472",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:3'></a>Shouldn't that currently only be done if we really (decide to) build the GCC spkg, i.e., in `spkg/install` (rather than `sage-env`)?

(Sage currently wouldn't fully build with Clang [3.x].)

Also, are all possible versions of XCode's Clang capable of building GCC?



---

archive/issue_comments_186473.json:
```json
{
    "body": "<a id='comment:4'></a>P.S.: The new spkgs for building Sage with GCC 4.7.x (see meta-ticket #12751) also fix *part of* the issues we have with `clang`.\n\n(The LinBox spkg at #12762 would still need work in order to build with `clang`, as it currently just adds `-fpermissive` to get around C++11 issues.)",
    "created_at": "2012-04-13T15:50:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12820#issuecomment-186473",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:4'></a>P.S.: The new spkgs for building Sage with GCC 4.7.x (see meta-ticket #12751) also fix *part of* the issues we have with `clang`.

(The LinBox spkg at #12762 would still need work in order to build with `clang`, as it currently just adds `-fpermissive` to get around C++11 issues.)



---

archive/issue_comments_186474.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2012-04-13T15:55:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12820#issuecomment-186474",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_186475.json:
```json
{
    "body": "<a id='comment:5'></a>Replying to [leif](#comment%3A3):\n> Shouldn't that currently only be done if we really (decide to) build the GCC spkg, i.e., in `spkg/install` (rather than `sage-env`)?\n\nOr in `sage-env` but depending on `SAGE_BUILD_TOOLCHAIN`.\n\n> (Sage currently wouldn't fully build with Clang [3.x].)\n\nIt doesn't build with Apple's GCC either... but you might have a point, in case somebody decides to install a custom GCC outside of Sage.\n\n> Also, are all possible versions of XCode's Clang capable of building GCC?\n\nI'm not sure.  But at least one person reported being able to build GCC with Apple's Clang but not with Apple's GCC.",
    "created_at": "2012-04-13T15:55:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12820#issuecomment-186475",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:5'></a>Replying to [leif](#comment%3A3):
> Shouldn't that currently only be done if we really (decide to) build the GCC spkg, i.e., in `spkg/install` (rather than `sage-env`)?

Or in `sage-env` but depending on `SAGE_BUILD_TOOLCHAIN`.

> (Sage currently wouldn't fully build with Clang [3.x].)

It doesn't build with Apple's GCC either... but you might have a point, in case somebody decides to install a custom GCC outside of Sage.

> Also, are all possible versions of XCode's Clang capable of building GCC?

I'm not sure.  But at least one person reported being able to build GCC with Apple's Clang but not with Apple's GCC.



---

archive/issue_comments_186476.json:
```json
{
    "body": "<a id='comment:6'></a>\n```\n+    # On OS X 10.7 (Lion) or later, use clang as default C compiler\n+    # as some older versions of XCode 4 ship broken versions of GCC\n+    # which fail to bootstrap GCC-4.6.3.\n```\n\nQuoting William:\n\n  *I dropped this [Sage 5.0.beta13] on my OS X 10.7.x laptop, didn't type anything special, typed \"make ptestlong\", and came back later to see \"All tests passed!\".  I.e., it built perfectly.*\n\n  *Note that I've upgraded both the OS and XCode to the latest released version on this laptop.  I think this is reasonable to require by anybody building Sage from source on OS X 10.7, given that XCode is free.*",
    "created_at": "2012-04-13T15:57:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12820#issuecomment-186476",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:6'></a>
```
+    # On OS X 10.7 (Lion) or later, use clang as default C compiler
+    # as some older versions of XCode 4 ship broken versions of GCC
+    # which fail to bootstrap GCC-4.6.3.
```

Quoting William:

  *I dropped this [Sage 5.0.beta13] on my OS X 10.7.x laptop, didn't type anything special, typed "make ptestlong", and came back later to see "All tests passed!".  I.e., it built perfectly.*

  *Note that I've upgraded both the OS and XCode to the latest released version on this laptop.  I think this is reasonable to require by anybody building Sage from source on OS X 10.7, given that XCode is free.*



---

archive/issue_comments_186477.json:
```json
{
    "body": "<a id='comment:7'></a>Replying to [jdemeyer](#comment%3A5):\n> Replying to [leif](#comment%3A3):\n> > Also, are all possible versions of XCode's Clang capable of building GCC?\n\n> I'm not sure.  But at least one person reported being able to build GCC with Apple's Clang but not with Apple's GCC.\n\nSomething worth to add to the README(?) / installation instructions.\n\nPerhaps some might have a broken [Apple] GCC, while others may have a Clang version of Apple not suited to build GCC.  Or just add that one should install the latest version of XCode on MacOS X 10.7.x, as William suggested.",
    "created_at": "2012-04-13T16:02:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12820#issuecomment-186477",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:7'></a>Replying to [jdemeyer](#comment%3A5):
> Replying to [leif](#comment%3A3):
> > Also, are all possible versions of XCode's Clang capable of building GCC?

> I'm not sure.  But at least one person reported being able to build GCC with Apple's Clang but not with Apple's GCC.

Something worth to add to the README(?) / installation instructions.

Perhaps some might have a broken [Apple] GCC, while others may have a Clang version of Apple not suited to build GCC.  Or just add that one should install the latest version of XCode on MacOS X 10.7.x, as William suggested.



---

archive/issue_comments_186478.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2012-04-14T11:25:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12820#issuecomment-186478",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_186479.json:
```json
{
    "body": "<a id='comment:8'></a>Attachment [12820_lion_clang.patch](tarball://root/attachments/some-uuid/ticket12820/12820_lion_clang.patch) by @jdemeyer created at 2012-04-14 11:25:22\n\nChanged the patch to only set `CC=clang` if `[ \"$SAGE_BUILD_TOOLCHAIN\" = yes ]`.",
    "created_at": "2012-04-14T11:25:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12820#issuecomment-186479",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:8'></a>Attachment [12820_lion_clang.patch](tarball://root/attachments/some-uuid/ticket12820/12820_lion_clang.patch) by @jdemeyer created at 2012-04-14 11:25:22

Changed the patch to only set `CC=clang` if `[ "$SAGE_BUILD_TOOLCHAIN" = yes ]`.



---

archive/issue_comments_186480.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Leif Leonhardy\"",
    "created_at": "2012-04-14T12:41:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12820#issuecomment-186480",
    "user": "https://github.com/nexttime"
}
```

Changing reviewer from "" to "Leif Leonhardy"



---

archive/issue_comments_186481.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2012-04-14T12:41:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12820#issuecomment-186481",
    "user": "https://github.com/nexttime"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_186482.json:
```json
{
    "body": "<a id='comment:9'></a>Replying to [jdemeyer](#comment%3A8):\n> Changed the patch to only set `CC=clang` if `[ \"$SAGE_BUILD_TOOLCHAIN\" = yes ]`.\n\n\nPatch looks ok (and applies to beta13), I just still wonder whether that shouldn't simply be done in `spkg/install` (where also `SAGE_BUILD_TOOLCHAIN` is set; it isn't set anywhere else).\n\nIs there any scenario where a script sourcing `sage-env` (or called by one which does) would need `CC=clang` if `SAGE_BUILD_TOOLCHAIN=yes`, but **isn't** (indirectly) called by `spkg/install` / the \"second level\" `make`?  (Probably you have some further plans...)\n\nI don't think I'm going to actually test that (on Lion), so rather \"theoretical\" positive review.  (If someone objects, of course feel free to revert it.)",
    "created_at": "2012-04-14T12:41:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12820#issuecomment-186482",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:9'></a>Replying to [jdemeyer](#comment%3A8):
> Changed the patch to only set `CC=clang` if `[ "$SAGE_BUILD_TOOLCHAIN" = yes ]`.


Patch looks ok (and applies to beta13), I just still wonder whether that shouldn't simply be done in `spkg/install` (where also `SAGE_BUILD_TOOLCHAIN` is set; it isn't set anywhere else).

Is there any scenario where a script sourcing `sage-env` (or called by one which does) would need `CC=clang` if `SAGE_BUILD_TOOLCHAIN=yes`, but **isn't** (indirectly) called by `spkg/install` / the "second level" `make`?  (Probably you have some further plans...)

I don't think I'm going to actually test that (on Lion), so rather "theoretical" positive review.  (If someone objects, of course feel free to revert it.)



---

archive/issue_comments_186483.json:
```json
{
    "body": "<a id='comment:10'></a>P.S.: Is the `config.log` from the failure in the description available?  Just curious how or why exactly it failed...",
    "created_at": "2012-04-14T12:44:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12820#issuecomment-186483",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:10'></a>P.S.: Is the `config.log` from the failure in the description available?  Just curious how or why exactly it failed...



---

archive/issue_comments_186484.json:
```json
{
    "body": "<a id='comment:11'></a>`config.log`: I've seen it once or a few times, but I can't find it again.  It's some kind of Internal Compiler Error.  Basically, the GCC built by Apple's gcc totally doesn't work, even `-O0` doesn't help.",
    "created_at": "2012-04-14T20:25:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12820#issuecomment-186484",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:11'></a>`config.log`: I've seen it once or a few times, but I can't find it again.  It's some kind of Internal Compiler Error.  Basically, the GCC built by Apple's gcc totally doesn't work, even `-O0` doesn't help.



---

archive/issue_comments_186485.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2012-04-19T06:40:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12820#issuecomment-186485",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: fixed



---

archive/issue_comments_186486.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-5.0.beta14\"",
    "created_at": "2012-04-19T06:40:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12820#issuecomment-186486",
    "user": "https://github.com/jdemeyer"
}
```

Changing merged from "" to "sage-5.0.beta14"



---

archive/issue_events_034891.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-04-19T06:40:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/12820",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12820#event-34891"
}
```
