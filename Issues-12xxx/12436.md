# Issue 12436: lcalc uses variable length arrays

archive/issues_012264.json:
```json
{
    "assignees": [],
    "body": "This is an issue for building on non-gnu compilers. While not perfect, a solution for this is to use std::vector.\n\nI've posted an spkg with this solution (fixes #12435, #12437) at  [http://wstein.org/home/ohanar/clang-port/sage-5.0.beta1-src/spkg/standard/lcalc-1.23.p10.spkg](http://wstein.org/home/ohanar/clang-port/sage-5.0.beta1-src/spkg/standard/lcalc-1.23.p10.spkg). (This is a review spkg in case there are more issues with clang down the road.) \n\nDepends on #12681\n\nAssignee: **GeorgSWeber**\n\nReviewer: **Fran\u00e7ois Bissey**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12436_\n\n",
    "closed_at": "2017-01-21T18:03:11Z",
    "created_at": "2012-02-04T18:31:52Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "lcalc uses variable length arrays",
    "type": "issue",
    "updated_at": "2017-01-21T18:03:11Z",
    "url": "https://github.com/sagemath/sage/issues/12436",
    "user": "https://github.com/ohanar"
}
```
This is an issue for building on non-gnu compilers. While not perfect, a solution for this is to use std::vector.

I've posted an spkg with this solution (fixes #12435, #12437) at  [http://wstein.org/home/ohanar/clang-port/sage-5.0.beta1-src/spkg/standard/lcalc-1.23.p10.spkg](http://wstein.org/home/ohanar/clang-port/sage-5.0.beta1-src/spkg/standard/lcalc-1.23.p10.spkg). (This is a review spkg in case there are more issues with clang down the road.) 

Depends on #12681

Assignee: **GeorgSWeber**

Reviewer: **François Bissey**

_Issue created by migration from https://trac.sagemath.org/ticket/12436_





---

archive/issue_events_155400.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2012-02-04T18:31:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12436#event-155400"
}
```



---

archive/issue_events_155401.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2012-02-04T18:31:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12436#event-155401"
}
```



---

archive/issue_events_155402.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2012-02-04T18:31:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12436#event-155402"
}
```



---

archive/issue_events_155403.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2012-02-04T18:31:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12436#event-155403"
}
```



---

archive/issue_comments_139207.json:
```json
{
    "body": "Attachment: **[lcalc-variable-length.patch.gz](https://github.com/sagemath/sage/files/ticket12436/lcalc-variable-length.patch.gz)**\n\nfor review purposes",
    "created_at": "2012-02-04T18:57:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12436#issuecomment-139207",
    "user": "https://github.com/ohanar"
}
```

Attachment: **[lcalc-variable-length.patch.gz](https://github.com/sagemath/sage/files/ticket12436/lcalc-variable-length.patch.gz)**

for review purposes



---

archive/issue_events_155404.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2012-02-04T18:59:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12436#event-155404"
}
```



---

archive/issue_comments_139208.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n This is an issue for building on non-gnu compilers. While not perfect, a solution for this is to use std::vector.\n+\n+I've posted an spkg with solution (fixes #12435, #12437) at  [http://wstein.org/home/ohanar/clang-port/sage-5.0.beta1-src/spkg/standard/lcalc-1.23.p10.spkg](http://wstein.org/home/ohanar/clang-port/sage-5.0.beta1-src/spkg/standard/lcalc-1.23.p10.spkg). (This is a review spkg in case there are more issues with clang down the road.) \n``````\n",
    "created_at": "2012-02-04T18:59:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12436#issuecomment-139208",
    "user": "https://github.com/ohanar"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
 This is an issue for building on non-gnu compilers. While not perfect, a solution for this is to use std::vector.
+
+I've posted an spkg with solution (fixes #12435, #12437) at  [http://wstein.org/home/ohanar/clang-port/sage-5.0.beta1-src/spkg/standard/lcalc-1.23.p10.spkg](http://wstein.org/home/ohanar/clang-port/sage-5.0.beta1-src/spkg/standard/lcalc-1.23.p10.spkg). (This is a review spkg in case there are more issues with clang down the road.) 
``````




---

archive/issue_comments_139209.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n This is an issue for building on non-gnu compilers. While not perfect, a solution for this is to use std::vector.\n \n-I've posted an spkg with solution (fixes #12435, #12437) at  [http://wstein.org/home/ohanar/clang-port/sage-5.0.beta1-src/spkg/standard/lcalc-1.23.p10.spkg](http://wstein.org/home/ohanar/clang-port/sage-5.0.beta1-src/spkg/standard/lcalc-1.23.p10.spkg). (This is a review spkg in case there are more issues with clang down the road.) \n+I've posted an spkg with this solution (fixes #12435, #12437) at  [http://wstein.org/home/ohanar/clang-port/sage-5.0.beta1-src/spkg/standard/lcalc-1.23.p10.spkg](http://wstein.org/home/ohanar/clang-port/sage-5.0.beta1-src/spkg/standard/lcalc-1.23.p10.spkg). (This is a review spkg in case there are more issues with clang down the road.) \n``````\n",
    "created_at": "2012-02-04T18:59:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12436#issuecomment-139209",
    "user": "https://github.com/ohanar"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
 This is an issue for building on non-gnu compilers. While not perfect, a solution for this is to use std::vector.
 
-I've posted an spkg with solution (fixes #12435, #12437) at  [http://wstein.org/home/ohanar/clang-port/sage-5.0.beta1-src/spkg/standard/lcalc-1.23.p10.spkg](http://wstein.org/home/ohanar/clang-port/sage-5.0.beta1-src/spkg/standard/lcalc-1.23.p10.spkg). (This is a review spkg in case there are more issues with clang down the road.) 
+I've posted an spkg with this solution (fixes #12435, #12437) at  [http://wstein.org/home/ohanar/clang-port/sage-5.0.beta1-src/spkg/standard/lcalc-1.23.p10.spkg](http://wstein.org/home/ohanar/clang-port/sage-5.0.beta1-src/spkg/standard/lcalc-1.23.p10.spkg). (This is a review spkg in case there are more issues with clang down the road.) 
``````




---

archive/issue_comments_139210.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nWe now need a new spkg (based on that of #12681) incorporating this patch.\n\nWonder whether we should submit it upstream; I personally would (currently) only apply the patch if `$CC` is *not* GCC.",
    "created_at": "2012-03-22T12:22:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12436#issuecomment-139210",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:3'>Comment 3:</a>
We now need a new spkg (based on that of #12681) incorporating this patch.

Wonder whether we should submit it upstream; I personally would (currently) only apply the patch if `$CC` is *not* GCC.



---

archive/issue_events_155405.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2012-03-22T12:22:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12436#event-155405"
}
```



---

archive/issue_events_155406.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2012-03-22T12:22:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12436#event-155406"
}
```



---

archive/issue_comments_139211.json:
```json
{
    "body": "Changed dependencies from **12435** to **#12681**",
    "created_at": "2012-03-22T12:22:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12436#issuecomment-139211",
    "user": "https://github.com/nexttime"
}
```

Changed dependencies from **12435** to **#12681**



---

archive/issue_comments_139212.json:
```json
{
    "body": "Work Issues: **Create a new spkg.**",
    "created_at": "2012-03-22T12:33:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12436#issuecomment-139212",
    "user": "https://github.com/nexttime"
}
```

Work Issues: **Create a new spkg.**



---

archive/issue_comments_139213.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nReplying to [@nexttime](#comment%3A3):\n> Wonder whether we should submit it upstream; I personally would (currently) only apply the patch if `$CC` is *not* GCC.\n\nBy now, ... if **`$CXX`** is not GCC's C++ compiler.\n\n(We shouldn't literally check `$CXX` for `g++`, but instead do something like `if ! $CXX --version | grep g++ >/dev/null; then # apply patch`.  Apparently the output depends of the filename of the executable btw., which shouldn't hurt since applying the patch is ok for `g++` as well; looking for \"GCC\" in contrast could give false positives, since some other compilers might report with what they've been built, too.)",
    "created_at": "2012-03-22T12:57:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12436#issuecomment-139213",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:5'>Comment 5:</a>
Replying to [@nexttime](#comment%3A3):
> Wonder whether we should submit it upstream; I personally would (currently) only apply the patch if `$CC` is *not* GCC.

By now, ... if **`$CXX`** is not GCC's C++ compiler.

(We shouldn't literally check `$CXX` for `g++`, but instead do something like `if ! $CXX --version | grep g++ >/dev/null; then # apply patch`.  Apparently the output depends of the filename of the executable btw., which shouldn't hurt since applying the patch is ok for `g++` as well; looking for "GCC" in contrast could give false positives, since some other compilers might report with what they've been built, too.)



---

archive/issue_comments_139214.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nReplying to [@nexttime](#comment%3A3):\n> Wonder whether we should submit it upstream; I personally would (currently) only apply the patch if `$CC` is *not* GCC.\n\nI haven't really pushed this ticket (which I think is important) since it is significant enough change to report upstream, which I have yet to get around to.\n\nI also think that the makefile patch you've made should be submitted to upstream.",
    "created_at": "2012-03-22T19:13:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12436#issuecomment-139214",
    "user": "https://github.com/ohanar"
}
```

<a id='comment:6'>Comment 6:</a>
Replying to [@nexttime](#comment%3A3):
> Wonder whether we should submit it upstream; I personally would (currently) only apply the patch if `$CC` is *not* GCC.

I haven't really pushed this ticket (which I think is important) since it is significant enough change to report upstream, which I have yet to get around to.

I also think that the makefile patch you've made should be submitted to upstream.



---

archive/issue_events_155407.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12436#event-155407"
}
```



---

archive/issue_events_155408.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12436#event-155408"
}
```



---

archive/issue_events_155409.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12436#event-155409"
}
```



---

archive/issue_events_155410.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12436#event-155410"
}
```



---

archive/issue_events_155411.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-02-04T08:00:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12436#event-155411"
}
```



---

archive/issue_events_155412.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-02-04T08:00:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12436#event-155412"
}
```



---

archive/issue_events_155413.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12436#event-155413"
}
```



---

archive/issue_events_155414.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12436#event-155414"
}
```



---

archive/issue_comments_139215.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nOk, so I've now created 7 new patches to fix all remaining clang/non-GNU/C++11 issues with Lcalc (except for some deprecation warnings):\n\n```\nMakefile_2-dont_compile_C_with_CXXFLAGS.patch\nlcalc-1.23-fix_VLA_1.patch\nlcalc-1.23-fix_VLA_2.patch\nlcalc-1.23-fix_control_reaches_end_of_non-void_fn.patch\nlcalc-1.23-fix_typeof.patch\nlcalc-1.23-missing_return_type.patch\nlcalc-1.23_default_parameters_2.patch\n```\n\nBut I'd prefer to put them on a single ticket, i.e., to either open a new one, or repurpose one of the two existing ones.  (But before doing so, I'll anyway polish them a bit, and probably merge some of them into another or into one of the existing ones.)\n\nI did take a slightly different approach to variable-length arrays; instead of using `std::vector<>`, I'm using `foo_t *_foo = new foo_t[dim1*dim2*dim3]`, along with a macro `foo(i,j,k)` which maps the indices to a single one, and of course added `delete [] _foo` where necessary.\n\nSo far tested with GCC 4.4, 4.8 and 4.9, clang 3.4.1, and a couple of `-std=...` variations.",
    "created_at": "2014-05-19T01:06:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12436#issuecomment-139215",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:11'>Comment 11:</a>
Ok, so I've now created 7 new patches to fix all remaining clang/non-GNU/C++11 issues with Lcalc (except for some deprecation warnings):

```
Makefile_2-dont_compile_C_with_CXXFLAGS.patch
lcalc-1.23-fix_VLA_1.patch
lcalc-1.23-fix_VLA_2.patch
lcalc-1.23-fix_control_reaches_end_of_non-void_fn.patch
lcalc-1.23-fix_typeof.patch
lcalc-1.23-missing_return_type.patch
lcalc-1.23_default_parameters_2.patch
```

But I'd prefer to put them on a single ticket, i.e., to either open a new one, or repurpose one of the two existing ones.  (But before doing so, I'll anyway polish them a bit, and probably merge some of them into another or into one of the existing ones.)

I did take a slightly different approach to variable-length arrays; instead of using `std::vector<>`, I'm using `foo_t *_foo = new foo_t[dim1*dim2*dim3]`, along with a macro `foo(i,j,k)` which maps the indices to a single one, and of course added `delete [] _foo` where necessary.

So far tested with GCC 4.4, 4.8 and 4.9, clang 3.4.1, and a couple of `-std=...` variations.



---

archive/issue_events_155415.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12436#event-155415"
}
```



---

archive/issue_events_155416.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12436#event-155416"
}
```



---

archive/issue_comments_139216.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nWe should probably fix the remaining issues (*including VLAs*) on #12437, and close this ticket as a duplicate.\n\nOr fix VLAs here, and let #12437 depend on this ticket.",
    "created_at": "2015-05-19T13:55:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12436#issuecomment-139216",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:13'>Comment 13:</a>
We should probably fix the remaining issues (*including VLAs*) on #12437, and close this ticket as a duplicate.

Or fix VLAs here, and let #12437 depend on this ticket.



---

archive/issue_comments_139217.json:
```json
{
    "body": "Changed work issues from **Create a new spkg.** to none",
    "created_at": "2016-10-06T01:41:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12436#issuecomment-139217",
    "user": "https://github.com/kiwifb"
}
```

Changed work issues from **Create a new spkg.** to none



---

archive/issue_events_155417.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2016-10-06T01:41:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12436#event-155417"
}
```



---

archive/issue_events_155418.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2016-10-06T01:41:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12436#event-155418"
}
```



---

archive/issue_comments_139218.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nObsolete and all useful content used in #12437.",
    "created_at": "2016-10-06T01:41:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12436#issuecomment-139218",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:14'>Comment 14:</a>
Obsolete and all useful content used in #12437.



---

archive/issue_events_155419.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2016-10-06T01:41:38Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12436#event-155419"
}
```



---

archive/issue_events_155420.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2016-10-06T01:41:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12436#event-155420"
}
```



---

archive/issue_events_155421.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2016-10-06T01:41:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12436#event-155421"
}
```



---

archive/issue_comments_139219.json:
```json
{
    "body": "Reviewer: **Fran\u00e7ois Bissey**",
    "created_at": "2016-10-06T01:41:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12436#issuecomment-139219",
    "user": "https://github.com/kiwifb"
}
```

Reviewer: **François Bissey**



---

archive/issue_events_155422.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-01-21T18:03:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12436#event-155422"
}
```



---

archive/issue_events_155423.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-01-21T18:03:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12436",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12436#event-155423"
}
```
