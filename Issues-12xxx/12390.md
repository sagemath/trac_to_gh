# Issue 12390: the documentation for number_of_partitions mentions a bug in PARI that was fixed long, long ago.  Delete this.

archive/issues_012218.json:
```json
{
    "assignees": [],
    "body": "the documentation says \"But PARI has a bug, e.g., on 64-bit Linux PARI-2.3.2 outputs numbpart(147007)%1000 as 536 when it should be 533!. So do not use this option.\"\n\nAssignee: **@sagetrac-mvngu**\n\nAuthor: **William Stein**\n\nReviewer: **David Roe**\n\nMerged: **sage-5.0.beta2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12390_\n\n",
    "closed_at": "2012-02-02T12:54:46Z",
    "created_at": "2012-01-30T22:01:34Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "the documentation for number_of_partitions mentions a bug in PARI that was fixed long, long ago.  Delete this.",
    "type": "issue",
    "updated_at": "2012-02-02T12:54:46Z",
    "url": "https://github.com/sagemath/sage/issues/12390",
    "user": "https://github.com/williamstein"
}
```
the documentation says "But PARI has a bug, e.g., on 64-bit Linux PARI-2.3.2 outputs numbpart(147007)%1000 as 536 when it should be 533!. So do not use this option."

Assignee: **@sagetrac-mvngu**

Author: **William Stein**

Reviewer: **David Roe**

Merged: **sage-5.0.beta2**

_Issue created by migration from https://trac.sagemath.org/ticket/12390_





---

archive/issue_events_154632.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-01-30T22:01:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12390",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12390#event-154632"
}
```



---

archive/issue_events_154633.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-01-30T22:01:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12390",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12390#event-154633"
}
```



---

archive/issue_events_154634.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-01-30T22:01:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12390",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12390#event-154634"
}
```



---

archive/issue_comments_137647.json:
```json
{
    "body": "Attachment: **[trac_12390.patch.gz](https://github.com/sagemath/sage/files/ticket12390/trac_12390.patch.gz)**",
    "created_at": "2012-01-30T22:16:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12390#issuecomment-137647",
    "user": "https://github.com/williamstein"
}
```

Attachment: **[trac_12390.patch.gz](https://github.com/sagemath/sage/files/ticket12390/trac_12390.patch.gz)**



---

archive/issue_events_154635.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-01-30T22:16:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12390",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12390#event-154635"
}
```



---

archive/issue_comments_137648.json:
```json
{
    "body": "Author: **wstein**",
    "created_at": "2012-01-30T22:16:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12390#issuecomment-137648",
    "user": "https://github.com/williamstein"
}
```

Author: **wstein**



---

archive/issue_events_154636.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2012-01-30T22:16:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12390",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12390#event-154636"
}
```



---

archive/issue_events_154637.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2012-01-30T22:16:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12390",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12390#event-154637"
}
```



---

archive/issue_comments_137649.json:
```json
{
    "body": "Reviewer: **David Roe**",
    "created_at": "2012-01-30T22:17:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12390#issuecomment-137649",
    "user": "https://github.com/roed314"
}
```

Reviewer: **David Roe**



---

archive/issue_comments_137650.json:
```json
{
    "body": "Changed author from **wstein** to **William Stein**",
    "created_at": "2012-01-31T09:31:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12390#issuecomment-137650",
    "user": "https://github.com/jdemeyer"
}
```

Changed author from **wstein** to **William Stein**



---

archive/issue_events_154638.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-01-31T09:31:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12390",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12390#event-154638"
}
```



---

archive/issue_events_154639.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-02T12:54:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12390",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12390#event-154639"
}
```



---

archive/issue_events_154640.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-02T12:54:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12390",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12390#event-154640"
}
```



---

archive/issue_comments_137651.json:
```json
{
    "body": "Merged: **sage-5.0.beta2**",
    "created_at": "2012-02-02T12:54:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12390#issuecomment-137651",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.0.beta2**
