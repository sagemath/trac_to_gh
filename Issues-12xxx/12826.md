# Issue 12826: minpoly() of quadratic number field element ignores var argument

archive/issues_012654.json:
```json
{
    "assignees": [],
    "body": "In the following example the optional parameter `var` is not honored.\n\n```\nsage: k=QQ\nsage: R.<T> = k[]\nsage: K.<y> = k.extension(T^2-2)\nsage: y.minpoly('T')\nx^2-2\n```\n\n(in sage-5.0-beta4)\n\nAssignee: **@loefflerd**\n\nReviewer: **Francis Clarke**\n\nAuthor: **Julian Rueth**\n\nMerged: **sage-5.1.beta0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12826_\n\n",
    "closed_at": "2012-05-06T12:17:53Z",
    "created_at": "2012-04-10T13:10:32Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
        "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/good%20first%20issue"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "minpoly() of quadratic number field element ignores var argument",
    "type": "issue",
    "updated_at": "2012-05-06T12:17:53Z",
    "url": "https://github.com/sagemath/sage/issues/12826",
    "user": "https://github.com/saraedum"
}
```
In the following example the optional parameter `var` is not honored.

```
sage: k=QQ
sage: R.<T> = k[]
sage: K.<y> = k.extension(T^2-2)
sage: y.minpoly('T')
x^2-2
```

(in sage-5.0-beta4)

Assignee: **@loefflerd**

Reviewer: **Francis Clarke**

Author: **Julian Rueth**

Merged: **sage-5.1.beta0**

_Issue created by migration from https://trac.sagemath.org/ticket/12826_





---

archive/issue_events_161961.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2012-04-10T13:10:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12826",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12826#event-161961"
}
```



---

archive/issue_events_161962.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2012-04-10T13:10:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12826",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
    "label_color": "0000ff",
    "label_name": "component: number fields",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12826#event-161962"
}
```



---

archive/issue_events_161963.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2012-04-10T13:10:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12826",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12826#event-161963"
}
```



---

archive/issue_events_161964.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2012-04-10T13:10:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12826",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12826#event-161964"
}
```



---

archive/issue_events_161965.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2012-04-10T13:18:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12826",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12826#event-161965"
}
```



---

archive/issue_comments_146599.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nMaybe I should add a doctest.",
    "created_at": "2012-04-10T13:18:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12826",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12826#issuecomment-146599",
    "user": "https://github.com/saraedum"
}
```

<div id="comment:2" align="right">Comment 2</div>

Maybe I should add a doctest.



---

archive/issue_events_161966.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2012-04-10T13:18:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12826",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12826#event-161966"
}
```



---

archive/issue_events_161967.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2012-04-10T13:18:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12826",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12826#event-161967"
}
```



---

archive/issue_comments_146600.json:
```json
{
    "body": "Attachment: **[trac_12826.patch.gz](https://github.com/sagemath/sage/files/ticket12826/trac_12826.patch.gz)**",
    "created_at": "2012-04-10T13:25:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12826",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12826#issuecomment-146600",
    "user": "https://github.com/saraedum"
}
```

Attachment: **[trac_12826.patch.gz](https://github.com/sagemath/sage/files/ticket12826/trac_12826.patch.gz)**



---

archive/issue_comments_146601.json:
```json
{
    "body": "Author: **Julian Rueth**",
    "created_at": "2012-04-10T13:25:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12826",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12826#issuecomment-146601",
    "user": "https://github.com/saraedum"
}
```

Author: **Julian Rueth**



---

archive/issue_events_161968.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2012-04-10T13:25:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12826",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12826#event-161968"
}
```



---

archive/issue_events_161969.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2012-04-10T13:25:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12826",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12826#event-161969"
}
```



---

archive/issue_events_161970.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2012-04-10T15:02:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12826",
    "label": "https://github.com/sagemath/sage/labels/good%20first%20issue",
    "label_color": "7057ff",
    "label_name": "good first issue",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12826#event-161970"
}
```



---

archive/issue_comments_146602.json:
```json
{
    "body": "Reviewer: **Francis Clarke**",
    "created_at": "2012-04-12T14:31:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12826",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12826#issuecomment-146602",
    "user": "https://github.com/sagetrac-fwclarke"
}
```

Reviewer: **Francis Clarke**



---

archive/issue_events_161971.json:
```json
{
    "actor": "https://github.com/sagetrac-fwclarke",
    "created_at": "2012-04-12T14:31:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12826",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12826#event-161971"
}
```



---

archive/issue_events_161972.json:
```json
{
    "actor": "https://github.com/sagetrac-fwclarke",
    "created_at": "2012-04-12T14:31:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12826",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12826#event-161972"
}
```



---

archive/issue_comments_146603.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nDoes the trick.  Positive review.",
    "created_at": "2012-04-12T14:31:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12826",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12826#issuecomment-146603",
    "user": "https://github.com/sagetrac-fwclarke"
}
```

<div id="comment:5" align="right">Comment 5</div>

Does the trick.  Positive review.



---

archive/issue_events_161973.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-04-19T09:45:14Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12826",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12826#event-161973"
}
```



---

archive/issue_events_161974.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-04-19T09:45:14Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12826",
    "milestone_number": null,
    "milestone_title": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12826#event-161974"
}
```



---

archive/issue_events_161975.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-06T12:17:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12826",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12826#event-161975"
}
```



---

archive/issue_events_161976.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-06T12:17:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12826",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12826#event-161976"
}
```



---

archive/issue_comments_146604.json:
```json
{
    "body": "Merged: **sage-5.1.beta0**",
    "created_at": "2012-05-06T12:17:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12826",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12826#issuecomment-146604",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.1.beta0**
