# Issue 12317: printing factorisations

archive/issues_012145.json:
```json
{
    "assignees": [],
    "body": "This is clearly wrong:\n\n```\nsage: P.<t> = QQ[]\nsage: (1/(t^2 + 6*t + 9)).factor()\nt + 3^-2\n```\n\n\nAssignee: **@aghitza**\n\nKeywords: **Factorization, printing**\n\nAuthor: **Francis Clarke**\n\nReviewer: **Robert Bradshaw**\n\nMerged: **sage-5.0.beta3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12317_\n\n",
    "closed_at": "2012-02-06T21:26:11Z",
    "created_at": "2012-01-17T10:45:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "printing factorisations",
    "type": "issue",
    "updated_at": "2012-02-06T21:26:11Z",
    "url": "https://github.com/sagemath/sage/issues/12317",
    "user": "https://github.com/sagetrac-fwclarke"
}
```
This is clearly wrong:

```
sage: P.<t> = QQ[]
sage: (1/(t^2 + 6*t + 9)).factor()
t + 3^-2
```


Assignee: **@aghitza**

Keywords: **Factorization, printing**

Author: **Francis Clarke**

Reviewer: **Robert Bradshaw**

Merged: **sage-5.0.beta3**

_Issue created by migration from https://trac.sagemath.org/ticket/12317_





---

archive/issue_events_153505.json:
```json
{
    "actor": "https://github.com/sagetrac-fwclarke",
    "created_at": "2012-01-17T10:45:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12317",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12317#event-153505"
}
```



---

archive/issue_events_153506.json:
```json
{
    "actor": "https://github.com/sagetrac-fwclarke",
    "created_at": "2012-01-17T10:45:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12317",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12317#event-153506"
}
```



---

archive/issue_events_153507.json:
```json
{
    "actor": "https://github.com/sagetrac-fwclarke",
    "created_at": "2012-01-17T10:45:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12317",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12317#event-153507"
}
```



---

archive/issue_events_153508.json:
```json
{
    "actor": "https://github.com/sagetrac-fwclarke",
    "created_at": "2012-01-17T10:55:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12317",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12317#event-153508"
}
```



---

archive/issue_comments_136211.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[trac_12317_factorization_printing_fix.patch.gz](https://github.com/sagemath/sage/files/ticket12317/trac_12317_factorization_printing_fix.patch.gz)**\n\nThe defect (which applies only to a single negative power of a \"prime\") is very simple to fix, after which\n\n```\nsage: P.<t> = QQ[]\nsage: (1/(t^2 + 6*t + 9)).factor()\n(t + 3)^-2\n```",
    "created_at": "2012-01-17T10:55:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12317",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12317#issuecomment-136211",
    "user": "https://github.com/sagetrac-fwclarke"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[trac_12317_factorization_printing_fix.patch.gz](https://github.com/sagemath/sage/files/ticket12317/trac_12317_factorization_printing_fix.patch.gz)**

The defect (which applies only to a single negative power of a "prime") is very simple to fix, after which

```
sage: P.<t> = QQ[]
sage: (1/(t^2 + 6*t + 9)).factor()
(t + 3)^-2
```



---

archive/issue_events_153509.json:
```json
{
    "actor": "https://github.com/sagetrac-fwclarke",
    "created_at": "2012-01-17T10:55:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12317",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12317#event-153509"
}
```



---

archive/issue_comments_136212.json:
```json
{
    "body": "Author: **Francis Clarke**",
    "created_at": "2012-01-17T10:55:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12317",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12317#issuecomment-136212",
    "user": "https://github.com/sagetrac-fwclarke"
}
```

Author: **Francis Clarke**



---

archive/issue_comments_136213.json:
```json
{
    "body": "Reviewer: **Robert Bradshaw**",
    "created_at": "2012-01-31T06:46:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12317",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12317#issuecomment-136213",
    "user": "https://github.com/robertwb"
}
```

Reviewer: **Robert Bradshaw**



---

archive/issue_events_153510.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2012-01-31T06:46:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12317",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12317#event-153510"
}
```



---

archive/issue_events_153511.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2012-01-31T06:46:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12317",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12317#event-153511"
}
```



---

archive/issue_comments_136214.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNice catch.",
    "created_at": "2012-01-31T06:46:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12317",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12317#issuecomment-136214",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:2'>Comment 2:</a>
Nice catch.



---

archive/issue_events_153512.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-06T21:26:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12317",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12317#event-153512"
}
```



---

archive/issue_events_153513.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-06T21:26:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12317",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12317#event-153513"
}
```



---

archive/issue_comments_136215.json:
```json
{
    "body": "Merged: **sage-5.0.beta3**",
    "created_at": "2012-02-06T21:26:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12317",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12317#issuecomment-136215",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.0.beta3**
