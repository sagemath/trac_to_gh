# Issue 12185: Bug in norm for orders of relative number fields

archive/issues_012013.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: K.<a,b> = NumberField([x^2 - 2, x^3 - 2])\nsage: OK = K.maximal_order()\nsage: OK(b).norm()\n...\nAttributeError: 'RelativeOrder' object has no attribute 'pari_polynomial'\n```\n\nDuplicate, fixed by #11904.\n\nAssignee: **@loefflerd**\n\nKeywords: **sd35**\n\nReviewer: **David Loeffler**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12185_\n\n",
    "closed_at": "2012-01-05T13:41:11Z",
    "created_at": "2011-12-18T19:53:45Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Bug in norm for orders of relative number fields",
    "type": "issue",
    "updated_at": "2012-01-05T13:41:11Z",
    "url": "https://github.com/sagemath/sage/issues/12185",
    "user": "https://github.com/sagetrac-johanbosman"
}
```

```
sage: K.<a,b> = NumberField([x^2 - 2, x^3 - 2])
sage: OK = K.maximal_order()
sage: OK(b).norm()
...
AttributeError: 'RelativeOrder' object has no attribute 'pari_polynomial'
```

Duplicate, fixed by #11904.

Assignee: **@loefflerd**

Keywords: **sd35**

Reviewer: **David Loeffler**

_Issue created by migration from https://trac.sagemath.org/ticket/12185_





---

archive/issue_events_151376.json:
```json
{
    "actor": "https://github.com/sagetrac-johanbosman",
    "created_at": "2011-12-18T19:53:45Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "milestone_number": null,
    "milestone_title": "sage-4.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12185#event-151376"
}
```



---

archive/issue_events_151377.json:
```json
{
    "actor": "https://github.com/sagetrac-johanbosman",
    "created_at": "2011-12-18T19:53:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
    "label_color": "0000ff",
    "label_name": "component: number fields",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12185#event-151377"
}
```



---

archive/issue_events_151378.json:
```json
{
    "actor": "https://github.com/sagetrac-johanbosman",
    "created_at": "2011-12-18T19:53:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12185#event-151378"
}
```



---

archive/issue_events_151379.json:
```json
{
    "actor": "https://github.com/sagetrac-johanbosman",
    "created_at": "2011-12-18T19:53:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12185#event-151379"
}
```



---

archive/issue_comments_133480.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nThe same problem occurs for the norm function of relative orders.",
    "created_at": "2011-12-18T19:56:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12185#issuecomment-133480",
    "user": "https://github.com/sagetrac-mkosters"
}
```

<a id='comment:1'>Comment 1:</a>
The same problem occurs for the norm function of relative orders.



---

archive/issue_comments_133481.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nReplying to [mkosters](#comment%3A1):\n> The same problem occurs for the TRACE function of relative orders.",
    "created_at": "2011-12-18T19:57:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12185#issuecomment-133481",
    "user": "https://github.com/sagetrac-mkosters"
}
```

<a id='comment:2'>Comment 2:</a>
Replying to [mkosters](#comment%3A1):
> The same problem occurs for the TRACE function of relative orders.



---

archive/issue_events_151380.json:
```json
{
    "actor": "https://github.com/sagetrac-johanbosman",
    "created_at": "2011-12-18T20:28:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12185#event-151380"
}
```



---

archive/issue_comments_133482.json:
```json
{
    "body": "Author: **Johan Bosman**",
    "created_at": "2011-12-18T20:28:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12185#issuecomment-133482",
    "user": "https://github.com/sagetrac-johanbosman"
}
```

Author: **Johan Bosman**



---

archive/issue_comments_133483.json:
```json
{
    "body": "Attachment: **[12185.patch.gz](https://github.com/sagemath/sage/files/ticket12185/12185.patch.gz)**\n\nThis patch fixes a similar problem with the trace",
    "created_at": "2011-12-18T22:54:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12185#issuecomment-133483",
    "user": "https://github.com/sagetrac-johanbosman"
}
```

Attachment: **[12185.patch.gz](https://github.com/sagemath/sage/files/ticket12185/12185.patch.gz)**

This patch fixes a similar problem with the trace



---

archive/issue_comments_133484.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nWait 1 second, maybe I've got a better fix.",
    "created_at": "2011-12-19T09:39:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12185#issuecomment-133484",
    "user": "https://github.com/sagetrac-johanbosman"
}
```

<a id='comment:4'>Comment 4:</a>
Wait 1 second, maybe I've got a better fix.



---

archive/issue_events_151381.json:
```json
{
    "actor": "https://github.com/sagetrac-johanbosman",
    "created_at": "2011-12-19T09:39:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12185#event-151381"
}
```



---

archive/issue_events_151382.json:
```json
{
    "actor": "https://github.com/sagetrac-johanbosman",
    "created_at": "2011-12-19T09:39:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12185#event-151382"
}
```



---

archive/issue_comments_133485.json:
```json
{
    "body": "Attachment: **[12185.2.patch.gz](https://github.com/sagemath/sage/files/ticket12185/12185.2.patch.gz)**",
    "created_at": "2011-12-19T10:01:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12185#issuecomment-133485",
    "user": "https://github.com/sagetrac-johanbosman"
}
```

Attachment: **[12185.2.patch.gz](https://github.com/sagemath/sage/files/ticket12185/12185.2.patch.gz)**



---

archive/issue_events_151383.json:
```json
{
    "actor": "https://github.com/sagetrac-johanbosman",
    "created_at": "2011-12-19T10:01:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12185#event-151383"
}
```



---

archive/issue_events_151384.json:
```json
{
    "actor": "https://github.com/sagetrac-johanbosman",
    "created_at": "2011-12-19T10:01:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12185#event-151384"
}
```



---

archive/issue_comments_133486.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -6,3 +6,4 @@\n ...\n AttributeError: 'RelativeOrder' object has no attribute 'pari_polynomial'\n ```\n+Apply [attachment: 12185.2.patch.gz](https://github.com/sagemath/sage/files/ticket12185/12185.2.patch.gz).\n``````\n",
    "created_at": "2011-12-19T10:01:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12185#issuecomment-133486",
    "user": "https://github.com/sagetrac-johanbosman"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -6,3 +6,4 @@
 ...
 AttributeError: 'RelativeOrder' object has no attribute 'pari_polynomial'
 ```
+Apply [attachment: 12185.2.patch.gz](https://github.com/sagemath/sage/files/ticket12185/12185.2.patch.gz).
``````




---

archive/issue_comments_133487.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -6,4 +6,4 @@\n ...\n AttributeError: 'RelativeOrder' object has no attribute 'pari_polynomial'\n ```\n-Apply [attachment: 12185.2.patch.gz](https://github.com/sagemath/sage/files/ticket12185/12185.2.patch.gz).\n+Apply [attachment: 12185.2.patch.gz](https://github.com/sagemath/sage/files/ticket12185/12185.2.patch.gz).  This patch applies to 4.8.alpha4.\n``````\n",
    "created_at": "2011-12-19T10:19:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12185#issuecomment-133487",
    "user": "https://github.com/sagetrac-johanbosman"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -6,4 +6,4 @@
 ...
 AttributeError: 'RelativeOrder' object has no attribute 'pari_polynomial'
 ```
-Apply [attachment: 12185.2.patch.gz](https://github.com/sagemath/sage/files/ticket12185/12185.2.patch.gz).
+Apply [attachment: 12185.2.patch.gz](https://github.com/sagemath/sage/files/ticket12185/12185.2.patch.gz).  This patch applies to 4.8.alpha4.
``````




---

archive/issue_events_151385.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2011-12-31T14:23:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12185#event-151385"
}
```



---

archive/issue_events_151386.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2011-12-31T14:23:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12185#event-151386"
}
```



---

archive/issue_events_151387.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2011-12-31T14:23:28Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "milestone_number": null,
    "milestone_title": "sage-4.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12185#event-151387"
}
```



---

archive/issue_events_151388.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2011-12-31T14:23:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12185#event-151388"
}
```



---

archive/issue_events_151389.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2011-12-31T14:23:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12185#event-151389"
}
```



---

archive/issue_comments_133488.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nJohan, the examples here work in 4.8.alpha5 without applying any patches! Apparently the issue has been fixed by some other patch merged in alpha5 (most likely #11904).",
    "created_at": "2011-12-31T14:23:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12185#issuecomment-133488",
    "user": "https://github.com/loefflerd"
}
```

<a id='comment:7'>Comment 7:</a>
Johan, the examples here work in 4.8.alpha5 without applying any patches! Apparently the issue has been fixed by some other patch merged in alpha5 (most likely #11904).



---

archive/issue_comments_133489.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nI confirm that the examples work in 4.8.alpha4 + #11904.",
    "created_at": "2012-01-05T11:33:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12185#issuecomment-133489",
    "user": "https://github.com/mstreng"
}
```

<a id='comment:8'>Comment 8:</a>
I confirm that the examples work in 4.8.alpha4 + #11904.



---

archive/issue_comments_133490.json:
```json
{
    "body": "Reviewer: **David Loeffler**",
    "created_at": "2012-01-05T11:33:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12185#issuecomment-133490",
    "user": "https://github.com/mstreng"
}
```

Reviewer: **David Loeffler**



---

archive/issue_comments_133491.json:
```json
{
    "body": "Changed keywords from **** to **sd35**",
    "created_at": "2012-01-05T11:33:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12185#issuecomment-133491",
    "user": "https://github.com/mstreng"
}
```

Changed keywords from **** to **sd35**



---

archive/issue_comments_133492.json:
```json
{
    "body": "Changed author from **Johan Bosman** to none",
    "created_at": "2012-01-05T11:54:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12185#issuecomment-133492",
    "user": "https://github.com/loefflerd"
}
```

Changed author from **Johan Bosman** to none



---

archive/issue_comments_133493.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -6,4 +6,5 @@\n ...\n AttributeError: 'RelativeOrder' object has no attribute 'pari_polynomial'\n ```\n-Apply [attachment: 12185.2.patch.gz](https://github.com/sagemath/sage/files/ticket12185/12185.2.patch.gz).  This patch applies to 4.8.alpha4.\n+\n+**Now fixed, please close as duplicate.**\n``````\n",
    "created_at": "2012-01-05T11:54:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12185#issuecomment-133493",
    "user": "https://github.com/loefflerd"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -6,4 +6,5 @@
 ...
 AttributeError: 'RelativeOrder' object has no attribute 'pari_polynomial'
 ```
-Apply [attachment: 12185.2.patch.gz](https://github.com/sagemath/sage/files/ticket12185/12185.2.patch.gz).  This patch applies to 4.8.alpha4.
+
+**Now fixed, please close as duplicate.**
``````




---

archive/issue_events_151390.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2012-01-05T11:54:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12185#event-151390"
}
```



---

archive/issue_events_151391.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2012-01-05T11:54:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12185#event-151391"
}
```



---

archive/issue_events_151392.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2012-01-05T11:54:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12185#event-151392"
}
```



---

archive/issue_events_151393.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2012-01-05T11:54:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12185#event-151393"
}
```



---

archive/issue_comments_133494.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nCool, my patch #11904 fixed a bug by accident.",
    "created_at": "2012-01-05T13:41:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12185#issuecomment-133494",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:11'>Comment 11:</a>
Cool, my patch #11904 fixed a bug by accident.



---

archive/issue_comments_133495.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -7,4 +7,4 @@\n AttributeError: 'RelativeOrder' object has no attribute 'pari_polynomial'\n ```\n \n-**Now fixed, please close as duplicate.**\n+Duplicate, fixed by #11904.\n``````\n",
    "created_at": "2012-01-05T13:41:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12185#issuecomment-133495",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -7,4 +7,4 @@
 AttributeError: 'RelativeOrder' object has no attribute 'pari_polynomial'
 ```
 
-**Now fixed, please close as duplicate.**
+Duplicate, fixed by #11904.
``````




---

archive/issue_events_151394.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-01-05T13:41:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12185#event-151394"
}
```



---

archive/issue_events_151395.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-01-05T13:41:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12185#event-151395"
}
```



---

archive/issue_events_151396.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-01-05T13:41:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12185",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12185#event-151396"
}
```
