...
```json
{
    "body": "> If you agree with these proposition, I will prepare a rev-rev patch ;-)\n\n\n+1 `:-)`\n\nNathann",
...
```json
{
    "body": "I have incorporated all proposed modification and updated some tests (mainly adding set_random_seed(0)).\n\nLet me know if it is better now.\n\nBest,\n\nD.",
...
```json
{
    "body": "oups sorry, I see some \"trailing spaces\" in the file :(",
...
```json
{
    "body": "That's much better ! For me the patch is ready to do :))\n\nD.",
...
```json
{
    "body": "Updated ! It must be because of some numerical noise on that machine. Actually, this doctest breaks when the CBC package is installed too, for the same reason. Let it be fixed with the updated version of ``trac_12362-edge_disjoint_spanning_tree.patch``.\n\nNathann",
...
```json
{
    "body": "OK, so let me know if I have something to do.\n\nBest,\n\nDavid.",
...
```json
{
    "body": "Yeah, something must have changed in networkx, but the last update seems to be 1.2 a veeeeeeery long time ago. Where the hell does this come from ? `O_o`",
