# Issue 12860: Incorrect computation of maximal orders in quaternion algebras

archive/issues_012688.json:
```json
{
    "assignees": [],
    "body": "The maximal_order function of QuaternionOrder can produce incorrect results.\nIt is implemented by means of an explicit formula, which expects not just the algebra to be ramified precisely at one finite prime p and infinity, but also expect the invariants a=i<sup>2</sup> and b=j<sup>2</sup> to have a special form. Not all the necessary conditions are checked by the code.\n\nE.g. the following is obviously not an order since it contains non-integral elements:\n\n```\nsage: A.<i,j,k> = QuaternionAlgebra(17)\nsage: print A.invariants()\n(-3, -17)\nsage: R = A.maximal_order()\nsage: b = R.basis()\nsage: print b\n(1/2 + 1/2*j, 1/2*i + 1/2*k, -1/3*j - 1/3*k, k)\nsage: b[0]*b[1]\n9/2*i\nsage: (b[0]*b[1]).reduced_norm() \n243/4\n```\n\nBut this is ok, because now the invariants are as in Pizer's paper:\n\n```\nsage: A.<i,j,k> = QuaternionAlgebra(-17,-3)\nsage: print A.maximal_order().basis()\n(1/2 + 1/2*j, 1/2*i + 1/2*k, -1/3*j - 1/3*k, k)\n```\n\nThis could be fixed by simply swapping (i,j) in the formula depending on the invariants, but the invariants can deviate from the required form in more than one way, again possibly resulting in failure of the formula:\n\n```\nsage: A.<i,j,k> = QuaternionAlgebra(-17*9,-3)\nsage: print A.maximal_order().basis()\n(1/2 + 1/2*j, 1/2*i + 1/2*k, -1/3*j - 1/3*k, k)\nsage: print (-1/3*j - 1/3*k).reduced_norm()\n154/3\n```\n\n(#11464 is related)\n\nAssignee: **@dansme**\n\nReviewer: **Aly Deines**\n\nAuthor: **Daniel Smertnig**\n\nMerged: **sage-5.3.beta0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12860_\n\n",
    "closed_at": "2012-08-01T12:11:19Z",
    "created_at": "2012-04-19T20:39:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Incorrect computation of maximal orders in quaternion algebras",
    "type": "issue",
    "updated_at": "2012-08-01T12:11:19Z",
    "url": "https://github.com/sagemath/sage/issues/12860",
    "user": "https://github.com/dansme"
}
```
The maximal_order function of QuaternionOrder can produce incorrect results.
It is implemented by means of an explicit formula, which expects not just the algebra to be ramified precisely at one finite prime p and infinity, but also expect the invariants a=i<sup>2</sup> and b=j<sup>2</sup> to have a special form. Not all the necessary conditions are checked by the code.

E.g. the following is obviously not an order since it contains non-integral elements:

```
sage: A.<i,j,k> = QuaternionAlgebra(17)
sage: print A.invariants()
(-3, -17)
sage: R = A.maximal_order()
sage: b = R.basis()
sage: print b
(1/2 + 1/2*j, 1/2*i + 1/2*k, -1/3*j - 1/3*k, k)
sage: b[0]*b[1]
9/2*i
sage: (b[0]*b[1]).reduced_norm() 
243/4
```

But this is ok, because now the invariants are as in Pizer's paper:

```
sage: A.<i,j,k> = QuaternionAlgebra(-17,-3)
sage: print A.maximal_order().basis()
(1/2 + 1/2*j, 1/2*i + 1/2*k, -1/3*j - 1/3*k, k)
```

This could be fixed by simply swapping (i,j) in the formula depending on the invariants, but the invariants can deviate from the required form in more than one way, again possibly resulting in failure of the formula:

```
sage: A.<i,j,k> = QuaternionAlgebra(-17*9,-3)
sage: print A.maximal_order().basis()
(1/2 + 1/2*j, 1/2*i + 1/2*k, -1/3*j - 1/3*k, k)
sage: print (-1/3*j - 1/3*k).reduced_norm()
154/3
```

(#11464 is related)

Assignee: **@dansme**

Reviewer: **Aly Deines**

Author: **Daniel Smertnig**

Merged: **sage-5.3.beta0**

_Issue created by migration from https://trac.sagemath.org/ticket/12860_





---

archive/issue_events_156899.json:
```json
{
    "actor": "https://github.com/dansme",
    "created_at": "2012-04-19T20:39:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12860",
    "milestone_number": null,
    "milestone_title": "sage-5.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12860#event-156899"
}
```



---

archive/issue_events_156900.json:
```json
{
    "actor": "https://github.com/dansme",
    "created_at": "2012-04-19T20:39:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12860",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12860#event-156900"
}
```



---

archive/issue_events_156901.json:
```json
{
    "actor": "https://github.com/dansme",
    "created_at": "2012-04-19T20:39:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12860",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12860#event-156901"
}
```



---

archive/issue_events_156902.json:
```json
{
    "actor": "https://github.com/dansme",
    "created_at": "2012-04-19T20:39:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12860",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12860#event-156902"
}
```



---

archive/issue_comments_147352.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nThe patch does the following:\n\n* Move `maximal_order` from `brandt.py` to `quaternion_algebra.py`.\n\n* Fix `maximal_order` to raise a NotImplementedError if the invariants are not of the expected form\n\n* Add checks in `*__init__*` of QuaternionOrder to verify if the given basis actually is the basis of an order (this would have catched the bug earlier).\n\n* Fix all the doctests that were expecting the mathematically incorrect results.",
    "created_at": "2012-04-19T21:04:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12860",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12860#issuecomment-147352",
    "user": "https://github.com/dansme"
}
```

<a id='comment:1'>Comment 1:</a>
The patch does the following:

* Move `maximal_order` from `brandt.py` to `quaternion_algebra.py`.

* Fix `maximal_order` to raise a NotImplementedError if the invariants are not of the expected form

* Add checks in `*__init__*` of QuaternionOrder to verify if the given basis actually is the basis of an order (this would have catched the bug earlier).

* Fix all the doctests that were expecting the mathematically incorrect results.



---

archive/issue_comments_147353.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,5 +1,5 @@\n The maximal_order function of QuaternionOrder can produce incorrect results.\n-It is implemented by means of an explicit formula, which expects not just the algebra to be ramified precisely at one finite prime p and infinity, but also expect the invariants a=i^2 and b=j^2 to have a special form. Not all the necessary conditions are checked by the code.\n+It is implemented by means of an explicit formula, which expects not just the algebra to be ramified precisely at one finite prime p and infinity, but also expect the invariants a=i<sup>2</sup> and b=j<sup>2</sup> to have a special form. Not all the necessary conditions are checked by the code.\n \n E.g. the following is obviously not an order since it contains non-integral elements:\n \n@@ -36,5 +36,3 @@\n ```\n \n (#11464 is related)\n-\n-**Patch follows.**\n``````\n",
    "created_at": "2012-04-19T21:04:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12860",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12860#issuecomment-147353",
    "user": "https://github.com/dansme"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,5 +1,5 @@
 The maximal_order function of QuaternionOrder can produce incorrect results.
-It is implemented by means of an explicit formula, which expects not just the algebra to be ramified precisely at one finite prime p and infinity, but also expect the invariants a=i^2 and b=j^2 to have a special form. Not all the necessary conditions are checked by the code.
+It is implemented by means of an explicit formula, which expects not just the algebra to be ramified precisely at one finite prime p and infinity, but also expect the invariants a=i<sup>2</sup> and b=j<sup>2</sup> to have a special form. Not all the necessary conditions are checked by the code.
 
 E.g. the following is obviously not an order since it contains non-integral elements:
 
@@ -36,5 +36,3 @@
 ```
 
 (#11464 is related)
-
-**Patch follows.**
``````




---

archive/issue_comments_147354.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nSee also #12861, for a patch extending this one by a more general algorithm.",
    "created_at": "2012-04-19T22:47:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12860",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12860#issuecomment-147354",
    "user": "https://github.com/dansme"
}
```

<a id='comment:2'>Comment 2:</a>
See also #12861, for a patch extending this one by a more general algorithm.



---

archive/issue_comments_147355.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nNew patch applies to sage-5.0.beta13 (old one conflicted with #12461)",
    "created_at": "2012-04-20T09:54:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12860",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12860#issuecomment-147355",
    "user": "https://github.com/dansme"
}
```

<a id='comment:3'>Comment 3:</a>
New patch applies to sage-5.0.beta13 (old one conflicted with #12461)



---

archive/issue_events_156903.json:
```json
{
    "actor": "https://github.com/adeines",
    "created_at": "2012-07-21T23:15:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12860",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12860#event-156903"
}
```



---

archive/issue_comments_147356.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nAttachment: **[trac_12860_quat_order.patch.gz](https://github.com/sagemath/sage/files/ticket12860/trac_12860_quat_order.patch.gz)**\n\nThis applies to sage-5.2.rc0 and all tests pass.  \n\nThis looks good to me!",
    "created_at": "2012-07-21T23:15:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12860",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12860#issuecomment-147356",
    "user": "https://github.com/adeines"
}
```

<a id='comment:4'>Comment 4:</a>
Attachment: **[trac_12860_quat_order.patch.gz](https://github.com/sagemath/sage/files/ticket12860/trac_12860_quat_order.patch.gz)**

This applies to sage-5.2.rc0 and all tests pass.  

This looks good to me!



---

archive/issue_events_156904.json:
```json
{
    "actor": "https://github.com/adeines",
    "created_at": "2012-07-21T23:16:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12860",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12860#event-156904"
}
```



---

archive/issue_events_156905.json:
```json
{
    "actor": "https://github.com/adeines",
    "created_at": "2012-07-21T23:16:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12860",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12860#event-156905"
}
```



---

archive/issue_comments_147357.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nPlease fill in your real name as Author.",
    "created_at": "2012-07-27T20:40:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12860",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12860#issuecomment-147357",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:6'>Comment 6:</a>
Please fill in your real name as Author.



---

archive/issue_comments_147358.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nPlease fill in your real name as Reviewer.",
    "created_at": "2012-07-27T20:44:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12860",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12860#issuecomment-147358",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:7'>Comment 7:</a>
Please fill in your real name as Reviewer.



---

archive/issue_comments_147359.json:
```json
{
    "body": "Reviewer: **Aly Deines**",
    "created_at": "2012-07-27T21:46:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12860",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12860#issuecomment-147359",
    "user": "https://github.com/adeines"
}
```

Reviewer: **Aly Deines**



---

archive/issue_comments_147360.json:
```json
{
    "body": "Author: **Daniel Smertnig**",
    "created_at": "2012-07-27T21:50:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12860",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12860#issuecomment-147360",
    "user": "https://github.com/dansme"
}
```

Author: **Daniel Smertnig**



---

archive/issue_events_156906.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-08-01T12:11:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12860",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12860#event-156906"
}
```



---

archive/issue_events_156907.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-08-01T12:11:19Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12860",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12860#event-156907"
}
```



---

archive/issue_comments_147361.json:
```json
{
    "body": "Merged: **sage-5.3.beta0**",
    "created_at": "2012-08-01T12:11:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12860",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12860#issuecomment-147361",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.3.beta0**
