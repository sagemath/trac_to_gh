# Issue 12658: Use the matrix displayhook hack for arithmetic subgroup elements

archive/issues_012486.json:
```json
{
    "assignees": [],
    "body": "This patch makes the print representation of lists of elements of SL2Z more legible.\nBefore:\n\n```\nsage: SL2Z.gens()\n([ 0 -1]\n[ 1  0], [1 1]\n[0 1])\n```\nAfter:\n\n```\nsage: SL2Z.gens()\n(\n[ 0 -1]  [1 1]\n[ 1  0], [0 1]\n)\n```\n\nDepends on #11709\n\nAssignee: **@williamstein**\n\nReviewer: **Martin Raum**\n\nAuthor: **David Loeffler**\n\nMerged: **sage-5.0.beta11**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12658_\n\n",
    "closed_at": "2012-03-28T10:05:43Z",
    "created_at": "2012-03-12T22:57:16Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Use the matrix displayhook hack for arithmetic subgroup elements",
    "type": "issue",
    "updated_at": "2012-03-28T10:05:43Z",
    "url": "https://github.com/sagemath/sage/issues/12658",
    "user": "https://github.com/loefflerd"
}
```
This patch makes the print representation of lists of elements of SL2Z more legible.
Before:

```
sage: SL2Z.gens()
([ 0 -1]
[ 1  0], [1 1]
[0 1])
```
After:

```
sage: SL2Z.gens()
(
[ 0 -1]  [1 1]
[ 1  0], [0 1]
)
```

Depends on #11709

Assignee: **@williamstein**

Reviewer: **Martin Raum**

Author: **David Loeffler**

Merged: **sage-5.0.beta11**

_Issue created by migration from https://trac.sagemath.org/ticket/12658_





---

archive/issue_events_153679.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2012-03-12T22:57:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12658",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12658#event-153679"
}
```



---

archive/issue_events_153680.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2012-03-12T22:57:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12658",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20user%20interface",
    "label_color": "000080",
    "label_name": "component: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12658#event-153680"
}
```



---

archive/issue_events_153681.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2012-03-12T22:57:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12658",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12658#event-153681"
}
```



---

archive/issue_events_153682.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2012-03-12T22:59:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12658",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12658#event-153682"
}
```



---

archive/issue_events_153683.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2012-03-12T22:59:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12658",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12658#event-153683"
}
```



---

archive/issue_comments_143386.json:
```json
{
    "body": "Patch against 5.0.beta7 + #11709",
    "created_at": "2012-03-13T00:59:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12658",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12658#issuecomment-143386",
    "user": "https://github.com/loefflerd"
}
```

Patch against 5.0.beta7 + #11709



---

archive/issue_events_153684.json:
```json
{
    "actor": "https://github.com/sagetrac-mraum",
    "created_at": "2012-03-13T11:18:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12658",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12658#event-153684"
}
```



---

archive/issue_events_153685.json:
```json
{
    "actor": "https://github.com/sagetrac-mraum",
    "created_at": "2012-03-13T11:18:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12658",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12658#event-153685"
}
```



---

archive/issue_comments_143387.json:
```json
{
    "body": "Reviewer: **Martin Raum**",
    "created_at": "2012-03-13T11:18:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12658",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12658#issuecomment-143387",
    "user": "https://github.com/sagetrac-mraum"
}
```

Reviewer: **Martin Raum**



---

archive/issue_comments_143388.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nAttachment: **[trac_12658-SL2Z_displayhook.patch.gz](https://github.com/sagemath/sage/files/ticket12658/trac_12658-SL2Z_displayhook.patch.gz)**\n\nThis is a good idea. The patch looks good. All test pass here, and the patch bot on beta7 is also fine with it.",
    "created_at": "2012-03-13T11:18:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12658",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12658#issuecomment-143388",
    "user": "https://github.com/sagetrac-mraum"
}
```

<a id='comment:2'>Comment 2:</a>
Attachment: **[trac_12658-SL2Z_displayhook.patch.gz](https://github.com/sagemath/sage/files/ticket12658/trac_12658-SL2Z_displayhook.patch.gz)**

This is a good idea. The patch looks good. All test pass here, and the patch bot on beta7 is also fine with it.



---

archive/issue_comments_143389.json:
```json
{
    "body": "Merged: **sage-5.0.beta11**",
    "created_at": "2012-03-28T10:05:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12658",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12658#issuecomment-143389",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.0.beta11**



---

archive/issue_events_153686.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-03-28T10:05:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12658",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12658#event-153686"
}
```



---

archive/issue_events_153687.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-03-28T10:05:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12658",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12658#event-153687"
}
```
