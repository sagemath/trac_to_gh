# Issue 12001: performance of dimension of cusp forms for Gamma(N) is insanely slow

archive/issues_011829.json:
```json
{
    "assignees": [],
    "body": "One of the easiest dimension formulas is for the dimension of `S_2(Gamma(p))`.  However, in Sage, computing this dimension is shockingly slow.  For example,\n\n```\nsage: time Gamma(7).dimension_cusp_forms()\n3\nTime: CPU 17.27 s, Wall: 17.30 s\nsage: time dimension_cusp_forms(Gamma(7),2)\n3\nTime: CPU 17.22 s, Wall: 17.25 s\n```\n\nFix this.\n\n\nAssignee: **@craigcitro**\n\nReviewer: **David Loeffler**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12001_\n\n",
    "closed_at": "2012-01-13T08:59:12Z",
    "created_at": "2011-11-07T19:34:33Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20modular%20forms",
        "https://github.com/sagemath/sage/labels/duplicate",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "performance of dimension of cusp forms for Gamma(N) is insanely slow",
    "type": "issue",
    "updated_at": "2012-01-13T08:59:12Z",
    "url": "https://github.com/sagemath/sage/issues/12001",
    "user": "https://github.com/williamstein"
}
```
One of the easiest dimension formulas is for the dimension of `S_2(Gamma(p))`.  However, in Sage, computing this dimension is shockingly slow.  For example,

```
sage: time Gamma(7).dimension_cusp_forms()
3
Time: CPU 17.27 s, Wall: 17.30 s
sage: time dimension_cusp_forms(Gamma(7),2)
3
Time: CPU 17.22 s, Wall: 17.25 s
```

Fix this.


Assignee: **@craigcitro**

Reviewer: **David Loeffler**

_Issue created by migration from https://trac.sagemath.org/ticket/12001_





---

archive/issue_events_148427.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-11-07T19:34:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12001",
    "milestone_number": null,
    "milestone_title": "sage-4.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12001#event-148427"
}
```



---

archive/issue_events_148428.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-11-07T19:34:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12001",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20modular%20forms",
    "label_color": "0000ff",
    "label_name": "component: modular forms",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12001#event-148428"
}
```



---

archive/issue_events_148429.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-11-07T19:34:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12001",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12001#event-148429"
}
```



---

archive/issue_events_148430.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-11-07T19:34:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12001",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12001#event-148430"
}
```



---

archive/issue_comments_129676.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nThis is fixed by #10546. With #10546 installed,\n\n```\nsage: time Gamma(17777).dimension_cusp_forms()\n233722059481\nTime: CPU 0.00 s, Wall: 0.00 s\n```\n\nSo I propose we close this ticket.",
    "created_at": "2011-12-26T11:35:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12001",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12001#issuecomment-129676",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:1" align="right">Comment 1</div>

This is fixed by #10546. With #10546 installed,

```
sage: time Gamma(17777).dimension_cusp_forms()
233722059481
Time: CPU 0.00 s, Wall: 0.00 s
```

So I propose we close this ticket.



---

archive/issue_events_148431.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2011-12-26T11:35:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12001",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12001#event-148431"
}
```



---

archive/issue_comments_129677.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nCorrection: It is #11601, not #10546, that fixes this, by implementing \"nu3\". Anyway, the point is that it's fixed.",
    "created_at": "2011-12-26T17:24:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12001",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12001#issuecomment-129677",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:2" align="right">Comment 2</div>

Correction: It is #11601, not #10546, that fixes this, by implementing "nu3". Anyway, the point is that it's fixed.



---

archive/issue_comments_129678.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nFantastic!",
    "created_at": "2011-12-26T21:13:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12001",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12001#issuecomment-129678",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:3" align="right">Comment 3</div>

Fantastic!



---

archive/issue_events_148432.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2011-12-27T09:04:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12001",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12001#event-148432"
}
```



---

archive/issue_events_148433.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2011-12-27T09:04:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12001",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12001#event-148433"
}
```



---

archive/issue_comments_129679.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nI'm setting this to \"positive review\" to bring it to the attention of the release manager. Release manager: please close as fixed once #11601 is merged.",
    "created_at": "2011-12-27T09:04:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12001",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12001#issuecomment-129679",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:4" align="right">Comment 4</div>

I'm setting this to "positive review" to bring it to the attention of the release manager. Release manager: please close as fixed once #11601 is merged.



---

archive/issue_events_148434.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-12-28T22:08:35Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12001",
    "milestone_number": null,
    "milestone_title": "sage-4.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12001#event-148434"
}
```



---

archive/issue_events_148435.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-12-28T22:08:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12001",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12001#event-148435"
}
```



---

archive/issue_events_148436.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-12-28T22:08:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12001",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12001#event-148436"
}
```



---

archive/issue_comments_129680.json:
```json
{
    "body": "Reviewer: **David Loeffler**",
    "created_at": "2012-01-13T08:59:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12001",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12001#issuecomment-129680",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **David Loeffler**



---

archive/issue_events_148437.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-01-13T08:59:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12001",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12001#event-148437"
}
```



---

archive/issue_events_148438.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-01-13T08:59:12Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12001",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12001#event-148438"
}
```



---

archive/issue_events_148439.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-01-13T08:59:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12001",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12001#event-148439"
}
```
