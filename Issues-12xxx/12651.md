# Issue 12651: Don't add each piece of piecewise functions to the legend

archive/issues_012479.json:
```json
{
    "assignees": [],
    "body": "This was separated from #11225. The fix is given in [AskSage 925](http://ask.sagemath.org/question/925/piecewise-functions-and-legend-label) by Douglas McNeil (dsm).\n\n\nAssignee: **@orlitzky**\n\nCC:  @kcrisman\n\nReviewer: **Karl-Dieter Crisman**\n\nAuthor: **Douglas McNeil, Michael Orlitzky**\n\nMerged: **sage-5.0.beta9**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12651_\n\n",
    "closed_at": "2012-03-21T22:09:00Z",
    "created_at": "2012-03-10T18:30:47Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graphics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Don't add each piece of piecewise functions to the legend",
    "type": "issue",
    "updated_at": "2012-03-21T22:09:00Z",
    "url": "https://github.com/sagemath/sage/issues/12651",
    "user": "https://github.com/orlitzky"
}
```
This was separated from #11225. The fix is given in [AskSage 925](http://ask.sagemath.org/question/925/piecewise-functions-and-legend-label) by Douglas McNeil (dsm).


Assignee: **@orlitzky**

CC:  @kcrisman

Reviewer: **Karl-Dieter Crisman**

Author: **Douglas McNeil, Michael Orlitzky**

Merged: **sage-5.0.beta9**

_Issue created by migration from https://trac.sagemath.org/ticket/12651_





---

archive/issue_events_159056.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2012-03-10T18:30:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12651",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12651#event-159056"
}
```



---

archive/issue_events_159057.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2012-03-10T18:30:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12651",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graphics",
    "label_color": "0000ff",
    "label_name": "component: graphics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12651#event-159057"
}
```



---

archive/issue_events_159058.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2012-03-10T18:30:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12651",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12651#event-159058"
}
```



---

archive/issue_events_159059.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2012-03-10T18:30:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12651",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12651#event-159059"
}
```



---

archive/issue_comments_143223.json:
```json
{
    "body": "Author: **Douglas McNeil, Michael Orlitzky**",
    "created_at": "2012-03-10T21:19:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12651#issuecomment-143223",
    "user": "https://github.com/orlitzky"
}
```

Author: **Douglas McNeil, Michael Orlitzky**



---

archive/issue_events_159060.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2012-03-10T21:19:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12651",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12651#event-159060"
}
```



---

archive/issue_comments_143224.json:
```json
{
    "body": "Fix the duplicate legend entries and add a doctest",
    "created_at": "2012-03-10T21:50:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12651#issuecomment-143224",
    "user": "https://github.com/orlitzky"
}
```

Fix the duplicate legend entries and add a doctest



---

archive/issue_comments_143225.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nAttachment: **[sage-trac_12651.patch.gz](https://github.com/sagemath/sage/files/ticket12651/sage-trac_12651.patch.gz)**\n\nFirst patch had two lines with trailing whitespace, I just removed them.",
    "created_at": "2012-03-10T21:51:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12651#issuecomment-143225",
    "user": "https://github.com/orlitzky"
}
```

<div id="comment:3" align="right">Comment 3</div>

Attachment: **[sage-trac_12651.patch.gz](https://github.com/sagemath/sage/files/ticket12651/sage-trac_12651.patch.gz)**

First patch had two lines with trailing whitespace, I just removed them.



---

archive/issue_events_159061.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-03-12T02:26:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12651",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12651#event-159061"
}
```



---

archive/issue_events_159062.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-03-12T02:26:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12651",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12651#event-159062"
}
```



---

archive/issue_comments_143226.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nIt seems like everything is okay here.  Certainly it fixes the problem, other keywords do apply to all pieces of the function.\n\nWhen did the `:trac:` thing come in?  This sounds vaguely familiar... at any rate, it works very nicely in the live doc!",
    "created_at": "2012-03-12T02:26:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12651#issuecomment-143226",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:4" align="right">Comment 4</div>

It seems like everything is okay here.  Certainly it fixes the problem, other keywords do apply to all pieces of the function.

When did the `:trac:` thing come in?  This sounds vaguely familiar... at any rate, it works very nicely in the live doc!



---

archive/issue_comments_143227.json:
```json
{
    "body": "Reviewer: **Karl-Dieter Crisman**",
    "created_at": "2012-03-12T02:26:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12651#issuecomment-143227",
    "user": "https://github.com/kcrisman"
}
```

Reviewer: **Karl-Dieter Crisman**



---

archive/issue_comments_143228.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nReplying to [@kcrisman](#comment%3A4):\n> When did the `:trac:` thing come in?  This sounds vaguely familiar... at any rate, it works very nicely in the live doc!\n\n5.0.beta4, apparently. Psyched that I no longer have to worry about word-wrapping \"trac #12345\" in the doctests.",
    "created_at": "2012-03-12T19:10:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12651#issuecomment-143228",
    "user": "https://github.com/orlitzky"
}
```

<div id="comment:5" align="right">Comment 5</div>

Replying to [@kcrisman](#comment%3A4):
> When did the `:trac:` thing come in?  This sounds vaguely familiar... at any rate, it works very nicely in the live doc!

5.0.beta4, apparently. Psyched that I no longer have to worry about word-wrapping "trac #12345" in the doctests.



---

archive/issue_events_159063.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-03-21T22:09:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12651",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12651#event-159063"
}
```



---

archive/issue_events_159064.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-03-21T22:09:00Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12651",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12651#event-159064"
}
```



---

archive/issue_comments_143229.json:
```json
{
    "body": "Merged: **sage-5.0.beta9**",
    "created_at": "2012-03-21T22:09:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12651#issuecomment-143229",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.0.beta9**
