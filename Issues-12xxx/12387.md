# Issue 12387: Everything is not well with dense graph backends

archive/issues_012215.json:
```json
{
    "assignees": [],
    "body": "We discovered in #12318 that some functions of sage/geometry/polyhedra and sage/geometry/triangulation/point_configuration.py return wrong results if Graph(sparse = True) is replaced by Graph(sparse = False)\n\n\nAssignee: **@jasongrout, @nathanncohen, @rlmill**\n\nCC:  brunellus @rlmill\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12387_\n\n",
    "created_at": "2012-01-30T11:38:40Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Everything is not well with dense graph backends",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/12387",
    "user": "https://github.com/nathanncohen"
}
```
We discovered in #12318 that some functions of sage/geometry/polyhedra and sage/geometry/triangulation/point_configuration.py return wrong results if Graph(sparse = True) is replaced by Graph(sparse = False)


Assignee: **@jasongrout, @nathanncohen, @rlmill**

CC:  brunellus @rlmill

_Issue created by migration from https://trac.sagemath.org/ticket/12387_





---

archive/issue_events_154593.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2012-01-30T11:38:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12387",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12387#event-154593"
}
```



---

archive/issue_events_154594.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2012-01-30T11:38:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12387",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12387#event-154594"
}
```



---

archive/issue_events_154595.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2012-01-30T11:38:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12387",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12387#event-154595"
}
```



---

archive/issue_events_154596.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2012-01-30T11:38:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12387",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12387#event-154596"
}
```



---

archive/issue_comments_137641.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nThe dense graph code doesn't support edge labeling, but the code in point_configuration.py rely on this. Maybe it is a mistake that dense_graph doesn't crash and burn whenever someone call add_edge(u, v, nonNone) -- but it is consistent with cases like\n\n```\nsage: G=Graph(multiedges=False)\nsage: G.add_edge(0,1)\nsage: G.add_edge(0,1)\nsage: G.edges()\n[(0, 1, None)]\n```",
    "created_at": "2012-02-06T13:39:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12387",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12387#issuecomment-137641",
    "user": "https://github.com/sagetrac-brunellus"
}
```

<a id='comment:1'>Comment 1:</a>
The dense graph code doesn't support edge labeling, but the code in point_configuration.py rely on this. Maybe it is a mistake that dense_graph doesn't crash and burn whenever someone call add_edge(u, v, nonNone) -- but it is consistent with cases like

```
sage: G=Graph(multiedges=False)
sage: G.add_edge(0,1)
sage: G.add_edge(0,1)
sage: G.edges()
[(0, 1, None)]
```



---

archive/issue_comments_137642.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nI wrote the code for polyhedra. It is definitely a bug if the graph silently gives the wrong answer just because some backend doesn't support edge labels.",
    "created_at": "2012-02-19T01:02:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12387",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12387#issuecomment-137642",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:2'>Comment 2:</a>
I wrote the code for polyhedra. It is definitely a bug if the graph silently gives the wrong answer just because some backend doesn't support edge labels.



---

archive/issue_comments_137643.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nI started #12540 for this discussion.\n\nI will investigate further if the lack of edge labels is the only source of the wrong results we spotted in #12318.",
    "created_at": "2012-02-19T17:06:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12387",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12387#issuecomment-137643",
    "user": "https://github.com/sagetrac-brunellus"
}
```

<a id='comment:3'>Comment 3:</a>
I started #12540 for this discussion.

I will investigate further if the lack of edge labels is the only source of the wrong results we spotted in #12318.



---

archive/issue_events_154597.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12387",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12387#event-154597"
}
```



---

archive/issue_events_154598.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12387",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12387#event-154598"
}
```



---

archive/issue_events_154599.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12387",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12387#event-154599"
}
```



---

archive/issue_events_154600.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12387",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12387#event-154600"
}
```



---

archive/issue_events_154601.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12387",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12387#event-154601"
}
```



---

archive/issue_events_154602.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12387",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12387#event-154602"
}
```



---

archive/issue_events_154603.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12387",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12387#event-154603"
}
```



---

archive/issue_events_154604.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12387",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12387#event-154604"
}
```



---

archive/issue_events_154605.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12387",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12387#event-154605"
}
```
