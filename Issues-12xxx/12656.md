# Issue 12656: Fix inconsistencies in the PolyBoRi interface

archive/issues_012484.json:
```json
{
    "assignees": [],
    "body": "There are some inconsistencies between PolyBoRi's original interface and Sage's Cython-based reimplementation. \n\nThere were uncovered by the newly introduced self-testing feature of the PolyBoRi-shell `ipbori`. Call `ipbori -t` with #12655 installed to see the issue. \n\n### Current patch\n* **Apply** [attachment: polybori-0.8.x.Trac12656.2.patch.gz](https://github.com/sagemath/sage/files/ticket12656/polybori-0.8.x.Trac12656.2.patch.gz)\n\nAssignee: **@alexanderdreyer**\n\nCC:  PolyBoRi @malb @burcin\n\nUpstream: **None of the above - read trac for reasoning.**\n\nReviewer: **Martin Albrecht**\n\nAuthor: **Alexander Dreyer**\n\nMerged: **sage-5.0.beta9**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12656_\n\n",
    "closed_at": "2012-03-21T22:09:26Z",
    "created_at": "2012-03-12T15:43:41Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix inconsistencies in the PolyBoRi interface",
    "type": "issue",
    "updated_at": "2012-03-21T22:09:26Z",
    "url": "https://github.com/sagemath/sage/issues/12656",
    "user": "https://github.com/alexanderdreyer"
}
```
There are some inconsistencies between PolyBoRi's original interface and Sage's Cython-based reimplementation. 

There were uncovered by the newly introduced self-testing feature of the PolyBoRi-shell `ipbori`. Call `ipbori -t` with #12655 installed to see the issue. 

### Current patch
* **Apply** [attachment: polybori-0.8.x.Trac12656.2.patch.gz](https://github.com/sagemath/sage/files/ticket12656/polybori-0.8.x.Trac12656.2.patch.gz)

Assignee: **@alexanderdreyer**

CC:  PolyBoRi @malb @burcin

Upstream: **None of the above - read trac for reasoning.**

Reviewer: **Martin Albrecht**

Author: **Alexander Dreyer**

Merged: **sage-5.0.beta9**

_Issue created by migration from https://trac.sagemath.org/ticket/12656_





---

archive/issue_events_159144.json:
```json
{
    "actor": "https://github.com/alexanderdreyer",
    "created_at": "2012-03-12T15:43:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12656",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12656#event-159144"
}
```



---

archive/issue_events_159145.json:
```json
{
    "actor": "https://github.com/alexanderdreyer",
    "created_at": "2012-03-12T15:43:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12656",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12656#event-159145"
}
```



---

archive/issue_events_159146.json:
```json
{
    "actor": "https://github.com/alexanderdreyer",
    "created_at": "2012-03-12T15:43:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12656",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12656#event-159146"
}
```



---

archive/issue_events_159147.json:
```json
{
    "actor": "https://github.com/alexanderdreyer",
    "created_at": "2012-03-12T15:43:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12656",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12656#event-159147"
}
```



---

archive/issue_comments_143345.json:
```json
{
    "body": "Cython interface consistent with PolyBoRi's built-in interface",
    "created_at": "2012-03-12T15:49:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12656",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12656#issuecomment-143345",
    "user": "https://github.com/alexanderdreyer"
}
```

Cython interface consistent with PolyBoRi's built-in interface



---

archive/issue_events_159148.json:
```json
{
    "actor": "https://github.com/alexanderdreyer",
    "created_at": "2012-03-12T15:58:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12656",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12656#event-159148"
}
```



---

archive/issue_comments_143346.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[polybori-0.8.x.Trac12656.patch.gz](https://github.com/sagemath/sage/files/ticket12656/polybori-0.8.x.Trac12656.patch.gz)**\n\nFor sage-5.0.beta7 (with #12655) the patch applies and tests well (`make ptestlong`) on a SuSE Enterprise 11 AMD64.",
    "created_at": "2012-03-12T15:58:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12656",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12656#issuecomment-143346",
    "user": "https://github.com/alexanderdreyer"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[polybori-0.8.x.Trac12656.patch.gz](https://github.com/sagemath/sage/files/ticket12656/polybori-0.8.x.Trac12656.patch.gz)**

For sage-5.0.beta7 (with #12655) the patch applies and tests well (`make ptestlong`) on a SuSE Enterprise 11 AMD64.



---

archive/issue_comments_143347.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,6 @@\n There are some inconsistencies between PolyBoRi's original interface and Sage's Cython-based reimplementation. \n \n There were uncovered by the newly introduced self-testing feature of the PolyBoRi-shell `ipbori`. Call `ipbori -t` with #12655 installed to see the issue. \n+\n+### Current patch\n+* **Apply** [attachment: polybori-0.8.x.Trac12656.patch.gz](https://github.com/sagemath/sage/files/ticket12656/polybori-0.8.x.Trac12656.patch.gz)\n``````\n",
    "created_at": "2012-03-12T23:26:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12656",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12656#issuecomment-143347",
    "user": "https://github.com/alexanderdreyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,6 @@
 There are some inconsistencies between PolyBoRi's original interface and Sage's Cython-based reimplementation. 
 
 There were uncovered by the newly introduced self-testing feature of the PolyBoRi-shell `ipbori`. Call `ipbori -t` with #12655 installed to see the issue. 
+
+### Current patch
+* **Apply** [attachment: polybori-0.8.x.Trac12656.patch.gz](https://github.com/sagemath/sage/files/ticket12656/polybori-0.8.x.Trac12656.patch.gz)
``````




---

archive/issue_comments_143348.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nAlso works with plain sage-5.0.beta7 (without #12655).",
    "created_at": "2012-03-12T23:26:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12656",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12656#issuecomment-143348",
    "user": "https://github.com/alexanderdreyer"
}
```

<a id='comment:2'>Comment 2:</a>
Also works with plain sage-5.0.beta7 (without #12655).



---

archive/issue_comments_143349.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nPatch looks good, applies cleanly and passes doctests.",
    "created_at": "2012-03-14T14:21:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12656",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12656#issuecomment-143349",
    "user": "https://github.com/malb"
}
```

<a id='comment:3'>Comment 3:</a>
Patch looks good, applies cleanly and passes doctests.



---

archive/issue_comments_143350.json:
```json
{
    "body": "Reviewer: **Martin Albrecht**",
    "created_at": "2012-03-14T14:21:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12656",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12656#issuecomment-143350",
    "user": "https://github.com/malb"
}
```

Reviewer: **Martin Albrecht**



---

archive/issue_events_159149.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2012-03-14T14:21:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12656",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12656#event-159149"
}
```



---

archive/issue_events_159150.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2012-03-14T14:21:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12656",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12656#event-159150"
}
```



---

archive/issue_comments_143351.json:
```json
{
    "body": "Fixed weird whitespace, apply only this",
    "created_at": "2012-03-15T20:14:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12656",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12656#issuecomment-143351",
    "user": "https://github.com/jdemeyer"
}
```

Fixed weird whitespace, apply only this



---

archive/issue_comments_143352.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nAttachment: **[polybori-0.8.x.Trac12656.2.patch.gz](https://github.com/sagemath/sage/files/ticket12656/polybori-0.8.x.Trac12656.2.patch.gz)**",
    "created_at": "2012-03-15T20:15:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12656",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12656#issuecomment-143352",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:4'>Comment 4:</a>
Attachment: **[polybori-0.8.x.Trac12656.2.patch.gz](https://github.com/sagemath/sage/files/ticket12656/polybori-0.8.x.Trac12656.2.patch.gz)**



---

archive/issue_comments_143353.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -3,4 +3,4 @@\n There were uncovered by the newly introduced self-testing feature of the PolyBoRi-shell `ipbori`. Call `ipbori -t` with #12655 installed to see the issue. \n \n ### Current patch\n-* **Apply** [attachment: polybori-0.8.x.Trac12656.patch.gz](https://github.com/sagemath/sage/files/ticket12656/polybori-0.8.x.Trac12656.patch.gz)\n+* **Apply** [attachment: polybori-0.8.x.Trac12656.2.patch.gz](https://github.com/sagemath/sage/files/ticket12656/polybori-0.8.x.Trac12656.2.patch.gz)\n``````\n",
    "created_at": "2012-03-15T20:15:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12656",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12656#issuecomment-143353",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -3,4 +3,4 @@
 There were uncovered by the newly introduced self-testing feature of the PolyBoRi-shell `ipbori`. Call `ipbori -t` with #12655 installed to see the issue. 
 
 ### Current patch
-* **Apply** [attachment: polybori-0.8.x.Trac12656.patch.gz](https://github.com/sagemath/sage/files/ticket12656/polybori-0.8.x.Trac12656.patch.gz)
+* **Apply** [attachment: polybori-0.8.x.Trac12656.2.patch.gz](https://github.com/sagemath/sage/files/ticket12656/polybori-0.8.x.Trac12656.2.patch.gz)
``````




---

archive/issue_comments_143354.json:
```json
{
    "body": "Merged: **sage-5.0.beta9**",
    "created_at": "2012-03-21T22:09:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12656",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12656#issuecomment-143354",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.0.beta9**



---

archive/issue_events_159151.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-03-21T22:09:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12656",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12656#event-159151"
}
```



---

archive/issue_events_159152.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-03-21T22:09:26Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12656",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12656#event-159152"
}
```
