...
<a id='comment:20'></a>The imaginary part turns out to be "simply" the fact that floating-point addition is not associative, and for some reason the terms in the expression `e` from comment:17 are added in random order by `.n()`.
