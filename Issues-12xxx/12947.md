...
<a id='comment:2'></a>Robert Dodier of Maxima suggests the following later in the thread.
...
<a id='comment:3'></a>Robert points out that in 5.27.0 this is at least a little better, though we still need to change domain.
...
```json
{
    "body": "<a id='comment:4'></a>But since we already use `domain:complex` in the initialization code for Maxima for integrals and such, presumably an upgrade would fix this?",
...
<a id='comment:4'></a>But since we already use `domain:complex` in the initialization code for Maxima for integrals and such, presumably an upgrade would fix this?
...
<a id='comment:5'></a>See my last comment on #11238 for why we can't have nice things.
...
```json
{
    "body": "<a id='comment:9'></a>This seems to have been fixed some time ago; it works correctly in both Sage 5.13 and 6.2.\n\n```\nsage: integrate(x*cos(x^3),(x,0,1/2)).n()\n0.124756040961038\n```",
    "created_at": "2014-05-30T22:49:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12947",
    "type": "issue_comment",
...
<a id='comment:9'></a>This seems to have been fixed some time ago; it works correctly in both Sage 5.13 and 6.2.
...
```json
{
    "body": "<a id='comment:14'></a>I would argue that the complex bit means that we are not quite ready to merge this if it happens on some platforms.  The question is whether this is a Maxima bug or a Sage/Pynac simplification bug.",
...
<a id='comment:14'></a>I would argue that the complex bit means that we are not quite ready to merge this if it happens on some platforms.  The question is whether this is a Maxima bug or a Sage/Pynac simplification bug.
...
```json
{
    "body": "<a id='comment:15'></a>Would you equally mind if there is a real digit wrong in the 17-th place? Regardless, it's fine on AMD now.",
...
<a id='comment:15'></a>Would you equally mind if there is a real digit wrong in the 17-th place? Regardless, it's fine on AMD now.
...
```json
{
    "body": "<a id='comment:16'></a>There are 60(!) lines in the sage code with `# abs tol` and nearly all have lower tolerance (in the sense of 'larger error')",
...
<a id='comment:16'></a>There are 60(!) lines in the sage code with `# abs tol` and nearly all have lower tolerance (in the sense of 'larger error')
...
<a id='comment:17'></a>The complex bit apparently comes from applying Pynac's `evalf()` to the following expression:
...
```json
{
    "body": "<a id='comment:18'></a>The presence of the imaginary part does not depend on #7099.  At least on ARM it is essentially random (maybe it depends on FPU rounding flags or something similar):\n\n```\n$ ./sage -c 'print integrate(x*cos(x^3),(x,0,1/2)).n()'\n0.124756040961038\n$ ./sage -c 'print integrate(x*cos(x^3),(x,0,1/2)).n()'\n0.124756040961038 - 2.77555756156289e-17*I\n$ ./sage -c 'print integrate(x*cos(x^3),(x,0,1/2)).n()'\n0.124756040961038 - 1.38777878078145e-17*I\n$ ./sage -c 'print integrate(x*cos(x^3),(x,0,1/2)).n()'\n0.124756040961038 + 1.38777878078145e-17*I\n```",
...
<a id='comment:18'></a>The presence of the imaginary part does not depend on #7099.  At least on ARM it is essentially random (maybe it depends on FPU rounding flags or something similar):
...
<a id='comment:19'></a>> Would you equally mind if there is a real digit wrong in the 17-th place? Regardless, it's fine on AMD now.
...
<a id='comment:20'></a>The imaginary part turns out to be "simply" the fact that floating-point addition is not associative, and for some reason the terms in the expression `e` from[comment:17](#comment%3A17) are added in random order by `.n()`.
...
```json
{
    "body": "<a id='comment:22'></a>As already said ...",
...
<a id='comment:22'></a>As already said ...
...
<a id='comment:23'></a>The `# abs tol` just changes how the floating point numbers are compared, it doesn't know about complex numbers. 
...
```json
{
    "body": "<a id='comment:26'></a>This should solve it; probably no reason to ask for another review.",
    "created_at": "2014-07-18T15:29:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12947",
    "type": "issue_comment",
...
<a id='comment:26'></a>This should solve it; probably no reason to ask for another review.
