# Issue 12736: More solver options for GLPK

archive/issues_012564.json:
```json
{
    "body": "Several requests for solver options to use as parameters for GLPK prompt us to allow the user to set quite a few parameters.\n\nIn addition, the name for the solver parameter in ticket #12309 (`preprocessing`) isn't very accurate (we didn't think it through carefully enough at the time) so that option will change, too.\n\nI propose to change/enable the following solver options. See the GLPK documentation for details on each option, and note that most have two names: one to correspond to GLPK, one for legibility.\n\n* glpk's solver\n  * default is `glp_intopt`, as at present\n  * `simplex_first`: setting this to `True` makes the solver invoke `glp_simplex` before `glp_intopt`\n    * useful to avoid problems in glpk\n    * **replaces** current `preprocessing` parameter\n  * `simplex_only`: setting this to `True makes the solver invoke `glp_simplex` **instead** of `glp_intopt`\n\n* glpk's `intopt` solver\n  * `msg_level` or `verbosity`\n  * `br_tech` or `branching`\n  * `bt_tech` or `backtracking`\n  * `pp_tech` or `preprocessing`\n  * `fp_heur` or `feasibility_pump`\n  * `gmi_cuts` or `gomory_cuts`\n  * `mir_cuts` or `mixed_int_rounding_cuts`\n  * `cov_cuts` or `mixed_cover_cuts`\n  * `clq_cuts` or `clique_cuts`\n  * `tol_int` or `absolute_tolerance`\n  * `tol_obj` or `relative_tolerance`\n  * `mip_gap` or `mip_gap_tolerance`\n  * `tm_lim` or `time_limit`\n  * `out_freq` or `output_frequency`\n  * `presolve` or `mip_presolver`\n  * `binarize`\n\n* glpk's `simplex` solver\n  * `msg_level` or `verbosity`\n  * `meth` or `primal_v_dual`\n  * `pricing`\n  * `r_test` or `ratio_test`\n  * `tol_bnd` or `tolerance_primal`\n  * `tol_dj` or `tolerance_dual`\n  * `tol_piv` or `tolerance_pivot`\n  * `obj_ll` or `obj_lower_limit`\n  * `obj_ul` or `obj_upper_limit`\n  * `it_lim` or `iteration_limit`\n  * `tm_lim` or `time_limit`\n  * `out_frq` or `output_frequency`\n  * `out_dly` or `output_delay`\n  * `presolve`\n\nFeel free to suggest better names, or that we stick with only the GLPK names, etc.\n\n**Apply**:\n* [attachment:trac_12736_more_solver_options_for_glpk.patch]\n* [attachment:trac_12736.patch]\n\nAssignee: @nathanncohen\n\nCC:  @nathanncohen r.gaia.cs\n\nKeywords: solver parameters\n\nReviewer: Nathann Cohen, John Perry\n\nAuthor: John Perry, Nathann Cohen\n\nMerged: sage-5.1.beta0\n\nDependencies: #12309\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/12736\n\n",
    "closed_at": "2012-05-06T12:15:58Z",
    "created_at": "2012-03-23T15:38:13Z",
    "labels": [
        "component: linear programming"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-5.1",
    "title": "More solver options for GLPK",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/12736",
    "user": "https://github.com/johnperry-math"
}
```
Several requests for solver options to use as parameters for GLPK prompt us to allow the user to set quite a few parameters.

In addition, the name for the solver parameter in ticket #12309 (`preprocessing`) isn't very accurate (we didn't think it through carefully enough at the time) so that option will change, too.

I propose to change/enable the following solver options. See the GLPK documentation for details on each option, and note that most have two names: one to correspond to GLPK, one for legibility.

* glpk's solver
  * default is `glp_intopt`, as at present
  * `simplex_first`: setting this to `True` makes the solver invoke `glp_simplex` before `glp_intopt`
    * useful to avoid problems in glpk
    * **replaces** current `preprocessing` parameter
  * `simplex_only`: setting this to `True makes the solver invoke `glp_simplex` **instead** of `glp_intopt`

* glpk's `intopt` solver
  * `msg_level` or `verbosity`
  * `br_tech` or `branching`
  * `bt_tech` or `backtracking`
  * `pp_tech` or `preprocessing`
  * `fp_heur` or `feasibility_pump`
  * `gmi_cuts` or `gomory_cuts`
  * `mir_cuts` or `mixed_int_rounding_cuts`
  * `cov_cuts` or `mixed_cover_cuts`
  * `clq_cuts` or `clique_cuts`
  * `tol_int` or `absolute_tolerance`
  * `tol_obj` or `relative_tolerance`
  * `mip_gap` or `mip_gap_tolerance`
  * `tm_lim` or `time_limit`
  * `out_freq` or `output_frequency`
  * `presolve` or `mip_presolver`
  * `binarize`

* glpk's `simplex` solver
  * `msg_level` or `verbosity`
  * `meth` or `primal_v_dual`
  * `pricing`
  * `r_test` or `ratio_test`
  * `tol_bnd` or `tolerance_primal`
  * `tol_dj` or `tolerance_dual`
  * `tol_piv` or `tolerance_pivot`
  * `obj_ll` or `obj_lower_limit`
  * `obj_ul` or `obj_upper_limit`
  * `it_lim` or `iteration_limit`
  * `tm_lim` or `time_limit`
  * `out_frq` or `output_frequency`
  * `out_dly` or `output_delay`
  * `presolve`

Feel free to suggest better names, or that we stick with only the GLPK names, etc.

**Apply**:
* [attachment:trac_12736_more_solver_options_for_glpk.patch]
* [attachment:trac_12736.patch]

Assignee: @nathanncohen

CC:  @nathanncohen r.gaia.cs

Keywords: solver parameters

Reviewer: Nathann Cohen, John Perry

Author: John Perry, Nathann Cohen

Merged: sage-5.1.beta0

Dependencies: #12309

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/12736





---

archive/issue_comments_184659.json:
```json
{
    "body": "Attachment [ticket_12736_add_glpk_solver_options.patch](tarball://root/attachments/some-uuid/ticket12736/ticket_12736_add_glpk_solver_options.patch) by r.gaia.cs created at 2012-03-25 18:58:00",
    "created_at": "2012-03-25T18:58:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184659",
    "user": "https://trac.sagemath.org/admin/accounts/users/r.gaia.cs"
}
```

Attachment [ticket_12736_add_glpk_solver_options.patch](tarball://root/attachments/some-uuid/ticket12736/ticket_12736_add_glpk_solver_options.patch) by r.gaia.cs created at 2012-03-25 18:58:00



---

archive/issue_comments_184660.json:
```json
{
    "body": "<a id='comment:2'></a>Raniere,\n\nI don't like the idea of using strings for the solver parameter name. This is primarily for efficiency reasons; you have to remember that in my projects, I create a lot of small LPs, & don't want to deal with the string comparisons when an integer comparison will do much better.\n\nI will upload another patch in a little bit (I finally figured out how to do it) and you can tell me what you think of this approach.",
    "created_at": "2012-03-26T14:39:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184660",
    "user": "https://github.com/johnperry-math"
}
```

<a id='comment:2'></a>Raniere,

I don't like the idea of using strings for the solver parameter name. This is primarily for efficiency reasons; you have to remember that in my projects, I create a lot of small LPs, & don't want to deal with the string comparisons when an integer comparison will do much better.

I will upload another patch in a little bit (I finally figured out how to do it) and you can tell me what you think of this approach.



---

archive/issue_comments_184661.json:
```json
{
    "body": "<a id='comment:3'></a>Replying to [john_perry](#comment%3A2):\n> I don't like the idea of using strings for the solver parameter name.\n\n\nArgh. Expressed myself badly: I meant,\n\n> ...for the solver parameter **value**.\n\n\nAlthough, given that the name is passed as a string, that may be a dumb objection.",
    "created_at": "2012-03-26T14:45:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184661",
    "user": "https://github.com/johnperry-math"
}
```

<a id='comment:3'></a>Replying to [john_perry](#comment%3A2):
> I don't like the idea of using strings for the solver parameter name.


Argh. Expressed myself badly: I meant,

> ...for the solver parameter **value**.


Although, given that the name is passed as a string, that may be a dumb objection.



---

archive/issue_comments_184662.json:
```json
{
    "body": "<a id='comment:4'></a>Helloooooo John !!!\n\nDid you try whether it really makes a difference in practice ? I mean, if you replace the arguments by integers and feed it with the right integer, is it really faster ? I am afraid of Python's own efficiency, and I wonder if this may not be the real bottleneck :-)\n\nNathann",
    "created_at": "2012-03-26T15:03:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184662",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:4'></a>Helloooooo John !!!

Did you try whether it really makes a difference in practice ? I mean, if you replace the arguments by integers and feed it with the right integer, is it really faster ? I am afraid of Python's own efficiency, and I wonder if this may not be the real bottleneck :-)

Nathann



---

archive/issue_comments_184663.json:
```json
{
    "body": "<a id='comment:5'></a>Replying to [ncohen](#comment%3A4):\n> Did you try whether it really makes a difference in practice ? I mean, if you replace the arguments by integers and feed it with the right integer, is it really faster ? I am afraid of Python's own efficiency, and I wonder if this may not be the real bottleneck :-)\n\n\nI create & modify these lp's all in Cython.",
    "created_at": "2012-03-26T15:10:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184663",
    "user": "https://github.com/johnperry-math"
}
```

<a id='comment:5'></a>Replying to [ncohen](#comment%3A4):
> Did you try whether it really makes a difference in practice ? I mean, if you replace the arguments by integers and feed it with the right integer, is it really faster ? I am afraid of Python's own efficiency, and I wonder if this may not be the real bottleneck :-)


I create & modify these lp's all in Cython.



---

archive/issue_comments_184664.json:
```json
{
    "body": "<a id='comment:6'></a>Oh. And does it make a difference, even in Cython ? I thought you were using the MixedIntegerLinearProgram.solver_parameter function, but it looks like you directly use the backend `;-)`\n\nPerhaps the best for you would be to create an empty LP with the right parameters and clone it afterwards !\n\nNathann",
    "created_at": "2012-03-26T15:12:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184664",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:6'></a>Oh. And does it make a difference, even in Cython ? I thought you were using the MixedIntegerLinearProgram.solver_parameter function, but it looks like you directly use the backend `;-)`

Perhaps the best for you would be to create an empty LP with the right parameters and clone it afterwards !

Nathann



---

archive/issue_comments_184665.json:
```json
{
    "body": "<a id='comment:7'></a>Replying to [john_perry](#comment%3A3):\nJohn,\nusing strings for name and value was my first and only ideia of how to do GLPKBackend.solver_parameter work with MixedIntegerLinearProgram.solver_parameter with not import GLPKBackend module.\nFor some parameter that only two values are avaliable it's possible to use True and False as values.\n\nRaniere",
    "created_at": "2012-03-26T15:55:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184665",
    "user": "https://trac.sagemath.org/admin/accounts/users/r.gaia.cs"
}
```

<a id='comment:7'></a>Replying to [john_perry](#comment%3A3):
John,
using strings for name and value was my first and only ideia of how to do GLPKBackend.solver_parameter work with MixedIntegerLinearProgram.solver_parameter with not import GLPKBackend module.
For some parameter that only two values are avaliable it's possible to use True and False as values.

Raniere



---

archive/issue_comments_184666.json:
```json
{
    "body": "<a id='comment:8'></a>The declaration of the `solve()` method of the `MixedIntegerLinearProgram` class is\n\n```\n    def solve(self, solver=None, log=0, objective_only=False):\n```\nWithin the function we have\n\n```\n        self._backend.set_verbosity(log)\n```\nMakes sense, you say, right? Well, no, for three reasons:\n\n1. It renders moot the `msg_lev` solver paramter.\n2. It's a bit weird to initialize `log`  to 0, since the solvers are already initialized to shut up in `__cinit__` .\n3. Someone might want to see whether the solver is making any progress, by setting the verbosity through the solver parameter, rather than through `solve()` .\n\nIMHO, the user ought to have that option; otherwise, there's no point to allowing a `msg_level` option at all. This is easily fixed, too: change the default value of `log` to `None`, and then the code to,\n\n```\n        if log != None:\n          self._backend.set_verbosity(log)\n```\nThis should retain compatibility with the previous setup.\n\nWhat do you guys think?",
    "created_at": "2012-03-26T18:11:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184666",
    "user": "https://github.com/johnperry-math"
}
```

<a id='comment:8'></a>The declaration of the `solve()` method of the `MixedIntegerLinearProgram` class is

```
    def solve(self, solver=None, log=0, objective_only=False):
```
Within the function we have

```
        self._backend.set_verbosity(log)
```
Makes sense, you say, right? Well, no, for three reasons:

1. It renders moot the `msg_lev` solver paramter.
2. It's a bit weird to initialize `log`  to 0, since the solvers are already initialized to shut up in `__cinit__` .
3. Someone might want to see whether the solver is making any progress, by setting the verbosity through the solver parameter, rather than through `solve()` .

IMHO, the user ought to have that option; otherwise, there's no point to allowing a `msg_level` option at all. This is easily fixed, too: change the default value of `log` to `None`, and then the code to,

```
        if log != None:
          self._backend.set_verbosity(log)
```
This should retain compatibility with the previous setup.

What do you guys think?



---

archive/issue_comments_184667.json:
```json
{
    "body": "<a id='comment:9'></a>Replying to [r.gaia.cs](#comment%3A7):\n> using strings for name and value was my first and only ideia of how to do GLPKBackend.solver_parameter work with MixedIntegerLinearProgram.solver_parameter with not import GLPKBackend module.\n\n\nThat's fine, and it even makes sense. The patch I'm preparing now will allow both approaches.",
    "created_at": "2012-03-26T18:12:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184667",
    "user": "https://github.com/johnperry-math"
}
```

<a id='comment:9'></a>Replying to [r.gaia.cs](#comment%3A7):
> using strings for name and value was my first and only ideia of how to do GLPKBackend.solver_parameter work with MixedIntegerLinearProgram.solver_parameter with not import GLPKBackend module.


That's fine, and it even makes sense. The patch I'm preparing now will allow both approaches.



---

archive/issue_comments_184668.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -46,3 +46,6 @@\n   * `presolve`\n \n Feel free to suggest better names, or that we stick with only the GLPK names, etc.\n+\n+**Apply**:\n+* trac_12736_more_solver_options_for_glpk.patch\n``````\n",
    "created_at": "2012-03-26T20:27:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184668",
    "user": "https://github.com/johnperry-math"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -46,3 +46,6 @@
   * `presolve`
 
 Feel free to suggest better names, or that we stick with only the GLPK names, etc.
+
+**Apply**:
+* trac_12736_more_solver_options_for_glpk.patch
``````




---

archive/issue_comments_184669.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2012-03-26T20:27:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184669",
    "user": "https://github.com/johnperry-math"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_184670.json:
```json
{
    "body": "<a id='comment:10'></a>I've added my version of a patch that should take care of the same thing. Although the coverage is more or less what we discussed, you may not like the approach I used to expose the GLPK constants: basically, I define them as module-level constants. I use this for both the names and the values. That way, the hard parts get done by integers, which should mitigate questions of efficiency. Well, sort of. `:-/`\n\nThe patch probably needs more doctests. Suggestions?",
    "created_at": "2012-03-26T20:27:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184670",
    "user": "https://github.com/johnperry-math"
}
```

<a id='comment:10'></a>I've added my version of a patch that should take care of the same thing. Although the coverage is more or less what we discussed, you may not like the approach I used to expose the GLPK constants: basically, I define them as module-level constants. I use this for both the names and the values. That way, the hard parts get done by integers, which should mitigate questions of efficiency. Well, sort of. `:-/`

The patch probably needs more doctests. Suggestions?



---

archive/issue_comments_184671.json:
```json
{
    "body": "<a id='comment:11'></a>Sorry -- the last patch was prepared in a hurry, right before teaching a class. I came back & discovered doctests, which are now fixed. This version passes all doctests in sage/numerical, so it should now be a question of adding new ones, & deciding if this captures the interface we really want.",
    "created_at": "2012-03-26T23:04:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184671",
    "user": "https://github.com/johnperry-math"
}
```

<a id='comment:11'></a>Sorry -- the last patch was prepared in a hurry, right before teaching a class. I came back & discovered doctests, which are now fixed. This version passes all doctests in sage/numerical, so it should now be a question of adding new ones, & deciding if this captures the interface we really want.



---

archive/issue_comments_184672.json:
```json
{
    "body": "<a id='comment:12'></a>Replying to [john_perry](#comment%3A11):\nI look very quickly your patch and I like it.\nIMHO, is better pass the number values in miliseconds as in GLPK.",
    "created_at": "2012-03-26T23:24:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184672",
    "user": "https://trac.sagemath.org/admin/accounts/users/r.gaia.cs"
}
```

<a id='comment:12'></a>Replying to [john_perry](#comment%3A11):
I look very quickly your patch and I like it.
IMHO, is better pass the number values in miliseconds as in GLPK.



---

archive/issue_comments_184673.json:
```json
{
    "body": "<a id='comment:13'></a>Replying to [r.gaia.cs](#comment%3A12):\n> IMHO, is better pass the number values in miliseconds as in GLPK.\n\n\nThanks. I remember that I wanted to check that, but forgot. I'll fix that once I have more input.",
    "created_at": "2012-03-26T23:26:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184673",
    "user": "https://github.com/johnperry-math"
}
```

<a id='comment:13'></a>Replying to [r.gaia.cs](#comment%3A12):
> IMHO, is better pass the number values in miliseconds as in GLPK.


Thanks. I remember that I wanted to check that, but forgot. I'll fix that once I have more input.



---

archive/issue_comments_184674.json:
```json
{
    "body": "<a id='comment:14'></a>Replying to [r.gaia.cs](#comment%3A12):\n> IMHO, is better pass the number values in miliseconds as in GLPK.\n\n\nWhile working on this tonight, I realized that mip.pyx expects the `timelimit` parameter for both CPLEX and GLPK to be in seconds. To get around this, I added a new parameter, `timelimit_intopt` and `timelimit_simplex`; these accept milliseconds.\n\nI also added a couple of new doctests, one to test this difference.\n\nI would like to doctest output when we set `msg_level` to something other than `...off`, but apparently the doctest facility doesn't recognize output produced by linked C code, only output produced by Python or Cython. If anyone has an idea on how to make that work, let me know.\n\nOtherwise, I guess this is ready. (I've tested it with some of my own code, too.)\n\nNEEDS REVIEW.",
    "created_at": "2012-04-04T03:31:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184674",
    "user": "https://github.com/johnperry-math"
}
```

<a id='comment:14'></a>Replying to [r.gaia.cs](#comment%3A12):
> IMHO, is better pass the number values in miliseconds as in GLPK.


While working on this tonight, I realized that mip.pyx expects the `timelimit` parameter for both CPLEX and GLPK to be in seconds. To get around this, I added a new parameter, `timelimit_intopt` and `timelimit_simplex`; these accept milliseconds.

I also added a couple of new doctests, one to test this difference.

I would like to doctest output when we set `msg_level` to something other than `...off`, but apparently the doctest facility doesn't recognize output produced by linked C code, only output produced by Python or Cython. If anyone has an idea on how to make that work, let me know.

Otherwise, I guess this is ready. (I've tested it with some of my own code, too.)

NEEDS REVIEW.



---

archive/issue_comments_184675.json:
```json
{
    "body": "<a id='comment:15'></a>Helloooooooo John !!\n\n> While working on this tonight, I realized that mip.pyx expects the `timelimit` parameter for both CPLEX and GLPK to be in seconds. To get around this, I added a new parameter, `timelimit_intopt` and `timelimit_simplex`; these accept milliseconds.\n\n\nHmmm... Why do you name this parameter timelimit_intopt or timelimit_simplex and not timelimit_seconds, or something like that ? What does it specifically have to do with simplex or intopt ? I think it would be better to have only one parameter for timelimit, and if you think that milliseconds are better what about having only one parameter which accepts milliseconds ? By the way, it is already possible to define the timelimit up to milisecond with this argument. The code actually multiplies the input by 1000 before updating GLPK's value `:-)`\n\nNathann",
    "created_at": "2012-04-05T14:08:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184675",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:15'></a>Helloooooooo John !!

> While working on this tonight, I realized that mip.pyx expects the `timelimit` parameter for both CPLEX and GLPK to be in seconds. To get around this, I added a new parameter, `timelimit_intopt` and `timelimit_simplex`; these accept milliseconds.


Hmmm... Why do you name this parameter timelimit_intopt or timelimit_simplex and not timelimit_seconds, or something like that ? What does it specifically have to do with simplex or intopt ? I think it would be better to have only one parameter for timelimit, and if you think that milliseconds are better what about having only one parameter which accepts milliseconds ? By the way, it is already possible to define the timelimit up to milisecond with this argument. The code actually multiplies the input by 1000 before updating GLPK's value `:-)`

Nathann



---

archive/issue_comments_184676.json:
```json
{
    "body": "<a id='comment:16'></a>Hmmmmmm `O_o`\n\nI quickly looked at the code again and you apparently also thought of a timelimit_seconds parameter, though it appears in the code and not in the documentation `^^;`\n\nNathann",
    "created_at": "2012-04-05T14:09:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184676",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:16'></a>Hmmmmmm `O_o`

I quickly looked at the code again and you apparently also thought of a timelimit_seconds parameter, though it appears in the code and not in the documentation `^^;`

Nathann



---

archive/issue_comments_184677.json:
```json
{
    "body": "<a id='comment:17'></a>Replying to [ncohen](#comment%3A15):\n> Hmmm... Why do you name this parameter timelimit_intopt or timelimit_simplex and not timelimit_seconds, or something like that ?\n\n\nI guess, just to be as specific as possible... in case someone needed it. I can't foresee all uses. But, for example, if someone wants to run `simplex_then_intopt`, she might want to set `timelimit_intopt` much higher than `timelimit_simplex`, because she knows that, in general, integer optimization takes much longer. Whereas if `simplex` doesn't end relatively quickly, there could be a bigger problem altogether.",
    "created_at": "2012-04-09T23:21:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184677",
    "user": "https://github.com/johnperry-math"
}
```

<a id='comment:17'></a>Replying to [ncohen](#comment%3A15):
> Hmmm... Why do you name this parameter timelimit_intopt or timelimit_simplex and not timelimit_seconds, or something like that ?


I guess, just to be as specific as possible... in case someone needed it. I can't foresee all uses. But, for example, if someone wants to run `simplex_then_intopt`, she might want to set `timelimit_intopt` much higher than `timelimit_simplex`, because she knows that, in general, integer optimization takes much longer. Whereas if `simplex` doesn't end relatively quickly, there could be a bigger problem altogether.



---

archive/issue_comments_184678.json:
```json
{
    "body": "<a id='comment:18'></a>Oh, you mean that timelimit_intopt limits the time it takes for the whole ILP to be solved while timelimit_simplex just bounds the time it takes to run the simplex algorithm for a *relaxed* instance.. Okok, I just didn't get that part `:-)`\n\nThe list of parameters is so long that I begin to look for a way to do it with dichotomy instead of trying all the different parameters, but that would really be messy `:-D`\n\nBy the way, about the string and integer values for the parameters.. Does it really make a meaningful difference in runtimes ? Just being curious `^^;`\n\nNathann",
    "created_at": "2012-04-10T14:23:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184678",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:18'></a>Oh, you mean that timelimit_intopt limits the time it takes for the whole ILP to be solved while timelimit_simplex just bounds the time it takes to run the simplex algorithm for a *relaxed* instance.. Okok, I just didn't get that part `:-)`

The list of parameters is so long that I begin to look for a way to do it with dichotomy instead of trying all the different parameters, but that would really be messy `:-D`

By the way, about the string and integer values for the parameters.. Does it really make a meaningful difference in runtimes ? Just being curious `^^;`

Nathann



---

archive/issue_comments_184679.json:
```json
{
    "body": "<a id='comment:19'></a>Replying to [ncohen](#comment%3A18):\n> The list of parameters is so long that I begin to look for a way to do it with dichotomy instead of trying all the different parameters, but that would really be messy `:-D`\n\n\nYeah. The long list of parameters can be a drag, but that's the price of enabling everything in GLPK. Alternately, we could decide to take a bunch of things off (would people actually use them?) but I'll bet that if we remove Gomory cuts, say, then we'll hear something in six weeks. `;-)`\n\n> By the way, about the string and integer values for the parameters.. Does it really make a meaningful difference in runtimes ? Just being curious `^^;`\n\n\nIf your focus is a handful of large linear programs, the penalty of Python strings v. machine integers might not matter. If your focus is a few thousand small linear programs, it could be an issue -- especially if you start mucking around w/options quite a bit.\n\nAdmittedly, in my own work I don't seem to need the options after all, and I've managed to revise, revise, revise my approach until I'm now creating only a few tens of programs, instead of a few thousand. So it's not that big an issue for me as it used to be. But it could become an issue again in the future.",
    "created_at": "2012-04-10T15:15:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184679",
    "user": "https://github.com/johnperry-math"
}
```

<a id='comment:19'></a>Replying to [ncohen](#comment%3A18):
> The list of parameters is so long that I begin to look for a way to do it with dichotomy instead of trying all the different parameters, but that would really be messy `:-D`


Yeah. The long list of parameters can be a drag, but that's the price of enabling everything in GLPK. Alternately, we could decide to take a bunch of things off (would people actually use them?) but I'll bet that if we remove Gomory cuts, say, then we'll hear something in six weeks. `;-)`

> By the way, about the string and integer values for the parameters.. Does it really make a meaningful difference in runtimes ? Just being curious `^^;`


If your focus is a handful of large linear programs, the penalty of Python strings v. machine integers might not matter. If your focus is a few thousand small linear programs, it could be an issue -- especially if you start mucking around w/options quite a bit.

Admittedly, in my own work I don't seem to need the options after all, and I've managed to revise, revise, revise my approach until I'm now creating only a few tens of programs, instead of a few thousand. So it's not that big an issue for me as it used to be. But it could become an issue again in the future.



---

archive/issue_comments_184680.json:
```json
{
    "body": "<a id='comment:20'></a>Hellooooooooo !!!\n\n> Yeah. The long list of parameters can be a drag, but that's the price of enabling everything in GLPK. Alternately, we could decide to take a bunch of things off (would people actually use them?) but I'll bet that if we remove Gomory cuts, say, then we'll hear something in six weeks. `;-)`\n\n\nOh, I really do not mind, I was just looking at the code and thinking that if the user wanted to set the LAST parameter the code would test all others first. That's why I would have wanted some dichotomy ! `:-D`\nBut that is just a natural reflex. Of course it is muuuuch better implemented like that.\n\n> If your focus is a handful of large linear programs, the penalty of Python strings v. machine integers might not matter. If your focus is a few thousand small linear programs, it could be an issue -- especially if you start mucking around w/options quite a bit.\n> \n> Admittedly, in my own work I don't seem to need the options after all, and I've managed to revise, revise, revise my approach until I'm now creating only a few tens of programs, instead of a few thousand. So it's not that big an issue for me as it used to be. But it could become an issue again in the future.\n\n\nWell, I was just surprised that comparing integers was *that much shorter* than comparing strings. And that it could become the bottleneck in LP codes `:-)`\n\nOk, I should give this code a proper review now !\n\nNathann",
    "created_at": "2012-04-10T16:30:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184680",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:20'></a>Hellooooooooo !!!

> Yeah. The long list of parameters can be a drag, but that's the price of enabling everything in GLPK. Alternately, we could decide to take a bunch of things off (would people actually use them?) but I'll bet that if we remove Gomory cuts, say, then we'll hear something in six weeks. `;-)`


Oh, I really do not mind, I was just looking at the code and thinking that if the user wanted to set the LAST parameter the code would test all others first. That's why I would have wanted some dichotomy ! `:-D`
But that is just a natural reflex. Of course it is muuuuch better implemented like that.

> If your focus is a handful of large linear programs, the penalty of Python strings v. machine integers might not matter. If your focus is a few thousand small linear programs, it could be an issue -- especially if you start mucking around w/options quite a bit.
> 
> Admittedly, in my own work I don't seem to need the options after all, and I've managed to revise, revise, revise my approach until I'm now creating only a few tens of programs, instead of a few thousand. So it's not that big an issue for me as it used to be. But it could become an issue again in the future.


Well, I was just surprised that comparing integers was *that much shorter* than comparing strings. And that it could become the bottleneck in LP codes `:-)`

Ok, I should give this code a proper review now !

Nathann



---

archive/issue_comments_184681.json:
```json
{
    "body": "<a id='comment:21'></a>Replying to [ncohen](#comment%3A20):\n> I was just looking at the code and thinking that if the user wanted to set the LAST parameter the code would test all others first. That's why I would have wanted some dichotomy ! `:-D`\n\n\nThat bothers me, too. I guess it should be organized so that the options most likely to come first would be tested first -- OR -- organize an if tree so that we have O(log(n)) comparisons instead of O(n). I'm completely open to suggestions on that.\n\n> Well, I was just surprised that comparing integers was *that much shorter* than comparing strings.\n\n\nI don't have hard tests, but it seems pretty obvious; string comparison is quite a bit more expensive.\n\n> And that it could become the bottleneck in LP codes `:-)`\n\n\nUnlikely, of course, but every bit helps `:-)`",
    "created_at": "2012-04-10T16:37:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184681",
    "user": "https://github.com/johnperry-math"
}
```

<a id='comment:21'></a>Replying to [ncohen](#comment%3A20):
> I was just looking at the code and thinking that if the user wanted to set the LAST parameter the code would test all others first. That's why I would have wanted some dichotomy ! `:-D`


That bothers me, too. I guess it should be organized so that the options most likely to come first would be tested first -- OR -- organize an if tree so that we have O(log(n)) comparisons instead of O(n). I'm completely open to suggestions on that.

> Well, I was just surprised that comparing integers was *that much shorter* than comparing strings.


I don't have hard tests, but it seems pretty obvious; string comparison is quite a bit more expensive.

> And that it could become the bottleneck in LP codes `:-)`


Unlikely, of course, but every bit helps `:-)`



---

archive/issue_comments_184682.json:
```json
{
    "body": "<a id='comment:22'></a>Hellooooooooo John !!\n\nSince #12816 I am really into Sphinx tables, so perhaps it was a bad idea in this case... But what do you think of the following patch ? It applies on top of yours `:-)`\n\nThere is nothing else that seems wrong at this level... Only some details in the documentation. Give me your advice on that ! `:-)`\n\nNathann",
    "created_at": "2012-04-10T19:01:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184682",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:22'></a>Hellooooooooo John !!

Since #12816 I am really into Sphinx tables, so perhaps it was a bad idea in this case... But what do you think of the following patch ? It applies on top of yours `:-)`

There is nothing else that seems wrong at this level... Only some details in the documentation. Give me your advice on that ! `:-)`

Nathann



---

archive/issue_comments_184683.json:
```json
{
    "body": "Attachment [trac_12736.patch](tarball://root/attachments/some-uuid/ticket12736/trac_12736.patch) by @nathanncohen created at 2012-04-10 19:02:46",
    "created_at": "2012-04-10T19:02:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184683",
    "user": "https://github.com/nathanncohen"
}
```

Attachment [trac_12736.patch](tarball://root/attachments/some-uuid/ticket12736/trac_12736.patch) by @nathanncohen created at 2012-04-10 19:02:46



---

archive/issue_comments_184684.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -48,4 +48,5 @@\n Feel free to suggest better names, or that we stick with only the GLPK names, etc.\n \n **Apply**:\n-* trac_12736_more_solver_options_for_glpk.patch\n+* [attachment:trac_12736_more_solver_options_for_glpk.patch]\n+* [attachment:trac_12736.patch]\n``````\n",
    "created_at": "2012-04-10T19:03:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184684",
    "user": "https://github.com/nathanncohen"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -48,4 +48,5 @@
 Feel free to suggest better names, or that we stick with only the GLPK names, etc.
 
 **Apply**:
-* trac_12736_more_solver_options_for_glpk.patch
+* [attachment:trac_12736_more_solver_options_for_glpk.patch]
+* [attachment:trac_12736.patch]
``````




---

archive/issue_comments_184685.json:
```json
{
    "body": "<a id='comment:24'></a>Replying to [ncohen](#comment%3A22):\n> Since #12816 I am really into Sphinx tables...\n\n\nI hadn't heard of them. Okay, the update is merely to the documentation. What's a good way of viewing it? Should it work even in text mode?",
    "created_at": "2012-04-10T19:18:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184685",
    "user": "https://github.com/johnperry-math"
}
```

<a id='comment:24'></a>Replying to [ncohen](#comment%3A22):
> Since #12816 I am really into Sphinx tables...


I hadn't heard of them. Okay, the update is merely to the documentation. What's a good way of viewing it? Should it work even in text mode?



---

archive/issue_comments_184686.json:
```json
{
    "body": "<a id='comment:25'></a>> I hadn't heard of them. Okay, the update is merely to the documentation. What's a good way of viewing it? Should it work even in text mode?\n\n\nNo, it should change nothing to the text mode, but the html doc should be much better !!\n\nTo compile it :\n\nsage -b <your_branch> && sage -docbuild reference html\n\nNathann",
    "created_at": "2012-04-10T19:19:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184686",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:25'></a>> I hadn't heard of them. Okay, the update is merely to the documentation. What's a good way of viewing it? Should it work even in text mode?


No, it should change nothing to the text mode, but the html doc should be much better !!

To compile it :

sage -b <your_branch> && sage -docbuild reference html

Nathann



---

archive/issue_comments_184687.json:
```json
{
    "body": "<a id='comment:26'></a>Replying to [ncohen](#comment%3A25):\n> To compile it :\n> \n> sage -b <your_branch> && sage -docbuild reference html\n\n\nBuilding the reference will take a while, won't it? If so, I'll wait until later to do this.",
    "created_at": "2012-04-10T19:25:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184687",
    "user": "https://github.com/johnperry-math"
}
```

<a id='comment:26'></a>Replying to [ncohen](#comment%3A25):
> To compile it :
> 
> sage -b <your_branch> && sage -docbuild reference html


Building the reference will take a while, won't it? If so, I'll wait until later to do this.



---

archive/issue_comments_184688.json:
```json
{
    "body": "<a id='comment:27'></a>It depends on your luck. Usually it just compiles \"the difference\", but with some Sage install it may mean building them from the beginning `:-)`\n\nYou can add a \"nice\" before, if that is too heavy :\nsage -b <your_branch> && nice sage -docbuild reference html\n\nNathann",
    "created_at": "2012-04-10T19:27:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184688",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:27'></a>It depends on your luck. Usually it just compiles "the difference", but with some Sage install it may mean building them from the beginning `:-)`

You can add a "nice" before, if that is too heavy :
sage -b <your_branch> && nice sage -docbuild reference html

Nathann



---

archive/issue_comments_184689.json:
```json
{
    "body": "<a id='comment:28'></a>I am now getting, on two different machines, doctest failures in `mip.pyx` when #12736 is applied. They report different lines (different betas, different patches applied, I guess) but the basic idea is this:\n\n```\nExpected:\n    Traceback (most recent call last):\n    ...\n    MIPSolverException: 'GLPK : Solution is undefined'\n...\nGot:\n    Traceback (most recent call last):\n    ...\n    MIPSolverException: 'GLPK: An error occurred in the solver'\n```\nI\nAt first, I thought it was just a matter of the space, but then I realized the errors were different, too. I'm going to look into this real quick.\n\nAlso, I added a dependency to the ticket description.",
    "created_at": "2012-04-12T20:06:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184689",
    "user": "https://github.com/johnperry-math"
}
```

<a id='comment:28'></a>I am now getting, on two different machines, doctest failures in `mip.pyx` when #12736 is applied. They report different lines (different betas, different patches applied, I guess) but the basic idea is this:

```
Expected:
    Traceback (most recent call last):
    ...
    MIPSolverException: 'GLPK : Solution is undefined'
...
Got:
    Traceback (most recent call last):
    ...
    MIPSolverException: 'GLPK: An error occurred in the solver'
```
I
At first, I thought it was just a matter of the space, but then I realized the errors were different, too. I'm going to look into this real quick.

Also, I added a dependency to the ticket description.



---

archive/issue_comments_184690.json:
```json
{
    "body": "Changing dependencies from \"\" to \"#12309\"",
    "created_at": "2012-04-12T20:06:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184690",
    "user": "https://github.com/johnperry-math"
}
```

Changing dependencies from "" to "#12309"



---

archive/issue_comments_184691.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2012-04-12T20:06:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184691",
    "user": "https://github.com/johnperry-math"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_184692.json:
```json
{
    "body": "<a id='comment:29'></a>Okay, it looks as if ** *I* ** had forgotten to test `mip.pyx` this time! Basically, I had added to `glpk_backend`'s `solve()` method a test for correctness when the timer runs out. I did this because otherwise `mip.pyx` thought there was a solution. (I don't remember exactly why.)\n\nFor now, I've changed the expected string in the doctest to reflect what the backend currently has, but another (better?) mofidication would be to change the backend to test for the precise signal (`GLP_ETMLIM`) and raise an exception telling the user that time ran out, so the solution will be invalid.\n\nWhich do youprefer?",
    "created_at": "2012-04-12T20:28:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184692",
    "user": "https://github.com/johnperry-math"
}
```

<a id='comment:29'></a>Okay, it looks as if ** *I* ** had forgotten to test `mip.pyx` this time! Basically, I had added to `glpk_backend`'s `solve()` method a test for correctness when the timer runs out. I did this because otherwise `mip.pyx` thought there was a solution. (I don't remember exactly why.)

For now, I've changed the expected string in the doctest to reflect what the backend currently has, but another (better?) mofidication would be to change the backend to test for the precise signal (`GLP_ETMLIM`) and raise an exception telling the user that time ran out, so the solution will be invalid.

Which do youprefer?



---

archive/issue_comments_184693.json:
```json
{
    "body": "<a id='comment:30'></a>Helloooooooo John !!\n\nI see no new method in your patch, but it is a bit hard to see what you *changed* inside of this large patch. By the way, if you modify your patch while mine already applies on top of it, there's a chance that it will break afterwards `^^;`\n\nI do not use this timelimit option myself, but it would surely be nice to be able to know whether the value of the variables when GLPK stops has any meaning.... Would it be possible to return a MIPSolverException or something similar when the computations stopped because of time ? It could probably detected along with the other ones (unbounded/impossible/no integer solution/...).\n\nNathann",
    "created_at": "2012-04-12T20:53:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184693",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:30'></a>Helloooooooo John !!

I see no new method in your patch, but it is a bit hard to see what you *changed* inside of this large patch. By the way, if you modify your patch while mine already applies on top of it, there's a chance that it will break afterwards `^^;`

I do not use this timelimit option myself, but it would surely be nice to be able to know whether the value of the variables when GLPK stops has any meaning.... Would it be possible to return a MIPSolverException or something similar when the computations stopped because of time ? It could probably detected along with the other ones (unbounded/impossible/no integer solution/...).

Nathann



---

archive/issue_comments_184694.json:
```json
{
    "body": "<a id='comment:31'></a>Replying to [ncohen](#comment%3A30):\n> I see no new method in your patch...\n\n\nCorrect. There wasn't one. I only changed a doctest.\n\n> ...but it is a bit hard to see what you *changed* inside of this large patch. By the way, if you modify your patch while mine already applies on top of it, there's a chance that it will break afterwards `^^;`\n\n\nI've tried it with your patch on top, too. `:-)`\n\n> Would it be possible to return a MIPSolverException or something similar when the computations stopped because of time ?\n\n\nOf course. I'll do that.",
    "created_at": "2012-04-12T22:07:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184694",
    "user": "https://github.com/johnperry-math"
}
```

<a id='comment:31'></a>Replying to [ncohen](#comment%3A30):
> I see no new method in your patch...


Correct. There wasn't one. I only changed a doctest.

> ...but it is a bit hard to see what you *changed* inside of this large patch. By the way, if you modify your patch while mine already applies on top of it, there's a chance that it will break afterwards `^^;`


I've tried it with your patch on top, too. `:-)`

> Would it be possible to return a MIPSolverException or something similar when the computations stopped because of time ?


Of course. I'll do that.



---

archive/issue_comments_184695.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2012-04-12T22:37:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184695",
    "user": "https://github.com/johnperry-math"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_184696.json:
```json
{
    "body": "<a id='comment:32'></a>Attachment [trac_12736_more_solver_options_for_glpk.patch](tarball://root/attachments/some-uuid/ticket12736/trac_12736_more_solver_options_for_glpk.patch) by @johnperry-math created at 2012-04-12 22:37:18\n\nI've uploaded a new patch that restores things as they were.\n\nAlso, perhaps you wrote the wrong thing earlier: it occurred to me later that I might have uploaded the wrong patch... This one is definitely different, as you can find the GLP_ETMLIM and GLP_EITLIM signals used.\n\nI looked at the generated html documentation, and it looked great! So, if you're okay with the current patch, I think we can award it a positive review?",
    "created_at": "2012-04-12T22:37:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184696",
    "user": "https://github.com/johnperry-math"
}
```

<a id='comment:32'></a>Attachment [trac_12736_more_solver_options_for_glpk.patch](tarball://root/attachments/some-uuid/ticket12736/trac_12736_more_solver_options_for_glpk.patch) by @johnperry-math created at 2012-04-12 22:37:18

I've uploaded a new patch that restores things as they were.

Also, perhaps you wrote the wrong thing earlier: it occurred to me later that I might have uploaded the wrong patch... This one is definitely different, as you can find the GLP_ETMLIM and GLP_EITLIM signals used.

I looked at the generated html documentation, and it looked great! So, if you're okay with the current patch, I think we can award it a positive review?



---

archive/issue_comments_184697.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2012-04-13T08:12:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184697",
    "user": "https://github.com/nathanncohen"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_184698.json:
```json
{
    "body": "<a id='comment:33'></a>Helloooooooo John !!\n\nWell, if you also agree with my changes then I guess this patch can go ! I regret that it is hard to test these timelimit exceptions, though I did a test myself by modifying a hard LP and solving it with a time limit. The exception is indeed thrown, and besides the last message printed by GLPK is \n\n```\nTIME LIMIT EXCEEDED; SEARCH TERMINATED\n```\nSo well... It will show anyway `:-)`\n\nOn the practical side, all doctests pass with GLPK as the default solver, the documentation is nice, well. Good addition `:-)`\n\nNathann",
    "created_at": "2012-04-13T08:12:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184698",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:33'></a>Helloooooooo John !!

Well, if you also agree with my changes then I guess this patch can go ! I regret that it is hard to test these timelimit exceptions, though I did a test myself by modifying a hard LP and solving it with a time limit. The exception is indeed thrown, and besides the last message printed by GLPK is 

```
TIME LIMIT EXCEEDED; SEARCH TERMINATED
```
So well... It will show anyway `:-)`

On the practical side, all doctests pass with GLPK as the default solver, the documentation is nice, well. Good addition `:-)`

Nathann



---

archive/issue_comments_184699.json:
```json
{
    "body": "<a id='comment:34'></a>Please fill in the Author/Reviewer fields with your real names.",
    "created_at": "2012-04-16T12:31:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184699",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:34'></a>Please fill in the Author/Reviewer fields with your real names.



---

archive/issue_comments_184700.json:
```json
{
    "body": "Changing author from \"\" to \"John Perry, Nathann Cohen\"",
    "created_at": "2012-04-16T13:49:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184700",
    "user": "https://github.com/johnperry-math"
}
```

Changing author from "" to "John Perry, Nathann Cohen"



---

archive/issue_comments_184701.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,6 +1,6 @@\n Several requests for solver options to use as parameters for GLPK prompt us to allow the user to set quite a few parameters.\n \n-In addition, the name for the solver paramter in ticket #12309 (`preprocessing`) isn't very accurate (we didn't think it through carefully enough at the time) so that option will change, too.\n+In addition, the name for the solver parameter in ticket #12309 (`preprocessing`) isn't very accurate (we didn't think it through carefully enough at the time) so that option will change, too.\n \n I propose to change/enable the following solver options. See the GLPK documentation for details on each option, and note that most have two names: one to correspond to GLPK, one for legibility.\n \n``````\n",
    "created_at": "2012-04-16T13:49:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184701",
    "user": "https://github.com/johnperry-math"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,6 +1,6 @@
 Several requests for solver options to use as parameters for GLPK prompt us to allow the user to set quite a few parameters.
 
-In addition, the name for the solver paramter in ticket #12309 (`preprocessing`) isn't very accurate (we didn't think it through carefully enough at the time) so that option will change, too.
+In addition, the name for the solver parameter in ticket #12309 (`preprocessing`) isn't very accurate (we didn't think it through carefully enough at the time) so that option will change, too.
 
 I propose to change/enable the following solver options. See the GLPK documentation for details on each option, and note that most have two names: one to correspond to GLPK, one for legibility.
 
``````




---

archive/issue_comments_184702.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Nathann Cohen, John Perry\"",
    "created_at": "2012-04-16T13:49:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184702",
    "user": "https://github.com/johnperry-math"
}
```

Changing reviewer from "" to "Nathann Cohen, John Perry"



---

archive/issue_events_034617.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-04-19T09:46:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "milestone": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12736#event-34617"
}
```



---

archive/issue_events_034618.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-06T12:15:58Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12736#event-34618"
}
```



---

archive/issue_comments_184703.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2012-05-06T12:15:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184703",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: fixed



---

archive/issue_comments_184704.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-5.1.beta0\"",
    "created_at": "2012-05-06T12:15:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12736#issuecomment-184704",
    "user": "https://github.com/jdemeyer"
}
```

Changing merged from "" to "sage-5.1.beta0"
