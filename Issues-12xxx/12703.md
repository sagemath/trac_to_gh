...
<a id='comment:2'></a>## To also motivate the upgrade:
...
```json
{
    "body": "<a id='comment:3'></a>The new p1 spkg fixes a potential error if `CPPFLAGS` aren't exported.  (`sage-env` currently **doesn't** export `CPPFLAGS`, which I assumed it also does.)",
...
<a id='comment:3'></a>The new p1 spkg fixes a potential error if `CPPFLAGS` aren't exported.  (`sage-env` currently **doesn't** export `CPPFLAGS`, which I assumed it also does.)
...
```json
{
    "body": "<a id='comment:5'></a>Well, this one works on my computer, and all tests pass in graph/ and numerical/ folders `:-)`\n\nNathann",
...
<a id='comment:5'></a>Well, this one works on my computer, and all tests pass in graph/ and numerical/ folders `:-)`
...
<a id='comment:6'></a>Attachment [glpk-4.44.p0-4.47.p1.diff](tarball://root/attachments/some-uuid/ticket12703/glpk-4.44.p0-4.47.p1.diff) by @nexttime created at 2012-03-22 21:04:36
...
<a id='comment:7'></a>I am currently making an experiment with sage-5.0.beta8:
...
<a id='comment:8'></a>Slightly off the ticket's topic:
...
<a id='comment:16'></a>I had a short communication with Hans Schönemann (Singular developer) and he said it is important that the `-flto` flag is not only part of CCFLAGS but also of LDFLAGS. But I did not define LDFLAGS. Hence, as soon as I know how long `sage -testall` needs in the given setting, I will download sage-5.0.beta9, and try again, but this time with adding `LDFLAGS=-lto`.
...
```json
{
    "body": "<a id='comment:18'></a>For the record: With my attempt to build sage-5.0.beta9 plus gcc with graphite and lto support and with -flto, the original GLPK spkg did not build (that is what this ticket is about) -- but apparently the spkg from here *did* build!",
...
<a id='comment:18'></a>For the record: With my attempt to build sage-5.0.beta9 plus gcc with graphite and lto support and with -flto, the original GLPK spkg did not build (that is what this ticket is about) -- but apparently the spkg from here *did* build!
...
<a id='comment:20'></a>But helas, even the new Singular spkg from 5.0-beta9 (namely singular-3-1-3-3.p6.spkg) failed to build. I used
...
```json
{
    "body": "<a id='comment:24'></a>See #12741: I found a similar problem with R.",
...
<a id='comment:24'></a>See #12741: I found a similar problem with R.
...
```json
{
    "body": "<a id='comment:29'></a>Upstream is now at 4.55.",
...
<a id='comment:29'></a>Upstream is now at 4.55.
...
```json
{
    "body": "<a id='comment:30'></a>For GLPK included zlib, see Fedora's choice: http://pkgs.fedoraproject.org/cgit/glpk.git/commit/?id=f00815ed66d7e71c48afba9cdcb20575a49c38bd",
...
<a id='comment:30'></a>For GLPK included zlib, see Fedora's choice: http://pkgs.fedoraproject.org/cgit/glpk.git/commit/?id=f00815ed66d7e71c48afba9cdcb20575a49c38bd
...
<a id='comment:31'></a>What version of autotools did you use to reconfigure the build system?
Even with quite recent ones, I still get a failing behavior...
That is a test with nm fails iduring configure, but the build goes on and fails during the final linking.
...
```json
{
    "body": "<a id='comment:32'></a>(That is when trying to build with \"CFLAGS=-flto ./sage -flto glpk\" with glpk 4.55.)",
...
<a id='comment:32'></a>(That is when trying to build with "CFLAGS=-flto ./sage -flto glpk" with glpk 4.55.)
...
```json
{
    "body": "<a id='comment:33'></a>I attached the spkg-src script used...",
...
<a id='comment:33'></a>I attached the spkg-src script used...
...
```json
{
    "body": "<a id='comment:34'></a>Ok, using `CC=gcc -flto` works...",
...
<a id='comment:34'></a>Ok, using `CC=gcc -flto` works...
...
```json
{
    "body": "<a id='comment:35'></a>But it seems GLPK does not care about CC.",
...
<a id='comment:35'></a>But it seems GLPK does not care about CC.
...
```json
{
    "body": "<a id='comment:36'></a>Or I just don't know enough about LTO to understand anything.",
...
<a id='comment:36'></a>Or I just don't know enough about LTO to understand anything.
...
```json
{
    "body": "<a id='comment:37'></a>Ok, in fact, the behovior of libtool changes when CC=gcc -flto, in particular the bugging nm command with pipes includes some __gnu_lto thingy.",
...
<a id='comment:37'></a>Ok, in fact, the behovior of libtool changes when CC=gcc -flto, in particular the bugging nm command with pipes includes some __gnu_lto thingy.
...
<a id='comment:38'></a>`@`leif: some feedback?
At the moment I'll assume that `CC=gcc -flto` is the way to go, so upstream version 4.55 is ok.
...
<a id='comment:39'></a>Replying to [comment:31 jpflori]:
> What version of autotools did you use to reconfigure the build system?
...
<a id='comment:40'></a>Everything I used was quite recent:
* GCC 4.9.2
* GNU nm version 2.23.51.0.1-7.fc18 20120806, not so new but should support LTO (also note this ticket started back earlier than march 2012...)
* Debian experimental autotools (not sure of the version but definitely post 2010/2012)
...
<a id='comment:41'></a>Could you give the upstream GLPK 4.55 a try?
The autotools stuff in there should be quite recent and so have no reason to fail.
...
<a id='comment:42'></a>Ahem, from `SPKG.txt`:
...
<a id='comment:43'></a>I don't get your post...
...
```json
{
    "body": "<a id='comment:44'></a>I you just wanted to point out that building with `CFLAGS=-flto` should work, then something very fishy is going on...\nOr it's just that binutils 2.23 don't support LTO :)",
...
<a id='comment:44'></a>I you just wanted to point out that building with `CFLAGS=-flto` should work, then something very fishy is going on...
Or it's just that binutils 2.23 don't support LTO :)
...
<a id='comment:46'></a>I checked that on my systems all the autotools stuff are recent enough, so it is not the problem, unless newer version of some of these tools got broken back.
...
<a id='comment:47'></a>Replying to [comment:44 jpflori]:
> I you just wanted to point out that building with `CFLAGS=-flto` should work, then something very fishy is going on...
...
<a id='comment:49'></a>In fact in config.log I see:
...
<a id='comment:50'></a>And this is nm version:
...
<a id='comment:51'></a>Ok, it seems to be a similar issue as in:
* https://bugzilla.redhat.com/show_bug.cgi?id=1091611
Passing CFLAGS="-flto -ffat-lto-objects" works fine...
...
```json
{
    "body": "<a id='comment:52'></a>Ah, so it's *the French* one that doesn't work! B)\n\n(Not sure what \"cannot run sed ...\" is supposed to mean.)",
...
<a id='comment:52'></a>Ah, so it's *the French* one that doesn't work! B)
...
<a id='comment:53'></a>Yeah, that's boring but out of our hands.
I'll just post my git branch for 4.55 including Cygwin shared lib generation and use of system zlib here tomorrow.
...
<a id='comment:54'></a>FWIW, with vanilla GLPK 4.55, `CFLAGS=-flto` works for me with GCC 4.7.4, but fails with GCC 4.9.1 (both with binutils 2.22).
...
<a id='comment:57'></a>I cannot comment on the build part but the commit addresses a good number of doctest failures I have seen in sage-on-gentoo with glpk 4.54. Only one bit really need to be changed:
...
<a id='comment:59'></a>The statements
...
```json
{
    "body": "<a id='comment:60'></a>Also, `CFALGS` surely is a typo?",
    "created_at": "2014-11-06T09:55:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12703",
    "type": "issue_comment",
...
<a id='comment:60'></a>Also, `CFALGS` surely is a typo?
...
<a id='comment:62'></a>Replying to [comment:58 jdemeyer]:
> Replying to [comment:57 fbissey]:
> > I doubt it would be caught by the tolerance.
...
<a id='comment:63'></a>I doubt that
...
```json
{
    "body": "<a id='comment:64'></a>With the philosophy that packages should be as close to upstream as possible, should we really add the patch the disable compiling the included `zlib`?",
...
<a id='comment:64'></a>With the philosophy that packages should be as close to upstream as possible, should we really add the patch the disable compiling the included `zlib`?
...
```json
{
    "body": "<a id='comment:67'></a>What does the \"Reported upstream. Developers acknowledge bug.\" refer to and where is the upstream report?",
...
<a id='comment:67'></a>What does the "Reported upstream. Developers acknowledge bug." refer to and where is the upstream report?
...
<a id='comment:69'></a>Replying to [comment:66 jpflori]:
> Replying to [comment:64 jdemeyer]:
> > With the philosophy that packages should be as close to upstream as possible, should we really add the patch the disable compiling the included `zlib`?
...
<a id='comment:70'></a>Replying to [comment:59 jdemeyer]:
> The statements
> 
> ```
> * As of version 4.55, zlib is included in the GLPK sources, and *that*
...
```json
{
    "body": "<a id='comment:71'></a>I think if you are going to modify configure and co. you need to generate a new tarball for sage. Patching configure directly is not really sustainable and touching may need to be done with more care. I am quite surprised you weren't bitten by it so far considering that configure.ac comes after configure in most ordering.",
...
<a id='comment:71'></a>I think if you are going to modify configure and co. you need to generate a new tarball for sage. Patching configure directly is not really sustainable and touching may need to be done with more care. I am quite surprised you weren't bitten by it so far considering that configure.ac comes after configure in most ordering.
...
<a id='comment:73'></a>Replying to [comment:72 jpflori]:
> Ensuring that configure.ac was patch before configure and Makefiles.am before Makefiles.in and then playing with m4 files timestamp just drove me crazy.
...
<a id='comment:75'></a>In the "dependencies" part", you can just write
...
```json
{
    "body": "<a id='comment:79'></a>FYI, the cygwin shared lib stuff should go upstream in one form or another.\nSee:\n* http://lists.gnu.org/archive/html/help-glpk/2014-11/msg00000.html",
...
<a id='comment:79'></a>FYI, the cygwin shared lib stuff should go upstream in one form or another.
See:
* http://lists.gnu.org/archive/html/help-glpk/2014-11/msg00000.html
...
```json
{
    "body": "<a id='comment:80'></a>There is something wrong on Cygwin with the shared lib stuff....",
    "created_at": "2014-11-12T15:03:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12703",
    "type": "issue_comment",
...
<a id='comment:80'></a>There is something wrong on Cygwin with the shared lib stuff....
...
<a id='comment:84'></a>Ok, this is now fixed and confirmed to work on Cygwin and ppc64.
Sorry about the forced push, I included some spurious autotools stuff in the previous commit.
...
```json
{
    "body": "<a id='comment:86'></a>The `-0.0` looks like numerical noise, its probably better to use `# abs tol 1e-15` or so. Rest looks good to me.",
...
<a id='comment:86'></a>The `-0.0` looks like numerical noise, its probably better to use `# abs tol 1e-15` or so. Rest looks good to me.
...
```json
{
    "body": "<a id='comment:88'></a>Tests modified.",
...
<a id='comment:88'></a>Tests modified.
...
```json
{
    "body": "<a id='comment:91'></a>Apart from that one the buildbot didn't find any other failures.",
...
<a id='comment:91'></a>Apart from that one the buildbot didn't find any other failures.
...
```json
{
    "body": "<a id='comment:97'></a>I just don't feel motivated to maintained a patch which isn't actually needed.",
...
<a id='comment:97'></a>I just don't feel motivated to maintained a patch which isn't actually needed.
