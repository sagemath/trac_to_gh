# Issue 12459: Interrupt test failures on OS X 10.7

archive/issues_012287.json:
```json
{
    "assignees": [
        "https://github.com/jdemeyer"
    ],
    "body": "There are several failures in `sage/tests/interrupt.pyx` on OS X 10.7 compiled with XCode.\n\nwontfix since #12369 doesn't have this problem.\n\nReviewer: **Jeroen Demeyer**\n\nComponent: **c_lib**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12459_\n\n",
    "closed_at": "2012-05-03T12:08:40Z",
    "created_at": "2012-02-07T10:16:55Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20c%20lib",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Interrupt test failures on OS X 10.7",
    "type": "issue",
    "updated_at": "2012-05-03T12:08:40Z",
    "url": "https://github.com/sagemath/sage/issues/12459",
    "user": "https://github.com/jdemeyer"
}
```
There are several failures in `sage/tests/interrupt.pyx` on OS X 10.7 compiled with XCode.

wontfix since #12369 doesn't have this problem.

Reviewer: **Jeroen Demeyer**

Component: **c_lib**

_Issue created by migration from https://trac.sagemath.org/ticket/12459_





---

archive/issue_events_170497.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-07T10:16:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12459",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12459#event-170497"
}
```



---

archive/issue_events_170498.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-07T10:16:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12459",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20c%20lib",
    "label_color": "000080",
    "label_name": "c: c lib",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12459#event-170498"
}
```



---

archive/issue_events_170499.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-07T10:16:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12459",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12459#event-170499"
}
```



---

archive/issue_events_170500.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-07T10:16:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12459",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12459#event-170500"
}
```



---

archive/issue_events_170501.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-07T10:16:55Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/12459",
    "subject": "https://github.com/jdemeyer",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12459#event-170501"
}
```



---

archive/issue_comments_137626.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nThere are also test failures for this file when compiled with the default XCode compiler.  See [this comment](https://github.com/sagemath/sage/issues/11881#comment:54) for details.  See also [this comment](https://github.com/sagemath/sage/issues/11881#comment:70).",
    "created_at": "2012-02-07T15:29:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12459",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12459#issuecomment-137626",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:1" align="right">comment:1</div>

There are also test failures for this file when compiled with the default XCode compiler.  See [this comment](https://github.com/sagemath/sage/issues/11881#comment:54) for details.  See also [this comment](https://github.com/sagemath/sage/issues/11881#comment:70).



---

archive/issue_comments_137627.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-There are several failures in `sage/tests/interrupt.pyx` on OS X 10.7, at least when compiled with a custom GCC (#12369).\n+There are several failures in `sage/tests/interrupt.pyx` on OS X 10.7 compiled with XCode.\n``````\n",
    "created_at": "2012-02-07T15:51:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12459",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12459#issuecomment-137627",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-There are several failures in `sage/tests/interrupt.pyx` on OS X 10.7, at least when compiled with a custom GCC (#12369).
+There are several failures in `sage/tests/interrupt.pyx` on OS X 10.7 compiled with XCode.
``````




---

archive/issue_comments_137628.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nIn fact, there are failures **only** when using XCode 4.  When I reported this ticket, I accidentally compiled the Sage library with Apple's compiler.  With #12369, interrupts work.",
    "created_at": "2012-02-07T15:51:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12459",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12459#issuecomment-137628",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">comment:2</div>

In fact, there are failures **only** when using XCode 4.  When I reported this ticket, I accidentally compiled the Sage library with Apple's compiler.  With #12369, interrupts work.



---

archive/issue_events_170502.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-02T06:53:31Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12459",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12459#event-170502"
}
```



---

archive/issue_events_170503.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-02T06:53:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12459",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12459#event-170503"
}
```



---

archive/issue_events_170504.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-02T06:53:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12459",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12459#event-170504"
}
```



---

archive/issue_comments_137629.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2012-05-02T06:53:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12459",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12459#issuecomment-137629",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_170505.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-02T06:53:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12459",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12459#event-170505"
}
```



---

archive/issue_comments_137630.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n There are several failures in `sage/tests/interrupt.pyx` on OS X 10.7 compiled with XCode.\n+\n+wontfix since #12369 doesn't have this problem.\n``````\n",
    "created_at": "2012-05-02T06:53:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12459",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12459#issuecomment-137630",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
 There are several failures in `sage/tests/interrupt.pyx` on OS X 10.7 compiled with XCode.
+
+wontfix since #12369 doesn't have this problem.
``````




---

archive/issue_events_170506.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-02T06:53:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12459",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12459#event-170506"
}
```



---

archive/issue_events_170507.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-02T06:53:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12459",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12459#event-170507"
}
```



---

archive/issue_events_170508.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-03T12:08:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12459",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "a6a6a6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12459#event-170508"
}
```



---

archive/issue_events_170509.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-03T12:08:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12459",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12459#event-170509"
}
```



---

archive/issue_events_170510.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-03T12:08:40Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12459",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12459#event-170510"
}
```
