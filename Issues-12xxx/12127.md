# Issue 12127: fix comparison R == Z

archive/issues_011955.json:
```json
{
    "assignees": [],
    "body": "Two methods in schemes/generic/homset.py contain a comparison 'R == Z' instead of 'R == ZZ'. \n\nFixed by #11526.\n\nAssignee: **@aghitza**\n\nCC:  @saraedum @vbraun\n\nKeywords: **sd40.5**\n\nReviewer: **Andrey Novoseltsev, Frithjof Schulze**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12127_\n\n",
    "closed_at": "2012-06-02T13:20:34Z",
    "created_at": "2011-12-08T20:13:51Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20geometry",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fix comparison R == Z",
    "type": "issue",
    "updated_at": "2012-06-02T13:20:34Z",
    "url": "https://github.com/sagemath/sage/issues/12127",
    "user": "https://github.com/sagetrac-fschulze"
}
```
Two methods in schemes/generic/homset.py contain a comparison 'R == Z' instead of 'R == ZZ'. 

Fixed by #11526.

Assignee: **@aghitza**

CC:  @saraedum @vbraun

Keywords: **sd40.5**

Reviewer: **Andrey Novoseltsev, Frithjof Schulze**

_Issue created by migration from https://trac.sagemath.org/ticket/12127_





---

archive/issue_events_150409.json:
```json
{
    "actor": "https://github.com/sagetrac-fschulze",
    "created_at": "2011-12-08T20:13:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12127",
    "milestone_number": null,
    "milestone_title": "sage-4.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12127#event-150409"
}
```



---

archive/issue_events_150410.json:
```json
{
    "actor": "https://github.com/sagetrac-fschulze",
    "created_at": "2011-12-08T20:13:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12127",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20geometry",
    "label_color": "0000ff",
    "label_name": "component: algebraic geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12127#event-150410"
}
```



---

archive/issue_events_150411.json:
```json
{
    "actor": "https://github.com/sagetrac-fschulze",
    "created_at": "2011-12-08T20:13:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12127",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12127#event-150411"
}
```



---

archive/issue_events_150412.json:
```json
{
    "actor": "https://github.com/sagetrac-fschulze",
    "created_at": "2011-12-08T20:13:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12127",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12127#event-150412"
}
```



---

archive/issue_comments_132057.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nApparently this was introduced in [changeset:15345:08d4a1aad8b5].\n\nAbout the doctests, could you provide some example which triggers the bug? We could probably use that for the doctest.",
    "created_at": "2011-12-09T01:37:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12127",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12127#issuecomment-132057",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:1'>Comment 1:</a>
Apparently this was introduced in [changeset:15345:08d4a1aad8b5].

About the doctests, could you provide some example which triggers the bug? We could probably use that for the doctest.



---

archive/issue_comments_132058.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nSee also #11526.",
    "created_at": "2011-12-09T04:11:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12127",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12127#issuecomment-132058",
    "user": "https://github.com/novoselt"
}
```

<a id='comment:2'>Comment 2:</a>
See also #11526.



---

archive/issue_events_150413.json:
```json
{
    "actor": "https://github.com/sagetrac-fschulze",
    "created_at": "2011-12-09T08:55:40Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12127",
    "milestone_number": null,
    "milestone_title": "sage-4.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12127#event-150413"
}
```



---

archive/issue_events_150414.json:
```json
{
    "actor": "https://github.com/sagetrac-fschulze",
    "created_at": "2011-12-09T08:55:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12127",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12127#event-150414"
}
```



---

archive/issue_events_150415.json:
```json
{
    "actor": "https://github.com/sagetrac-fschulze",
    "created_at": "2011-12-09T08:55:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12127",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12127#event-150415"
}
```



---

archive/issue_comments_132059.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nI changed the #11526 patch to also fix the typo in the projective case.",
    "created_at": "2011-12-09T08:55:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12127",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12127#issuecomment-132059",
    "user": "https://github.com/sagetrac-fschulze"
}
```

<a id='comment:3'>Comment 3:</a>
I changed the #11526 patch to also fix the typo in the projective case.



---

archive/issue_events_150416.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2012-05-24T22:52:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12127",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12127#event-150416"
}
```



---

archive/issue_comments_132060.json:
```json
{
    "body": "Changed keywords from **** to **sd40.5**",
    "created_at": "2012-05-24T22:52:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12127",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12127#issuecomment-132060",
    "user": "https://github.com/novoselt"
}
```

Changed keywords from **** to **sd40.5**



---

archive/issue_comments_132061.json:
```json
{
    "body": "Reviewer: **Andrey Novoseltsev**",
    "created_at": "2012-05-24T22:52:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12127",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12127#issuecomment-132061",
    "user": "https://github.com/novoselt"
}
```

Reviewer: **Andrey Novoseltsev**



---

archive/issue_events_150417.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2012-05-24T22:52:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12127",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12127#event-150417"
}
```



---

archive/issue_events_150418.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2012-05-24T22:52:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12127",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12127#event-150418"
}
```



---

archive/issue_comments_132062.json:
```json
{
    "body": "Author: **Frithjof Schulze**",
    "created_at": "2012-05-24T23:10:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12127",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12127#issuecomment-132062",
    "user": "https://github.com/saraedum"
}
```

Author: **Frithjof Schulze**



---

archive/issue_comments_132063.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nReplying to [@saraedum](#comment%3A6):\n\nOops. Should have read what was written in the ticket. Setting an author here doesn't make much sense.",
    "created_at": "2012-05-24T23:12:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12127",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12127#issuecomment-132063",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:7'>Comment 7:</a>
Replying to [@saraedum](#comment%3A6):

Oops. Should have read what was written in the ticket. Setting an author here doesn't make much sense.



---

archive/issue_events_150419.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-26T02:21:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12127",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12127#event-150419"
}
```



---

archive/issue_events_150420.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-26T02:21:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12127",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12127#event-150420"
}
```



---

archive/issue_comments_132064.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nThis needs to be rebased to sage-5.1.beta0:\n\n```\npatching file sage/schemes/generic/homset.py\nHunk #1 FAILED at 159\nHunk #2 FAILED at 218\n2 out of 2 hunks FAILED -- saving rejects to file sage/schemes/generic/homset.py.rej\n```",
    "created_at": "2012-05-26T02:21:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12127",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12127#issuecomment-132064",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:8'>Comment 8:</a>
This needs to be rebased to sage-5.1.beta0:

```
patching file sage/schemes/generic/homset.py
Hunk #1 FAILED at 159
Hunk #2 FAILED at 218
2 out of 2 hunks FAILED -- saving rejects to file sage/schemes/generic/homset.py.rej
```



---

archive/issue_events_150421.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2012-05-26T02:33:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12127",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12127#event-150421"
}
```



---

archive/issue_events_150422.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2012-05-26T02:33:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12127",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12127#event-150422"
}
```



---

archive/issue_comments_132065.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nThis ticket just has to be closed without merging anything!",
    "created_at": "2012-05-26T02:33:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12127",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12127#issuecomment-132065",
    "user": "https://github.com/novoselt"
}
```

<a id='comment:9'>Comment 9:</a>
This ticket just has to be closed without merging anything!



---

archive/issue_comments_132066.json:
```json
{
    "body": "Changed reviewer from **Andrey Novoseltsev** to **Andrey Novoseltsev, Frithjof Schulze**",
    "created_at": "2012-06-02T13:20:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12127",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12127#issuecomment-132066",
    "user": "https://github.com/jdemeyer"
}
```

Changed reviewer from **Andrey Novoseltsev** to **Andrey Novoseltsev, Frithjof Schulze**



---

archive/issue_comments_132067.json:
```json
{
    "body": "Changed author from **Frithjof Schulze** to none",
    "created_at": "2012-06-02T13:20:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12127",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12127#issuecomment-132067",
    "user": "https://github.com/jdemeyer"
}
```

Changed author from **Frithjof Schulze** to none



---

archive/issue_events_150423.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-02T13:20:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12127",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12127#event-150423"
}
```



---

archive/issue_events_150424.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-02T13:20:34Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12127",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12127#event-150424"
}
```



---

archive/issue_events_150425.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-02T13:20:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12127",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12127#event-150425"
}
```



---

archive/issue_comments_132068.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n Two methods in schemes/generic/homset.py contain a comparison 'R == Z' instead of 'R == ZZ'. \n \n-Both methods don't have any doctests.\n+Fixed by #11526.\n``````\n",
    "created_at": "2012-06-02T13:20:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12127",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12127#issuecomment-132068",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
 Two methods in schemes/generic/homset.py contain a comparison 'R == Z' instead of 'R == ZZ'. 
 
-Both methods don't have any doctests.
+Fixed by #11526.
``````

