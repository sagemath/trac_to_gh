# Issue 12245: M4RIE test suite fails due to missing dependencies

archive/issues_012073.json:
```json
{
    "assignees": [],
    "body": "The M4RIE (cf. #9562) test script has dependencies on some libraries which are not made explicit in `spkg/standard/deps` or `SPKG.txt`. These include (at least) Givaro, NTL and GMP/MPIR. Since the Sage build script doesn't know about these dependencies, this means that building Sage in parallel with `SAGE_CHECK=yes` can fail.\n\n**Apply** [attachment: trac12245_m4rie_fix_deps.2.patch.gz](https://github.com/sagemath/sage/files/ticket12245/trac12245_m4rie_fix_deps.2.patch.gz) to Sage root repository.\n\n**Install** http://sage.math.washington.edu/home/malb/spkgs/libm4rie-20111004.p1.spkg\n\nCC:  @malb\n\nKeywords: **m4rie**\n\nAuthor: **Martin Albrecht, David Loeffler**\n\nReviewer: **Martin Albrecht, David Loeffler**\n\nUpstream: **None of the above - read trac for reasoning.**\n\nMerged: **sage-4.8.rc0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12245_\n\n",
    "closed_at": "2012-01-03T08:49:31Z",
    "created_at": "2012-01-02T10:24:42Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "M4RIE test suite fails due to missing dependencies",
    "type": "issue",
    "updated_at": "2012-01-03T08:49:31Z",
    "url": "https://github.com/sagemath/sage/issues/12245",
    "user": "https://github.com/loefflerd"
}
```
The M4RIE (cf. #9562) test script has dependencies on some libraries which are not made explicit in `spkg/standard/deps` or `SPKG.txt`. These include (at least) Givaro, NTL and GMP/MPIR. Since the Sage build script doesn't know about these dependencies, this means that building Sage in parallel with `SAGE_CHECK=yes` can fail.

**Apply** [attachment: trac12245_m4rie_fix_deps.2.patch.gz](https://github.com/sagemath/sage/files/ticket12245/trac12245_m4rie_fix_deps.2.patch.gz) to Sage root repository.

**Install** http://sage.math.washington.edu/home/malb/spkgs/libm4rie-20111004.p1.spkg

CC:  @malb

Keywords: **m4rie**

Author: **Martin Albrecht, David Loeffler**

Reviewer: **Martin Albrecht, David Loeffler**

Upstream: **None of the above - read trac for reasoning.**

Merged: **sage-4.8.rc0**

_Issue created by migration from https://trac.sagemath.org/ticket/12245_





---

archive/issue_events_152357.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2012-01-02T10:24:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12245",
    "milestone_number": null,
    "milestone_title": "sage-4.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12245#event-152357"
}
```



---

archive/issue_events_152358.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2012-01-02T10:24:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12245",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12245#event-152358"
}
```



---

archive/issue_events_152359.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2012-01-02T10:24:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12245",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12245#event-152359"
}
```



---

archive/issue_events_152360.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2012-01-02T10:24:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12245",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12245#event-152360"
}
```



---

archive/issue_comments_134746.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-The M4RIE test script has dependencies on some libraries which are not made explicit in `spkg/standard/deps` or `SPKG.txt`. These include (at least) Givaro, NTL and GMP/MPIR. Since the Sage build script doesn't know about these dependencies, this means that building Sage in parallel with `SAGE_CHECK=yes` can fail.\n+The M4RIE (cf. #9562) test script has dependencies on some libraries which are not made explicit in `spkg/standard/deps` or `SPKG.txt`. These include (at least) Givaro, NTL and GMP/MPIR. Since the Sage build script doesn't know about these dependencies, this means that building Sage in parallel with `SAGE_CHECK=yes` can fail.\n``````\n",
    "created_at": "2012-01-02T10:28:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12245",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12245#issuecomment-134746",
    "user": "https://github.com/malb"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-The M4RIE test script has dependencies on some libraries which are not made explicit in `spkg/standard/deps` or `SPKG.txt`. These include (at least) Givaro, NTL and GMP/MPIR. Since the Sage build script doesn't know about these dependencies, this means that building Sage in parallel with `SAGE_CHECK=yes` can fail.
+The M4RIE (cf. #9562) test script has dependencies on some libraries which are not made explicit in `spkg/standard/deps` or `SPKG.txt`. These include (at least) Givaro, NTL and GMP/MPIR. Since the Sage build script doesn't know about these dependencies, this means that building Sage in parallel with `SAGE_CHECK=yes` can fail.
``````




---

archive/issue_comments_134747.json:
```json
{
    "body": "Attachment: **[trac12245_m4rie_fix_deps.patch.gz](https://github.com/sagemath/sage/files/ticket12245/trac12245_m4rie_fix_deps.patch.gz)**",
    "created_at": "2012-01-02T10:31:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12245",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12245#issuecomment-134747",
    "user": "https://github.com/malb"
}
```

Attachment: **[trac12245_m4rie_fix_deps.patch.gz](https://github.com/sagemath/sage/files/ticket12245/trac12245_m4rie_fix_deps.patch.gz)**



---

archive/issue_comments_134748.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nthe attached patch should fix the issue but I haven't tested it.",
    "created_at": "2012-01-02T10:32:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12245",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12245#issuecomment-134748",
    "user": "https://github.com/malb"
}
```

<a id='comment:2'>Comment 2:</a>
the attached patch should fix the issue but I haven't tested it.



---

archive/issue_events_152361.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2012-01-02T10:32:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12245",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12245#event-152361"
}
```



---

archive/issue_comments_134749.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n The M4RIE (cf. #9562) test script has dependencies on some libraries which are not made explicit in `spkg/standard/deps` or `SPKG.txt`. These include (at least) Givaro, NTL and GMP/MPIR. Since the Sage build script doesn't know about these dependencies, this means that building Sage in parallel with `SAGE_CHECK=yes` can fail.\n+\n+Apply [attachment: trac12245_m4rie_fix_deps.patch.gz](https://github.com/sagemath/sage/files/ticket12245/trac12245_m4rie_fix_deps.patch.gz) to Sage root repository.\n``````\n",
    "created_at": "2012-01-02T10:32:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12245",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12245#issuecomment-134749",
    "user": "https://github.com/malb"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
 The M4RIE (cf. #9562) test script has dependencies on some libraries which are not made explicit in `spkg/standard/deps` or `SPKG.txt`. These include (at least) Givaro, NTL and GMP/MPIR. Since the Sage build script doesn't know about these dependencies, this means that building Sage in parallel with `SAGE_CHECK=yes` can fail.
+
+Apply [attachment: trac12245_m4rie_fix_deps.patch.gz](https://github.com/sagemath/sage/files/ticket12245/trac12245_m4rie_fix_deps.patch.gz) to Sage root repository.
``````




---

archive/issue_comments_134750.json:
```json
{
    "body": "Upstream: **None of the above - read trac for reasoning.**",
    "created_at": "2012-01-02T10:32:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12245",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12245#issuecomment-134750",
    "user": "https://github.com/malb"
}
```

Upstream: **None of the above - read trac for reasoning.**



---

archive/issue_comments_134751.json:
```json
{
    "body": "Author: **Martin Albrecht**",
    "created_at": "2012-01-02T10:32:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12245",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12245#issuecomment-134751",
    "user": "https://github.com/malb"
}
```

Author: **Martin Albrecht**



---

archive/issue_comments_134752.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nShould we not change SPKG.txt to match?",
    "created_at": "2012-01-02T10:34:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12245",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12245#issuecomment-134752",
    "user": "https://github.com/loefflerd"
}
```

<a id='comment:3'>Comment 3:</a>
Should we not change SPKG.txt to match?



---

archive/issue_comments_134753.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nPatch does not apply (conflicts with #12016)",
    "created_at": "2012-01-02T10:39:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12245",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12245#issuecomment-134753",
    "user": "https://github.com/loefflerd"
}
```

<a id='comment:4'>Comment 4:</a>
Patch does not apply (conflicts with #12016)



---

archive/issue_events_152362.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2012-01-02T10:39:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12245",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12245#event-152362"
}
```



---

archive/issue_events_152363.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2012-01-02T10:39:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12245",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12245#event-152363"
}
```



---

archive/issue_comments_134754.json:
```json
{
    "body": "Attachment: **[trac12245_m4rie_fix_deps.2.patch.gz](https://github.com/sagemath/sage/files/ticket12245/trac12245_m4rie_fix_deps.2.patch.gz)**\n\nsage root repo -- apply only this patch",
    "created_at": "2012-01-02T11:51:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12245",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12245#issuecomment-134754",
    "user": "https://github.com/loefflerd"
}
```

Attachment: **[trac12245_m4rie_fix_deps.2.patch.gz](https://github.com/sagemath/sage/files/ticket12245/trac12245_m4rie_fix_deps.2.patch.gz)**

sage root repo -- apply only this patch



---

archive/issue_comments_134755.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nHere's an essentially equivalent patch which will apply to versions from 4.8.alpha5 onwards. But we should still fix the spkg.txt file.",
    "created_at": "2012-01-02T11:52:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12245",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12245#issuecomment-134755",
    "user": "https://github.com/loefflerd"
}
```

<a id='comment:5'>Comment 5:</a>
Here's an essentially equivalent patch which will apply to versions from 4.8.alpha5 onwards. But we should still fix the spkg.txt file.



---

archive/issue_comments_134756.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nAh, thanks. I was in the process of building alpha5. I'll provide an updated SPKG with an updated SPKG.txt.",
    "created_at": "2012-01-02T12:02:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12245",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12245#issuecomment-134756",
    "user": "https://github.com/malb"
}
```

<a id='comment:6'>Comment 6:</a>
Ah, thanks. I was in the process of building alpha5. I'll provide an updated SPKG with an updated SPKG.txt.



---

archive/issue_comments_134757.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,5 @@\n The M4RIE (cf. #9562) test script has dependencies on some libraries which are not made explicit in `spkg/standard/deps` or `SPKG.txt`. These include (at least) Givaro, NTL and GMP/MPIR. Since the Sage build script doesn't know about these dependencies, this means that building Sage in parallel with `SAGE_CHECK=yes` can fail.\n \n-Apply [attachment: trac12245_m4rie_fix_deps.patch.gz](https://github.com/sagemath/sage/files/ticket12245/trac12245_m4rie_fix_deps.patch.gz) to Sage root repository.\n+**Apply** [attachment: trac12245_m4rie_fix_deps.patch.gz](https://github.com/sagemath/sage/files/ticket12245/trac12245_m4rie_fix_deps.patch.gz) to Sage root repository.\n+\n+**Install** http://sage.math.washington.edu/home/malb/spkgs/libm4rie-20111004.p1.spkg\n``````\n",
    "created_at": "2012-01-02T12:20:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12245",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12245#issuecomment-134757",
    "user": "https://github.com/malb"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,5 @@
 The M4RIE (cf. #9562) test script has dependencies on some libraries which are not made explicit in `spkg/standard/deps` or `SPKG.txt`. These include (at least) Givaro, NTL and GMP/MPIR. Since the Sage build script doesn't know about these dependencies, this means that building Sage in parallel with `SAGE_CHECK=yes` can fail.
 
-Apply [attachment: trac12245_m4rie_fix_deps.patch.gz](https://github.com/sagemath/sage/files/ticket12245/trac12245_m4rie_fix_deps.patch.gz) to Sage root repository.
+**Apply** [attachment: trac12245_m4rie_fix_deps.patch.gz](https://github.com/sagemath/sage/files/ticket12245/trac12245_m4rie_fix_deps.patch.gz) to Sage root repository.
+
+**Install** http://sage.math.washington.edu/home/malb/spkgs/libm4rie-20111004.p1.spkg
``````




---

archive/issue_comments_134758.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nNew SPKG here: http://sage.math.washington.edu/home/malb/spkgs/libm4rie-20111004.p1.spkg",
    "created_at": "2012-01-02T12:20:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12245",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12245#issuecomment-134758",
    "user": "https://github.com/malb"
}
```

<a id='comment:7'>Comment 7:</a>
New SPKG here: http://sage.math.washington.edu/home/malb/spkgs/libm4rie-20111004.p1.spkg



---

archive/issue_events_152364.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2012-01-02T12:29:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12245",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12245#event-152364"
}
```



---

archive/issue_events_152365.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2012-01-02T12:29:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12245",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12245#event-152365"
}
```



---

archive/issue_comments_134759.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nI'm happy with the changes to SPKG.txt, so if you are happy with the deps patch, everybody's happy :-)",
    "created_at": "2012-01-02T12:29:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12245",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12245#issuecomment-134759",
    "user": "https://github.com/loefflerd"
}
```

<a id='comment:8'>Comment 8:</a>
I'm happy with the changes to SPKG.txt, so if you are happy with the deps patch, everybody's happy :-)



---

archive/issue_comments_134760.json:
```json
{
    "body": "Changed author from **Martin Albrecht** to **Martin Albrecht, David Loeffler**",
    "created_at": "2012-01-02T12:29:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12245",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12245#issuecomment-134760",
    "user": "https://github.com/loefflerd"
}
```

Changed author from **Martin Albrecht** to **Martin Albrecht, David Loeffler**



---

archive/issue_events_152366.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2012-01-02T12:44:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12245",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12245#event-152366"
}
```



---

archive/issue_events_152367.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2012-01-02T12:44:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12245",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12245#event-152367"
}
```



---

archive/issue_comments_134761.json:
```json
{
    "body": "Reviewer: **Martin Albrecht, David Loeffler**",
    "created_at": "2012-01-02T12:44:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12245",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12245#issuecomment-134761",
    "user": "https://github.com/malb"
}
```

Reviewer: **Martin Albrecht, David Loeffler**



---

archive/issue_comments_134762.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nEverybody happy",
    "created_at": "2012-01-02T12:44:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12245",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12245#issuecomment-134762",
    "user": "https://github.com/malb"
}
```

<a id='comment:10'>Comment 10:</a>
Everybody happy



---

archive/issue_comments_134763.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,5 +1,5 @@\n The M4RIE (cf. #9562) test script has dependencies on some libraries which are not made explicit in `spkg/standard/deps` or `SPKG.txt`. These include (at least) Givaro, NTL and GMP/MPIR. Since the Sage build script doesn't know about these dependencies, this means that building Sage in parallel with `SAGE_CHECK=yes` can fail.\n \n-**Apply** [attachment: trac12245_m4rie_fix_deps.patch.gz](https://github.com/sagemath/sage/files/ticket12245/trac12245_m4rie_fix_deps.patch.gz) to Sage root repository.\n+**Apply** [attachment: trac12245_m4rie_fix_deps.2.patch.gz](https://github.com/sagemath/sage/files/ticket12245/trac12245_m4rie_fix_deps.2.patch.gz) to Sage root repository.\n \n **Install** http://sage.math.washington.edu/home/malb/spkgs/libm4rie-20111004.p1.spkg\n``````\n",
    "created_at": "2012-01-02T19:27:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12245",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12245#issuecomment-134763",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,5 +1,5 @@
 The M4RIE (cf. #9562) test script has dependencies on some libraries which are not made explicit in `spkg/standard/deps` or `SPKG.txt`. These include (at least) Givaro, NTL and GMP/MPIR. Since the Sage build script doesn't know about these dependencies, this means that building Sage in parallel with `SAGE_CHECK=yes` can fail.
 
-**Apply** [attachment: trac12245_m4rie_fix_deps.patch.gz](https://github.com/sagemath/sage/files/ticket12245/trac12245_m4rie_fix_deps.patch.gz) to Sage root repository.
+**Apply** [attachment: trac12245_m4rie_fix_deps.2.patch.gz](https://github.com/sagemath/sage/files/ticket12245/trac12245_m4rie_fix_deps.2.patch.gz) to Sage root repository.
 
 **Install** http://sage.math.washington.edu/home/malb/spkgs/libm4rie-20111004.p1.spkg
``````




---

archive/issue_events_152368.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-01-03T08:49:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12245",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12245#event-152368"
}
```



---

archive/issue_events_152369.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-01-03T08:49:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12245",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12245#event-152369"
}
```



---

archive/issue_comments_134764.json:
```json
{
    "body": "Merged: **sage-4.8.rc0**",
    "created_at": "2012-01-03T08:49:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12245",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12245#issuecomment-134764",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.8.rc0**
