# Issue 12933: Speedup in DiGraph.stronly_connected_components_digraph

archive/issues_012761.json:
```json
{
    "assignees": [],
    "body": "Let it be said that I hate vertex labels.\n\nBefore:\n\n```\nsage: g = digraphs.RandomDirectedGNP(2000,.2)  \nsage: g.strongly_connected_components_digraph()\nException: The user died of old age\n```\n\nAfter :\n\n```\nsage: time g.strongly_connected_components_digraph()\nDigraph on 1 vertex\nTime: CPU 2.78 s, Wall: 2.79 s\n```\n\nWhich is already far, far too much. And if it is still slow it is -- of course -- only because of labels.\n\nNathann\n\nAssignee: **@jasongrout, @nathanncohen, @rlmill**\n\nCC:  @nthiery\n\nReviewer: **David Coudert**\n\nAuthor: **Nathann Cohen**\n\nMerged: **sage-5.1.beta5**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12933_\n\n",
    "closed_at": "2012-06-18T10:24:53Z",
    "created_at": "2012-05-10T09:48:48Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Speedup in DiGraph.stronly_connected_components_digraph",
    "type": "issue",
    "updated_at": "2012-07-05T15:37:01Z",
    "url": "https://github.com/sagemath/sage/issues/12933",
    "user": "https://github.com/nathanncohen"
}
```
Let it be said that I hate vertex labels.

Before:

```
sage: g = digraphs.RandomDirectedGNP(2000,.2)  
sage: g.strongly_connected_components_digraph()
Exception: The user died of old age
```

After :

```
sage: time g.strongly_connected_components_digraph()
Digraph on 1 vertex
Time: CPU 2.78 s, Wall: 2.79 s
```

Which is already far, far too much. And if it is still slow it is -- of course -- only because of labels.

Nathann

Assignee: **@jasongrout, @nathanncohen, @rlmill**

CC:  @nthiery

Reviewer: **David Coudert**

Author: **Nathann Cohen**

Merged: **sage-5.1.beta5**

_Issue created by migration from https://trac.sagemath.org/ticket/12933_





---

archive/issue_events_163792.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2012-05-10T09:48:48Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12933",
    "milestone_number": null,
    "milestone_title": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12933#event-163792"
}
```



---

archive/issue_events_163793.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2012-05-10T09:48:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12933",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12933#event-163793"
}
```



---

archive/issue_events_163794.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2012-05-10T09:48:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12933",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12933#event-163794"
}
```



---

archive/issue_events_163795.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2012-05-10T09:48:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12933",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12933#event-163795"
}
```



---

archive/issue_events_163796.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2012-05-10T09:49:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12933",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12933#event-163796"
}
```



---

archive/issue_comments_148780.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\n(Ninja patch -- nobody will notice that I'm exposing a parameter for graph coloring functions in this ticket `:-p`)",
    "created_at": "2012-05-10T12:22:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12933#issuecomment-148780",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:2'>Comment 2:</a>
(Ninja patch -- nobody will notice that I'm exposing a parameter for graph coloring functions in this ticket `:-p`)



---

archive/issue_comments_148781.json:
```json
{
    "body": "Attachment: **[trac_12933.patch.gz](https://github.com/sagemath/sage/files/ticket12933/trac_12933.patch.gz)**",
    "created_at": "2012-05-10T12:23:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12933#issuecomment-148781",
    "user": "https://github.com/nathanncohen"
}
```

Attachment: **[trac_12933.patch.gz](https://github.com/sagemath/sage/files/ticket12933/trac_12933.patch.gz)**



---

archive/issue_comments_148782.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nThe running time improvement is impressive, although I had no \"user died of old age\" exception. Considering a linear time algorithm, the running time is still high, but I don't know how to be faster in python.\n\nThe patch is working perfectly and passes all tests (graph.py and digraph.py).\nI give positive review.",
    "created_at": "2012-06-02T17:10:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12933#issuecomment-148782",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:3'>Comment 3:</a>
The running time improvement is impressive, although I had no "user died of old age" exception. Considering a linear time algorithm, the running time is still high, but I don't know how to be faster in python.

The patch is working perfectly and passes all tests (graph.py and digraph.py).
I give positive review.



---

archive/issue_comments_148783.json:
```json
{
    "body": "Reviewer: **David Coudert**",
    "created_at": "2012-06-02T17:10:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12933#issuecomment-148783",
    "user": "https://github.com/dcoudert"
}
```

Reviewer: **David Coudert**



---

archive/issue_events_163797.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2012-06-02T17:10:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12933",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12933#event-163797"
}
```



---

archive/issue_events_163798.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2012-06-02T17:10:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12933",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12933#event-163798"
}
```



---

archive/issue_comments_148784.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nThaaaaaaaaaanks ! `:-)`\n\nNathann",
    "created_at": "2012-06-02T17:12:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12933#issuecomment-148784",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:4'>Comment 4:</a>
Thaaaaaaaaaanks ! `:-)`

Nathann



---

archive/issue_comments_148785.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nPlease fill in your real name in the Author / Reviewer fields.",
    "created_at": "2012-06-09T19:21:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12933#issuecomment-148785",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:5'>Comment 5:</a>
Please fill in your real name in the Author / Reviewer fields.



---

archive/issue_comments_148786.json:
```json
{
    "body": "Author: **Nathann Cohen**",
    "created_at": "2012-06-09T19:22:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12933#issuecomment-148786",
    "user": "https://github.com/dcoudert"
}
```

Author: **Nathann Cohen**



---

archive/issue_events_163799.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-18T10:24:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12933",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12933#event-163799"
}
```



---

archive/issue_events_163800.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-18T10:24:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12933",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12933#event-163800"
}
```



---

archive/issue_comments_148787.json:
```json
{
    "body": "Merged: **sage-5.1.beta5**",
    "created_at": "2012-06-18T10:24:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12933#issuecomment-148787",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.1.beta5**



---

archive/issue_comments_148788.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nNathann, you are my hero of the day!\n\nI was running a calculation while Anne was presenting a talk about monoids. My calculation was taking forever (minutes? more? I killed it). I applied your patch, and it went down to 5s. And now I have a little counter example that she can announce at the end of her talk :-)",
    "created_at": "2012-07-05T15:33:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12933#issuecomment-148788",
    "user": "https://github.com/nthiery"
}
```

<a id='comment:8'>Comment 8:</a>
Nathann, you are my hero of the day!

I was running a calculation while Anne was presenting a talk about monoids. My calculation was taking forever (minutes? more? I killed it). I applied your patch, and it went down to 5s. And now I have a little counter example that she can announce at the end of her talk :-)



---

archive/issue_comments_148789.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nWOooooooooooohooooooooooooooooo !!!! I am a HeroooOOOOOOooooooooooooooo !!!! `O_O`\n\nGod ! At long, long last... `:-D`\n\nNathann",
    "created_at": "2012-07-05T15:37:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12933#issuecomment-148789",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:9'>Comment 9:</a>
WOooooooooooohooooooooooooooooo !!!! I am a HeroooOOOOOOooooooooooooooo !!!! `O_O`

God ! At long, long last... `:-D`

Nathann
