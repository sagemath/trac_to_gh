# Issue 12534: Negative exponents of p-adic capped relative zeros are broken

archive/issues_012362.json:
```json
{
    "body": "```\nsage: Zp(5)(0)^-1\n0\nsage: sage: Zp(5)(0,1)^-1\nO(5^-1)\n```\nThese should both raise errors.\n\nAssignee: @roed314\n\nResolution: wontfix\n\nIssue created by migration from https://trac.sagemath.org/ticket/12534\n\n",
    "closed_at": "2012-02-22T20:45:08Z",
    "created_at": "2012-02-18T14:48:40Z",
    "labels": [
        "component: padics",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "Negative exponents of p-adic capped relative zeros are broken",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/12534",
    "user": "https://github.com/roed314"
}
```
```
sage: Zp(5)(0)^-1
0
sage: sage: Zp(5)(0,1)^-1
O(5^-1)
```
These should both raise errors.

Assignee: @roed314

Resolution: wontfix

Issue created by migration from https://trac.sagemath.org/ticket/12534





---

archive/issue_comments_145481.json:
```json
{
    "body": "Resolution: wontfix",
    "created_at": "2012-02-22T20:45:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12534#issuecomment-145481",
    "user": "https://trac.sagemath.org/admin/accounts/users/justin"
}
```

Resolution: wontfix



---

archive/issue_comments_145482.json:
```json
{
    "body": "<a id='comment:1'></a>This is fixed by the patch in #12555, so will not be fixed in the current code.  We could revisit this if #12555 isn't positively reviewed in reasonable time.",
    "created_at": "2012-02-22T20:45:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12534#issuecomment-145482",
    "user": "https://trac.sagemath.org/admin/accounts/users/justin"
}
```

<a id='comment:1'></a>This is fixed by the patch in #12555, so will not be fixed in the current code.  We could revisit this if #12555 isn't positively reviewed in reasonable time.



---

archive/issue_events_033801.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/justin",
    "created_at": "2012-02-22T20:45:08Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/12534",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12534#event-33801"
}
```



---

archive/issue_events_033802.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2012-02-22T22:11:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/12534",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12534#event-33802"
}
```
