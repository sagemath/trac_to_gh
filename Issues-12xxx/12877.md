# Issue 12877: Categories for padics, schemes, and some more rings

archive/issues_012705.json:
```json
{
    "body": "This patch fixes the following classes to use categories:\n\n- padics\n- RealLazyField, ComplexLazyField\n- AlgebraicScheme's and subclasses\n\n#11935 depends on this one\n\nCC:  sage-combinat\n\nReviewer: Simon King\n\nAuthor: Nicolas M. Thi\u00e9ry\n\nMerged: sage-5.1.beta1\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/12877\n\n",
    "closed_at": "2012-05-23T21:36:27Z",
    "created_at": "2012-04-24T22:00:48Z",
    "labels": [
        "component: categories"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-5.1",
    "title": "Categories for padics, schemes, and some more rings",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/12877",
    "user": "https://github.com/nthiery"
}
```
This patch fixes the following classes to use categories:

- padics
- RealLazyField, ComplexLazyField
- AlgebraicScheme's and subclasses

#11935 depends on this one

CC:  sage-combinat

Reviewer: Simon King

Author: Nicolas M. Thiéry

Merged: sage-5.1.beta1

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/12877





---

archive/issue_comments_188244.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2012-04-24T22:02:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12877",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12877#issuecomment-188244",
    "user": "https://github.com/nthiery"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_188245.json:
```json
{
    "body": "<a id='comment:2'></a>Attachment [trac_12877-category-for_more_rings_and_schemes-nt.patch](tarball://root/attachments/some-uuid/ticket12877/trac_12877-category-for_more_rings_and_schemes-nt.patch) by @simon-king-jena created at 2012-04-26 08:19:56\n\nHooray, finally the category of schemes is used in Sage! Question: Are there morphisms of schemes, yet? I am first reviewing #12875 and will then also look at the ticket here.",
    "created_at": "2012-04-26T08:19:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12877",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12877#issuecomment-188245",
    "user": "https://github.com/simon-king-jena"
}
```

<a id='comment:2'></a>Attachment [trac_12877-category-for_more_rings_and_schemes-nt.patch](tarball://root/attachments/some-uuid/ticket12877/trac_12877-category-for_more_rings_and_schemes-nt.patch) by @simon-king-jena created at 2012-04-26 08:19:56

Hooray, finally the category of schemes is used in Sage! Question: Are there morphisms of schemes, yet? I am first reviewing #12875 and will then also look at the ticket here.



---

archive/issue_comments_188246.json:
```json
{
    "body": "<a id='comment:3'></a>Replying to [comment:2 SimonKing]:\n> Hooray, finally the category of schemes is used in Sage! Question: Are there morphisms of schemes, yet? I am first reviewing #12875 and will then also look at the ticket here.\n\n\nApparently yes :-)\n\n```\nsage: sage: k = GF(11)\nsage: E = EllipticCurve(k,[1,1])\nsage: Q = E(6,5)\nsage: phi = E.isogeny(Q)\nsage: phi.parent()\nSet of Morphisms from Abelian group of points on Elliptic Curve defined by y^2 = x^3 + x + 1 over Finite Field of size 11 to Abelian group of points on Elliptic Curve defined by y^2 = x^3 + 7*x + 8 over Finite Field of size 11 in Category of hom sets in Category of Schemes\nsage: phi.parent().homset_category()\nCategory of hom sets in Category of Schemes\n```\n\n(which by the way should really be Category of Schemes; see #12880)",
    "created_at": "2012-04-26T09:32:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12877",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12877#issuecomment-188246",
    "user": "https://github.com/nthiery"
}
```

<a id='comment:3'></a>Replying to [comment:2 SimonKing]:
> Hooray, finally the category of schemes is used in Sage! Question: Are there morphisms of schemes, yet? I am first reviewing #12875 and will then also look at the ticket here.


Apparently yes :-)

```
sage: sage: k = GF(11)
sage: E = EllipticCurve(k,[1,1])
sage: Q = E(6,5)
sage: phi = E.isogeny(Q)
sage: phi.parent()
Set of Morphisms from Abelian group of points on Elliptic Curve defined by y^2 = x^3 + x + 1 over Finite Field of size 11 to Abelian group of points on Elliptic Curve defined by y^2 = x^3 + 7*x + 8 over Finite Field of size 11 in Category of hom sets in Category of Schemes
sage: phi.parent().homset_category()
Category of hom sets in Category of Schemes
```

(which by the way should really be Category of Schemes; see #12880)



---

archive/issue_comments_188247.json:
```json
{
    "body": "<a id='comment:4'></a>As you point out in a comment, due to the custom `__getattr__` method, subclasses of `LazyField` can not be extension classes (because `Parent.__getattr__` is only needed when the class of the parent can not be turned into a subclass of the category's parent_class).\n\nI am sure that you did try something like `getattr(super(self,LazyField),name)`. Did that not work? Why?\n\nBy consequence, you (have to) turn `cdef class ComplexLazyField_class(LazyField)` into `class ComplexLazyField_class(LazyField)`. Did you investigate potential speed penalties?\n\nIn the initialisation of `LocalGeneric`, which inherits from `CommutativeRing`, you still call `Parent.__init__` (with category support added), but not `CommutativeRing.__init__`, which would automatically grant category support. Why?",
    "created_at": "2012-04-26T10:46:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12877",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12877#issuecomment-188247",
    "user": "https://github.com/simon-king-jena"
}
```

<a id='comment:4'></a>As you point out in a comment, due to the custom `__getattr__` method, subclasses of `LazyField` can not be extension classes (because `Parent.__getattr__` is only needed when the class of the parent can not be turned into a subclass of the category's parent_class).

I am sure that you did try something like `getattr(super(self,LazyField),name)`. Did that not work? Why?

By consequence, you (have to) turn `cdef class ComplexLazyField_class(LazyField)` into `class ComplexLazyField_class(LazyField)`. Did you investigate potential speed penalties?

In the initialisation of `LocalGeneric`, which inherits from `CommutativeRing`, you still call `Parent.__init__` (with category support added), but not `CommutativeRing.__init__`, which would automatically grant category support. Why?



---

archive/issue_comments_188248.json:
```json
{
    "body": "Remove assignee @nthiery.",
    "created_at": "2012-04-26T11:04:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12877",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12877#issuecomment-188248",
    "user": "https://github.com/simon-king-jena"
}
```

Remove assignee @nthiery.



---

archive/issue_comments_188249.json:
```json
{
    "body": "<a id='comment:5'></a>Replying to [comment:4 SimonKing]:\n> I am sure that you did try something like `getattr(super(self,LazyField),name)`. Did that not work? Why?\n\n\nI tested (it should be `super(LazyField,self)`), and indeed it does not work. But still, one could add the stuff from the custom `Parent.__getattr__` into the custom `LazyField.__getattr__`:\n\n```\nsage: cython(\"\"\"         \n....: from sage.structure.parent cimport Parent\n....: from sage.structure.parent import getattr_from_other_class\n....: cdef class MyParent(Parent):\n....:     def __getattr__(self, name):\n....:         print \"here is the custom getattr with\", name\n....:         return getattr_from_other_class(self, self.category().parent_class, name)\n....: \"\"\")\nsage: P = MyParent(category=CommutativeAdditiveSemigroups())\nhere is the custom getattr with _element_constructor_\nhere is the custom getattr with Element\nhere is the custom getattr with element_class\nsage: P.addition_table\nhere is the custom getattr with addition_table\nhere is the custom getattr with __custom_name\nhere is the custom getattr with _repr_\n<bound method CommutativeAdditiveSemigroups.parent_class.addition_table of <type '_mnt_local_king__sage_temp_mpc622_12258_tmp_4_spyx_0.MyParent'>>\n```\nThere is also some more stuff in `Parent.__getattr__`, that was written for cached methods.\n\nThe question is whether the code duplication would be worth the effort.",
    "created_at": "2012-04-26T11:04:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12877",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12877#issuecomment-188249",
    "user": "https://github.com/simon-king-jena"
}
```

<a id='comment:5'></a>Replying to [comment:4 SimonKing]:
> I am sure that you did try something like `getattr(super(self,LazyField),name)`. Did that not work? Why?


I tested (it should be `super(LazyField,self)`), and indeed it does not work. But still, one could add the stuff from the custom `Parent.__getattr__` into the custom `LazyField.__getattr__`:

```
sage: cython("""         
....: from sage.structure.parent cimport Parent
....: from sage.structure.parent import getattr_from_other_class
....: cdef class MyParent(Parent):
....:     def __getattr__(self, name):
....:         print "here is the custom getattr with", name
....:         return getattr_from_other_class(self, self.category().parent_class, name)
....: """)
sage: P = MyParent(category=CommutativeAdditiveSemigroups())
here is the custom getattr with _element_constructor_
here is the custom getattr with Element
here is the custom getattr with element_class
sage: P.addition_table
here is the custom getattr with addition_table
here is the custom getattr with __custom_name
here is the custom getattr with _repr_
<bound method CommutativeAdditiveSemigroups.parent_class.addition_table of <type '_mnt_local_king__sage_temp_mpc622_12258_tmp_4_spyx_0.MyParent'>>
```
There is also some more stuff in `Parent.__getattr__`, that was written for cached methods.

The question is whether the code duplication would be worth the effort.



---

archive/issue_comments_188250.json:
```json
{
    "body": "<a id='comment:6'></a>But it is really strange:\n\n```\nsage: MyParent.__getattr__  \n<method '__getattr__' of '_mnt_local_king__sage_temp_mpc622_12258_tmp_4_spyx_0.MyParent' objects>\nsage: Parent.__getattr__  \n---------------------------------------------------------------------------\nAttributeError                            Traceback (most recent call last)\n\n/mnt/local/king/SAGE/experiment/notebook/sage-5.1.notebook/devel/sage-main/<ipython console> in <module>()\n\nAttributeError: type object 'sage.structure.parent.Parent' has no attribute '__getattr__'\n```\n\nSo, why is `Parent.__getattr__` seemingly unavailable, although it is frequently used?",
    "created_at": "2012-04-26T11:08:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12877",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12877#issuecomment-188250",
    "user": "https://github.com/simon-king-jena"
}
```

<a id='comment:6'></a>But it is really strange:

```
sage: MyParent.__getattr__  
<method '__getattr__' of '_mnt_local_king__sage_temp_mpc622_12258_tmp_4_spyx_0.MyParent' objects>
sage: Parent.__getattr__  
---------------------------------------------------------------------------
AttributeError                            Traceback (most recent call last)

/mnt/local/king/SAGE/experiment/notebook/sage-5.1.notebook/devel/sage-main/<ipython console> in <module>()

AttributeError: type object 'sage.structure.parent.Parent' has no attribute '__getattr__'
```

So, why is `Parent.__getattr__` seemingly unavailable, although it is frequently used?



---

archive/issue_comments_188251.json:
```json
{
    "body": "<a id='comment:7'></a>Thanks for reproducing here my unsuccessful attempts :-)\n\nNow is it worth the trouble? It's about the parent, not the elements. In the similar situation for QQ we decided for QQ not to be an extension type. It would be surprising if there would be methods on RLF that would be more critical than on QQ, wouldn't it?\n\nCheers,\n                 Nicolas",
    "created_at": "2012-04-26T11:25:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12877",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12877#issuecomment-188251",
    "user": "https://github.com/nthiery"
}
```

<a id='comment:7'></a>Thanks for reproducing here my unsuccessful attempts :-)

Now is it worth the trouble? It's about the parent, not the elements. In the similar situation for QQ we decided for QQ not to be an extension type. It would be surprising if there would be methods on RLF that would be more critical than on QQ, wouldn't it?

Cheers,
                 Nicolas



---

archive/issue_comments_188252.json:
```json
{
    "body": "<a id='comment:8'></a>Replying to [comment:7 nthiery]:\n> Thanks for reproducing here my unsuccessful attempts :-)\n\n\nYou're welcome; but still I'd like to understand why one can see `__getattr__` of a custom cdef class that is derived from Parent, but can not see `__getattr__` of Parent.",
    "created_at": "2012-04-26T12:00:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12877",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12877#issuecomment-188252",
    "user": "https://github.com/simon-king-jena"
}
```

<a id='comment:8'></a>Replying to [comment:7 nthiery]:
> Thanks for reproducing here my unsuccessful attempts :-)


You're welcome; but still I'd like to understand why one can see `__getattr__` of a custom cdef class that is derived from Parent, but can not see `__getattr__` of Parent.



---

archive/issue_comments_188253.json:
```json
{
    "body": "<a id='comment:9'></a>There only remains one question: Why is `Parent.__init__` and not `CommutativeRing.__init__` called in `LocalGeneric`? Well, I'll test it, and will submit a reviewer patch if it works. With your patch, all doctests pass...",
    "created_at": "2012-04-26T12:05:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12877",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12877#issuecomment-188253",
    "user": "https://github.com/simon-king-jena"
}
```

<a id='comment:9'></a>There only remains one question: Why is `Parent.__init__` and not `CommutativeRing.__init__` called in `LocalGeneric`? Well, I'll test it, and will submit a reviewer patch if it works. With your patch, all doctests pass...



---

archive/issue_comments_188254.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Simon King\"",
    "created_at": "2012-04-26T12:17:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12877",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12877#issuecomment-188254",
    "user": "https://github.com/simon-king-jena"
}
```

Changing reviewer from "" to "Simon King"



---

archive/issue_comments_188255.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2012-04-26T12:17:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12877",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12877#issuecomment-188255",
    "user": "https://github.com/simon-king-jena"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_188256.json:
```json
{
    "body": "<a id='comment:10'></a>I see. Keywords such as element_constructor aren't accepted by `CommutativeRing.__init__`. So, no reviewer patch.\n\nI accept your argument about `RLF` not being of extension type. Since the doctests pass and the patch looks fine, I give it a positive review.\n\nHowever, I'd like to understand how Parent differs from other classes with a custom `__getattr__`.",
    "created_at": "2012-04-26T12:17:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12877",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12877#issuecomment-188256",
    "user": "https://github.com/simon-king-jena"
}
```

<a id='comment:10'></a>I see. Keywords such as element_constructor aren't accepted by `CommutativeRing.__init__`. So, no reviewer patch.

I accept your argument about `RLF` not being of extension type. Since the doctests pass and the patch looks fine, I give it a positive review.

However, I'd like to understand how Parent differs from other classes with a custom `__getattr__`.



---

archive/issue_comments_188257.json:
```json
{
    "body": "<a id='comment:11'></a>Replying to [comment:10 SimonKing]:\n> I accept your argument about `RLF` not being of extension type. Since the doctests pass and the patch looks fine, I give it a positive review.\n\n\nThanks for the quick review!\n\n> However, I'd like to understand how Parent differs from other classes with a custom `__getattr__`.\n\n\nI guess it's a question of extension types versus non extension types.",
    "created_at": "2012-04-26T15:47:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12877",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12877#issuecomment-188257",
    "user": "https://github.com/nthiery"
}
```

<a id='comment:11'></a>Replying to [comment:10 SimonKing]:
> I accept your argument about `RLF` not being of extension type. Since the doctests pass and the patch looks fine, I give it a positive review.


Thanks for the quick review!

> However, I'd like to understand how Parent differs from other classes with a custom `__getattr__`.


I guess it's a question of extension types versus non extension types.



---

archive/issue_comments_188258.json:
```json
{
    "body": "<a id='comment:12'></a>Replying to [comment:11 nthiery]:\n> I guess it's a question of extension types versus non extension types.\n\n\nNo, it isn't.\n\n```\nsage: cython(\"\"\"                  \n....: cdef class MyParent(object):               \n....:     def __getattr__(self, str name):\n....:         raise AttributeError,name\n....: \"\"\")\nsage: MyParent.__getattr__\n<method '__getattr__' of '_mnt_local_king__sage_temp_mpc622_20365_tmp_0_spyx_0.MyParent' objects>\n```\n\nAt least that is what I find with sage-5.1.notebook. I do get an error in an old version of Sage that still uses the old Cython spkg.",
    "created_at": "2012-04-26T16:21:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12877",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12877#issuecomment-188258",
    "user": "https://github.com/simon-king-jena"
}
```

<a id='comment:12'></a>Replying to [comment:11 nthiery]:
> I guess it's a question of extension types versus non extension types.


No, it isn't.

```
sage: cython("""                  
....: cdef class MyParent(object):               
....:     def __getattr__(self, str name):
....:         raise AttributeError,name
....: """)
sage: MyParent.__getattr__
<method '__getattr__' of '_mnt_local_king__sage_temp_mpc622_20365_tmp_0_spyx_0.MyParent' objects>
```

At least that is what I find with sage-5.1.notebook. I do get an error in an old version of Sage that still uses the old Cython spkg.



---

archive/issue_comments_188259.json:
```json
{
    "body": "<a id='comment:13'></a>Replying to [comment:12 SimonKing]:\n> Replying to [comment:11 nthiery]:\n> > I guess it's a question of extension types versus non extension types.\n\n> \n> No, it isn't.\n\n\nOuch, that's weird ...",
    "created_at": "2012-04-26T17:10:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12877",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12877#issuecomment-188259",
    "user": "https://github.com/nthiery"
}
```

<a id='comment:13'></a>Replying to [comment:12 SimonKing]:
> Replying to [comment:11 nthiery]:
> > I guess it's a question of extension types versus non extension types.

> 
> No, it isn't.


Ouch, that's weird ...



---

archive/issue_comments_188260.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2012-05-23T21:36:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12877",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12877#issuecomment-188260",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: fixed



---

archive/issue_events_035123.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-23T21:36:27Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/12877",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12877#event-35123"
}
```



---

archive/issue_comments_188261.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-5.1.beta1\"",
    "created_at": "2012-05-23T21:36:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12877",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12877#issuecomment-188261",
    "user": "https://github.com/jdemeyer"
}
```

Changing merged from "" to "sage-5.1.beta1"
