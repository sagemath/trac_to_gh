# Issue 12451: Syntax highlighting for notebook debugger

archive/issues_012279.json:
```json
{
    "assignees": [
        "https://github.com/jasongrout",
        "https://github.com/qed777",
        "https://github.com/williamstein"
    ],
    "body": "This adds syntax highlighting for the notebook debugger of ticket #7509.\n\nDepends on patch [attachment: trac_7509.5.patch:ticket:7509.](https://github.com/sagemath/sage/files/ticket12451/ccf3a4424a077121bcb5f8d110c0c7bc.gz)\n\nFor reasons of speed only a small part of the source code around the current position is loaded and highlighted. We detect when the\ncode fragment starts in the middle of a docstring to ensure correct\nhighlighting in this case.\n\nCC:  @fchapoton\n\nAuthor: **Wilfried Huss**\n\nComponent: **notebook**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12451_\n\n",
    "closed_at": "2020-09-02T07:48:41Z",
    "created_at": "2012-02-06T11:17:12Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Syntax highlighting for notebook debugger",
    "type": "issue",
    "updated_at": "2020-09-02T07:48:41Z",
    "url": "https://github.com/sagemath/sage/issues/12451",
    "user": "https://github.com/sagetrac-whuss"
}
```
This adds syntax highlighting for the notebook debugger of ticket #7509.

Depends on patch [attachment: trac_7509.5.patch:ticket:7509.](https://github.com/sagemath/sage/files/ticket12451/ccf3a4424a077121bcb5f8d110c0c7bc.gz)

For reasons of speed only a small part of the source code around the current position is loaded and highlighted. We detect when the
code fragment starts in the middle of a docstring to ensure correct
highlighting in this case.

CC:  @fchapoton

Author: **Wilfried Huss**

Component: **notebook**

_Issue created by migration from https://trac.sagemath.org/ticket/12451_





---

archive/issue_events_170294.json:
```json
{
    "actor": "https://github.com/sagetrac-whuss",
    "created_at": "2012-02-06T11:17:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-170294"
}
```



---

archive/issue_events_170295.json:
```json
{
    "actor": "https://github.com/sagetrac-whuss",
    "created_at": "2012-02-06T11:17:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "000080",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-170295"
}
```



---

archive/issue_events_170296.json:
```json
{
    "actor": "https://github.com/sagetrac-whuss",
    "created_at": "2012-02-06T11:17:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-170296"
}
```



---

archive/issue_events_170297.json:
```json
{
    "actor": "https://github.com/sagetrac-whuss",
    "created_at": "2012-02-06T11:17:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-170297"
}
```



---

archive/issue_events_170298.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2012-02-06T11:17:12Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "subject": "https://github.com/sagetrac-whuss",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-170298"
}
```



---

archive/issue_events_170299.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2012-02-06T11:17:12Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "subject": "https://github.com/sagetrac-whuss",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-170299"
}
```



---

archive/issue_events_170300.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-02-06T11:17:12Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "subject": "https://github.com/sagetrac-whuss",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-170300"
}
```



---

archive/issue_comments_137347.json:
```json
{
    "body": "Attachment: **[trac_12451.patch.gz](https://github.com/sagemath/sage/files/ticket12451/trac_12451.patch.gz)**",
    "created_at": "2012-02-06T11:18:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12451#issuecomment-137347",
    "user": "https://github.com/sagetrac-whuss"
}
```

Attachment: **[trac_12451.patch.gz](https://github.com/sagemath/sage/files/ticket12451/trac_12451.patch.gz)**



---

archive/issue_events_170301.json:
```json
{
    "actor": "https://github.com/sagetrac-whuss",
    "created_at": "2012-02-06T11:28:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-170301"
}
```



---

archive/issue_comments_137348.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nPlease fill in your real name as Author.",
    "created_at": "2012-07-27T20:41:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12451#issuecomment-137348",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">comment:2</div>

Please fill in your real name as Author.



---

archive/issue_comments_137349.json:
```json
{
    "body": "Author: **Wilfried Huss**",
    "created_at": "2012-07-30T08:57:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12451#issuecomment-137349",
    "user": "https://github.com/sagetrac-whuss"
}
```

Author: **Wilfried Huss**



---

archive/issue_comments_137350.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nNeeds rebase, and probably an upstream request...",
    "created_at": "2013-06-18T20:06:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12451#issuecomment-137350",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:4" align="right">comment:4</div>

Needs rebase, and probably an upstream request...



---

archive/issue_events_170302.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-06-18T20:06:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-170302"
}
```



---

archive/issue_events_170303.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-06-18T20:06:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-170303"
}
```



---

archive/issue_events_170304.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-170304"
}
```



---

archive/issue_events_170305.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-170305"
}
```



---

archive/issue_events_170306.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-170306"
}
```



---

archive/issue_events_170307.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-170307"
}
```



---

archive/issue_events_170308.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-170308"
}
```



---

archive/issue_events_170309.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-170309"
}
```



---

archive/issue_events_170310.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-170310"
}
```



---

archive/issue_events_170311.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-170311"
}
```



---

archive/issue_events_170312.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-18T00:36:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-170312"
}
```



---

archive/issue_events_170313.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-18T00:36:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-170313"
}
```



---

archive/issue_comments_137351.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nProposing to close all sagenb tickets as outdated, so that all remaining open tickets in the notebook component are about the Jupyter notebook.",
    "created_at": "2020-08-18T00:36:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12451#issuecomment-137351",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:9" align="right">comment:9</div>

Proposing to close all sagenb tickets as outdated, so that all remaining open tickets in the notebook component are about the Jupyter notebook.



---

archive/issue_events_170314.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-18T00:36:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-170314"
}
```



---

archive/issue_events_170315.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-18T00:36:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-170315"
}
```



---

archive/issue_events_170316.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-18T00:36:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-170316"
}
```



---

archive/issue_events_170317.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-09-02T07:48:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-170317"
}
```



---

archive/issue_events_170318.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-09-02T07:48:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12451",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12451#event-170318"
}
```
