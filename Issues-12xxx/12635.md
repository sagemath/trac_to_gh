# Issue 12635: Remove pbuild files

archive/issues_012463.json:
```json
{
    "assignees": [],
    "body": "The \"pbuild\" system was some different system for building Sage.  It hasn't been used since sage-3.1.2 (#4124), so it can safely be deleted.\n\nAssignee: **@jasongrout**\n\nAuthor: **Jeroen Demeyer**\n\nReviewer: **Punarbasu Purkayastha**\n\nMerged: **sage-5.0.beta8**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12635_\n\n",
    "closed_at": "2012-03-13T08:26:04Z",
    "created_at": "2012-03-06T09:24:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Remove pbuild files",
    "type": "issue",
    "updated_at": "2012-03-13T08:26:04Z",
    "url": "https://github.com/sagemath/sage/issues/12635",
    "user": "https://github.com/jdemeyer"
}
```
The "pbuild" system was some different system for building Sage.  It hasn't been used since sage-3.1.2 (#4124), so it can safely be deleted.

Assignee: **@jasongrout**

Author: **Jeroen Demeyer**

Reviewer: **Punarbasu Purkayastha**

Merged: **sage-5.0.beta8**

_Issue created by migration from https://trac.sagemath.org/ticket/12635_





---

archive/issue_events_158857.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-03-06T09:24:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12635",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12635#event-158857"
}
```



---

archive/issue_events_158858.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-03-06T09:24:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12635",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12635#event-158858"
}
```



---

archive/issue_events_158859.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-03-06T09:24:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12635",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12635#event-158859"
}
```



---

archive/issue_comments_143068.json:
```json
{
    "body": "Attachment: **[12635_remove_pbuild.patch.gz](https://github.com/sagemath/sage/files/ticket12635/12635_remove_pbuild.patch.gz)**\n\nApply to EXTCODE repository",
    "created_at": "2012-03-06T09:26:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12635",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12635#issuecomment-143068",
    "user": "https://github.com/jdemeyer"
}
```

Attachment: **[12635_remove_pbuild.patch.gz](https://github.com/sagemath/sage/files/ticket12635/12635_remove_pbuild.patch.gz)**

Apply to EXTCODE repository



---

archive/issue_events_158860.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-03-06T09:27:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12635",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12635#event-158860"
}
```



---

archive/issue_comments_143069.json:
```json
{
    "body": "Author: **Jeroen Demeyer**",
    "created_at": "2012-03-06T09:27:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12635",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12635#issuecomment-143069",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Jeroen Demeyer**



---

archive/issue_comments_143070.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNo harm in removing cruft. Positive review from me. There doesn't seem to be any mention of the removed files in the remaining files. Typical command I used (in zsh):\n\n```\n...tions/sage-5.0.beta2 [1] > LC_ALL=\"C\" grep -rnIw 'sageenv' ^(devel)/**/*.py | grep -v 'local/lib'\n...tions/sage-5.0.beta2 [1] >\n```",
    "created_at": "2012-03-07T10:02:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12635",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12635#issuecomment-143070",
    "user": "https://github.com/ppurka"
}
```

<div id="comment:2" align="right">Comment 2</div>

No harm in removing cruft. Positive review from me. There doesn't seem to be any mention of the removed files in the remaining files. Typical command I used (in zsh):

```
...tions/sage-5.0.beta2 [1] > LC_ALL="C" grep -rnIw 'sageenv' ^(devel)/**/*.py | grep -v 'local/lib'
...tions/sage-5.0.beta2 [1] >
```



---

archive/issue_events_158861.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2012-03-07T10:02:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12635",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12635#event-158861"
}
```



---

archive/issue_events_158862.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2012-03-07T10:02:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12635",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12635#event-158862"
}
```



---

archive/issue_comments_143071.json:
```json
{
    "body": "Reviewer: **Punarbasu Purkayastha**",
    "created_at": "2012-03-07T10:08:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12635",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12635#issuecomment-143071",
    "user": "https://github.com/ppurka"
}
```

Reviewer: **Punarbasu Purkayastha**



---

archive/issue_comments_143072.json:
```json
{
    "body": "Merged: **sage-5.0.beta8**",
    "created_at": "2012-03-13T08:26:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12635",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12635#issuecomment-143072",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.0.beta8**



---

archive/issue_events_158863.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-03-13T08:26:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12635",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12635#event-158863"
}
```



---

archive/issue_events_158864.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-03-13T08:26:04Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12635",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12635#event-158864"
}
```
