# Issue 12328: GMP_BITS_PER_MP_LIMB is deprecated in gmp/mpir

archive/issues_012156.json:
```json
{
    "assignees": [],
    "body": "GMP_BITS_PER_MP_LIMB is deprecated in gmp and mpir. It is actually gone in gmp-5. It is only a matter of time before mpir follows. GMP_BITS_PER_MP_LIMB should be replaced by GMP_LIMB_BITS. The only place where this is used in sage is sage/rings/integer.pyx.\n\nApply:\n* [attachment: trac_12328-gmp_limb_bits.patch.gz](https://github.com/sagemath/sage/files/ticket12328/trac_12328-gmp_limb_bits.patch.gz)\n\nAssignee: **@aghitza**\n\nCC:  pcpa cschwan\n\nAuthor: **Fran\u00e7ois Bissey**\n\nReviewer: **Robert Bradshaw**\n\nMerged: **sage-5.0.beta3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12328_\n\n",
    "closed_at": "2012-02-06T21:26:24Z",
    "created_at": "2012-01-20T02:05:30Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "GMP_BITS_PER_MP_LIMB is deprecated in gmp/mpir",
    "type": "issue",
    "updated_at": "2012-02-06T21:26:24Z",
    "url": "https://github.com/sagemath/sage/issues/12328",
    "user": "https://github.com/kiwifb"
}
```
GMP_BITS_PER_MP_LIMB is deprecated in gmp and mpir. It is actually gone in gmp-5. It is only a matter of time before mpir follows. GMP_BITS_PER_MP_LIMB should be replaced by GMP_LIMB_BITS. The only place where this is used in sage is sage/rings/integer.pyx.

Apply:
* [attachment: trac_12328-gmp_limb_bits.patch.gz](https://github.com/sagemath/sage/files/ticket12328/trac_12328-gmp_limb_bits.patch.gz)

Assignee: **@aghitza**

CC:  pcpa cschwan

Author: **François Bissey**

Reviewer: **Robert Bradshaw**

Merged: **sage-5.0.beta3**

_Issue created by migration from https://trac.sagemath.org/ticket/12328_





---

archive/issue_events_148414.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2012-01-20T02:05:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12328",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12328#event-148414"
}
```



---

archive/issue_events_148415.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2012-01-20T02:05:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12328",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "component: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12328#event-148415"
}
```



---

archive/issue_events_148416.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2012-01-20T02:05:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12328",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12328#event-148416"
}
```



---

archive/issue_events_148417.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2012-01-20T02:05:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12328",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12328#event-148417"
}
```



---

archive/issue_comments_136356.json:
```json
{
    "body": "patch to convert GMP_BITS_PER_MP_LIMB to GMP_LIMB_BITS",
    "created_at": "2012-01-20T02:12:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12328#issuecomment-136356",
    "user": "https://github.com/kiwifb"
}
```

patch to convert GMP_BITS_PER_MP_LIMB to GMP_LIMB_BITS



---

archive/issue_comments_136357.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,4 @@\n GMP_BITS_PER_MP_LIMB is deprecated in gmp and mpir. It is actually gone in gmp-5. It is only a matter of time before mpir follows. GMP_BITS_PER_MP_LIMB should be replaced by GMP_LIMB_BITS. The only place where this is used in sage is sage/rings/integer.pyx.\n+\n+Apply:\n+* [attachment: trac_12328-gmp_limb_bits.patch.gz](https://github.com/sagemath/sage/files/ticket12328/trac_12328-gmp_limb_bits.patch.gz)\n``````\n",
    "created_at": "2012-01-20T02:16:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12328#issuecomment-136357",
    "user": "https://github.com/kiwifb"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,4 @@
 GMP_BITS_PER_MP_LIMB is deprecated in gmp and mpir. It is actually gone in gmp-5. It is only a matter of time before mpir follows. GMP_BITS_PER_MP_LIMB should be replaced by GMP_LIMB_BITS. The only place where this is used in sage is sage/rings/integer.pyx.
+
+Apply:
+* [attachment: trac_12328-gmp_limb_bits.patch.gz](https://github.com/sagemath/sage/files/ticket12328/trac_12328-gmp_limb_bits.patch.gz)
``````




---

archive/issue_comments_136358.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[trac_12328-gmp_limb_bits.patch.gz](https://github.com/sagemath/sage/files/ticket12328/trac_12328-gmp_limb_bits.patch.gz)**\n\nAttaching patch and copying the people most interested in giving this a review.",
    "created_at": "2012-01-20T02:16:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12328#issuecomment-136358",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[trac_12328-gmp_limb_bits.patch.gz](https://github.com/sagemath/sage/files/ticket12328/trac_12328-gmp_limb_bits.patch.gz)**

Attaching patch and copying the people most interested in giving this a review.



---

archive/issue_events_148418.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2012-01-20T02:16:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12328",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12328#event-148418"
}
```



---

archive/issue_events_148419.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2012-01-31T06:43:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12328",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12328#event-148419"
}
```



---

archive/issue_events_148420.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2012-01-31T06:43:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12328",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12328#event-148420"
}
```



---

archive/issue_comments_136359.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nLooks good to me.",
    "created_at": "2012-01-31T06:43:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12328#issuecomment-136359",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:2'>Comment 2:</a>
Looks good to me.



---

archive/issue_comments_136360.json:
```json
{
    "body": "Reviewer: **Robert Bradshaw**",
    "created_at": "2012-01-31T06:43:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12328#issuecomment-136360",
    "user": "https://github.com/robertwb"
}
```

Reviewer: **Robert Bradshaw**



---

archive/issue_comments_136361.json:
```json
{
    "body": "Author: **Fran\u00e7ois Bissey**",
    "created_at": "2012-01-31T10:01:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12328#issuecomment-136361",
    "user": "https://github.com/jdemeyer"
}
```

Author: **François Bissey**



---

archive/issue_comments_136362.json:
```json
{
    "body": "Merged: **sage-5.0.beta3**",
    "created_at": "2012-02-06T21:26:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12328#issuecomment-136362",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.0.beta3**



---

archive/issue_events_148421.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-06T21:26:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12328",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12328#event-148421"
}
```



---

archive/issue_events_148422.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-06T21:26:24Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12328",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12328#event-148422"
}
```
