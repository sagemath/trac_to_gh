# Issue 12168: statistical analysis for calculation timings

archive/issues_011996.json:
```json
{
    "assignees": [],
    "body": "The attached script does some statistical analysis and plotting for a series of timings done with `timeit`. It should help to understand how the performance of an algorithm does work out and also how the underlying framework and hardware behaves under load. It shows a time-series, a histogram and CDF plot. It does reject outlyers based on quantile-margins and starts with a dedicated warmup phase. Additional features are possible. Plotting is done via matplotlib, this should be changed to Sage-specific routines.\n\nLinks: \n\n[sage-devel discussion about this](https://groups.google.com/d/topic/sage-devel/8lq3twm9Olc/discussion)\n\n[script in action on sagenb.org](http://sagenb.org/home/pub/3857/)\n\nAssignee: **@jasongrout**\n\nAuthor: **schilly**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12168_\n\n",
    "created_at": "2011-12-16T18:09:22Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "statistical analysis for calculation timings",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/12168",
    "user": "https://github.com/haraldschilly"
}
```
The attached script does some statistical analysis and plotting for a series of timings done with `timeit`. It should help to understand how the performance of an algorithm does work out and also how the underlying framework and hardware behaves under load. It shows a time-series, a histogram and CDF plot. It does reject outlyers based on quantile-margins and starts with a dedicated warmup phase. Additional features are possible. Plotting is done via matplotlib, this should be changed to Sage-specific routines.

Links: 

[sage-devel discussion about this](https://groups.google.com/d/topic/sage-devel/8lq3twm9Olc/discussion)

[script in action on sagenb.org](http://sagenb.org/home/pub/3857/)

Assignee: **@jasongrout**

Author: **schilly**

_Issue created by migration from https://trac.sagemath.org/ticket/12168_





---

archive/issue_events_151000.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2011-12-16T18:09:22Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12168",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12168#event-151000"
}
```



---

archive/issue_events_151001.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2011-12-16T18:09:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12168",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12168#event-151001"
}
```



---

archive/issue_events_151002.json:
```json
{
    "actor": "https://github.com/haraldschilly",
    "created_at": "2011-12-16T18:09:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12168",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12168#event-151002"
}
```



---

archive/issue_events_151003.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12168",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12168#event-151003"
}
```



---

archive/issue_events_151004.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12168",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12168#event-151004"
}
```



---

archive/issue_comments_132690.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nAttachment: **[timehisto.py.gz](https://github.com/sagemath/sage/files/ticket12168/timehisto.py.gz)**",
    "created_at": "2013-08-13T15:35:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12168",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12168#issuecomment-132690",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:1" align="right">Comment 1</div>

Attachment: **[timehisto.py.gz](https://github.com/sagemath/sage/files/ticket12168/timehisto.py.gz)**



---

archive/issue_events_151005.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12168",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12168#event-151005"
}
```



---

archive/issue_events_151006.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12168",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12168#event-151006"
}
```



---

archive/issue_events_151007.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12168",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12168#event-151007"
}
```



---

archive/issue_events_151008.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12168",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12168#event-151008"
}
```



---

archive/issue_events_151009.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12168",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12168#event-151009"
}
```



---

archive/issue_events_151010.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12168",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12168#event-151010"
}
```



---

archive/issue_events_151011.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12168",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12168#event-151011"
}
```
