# Issue 12732: Stopgap for #11493

archive/issues_012560.json:
```json
{
    "assignees": [],
    "body": "This is the second instance of a known bug in integrate (see also #11590/#12731).\n\nAssignee: **@burcin**\n\nCC:  @kcrisman\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12732_\n\n",
    "created_at": "2012-03-22T19:06:38Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20calculus",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Stopgap for #11493",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/12732",
    "user": "https://github.com/roed314"
}
```
This is the second instance of a known bug in integrate (see also #11590/#12731).

Assignee: **@burcin**

CC:  @kcrisman

_Issue created by migration from https://trac.sagemath.org/ticket/12732_





---

archive/issue_events_160479.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2012-03-22T19:06:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12732",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12732#event-160479"
}
```



---

archive/issue_events_160480.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2012-03-22T19:06:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12732",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20calculus",
    "label_color": "0000ff",
    "label_name": "component: calculus",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12732#event-160480"
}
```



---

archive/issue_events_160481.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2012-03-22T19:06:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12732",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12732#event-160481"
}
```



---

archive/issue_events_160482.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2012-03-22T19:06:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12732",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12732#event-160482"
}
```



---

archive/issue_comments_144908.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nSee #12691 for a background on stopgaps.",
    "created_at": "2012-03-22T19:08:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12732#issuecomment-144908",
    "user": "https://github.com/roed314"
}
```

<a id='comment:2'>Comment 2:</a>
See #12691 for a background on stopgaps.



---

archive/issue_comments_144909.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nSimilarly to my comment at #12731, I don't see how putting in a stopgap for #11493 is realistic.  We can't just tell people the integration code is buggy, because no matter how how many integrals you can do, there will always be some the pattern recognition misses... again, unless someone can do a detailed pattern search for things with `exp` with operands that are quadratic and then multiplied by something unusual...",
    "created_at": "2012-03-23T01:06:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12732",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12732#issuecomment-144909",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:3'>Comment 3:</a>
Similarly to my comment at #12731, I don't see how putting in a stopgap for #11493 is realistic.  We can't just tell people the integration code is buggy, because no matter how how many integrals you can do, there will always be some the pattern recognition misses... again, unless someone can do a detailed pattern search for things with `exp` with operands that are quadratic and then multiplied by something unusual...



---

archive/issue_events_160483.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-04-08T08:52:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12732",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12732#event-160483"
}
```



---

archive/issue_events_160484.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-04-08T08:52:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12732",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12732#event-160484"
}
```



---

archive/issue_events_160485.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12732",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12732#event-160485"
}
```



---

archive/issue_events_160486.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12732",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12732#event-160486"
}
```



---

archive/issue_events_160487.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12732",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12732#event-160487"
}
```



---

archive/issue_events_160488.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12732",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12732#event-160488"
}
```



---

archive/issue_events_160489.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12732",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12732#event-160489"
}
```



---

archive/issue_events_160490.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12732",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12732#event-160490"
}
```



---

archive/issue_events_160491.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12732",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12732#event-160491"
}
```



---

archive/issue_events_160492.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12732",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12732#event-160492"
}
```



---

archive/issue_events_160493.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12732",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12732#event-160493"
}
```
