# Issue 12878: Add a list of methods for long (maybe all) classes

archive/issues_012706.json:
```json
{
    "assignees": [],
    "body": "See\n[Some Experiment](http://sage.math.washington.edu/home/wstein/www/home/hivert/html/en/reference/index.html) and in particular\n[The graph ref page](http://sage.math.washington.edu/home/wstein/www/home/hivert/html/en/reference/graphs/sage/graphs/generic_graph.html)\n\nFlorent\n\n\nAssignee: **@hivert**\n\nCC:  @nathanncohen\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12878_\n\n",
    "created_at": "2012-04-24T23:11:40Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add a list of methods for long (maybe all) classes",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/12878",
    "user": "https://github.com/hivert"
}
```
See
[Some Experiment](http://sage.math.washington.edu/home/wstein/www/home/hivert/html/en/reference/index.html) and in particular
[The graph ref page](http://sage.math.washington.edu/home/wstein/www/home/hivert/html/en/reference/graphs/sage/graphs/generic_graph.html)

Florent


Assignee: **@hivert**

CC:  @nathanncohen

_Issue created by migration from https://trac.sagemath.org/ticket/12878_





---

archive/issue_events_162847.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2012-04-24T23:11:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12878",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12878#event-162847"
}
```



---

archive/issue_events_162848.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2012-04-24T23:11:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12878",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12878#event-162848"
}
```



---

archive/issue_events_162849.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2012-04-24T23:11:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12878",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12878#event-162849"
}
```



---

archive/issue_events_162850.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2012-04-24T23:11:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12878",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12878#event-162850"
}
```



---

archive/issue_comments_147833.json:
```json
{
    "body": "Replying to [ticket:12878 hivert]:\n> See [Some Experiment](http://sage.math.washington.edu/home/wstein/www/home/hivert/reference/graphs/sage/graphs/generic_graph.html)\n\nI get a 404.\n\n(And the patch contains a `.pyc`?!)",
    "created_at": "2012-04-25T08:27:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12878",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12878#issuecomment-147833",
    "user": "https://github.com/nexttime"
}
```

Replying to [ticket:12878 hivert]:
> See [Some Experiment](http://sage.math.washington.edu/home/wstein/www/home/hivert/reference/graphs/sage/graphs/generic_graph.html)

I get a 404.

(And the patch contains a `.pyc`?!)



---

archive/issue_comments_147834.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,6 @@\n-See [Some Experiment](http://sage.math.washington.edu/home/wstein/www/home/hivert/reference/graphs/sage/graphs/generic_graph.html)\n+See\n+[Some Experiment](http://sage.math.washington.edu/home/wstein/www/home/hivert/html/en/reference/index.html) and in particular\n+[The graph ref page](http://sage.math.washington.edu/home/wstein/www/home/hivert/html/en/reference/graphs/sage/graphs/generic_graph.html)\n \n Florent\n+\n``````\n",
    "created_at": "2012-04-25T10:07:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12878",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12878#issuecomment-147834",
    "user": "https://github.com/hivert"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,6 @@
-See [Some Experiment](http://sage.math.washington.edu/home/wstein/www/home/hivert/reference/graphs/sage/graphs/generic_graph.html)
+See
+[Some Experiment](http://sage.math.washington.edu/home/wstein/www/home/hivert/html/en/reference/index.html) and in particular
+[The graph ref page](http://sage.math.washington.edu/home/wstein/www/home/hivert/html/en/reference/graphs/sage/graphs/generic_graph.html)
 
 Florent
+
``````




---

archive/issue_comments_147835.json:
```json
{
    "body": "Attachment: **[class_summary.patch.gz](https://github.com/sagemath/sage/files/ticket12878/class_summary.patch.gz)**\n\nExperimental patch not ready for review",
    "created_at": "2012-04-25T10:12:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12878",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12878#issuecomment-147835",
    "user": "https://github.com/hivert"
}
```

Attachment: **[class_summary.patch.gz](https://github.com/sagemath/sage/files/ticket12878/class_summary.patch.gz)**

Experimental patch not ready for review



---

archive/issue_comments_147836.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nReplying to [@nexttime](#comment%3A2):\n> I get a 404.\n\nI was recompiling the doc on the Sage machine because my rsync lost some css files.\n\n> (And the patch contains a `.pyc`?!)\n\nOops ! As I said, the patch is not ready for review. I just put it here if some people want to play with it.\nIt should be fixed now.",
    "created_at": "2012-04-25T10:15:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12878",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12878#issuecomment-147836",
    "user": "https://github.com/hivert"
}
```

<div id="comment:4" align="right">Comment 4</div>

Replying to [@nexttime](#comment%3A2):
> I get a 404.

I was recompiling the doc on the Sage machine because my rsync lost some css files.

> (And the patch contains a `.pyc`?!)

Oops ! As I said, the patch is not ready for review. I just put it here if some people want to play with it.
It should be fixed now.



---

archive/issue_events_162851.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12878",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12878#event-162851"
}
```



---

archive/issue_events_162852.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12878",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12878#event-162852"
}
```



---

archive/issue_events_162853.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12878",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12878#event-162853"
}
```



---

archive/issue_events_162854.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12878",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12878#event-162854"
}
```



---

archive/issue_events_162855.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12878",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12878#event-162855"
}
```



---

archive/issue_events_162856.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12878",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12878#event-162856"
}
```



---

archive/issue_events_162857.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12878",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12878#event-162857"
}
```



---

archive/issue_events_162858.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12878",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12878#event-162858"
}
```



---

archive/issue_events_162859.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12878",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12878#event-162859"
}
```
