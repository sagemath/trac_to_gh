# Issue 12330: Increase eval_using_file_cutoff for gp interface

archive/issues_012158.json:
```json
{
    "assignees": [],
    "body": "The gp pexpect interface uses \"eval_using_file_cutoff=50\", which is extremely short.  This almost certainly causes inefficiencies because files will need to be used a lot.  Most likely, this was set to 50 to mask bug #12221.  Now that #12221 is fixed, this can be put back to a more reasonable value like 1024.\n\nDepends on #12221\n\nAssignee: **@williamstein**\n\nAuthor: **Jeroen Demeyer**\n\nReviewer: **Georg S. Weber**\n\nMerged: **sage-5.0.beta1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12330_\n\n",
    "closed_at": "2012-01-21T23:39:01Z",
    "created_at": "2012-01-20T13:39:10Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Increase eval_using_file_cutoff for gp interface",
    "type": "issue",
    "updated_at": "2012-01-21T23:39:01Z",
    "url": "https://github.com/sagemath/sage/issues/12330",
    "user": "https://github.com/jdemeyer"
}
```
The gp pexpect interface uses "eval_using_file_cutoff=50", which is extremely short.  This almost certainly causes inefficiencies because files will need to be used a lot.  Most likely, this was set to 50 to mask bug #12221.  Now that #12221 is fixed, this can be put back to a more reasonable value like 1024.

Depends on #12221

Assignee: **@williamstein**

Author: **Jeroen Demeyer**

Reviewer: **Georg S. Weber**

Merged: **sage-5.0.beta1**

_Issue created by migration from https://trac.sagemath.org/ticket/12330_





---

archive/issue_events_153703.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-01-20T13:39:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12330",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12330#event-153703"
}
```



---

archive/issue_events_153704.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-01-20T13:39:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12330",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
    "label_color": "0000ff",
    "label_name": "component: interfaces",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12330#event-153704"
}
```



---

archive/issue_events_153705.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-01-20T13:39:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12330",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12330#event-153705"
}
```



---

archive/issue_events_153706.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-01-20T13:39:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12330",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12330#event-153706"
}
```



---

archive/issue_comments_136376.json:
```json
{
    "body": "Dependencies: **#12221**",
    "created_at": "2012-01-20T13:40:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12330#issuecomment-136376",
    "user": "https://github.com/jdemeyer"
}
```

Dependencies: **#12221**



---

archive/issue_comments_136377.json:
```json
{
    "body": "Attachment: **[12330_gp.patch.gz](https://github.com/sagemath/sage/files/ticket12330/12330_gp.patch.gz)**",
    "created_at": "2012-01-20T13:42:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12330#issuecomment-136377",
    "user": "https://github.com/jdemeyer"
}
```

Attachment: **[12330_gp.patch.gz](https://github.com/sagemath/sage/files/ticket12330/12330_gp.patch.gz)**



---

archive/issue_events_153707.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-01-20T13:43:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12330",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12330#event-153707"
}
```



---

archive/issue_comments_136378.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLooks good to me.\n\n;-)",
    "created_at": "2012-01-20T19:13:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12330#issuecomment-136378",
    "user": "https://github.com/sagetrac-GeorgSWeber"
}
```

<div id="comment:3" align="right">Comment 3</div>

Looks good to me.

;-)



---

archive/issue_comments_136379.json:
```json
{
    "body": "Reviewer: **Georg S. Weber**",
    "created_at": "2012-01-20T19:13:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12330#issuecomment-136379",
    "user": "https://github.com/sagetrac-GeorgSWeber"
}
```

Reviewer: **Georg S. Weber**



---

archive/issue_events_153708.json:
```json
{
    "actor": "https://github.com/sagetrac-GeorgSWeber",
    "created_at": "2012-01-20T19:13:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12330",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12330#event-153708"
}
```



---

archive/issue_events_153709.json:
```json
{
    "actor": "https://github.com/sagetrac-GeorgSWeber",
    "created_at": "2012-01-20T19:13:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12330",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12330#event-153709"
}
```



---

archive/issue_comments_136380.json:
```json
{
    "body": "Merged: **sage-5.0.beta1**",
    "created_at": "2012-01-21T23:39:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12330",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12330#issuecomment-136380",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.0.beta1**



---

archive/issue_events_153710.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-01-21T23:39:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12330",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12330#event-153710"
}
```



---

archive/issue_events_153711.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-01-21T23:39:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12330",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12330#event-153711"
}
```
