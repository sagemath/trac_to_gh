# Issue 12301: Conventions for Coding SEEALSO format

archive/issues_012129.json:
```json
{
    "body": "In the docstrings, the format:\n\n.. SEEALSO::\n\n    Material goes here indented some from the see also above.\n\n\nproduces a fancy little box in the html pages similar to the note or warning boxes. However, this is not specified in the Conventions for Coding document.  It would be great if the Conventions for Coding document could be appended so people will know that they can use this.\n\n**Assignee:** mvngu\n\n**Keywords:** sd35.5\n\n**Reviewer:** Florent Hivert\n\nIssue created by migration from https://trac.sagemath.org/ticket/12301\n\n",
    "closed_at": "2012-02-27T11:29:06Z",
    "created_at": "2012-01-12T19:21:44Z",
    "labels": [
        "component: documentation",
        "minor",
        "enhancement",
        "duplicate"
    ],
    "title": "Conventions for Coding SEEALSO format",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/12301",
    "user": "https://trac.sagemath.org/admin/accounts/users/ksmith"
}
```
In the docstrings, the format:

.. SEEALSO::

    Material goes here indented some from the see also above.


produces a fancy little box in the html pages similar to the note or warning boxes. However, this is not specified in the Conventions for Coding document.  It would be great if the Conventions for Coding document could be appended so people will know that they can use this.

**Assignee:** mvngu

**Keywords:** sd35.5

**Reviewer:** Florent Hivert

Issue created by migration from https://trac.sagemath.org/ticket/12301





---

archive/issue_events_100506.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2012-02-08T02:00:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/12301",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12301#event-100506"
}
```



---

archive/issue_comments_135596.json:
```json
{
    "body": "<a id='comment:1'></a>\nThis is a duplicate for #12078.\n\nPlease close.",
    "created_at": "2012-02-08T02:00:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12301",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12301#issuecomment-135596",
    "user": "https://github.com/hivert"
}
```

<a id='comment:1'></a>
This is a duplicate for #12078.

Please close.



---

archive/issue_events_100507.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2012-02-08T02:00:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/12301",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12301#event-100507"
}
```



---

archive/issue_events_100508.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2012-02-08T02:00:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/12301",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12301#event-100508"
}
```



---

archive/issue_comments_135597.json:
```json
{
    "body": "**Reviewer:** Florent Hivert",
    "created_at": "2012-02-25T20:55:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12301",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12301#issuecomment-135597",
    "user": "https://github.com/kcrisman"
}
```

**Reviewer:** Florent Hivert



---

archive/issue_events_100509.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-27T11:29:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/12301",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12301#event-100509"
}
```



---

archive/issue_events_100510.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-27T11:29:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/12301",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12301#event-100510"
}
```



---

archive/issue_events_100511.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-27T11:29:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/12301",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12301#event-100511"
}
```
