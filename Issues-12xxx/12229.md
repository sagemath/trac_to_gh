# Issue 12229: Add section in the developers manual about sagenb development

archive/issues_012057.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mvngu"
    ],
    "body": "Since some Sage tickets will require patches to sagenb, there should be a section in the Sage developers manual about how to contribute to the flask-sagenb project.  We really want every Sage developer to be able to contribute to sagenb.  It's okay if this requires some work (like installing `git`), but this should be documented well.\n\nContext: [http://groups.google.com/group/sage-notebook/t/3fa33efda8db5c94](http://groups.google.com/group/sage-notebook/t/3fa33efda8db5c94)\n\n**Apply**: [attachment: trac_12229-sagenb-developer-doc.3.patch](https://github.com/sagemath/sage/files/ticket12229/trac_12229-sagenb-developer-doc.3.patch.gz) and [attachment: trac-12229-manifest.patch](https://github.com/sagemath/sage/files/ticket12229/trac-12229-manifest.patch.gz) and [attachment: trac_12229-reviewer.patch](https://github.com/sagemath/sage/files/ticket12229/trac_12229-reviewer.patch.gz).\n\nDepends on #11080\n\nCC:  @jasongrout @kcrisman\n\nReviewer: **William Stein, Jeroen Demeyer, Punarbasu Purkayastha, Karl-Dieter Crisman, Keshav Kini**\n\nAuthor: **Jason Grout, Karl-Dieter Crisman**\n\nMerged: **sage-5.2.beta0**\n\nComponent: **documentation**\n\nDependencies: **to be merged with #11080**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12229_\n\n",
    "closed_at": "2012-07-25T06:40:28Z",
    "created_at": "2011-12-23T14:05:18Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%201%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add section in the developers manual about sagenb development",
    "type": "issue",
    "updated_at": "2012-07-25T06:40:28Z",
    "url": "https://github.com/sagemath/sage/issues/12229",
    "user": "https://github.com/jdemeyer"
}
```
Since some Sage tickets will require patches to sagenb, there should be a section in the Sage developers manual about how to contribute to the flask-sagenb project.  We really want every Sage developer to be able to contribute to sagenb.  It's okay if this requires some work (like installing `git`), but this should be documented well.

Context: [http://groups.google.com/group/sage-notebook/t/3fa33efda8db5c94](http://groups.google.com/group/sage-notebook/t/3fa33efda8db5c94)

**Apply**: [attachment: trac_12229-sagenb-developer-doc.3.patch](https://github.com/sagemath/sage/files/ticket12229/trac_12229-sagenb-developer-doc.3.patch.gz) and [attachment: trac-12229-manifest.patch](https://github.com/sagemath/sage/files/ticket12229/trac-12229-manifest.patch.gz) and [attachment: trac_12229-reviewer.patch](https://github.com/sagemath/sage/files/ticket12229/trac_12229-reviewer.patch.gz).

Depends on #11080

CC:  @jasongrout @kcrisman

Reviewer: **William Stein, Jeroen Demeyer, Punarbasu Purkayastha, Karl-Dieter Crisman, Keshav Kini**

Author: **Jason Grout, Karl-Dieter Crisman**

Merged: **sage-5.2.beta0**

Component: **documentation**

Dependencies: **to be merged with #11080**

_Issue created by migration from https://trac.sagemath.org/ticket/12229_





---

archive/issue_events_167011.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-12-23T14:05:18Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167011"
}
```



---

archive/issue_events_167012.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-12-23T14:05:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167012"
}
```



---

archive/issue_events_167013.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-12-23T14:05:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "label": "https://github.com/sagemath/sage/labels/p%3A%201%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p: 1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167013"
}
```



---

archive/issue_events_167014.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-12-23T14:05:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167014"
}
```



---

archive/issue_events_167015.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2011-12-23T14:05:18Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "subject": "https://github.com/jdemeyer",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167015"
}
```



---

archive/issue_comments_132274.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n-Cince some Sage tickets will require patches to sagenb, there should be a section in the Sage developers manual about how to contribute to the flask-sagenb project.  We really want every Sage developer to be able to contribute to sagenb.  It's okay if this requires some work (like installing `git`), but this should be documented well.\n+Since some Sage tickets will require patches to sagenb, there should be a section in the Sage developers manual about how to contribute to the flask-sagenb project.  We really want every Sage developer to be able to contribute to sagenb.  It's okay if this requires some work (like installing `git`), but this should be documented well.\n \n Context: [http://groups.google.com/group/sage-notebook/t/3fa33efda8db5c94](http://groups.google.com/group/sage-notebook/t/3fa33efda8db5c94)\n``````\n",
    "created_at": "2011-12-23T14:12:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132274",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
-Cince some Sage tickets will require patches to sagenb, there should be a section in the Sage developers manual about how to contribute to the flask-sagenb project.  We really want every Sage developer to be able to contribute to sagenb.  It's okay if this requires some work (like installing `git`), but this should be documented well.
+Since some Sage tickets will require patches to sagenb, there should be a section in the Sage developers manual about how to contribute to the flask-sagenb project.  We really want every Sage developer to be able to contribute to sagenb.  It's okay if this requires some work (like installing `git`), but this should be documented well.
 
 Context: [http://groups.google.com/group/sage-notebook/t/3fa33efda8db5c94](http://groups.google.com/group/sage-notebook/t/3fa33efda8db5c94)
``````




---

archive/issue_comments_132275.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nThis is a preliminary patch, based on the gitwash project for generating documentation for github projects: https://github.com/matthew-brett/gitwash\n\nAny general improvements to this documentation should be contributed back to the gitwash project.  Also, we should probably add the paragraph or two in the Development section in the sagenb README.rst about converting between hg and git repositories: https://github.com/sagemath/sagenb/blob/master/README.rst\n\nIdeally, sagenb would be documented totally separately in its source tree, and that documentation would somehow be available to the users.",
    "created_at": "2011-12-31T14:07:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132275",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:3" align="right">comment:3</div>

This is a preliminary patch, based on the gitwash project for generating documentation for github projects: https://github.com/matthew-brett/gitwash

Any general improvements to this documentation should be contributed back to the gitwash project.  Also, we should probably add the paragraph or two in the Development section in the sagenb README.rst about converting between hg and git repositories: https://github.com/sagemath/sagenb/blob/master/README.rst

Ideally, sagenb would be documented totally separately in its source tree, and that documentation would somehow be available to the users.



---

archive/issue_events_167016.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2012-01-06T03:06:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167016"
}
```



---

archive/issue_comments_132276.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nOkay, I added a little bit about switching to git, almost straight from the sagenb readme.  I think it's ready for review, though we should eventually move this to sagenb.",
    "created_at": "2012-01-06T03:06:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132276",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:4" align="right">comment:4</div>

Okay, I added a little bit about switching to git, almost straight from the sagenb readme.  I think it's ready for review, though we should eventually move this to sagenb.



---

archive/issue_comments_132277.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nAfter hours of work and nitpicking... positive review!",
    "created_at": "2012-01-06T05:39:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132277",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:5" align="right">comment:5</div>

After hours of work and nitpicking... positive review!



---

archive/issue_events_167017.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-01-06T05:39:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167017"
}
```



---

archive/issue_events_167018.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-01-06T05:39:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167018"
}
```



---

archive/issue_comments_132278.json:
```json
{
    "body": "Author: **Jason Grout**",
    "created_at": "2012-01-06T09:16:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132278",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Jason Grout**



---

archive/issue_comments_132279.json:
```json
{
    "body": "Reviewer: **William Stein**",
    "created_at": "2012-01-06T09:16:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132279",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **William Stein**



---

archive/issue_comments_132280.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nReplying to [@jasongrout](#comment:4):\n> We should eventually move this to sagenb.\n\nHmm, I'm not sure about this.  The documentation should be obviously findable for non-sagenb developers.",
    "created_at": "2012-01-06T09:46:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132280",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:7" align="right">comment:7</div>

Replying to [@jasongrout](#comment:4):
> We should eventually move this to sagenb.

Hmm, I'm not sure about this.  The documentation should be obviously findable for non-sagenb developers.



---

archive/issue_comments_132281.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nReplying to [@jdemeyer](#comment:7):\n> Replying to [@jasongrout](#comment:4):\n> > We should eventually move this to sagenb.\n\n> \n> Hmm, I'm not sure about this.  The documentation should be obviously findable for non-sagenb developers.\n\nYes, that is the reason why we are not doing that right now---something needs to be figured out to make that doc easily accessible.  This ticket just adds the docs to the sage developer's guide for now.",
    "created_at": "2012-01-06T14:23:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132281",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:8" align="right">comment:8</div>

Replying to [@jdemeyer](#comment:7):
> Replying to [@jasongrout](#comment:4):
> > We should eventually move this to sagenb.

> 
> Hmm, I'm not sure about this.  The documentation should be obviously findable for non-sagenb developers.

Yes, that is the reason why we are not doing that right now---something needs to be figured out to make that doc easily accessible.  This ticket just adds the docs to the sage developer's guide for now.



---

archive/issue_events_167019.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-01-09T23:34:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167019"
}
```



---

archive/issue_events_167020.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-01-09T23:34:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167020"
}
```



---

archive/issue_comments_132282.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nCurrently, the commit message of this patch has some very long lines.  Could you wrap these long lines?  Make sure the first line (shown by `hg log`) makes sense by itself.",
    "created_at": "2012-01-09T23:34:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132282",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:9" align="right">comment:9</div>

Currently, the commit message of this patch has some very long lines.  Could you wrap these long lines?  Make sure the first line (shown by `hg log`) makes sense by itself.



---

archive/issue_events_167021.json:
```json
{
    "actor": "https://github.com/sagetrac-ijstokes",
    "created_at": "2012-01-10T01:17:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167021"
}
```



---

archive/issue_events_167022.json:
```json
{
    "actor": "https://github.com/sagetrac-ijstokes",
    "created_at": "2012-01-10T01:17:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167022"
}
```



---

archive/issue_comments_132283.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,5 @@\n Since some Sage tickets will require patches to sagenb, there should be a section in the Sage developers manual about how to contribute to the flask-sagenb project.  We really want every Sage developer to be able to contribute to sagenb.  It's okay if this requires some work (like installing `git`), but this should be documented well.\n \n Context: [http://groups.google.com/group/sage-notebook/t/3fa33efda8db5c94](http://groups.google.com/group/sage-notebook/t/3fa33efda8db5c94)\n+\n+Apply: [attachment: trac-12229-sagenb-developer-doc.2.patch](https://github.com/sagemath/sage/files/ticket12229/trac-12229-sagenb-developer-doc.2.patch.gz)\n``````\n",
    "created_at": "2012-01-10T01:17:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132283",
    "user": "https://github.com/sagetrac-ijstokes"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,5 @@
 Since some Sage tickets will require patches to sagenb, there should be a section in the Sage developers manual about how to contribute to the flask-sagenb project.  We really want every Sage developer to be able to contribute to sagenb.  It's okay if this requires some work (like installing `git`), but this should be documented well.
 
 Context: [http://groups.google.com/group/sage-notebook/t/3fa33efda8db5c94](http://groups.google.com/group/sage-notebook/t/3fa33efda8db5c94)
+
+Apply: [attachment: trac-12229-sagenb-developer-doc.2.patch](https://github.com/sagemath/sage/files/ticket12229/trac-12229-sagenb-developer-doc.2.patch.gz)
``````




---

archive/issue_comments_132284.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">comment:11</div>\n\n`make doc-html-jsmath` fails:\n\n```\nsphinx-build -b html -d /home/buildbot/build/sage/eno-1/eno_full/build/sage-5.0.beta0/devel/sage/doc/output/doctrees/en/developer   -A hide_pdf_links=1 /home/buildbot/build/sage/eno-1/eno_full/build/sage-5.0.beta0/devel/sage/doc/en/developer /home/buildbot/build/sage/eno-1/eno_full/build/sage-5.0.beta0/devel/sage/doc/output/html/en/developer\nRunning Sphinx v1.1.2\nloading pickled environment... not yet created\nbuilding [html]: targets for 27 source files that are out of date\nupdating environment: 27 added, 0 changed, 0 removed\nreading sources... [  3%] coding_in_cython\nreading sources... [  7%] coding_in_other\nreading sources... [ 11%] coding_in_python\nreading sources... [ 14%] conventions\nreading sources... [ 18%] disseminating_code\nreading sources... [ 22%] doctesting\nreading sources... [ 25%] inclusion\nreading sources... [ 29%] index\nreading sources... [ 33%] patching_spkgs\nreading sources... [ 37%] producing_patches\nreading sources... [ 40%] producing_spkgs\nreading sources... [ 44%] sage_manuals\nreading sources... [ 48%] sagenb/configure_git\nreading sources... [ 51%] sagenb/development_workflow\nreading sources... [ 55%] sagenb/following_latest\nreading sources... [ 59%] sagenb/forking_hell\nreading sources... [ 62%] sagenb/git_development\nreading sources... [ 66%] sagenb/git_install\nreading sources... [ 70%] sagenb/git_intro\nreading sources... [ 74%] sagenb/git_resources\nreading sources... [ 77%] sagenb/index\nreading sources... [ 81%] sagenb/maintainer_workflow\nreading sources... [ 85%] sagenb/patching\nreading sources... [ 88%] sagenb/set_up_fork\nreading sources... [ 92%] trac\nreading sources... [ 96%] walk_through\nreading sources... [100%] writing_code\n\n/home/buildbot/build/sage/eno-1/eno_full/build/sage-5.0.beta0/devel/sage/doc/en/developer/sagenb/configure_git.rst:158: SEVERE: Problems with \"include\" directive path:\nIOError: [Errno 2] No such file or directory: '../../../../sage/doc/en/developer/sagenb/links.inc'.\n/home/buildbot/build/sage/eno-1/eno_full/build/sage-5.0.beta0/devel/sage/doc/en/developer/sagenb/configure_git.rst:58: ERROR: Unknown target name: \"git\".\n/home/buildbot/build/sage/eno-1/eno_full/build/sage-5.0.beta0/devel/sage/doc/en/developer/sagenb/development_workflow.rst:415: SEVERE: Problems with \"include\" directive path:\nIOError: [Errno 2] No such file or directory: '../../../../sage/doc/en/developer/sagenb/links.inc'.\n/home/buildbot/build/sage/eno-1/eno_full/build/sage-5.0.beta0/devel/sage/doc/en/developer/sagenb/development_workflow.rst:20: ERROR: Undefined substitution referenced: \"emdash\".\n/home/buildbot/build/sage/eno-1/eno_full/build/sage-5.0.beta0/devel/sage/doc/en/developer/sagenb/development_workflow.rst:137: ERROR: Undefined substitution referenced: \"emdash\".\n[...many more...]\n```",
    "created_at": "2012-01-13T11:33:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132284",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:11" align="right">comment:11</div>

`make doc-html-jsmath` fails:

```
sphinx-build -b html -d /home/buildbot/build/sage/eno-1/eno_full/build/sage-5.0.beta0/devel/sage/doc/output/doctrees/en/developer   -A hide_pdf_links=1 /home/buildbot/build/sage/eno-1/eno_full/build/sage-5.0.beta0/devel/sage/doc/en/developer /home/buildbot/build/sage/eno-1/eno_full/build/sage-5.0.beta0/devel/sage/doc/output/html/en/developer
Running Sphinx v1.1.2
loading pickled environment... not yet created
building [html]: targets for 27 source files that are out of date
updating environment: 27 added, 0 changed, 0 removed
reading sources... [  3%] coding_in_cython
reading sources... [  7%] coding_in_other
reading sources... [ 11%] coding_in_python
reading sources... [ 14%] conventions
reading sources... [ 18%] disseminating_code
reading sources... [ 22%] doctesting
reading sources... [ 25%] inclusion
reading sources... [ 29%] index
reading sources... [ 33%] patching_spkgs
reading sources... [ 37%] producing_patches
reading sources... [ 40%] producing_spkgs
reading sources... [ 44%] sage_manuals
reading sources... [ 48%] sagenb/configure_git
reading sources... [ 51%] sagenb/development_workflow
reading sources... [ 55%] sagenb/following_latest
reading sources... [ 59%] sagenb/forking_hell
reading sources... [ 62%] sagenb/git_development
reading sources... [ 66%] sagenb/git_install
reading sources... [ 70%] sagenb/git_intro
reading sources... [ 74%] sagenb/git_resources
reading sources... [ 77%] sagenb/index
reading sources... [ 81%] sagenb/maintainer_workflow
reading sources... [ 85%] sagenb/patching
reading sources... [ 88%] sagenb/set_up_fork
reading sources... [ 92%] trac
reading sources... [ 96%] walk_through
reading sources... [100%] writing_code

/home/buildbot/build/sage/eno-1/eno_full/build/sage-5.0.beta0/devel/sage/doc/en/developer/sagenb/configure_git.rst:158: SEVERE: Problems with "include" directive path:
IOError: [Errno 2] No such file or directory: '../../../../sage/doc/en/developer/sagenb/links.inc'.
/home/buildbot/build/sage/eno-1/eno_full/build/sage-5.0.beta0/devel/sage/doc/en/developer/sagenb/configure_git.rst:58: ERROR: Unknown target name: "git".
/home/buildbot/build/sage/eno-1/eno_full/build/sage-5.0.beta0/devel/sage/doc/en/developer/sagenb/development_workflow.rst:415: SEVERE: Problems with "include" directive path:
IOError: [Errno 2] No such file or directory: '../../../../sage/doc/en/developer/sagenb/links.inc'.
/home/buildbot/build/sage/eno-1/eno_full/build/sage-5.0.beta0/devel/sage/doc/en/developer/sagenb/development_workflow.rst:20: ERROR: Undefined substitution referenced: "emdash".
/home/buildbot/build/sage/eno-1/eno_full/build/sage-5.0.beta0/devel/sage/doc/en/developer/sagenb/development_workflow.rst:137: ERROR: Undefined substitution referenced: "emdash".
[...many more...]
```



---

archive/issue_events_167023.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-01-13T11:33:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167023"
}
```



---

archive/issue_events_167024.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-01-13T11:33:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167024"
}
```



---

archive/issue_comments_132285.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">comment:12</div>\n\nI'm trying to reproduce this, but everything works for me. I deleted the output for the developer's guide and ran make-doc-jsmath, and there were no complaints.\n\nIs the error reproducible on eno?  That's on skynet?  I might be able to log in and try to reproduce it there.",
    "created_at": "2012-01-16T20:44:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132285",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:12" align="right">comment:12</div>

I'm trying to reproduce this, but everything works for me. I deleted the output for the developer's guide and ran make-doc-jsmath, and there were no complaints.

Is the error reproducible on eno?  That's on skynet?  I might be able to log in and try to reproduce it there.



---

archive/issue_comments_132286.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">comment:13</div>\n\nReplying to [@jasongrout](#comment:12):\n> I'm trying to reproduce this, but everything works for me. I deleted the output for the developer's guide and ran make-doc-jsmath, and there were no complaints.\n> \n> Is the error reproducible on eno?  That's on skynet?  I might be able to log in and try to reproduce it there.\n\nIt's reproducible on **every** machine on the Buildbot.  But I just think of something: are you adding files which are not in `devel/sage/MANIFEST.in`?",
    "created_at": "2012-01-16T20:51:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132286",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:13" align="right">comment:13</div>

Replying to [@jasongrout](#comment:12):
> I'm trying to reproduce this, but everything works for me. I deleted the output for the developer's guide and ran make-doc-jsmath, and there were no complaints.
> 
> Is the error reproducible on eno?  That's on skynet?  I might be able to log in and try to reproduce it there.

It's reproducible on **every** machine on the Buildbot.  But I just think of something: are you adding files which are not in `devel/sage/MANIFEST.in`?



---

archive/issue_comments_132287.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">comment:14</div>\n\nI didn't touch the MANIFEST.in.  That probably explains the problem.  I'll post a patch.",
    "created_at": "2012-01-16T20:55:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132287",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:14" align="right">comment:14</div>

I didn't touch the MANIFEST.in.  That probably explains the problem.  I'll post a patch.



---

archive/issue_events_167025.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2012-01-16T21:14:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167025"
}
```



---

archive/issue_events_167026.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2012-01-16T21:14:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167026"
}
```



---

archive/issue_comments_132288.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">comment:15</div>\n\nThe attached patch should fix the MANIFEST.in issues.",
    "created_at": "2012-01-16T21:14:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132288",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:15" align="right">comment:15</div>

The attached patch should fix the MANIFEST.in issues.



---

archive/issue_comments_132289.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,4 +2,4 @@\n \n Context: [http://groups.google.com/group/sage-notebook/t/3fa33efda8db5c94](http://groups.google.com/group/sage-notebook/t/3fa33efda8db5c94)\n \n-Apply: [attachment: trac-12229-sagenb-developer-doc.2.patch](https://github.com/sagemath/sage/files/ticket12229/trac-12229-sagenb-developer-doc.2.patch.gz)\n+Apply: [attachment: trac-12229-sagenb-developer-doc.2.patch](https://github.com/sagemath/sage/files/ticket12229/trac-12229-sagenb-developer-doc.2.patch.gz) and [attachment: trac-12229-manifest.patch](https://github.com/sagemath/sage/files/ticket12229/trac-12229-manifest.patch.gz)\n``````\n",
    "created_at": "2012-01-16T22:04:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132289",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,4 +2,4 @@
 
 Context: [http://groups.google.com/group/sage-notebook/t/3fa33efda8db5c94](http://groups.google.com/group/sage-notebook/t/3fa33efda8db5c94)
 
-Apply: [attachment: trac-12229-sagenb-developer-doc.2.patch](https://github.com/sagemath/sage/files/ticket12229/trac-12229-sagenb-developer-doc.2.patch.gz)
+Apply: [attachment: trac-12229-sagenb-developer-doc.2.patch](https://github.com/sagemath/sage/files/ticket12229/trac-12229-sagenb-developer-doc.2.patch.gz) and [attachment: trac-12229-manifest.patch](https://github.com/sagemath/sage/files/ticket12229/trac-12229-manifest.patch.gz)
``````




---

archive/issue_comments_132290.json:
```json
{
    "body": "<div id=\"comment:17\" align=\"right\">comment:17</div>\n\nSince it's just the MANIFEST.in thing, I'll have a look at it.  I forgot this still needed review...",
    "created_at": "2012-02-07T22:27:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132290",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:17" align="right">comment:17</div>

Since it's just the MANIFEST.in thing, I'll have a look at it.  I forgot this still needed review...



---

archive/issue_comments_132291.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">comment:18</div>\n\nThe links to kernel.org in `doc/en/developer/sagenb/git_links.inc` no longer work. Is that just a fallout of the kernel.org downtime from a couple of months ago, or has the location of the docs changed? From a quick Google search, I don't see any other \"official\" place which can be referred.",
    "created_at": "2012-03-05T10:43:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132291",
    "user": "https://github.com/ppurka"
}
```

<div id="comment:18" align="right">comment:18</div>

The links to kernel.org in `doc/en/developer/sagenb/git_links.inc` no longer work. Is that just a fallout of the kernel.org downtime from a couple of months ago, or has the location of the docs changed? From a quick Google search, I don't see any other "official" place which can be referred.



---

archive/issue_comments_132292.json:
```json
{
    "body": "Changed reviewer from **William Stein** to **William Stein, Jeroen Demeyer, Punarbasu Purkayastha**",
    "created_at": "2012-03-15T18:53:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132292",
    "user": "https://github.com/kcrisman"
}
```

Changed reviewer from **William Stein** to **William Stein, Jeroen Demeyer, Punarbasu Purkayastha**



---

archive/issue_comments_132293.json:
```json
{
    "body": "<div id=\"comment:19\" align=\"right\">comment:19</div>\n\n> The links to kernel.org in `doc/en/developer/sagenb/git_links.inc` no longer work. Is that just a fallout of the kernel.org downtime from a couple of months ago, or has the location of the docs changed? From a quick Google search, I don't see any other \"official\" place which can be referred.\n\nIt looks like most of the doc has moved, e.g. [the user manual](http://schacon.github.com/git/user-manual.html) and the [tutorial](http://schacon.github.com/git/gittutorial.html).  Since everything looks the same, just with the `http://shacon.github.com/git/` prefix, presumably that's all that has to be changed.  In fact, it looks like some kind of `sed` on the patch replacing `http://www.kernel.org/pub/software/scm/git/docs` with `http://schacon.github.com/git` would suffice.\n\nSo for positive review one needs\n* to fix the links (which wouldn't even need a separate export, it could just be done on the patch file)\n* to check the MANIFEST.in patch solves the other problem, which I won't touch with a ten-foot pole.",
    "created_at": "2012-03-15T18:53:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132293",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:19" align="right">comment:19</div>

> The links to kernel.org in `doc/en/developer/sagenb/git_links.inc` no longer work. Is that just a fallout of the kernel.org downtime from a couple of months ago, or has the location of the docs changed? From a quick Google search, I don't see any other "official" place which can be referred.

It looks like most of the doc has moved, e.g. [the user manual](http://schacon.github.com/git/user-manual.html) and the [tutorial](http://schacon.github.com/git/gittutorial.html).  Since everything looks the same, just with the `http://shacon.github.com/git/` prefix, presumably that's all that has to be changed.  In fact, it looks like some kind of `sed` on the patch replacing `http://www.kernel.org/pub/software/scm/git/docs` with `http://schacon.github.com/git` would suffice.

So for positive review one needs
* to fix the links (which wouldn't even need a separate export, it could just be done on the patch file)
* to check the MANIFEST.in patch solves the other problem, which I won't touch with a ten-foot pole.



---

archive/issue_comments_132294.json:
```json
{
    "body": "<div id=\"comment:20\" align=\"right\">comment:20</div>\n\nPositive review to [attachment: trac-12229-manifest.patch](https://github.com/sagemath/sage/files/ticket12229/trac-12229-manifest.patch.gz) (I posted a new version which is simply a rebase).",
    "created_at": "2012-03-19T10:18:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132294",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:20" align="right">comment:20</div>

Positive review to [attachment: trac-12229-manifest.patch](https://github.com/sagemath/sage/files/ticket12229/trac-12229-manifest.patch.gz) (I posted a new version which is simply a rebase).



---

archive/issue_comments_132295.json:
```json
{
    "body": "<div id=\"comment:21\" align=\"right\">comment:21</div>\n\nThe index.rst also needs to be updated to reflect the move to git and github.",
    "created_at": "2012-03-30T06:26:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132295",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:21" align="right">comment:21</div>

The index.rst also needs to be updated to reflect the move to git and github.



---

archive/issue_comments_132296.json:
```json
{
    "body": "<div id=\"comment:22\" align=\"right\">comment:22</div>\n\nAttachment: **[trac-12229-manifest.patch.gz](https://github.com/sagemath/sage/files/ticket12229/trac-12229-manifest.patch.gz)**",
    "created_at": "2012-04-08T08:39:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132296",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:22" align="right">comment:22</div>

Attachment: **[trac-12229-manifest.patch.gz](https://github.com/sagemath/sage/files/ticket12229/trac-12229-manifest.patch.gz)**



---

archive/issue_events_167027.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-04-08T08:39:32Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167027"
}
```



---

archive/issue_events_167028.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-04-08T08:39:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "milestone_number": null,
    "milestone_title": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167028"
}
```



---

archive/issue_comments_132297.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,4 +2,6 @@\n \n Context: [http://groups.google.com/group/sage-notebook/t/3fa33efda8db5c94](http://groups.google.com/group/sage-notebook/t/3fa33efda8db5c94)\n \n-Apply: [attachment: trac-12229-sagenb-developer-doc.2.patch](https://github.com/sagemath/sage/files/ticket12229/trac-12229-sagenb-developer-doc.2.patch.gz) and [attachment: trac-12229-manifest.patch](https://github.com/sagemath/sage/files/ticket12229/trac-12229-manifest.patch.gz)\n+**Apply**: [attachment: trac-12229-sagenb-developer-doc.2.patch](https://github.com/sagemath/sage/files/ticket12229/trac-12229-sagenb-developer-doc.2.patch.gz) and [attachment: trac-12229-manifest.patch](https://github.com/sagemath/sage/files/ticket12229/trac-12229-manifest.patch.gz)\n+\n+A built version of the documentation can be found at [http://boxen.math.washington.edu/home/release/sage-5.0.notebook/doc/en/developer/sagenb/index.html](http://boxen.math.washington.edu/home/release/sage-5.0.notebook/doc/en/developer/sagenb/index.html)\n``````\n",
    "created_at": "2012-04-10T21:20:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132297",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,4 +2,6 @@
 
 Context: [http://groups.google.com/group/sage-notebook/t/3fa33efda8db5c94](http://groups.google.com/group/sage-notebook/t/3fa33efda8db5c94)
 
-Apply: [attachment: trac-12229-sagenb-developer-doc.2.patch](https://github.com/sagemath/sage/files/ticket12229/trac-12229-sagenb-developer-doc.2.patch.gz) and [attachment: trac-12229-manifest.patch](https://github.com/sagemath/sage/files/ticket12229/trac-12229-manifest.patch.gz)
+**Apply**: [attachment: trac-12229-sagenb-developer-doc.2.patch](https://github.com/sagemath/sage/files/ticket12229/trac-12229-sagenb-developer-doc.2.patch.gz) and [attachment: trac-12229-manifest.patch](https://github.com/sagemath/sage/files/ticket12229/trac-12229-manifest.patch.gz)
+
+A built version of the documentation can be found at [http://boxen.math.washington.edu/home/release/sage-5.0.notebook/doc/en/developer/sagenb/index.html](http://boxen.math.washington.edu/home/release/sage-5.0.notebook/doc/en/developer/sagenb/index.html)
``````




---

archive/issue_comments_132298.json:
```json
{
    "body": "<div id=\"comment:24\" align=\"right\">comment:24</div>\n\nI think the documentation should mention [http://code.google.com/p/sagenb/](http://code.google.com/p/sagenb/) and what its purpose is, something equivalent to [http://boxen.math.washington.edu/home/release/sage-5.0.notebook/doc/en/developer/trac.html](http://boxen.math.washington.edu/home/release/sage-5.0.notebook/doc/en/developer/trac.html).  On that page about Trac, add a comment that this should not be used for Notebook-specific things (unfortunately, it's not always clear whether a bug is notebook-specific).",
    "created_at": "2012-04-10T21:36:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132298",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:24" align="right">comment:24</div>

I think the documentation should mention [http://code.google.com/p/sagenb/](http://code.google.com/p/sagenb/) and what its purpose is, something equivalent to [http://boxen.math.washington.edu/home/release/sage-5.0.notebook/doc/en/developer/trac.html](http://boxen.math.washington.edu/home/release/sage-5.0.notebook/doc/en/developer/trac.html).  On that page about Trac, add a comment that this should not be used for Notebook-specific things (unfortunately, it's not always clear whether a bug is notebook-specific).



---

archive/issue_events_167029.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-04-10T21:36:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167029"
}
```



---

archive/issue_events_167030.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-04-10T21:36:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167030"
}
```



---

archive/issue_comments_132299.json:
```json
{
    "body": "Work Issues: **Google code**",
    "created_at": "2012-04-10T21:36:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132299",
    "user": "https://github.com/jdemeyer"
}
```

Work Issues: **Google code**



---

archive/issue_comments_132300.json:
```json
{
    "body": "<div id=\"comment:25\" align=\"right\">comment:25</div>\n\ncode.google.com serves no purpose any more.  All development has moved to github.  There may be tickets at code.google.com that still need to be moved over, thought.",
    "created_at": "2012-04-10T23:06:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132300",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:25" align="right">comment:25</div>

code.google.com serves no purpose any more.  All development has moved to github.  There may be tickets at code.google.com that still need to be moved over, thought.



---

archive/issue_comments_132301.json:
```json
{
    "body": "<div id=\"comment:26\" align=\"right\">comment:26</div>\n\nReplying to [@jasongrout](#comment:25):\n> code.google.com serves no purpose any more.  All development has moved to github.  There may be tickets at code.google.com that still need to be moved over, thought.\n\nThen I guess http://nb.sagemath.org/ should be updated too.",
    "created_at": "2012-04-11T06:21:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132301",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:26" align="right">comment:26</div>

Replying to [@jasongrout](#comment:25):
> code.google.com serves no purpose any more.  All development has moved to github.  There may be tickets at code.google.com that still need to be moved over, thought.

Then I guess http://nb.sagemath.org/ should be updated too.



---

archive/issue_events_167031.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-04-11T06:21:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167031"
}
```



---

archive/issue_events_167032.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-04-11T06:21:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167032"
}
```



---

archive/issue_comments_132302.json:
```json
{
    "body": "Changed work issues from **Google code** to none",
    "created_at": "2012-04-11T06:21:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132302",
    "user": "https://github.com/jdemeyer"
}
```

Changed work issues from **Google code** to none



---

archive/issue_comments_132303.json:
```json
{
    "body": "<div id=\"comment:27\" align=\"right\">comment:27</div>\n\nReplying to [@jdemeyer](#comment:26):\n> Then I guess http://nb.sagemath.org/ should be updated too.\n\nAnd the google code page should inform the reader that they should look at github.",
    "created_at": "2012-04-11T06:26:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132303",
    "user": "https://github.com/ppurka"
}
```

<div id="comment:27" align="right">comment:27</div>

Replying to [@jdemeyer](#comment:26):
> Then I guess http://nb.sagemath.org/ should be updated too.

And the google code page should inform the reader that they should look at github.



---

archive/issue_comments_132304.json:
```json
{
    "body": "<div id=\"comment:28\" align=\"right\">comment:28</div>\n\nReplying to [@ppurka](#comment:27):\n> And the google code page should inform the reader that they should look at github.\n\nDone.",
    "created_at": "2012-04-11T06:32:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132304",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:28" align="right">comment:28</div>

Replying to [@ppurka](#comment:27):
> And the google code page should inform the reader that they should look at github.

Done.



---

archive/issue_comments_132305.json:
```json
{
    "body": "<div id=\"comment:29\" align=\"right\">comment:29</div>\n\nIt would be nice if we could keep this documentation in sagenb itself and somehow link it into the Sage doc building process. But I guess that can be fixed later.\n\nBTW, for reasons I stated [here](http://article.gmane.org/gmane.comp.mathematics.sage.devel/56956/) I don't think we should be teaching people how to rebase...",
    "created_at": "2012-05-02T11:59:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132305",
    "user": "https://github.com/kini"
}
```

<div id="comment:29" align="right">comment:29</div>

It would be nice if we could keep this documentation in sagenb itself and somehow link it into the Sage doc building process. But I guess that can be fixed later.

BTW, for reasons I stated [here](http://article.gmane.org/gmane.comp.mathematics.sage.devel/56956/) I don't think we should be teaching people how to rebase...



---

archive/issue_comments_132306.json:
```json
{
    "body": "Updated patch with fixed git links",
    "created_at": "2012-05-05T17:00:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132306",
    "user": "https://github.com/ppurka"
}
```

Updated patch with fixed git links



---

archive/issue_comments_132307.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,6 +2,6 @@\n \n Context: [http://groups.google.com/group/sage-notebook/t/3fa33efda8db5c94](http://groups.google.com/group/sage-notebook/t/3fa33efda8db5c94)\n \n-**Apply**: [attachment: trac-12229-sagenb-developer-doc.2.patch](https://github.com/sagemath/sage/files/ticket12229/trac-12229-sagenb-developer-doc.2.patch.gz) and [attachment: trac-12229-manifest.patch](https://github.com/sagemath/sage/files/ticket12229/trac-12229-manifest.patch.gz)\n+**Apply**: [attachment: trac_12229-sagenb-developer-doc.3.patch](https://github.com/sagemath/sage/files/ticket12229/trac_12229-sagenb-developer-doc.3.patch.gz) and [attachment: trac-12229-manifest.patch](https://github.com/sagemath/sage/files/ticket12229/trac-12229-manifest.patch.gz)\n \n A built version of the documentation can be found at [http://boxen.math.washington.edu/home/release/sage-5.0.notebook/doc/en/developer/sagenb/index.html](http://boxen.math.washington.edu/home/release/sage-5.0.notebook/doc/en/developer/sagenb/index.html)\n``````\n",
    "created_at": "2012-05-05T17:04:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132307",
    "user": "https://github.com/ppurka"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,6 +2,6 @@
 
 Context: [http://groups.google.com/group/sage-notebook/t/3fa33efda8db5c94](http://groups.google.com/group/sage-notebook/t/3fa33efda8db5c94)
 
-**Apply**: [attachment: trac-12229-sagenb-developer-doc.2.patch](https://github.com/sagemath/sage/files/ticket12229/trac-12229-sagenb-developer-doc.2.patch.gz) and [attachment: trac-12229-manifest.patch](https://github.com/sagemath/sage/files/ticket12229/trac-12229-manifest.patch.gz)
+**Apply**: [attachment: trac_12229-sagenb-developer-doc.3.patch](https://github.com/sagemath/sage/files/ticket12229/trac_12229-sagenb-developer-doc.3.patch.gz) and [attachment: trac-12229-manifest.patch](https://github.com/sagemath/sage/files/ticket12229/trac-12229-manifest.patch.gz)
 
 A built version of the documentation can be found at [http://boxen.math.washington.edu/home/release/sage-5.0.notebook/doc/en/developer/sagenb/index.html](http://boxen.math.washington.edu/home/release/sage-5.0.notebook/doc/en/developer/sagenb/index.html)
``````




---

archive/issue_comments_132308.json:
```json
{
    "body": "<div id=\"comment:30\" align=\"right\">comment:30</div>\n\nAttachment: **[trac_12229-sagenb-developer-doc.3.patch.gz](https://github.com/sagemath/sage/files/ticket12229/trac_12229-sagenb-developer-doc.3.patch.gz)**\n\nThe git documentation url hasn't changed in several months, so I updated the patch with the fixed documentation. The [diff file](https://github.com/sagemath/sage/files/ticket12229/sagenb-developer-doc-patch-diff.patch.gz) shows the changes to the *patch*.\n\nUnfortunately, I don't have permission to overwrite the earlier patch. So, I uploaded a differently named one.",
    "created_at": "2012-05-05T17:04:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132308",
    "user": "https://github.com/ppurka"
}
```

<div id="comment:30" align="right">comment:30</div>

Attachment: **[trac_12229-sagenb-developer-doc.3.patch.gz](https://github.com/sagemath/sage/files/ticket12229/trac_12229-sagenb-developer-doc.3.patch.gz)**

The git documentation url hasn't changed in several months, so I updated the patch with the fixed documentation. The [diff file](https://github.com/sagemath/sage/files/ticket12229/sagenb-developer-doc-patch-diff.patch.gz) shows the changes to the *patch*.

Unfortunately, I don't have permission to overwrite the earlier patch. So, I uploaded a differently named one.



---

archive/issue_comments_132309.json:
```json
{
    "body": "<div id=\"comment:31\" align=\"right\">comment:31</div>\n\nA question: where does the documentation get built? And how can I view it? I don't see it from the help link in the notebook.",
    "created_at": "2012-05-05T17:15:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132309",
    "user": "https://github.com/ppurka"
}
```

<div id="comment:31" align="right">comment:31</div>

A question: where does the documentation get built? And how can I view it? I don't see it from the help link in the notebook.



---

archive/issue_comments_132310.json:
```json
{
    "body": "<div id=\"comment:32\" align=\"right\">comment:32</div>\n\nReplying to [@ppurka](#comment:31):\n> A question: where does the documentation get built? And how can I view it? I don't see it from the help link in the notebook.\n\nSorry, I needed to build the documentation. Now it appears.",
    "created_at": "2012-05-05T17:50:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132310",
    "user": "https://github.com/ppurka"
}
```

<div id="comment:32" align="right">comment:32</div>

Replying to [@ppurka](#comment:31):
> A question: where does the documentation get built? And how can I view it? I don't see it from the help link in the notebook.

Sorry, I needed to build the documentation. Now it appears.



---

archive/issue_comments_132311.json:
```json
{
    "body": "<div id=\"comment:33\" align=\"right\">comment:33</div>\n\nIt would be nice to know what is remaining to review here.",
    "created_at": "2012-05-24T22:26:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132311",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:33" align="right">comment:33</div>

It would be nice to know what is remaining to review here.



---

archive/issue_comments_132312.json:
```json
{
    "body": "<div id=\"comment:34\" align=\"right\">comment:34</div>\n\npatchbot: apply trac_12229-sagenb-developer-doc.3.patch trac-12229-manifest.patch",
    "created_at": "2012-05-24T22:28:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132312",
    "user": "https://github.com/kini"
}
```

<div id="comment:34" align="right">comment:34</div>

patchbot: apply trac_12229-sagenb-developer-doc.3.patch trac-12229-manifest.patch



---

archive/issue_comments_132313.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -3,5 +3,3 @@\n Context: [http://groups.google.com/group/sage-notebook/t/3fa33efda8db5c94](http://groups.google.com/group/sage-notebook/t/3fa33efda8db5c94)\n \n **Apply**: [attachment: trac_12229-sagenb-developer-doc.3.patch](https://github.com/sagemath/sage/files/ticket12229/trac_12229-sagenb-developer-doc.3.patch.gz) and [attachment: trac-12229-manifest.patch](https://github.com/sagemath/sage/files/ticket12229/trac-12229-manifest.patch.gz)\n-\n-A built version of the documentation can be found at [http://boxen.math.washington.edu/home/release/sage-5.0.notebook/doc/en/developer/sagenb/index.html](http://boxen.math.washington.edu/home/release/sage-5.0.notebook/doc/en/developer/sagenb/index.html)\n``````\n",
    "created_at": "2012-06-01T16:40:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132313",
    "user": "https://github.com/kcrisman"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -3,5 +3,3 @@
 Context: [http://groups.google.com/group/sage-notebook/t/3fa33efda8db5c94](http://groups.google.com/group/sage-notebook/t/3fa33efda8db5c94)
 
 **Apply**: [attachment: trac_12229-sagenb-developer-doc.3.patch](https://github.com/sagemath/sage/files/ticket12229/trac_12229-sagenb-developer-doc.3.patch.gz) and [attachment: trac-12229-manifest.patch](https://github.com/sagemath/sage/files/ticket12229/trac-12229-manifest.patch.gz)
-
-A built version of the documentation can be found at [http://boxen.math.washington.edu/home/release/sage-5.0.notebook/doc/en/developer/sagenb/index.html](http://boxen.math.washington.edu/home/release/sage-5.0.notebook/doc/en/developer/sagenb/index.html)
``````




---

archive/issue_events_167033.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-06-01T17:39:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167033"
}
```



---

archive/issue_events_167034.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-06-01T17:39:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167034"
}
```



---

archive/issue_comments_132314.json:
```json
{
    "body": "Changed reviewer from **William Stein, Jeroen Demeyer, Punarbasu Purkayastha** to **William Stein, Jeroen Demeyer, Punarbasu Purkayastha, Karl-Dieter Crisman**",
    "created_at": "2012-06-01T17:39:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132314",
    "user": "https://github.com/kcrisman"
}
```

Changed reviewer from **William Stein, Jeroen Demeyer, Punarbasu Purkayastha** to **William Stein, Jeroen Demeyer, Punarbasu Purkayastha, Karl-Dieter Crisman**



---

archive/issue_comments_132315.json:
```json
{
    "body": "<div id=\"comment:36\" align=\"right\">comment:36</div>\n\nOkay, I have a few questions.  Mostly this is because of the annoying autogenerated thing this is from.\n* The file `this_project.inc` has\n\n```\n.. Sage Notebook\n.. _`Sage Notebook`: http://sagemath.org\n.. _`Sage Notebook github`: http://github.com/sagemath/sagenb\n\n.. _`Sage Notebook mailing list`: http://groups.google.com/group/sage-notebook\n```\n  but I doubt we want the Sage notebook site to be sagemath.org, as there are a number of links to this.  What is the best \"generic\" Sage notebook URL?\n* I don't understand why there are several references to sending files to the Sage notebook devel list.  That is really not at all what we want it to do!  So what *do* we want it to do?  Surely not to upload to Trac ;-)\n* \"Hardcore forking action\" is semi-offensive and unfortunately part of the old crude hacking humor thing.  We should delete it.\n* There is a lot of reference to just sagenb, but in reality one might want it to be inside the devel directory (well, or whatever happens in the \"new\" Sage tree someday).\n\n\nMy recommendation: make it **really really obvious** that this is absolutely generic documentation, and fix the things above.  This should all be doable in index.rst and a few other places.   I can do some of this, but am not sure what the answers to the first two questions are.\n\nAlso, although this should still be a blocker for whatever release the new Sage notebook goes in, I don't think that the new notebook should depend on it; we want people testing it as soon as possible.  By the way, once the new notebook goes in, do we still need the removal and recreation of the symlink?  I guess so...",
    "created_at": "2012-06-01T17:39:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132315",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:36" align="right">comment:36</div>

Okay, I have a few questions.  Mostly this is because of the annoying autogenerated thing this is from.
* The file `this_project.inc` has

```
.. Sage Notebook
.. _`Sage Notebook`: http://sagemath.org
.. _`Sage Notebook github`: http://github.com/sagemath/sagenb

.. _`Sage Notebook mailing list`: http://groups.google.com/group/sage-notebook
```
  but I doubt we want the Sage notebook site to be sagemath.org, as there are a number of links to this.  What is the best "generic" Sage notebook URL?
* I don't understand why there are several references to sending files to the Sage notebook devel list.  That is really not at all what we want it to do!  So what *do* we want it to do?  Surely not to upload to Trac ;-)
* "Hardcore forking action" is semi-offensive and unfortunately part of the old crude hacking humor thing.  We should delete it.
* There is a lot of reference to just sagenb, but in reality one might want it to be inside the devel directory (well, or whatever happens in the "new" Sage tree someday).


My recommendation: make it **really really obvious** that this is absolutely generic documentation, and fix the things above.  This should all be doable in index.rst and a few other places.   I can do some of this, but am not sure what the answers to the first two questions are.

Also, although this should still be a blocker for whatever release the new Sage notebook goes in, I don't think that the new notebook should depend on it; we want people testing it as soon as possible.  By the way, once the new notebook goes in, do we still need the removal and recreation of the symlink?  I guess so...



---

archive/issue_events_167035.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-02T14:39:40Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "milestone_number": null,
    "milestone_title": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167035"
}
```



---

archive/issue_events_167036.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-02T14:39:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "milestone_number": null,
    "milestone_title": "sage-5.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167036"
}
```



---

archive/issue_comments_132316.json:
```json
{
    "body": "<div id=\"comment:38\" align=\"right\">comment:38</div>\n\nReplying to [@kcrisman](#comment:36):\n> Okay, I have a few questions.  Mostly this is because of the annoying autogenerated thing this is from.\n\nI agree about it being annoying - why are we using an autogenerated thing, anyway? Our mercurial docs aren't autogenerated.\n\n> * The file `this_project.inc` has\n> \n> ```\n> .. Sage Notebook\n> .. _`Sage Notebook`: http://sagemath.org\n> .. _`Sage Notebook github`: http://github.com/sagemath/sagenb\n> \n> .. _`Sage Notebook mailing list`: http://groups.google.com/group/sage-notebook\n> ```\n>   but I doubt we want the Sage notebook site to be sagemath.org, as there are a number of links to this.  What is the best \"generic\" Sage notebook URL?\n\nWell, ideally it is http://nb.sagemath.org/ , but that page is extremely outdated right now. Someone should update it.\n\n> * I don't understand why there are several references to sending files to the Sage notebook devel list.  That is really not at all what we want it to do!  So what *do* we want it to do?  Surely not to upload to Trac ;-)\n> * \"Hardcore forking action\" is semi-offensive and unfortunately part of the old crude hacking humor thing.  We should delete it.\n\nThose actual words appear on github when you fork a repository, by the way.\n\n> * There is a lot of reference to just sagenb, but in reality one might want it to be inside the devel directory (well, or whatever happens in the \"new\" Sage tree someday).\n\nOnce the whole Sage tree switches to git (#13015), this autogenerated stuff will be even less adequate. At least now it seems that most sagenb contributors are fairly experienced at contributing to open source projects. That is not true of sage library contributors.\n\n> My recommendation: make it **really really obvious** that this is absolutely generic documentation, and fix the things above.  This should all be doable in index.rst and a few other places.   I can do some of this, but am not sure what the answers to the first two questions are.\n> \n> Also, although this should still be a blocker for whatever release the new Sage notebook goes in, I don't think that the new notebook should depend on it; we want people testing it as soon as possible.  By the way, once the new notebook goes in, do we still need the removal and recreation of the symlink?  I guess so...\n\nYou mean the symlink `$SAGE_ROOT/devel/sagenb`? Of course not :)",
    "created_at": "2012-06-11T02:21:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132316",
    "user": "https://github.com/kini"
}
```

<div id="comment:38" align="right">comment:38</div>

Replying to [@kcrisman](#comment:36):
> Okay, I have a few questions.  Mostly this is because of the annoying autogenerated thing this is from.

I agree about it being annoying - why are we using an autogenerated thing, anyway? Our mercurial docs aren't autogenerated.

> * The file `this_project.inc` has
> 
> ```
> .. Sage Notebook
> .. _`Sage Notebook`: http://sagemath.org
> .. _`Sage Notebook github`: http://github.com/sagemath/sagenb
> 
> .. _`Sage Notebook mailing list`: http://groups.google.com/group/sage-notebook
> ```
>   but I doubt we want the Sage notebook site to be sagemath.org, as there are a number of links to this.  What is the best "generic" Sage notebook URL?

Well, ideally it is http://nb.sagemath.org/ , but that page is extremely outdated right now. Someone should update it.

> * I don't understand why there are several references to sending files to the Sage notebook devel list.  That is really not at all what we want it to do!  So what *do* we want it to do?  Surely not to upload to Trac ;-)
> * "Hardcore forking action" is semi-offensive and unfortunately part of the old crude hacking humor thing.  We should delete it.

Those actual words appear on github when you fork a repository, by the way.

> * There is a lot of reference to just sagenb, but in reality one might want it to be inside the devel directory (well, or whatever happens in the "new" Sage tree someday).

Once the whole Sage tree switches to git (#13015), this autogenerated stuff will be even less adequate. At least now it seems that most sagenb contributors are fairly experienced at contributing to open source projects. That is not true of sage library contributors.

> My recommendation: make it **really really obvious** that this is absolutely generic documentation, and fix the things above.  This should all be doable in index.rst and a few other places.   I can do some of this, but am not sure what the answers to the first two questions are.
> 
> Also, although this should still be a blocker for whatever release the new Sage notebook goes in, I don't think that the new notebook should depend on it; we want people testing it as soon as possible.  By the way, once the new notebook goes in, do we still need the removal and recreation of the symlink?  I guess so...

You mean the symlink `$SAGE_ROOT/devel/sagenb`? Of course not :)



---

archive/issue_comments_132317.json:
```json
{
    "body": "<div id=\"comment:39\" align=\"right\">comment:39</div>\n\n> > Okay, I have a few questions.  Mostly this is because of the annoying autogenerated thing this is from.\n\n> I agree about it being annoying - why are we using an autogenerated thing, anyway? Our mercurial docs aren't autogenerated.\n\nBecause silly people like me complained that the sagenb should have documentation for how to develop included when upgraded, and this was convenient.  Really, the autogen is not a problem, it's that it hasn't been made more clear what is boilerplate.\n\n> > * The file `this_project.inc` has\n> > .. _`Sage Notebook`: http://sagemath.org\n\n> Well, ideally it is http://nb.sagemath.org/ , but that page is extremely outdated right now. Someone should update it.\n\nRight, but who can do that?  I don't know who has admin rights to that other than William, who isn't really involved in this upgrade that much.\n\n> > * I don't understand why there are several references to sending files to the Sage notebook devel list.  That is really not at all what we want it to do!  So what *do* we want it to do?  Surely not to upload to Trac ;-)\n\nAnswering my own question - presumably to open an issue on github and post code there?\n\n> > * \"Hardcore forking action\" is semi-offensive and unfortunately part of the old crude hacking humor thing.  We should delete it.\n\n> Those actual words appear on github when you fork a repository, by the way.\n\nTotally unprofessional.\n\n> > * There is a lot of reference to just sagenb, but in reality one might want it to be inside the devel directory (well, or whatever happens in the \"new\" Sage tree someday).\n\n> \n> Once the whole Sage tree switches to git (#13015), this autogenerated stuff will be even less adequate. At least now it seems that most sagenb contributors are fairly experienced at contributing to open source projects. That is not true of sage library contributors.\n\nI would distinguish between \"contributing to open source projects\" and \"using these specific tools in such contribution.  Anyway, for now I think this is less problematic.\n\n> > My recommendation: make it **really really obvious** that this is absolutely generic documentation, and fix the things above.  This should all be doable in index.rst and a few other places.   I can do some of this, but am not sure what the answers to the first two questions are.\n\n> > Also, although this should still be a blocker for whatever release the new Sage notebook goes in, I don't think that the new notebook should depend on it; we want people testing it as soon as possible.  By the way, once the new notebook goes in, do we still need the removal and recreation of the symlink?  I guess so...\n\n> You mean the symlink `$SAGE_ROOT/devel/sagenb`? Of course not :)\n\nSo yet another thing that should be changed.\n\nKeshav, do you think you can take a stab at making a new patch with these changes?  Or ppurka if he is participating in the notebook days remotely.  I figure that this would be pretty compelling for a good use of time at a **notebook** days :) but let me know if not; probably over the next couple weeks I could do a very bad job at fixing a few of them.\n\nMaybe you can bug someone about the nb.sagemath.org stuff this week too.  That would be really great.",
    "created_at": "2012-06-11T18:06:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132317",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:39" align="right">comment:39</div>

> > Okay, I have a few questions.  Mostly this is because of the annoying autogenerated thing this is from.

> I agree about it being annoying - why are we using an autogenerated thing, anyway? Our mercurial docs aren't autogenerated.

Because silly people like me complained that the sagenb should have documentation for how to develop included when upgraded, and this was convenient.  Really, the autogen is not a problem, it's that it hasn't been made more clear what is boilerplate.

> > * The file `this_project.inc` has
> > .. _`Sage Notebook`: http://sagemath.org

> Well, ideally it is http://nb.sagemath.org/ , but that page is extremely outdated right now. Someone should update it.

Right, but who can do that?  I don't know who has admin rights to that other than William, who isn't really involved in this upgrade that much.

> > * I don't understand why there are several references to sending files to the Sage notebook devel list.  That is really not at all what we want it to do!  So what *do* we want it to do?  Surely not to upload to Trac ;-)

Answering my own question - presumably to open an issue on github and post code there?

> > * "Hardcore forking action" is semi-offensive and unfortunately part of the old crude hacking humor thing.  We should delete it.

> Those actual words appear on github when you fork a repository, by the way.

Totally unprofessional.

> > * There is a lot of reference to just sagenb, but in reality one might want it to be inside the devel directory (well, or whatever happens in the "new" Sage tree someday).

> 
> Once the whole Sage tree switches to git (#13015), this autogenerated stuff will be even less adequate. At least now it seems that most sagenb contributors are fairly experienced at contributing to open source projects. That is not true of sage library contributors.

I would distinguish between "contributing to open source projects" and "using these specific tools in such contribution.  Anyway, for now I think this is less problematic.

> > My recommendation: make it **really really obvious** that this is absolutely generic documentation, and fix the things above.  This should all be doable in index.rst and a few other places.   I can do some of this, but am not sure what the answers to the first two questions are.

> > Also, although this should still be a blocker for whatever release the new Sage notebook goes in, I don't think that the new notebook should depend on it; we want people testing it as soon as possible.  By the way, once the new notebook goes in, do we still need the removal and recreation of the symlink?  I guess so...

> You mean the symlink `$SAGE_ROOT/devel/sagenb`? Of course not :)

So yet another thing that should be changed.

Keshav, do you think you can take a stab at making a new patch with these changes?  Or ppurka if he is participating in the notebook days remotely.  I figure that this would be pretty compelling for a good use of time at a **notebook** days :) but let me know if not; probably over the next couple weeks I could do a very bad job at fixing a few of them.

Maybe you can bug someone about the nb.sagemath.org stuff this week too.  That would be really great.



---

archive/issue_comments_132318.json:
```json
{
    "body": "<div id=\"comment:40\" align=\"right\">comment:40</div>\n\n> Maybe you can bug someone about the nb.sagemath.org stuff this week too.  That would be really great.\n\nUpdate: sounds like we can just ask Harald to do something.  I've done that.\n\nWe definitely *do* need some ideas on what the right place to send people is for developing.  The current page [here](http://nb.sagemath.org/dev.html) is not right at all, but I don't know whether pointing people to this documentation is best or not.  However, I don't see much on the github site that is accessible, and we don't want to point them to #11080 once the new notebook is in!",
    "created_at": "2012-06-11T18:10:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132318",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:40" align="right">comment:40</div>

> Maybe you can bug someone about the nb.sagemath.org stuff this week too.  That would be really great.

Update: sounds like we can just ask Harald to do something.  I've done that.

We definitely *do* need some ideas on what the right place to send people is for developing.  The current page [here](http://nb.sagemath.org/dev.html) is not right at all, but I don't know whether pointing people to this documentation is best or not.  However, I don't see much on the github site that is accessible, and we don't want to point them to #11080 once the new notebook is in!



---

archive/issue_comments_132319.json:
```json
{
    "body": "<div id=\"comment:41\" align=\"right\">comment:41</div>\n\n+1 to KDC's comments about changing the Hardcore thing...",
    "created_at": "2012-06-11T23:19:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132319",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:41" align="right">comment:41</div>

+1 to KDC's comments about changing the Hardcore thing...



---

archive/issue_comments_132320.json:
```json
{
    "body": "<div id=\"comment:42\" align=\"right\">comment:42</div>\n\nI haven't heard back from Harald.   \n\nThis is clearly not an ideal setup overall, but as long as we have a very clear first page saying that everything is generic, and giving all relevant links, it's better than not having documentation at all.  I don't know who would have time, interest, and expertise to do something more, though that would clearly be better.",
    "created_at": "2012-06-15T03:14:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132320",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:42" align="right">comment:42</div>

I haven't heard back from Harald.   

This is clearly not an ideal setup overall, but as long as we have a very clear first page saying that everything is generic, and giving all relevant links, it's better than not having documentation at all.  I don't know who would have time, interest, and expertise to do something more, though that would clearly be better.



---

archive/issue_comments_132321.json:
```json
{
    "body": "<div id=\"comment:43\" align=\"right\">comment:43</div>\n\n> I haven't heard back from Harald.   \n\nOkay, he has graciously updated the page a lot.  If we change\n\n```\n.. _`Sage Notebook`: http://sagemath.org\n```\nto \n\n```\n.. _`Sage Notebook`: http://nb.sagemath.org\n```\nin `this_project.inc` that will solve many of our problems.\n\n> This is clearly not an ideal setup overall, but as long as we have a very clear first page saying that everything is generic, and giving all relevant links, it's better than not having documentation at all.  I don't know who would have time, interest, and expertise to do something more, though that would clearly be better.\n\nI'm just going to change the things that need to be done myself.  We need to get this stuff in.  Patch hopefully coming up.",
    "created_at": "2012-06-15T15:11:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132321",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:43" align="right">comment:43</div>

> I haven't heard back from Harald.   

Okay, he has graciously updated the page a lot.  If we change

```
.. _`Sage Notebook`: http://sagemath.org
```
to 

```
.. _`Sage Notebook`: http://nb.sagemath.org
```
in `this_project.inc` that will solve many of our problems.

> This is clearly not an ideal setup overall, but as long as we have a very clear first page saying that everything is generic, and giving all relevant links, it's better than not having documentation at all.  I don't know who would have time, interest, and expertise to do something more, though that would clearly be better.

I'm just going to change the things that need to be done myself.  We need to get this stuff in.  Patch hopefully coming up.



---

archive/issue_comments_132322.json:
```json
{
    "body": "Changed author from **Jason Grout** to **Jason Grout, Karl-Dieter Crisman**",
    "created_at": "2012-06-15T17:36:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132322",
    "user": "https://github.com/kcrisman"
}
```

Changed author from **Jason Grout** to **Jason Grout, Karl-Dieter Crisman**



---

archive/issue_comments_132323.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,4 +2,4 @@\n \n Context: [http://groups.google.com/group/sage-notebook/t/3fa33efda8db5c94](http://groups.google.com/group/sage-notebook/t/3fa33efda8db5c94)\n \n-**Apply**: [attachment: trac_12229-sagenb-developer-doc.3.patch](https://github.com/sagemath/sage/files/ticket12229/trac_12229-sagenb-developer-doc.3.patch.gz) and [attachment: trac-12229-manifest.patch](https://github.com/sagemath/sage/files/ticket12229/trac-12229-manifest.patch.gz)\n+**Apply**: [attachment: trac_12229-sagenb-developer-doc.3.patch](https://github.com/sagemath/sage/files/ticket12229/trac_12229-sagenb-developer-doc.3.patch.gz) and [attachment: trac-12229-manifest.patch](https://github.com/sagemath/sage/files/ticket12229/trac-12229-manifest.patch.gz) and [attachment: trac_12229-reviewer.patch](https://github.com/sagemath/sage/files/ticket12229/trac_12229-reviewer.patch.gz).\n``````\n",
    "created_at": "2012-06-15T17:36:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132323",
    "user": "https://github.com/kcrisman"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,4 +2,4 @@
 
 Context: [http://groups.google.com/group/sage-notebook/t/3fa33efda8db5c94](http://groups.google.com/group/sage-notebook/t/3fa33efda8db5c94)
 
-**Apply**: [attachment: trac_12229-sagenb-developer-doc.3.patch](https://github.com/sagemath/sage/files/ticket12229/trac_12229-sagenb-developer-doc.3.patch.gz) and [attachment: trac-12229-manifest.patch](https://github.com/sagemath/sage/files/ticket12229/trac-12229-manifest.patch.gz)
+**Apply**: [attachment: trac_12229-sagenb-developer-doc.3.patch](https://github.com/sagemath/sage/files/ticket12229/trac_12229-sagenb-developer-doc.3.patch.gz) and [attachment: trac-12229-manifest.patch](https://github.com/sagemath/sage/files/ticket12229/trac-12229-manifest.patch.gz) and [attachment: trac_12229-reviewer.patch](https://github.com/sagemath/sage/files/ticket12229/trac_12229-reviewer.patch.gz).
``````




---

archive/issue_comments_132324.json:
```json
{
    "body": "<div id=\"comment:44\" align=\"right\">comment:44</div>\n\nAttachment: **[trac_12229-reviewer.patch.gz](https://github.com/sagemath/sage/files/ticket12229/trac_12229-reviewer.patch.gz)**\n\nPatchbot: apply trac_12229-sagenb-developer-doc.3.patch and trac-12229-manifest.patch and trac_12229-reviewer.patch \n\nAll others: I think this is now ready for review.  It was very tempting to completely rewrite this but having to get something done today won out.   All that requires review is the reviewer patch, including that documentation builds correctly.",
    "created_at": "2012-06-15T17:36:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132324",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:44" align="right">comment:44</div>

Attachment: **[trac_12229-reviewer.patch.gz](https://github.com/sagemath/sage/files/ticket12229/trac_12229-reviewer.patch.gz)**

Patchbot: apply trac_12229-sagenb-developer-doc.3.patch and trac-12229-manifest.patch and trac_12229-reviewer.patch 

All others: I think this is now ready for review.  It was very tempting to completely rewrite this but having to get something done today won out.   All that requires review is the reviewer patch, including that documentation builds correctly.



---

archive/issue_events_167037.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-06-15T17:36:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167037"
}
```



---

archive/issue_events_167038.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-06-15T17:36:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167038"
}
```



---

archive/issue_comments_132325.json:
```json
{
    "body": "<div id=\"comment:45\" align=\"right\">comment:45</div>\n\nThis is good enough for now. We already have stuff lined up for the successor to #11080, so let's at least get #11080 in ASAP :)",
    "created_at": "2012-06-15T18:24:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132325",
    "user": "https://github.com/kini"
}
```

<div id="comment:45" align="right">comment:45</div>

This is good enough for now. We already have stuff lined up for the successor to #11080, so let's at least get #11080 in ASAP :)



---

archive/issue_events_167039.json:
```json
{
    "actor": "https://github.com/kini",
    "created_at": "2012-06-15T18:24:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167039"
}
```



---

archive/issue_events_167040.json:
```json
{
    "actor": "https://github.com/kini",
    "created_at": "2012-06-15T18:24:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167040"
}
```



---

archive/issue_comments_132326.json:
```json
{
    "body": "Changed reviewer from **William Stein, Jeroen Demeyer, Punarbasu Purkayastha, Karl-Dieter Crisman** to **William Stein, Jeroen Demeyer, Punarbasu Purkayastha, Karl-Dieter Crisman, Keshav Kini**",
    "created_at": "2012-06-15T18:24:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132326",
    "user": "https://github.com/kini"
}
```

Changed reviewer from **William Stein, Jeroen Demeyer, Punarbasu Purkayastha, Karl-Dieter Crisman** to **William Stein, Jeroen Demeyer, Punarbasu Purkayastha, Karl-Dieter Crisman, Keshav Kini**



---

archive/issue_comments_132327.json:
```json
{
    "body": "<div id=\"comment:46\" align=\"right\">comment:46</div>\n\nTo quote my recent comment on #11080:\n\n>John Palmieri, who is sitting next to me at sage days 41, suggests that we just not package a repository with sagenb at all. What do you think? After all, sagenb is considered an upstream package at this point, and we don't package repositories for those. Plus it's using a revision control system that isn't included with sage anyway (yet). Also, due to various decisions by people over the lifetime of the repository, the repository is really large - 14 MB at maximum compression, at the current master. So removing it would slim down the sage source and binary distributions by a non-negligible amount. The docs added at #12229 already give instructions on how to clone the repository from github anyway. So why do we need to ship a repo?",
    "created_at": "2012-06-15T22:39:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132327",
    "user": "https://github.com/kini"
}
```

<div id="comment:46" align="right">comment:46</div>

To quote my recent comment on #11080:

>John Palmieri, who is sitting next to me at sage days 41, suggests that we just not package a repository with sagenb at all. What do you think? After all, sagenb is considered an upstream package at this point, and we don't package repositories for those. Plus it's using a revision control system that isn't included with sage anyway (yet). Also, due to various decisions by people over the lifetime of the repository, the repository is really large - 14 MB at maximum compression, at the current master. So removing it would slim down the sage source and binary distributions by a non-negligible amount. The docs added at #12229 already give instructions on how to clone the repository from github anyway. So why do we need to ship a repo?



---

archive/issue_comments_132328.json:
```json
{
    "body": "<div id=\"comment:47\" align=\"right\">comment:47</div>\n\nI guess the question is if we want it to be easy for sage developers to jump into sagenb development.  I say yes, but I'm a bit biased.",
    "created_at": "2012-06-16T00:05:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132328",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:47" align="right">comment:47</div>

I guess the question is if we want it to be easy for sage developers to jump into sagenb development.  I say yes, but I'm a bit biased.



---

archive/issue_comments_132329.json:
```json
{
    "body": "<div id=\"comment:48\" align=\"right\">comment:48</div>\n\nWell, we always want things to be easy, so that question has a clear answer, but there are other questions too, such as whether we want to double the size of the sagenb SPKG, or whether we want to ship more binary data, etc. etc. In any case it's not really much easier to develop for sagenb if we ship a repository, because you still need to install git on your system, or install the git SPKG. After that, cloning is a single command, and how to run that command is even explained in the patches on this ticket.",
    "created_at": "2012-06-16T00:11:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132329",
    "user": "https://github.com/kini"
}
```

<div id="comment:48" align="right">comment:48</div>

Well, we always want things to be easy, so that question has a clear answer, but there are other questions too, such as whether we want to double the size of the sagenb SPKG, or whether we want to ship more binary data, etc. etc. In any case it's not really much easier to develop for sagenb if we ship a repository, because you still need to install git on your system, or install the git SPKG. After that, cloning is a single command, and how to run that command is even explained in the patches on this ticket.



---

archive/issue_comments_132330.json:
```json
{
    "body": "<div id=\"comment:49\" align=\"right\">comment:49</div>\n\nBut then you need to switch the sagenb symbolic link and do another setup.py develop.\n\nMaybe an optional sagenb-dev spkg is the solution?",
    "created_at": "2012-06-16T00:13:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132330",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:49" align="right">comment:49</div>

But then you need to switch the sagenb symbolic link and do another setup.py develop.

Maybe an optional sagenb-dev spkg is the solution?



---

archive/issue_comments_132331.json:
```json
{
    "body": "<div id=\"comment:50\" align=\"right\">comment:50</div>\n\nIMO switching a symbolic link and doing setup.py develop is not an unreasonable thing to expect a potential sagenb developer to do. At the same time as making things easy for sage developers who want to hack on sagenb, I also want to make things easy for whoever is making sagenb SPKGs (currently me, so that's a kind of selfish desire :) ).",
    "created_at": "2012-06-16T00:16:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132331",
    "user": "https://github.com/kini"
}
```

<div id="comment:50" align="right">comment:50</div>

IMO switching a symbolic link and doing setup.py develop is not an unreasonable thing to expect a potential sagenb developer to do. At the same time as making things easy for sage developers who want to hack on sagenb, I also want to make things easy for whoever is making sagenb SPKGs (currently me, so that's a kind of selfish desire :) ).



---

archive/issue_comments_132332.json:
```json
{
    "body": "Merged: **sage-5.2.beta0**",
    "created_at": "2012-07-02T15:23:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132332",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.2.beta0**



---

archive/issue_events_167041.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-02T15:23:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167041"
}
```



---

archive/issue_events_167042.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-02T15:23:00Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167042"
}
```



---

archive/issue_events_167043.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-23T06:53:45Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "milestone_number": null,
    "milestone_title": "sage-5.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167043"
}
```



---

archive/issue_events_167044.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-23T06:53:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "label": "https://github.com/sagemath/sage/labels/pending",
    "label_color": "008080",
    "label_name": "pending",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167044"
}
```



---

archive/issue_comments_132333.json:
```json
{
    "body": "<div id=\"comment:52\" align=\"right\">comment:52</div>\n\nUnmerging this from sage-5.2 due to the serious security issue at #13270.",
    "created_at": "2012-07-23T06:53:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132333",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:52" align="right">comment:52</div>

Unmerging this from sage-5.2 due to the serious security issue at #13270.



---

archive/issue_comments_132334.json:
```json
{
    "body": "Changed merged from **sage-5.2.beta0** to none",
    "created_at": "2012-07-23T06:53:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132334",
    "user": "https://github.com/jdemeyer"
}
```

Changed merged from **sage-5.2.beta0** to none



---

archive/issue_events_167045.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-23T06:53:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167045"
}
```



---

archive/issue_events_167046.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-23T06:53:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167046"
}
```



---

archive/issue_events_167047.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-23T06:53:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167047"
}
```



---

archive/issue_comments_132335.json:
```json
{
    "body": "Dependencies: **to be merged with #11080**",
    "created_at": "2012-07-23T06:59:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132335",
    "user": "https://github.com/jdemeyer"
}
```

Dependencies: **to be merged with #11080**



---

archive/issue_comments_132336.json:
```json
{
    "body": "Merged: **sage-5.2.beta0**",
    "created_at": "2012-07-25T06:40:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12229#issuecomment-132336",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.2.beta0**



---

archive/issue_events_167048.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-25T06:40:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "milestone_number": null,
    "milestone_title": "sage-5.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167048"
}
```



---

archive/issue_events_167049.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-25T06:40:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "label": "https://github.com/sagemath/sage/labels/pending",
    "label_color": "008080",
    "label_name": "pending",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167049"
}
```



---

archive/issue_events_167050.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-25T06:40:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167050"
}
```



---

archive/issue_events_167051.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-25T06:40:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12229",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12229#event-167051"
}
```
