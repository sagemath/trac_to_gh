...
```json
{
    "body": "`change_ring_of_constants` is not a terrible name--it's pretty accurate.  Maybe we could do a bit better but I think long an clear is better than short and vague.",
...
```json
{
    "body": "The problem with \"change\" also kind of problematic since it implies (to me anyways) an in-place modification, when really you're returning a new expression.  Maybe `convert_constants_to_ring(RR)`?",
