# Issue 12475: Define WordMorphism.__ne__

archive/issues_012303.json:
```json
{
    "body": "Solve the following problem:\n\n```\nsage: s = WordMorphism('1->121,2->131,3->4,4->1')\nsage: s == s.reversal()\nTrue\nsage: s != s.reversal()\nTrue\n```\n\nwhich is due to:\n\n```\nsage: s.__ne__?\nObject `s.__ne__` not found.\n```\n\nNot much more than this should be added:\n\n```\ndef __ne__(self, other):\n    return not self.__eq__(other)\n```\n\nAssignee: sage-combinat\n\nCC:  @seblabbe\n\nResolution: fixed\n\nAuthor: S\u00e9bastien Labb\u00e9\n\nReviewer: Timo Jolivet\n\nMerged: sage-5.0.beta5\n\nIssue created by migration from https://trac.sagemath.org/ticket/12475\n\n",
    "closed_at": "2012-02-22T10:46:09Z",
    "created_at": "2012-02-08T16:42:45Z",
    "labels": [
        "component: combinatorics",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-5.0",
    "title": "Define WordMorphism.__ne__",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/12475",
    "user": "https://trac.sagemath.org/admin/accounts/users/tjolivet"
}
```
Solve the following problem:

```
sage: s = WordMorphism('1->121,2->131,3->4,4->1')
sage: s == s.reversal()
True
sage: s != s.reversal()
True
```

which is due to:

```
sage: s.__ne__?
Object `s.__ne__` not found.
```

Not much more than this should be added:

```
def __ne__(self, other):
    return not self.__eq__(other)
```

Assignee: sage-combinat

CC:  @seblabbe

Resolution: fixed

Author: Sébastien Labbé

Reviewer: Timo Jolivet

Merged: sage-5.0.beta5

Issue created by migration from https://trac.sagemath.org/ticket/12475





---

archive/issue_comments_178499.json:
```json
{
    "body": "Attachment [12475-sl.patch](tarball://root/attachments/some-uuid/ticket12475/12475-sl.patch) by @seblabbe created at 2012-02-08 18:16:36",
    "created_at": "2012-02-08T18:16:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12475",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12475#issuecomment-178499",
    "user": "https://github.com/seblabbe"
}
```

Attachment [12475-sl.patch](tarball://root/attachments/some-uuid/ticket12475/12475-sl.patch) by @seblabbe created at 2012-02-08 18:16:36



---

archive/issue_comments_178500.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2012-02-08T18:17:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12475",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12475#issuecomment-178500",
    "user": "https://github.com/seblabbe"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_178501.json:
```json
{
    "body": "Changing author from \"\" to \"S\u00e9bastien Labb\u00e9\"",
    "created_at": "2012-02-08T18:17:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12475",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12475#issuecomment-178501",
    "user": "https://github.com/seblabbe"
}
```

Changing author from "" to "Sébastien Labbé"



---

archive/issue_comments_178502.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2012-02-12T18:34:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12475",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12475#issuecomment-178502",
    "user": "https://trac.sagemath.org/admin/accounts/users/tjolivet"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_178503.json:
```json
{
    "body": "<a id='comment:2'></a>The patch applies correctly on my system and fixes the reported problem: positive review.\n\nI couldn't figure out where the PatchBot fails (yellow icon as of now).",
    "created_at": "2012-02-12T18:34:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12475",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12475#issuecomment-178503",
    "user": "https://trac.sagemath.org/admin/accounts/users/tjolivet"
}
```

<a id='comment:2'></a>The patch applies correctly on my system and fixes the reported problem: positive review.

I couldn't figure out where the PatchBot fails (yellow icon as of now).



---

archive/issue_comments_178504.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Timo Jolivet\"",
    "created_at": "2012-02-12T18:34:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12475",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12475#issuecomment-178504",
    "user": "https://trac.sagemath.org/admin/accounts/users/tjolivet"
}
```

Changing reviewer from "" to "Timo Jolivet"



---

archive/issue_comments_178505.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-5.0.beta5\"",
    "created_at": "2012-02-22T10:46:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12475",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12475#issuecomment-178505",
    "user": "https://github.com/jdemeyer"
}
```

Changing merged from "" to "sage-5.0.beta5"



---

archive/issue_events_033624.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-22T10:46:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/12475",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12475#event-33624"
}
```



---

archive/issue_comments_178506.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2012-02-22T10:46:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12475",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12475#issuecomment-178506",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: fixed
