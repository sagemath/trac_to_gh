# Issue 12488: Make sage-make_devel_packages not fail if there is nothing to commit

archive/issues_012316.json:
```json
{
    "assignees": [],
    "body": "When doing `hg commit` but there is nothing to commit, the exit status is 1.  Since this isn't a failure, we should allow this in `sage-make_devel_packages`.\n\nThis problem for example occurs when doing \"sage --sdist\" twice with the same version number.\n\nAssignee: **@nexttime**\n\nAuthor: **Jeroen Demeyer**\n\nReviewer: **Volker Braun**\n\nMerged: **sage-5.0.beta5**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12488_\n\n",
    "closed_at": "2012-02-22T10:46:50Z",
    "created_at": "2012-02-10T10:45:43Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20scripts",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Make sage-make_devel_packages not fail if there is nothing to commit",
    "type": "issue",
    "updated_at": "2012-02-22T10:46:50Z",
    "url": "https://github.com/sagemath/sage/issues/12488",
    "user": "https://github.com/jdemeyer"
}
```
When doing `hg commit` but there is nothing to commit, the exit status is 1.  Since this isn't a failure, we should allow this in `sage-make_devel_packages`.

This problem for example occurs when doing "sage --sdist" twice with the same version number.

Assignee: **@nexttime**

Author: **Jeroen Demeyer**

Reviewer: **Volker Braun**

Merged: **sage-5.0.beta5**

_Issue created by migration from https://trac.sagemath.org/ticket/12488_





---

archive/issue_events_156367.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-10T10:45:43Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12488",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12488#event-156367"
}
```



---

archive/issue_events_156368.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-10T10:45:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12488",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20scripts",
    "label_color": "000080",
    "label_name": "component: scripts",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12488#event-156368"
}
```



---

archive/issue_events_156369.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-10T10:45:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12488",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12488#event-156369"
}
```



---

archive/issue_events_156370.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-10T10:45:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12488",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12488#event-156370"
}
```



---

archive/issue_events_156371.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-10T11:10:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12488",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12488#event-156371"
}
```



---

archive/issue_comments_140269.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n When doing `hg commit` but there is nothing to commit, the exit status is 1.  Since this isn't a failure, we should allow this in `sage-make_devel_packages`.\n+\n+This problem for example occurs when doing \"sage --sdist\" twice with the same version number.\n``````\n",
    "created_at": "2012-02-10T11:10:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12488",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12488#issuecomment-140269",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
 When doing `hg commit` but there is nothing to commit, the exit status is 1.  Since this isn't a failure, we should allow this in `sage-make_devel_packages`.
+
+This problem for example occurs when doing "sage --sdist" twice with the same version number.
``````




---

archive/issue_comments_140270.json:
```json
{
    "body": "Attachment: **[12488_commit.patch.gz](https://github.com/sagemath/sage/files/ticket12488/12488_commit.patch.gz)**",
    "created_at": "2012-02-10T11:11:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12488",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12488#issuecomment-140270",
    "user": "https://github.com/jdemeyer"
}
```

Attachment: **[12488_commit.patch.gz](https://github.com/sagemath/sage/files/ticket12488/12488_commit.patch.gz)**



---

archive/issue_events_156372.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-02-18T00:38:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12488",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12488#event-156372"
}
```



---

archive/issue_events_156373.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-02-18T00:38:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12488",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12488#event-156373"
}
```



---

archive/issue_comments_140271.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2012-02-18T00:38:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12488",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12488#issuecomment-140271",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_comments_140272.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nLooks good.",
    "created_at": "2012-02-18T00:38:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12488",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12488#issuecomment-140272",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:2" align="right">Comment 2</div>

Looks good.



---

archive/issue_events_156374.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-22T10:46:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12488",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12488#event-156374"
}
```



---

archive/issue_events_156375.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-22T10:46:50Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12488",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12488#event-156375"
}
```



---

archive/issue_comments_140273.json:
```json
{
    "body": "Merged: **sage-5.0.beta5**",
    "created_at": "2012-02-22T10:46:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12488",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12488#issuecomment-140273",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.0.beta5**
