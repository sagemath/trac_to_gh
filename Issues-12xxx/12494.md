# Issue 12494: add a new (and improved!) implementation of dirichlet characters

archive/issues_012322.json:
```json
{
    "assignees": [],
    "body": "I have some preliminary stuff at \nhttps://github.com/jwbober/conrey-dirichlet-characters\n\nSee also some notes by Brian Conrey: http://sage.math.washington.edu/home/bober/sage-stuff/characters/conrey_notes.pdf\n\nAssignee: **@sagetrac-bober**\n\nCC:  @williamstein\n\nKeywords: **dirichlet characters**\n\nAuthor: **Jonathan Bober**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12494_\n\n",
    "created_at": "2012-02-10T22:24:38Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20number%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/feature"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "add a new (and improved!) implementation of dirichlet characters",
    "type": "issue",
    "updated_at": "2012-03-19T05:38:09Z",
    "url": "https://github.com/sagemath/sage/issues/12494",
    "user": "https://github.com/sagetrac-bober"
}
```
I have some preliminary stuff at 
https://github.com/jwbober/conrey-dirichlet-characters

See also some notes by Brian Conrey: http://sage.math.washington.edu/home/bober/sage-stuff/characters/conrey_notes.pdf

Assignee: **@sagetrac-bober**

CC:  @williamstein

Keywords: **dirichlet characters**

Author: **Jonathan Bober**

_Issue created by migration from https://trac.sagemath.org/ticket/12494_





---

archive/issue_events_156447.json:
```json
{
    "actor": "https://github.com/sagetrac-bober",
    "created_at": "2012-02-10T22:24:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12494",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20theory",
    "label_color": "0000ff",
    "label_name": "component: number theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12494#event-156447"
}
```



---

archive/issue_events_156448.json:
```json
{
    "actor": "https://github.com/sagetrac-bober",
    "created_at": "2012-02-10T22:24:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12494",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12494#event-156448"
}
```



---

archive/issue_events_156449.json:
```json
{
    "actor": "https://github.com/sagetrac-bober",
    "created_at": "2012-02-10T22:24:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12494",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12494#event-156449"
}
```



---

archive/issue_events_156450.json:
```json
{
    "actor": "https://github.com/sagetrac-bober",
    "created_at": "2012-02-10T22:24:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12494",
    "label": "https://github.com/sagemath/sage/labels/feature",
    "label_color": "008080",
    "label_name": "feature",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12494#event-156450"
}
```



---

archive/issue_comments_140348.json:
```json
{
    "body": "Changed assignee from **@williamstein** to **@sagetrac-bober**",
    "created_at": "2012-02-10T22:25:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12494#issuecomment-140348",
    "user": "https://github.com/sagetrac-bober"
}
```

Changed assignee from **@williamstein** to **@sagetrac-bober**



---

archive/issue_comments_140349.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n I'll post more here later. For now: I have some preliminary stuff at http://sage.math.washington.edu/home/bober/sage-stuff/characters/dirichlet_conrey.pyx .\n+\n+See also some notes by Brian Conrey: http://sage.math.washington.edu/home/bober/sage-stuff/characters/conrey_notes.pdf\n``````\n",
    "created_at": "2012-02-11T01:12:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12494#issuecomment-140349",
    "user": "https://github.com/sagetrac-bober"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
 I'll post more here later. For now: I have some preliminary stuff at http://sage.math.washington.edu/home/bober/sage-stuff/characters/dirichlet_conrey.pyx .
+
+See also some notes by Brian Conrey: http://sage.math.washington.edu/home/bober/sage-stuff/characters/conrey_notes.pdf
``````




---

archive/issue_comments_140350.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,4 @@\n-I'll post more here later. For now: I have some preliminary stuff at http://sage.math.washington.edu/home/bober/sage-stuff/characters/dirichlet_conrey.pyx .\n+I have some preliminary stuff at \n+https://github.com/jwbober/conrey-dirichlet-characters\n \n See also some notes by Brian Conrey: http://sage.math.washington.edu/home/bober/sage-stuff/characters/conrey_notes.pdf\n``````\n",
    "created_at": "2012-03-05T12:26:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12494#issuecomment-140350",
    "user": "https://github.com/sagetrac-bober"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,4 @@
-I'll post more here later. For now: I have some preliminary stuff at http://sage.math.washington.edu/home/bober/sage-stuff/characters/dirichlet_conrey.pyx .
+I have some preliminary stuff at 
+https://github.com/jwbober/conrey-dirichlet-characters
 
 See also some notes by Brian Conrey: http://sage.math.washington.edu/home/bober/sage-stuff/characters/conrey_notes.pdf
``````




---

archive/issue_comments_140351.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nThis is a comment. Are these still working?",
    "created_at": "2012-03-19T05:38:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12494#issuecomment-140351",
    "user": "https://github.com/sagetrac-bober"
}
```

<div id="comment:5" align="right">Comment 5</div>

This is a comment. Are these still working?



---

archive/issue_comments_140352.json:
```json
{
    "body": "Attachment: **[dirichlet_conrey.pyx.gz](https://github.com/sagemath/sage/files/ticket12494/dirichlet_conrey.pyx.gz)**",
    "created_at": "2012-03-19T06:21:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12494#issuecomment-140352",
    "user": "https://github.com/sagetrac-bober"
}
```

Attachment: **[dirichlet_conrey.pyx.gz](https://github.com/sagemath/sage/files/ticket12494/dirichlet_conrey.pyx.gz)**
