# Issue 12494: add a new (and improved!) implementation of dirichlet characters

archive/issues_012322.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-bober"
    ],
    "body": "I have some preliminary stuff at \nhttps://github.com/jwbober/conrey-dirichlet-characters\n\nSee also some notes by Brian Conrey: http://sage.math.washington.edu/home/bober/sage-stuff/characters/conrey_notes.pdf\n\nCC:  @williamstein\n\nKeywords: **dirichlet characters**\n\nAuthor: **Jonathan Bober**\n\nComponent: **number theory**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12494_\n\n",
    "created_at": "2012-02-10T22:24:38Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20number%20theory",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/feature"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "add a new (and improved!) implementation of dirichlet characters",
    "type": "issue",
    "updated_at": "2012-03-19T05:38:09Z",
    "url": "https://github.com/sagemath/sage/issues/12494",
    "user": "https://github.com/sagetrac-bober"
}
```
I have some preliminary stuff at 
https://github.com/jwbober/conrey-dirichlet-characters

See also some notes by Brian Conrey: http://sage.math.washington.edu/home/bober/sage-stuff/characters/conrey_notes.pdf

CC:  @williamstein

Keywords: **dirichlet characters**

Author: **Jonathan Bober**

Component: **number theory**

_Issue created by migration from https://trac.sagemath.org/ticket/12494_





---

archive/issue_events_171023.json:
```json
{
    "actor": "https://github.com/sagetrac-bober",
    "created_at": "2012-02-10T22:24:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12494",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20number%20theory",
    "label_color": "0000ff",
    "label_name": "c: number theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12494#event-171023"
}
```



---

archive/issue_events_171024.json:
```json
{
    "actor": "https://github.com/sagetrac-bober",
    "created_at": "2012-02-10T22:24:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12494",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12494#event-171024"
}
```



---

archive/issue_events_171025.json:
```json
{
    "actor": "https://github.com/sagetrac-bober",
    "created_at": "2012-02-10T22:24:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12494",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12494#event-171025"
}
```



---

archive/issue_events_171026.json:
```json
{
    "actor": "https://github.com/sagetrac-bober",
    "created_at": "2012-02-10T22:24:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12494",
    "label": "https://github.com/sagemath/sage/labels/feature",
    "label_color": "008080",
    "label_name": "feature",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12494#event-171026"
}
```



---

archive/issue_events_171027.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-02-10T22:24:38Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/12494",
    "subject": "https://github.com/sagetrac-bober",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12494#event-171027"
}
```



---

archive/issue_events_171028.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-02-10T22:25:04Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/12494",
    "subject": "https://github.com/sagetrac-bober",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12494#event-171028"
}
```



---

archive/issue_events_171029.json:
```json
{
    "actor": "https://github.com/sagetrac-bober",
    "created_at": "2012-02-10T22:25:04Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/12494",
    "subject": "https://github.com/sagetrac-bober",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12494#event-171029"
}
```



---

archive/issue_comments_138097.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n I'll post more here later. For now: I have some preliminary stuff at http://sage.math.washington.edu/home/bober/sage-stuff/characters/dirichlet_conrey.pyx .\n+\n+See also some notes by Brian Conrey: http://sage.math.washington.edu/home/bober/sage-stuff/characters/conrey_notes.pdf\n``````\n",
    "created_at": "2012-02-11T01:12:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12494#issuecomment-138097",
    "user": "https://github.com/sagetrac-bober"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
 I'll post more here later. For now: I have some preliminary stuff at http://sage.math.washington.edu/home/bober/sage-stuff/characters/dirichlet_conrey.pyx .
+
+See also some notes by Brian Conrey: http://sage.math.washington.edu/home/bober/sage-stuff/characters/conrey_notes.pdf
``````




---

archive/issue_comments_138098.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,4 @@\n-I'll post more here later. For now: I have some preliminary stuff at http://sage.math.washington.edu/home/bober/sage-stuff/characters/dirichlet_conrey.pyx .\n+I have some preliminary stuff at \n+https://github.com/jwbober/conrey-dirichlet-characters\n \n See also some notes by Brian Conrey: http://sage.math.washington.edu/home/bober/sage-stuff/characters/conrey_notes.pdf\n``````\n",
    "created_at": "2012-03-05T12:26:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12494#issuecomment-138098",
    "user": "https://github.com/sagetrac-bober"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,4 @@
-I'll post more here later. For now: I have some preliminary stuff at http://sage.math.washington.edu/home/bober/sage-stuff/characters/dirichlet_conrey.pyx .
+I have some preliminary stuff at 
+https://github.com/jwbober/conrey-dirichlet-characters
 
 See also some notes by Brian Conrey: http://sage.math.washington.edu/home/bober/sage-stuff/characters/conrey_notes.pdf
``````




---

archive/issue_comments_138099.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nThis is a comment. Are these still working?",
    "created_at": "2012-03-19T05:38:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12494#issuecomment-138099",
    "user": "https://github.com/sagetrac-bober"
}
```

<div id="comment:5" align="right">comment:5</div>

This is a comment. Are these still working?



---

archive/issue_comments_138100.json:
```json
{
    "body": "Attachment: **[dirichlet_conrey.pyx.gz](https://github.com/sagemath/sage/files/ticket12494/dirichlet_conrey.pyx.gz)**",
    "created_at": "2012-03-19T06:21:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12494#issuecomment-138100",
    "user": "https://github.com/sagetrac-bober"
}
```

Attachment: **[dirichlet_conrey.pyx.gz](https://github.com/sagemath/sage/files/ticket12494/dirichlet_conrey.pyx.gz)**
