# Issue 12673: Deprecate sage.media.wav, since it is totally undoctested, and evidently full  of confusing bugs

archive/issues_012501.json:
```json
{
    "assignees": [
        "https://github.com/jasongrout"
    ],
    "body": "As I think http://480.sagenb.org/home/pub/137/ amply illustrates, the mere existence of the untested and very buggy module sage.media.wav causes more confusion than it is worth for now.  It would be best to remove it from Sage, and only add something back as a result of a new (probably student) project to do things right.\n\nCC:  @jasongrout @sagetrac-mhampton\n\nBranch/Commit: **[`2bf2bd0`](https://github.com/sagemath/sagetrac-mirror/commit/2bf2bd0eceb73847083194a2c3eeb5d41775cf14)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Matthias Koeppe**\n\nComponent: **misc**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12673_\n\n",
    "closed_at": "2021-08-31T22:00:59Z",
    "created_at": "2012-03-15T14:45:13Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Deprecate sage.media.wav, since it is totally undoctested, and evidently full  of confusing bugs",
    "type": "issue",
    "updated_at": "2021-08-31T22:00:59Z",
    "url": "https://github.com/sagemath/sage/issues/12673",
    "user": "https://github.com/williamstein"
}
```
As I think http://480.sagenb.org/home/pub/137/ amply illustrates, the mere existence of the untested and very buggy module sage.media.wav causes more confusion than it is worth for now.  It would be best to remove it from Sage, and only add something back as a result of a new (probably student) project to do things right.

CC:  @jasongrout @sagetrac-mhampton

Branch/Commit: **[`2bf2bd0`](https://github.com/sagemath/sagetrac-mirror/commit/2bf2bd0eceb73847083194a2c3eeb5d41775cf14)**

Reviewer: **Travis Scrimshaw**

Author: **Matthias Koeppe**

Component: **misc**

_Issue created by migration from https://trac.sagemath.org/ticket/12673_





---

archive/issue_events_174214.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-03-15T14:45:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174214"
}
```



---

archive/issue_events_174215.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-03-15T14:45:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174215"
}
```



---

archive/issue_events_174216.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-03-15T14:45:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174216"
}
```



---

archive/issue_events_174217.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2012-03-15T14:45:13Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "subject": "https://github.com/williamstein",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174217"
}
```



---

archive/issue_comments_141359.json:
```json
{
    "body": "Attachment: **[trac_12673.patch.gz](https://github.com/sagemath/sage/files/ticket12673/trac_12673.patch.gz)**",
    "created_at": "2012-03-15T14:47:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12673#issuecomment-141359",
    "user": "https://github.com/williamstein"
}
```

Attachment: **[trac_12673.patch.gz](https://github.com/sagemath/sage/files/ticket12673/trac_12673.patch.gz)**



---

archive/issue_events_174218.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-03-15T14:48:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174218"
}
```



---

archive/issue_comments_141360.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nAlternatively, doctest the code.  I've attached a patch to delete it, but maybe that is too draconian.",
    "created_at": "2012-03-15T14:59:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12673#issuecomment-141360",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:2" align="right">comment:2</div>

Alternatively, doctest the code.  I've attached a patch to delete it, but maybe that is too draconian.



---

archive/issue_events_174219.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-03-15T14:59:24Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "title_is": "fix sage.media.wav, since it is totally undoctested, and evidently full  of confusing bugs",
    "title_was": "remove sage.media.wav, since it is totally undoctested, and evidently full  of confusing bugs",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174219"
}
```



---

archive/issue_comments_141361.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nThis is (very tangentially) related to #7668 and [this interact by Marshall](http://wiki.sagemath.org/interact/misc#Hearingatrigonometricidentity).\n\nGiven that it's not the worst to eventually have easy-to-use wave handling, perhaps it would be appropriate to split the difference between doctesting and deleting.   Could one simply comment out all the code and raise (doctested?) `NotImplementedError`s for everything here?",
    "created_at": "2012-03-15T15:35:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12673#issuecomment-141361",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:3" align="right">comment:3</div>

This is (very tangentially) related to #7668 and [this interact by Marshall](http://wiki.sagemath.org/interact/misc#Hearingatrigonometricidentity).

Given that it's not the worst to eventually have easy-to-use wave handling, perhaps it would be appropriate to split the difference between doctesting and deleting.   Could one simply comment out all the code and raise (doctested?) `NotImplementedError`s for everything here?



---

archive/issue_comments_141362.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nPlease fill in your real name as Author.",
    "created_at": "2012-07-27T20:40:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12673#issuecomment-141362",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:4" align="right">comment:4</div>

Please fill in your real name as Author.



---

archive/issue_comments_141363.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nI've changed my mind.  I think that we should just replace this by some really nice documentation about how to use the `wave` module, given [this sage-devel discussion](https://groups.google.com/forum/?fromgroups=#!topic/sage-devel/3-Y1RUFkq14) and [this ask.sagemath question](http://ask.sagemath.org/question/2542/sound-how-is-it-done).  What would be appropriate to roughly replace this stuff with?",
    "created_at": "2013-05-08T01:50:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12673#issuecomment-141363",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:5" align="right">comment:5</div>

I've changed my mind.  I think that we should just replace this by some really nice documentation about how to use the `wave` module, given [this sage-devel discussion](https://groups.google.com/forum/?fromgroups=#!topic/sage-devel/3-Y1RUFkq14) and [this ask.sagemath question](http://ask.sagemath.org/question/2542/sound-how-is-it-done).  What would be appropriate to roughly replace this stuff with?



---

archive/issue_events_174220.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-05-08T01:50:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174220"
}
```



---

archive/issue_events_174221.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-05-08T01:50:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174221"
}
```



---

archive/issue_events_174222.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174222"
}
```



---

archive/issue_events_174223.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174223"
}
```



---

archive/issue_events_174224.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174224"
}
```



---

archive/issue_events_174225.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174225"
}
```



---

archive/issue_events_174226.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174226"
}
```



---

archive/issue_events_174227.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174227"
}
```



---

archive/issue_events_174228.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174228"
}
```



---

archive/issue_events_174229.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174229"
}
```



---

archive/issue_events_174230.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-26T02:18:41Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "title_is": "Deprecate sage.media.wav, since it is totally undoctested, and evidently full  of confusing bugs",
    "title_was": "fix sage.media.wav, since it is totally undoctested, and evidently full  of confusing bugs",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174230"
}
```



---

archive/issue_events_174231.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-26T02:18:41Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174231"
}
```



---

archive/issue_events_174232.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-26T02:18:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174232"
}
```



---

archive/issue_comments_141364.json:
```json
{
    "body": "Branch: **[u/mkoeppe/deprecate_sage_media_wav__since_it_is_totally_undoctested__and_evidently_full__of_confusing_bugs](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/deprecate_sage_media_wav__since_it_is_totally_undoctested__and_evidently_full__of_confusing_bugs)**",
    "created_at": "2021-08-26T02:35:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12673#issuecomment-141364",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/deprecate_sage_media_wav__since_it_is_totally_undoctested__and_evidently_full__of_confusing_bugs](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/deprecate_sage_media_wav__since_it_is_totally_undoctested__and_evidently_full__of_confusing_bugs)**



---

archive/issue_comments_141365.json:
```json
{
    "body": "<div id=\"comment:12\"></div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2db5da688355604adf4ae572d079c91073fc0222\"><code>2db5da6</code></a></td><td><code>Deprecate sage.media</code></td></tr></table>\n",
    "created_at": "2021-08-26T02:36:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12673#issuecomment-141365",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:12"></div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2db5da688355604adf4ae572d079c91073fc0222"><code>2db5da6</code></a></td><td><code>Deprecate sage.media</code></td></tr></table>




---

archive/issue_comments_141366.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2021-08-26T02:36:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12673#issuecomment-141366",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_events_174233.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-26T02:36:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174233"
}
```



---

archive/issue_events_174234.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-26T02:36:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174234"
}
```



---

archive/issue_comments_141367.json:
```json
{
    "body": "Commit: **[`2db5da6`](https://github.com/sagemath/sagetrac-mirror/commit/2db5da688355604adf4ae572d079c91073fc0222)**",
    "created_at": "2021-08-26T02:36:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12673#issuecomment-141367",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[`2db5da6`](https://github.com/sagemath/sagetrac-mirror/commit/2db5da688355604adf4ae572d079c91073fc0222)**



---

archive/issue_comments_141368.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2021-08-26T23:49:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12673#issuecomment-141368",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_141369.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">comment:13</div>\n\nLGTM.",
    "created_at": "2021-08-26T23:49:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12673#issuecomment-141369",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:13" align="right">comment:13</div>

LGTM.



---

archive/issue_events_174235.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2021-08-26T23:49:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174235"
}
```



---

archive/issue_events_174236.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2021-08-26T23:49:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174236"
}
```



---

archive/issue_comments_141370.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">comment:14</div>\n\nThanks!",
    "created_at": "2021-08-27T00:07:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12673#issuecomment-141370",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:14" align="right">comment:14</div>

Thanks!



---

archive/issue_comments_141371.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">comment:15</div>\n\nAre there no doctests at all that will get confused here?  Maybe we should add *one* along the lines of the ones in sage.finance.",
    "created_at": "2021-08-27T00:34:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12673#issuecomment-141371",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:15" align="right">comment:15</div>

Are there no doctests at all that will get confused here?  Maybe we should add *one* along the lines of the ones in sage.finance.



---

archive/issue_events_174237.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2021-08-27T00:34:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174237"
}
```



---

archive/issue_events_174238.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2021-08-27T00:34:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174238"
}
```



---

archive/issue_comments_141372.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">comment:16</div>\n\n(I mean #32427.)",
    "created_at": "2021-08-27T00:35:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12673#issuecomment-141372",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:16" align="right">comment:16</div>

(I mean #32427.)



---

archive/issue_comments_141373.json:
```json
{
    "body": "Changed commit from **[`2db5da6`](https://github.com/sagemath/sagetrac-mirror/commit/2db5da688355604adf4ae572d079c91073fc0222)** to **[`2bf2bd0`](https://github.com/sagemath/sagetrac-mirror/commit/2bf2bd0eceb73847083194a2c3eeb5d41775cf14)**",
    "created_at": "2021-08-27T00:40:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12673#issuecomment-141373",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[`2db5da6`](https://github.com/sagemath/sagetrac-mirror/commit/2db5da688355604adf4ae572d079c91073fc0222)** to **[`2bf2bd0`](https://github.com/sagemath/sagetrac-mirror/commit/2bf2bd0eceb73847083194a2c3eeb5d41775cf14)**



---

archive/issue_comments_141374.json:
```json
{
    "body": "<div id=\"comment:17\"></div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2bf2bd0eceb73847083194a2c3eeb5d41775cf14\"><code>2bf2bd0</code></a></td><td><code>src/sage/media/wav.py: Add deprecation to doc</code></td></tr></table>\n",
    "created_at": "2021-08-27T00:40:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12673#issuecomment-141374",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:17"></div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2bf2bd0eceb73847083194a2c3eeb5d41775cf14"><code>2bf2bd0</code></a></td><td><code>src/sage/media/wav.py: Add deprecation to doc</code></td></tr></table>




---

archive/issue_comments_141375.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">comment:18</div>\n\nThere was not a single doctest. Now there is one.",
    "created_at": "2021-08-27T00:40:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12673#issuecomment-141375",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:18" align="right">comment:18</div>

There was not a single doctest. Now there is one.



---

archive/issue_events_174239.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-27T00:40:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174239"
}
```



---

archive/issue_events_174240.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-27T00:40:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174240"
}
```



---

archive/issue_comments_141376.json:
```json
{
    "body": "<div id=\"comment:20\" align=\"right\">comment:20</div>\n\n> There was not a single doctest. Now there is one.\n\nIt was definitely the wild wild west in those days.  \n\nThanks, and sorry to override Travis' review :) I will assume patchbot will be happy and restore his assessment.",
    "created_at": "2021-08-27T00:47:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12673#issuecomment-141376",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:20" align="right">comment:20</div>

> There was not a single doctest. Now there is one.

It was definitely the wild wild west in those days.  

Thanks, and sorry to override Travis' review :) I will assume patchbot will be happy and restore his assessment.



---

archive/issue_events_174241.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2021-08-27T00:47:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174241"
}
```



---

archive/issue_events_174242.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2021-08-27T00:47:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174242"
}
```



---

archive/issue_comments_141377.json:
```json
{
    "body": "<div id=\"comment:21\" align=\"right\">comment:21</div>\n\nReplying to [@kcrisman](#comment:20):\n> Thanks, and sorry to override Travis' review :) I will assume patchbot will be happy and restore his assessment.\n\nDon't worry about it at all. We are a community with different options, and we as a collective should agree about positive reviews.",
    "created_at": "2021-08-27T01:05:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12673#issuecomment-141377",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:21" align="right">comment:21</div>

Replying to [@kcrisman](#comment:20):
> Thanks, and sorry to override Travis' review :) I will assume patchbot will be happy and restore his assessment.

Don't worry about it at all. We are a community with different options, and we as a collective should agree about positive reviews.



---

archive/issue_events_174243.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-27T19:32:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174243"
}
```



---

archive/issue_events_174244.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-27T19:32:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174244"
}
```



---

archive/issue_comments_141378.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/deprecate_sage_media_wav__since_it_is_totally_undoctested__and_evidently_full__of_confusing_bugs](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/deprecate_sage_media_wav__since_it_is_totally_undoctested__and_evidently_full__of_confusing_bugs)** to **[`2bf2bd0`](https://github.com/sagemath/sagetrac-mirror/commit/2bf2bd0eceb73847083194a2c3eeb5d41775cf14)**",
    "created_at": "2021-08-31T22:00:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12673#issuecomment-141378",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/deprecate_sage_media_wav__since_it_is_totally_undoctested__and_evidently_full__of_confusing_bugs](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/deprecate_sage_media_wav__since_it_is_totally_undoctested__and_evidently_full__of_confusing_bugs)** to **[`2bf2bd0`](https://github.com/sagemath/sagetrac-mirror/commit/2bf2bd0eceb73847083194a2c3eeb5d41775cf14)**



---

archive/issue_events_174245.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-08-31T22:00:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174245"
}
```



---

archive/issue_events_174246.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "c3269057fbcef59ab884c6c4d676959240c310cb",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-08-31T22:00:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12673",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12673#event-174246"
}
```
