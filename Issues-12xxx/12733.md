# Issue 12733: norm of symbolic matrices gives an error

archive/issues_012561.json:
```json
{
    "assignees": [],
    "body": "This was reported on ask.sagemath.org: http://ask.sagemath.org/question/1244/symbolic-vector-norm\n\nHere is an example\n\n```\nsage: matrix(SR,[[x]]).norm()\n\n---------------------------------------------------------------------------\nTypeError                                 Traceback (most recent call last)\n\n/Users/grout/<ipython console> in <module>()\n\n/Users/grout/sage-trees/sage-5.0.beta6/local/lib/python2.7/site-packages/sage/matrix/matrix2.so in sage.matrix.matrix2.Matrix.norm (sage/matrix/matrix2.c:47759)()\n\n/Users/grout/sage-trees/sage-5.0.beta6/local/lib/python2.7/site-packages/sage/matrix/matrix0.so in sage.matrix.matrix0.Matrix.change_ring (sage/matrix/matrix0.c:8151)()\n\n/Users/grout/sage-trees/sage-5.0.beta6/local/lib/python2.7/site-packages/sage/matrix/matrix_space.pyc in __call__(self, entries, coerce, copy, rows)\n    554             return self(entries.matrix(), copy=False)\n    555 \n--> 556         return self.matrix(entries, copy=copy, coerce=coerce, rows=rows)\n    557 \n    558     def change_ring(self, R):\n\n/Users/grout/sage-trees/sage-5.0.beta6/local/lib/python2.7/site-packages/sage/matrix/matrix_space.pyc in matrix(self, x, coerce, copy, rows)\n   1403                     x = new_x\n   1404                     copy = False\n-> 1405         return self.__matrix_class(self, entries=x, copy=copy, coerce=coerce) \n   1406      \n   1407     def matrix_space(self, nrows=None, ncols=None, sparse=False):\n\n/Users/grout/sage-trees/sage-5.0.beta6/local/lib/python2.7/site-packages/sage/matrix/matrix_double_dense.so in sage.matrix.matrix_double_dense.Matrix_double_dense.__init__ (sage/matrix/matrix_double_dense.c:3215)()\n\n/Users/grout/sage-trees/sage-5.0.beta6/local/lib/python2.7/site-packages/sage/symbolic/expression.so in sage.symbolic.expression.Expression.__complex__ (sage/symbolic/expression.cpp:5926)()\n\nTypeError: unable to simplify to complex approximation\n```\n\nThe problem appears to be an automatic coercion to RDF or CDF.  Maybe symbolic matrices should override the method.\n\nAssignee: **@jasongrout, @williamstein**\n\nCC:  @rbeezer @mmasdeu\n\nKeywords: **matrix, symbolic**\n\nWork Issues: **incorrect tests**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12733_\n\n",
    "created_at": "2012-03-23T07:51:55Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/needs%20work"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "norm of symbolic matrices gives an error",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/12733",
    "user": "https://github.com/jasongrout"
}
```
This was reported on ask.sagemath.org: http://ask.sagemath.org/question/1244/symbolic-vector-norm

Here is an example

```
sage: matrix(SR,[[x]]).norm()

---------------------------------------------------------------------------
TypeError                                 Traceback (most recent call last)

/Users/grout/<ipython console> in <module>()

/Users/grout/sage-trees/sage-5.0.beta6/local/lib/python2.7/site-packages/sage/matrix/matrix2.so in sage.matrix.matrix2.Matrix.norm (sage/matrix/matrix2.c:47759)()

/Users/grout/sage-trees/sage-5.0.beta6/local/lib/python2.7/site-packages/sage/matrix/matrix0.so in sage.matrix.matrix0.Matrix.change_ring (sage/matrix/matrix0.c:8151)()

/Users/grout/sage-trees/sage-5.0.beta6/local/lib/python2.7/site-packages/sage/matrix/matrix_space.pyc in __call__(self, entries, coerce, copy, rows)
    554             return self(entries.matrix(), copy=False)
    555 
--> 556         return self.matrix(entries, copy=copy, coerce=coerce, rows=rows)
    557 
    558     def change_ring(self, R):

/Users/grout/sage-trees/sage-5.0.beta6/local/lib/python2.7/site-packages/sage/matrix/matrix_space.pyc in matrix(self, x, coerce, copy, rows)
   1403                     x = new_x
   1404                     copy = False
-> 1405         return self.__matrix_class(self, entries=x, copy=copy, coerce=coerce) 
   1406      
   1407     def matrix_space(self, nrows=None, ncols=None, sparse=False):

/Users/grout/sage-trees/sage-5.0.beta6/local/lib/python2.7/site-packages/sage/matrix/matrix_double_dense.so in sage.matrix.matrix_double_dense.Matrix_double_dense.__init__ (sage/matrix/matrix_double_dense.c:3215)()

/Users/grout/sage-trees/sage-5.0.beta6/local/lib/python2.7/site-packages/sage/symbolic/expression.so in sage.symbolic.expression.Expression.__complex__ (sage/symbolic/expression.cpp:5926)()

TypeError: unable to simplify to complex approximation
```

The problem appears to be an automatic coercion to RDF or CDF.  Maybe symbolic matrices should override the method.

Assignee: **@jasongrout, @williamstein**

CC:  @rbeezer @mmasdeu

Keywords: **matrix, symbolic**

Work Issues: **incorrect tests**

_Issue created by migration from https://trac.sagemath.org/ticket/12733_





---

archive/issue_events_160494.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2012-03-23T07:51:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12733",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12733#event-160494"
}
```



---

archive/issue_events_160495.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2012-03-23T07:51:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12733",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "component: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12733#event-160495"
}
```



---

archive/issue_events_160496.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2012-03-23T07:51:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12733",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12733#event-160496"
}
```



---

archive/issue_events_160497.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2012-03-23T07:51:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12733",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12733#event-160497"
}
```



---

archive/issue_comments_144910.json:
```json
{
    "body": "Attachment: **[trac_12733_symbolic_matrix_norm.patch.gz](https://github.com/sagemath/sage/files/ticket12733/trac_12733_symbolic_matrix_norm.patch.gz)**",
    "created_at": "2012-06-12T19:55:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12733",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12733#issuecomment-144910",
    "user": "https://github.com/abrochard"
}
```

Attachment: **[trac_12733_symbolic_matrix_norm.patch.gz](https://github.com/sagemath/sage/files/ticket12733/trac_12733_symbolic_matrix_norm.patch.gz)**



---

archive/issue_comments_144911.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nPatch submitted. I also added an example to be sure, and corrected a little mistake in a test in sage/misc/functional.py",
    "created_at": "2012-06-12T19:58:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12733",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12733#issuecomment-144911",
    "user": "https://github.com/abrochard"
}
```

<a id='comment:1'>Comment 1:</a>
Patch submitted. I also added an example to be sure, and corrected a little mistake in a test in sage/misc/functional.py



---

archive/issue_events_160498.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2012-07-10T14:46:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12733",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12733#event-160498"
}
```



---

archive/issue_comments_144912.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nThis ticket (and the added tests in particular) does not make much sense, unless there exists a symbolic max function. In my opinion, it would be better to return a NotImplementedError than to return nonsense results.",
    "created_at": "2012-07-10T16:00:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12733",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12733#issuecomment-144912",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:4'>Comment 4:</a>
This ticket (and the added tests in particular) does not make much sense, unless there exists a symbolic max function. In my opinion, it would be better to return a NotImplementedError than to return nonsense results.



---

archive/issue_comments_144913.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nUse max_symbolic then: \n\n```\nsage: max_symbolic(a,b)\nmax(a, b)\n```",
    "created_at": "2012-07-10T16:45:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12733",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12733#issuecomment-144913",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:5'>Comment 5:</a>
Use max_symbolic then: 

```
sage: max_symbolic(a,b)
max(a, b)
```



---

archive/issue_comments_144914.json:
```json
{
    "body": "Work Issues: **incorrect tests**",
    "created_at": "2012-07-10T19:15:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12733",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12733#issuecomment-144914",
    "user": "https://github.com/fchapoton"
}
```

Work Issues: **incorrect tests**



---

archive/issue_events_160499.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2012-07-10T19:15:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12733",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12733#event-160499"
}
```



---

archive/issue_events_160500.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2012-07-10T19:15:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12733",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12733#event-160500"
}
```



---

archive/issue_comments_144915.json:
```json
{
    "body": "Changed keywords from **** to **matrix, symbolic**",
    "created_at": "2012-07-10T19:15:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12733",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12733#issuecomment-144915",
    "user": "https://github.com/fchapoton"
}
```

Changed keywords from **** to **matrix, symbolic**



---

archive/issue_events_160501.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12733",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12733#event-160501"
}
```



---

archive/issue_events_160502.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12733",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12733#event-160502"
}
```



---

archive/issue_events_160503.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12733",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12733#event-160503"
}
```



---

archive/issue_events_160504.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12733",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12733#event-160504"
}
```



---

archive/issue_events_160505.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12733",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12733#event-160505"
}
```



---

archive/issue_events_160506.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12733",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12733#event-160506"
}
```



---

archive/issue_events_160507.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12733",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12733#event-160507"
}
```



---

archive/issue_events_160508.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12733",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12733#event-160508"
}
```



---

archive/issue_events_160509.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12733",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12733#event-160509"
}
```
