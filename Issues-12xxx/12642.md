# Issue 12642: magma_free interface is broken

archive/issues_012470.json:
```json
{
    "assignees": [],
    "body": "Just like #10499, http://magma.maths.usyd.edu.au/calc/ is broken again from sage:\n\n```\nsage: magma_free('2+3')\nTraceback (most recent call last):\n...\nIndexError: list index out of range\n```\n\nAssignee: **@williamstein**\n\nAuthor: **Nils Bruin**\n\nReviewer: **William Stein**\n\nMerged: **sage-5.0.beta8**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12642_\n\n",
    "closed_at": "2012-03-13T08:26:20Z",
    "created_at": "2012-03-08T22:56:04Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "magma_free interface is broken",
    "type": "issue",
    "updated_at": "2012-03-13T08:26:20Z",
    "url": "https://github.com/sagemath/sage/issues/12642",
    "user": "https://github.com/williamstein"
}
```
Just like #10499, http://magma.maths.usyd.edu.au/calc/ is broken again from sage:

```
sage: magma_free('2+3')
Traceback (most recent call last):
...
IndexError: list index out of range
```

Assignee: **@williamstein**

Author: **Nils Bruin**

Reviewer: **William Stein**

Merged: **sage-5.0.beta8**

_Issue created by migration from https://trac.sagemath.org/ticket/12642_





---

archive/issue_events_158943.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-03-08T22:56:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12642",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12642#event-158943"
}
```



---

archive/issue_events_158944.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-03-08T22:56:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12642",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
    "label_color": "0000ff",
    "label_name": "component: interfaces",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12642#event-158944"
}
```



---

archive/issue_events_158945.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-03-08T22:56:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12642",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12642#event-158945"
}
```



---

archive/issue_events_158946.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-03-08T22:56:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12642",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12642#event-158946"
}
```



---

archive/issue_comments_143122.json:
```json
{
    "body": "Attachment: **[trac_12642.patch.gz](https://github.com/sagemath/sage/files/ticket12642/trac_12642.patch.gz)**",
    "created_at": "2012-03-09T00:13:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12642#issuecomment-143122",
    "user": "https://github.com/nbruin"
}
```

Attachment: **[trac_12642.patch.gz](https://github.com/sagemath/sage/files/ticket12642/trac_12642.patch.gz)**



---

archive/issue_comments_143123.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nThat was interesting to see! As coded, it seemed a bit fragile. The replacement code should be less likely to throw errors. It will likely return junk when the site changes again, though.",
    "created_at": "2012-03-09T00:16:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12642#issuecomment-143123",
    "user": "https://github.com/nbruin"
}
```

<div id="comment:1" align="right">Comment 1</div>

That was interesting to see! As coded, it seemed a bit fragile. The replacement code should be less likely to throw errors. It will likely return junk when the site changes again, though.



---

archive/issue_events_158947.json:
```json
{
    "actor": "https://github.com/nbruin",
    "created_at": "2012-03-09T00:16:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12642",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12642#event-158947"
}
```



---

archive/issue_events_158948.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-03-09T15:54:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12642",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12642#event-158948"
}
```



---

archive/issue_events_158949.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-03-09T15:54:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12642",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12642#event-158949"
}
```



---

archive/issue_comments_143124.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nReplying to [@nbruin](#comment%3A1):\n> That was interesting to see! As coded, it seemed a bit fragile. The replacement code should \n> be less likely to throw errors. It will likely return junk when the site changes again, though.\n\nOf course.  I noticed that there is an \"interesting\" typo in the code \"resluts\", i.e., \"re - sluts\". I've posted a referee patch that fixes that typo.",
    "created_at": "2012-03-09T15:54:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12642#issuecomment-143124",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:2" align="right">Comment 2</div>

Replying to [@nbruin](#comment%3A1):
> That was interesting to see! As coded, it seemed a bit fragile. The replacement code should 
> be less likely to throw errors. It will likely return junk when the site changes again, though.

Of course.  I noticed that there is an "interesting" typo in the code "resluts", i.e., "re - sluts". I've posted a referee patch that fixes that typo.



---

archive/issue_comments_143125.json:
```json
{
    "body": "Attachment: **[trac_12642-referee.patch.gz](https://github.com/sagemath/sage/files/ticket12642/trac_12642-referee.patch.gz)**\n\napply this after the other patch",
    "created_at": "2012-03-09T15:55:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12642#issuecomment-143125",
    "user": "https://github.com/williamstein"
}
```

Attachment: **[trac_12642-referee.patch.gz](https://github.com/sagemath/sage/files/ticket12642/trac_12642-referee.patch.gz)**

apply this after the other patch



---

archive/issue_comments_143126.json:
```json
{
    "body": "Author: **Nils Bruin**",
    "created_at": "2012-03-10T09:38:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12642#issuecomment-143126",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Nils Bruin**



---

archive/issue_comments_143127.json:
```json
{
    "body": "Reviewer: **William Stein**",
    "created_at": "2012-03-10T09:38:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12642#issuecomment-143127",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **William Stein**



---

archive/issue_comments_143128.json:
```json
{
    "body": "Merged: **sage-5.0.beta8**",
    "created_at": "2012-03-13T08:26:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12642#issuecomment-143128",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.0.beta8**



---

archive/issue_events_158950.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-03-13T08:26:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12642",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12642#event-158950"
}
```



---

archive/issue_events_158951.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-03-13T08:26:20Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12642",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12642#event-158951"
}
```
