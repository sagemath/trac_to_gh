...
<a id='comment:1'></a>Attachment [trac_12036.patch](tarball://root/attachments/some-uuid/ticket12036/trac_12036.patch) by @williamstein created at 2011-11-15 16:51:47
...
<a id='comment:3'></a>Adding a `TestSuite(s).run()` run would be nice, but it will fail, since there is no support for pickling in this code.  Adding this to reference manual would be nice too, but that's other work.   
...
```json
{
    "body": "<a id='comment:4'></a>I wasn't too worried about `TestSuite`, just making sure (note I said \"this *other* ticket\", not meaning the current one).  I'll take your point on the `__setitem__` example as well, I don't use splines myself too much.\n\nThe reference manual issue is now #12045.",
...
<a id='comment:4'></a>I wasn't too worried about `TestSuite`, just making sure (note I said "this *other* ticket", not meaning the current one).  I'll take your point on the `__setitem__` example as well, I don't use splines myself too much.
