# Issue 12481: Fix branch_current_hg()

archive/issues_012309.json:
```json
{
    "body": "This is just horrible code (`sage/misc/misc.py`):\n\n```\ndef branch_current_hg():\n    \"\"\"\n    Return the current hg Mercurial branch name. If the branch is\n    'main', which is the default branch, then just '' is returned.\n    \"\"\"\n    try:\n        s = os.popen('ls -l %s/devel/sage'%os.environ['SAGE_ROOT']).read()\n    except IOError:\n        # this happens when running sage under gdb on macs\n        return 'gdb'\n    if 'No such file or directory' in s:\n        raise RuntimeError, \"unable to determine branch?!\"\n    # do ls -l and look for a symlink, which `ls` represents by a '->'\n    i = s.rfind('->')\n    if i == -1:\n        raise RuntimeError, \"unable to determine branch?!\"\n    s = s[i+2:]\n    i = s.find('-')\n    if i == -1:\n        return ''\n    br = s[i+1:].strip()\n    return br\n```\n\nFor one, this clearly needs an \"os.readlink()\" instead of parsing \"ls\" output.  The \"gdb\" is also broken, since the string \"gdb\" doesn't contain \"->\".  Also the documentation is wrong: is the branch is \"main\", then \"main\" is returned.\n\nAssignee: @jasongrout\n\nResolution: fixed\n\nAuthor: Jeroen Demeyer\n\nReviewer: Andr\u00e9 Apitzsch\n\nMerged: sage-5.0.beta4\n\nIssue created by migration from https://trac.sagemath.org/ticket/12481\n\n",
    "closed_at": "2012-02-11T09:43:45Z",
    "created_at": "2012-02-09T11:01:54Z",
    "labels": [
        "component: misc",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-5.0",
    "title": "Fix branch_current_hg()",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/12481",
    "user": "https://github.com/jdemeyer"
}
```
This is just horrible code (`sage/misc/misc.py`):

```
def branch_current_hg():
    """
    Return the current hg Mercurial branch name. If the branch is
    'main', which is the default branch, then just '' is returned.
    """
    try:
        s = os.popen('ls -l %s/devel/sage'%os.environ['SAGE_ROOT']).read()
    except IOError:
        # this happens when running sage under gdb on macs
        return 'gdb'
    if 'No such file or directory' in s:
        raise RuntimeError, "unable to determine branch?!"
    # do ls -l and look for a symlink, which `ls` represents by a '->'
    i = s.rfind('->')
    if i == -1:
        raise RuntimeError, "unable to determine branch?!"
    s = s[i+2:]
    i = s.find('-')
    if i == -1:
        return ''
    br = s[i+1:].strip()
    return br
```

For one, this clearly needs an "os.readlink()" instead of parsing "ls" output.  The "gdb" is also broken, since the string "gdb" doesn't contain "->".  Also the documentation is wrong: is the branch is "main", then "main" is returned.

Assignee: @jasongrout

Resolution: fixed

Author: Jeroen Demeyer

Reviewer: André Apitzsch

Merged: sage-5.0.beta4

Issue created by migration from https://trac.sagemath.org/ticket/12481





---

archive/issue_comments_178612.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -25,4 +25,4 @@\n     return br\n ```\n \n-For one, this clearly needs an \"lstat\" instead of parsing \"ls\" output.  The \"gdb\" is also broken, since the string \"gdb\" doesn't contain \"->\".\n+For one, this clearly needs an \"lstat\" instead of parsing \"ls\" output.  The \"gdb\" is also broken, since the string \"gdb\" doesn't contain \"->\".  Also the documentation is wrong: is the branch is \"main\", then \"main\" is returned.\n``````\n",
    "created_at": "2012-02-09T11:03:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12481#issuecomment-178612",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -25,4 +25,4 @@
     return br
 ```
 
-For one, this clearly needs an "lstat" instead of parsing "ls" output.  The "gdb" is also broken, since the string "gdb" doesn't contain "->".
+For one, this clearly needs an "lstat" instead of parsing "ls" output.  The "gdb" is also broken, since the string "gdb" doesn't contain "->".  Also the documentation is wrong: is the branch is "main", then "main" is returned.
``````




---

archive/issue_comments_178613.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -25,4 +25,4 @@\n     return br\n ```\n \n-For one, this clearly needs an \"lstat\" instead of parsing \"ls\" output.  The \"gdb\" is also broken, since the string \"gdb\" doesn't contain \"->\".  Also the documentation is wrong: is the branch is \"main\", then \"main\" is returned.\n+For one, this clearly needs an \"os.readlink()\" instead of parsing \"ls\" output.  The \"gdb\" is also broken, since the string \"gdb\" doesn't contain \"->\".  Also the documentation is wrong: is the branch is \"main\", then \"main\" is returned.\n``````\n",
    "created_at": "2012-02-09T11:07:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12481#issuecomment-178613",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -25,4 +25,4 @@
     return br
 ```
 
-For one, this clearly needs an "lstat" instead of parsing "ls" output.  The "gdb" is also broken, since the string "gdb" doesn't contain "->".  Also the documentation is wrong: is the branch is "main", then "main" is returned.
+For one, this clearly needs an "os.readlink()" instead of parsing "ls" output.  The "gdb" is also broken, since the string "gdb" doesn't contain "->".  Also the documentation is wrong: is the branch is "main", then "main" is returned.
``````




---

archive/issue_comments_178614.json:
```json
{
    "body": "Changing author from \"\" to \"Jeroen Demeyer\"",
    "created_at": "2012-02-09T13:10:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12481#issuecomment-178614",
    "user": "https://github.com/jdemeyer"
}
```

Changing author from "" to "Jeroen Demeyer"



---

archive/issue_comments_178615.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2012-02-09T13:10:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12481#issuecomment-178615",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_178616.json:
```json
{
    "body": "<a id='comment:4'></a>Could you use\n\n```\nraise RuntimeError(\"Unable to determine branch\")\n```\nto have less work in the future (when we are moving to Python 3).",
    "created_at": "2012-02-09T14:08:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12481#issuecomment-178616",
    "user": "https://github.com/a-andre"
}
```

<a id='comment:4'></a>Could you use

```
raise RuntimeError("Unable to determine branch")
```
to have less work in the future (when we are moving to Python 3).



---

archive/issue_comments_178617.json:
```json
{
    "body": "<a id='comment:5'></a>Attachment [12481.patch](tarball://root/attachments/some-uuid/ticket12481/12481.patch) by @jdemeyer created at 2012-02-09 14:32:32\n\nReplying to [aapitzsch](#comment%3A4):\n> Could you use\n> \n> ```\n> raise RuntimeError(\"Unable to determine branch\")\n> ```\n> to have less work in the future (when we are moving to Python 3).\n\n\nDone.",
    "created_at": "2012-02-09T14:32:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12481#issuecomment-178617",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:5'></a>Attachment [12481.patch](tarball://root/attachments/some-uuid/ticket12481/12481.patch) by @jdemeyer created at 2012-02-09 14:32:32

Replying to [aapitzsch](#comment%3A4):
> Could you use
> 
> ```
> raise RuntimeError("Unable to determine branch")
> ```
> to have less work in the future (when we are moving to Python 3).


Done.



---

archive/issue_comments_178618.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Andr\u00e9 Apitzsch\"",
    "created_at": "2012-02-09T18:43:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12481#issuecomment-178618",
    "user": "https://github.com/a-andre"
}
```

Changing reviewer from "" to "André Apitzsch"



---

archive/issue_comments_178619.json:
```json
{
    "body": "<a id='comment:6'></a>Looks good.",
    "created_at": "2012-02-09T18:43:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12481#issuecomment-178619",
    "user": "https://github.com/a-andre"
}
```

<a id='comment:6'></a>Looks good.



---

archive/issue_comments_178620.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2012-02-09T18:43:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12481#issuecomment-178620",
    "user": "https://github.com/a-andre"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_178621.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-5.0.beta4\"",
    "created_at": "2012-02-11T09:43:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12481#issuecomment-178621",
    "user": "https://github.com/jdemeyer"
}
```

Changing merged from "" to "sage-5.0.beta4"



---

archive/issue_events_033630.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-11T09:43:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/12481",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12481#event-33630"
}
```



---

archive/issue_comments_178622.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2012-02-11T09:43:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12481#issuecomment-178622",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: fixed
