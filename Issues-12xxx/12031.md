# Issue 12031: Remove sage-sage.py

archive/issues_011859.json:
```json
{
    "body": "The file `SAGE_ROOT/local/bin/sage-sage.py` is not tracked by Mercurial, so the release manager should simply do\n\n```\nrm local/bin/sage-sage.py\n```\n\n**Assignee:** @nexttime\n\n**CC:**  @kini\n\n**Author:** Jeroen Demeyer\n\n**Reviewer:** John Palmieri\n\nIssue created by migration from https://trac.sagemath.org/ticket/12031\n\n",
    "closed_at": "2011-11-14T22:01:09Z",
    "created_at": "2011-11-14T17:26:15Z",
    "labels": [
        "component: scripts",
        "critical",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.8",
    "title": "Remove sage-sage.py",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/12031",
    "user": "https://github.com/jdemeyer"
}
```
The file `SAGE_ROOT/local/bin/sage-sage.py` is not tracked by Mercurial, so the release manager should simply do

```
rm local/bin/sage-sage.py
```

**Assignee:** @nexttime

**CC:**  @kini

**Author:** Jeroen Demeyer

**Reviewer:** John Palmieri

Issue created by migration from https://trac.sagemath.org/ticket/12031





---

archive/issue_comments_131829.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1 +1,5 @@\n+The file `SAGE_ROOT/local/bin/sage-sage.py` is not tracked by Mercurial, so the release manager should simply do\n \n+```\n+rm local/bin/sage-sage.py\n+```\n``````\n",
    "created_at": "2011-11-14T17:29:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12031",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12031#issuecomment-131829",
    "user": "https://github.com/jdemeyer"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1 +1,5 @@
+The file `SAGE_ROOT/local/bin/sage-sage.py` is not tracked by Mercurial, so the release manager should simply do
 
+```
+rm local/bin/sage-sage.py
+```
``````




---

archive/issue_events_099585.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-14T17:29:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/12031",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12031#event-99585"
}
```



---

archive/issue_comments_131830.json:
```json
{
    "body": "**Reviewer:** John Palmieri",
    "created_at": "2011-11-14T17:34:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12031",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12031#issuecomment-131830",
    "user": "https://github.com/jdemeyer"
}
```

**Reviewer:** John Palmieri



---

archive/issue_events_099586.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-14T17:34:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/12031",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12031#event-99586"
}
```



---

archive/issue_events_099587.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-14T17:34:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/12031",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12031#event-99587"
}
```



---

archive/issue_comments_131831.json:
```json
{
    "body": "<a id='comment:3'></a>\nIf I'm listed as a reviewer, I should probably be cc'ed on the ticket :)\n\nAnyway, I agree, positive review.",
    "created_at": "2011-11-14T17:53:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12031",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12031#issuecomment-131831",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:3'></a>
If I'm listed as a reviewer, I should probably be cc'ed on the ticket :)

Anyway, I agree, positive review.



---

archive/issue_events_099588.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-14T22:01:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/12031",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12031#event-99588"
}
```



---

archive/issue_events_099589.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-14T22:01:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/12031",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12031#event-99589"
}
```
