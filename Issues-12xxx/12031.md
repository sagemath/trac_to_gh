# Issue 12031: Remove sage-sage.py

archive/issues_011859.json:
```json
{
    "assignees": [],
    "body": "The file `SAGE_ROOT/local/bin/sage-sage.py` is not tracked by Mercurial, so the release manager should simply do\n\n```\nrm local/bin/sage-sage.py\n```\n\nAssignee: **@nexttime**\n\nCC:  @kini\n\nAuthor: **Jeroen Demeyer**\n\nReviewer: **John Palmieri**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12031_\n\n",
    "closed_at": "2011-11-14T22:01:09Z",
    "created_at": "2011-11-14T17:26:15Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20scripts",
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Remove sage-sage.py",
    "type": "issue",
    "updated_at": "2011-11-14T22:01:09Z",
    "url": "https://github.com/sagemath/sage/issues/12031",
    "user": "https://github.com/jdemeyer"
}
```
The file `SAGE_ROOT/local/bin/sage-sage.py` is not tracked by Mercurial, so the release manager should simply do

```
rm local/bin/sage-sage.py
```

Assignee: **@nexttime**

CC:  @kini

Author: **Jeroen Demeyer**

Reviewer: **John Palmieri**

_Issue created by migration from https://trac.sagemath.org/ticket/12031_





---

archive/issue_events_148858.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-14T17:26:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12031",
    "milestone_number": null,
    "milestone_title": "sage-4.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12031#event-148858"
}
```



---

archive/issue_events_148859.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-14T17:26:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12031",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20scripts",
    "label_color": "000080",
    "label_name": "component: scripts",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12031#event-148859"
}
```



---

archive/issue_events_148860.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-14T17:26:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12031",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12031#event-148860"
}
```



---

archive/issue_events_148861.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-14T17:26:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12031",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12031#event-148861"
}
```



---

archive/issue_comments_130262.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,5 @@\n+The file `SAGE_ROOT/local/bin/sage-sage.py` is not tracked by Mercurial, so the release manager should simply do\n \n+```\n+rm local/bin/sage-sage.py\n+```\n``````\n",
    "created_at": "2011-11-14T17:29:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12031",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12031#issuecomment-130262",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,5 @@
+The file `SAGE_ROOT/local/bin/sage-sage.py` is not tracked by Mercurial, so the release manager should simply do
 
+```
+rm local/bin/sage-sage.py
+```
``````




---

archive/issue_events_148862.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-14T17:29:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12031",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12031#event-148862"
}
```



---

archive/issue_comments_130263.json:
```json
{
    "body": "Reviewer: **John Palmieri**",
    "created_at": "2011-11-14T17:34:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12031",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12031#issuecomment-130263",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **John Palmieri**



---

archive/issue_events_148863.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-14T17:34:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12031",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12031#event-148863"
}
```



---

archive/issue_events_148864.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-14T17:34:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12031",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12031#event-148864"
}
```



---

archive/issue_comments_130264.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nIf I'm listed as a reviewer, I should probably be cc'ed on the ticket :)\n\nAnyway, I agree, positive review.",
    "created_at": "2011-11-14T17:53:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12031",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12031#issuecomment-130264",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:3" align="right">Comment 3</div>

If I'm listed as a reviewer, I should probably be cc'ed on the ticket :)

Anyway, I agree, positive review.



---

archive/issue_events_148865.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-14T22:01:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12031",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12031#event-148865"
}
```



---

archive/issue_events_148866.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-14T22:01:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12031",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12031#event-148866"
}
```
