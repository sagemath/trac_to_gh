# Issue 12316: find_fit does not work for piecewise functions

archive/issues_012144.json:
```json
{
    "assignees": [],
    "body": "find_fit calls len(variables) which is not defined for piecewise functions. There is a variable \"length\" that is equivalent. If the function passed to find_fit is piecewise it should call \"length\" instead of \"len(variables)\".\n\nAssignee: **@jasongrout, @sagetrac-jkantor**\n\nKeywords: **piecewise find_fit**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12316_\n\n",
    "created_at": "2012-01-16T20:42:43Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20numerical",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/needs%20info"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "find_fit does not work for piecewise functions",
    "type": "issue",
    "updated_at": "2022-12-29T01:36:35Z",
    "url": "https://github.com/sagemath/sage/issues/12316",
    "user": "https://github.com/sagetrac-Shashank"
}
```
find_fit calls len(variables) which is not defined for piecewise functions. There is a variable "length" that is equivalent. If the function passed to find_fit is piecewise it should call "length" instead of "len(variables)".

Assignee: **@jasongrout, @sagetrac-jkantor**

Keywords: **piecewise find_fit**

_Issue created by migration from https://trac.sagemath.org/ticket/12316_





---

archive/issue_events_153492.json:
```json
{
    "actor": "https://github.com/sagetrac-Shashank",
    "created_at": "2012-01-16T20:42:43Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12316",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12316#event-153492"
}
```



---

archive/issue_events_153493.json:
```json
{
    "actor": "https://github.com/sagetrac-Shashank",
    "created_at": "2012-01-16T20:42:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12316",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20numerical",
    "label_color": "0000ff",
    "label_name": "component: numerical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12316#event-153493"
}
```



---

archive/issue_events_153494.json:
```json
{
    "actor": "https://github.com/sagetrac-Shashank",
    "created_at": "2012-01-16T20:42:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12316",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12316#event-153494"
}
```



---

archive/issue_events_153495.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-01-17T16:22:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12316",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12316#event-153495"
}
```



---

archive/issue_comments_136207.json:
```json
{
    "body": "Changed upstream from **Not yet reported upstream; Will do shortly.** to none",
    "created_at": "2012-01-17T16:22:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12316",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12316#issuecomment-136207",
    "user": "https://github.com/kcrisman"
}
```

Changed upstream from **Not yet reported upstream; Will do shortly.** to none



---

archive/issue_comments_136208.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nIt would be great if you could post an example of what *should* work, by the way, just for reference.  Thanks!",
    "created_at": "2012-02-09T03:26:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12316",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12316#issuecomment-136208",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:3" align="right">Comment 3</div>

It would be great if you could post an example of what *should* work, by the way, just for reference.  Thanks!



---

archive/issue_events_153496.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12316",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12316#event-153496"
}
```



---

archive/issue_events_153497.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12316",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12316#event-153497"
}
```



---

archive/issue_events_153498.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12316",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12316#event-153498"
}
```



---

archive/issue_events_153499.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12316",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12316#event-153499"
}
```



---

archive/issue_events_153500.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12316",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12316#event-153500"
}
```



---

archive/issue_events_153501.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12316",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12316#event-153501"
}
```



---

archive/issue_events_153502.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12316",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12316#event-153502"
}
```



---

archive/issue_events_153503.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12316",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12316#event-153503"
}
```



---

archive/issue_events_153504.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2016-06-25T18:14:30Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12316",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12316#event-153504"
}
```



---

archive/issue_events_153505.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2016-06-25T18:14:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12316",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12316#event-153505"
}
```



---

archive/issue_events_153506.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2016-06-25T18:14:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12316",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12316#event-153506"
}
```



---

archive/issue_events_153507.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:36:35Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12316",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12316#event-153507"
}
```
