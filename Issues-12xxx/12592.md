# Issue 12592: plot error for message for too large figsize

archive/issues_012420.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: show(stats.IntList((1,2,3,4)).plot(figsize=(1,328)))\nTraceback (most recent call last):\n...\nValueError: width and height must each be below 32768\n```\nIt seems that there is a scaling of the size by a factor 100. At least the error message should be altered to give the correct value.\n\nThis was originally reported on [the public bug reports from the notebook interface](https://spreadsheets.google.com/pub?key=pCwvGVwSMxTzT6E2xNdo5fA) by Lukas Prokop on 12/13/2010\n\nAssignee: **@jasongrout, @williamstein**\n\nCC:  @kcrisman\n\nKeywords: **plot, error, figsize**\n\nReviewer: **Karl-Dieter Crisman**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12592_\n\n",
    "closed_at": "2013-03-29T18:57:25Z",
    "created_at": "2012-02-26T13:43:58Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20graphics",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "plot error for message for too large figsize",
    "type": "issue",
    "updated_at": "2013-03-29T18:57:25Z",
    "url": "https://github.com/sagemath/sage/issues/12592",
    "user": "https://github.com/dkrenn"
}
```

```
sage: show(stats.IntList((1,2,3,4)).plot(figsize=(1,328)))
Traceback (most recent call last):
...
ValueError: width and height must each be below 32768
```
It seems that there is a scaling of the size by a factor 100. At least the error message should be altered to give the correct value.

This was originally reported on [the public bug reports from the notebook interface](https://spreadsheets.google.com/pub?key=pCwvGVwSMxTzT6E2xNdo5fA) by Lukas Prokop on 12/13/2010

Assignee: **@jasongrout, @williamstein**

CC:  @kcrisman

Keywords: **plot, error, figsize**

Reviewer: **Karl-Dieter Crisman**

_Issue created by migration from https://trac.sagemath.org/ticket/12592_





---

archive/issue_events_158168.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2012-02-26T13:43:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12592",
    "milestone_number": null,
    "milestone_title": "sage-5.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12592#event-158168"
}
```



---

archive/issue_events_158169.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2012-02-26T13:43:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12592",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graphics",
    "label_color": "0000ff",
    "label_name": "component: graphics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12592#event-158169"
}
```



---

archive/issue_events_158170.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2012-02-26T13:43:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12592",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12592#event-158170"
}
```



---

archive/issue_events_158171.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2012-02-26T13:43:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12592",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12592#event-158171"
}
```



---

archive/issue_comments_142164.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -7,4 +7,4 @@\n ```\n It seems that there is a scaling of the size by a factor 100. At least the error message should be altered to give the correct value.\n \n-This was originally reported by Lukas Prokop on [the public bug reports from the notebook interface](https://spreadsheets.google.com/pub?key=pCwvGVwSMxTzT6E2xNdo5fA)\n+This was originally reported on [the public bug reports from the notebook interface](https://spreadsheets.google.com/pub?key=pCwvGVwSMxTzT6E2xNdo5fA) by Lukas Prokop on 12/13/2010\n``````\n",
    "created_at": "2012-02-26T19:20:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12592",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12592#issuecomment-142164",
    "user": "https://github.com/dkrenn"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -7,4 +7,4 @@
 ```
 It seems that there is a scaling of the size by a factor 100. At least the error message should be altered to give the correct value.
 
-This was originally reported by Lukas Prokop on [the public bug reports from the notebook interface](https://spreadsheets.google.com/pub?key=pCwvGVwSMxTzT6E2xNdo5fA)
+This was originally reported on [the public bug reports from the notebook interface](https://spreadsheets.google.com/pub?key=pCwvGVwSMxTzT6E2xNdo5fA) by Lukas Prokop on 12/13/2010
``````




---

archive/issue_events_158172.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-03-22T21:03:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12592",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12592#event-158172"
}
```



---

archive/issue_events_158173.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-03-22T21:03:41Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12592",
    "milestone_number": null,
    "milestone_title": "sage-5.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12592#event-158173"
}
```



---

archive/issue_events_158174.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-03-22T21:03:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12592",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12592#event-158174"
}
```



---

archive/issue_events_158175.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-03-22T21:03:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12592",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12592#event-158175"
}
```



---

archive/issue_comments_142165.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nI think this is a duplicate of #5956.",
    "created_at": "2013-03-22T21:03:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12592",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12592#issuecomment-142165",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:3'>Comment 3:</a>
I think this is a duplicate of #5956.



---

archive/issue_comments_142166.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nYup.",
    "created_at": "2013-03-23T00:56:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12592",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12592#issuecomment-142166",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:4'>Comment 4:</a>
Yup.



---

archive/issue_comments_142167.json:
```json
{
    "body": "Reviewer: **Karl-Dieter Crisman**",
    "created_at": "2013-03-23T00:56:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12592",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12592#issuecomment-142167",
    "user": "https://github.com/kcrisman"
}
```

Reviewer: **Karl-Dieter Crisman**



---

archive/issue_events_158176.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-03-23T00:56:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12592",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12592#event-158176"
}
```



---

archive/issue_events_158177.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-03-23T00:56:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12592",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12592#event-158177"
}
```



---

archive/issue_events_158178.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-03-29T18:57:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12592",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12592#event-158178"
}
```



---

archive/issue_events_158179.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-03-29T18:57:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12592",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12592#event-158179"
}
```



---

archive/issue_events_158180.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-03-29T18:57:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12592",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12592#event-158180"
}
```
