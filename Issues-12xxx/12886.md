# Issue 12886: Metaclass infrastructure for Sage

archive/issues_012714.json:
```json
{
    "assignees": [],
    "body": "They are currently several metaclasses in Sage for various feature. There is an ongoing discussion to improve the framework. \n\nSee #12808 and [this thread](http://groups.google.com/group/sage-combinat-devel/browse_thread/thread/ae09de5a608525e5)\n\nDepends on #11791\n\nAssignee: **@jasongrout**\n\nCC:  @sagetrac-sage-combinat @simon-king-jena\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12886_\n\n",
    "closed_at": "2017-04-07T08:43:06Z",
    "created_at": "2012-04-26T21:12:10Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/duplicate",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Metaclass infrastructure for Sage",
    "type": "issue",
    "updated_at": "2017-04-07T09:55:53Z",
    "url": "https://github.com/sagemath/sage/issues/12886",
    "user": "https://github.com/hivert"
}
```
They are currently several metaclasses in Sage for various feature. There is an ongoing discussion to improve the framework. 

See #12808 and [this thread](http://groups.google.com/group/sage-combinat-devel/browse_thread/thread/ae09de5a608525e5)

Depends on #11791

Assignee: **@jasongrout**

CC:  @sagetrac-sage-combinat @simon-king-jena

_Issue created by migration from https://trac.sagemath.org/ticket/12886_





---

archive/issue_events_163046.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2012-04-26T21:12:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12886",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12886#event-163046"
}
```



---

archive/issue_events_163047.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2012-04-26T21:12:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12886",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12886#event-163047"
}
```



---

archive/issue_events_163048.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2012-04-26T21:12:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12886",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12886#event-163048"
}
```



---

archive/issue_events_163049.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12886",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12886#event-163049"
}
```



---

archive/issue_events_163050.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12886",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12886#event-163050"
}
```



---

archive/issue_comments_148085.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNote to myself: Finally post a proof of concept.",
    "created_at": "2013-08-15T09:42:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12886",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12886#issuecomment-148085",
    "user": "https://github.com/simon-king-jena"
}
```

<div id="comment:2" align="right">Comment 2</div>

Note to myself: Finally post a proof of concept.



---

archive/issue_comments_148086.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nI think the meta-metaclasses only make sense if introspection would properly work, which requires #11791.",
    "created_at": "2013-08-15T09:51:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12886",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12886#issuecomment-148086",
    "user": "https://github.com/simon-king-jena"
}
```

<div id="comment:3" align="right">Comment 3</div>

I think the meta-metaclasses only make sense if introspection would properly work, which requires #11791.



---

archive/issue_comments_148087.json:
```json
{
    "body": "Dependencies: **#11791**",
    "created_at": "2013-08-15T09:51:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12886",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12886#issuecomment-148087",
    "user": "https://github.com/simon-king-jena"
}
```

Dependencies: **#11791**



---

archive/issue_events_163051.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12886",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12886#event-163051"
}
```



---

archive/issue_events_163052.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12886",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12886#event-163052"
}
```



---

archive/issue_events_163053.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12886",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12886#event-163053"
}
```



---

archive/issue_events_163054.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12886",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12886#event-163054"
}
```



---

archive/issue_events_163055.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12886",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12886#event-163055"
}
```



---

archive/issue_events_163056.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12886",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12886#event-163056"
}
```



---

archive/issue_events_163057.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-07T08:43:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12886",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12886#event-163057"
}
```



---

archive/issue_events_163058.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-07T08:43:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12886",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12886#event-163058"
}
```



---

archive/issue_events_163059.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-07T08:43:06Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12886",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12886#event-163059"
}
```



---

archive/issue_events_163060.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-07T08:43:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12886",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12886#event-163060"
}
```



---

archive/issue_comments_148088.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nDuplicate of #21681 I guess.",
    "created_at": "2017-04-07T08:43:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12886",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12886#issuecomment-148088",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:7" align="right">Comment 7</div>

Duplicate of #21681 I guess.



---

archive/issue_comments_148089.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nReplying to [@jdemeyer](#comment%3A7):\n> Duplicate of #21681 I guess.\n\nYes, I had forgotten about the old ticket when I opened the new one some years later.",
    "created_at": "2017-04-07T09:55:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12886",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12886#issuecomment-148089",
    "user": "https://github.com/simon-king-jena"
}
```

<div id="comment:8" align="right">Comment 8</div>

Replying to [@jdemeyer](#comment%3A7):
> Duplicate of #21681 I guess.

Yes, I had forgotten about the old ticket when I opened the new one some years later.
