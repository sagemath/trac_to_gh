# Issue 12146: hidden markov models: add option to sample/generate to specify the starting state of sequence

archive/issues_011974.json:
```json
{
    "assignees": [],
    "body": "It would be useful when using Monte Carlo techniques to do forecasting if the sample/generate_sequence commands had an option to specify the starting state. \n\nAssignee: **@sagetrac-amhou**\n\nAuthor: **William Stein**\n\nReviewer: **Jason Grout**\n\nMerged: **sage-4.8.alpha5**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12146_\n\n",
    "closed_at": "2011-12-17T09:10:39Z",
    "created_at": "2011-12-12T06:26:46Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20statistics",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "hidden markov models: add option to sample/generate to specify the starting state of sequence",
    "type": "issue",
    "updated_at": "2011-12-17T09:10:39Z",
    "url": "https://github.com/sagemath/sage/issues/12146",
    "user": "https://github.com/williamstein"
}
```
It would be useful when using Monte Carlo techniques to do forecasting if the sample/generate_sequence commands had an option to specify the starting state. 

Assignee: **@sagetrac-amhou**

Author: **William Stein**

Reviewer: **Jason Grout**

Merged: **sage-4.8.alpha5**

_Issue created by migration from https://trac.sagemath.org/ticket/12146_





---

archive/issue_events_150713.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-12-12T06:26:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12146",
    "milestone_number": null,
    "milestone_title": "sage-4.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12146#event-150713"
}
```



---

archive/issue_events_150714.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-12-12T06:26:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12146",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20statistics",
    "label_color": "0000ff",
    "label_name": "component: statistics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12146#event-150714"
}
```



---

archive/issue_events_150715.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-12-12T06:26:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12146",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12146#event-150715"
}
```



---

archive/issue_events_150716.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-12-12T06:26:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12146",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12146#event-150716"
}
```



---

archive/issue_comments_132391.json:
```json
{
    "body": "Attachment: **[trac_12146.patch.gz](https://github.com/sagemath/sage/files/ticket12146/trac_12146.patch.gz)**",
    "created_at": "2011-12-12T06:42:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12146",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12146#issuecomment-132391",
    "user": "https://github.com/williamstein"
}
```

Attachment: **[trac_12146.patch.gz](https://github.com/sagemath/sage/files/ticket12146/trac_12146.patch.gz)**



---

archive/issue_events_150717.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2011-12-13T16:30:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12146",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12146#event-150717"
}
```



---

archive/issue_comments_132392.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nLooks great!  Tests pass, code and docs make sense.",
    "created_at": "2011-12-13T16:30:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12146",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12146#issuecomment-132392",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:1" align="right">Comment 1</div>

Looks great!  Tests pass, code and docs make sense.



---

archive/issue_comments_132393.json:
```json
{
    "body": "Reviewer: **Jason Grout**",
    "created_at": "2011-12-13T16:31:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12146",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12146#issuecomment-132393",
    "user": "https://github.com/jasongrout"
}
```

Reviewer: **Jason Grout**



---

archive/issue_comments_132394.json:
```json
{
    "body": "Author: **William Stein**",
    "created_at": "2011-12-13T16:31:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12146",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12146#issuecomment-132394",
    "user": "https://github.com/jasongrout"
}
```

Author: **William Stein**



---

archive/issue_comments_132395.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\n(I assume it was an oversight that you didn't set this as needs_review.  It certainly looks like it can be reviewed.",
    "created_at": "2011-12-13T16:31:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12146",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12146#issuecomment-132395",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:2" align="right">Comment 2</div>

(I assume it was an oversight that you didn't set this as needs_review.  It certainly looks like it can be reviewed.



---

archive/issue_events_150718.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2011-12-13T16:31:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12146",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12146#event-150718"
}
```



---

archive/issue_events_150719.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2011-12-13T16:31:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12146",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12146#event-150719"
}
```



---

archive/issue_comments_132396.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nYes it was an oversight.",
    "created_at": "2011-12-13T18:14:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12146",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12146#issuecomment-132396",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:3" align="right">Comment 3</div>

Yes it was an oversight.



---

archive/issue_events_150720.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-12-17T09:10:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12146",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12146#event-150720"
}
```



---

archive/issue_events_150721.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-12-17T09:10:39Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12146",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12146#event-150721"
}
```



---

archive/issue_comments_132397.json:
```json
{
    "body": "Merged: **sage-4.8.alpha5**",
    "created_at": "2011-12-17T09:10:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12146",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12146#issuecomment-132397",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.8.alpha5**
