# Issue 12764: Remove ill-advised doctest for testing normal matrices

archive/issues_012592.json:
```json
{
    "assignees": [],
    "body": "Patch removes a doctest that was just too clever for its own good.  It was more educational than about exercising routines, and fails irrepairably on OS X Lion.  Full backstory at #11104.\n\n**Apply**\n\n[attachment: trac_12764-doctest-normal-matrices.patch](https://github.com/sagemath/sage/files/ticket12764/trac_12764-doctest-normal-matrices.patch.gz)\n\nAssignee: **@jasongrout, @williamstein**\n\nCC:  @jhpalmieri\n\nReviewer: **John Palmieri**\n\nAuthor: **Rob Beezer**\n\nMerged: **sage-5.0.beta12**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12764_\n\n",
    "closed_at": "2012-04-02T15:26:41Z",
    "created_at": "2012-03-28T04:17:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Remove ill-advised doctest for testing normal matrices",
    "type": "issue",
    "updated_at": "2012-04-02T15:26:41Z",
    "url": "https://github.com/sagemath/sage/issues/12764",
    "user": "https://github.com/rbeezer"
}
```
Patch removes a doctest that was just too clever for its own good.  It was more educational than about exercising routines, and fails irrepairably on OS X Lion.  Full backstory at #11104.

**Apply**

[attachment: trac_12764-doctest-normal-matrices.patch](https://github.com/sagemath/sage/files/ticket12764/trac_12764-doctest-normal-matrices.patch.gz)

Assignee: **@jasongrout, @williamstein**

CC:  @jhpalmieri

Reviewer: **John Palmieri**

Author: **Rob Beezer**

Merged: **sage-5.0.beta12**

_Issue created by migration from https://trac.sagemath.org/ticket/12764_





---

archive/issue_events_160960.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2012-03-28T04:17:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12764",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12764#event-160960"
}
```



---

archive/issue_events_160961.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2012-03-28T04:17:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12764",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "component: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12764#event-160961"
}
```



---

archive/issue_events_160962.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2012-03-28T04:17:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12764",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12764#event-160962"
}
```



---

archive/issue_events_160963.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2012-03-28T04:17:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12764",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12764#event-160963"
}
```



---

archive/issue_comments_145403.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,5 @@\n Patch removes a doctest that was just too clever for its own good.  It was more educational than about exercising routines, and fails irrepairably on OS X Lion.  Full backstory at #11104.\n+\n+**Apply**\n+\n+[attachment: trac_12764-doctest-normal-matrices.patch](https://github.com/sagemath/sage/files/ticket12764/trac_12764-doctest-normal-matrices.patch.gz)\n``````\n",
    "created_at": "2012-03-28T04:34:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12764#issuecomment-145403",
    "user": "https://github.com/rbeezer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,5 @@
 Patch removes a doctest that was just too clever for its own good.  It was more educational than about exercising routines, and fails irrepairably on OS X Lion.  Full backstory at #11104.
+
+**Apply**
+
+[attachment: trac_12764-doctest-normal-matrices.patch](https://github.com/sagemath/sage/files/ticket12764/trac_12764-doctest-normal-matrices.patch.gz)
``````




---

archive/issue_comments_145404.json:
```json
{
    "body": "Author: **Rob Beezer**",
    "created_at": "2012-03-28T04:34:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12764#issuecomment-145404",
    "user": "https://github.com/rbeezer"
}
```

Author: **Rob Beezer**



---

archive/issue_comments_145405.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nAttachment: **[trac_12764-doctest-normal-matrices.patch.gz](https://github.com/sagemath/sage/files/ticket12764/trac_12764-doctest-normal-matrices.patch.gz)**",
    "created_at": "2012-03-28T04:34:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12764#issuecomment-145405",
    "user": "https://github.com/rbeezer"
}
```

<div id="comment:1" align="right">Comment 1</div>

Attachment: **[trac_12764-doctest-normal-matrices.patch.gz](https://github.com/sagemath/sage/files/ticket12764/trac_12764-doctest-normal-matrices.patch.gz)**



---

archive/issue_events_160964.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2012-03-28T04:34:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12764",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12764#event-160964"
}
```



---

archive/issue_events_160965.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-03-29T18:42:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12764",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12764#event-160965"
}
```



---

archive/issue_events_160966.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-03-29T18:42:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12764",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12764#event-160966"
}
```



---

archive/issue_comments_145406.json:
```json
{
    "body": "Reviewer: **John Palmieri**",
    "created_at": "2012-03-29T18:42:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12764#issuecomment-145406",
    "user": "https://github.com/jhpalmieri"
}
```

Reviewer: **John Palmieri**



---

archive/issue_comments_145407.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nLooks good to me, and passes tests.",
    "created_at": "2012-03-29T18:42:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12764#issuecomment-145407",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:2" align="right">Comment 2</div>

Looks good to me, and passes tests.



---

archive/issue_events_160967.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-03-29T18:42:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12764",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12764#event-160967"
}
```



---

archive/issue_events_160968.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-03-29T18:42:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12764",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12764#event-160968"
}
```



---

archive/issue_comments_145408.json:
```json
{
    "body": "Merged: **sage-5.0.beta12**",
    "created_at": "2012-04-02T15:26:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12764#issuecomment-145408",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.0.beta12**



---

archive/issue_events_160969.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-04-02T15:26:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12764",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12764#event-160969"
}
```



---

archive/issue_events_160970.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-04-02T15:26:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12764",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12764#event-160970"
}
```
