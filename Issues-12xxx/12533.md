...
```json
{
    "body": "The PPL Cython wrapper should be fully functional for LP solving, its just providing a different interface than the `MixedIntegerLinearProgram`. I always wanted to have this more integrated but I don't have to solve plain LP's often (at least not in that language).",
...
```json
{
    "body": "David, it actually is because this solver does not support integer variables. It solves exactly, but only continuous problems. Hence these methods are actually no problem. I thought that p.new_variable(binary = True) should raise an exception, however. What this patch actually needs is a big and deep review, because it really is a very nice thing `:-)`",
...
```json
{
    "body": "A couple of doctests fail on the current beta (see patchbot)",
    "created_at": "2012-03-13T15:04:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12533",
    "type": "issue_comment",
...
```json
{
    "body": "Dima - it's the big circular gray icon at the top of this page - i.e. http://patchbot.sagemath.org/ticket/12533/\n\nI can fix this once I get to Portland if you want, also fix up some formatting issues.",
...
```json
{
    "body": "Yeah, the memory address should be elided, of course.",
...
```json
{
    "body": "Thank you! I have fixed the doctesting errors.",
    "created_at": "2012-03-17T20:48:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12533",
    "type": "issue_comment",
...
```json
{
    "body": "Its a bit tricky because the `PPL_Generator` C++ class has a private ctor and declaring Cython exceptions while returning a const reference has a bug. I'm pretty sure we need to copy the returned `PPL_Generator` in `optimize_point` and other functions.\n\nAlso, can we make `optimize_point` return a PPL point and not a list of gmp rationals? We needlessly deviate from the PPL api here.",
...
Attachment [trac_12533_catch_ppl_exception.patch](tarball://root/attachments/some-uuid/ticket12533/trac_12533_catch_ppl_exception.patch) by @vbraun created at 2012-03-18 18:31:00
...
```json
{
    "body": "Attachment [trac_12533-rebased.patch](tarball://root/attachments/some-uuid/ticket12533/trac_12533-rebased.patch) by @kini created at 2012-05-13 17:40:47\n\nI rebased Risan's patch to 5.0.rc1. I also removed trailing whitespace from the patch.",
...
Attachment [trac_12533-rebased.patch](tarball://root/attachments/some-uuid/ticket12533/trac_12533-rebased.patch) by @kini created at 2012-05-13 17:40:47
...
```json
{
    "body": "Attachment [trac_12533-rebased.2.patch](tarball://root/attachments/some-uuid/ticket12533/trac_12533-rebased.2.patch) by @kini created at 2012-05-28 07:03:29\n\npatchbot: apply trac_12533-rebased.2.patch",
    "created_at": "2012-05-28T07:03:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12533",
    "type": "issue_comment",
...
Attachment [trac_12533-rebased.2.patch](tarball://root/attachments/some-uuid/ticket12533/trac_12533-rebased.2.patch) by @kini created at 2012-05-28 07:03:29
...
```json
{
    "body": "(Does it fail to compile on beta1 or is it just me ? It applied fine though !)",
...
```json
{
    "body": "Oh, and I forgot to add that all import from the PPL libraries should be done in a .pxd file (like for ppl_backend.pxd) instead `:-)`\n\nNathann",
...
```json
{
    "body": "Never mind, I just looked up the PPL docs with respect to `MIP_Problem`.",
...
```json
{
    "body": "Please also see a related #13142, which points out to the fact that show() would eventually need more work to be perfect for arbitrary precision.",
...
```json
{
    "body": "I do not understand a strange kludge with `get_exact_variable_value` vs `get_variable_value`. Why don't you implement `get_ariable_value` to return what `get_exact_variable_value` does?\n\nYes, something needs to be changed in the `GenericBackend` to accommodate this. Is it hard? It certainly needs a generic type then.",
...
```json
{
    "body": "Still builds and works on `Sage 5.2.rc0`.",
    "created_at": "2012-07-20T15:42:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12533",
    "type": "issue_comment",
...
```json
{
    "body": "Attachment [trac_12533_arbitrary_precision_LP_solver_backend.patch](tarball://root/attachments/some-uuid/ticket12533/trac_12533_arbitrary_precision_LP_solver_backend.patch) by @ptrrsn created at 2012-09-11 03:35:20",
    "created_at": "2012-09-11T03:35:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12533",
    "type": "issue_comment",
...
Attachment [trac_12533_arbitrary_precision_LP_solver_backend.patch](tarball://root/attachments/some-uuid/ticket12533/trac_12533_arbitrary_precision_LP_solver_backend.patch) by @ptrrsn created at 2012-09-11 03:35:20
...
```json
{
    "body": "The \"referenced before assignment\" warning is invalid, see http://trac.cython.org/cython_trac/ticket/714. Its a tricky case because the code in question involves C++ call by reference, but Python of course only knows call by value.",
...
```json
{
    "body": "patchbot: apply trac_12533_arbitrary_precision_LP_solver_backend.patch",
...
```json
{
    "body": "patchbot: apply only trac_12533_arbitrary_precision_LP_solver_backend.patch",
...
```json
{
    "body": "Works on `5.4.rc1`, Linux and OSX 10.6. Positive review!",
    "created_at": "2012-10-14T12:49:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12533",
    "type": "issue_comment",
...
```json
{
    "body": "Rebased on top of #13646. No real changes were necessary.",
...
Attachment [trac_12533_arbitrary_precision_LP_solver_backend_for_5.3_vb.patch](tarball://root/attachments/some-uuid/ticket12533/trac_12533_arbitrary_precision_LP_solver_backend_for_5.3_vb.patch) by @vbraun created at 2012-10-23 17:30:09
...
```json
{
    "body": "It doesn't make sense here, you need the parents for constraints to keep the coefficients rational if `solver='PPL'`. I've added it to #12091.",
