# Issue 12097: General clean-up of sage-make_relative

archive/issues_011925.json:
```json
{
    "assignees": [],
    "body": "The attached patch essentially rewrites `sage-make_relative`.  The new code is cleaner and simpler.  It also changes files only once and only when needed.  The previous version changed every Python file every time `sage-make_relative` was run.\n\nAssignee: **@nexttime**\n\nAuthor: **Jeroen Demeyer**\n\nReviewer: **Volker Braun**\n\nMerged: **sage-4.8.alpha3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12097_\n\n",
    "closed_at": "2011-11-30T20:45:16Z",
    "created_at": "2011-11-29T11:11:21Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20scripts",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "General clean-up of sage-make_relative",
    "type": "issue",
    "updated_at": "2011-11-30T20:45:16Z",
    "url": "https://github.com/sagemath/sage/issues/12097",
    "user": "https://github.com/jdemeyer"
}
```
The attached patch essentially rewrites `sage-make_relative`.  The new code is cleaner and simpler.  It also changes files only once and only when needed.  The previous version changed every Python file every time `sage-make_relative` was run.

Assignee: **@nexttime**

Author: **Jeroen Demeyer**

Reviewer: **Volker Braun**

Merged: **sage-4.8.alpha3**

_Issue created by migration from https://trac.sagemath.org/ticket/12097_





---

archive/issue_events_149910.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-29T11:11:21Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12097",
    "milestone_number": null,
    "milestone_title": "sage-4.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12097#event-149910"
}
```



---

archive/issue_events_149911.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-29T11:11:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12097",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20scripts",
    "label_color": "000080",
    "label_name": "component: scripts",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12097#event-149911"
}
```



---

archive/issue_events_149912.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-29T11:11:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12097",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12097#event-149912"
}
```



---

archive/issue_events_149913.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-29T11:11:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12097",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12097#event-149913"
}
```



---

archive/issue_comments_131317.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-\n+The attached patch essentially rewrites `sage-make_relative`.  The new code is simpler and more logical.  It also changes files only once.  The previous version changed every Python file every time `sage-make_relative` was run.\n``````\n",
    "created_at": "2011-11-29T11:13:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12097",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12097#issuecomment-131317",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-
+The attached patch essentially rewrites `sage-make_relative`.  The new code is simpler and more logical.  It also changes files only once.  The previous version changed every Python file every time `sage-make_relative` was run.
``````




---

archive/issue_events_149914.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-29T11:13:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12097",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12097#event-149914"
}
```



---

archive/issue_comments_131318.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-The attached patch essentially rewrites `sage-make_relative`.  The new code is simpler and more logical.  It also changes files only once.  The previous version changed every Python file every time `sage-make_relative` was run.\n+The attached patch essentially rewrites `sage-make_relative`.  The new code is cleaner and simpler.  It also changes files only once.  The previous version changed every Python file every time `sage-make_relative` was run.\n``````\n",
    "created_at": "2011-11-29T11:14:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12097",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12097#issuecomment-131318",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-The attached patch essentially rewrites `sage-make_relative`.  The new code is simpler and more logical.  It also changes files only once.  The previous version changed every Python file every time `sage-make_relative` was run.
+The attached patch essentially rewrites `sage-make_relative`.  The new code is cleaner and simpler.  It also changes files only once.  The previous version changed every Python file every time `sage-make_relative` was run.
``````




---

archive/issue_comments_131319.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-The attached patch essentially rewrites `sage-make_relative`.  The new code is cleaner and simpler.  It also changes files only once.  The previous version changed every Python file every time `sage-make_relative` was run.\n+The attached patch essentially rewrites `sage-make_relative`.  The new code is cleaner and simpler.  It also changes files only once and only when needed.  The previous version changed every Python file every time `sage-make_relative` was run.\n``````\n",
    "created_at": "2011-11-29T11:23:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12097",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12097#issuecomment-131319",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-The attached patch essentially rewrites `sage-make_relative`.  The new code is cleaner and simpler.  It also changes files only once.  The previous version changed every Python file every time `sage-make_relative` was run.
+The attached patch essentially rewrites `sage-make_relative`.  The new code is cleaner and simpler.  It also changes files only once and only when needed.  The previous version changed every Python file every time `sage-make_relative` was run.
``````




---

archive/issue_comments_131320.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nLooks good!",
    "created_at": "2011-11-30T09:47:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12097",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12097#issuecomment-131320",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:4'>Comment 4:</a>
Looks good!



---

archive/issue_events_149915.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2011-11-30T09:47:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12097",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12097#event-149915"
}
```



---

archive/issue_events_149916.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2011-11-30T09:47:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12097",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12097#event-149916"
}
```



---

archive/issue_comments_131321.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2011-11-30T09:47:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12097",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12097#issuecomment-131321",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_comments_131322.json:
```json
{
    "body": "Attachment: **[12097_make_relative.patch.gz](https://github.com/sagemath/sage/files/ticket12097/12097_make_relative.patch.gz)**",
    "created_at": "2011-11-30T16:06:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12097",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12097#issuecomment-131322",
    "user": "https://github.com/jdemeyer"
}
```

Attachment: **[12097_make_relative.patch.gz](https://github.com/sagemath/sage/files/ticket12097/12097_make_relative.patch.gz)**



---

archive/issue_comments_131323.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nAfter `F.truncate()`, should there also be a line `F.close()`?  Or does the `with` clause automatically close `F`?",
    "created_at": "2011-11-30T19:46:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12097",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12097#issuecomment-131323",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:5'>Comment 5:</a>
After `F.truncate()`, should there also be a line `F.close()`?  Or does the `with` clause automatically close `F`?



---

archive/issue_comments_131324.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nYes! From http://docs.python.org/tutorial/inputoutput.html#methods-of-file-objects\n\n```\nsage: with open('/tmp/workfile', 'rw') as f:\n....:     read_data = f.read()\n....:     \nsage: f.closed\nTrue\n```",
    "created_at": "2011-11-30T20:05:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12097",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12097#issuecomment-131324",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:6'>Comment 6:</a>
Yes! From http://docs.python.org/tutorial/inputoutput.html#methods-of-file-objects

```
sage: with open('/tmp/workfile', 'rw') as f:
....:     read_data = f.read()
....:     
sage: f.closed
True
```



---

archive/issue_events_149917.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-30T20:45:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12097",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12097#event-149917"
}
```



---

archive/issue_events_149918.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-30T20:45:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12097",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12097#event-149918"
}
```



---

archive/issue_comments_131325.json:
```json
{
    "body": "Merged: **sage-4.8.alpha3**",
    "created_at": "2011-11-30T20:45:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12097",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12097#issuecomment-131325",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.8.alpha3**
