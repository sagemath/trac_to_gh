# Issue 12960: legend not properly set in Graphics().matplotlib()

archive/issues_012788.json:
```json
{
    "assignees": [
        "https://github.com/jasongrout",
        "https://github.com/williamstein"
    ],
    "body": "Bug :) Should be easy to fix.\n\n```\nsage: q = plot(x, legend_label='aha')\nsage: q.legend(True)                 \nsage: qm = q.matplotlib()\n---------------------------------------------------------------------------\nKeyError                                  Traceback (most recent call last)\n\n/home/punarbasu/Installations/sage-5.0.rc0.11080/devel/sage-main/<ipython console> in <module>()\n\n/home/punarbasu/Installations/sage-5.0.rc0.11080/local/lib/python2.7/site-packages/sage/plot/graphics.pyc in matplotlib(self, filename, xmin, xmax, ymin, ymax, figsize, figure, sub, axes, axes_labels, fontsize, frame, verify, aspect_ratio, gridlines, gridlinesstyle, vgridlinesstyle, hgridlinesstyle, show_legend, legend_options, axes_pad, ticks_integer, tick_formatter, ticks)\n   1714             lopts.update(legend_options)\n   1715             lopts.update(self._legend_opts)\n-> 1716             prop = FontProperties(family=lopts.pop('font_family'), weight=lopts.pop('font_weight'), \\\n   1717                     size=lopts.pop('font_size'), style=lopts.pop('font_style'), variant=lopts.pop('font_variant'))\n   1718             color = lopts.pop('back_color')\n\nKeyError: 'font_family'\nsage: \n```\n\n---\nApply [attachment: trac_12960-fix_legend.2.patch](https://github.com/sagemath/sage/files/ticket12960/trac_12960-fix_legend.2.patch.gz) to `devel/sage`\n\nKeywords: **legend matplotlib sd40.5**\n\nReviewer: **Karl-Dieter Crisman**\n\nAuthor: **Punarbasu Purkayastha**\n\nMerged: **sage-5.1.beta2**\n\nComponent: **graphics**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12960_\n\n",
    "closed_at": "2012-06-02T12:15:46Z",
    "created_at": "2012-05-17T07:02:06Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20graphics",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "legend not properly set in Graphics().matplotlib()",
    "type": "issue",
    "updated_at": "2012-06-02T12:15:46Z",
    "url": "https://github.com/sagemath/sage/issues/12960",
    "user": "https://github.com/ppurka"
}
```
Bug :) Should be easy to fix.

```
sage: q = plot(x, legend_label='aha')
sage: q.legend(True)                 
sage: qm = q.matplotlib()
---------------------------------------------------------------------------
KeyError                                  Traceback (most recent call last)

/home/punarbasu/Installations/sage-5.0.rc0.11080/devel/sage-main/<ipython console> in <module>()

/home/punarbasu/Installations/sage-5.0.rc0.11080/local/lib/python2.7/site-packages/sage/plot/graphics.pyc in matplotlib(self, filename, xmin, xmax, ymin, ymax, figsize, figure, sub, axes, axes_labels, fontsize, frame, verify, aspect_ratio, gridlines, gridlinesstyle, vgridlinesstyle, hgridlinesstyle, show_legend, legend_options, axes_pad, ticks_integer, tick_formatter, ticks)
   1714             lopts.update(legend_options)
   1715             lopts.update(self._legend_opts)
-> 1716             prop = FontProperties(family=lopts.pop('font_family'), weight=lopts.pop('font_weight'), \
   1717                     size=lopts.pop('font_size'), style=lopts.pop('font_style'), variant=lopts.pop('font_variant'))
   1718             color = lopts.pop('back_color')

KeyError: 'font_family'
sage: 
```

---
Apply [attachment: trac_12960-fix_legend.2.patch](https://github.com/sagemath/sage/files/ticket12960/trac_12960-fix_legend.2.patch.gz) to `devel/sage`

Keywords: **legend matplotlib sd40.5**

Reviewer: **Karl-Dieter Crisman**

Author: **Punarbasu Purkayastha**

Merged: **sage-5.1.beta2**

Component: **graphics**

_Issue created by migration from https://trac.sagemath.org/ticket/12960_





---

archive/issue_events_179343.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2012-05-17T07:02:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "milestone_number": null,
    "milestone_title": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12960#event-179343"
}
```



---

archive/issue_events_179344.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2012-05-17T07:02:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20graphics",
    "label_color": "0000ff",
    "label_name": "c: graphics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12960#event-179344"
}
```



---

archive/issue_events_179345.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2012-05-17T07:02:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12960#event-179345"
}
```



---

archive/issue_events_179346.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2012-05-17T07:02:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12960#event-179346"
}
```



---

archive/issue_events_179347.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2012-05-17T07:02:06Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "subject": "https://github.com/ppurka",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12960#event-179347"
}
```



---

archive/issue_events_179348.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-05-17T07:02:06Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "subject": "https://github.com/ppurka",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12960#event-179348"
}
```



---

archive/issue_comments_146980.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -20,3 +20,5 @@\n sage: \n ```\n \n+---\n+Apply [attachment: trac_12960-fix_legend.patch](https://github.com/sagemath/sage/files/ticket12960/trac_12960-fix_legend.patch.gz) to `devel/sage`\n``````\n",
    "created_at": "2012-05-17T07:37:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12960#issuecomment-146980",
    "user": "https://github.com/ppurka"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -20,3 +20,5 @@
 sage: 
 ```
 
+---
+Apply [attachment: trac_12960-fix_legend.patch](https://github.com/sagemath/sage/files/ticket12960/trac_12960-fix_legend.patch.gz) to `devel/sage`
``````




---

archive/issue_events_179349.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2012-05-17T07:37:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12960#event-179349"
}
```



---

archive/issue_comments_146981.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nHmm.. the matplotlib arguments are also changed it seems. I will revert that.",
    "created_at": "2012-05-17T07:43:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12960#issuecomment-146981",
    "user": "https://github.com/ppurka"
}
```

<div id="comment:2" align="right">comment:2</div>

Hmm.. the matplotlib arguments are also changed it seems. I will revert that.



---

archive/issue_comments_146982.json:
```json
{
    "body": "Attachment: **[trac_12960-fix_legend.patch.gz](https://github.com/sagemath/sage/files/ticket12960/trac_12960-fix_legend.patch.gz)**\n\napply to devel/sage",
    "created_at": "2012-05-17T08:47:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12960#issuecomment-146982",
    "user": "https://github.com/ppurka"
}
```

Attachment: **[trac_12960-fix_legend.patch.gz](https://github.com/sagemath/sage/files/ticket12960/trac_12960-fix_legend.patch.gz)**

apply to devel/sage



---

archive/issue_comments_146983.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nDoes this really work?  I'm unclear as to how this would work in the case `show is None`.  Or maybe it's not supposed to then, since it's not being shown... in which case why not just put this in the `matplotlib` definition, which currently has `legend_options={`}?  Or maybe with a `@suboptions` decorator like in `show`?  This seems more \"Sage-ic\".",
    "created_at": "2012-05-17T17:43:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12960#issuecomment-146983",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:3" align="right">comment:3</div>

Does this really work?  I'm unclear as to how this would work in the case `show is None`.  Or maybe it's not supposed to then, since it's not being shown... in which case why not just put this in the `matplotlib` definition, which currently has `legend_options={`}?  Or maybe with a `@suboptions` decorator like in `show`?  This seems more "Sage-ic".



---

archive/issue_comments_146984.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nWhoever wrote the legend code probably didn't want unnecessary initializations. I kept it the same, that is the `legend_opts` don't get populated unless it is absolutely required. Otherwise we could just initialize it in `__init__` instead of passing `@options` or `@suboptions` all over the place.\n\nIf `show_legend` is None, then of course, nothing happens. If it is set to True, then something weird can happen. I will check this case.",
    "created_at": "2012-05-17T18:28:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12960#issuecomment-146984",
    "user": "https://github.com/ppurka"
}
```

<div id="comment:4" align="right">comment:4</div>

Whoever wrote the legend code probably didn't want unnecessary initializations. I kept it the same, that is the `legend_opts` don't get populated unless it is absolutely required. Otherwise we could just initialize it in `__init__` instead of passing `@options` or `@suboptions` all over the place.

If `show_legend` is None, then of course, nothing happens. If it is set to True, then something weird can happen. I will check this case.



---

archive/issue_events_179350.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2012-05-17T18:28:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12960#event-179350"
}
```



---

archive/issue_events_179351.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2012-05-17T18:28:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12960#event-179351"
}
```



---

archive/issue_comments_146985.json:
```json
{
    "body": "Work Issues: **check show_legend**",
    "created_at": "2012-05-17T18:28:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12960#issuecomment-146985",
    "user": "https://github.com/ppurka"
}
```

Work Issues: **check show_legend**



---

archive/issue_events_179352.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2012-05-18T06:16:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12960#event-179352"
}
```



---

archive/issue_events_179353.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2012-05-18T06:16:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12960#event-179353"
}
```



---

archive/issue_comments_146986.json:
```json
{
    "body": "Changed work issues from **check show_legend** to none",
    "created_at": "2012-05-18T06:16:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12960#issuecomment-146986",
    "user": "https://github.com/ppurka"
}
```

Changed work issues from **check show_legend** to none



---

archive/issue_comments_146987.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -21,4 +21,4 @@\n ```\n \n ---\n-Apply [attachment: trac_12960-fix_legend.patch](https://github.com/sagemath/sage/files/ticket12960/trac_12960-fix_legend.patch.gz) to `devel/sage`\n+Apply [attachment: trac_12960-fix_legend.2.patch](https://github.com/sagemath/sage/files/ticket12960/trac_12960-fix_legend.2.patch.gz) to `devel/sage`\n``````\n",
    "created_at": "2012-05-18T06:16:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12960#issuecomment-146987",
    "user": "https://github.com/ppurka"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -21,4 +21,4 @@
 ```
 
 ---
-Apply [attachment: trac_12960-fix_legend.patch](https://github.com/sagemath/sage/files/ticket12960/trac_12960-fix_legend.patch.gz) to `devel/sage`
+Apply [attachment: trac_12960-fix_legend.2.patch](https://github.com/sagemath/sage/files/ticket12960/trac_12960-fix_legend.2.patch.gz) to `devel/sage`
``````




---

archive/issue_comments_146988.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nOk. This is a much better fix since it takes care of all cases, for example if you pass an incomplete set of options in the `legend={}` argument, set `show_legend=True`, etc.",
    "created_at": "2012-05-18T06:16:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12960#issuecomment-146988",
    "user": "https://github.com/ppurka"
}
```

<div id="comment:6" align="right">comment:6</div>

Ok. This is a much better fix since it takes care of all cases, for example if you pass an incomplete set of options in the `legend={}` argument, set `show_legend=True`, etc.



---

archive/issue_comments_146989.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nYes, this makes a lot more sense.  Needs doctests!",
    "created_at": "2012-05-25T00:58:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12960#issuecomment-146989",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:7" align="right">comment:7</div>

Yes, this makes a lot more sense.  Needs doctests!



---

archive/issue_comments_146990.json:
```json
{
    "body": "Changed keywords from **legend matplotlib** to **legend matplotlib sd40.5**",
    "created_at": "2012-05-25T00:58:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12960#issuecomment-146990",
    "user": "https://github.com/kcrisman"
}
```

Changed keywords from **legend matplotlib** to **legend matplotlib sd40.5**



---

archive/issue_comments_146991.json:
```json
{
    "body": "Reviewer: **Karl-Dieter Crisman**",
    "created_at": "2012-05-25T00:58:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12960#issuecomment-146991",
    "user": "https://github.com/kcrisman"
}
```

Reviewer: **Karl-Dieter Crisman**



---

archive/issue_comments_146992.json:
```json
{
    "body": "Author: **Punarbasu Purkayastha**",
    "created_at": "2012-05-25T00:58:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12960#issuecomment-146992",
    "user": "https://github.com/kcrisman"
}
```

Author: **Punarbasu Purkayastha**



---

archive/issue_events_179354.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-05-25T00:58:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12960#event-179354"
}
```



---

archive/issue_events_179355.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-05-25T00:58:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12960#event-179355"
}
```



---

archive/issue_comments_146993.json:
```json
{
    "body": "Attachment: **[trac_12960-fix_legend.2.patch.gz](https://github.com/sagemath/sage/files/ticket12960/trac_12960-fix_legend.2.patch.gz)**\n\napply to devel/sage",
    "created_at": "2012-05-25T05:07:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12960#issuecomment-146993",
    "user": "https://github.com/ppurka"
}
```

Attachment: **[trac_12960-fix_legend.2.patch.gz](https://github.com/sagemath/sage/files/ticket12960/trac_12960-fix_legend.2.patch.gz)**

apply to devel/sage



---

archive/issue_comments_146994.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nUpdated the patch with a test.",
    "created_at": "2012-05-25T05:08:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12960#issuecomment-146994",
    "user": "https://github.com/ppurka"
}
```

<div id="comment:8" align="right">comment:8</div>

Updated the patch with a test.



---

archive/issue_events_179356.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2012-05-25T05:08:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12960#event-179356"
}
```



---

archive/issue_events_179357.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2012-05-25T05:08:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12960#event-179357"
}
```



---

archive/issue_comments_146995.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nLooks great, thanks for the good work!",
    "created_at": "2012-05-25T14:49:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12960#issuecomment-146995",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:9" align="right">comment:9</div>

Looks great, thanks for the good work!



---

archive/issue_events_179358.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-05-25T14:49:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12960#event-179358"
}
```



---

archive/issue_events_179359.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-05-25T14:49:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12960#event-179359"
}
```



---

archive/issue_comments_146996.json:
```json
{
    "body": "Merged: **sage-5.1.beta2**",
    "created_at": "2012-06-02T12:15:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12960#issuecomment-146996",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.1.beta2**



---

archive/issue_events_179360.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-02T12:15:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12960#event-179360"
}
```



---

archive/issue_events_179361.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-02T12:15:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12960",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12960#event-179361"
}
```
