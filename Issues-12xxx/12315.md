# Issue 12315: OS X Lion: pari fails self tests

archive/issues_012143.json:
```json
{
    "body": "On OS X Lion, if you set SAGE_CHECK=yes and build pari, some tests fail. See\n\n  [http://sage.math.washington.edu/home/palmieri/misc/pari-2.5.0.p3.log](http://sage.math.washington.edu/home/palmieri/misc/pari-2.5.0.p3.log)\n\nfor a log file.\n\n---\n\nNew spkg: [http://sage.math.washington.edu/home/palmieri/SPKG/pari-2.5.0.p4.spkg](http://sage.math.washington.edu/home/palmieri/SPKG/pari-2.5.0.p4.spkg)\n\n\nAssignee: tbd\n\nCC:  @williamstein @jdemeyer georgsweber\n\nKeywords: pari darwin lion osx\n\nReviewer: John Palmieri\n\nResolution: wontfix\n\nIssue created by migration from https://trac.sagemath.org/ticket/12315\n\n",
    "closed_at": "2012-04-26T20:13:13Z",
    "created_at": "2012-01-16T17:39:08Z",
    "labels": [
        "component: packages: standard",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "OS X Lion: pari fails self tests",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/12315",
    "user": "https://github.com/jhpalmieri"
}
```
On OS X Lion, if you set SAGE_CHECK=yes and build pari, some tests fail. See

  [http://sage.math.washington.edu/home/palmieri/misc/pari-2.5.0.p3.log](http://sage.math.washington.edu/home/palmieri/misc/pari-2.5.0.p3.log)

for a log file.

---

New spkg: [http://sage.math.washington.edu/home/palmieri/SPKG/pari-2.5.0.p4.spkg](http://sage.math.washington.edu/home/palmieri/SPKG/pari-2.5.0.p4.spkg)


Assignee: tbd

CC:  @williamstein @jdemeyer georgsweber

Keywords: pari darwin lion osx

Reviewer: John Palmieri

Resolution: wontfix

Issue created by migration from https://trac.sagemath.org/ticket/12315





---

archive/issue_comments_173865.json:
```json
{
    "body": "<a id='comment:1'></a>was pointed out [on sage-devel](https://groups.google.com/d/topic/sage-devel/j5lsn50_tN8/discussion) that with `SAGE_DEBUG=True`, self-tests pass.  So it may be a compiler error; compiling with `-O0` seems to work.  It may be just a single file (the one containing the bezout function) causing the problems.",
    "created_at": "2012-01-18T02:12:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173865",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:1'></a>was pointed out [on sage-devel](https://groups.google.com/d/topic/sage-devel/j5lsn50_tN8/discussion) that with `SAGE_DEBUG=True`, self-tests pass.  So it may be a compiler error; compiling with `-O0` seems to work.  It may be just a single file (the one containing the bezout function) causing the problems.



---

archive/issue_comments_173866.json:
```json
{
    "body": "<a id='comment:2'></a>An exchange on #11881 about this.\n> > I am currently at the PARI atelier and we're discussing OS X Lion. Essentially the PARI developers are saying that Apple's compiler is completely broken and that the Only Right Solution is to install a real compiler, i.e. gcc.\n> > So maybe the best solution in practice for Sage is to add a check in prereq that users aren't using Apple's broken compiler.  \n\n> ?  So that no one using \"just Xcode\" can build Sage?  I'm a little unclear on what would have to change for ordinary users.\n> > Also we should have one server machine running Lion with a good compiler to build binaries.\n\n> Certainly.\n> jhpalmieri points out on #12315 (where these comments should probably live anyway)\n> > was pointed out  on sage-devel that with SAGE_DEBUG=True, self-tests pass. So it may be a compiler error; compiling with -O0 seems to work. It may be just a single file (the one containing the bezout function) causing the problems.\n\n> So perhaps a workaround would be to check for the \"bad\" compiler in the spkg-install somehow and then do no optimization if that's the case.  Power users who need Pari optimized would be expected to have the technical know-how to use the right one, and as you say the binary builder could have this as well.",
    "created_at": "2012-01-26T13:19:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173866",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:2'></a>An exchange on #11881 about this.
> > I am currently at the PARI atelier and we're discussing OS X Lion. Essentially the PARI developers are saying that Apple's compiler is completely broken and that the Only Right Solution is to install a real compiler, i.e. gcc.
> > So maybe the best solution in practice for Sage is to add a check in prereq that users aren't using Apple's broken compiler.  

> ?  So that no one using "just Xcode" can build Sage?  I'm a little unclear on what would have to change for ordinary users.
> > Also we should have one server machine running Lion with a good compiler to build binaries.

> Certainly.
> jhpalmieri points out on #12315 (where these comments should probably live anyway)
> > was pointed out  on sage-devel that with SAGE_DEBUG=True, self-tests pass. So it may be a compiler error; compiling with -O0 seems to work. It may be just a single file (the one containing the bezout function) causing the problems.

> So perhaps a workaround would be to check for the "bad" compiler in the spkg-install somehow and then do no optimization if that's the case.  Power users who need Pari optimized would be expected to have the technical know-how to use the right one, and as you say the binary builder could have this as well.



---

archive/issue_comments_173867.json:
```json
{
    "body": "<a id='comment:3'></a>Another datapoint, from [comment:23:ticket:11881 here]:\n> I took sage-4.8.alpha0, added in the spkg's from the various tickets here, including the Python spkg from #11967, and built Sage with SAGE_CHECK=yes. Tests failed for the Python spkg, as usual, but they passed for PARI.\n\n\nWas Pari upgraded in the meantime? I assume so.",
    "created_at": "2012-01-26T14:20:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173867",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:3'></a>Another datapoint, from [comment:23:ticket:11881 here]:
> I took sage-4.8.alpha0, added in the spkg's from the various tickets here, including the Python spkg from #11967, and built Sage with SAGE_CHECK=yes. Tests failed for the Python spkg, as usual, but they passed for PARI.


Was Pari upgraded in the meantime? I assume so.



---

archive/issue_comments_173868.json:
```json
{
    "body": "<a id='comment:4'></a>Replying to [comment:3 kcrisman]:\n> Another datapoint, from [comment:23:ticket:11881 here]:\n> > I took sage-4.8.alpha0, added in the spkg's from the various tickets here, including the Python spkg from #11967, and built Sage with SAGE_CHECK=yes. Tests failed for the Python spkg, as usual, but they passed for PARI.\n\n\nI've been unable to reproduce that, so I don't know how I got Pari's self-tests to pass that one time.\n\nIt seems to me that our current viable choices are\n\n1. Test whether the platform is Lion, and if so, turn off optimization.  (Using `-O0` should work; I don't know about `-O1`, but I'm testing that now.)\n\n2. Test whether the platform is Lion, and if so, turn off optimization for the single file causing problems.\n\n3. Test whether the platform is Lion and the compiler is a bad one (I'm not sure what qualifies beyond the current GCC/LLVM thing in !XCode), and if so, turn off optimization.\n\nI can prepare an spkg doing the first of these, but I'm not sure about how to do #2 the right way, and I don't know enough about which compiler(s) are bad and which are good to do #3.",
    "created_at": "2012-01-26T17:08:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173868",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:4'></a>Replying to [comment:3 kcrisman]:
> Another datapoint, from [comment:23:ticket:11881 here]:
> > I took sage-4.8.alpha0, added in the spkg's from the various tickets here, including the Python spkg from #11967, and built Sage with SAGE_CHECK=yes. Tests failed for the Python spkg, as usual, but they passed for PARI.


I've been unable to reproduce that, so I don't know how I got Pari's self-tests to pass that one time.

It seems to me that our current viable choices are

1. Test whether the platform is Lion, and if so, turn off optimization.  (Using `-O0` should work; I don't know about `-O1`, but I'm testing that now.)

2. Test whether the platform is Lion, and if so, turn off optimization for the single file causing problems.

3. Test whether the platform is Lion and the compiler is a bad one (I'm not sure what qualifies beyond the current GCC/LLVM thing in !XCode), and if so, turn off optimization.

I can prepare an spkg doing the first of these, but I'm not sure about how to do #2 the right way, and I don't know enough about which compiler(s) are bad and which are good to do #3.



---

archive/issue_comments_173869.json:
```json
{
    "body": "<a id='comment:5'></a>This should be coordinated with #12363, but go ahead.  I would prefer option 3. to option 1.  Could you not simply blacklist the default compiler on OS X 10.7?",
    "created_at": "2012-01-26T17:18:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173869",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:5'></a>This should be coordinated with #12363, but go ahead.  I would prefer option 3. to option 1.  Could you not simply blacklist the default compiler on OS X 10.7?



---

archive/issue_comments_173870.json:
```json
{
    "body": "<a id='comment:6'></a>> Could you not simply blacklist the default compiler on OS X 10.7?\n\nCf: William's recent comment on #11881 about whether he wants to do so on Lion:\n> > Power users who need Pari optimized would be expected to have the technical know-how to use the right one,\n\n> I'm such a user and I don't want to have to build GCC from source.\nIt's pretty painful to deal with anything else on OS X.  We couldn't even really use the fink or macports gcc, because we currently require people to rename or remove from their path anything in /sw.",
    "created_at": "2012-01-26T17:32:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173870",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:6'></a>> Could you not simply blacklist the default compiler on OS X 10.7?

Cf: William's recent comment on #11881 about whether he wants to do so on Lion:
> > Power users who need Pari optimized would be expected to have the technical know-how to use the right one,

> I'm such a user and I don't want to have to build GCC from source.
It's pretty painful to deal with anything else on OS X.  We couldn't even really use the fink or macports gcc, because we currently require people to rename or remove from their path anything in /sw.



---

archive/issue_comments_173871.json:
```json
{
    "body": "<a id='comment:7'></a>Replying to [comment:6 kcrisman]:\n> > Could you not simply blacklist the default compiler on OS X 10.7?\n\n> Cf: William's recent comment on #11881 about whether he wants to do so on Lion:\n> > > Power users who need Pari optimized would be expected to have the technical know-how to use the right one,\n\n> > I'm such a user and I don't want to have to build GCC from source.\n> It's pretty painful to deal with anything else on OS X.  We couldn't even really use the fink or macports gcc, because we currently require people to rename or remove from their path anything in /sw.\n\n\nUnless we create an spkg \"gcc\" which works properly with Sage.  I noticed somebody made an experimental package \"gcc\" but I haven't tried it.",
    "created_at": "2012-01-26T17:48:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173871",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:7'></a>Replying to [comment:6 kcrisman]:
> > Could you not simply blacklist the default compiler on OS X 10.7?

> Cf: William's recent comment on #11881 about whether he wants to do so on Lion:
> > > Power users who need Pari optimized would be expected to have the technical know-how to use the right one,

> > I'm such a user and I don't want to have to build GCC from source.
> It's pretty painful to deal with anything else on OS X.  We couldn't even really use the fink or macports gcc, because we currently require people to rename or remove from their path anything in /sw.


Unless we create an spkg "gcc" which works properly with Sage.  I noticed somebody made an experimental package "gcc" but I haven't tried it.



---

archive/issue_comments_173872.json:
```json
{
    "body": "<a id='comment:8'></a>If the problem is a single file, the file [should be](http://pari.math.u-bordeaux.fr/archives/pari-dev-1108/msg00007.html) `src/src/kernel/none/gcdll.c`.  I tried compiling just that file with `-O0`, but I still got self test failures -- not as many, but a few:\n\n```\nell-sta.dif\nell.dyn.dif\n```\nTo build this way, I patched the file `src/src/kernel/gmp/MakeLVL1.SH`, adding in the `-O0` flag.\n\nFor what it's worth, on this particular machine, it took about 19 minutes to run self tests after compiling all of Pari with `-O0`, and it took about 15 minutes compiling most with `-O3` and just the one file with `-O0`.",
    "created_at": "2012-01-26T19:49:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173872",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:8'></a>If the problem is a single file, the file [should be](http://pari.math.u-bordeaux.fr/archives/pari-dev-1108/msg00007.html) `src/src/kernel/none/gcdll.c`.  I tried compiling just that file with `-O0`, but I still got self test failures -- not as many, but a few:

```
ell-sta.dif
ell.dyn.dif
```
To build this way, I patched the file `src/src/kernel/gmp/MakeLVL1.SH`, adding in the `-O0` flag.

For what it's worth, on this particular machine, it took about 19 minutes to run self tests after compiling all of Pari with `-O0`, and it took about 15 minutes compiling most with `-O3` and just the one file with `-O0`.



---

archive/issue_comments_173873.json:
```json
{
    "body": "<a id='comment:9'></a>Please post the file `ell-sta.dif` (it should be inside `Odarwin-*` in the pari build directory).",
    "created_at": "2012-01-26T19:55:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173873",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:9'></a>Please post the file `ell-sta.dif` (it should be inside `Odarwin-*` in the pari build directory).



---

archive/issue_comments_173874.json:
```json
{
    "body": "<a id='comment:10'></a>Replying to [comment:5 jdemeyer]:\n> This should be coordinated with #12363, but go ahead.  I would prefer option 3.\n>  to option 1.  Could you not simply blacklist the default compiler on OS X 10.7?\n\n\nI consider option 3 a total non-option unless there is a fully working alternative GCC spkg available.   If there were, that would be the way to go.  I posted to sage-devel to see if anybody has something like this.\n\nI prefer option 2 by far, along with reporting the bugs to APPLE.  Then just turn off 2 once the bug is fixed (but of course don't just build broken sage's by people who haven't upgraded yet).",
    "created_at": "2012-01-26T20:45:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173874",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:10'></a>Replying to [comment:5 jdemeyer]:
> This should be coordinated with #12363, but go ahead.  I would prefer option 3.
>  to option 1.  Could you not simply blacklist the default compiler on OS X 10.7?


I consider option 3 a total non-option unless there is a fully working alternative GCC spkg available.   If there were, that would be the way to go.  I posted to sage-devel to see if anybody has something like this.

I prefer option 2 by far, along with reporting the bugs to APPLE.  Then just turn off 2 once the bug is fixed (but of course don't just build broken sage's by people who haven't upgraded yet).



---

archive/issue_comments_173875.json:
```json
{
    "body": "<a id='comment:11'></a>Since the file is small, I'm attaching `ell-sta.dif`.",
    "created_at": "2012-01-26T20:50:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173875",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:11'></a>Since the file is small, I'm attaching `ell-sta.dif`.



---

archive/issue_comments_173876.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -4,6 +4,11 @@\n \n for a log file.\n \n+---\n+\n+New spkg: [http://sage.math.washington.edu/home/palmieri/SPKG/pari-2.5.1.p0.spkg](http://sage.math.washington.edu/home/palmieri/SPKG/pari-2.5.1.p0.spkg)\n+\n+\n Comment: 1\n \n Issue created by migration from https://trac.sagemath.org/ticket/12315\n``````\n",
    "created_at": "2012-01-26T22:44:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173876",
    "user": "https://github.com/jhpalmieri"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -4,6 +4,11 @@
 
 for a log file.
 
+---
+
+New spkg: [http://sage.math.washington.edu/home/palmieri/SPKG/pari-2.5.1.p0.spkg](http://sage.math.washington.edu/home/palmieri/SPKG/pari-2.5.1.p0.spkg)
+
+
 Comment: 1
 
 Issue created by migration from https://trac.sagemath.org/ticket/12315
``````




---

archive/issue_comments_173877.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2012-01-26T22:44:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173877",
    "user": "https://github.com/jhpalmieri"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_173878.json:
```json
{
    "body": "<a id='comment:12'></a>Attachment [ell-sta.dif](tarball://root/attachments/some-uuid/ticket12315/ell-sta.dif) by @jhpalmieri created at 2012-01-26 22:44:02\n\nOkay, nailed it.  On OS X Lion, 'gcc' is a symbolic link to '/usr/bin/llvm-gcc-4.2', but '/usr/bin/gcc-4.2' is a plain version of gcc.  So if, when building Pari, we do `export CC=gcc-4.2`, then self-tests pass when building with standard (`-O3`) optimization.  So we have an easy solution, but it leads to several questions:\n\n- Is the version of OS X (Lion vs. Snow Leopard) important, or is it only the version of XCode?  That is, should we just test whether 'LLVM' is in the output from `gcc --version` (or perhaps something more refined, like 'LLVM' is in the output from `gcc --version` and gcc is version 4.2.1, or `gcc --version` contains the string \"LLVM build 2.335.15.00\")?\n\n- Should we make this change only for Pari, or should we do this when building all of Sage?  I have a feeling we do it on a case-by-case basis, since I think that not all spkgs respect the CC variable.  I tried it with cvxopt (see #12011), and it didn't seem to help, but it's worth exploring more...\n\nI've posted an spkg which tests whether we're using an 'LLVM' version of gcc 4.2.1.",
    "created_at": "2012-01-26T22:44:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173878",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:12'></a>Attachment [ell-sta.dif](tarball://root/attachments/some-uuid/ticket12315/ell-sta.dif) by @jhpalmieri created at 2012-01-26 22:44:02

Okay, nailed it.  On OS X Lion, 'gcc' is a symbolic link to '/usr/bin/llvm-gcc-4.2', but '/usr/bin/gcc-4.2' is a plain version of gcc.  So if, when building Pari, we do `export CC=gcc-4.2`, then self-tests pass when building with standard (`-O3`) optimization.  So we have an easy solution, but it leads to several questions:

- Is the version of OS X (Lion vs. Snow Leopard) important, or is it only the version of XCode?  That is, should we just test whether 'LLVM' is in the output from `gcc --version` (or perhaps something more refined, like 'LLVM' is in the output from `gcc --version` and gcc is version 4.2.1, or `gcc --version` contains the string "LLVM build 2.335.15.00")?

- Should we make this change only for Pari, or should we do this when building all of Sage?  I have a feeling we do it on a case-by-case basis, since I think that not all spkgs respect the CC variable.  I tried it with cvxopt (see #12011), and it didn't seem to help, but it's worth exploring more...

I've posted an spkg which tests whether we're using an 'LLVM' version of gcc 4.2.1.



---

archive/issue_comments_173879.json:
```json
{
    "body": "<a id='comment:13'></a>NICE!!!\n\nCould you look into symmetrica too?   \n\nNICE!!!",
    "created_at": "2012-01-26T22:56:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173879",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:13'></a>NICE!!!

Could you look into symmetrica too?   

NICE!!!



---

archive/issue_comments_173880.json:
```json
{
    "body": "<a id='comment:14'></a>Sure, I'll try.  Is the problem with symmetrica only revealed by failed Sage doctests?  (I don't see a ticket for symmetrica on Lion at #11881).",
    "created_at": "2012-01-26T23:09:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173880",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:14'></a>Sure, I'll try.  Is the problem with symmetrica only revealed by failed Sage doctests?  (I don't see a ticket for symmetrica on Lion at #11881).



---

archive/issue_comments_173881.json:
```json
{
    "body": "<a id='comment:15'></a>Replying to [comment:14 jhpalmieri]:\n> Sure, I'll try.  Is the problem with symmetrica only revealed by failed Sage doctests?\n\n\nYes.",
    "created_at": "2012-01-26T23:11:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173881",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:15'></a>Replying to [comment:14 jhpalmieri]:
> Sure, I'll try.  Is the problem with symmetrica only revealed by failed Sage doctests?


Yes.



---

archive/issue_comments_173882.json:
```json
{
    "body": "<a id='comment:16'></a>Great work, jhpalmieri.  Question: is there any (tiny) chance that the '/usr/bin/gcc-4.2' is a leftover from a previous Xcode you had on this machine?  It would be good to test this on a machine that has only had Xcode 4 and Lion on it.",
    "created_at": "2012-01-27T01:51:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173882",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:16'></a>Great work, jhpalmieri.  Question: is there any (tiny) chance that the '/usr/bin/gcc-4.2' is a leftover from a previous Xcode you had on this machine?  It would be good to test this on a machine that has only had Xcode 4 and Lion on it.



---

archive/issue_comments_173883.json:
```json
{
    "body": "<a id='comment:17'></a>Replying to [comment:16 kcrisman]:\n> Great work, jhpalmieri.  Question: is there any (tiny) chance that the '/usr/bin/gcc-4.2' is a leftover from a previous Xcode you had on this machine?  It would be good to test this on a machine that has only had Xcode 4 and Lion on it.\n\n\nI think there is a tiny chance, but all of these different versions of gcc (gcc-4.2, llvm-gcc-4.2, etc) have the same modification date, so I think they all came from XCode 4.\n\nMeanwhile, I'm having problems which I'm trying to track down: with sage-5.0.prealpha1, I get no test failures with this Pari spkg.  With sage-5.0.beta1, I get a few test failures.  I'm not sure what the difference is, though.  Maybe the termcap spkg?",
    "created_at": "2012-01-27T05:45:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173883",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:17'></a>Replying to [comment:16 kcrisman]:
> Great work, jhpalmieri.  Question: is there any (tiny) chance that the '/usr/bin/gcc-4.2' is a leftover from a previous Xcode you had on this machine?  It would be good to test this on a machine that has only had Xcode 4 and Lion on it.


I think there is a tiny chance, but all of these different versions of gcc (gcc-4.2, llvm-gcc-4.2, etc) have the same modification date, so I think they all came from XCode 4.

Meanwhile, I'm having problems which I'm trying to track down: with sage-5.0.prealpha1, I get no test failures with this Pari spkg.  With sage-5.0.beta1, I get a few test failures.  I'm not sure what the difference is, though.  Maybe the termcap spkg?



---

archive/issue_comments_173884.json:
```json
{
    "body": "<a id='comment:18'></a>> > Great work, jhpalmieri.  Question: is there any (tiny) chance that the '/usr/bin/gcc-4.2' is a leftover from a previous Xcode you had on this machine?  It would be good to test this on a machine that has only had Xcode 4 and Lion on it.\n\n> \n> I think there is a tiny chance, but all of these different versions of gcc (gcc-4.2, llvm-gcc-4.2, etc) have the same modification date, so I think they all came from XCode 4.\n\n\nHmm, but see [Georg's post](http://groups.google.com/group/sage-devel/browse_thread/thread/d9a00d4bb84474c4/f88220b8e72e5b12?show_docid=f88220b8e72e5b12) about this on sage-devel:\n\n```\nStay with the old \"FSF-flavor\" Apple version of gcc v4.2.1 \"build \n5666.3\", as has been included in XCode 3.2.6, 4.0. and 4.1 (but no \nlonger from 4.2 onwards), and that gcc42 package. \n```\nSo maybe this will go away, and we might not want to rely on it.",
    "created_at": "2012-01-27T15:43:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173884",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:18'></a>> > Great work, jhpalmieri.  Question: is there any (tiny) chance that the '/usr/bin/gcc-4.2' is a leftover from a previous Xcode you had on this machine?  It would be good to test this on a machine that has only had Xcode 4 and Lion on it.

> 
> I think there is a tiny chance, but all of these different versions of gcc (gcc-4.2, llvm-gcc-4.2, etc) have the same modification date, so I think they all came from XCode 4.


Hmm, but see [Georg's post](http://groups.google.com/group/sage-devel/browse_thread/thread/d9a00d4bb84474c4/f88220b8e72e5b12?show_docid=f88220b8e72e5b12) about this on sage-devel:

```
Stay with the old "FSF-flavor" Apple version of gcc v4.2.1 "build 
5666.3", as has been included in XCode 3.2.6, 4.0. and 4.1 (but no 
longer from 4.2 onwards), and that gcc42 package. 
```
So maybe this will go away, and we might not want to rely on it.



---

archive/issue_comments_173885.json:
```json
{
    "body": "<a id='comment:19'></a>Okay, at least I've figured out the problem with test failures: I based my spkg on a prerelease version of 2.5.1 (from #12363), and the installation process wasn't installing all of the required data files.  This was making tests fail.  In the prealpha1 installation, I must have installed an earlier version of Pari (without SAGE_CHECK set), so it had installed the data earlier.\n\nNow I'm working on a different spkg based on the old version of Pari, and things are looking good.",
    "created_at": "2012-01-28T03:38:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173885",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:19'></a>Okay, at least I've figured out the problem with test failures: I based my spkg on a prerelease version of 2.5.1 (from #12363), and the installation process wasn't installing all of the required data files.  This was making tests fail.  In the prealpha1 installation, I must have installed an earlier version of Pari (without SAGE_CHECK set), so it had installed the data earlier.

Now I'm working on a different spkg based on the old version of Pari, and things are looking good.



---

archive/issue_comments_173886.json:
```json
{
    "body": "<a id='comment:20'></a>(The posted spkg is broken because I used an incomplete tar ball.  Once the spkg at #12363 is in good shape -- it's too big right now -- I'll post a new one based on that.)",
    "created_at": "2012-01-28T03:53:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173886",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:20'></a>(The posted spkg is broken because I used an incomplete tar ball.  Once the spkg at #12363 is in good shape -- it's too big right now -- I'll post a new one based on that.)



---

archive/issue_comments_173887.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2012-01-28T03:53:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173887",
    "user": "https://github.com/jhpalmieri"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_173888.json:
```json
{
    "body": "<a id='comment:21'></a>Here's a new spkg, based on the old version of Pari.  This gives us something to test with on Lion.  If it works and if we're happy with the solution, the patch here should be easy to merge with the spkg at #12363.\n\nI'm not what to do about the possible lack of gcc-4.2 in newer versions of XCode.  I can't find any verification of Georg's statement about this.",
    "created_at": "2012-01-28T04:39:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173888",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:21'></a>Here's a new spkg, based on the old version of Pari.  This gives us something to test with on Lion.  If it works and if we're happy with the solution, the patch here should be easy to merge with the spkg at #12363.

I'm not what to do about the possible lack of gcc-4.2 in newer versions of XCode.  I can't find any verification of Georg's statement about this.



---

archive/issue_comments_173889.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2012-01-28T04:39:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173889",
    "user": "https://github.com/jhpalmieri"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_173890.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,12 @@\n+On OS X Lion, if you set SAGE_CHECK=yes and build pari, some tests fail. See\n+\n+  [http://sage.math.washington.edu/home/palmieri/misc/pari-2.5.0.p3.log](http://sage.math.washington.edu/home/palmieri/misc/pari-2.5.0.p3.log)\n+\n+for a log file.\n+\n+---\n+\n+New spkg: [http://sage.math.washington.edu/home/palmieri/SPKG/pari-2.5.0.p4.spkg](http://sage.math.washington.edu/home/palmieri/SPKG/pari-2.5.0.p4.spkg)\n \n \n Comment: 1\n``````\n",
    "created_at": "2012-01-28T04:39:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173890",
    "user": "https://github.com/jhpalmieri"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,12 @@
+On OS X Lion, if you set SAGE_CHECK=yes and build pari, some tests fail. See
+
+  [http://sage.math.washington.edu/home/palmieri/misc/pari-2.5.0.p3.log](http://sage.math.washington.edu/home/palmieri/misc/pari-2.5.0.p3.log)
+
+for a log file.
+
+---
+
+New spkg: [http://sage.math.washington.edu/home/palmieri/SPKG/pari-2.5.0.p4.spkg](http://sage.math.washington.edu/home/palmieri/SPKG/pari-2.5.0.p4.spkg)
 
 
 Comment: 1
``````




---

archive/issue_comments_173891.json:
```json
{
    "body": "<a id='comment:22'></a>By the way, I tried\n\n```\nCC=clang  # instead of gcc-4.2\nexport CC\n```\nbut spkg-check had lots of failures.",
    "created_at": "2012-01-28T17:25:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173891",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:22'></a>By the way, I tried

```
CC=clang  # instead of gcc-4.2
export CC
```
but spkg-check had lots of failures.



---

archive/issue_comments_173892.json:
```json
{
    "body": "<a id='comment:23'></a>Maybe I messed up last time, but using clang is working for me now.  Since clang is part of XCode 4, it is guaranteed to be there (as opposed to gcc-4.2).  Using clang with the standard CFLAGS is presumably faster than using the default compiler with optimization turned off.\n\nSo I've updated the spkg to use clang.",
    "created_at": "2012-02-04T06:19:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173892",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:23'></a>Maybe I messed up last time, but using clang is working for me now.  Since clang is part of XCode 4, it is guaranteed to be there (as opposed to gcc-4.2).  Using clang with the standard CFLAGS is presumably faster than using the default compiler with optimization turned off.

So I've updated the spkg to use clang.



---

archive/issue_comments_173893.json:
```json
{
    "body": "patch for pari spkg; for reference only",
    "created_at": "2012-02-05T16:56:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173893",
    "user": "https://github.com/jhpalmieri"
}
```

patch for pari spkg; for reference only



---

archive/issue_comments_173894.json:
```json
{
    "body": "<a id='comment:24'></a>Attachment [trac_12315-pari.patch](tarball://root/attachments/some-uuid/ticket12315/trac_12315-pari.patch) by @jdemeyer created at 2012-02-08 07:51:24\n\nSince PARI-2.5.0 doesn't work with gcc-4.6.2 on OS X, but PARI-2.5.1 (#12363) does work, you might want to work with the new PARI.",
    "created_at": "2012-02-08T07:51:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173894",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:24'></a>Attachment [trac_12315-pari.patch](tarball://root/attachments/some-uuid/ticket12315/trac_12315-pari.patch) by @jdemeyer created at 2012-02-08 07:51:24

Since PARI-2.5.0 doesn't work with gcc-4.6.2 on OS X, but PARI-2.5.1 (#12363) does work, you might want to work with the new PARI.



---

archive/issue_comments_173895.json:
```json
{
    "body": "<a id='comment:25'></a>I think this ticket should be closed, since the Pari issues are taken care of via #12369.",
    "created_at": "2012-04-21T17:21:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173895",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:25'></a>I think this ticket should be closed, since the Pari issues are taken care of via #12369.



---

archive/issue_comments_173896.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2012-04-21T17:21:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173896",
    "user": "https://github.com/jhpalmieri"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_events_032970.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-04-21T17:21:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12315#event-32970"
}
```



---

archive/issue_events_032971.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-04-26T20:13:13Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12315#event-32971"
}
```



---

archive/issue_comments_173897.json:
```json
{
    "body": "Changing reviewer from \"\" to \"John Palmieri\"",
    "created_at": "2012-04-26T20:13:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173897",
    "user": "https://github.com/jdemeyer"
}
```

Changing reviewer from "" to "John Palmieri"



---

archive/issue_comments_173898.json:
```json
{
    "body": "Resolution: wontfix",
    "created_at": "2012-04-26T20:13:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12315#issuecomment-173898",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: wontfix
