# Issue 12256: if x=var('x') and n is an integer then n.binomial(x) should return binomial(SR(n),x)

archive/issues_012084.json:
```json
{
    "assignees": [],
    "body": "Somebody tried to do \n\n```\nsum(binomial(3, k), k, 0, 3)\n```\nand got a weird error, but\n\n```\nsum(binomial(SR(3), k), k, 0, 3)\n```\nworks.  This is because 3.binomial isn't sophisticated enough... or maybe `binomial(-, -)` isn't either.\n\nThis is a duplicate of #9634.\n\nAssignee: **@burcin**\n\nReviewer: **Burcin Erocal**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12256_\n\n",
    "closed_at": "2012-01-31T09:40:14Z",
    "created_at": "2012-01-03T21:17:02Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "if x=var('x') and n is an integer then n.binomial(x) should return binomial(SR(n),x)",
    "type": "issue",
    "updated_at": "2012-01-31T09:40:14Z",
    "url": "https://github.com/sagemath/sage/issues/12256",
    "user": "https://github.com/williamstein"
}
```
Somebody tried to do 

```
sum(binomial(3, k), k, 0, 3)
```
and got a weird error, but

```
sum(binomial(SR(3), k), k, 0, 3)
```
works.  This is because 3.binomial isn't sophisticated enough... or maybe `binomial(-, -)` isn't either.

This is a duplicate of #9634.

Assignee: **@burcin**

Reviewer: **Burcin Erocal**

_Issue created by migration from https://trac.sagemath.org/ticket/12256_





---

archive/issue_events_152524.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-01-03T21:17:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12256",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12256#event-152524"
}
```



---

archive/issue_events_152525.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-01-03T21:17:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12256",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12256#event-152525"
}
```



---

archive/issue_events_152526.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-01-03T21:17:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12256",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12256#event-152526"
}
```



---

archive/issue_events_152527.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-01-03T21:17:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12256",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12256#event-152527"
}
```



---

archive/issue_comments_134876.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -8,4 +8,4 @@\n ```\n sum(binomial(SR(3), k), k, 0, 3)\n ```\n-works.  This is because 3.binomial isn't sophisticated enough.\n+works.  This is because 3.binomial isn't sophisticated enough... or maybe `binomial(-, -)` isn't either.\n``````\n",
    "created_at": "2012-01-03T21:18:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12256",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12256#issuecomment-134876",
    "user": "https://github.com/williamstein"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -8,4 +8,4 @@
 ```
 sum(binomial(SR(3), k), k, 0, 3)
 ```
-works.  This is because 3.binomial isn't sophisticated enough.
+works.  This is because 3.binomial isn't sophisticated enough... or maybe `binomial(-, -)` isn't either.
``````




---

archive/issue_comments_134877.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nThis is a duplicate of #9634.",
    "created_at": "2012-01-03T21:59:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12256",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12256#issuecomment-134877",
    "user": "https://github.com/burcin"
}
```

<div id="comment:2" align="right">Comment 2</div>

This is a duplicate of #9634.



---

archive/issue_comments_134878.json:
```json
{
    "body": "Reviewer: **Burcin Erocal**",
    "created_at": "2012-01-03T21:59:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12256",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12256#issuecomment-134878",
    "user": "https://github.com/burcin"
}
```

Reviewer: **Burcin Erocal**



---

archive/issue_events_152528.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2012-01-03T21:59:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12256",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12256#event-152528"
}
```



---

archive/issue_events_152529.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2012-01-03T21:59:00Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12256",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12256#event-152529"
}
```



---

archive/issue_events_152530.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2012-01-03T21:59:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12256",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12256#event-152530"
}
```



---

archive/issue_events_152531.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2012-01-03T21:59:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12256",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12256#event-152531"
}
```



---

archive/issue_events_152532.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2012-01-17T14:05:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12256",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12256#event-152532"
}
```



---

archive/issue_events_152533.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2012-01-17T14:05:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12256",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12256#event-152533"
}
```



---

archive/issue_comments_134879.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -9,3 +9,5 @@\n sum(binomial(SR(3), k), k, 0, 3)\n ```\n works.  This is because 3.binomial isn't sophisticated enough... or maybe `binomial(-, -)` isn't either.\n+\n+This is a duplicate of #9634.\n``````\n",
    "created_at": "2012-01-31T09:40:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12256",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12256#issuecomment-134879",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -9,3 +9,5 @@
 sum(binomial(SR(3), k), k, 0, 3)
 ```
 works.  This is because 3.binomial isn't sophisticated enough... or maybe `binomial(-, -)` isn't either.
+
+This is a duplicate of #9634.
``````




---

archive/issue_events_152534.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-01-31T09:40:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12256",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12256#event-152534"
}
```



---

archive/issue_events_152535.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-01-31T09:40:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12256",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12256#event-152535"
}
```



---

archive/issue_events_152536.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-01-31T09:40:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12256",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12256#event-152536"
}
```
