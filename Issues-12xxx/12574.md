# Issue 12574: python spkg: don't use xcodebuild to check Xcode version

archive/issues_012402.json:
```json
{
    "assignees": [],
    "body": "This is a followup to #11967. On OS X with Xcode 4.3, the command `xcodebuild` is no longer maintained, so we shouldn't use it to check the Xcode version number. So in spkg-install, replace its use with just a check on the version of OS X.\n\n---\n\nNew spkg:\n\n- [http://sage.math.washington.edu/home/palmieri/SPKG/python-2.7.2.p4.spkg](http://sage.math.washington.edu/home/palmieri/SPKG/python-2.7.2.p4.spkg)\n\n\nAuthor: **John Palmieri**\n\nReviewer: **Jeroen Demeyer**\n\nMerged: **sage-5.0.beta6**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12574_\n\n",
    "closed_at": "2012-02-27T11:23:47Z",
    "created_at": "2012-02-23T17:33:08Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "python spkg: don't use xcodebuild to check Xcode version",
    "type": "issue",
    "updated_at": "2012-02-27T11:23:47Z",
    "url": "https://github.com/sagemath/sage/issues/12574",
    "user": "https://github.com/jhpalmieri"
}
```
This is a followup to #11967. On OS X with Xcode 4.3, the command `xcodebuild` is no longer maintained, so we shouldn't use it to check the Xcode version number. So in spkg-install, replace its use with just a check on the version of OS X.

---

New spkg:

- [http://sage.math.washington.edu/home/palmieri/SPKG/python-2.7.2.p4.spkg](http://sage.math.washington.edu/home/palmieri/SPKG/python-2.7.2.p4.spkg)


Author: **John Palmieri**

Reviewer: **Jeroen Demeyer**

Merged: **sage-5.0.beta6**

_Issue created by migration from https://trac.sagemath.org/ticket/12574_





---

archive/issue_events_157832.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-02-23T17:33:08Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12574",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12574#event-157832"
}
```



---

archive/issue_events_157833.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-02-23T17:33:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12574",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12574#event-157833"
}
```



---

archive/issue_events_157834.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-02-23T17:33:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12574",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12574#event-157834"
}
```



---

archive/issue_events_157835.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-02-23T17:33:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12574",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12574#event-157835"
}
```



---

archive/issue_comments_141861.json:
```json
{
    "body": "patch for python spkg; for reference only",
    "created_at": "2012-02-23T17:34:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12574",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12574#issuecomment-141861",
    "user": "https://github.com/jhpalmieri"
}
```

patch for python spkg; for reference only



---

archive/issue_events_157836.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-02-23T17:36:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12574",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12574#event-157836"
}
```



---

archive/issue_comments_141862.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,2 +1,8 @@\n This is a followup to #11967. On OS X with Xcode 4.3, the command `xcodebuild` is no longer maintained, so we shouldn't use it to check the Xcode version number. So in spkg-install, replace its use with just a check on the version of OS X.\n \n+---\n+\n+New spkg:\n+\n+- [http://sage.math.washington.edu/home/palmieri/SPKG/python-2.7.2.p4.spkg](http://sage.math.washington.edu/home/palmieri/SPKG/python-2.7.2.p4.spkg)\n+\n``````\n",
    "created_at": "2012-02-23T17:36:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12574",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12574#issuecomment-141862",
    "user": "https://github.com/jhpalmieri"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,2 +1,8 @@
 This is a followup to #11967. On OS X with Xcode 4.3, the command `xcodebuild` is no longer maintained, so we shouldn't use it to check the Xcode version number. So in spkg-install, replace its use with just a check on the version of OS X.
 
+---
+
+New spkg:
+
+- [http://sage.math.washington.edu/home/palmieri/SPKG/python-2.7.2.p4.spkg](http://sage.math.washington.edu/home/palmieri/SPKG/python-2.7.2.p4.spkg)
+
``````




---

archive/issue_comments_141863.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nAttachment: **[trac_12574-python.patch.gz](https://github.com/sagemath/sage/files/ticket12574/trac_12574-python.patch.gz)**",
    "created_at": "2012-02-23T17:36:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12574",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12574#issuecomment-141863",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:1" align="right">Comment 1</div>

Attachment: **[trac_12574-python.patch.gz](https://github.com/sagemath/sage/files/ticket12574/trac_12574-python.patch.gz)**



---

archive/issue_comments_141864.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2012-02-23T19:20:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12574",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12574#issuecomment-141864",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_comments_141865.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nOn first sight, this looks okay.  It needs testing though...",
    "created_at": "2012-02-23T19:20:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12574",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12574#issuecomment-141865",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

On first sight, this looks okay.  It needs testing though...



---

archive/issue_events_157837.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-24T14:59:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12574",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12574#event-157837"
}
```



---

archive/issue_events_157838.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-24T14:59:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12574",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12574#event-157838"
}
```



---

archive/issue_events_157839.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-27T11:23:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12574",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12574#event-157839"
}
```



---

archive/issue_events_157840.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-27T11:23:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12574",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12574#event-157840"
}
```



---

archive/issue_comments_141866.json:
```json
{
    "body": "Merged: **sage-5.0.beta6**",
    "created_at": "2012-02-27T11:23:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12574",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12574#issuecomment-141866",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.0.beta6**
