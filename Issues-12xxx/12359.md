# Issue 12359: probabilistic hermite_form recurses instead of loops

archive/issues_012187.json:
```json
{
    "body": "The default hnf for integral matrices uses a heuristic choice of pivots and loops until it found working pivots. But a subroutine in the loop can call hnf again, which soon leads to `RuntimeError: maximum recursion depth exceeded`. The actual loop is\n\n```\n  File \"/home/vbraun/opt/sage-5.0.beta1/local/lib/python2.7/site-packages/sage/matrix/matrix_integer_dense_hnf.py\", line 1006, in hnf\n    H, pivots = probable_hnf(A, include_zero_rows = include_zero_rows, proof=True)\n  File \"/home/vbraun/opt/sage-5.0.beta1/local/lib/python2.7/site-packages/sage/matrix/matrix_integer_dense_hnf.py\", line 838, in probable_hnf\n    H = hnf_square(C, proof=proof)\n  File \"/home/vbraun/opt/sage-5.0.beta1/local/lib/python2.7/site-packages/sage/matrix/matrix_integer_dense_hnf.py\", line 574, in hnf_square\n    x = add_column(W, H, b.stack(matrix(1,1,[k*A[m-2,m-1] + l*A[m-1,m-1]])), proof)\n  File \"/home/vbraun/opt/sage-5.0.beta1/local/lib/python2.7/site-packages/sage/matrix/matrix_integer_dense_hnf.py\", line 413, in add_column\n    return add_column_fallback(B, a, proof)\n  File \"/home/vbraun/opt/sage-5.0.beta1/local/lib/python2.7/site-packages/sage/matrix/matrix_integer_dense_hnf.py\", line 292, in add_column_fallback\n    H, _ = hnf(W, proof)\n  File \"/home/vbraun/opt/sage-5.0.beta1/local/lib/python2.7/site-packages/sage/matrix/matrix_integer_dense_hnf.py\", line 1006, in hnf\n    H, pivots = probable_hnf(A, include_zero_rows = include_zero_rows, proof=True)\n```\nAttached is a Sage script that constructs a particular 391x391 matrix that exhibits this problem. Computing the hnf with `algorithm='pari'` gives the result in seconds, but by default the echelon form crashes with the above `RuntimeError` after a much longer wait.\n\nAssignee: jason, was\n\nCC:  @ClementPernet @williamstein @rbeezer @burcin\n\nResolution: worksforme\n\nIssue created by migration from https://trac.sagemath.org/ticket/12359\n\n",
    "closed_at": "2018-08-14T17:27:21Z",
    "created_at": "2012-01-25T20:37:34Z",
    "labels": [
        "component: linear algebra",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "probabilistic hermite_form recurses instead of loops",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/12359",
    "user": "https://github.com/vbraun"
}
```
The default hnf for integral matrices uses a heuristic choice of pivots and loops until it found working pivots. But a subroutine in the loop can call hnf again, which soon leads to `RuntimeError: maximum recursion depth exceeded`. The actual loop is

```
  File "/home/vbraun/opt/sage-5.0.beta1/local/lib/python2.7/site-packages/sage/matrix/matrix_integer_dense_hnf.py", line 1006, in hnf
    H, pivots = probable_hnf(A, include_zero_rows = include_zero_rows, proof=True)
  File "/home/vbraun/opt/sage-5.0.beta1/local/lib/python2.7/site-packages/sage/matrix/matrix_integer_dense_hnf.py", line 838, in probable_hnf
    H = hnf_square(C, proof=proof)
  File "/home/vbraun/opt/sage-5.0.beta1/local/lib/python2.7/site-packages/sage/matrix/matrix_integer_dense_hnf.py", line 574, in hnf_square
    x = add_column(W, H, b.stack(matrix(1,1,[k*A[m-2,m-1] + l*A[m-1,m-1]])), proof)
  File "/home/vbraun/opt/sage-5.0.beta1/local/lib/python2.7/site-packages/sage/matrix/matrix_integer_dense_hnf.py", line 413, in add_column
    return add_column_fallback(B, a, proof)
  File "/home/vbraun/opt/sage-5.0.beta1/local/lib/python2.7/site-packages/sage/matrix/matrix_integer_dense_hnf.py", line 292, in add_column_fallback
    H, _ = hnf(W, proof)
  File "/home/vbraun/opt/sage-5.0.beta1/local/lib/python2.7/site-packages/sage/matrix/matrix_integer_dense_hnf.py", line 1006, in hnf
    H, pivots = probable_hnf(A, include_zero_rows = include_zero_rows, proof=True)
```
Attached is a Sage script that constructs a particular 391x391 matrix that exhibits this problem. Computing the hnf with `algorithm='pari'` gives the result in seconds, but by default the echelon form crashes with the above `RuntimeError` after a much longer wait.

Assignee: jason, was

CC:  @ClementPernet @williamstein @rbeezer @burcin

Resolution: worksforme

Issue created by migration from https://trac.sagemath.org/ticket/12359





---

archive/issue_comments_174733.json:
```json
{
    "body": "Example matrix exhibiting the problem",
    "created_at": "2012-01-25T20:38:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12359#issuecomment-174733",
    "user": "https://github.com/vbraun"
}
```

Example matrix exhibiting the problem



---

archive/issue_comments_174734.json:
```json
{
    "body": "<a id='comment:1'></a>Attachment [big_matrix.sage](tarball://root/attachments/some-uuid/ticket12359/big_matrix.sage) by @vbraun created at 2012-01-25 20:39:17",
    "created_at": "2012-01-25T20:39:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12359#issuecomment-174734",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:1'></a>Attachment [big_matrix.sage](tarball://root/attachments/some-uuid/ticket12359/big_matrix.sage) by @vbraun created at 2012-01-25 20:39:17



---

archive/issue_events_033106.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/12359",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12359#event-33106"
}
```



---

archive/issue_events_033107.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/12359",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12359#event-33107"
}
```



---

archive/issue_events_033108.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/12359",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12359#event-33108"
}
```



---

archive/issue_events_033109.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/12359",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12359#event-33109"
}
```



---

archive/issue_events_033110.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/12359",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12359#event-33110"
}
```



---

archive/issue_events_033111.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/12359",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12359#event-33111"
}
```



---

archive/issue_events_033112.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/12359",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12359#event-33112"
}
```



---

archive/issue_events_033113.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/kartikv",
    "created_at": "2015-08-24T21:12:18Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/12359",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12359#event-33113"
}
```



---

archive/issue_events_033114.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/kartikv",
    "created_at": "2015-08-24T21:12:18Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/12359",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12359#event-33114"
}
```



---

archive/issue_comments_174735.json:
```json
{
    "body": "<a id='comment:7'></a>This ticket should probably closed, as the pathway through the code taken by this example (and similar others) no longer occurs: instead, this matrix simply goes back into pari, and then hangs due to pari's choice of algorithm. Correct functionality should be mostly included in the fix to ticket #19081",
    "created_at": "2015-08-24T21:12:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12359#issuecomment-174735",
    "user": "https://trac.sagemath.org/admin/accounts/users/kartikv"
}
```

<a id='comment:7'></a>This ticket should probably closed, as the pathway through the code taken by this example (and similar others) no longer occurs: instead, this matrix simply goes back into pari, and then hangs due to pari's choice of algorithm. Correct functionality should be mostly included in the fix to ticket #19081



---

archive/issue_comments_174736.json:
```json
{
    "body": "Changing status from new to needs_info.",
    "created_at": "2015-08-24T21:12:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12359#issuecomment-174736",
    "user": "https://trac.sagemath.org/admin/accounts/users/kartikv"
}
```

Changing status from new to needs_info.



---

archive/issue_events_033115.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-08-14T17:27:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/12359",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/12359#event-33115"
}
```



---

archive/issue_comments_174737.json:
```json
{
    "body": "Resolution: worksforme",
    "created_at": "2018-08-14T17:27:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12359#issuecomment-174737",
    "user": "https://github.com/embray"
}
```

Resolution: worksforme



---

archive/issue_comments_174738.json:
```json
{
    "body": "<a id='comment:8'></a>Confirmed worksforme. In fact I'm pretty sure what caused the original bug and it's been fixed in Python as well.",
    "created_at": "2018-08-14T17:27:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12359#issuecomment-174738",
    "user": "https://github.com/embray"
}
```

<a id='comment:8'></a>Confirmed worksforme. In fact I'm pretty sure what caused the original bug and it's been fixed in Python as well.
