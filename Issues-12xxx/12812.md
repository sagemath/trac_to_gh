# Issue 12812: Bug in summation of toric divisors

archive/issues_012640.json:
```json
{
    "assignees": [],
    "body": "Here is the problem:\n\n```\nsage: P1 = toric_varieties.P1()\nsage: print sum([P1.divisor(1)])\nV(s) + V(t)\n```\nWhich I think comes from\n\n```\nsage: print P1.divisor(1).parent()(0)\nV(s)\n```\nPerhaps, we can allow creation of divisors by indices of rays/variables, but only in the form `P1.divisor(0)`? Since zero in the group of divisors should be just zero and other integers don't really make sense.\n\nAssignee: **@aghitza**\n\nCC:  @vbraun\n\nKeywords: **toric**\n\nReviewer: **Volker Braun**\n\nAuthor: **Andrey Novoseltsev**\n\nMerged: **sage-5.0.rc0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/12812_\n\n",
    "closed_at": "2012-04-26T21:57:49Z",
    "created_at": "2012-04-05T17:37:50Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20geometry",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Bug in summation of toric divisors",
    "type": "issue",
    "updated_at": "2012-04-29T14:12:58Z",
    "url": "https://github.com/sagemath/sage/issues/12812",
    "user": "https://github.com/novoselt"
}
```
Here is the problem:

```
sage: P1 = toric_varieties.P1()
sage: print sum([P1.divisor(1)])
V(s) + V(t)
```
Which I think comes from

```
sage: print P1.divisor(1).parent()(0)
V(s)
```
Perhaps, we can allow creation of divisors by indices of rays/variables, but only in the form `P1.divisor(0)`? Since zero in the group of divisors should be just zero and other integers don't really make sense.

Assignee: **@aghitza**

CC:  @vbraun

Keywords: **toric**

Reviewer: **Volker Braun**

Author: **Andrey Novoseltsev**

Merged: **sage-5.0.rc0**

_Issue created by migration from https://trac.sagemath.org/ticket/12812_





---

archive/issue_events_161690.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2012-04-05T17:37:50Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12812",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12812#event-161690"
}
```



---

archive/issue_events_161691.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2012-04-05T17:37:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12812",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20geometry",
    "label_color": "0000ff",
    "label_name": "component: algebraic geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12812#event-161691"
}
```



---

archive/issue_events_161692.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2012-04-05T17:37:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12812",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12812#event-161692"
}
```



---

archive/issue_events_161693.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2012-04-05T17:37:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12812",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12812#event-161693"
}
```



---

archive/issue_comments_146246.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nHi Volker,\n\nI looked through the code and a possible solution is to catch integers in `_element_constructor_` of `ToricDivisorGroup` instead of passing them down to `ToricDivisor`. \"Catch\" means replace 0 with `None` to construct the trivial divisor and raise an exception for anything else.\n\nIt is not very elegant, but keeps current functionality and `X.divisor(i)` returning the divisor of the i-th ray still seems quite reasonable to me, while `X.divisor_group()(i)` is questionable.\n\nIf you agree, I'll make the change and try to make it clear in the documentation of all the methods involved.",
    "created_at": "2012-04-06T17:46:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12812#issuecomment-146246",
    "user": "https://github.com/novoselt"
}
```

<a id='comment:1'>Comment 1:</a>
Hi Volker,

I looked through the code and a possible solution is to catch integers in `_element_constructor_` of `ToricDivisorGroup` instead of passing them down to `ToricDivisor`. "Catch" means replace 0 with `None` to construct the trivial divisor and raise an exception for anything else.

It is not very elegant, but keeps current functionality and `X.divisor(i)` returning the divisor of the i-th ray still seems quite reasonable to me, while `X.divisor_group()(i)` is questionable.

If you agree, I'll make the change and try to make it clear in the documentation of all the methods involved.



---

archive/issue_comments_146247.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nThe whole `ToricDivisor(X, int)` construction is a bit wonky. Thinking about it, the fact that  `ToricDivisor(X,0)` is not the trivial element in this Abelian group is just confusing. I'd prefer to get rid of integer arguments in it altogether, there are already enough ways to construct a toric divisor. And you can always replace it with `ToricDivisorGroup(X).gen(int)`, so its not like its a big loss. I'd like to keep the convenience method `X.divisor(i)`, mind you. What do you think?",
    "created_at": "2012-04-06T19:15:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12812#issuecomment-146247",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:2'>Comment 2:</a>
The whole `ToricDivisor(X, int)` construction is a bit wonky. Thinking about it, the fact that  `ToricDivisor(X,0)` is not the trivial element in this Abelian group is just confusing. I'd prefer to get rid of integer arguments in it altogether, there are already enough ways to construct a toric divisor. And you can always replace it with `ToricDivisorGroup(X).gen(int)`, so its not like its a big loss. I'd like to keep the convenience method `X.divisor(i)`, mind you. What do you think?



---

archive/issue_comments_146248.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nSounds good! Will post a patch over the weekend.",
    "created_at": "2012-04-06T20:11:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12812#issuecomment-146248",
    "user": "https://github.com/novoselt"
}
```

<a id='comment:3'>Comment 3:</a>
Sounds good! Will post a patch over the weekend.



---

archive/issue_comments_146249.json:
```json
{
    "body": "Attachment: **[trac_12812_toric_divisor_summation_bug.patch.gz](https://github.com/sagemath/sage/files/ticket12812/trac_12812_toric_divisor_summation_bug.patch.gz)**",
    "created_at": "2012-04-07T14:18:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12812#issuecomment-146249",
    "user": "https://github.com/novoselt"
}
```

Attachment: **[trac_12812_toric_divisor_summation_bug.patch.gz](https://github.com/sagemath/sage/files/ticket12812/trac_12812_toric_divisor_summation_bug.patch.gz)**



---

archive/issue_events_161694.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2012-04-07T14:20:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12812",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12812#event-161694"
}
```



---

archive/issue_comments_146250.json:
```json
{
    "body": "Author: **Andrey Novoseltsev**",
    "created_at": "2012-04-07T14:20:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12812#issuecomment-146250",
    "user": "https://github.com/novoselt"
}
```

Author: **Andrey Novoseltsev**



---

archive/issue_comments_146251.json:
```json
{
    "body": "Changed keywords from **** to **toric**",
    "created_at": "2012-04-07T14:20:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12812#issuecomment-146251",
    "user": "https://github.com/novoselt"
}
```

Changed keywords from **** to **toric**



---

archive/issue_comments_146252.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nLooks good!",
    "created_at": "2012-04-19T14:40:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12812#issuecomment-146252",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:5'>Comment 5:</a>
Looks good!



---

archive/issue_events_161695.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-04-19T14:40:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12812",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12812#event-161695"
}
```



---

archive/issue_events_161696.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-04-19T14:40:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/12812",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12812#event-161696"
}
```



---

archive/issue_comments_146253.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2012-04-19T14:40:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12812#issuecomment-146253",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_events_161697.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-04-22T19:51:39Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12812",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12812#event-161697"
}
```



---

archive/issue_events_161698.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-04-22T19:51:39Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12812",
    "milestone_number": null,
    "milestone_title": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12812#event-161698"
}
```



---

archive/issue_comments_146254.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nWhy can't this go into 5.0? It is a bugfix, not an enhancement, and it is small and non-invasive.",
    "created_at": "2012-04-23T22:14:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12812#issuecomment-146254",
    "user": "https://github.com/novoselt"
}
```

<a id='comment:7'>Comment 7:</a>
Why can't this go into 5.0? It is a bugfix, not an enhancement, and it is small and non-invasive.



---

archive/issue_events_161699.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-04-24T09:03:19Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/12812",
    "milestone_number": null,
    "milestone_title": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12812#event-161699"
}
```



---

archive/issue_events_161700.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-04-24T09:03:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/12812",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12812#event-161700"
}
```



---

archive/issue_events_161701.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-04-26T21:57:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/12812",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12812#event-161701"
}
```



---

archive/issue_events_161702.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-04-26T21:57:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/12812",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/12812#event-161702"
}
```



---

archive/issue_comments_146255.json:
```json
{
    "body": "Merged: **sage-5.0.beta15**",
    "created_at": "2012-04-26T21:57:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12812#issuecomment-146255",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.0.beta15**



---

archive/issue_comments_146256.json:
```json
{
    "body": "Changed merged from **sage-5.0.beta15** to **sage-5.0.rc0**",
    "created_at": "2012-04-29T14:12:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12812#issuecomment-146256",
    "user": "https://github.com/jdemeyer"
}
```

Changed merged from **sage-5.0.beta15** to **sage-5.0.rc0**



---

archive/issue_comments_146257.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nsage-5.0.beta15 == sage-5.0.rc0",
    "created_at": "2012-04-29T14:12:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/12812",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/12812#issuecomment-146257",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:10'>Comment 10:</a>
sage-5.0.beta15 == sage-5.0.rc0
