# Issue 7393: Update ECL to the latest version

archive/issues_007393.json:
```json
{
    "body": "Assignee: tbd\n\nCC:  dkirkby\n\nThe new spkg can be found at http://sage.math.washington.edu/home/mhansen/ecl-9.10.2-20091105cvs.p0.spkg\n\nThis should fix #7113.\n\nIssue created by migration from https://trac.sagemath.org/ticket/7393\n\n",
    "created_at": "2009-11-05T03:28:52Z",
    "labels": [
        "component: packages: standard",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.2.1",
    "title": "Update ECL to the latest version",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/7393",
    "user": "https://github.com/mwhansen"
}
```
Assignee: tbd

CC:  dkirkby

The new spkg can be found at http://sage.math.washington.edu/home/mhansen/ecl-9.10.2-20091105cvs.p0.spkg

This should fix #7113.

Issue created by migration from https://trac.sagemath.org/ticket/7393





---

archive/issue_comments_062061.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2009-11-05T03:29:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7393#issuecomment-62061",
    "user": "https://github.com/mwhansen"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_062062.json:
```json
{
    "body": "This *does* fix 7113.   Also, what you actually did looks good (in the spkg).  So positive review.",
    "created_at": "2009-11-06T05:49:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7393#issuecomment-62062",
    "user": "https://github.com/williamstein"
}
```

This *does* fix 7113.   Also, what you actually did looks good (in the spkg).  So positive review.



---

archive/issue_comments_062063.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2009-11-06T05:49:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7393#issuecomment-62063",
    "user": "https://github.com/williamstein"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_062064.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-11-06T07:21:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7393",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7393#issuecomment-62064",
    "user": "https://github.com/mwhansen"
}
```

Resolution: fixed



---

archive/issue_events_007616.json:
```json
{
    "actor": "@mwhansen",
    "created_at": "2009-11-06T07:21:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/7393",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7393#event-7616"
}
```
