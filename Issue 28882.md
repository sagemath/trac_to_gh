...
Original creation time: 2020-01-31 13:13:55

CC:  dimpase mkoeppe jdemeyer mjo
...
Comment by embray created at 2020-01-31 13:40:25
...
Comment by dimpase created at 2020-01-31 14:24:59
...
Comment by mkoeppe created at 2020-01-31 15:32:17
...
Comment by mjo created at 2020-02-01 00:38:07
...
Comment by embray created at 2020-02-05 10:40:01
...
Comment by embray created at 2020-02-05 10:55:04

Replying to [comment:5 dimpase]:
> Many distros don't use `./configure` to build Sage, so for them it would be quite a change.

That's the sage-the-distribution ./configure.  Of course they don't use it, because that's for building an entire software distribution which is obviously moot when packaging sage for _their_ distribution.  This is about adding a `./configure` script just for sagelib itself, under `src/`.  It would do much of the same dependency detection, but would be entirely independent of sage-the-distribution (references to "$SAGE_LOCAL", if there even are any, would just be an alias for `$prefix` in this case).


---

Comment by embray created at 2020-02-05 10:56:07
...
Comment by embray created at 2020-02-05 11:05:55

Replying to [comment:6 mkoeppe]:
>   However, I don't think it is actionable at the moment because sagelib is not modular enough and depends on too many packages. Existing efforts such as using `Feature`s for sagelib configuration / modularization seem to have stalled. (by the way - #25828 needs help).

Yeah, it's still a bit of a mess, but actually not _too_ bad.  This would only be for detecting dependencies needed to build and link sagelib's extension modules.  
...
Comment by embray created at 2020-02-05 11:14:50
...
Yes, this is the right idea in general.  In the case of `graph_generators.py` though I would _not_ do this.  It would be much simpler and more manageable if all the dependencies detected by `configure` were output to a single file, such as `@`mkoeppe's `sage/env_config.py.in` from #29022, and individual sub-modules like `graph_generators.py` can just get what they need from `sage.env`.
...
And as I mentioned above, if there _was_ a need to create a plain-text file with Sage configuration, that could also be just as easily output by a `configure` script alongside the .py file.
...
The proposed configure for sagelib (which in the current repository layout would be under `src/configure`) would have nothing to do directly with SPKGs.  It would only detect the presence of the dependencies, and not do anything about installing them if they're missing.  The only relation it would have to SPKGs, is that in _generating_ this configure script we can re-use all the checks we already implemented in the `spkg-configure.m4` files.


---

Comment by dimpase created at 2020-02-05 11:28:02
...
Comment by embray created at 2020-02-05 11:31:41
...
Comment by embray created at 2020-02-05 11:37:41
...
Comment by mjo created at 2020-02-05 15:48:00
...
Comment by dimpase created at 2020-02-05 19:41:10
...
Comment by embray created at 2020-02-06 16:41:27
...
Comment by embray created at 2020-02-12 17:36:50
...
Comment by mkoeppe created at 2021-04-05 17:15:07
...
Comment by dimpase created at 2021-04-07 14:34:54
