# Issue 9018: Better latex for quadratic fields

archive/issues_009018.json:
```json
{
    "body": "Assignee: davidloeffler\n\nBy default, things could print as a+b\\sqrt{D}\n\nIssue created by migration from https://trac.sagemath.org/ticket/9018\n\n",
    "created_at": "2010-05-22T16:31:28Z",
    "labels": [
        "number fields",
        "major",
        "bug"
    ],
    "title": "Better latex for quadratic fields",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/9018",
    "user": "robertwb"
}
```
Assignee: davidloeffler

By default, things could print as a+b\sqrt{D}

Issue created by migration from https://trac.sagemath.org/ticket/9018





---

archive/issue_comments_083448.json:
```json
{
    "body": "Attachment",
    "created_at": "2010-05-22T16:31:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9018",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9018#issuecomment-83448",
    "user": "robertwb"
}
```

Attachment



---

archive/issue_comments_083449.json:
```json
{
    "body": "Depends on #9017.",
    "created_at": "2010-05-22T16:32:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9018",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9018#issuecomment-83449",
    "user": "robertwb"
}
```

Depends on #9017.



---

archive/issue_comments_083450.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2010-05-22T16:32:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9018",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9018#issuecomment-83450",
    "user": "robertwb"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_083451.json:
```json
{
    "body": "Attachment\n\nTo apply after 9018-quadratic-latex.patch",
    "created_at": "2010-05-23T07:39:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9018",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9018#issuecomment-83451",
    "user": "fwclarke"
}
```

Attachment

To apply after 9018-quadratic-latex.patch



---

archive/issue_comments_083452.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2010-05-23T07:41:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9018",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9018#issuecomment-83452",
    "user": "fwclarke"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_083453.json:
```json
{
    "body": "This is all fine, apart from two trivial things: a revised doctest, and a style change to a docstring.  Both are included in the reviewer patch.",
    "created_at": "2010-05-23T07:41:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9018",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9018#issuecomment-83453",
    "user": "fwclarke"
}
```

This is all fine, apart from two trivial things: a revised doctest, and a style change to a docstring.  Both are included in the reviewer patch.



---

archive/issue_comments_083454.json:
```json
{
    "body": "Changing type from defect to enhancement.",
    "created_at": "2010-06-05T09:17:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9018",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9018#issuecomment-83454",
    "user": "robertwb"
}
```

Changing type from defect to enhancement.



---

archive/issue_comments_083455.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2010-06-06T07:25:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9018",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9018#issuecomment-83455",
    "user": "mhansen"
}
```

Resolution: fixed
