# Issue 1695: on osx 10.4 missing libintl.3.dylib in 2.9.2 dmg

archive/issues_001695.json:
```json
{
    "body": "Assignee: boothby\n\nThe sage-2.9.2 in the osx 10.4 dmg failed to work because of missing libintl.\nThis failure occurred when starting the notebook.\nPutting  http://sagemath.org/SAGEbin/apple_osx/intel/10.4-extra_files/libintl.3.dylib\nin local/lib fixed it\n\n\n\n\nIssue created by migration from https://trac.sagemath.org/ticket/1695\n\n",
    "created_at": "2008-01-05T23:21:03Z",
    "labels": [
        "notebook",
        "major",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "on osx 10.4 missing libintl.3.dylib in 2.9.2 dmg",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/1695",
    "user": "jkantor"
}
```
Assignee: boothby

The sage-2.9.2 in the osx 10.4 dmg failed to work because of missing libintl.
This failure occurred when starting the notebook.
Putting  http://sagemath.org/SAGEbin/apple_osx/intel/10.4-extra_files/libintl.3.dylib
in local/lib fixed it




Issue created by migration from https://trac.sagemath.org/ticket/1695





---

archive/issue_comments_010760.json:
```json
{
    "body": "Changing component from notebook to distribution.",
    "created_at": "2008-03-16T19:07:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1695",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1695#issuecomment-10760",
    "user": "boothby"
}
```

Changing component from notebook to distribution.



---

archive/issue_comments_010761.json:
```json
{
    "body": "Changing assignee from boothby to mabshoff.",
    "created_at": "2008-03-16T19:07:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1695",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1695#issuecomment-10761",
    "user": "boothby"
}
```

Changing assignee from boothby to mabshoff.



---

archive/issue_comments_010762.json:
```json
{
    "body": "Closed because I don't build sage on the machine that led to this problem anymore.  In fact that machine physically no longer exists.",
    "created_at": "2008-03-16T20:33:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1695",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1695#issuecomment-10762",
    "user": "was"
}
```

Closed because I don't build sage on the machine that led to this problem anymore.  In fact that machine physically no longer exists.



---

archive/issue_comments_010763.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2008-03-16T20:33:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1695",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1695#issuecomment-10763",
    "user": "was"
}
```

Resolution: invalid
