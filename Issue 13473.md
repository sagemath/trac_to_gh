# Issue 13473: Improve __repr__ and __str__ methods of WordMorphism

archive/issues_013473.json:
```json
{
    "body": "Assignee: slabbe\n\nCC:  tjolivet vdelecroix\n\nKeywords: wordmorphism\n\nThe `__repr__` method is not fun:\n\n\n```\nsage: WordMorphism('a->ab,b->ba')\nMorphism from Words over Ordered Alphabet ['a', 'b'] to Words over Ordered Alphabet ['a', 'b']\nsage: WordMorphism({0:[0,1],1:[1,0]})  \nMorphism from Words over Ordered Alphabet [0, 1] to Words over Ordered Alphabet [0, 1]\n```\n\n\nOne always have to use the print statement (which calls `__str__`) ::\n\n\n```\nsage: print WordMorphism('a->ab,b->ba')\nWordMorphism: a->ab, b->ba\nsage: print WordMorphism({0:[0,1],1:[1,0]})\nWordMorphism: 0->01, 1->10\n```\n\n\nAlso, this is ugly:\n\n\n```\nsage: sigma = WordMorphism({1:[1,2], 2:[1,3], 3:[1]})\nsage: E = E1Star(sigma)                              \nsage: E                                              \nE_1^*(WordMorphism: 1->12, 2->13, 3->1)\n```\n\n\nThe proposed solution is to change the code of `__repr__` so that it behaves like the actual `__str__` and change the `__str__` to remove the `WordMorphism: ` part. See examples below :\n\nThe new `__repr__` method :\n\n\n```\nsage: WordMorphism('a->ab,b->ba')\nWordMorphism: a->ab, b->ba\nsage: WordMorphism({0:[0,1],1:[1,0]})\nWordMorphism: 0->01, 1->10\n```\n\n\nThe new `__str__` method :\n\n\n```\nsage: print WordMorphism('a->ab,b->ba')\na->ab, b->ba\n```\n\n\n\n```\nsage: sigma = WordMorphism({1:[1,2], 2:[1,3], 3:[1]})\nsage: E = E1Star(sigma)                              \nsage: E  \nE_1^*(1->12, 2->13, 3->1)\n```\n\n\n\n\n\nIssue created by migration from https://trac.sagemath.org/ticket/13677\n\n",
    "created_at": "2012-10-31T16:03:34Z",
    "labels": [
        "combinatorics",
        "major",
        "enhancement"
    ],
    "title": "Improve __repr__ and __str__ methods of WordMorphism",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/13473",
    "user": "slabbe"
}
```
Assignee: slabbe

CC:  tjolivet vdelecroix

Keywords: wordmorphism

The `__repr__` method is not fun:


```
sage: WordMorphism('a->ab,b->ba')
Morphism from Words over Ordered Alphabet ['a', 'b'] to Words over Ordered Alphabet ['a', 'b']
sage: WordMorphism({0:[0,1],1:[1,0]})  
Morphism from Words over Ordered Alphabet [0, 1] to Words over Ordered Alphabet [0, 1]
```


One always have to use the print statement (which calls `__str__`) ::


```
sage: print WordMorphism('a->ab,b->ba')
WordMorphism: a->ab, b->ba
sage: print WordMorphism({0:[0,1],1:[1,0]})
WordMorphism: 0->01, 1->10
```


Also, this is ugly:


```
sage: sigma = WordMorphism({1:[1,2], 2:[1,3], 3:[1]})
sage: E = E1Star(sigma)                              
sage: E                                              
E_1^*(WordMorphism: 1->12, 2->13, 3->1)
```


The proposed solution is to change the code of `__repr__` so that it behaves like the actual `__str__` and change the `__str__` to remove the `WordMorphism: ` part. See examples below :

The new `__repr__` method :


```
sage: WordMorphism('a->ab,b->ba')
WordMorphism: a->ab, b->ba
sage: WordMorphism({0:[0,1],1:[1,0]})
WordMorphism: 0->01, 1->10
```


The new `__str__` method :


```
sage: print WordMorphism('a->ab,b->ba')
a->ab, b->ba
```



```
sage: sigma = WordMorphism({1:[1,2], 2:[1,3], 3:[1]})
sage: E = E1Star(sigma)                              
sage: E  
E_1^*(1->12, 2->13, 3->1)
```





Issue created by migration from https://trac.sagemath.org/ticket/13677





---

archive/issue_comments_166146.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2012-10-31T16:09:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13473",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13473#issuecomment-166146",
    "user": "slabbe"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_166147.json:
```json
{
    "body": "Attachment [trac_13677-sl.patch](tarball://root/attachments/some-uuid/ticket13677/trac_13677-sl.patch) by slabbe created at 2012-11-01 11:43:41",
    "created_at": "2012-11-01T11:43:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13473",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13473#issuecomment-166147",
    "user": "slabbe"
}
```

Attachment [trac_13677-sl.patch](tarball://root/attachments/some-uuid/ticket13677/trac_13677-sl.patch) by slabbe created at 2012-11-01 11:43:41



---

archive/issue_comments_166148.json:
```json
{
    "body": "Attachment [trac_13677_docfixes_iet-sl.patch](tarball://root/attachments/some-uuid/ticket13677/trac_13677_docfixes_iet-sl.patch) by slabbe created at 2012-11-01 11:48:24\n\nI changed #13676 to remove the dependancies (by not including new `_latex_` method next to the `__repr__` and `__str__` methods).\n\nI separated the patch into 3 pieces : the code changed, docfixes in `sage/combinat/words`, docfixes in `sage/combinat/iet`. It will be easier to review like that and also easier to update if future conflicts occur.\n\nNeeds review!",
    "created_at": "2012-11-01T11:48:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13473",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13473#issuecomment-166148",
    "user": "slabbe"
}
```

Attachment [trac_13677_docfixes_iet-sl.patch](tarball://root/attachments/some-uuid/ticket13677/trac_13677_docfixes_iet-sl.patch) by slabbe created at 2012-11-01 11:48:24

I changed #13676 to remove the dependancies (by not including new `_latex_` method next to the `__repr__` and `__str__` methods).

I separated the patch into 3 pieces : the code changed, docfixes in `sage/combinat/words`, docfixes in `sage/combinat/iet`. It will be easier to review like that and also easier to update if future conflicts occur.

Needs review!



---

archive/issue_comments_166149.json:
```json
{
    "body": "Attachment [trac_13677_docfixes_words-sl.patch](tarball://root/attachments/some-uuid/ticket13677/trac_13677_docfixes_words-sl.patch) by tjolivet created at 2012-11-12 07:56:53",
    "created_at": "2012-11-12T07:56:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13473",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13473#issuecomment-166149",
    "user": "tjolivet"
}
```

Attachment [trac_13677_docfixes_words-sl.patch](tarball://root/attachments/some-uuid/ticket13677/trac_13677_docfixes_words-sl.patch) by tjolivet created at 2012-11-12 07:56:53



---

archive/issue_comments_166150.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2012-11-12T07:56:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13473",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13473#issuecomment-166150",
    "user": "tjolivet"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_166151.json:
```json
{
    "body": "Please clarify which patches need to be applied.",
    "created_at": "2012-11-15T08:45:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13473",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13473#issuecomment-166151",
    "user": "jdemeyer"
}
```

Please clarify which patches need to be applied.



---

archive/issue_comments_166152.json:
```json
{
    "body": "Replying to [comment:7 jdemeyer]:\n> Please clarify which patches need to be applied.\n\nAll of them, in any order.",
    "created_at": "2012-11-15T12:32:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13473",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13473#issuecomment-166152",
    "user": "slabbe"
}
```

Replying to [comment:7 jdemeyer]:
> Please clarify which patches need to be applied.

All of them, in any order.



---

archive/issue_comments_166153.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2012-11-17T08:56:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13473",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13473#issuecomment-166153",
    "user": "jdemeyer"
}
```

Resolution: fixed
