# Issue 5993: rewrite numerical_approx (the n function) to use interval arithmetic

archive/issues_005993.json:
```json
{
    "body": "Assignee: @burcin\n\nThis email snippet pretty much describes the issue:\n\n```\n\n\nOn Tue, May 5, 2009 at 11:16 PM, Henryk Trappmann <bo198214@googlemail.com> wrote:\n>\n> Ah now I see, you mean though it displays 1/384 it is internally still\n> the above sum, which is computed when evaluated with n.\n\nTrue.  In Sage right now the internal form of the expression (not the simplified form) is used by the \"n\" command.  This will change when Sage switches from using maxima-based symbolics to Pynac. \n\nBy the way, you can use the real interval field to get a floating point approximation to any symbolic expression to a given number of digits of precision.  Every digit is definitely right except the one right before the question mark:\n\nsage: a = 1/(48*sqrt(1)) - 7/(96*1**(3/2)) + 3/(32*1**(5/2)) - 5/ (128*1**(7/2))\nsage: RealIntervalField(100)(a)             # increase 100 for more digits\n0.002604166666666666666666666667?\n\nI think the n function (numerical_approx) for symbolics should be changed to using interval arithmetic.  This will be a huge improvement.  Notice, e.g., that interval arithmetic very nicely gives the right answer for the infamous sin(10^50):\n\nsage: a = sin(10^50)\nsage: a.n(53)\n-0.480500143493759\nsage: a.n(100)\n0.60974154556722786199645650055\nsage: RealIntervalField(100)(a)\n0.?\nsage: RealIntervalField(1000)(a)\n-0.?\nsage: RealIntervalField(2000)(a)\n-0.?\n\n```\n\n\n\nIssue created by migration from https://trac.sagemath.org/ticket/5993\n\n",
    "created_at": "2009-05-06T06:30:59Z",
    "labels": [
        "component: calculus"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.4",
    "title": "rewrite numerical_approx (the n function) to use interval arithmetic",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/5993",
    "user": "https://github.com/williamstein"
}
```
Assignee: @burcin

This email snippet pretty much describes the issue:

```


On Tue, May 5, 2009 at 11:16 PM, Henryk Trappmann <bo198214@googlemail.com> wrote:
>
> Ah now I see, you mean though it displays 1/384 it is internally still
> the above sum, which is computed when evaluated with n.

True.  In Sage right now the internal form of the expression (not the simplified form) is used by the "n" command.  This will change when Sage switches from using maxima-based symbolics to Pynac. 

By the way, you can use the real interval field to get a floating point approximation to any symbolic expression to a given number of digits of precision.  Every digit is definitely right except the one right before the question mark:

sage: a = 1/(48*sqrt(1)) - 7/(96*1**(3/2)) + 3/(32*1**(5/2)) - 5/ (128*1**(7/2))
sage: RealIntervalField(100)(a)             # increase 100 for more digits
0.002604166666666666666666666667?

I think the n function (numerical_approx) for symbolics should be changed to using interval arithmetic.  This will be a huge improvement.  Notice, e.g., that interval arithmetic very nicely gives the right answer for the infamous sin(10^50):

sage: a = sin(10^50)
sage: a.n(53)
-0.480500143493759
sage: a.n(100)
0.60974154556722786199645650055
sage: RealIntervalField(100)(a)
0.?
sage: RealIntervalField(1000)(a)
-0.?
sage: RealIntervalField(2000)(a)
-0.?

```



Issue created by migration from https://trac.sagemath.org/ticket/5993





---

archive/issue_comments_047537.json:
```json
{
    "body": "Disagree, primarily because it's been this way for quite a while now, and because a lot of users would be *really* confused by the question mark.  We certainly could update the documentation to make it clear how to get much more accurate answers using RIF.",
    "created_at": "2011-03-16T15:54:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5993",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5993#issuecomment-47537",
    "user": "https://github.com/kcrisman"
}
```

Disagree, primarily because it's been this way for quite a while now, and because a lot of users would be *really* confused by the question mark.  We certainly could update the documentation to make it clear how to get much more accurate answers using RIF.



---

archive/issue_comments_047538.json:
```json
{
    "body": "Changing priority from major to minor.",
    "created_at": "2011-03-16T15:54:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5993",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5993#issuecomment-47538",
    "user": "https://github.com/kcrisman"
}
```

Changing priority from major to minor.



---

archive/issue_events_014068.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5993",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5993#event-14068"
}
```



---

archive/issue_events_014069.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5993",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5993#event-14069"
}
```



---

archive/issue_events_014070.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5993",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5993#event-14070"
}
```



---

archive/issue_events_014071.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5993",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5993#event-14071"
}
```



---

archive/issue_events_014072.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5993",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5993#event-14072"
}
```



---

archive/issue_events_014073.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5993",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5993#event-14073"
}
```



---

archive/issue_events_014074.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/5993",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/5993#event-14074"
}
```



---

archive/issue_comments_047539.json:
```json
{
    "body": "Changing assignee from @burcin to mvngu.",
    "created_at": "2015-02-01T15:53:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5993",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5993#issuecomment-47539",
    "user": "https://github.com/rwst"
}
```

Changing assignee from @burcin to mvngu.



---

archive/issue_comments_047540.json:
```json
{
    "body": "Changing component from calculus to documentation.",
    "created_at": "2015-02-01T15:53:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5993",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5993#issuecomment-47540",
    "user": "https://github.com/rwst"
}
```

Changing component from calculus to documentation.
