# Issue 16057: Fix multiplicity of nodes in Kleber tree

archive/issues_015820.json:
```json
{
    "assignees": [],
    "body": "The formula should read:\n\n```\np_i^{(a)} = ( \\alpha_a | \\lambda^{(i)} ) - \\sum_{j > i} (j - i) L_j^{(a)}.\n```\nThe old formula caused the multiplicities to become too large.\n\nAssignee: **sage-combinat**\n\nCC:  sage-combinat @anneschilling\n\nKeywords: **rigged configurations**\n\nBranch/Commit: **[f86beb5](https://github.com/sagemath/sagetrac-mirror/commit/f86beb59566af98e13af4ee6ae032eaa68a82bc7)**\n\nReviewer: **Anne Schilling**\n\nAuthor: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16057_\n\n",
    "closed_at": "2014-04-07T19:09:02Z",
    "created_at": "2014-04-04T07:46:08Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix multiplicity of nodes in Kleber tree",
    "type": "issue",
    "updated_at": "2014-04-07T19:09:02Z",
    "url": "https://github.com/sagemath/sage/issues/16057",
    "user": "https://github.com/tscrim"
}
```
The formula should read:

```
p_i^{(a)} = ( \alpha_a | \lambda^{(i)} ) - \sum_{j > i} (j - i) L_j^{(a)}.
```
The old formula caused the multiplicities to become too large.

Assignee: **sage-combinat**

CC:  sage-combinat @anneschilling

Keywords: **rigged configurations**

Branch/Commit: **[f86beb5](https://github.com/sagemath/sagetrac-mirror/commit/f86beb59566af98e13af4ee6ae032eaa68a82bc7)**

Reviewer: **Anne Schilling**

Author: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/16057_





---

archive/issue_events_211651.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-04-04T07:46:08Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16057#event-211651"
}
```



---

archive/issue_events_211652.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-04-04T07:46:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16057#event-211652"
}
```



---

archive/issue_events_211653.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-04-04T07:46:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16057#event-211653"
}
```



---

archive/issue_events_211654.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-04-04T07:46:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16057#event-211654"
}
```



---

archive/issue_comments_206938.json:
```json
{
    "body": "Branch: **[public/combinat/fix_kleber_tree_multiplicities-16057](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/fix_kleber_tree_multiplicities-16057)**",
    "created_at": "2014-04-04T07:47:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16057#issuecomment-206938",
    "user": "https://github.com/tscrim"
}
```

Branch: **[public/combinat/fix_kleber_tree_multiplicities-16057](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/fix_kleber_tree_multiplicities-16057)**



---

archive/issue_events_211655.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-04-04T07:47:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16057#event-211655"
}
```



---

archive/issue_comments_206939.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/395a6493d10e9534f3908ef478de42a4c8260144\">395a649</a></td><td><code>Fix for the multiplicitys of the Kleber tree nodes.</code></td></tr></table>\n",
    "created_at": "2014-04-04T07:47:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16057#issuecomment-206939",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/395a6493d10e9534f3908ef478de42a4c8260144">395a649</a></td><td><code>Fix for the multiplicitys of the Kleber tree nodes.</code></td></tr></table>




---

archive/issue_comments_206940.json:
```json
{
    "body": "Changed branch from **[public/combinat/fix_kleber_tree_multiplicities-16057](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/fix_kleber_tree_multiplicities-16057)** to **[public/combinat/fix_kleber_tree_multiplicity-16057](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/fix_kleber_tree_multiplicity-16057)**",
    "created_at": "2014-04-04T07:47:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16057#issuecomment-206940",
    "user": "https://github.com/tscrim"
}
```

Changed branch from **[public/combinat/fix_kleber_tree_multiplicities-16057](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/fix_kleber_tree_multiplicities-16057)** to **[public/combinat/fix_kleber_tree_multiplicity-16057](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/fix_kleber_tree_multiplicity-16057)**



---

archive/issue_comments_206941.json:
```json
{
    "body": "Commit: **[395a649](https://github.com/sagemath/sagetrac-mirror/commit/395a6493d10e9534f3908ef478de42a4c8260144)**",
    "created_at": "2014-04-04T07:47:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16057#issuecomment-206941",
    "user": "https://github.com/tscrim"
}
```

Commit: **[395a649](https://github.com/sagemath/sagetrac-mirror/commit/395a6493d10e9534f3908ef478de42a4c8260144)**



---

archive/issue_comments_206942.json:
```json
{
    "body": "Changed commit from **[395a649](https://github.com/sagemath/sagetrac-mirror/commit/395a6493d10e9534f3908ef478de42a4c8260144)** to **[d17f2cd](https://github.com/sagemath/sagetrac-mirror/commit/d17f2cd2a36db59af1a85f118c837dc90944d4e1)**",
    "created_at": "2014-04-04T07:48:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16057#issuecomment-206942",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[395a649](https://github.com/sagemath/sagetrac-mirror/commit/395a6493d10e9534f3908ef478de42a4c8260144)** to **[d17f2cd](https://github.com/sagemath/sagetrac-mirror/commit/d17f2cd2a36db59af1a85f118c837dc90944d4e1)**



---

archive/issue_comments_206943.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d17f2cd2a36db59af1a85f118c837dc90944d4e1\">d17f2cd</a></td><td><code>The max is unnecessary.</code></td></tr></table>\n",
    "created_at": "2014-04-04T07:48:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16057#issuecomment-206943",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:3'>Comment 3:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d17f2cd2a36db59af1a85f118c837dc90944d4e1">d17f2cd</a></td><td><code>The max is unnecessary.</code></td></tr></table>




---

archive/issue_comments_206944.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,6 +1,6 @@\n The formula should read:\n \n ```\n-p_i^{(a)} = ( \\alpha_a | \\lambda^{(i)} ) - \\sum_{j > i} \\max(j - i, 0) L_j^{(a)}.\n+p_i^{(a)} = ( \\alpha_a | \\lambda^{(i)} ) - \\sum_{j > i} (j - i) L_j^{(a)}.\n ```\n The old formula caused the multiplicities to become too large.\n``````\n",
    "created_at": "2014-04-04T07:49:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16057#issuecomment-206944",
    "user": "https://github.com/tscrim"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,6 +1,6 @@
 The formula should read:
 
 ```
-p_i^{(a)} = ( \alpha_a | \lambda^{(i)} ) - \sum_{j > i} \max(j - i, 0) L_j^{(a)}.
+p_i^{(a)} = ( \alpha_a | \lambda^{(i)} ) - \sum_{j > i} (j - i) L_j^{(a)}.
 ```
 The old formula caused the multiplicities to become too large.
``````




---

archive/issue_comments_206945.json:
```json
{
    "body": "Reviewer: **Anne Schilling**",
    "created_at": "2014-04-04T21:42:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16057#issuecomment-206945",
    "user": "https://github.com/anneschilling"
}
```

Reviewer: **Anne Schilling**



---

archive/issue_comments_206946.json:
```json
{
    "body": "Changed keywords from **** to **rigged configurations**",
    "created_at": "2014-04-04T21:42:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16057#issuecomment-206946",
    "user": "https://github.com/anneschilling"
}
```

Changed keywords from **** to **rigged configurations**



---

archive/issue_events_211656.json:
```json
{
    "actor": "https://github.com/anneschilling",
    "created_at": "2014-04-04T21:42:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16057#event-211656"
}
```



---

archive/issue_events_211657.json:
```json
{
    "actor": "https://github.com/anneschilling",
    "created_at": "2014-04-04T21:42:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16057#event-211657"
}
```



---

archive/issue_comments_206947.json:
```json
{
    "body": "Changed commit from **[d17f2cd](https://github.com/sagemath/sagetrac-mirror/commit/d17f2cd2a36db59af1a85f118c837dc90944d4e1)** to **[46ea05b](https://github.com/sagemath/sagetrac-mirror/commit/46ea05bf9b70f66becb1f8e8b9660b9bcfeee261)**",
    "created_at": "2014-04-04T21:57:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16057#issuecomment-206947",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[d17f2cd](https://github.com/sagemath/sagetrac-mirror/commit/d17f2cd2a36db59af1a85f118c837dc90944d4e1)** to **[46ea05b](https://github.com/sagemath/sagetrac-mirror/commit/46ea05bf9b70f66becb1f8e8b9660b9bcfeee261)**



---

archive/issue_events_211658.json:
```json
{
    "actor": "https://github.com/sagetrac-git",
    "created_at": "2014-04-04T21:57:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16057#event-211658"
}
```



---

archive/issue_events_211659.json:
```json
{
    "actor": "https://github.com/sagetrac-git",
    "created_at": "2014-04-04T21:57:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16057#event-211659"
}
```



---

archive/issue_comments_206948.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nBranch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/46ea05bf9b70f66becb1f8e8b9660b9bcfeee261\">46ea05b</a></td><td><code>Minor speedup to Kleber tree.</code></td></tr></table>\n",
    "created_at": "2014-04-04T21:57:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16057#issuecomment-206948",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:6'>Comment 6:</a>
Branch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/46ea05bf9b70f66becb1f8e8b9660b9bcfeee261">46ea05b</a></td><td><code>Minor speedup to Kleber tree.</code></td></tr></table>




---

archive/issue_comments_206949.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nIt's faster to do it this way:\n\n```\nsage: B = [[1,2]]*50\nsage: %timeit for (r,s) in B: r,s\n100000 loops, best of 3: 6.61 \u00b5s per loop\nsage: %timeit for b in B: b[0], b[1]\n10000 loops, best of 3: 22.4 \u00b5s per loop\n```",
    "created_at": "2014-04-04T21:57:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16057#issuecomment-206949",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:7'>Comment 7:</a>
It's faster to do it this way:

```
sage: B = [[1,2]]*50
sage: %timeit for (r,s) in B: r,s
100000 loops, best of 3: 6.61 µs per loop
sage: %timeit for b in B: b[0], b[1]
10000 loops, best of 3: 22.4 µs per loop
```



---

archive/issue_events_211660.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-04-04T21:58:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16057#event-211660"
}
```



---

archive/issue_events_211661.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-04-04T21:58:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16057#event-211661"
}
```



---

archive/issue_comments_206950.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nThanks for doing the review.",
    "created_at": "2014-04-04T21:58:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16057#issuecomment-206950",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:9'>Comment 9:</a>
Thanks for doing the review.



---

archive/issue_events_211662.json:
```json
{
    "actor": "https://github.com/sagetrac-git",
    "created_at": "2014-04-04T21:59:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16057#event-211662"
}
```



---

archive/issue_events_211663.json:
```json
{
    "actor": "https://github.com/sagetrac-git",
    "created_at": "2014-04-04T21:59:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16057#event-211663"
}
```



---

archive/issue_comments_206951.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nBranch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f86beb59566af98e13af4ee6ae032eaa68a82bc7\">f86beb5</a></td><td><code>Removed extraneous comment.</code></td></tr></table>\n",
    "created_at": "2014-04-04T21:59:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16057#issuecomment-206951",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:10'>Comment 10:</a>
Branch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f86beb59566af98e13af4ee6ae032eaa68a82bc7">f86beb5</a></td><td><code>Removed extraneous comment.</code></td></tr></table>




---

archive/issue_comments_206952.json:
```json
{
    "body": "Changed commit from **[46ea05b](https://github.com/sagemath/sagetrac-mirror/commit/46ea05bf9b70f66becb1f8e8b9660b9bcfeee261)** to **[f86beb5](https://github.com/sagemath/sagetrac-mirror/commit/f86beb59566af98e13af4ee6ae032eaa68a82bc7)**",
    "created_at": "2014-04-04T21:59:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16057#issuecomment-206952",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[46ea05b](https://github.com/sagemath/sagetrac-mirror/commit/46ea05bf9b70f66becb1f8e8b9660b9bcfeee261)** to **[f86beb5](https://github.com/sagemath/sagetrac-mirror/commit/f86beb59566af98e13af4ee6ae032eaa68a82bc7)**



---

archive/issue_comments_206953.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nSorry, one other micro change.",
    "created_at": "2014-04-04T21:59:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16057#issuecomment-206953",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:11'>Comment 11:</a>
Sorry, one other micro change.



---

archive/issue_events_211664.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-04-04T21:59:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16057#event-211664"
}
```



---

archive/issue_events_211665.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-04-04T21:59:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16057#event-211665"
}
```



---

archive/issue_comments_206954.json:
```json
{
    "body": "Changed branch from **[public/combinat/fix_kleber_tree_multiplicity-16057](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/fix_kleber_tree_multiplicity-16057)** to **[f86beb5](https://github.com/sagemath/sagetrac-mirror/commit/f86beb59566af98e13af4ee6ae032eaa68a82bc7)**",
    "created_at": "2014-04-07T19:09:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16057#issuecomment-206954",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/combinat/fix_kleber_tree_multiplicity-16057](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/fix_kleber_tree_multiplicity-16057)** to **[f86beb5](https://github.com/sagemath/sagetrac-mirror/commit/f86beb59566af98e13af4ee6ae032eaa68a82bc7)**



---

archive/issue_events_211666.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-04-07T19:09:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16057#event-211666"
}
```



---

archive/issue_events_211667.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "d7c9d52b379394a9eb4ce5ec6dbb2232fa832d41",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-04-07T19:09:02Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16057",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16057#event-211667"
}
```
