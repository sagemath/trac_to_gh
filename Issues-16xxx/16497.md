# Issue 16497: Rounding problem with CBC and gurobi LP solvers

archive/issues_016260.json:
```json
{
    "assignees": [],
    "body": "Running `make ptestlong` with CBC package installed leads to the following doctest errors:\n\n```\nFile \"src/doc/en/thematic_tutorials/linear_programming.rst\", line 442, in doc.en.thematic_tutorials.linear_programming\nFailed example:\n    p.solve()\nExpected:\n    2.0\nGot:\n    1.0\n```\n\n```\nFile \"src/sage/graphs/graph.py\", line 3215, in sage.graphs.graph.Graph.minimum_outdegree_orientation\nFailed example:\n    max(o.out_degree()) == ceil((4*3)/(3+4))\nExpected:\n    True\nGot:\n    False\n```\n\n```\nFile \"src/sage/numerical/optimize.py\", line 777, in sage.numerical.optimize.binpacking\nFailed example:\n    all([ (v in b1 or v in b2 or v in b3) for v in values ])\nExpected:\n    True\nGot:\n    False\n```\n\nThe problem seems to come from `get_variable_value` method in the CBC backend that does not round its result when the variable is binary. Actually, the same problems exists for integer variables in both gurobi and CBC backends (at least), but no doctest detects it.\n\n\nCC:  @nathanncohen\n\nBranch/Commit: **[2239413](https://github.com/sagemath/sagetrac-mirror/commit/223941347e7ae66304c99ca73e9a531f8cc2c976)**\n\nReviewer: **Nathann Cohen**\n\nAuthor: **Thierry Monteil**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16497_\n\n",
    "closed_at": "2014-06-27T15:14:50Z",
    "created_at": "2014-06-18T15:46:40Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Rounding problem with CBC and gurobi LP solvers",
    "type": "issue",
    "updated_at": "2014-06-27T15:14:50Z",
    "url": "https://github.com/sagemath/sage/issues/16497",
    "user": "https://github.com/sagetrac-tmonteil"
}
```
Running `make ptestlong` with CBC package installed leads to the following doctest errors:

```
File "src/doc/en/thematic_tutorials/linear_programming.rst", line 442, in doc.en.thematic_tutorials.linear_programming
Failed example:
    p.solve()
Expected:
    2.0
Got:
    1.0
```

```
File "src/sage/graphs/graph.py", line 3215, in sage.graphs.graph.Graph.minimum_outdegree_orientation
Failed example:
    max(o.out_degree()) == ceil((4*3)/(3+4))
Expected:
    True
Got:
    False
```

```
File "src/sage/numerical/optimize.py", line 777, in sage.numerical.optimize.binpacking
Failed example:
    all([ (v in b1 or v in b2 or v in b3) for v in values ])
Expected:
    True
Got:
    False
```

The problem seems to come from `get_variable_value` method in the CBC backend that does not round its result when the variable is binary. Actually, the same problems exists for integer variables in both gurobi and CBC backends (at least), but no doctest detects it.


CC:  @nathanncohen

Branch/Commit: **[2239413](https://github.com/sagemath/sagetrac-mirror/commit/223941347e7ae66304c99ca73e9a531f8cc2c976)**

Reviewer: **Nathann Cohen**

Author: **Thierry Monteil**

_Issue created by migration from https://trac.sagemath.org/ticket/16497_





---

archive/issue_events_218029.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2014-06-18T15:46:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16497#event-218029"
}
```



---

archive/issue_events_218030.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2014-06-18T15:46:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
    "label_color": "000080",
    "label_name": "component: packages: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16497#event-218030"
}
```



---

archive/issue_events_218031.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2014-06-18T15:46:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16497#event-218031"
}
```



---

archive/issue_events_218032.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2014-06-18T15:46:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16497#event-218032"
}
```



---

archive/issue_comments_216198.json:
```json
{
    "body": "Branch: **[u/tmonteil/rounding_problem_with_cbc](https://github.com/sagemath/sagetrac-mirror/tree/u/tmonteil/rounding_problem_with_cbc)**",
    "created_at": "2014-06-24T22:22:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16497#issuecomment-216198",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

Branch: **[u/tmonteil/rounding_problem_with_cbc](https://github.com/sagemath/sagetrac-mirror/tree/u/tmonteil/rounding_problem_with_cbc)**



---

archive/issue_comments_216199.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7c2fb38a2ae24e8c59c5fce8dc0c42967a6ef7c2\">7c2fb38</a></td><td><code>16497 : fix the integer case for gurobi and cplex</code></td></tr></table>\n",
    "created_at": "2014-06-25T15:48:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16497#issuecomment-216199",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:2'>Comment 2:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7c2fb38a2ae24e8c59c5fce8dc0c42967a6ef7c2">7c2fb38</a></td><td><code>16497 : fix the integer case for gurobi and cplex</code></td></tr></table>




---

archive/issue_comments_216200.json:
```json
{
    "body": "Commit: **[7c2fb38](https://github.com/sagemath/sagetrac-mirror/commit/7c2fb38a2ae24e8c59c5fce8dc0c42967a6ef7c2)**",
    "created_at": "2014-06-25T15:48:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16497#issuecomment-216200",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[7c2fb38](https://github.com/sagemath/sagetrac-mirror/commit/7c2fb38a2ae24e8c59c5fce8dc0c42967a6ef7c2)**



---

archive/issue_events_218033.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2014-06-25T15:52:51Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "title_is": "Rounding problem with CBC and gurobi LP solvers",
    "title_was": "Rounding problem with CBC",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16497#event-218033"
}
```



---

archive/issue_comments_216201.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -30,3 +30,5 @@\n     False\n ```\n \n+The problem seems to come from `get_variable_value` method in the CBC backend that does not round its result when the variable is binary. Actually, the same problems exists for integer variables in both gurobi and CBC backends (at least), but no doctest detects it.\n+\n``````\n",
    "created_at": "2014-06-25T15:52:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16497#issuecomment-216201",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -30,3 +30,5 @@
     False
 ```
 
+The problem seems to come from `get_variable_value` method in the CBC backend that does not round its result when the variable is binary. Actually, the same problems exists for integer variables in both gurobi and CBC backends (at least), but no doctest detects it.
+
``````




---

archive/issue_comments_216202.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/bc1cc7d411fd805879f3c179b6dfaf28c69fe5fc\">bc1cc7d</a></td><td><code>first attempt inspired from gurobi_backend</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7c2fb38a2ae24e8c59c5fce8dc0c42967a6ef7c2\">7c2fb38</a></td><td><code>16497 : fix the integer case for gurobi and cplex</code></td></tr></table>\n",
    "created_at": "2014-06-25T15:52:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16497#issuecomment-216202",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

<a id='comment:3'>Comment 3:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/bc1cc7d411fd805879f3c179b6dfaf28c69fe5fc">bc1cc7d</a></td><td><code>first attempt inspired from gurobi_backend</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7c2fb38a2ae24e8c59c5fce8dc0c42967a6ef7c2">7c2fb38</a></td><td><code>16497 : fix the integer case for gurobi and cplex</code></td></tr></table>




---

archive/issue_events_218034.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-06-25T16:30:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16497#event-218034"
}
```



---

archive/issue_comments_216203.json:
```json
{
    "body": "Reviewer: **Nathann Cohen**",
    "created_at": "2014-06-25T16:30:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16497#issuecomment-216203",
    "user": "https://github.com/nathanncohen"
}
```

Reviewer: **Nathann Cohen**



---

archive/issue_comments_216204.json:
```json
{
    "body": "Author: **Thierry Monteil**",
    "created_at": "2014-06-25T16:30:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16497#issuecomment-216204",
    "user": "https://github.com/nathanncohen"
}
```

Author: **Thierry Monteil**



---

archive/issue_comments_216205.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nThank you for that !\n\nNathann",
    "created_at": "2014-06-25T16:30:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16497#issuecomment-216205",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:4'>Comment 4:</a>
Thank you for that !

Nathann



---

archive/issue_events_218035.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-06-25T16:30:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16497#event-218035"
}
```



---

archive/issue_events_218036.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-06-25T16:30:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16497#event-218036"
}
```



---

archive/issue_comments_216206.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nWell, assuming it was in \"needs_review\" ... ?\n\nNathann",
    "created_at": "2014-06-25T16:30:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16497#issuecomment-216206",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:6'>Comment 6:</a>
Well, assuming it was in "needs_review" ... ?

Nathann



---

archive/issue_comments_216207.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nWell not really, i still need to find a doctest for non-binary integer variables.",
    "created_at": "2014-06-25T16:35:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16497#issuecomment-216207",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

<a id='comment:7'>Comment 7:</a>
Well not really, i still need to find a doctest for non-binary integer variables.



---

archive/issue_events_218037.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2014-06-25T16:43:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16497#event-218037"
}
```



---

archive/issue_events_218038.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2014-06-25T16:43:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16497#event-218038"
}
```



---

archive/issue_comments_216208.json:
```json
{
    "body": "Changed commit from **[7c2fb38](https://github.com/sagemath/sagetrac-mirror/commit/7c2fb38a2ae24e8c59c5fce8dc0c42967a6ef7c2)** to **[2239413](https://github.com/sagemath/sagetrac-mirror/commit/223941347e7ae66304c99ca73e9a531f8cc2c976)**",
    "created_at": "2014-06-26T15:02:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16497#issuecomment-216208",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[7c2fb38](https://github.com/sagemath/sagetrac-mirror/commit/7c2fb38a2ae24e8c59c5fce8dc0c42967a6ef7c2)** to **[2239413](https://github.com/sagemath/sagetrac-mirror/commit/223941347e7ae66304c99ca73e9a531f8cc2c976)**



---

archive/issue_comments_216209.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/223941347e7ae66304c99ca73e9a531f8cc2c976\">2239413</a></td><td><code>16497 : add doctest</code></td></tr></table>\n",
    "created_at": "2014-06-26T15:02:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16497#issuecomment-216209",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:9'>Comment 9:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/223941347e7ae66304c99ca73e9a531f8cc2c976">2239413</a></td><td><code>16497 : add doctest</code></td></tr></table>




---

archive/issue_comments_216210.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nAHahahhh.. Is it in needs_review now ? `:-P`\n\nNathann",
    "created_at": "2014-06-26T15:06:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16497#issuecomment-216210",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:10'>Comment 10:</a>
AHahahhh.. Is it in needs_review now ? `:-P`

Nathann



---

archive/issue_comments_216211.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nI added a doctest, not sure it is a valuable one. At least on my machine with CBC installed, it returns a wrong answser without the fix (both integer and binary cases), and a good answer with the fix.\n\nIt could be nice to have some buildbots running with some optional packages installed.",
    "created_at": "2014-06-26T15:06:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16497#issuecomment-216211",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

<a id='comment:11'>Comment 11:</a>
I added a doctest, not sure it is a valuable one. At least on my machine with CBC installed, it returns a wrong answser without the fix (both integer and binary cases), and a good answer with the fix.

It could be nice to have some buildbots running with some optional packages installed.



---

archive/issue_comments_216212.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nBy the way you added a doctest which checks something that was already checked by doctest, i.e. the ones you reported in the message's description `:-P`\n\nNathann",
    "created_at": "2014-06-26T15:07:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16497#issuecomment-216212",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:12'>Comment 12:</a>
By the way you added a doctest which checks something that was already checked by doctest, i.e. the ones you reported in the message's description `:-P`

Nathann



---

archive/issue_comments_216213.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nWell not completely, all doctests passed if i only fixed the binary case.",
    "created_at": "2014-06-26T15:08:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16497#issuecomment-216213",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

<a id='comment:13'>Comment 13:</a>
Well not completely, all doctests passed if i only fixed the binary case.



---

archive/issue_events_218039.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2014-06-26T15:08:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16497#event-218039"
}
```



---

archive/issue_events_218040.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2014-06-26T15:08:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16497#event-218040"
}
```



---

archive/issue_events_218041.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-06-26T20:46:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16497#event-218041"
}
```



---

archive/issue_events_218042.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-06-26T20:46:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16497#event-218042"
}
```



---

archive/issue_comments_216214.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nYo !\n\nOkayyyyyyyy, good to go `:-)`\n\nNathann",
    "created_at": "2014-06-26T20:46:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16497#issuecomment-216214",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:14'>Comment 14:</a>
Yo !

Okayyyyyyyy, good to go `:-)`

Nathann



---

archive/issue_comments_216215.json:
```json
{
    "body": "Changed branch from **[u/tmonteil/rounding_problem_with_cbc](https://github.com/sagemath/sagetrac-mirror/tree/u/tmonteil/rounding_problem_with_cbc)** to **[2239413](https://github.com/sagemath/sagetrac-mirror/commit/223941347e7ae66304c99ca73e9a531f8cc2c976)**",
    "created_at": "2014-06-27T15:14:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16497#issuecomment-216215",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/tmonteil/rounding_problem_with_cbc](https://github.com/sagemath/sagetrac-mirror/tree/u/tmonteil/rounding_problem_with_cbc)** to **[2239413](https://github.com/sagemath/sagetrac-mirror/commit/223941347e7ae66304c99ca73e9a531f8cc2c976)**



---

archive/issue_events_218043.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-06-27T15:14:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16497#event-218043"
}
```



---

archive/issue_events_218044.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "dcae0bdb7c4e90ff1e79ae8622b32ea5580024eb",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-06-27T15:14:50Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16497",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16497#event-218044"
}
```
