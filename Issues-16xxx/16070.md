# Issue 16070: Python 3 preparation: Some functions (ifilter, imap, izip) are removed from itertools

archive/issues_015833.json:
```json
{
    "assignees": [],
    "body": "The tool 2to3 removes the imports. \n\n\nBut the code has to depend on the Python version!\n\n\nAn alternative approach is to add `from future_builtins import filter` and to check where a wrapping with `list()` is required.\n\nThere are 13 effected modules. \n\nThis ticket is tracked as a dependency of meta-ticket ticket:16052.\n\nKeywords: **python3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16070_\n\n",
    "closed_at": "2017-01-21T18:03:11Z",
    "created_at": "2014-04-07T08:43:09Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20python3",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Python 3 preparation: Some functions (ifilter, imap, izip) are removed from itertools",
    "type": "issue",
    "updated_at": "2017-01-21T18:03:11Z",
    "url": "https://github.com/sagemath/sage/issues/16070",
    "user": "https://github.com/wluebbe"
}
```
The tool 2to3 removes the imports. 


But the code has to depend on the Python version!


An alternative approach is to add `from future_builtins import filter` and to check where a wrapping with `list()` is required.

There are 13 effected modules. 

This ticket is tracked as a dependency of meta-ticket ticket:16052.

Keywords: **python3**

_Issue created by migration from https://trac.sagemath.org/ticket/16070_





---

archive/issue_events_211854.json:
```json
{
    "actor": "https://github.com/wluebbe",
    "created_at": "2014-04-07T08:43:09Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16070",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16070#event-211854"
}
```



---

archive/issue_events_211855.json:
```json
{
    "actor": "https://github.com/wluebbe",
    "created_at": "2014-04-07T08:43:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16070",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20distribution",
    "label_color": "000080",
    "label_name": "component: distribution",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16070#event-211855"
}
```



---

archive/issue_events_211856.json:
```json
{
    "actor": "https://github.com/wluebbe",
    "created_at": "2014-04-07T08:43:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16070",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16070#event-211856"
}
```



---

archive/issue_events_211857.json:
```json
{
    "actor": "https://github.com/wluebbe",
    "created_at": "2014-04-07T08:43:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16070",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16070#event-211857"
}
```



---

archive/issue_events_211858.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16070",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16070#event-211858"
}
```



---

archive/issue_events_211859.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16070",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16070#event-211859"
}
```



---

archive/issue_comments_207150.json:
```json
{
    "body": "Dependencies: **#16071**",
    "created_at": "2014-05-22T14:23:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16070",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16070#issuecomment-207150",
    "user": "https://github.com/wluebbe"
}
```

Dependencies: **#16071**



---

archive/issue_comments_207151.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nAdded dependency: The imports may only be removed when their usages have been removed in #16071.",
    "created_at": "2014-05-22T14:23:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16070",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16070#issuecomment-207151",
    "user": "https://github.com/wluebbe"
}
```

<div id="comment:2" align="right">Comment 2</div>

Added dependency: The imports may only be removed when their usages have been removed in #16071.



---

archive/issue_events_211860.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16070",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16070#event-211860"
}
```



---

archive/issue_events_211861.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16070",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16070#event-211861"
}
```



---

archive/issue_events_211862.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-05-02T13:34:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16070",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20distribution",
    "label_color": "000080",
    "label_name": "component: distribution",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16070#event-211862"
}
```



---

archive/issue_events_211863.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-05-02T13:34:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16070",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20python3",
    "label_color": "000080",
    "label_name": "component: python3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16070#event-211863"
}
```



---

archive/issue_comments_207152.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nfor imap, see #21364",
    "created_at": "2016-08-29T19:08:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16070",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16070#issuecomment-207152",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:5" align="right">Comment 5</div>

for imap, see #21364



---

archive/issue_comments_207153.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nThere is no longer any ifilter to take care of.\n\nFor imap, see tickets #21364 and #21373\n\nand for izip, see #21374",
    "created_at": "2016-08-30T13:38:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16070",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16070#issuecomment-207153",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:6" align="right">Comment 6</div>

There is no longer any ifilter to take care of.

For imap, see tickets #21364 and #21373

and for izip, see #21374



---

archive/issue_events_211864.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-08-30T13:41:32Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16070",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16070#event-211864"
}
```



---

archive/issue_events_211865.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-08-30T13:41:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16070",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16070#event-211865"
}
```



---

archive/issue_events_211866.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-08-30T13:41:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16070",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16070#event-211866"
}
```



---

archive/issue_comments_207154.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nlet us close this as duplicate",
    "created_at": "2016-08-30T13:41:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16070",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16070#issuecomment-207154",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:7" align="right">Comment 7</div>

let us close this as duplicate



---

archive/issue_events_211867.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-08-30T13:41:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16070",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16070#event-211867"
}
```



---

archive/issue_comments_207155.json:
```json
{
    "body": "Changed dependencies from **#16071** to none",
    "created_at": "2016-08-30T13:41:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16070",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16070#issuecomment-207155",
    "user": "https://github.com/fchapoton"
}
```

Changed dependencies from **#16071** to none



---

archive/issue_events_211868.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2016-09-03T21:30:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16070",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16070#event-211868"
}
```



---

archive/issue_events_211869.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2016-09-03T21:30:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16070",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16070#event-211869"
}
```



---

archive/issue_events_211870.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-01-21T18:03:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16070",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16070#event-211870"
}
```



---

archive/issue_events_211871.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-01-21T18:03:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16070",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16070#event-211871"
}
```
