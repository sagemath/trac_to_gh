# Issue 16682: Fix random_element of polynomial ring

archive/issues_016445.json:
```json
{
    "assignees": [],
    "body": "The degree is not the one expected (as there is no check that the dominant coefficient is nonzero):\n\n```\nsage: R = PolynomialRing(GF(2),'z')\nsage: R.random_element(degree=5).degree()\n3\n```\n\nThis was found in #16681\n\nCC:  @pjbruin @defeo @miguelmarco @vbraun\n\nBranch/Commit: **[f0b7bdb](https://github.com/sagemath/sagetrac-mirror/commit/f0b7bdbe70e75bc16f63ae3163315785ea4c777c)**\n\nReviewer: **Volker Braun**\n\nAuthor: **Vincent Delecroix**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16682_\n\n",
    "closed_at": "2014-08-25T10:27:52Z",
    "created_at": "2014-07-19T10:15:27Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20number%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix random_element of polynomial ring",
    "type": "issue",
    "updated_at": "2014-08-25T10:27:52Z",
    "url": "https://github.com/sagemath/sage/issues/16682",
    "user": "https://github.com/videlec"
}
```
The degree is not the one expected (as there is no check that the dominant coefficient is nonzero):

```
sage: R = PolynomialRing(GF(2),'z')
sage: R.random_element(degree=5).degree()
3
```

This was found in #16681

CC:  @pjbruin @defeo @miguelmarco @vbraun

Branch/Commit: **[f0b7bdb](https://github.com/sagemath/sagetrac-mirror/commit/f0b7bdbe70e75bc16f63ae3163315785ea4c777c)**

Reviewer: **Volker Braun**

Author: **Vincent Delecroix**

_Issue created by migration from https://trac.sagemath.org/ticket/16682_





---

archive/issue_events_220919.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2014-07-19T10:15:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16682#event-220919"
}
```



---

archive/issue_events_220920.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2014-07-19T10:15:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20theory",
    "label_color": "0000ff",
    "label_name": "component: number theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16682#event-220920"
}
```



---

archive/issue_events_220921.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2014-07-19T10:15:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16682#event-220921"
}
```



---

archive/issue_events_220922.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2014-07-19T10:15:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16682#event-220922"
}
```



---

archive/issue_comments_220510.json:
```json
{
    "body": "Branch: **[u/vdelecroix/16682](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/16682)**",
    "created_at": "2014-07-19T10:22:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16682#issuecomment-220510",
    "user": "https://github.com/videlec"
}
```

Branch: **[u/vdelecroix/16682](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/16682)**



---

archive/issue_comments_220511.json:
```json
{
    "body": "Commit: **[a2fa480](https://github.com/sagemath/sagetrac-mirror/commit/a2fa4809681cfb0519b4c137d215ec6963f7dac1)**",
    "created_at": "2014-07-19T10:22:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16682#issuecomment-220511",
    "user": "https://github.com/videlec"
}
```

Commit: **[a2fa480](https://github.com/sagemath/sagetrac-mirror/commit/a2fa4809681cfb0519b4c137d215ec6963f7dac1)**



---

archive/issue_events_220923.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2014-07-19T10:22:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16682#event-220923"
}
```



---

archive/issue_comments_220512.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a2fa4809681cfb0519b4c137d215ec6963f7dac1\">a2fa480</a></td><td><code>trac #16682: fix random_element in polynomial ring</code></td></tr></table>\n",
    "created_at": "2014-07-19T10:22:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16682#issuecomment-220512",
    "user": "https://github.com/videlec"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a2fa4809681cfb0519b4c137d215ec6963f7dac1">a2fa480</a></td><td><code>trac #16682: fix random_element in polynomial ring</code></td></tr></table>




---

archive/issue_comments_220513.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nHi Vincent,\n\nwhy are you treating the degree 0 case differently? Since the degree of 0 is -1, wouldn't it be better to guarantee that random_element with degree 0 doesn't return 0?\nThat is, why don't you just remove these two lines:\n\n```\nif degree == 0:\nreturn self(R.random_element(*args, **kwds))\n```",
    "created_at": "2014-07-21T18:03:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16682#issuecomment-220513",
    "user": "https://github.com/sagetrac-jkeitel"
}
```

<div id="comment:2" align="right">Comment 2</div>

Hi Vincent,

why are you treating the degree 0 case differently? Since the degree of 0 is -1, wouldn't it be better to guarantee that random_element with degree 0 doesn't return 0?
That is, why don't you just remove these two lines:

```
if degree == 0:
return self(R.random_element(*args, **kwds))
```



---

archive/issue_events_220924.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-07-25T05:04:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16682#event-220924"
}
```



---

archive/issue_events_220925.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-07-25T05:04:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16682#event-220925"
}
```



---

archive/issue_comments_220514.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nAgreed.",
    "created_at": "2014-07-25T05:04:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16682#issuecomment-220514",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:3" align="right">Comment 3</div>

Agreed.



---

archive/issue_comments_220515.json:
```json
{
    "body": "Changed branch from **[u/vdelecroix/16682](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/16682)** to **[u/jkeitel/16682](https://github.com/sagemath/sagetrac-mirror/tree/u/jkeitel/16682)**",
    "created_at": "2014-07-25T07:38:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16682#issuecomment-220515",
    "user": "https://github.com/sagetrac-jkeitel"
}
```

Changed branch from **[u/vdelecroix/16682](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/16682)** to **[u/jkeitel/16682](https://github.com/sagemath/sagetrac-mirror/tree/u/jkeitel/16682)**



---

archive/issue_events_220926.json:
```json
{
    "actor": "https://github.com/sagetrac-jkeitel",
    "created_at": "2014-07-25T07:38:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16682#event-220926"
}
```



---

archive/issue_events_220927.json:
```json
{
    "actor": "https://github.com/sagetrac-jkeitel",
    "created_at": "2014-07-25T07:38:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16682#event-220927"
}
```



---

archive/issue_comments_220516.json:
```json
{
    "body": "Changed commit from **[a2fa480](https://github.com/sagemath/sagetrac-mirror/commit/a2fa4809681cfb0519b4c137d215ec6963f7dac1)** to **[d427b28](https://github.com/sagemath/sagetrac-mirror/commit/d427b28f5d591c7e11d93a2601a36740965c8f73)**",
    "created_at": "2014-07-25T07:38:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16682#issuecomment-220516",
    "user": "https://github.com/sagetrac-jkeitel"
}
```

Changed commit from **[a2fa480](https://github.com/sagemath/sagetrac-mirror/commit/a2fa4809681cfb0519b4c137d215ec6963f7dac1)** to **[d427b28](https://github.com/sagemath/sagetrac-mirror/commit/d427b28f5d591c7e11d93a2601a36740965c8f73)**



---

archive/issue_comments_220517.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nI simply removed these two lines and extended the doctest. If you're happy with that, you can it to positive review.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d427b28f5d591c7e11d93a2601a36740965c8f73\">d427b28</a></td><td><code>#16682: Make sure that degree=0 random elements also work.</code></td></tr></table>\n",
    "created_at": "2014-07-25T07:38:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16682#issuecomment-220517",
    "user": "https://github.com/sagetrac-jkeitel"
}
```

<div id="comment:4" align="right">Comment 4</div>

I simply removed these two lines and extended the doctest. If you're happy with that, you can it to positive review.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d427b28f5d591c7e11d93a2601a36740965c8f73">d427b28</a></td><td><code>#16682: Make sure that degree=0 random elements also work.</code></td></tr></table>




---

archive/issue_comments_220518.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nHi Jan,\n\nIf you have a suggestion, I don't mind that you do it. And we can discuss it. I do not find acceptable that you come and set your branch as default.\n\nNow, concretely about your commit: `R.random_element(degree=whatever)` will **never** return the zero polynomial. So I would not say that it \"works\".\n\nThe new commit handles the case of `degree=-1` which correspond to the zero polynomial.\n\nVincent\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a2fa4809681cfb0519b4c137d215ec6963f7dac1\">a2fa480</a></td><td><code>trac #16682: fix random_element in polynomial ring</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/36ee521f145b9d17feed59e60221171e25eb3e2b\">36ee521</a></td><td><code>trac #16682: the zero polynomial has degree -1</code></td></tr></table>\n",
    "created_at": "2014-07-25T07:57:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16682#issuecomment-220518",
    "user": "https://github.com/videlec"
}
```

<div id="comment:5" align="right">Comment 5</div>

Hi Jan,

If you have a suggestion, I don't mind that you do it. And we can discuss it. I do not find acceptable that you come and set your branch as default.

Now, concretely about your commit: `R.random_element(degree=whatever)` will **never** return the zero polynomial. So I would not say that it "works".

The new commit handles the case of `degree=-1` which correspond to the zero polynomial.

Vincent

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a2fa4809681cfb0519b4c137d215ec6963f7dac1">a2fa480</a></td><td><code>trac #16682: fix random_element in polynomial ring</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/36ee521f145b9d17feed59e60221171e25eb3e2b">36ee521</a></td><td><code>trac #16682: the zero polynomial has degree -1</code></td></tr></table>




---

archive/issue_comments_220519.json:
```json
{
    "body": "Changed commit from **[d427b28](https://github.com/sagemath/sagetrac-mirror/commit/d427b28f5d591c7e11d93a2601a36740965c8f73)** to **[36ee521](https://github.com/sagemath/sagetrac-mirror/commit/36ee521f145b9d17feed59e60221171e25eb3e2b)**",
    "created_at": "2014-07-25T07:57:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16682#issuecomment-220519",
    "user": "https://github.com/videlec"
}
```

Changed commit from **[d427b28](https://github.com/sagemath/sagetrac-mirror/commit/d427b28f5d591c7e11d93a2601a36740965c8f73)** to **[36ee521](https://github.com/sagemath/sagetrac-mirror/commit/36ee521f145b9d17feed59e60221171e25eb3e2b)**



---

archive/issue_comments_220520.json:
```json
{
    "body": "Changed branch from **[u/jkeitel/16682](https://github.com/sagemath/sagetrac-mirror/tree/u/jkeitel/16682)** to **[u/vdelecroix/16682](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/16682)**",
    "created_at": "2014-07-25T07:57:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16682#issuecomment-220520",
    "user": "https://github.com/videlec"
}
```

Changed branch from **[u/jkeitel/16682](https://github.com/sagemath/sagetrac-mirror/tree/u/jkeitel/16682)** to **[u/vdelecroix/16682](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/16682)**



---

archive/issue_comments_220521.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nHi Vincent,\n\nsorry, I certainly didn't mean to offend you and your current branch is a better solution.\n\nHowever, what about doing one more check that `degree >-1` and raising an error if that's not the case? At the moment we have\n\n```\nsage: R = PolynomialRing(GF(2), 'z')\nsage: R.random_element(degree=-2)\n1\n```",
    "created_at": "2014-07-25T08:08:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16682#issuecomment-220521",
    "user": "https://github.com/sagetrac-jkeitel"
}
```

<div id="comment:6" align="right">Comment 6</div>

Hi Vincent,

sorry, I certainly didn't mean to offend you and your current branch is a better solution.

However, what about doing one more check that `degree >-1` and raising an error if that's not the case? At the moment we have

```
sage: R = PolynomialRing(GF(2), 'z')
sage: R.random_element(degree=-2)
1
```



---

archive/issue_comments_220522.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nVincent, changing the \"Branch\" field on the trac ticket is the recommended way of collaborating. Change it back if you don't like the proposed commits, easy.",
    "created_at": "2014-07-25T14:20:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16682#issuecomment-220522",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:7" align="right">Comment 7</div>

Vincent, changing the "Branch" field on the trac ticket is the recommended way of collaborating. Change it back if you don't like the proposed commits, easy.



---

archive/issue_comments_220523.json:
```json
{
    "body": "Changed commit from **[36ee521](https://github.com/sagemath/sagetrac-mirror/commit/36ee521f145b9d17feed59e60221171e25eb3e2b)** to **[342d1ef](https://github.com/sagemath/sagetrac-mirror/commit/342d1ef879c59781e8b51af0a37c87904c837ac9)**",
    "created_at": "2014-08-07T14:26:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16682#issuecomment-220523",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[36ee521](https://github.com/sagemath/sagetrac-mirror/commit/36ee521f145b9d17feed59e60221171e25eb3e2b)** to **[342d1ef](https://github.com/sagemath/sagetrac-mirror/commit/342d1ef879c59781e8b51af0a37c87904c837ac9)**



---

archive/issue_comments_220524.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/49b9331a39455ac33d52e971b0b99294a0bc8c8f\">49b9331</a></td><td><code>trac #16682: merge 6.3.rc0</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/342d1ef879c59781e8b51af0a37c87904c837ac9\">342d1ef</a></td><td><code>trac #16682: input check</code></td></tr></table>\n",
    "created_at": "2014-08-07T14:26:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16682#issuecomment-220524",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:8" align="right">Comment 8</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/49b9331a39455ac33d52e971b0b99294a0bc8c8f">49b9331</a></td><td><code>trac #16682: merge 6.3.rc0</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/342d1ef879c59781e8b51af0a37c87904c837ac9">342d1ef</a></td><td><code>trac #16682: input check</code></td></tr></table>




---

archive/issue_comments_220525.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nReplying to [@sagetrac-jkeitel](#comment%3A6):\n> Hi Vincent,\n> \n> sorry, I certainly didn't mean to offend you and your current branch is a better solution.\n\nEverybody works differently, so no offense at all.\n\n> However, what about doing one more check that `degree >-1` and raising an error if that's not the case? At the moment we have\n\nI always assume that the user is intelligent enough... done in the last commit.\n\nVincent\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/49b9331a39455ac33d52e971b0b99294a0bc8c8f\">49b9331</a></td><td><code>trac #16682: merge 6.3.rc0</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/342d1ef879c59781e8b51af0a37c87904c837ac9\">342d1ef</a></td><td><code>trac #16682: input check</code></td></tr></table>\n",
    "created_at": "2014-08-07T14:26:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16682#issuecomment-220525",
    "user": "https://github.com/videlec"
}
```

<div id="comment:9" align="right">Comment 9</div>

Replying to [@sagetrac-jkeitel](#comment%3A6):
> Hi Vincent,
> 
> sorry, I certainly didn't mean to offend you and your current branch is a better solution.

Everybody works differently, so no offense at all.

> However, what about doing one more check that `degree >-1` and raising an error if that's not the case? At the moment we have

I always assume that the user is intelligent enough... done in the last commit.

Vincent

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/49b9331a39455ac33d52e971b0b99294a0bc8c8f">49b9331</a></td><td><code>trac #16682: merge 6.3.rc0</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/342d1ef879c59781e8b51af0a37c87904c837ac9">342d1ef</a></td><td><code>trac #16682: input check</code></td></tr></table>




---

archive/issue_comments_220526.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nThere are several doctest that need to be fixed... I am on it.\n\nVincent",
    "created_at": "2014-08-07T14:34:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16682#issuecomment-220526",
    "user": "https://github.com/videlec"
}
```

<div id="comment:10" align="right">Comment 10</div>

There are several doctest that need to be fixed... I am on it.

Vincent



---

archive/issue_events_220928.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16682#event-220928"
}
```



---

archive/issue_events_220929.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16682#event-220929"
}
```



---

archive/issue_comments_220527.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/75324c715e844fc4e9eadae905a1d1706f7d3db8\">75324c7</a></td><td><code>trac #16682: better random element</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3b65591a5f288a6632f52b8242ad73340cd26180\">3b65591</a></td><td><code>trac #16682: fix doctests</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7f409d890fd09de3009d27e3239051ed6aa2f469\">7f409d8</a></td><td><code>trac #16682: merge 6.3</code></td></tr></table>\n",
    "created_at": "2014-08-13T09:22:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16682#issuecomment-220527",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:12" align="right">Comment 12</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/75324c715e844fc4e9eadae905a1d1706f7d3db8">75324c7</a></td><td><code>trac #16682: better random element</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3b65591a5f288a6632f52b8242ad73340cd26180">3b65591</a></td><td><code>trac #16682: fix doctests</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7f409d890fd09de3009d27e3239051ed6aa2f469">7f409d8</a></td><td><code>trac #16682: merge 6.3</code></td></tr></table>




---

archive/issue_comments_220528.json:
```json
{
    "body": "Changed commit from **[342d1ef](https://github.com/sagemath/sagetrac-mirror/commit/342d1ef879c59781e8b51af0a37c87904c837ac9)** to **[7f409d8](https://github.com/sagemath/sagetrac-mirror/commit/7f409d890fd09de3009d27e3239051ed6aa2f469)**",
    "created_at": "2014-08-13T09:22:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16682#issuecomment-220528",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[342d1ef](https://github.com/sagemath/sagetrac-mirror/commit/342d1ef879c59781e8b51af0a37c87904c837ac9)** to **[7f409d8](https://github.com/sagemath/sagetrac-mirror/commit/7f409d890fd09de3009d27e3239051ed6aa2f469)**



---

archive/issue_comments_220529.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nNeeds review.\n\nVincent",
    "created_at": "2014-08-13T09:22:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16682#issuecomment-220529",
    "user": "https://github.com/videlec"
}
```

<div id="comment:13" align="right">Comment 13</div>

Needs review.

Vincent



---

archive/issue_comments_220530.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/589caad6a78988d4bb3aab3c55bd6e0be6d20c29\">589caad</a></td><td><code>trac #16682: input check</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6b9ac1b8d1bf915db89a704fd8c491f6d130e8ad\">6b9ac1b</a></td><td><code>trac #16682: better random element</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e1b61ff878eeb8c894a5cb00c672cbd3e3b4cddd\">e1b61ff</a></td><td><code>trac #16682: fix doctests</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1e0f8d9e9ac0bf228959b68d480b12a93cc911a4\">1e0f8d9</a></td><td><code>trac #16682: merge sage 6.3</code></td></tr></table>\n",
    "created_at": "2014-08-13T09:31:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16682#issuecomment-220530",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:14" align="right">Comment 14</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/589caad6a78988d4bb3aab3c55bd6e0be6d20c29">589caad</a></td><td><code>trac #16682: input check</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6b9ac1b8d1bf915db89a704fd8c491f6d130e8ad">6b9ac1b</a></td><td><code>trac #16682: better random element</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e1b61ff878eeb8c894a5cb00c672cbd3e3b4cddd">e1b61ff</a></td><td><code>trac #16682: fix doctests</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1e0f8d9e9ac0bf228959b68d480b12a93cc911a4">1e0f8d9</a></td><td><code>trac #16682: merge sage 6.3</code></td></tr></table>




---

archive/issue_comments_220531.json:
```json
{
    "body": "Changed commit from **[7f409d8](https://github.com/sagemath/sagetrac-mirror/commit/7f409d890fd09de3009d27e3239051ed6aa2f469)** to **[1e0f8d9](https://github.com/sagemath/sagetrac-mirror/commit/1e0f8d9e9ac0bf228959b68d480b12a93cc911a4)**",
    "created_at": "2014-08-13T09:31:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16682#issuecomment-220531",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[7f409d8](https://github.com/sagemath/sagetrac-mirror/commit/7f409d890fd09de3009d27e3239051ed6aa2f469)** to **[1e0f8d9](https://github.com/sagemath/sagetrac-mirror/commit/1e0f8d9e9ac0bf228959b68d480b12a93cc911a4)**



---

archive/issue_comments_220532.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">Comment 15</div>\n\nCan you raise a ValueError of input is out of bounds (degree <= -2)? Assertions are for spelling out contracts, not validating user input. In particular, assertions can be disabled at compile-time.",
    "created_at": "2014-08-24T15:59:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16682#issuecomment-220532",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:15" align="right">Comment 15</div>

Can you raise a ValueError of input is out of bounds (degree <= -2)? Assertions are for spelling out contracts, not validating user input. In particular, assertions can be disabled at compile-time.



---

archive/issue_comments_220533.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">Comment 16</div>\n\nApart from that, positive review.",
    "created_at": "2014-08-24T15:59:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16682#issuecomment-220533",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:16" align="right">Comment 16</div>

Apart from that, positive review.



---

archive/issue_events_220930.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-08-24T15:59:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16682#event-220930"
}
```



---

archive/issue_events_220931.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-08-24T15:59:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16682#event-220931"
}
```



---

archive/issue_comments_220534.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2014-08-24T15:59:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16682#issuecomment-220534",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_comments_220535.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">Comment 18</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f0b7bdbe70e75bc16f63ae3163315785ea4c777c\">f0b7bdb</a></td><td><code>trac #16682: AssertionError -> ValueError</code></td></tr></table>\n",
    "created_at": "2014-08-24T16:46:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16682#issuecomment-220535",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:18" align="right">Comment 18</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f0b7bdbe70e75bc16f63ae3163315785ea4c777c">f0b7bdb</a></td><td><code>trac #16682: AssertionError -> ValueError</code></td></tr></table>




---

archive/issue_comments_220536.json:
```json
{
    "body": "Changed commit from **[1e0f8d9](https://github.com/sagemath/sagetrac-mirror/commit/1e0f8d9e9ac0bf228959b68d480b12a93cc911a4)** to **[f0b7bdb](https://github.com/sagemath/sagetrac-mirror/commit/f0b7bdbe70e75bc16f63ae3163315785ea4c777c)**",
    "created_at": "2014-08-24T16:46:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16682#issuecomment-220536",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[1e0f8d9](https://github.com/sagemath/sagetrac-mirror/commit/1e0f8d9e9ac0bf228959b68d480b12a93cc911a4)** to **[f0b7bdb](https://github.com/sagemath/sagetrac-mirror/commit/f0b7bdbe70e75bc16f63ae3163315785ea4c777c)**



---

archive/issue_events_220932.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2014-08-24T16:47:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16682#event-220932"
}
```



---

archive/issue_events_220933.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2014-08-24T16:47:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16682#event-220933"
}
```



---

archive/issue_comments_220537.json:
```json
{
    "body": "<div id=\"comment:19\" align=\"right\">Comment 19</div>\n\ndone",
    "created_at": "2014-08-24T16:47:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16682#issuecomment-220537",
    "user": "https://github.com/videlec"
}
```

<div id="comment:19" align="right">Comment 19</div>

done



---

archive/issue_events_220934.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2014-08-24T16:47:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16682#event-220934"
}
```



---

archive/issue_events_220935.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2014-08-24T16:47:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16682#event-220935"
}
```



---

archive/issue_comments_220538.json:
```json
{
    "body": "<div id=\"comment:21\" align=\"right\">Comment 21</div>\n\nThanks!",
    "created_at": "2014-08-24T16:48:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16682#issuecomment-220538",
    "user": "https://github.com/videlec"
}
```

<div id="comment:21" align="right">Comment 21</div>

Thanks!



---

archive/issue_comments_220539.json:
```json
{
    "body": "Changed branch from **[u/vdelecroix/16682](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/16682)** to **[f0b7bdb](https://github.com/sagemath/sagetrac-mirror/commit/f0b7bdbe70e75bc16f63ae3163315785ea4c777c)**",
    "created_at": "2014-08-25T10:27:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16682#issuecomment-220539",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/vdelecroix/16682](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/16682)** to **[f0b7bdb](https://github.com/sagemath/sagetrac-mirror/commit/f0b7bdbe70e75bc16f63ae3163315785ea4c777c)**



---

archive/issue_events_220936.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-08-25T10:27:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16682#event-220936"
}
```



---

archive/issue_events_220937.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "e2c1c820ca4163c0fc5b06b6d2d6516d8e0a6d2c",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-08-25T10:27:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16682",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16682#event-220937"
}
```
