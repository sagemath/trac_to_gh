# Issue 16129: Improve caching in EllipticCurve_generic

archive/issues_015892.json:
```json
{
    "assignees": [],
    "body": "The way caching is implemented in `EllipticCurve_generic` causes some trouble\nat #11895. Several methods, such as `_multiple_x_numerator`,\n`_multiple_x_denominator`, `division_polynomial_0` should be rewritten such that they do not use a `cache` parameter anymore.\n\nCC:  @JohnCremona @defeo sbesnier\n\nBranch/Commit: **[4c00ee1](https://github.com/sagemath/sagetrac-mirror/commit/4c00ee1267a58234619669f99d17510e648bc53e)**\n\nReviewer: **Peter Bruin**\n\nAuthor: **Julian R\u00fcth**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16129_\n\n",
    "closed_at": "2014-05-07T08:30:59Z",
    "created_at": "2014-04-11T08:14:51Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20elliptic%20curves",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Improve caching in EllipticCurve_generic",
    "type": "issue",
    "updated_at": "2014-05-07T08:30:59Z",
    "url": "https://github.com/sagemath/sage/issues/16129",
    "user": "https://github.com/saraedum"
}
```
The way caching is implemented in `EllipticCurve_generic` causes some trouble
at #11895. Several methods, such as `_multiple_x_numerator`,
`_multiple_x_denominator`, `division_polynomial_0` should be rewritten such that they do not use a `cache` parameter anymore.

CC:  @JohnCremona @defeo sbesnier

Branch/Commit: **[4c00ee1](https://github.com/sagemath/sagetrac-mirror/commit/4c00ee1267a58234619669f99d17510e648bc53e)**

Reviewer: **Peter Bruin**

Author: **Julian Rüth**

_Issue created by migration from https://trac.sagemath.org/ticket/16129_





---

archive/issue_events_205152.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2014-04-11T08:14:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16129#event-205152"
}
```



---

archive/issue_events_205153.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2014-04-11T08:14:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20elliptic%20curves",
    "label_color": "0000ff",
    "label_name": "component: elliptic curves",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16129#event-205153"
}
```



---

archive/issue_events_205154.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2014-04-11T08:14:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16129#event-205154"
}
```



---

archive/issue_events_205155.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2014-04-11T08:14:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16129#event-205155"
}
```



---

archive/issue_comments_208368.json:
```json
{
    "body": "Branch: **[u/saraedum/ticket/16129](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/16129)**",
    "created_at": "2014-04-11T08:19:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16129#issuecomment-208368",
    "user": "https://github.com/saraedum"
}
```

Branch: **[u/saraedum/ticket/16129](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/16129)**



---

archive/issue_comments_208369.json:
```json
{
    "body": "Commit: **[b9b79ca](https://github.com/sagemath/sagetrac-mirror/commit/b9b79ca4045f836b0476d9a030265646b59afa55)**",
    "created_at": "2014-04-11T08:42:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16129#issuecomment-208369",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[b9b79ca](https://github.com/sagemath/sagetrac-mirror/commit/b9b79ca4045f836b0476d9a030265646b59afa55)**



---

archive/issue_comments_208370.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b9b79ca4045f836b0476d9a030265646b59afa55\">b9b79ca</a></td><td><code>Use @cached_method decorator for _multiple_x_numerator/denominator in EllipticCurve_generic</code></td></tr></table>\n",
    "created_at": "2014-04-11T08:42:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16129#issuecomment-208370",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:2'>Comment 2:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b9b79ca4045f836b0476d9a030265646b59afa55">b9b79ca</a></td><td><code>Use @cached_method decorator for _multiple_x_numerator/denominator in EllipticCurve_generic</code></td></tr></table>




---

archive/issue_comments_208371.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,3 @@\n The way caching is implemented in `EllipticCurve_generic` causes some trouble\n at #11895. Several methods, such as `_multiple_x_numerator`,\n-`_multiple_x_denominator`, `division_polynomial_0` should be rewritten to use\n-the `cached_method` decorator.\n+`_multiple_x_denominator`, `division_polynomial_0` should be rewritten such that they do not use a `cache` parameter anymore.\n``````\n",
    "created_at": "2014-04-11T21:38:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16129#issuecomment-208371",
    "user": "https://github.com/saraedum"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,3 @@
 The way caching is implemented in `EllipticCurve_generic` causes some trouble
 at #11895. Several methods, such as `_multiple_x_numerator`,
-`_multiple_x_denominator`, `division_polynomial_0` should be rewritten to use
-the `cached_method` decorator.
+`_multiple_x_denominator`, `division_polynomial_0` should be rewritten such that they do not use a `cache` parameter anymore.
``````




---

archive/issue_comments_208372.json:
```json
{
    "body": "Changed commit from **[b9b79ca](https://github.com/sagemath/sagetrac-mirror/commit/b9b79ca4045f836b0476d9a030265646b59afa55)** to **[e12fc10](https://github.com/sagemath/sagetrac-mirror/commit/e12fc109e9edebc37dddf8c33d0bd7ddc8683bb7)**",
    "created_at": "2014-04-11T22:26:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16129#issuecomment-208372",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[b9b79ca](https://github.com/sagemath/sagetrac-mirror/commit/b9b79ca4045f836b0476d9a030265646b59afa55)** to **[e12fc10](https://github.com/sagemath/sagetrac-mirror/commit/e12fc109e9edebc37dddf8c33d0bd7ddc8683bb7)**



---

archive/issue_comments_208373.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e12fc109e9edebc37dddf8c33d0bd7ddc8683bb7\">e12fc10</a></td><td><code>Use cached_function to cache computations in division_polynomial_0 of elliptic curves</code></td></tr></table>\n",
    "created_at": "2014-04-11T22:26:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16129#issuecomment-208373",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:5'>Comment 5:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e12fc109e9edebc37dddf8c33d0bd7ddc8683bb7">e12fc10</a></td><td><code>Use cached_function to cache computations in division_polynomial_0 of elliptic curves</code></td></tr></table>




---

archive/issue_events_205156.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2014-04-11T22:32:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16129#event-205156"
}
```



---

archive/issue_comments_208374.json:
```json
{
    "body": "Author: **Julian R\u00fcth**",
    "created_at": "2014-04-11T22:33:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16129#issuecomment-208374",
    "user": "https://github.com/saraedum"
}
```

Author: **Julian Rüth**



---

archive/issue_comments_208375.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/72c46da736411258f13a25ebf7d4e6713372d257\">72c46da</a></td><td><code>Fixed a typo in a reference.</code></td></tr></table>\n",
    "created_at": "2014-04-11T22:34:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16129#issuecomment-208375",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:8'>Comment 8:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/72c46da736411258f13a25ebf7d4e6713372d257">72c46da</a></td><td><code>Fixed a typo in a reference.</code></td></tr></table>




---

archive/issue_comments_208376.json:
```json
{
    "body": "Changed commit from **[e12fc10](https://github.com/sagemath/sagetrac-mirror/commit/e12fc109e9edebc37dddf8c33d0bd7ddc8683bb7)** to **[72c46da](https://github.com/sagemath/sagetrac-mirror/commit/72c46da736411258f13a25ebf7d4e6713372d257)**",
    "created_at": "2014-04-11T22:34:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16129#issuecomment-208376",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[e12fc10](https://github.com/sagemath/sagetrac-mirror/commit/e12fc109e9edebc37dddf8c33d0bd7ddc8683bb7)** to **[72c46da](https://github.com/sagemath/sagetrac-mirror/commit/72c46da736411258f13a25ebf7d4e6713372d257)**



---

archive/issue_comments_208377.json:
```json
{
    "body": "Work Issues: **resolve merge conflict**",
    "created_at": "2014-04-23T11:32:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16129#issuecomment-208377",
    "user": "https://github.com/pjbruin"
}
```

Work Issues: **resolve merge conflict**



---

archive/issue_events_205157.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2014-04-23T11:32:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16129#event-205157"
}
```



---

archive/issue_events_205158.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2014-04-23T11:32:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16129#event-205158"
}
```



---

archive/issue_comments_208378.json:
```json
{
    "body": "Changed commit from **[72c46da](https://github.com/sagemath/sagetrac-mirror/commit/72c46da736411258f13a25ebf7d4e6713372d257)** to **[fe8b5a9](https://github.com/sagemath/sagetrac-mirror/commit/fe8b5a96a423d0270057fd1dff735fbcf9b51996)**",
    "created_at": "2014-04-27T11:14:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16129#issuecomment-208378",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[72c46da](https://github.com/sagemath/sagetrac-mirror/commit/72c46da736411258f13a25ebf7d4e6713372d257)** to **[fe8b5a9](https://github.com/sagemath/sagetrac-mirror/commit/fe8b5a96a423d0270057fd1dff735fbcf9b51996)**



---

archive/issue_comments_208379.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/fe8b5a96a423d0270057fd1dff735fbcf9b51996\">fe8b5a9</a></td><td><code>Merge branch 'develop' into ticket/16129</code></td></tr></table>\n",
    "created_at": "2014-04-27T11:14:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16129#issuecomment-208379",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:11'>Comment 11:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/fe8b5a96a423d0270057fd1dff735fbcf9b51996">fe8b5a9</a></td><td><code>Merge branch 'develop' into ticket/16129</code></td></tr></table>




---

archive/issue_comments_208380.json:
```json
{
    "body": "Changed commit from **[fe8b5a9](https://github.com/sagemath/sagetrac-mirror/commit/fe8b5a96a423d0270057fd1dff735fbcf9b51996)** to **[afdfca4](https://github.com/sagemath/sagetrac-mirror/commit/afdfca48b1a7f2e23905e60d7dc43dc9e0475e40)**",
    "created_at": "2014-04-27T11:16:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16129#issuecomment-208380",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[fe8b5a9](https://github.com/sagemath/sagetrac-mirror/commit/fe8b5a96a423d0270057fd1dff735fbcf9b51996)** to **[afdfca4](https://github.com/sagemath/sagetrac-mirror/commit/afdfca48b1a7f2e23905e60d7dc43dc9e0475e40)**



---

archive/issue_comments_208381.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/afdfca48b1a7f2e23905e60d7dc43dc9e0475e40\">afdfca4</a></td><td><code>Merge branch 'develop' into ticket/16129</code></td></tr></table>\n",
    "created_at": "2014-04-27T11:16:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16129#issuecomment-208381",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:12'>Comment 12:</a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/afdfca48b1a7f2e23905e60d7dc43dc9e0475e40">afdfca4</a></td><td><code>Merge branch 'develop' into ticket/16129</code></td></tr></table>




---

archive/issue_events_205159.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2014-04-27T11:17:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16129#event-205159"
}
```



---

archive/issue_events_205160.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2014-04-27T11:17:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16129#event-205160"
}
```



---

archive/issue_comments_208382.json:
```json
{
    "body": "Changed work issues from **resolve merge conflict** to none",
    "created_at": "2014-04-27T11:17:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16129#issuecomment-208382",
    "user": "https://github.com/saraedum"
}
```

Changed work issues from **resolve merge conflict** to none



---

archive/issue_comments_208383.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nLooks good to me; now running doctests.\n\nI'm just slightly worried that some of the methods may cache too much, because the caching key depends on `x`, which can just be a concrete *x*-value rather than a polynomial variable.  However, this already seems to have been the case before this ticket, so maybe we can keep it like that for now.",
    "created_at": "2014-04-28T13:41:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16129#issuecomment-208383",
    "user": "https://github.com/pjbruin"
}
```

<a id='comment:14'>Comment 14:</a>
Looks good to me; now running doctests.

I'm just slightly worried that some of the methods may cache too much, because the caching key depends on `x`, which can just be a concrete *x*-value rather than a polynomial variable.  However, this already seems to have been the case before this ticket, so maybe we can keep it like that for now.



---

archive/issue_comments_208384.json:
```json
{
    "body": "Reviewer: **Peter Bruin**",
    "created_at": "2014-04-28T13:41:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16129#issuecomment-208384",
    "user": "https://github.com/pjbruin"
}
```

Reviewer: **Peter Bruin**



---

archive/issue_comments_208385.json:
```json
{
    "body": "Changed branch from **[u/saraedum/ticket/16129](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/16129)** to **[u/pbruin/16129-EllipticCurve_caching](https://github.com/sagemath/sagetrac-mirror/tree/u/pbruin/16129-EllipticCurve_caching)**",
    "created_at": "2014-04-28T17:20:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16129#issuecomment-208385",
    "user": "https://github.com/pjbruin"
}
```

Changed branch from **[u/saraedum/ticket/16129](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/16129)** to **[u/pbruin/16129-EllipticCurve_caching](https://github.com/sagemath/sagetrac-mirror/tree/u/pbruin/16129-EllipticCurve_caching)**



---

archive/issue_comments_208386.json:
```json
{
    "body": "<a id='comment:15'>Comment 15:</a>\nAll doctests pass, but building documentation was interrupted due to a warning which was caused by missing indentation.  Hence a trivial reviewer patch.",
    "created_at": "2014-04-28T17:20:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16129#issuecomment-208386",
    "user": "https://github.com/pjbruin"
}
```

<a id='comment:15'>Comment 15:</a>
All doctests pass, but building documentation was interrupted due to a warning which was caused by missing indentation.  Hence a trivial reviewer patch.



---

archive/issue_comments_208387.json:
```json
{
    "body": "Changed commit from **[afdfca4](https://github.com/sagemath/sagetrac-mirror/commit/afdfca48b1a7f2e23905e60d7dc43dc9e0475e40)** to **[4c00ee1](https://github.com/sagemath/sagetrac-mirror/commit/4c00ee1267a58234619669f99d17510e648bc53e)**",
    "created_at": "2014-04-28T17:20:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16129#issuecomment-208387",
    "user": "https://github.com/pjbruin"
}
```

Changed commit from **[afdfca4](https://github.com/sagemath/sagetrac-mirror/commit/afdfca48b1a7f2e23905e60d7dc43dc9e0475e40)** to **[4c00ee1](https://github.com/sagemath/sagetrac-mirror/commit/4c00ee1267a58234619669f99d17510e648bc53e)**



---

archive/issue_events_205161.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2014-04-28T17:20:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16129#event-205161"
}
```



---

archive/issue_events_205162.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2014-04-28T17:20:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16129#event-205162"
}
```



---

archive/issue_events_205163.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16129#event-205163"
}
```



---

archive/issue_events_205164.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16129#event-205164"
}
```



---

archive/issue_comments_208388.json:
```json
{
    "body": "Changed branch from **[u/pbruin/16129-EllipticCurve_caching](https://github.com/sagemath/sagetrac-mirror/tree/u/pbruin/16129-EllipticCurve_caching)** to **[4c00ee1](https://github.com/sagemath/sagetrac-mirror/commit/4c00ee1267a58234619669f99d17510e648bc53e)**",
    "created_at": "2014-05-07T08:30:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16129#issuecomment-208388",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/pbruin/16129-EllipticCurve_caching](https://github.com/sagemath/sagetrac-mirror/tree/u/pbruin/16129-EllipticCurve_caching)** to **[4c00ee1](https://github.com/sagemath/sagetrac-mirror/commit/4c00ee1267a58234619669f99d17510e648bc53e)**



---

archive/issue_events_205165.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-05-07T08:30:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16129#event-205165"
}
```



---

archive/issue_events_205166.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "c9d7bb74ed1e4279cc6733e9f0af21961455c554",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-05-07T08:30:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16129",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16129#event-205166"
}
```
