...
```json
{
    "body": "`str` is more for human consumption, so if you have to choose to handle something via a string, `repr` is the better one to go for:\n\n```\nsage: str(k*x)\n'                                      f x'\nsage: repr(k*x)\n'f*x'\n```\nor with format strings:\n\n```\nsage: \"%r\"%(k*x)\n'f*x'\nsage: \"%s\"%(k*x)\n'                                      f x'\n```\nThe len doesn't come i, by the way:\n\n```\nsage: def tt(*args): return len(args)\nsage: tt(k*x)\n1\n```\nI'd say `%r` is the better one to go for.",
    "created_at": "2014-07-01T17:35:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16355#issuecomment-214296",
...
```json
{
    ...
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16355#event-48679"
}
```
