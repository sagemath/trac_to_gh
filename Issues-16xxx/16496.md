# Issue 16496: New class for Selmer groups of number fields

archive/issues_016259.json:
```json
{
    "body": "CC:  @pjbruin\n\nThere should be a proper class for Selmer groups of number fields K(S,m), similar to the existing and related S-unit groups and S-class-group classes: it fits into the short exact sequence\n\n  ` 1 --> O<sup>*_{K,S}/(O</sup>*_{K,S})^m --> K(S,m) --> Cl_{K,S}[m] --> 1 . `\n\nCurrently K.selmer_group(S,m) just returns a list of generators; it should be a proper finite abelian group.  Angelos Koutsianas already has code which expresses a given element in terms of the generators.\n\n\nIssue created by migration from https://trac.sagemath.org/ticket/16496\n\n",
    "created_at": "2014-06-18T11:43:21Z",
    "labels": [
        "component: number fields"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.4",
    "title": "New class for Selmer groups of number fields",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/16496",
    "user": "https://github.com/JohnCremona"
}
```
CC:  @pjbruin

There should be a proper class for Selmer groups of number fields K(S,m), similar to the existing and related S-unit groups and S-class-group classes: it fits into the short exact sequence

  ` 1 --> O<sup>*_{K,S}/(O</sup>*_{K,S})^m --> K(S,m) --> Cl_{K,S}[m] --> 1 . `

Currently K.selmer_group(S,m) just returns a list of generators; it should be a proper finite abelian group.  Angelos Koutsianas already has code which expresses a given element in terms of the generators.


Issue created by migration from https://trac.sagemath.org/ticket/16496





---

archive/issue_events_048474.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16496",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16496#event-48474"
}
```



---

archive/issue_comments_212388.json:
```json
{
    "body": "I have code which implements a new function which on input (K,S,p) with p prime, returns K(S,p) as an abstract vector space together with two maps, one from the abstract space to K^* and one (the harder one) which takes an element of K^* whose image in K<sup>*/(K</sup>*)^p lies in K(S,p) and returns the associated abstract vector space element.  This is needed from some applications (in progress).  \n\nMy implementation is based on code I wrote ~15 years ago in Magma.  It does not actually define a new class but provides all the functionality one needs.",
    "created_at": "2018-04-19T13:47:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16496",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16496#issuecomment-212388",
    "user": "https://github.com/JohnCremona"
}
```

I have code which implements a new function which on input (K,S,p) with p prime, returns K(S,p) as an abstract vector space together with two maps, one from the abstract space to K^* and one (the harder one) which takes an element of K^* whose image in K<sup>*/(K</sup>*)^p lies in K(S,p) and returns the associated abstract vector space element.  This is needed from some applications (in progress).  

My implementation is based on code I wrote ~15 years ago in Magma.  It does not actually define a new class but provides all the functionality one needs.



---

archive/issue_events_048475.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2018-04-19T13:47:35Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16496",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16496#event-48475"
}
```



---

archive/issue_events_048476.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2018-04-19T13:47:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16496",
    "milestone": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16496#event-48476"
}
```



---

archive/issue_events_048477.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-08-03T19:20:18Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16496",
    "milestone": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16496#event-48477"
}
```



---

archive/issue_events_048478.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-08-03T19:20:18Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16496",
    "milestone": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16496#event-48478"
}
```



---

archive/issue_comments_212389.json:
```json
{
    "body": "update milestone 8.3 -> 8.4",
    "created_at": "2018-08-03T19:20:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16496",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16496#issuecomment-212389",
    "user": "https://github.com/videlec"
}
```

update milestone 8.3 -> 8.4



---

archive/issue_comments_212390.json:
```json
{
    "body": "I created a new ticket #31345 for this in the case m=p.  I probably should have reused this ticket instead, but this one can stay in case the improved versions are extended to general m in future.",
    "created_at": "2021-02-05T14:52:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16496",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16496#issuecomment-212390",
    "user": "https://github.com/JohnCremona"
}
```

I created a new ticket #31345 for this in the case m=p.  I probably should have reused this ticket instead, but this one can stay in case the improved versions are extended to general m in future.
