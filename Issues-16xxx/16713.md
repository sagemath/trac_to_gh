# Issue 16713: Random failure in lseries_ell.py

archive/issues_016476.json:
```json
{
    "assignees": [],
    "body": "This is a somewhat regular occurrence on mod:\n\n```\nsage -t --long src/sage/schemes/elliptic_curves/lseries_ell.py\n**********************************************************************\nFile \"src/sage/schemes/elliptic_curves/lseries_ell.py\", line 243, in sage.schemes.elliptic_curves.lseries_ell.Lseries_ell.zeros\nFailed example:\n    E.lseries().zeros(2)\nExpected:\n    [0.000000000, 5.00317001]\nGot:\n      ***   Warning: not enough memory, new stack 1000000000.\n    [0.000000000, 5.00317001]\n```\nThere seems to be plenty of free space. Obviously there is enough memory to finish the computation and arrive at the correct result, so there shouldn't be any warning printed. Related tickets that patched up similar issues:\n* Trac #11985: lcalc is too verbose (mysterious \"Warning: new stack size\")\n* Trac #14470: Relax PARI warnings\n\nCC:  @jdemeyer @nexttime @williamstein\n\nKeywords: **random_fail pari lcalc**\n\nReviewer: **Jeroen Demeyer**\n\nComponent: **elliptic curves**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16713_\n\n",
    "closed_at": "2015-09-25T08:08:15Z",
    "created_at": "2014-07-25T04:51:31Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20elliptic%20curves",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Random failure in lseries_ell.py",
    "type": "issue",
    "updated_at": "2015-09-25T08:08:15Z",
    "url": "https://github.com/sagemath/sage/issues/16713",
    "user": "https://github.com/vbraun"
}
```
This is a somewhat regular occurrence on mod:

```
sage -t --long src/sage/schemes/elliptic_curves/lseries_ell.py
**********************************************************************
File "src/sage/schemes/elliptic_curves/lseries_ell.py", line 243, in sage.schemes.elliptic_curves.lseries_ell.Lseries_ell.zeros
Failed example:
    E.lseries().zeros(2)
Expected:
    [0.000000000, 5.00317001]
Got:
      ***   Warning: not enough memory, new stack 1000000000.
    [0.000000000, 5.00317001]
```
There seems to be plenty of free space. Obviously there is enough memory to finish the computation and arrive at the correct result, so there shouldn't be any warning printed. Related tickets that patched up similar issues:
* Trac #11985: lcalc is too verbose (mysterious "Warning: new stack size")
* Trac #14470: Relax PARI warnings

CC:  @jdemeyer @nexttime @williamstein

Keywords: **random_fail pari lcalc**

Reviewer: **Jeroen Demeyer**

Component: **elliptic curves**

_Issue created by migration from https://trac.sagemath.org/ticket/16713_





---

archive/issue_events_238930.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-07-25T04:51:31Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16713",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16713#event-238930"
}
```



---

archive/issue_events_238931.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-07-25T04:51:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16713",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20elliptic%20curves",
    "label_color": "0000ff",
    "label_name": "c: elliptic curves",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16713#event-238931"
}
```



---

archive/issue_events_238932.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-07-25T04:51:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16713",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16713#event-238932"
}
```



---

archive/issue_events_238933.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-07-25T04:51:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16713",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16713#event-238933"
}
```



---

archive/issue_comments_218553.json:
```json
{
    "body": "Changed keywords from **random_fail** to **random_fail pari lcalc**",
    "created_at": "2014-07-25T04:53:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16713",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16713#issuecomment-218553",
    "user": "https://github.com/vbraun"
}
```

Changed keywords from **random_fail** to **random_fail pari lcalc**



---

archive/issue_comments_218554.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nWell, 8.04 is out of LTS... ;-)\n\nDid anybody else face this on another system / platform?",
    "created_at": "2014-07-25T12:27:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16713",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16713#issuecomment-218554",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:2" align="right">comment:2</div>

Well, 8.04 is out of LTS... ;-)

Did anybody else face this on another system / platform?



---

archive/issue_comments_218555.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nI'll keep an eye out for it... Still, it doesn't look like a compiler bug.",
    "created_at": "2014-07-25T14:14:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16713",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16713#issuecomment-218555",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:3" align="right">comment:3</div>

I'll keep an eye out for it... Still, it doesn't look like a compiler bug.



---

archive/issue_comments_218556.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nLet's hope that this problem disappears with #15767.",
    "created_at": "2014-07-31T07:36:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16713",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16713#issuecomment-218556",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:4" align="right">comment:4</div>

Let's hope that this problem disappears with #15767.



---

archive/issue_events_238934.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16713",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16713#event-238934"
}
```



---

archive/issue_events_238935.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16713",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16713#event-238935"
}
```



---

archive/issue_comments_218557.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nHas this happened recently? We upgraded PARI several times in the mean time...",
    "created_at": "2015-06-26T11:28:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16713",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16713#issuecomment-218557",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:6" align="right">comment:6</div>

Has this happened recently? We upgraded PARI several times in the mean time...



---

archive/issue_events_238936.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-09-19T10:57:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16713",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16713#event-238936"
}
```



---

archive/issue_events_238937.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-09-19T10:57:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16713",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16713#event-238937"
}
```



---

archive/issue_events_238938.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-09-19T10:57:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16713",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16713#event-238938"
}
```



---

archive/issue_events_238939.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-09-19T10:57:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16713",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16713#event-238939"
}
```



---

archive/issue_comments_218558.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2015-09-19T10:57:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16713",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16713#issuecomment-218558",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_238940.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-09-19T10:57:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16713",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16713#event-238940"
}
```



---

archive/issue_events_238941.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-09-19T10:57:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16713",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16713#event-238941"
}
```



---

archive/issue_events_238942.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-09-25T08:08:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16713",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16713#event-238942"
}
```



---

archive/issue_events_238943.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-09-25T08:08:15Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16713",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16713#event-238943"
}
```
