# Issue 16909: transitive_closure() and mutable graphs

archive/issues_016672.json:
```json
{
    "assignees": [],
    "body": "In graph/generic_graph.py at `transitive_closure()` line `G = copy(self)` should probably be `G = self.copy(immutable=False)`. Otherwise at least on hasse_diagram code one must use `DiGraph(self).transitive_closure()` instead of `self.transitive_closure()`. See discussion on ticket #16892.\n\n\nCC:  @nathanncohen @tscrim Simon\n\nBranch/Commit: **[ad47132](https://github.com/sagemath/sagetrac-mirror/commit/ad47132453d5bca8ea076357be87db9313f0ae64)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Nathann Cohen**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16909_\n\n",
    "closed_at": "2014-09-16T18:45:52Z",
    "created_at": "2014-08-30T07:00:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "transitive_closure() and mutable graphs",
    "type": "issue",
    "updated_at": "2014-09-16T18:45:52Z",
    "url": "https://github.com/sagemath/sage/issues/16909",
    "user": "https://github.com/jm58660"
}
```
In graph/generic_graph.py at `transitive_closure()` line `G = copy(self)` should probably be `G = self.copy(immutable=False)`. Otherwise at least on hasse_diagram code one must use `DiGraph(self).transitive_closure()` instead of `self.transitive_closure()`. See discussion on ticket #16892.


CC:  @nathanncohen @tscrim Simon

Branch/Commit: **[ad47132](https://github.com/sagemath/sagetrac-mirror/commit/ad47132453d5bca8ea076357be87db9313f0ae64)**

Reviewer: **Travis Scrimshaw**

Author: **Nathann Cohen**

_Issue created by migration from https://trac.sagemath.org/ticket/16909_





---

archive/issue_events_224118.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2014-08-30T07:00:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16909",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16909#event-224118"
}
```



---

archive/issue_events_224119.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2014-08-30T07:00:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16909",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16909#event-224119"
}
```



---

archive/issue_events_224120.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2014-08-30T07:00:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16909",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16909#event-224120"
}
```



---

archive/issue_events_224121.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2014-08-30T07:00:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16909",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16909#event-224121"
}
```



---

archive/issue_comments_225369.json:
```json
{
    "body": "Author: **Nathann Cohen**",
    "created_at": "2014-08-30T07:33:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16909#issuecomment-225369",
    "user": "https://github.com/nathanncohen"
}
```

Author: **Nathann Cohen**



---

archive/issue_events_224122.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-08-30T07:33:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16909",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16909#event-224122"
}
```



---

archive/issue_comments_225370.json:
```json
{
    "body": "Branch: **[public/16909](https://github.com/sagemath/sagetrac-mirror/tree/public/16909)**",
    "created_at": "2014-08-30T07:33:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16909#issuecomment-225370",
    "user": "https://github.com/nathanncohen"
}
```

Branch: **[public/16909](https://github.com/sagemath/sagetrac-mirror/tree/public/16909)**



---

archive/issue_comments_225371.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ad47132453d5bca8ea076357be87db9313f0ae64\">ad47132</a></td><td><code>trac #16909: transitive_closure() and mutable graphs</code></td></tr></table>\n",
    "created_at": "2014-08-30T07:33:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16909#issuecomment-225371",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:2'>Comment 2:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ad47132453d5bca8ea076357be87db9313f0ae64">ad47132</a></td><td><code>trac #16909: transitive_closure() and mutable graphs</code></td></tr></table>




---

archive/issue_comments_225372.json:
```json
{
    "body": "Commit: **[ad47132](https://github.com/sagemath/sagetrac-mirror/commit/ad47132453d5bca8ea076357be87db9313f0ae64)**",
    "created_at": "2014-08-30T07:33:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16909#issuecomment-225372",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[ad47132](https://github.com/sagemath/sagetrac-mirror/commit/ad47132453d5bca8ea076357be87db9313f0ae64)**



---

archive/issue_comments_225373.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2014-09-15T20:12:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16909#issuecomment-225373",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_225374.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nLGTM.",
    "created_at": "2014-09-15T20:12:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16909#issuecomment-225374",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:3'>Comment 3:</a>
LGTM.



---

archive/issue_events_224123.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-09-15T20:12:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16909",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16909#event-224123"
}
```



---

archive/issue_events_224124.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-09-15T20:12:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16909",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16909#event-224124"
}
```



---

archive/issue_comments_225375.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nThanks !\n\nNathann",
    "created_at": "2014-09-16T09:04:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16909#issuecomment-225375",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:4'>Comment 4:</a>
Thanks !

Nathann



---

archive/issue_comments_225376.json:
```json
{
    "body": "Changed branch from **[public/16909](https://github.com/sagemath/sagetrac-mirror/tree/public/16909)** to **[ad47132](https://github.com/sagemath/sagetrac-mirror/commit/ad47132453d5bca8ea076357be87db9313f0ae64)**",
    "created_at": "2014-09-16T18:45:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16909#issuecomment-225376",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/16909](https://github.com/sagemath/sagetrac-mirror/tree/public/16909)** to **[ad47132](https://github.com/sagemath/sagetrac-mirror/commit/ad47132453d5bca8ea076357be87db9313f0ae64)**



---

archive/issue_events_224125.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-09-16T18:45:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16909",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16909#event-224125"
}
```



---

archive/issue_events_224126.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "447cffd1da2c8ffd16b9473f8e3df6340903997a",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-09-16T18:45:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16909",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16909#event-224126"
}
```
