# Issue 16413: upgrade setuptools to version 3.6

archive/issues_016176.json:
```json
{
    "assignees": [],
    "body": "Alternatively, if upgrade not possible, apply the attached patch. There is however no reason known atm for not upgrading. Version 3.5.2 contains a patch to fix stale zip caches which hit unsafe ticket testing with patchbot. But many other issues were fixed too.\n\nTarball is at https://pypi.python.org/packages/source/s/setuptools/setuptools-3.6.tar.gz#md5=8f3a1dcdc14313c8334eb6af4f66ea0a\n\nKeywords: **patchbot unsafe tickets**\n\nBranch/Commit: **[95d3a69](https://github.com/sagemath/sagetrac-mirror/commit/95d3a6973807b2893c985c1205b2d29e57c52a4c)**\n\nReviewer: **Volker Braun**\n\nAuthor: **Ralf Stephan**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16413_\n\n",
    "closed_at": "2014-06-01T18:23:52Z",
    "created_at": "2014-05-30T07:24:43Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "upgrade setuptools to version 3.6",
    "type": "issue",
    "updated_at": "2014-06-01T18:23:52Z",
    "url": "https://github.com/sagemath/sage/issues/16413",
    "user": "https://github.com/rwst"
}
```
Alternatively, if upgrade not possible, apply the attached patch. There is however no reason known atm for not upgrading. Version 3.5.2 contains a patch to fix stale zip caches which hit unsafe ticket testing with patchbot. But many other issues were fixed too.

Tarball is at https://pypi.python.org/packages/source/s/setuptools/setuptools-3.6.tar.gz#md5=8f3a1dcdc14313c8334eb6af4f66ea0a

Keywords: **patchbot unsafe tickets**

Branch/Commit: **[95d3a69](https://github.com/sagemath/sagetrac-mirror/commit/95d3a6973807b2893c985c1205b2d29e57c52a4c)**

Reviewer: **Volker Braun**

Author: **Ralf Stephan**

_Issue created by migration from https://trac.sagemath.org/ticket/16413_





---

archive/issue_events_216909.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-05-30T07:24:43Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16413",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16413#event-216909"
}
```



---

archive/issue_events_216910.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-05-30T07:24:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16413",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16413#event-216910"
}
```



---

archive/issue_events_216911.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-05-30T07:24:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16413",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16413#event-216911"
}
```



---

archive/issue_events_216912.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-05-30T07:24:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16413",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16413#event-216912"
}
```



---

archive/issue_comments_214443.json:
```json
{
    "body": "Attachment: **[t.gz](https://github.com/sagemath/sage/files/ticket16413/t.gz)**\n\npatch, fix for http://bitbucket.org/pypa/setuptools/issue/168",
    "created_at": "2014-05-30T07:28:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16413",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16413#issuecomment-214443",
    "user": "https://github.com/rwst"
}
```

Attachment: **[t.gz](https://github.com/sagemath/sage/files/ticket16413/t.gz)**

patch, fix for http://bitbucket.org/pypa/setuptools/issue/168



---

archive/issue_comments_214444.json:
```json
{
    "body": "Branch: **[u/rws/upgrade_setuptools_to_at_least_3_5_2](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/upgrade_setuptools_to_at_least_3_5_2)**",
    "created_at": "2014-05-30T07:34:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16413",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16413#issuecomment-214444",
    "user": "https://github.com/rwst"
}
```

Branch: **[u/rws/upgrade_setuptools_to_at_least_3_5_2](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/upgrade_setuptools_to_at_least_3_5_2)**



---

archive/issue_events_216913.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-05-30T07:35:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16413",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16413#event-216913"
}
```



---

archive/issue_comments_214445.json:
```json
{
    "body": "Commit: **[a3c4cf3](https://github.com/sagemath/sagetrac-mirror/commit/a3c4cf39c3625a311d72bf0c88e9bfbd028d74f3)**",
    "created_at": "2014-05-30T07:35:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16413",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16413#issuecomment-214445",
    "user": "https://github.com/rwst"
}
```

Commit: **[a3c4cf3](https://github.com/sagemath/sagetrac-mirror/commit/a3c4cf39c3625a311d72bf0c88e9bfbd028d74f3)**



---

archive/issue_comments_214446.json:
```json
{
    "body": "Changed commit from **[a3c4cf3](https://github.com/sagemath/sagetrac-mirror/commit/a3c4cf39c3625a311d72bf0c88e9bfbd028d74f3)** to **[95d3a69](https://github.com/sagemath/sagetrac-mirror/commit/95d3a6973807b2893c985c1205b2d29e57c52a4c)**",
    "created_at": "2014-05-30T07:57:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16413",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16413#issuecomment-214446",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[a3c4cf3](https://github.com/sagemath/sagetrac-mirror/commit/a3c4cf39c3625a311d72bf0c88e9bfbd028d74f3)** to **[95d3a69](https://github.com/sagemath/sagetrac-mirror/commit/95d3a6973807b2893c985c1205b2d29e57c52a4c)**



---

archive/issue_comments_214447.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/95d3a6973807b2893c985c1205b2d29e57c52a4c\">95d3a69</a></td><td><code>16413: bump setuptools version</code></td></tr></table>\n",
    "created_at": "2014-05-30T07:57:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16413",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16413#issuecomment-214447",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/95d3a6973807b2893c985c1205b2d29e57c52a4c">95d3a69</a></td><td><code>16413: bump setuptools version</code></td></tr></table>




---

archive/issue_events_216914.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-05-30T11:05:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16413",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16413#event-216914"
}
```



---

archive/issue_events_216915.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-05-30T11:05:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16413",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16413#event-216915"
}
```



---

archive/issue_comments_214448.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2014-05-30T11:05:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16413",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16413#issuecomment-214448",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_events_216916.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2014-05-30T16:37:40Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/16413",
    "title_is": "upgrade setuptools to version 3.6",
    "title_was": "upgrade setuptools to at least 3.5.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16413#event-216916"
}
```



---

archive/issue_comments_214449.json:
```json
{
    "body": "Changed keywords from **** to **patchbot unsafe tickets**",
    "created_at": "2014-05-30T16:37:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16413",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16413#issuecomment-214449",
    "user": "https://github.com/nexttime"
}
```

Changed keywords from **** to **patchbot unsafe tickets**



---

archive/issue_comments_214450.json:
```json
{
    "body": "Changed branch from **[u/rws/upgrade_setuptools_to_at_least_3_5_2](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/upgrade_setuptools_to_at_least_3_5_2)** to **[95d3a69](https://github.com/sagemath/sagetrac-mirror/commit/95d3a6973807b2893c985c1205b2d29e57c52a4c)**",
    "created_at": "2014-06-01T18:23:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16413",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16413#issuecomment-214450",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/rws/upgrade_setuptools_to_at_least_3_5_2](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/upgrade_setuptools_to_at_least_3_5_2)** to **[95d3a69](https://github.com/sagemath/sagetrac-mirror/commit/95d3a6973807b2893c985c1205b2d29e57c52a4c)**



---

archive/issue_events_216917.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-06-01T18:23:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16413",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16413#event-216917"
}
```



---

archive/issue_events_216918.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "f43bd4cf8a037fa03c195b6cf972fae18a45f5fd",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-06-01T18:23:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16413",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16413#event-216918"
}
```
