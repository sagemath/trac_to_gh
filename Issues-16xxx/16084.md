...
```json
{
    "body": "<a id='comment:10'></a>I see the same as [above](#comment%3A7): `make build-serial` failed without this because matplotlib tried to build with only `pkgconfig`, not `pkgconf`. With this, `make build-serial` worked.",
    "created_at": "2014-04-07T20:36:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16084",
    "type": "issue_comment",
...
<a id='comment:10'></a>I see the same as [above](#comment%3A7): `make build-serial` failed without this because matplotlib tried to build with only `pkgconfig`, not `pkgconf`. With this, `make build-serial` worked.
