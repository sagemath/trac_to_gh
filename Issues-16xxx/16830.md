# Issue 16830: Can't multiply a linear morphism defined by a matrix by an element of the base field

archive/issues_016593.json:
```json
{
    "body": "```\nsage: K = GF(7); K\nFinite Field of size 7\nsage: sage: phi = End(K^2)([[1,1],[1,1]])\nsage: phi\nVector space morphism represented by the matrix:\n[1 1]\n[1 1]\nDomain: Vector space of dimension 2 over Finite Field of size 7\nCodomain: Vector space of dimension 2 over Finite Field of size 7\nsage: 2*phi\nVector space morphism represented by the matrix:\n[2 2]\n[2 2]\nDomain: Vector space of dimension 2 over Finite Field of size 7\nCodomain: Vector space of dimension 2 over Finite Field of size 7\n```\n\nSo far, so good. But:\n\n```\nsage: K(2)*phi\n---------------------------------------------------------------------------\nTypeError                                 Traceback (most recent call last)\n<ipython-input-35-81e4c450e31e> in <module>()\n----> 1 K(Integer(2))*phi\n\n/opt/sage-git/local/lib/python2.7/site-packages/sage/structure/element.so in sage.structure.element.RingElement.__mul__ (build/cythonized/sage/structure/element.c:15355)()\n\n/opt/sage-git/local/lib/python2.7/site-packages/sage/structure/coerce.so in sage.structure.coerce.CoercionModel_cache_maps.bin_op (build/cythonized/sage/structure/coerce.c:8323)()\n\nTypeError: unsupported operand parent(s) for '*': 'Finite Field of size 7' and 'Set of Morphisms (Linear Transformations) from Vector space of dimension 2 over Finite Field of size 7 to Vector space of dimension 2 over Finite Field of size 7'\n```\n\nIn particular this prevents illustrating Cayley-Hamilton on the morphism side:\n\n```\nsage: phi.characteristic_polynomial()(phi)\n...\nTypeError: unsupported operand parent(s) for '*': 'Finite Field of size 7' and 'Set of Morphisms (Linear Transformations) from Vector space of dimension 2 over Finite Field of size 7 to Vector space of dimension 2 over Finite Field of size 7'\n```\n\nUpdate 2016/09/20: things have gotten even worse. Now `2*phi` does not work anymore.\n\nCC:  @tscrim\n\nStatus: new\n\nIssue created by migration from https://trac.sagemath.org/ticket/16830\n\n",
    "created_at": "2014-08-14T17:21:04Z",
    "labels": [
        "component: linear algebra",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.8",
    "title": "Can't multiply a linear morphism defined by a matrix by an element of the base field",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/16830",
    "user": "https://github.com/nthiery"
}
```
```
sage: K = GF(7); K
Finite Field of size 7
sage: sage: phi = End(K^2)([[1,1],[1,1]])
sage: phi
Vector space morphism represented by the matrix:
[1 1]
[1 1]
Domain: Vector space of dimension 2 over Finite Field of size 7
Codomain: Vector space of dimension 2 over Finite Field of size 7
sage: 2*phi
Vector space morphism represented by the matrix:
[2 2]
[2 2]
Domain: Vector space of dimension 2 over Finite Field of size 7
Codomain: Vector space of dimension 2 over Finite Field of size 7
```

So far, so good. But:

```
sage: K(2)*phi
---------------------------------------------------------------------------
TypeError                                 Traceback (most recent call last)
<ipython-input-35-81e4c450e31e> in <module>()
----> 1 K(Integer(2))*phi

/opt/sage-git/local/lib/python2.7/site-packages/sage/structure/element.so in sage.structure.element.RingElement.__mul__ (build/cythonized/sage/structure/element.c:15355)()

/opt/sage-git/local/lib/python2.7/site-packages/sage/structure/coerce.so in sage.structure.coerce.CoercionModel_cache_maps.bin_op (build/cythonized/sage/structure/coerce.c:8323)()

TypeError: unsupported operand parent(s) for '*': 'Finite Field of size 7' and 'Set of Morphisms (Linear Transformations) from Vector space of dimension 2 over Finite Field of size 7 to Vector space of dimension 2 over Finite Field of size 7'
```

In particular this prevents illustrating Cayley-Hamilton on the morphism side:

```
sage: phi.characteristic_polynomial()(phi)
...
TypeError: unsupported operand parent(s) for '*': 'Finite Field of size 7' and 'Set of Morphisms (Linear Transformations) from Vector space of dimension 2 over Finite Field of size 7 to Vector space of dimension 2 over Finite Field of size 7'
```

Update 2016/09/20: things have gotten even worse. Now `2*phi` does not work anymore.

CC:  @tscrim

Status: new

Issue created by migration from https://trac.sagemath.org/ticket/16830





---

archive/issue_comments_232882.json:
```json
{
    "body": "Description changed:\n```diff\n--- \n+++ \n@@ -12,14 +12,13 @@\n Vector space morphism represented by the matrix:\n [2 2]\n [2 2]\n+Domain: Vector space of dimension 2 over Finite Field of size 7\n+Codomain: Vector space of dimension 2 over Finite Field of size 7\n \\`\\`\\`\n \n So far, so good. But:\n \n \\`\\`\\`\n-\n-Domain: Vector space of dimension 2 over Finite Field of size 7\n-Codomain: Vector space of dimension 2 over Finite Field of size 7\n sage: K(2)*phi\n ---------------------------------------------------------------------------\n TypeError                                 Traceback (most recent call last)\n@@ -33,6 +32,14 @@\n TypeError: unsupported operand parent(s) for '*': 'Finite Field of size 7' and 'Set of Morphisms (Linear Transformations) from Vector space of dimension 2 over Finite Field of size 7 to Vector space of dimension 2 over Finite Field of size 7'\n \\`\\`\\`\n \n+In particular this prevents illustrating Cayley-Hamilton on the morphism side:\n+\n+\\`\\`\\`\n+sage: phi.characteristic_polynomial()(phi)\n+...\n+TypeError: unsupported operand parent(s) for '*': 'Finite Field of size 7' and 'Set of Morphisms (Linear Transformations) from Vector space of dimension 2 over Finite Field of size 7 to Vector space of dimension 2 over Finite Field of size 7'\n+\\`\\`\\`\n+\n Comment: 1\n \n Issue created by migration from https://trac.sagemath.org/ticket/16830\n```\n",
    "created_at": "2014-08-14T17:22:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16830",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16830#issuecomment-232882",
    "user": "https://github.com/nthiery"
}
```

Description changed:
```diff
--- 
+++ 
@@ -12,14 +12,13 @@
 Vector space morphism represented by the matrix:
 [2 2]
 [2 2]
+Domain: Vector space of dimension 2 over Finite Field of size 7
+Codomain: Vector space of dimension 2 over Finite Field of size 7
 \`\`\`
 
 So far, so good. But:
 
 \`\`\`
-
-Domain: Vector space of dimension 2 over Finite Field of size 7
-Codomain: Vector space of dimension 2 over Finite Field of size 7
 sage: K(2)*phi
 ---------------------------------------------------------------------------
 TypeError                                 Traceback (most recent call last)
@@ -33,6 +32,14 @@
 TypeError: unsupported operand parent(s) for '*': 'Finite Field of size 7' and 'Set of Morphisms (Linear Transformations) from Vector space of dimension 2 over Finite Field of size 7 to Vector space of dimension 2 over Finite Field of size 7'
 \`\`\`
 
+In particular this prevents illustrating Cayley-Hamilton on the morphism side:
+
+\`\`\`
+sage: phi.characteristic_polynomial()(phi)
+...
+TypeError: unsupported operand parent(s) for '*': 'Finite Field of size 7' and 'Set of Morphisms (Linear Transformations) from Vector space of dimension 2 over Finite Field of size 7 to Vector space of dimension 2 over Finite Field of size 7'
+\`\`\`
+
 Comment: 1
 
 Issue created by migration from https://trac.sagemath.org/ticket/16830
```




---

archive/issue_comments_232883.json:
```json
{
    "body": "<a id='comment:2'></a>That looks like the category machinery is not cooperating with coercion properly. The scalar multiplication should be there according to the categories:\n\n```\nsage: parent(phi).categories()\n[Category of hom sets in Category of modules over Finite Field of size 7,\n Category of vector spaces over Finite Field of size 7,\n Category of modules over Finite Field of size 7,\n Category of bimodules over Finite Field of size 7 on the left and Finite Field of size 7 on the right,\n Category of right modules over Finite Field of size 7,\n Category of left modules over Finite Field of size 7,\n Category of commutative additive groups,\n Category of additive groups,\n Category of additive inverse additive unital additive magmas,\n Category of commutative additive monoids,\n Category of additive monoids,\n Category of additive unital additive magmas,\n Category of commutative additive semigroups,\n Category of additive commutative additive magmas,\n Category of additive semigroups,\n Category of additive magmas,\n Category of hom sets in Category of sets,\n Category of hom sets in Category of sets with partial maps,\n Category of hom sets in Category of objects,\n Category of sets,\n Category of sets with partial maps,\n Category of objects]\n```\nBy the way, going up the inheritance in the code:\n\n```\nsage: sage.modules.vector_space_homspace.VectorSpaceHomspace.mro()\n[sage.modules.vector_space_homspace.VectorSpaceHomspace,\n sage.modules.free_module_homspace.FreeModuleHomspace,\n sage.categories.homset.HomsetWithBase,\n sage.categories.homset.Homset,\n sage.structure.parent.Set_generic,\n sage.structure.parent.Parent,\n sage.structure.category_object.CategoryObject,\n sage.structure.sage_object.SageObject,\n object]\n```\nand reading the code, it's completely unclear how `parent(phi)` got those module categories attached to it. It would have to happen somewhere in `sage.modules.vector_space_homspace.VectorSpaceHomspace, sage.modules.free_module_homspace.FreeModuleHomspace` but none of those classes have an `__init__`.",
    "created_at": "2014-08-14T19:30:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16830",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16830#issuecomment-232883",
    "user": "https://github.com/nbruin"
}
```

<a id='comment:2'></a>That looks like the category machinery is not cooperating with coercion properly. The scalar multiplication should be there according to the categories:

```
sage: parent(phi).categories()
[Category of hom sets in Category of modules over Finite Field of size 7,
 Category of vector spaces over Finite Field of size 7,
 Category of modules over Finite Field of size 7,
 Category of bimodules over Finite Field of size 7 on the left and Finite Field of size 7 on the right,
 Category of right modules over Finite Field of size 7,
 Category of left modules over Finite Field of size 7,
 Category of commutative additive groups,
 Category of additive groups,
 Category of additive inverse additive unital additive magmas,
 Category of commutative additive monoids,
 Category of additive monoids,
 Category of additive unital additive magmas,
 Category of commutative additive semigroups,
 Category of additive commutative additive magmas,
 Category of additive semigroups,
 Category of additive magmas,
 Category of hom sets in Category of sets,
 Category of hom sets in Category of sets with partial maps,
 Category of hom sets in Category of objects,
 Category of sets,
 Category of sets with partial maps,
 Category of objects]
```
By the way, going up the inheritance in the code:

```
sage: sage.modules.vector_space_homspace.VectorSpaceHomspace.mro()
[sage.modules.vector_space_homspace.VectorSpaceHomspace,
 sage.modules.free_module_homspace.FreeModuleHomspace,
 sage.categories.homset.HomsetWithBase,
 sage.categories.homset.Homset,
 sage.structure.parent.Set_generic,
 sage.structure.parent.Parent,
 sage.structure.category_object.CategoryObject,
 sage.structure.sage_object.SageObject,
 object]
```
and reading the code, it's completely unclear how `parent(phi)` got those module categories attached to it. It would have to happen somewhere in `sage.modules.vector_space_homspace.VectorSpaceHomspace, sage.modules.free_module_homspace.FreeModuleHomspace` but none of those classes have an `__init__`.



---

archive/issue_comments_232884.json:
```json
{
    "body": "<a id='comment:3'></a>Replying to [comment:2 nbruin]:\n> That looks like the category machinery is not cooperating with\n> coercion properly. The scalar multiplication should be there\n> according to the categories:\n\n\nWell, categories are doing nothing here but stating the math. This\nissue is really about the concrete class VectorSpaceMorphism for\nmorphisms defined by a matrix which is responsible for implementing\nmultiplication by scalars, and apparently does not.\n\n> By the way, going up the inheritance in the code:\n> \n> ```\n> sage: sage.modules.vector_space_homspace.VectorSpaceHomspace.mro()\n> [sage.modules.vector_space_homspace.VectorSpaceHomspace,\n>  sage.modules.free_module_homspace.FreeModuleHomspace,\n>  sage.categories.homset.HomsetWithBase,\n>  sage.categories.homset.Homset,\n>  sage.structure.parent.Set_generic,\n>  sage.structure.parent.Parent,\n>  sage.structure.category_object.CategoryObject,\n>  sage.structure.sage_object.SageObject,\n>  object]\n> ```\n> and reading the code, it's completely unclear how `parent(phi)` got those module categories attached to it. It would have to happen somewhere in `sage.modules.vector_space_homspace.VectorSpaceHomspace, sage.modules.free_module_homspace.FreeModuleHomspace` but none of those classes have an `__init__`.\n\n\nThe setting of the category for a homset is handled generically in\nHomset.__init__.\n\nCheers,\n\t\t\t\tNicolas",
    "created_at": "2014-08-29T05:47:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16830",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16830#issuecomment-232884",
    "user": "https://github.com/nthiery"
}
```

<a id='comment:3'></a>Replying to [comment:2 nbruin]:
> That looks like the category machinery is not cooperating with
> coercion properly. The scalar multiplication should be there
> according to the categories:


Well, categories are doing nothing here but stating the math. This
issue is really about the concrete class VectorSpaceMorphism for
morphisms defined by a matrix which is responsible for implementing
multiplication by scalars, and apparently does not.

> By the way, going up the inheritance in the code:
> 
> ```
> sage: sage.modules.vector_space_homspace.VectorSpaceHomspace.mro()
> [sage.modules.vector_space_homspace.VectorSpaceHomspace,
>  sage.modules.free_module_homspace.FreeModuleHomspace,
>  sage.categories.homset.HomsetWithBase,
>  sage.categories.homset.Homset,
>  sage.structure.parent.Set_generic,
>  sage.structure.parent.Parent,
>  sage.structure.category_object.CategoryObject,
>  sage.structure.sage_object.SageObject,
>  object]
> ```
> and reading the code, it's completely unclear how `parent(phi)` got those module categories attached to it. It would have to happen somewhere in `sage.modules.vector_space_homspace.VectorSpaceHomspace, sage.modules.free_module_homspace.FreeModuleHomspace` but none of those classes have an `__init__`.


The setting of the category for a homset is handled generically in
Homset.__init__.

Cheers,
				Nicolas



---

archive/issue_comments_232885.json:
```json
{
    "body": "Description changed:\n```diff\n--- \n+++ \n@@ -1,4 +1,46 @@\n+\\`\\`\\`\n+sage: K = GF(7); K\n+Finite Field of size 7\n+sage: sage: phi = End(K^2)([[1,1],[1,1]])\n+sage: phi\n+Vector space morphism represented by the matrix:\n+[1 1]\n+[1 1]\n+Domain: Vector space of dimension 2 over Finite Field of size 7\n+Codomain: Vector space of dimension 2 over Finite Field of size 7\n+sage: 2*phi\n+Vector space morphism represented by the matrix:\n+[2 2]\n+[2 2]\n+Domain: Vector space of dimension 2 over Finite Field of size 7\n+Codomain: Vector space of dimension 2 over Finite Field of size 7\n+\\`\\`\\`\n \n+So far, so good. But:\n+\n+\\`\\`\\`\n+sage: K(2)*phi\n+---------------------------------------------------------------------------\n+TypeError                                 Traceback (most recent call last)\n+<ipython-input-35-81e4c450e31e> in <module>()\n+----> 1 K(Integer(2))*phi\n+\n+/opt/sage-git/local/lib/python2.7/site-packages/sage/structure/element.so in sage.structure.element.RingElement.__mul__ (build/cythonized/sage/structure/element.c:15355)()\n+\n+/opt/sage-git/local/lib/python2.7/site-packages/sage/structure/coerce.so in sage.structure.coerce.CoercionModel_cache_maps.bin_op (build/cythonized/sage/structure/coerce.c:8323)()\n+\n+TypeError: unsupported operand parent(s) for '*': 'Finite Field of size 7' and 'Set of Morphisms (Linear Transformations) from Vector space of dimension 2 over Finite Field of size 7 to Vector space of dimension 2 over Finite Field of size 7'\n+\\`\\`\\`\n+\n+In particular this prevents illustrating Cayley-Hamilton on the morphism side:\n+\n+\\`\\`\\`\n+sage: phi.characteristic_polynomial()(phi)\n+...\n+TypeError: unsupported operand parent(s) for '*': 'Finite Field of size 7' and 'Set of Morphisms (Linear Transformations) from Vector space of dimension 2 over Finite Field of size 7 to Vector space of dimension 2 over Finite Field of size 7'\n+\\`\\`\\`\n+\n+Update 2016/09/20: things have gotten even worse. Now \\`2*phi\\` does not work anymore.\n \n Comment: 1\n \n```\n",
    "created_at": "2016-09-20T21:57:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16830",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16830#issuecomment-232885",
    "user": "https://github.com/nthiery"
}
```

Description changed:
```diff
--- 
+++ 
@@ -1,4 +1,46 @@
+\`\`\`
+sage: K = GF(7); K
+Finite Field of size 7
+sage: sage: phi = End(K^2)([[1,1],[1,1]])
+sage: phi
+Vector space morphism represented by the matrix:
+[1 1]
+[1 1]
+Domain: Vector space of dimension 2 over Finite Field of size 7
+Codomain: Vector space of dimension 2 over Finite Field of size 7
+sage: 2*phi
+Vector space morphism represented by the matrix:
+[2 2]
+[2 2]
+Domain: Vector space of dimension 2 over Finite Field of size 7
+Codomain: Vector space of dimension 2 over Finite Field of size 7
+\`\`\`
 
+So far, so good. But:
+
+\`\`\`
+sage: K(2)*phi
+---------------------------------------------------------------------------
+TypeError                                 Traceback (most recent call last)
+<ipython-input-35-81e4c450e31e> in <module>()
+----> 1 K(Integer(2))*phi
+
+/opt/sage-git/local/lib/python2.7/site-packages/sage/structure/element.so in sage.structure.element.RingElement.__mul__ (build/cythonized/sage/structure/element.c:15355)()
+
+/opt/sage-git/local/lib/python2.7/site-packages/sage/structure/coerce.so in sage.structure.coerce.CoercionModel_cache_maps.bin_op (build/cythonized/sage/structure/coerce.c:8323)()
+
+TypeError: unsupported operand parent(s) for '*': 'Finite Field of size 7' and 'Set of Morphisms (Linear Transformations) from Vector space of dimension 2 over Finite Field of size 7 to Vector space of dimension 2 over Finite Field of size 7'
+\`\`\`
+
+In particular this prevents illustrating Cayley-Hamilton on the morphism side:
+
+\`\`\`
+sage: phi.characteristic_polynomial()(phi)
+...
+TypeError: unsupported operand parent(s) for '*': 'Finite Field of size 7' and 'Set of Morphisms (Linear Transformations) from Vector space of dimension 2 over Finite Field of size 7 to Vector space of dimension 2 over Finite Field of size 7'
+\`\`\`
+
+Update 2016/09/20: things have gotten even worse. Now \`2*phi\` does not work anymore.
 
 Comment: 1
 
```




---

archive/issue_comments_232886.json:
```json
{
    "body": "<a id='comment:5'></a>Having a brief look at the code, `MatrixMorphism` implements a `__mul__` method to handle multiplication by other types of morphisms. But this prevents the coercion model to take over when needed!\n\nInstead, `MatrixMorphism` should implement `_mul_` for basic multiplication, handle multiplication by other types of morphisms by coercion, and implement `_lmul_` or `_acted_upon` (I never quite remember from the top of my head which one should be used in this situation) to implement multiplication by scalar).",
    "created_at": "2016-09-20T22:02:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16830",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16830#issuecomment-232886",
    "user": "https://github.com/nthiery"
}
```

<a id='comment:5'></a>Having a brief look at the code, `MatrixMorphism` implements a `__mul__` method to handle multiplication by other types of morphisms. But this prevents the coercion model to take over when needed!

Instead, `MatrixMorphism` should implement `_mul_` for basic multiplication, handle multiplication by other types of morphisms by coercion, and implement `_lmul_` or `_acted_upon` (I never quite remember from the top of my head which one should be used in this situation) to implement multiplication by scalar).



---

archive/issue_events_049266.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-09-20T22:39:21Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16830",
    "milestone": "sage-7.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16830#event-49266"
}
```



---

archive/issue_events_049267.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-17T03:38:32Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16830",
    "milestone": "sage-7.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16830#event-49267"
}
```



---

archive/issue_events_049268.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-17T03:38:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16830",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16830#event-49268"
}
```



---

archive/issue_events_049269.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16830",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16830#event-49269"
}
```



---

archive/issue_events_049270.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16830",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16830#event-49270"
}
```



---

archive/issue_comments_232887.json:
```json
{
    "body": "<a id='comment:8'></a>Setting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-02-13T20:51:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16830",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16830#issuecomment-232887",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:8'></a>Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_049271.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16830",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16830#event-49271"
}
```



---

archive/issue_events_049272.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16830",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16830#event-49272"
}
```



---

archive/issue_events_049273.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16830",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16830#event-49273"
}
```



---

archive/issue_events_049274.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16830",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16830#event-49274"
}
```



---

archive/issue_events_049275.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-03T15:17:06Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16830",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16830#event-49275"
}
```



---

archive/issue_events_049276.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-03T15:17:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16830",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16830#event-49276"
}
```



---

archive/issue_events_049277.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16830",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16830#event-49277"
}
```



---

archive/issue_events_049278.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16830",
    "milestone": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16830#event-49278"
}
```
