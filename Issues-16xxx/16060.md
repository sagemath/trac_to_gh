# Issue 16060: package patch mechanism can't patch spkg-install inside spkg

archive/issues_015823.json:
```json
{
    "body": "See the scenario in #15814 where the original tarball is an SPKG with defect `spkg-install`. A possible fix in `sage-spkg` would be to first check inside `build/pkgs/packagename` for the `spkg-install` to run.\n\nReviewer: Jeroen Demeyer\n\nResolution: invalid\n\nIssue created by migration from https://trac.sagemath.org/ticket/16060\n\n",
    "closed_at": "2014-04-13T14:55:17Z",
    "created_at": "2014-04-05T07:30:13Z",
    "labels": [
        "component: scripts",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "package patch mechanism can't patch spkg-install inside spkg",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/16060",
    "user": "https://github.com/rwst"
}
```
See the scenario in #15814 where the original tarball is an SPKG with defect `spkg-install`. A possible fix in `sage-spkg` would be to first check inside `build/pkgs/packagename` for the `spkg-install` to run.

Reviewer: Jeroen Demeyer

Resolution: invalid

Issue created by migration from https://trac.sagemath.org/ticket/16060





---

archive/issue_comments_261003.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Jeroen Demeyer\"",
    "created_at": "2014-04-12T12:14:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16060",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16060#issuecomment-261003",
    "user": "https://github.com/jdemeyer"
}
```

Changing reviewer from "" to "Jeroen Demeyer"



---

archive/issue_comments_261004.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2014-04-12T12:14:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16060",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16060#issuecomment-261004",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_261005.json:
```json
{
    "body": "<a id='comment:1'></a>`spkg-install` is a file provided by Sage. No need to add a patch, just edit the file.\n\nClose as invalid.",
    "created_at": "2014-04-12T12:14:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16060",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16060#issuecomment-261005",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:1'></a>`spkg-install` is a file provided by Sage. No need to add a patch, just edit the file.

Close as invalid.



---

archive/issue_events_047215.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-04-12T12:14:22Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16060",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16060#event-47215"
}
```



---

archive/issue_comments_261006.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2014-04-12T12:14:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16060",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16060#issuecomment-261006",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_261007.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2014-04-13T14:55:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16060",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16060#issuecomment-261007",
    "user": "https://github.com/vbraun"
}
```

Resolution: invalid



---

archive/issue_events_047216.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-04-13T14:55:17Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/16060",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16060#event-47216"
}
```
