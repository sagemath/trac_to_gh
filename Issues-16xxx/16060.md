# Issue 16060: package patch mechanism can't patch spkg-install inside spkg

archive/issues_015823.json:
```json
{
    "assignees": [],
    "body": "See the scenario in #15814 where the original tarball is an SPKG with defect `spkg-install`. A possible fix in `sage-spkg` would be to first check inside `build/pkgs/packagename` for the `spkg-install` to run.\n\nReviewer: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16060_\n\n",
    "closed_at": "2014-04-13T14:55:17Z",
    "created_at": "2014-04-05T07:30:13Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20scripts",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "package patch mechanism can't patch spkg-install inside spkg",
    "type": "issue",
    "updated_at": "2014-04-13T14:55:17Z",
    "url": "https://github.com/sagemath/sage/issues/16060",
    "user": "https://github.com/rwst"
}
```
See the scenario in #15814 where the original tarball is an SPKG with defect `spkg-install`. A possible fix in `sage-spkg` would be to first check inside `build/pkgs/packagename` for the `spkg-install` to run.

Reviewer: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/16060_





---

archive/issue_events_211693.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-04-05T07:30:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16060",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16060#event-211693"
}
```



---

archive/issue_events_211694.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-04-05T07:30:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16060",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20scripts",
    "label_color": "000080",
    "label_name": "component: scripts",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16060#event-211694"
}
```



---

archive/issue_events_211695.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-04-05T07:30:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16060",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16060#event-211695"
}
```



---

archive/issue_events_211696.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-04-05T07:30:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16060",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16060#event-211696"
}
```



---

archive/issue_comments_207007.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2014-04-12T12:14:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16060",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16060#issuecomment-207007",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_211697.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-04-12T12:14:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16060",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16060#event-211697"
}
```



---

archive/issue_comments_207008.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\n`spkg-install` is a file provided by Sage. No need to add a patch, just edit the file.\n\nClose as invalid.",
    "created_at": "2014-04-12T12:14:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16060",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16060#issuecomment-207008",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:1'>Comment 1:</a>
`spkg-install` is a file provided by Sage. No need to add a patch, just edit the file.

Close as invalid.



---

archive/issue_events_211698.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-04-12T12:14:22Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16060",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16060#event-211698"
}
```



---

archive/issue_events_211699.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-04-12T12:14:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16060",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16060#event-211699"
}
```



---

archive/issue_events_211700.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-04-12T12:14:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16060",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16060#event-211700"
}
```



---

archive/issue_events_211701.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-04-12T12:14:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16060",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16060#event-211701"
}
```



---

archive/issue_events_211702.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-04-12T12:14:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16060",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16060#event-211702"
}
```



---

archive/issue_events_211703.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-04-13T14:55:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16060",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16060#event-211703"
}
```



---

archive/issue_events_211704.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-04-13T14:55:17Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16060",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16060#event-211704"
}
```
