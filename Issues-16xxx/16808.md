# Issue 16808: PDF error in italian: "not a shorthand"

archive/issues_016571.json:
```json
{
    "assignees": [],
    "body": "\n```\n! Package babel Error: I cannot switch `\"' on or off--not a shorthand.\n\nSee the babel package documentation for explanation.\nType  H <return>  for immediate help.\n ...                                              \n                                                  \nl.145 \\shorthandoff{\"}\n                      \n? \n! Emergency stop.\n ...                                              \n                                                  \nl.145 \\shorthandoff{\"}\n                      \n!  ==> Fatal error occurred, no output PDF file produced!\nTranscript written on a_tour_of_sage.log.\nmake: *** [a_tour_of_sage.pdf] Error 1\nError building the documentation.\n```\n\nCC:  @jhpalmieri\n\nBranch/Commit: **[c475401](https://github.com/sagemath/sagetrac-mirror/commit/c475401f29e849a67e5d7c7f5eab7ec96d807daa)**\n\nReviewer: **Karl-Dieter Crisman, John Palmieri**\n\nAuthor: **Volker Braun**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16808_\n\n",
    "closed_at": "2014-08-16T09:32:13Z",
    "created_at": "2014-08-12T21:58:44Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "PDF error in italian: \"not a shorthand\"",
    "type": "issue",
    "updated_at": "2014-08-16T09:32:13Z",
    "url": "https://github.com/sagemath/sage/issues/16808",
    "user": "https://github.com/vbraun"
}
```

```
! Package babel Error: I cannot switch `"' on or off--not a shorthand.

See the babel package documentation for explanation.
Type  H <return>  for immediate help.
 ...                                              
                                                  
l.145 \shorthandoff{"}
                      
? 
! Emergency stop.
 ...                                              
                                                  
l.145 \shorthandoff{"}
                      
!  ==> Fatal error occurred, no output PDF file produced!
Transcript written on a_tour_of_sage.log.
make: *** [a_tour_of_sage.pdf] Error 1
Error building the documentation.
```

CC:  @jhpalmieri

Branch/Commit: **[c475401](https://github.com/sagemath/sagetrac-mirror/commit/c475401f29e849a67e5d7c7f5eab7ec96d807daa)**

Reviewer: **Karl-Dieter Crisman, John Palmieri**

Author: **Volker Braun**

_Issue created by migration from https://trac.sagemath.org/ticket/16808_





---

archive/issue_events_222669.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-08-12T21:58:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16808",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16808#event-222669"
}
```



---

archive/issue_events_222670.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-08-12T21:58:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16808",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16808#event-222670"
}
```



---

archive/issue_events_222671.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-08-12T21:58:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16808",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16808#event-222671"
}
```



---

archive/issue_events_222672.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-08-12T21:58:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16808",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16808#event-222672"
}
```



---

archive/issue_comments_222922.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nSee also: #10149\n\nI can reproduce this with TeXlive-2013",
    "created_at": "2014-08-12T21:59:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16808",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16808#issuecomment-222922",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:2" align="right">Comment 2</div>

See also: #10149

I can reproduce this with TeXlive-2013



---

archive/issue_comments_222923.json:
```json
{
    "body": "Branch: **[u/vbraun/pdf_it_shorthand](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/pdf_it_shorthand)**",
    "created_at": "2014-08-12T22:34:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16808",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16808#issuecomment-222923",
    "user": "https://github.com/vbraun"
}
```

Branch: **[u/vbraun/pdf_it_shorthand](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/pdf_it_shorthand)**



---

archive/issue_comments_222924.json:
```json
{
    "body": "Commit: **[8e4bddd](https://github.com/sagemath/sagetrac-mirror/commit/8e4bdddfcf264001f00fea644bb1be713aa0224b)**",
    "created_at": "2014-08-12T22:36:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16808",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16808#issuecomment-222924",
    "user": "https://github.com/vbraun"
}
```

Commit: **[8e4bddd](https://github.com/sagemath/sagetrac-mirror/commit/8e4bdddfcf264001f00fea644bb1be713aa0224b)**



---

archive/issue_comments_222925.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nSame error with TexLive-2014. The latex docs tell us to use `\\setactivedoublequote`\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8e4bdddfcf264001f00fea644bb1be713aa0224b\">8e4bddd</a></td><td><code>Activate double quote in italian LaTeX</code></td></tr></table>\n",
    "created_at": "2014-08-12T22:36:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16808",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16808#issuecomment-222925",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:4" align="right">Comment 4</div>

Same error with TexLive-2014. The latex docs tell us to use `\setactivedoublequote`

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8e4bdddfcf264001f00fea644bb1be713aa0224b">8e4bddd</a></td><td><code>Activate double quote in italian LaTeX</code></td></tr></table>




---

archive/issue_comments_222926.json:
```json
{
    "body": "Changed commit from **[8e4bddd](https://github.com/sagemath/sagetrac-mirror/commit/8e4bdddfcf264001f00fea644bb1be713aa0224b)** to **[c475401](https://github.com/sagemath/sagetrac-mirror/commit/c475401f29e849a67e5d7c7f5eab7ec96d807daa)**",
    "created_at": "2014-08-12T22:44:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16808",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16808#issuecomment-222926",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[8e4bddd](https://github.com/sagemath/sagetrac-mirror/commit/8e4bdddfcf264001f00fea644bb1be713aa0224b)** to **[c475401](https://github.com/sagemath/sagetrac-mirror/commit/c475401f29e849a67e5d7c7f5eab7ec96d807daa)**



---

archive/issue_comments_222927.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c475401f29e849a67e5d7c7f5eab7ec96d807daa\">c475401</a></td><td><code>Provide fallback for older babel-italian versions</code></td></tr></table>\n",
    "created_at": "2014-08-12T22:44:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16808",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16808#issuecomment-222927",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:5" align="right">Comment 5</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c475401f29e849a67e5d7c7f5eab7ec96d807daa">c475401</a></td><td><code>Provide fallback for older babel-italian versions</code></td></tr></table>




---

archive/issue_comments_222928.json:
```json
{
    "body": "Author: **Volker Braun**",
    "created_at": "2014-08-12T22:50:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16808",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16808#issuecomment-222928",
    "user": "https://github.com/vbraun"
}
```

Author: **Volker Braun**



---

archive/issue_comments_222929.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nThis now works in both old and new-style babel-italian.",
    "created_at": "2014-08-12T22:50:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16808",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16808#issuecomment-222929",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:6" align="right">Comment 6</div>

This now works in both old and new-style babel-italian.



---

archive/issue_events_222673.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-08-12T22:50:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16808",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16808#event-222673"
}
```



---

archive/issue_comments_222930.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nThis works for me (at least, it compiles and looks okay) with TeXLive-2012.  I don't have 2013 or 2014 but I can verify it doesn't regress.  The code is correct assuming the command is the right one.  Is that enough for positive review, or should John verify it works for him?\n\nBy the way, I don't see this or the Turkish or Portuguese version of this at http://www.sagemath.org/help.html#SageStandardDoc ...",
    "created_at": "2014-08-15T10:14:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16808",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16808#issuecomment-222930",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:7" align="right">Comment 7</div>

This works for me (at least, it compiles and looks okay) with TeXLive-2012.  I don't have 2013 or 2014 but I can verify it doesn't regress.  The code is correct assuming the command is the right one.  Is that enough for positive review, or should John verify it works for him?

By the way, I don't see this or the Turkish or Portuguese version of this at http://www.sagemath.org/help.html#SageStandardDoc ...



---

archive/issue_comments_222931.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nWorks for me, too.",
    "created_at": "2014-08-16T02:54:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16808",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16808#issuecomment-222931",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:8" align="right">Comment 8</div>

Works for me, too.



---

archive/issue_events_222674.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2014-08-16T02:54:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16808",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16808#event-222674"
}
```



---

archive/issue_events_222675.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2014-08-16T02:54:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16808",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16808#event-222675"
}
```



---

archive/issue_comments_222932.json:
```json
{
    "body": "Reviewer: **Karl-Dieter Crisman, John Palmieri**",
    "created_at": "2014-08-16T02:54:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16808",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16808#issuecomment-222932",
    "user": "https://github.com/jhpalmieri"
}
```

Reviewer: **Karl-Dieter Crisman, John Palmieri**



---

archive/issue_events_222676.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-08-16T09:32:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16808",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16808#event-222676"
}
```



---

archive/issue_events_222677.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "49f193e02f054db129da6384ec530a34227cde70",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-08-16T09:32:13Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16808",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16808#event-222677"
}
```



---

archive/issue_comments_222933.json:
```json
{
    "body": "Changed branch from **[u/vbraun/pdf_it_shorthand](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/pdf_it_shorthand)** to **[c475401](https://github.com/sagemath/sagetrac-mirror/commit/c475401f29e849a67e5d7c7f5eab7ec96d807daa)**",
    "created_at": "2014-08-16T09:32:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16808",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16808#issuecomment-222933",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/vbraun/pdf_it_shorthand](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/pdf_it_shorthand)** to **[c475401](https://github.com/sagemath/sagetrac-mirror/commit/c475401f29e849a67e5d7c7f5eab7ec96d807daa)**
