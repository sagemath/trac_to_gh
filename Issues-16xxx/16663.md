# Issue 16663: New common transducers all and any

archive/issues_016426.json:
```json
{
    "assignees": [],
    "body": "Analogously to the `add` transducer, two common transducers for logical `and` and `or`, falling back to python's `all` and `any` and hence also named in that way.\n\nCC:  @sagetrac-skropf @dkrenn\n\nKeywords: **finite_state_machine, generators**\n\nBranch/Commit: **[dbddb31](https://github.com/sagemath/sagetrac-mirror/commit/dbddb31334c9d60e9e30cdb8a689e0e41b8f88e6)**\n\nReviewer: **Daniel Krenn, Clemens Heuberger**\n\nAuthor: **Clemens Heuberger, Daniel Krenn**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16663_\n\n",
    "closed_at": "2014-07-19T04:57:47Z",
    "created_at": "2014-07-16T13:28:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20finite%20state%20machines",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "New common transducers all and any",
    "type": "issue",
    "updated_at": "2014-07-19T04:57:47Z",
    "url": "https://github.com/sagemath/sage/issues/16663",
    "user": "https://github.com/cheuberg"
}
```
Analogously to the `add` transducer, two common transducers for logical `and` and `or`, falling back to python's `all` and `any` and hence also named in that way.

CC:  @sagetrac-skropf @dkrenn

Keywords: **finite_state_machine, generators**

Branch/Commit: **[dbddb31](https://github.com/sagemath/sagetrac-mirror/commit/dbddb31334c9d60e9e30cdb8a689e0e41b8f88e6)**

Reviewer: **Daniel Krenn, Clemens Heuberger**

Author: **Clemens Heuberger, Daniel Krenn**

_Issue created by migration from https://trac.sagemath.org/ticket/16663_





---

archive/issue_events_220639.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-07-16T13:28:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16663",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16663#event-220639"
}
```



---

archive/issue_events_220640.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-07-16T13:28:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16663",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20finite%20state%20machines",
    "label_color": "0000ff",
    "label_name": "component: finite state machines",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16663#event-220640"
}
```



---

archive/issue_events_220641.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-07-16T13:28:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16663",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16663#event-220641"
}
```



---

archive/issue_events_220642.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-07-16T13:28:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16663",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16663#event-220642"
}
```



---

archive/issue_events_220643.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-07-16T13:28:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16663",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16663#event-220643"
}
```



---

archive/issue_comments_220045.json:
```json
{
    "body": "Changed branch from **[u/cheuberg/fsm/all-any](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/all-any)** to **[u/dkrenn/fsm/all-any](https://github.com/sagemath/sagetrac-mirror/tree/u/dkrenn/fsm/all-any)**",
    "created_at": "2014-07-18T08:54:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16663",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16663#issuecomment-220045",
    "user": "https://github.com/dkrenn"
}
```

Changed branch from **[u/cheuberg/fsm/all-any](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/all-any)** to **[u/dkrenn/fsm/all-any](https://github.com/sagemath/sagetrac-mirror/tree/u/dkrenn/fsm/all-any)**



---

archive/issue_comments_220046.json:
```json
{
    "body": "Changed author from **Clemens Heuberger** to **Clemens Heuberger, Daniel Krenn**",
    "created_at": "2014-07-18T08:56:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16663",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16663#issuecomment-220046",
    "user": "https://github.com/dkrenn"
}
```

Changed author from **Clemens Heuberger** to **Clemens Heuberger, Daniel Krenn**



---

archive/issue_comments_220047.json:
```json
{
    "body": "Reviewer: **Daniel Krenn**",
    "created_at": "2014-07-18T08:56:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16663",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16663#issuecomment-220047",
    "user": "https://github.com/dkrenn"
}
```

Reviewer: **Daniel Krenn**



---

archive/issue_comments_220048.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nRewrote code: a function with def is now a lambda. Needs cross-review.\n\nApart from this, code and docs looks fine, tests pass. Give it a positive review, when positive cross-reviewed.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/dbddb31334c9d60e9e30cdb8a689e0e41b8f88e6\">dbddb31</a></td><td><code>make def logical_and, def logical_or inline (lambda) functions</code></td></tr></table>\n",
    "created_at": "2014-07-18T08:56:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16663",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16663#issuecomment-220048",
    "user": "https://github.com/dkrenn"
}
```

<div id="comment:3" align="right">Comment 3</div>

Rewrote code: a function with def is now a lambda. Needs cross-review.

Apart from this, code and docs looks fine, tests pass. Give it a positive review, when positive cross-reviewed.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/dbddb31334c9d60e9e30cdb8a689e0e41b8f88e6">dbddb31</a></td><td><code>make def logical_and, def logical_or inline (lambda) functions</code></td></tr></table>




---

archive/issue_comments_220049.json:
```json
{
    "body": "Changed commit from **[d9c48e1](https://github.com/sagemath/sagetrac-mirror/commit/d9c48e12ae093121eb5977c12e75cc1eee53dd54)** to **[dbddb31](https://github.com/sagemath/sagetrac-mirror/commit/dbddb31334c9d60e9e30cdb8a689e0e41b8f88e6)**",
    "created_at": "2014-07-18T08:56:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16663",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16663#issuecomment-220049",
    "user": "https://github.com/dkrenn"
}
```

Changed commit from **[d9c48e1](https://github.com/sagemath/sagetrac-mirror/commit/d9c48e12ae093121eb5977c12e75cc1eee53dd54)** to **[dbddb31](https://github.com/sagemath/sagetrac-mirror/commit/dbddb31334c9d60e9e30cdb8a689e0e41b8f88e6)**



---

archive/issue_comments_220050.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\ncross-reviewed; setting to positive_review as instructed.",
    "created_at": "2014-07-18T09:29:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16663",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16663#issuecomment-220050",
    "user": "https://github.com/cheuberg"
}
```

<div id="comment:4" align="right">Comment 4</div>

cross-reviewed; setting to positive_review as instructed.



---

archive/issue_comments_220051.json:
```json
{
    "body": "Changed reviewer from **Daniel Krenn** to **Daniel Krenn, Clemens Heuberger**",
    "created_at": "2014-07-18T09:29:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16663",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16663#issuecomment-220051",
    "user": "https://github.com/cheuberg"
}
```

Changed reviewer from **Daniel Krenn** to **Daniel Krenn, Clemens Heuberger**



---

archive/issue_events_220644.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-07-18T09:29:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16663",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16663#event-220644"
}
```



---

archive/issue_events_220645.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-07-18T09:29:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16663",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16663#event-220645"
}
```



---

archive/issue_comments_220052.json:
```json
{
    "body": "Changed branch from **[u/dkrenn/fsm/all-any](https://github.com/sagemath/sagetrac-mirror/tree/u/dkrenn/fsm/all-any)** to **[dbddb31](https://github.com/sagemath/sagetrac-mirror/commit/dbddb31334c9d60e9e30cdb8a689e0e41b8f88e6)**",
    "created_at": "2014-07-19T04:57:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16663",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16663#issuecomment-220052",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/dkrenn/fsm/all-any](https://github.com/sagemath/sagetrac-mirror/tree/u/dkrenn/fsm/all-any)** to **[dbddb31](https://github.com/sagemath/sagetrac-mirror/commit/dbddb31334c9d60e9e30cdb8a689e0e41b8f88e6)**



---

archive/issue_events_220646.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-07-19T04:57:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16663",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16663#event-220646"
}
```



---

archive/issue_events_220647.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "fafe02831bd62ea14b32daf50640343923d042a1",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-07-19T04:57:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16663",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16663#event-220647"
}
```
