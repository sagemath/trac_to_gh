# Issue 16536: Python 3 preparation: cmp parameter of sorted() function and L.sort() method are gone

archive/issues_016299.json:
```json
{
    "body": "In Py2 the standard function [sorted()](https://docs.python.org/2/library/functions.html#sorted) and the mutable list method [sort()](https://docs.python.org/2/library/stdtypes.html#mutable-sequence-types) both have an optional parameter `cmp=` that takes a custom comparison function of two arguments.\n\nThe parameter is no more available in Py3. \n\nThe recommended way is to use the `key=` which specifies a function of one argument that is used to extract a comparison key from each list element. This has also better performance.\n\nWhile there is a function [functools.cmp_to_key()](https://docs.python.org/2/library/functools.html#functools.cmp_to_key) to convert an old-style cmp function to a key function, it is preferable to directly use an appropriate key function.\n\nThis ticket is tracked as a dependency of meta-ticket ticket:15980.\n\nCC:  @tscrim @embray @jdemeyer\n\nKeywords: python3\n\nDependencies: #16853, #17200, #17229\n\nResolution: wontfix\n\nIssue created by migration from https://trac.sagemath.org/ticket/16536\n\n",
    "closed_at": "2018-01-10T13:20:42Z",
    "created_at": "2014-06-25T13:17:43Z",
    "labels": [
        "component: python3"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "Python 3 preparation: cmp parameter of sorted() function and L.sort() method are gone",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/16536",
    "user": "https://github.com/wluebbe"
}
```
In Py2 the standard function [sorted()](https://docs.python.org/2/library/functions.html#sorted) and the mutable list method [sort()](https://docs.python.org/2/library/stdtypes.html#mutable-sequence-types) both have an optional parameter `cmp=` that takes a custom comparison function of two arguments.

The parameter is no more available in Py3. 

The recommended way is to use the `key=` which specifies a function of one argument that is used to extract a comparison key from each list element. This has also better performance.

While there is a function [functools.cmp_to_key()](https://docs.python.org/2/library/functools.html#functools.cmp_to_key) to convert an old-style cmp function to a key function, it is preferable to directly use an appropriate key function.

This ticket is tracked as a dependency of meta-ticket ticket:15980.

CC:  @tscrim @embray @jdemeyer

Keywords: python3

Dependencies: #16853, #17200, #17229

Resolution: wontfix

Issue created by migration from https://trac.sagemath.org/ticket/16536





---

archive/issue_comments_273122.json:
```json
{
    "body": "Demonstrate behavior of cmp= and key= in Py2 and Py3",
    "created_at": "2014-06-27T14:31:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16536",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16536#issuecomment-273122",
    "user": "https://github.com/wluebbe"
}
```

Demonstrate behavior of cmp= and key= in Py2 and Py3



---

archive/issue_comments_273123.json:
```json
{
    "body": "<a id='comment:1'></a>Attachment [exercise-cmp-key-parameters.py](tarball://root/attachments/some-uuid/ticket16536/exercise-cmp-key-parameters.py) by @wluebbe created at 2014-06-27 14:46:13\n\nThe workings of `functools.cmp_to_key` is nicely explained in [stackoverflow](http://stackoverflow.com/questions/16362744/how-does-pythons-cmp-to-key-function-work).\n\nThis is also demonstrated in the attached script.\n\nIt is also shown that it is not a good idea the use \"cmp=\" and \"key=\" together even in Py2!",
    "created_at": "2014-06-27T14:46:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16536",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16536#issuecomment-273123",
    "user": "https://github.com/wluebbe"
}
```

<a id='comment:1'></a>Attachment [exercise-cmp-key-parameters.py](tarball://root/attachments/some-uuid/ticket16536/exercise-cmp-key-parameters.py) by @wluebbe created at 2014-06-27 14:46:13

The workings of `functools.cmp_to_key` is nicely explained in [stackoverflow](http://stackoverflow.com/questions/16362744/how-does-pythons-cmp-to-key-function-work).

This is also demonstrated in the attached script.

It is also shown that it is not a good idea the use "cmp=" and "key=" together even in Py2!



---

archive/issue_events_048545.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16536",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16536#event-48545"
}
```



---

archive/issue_comments_273124.json:
```json
{
    "body": "Changing dependencies from \"\" to \"#16853\"",
    "created_at": "2014-08-19T13:05:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16536",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16536#issuecomment-273124",
    "user": "https://github.com/a-andre"
}
```

Changing dependencies from "" to "#16853"



---

archive/issue_comments_273125.json:
```json
{
    "body": "Changing dependencies from \"#16853\" to \"#16853, #17200\"",
    "created_at": "2014-10-22T20:30:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16536",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16536#issuecomment-273125",
    "user": "https://github.com/a-andre"
}
```

Changing dependencies from "#16853" to "#16853, #17200"



---

archive/issue_comments_273126.json:
```json
{
    "body": "Changing dependencies from \"#16853, #17200\" to \"#16853, #17200, #17229\"",
    "created_at": "2014-10-26T21:32:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16536",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16536#issuecomment-273126",
    "user": "https://github.com/a-andre"
}
```

Changing dependencies from "#16853, #17200" to "#16853, #17200, #17229"



---

archive/issue_comments_273127.json:
```json
{
    "body": "Changing component from distribution to python3.",
    "created_at": "2016-05-02T13:32:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16536",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16536#issuecomment-273127",
    "user": "https://github.com/jdemeyer"
}
```

Changing component from distribution to python3.



---

archive/issue_comments_273128.json:
```json
{
    "body": "<a id='comment:7'></a>see #21035 for a step in that direction",
    "created_at": "2016-07-17T12:26:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16536",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16536#issuecomment-273128",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:7'></a>see #21035 for a step in that direction



---

archive/issue_events_048546.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-09-23T06:08:43Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16536",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16536#event-48546"
}
```



---

archive/issue_events_048547.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-09-23T06:08:43Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16536",
    "milestone": "sage-7.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16536#event-48547"
}
```



---

archive/issue_comments_273129.json:
```json
{
    "body": "Changing type from PLEASE CHANGE to enhancement.",
    "created_at": "2016-09-23T06:08:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16536",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16536#issuecomment-273129",
    "user": "https://github.com/fchapoton"
}
```

Changing type from PLEASE CHANGE to enhancement.



---

archive/issue_comments_273130.json:
```json
{
    "body": "<a id='comment:9'></a>This is now mostly done. In some places, cmp was only deprecated.",
    "created_at": "2016-11-12T17:02:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16536",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16536#issuecomment-273130",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:9'></a>This is now mostly done. In some places, cmp was only deprecated.



---

archive/issue_events_048548.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-01-12T18:05:00Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16536",
    "milestone": "sage-7.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16536#event-48548"
}
```



---

archive/issue_events_048549.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-01-12T18:05:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16536",
    "milestone": "sage-7.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16536#event-48549"
}
```



---

archive/issue_events_048550.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-05-01T12:52:44Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16536",
    "milestone": "sage-7.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16536#event-48550"
}
```



---

archive/issue_events_048551.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-05-01T12:52:44Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16536",
    "milestone": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16536#event-48551"
}
```



---

archive/issue_comments_273131.json:
```json
{
    "body": "<a id='comment:12'></a>another step in #23698",
    "created_at": "2017-08-24T13:04:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16536",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16536#issuecomment-273131",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:12'></a>another step in #23698



---

archive/issue_comments_273132.json:
```json
{
    "body": "<a id='comment:13'></a>One should soon take care of removing what was deprecated in #21766.\n\nSee #24409 for that",
    "created_at": "2017-11-16T19:55:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16536",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16536#issuecomment-273132",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:13'></a>One should soon take care of removing what was deprecated in #21766.

See #24409 for that



---

archive/issue_comments_273133.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2018-01-04T10:08:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16536",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16536#issuecomment-273133",
    "user": "https://github.com/fchapoton"
}
```

Changing status from new to needs_review.



---

archive/issue_events_048552.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-01-04T10:08:34Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16536",
    "milestone": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16536#event-48552"
}
```



---

archive/issue_events_048553.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-01-04T10:08:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16536",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16536#event-48553"
}
```



---

archive/issue_comments_273134.json:
```json
{
    "body": "<a id='comment:14'></a>Maybe we could now close this one ?",
    "created_at": "2018-01-04T10:08:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16536",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16536#issuecomment-273134",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:14'></a>Maybe we could now close this one ?



---

archive/issue_comments_273135.json:
```json
{
    "body": "<a id='comment:15'></a>ping ? any objection to close as invalid ?",
    "created_at": "2018-01-10T13:18:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16536",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16536#issuecomment-273135",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:15'></a>ping ? any objection to close as invalid ?



---

archive/issue_events_048554.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-01-10T13:20:42Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/16536",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16536#event-48554"
}
```



---

archive/issue_comments_273136.json:
```json
{
    "body": "Resolution: wontfix",
    "created_at": "2018-01-10T13:20:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16536",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16536#issuecomment-273136",
    "user": "https://github.com/embray"
}
```

Resolution: wontfix
