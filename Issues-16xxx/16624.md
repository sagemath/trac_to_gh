...
```json
{
    "body": "Attachment [0001-Update-the-sympy-package.patch.gz](tarball://root/attachments/some-uuid/ticket16624/0001-Update-the-sympy-package.patch.gz) by skirpichev created at 2014-09-30 16:14:19\n\nIf you include patch to unbundle mpmath (see attached patch) - it will solve your problem.",
...
```json
{
    "body": "Your big patch is unreadable - too big. most of it is the removal of mpmath sources from sympy. This could be achieved in spkg-src by doing a \"rm -rf\" in the appropriate place. Once that's out of the way we can look at the patch to do the rest and it won't be 2.1MB and we'll be able to see something.",
...
```json
{
    "body": "It looks like the only file not being deleted is `mpmath/tests/__init__.py`.",
...
```json
{
    "body": "More can be deleted (mpmath docs). This passes `make ptestlong`.",
