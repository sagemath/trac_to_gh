# Issue 16634: Expand conversions to RR

archive/issues_016397.json:
```json
{
    "assignees": [],
    "body": "If we can't do a conversion to `RR` as is, try to see if an element can convert to `QQ` first. In particular, this allows conversion from the universal cyclotomic field.\n\n```\nsage: UCF = UniversalCyclotomicField()\nsage: RR(UCF(1/2)) # Boom currently\n```\n\nAssignee: **@tscrim**\n\nCC:  @mezzarobba\n\nKeywords: **conversion RR**\n\nBranch/Commit: **[public/rings/expand_RR_conversion-16634-rebased](https://github.com/sagemath/sagetrac-mirror/tree/public/rings/expand_RR_conversion-16634-rebased) @ [98f77ec](https://github.com/sagemath/sagetrac-mirror/commit/98f77ecaae42d66b9ce66ad9ff92c9f56f5b0991)**\n\nAuthor: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16634_\n\n",
    "created_at": "2014-07-09T06:11:02Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20coercion",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/needs%20work"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Expand conversions to RR",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/16634",
    "user": "https://github.com/tscrim"
}
```
If we can't do a conversion to `RR` as is, try to see if an element can convert to `QQ` first. In particular, this allows conversion from the universal cyclotomic field.

```
sage: UCF = UniversalCyclotomicField()
sage: RR(UCF(1/2)) # Boom currently
```

Assignee: **@tscrim**

CC:  @mezzarobba

Keywords: **conversion RR**

Branch/Commit: **[public/rings/expand_RR_conversion-16634-rebased](https://github.com/sagemath/sagetrac-mirror/tree/public/rings/expand_RR_conversion-16634-rebased) @ [98f77ec](https://github.com/sagemath/sagetrac-mirror/commit/98f77ecaae42d66b9ce66ad9ff92c9f56f5b0991)**

Author: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/16634_





---

archive/issue_events_220219.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-07-09T06:11:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16634",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16634#event-220219"
}
```



---

archive/issue_events_220220.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-07-09T06:11:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16634",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20coercion",
    "label_color": "0000ff",
    "label_name": "component: coercion",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16634#event-220220"
}
```



---

archive/issue_events_220221.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-07-09T06:11:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16634",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16634#event-220221"
}
```



---

archive/issue_events_220222.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-07-09T06:11:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16634",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16634#event-220222"
}
```



---

archive/issue_comments_219419.json:
```json
{
    "body": "Commit: **[78cfb38](https://github.com/sagemath/sagetrac-mirror/commit/78cfb381b90d28dcbc6301c539dd293d5910b7f2)**",
    "created_at": "2014-07-09T06:27:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16634",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16634#issuecomment-219419",
    "user": "https://github.com/tscrim"
}
```

Commit: **[78cfb38](https://github.com/sagemath/sagetrac-mirror/commit/78cfb381b90d28dcbc6301c539dd293d5910b7f2)**



---

archive/issue_comments_219420.json:
```json
{
    "body": "Branch: **[public/rings/expand_RR_conversion-16634](https://github.com/sagemath/sagetrac-mirror/tree/public/rings/expand_RR_conversion-16634)**",
    "created_at": "2014-07-09T06:27:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16634",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16634#issuecomment-219420",
    "user": "https://github.com/tscrim"
}
```

Branch: **[public/rings/expand_RR_conversion-16634](https://github.com/sagemath/sagetrac-mirror/tree/public/rings/expand_RR_conversion-16634)**



---

archive/issue_comments_219421.json:
```json
{
    "body": "Changed keywords from **** to **conversion RR**",
    "created_at": "2014-07-09T06:27:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16634",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16634#issuecomment-219421",
    "user": "https://github.com/tscrim"
}
```

Changed keywords from **** to **conversion RR**



---

archive/issue_comments_219422.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/78cfb381b90d28dcbc6301c539dd293d5910b7f2\">78cfb38</a></td><td><code>Expanded conversions for real numbers.</code></td></tr></table>\n",
    "created_at": "2014-07-09T06:27:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16634",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16634#issuecomment-219422",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/78cfb381b90d28dcbc6301c539dd293d5910b7f2">78cfb38</a></td><td><code>Expanded conversions for real numbers.</code></td></tr></table>




---

archive/issue_events_220223.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-07-09T06:27:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16634",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16634#event-220223"
}
```



---

archive/issue_events_220224.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16634",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16634#event-220224"
}
```



---

archive/issue_events_220225.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16634",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16634#event-220225"
}
```



---

archive/issue_events_220226.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-09-08T18:46:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16634",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16634#event-220226"
}
```



---

archive/issue_events_220227.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-09-08T18:46:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16634",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16634#event-220227"
}
```



---

archive/issue_comments_219423.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nSeveral failing doctests, see buildbot report.",
    "created_at": "2014-09-08T18:46:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16634",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16634#issuecomment-219423",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:3'>Comment 3:</a>
Several failing doctests, see buildbot report.



---

archive/issue_comments_219424.json:
```json
{
    "body": "Changed commit from **[78cfb38](https://github.com/sagemath/sagetrac-mirror/commit/78cfb381b90d28dcbc6301c539dd293d5910b7f2)** to **[98f77ec](https://github.com/sagemath/sagetrac-mirror/commit/98f77ecaae42d66b9ce66ad9ff92c9f56f5b0991)**",
    "created_at": "2014-11-17T06:52:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16634",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16634#issuecomment-219424",
    "user": "https://github.com/darijgr"
}
```

Changed commit from **[78cfb38](https://github.com/sagemath/sagetrac-mirror/commit/78cfb381b90d28dcbc6301c539dd293d5910b7f2)** to **[98f77ec](https://github.com/sagemath/sagetrac-mirror/commit/98f77ecaae42d66b9ce66ad9ff92c9f56f5b0991)**



---

archive/issue_comments_219425.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nRebased the branch, since it is currently applied and unapplied in #16630 and thus will likely disappear when #16630 is merged. Old branch was public/rings/expand_RR_conversion-16634 .\n\nWarning: branch change and forced push.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/98f77ecaae42d66b9ce66ad9ff92c9f56f5b0991\">98f77ec</a></td><td><code>Expanded conversions for real numbers. (The original author of this commit is Travis Scrimshaw; this is a rebased version.)</code></td></tr></table>\n",
    "created_at": "2014-11-17T06:52:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16634",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16634#issuecomment-219425",
    "user": "https://github.com/darijgr"
}
```

<a id='comment:4'>Comment 4:</a>
Rebased the branch, since it is currently applied and unapplied in #16630 and thus will likely disappear when #16630 is merged. Old branch was public/rings/expand_RR_conversion-16634 .

Warning: branch change and forced push.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/98f77ecaae42d66b9ce66ad9ff92c9f56f5b0991">98f77ec</a></td><td><code>Expanded conversions for real numbers. (The original author of this commit is Travis Scrimshaw; this is a rebased version.)</code></td></tr></table>




---

archive/issue_comments_219426.json:
```json
{
    "body": "Changed branch from **[public/rings/expand_RR_conversion-16634](https://github.com/sagemath/sagetrac-mirror/tree/public/rings/expand_RR_conversion-16634)** to **[public/rings/expand_RR_conversion-16634-rebased](https://github.com/sagemath/sagetrac-mirror/tree/public/rings/expand_RR_conversion-16634-rebased)**",
    "created_at": "2014-11-17T06:52:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16634",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16634#issuecomment-219426",
    "user": "https://github.com/darijgr"
}
```

Changed branch from **[public/rings/expand_RR_conversion-16634](https://github.com/sagemath/sagetrac-mirror/tree/public/rings/expand_RR_conversion-16634)** to **[public/rings/expand_RR_conversion-16634-rebased](https://github.com/sagemath/sagetrac-mirror/tree/public/rings/expand_RR_conversion-16634-rebased)**



---

archive/issue_comments_219427.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nReplying to [@darijgr](#comment%3A4):\n> The original author of this commit is Travis Scrimshaw\n\nYou can fix this with\n\n```\ngit commit --amend --author=\"Travis Scrimshaw\"\n```",
    "created_at": "2014-11-17T08:44:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16634",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16634#issuecomment-219427",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:5'>Comment 5:</a>
Replying to [@darijgr](#comment%3A4):
> The original author of this commit is Travis Scrimshaw

You can fix this with

```
git commit --amend --author="Travis Scrimshaw"
```



---

archive/issue_comments_219428.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nOn the topic of this ticket: whatever you do, test also `CC` in addition to `RR`.",
    "created_at": "2014-11-17T08:46:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16634",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16634#issuecomment-219428",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:6'>Comment 6:</a>
On the topic of this ticket: whatever you do, test also `CC` in addition to `RR`.



---

archive/issue_events_220228.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-11-17T08:46:41Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/16634",
    "title_is": "Expand conversions to RR",
    "title_was": "Expand coercions of RR",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16634#event-220228"
}
```



---

archive/issue_events_220229.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16634",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16634#event-220229"
}
```
