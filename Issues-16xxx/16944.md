# Issue 16944: GF(q) -> PARI: always return FFELT

archive/issues_016707.json:
```json
{
    "assignees": [],
    "body": "The finite field implementations `givaro` and `ntl` should return a `t_FFELT` when a finite field element is converted to PARI.\n\nCC:  @pjbruin\n\nBranch/Commit: **[5dc807c](https://github.com/sagemath/sagetrac-mirror/commit/5dc807ccefd44cdb3b3932f34a8b2e2d5cb3b6d1)**\n\nReviewer: **Volker Braun**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16944_\n\n",
    "closed_at": "2014-09-10T21:45:10Z",
    "created_at": "2014-09-08T12:43:48Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20finite%20rings",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "GF(q) -> PARI: always return FFELT",
    "type": "issue",
    "updated_at": "2014-09-10T21:45:10Z",
    "url": "https://github.com/sagemath/sage/issues/16944",
    "user": "https://github.com/jdemeyer"
}
```
The finite field implementations `givaro` and `ntl` should return a `t_FFELT` when a finite field element is converted to PARI.

CC:  @pjbruin

Branch/Commit: **[5dc807c](https://github.com/sagemath/sagetrac-mirror/commit/5dc807ccefd44cdb3b3932f34a8b2e2d5cb3b6d1)**

Reviewer: **Volker Braun**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/16944_





---

archive/issue_events_216537.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-09-08T12:43:48Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16944",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16944#event-216537"
}
```



---

archive/issue_events_216538.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-09-08T12:43:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16944",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20finite%20rings",
    "label_color": "0000ff",
    "label_name": "component: finite rings",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16944#event-216538"
}
```



---

archive/issue_events_216539.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-09-08T12:43:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16944",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16944#event-216539"
}
```



---

archive/issue_events_216540.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-09-08T12:43:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16944",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16944#event-216540"
}
```



---

archive/issue_comments_226268.json:
```json
{
    "body": "Branch: **[u/jdemeyer/ticket/16944](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/16944)**",
    "created_at": "2014-09-08T15:58:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16944",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16944#issuecomment-226268",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/ticket/16944](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/16944)**



---

archive/issue_comments_226269.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9ff6d122251c38719f6e7c3340722802f52b160e\">9ff6d12</a></td><td><code>Return FFELT in GF(q) -> PARI conversion</code></td></tr></table>\n",
    "created_at": "2014-09-08T17:22:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16944",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16944#issuecomment-226269",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9ff6d122251c38719f6e7c3340722802f52b160e">9ff6d12</a></td><td><code>Return FFELT in GF(q) -> PARI conversion</code></td></tr></table>




---

archive/issue_events_216541.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-09-08T17:22:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16944",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16944#event-216541"
}
```



---

archive/issue_comments_226270.json:
```json
{
    "body": "Commit: **[9ff6d12](https://github.com/sagemath/sagetrac-mirror/commit/9ff6d122251c38719f6e7c3340722802f52b160e)**",
    "created_at": "2014-09-08T17:22:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16944",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16944#issuecomment-226270",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[9ff6d12](https://github.com/sagemath/sagetrac-mirror/commit/9ff6d122251c38719f6e7c3340722802f52b160e)**



---

archive/issue_comments_226271.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2014-09-10T10:22:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16944",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16944#issuecomment-226271",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_events_216542.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-09-10T10:22:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16944",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16944#event-216542"
}
```



---

archive/issue_events_216543.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-09-10T10:22:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16944",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16944#event-216543"
}
```



---

archive/issue_comments_226272.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nlgtm",
    "created_at": "2014-09-10T10:22:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16944",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16944#issuecomment-226272",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:3'>Comment 3:</a>
lgtm



---

archive/issue_events_216544.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-09-10T11:52:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16944",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16944#event-216544"
}
```



---

archive/issue_events_216545.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-09-10T11:52:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16944",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16944#event-216545"
}
```



---

archive/issue_comments_226273.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nI get the following:\n\n```\nsage -t --long --warn-long 76.8 src/sage/libs/pari/gen.pyx\n**********************************************************************\nFile \"src/sage/libs/pari/gen.pyx\", line 5179, in sage.libs.pari.gen.gen.fflog\nFailed example:\n    pari(a).ffprimroot()\nExpected:\n    Traceback (most recent call last):\n    ...\n    PariError: incorrect type in ffprimroot (t_POLMOD)\nGot:\n    a^11 + a^10 + a^9 + a^8 + a^7 + a^6 + a^5 + a^4 + a^2 + 1\n**********************************************************************\n1 item had failures:\n   1 of  11 in sage.libs.pari.gen.gen.fflog\n    [1299 tests, 1 failure, 1.25 s]\n```",
    "created_at": "2014-09-10T11:52:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16944",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16944#issuecomment-226273",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:4'>Comment 4:</a>
I get the following:

```
sage -t --long --warn-long 76.8 src/sage/libs/pari/gen.pyx
**********************************************************************
File "src/sage/libs/pari/gen.pyx", line 5179, in sage.libs.pari.gen.gen.fflog
Failed example:
    pari(a).ffprimroot()
Expected:
    Traceback (most recent call last):
    ...
    PariError: incorrect type in ffprimroot (t_POLMOD)
Got:
    a^11 + a^10 + a^9 + a^8 + a^7 + a^6 + a^5 + a^4 + a^2 + 1
**********************************************************************
1 item had failures:
   1 of  11 in sage.libs.pari.gen.gen.fflog
    [1299 tests, 1 failure, 1.25 s]
```



---

archive/issue_comments_226274.json:
```json
{
    "body": "Changed commit from **[9ff6d12](https://github.com/sagemath/sagetrac-mirror/commit/9ff6d122251c38719f6e7c3340722802f52b160e)** to **[5dc807c](https://github.com/sagemath/sagetrac-mirror/commit/5dc807ccefd44cdb3b3932f34a8b2e2d5cb3b6d1)**",
    "created_at": "2014-09-10T12:05:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16944",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16944#issuecomment-226274",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[9ff6d12](https://github.com/sagemath/sagetrac-mirror/commit/9ff6d122251c38719f6e7c3340722802f52b160e)** to **[5dc807c](https://github.com/sagemath/sagetrac-mirror/commit/5dc807ccefd44cdb3b3932f34a8b2e2d5cb3b6d1)**



---

archive/issue_comments_226275.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8286a4c3e661246576813bbfb470a4b5e05eca77\">8286a4c</a></td><td><code>Add ffprimroot, fflog, fforder</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/63ec17af4cc391fb5bcfef7c9c113488806dac21\">63ec17a</a></td><td><code>Allow modulus=\"primitive\" in finite field constructor</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/abcf8a6d4042dc08a3d10d494da1df2eba62a592\">abcf8a6</a></td><td><code>Deprecate non-polynomial modulus argument in constructor of implementations of finite fields</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8cdbd7920c73713a3a8dbea697451649f0434f0a\">8cdbd79</a></td><td><code>Improve computation of j-invariant over smaller field</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b930fa1c28a0bfb8eff706239a65ba2b152718cb\">b930fa1</a></td><td><code>Merge branch 'ticket/16930' into ticket/16944</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5dc807ccefd44cdb3b3932f34a8b2e2d5cb3b6d1\">5dc807c</a></td><td><code>Remove impl=\"pari_ffelt\" options for finite fields</code></td></tr></table>\n",
    "created_at": "2014-09-10T12:05:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16944",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16944#issuecomment-226275",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:5'>Comment 5:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8286a4c3e661246576813bbfb470a4b5e05eca77">8286a4c</a></td><td><code>Add ffprimroot, fflog, fforder</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/63ec17af4cc391fb5bcfef7c9c113488806dac21">63ec17a</a></td><td><code>Allow modulus="primitive" in finite field constructor</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/abcf8a6d4042dc08a3d10d494da1df2eba62a592">abcf8a6</a></td><td><code>Deprecate non-polynomial modulus argument in constructor of implementations of finite fields</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8cdbd7920c73713a3a8dbea697451649f0434f0a">8cdbd79</a></td><td><code>Improve computation of j-invariant over smaller field</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b930fa1c28a0bfb8eff706239a65ba2b152718cb">b930fa1</a></td><td><code>Merge branch 'ticket/16930' into ticket/16944</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5dc807ccefd44cdb3b3932f34a8b2e2d5cb3b6d1">5dc807c</a></td><td><code>Remove impl="pari_ffelt" options for finite fields</code></td></tr></table>




---

archive/issue_events_216546.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-09-10T12:06:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16944",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16944#event-216546"
}
```



---

archive/issue_events_216547.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-09-10T12:06:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16944",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16944#event-216547"
}
```



---

archive/issue_comments_226276.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\n#16930 documented a problem which this ticket solved. Please review the last commit (the other commits come from merging #16930).",
    "created_at": "2014-09-10T12:09:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16944",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16944#issuecomment-226276",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:7'>Comment 7:</a>
#16930 documented a problem which this ticket solved. Please review the last commit (the other commits come from merging #16930).



---

archive/issue_events_216548.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-09-10T14:26:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16944",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16944#event-216548"
}
```



---

archive/issue_events_216549.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-09-10T14:26:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16944",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16944#event-216549"
}
```



---

archive/issue_comments_226277.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/ticket/16944](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/16944)** to **[5dc807c](https://github.com/sagemath/sagetrac-mirror/commit/5dc807ccefd44cdb3b3932f34a8b2e2d5cb3b6d1)**",
    "created_at": "2014-09-10T21:45:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16944",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16944#issuecomment-226277",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/ticket/16944](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/16944)** to **[5dc807c](https://github.com/sagemath/sagetrac-mirror/commit/5dc807ccefd44cdb3b3932f34a8b2e2d5cb3b6d1)**



---

archive/issue_events_216550.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-09-10T21:45:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16944",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16944#event-216550"
}
```



---

archive/issue_events_216551.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "8aa426361f6a327b221937956f5e53f69ca3cbe3",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-09-10T21:45:10Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16944",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16944#event-216551"
}
```
