# Issue 16457: Rename xrange() to range()

archive/issues_016220.json:
```json
{
    "assignees": [],
    "body": "In python3 xrange() was renamed to range().\n\nWith this patch iterators will become lists till we switch to python 3. But this should be okay for small intervals. (That's the reason for not changing all.)\n\nThis is part of #16081.\n\nNOTE: xrange can (and should) stay inside the code of .pyx files\n\nCC:  @jdemeyer @tscrim @a-andre @embray\n\nKeywords: **python3**\n\nBranch/Commit: **[dd549fa](https://github.com/sagemath/sagetrac-mirror/commit/dd549fa80f073333fb53fe8d0634eb19281475e6)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Andr\u00e9 Apitzsch, Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16457_\n\n",
    "closed_at": "2016-12-05T00:44:38Z",
    "created_at": "2014-06-07T21:58:01Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20python3",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Rename xrange() to range()",
    "type": "issue",
    "updated_at": "2016-12-05T00:44:38Z",
    "url": "https://github.com/sagemath/sage/issues/16457",
    "user": "https://github.com/a-andre"
}
```
In python3 xrange() was renamed to range().

With this patch iterators will become lists till we switch to python 3. But this should be okay for small intervals. (That's the reason for not changing all.)

This is part of #16081.

NOTE: xrange can (and should) stay inside the code of .pyx files

CC:  @jdemeyer @tscrim @a-andre @embray

Keywords: **python3**

Branch/Commit: **[dd549fa](https://github.com/sagemath/sagetrac-mirror/commit/dd549fa80f073333fb53fe8d0634eb19281475e6)**

Reviewer: **Travis Scrimshaw**

Author: **André Apitzsch, Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/16457_





---

archive/issue_events_217473.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2014-06-07T21:58:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16457#event-217473"
}
```



---

archive/issue_events_217474.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2014-06-07T21:58:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16457#event-217474"
}
```



---

archive/issue_events_217475.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2014-06-07T21:58:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16457#event-217475"
}
```



---

archive/issue_events_217476.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2014-06-07T22:01:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16457#event-217476"
}
```



---

archive/issue_comments_215335.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d3bae80fba004f3a633c3e0759293a61854f0208\">d3bae80</a></td><td><code>rename xrange() to range()</code></td></tr></table>\n",
    "created_at": "2014-06-07T22:01:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16457#issuecomment-215335",
    "user": "https://github.com/a-andre"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d3bae80fba004f3a633c3e0759293a61854f0208">d3bae80</a></td><td><code>rename xrange() to range()</code></td></tr></table>




---

archive/issue_comments_215336.json:
```json
{
    "body": "Author: **Andr\u00e9 Apitzsch**",
    "created_at": "2014-06-07T22:01:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16457#issuecomment-215336",
    "user": "https://github.com/a-andre"
}
```

Author: **André Apitzsch**



---

archive/issue_comments_215337.json:
```json
{
    "body": "Changed keywords from none to **python3**",
    "created_at": "2014-06-07T22:01:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16457#issuecomment-215337",
    "user": "https://github.com/a-andre"
}
```

Changed keywords from none to **python3**



---

archive/issue_comments_215338.json:
```json
{
    "body": "Commit: **[d3bae80](https://github.com/sagemath/sagetrac-mirror/commit/d3bae80fba004f3a633c3e0759293a61854f0208)**",
    "created_at": "2014-06-07T22:01:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16457#issuecomment-215338",
    "user": "https://github.com/a-andre"
}
```

Commit: **[d3bae80](https://github.com/sagemath/sagetrac-mirror/commit/d3bae80fba004f3a633c3e0759293a61854f0208)**



---

archive/issue_comments_215339.json:
```json
{
    "body": "Branch: **[u/aapitzsch/ticket/16457](https://github.com/sagemath/sagetrac-mirror/tree/u/aapitzsch/ticket/16457)**",
    "created_at": "2014-06-07T22:01:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16457#issuecomment-215339",
    "user": "https://github.com/a-andre"
}
```

Branch: **[u/aapitzsch/ticket/16457](https://github.com/sagemath/sagetrac-mirror/tree/u/aapitzsch/ticket/16457)**



---

archive/issue_comments_215340.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nI am strongly against that this ticket is being merge now. We are far from having the Python 3 switch tomorrow (is it even a blocker for sage-6.3?). It will slow down everything. `xrange` is faster by 40% on lists of size 100:\n\n```\nsage: timeit(\"for i in xrange(100):\\n for j in xrange(100): pass\")\n625 loops, best of 3: 362 \u00b5s per loop\nsage: timeit(\"for i in range(100):\\n for j in range(100): pass\")\n625 loops, best of 3: 501 \u00b5s per loop\n```\nAnd there are many of them in Sage source code.",
    "created_at": "2014-06-08T17:22:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16457#issuecomment-215340",
    "user": "https://github.com/videlec"
}
```

<div id="comment:2" align="right">Comment 2</div>

I am strongly against that this ticket is being merge now. We are far from having the Python 3 switch tomorrow (is it even a blocker for sage-6.3?). It will slow down everything. `xrange` is faster by 40% on lists of size 100:

```
sage: timeit("for i in xrange(100):\n for j in xrange(100): pass")
625 loops, best of 3: 362 µs per loop
sage: timeit("for i in range(100):\n for j in range(100): pass")
625 loops, best of 3: 501 µs per loop
```
And there are many of them in Sage source code.



---

archive/issue_comments_215341.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nYeah, this does seem somewhat premature due to this.  However, it's still good to have a trial version here, especially if someone decided to make a 'Sage-py3' branch for testing at some point.",
    "created_at": "2014-06-10T00:10:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16457#issuecomment-215341",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:3" align="right">Comment 3</div>

Yeah, this does seem somewhat premature due to this.  However, it's still good to have a trial version here, especially if someone decided to make a 'Sage-py3' branch for testing at some point.



---

archive/issue_events_217477.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16457#event-217477"
}
```



---

archive/issue_events_217478.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16457#event-217478"
}
```



---

archive/issue_events_217479.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-08-25T13:47:54Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16457#event-217479"
}
```



---

archive/issue_events_217480.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-08-25T13:47:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "label": "https://github.com/sagemath/sage/labels/pending",
    "label_color": "008080",
    "label_name": "pending",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16457#event-217480"
}
```



---

archive/issue_events_217481.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-10-16T11:25:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16457#event-217481"
}
```



---

archive/issue_events_217482.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-10-16T11:25:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16457#event-217482"
}
```



---

archive/issue_events_217483.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-08-16T16:29:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20python3",
    "label_color": "000080",
    "label_name": "component: python3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16457#event-217483"
}
```



---

archive/issue_comments_215342.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nsee #21375 for a step in this direction",
    "created_at": "2016-08-30T18:30:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16457#issuecomment-215342",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:8" align="right">Comment 8</div>

see #21375 for a step in this direction



---

archive/issue_events_217484.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-08-30T18:31:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "milestone_number": null,
    "milestone_title": "sage-7.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16457#event-217484"
}
```



---

archive/issue_events_217485.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-08-30T18:31:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "label": "https://github.com/sagemath/sage/labels/pending",
    "label_color": "008080",
    "label_name": "pending",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16457#event-217485"
}
```



---

archive/issue_comments_215343.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nNOTE: xrange can (and should) stay inside the code of .pyx files",
    "created_at": "2016-09-12T14:53:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16457#issuecomment-215343",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:10" align="right">Comment 10</div>

NOTE: xrange can (and should) stay inside the code of .pyx files



---

archive/issue_comments_215344.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -3,3 +3,5 @@\n With this patch iterators will become lists till we switch to python 3. But this should be okay for small intervals. (That's the reason for not changing all.)\n \n This is part of #16081.\n+\n+NOTE: xrange can (and should) stay inside the code of .pyx files\n``````\n",
    "created_at": "2016-09-12T14:53:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16457#issuecomment-215344",
    "user": "https://github.com/fchapoton"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -3,3 +3,5 @@
 With this patch iterators will become lists till we switch to python 3. But this should be okay for small intervals. (That's the reason for not changing all.)
 
 This is part of #16081.
+
+NOTE: xrange can (and should) stay inside the code of .pyx files
``````




---

archive/issue_comments_215345.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nsee #21478, #21530, #21531, #21548 for some further steps",
    "created_at": "2016-09-21T17:35:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16457#issuecomment-215345",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:11" align="right">Comment 11</div>

see #21478, #21530, #21531, #21548 for some further steps



---

archive/issue_comments_215346.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nafter #21763, #21751, #21677 we will be almost done",
    "created_at": "2016-10-26T11:47:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16457#issuecomment-215346",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:12" align="right">Comment 12</div>

after #21763, #21751, #21677 we will be almost done



---

archive/issue_comments_215347.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nThis is now essentially done. There remains only very few xrange in the code. Let us get rid of them here if possible.",
    "created_at": "2016-11-07T20:44:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16457#issuecomment-215347",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:13" align="right">Comment 13</div>

This is now essentially done. There remains only very few xrange in the code. Let us get rid of them here if possible.



---

archive/issue_comments_215348.json:
```json
{
    "body": "Changed commit from **[d3bae80](https://github.com/sagemath/sagetrac-mirror/commit/d3bae80fba004f3a633c3e0759293a61854f0208)** to none",
    "created_at": "2016-11-07T20:44:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16457#issuecomment-215348",
    "user": "https://github.com/fchapoton"
}
```

Changed commit from **[d3bae80](https://github.com/sagemath/sagetrac-mirror/commit/d3bae80fba004f3a633c3e0759293a61854f0208)** to none



---

archive/issue_comments_215349.json:
```json
{
    "body": "Changed branch from **[u/aapitzsch/ticket/16457](https://github.com/sagemath/sagetrac-mirror/tree/u/aapitzsch/ticket/16457)** to none",
    "created_at": "2016-11-07T20:44:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16457#issuecomment-215349",
    "user": "https://github.com/fchapoton"
}
```

Changed branch from **[u/aapitzsch/ticket/16457](https://github.com/sagemath/sagetrac-mirror/tree/u/aapitzsch/ticket/16457)** to none



---

archive/issue_comments_215350.json:
```json
{
    "body": "Commit: **[dd549fa](https://github.com/sagemath/sagetrac-mirror/commit/dd549fa80f073333fb53fe8d0634eb19281475e6)**",
    "created_at": "2016-11-19T20:24:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16457#issuecomment-215350",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[dd549fa](https://github.com/sagemath/sagetrac-mirror/commit/dd549fa80f073333fb53fe8d0634eb19281475e6)**



---

archive/issue_events_217486.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-11-19T20:24:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "milestone_number": null,
    "milestone_title": "sage-7.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16457#event-217486"
}
```



---

archive/issue_events_217487.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-11-19T20:24:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "milestone_number": null,
    "milestone_title": "sage-7.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16457#event-217487"
}
```



---

archive/issue_comments_215351.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nok, here is a simple branch eliminating essentially all the remaining instances of xrange, mostly in the doc.\n\nThere will remain to handle ticket #16081.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/dd549fa80f073333fb53fe8d0634eb19281475e6\">dd549fa</a></td><td><code>trac 16457 eliminate last traces of xrange</code></td></tr></table>\n",
    "created_at": "2016-11-19T20:24:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16457#issuecomment-215351",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:14" align="right">Comment 14</div>

ok, here is a simple branch eliminating essentially all the remaining instances of xrange, mostly in the doc.

There will remain to handle ticket #16081.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/dd549fa80f073333fb53fe8d0634eb19281475e6">dd549fa</a></td><td><code>trac 16457 eliminate last traces of xrange</code></td></tr></table>




---

archive/issue_events_217488.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-11-19T20:24:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16457#event-217488"
}
```



---

archive/issue_events_217489.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-11-19T20:24:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16457#event-217489"
}
```



---

archive/issue_comments_215352.json:
```json
{
    "body": "Branch: **[u/chapoton/16457](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/16457)**",
    "created_at": "2016-11-19T20:24:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16457#issuecomment-215352",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/16457](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/16457)**



---

archive/issue_comments_215353.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">Comment 15</div>\n\nat least one bot is green, please review",
    "created_at": "2016-11-25T09:28:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16457#issuecomment-215353",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:15" align="right">Comment 15</div>

at least one bot is green, please review



---

archive/issue_comments_215354.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">Comment 16</div>\n\n*ping* ?",
    "created_at": "2016-12-02T12:45:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16457#issuecomment-215354",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:16" align="right">Comment 16</div>

*ping* ?



---

archive/issue_events_217490.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-12-02T14:54:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16457#event-217490"
}
```



---

archive/issue_events_217491.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-12-02T14:54:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16457#event-217491"
}
```



---

archive/issue_comments_215355.json:
```json
{
    "body": "Changed author from **Andr\u00e9 Apitzsch** to **Andr\u00e9 Apitzsch, Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2016-12-02T14:54:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16457#issuecomment-215355",
    "user": "https://github.com/tscrim"
}
```

Changed author from **André Apitzsch** to **André Apitzsch, Frédéric Chapoton**



---

archive/issue_comments_215356.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2016-12-02T14:54:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16457#issuecomment-215356",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_215357.json:
```json
{
    "body": "Changed branch from **[u/chapoton/16457](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/16457)** to **[dd549fa](https://github.com/sagemath/sagetrac-mirror/commit/dd549fa80f073333fb53fe8d0634eb19281475e6)**",
    "created_at": "2016-12-05T00:44:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16457#issuecomment-215357",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/chapoton/16457](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/16457)** to **[dd549fa](https://github.com/sagemath/sagetrac-mirror/commit/dd549fa80f073333fb53fe8d0634eb19281475e6)**



---

archive/issue_events_217492.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-12-05T00:44:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16457#event-217492"
}
```



---

archive/issue_events_217493.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "d5bb829013c8b7a088f45a07af59b807e6e956b8",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-12-05T00:44:38Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16457",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16457#event-217493"
}
```
