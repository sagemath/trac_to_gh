# Issue 16345: ATLAS fails to build on ARM

archive/issues_016108.json:
```json
{
    "assignees": [],
    "body": "A whitespace is missing before `-D c -DATL_ARM_HARDFP=1` in `spkg-install`.\n\nCC:  @vbraun\n\nKeywords: **atlas spkg arm**\n\nBranch/Commit: **[ed31cbe](https://github.com/sagemath/sagetrac-mirror/commit/ed31cbeb32369209e8753d55df635498e65d11a4)**\n\nReviewer: **Volker Braun, Peter Bruin**\n\nAuthor: **Jean-Pierre Flori**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16345_\n\n",
    "closed_at": "2014-05-15T17:18:36Z",
    "created_at": "2014-05-13T09:59:05Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "ATLAS fails to build on ARM",
    "type": "issue",
    "updated_at": "2014-05-15T17:18:36Z",
    "url": "https://github.com/sagemath/sage/issues/16345",
    "user": "https://github.com/jpflori"
}
```
A whitespace is missing before `-D c -DATL_ARM_HARDFP=1` in `spkg-install`.

CC:  @vbraun

Keywords: **atlas spkg arm**

Branch/Commit: **[ed31cbe](https://github.com/sagemath/sagetrac-mirror/commit/ed31cbeb32369209e8753d55df635498e65d11a4)**

Reviewer: **Volker Braun, Peter Bruin**

Author: **Jean-Pierre Flori**

_Issue created by migration from https://trac.sagemath.org/ticket/16345_





---

archive/issue_events_215967.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2014-05-13T09:59:05Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16345",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16345#event-215967"
}
```



---

archive/issue_events_215968.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2014-05-13T09:59:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16345",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16345#event-215968"
}
```



---

archive/issue_events_215969.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2014-05-13T09:59:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16345",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16345#event-215969"
}
```



---

archive/issue_events_215970.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2014-05-13T09:59:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16345",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16345#event-215970"
}
```



---

archive/issue_comments_213097.json:
```json
{
    "body": "Branch: **[u/jpflori/ticket/16345](https://github.com/sagemath/sagetrac-mirror/tree/u/jpflori/ticket/16345)**",
    "created_at": "2014-05-13T10:04:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16345#issuecomment-213097",
    "user": "https://github.com/jpflori"
}
```

Branch: **[u/jpflori/ticket/16345](https://github.com/sagemath/sagetrac-mirror/tree/u/jpflori/ticket/16345)**



---

archive/issue_comments_213098.json:
```json
{
    "body": "Commit: **[ed31cbe](https://github.com/sagemath/sagetrac-mirror/commit/ed31cbeb32369209e8753d55df635498e65d11a4)**",
    "created_at": "2014-05-13T10:04:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16345#issuecomment-213098",
    "user": "https://github.com/jpflori"
}
```

Commit: **[ed31cbe](https://github.com/sagemath/sagetrac-mirror/commit/ed31cbeb32369209e8753d55df635498e65d11a4)**



---

archive/issue_comments_213099.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ed31cbeb32369209e8753d55df635498e65d11a4\">ed31cbe</a></td><td><code>Fix atlas spkg-install script for ARM.</code></td></tr></table>\n",
    "created_at": "2014-05-13T10:04:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16345#issuecomment-213099",
    "user": "https://github.com/jpflori"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ed31cbeb32369209e8753d55df635498e65d11a4">ed31cbe</a></td><td><code>Fix atlas spkg-install script for ARM.</code></td></tr></table>




---

archive/issue_events_215971.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-05-13T10:55:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16345",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16345#event-215971"
}
```



---

archive/issue_events_215972.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-05-13T10:55:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16345",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16345#event-215972"
}
```



---

archive/issue_comments_213100.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nAuthor name?",
    "created_at": "2014-05-13T10:55:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16345#issuecomment-213100",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:2" align="right">Comment 2</div>

Author name?



---

archive/issue_comments_213101.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2014-05-13T10:59:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16345#issuecomment-213101",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_comments_213102.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\npatch looks good to me",
    "created_at": "2014-05-13T10:59:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16345#issuecomment-213102",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:3" align="right">Comment 3</div>

patch looks good to me



---

archive/issue_comments_213103.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nI can confirm that this solves the problem.\n\nSetting it to needs_review so that Jean-Pierre can make it a positive_review after adding his name. :-)",
    "created_at": "2014-05-14T14:17:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16345#issuecomment-213103",
    "user": "https://github.com/pjbruin"
}
```

<div id="comment:4" align="right">Comment 4</div>

I can confirm that this solves the problem.

Setting it to needs_review so that Jean-Pierre can make it a positive_review after adding his name. :-)



---

archive/issue_events_215973.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2014-05-14T14:17:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16345",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16345#event-215973"
}
```



---

archive/issue_events_215974.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2014-05-14T14:19:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16345",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16345#event-215974"
}
```



---

archive/issue_events_215975.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2014-05-14T14:19:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16345",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16345#event-215975"
}
```



---

archive/issue_comments_213104.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nI also confirm it actually works on an actual ARM board.",
    "created_at": "2014-05-14T14:19:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16345#issuecomment-213104",
    "user": "https://github.com/jpflori"
}
```

<div id="comment:5" align="right">Comment 5</div>

I also confirm it actually works on an actual ARM board.



---

archive/issue_comments_213105.json:
```json
{
    "body": "Changed reviewer from **Volker Braun** to **Volker Braun, Peter Bruin**",
    "created_at": "2014-05-14T14:19:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16345#issuecomment-213105",
    "user": "https://github.com/jpflori"
}
```

Changed reviewer from **Volker Braun** to **Volker Braun, Peter Bruin**



---

archive/issue_comments_213106.json:
```json
{
    "body": "Author: **Jean-Pierre Flori**",
    "created_at": "2014-05-14T14:19:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16345#issuecomment-213106",
    "user": "https://github.com/jpflori"
}
```

Author: **Jean-Pierre Flori**



---

archive/issue_events_215976.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-05-15T17:18:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16345",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16345#event-215976"
}
```



---

archive/issue_events_215977.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "9d858c0d59ae0b48dd55d900af4e440a4fb08116",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-05-15T17:18:36Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16345",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16345#event-215977"
}
```



---

archive/issue_comments_213107.json:
```json
{
    "body": "Changed branch from **[u/jpflori/ticket/16345](https://github.com/sagemath/sagetrac-mirror/tree/u/jpflori/ticket/16345)** to **[ed31cbe](https://github.com/sagemath/sagetrac-mirror/commit/ed31cbeb32369209e8753d55df635498e65d11a4)**",
    "created_at": "2014-05-15T17:18:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16345#issuecomment-213107",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jpflori/ticket/16345](https://github.com/sagemath/sagetrac-mirror/tree/u/jpflori/ticket/16345)** to **[ed31cbe](https://github.com/sagemath/sagetrac-mirror/commit/ed31cbeb32369209e8753d55df635498e65d11a4)**
