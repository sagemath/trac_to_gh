# Issue 16304: Non-deterministic ordering of GMP-ECM output

archive/issues_016067.json:
```json
{
    "assignees": [],
    "body": "It seems that there is a small but positive probability that the factors found by the GMP-ECM elliptic curve factorisation program are in non-increasing order.  From [a recent patchbot report](http://patchbot.sagemath.org/log/11475/debian/wheezy/sid/x86_64/3.2.0-57-generic/selmer/2014-05-07%2007:23:33%20+0100):\n\n```\nsage -t --long src/sage/interfaces/ecm.py\n**********************************************************************\nFile \"src/sage/interfaces/ecm.py\", line 548, in sage.interfaces.ecm.ECM.find_factor\nFailed example:\n    f.find_factor(n)\nExpected:\n    [79792266297612017, 6366805760909027985741435139224233]\nGot:\n    [6366805760909027985741435139224233, 79792266297612017]\n**********************************************************************\n1 item had failures:\n   1 of   6 in sage.interfaces.ecm.ECM.find_factor\n    [44 tests, 1 failure, 15.27 s]\n```\nI have not been able to reproduce this, nor can Google find it in other patchbot reports.\n\nThis is related to #10951, which makes GMP-ECM's optional `sigma` argument accessible in the interface; passing such a sigma would give deterministic behaviour.\n\nCC:  @zimmermann6\n\nKeywords: **gmp-ecm elliptic curve factoring**\n\nComponent: **interfaces**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16304_\n\n",
    "created_at": "2014-05-07T14:52:48Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20interfaces",
        "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/needs%20info"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Non-deterministic ordering of GMP-ECM output",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/16304",
    "user": "https://github.com/pjbruin"
}
```
It seems that there is a small but positive probability that the factors found by the GMP-ECM elliptic curve factorisation program are in non-increasing order.  From [a recent patchbot report](http://patchbot.sagemath.org/log/11475/debian/wheezy/sid/x86_64/3.2.0-57-generic/selmer/2014-05-07%2007:23:33%20+0100):

```
sage -t --long src/sage/interfaces/ecm.py
**********************************************************************
File "src/sage/interfaces/ecm.py", line 548, in sage.interfaces.ecm.ECM.find_factor
Failed example:
    f.find_factor(n)
Expected:
    [79792266297612017, 6366805760909027985741435139224233]
Got:
    [6366805760909027985741435139224233, 79792266297612017]
**********************************************************************
1 item had failures:
   1 of   6 in sage.interfaces.ecm.ECM.find_factor
    [44 tests, 1 failure, 15.27 s]
```
I have not been able to reproduce this, nor can Google find it in other patchbot reports.

This is related to #10951, which makes GMP-ECM's optional `sigma` argument accessible in the interface; passing such a sigma would give deterministic behaviour.

CC:  @zimmermann6

Keywords: **gmp-ecm elliptic curve factoring**

Component: **interfaces**

_Issue created by migration from https://trac.sagemath.org/ticket/16304_





---

archive/issue_events_232913.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2014-05-07T14:52:48Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16304",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16304#event-232913"
}
```



---

archive/issue_events_232914.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2014-05-07T14:52:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16304",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20interfaces",
    "label_color": "0000ff",
    "label_name": "c: interfaces",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16304#event-232914"
}
```



---

archive/issue_events_232915.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2014-05-07T14:52:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16304",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16304#event-232915"
}
```



---

archive/issue_events_232916.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2014-05-07T14:52:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16304",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16304#event-232916"
}
```



---

archive/issue_comments_209495.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nindeed, the closer the (smallest) factors, the higher is the probability that the factor\nfound is non-deterministic:\n\n```\nsage: f = ECM()\nsage: n=6366805760909033411615543376840901\nsage: f.find_factor(n)\n[79792266297612017, 79792266297612053]\nsage: f.find_factor(n)\n[79792266297612053, 79792266297612017]\n```\nA better check would be to first sort the liste returned (in case of two prime factors).\n\nPaul",
    "created_at": "2014-05-07T15:16:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16304",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16304#issuecomment-209495",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:1" align="right">comment:1</div>

indeed, the closer the (smallest) factors, the higher is the probability that the factor
found is non-deterministic:

```
sage: f = ECM()
sage: n=6366805760909033411615543376840901
sage: f.find_factor(n)
[79792266297612017, 79792266297612053]
sage: f.find_factor(n)
[79792266297612053, 79792266297612017]
```
A better check would be to first sort the liste returned (in case of two prime factors).

Paul



---

archive/issue_comments_209496.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\n> A better check would be to first sort the liste returned (in case of two prime factors).\n\nsorry, I meant \"list\" instead of \"liste\".\n\nPaul",
    "created_at": "2014-05-07T15:16:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16304",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16304#issuecomment-209496",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:2" align="right">comment:2</div>

> A better check would be to first sort the liste returned (in case of two prime factors).

sorry, I meant "list" instead of "liste".

Paul



---

archive/issue_comments_209497.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nReplying to [@zimmermann6](#comment:1):\n> A better check would be to first sort the liste returned (in case of two prime factors).\n\nHow about letting `_parse_output()` always sort the list of pairs `(factor, probable_prime)` before returning it, regardless of the number of factors and the value of `probable_prime` for each?",
    "created_at": "2014-05-07T15:30:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16304",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16304#issuecomment-209497",
    "user": "https://github.com/pjbruin"
}
```

<div id="comment:3" align="right">comment:3</div>

Replying to [@zimmermann6](#comment:1):
> A better check would be to first sort the liste returned (in case of two prime factors).

How about letting `_parse_output()` always sort the list of pairs `(factor, probable_prime)` before returning it, regardless of the number of factors and the value of `probable_prime` for each?



---

archive/issue_comments_209498.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\n> How about letting `_parse_output()` always sort the list of pairs `(factor, probable_prime)` before returning it, regardless of the number of factors and the value of `probable_prime` for each?\n\nbut then we would lose some information, since the first factor is the one which is really found by ECM.\n\nAnd with 3 prime factors or more, we would still have non-deterministic output.\n\nPaul",
    "created_at": "2014-05-07T15:39:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16304",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16304#issuecomment-209498",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:4" align="right">comment:4</div>

> How about letting `_parse_output()` always sort the list of pairs `(factor, probable_prime)` before returning it, regardless of the number of factors and the value of `probable_prime` for each?

but then we would lose some information, since the first factor is the one which is really found by ECM.

And with 3 prime factors or more, we would still have non-deterministic output.

Paul



---

archive/issue_comments_209499.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nReplying to [@zimmermann6](#comment:4):\n> > How about letting `_parse_output()` always sort the list of pairs `(factor, probable_prime)` before returning it, regardless of the number of factors and the value of `probable_prime` for each?\n\n> \n> but then we would lose some information, since the first factor is the one which is really found by ECM.\n> \n> And with 3 prime factors or more, we would still have non-deterministic output.\n\nAh, right.  Then maybe the most reasonable solution is not to change the code at all, but to make the doctests resistant to non-deterministic results: sort the output in those tests where *n* has only two prime factors, and check that the output consists of at least 2 factors with the correct product if *n* has at least three prime factors.",
    "created_at": "2014-05-07T15:47:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16304",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16304#issuecomment-209499",
    "user": "https://github.com/pjbruin"
}
```

<div id="comment:5" align="right">comment:5</div>

Replying to [@zimmermann6](#comment:4):
> > How about letting `_parse_output()` always sort the list of pairs `(factor, probable_prime)` before returning it, regardless of the number of factors and the value of `probable_prime` for each?

> 
> but then we would lose some information, since the first factor is the one which is really found by ECM.
> 
> And with 3 prime factors or more, we would still have non-deterministic output.

Ah, right.  Then maybe the most reasonable solution is not to change the code at all, but to make the doctests resistant to non-deterministic results: sort the output in those tests where *n* has only two prime factors, and check that the output consists of at least 2 factors with the correct product if *n* has at least three prime factors.



---

archive/issue_events_232917.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2014-05-11T14:29:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16304",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16304#event-232917"
}
```



---

archive/issue_events_232918.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2014-05-11T14:29:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16304",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16304#event-232918"
}
```



---

archive/issue_events_232919.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16304",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16304#event-232919"
}
```



---

archive/issue_events_232920.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16304",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16304#event-232920"
}
```



---

archive/issue_comments_209500.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nAm I right in thinking that #10951 makes the factorisation deterministic, thereby solving the problem in this ticket?",
    "created_at": "2015-08-17T12:17:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16304",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16304#issuecomment-209500",
    "user": "https://github.com/pjbruin"
}
```

<div id="comment:8" align="right">comment:8</div>

Am I right in thinking that #10951 makes the factorisation deterministic, thereby solving the problem in this ticket?



---

archive/issue_events_232921.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2015-08-17T12:17:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16304",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16304#event-232921"
}
```



---

archive/issue_comments_209501.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nReplying to [@pjbruin](#comment:8):\n> Am I right in thinking that #10951 makes the factorisation deterministic, thereby solving the problem in this ticket?\n\nno, unless we fix the sigma values used by ECM, it should still be non deterministic.",
    "created_at": "2015-08-17T16:59:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16304",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16304#issuecomment-209501",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:9" align="right">comment:9</div>

Replying to [@pjbruin](#comment:8):
> Am I right in thinking that #10951 makes the factorisation deterministic, thereby solving the problem in this ticket?

no, unless we fix the sigma values used by ECM, it should still be non deterministic.



---

archive/issue_comments_209502.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">comment:10</div>\n\nJust put a `sorted(...)` around each of the problematic lines.",
    "created_at": "2015-08-17T17:15:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16304",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16304#issuecomment-209502",
    "user": "https://github.com/nbruin"
}
```

<div id="comment:10" align="right">comment:10</div>

Just put a `sorted(...)` around each of the problematic lines.



---

archive/issue_comments_209503.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">comment:11</div>\n\nReplying to [@zimmermann6](#comment:9):\n> Replying to [@pjbruin](#comment:8):\n> > Am I right in thinking that #10951 makes the factorisation deterministic, thereby solving the problem in this ticket?\n\n> \n> no, unless we fix the sigma values used by ECM, it should still be non deterministic.\n\nOK, I see, #10951 only makes it possible to pass a sigma value; the default `sigma=0` apparently means that a random one is chosen.",
    "created_at": "2015-08-17T17:35:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16304",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16304#issuecomment-209503",
    "user": "https://github.com/pjbruin"
}
```

<div id="comment:11" align="right">comment:11</div>

Replying to [@zimmermann6](#comment:9):
> Replying to [@pjbruin](#comment:8):
> > Am I right in thinking that #10951 makes the factorisation deterministic, thereby solving the problem in this ticket?

> 
> no, unless we fix the sigma values used by ECM, it should still be non deterministic.

OK, I see, #10951 only makes it possible to pass a sigma value; the default `sigma=0` apparently means that a random one is chosen.



---

archive/issue_comments_209504.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">comment:12</div>\n\nReplying to [@nbruin](#comment:10):\n> Just put a `sorted(...)` around each of the problematic lines.\n\nWe could do that, but see [comment:4](#comment:4).",
    "created_at": "2015-08-17T17:35:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16304",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16304#issuecomment-209504",
    "user": "https://github.com/pjbruin"
}
```

<div id="comment:12" align="right">comment:12</div>

Replying to [@nbruin](#comment:10):
> Just put a `sorted(...)` around each of the problematic lines.

We could do that, but see [comment:4](#comment:4).



---

archive/issue_events_232922.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16304",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16304#event-232922"
}
```
