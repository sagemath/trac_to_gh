# Issue 16965: Speedup of the method to reduce ternary quadratic forms in the class TernaryQF

archive/issues_016728.json:
```json
{
    "assignees": [],
    "body": "Speedup of the method to reduce ternary quadratic forms in the class TernaryQF.\n\n\nBranch/Commit: **[u/gdrama/speedup_of_the_method_to_reduce_ternary_quadratic_forms_in_the_class_ternaryqf](https://github.com/sagemath/sagetrac-mirror/tree/u/gdrama/speedup_of_the_method_to_reduce_ternary_quadratic_forms_in_the_class_ternaryqf) @ [091cbbd](https://github.com/sagemath/sagetrac-mirror/commit/091cbbdf6eed2c8bccca407addd07ff08e429c17)**\n\nAuthor: **Gustavo Rama**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16965_\n\n",
    "created_at": "2014-09-11T08:07:11Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20quadratic%20forms",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Speedup of the method to reduce ternary quadratic forms in the class TernaryQF",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/16965",
    "user": "https://github.com/sagetrac-gdrama"
}
```
Speedup of the method to reduce ternary quadratic forms in the class TernaryQF.


Branch/Commit: **[u/gdrama/speedup_of_the_method_to_reduce_ternary_quadratic_forms_in_the_class_ternaryqf](https://github.com/sagemath/sagetrac-mirror/tree/u/gdrama/speedup_of_the_method_to_reduce_ternary_quadratic_forms_in_the_class_ternaryqf) @ [091cbbd](https://github.com/sagemath/sagetrac-mirror/commit/091cbbdf6eed2c8bccca407addd07ff08e429c17)**

Author: **Gustavo Rama**

_Issue created by migration from https://trac.sagemath.org/ticket/16965_





---

archive/issue_events_224890.json:
```json
{
    "actor": "https://github.com/sagetrac-gdrama",
    "created_at": "2014-09-11T08:07:11Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16965",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16965#event-224890"
}
```



---

archive/issue_events_224891.json:
```json
{
    "actor": "https://github.com/sagetrac-gdrama",
    "created_at": "2014-09-11T08:07:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16965",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20quadratic%20forms",
    "label_color": "0000ff",
    "label_name": "component: quadratic forms",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16965#event-224891"
}
```



---

archive/issue_events_224892.json:
```json
{
    "actor": "https://github.com/sagetrac-gdrama",
    "created_at": "2014-09-11T08:07:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16965",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16965#event-224892"
}
```



---

archive/issue_events_224893.json:
```json
{
    "actor": "https://github.com/sagetrac-gdrama",
    "created_at": "2014-09-11T08:07:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16965",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16965#event-224893"
}
```



---

archive/issue_comments_226660.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAn example:\n\n```\n# old code\nsage: Q = TernaryQF([127, 2, 471, 9, 450, -8])\nsage: %timeit Q.reduced_form_eisenstein(matrix = True)\n10000 loops, best of 3: 140 \u00b5s per loop\n# new code\nsage: Q = TernaryQF([127, 2, 471, 9, 450, -8])\nsage: %timeit Q.reduced_form_eisenstein(matrix = True)\n10000 loops, best of 3: 40 \u00b5s per loop\n```",
    "created_at": "2014-09-11T08:18:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16965#issuecomment-226660",
    "user": "https://github.com/sagetrac-gdrama"
}
```

<a id='comment:1'>Comment 1:</a>
An example:

```
# old code
sage: Q = TernaryQF([127, 2, 471, 9, 450, -8])
sage: %timeit Q.reduced_form_eisenstein(matrix = True)
10000 loops, best of 3: 140 µs per loop
# new code
sage: Q = TernaryQF([127, 2, 471, 9, 450, -8])
sage: %timeit Q.reduced_form_eisenstein(matrix = True)
10000 loops, best of 3: 40 µs per loop
```



---

archive/issue_comments_226661.json:
```json
{
    "body": "Branch: **[u/gdrama/master](https://github.com/sagemath/sagetrac-mirror/tree/u/gdrama/master)**",
    "created_at": "2014-09-11T08:51:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16965#issuecomment-226661",
    "user": "https://github.com/sagetrac-gdrama"
}
```

Branch: **[u/gdrama/master](https://github.com/sagemath/sagetrac-mirror/tree/u/gdrama/master)**



---

archive/issue_comments_226662.json:
```json
{
    "body": "Changed branch from **[u/gdrama/master](https://github.com/sagemath/sagetrac-mirror/tree/u/gdrama/master)** to none",
    "created_at": "2014-09-11T08:52:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16965#issuecomment-226662",
    "user": "https://github.com/sagetrac-gdrama"
}
```

Changed branch from **[u/gdrama/master](https://github.com/sagemath/sagetrac-mirror/tree/u/gdrama/master)** to none



---

archive/issue_comments_226663.json:
```json
{
    "body": "Branch: **[u/gdrama/speedup_of_the_method_to_reduce_ternary_quadratic_forms_in_the_class_ternaryqf](https://github.com/sagemath/sagetrac-mirror/tree/u/gdrama/speedup_of_the_method_to_reduce_ternary_quadratic_forms_in_the_class_ternaryqf)**",
    "created_at": "2014-09-11T08:54:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16965#issuecomment-226663",
    "user": "https://github.com/sagetrac-gdrama"
}
```

Branch: **[u/gdrama/speedup_of_the_method_to_reduce_ternary_quadratic_forms_in_the_class_ternaryqf](https://github.com/sagemath/sagetrac-mirror/tree/u/gdrama/speedup_of_the_method_to_reduce_ternary_quadratic_forms_in_the_class_ternaryqf)**



---

archive/issue_comments_226664.json:
```json
{
    "body": "Commit: **[091cbbd](https://github.com/sagemath/sagetrac-mirror/commit/091cbbdf6eed2c8bccca407addd07ff08e429c17)**",
    "created_at": "2014-09-11T11:17:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16965#issuecomment-226664",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[091cbbd](https://github.com/sagemath/sagetrac-mirror/commit/091cbbdf6eed2c8bccca407addd07ff08e429c17)**



---

archive/issue_comments_226665.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/091cbbdf6eed2c8bccca407addd07ff08e429c17\">091cbbd</a></td><td><code>Eliminates the use of matrix constructor in internal loop of the function sage.quadratic_forms.ternary._reduced_ternary_form_eisenstein_with_matrix.</code></td></tr></table>\n",
    "created_at": "2014-09-11T11:17:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16965#issuecomment-226665",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:5'>Comment 5:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/091cbbdf6eed2c8bccca407addd07ff08e429c17">091cbbd</a></td><td><code>Eliminates the use of matrix constructor in internal loop of the function sage.quadratic_forms.ternary._reduced_ternary_form_eisenstein_with_matrix.</code></td></tr></table>




---

archive/issue_comments_226666.json:
```json
{
    "body": "Author: **Gustavo Rama**",
    "created_at": "2018-04-30T11:30:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16965#issuecomment-226666",
    "user": "https://github.com/fchapoton"
}
```

Author: **Gustavo Rama**



---

archive/issue_events_224894.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16965",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16965#event-224894"
}
```
