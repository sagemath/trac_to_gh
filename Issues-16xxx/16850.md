# Issue 16850: Interrupt in NTL malloc() hangs Sage

archive/issues_016613.json:
```json
{
    "assignees": [],
    "body": "Inspired by #16667, the following usually breaks Sage badly:\n\n```\nsage: x = polygen(GF(2^8, 'a')); alarm(0.1); x^1000000\n```\nThis reason is that this tends to interrupt during a `malloc()` call, which can totally mess up the heap. We always knew this could happen, but now we have a case which is relatively easy to reproduce.\n\nCC:  @vbraun @sagetrac-jpflori @defeo @pjbruin @sagetrac-sbesnier @nexttime\n\nKeywords: **interrupt**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16850_\n\n",
    "created_at": "2014-08-18T21:44:40Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20cython",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Interrupt in NTL malloc() hangs Sage",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/16850",
    "user": "https://github.com/jdemeyer"
}
```
Inspired by #16667, the following usually breaks Sage badly:

```
sage: x = polygen(GF(2^8, 'a')); alarm(0.1); x^1000000
```
This reason is that this tends to interrupt during a `malloc()` call, which can totally mess up the heap. We always knew this could happen, but now we have a case which is relatively easy to reproduce.

CC:  @vbraun @sagetrac-jpflori @defeo @pjbruin @sagetrac-sbesnier @nexttime

Keywords: **interrupt**

_Issue created by migration from https://trac.sagemath.org/ticket/16850_





---

archive/issue_events_223282.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-08-18T21:44:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16850",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16850#event-223282"
}
```



---

archive/issue_events_223283.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-08-18T21:44:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16850",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20cython",
    "label_color": "000080",
    "label_name": "component: cython",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16850#event-223283"
}
```



---

archive/issue_events_223284.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-08-18T21:44:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16850",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16850#event-223284"
}
```



---

archive/issue_events_223285.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-08-18T21:44:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16850",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16850#event-223285"
}
```



---

archive/issue_comments_224056.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -3,3 +3,4 @@\n ```\n sage: x = polygen(GF(2^8, 'a')); alarm(0.1); x^1000000\n ```\n+This reason is that this tends to interrupt during a `malloc()` call, which can totally mess up the heap. We always knew this could happen, but now we have a case which is relatively easy to reproduce.\n``````\n",
    "created_at": "2014-08-19T08:48:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16850",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16850#issuecomment-224056",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -3,3 +3,4 @@
 ```
 sage: x = polygen(GF(2^8, 'a')); alarm(0.1); x^1000000
 ```
+This reason is that this tends to interrupt during a `malloc()` call, which can totally mess up the heap. We always knew this could happen, but now we have a case which is relatively easy to reproduce.
``````




---

archive/issue_events_223286.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16850",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16850#event-223286"
}
```
