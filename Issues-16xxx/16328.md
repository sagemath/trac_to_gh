# Issue 16328: Add a category parameter for Module

archive/issues_016091.json:
```json
{
    "assignees": [],
    "body": "Currently, a `Module` (not to be confused with a `Module_old`) does not have a `category` parameter to overwrite the default category `Modules(base_ring)` with a more specific one. Such a parameter is necessary, e.g., for porting Hecke modules to use `Module` instead of `Module_old`.\n\nBranch/Commit: **[324bef3](https://github.com/sagemath/sagetrac-mirror/commit/324bef389799080b91bb6f34672f09974f7a0064)**\n\nReviewer: **Peter Bruin**\n\nAuthor: **Julian R\u00fcth**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16328_\n\n",
    "closed_at": "2014-05-13T07:58:31Z",
    "created_at": "2014-05-11T11:03:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20categories",
        "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add a category parameter for Module",
    "type": "issue",
    "updated_at": "2014-05-13T07:58:31Z",
    "url": "https://github.com/sagemath/sage/issues/16328",
    "user": "https://github.com/saraedum"
}
```
Currently, a `Module` (not to be confused with a `Module_old`) does not have a `category` parameter to overwrite the default category `Modules(base_ring)` with a more specific one. Such a parameter is necessary, e.g., for porting Hecke modules to use `Module` instead of `Module_old`.

Branch/Commit: **[324bef3](https://github.com/sagemath/sagetrac-mirror/commit/324bef389799080b91bb6f34672f09974f7a0064)**

Reviewer: **Peter Bruin**

Author: **Julian Rüth**

_Issue created by migration from https://trac.sagemath.org/ticket/16328_





---

archive/issue_events_215715.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2014-05-11T11:03:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16328",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16328#event-215715"
}
```



---

archive/issue_events_215716.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2014-05-11T11:03:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16328",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20categories",
    "label_color": "0000ff",
    "label_name": "component: categories",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16328#event-215716"
}
```



---

archive/issue_events_215717.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2014-05-11T11:03:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16328",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16328#event-215717"
}
```



---

archive/issue_events_215718.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2014-05-11T11:03:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16328",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16328#event-215718"
}
```



---

archive/issue_comments_212548.json:
```json
{
    "body": "Branch: **[u/saraedum/ticket/16328](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/16328)**",
    "created_at": "2014-05-11T13:03:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16328#issuecomment-212548",
    "user": "https://github.com/saraedum"
}
```

Branch: **[u/saraedum/ticket/16328](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/16328)**



---

archive/issue_events_215719.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2014-05-11T13:04:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16328",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16328#event-215719"
}
```



---

archive/issue_comments_212549.json:
```json
{
    "body": "Commit: **[324bef3](https://github.com/sagemath/sagetrac-mirror/commit/324bef389799080b91bb6f34672f09974f7a0064)**",
    "created_at": "2014-05-11T13:04:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16328#issuecomment-212549",
    "user": "https://github.com/saraedum"
}
```

Commit: **[324bef3](https://github.com/sagemath/sagetrac-mirror/commit/324bef389799080b91bb6f34672f09974f7a0064)**



---

archive/issue_comments_212550.json:
```json
{
    "body": "Author: **Julian R\u00fcth**",
    "created_at": "2014-05-11T13:04:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16328#issuecomment-212550",
    "user": "https://github.com/saraedum"
}
```

Author: **Julian Rüth**



---

archive/issue_comments_212551.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/324bef389799080b91bb6f34672f09974f7a0064\">324bef3</a></td><td><code>Added a category parameter to Module</code></td></tr></table>\n",
    "created_at": "2014-05-11T13:04:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16328#issuecomment-212551",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/324bef389799080b91bb6f34672f09974f7a0064">324bef3</a></td><td><code>Added a category parameter to Module</code></td></tr></table>




---

archive/issue_events_215720.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2014-05-11T14:29:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16328",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16328#event-215720"
}
```



---

archive/issue_events_215721.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2014-05-11T14:29:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16328",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16328#event-215721"
}
```



---

archive/issue_comments_212552.json:
```json
{
    "body": "Reviewer: **Peter Bruin**",
    "created_at": "2014-05-12T09:43:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16328#issuecomment-212552",
    "user": "https://github.com/pjbruin"
}
```

Reviewer: **Peter Bruin**



---

archive/issue_comments_212553.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nLooks good and useful, and passes all tests.",
    "created_at": "2014-05-12T09:43:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16328#issuecomment-212553",
    "user": "https://github.com/pjbruin"
}
```

<a id='comment:4'>Comment 4:</a>
Looks good and useful, and passes all tests.



---

archive/issue_events_215722.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2014-05-12T09:43:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16328",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16328#event-215722"
}
```



---

archive/issue_events_215723.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2014-05-12T09:43:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16328",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16328#event-215723"
}
```



---

archive/issue_events_215724.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-05-13T07:58:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16328",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16328#event-215724"
}
```



---

archive/issue_events_215725.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "5c3a0890d49f14c48d03ee19b4a31475ba721301",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-05-13T07:58:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16328",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16328#event-215725"
}
```



---

archive/issue_comments_212554.json:
```json
{
    "body": "Changed branch from **[u/saraedum/ticket/16328](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/16328)** to **[324bef3](https://github.com/sagemath/sagetrac-mirror/commit/324bef389799080b91bb6f34672f09974f7a0064)**",
    "created_at": "2014-05-13T07:58:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16328#issuecomment-212554",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/saraedum/ticket/16328](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/16328)** to **[324bef3](https://github.com/sagemath/sagetrac-mirror/commit/324bef389799080b91bb6f34672f09974f7a0064)**
