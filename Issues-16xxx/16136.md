# Issue 16136: Prevent user from doing stupid thing with the doc building system

archive/issues_015899.json:
```json
{
    "assignees": [],
    "body": "Right now there are at least two stupid thing the user can do which can get him in trouble:\n\n- type make in the `src/doc` directory : this **delete** the doc !\n- type `sage docbuild reference/non_existing_doc` : this will create a directory is the doc system that prevents the doc from compiling and which even `make doc-clean` doesn't remove.\n\nI'm adding two small safe guard to prevent those.\n\nKeywords: **days57**\n\nBranch: **[e708895](https://github.com/sagemath/sagetrac-mirror/commit/e70889554269677cec8a7a268829a441c4e61eef)**\n\nReviewer: **Volker Braun**\n\nAuthor: **Florent Hivert**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16136_\n\n",
    "closed_at": "2014-04-11T19:41:26Z",
    "created_at": "2014-04-11T16:44:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Prevent user from doing stupid thing with the doc building system",
    "type": "issue",
    "updated_at": "2014-04-11T19:44:17Z",
    "url": "https://github.com/sagemath/sage/issues/16136",
    "user": "https://github.com/hivert"
}
```
Right now there are at least two stupid thing the user can do which can get him in trouble:

- type make in the `src/doc` directory : this **delete** the doc !
- type `sage docbuild reference/non_existing_doc` : this will create a directory is the doc system that prevents the doc from compiling and which even `make doc-clean` doesn't remove.

I'm adding two small safe guard to prevent those.

Keywords: **days57**

Branch: **[e708895](https://github.com/sagemath/sagetrac-mirror/commit/e70889554269677cec8a7a268829a441c4e61eef)**

Reviewer: **Volker Braun**

Author: **Florent Hivert**

_Issue created by migration from https://trac.sagemath.org/ticket/16136_





---

archive/issue_events_205243.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2014-04-11T16:44:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16136",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16136#event-205243"
}
```



---

archive/issue_events_205244.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2014-04-11T16:44:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16136",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16136#event-205244"
}
```



---

archive/issue_comments_208491.json:
```json
{
    "body": "Branch: **[u/hivert/prevent_user_from_doing_stupid_thing_with_the_doc_building_system](https://github.com/sagemath/sagetrac-mirror/tree/u/hivert/prevent_user_from_doing_stupid_thing_with_the_doc_building_system)**",
    "created_at": "2014-04-11T16:50:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16136",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16136#issuecomment-208491",
    "user": "https://github.com/hivert"
}
```

Branch: **[u/hivert/prevent_user_from_doing_stupid_thing_with_the_doc_building_system](https://github.com/sagemath/sagetrac-mirror/tree/u/hivert/prevent_user_from_doing_stupid_thing_with_the_doc_building_system)**



---

archive/issue_comments_208492.json:
```json
{
    "body": "Commit: **[3bbfb3b](https://github.com/sagemath/sagetrac-mirror/commit/3bbfb3b41ebc72bb29adda5326ce9f0037814fdb)**",
    "created_at": "2014-04-11T16:52:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16136",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16136#issuecomment-208492",
    "user": "https://github.com/hivert"
}
```

Commit: **[3bbfb3b](https://github.com/sagemath/sagetrac-mirror/commit/3bbfb3b41ebc72bb29adda5326ce9f0037814fdb)**



---

archive/issue_events_205245.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2014-04-11T16:52:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16136",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16136#event-205245"
}
```



---

archive/issue_events_205246.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2014-04-11T16:52:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16136",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16136#event-205246"
}
```



---

archive/issue_comments_208493.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,6 @@\n+Right now there are at least two stupid thing the user can do which can get him in trouble:\n \n+- type make in the `src/doc` directory : this **delete** the doc !\n+- type `sage docbuild reference/non_existing_doc` : this will create a directory is the doc system that prevents the doc from compiling and which even `make doc-clean` doesn't remove.\n+\n+I'm adding two small safe guard to prevent those.\n``````\n",
    "created_at": "2014-04-11T16:52:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16136",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16136#issuecomment-208493",
    "user": "https://github.com/hivert"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,6 @@
+Right now there are at least two stupid thing the user can do which can get him in trouble:
 
+- type make in the `src/doc` directory : this **delete** the doc !
+- type `sage docbuild reference/non_existing_doc` : this will create a directory is the doc system that prevents the doc from compiling and which even `make doc-clean` doesn't remove.
+
+I'm adding two small safe guard to prevent those.
``````




---

archive/issue_comments_208494.json:
```json
{
    "body": "Author: **Florent Hivert**",
    "created_at": "2014-04-11T16:52:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16136",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16136#issuecomment-208494",
    "user": "https://github.com/hivert"
}
```

Author: **Florent Hivert**



---

archive/issue_comments_208495.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3bbfb3b41ebc72bb29adda5326ce9f0037814fdb\">3bbfb3b</a></td><td><code>Safe guard against wrong doc build command</code></td></tr></table>\n",
    "created_at": "2014-04-11T16:52:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16136",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16136#issuecomment-208495",
    "user": "https://github.com/hivert"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3bbfb3b41ebc72bb29adda5326ce9f0037814fdb">3bbfb3b</a></td><td><code>Safe guard against wrong doc build command</code></td></tr></table>




---

archive/issue_events_205247.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2014-04-11T16:52:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16136",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16136#event-205247"
}
```



---

archive/issue_comments_208496.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2014-04-11T16:53:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16136",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16136#issuecomment-208496",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_comments_208497.json:
```json
{
    "body": "Changed keywords from **** to **days57**",
    "created_at": "2014-04-11T16:53:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16136",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16136#issuecomment-208497",
    "user": "https://github.com/vbraun"
}
```

Changed keywords from **** to **days57**



---

archive/issue_comments_208498.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d103c517c2755b3668d963b193e0db1fa6ff21c9\">d103c51</a></td><td><code>Prevent the user from getting in trouble when building the doc</code></td></tr></table>\n",
    "created_at": "2014-04-11T17:15:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16136",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16136#issuecomment-208498",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:4'>Comment 4:</a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d103c517c2755b3668d963b193e0db1fa6ff21c9">d103c51</a></td><td><code>Prevent the user from getting in trouble when building the doc</code></td></tr></table>




---

archive/issue_comments_208499.json:
```json
{
    "body": "Changed commit from **[3bbfb3b](https://github.com/sagemath/sagetrac-mirror/commit/3bbfb3b41ebc72bb29adda5326ce9f0037814fdb)** to **[d103c51](https://github.com/sagemath/sagetrac-mirror/commit/d103c517c2755b3668d963b193e0db1fa6ff21c9)**",
    "created_at": "2014-04-11T17:15:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16136",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16136#issuecomment-208499",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[3bbfb3b](https://github.com/sagemath/sagetrac-mirror/commit/3bbfb3b41ebc72bb29adda5326ce9f0037814fdb)** to **[d103c51](https://github.com/sagemath/sagetrac-mirror/commit/d103c517c2755b3668d963b193e0db1fa6ff21c9)**



---

archive/issue_comments_208500.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nLooks good. I wonder if the message when a user types `make` from the `src/doc` directory should mention (instead, or in addition) `sage --docbuild ...`.",
    "created_at": "2014-04-11T18:25:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16136",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16136#issuecomment-208500",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:5'>Comment 5:</a>
Looks good. I wonder if the message when a user types `make` from the `src/doc` directory should mention (instead, or in addition) `sage --docbuild ...`.



---

archive/issue_comments_208501.json:
```json
{
    "body": "Changed commit from **[d103c51](https://github.com/sagemath/sagetrac-mirror/commit/d103c517c2755b3668d963b193e0db1fa6ff21c9)** to **[e708895](https://github.com/sagemath/sagetrac-mirror/commit/e70889554269677cec8a7a268829a441c4e61eef)**",
    "created_at": "2014-04-11T19:24:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16136",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16136#issuecomment-208501",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[d103c51](https://github.com/sagemath/sagetrac-mirror/commit/d103c517c2755b3668d963b193e0db1fa6ff21c9)** to **[e708895](https://github.com/sagemath/sagetrac-mirror/commit/e70889554269677cec8a7a268829a441c4e61eef)**



---

archive/issue_comments_208502.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e70889554269677cec8a7a268829a441c4e61eef\">e708895</a></td><td><code>Implemented John's suggestion.</code></td></tr></table>\n",
    "created_at": "2014-04-11T19:24:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16136",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16136#issuecomment-208502",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:6'>Comment 6:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e70889554269677cec8a7a268829a441c4e61eef">e708895</a></td><td><code>Implemented John's suggestion.</code></td></tr></table>




---

archive/issue_comments_208503.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nReplying to [@jhpalmieri](#comment%3A5):\n> Looks good. I wonder if the message when a user types `make` from the `src/doc` directory should mention (instead, or in addition) `sage --docbuild ...`.\n\nGood idea ! Done.",
    "created_at": "2014-04-11T19:24:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16136",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16136#issuecomment-208503",
    "user": "https://github.com/hivert"
}
```

<a id='comment:7'>Comment 7:</a>
Replying to [@jhpalmieri](#comment%3A5):
> Looks good. I wonder if the message when a user types `make` from the `src/doc` directory should mention (instead, or in addition) `sage --docbuild ...`.

Good idea ! Done.



---

archive/issue_comments_208504.json:
```json
{
    "body": "Changed branch from **[u/hivert/prevent_user_from_doing_stupid_thing_with_the_doc_building_system](https://github.com/sagemath/sagetrac-mirror/tree/u/hivert/prevent_user_from_doing_stupid_thing_with_the_doc_building_system)** to **[e708895](https://github.com/sagemath/sagetrac-mirror/commit/e70889554269677cec8a7a268829a441c4e61eef)**",
    "created_at": "2014-04-11T19:41:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16136",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16136#issuecomment-208504",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/hivert/prevent_user_from_doing_stupid_thing_with_the_doc_building_system](https://github.com/sagemath/sagetrac-mirror/tree/u/hivert/prevent_user_from_doing_stupid_thing_with_the_doc_building_system)** to **[e708895](https://github.com/sagemath/sagetrac-mirror/commit/e70889554269677cec8a7a268829a441c4e61eef)**



---

archive/issue_events_205248.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-04-11T19:41:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16136",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16136#event-205248"
}
```



---

archive/issue_events_205249.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "8b3ae1c071f2a09b5a528852dae2a3ae3d415053",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-04-11T19:41:26Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16136",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16136#event-205249"
}
```



---

archive/issue_comments_208505.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nI would have preferred `--docbuild` to `-docbuild`, and also \"information\" to \"informations\". Oh well, too late.",
    "created_at": "2014-04-11T19:44:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16136",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16136#issuecomment-208505",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:9'>Comment 9:</a>
I would have preferred `--docbuild` to `-docbuild`, and also "information" to "informations". Oh well, too late.



---

archive/issue_comments_208506.json:
```json
{
    "body": "Changed commit from **[e708895](https://github.com/sagemath/sagetrac-mirror/commit/e70889554269677cec8a7a268829a441c4e61eef)** to none",
    "created_at": "2014-04-11T19:44:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16136",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16136#issuecomment-208506",
    "user": "https://github.com/jhpalmieri"
}
```

Changed commit from **[e708895](https://github.com/sagemath/sagetrac-mirror/commit/e70889554269677cec8a7a268829a441c4e61eef)** to none
