# Issue 16554: Sage's development release is 6.1beta1 on sagemath.org

archive/issues_016317.json:
```json
{
    "body": "I think this should be something like 6.3betaN for some N. \n\n**[edit]** Not true anymore!\n\n\n\nIssue created by migration from https://trac.sagemath.org/ticket/16554\n\n",
    "closed_at": "2015-02-19T12:03:54Z",
    "created_at": "2014-06-26T16:41:01Z",
    "labels": [
        "component: website/wiki",
        "minor",
        "invalid"
    ],
    "title": "Sage's development release is 6.1beta1 on sagemath.org",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/16554",
    "user": "https://github.com/bgrenet"
}
```
I think this should be something like 6.3betaN for some N. 

**[edit]** Not true anymore!



Issue created by migration from https://trac.sagemath.org/ticket/16554





---

archive/issue_events_148019.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16554",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16554#event-148019"
}
```



---

archive/issue_events_148020.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16554",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16554#event-148020"
}
```



---

archive/issue_events_148021.json:
```json
{
    "actor": "https://github.com/bgrenet",
    "created_at": "2015-02-18T20:20:18Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16554",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16554#event-148021"
}
```



---

archive/issue_comments_217427.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1 +1,5 @@\n I think this should be something like 6.3betaN for some N. \n+\n+**[edit]** Not true anymore!\n+\n+\n``````\n",
    "created_at": "2015-02-18T20:20:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16554",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16554#issuecomment-217427",
    "user": "https://github.com/bgrenet"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1 +1,5 @@
 I think this should be something like 6.3betaN for some N. 
+
+**[edit]** Not true anymore!
+
+
``````




---

archive/issue_events_148022.json:
```json
{
    "actor": "https://github.com/bgrenet",
    "created_at": "2015-02-18T20:20:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/16554",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16554#event-148022"
}
```



---

archive/issue_events_148023.json:
```json
{
    "actor": "https://github.com/bgrenet",
    "created_at": "2015-02-18T20:20:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/16554",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16554#event-148023"
}
```



---

archive/issue_events_148024.json:
```json
{
    "actor": "https://github.com/bgrenet",
    "created_at": "2015-02-18T20:20:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/16554",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16554#event-148024"
}
```



---

archive/issue_events_148025.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-02-19T12:03:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/16554",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16554#event-148025"
}
```



---

archive/issue_events_148026.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-02-19T12:03:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/16554",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16554#event-148026"
}
```
