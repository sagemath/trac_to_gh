# Issue 16972: Factor out common code in fullerenes and fusenes

archive/issues_016735.json:
```json
{
    "assignees": [],
    "body": "At the moment the methods fullerenes and the method fusenes in graph_generators have common code to read the input planar_code format. If plantri is added, then even more methods will use this code, so this ticket moves that code to a separate method.\n\nBranch/Commit: **[bb9be9e](https://github.com/sagemath/sagetrac-mirror/commit/bb9be9e922a8ef1c4621fda643f1fd4cd10f7891)**\n\nReviewer: **Nathann Cohen**\n\nAuthor: **Nico Van Cleemput**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16972_\n\n",
    "closed_at": "2014-09-16T18:45:08Z",
    "created_at": "2014-09-11T18:53:00Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Factor out common code in fullerenes and fusenes",
    "type": "issue",
    "updated_at": "2014-09-16T18:45:08Z",
    "url": "https://github.com/sagemath/sage/issues/16972",
    "user": "https://github.com/nvcleemp"
}
```
At the moment the methods fullerenes and the method fusenes in graph_generators have common code to read the input planar_code format. If plantri is added, then even more methods will use this code, so this ticket moves that code to a separate method.

Branch/Commit: **[bb9be9e](https://github.com/sagemath/sagetrac-mirror/commit/bb9be9e922a8ef1c4621fda643f1fd4cd10f7891)**

Reviewer: **Nathann Cohen**

Author: **Nico Van Cleemput**

_Issue created by migration from https://trac.sagemath.org/ticket/16972_





---

archive/issue_events_224961.json:
```json
{
    "actor": "https://github.com/nvcleemp",
    "created_at": "2014-09-11T18:53:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16972",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16972#event-224961"
}
```



---

archive/issue_events_224962.json:
```json
{
    "actor": "https://github.com/nvcleemp",
    "created_at": "2014-09-11T18:53:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16972",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16972#event-224962"
}
```



---

archive/issue_events_224963.json:
```json
{
    "actor": "https://github.com/nvcleemp",
    "created_at": "2014-09-11T18:53:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16972",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16972#event-224963"
}
```



---

archive/issue_events_224964.json:
```json
{
    "actor": "https://github.com/nvcleemp",
    "created_at": "2014-09-11T18:53:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16972",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16972#event-224964"
}
```



---

archive/issue_comments_226730.json:
```json
{
    "body": "Commit: **[bb9be9e](https://github.com/sagemath/sagetrac-mirror/commit/bb9be9e922a8ef1c4621fda643f1fd4cd10f7891)**",
    "created_at": "2014-09-11T19:49:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16972",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16972#issuecomment-226730",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[bb9be9e](https://github.com/sagemath/sagetrac-mirror/commit/bb9be9e922a8ef1c4621fda643f1fd4cd10f7891)**



---

archive/issue_comments_226731.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/cedb88165076a7195d8253f6c0d49b05f2c148b9\">cedb881</a></td><td><code>Trac #16972: Moved code to read planar code to separate method</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8d0c0d500dc3c8dbe0351bc77e38e05c86e22877\">8d0c0d5</a></td><td><code>Trac #16972: Extended documentation of _read_planar_code</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/bb9be9e922a8ef1c4621fda643f1fd4cd10f7891\">bb9be9e</a></td><td><code>Trac #16972: Use the new _read_planar_code method in fullerenes and fusenes.</code></td></tr></table>\n",
    "created_at": "2014-09-11T19:49:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16972",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16972#issuecomment-226731",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:1" align="right">Comment 1</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/cedb88165076a7195d8253f6c0d49b05f2c148b9">cedb881</a></td><td><code>Trac #16972: Moved code to read planar code to separate method</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8d0c0d500dc3c8dbe0351bc77e38e05c86e22877">8d0c0d5</a></td><td><code>Trac #16972: Extended documentation of _read_planar_code</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/bb9be9e922a8ef1c4621fda643f1fd4cd10f7891">bb9be9e</a></td><td><code>Trac #16972: Use the new _read_planar_code method in fullerenes and fusenes.</code></td></tr></table>




---

archive/issue_comments_226732.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nI'm not sure whether this really is the correct place for this method, but I'm sure somebody will enlighten me.",
    "created_at": "2014-09-11T19:53:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16972",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16972#issuecomment-226732",
    "user": "https://github.com/nvcleemp"
}
```

<div id="comment:2" align="right">Comment 2</div>

I'm not sure whether this really is the correct place for this method, but I'm sure somebody will enlighten me.



---

archive/issue_comments_226733.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nThis method will also be used by the new methods introduced in #16970.",
    "created_at": "2014-09-12T11:07:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16972",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16972#issuecomment-226733",
    "user": "https://github.com/nvcleemp"
}
```

<div id="comment:3" align="right">Comment 3</div>

This method will also be used by the new methods introduced in #16970.



---

archive/issue_events_224965.json:
```json
{
    "actor": "https://github.com/nvcleemp",
    "created_at": "2014-09-12T11:07:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16972",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16972#event-224965"
}
```



---

archive/issue_comments_226734.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nWow. Totally clean patch. Couldn't add a thing `:-P`\n\nNathann",
    "created_at": "2014-09-16T10:03:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16972",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16972#issuecomment-226734",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:4" align="right">Comment 4</div>

Wow. Totally clean patch. Couldn't add a thing `:-P`

Nathann



---

archive/issue_comments_226735.json:
```json
{
    "body": "Reviewer: **Nathann Cohen**",
    "created_at": "2014-09-16T10:03:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16972",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16972#issuecomment-226735",
    "user": "https://github.com/nathanncohen"
}
```

Reviewer: **Nathann Cohen**



---

archive/issue_events_224966.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-09-16T10:03:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16972",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16972#event-224966"
}
```



---

archive/issue_events_224967.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-09-16T10:03:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16972",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16972#event-224967"
}
```



---

archive/issue_comments_226736.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nThanks, I occasionally have good moments. ;-)",
    "created_at": "2014-09-16T13:03:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16972",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16972#issuecomment-226736",
    "user": "https://github.com/nvcleemp"
}
```

<div id="comment:5" align="right">Comment 5</div>

Thanks, I occasionally have good moments. ;-)



---

archive/issue_events_224968.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-09-16T18:45:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16972",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16972#event-224968"
}
```



---

archive/issue_events_224969.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "c06126f92bf120be4813c1be84973849d30b2c5e",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-09-16T18:45:08Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16972",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16972#event-224969"
}
```



---

archive/issue_comments_226737.json:
```json
{
    "body": "Changed branch from **[u/nvcleemp/read-planarcode](https://github.com/sagemath/sagetrac-mirror/tree/u/nvcleemp/read-planarcode)** to **[bb9be9e](https://github.com/sagemath/sagetrac-mirror/commit/bb9be9e922a8ef1c4621fda643f1fd4cd10f7891)**",
    "created_at": "2014-09-16T18:45:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16972",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16972#issuecomment-226737",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/nvcleemp/read-planarcode](https://github.com/sagemath/sagetrac-mirror/tree/u/nvcleemp/read-planarcode)** to **[bb9be9e](https://github.com/sagemath/sagetrac-mirror/commit/bb9be9e922a8ef1c4621fda643f1fd4cd10f7891)**
