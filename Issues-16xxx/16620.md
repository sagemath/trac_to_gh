# Issue 16620: Don't mention setup_latex_preamble in introductory documentation

archive/issues_016383.json:
```json
{
    "assignees": [],
    "body": "Previously, the introductory documentation of `graph_latex` recommended calling `setup_latex_preamble`. However, as pointed out in [#16557 comment:2](https://github.com/sagemath/sage/issues/16557#comment:2), this is not necessary: `graph._latex_` calls `setup_latex_preamble` anyway. Therefore, the corresponding text has now been removed from this introduction.\n\nNote, however, that the tutorial (http://www.sagemath.org/doc/tutorial/latex.html#an-example-combinatorial-graphs-with-tkz-graph) has not been touched: there, the use of `setup_latex_preamble` rather seems to illustrate the behaviour of the `latex` module.\n\n\nCC:  @dkrenn @sagetrac-elixyre @nathanncohen\n\nKeywords: **graph, latex**\n\nBranch/Commit: **[558f47d](https://github.com/sagemath/sagetrac-mirror/commit/558f47d466863c13eb4e467399cf28c2907b9041)**\n\nReviewer: **Daniel Krenn**\n\nAuthor: **Clemens Heuberger**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16620_\n\n",
    "closed_at": "2014-07-20T03:18:48Z",
    "created_at": "2014-07-06T16:18:37Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Don't mention setup_latex_preamble in introductory documentation",
    "type": "issue",
    "updated_at": "2014-07-20T03:18:48Z",
    "url": "https://github.com/sagemath/sage/issues/16620",
    "user": "https://github.com/cheuberg"
}
```
Previously, the introductory documentation of `graph_latex` recommended calling `setup_latex_preamble`. However, as pointed out in [#16557 comment:2](https://github.com/sagemath/sage/issues/16557#comment:2), this is not necessary: `graph._latex_` calls `setup_latex_preamble` anyway. Therefore, the corresponding text has now been removed from this introduction.

Note, however, that the tutorial (http://www.sagemath.org/doc/tutorial/latex.html#an-example-combinatorial-graphs-with-tkz-graph) has not been touched: there, the use of `setup_latex_preamble` rather seems to illustrate the behaviour of the `latex` module.


CC:  @dkrenn @sagetrac-elixyre @nathanncohen

Keywords: **graph, latex**

Branch/Commit: **[558f47d](https://github.com/sagemath/sagetrac-mirror/commit/558f47d466863c13eb4e467399cf28c2907b9041)**

Reviewer: **Daniel Krenn**

Author: **Clemens Heuberger**

_Issue created by migration from https://trac.sagemath.org/ticket/16620_





---

archive/issue_events_220021.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-07-06T16:18:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16620",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16620#event-220021"
}
```



---

archive/issue_events_220022.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-07-06T16:18:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16620",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16620#event-220022"
}
```



---

archive/issue_events_220023.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-07-06T16:18:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16620",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16620#event-220023"
}
```



---

archive/issue_events_220024.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-07-06T16:18:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16620",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16620#event-220024"
}
```



---

archive/issue_events_220025.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-07-06T16:18:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16620",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16620#event-220025"
}
```



---

archive/issue_comments_219121.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nThe patch looks fine, documentation builds, all tests pass.",
    "created_at": "2014-07-19T18:02:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16620",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16620#issuecomment-219121",
    "user": "https://github.com/dkrenn"
}
```

<div id="comment:2" align="right">Comment 2</div>

The patch looks fine, documentation builds, all tests pass.



---

archive/issue_events_220026.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2014-07-19T18:02:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16620",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16620#event-220026"
}
```



---

archive/issue_events_220027.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2014-07-19T18:02:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16620",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16620#event-220027"
}
```



---

archive/issue_comments_219122.json:
```json
{
    "body": "Reviewer: **Daniel Krenn**",
    "created_at": "2014-07-19T18:02:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16620",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16620#issuecomment-219122",
    "user": "https://github.com/dkrenn"
}
```

Reviewer: **Daniel Krenn**



---

archive/issue_events_220028.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-07-20T03:18:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16620",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16620#event-220028"
}
```



---

archive/issue_events_220029.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-07-20T03:18:48Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16620",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16620#event-220029"
}
```



---

archive/issue_comments_219123.json:
```json
{
    "body": "Changed branch from **[u/cheuberg/graphs/remove-setup-latex-preamble](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/graphs/remove-setup-latex-preamble)** to **[558f47d](https://github.com/sagemath/sagetrac-mirror/commit/558f47d466863c13eb4e467399cf28c2907b9041)**",
    "created_at": "2014-07-20T03:18:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16620",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16620#issuecomment-219123",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/cheuberg/graphs/remove-setup-latex-preamble](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/graphs/remove-setup-latex-preamble)** to **[558f47d](https://github.com/sagemath/sagetrac-mirror/commit/558f47d466863c13eb4e467399cf28c2907b9041)**
