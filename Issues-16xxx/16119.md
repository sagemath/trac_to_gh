# Issue 16119: Patch Python uuid module

archive/issues_015882.json:
```json
{
    "assignees": [],
    "body": "This prevents \"heavy side effects\" which include segfaults when building sagenb on Cygwin64.\nSee http://bugs.python.org/issue11063.\n\nKeywords: **cygwin days57 python uuid**\n\nBranch/Commit: **[750eff1](https://github.com/sagemath/sagetrac-mirror/commit/750eff146c3a5da7b469f5c3cba627eeb3b596d3)**\n\nReviewer: **Volker Braun**\n\nAuthor: **Jean-Pierre Flori**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16119_\n\n",
    "closed_at": "2014-04-14T16:55:42Z",
    "created_at": "2014-04-10T09:04:27Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Patch Python uuid module",
    "type": "issue",
    "updated_at": "2014-04-14T16:55:42Z",
    "url": "https://github.com/sagemath/sage/issues/16119",
    "user": "https://github.com/sagetrac-jpflori"
}
```
This prevents "heavy side effects" which include segfaults when building sagenb on Cygwin64.
See http://bugs.python.org/issue11063.

Keywords: **cygwin days57 python uuid**

Branch/Commit: **[750eff1](https://github.com/sagemath/sagetrac-mirror/commit/750eff146c3a5da7b469f5c3cba627eeb3b596d3)**

Reviewer: **Volker Braun**

Author: **Jean-Pierre Flori**

_Issue created by migration from https://trac.sagemath.org/ticket/16119_





---

archive/issue_events_212675.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2014-04-10T09:04:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16119",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16119#event-212675"
}
```



---

archive/issue_events_212676.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2014-04-10T09:04:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16119",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16119#event-212676"
}
```



---

archive/issue_events_212677.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2014-04-10T09:04:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16119",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16119#event-212677"
}
```



---

archive/issue_events_212678.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2014-04-10T09:04:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16119",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16119#event-212678"
}
```



---

archive/issue_comments_208233.json:
```json
{
    "body": "Author: **Jean-Pierre Flori**",
    "created_at": "2014-04-13T19:34:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16119",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16119#issuecomment-208233",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Author: **Jean-Pierre Flori**



---

archive/issue_comments_208234.json:
```json
{
    "body": "Commit: **[750eff1](https://github.com/sagemath/sagetrac-mirror/commit/750eff146c3a5da7b469f5c3cba627eeb3b596d3)**",
    "created_at": "2014-04-13T19:34:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16119",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16119#issuecomment-208234",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Commit: **[750eff1](https://github.com/sagemath/sagetrac-mirror/commit/750eff146c3a5da7b469f5c3cba627eeb3b596d3)**



---

archive/issue_comments_208235.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/750eff146c3a5da7b469f5c3cba627eeb3b596d3\">750eff1</a></td><td><code>Patch Python uuid module to lower side effects when it is imported.</code></td></tr></table>\n",
    "created_at": "2014-04-13T19:34:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16119",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16119#issuecomment-208235",
    "user": "https://github.com/sagetrac-jpflori"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/750eff146c3a5da7b469f5c3cba627eeb3b596d3">750eff1</a></td><td><code>Patch Python uuid module to lower side effects when it is imported.</code></td></tr></table>




---

archive/issue_comments_208236.json:
```json
{
    "body": "Branch: **[u/jpflori/ticket/16119](https://github.com/sagemath/sagetrac-mirror/tree/u/jpflori/ticket/16119)**",
    "created_at": "2014-04-13T19:34:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16119",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16119#issuecomment-208236",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Branch: **[u/jpflori/ticket/16119](https://github.com/sagemath/sagetrac-mirror/tree/u/jpflori/ticket/16119)**



---

archive/issue_events_212679.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2014-04-13T19:34:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16119",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16119#event-212679"
}
```



---

archive/issue_comments_208237.json:
```json
{
    "body": "Changed keywords from **cygwin** to **cygwin days57 python uuid**",
    "created_at": "2014-04-13T19:34:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16119",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16119#issuecomment-208237",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Changed keywords from **cygwin** to **cygwin days57 python uuid**



---

archive/issue_comments_208238.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2014-04-13T22:00:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16119",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16119#issuecomment-208238",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_events_212680.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-04-13T22:00:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16119",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16119#event-212680"
}
```



---

archive/issue_events_212681.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-04-13T22:00:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16119",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16119#event-212681"
}
```



---

archive/issue_events_212682.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-04-14T16:55:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16119",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16119#event-212682"
}
```



---

archive/issue_events_212683.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "12a3beb3932a07538dfedfa11057ff7629ca7184",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-04-14T16:55:42Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16119",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16119#event-212683"
}
```



---

archive/issue_comments_208239.json:
```json
{
    "body": "Changed branch from **[u/jpflori/ticket/16119](https://github.com/sagemath/sagetrac-mirror/tree/u/jpflori/ticket/16119)** to **[750eff1](https://github.com/sagemath/sagetrac-mirror/commit/750eff146c3a5da7b469f5c3cba627eeb3b596d3)**",
    "created_at": "2014-04-14T16:55:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16119",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16119#issuecomment-208239",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jpflori/ticket/16119](https://github.com/sagemath/sagetrac-mirror/tree/u/jpflori/ticket/16119)** to **[750eff1](https://github.com/sagemath/sagetrac-mirror/commit/750eff146c3a5da7b469f5c3cba627eeb3b596d3)**
