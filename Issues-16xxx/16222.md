# Issue 16222: Faster exactification using numeric minpoly

archive/issues_015985.json:
```json
{
    "assignees": [],
    "body": "This is a spinoff from [#14239 comment:13](https://github.com/sagemath/sage/issues/14239#comment:13). There I noticed that for a large symbolic expressions `b`, the call `b.minpoly()` was a lot (by several orders of magnitude) faster than the call `QQbar(b).minpoly()`. We should try to make this speed gain available to those algebraic numbers which were constructed from symbolic expressions.\n\nI now know that the speed gain is almost certainly due to the numeric algorithm in `calculus.minpoly`. So what we should in my opinion do is use that numeric algorithm to obtain a minimal polynomial of the whole expression, and if that succeeds, base subsequent exactifications on that polynomial instead of the nested tree of algebraic number descriptors.\n\nDepends on #15605\n\nBranch/Commit: **[u/vdelecroix/16222](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/16222) @ [8773468](https://github.com/sagemath/sagetrac-mirror/commit/8773468739544ed9f12b836c7f28a06be37ffd94)**\n\nReviewer: **Vincent Delecroix**\n\nAuthor: **Martin von Gagern**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16222_\n\n",
    "created_at": "2014-04-23T19:33:11Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/needs%20info"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Faster exactification using numeric minpoly",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/16222",
    "user": "https://github.com/gagern"
}
```
This is a spinoff from [#14239 comment:13](https://github.com/sagemath/sage/issues/14239#comment:13). There I noticed that for a large symbolic expressions `b`, the call `b.minpoly()` was a lot (by several orders of magnitude) faster than the call `QQbar(b).minpoly()`. We should try to make this speed gain available to those algebraic numbers which were constructed from symbolic expressions.

I now know that the speed gain is almost certainly due to the numeric algorithm in `calculus.minpoly`. So what we should in my opinion do is use that numeric algorithm to obtain a minimal polynomial of the whole expression, and if that succeeds, base subsequent exactifications on that polynomial instead of the nested tree of algebraic number descriptors.

Depends on #15605

Branch/Commit: **[u/vdelecroix/16222](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/16222) @ [8773468](https://github.com/sagemath/sagetrac-mirror/commit/8773468739544ed9f12b836c7f28a06be37ffd94)**

Reviewer: **Vincent Delecroix**

Author: **Martin von Gagern**

_Issue created by migration from https://trac.sagemath.org/ticket/16222_





---

archive/issue_events_214069.json:
```json
{
    "actor": "https://github.com/gagern",
    "created_at": "2014-04-23T19:33:11Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16222#event-214069"
}
```



---

archive/issue_events_214070.json:
```json
{
    "actor": "https://github.com/gagern",
    "created_at": "2014-04-23T19:33:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
    "label_color": "0000ff",
    "label_name": "component: number fields",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16222#event-214070"
}
```



---

archive/issue_events_214071.json:
```json
{
    "actor": "https://github.com/gagern",
    "created_at": "2014-04-23T19:33:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16222#event-214071"
}
```



---

archive/issue_events_214072.json:
```json
{
    "actor": "https://github.com/gagern",
    "created_at": "2014-04-23T19:33:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16222#event-214072"
}
```



---

archive/issue_comments_209873.json:
```json
{
    "body": "Branch: **[u/gagern/ticket/16222](https://github.com/sagemath/sagetrac-mirror/tree/u/gagern/ticket/16222)**",
    "created_at": "2014-04-23T19:41:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16222#issuecomment-209873",
    "user": "https://github.com/gagern"
}
```

Branch: **[u/gagern/ticket/16222](https://github.com/sagemath/sagetrac-mirror/tree/u/gagern/ticket/16222)**



---

archive/issue_comments_209874.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nThese modifications are not ready for inclusion yet, they need some discussion first.\n\n1. How to get the symbolic expression from `expression_conversion` into `AlgebraicNumber_base`.\n\n   Right now, I use direct access to an underscore-prefixed variable called `_symbolic_expression`. I'm not sure whether that is acceptable. If cross-module interfaces should avoid using underscores, we'll have to look for alternatives. But I rely on the fact that the expression really describes the number in question, so this certainly isn't a variable the user should be allowed to tweak. So I don't know a better way yet.\n\n2. Default state of that variable.\n\n   Right now, I have a class variable which defaults to `None` and instance variables which will override this when required. Alternatives would be setting an instance variable in some (as of now non-existant) constructor, or avoding a default value altogether and using `hasattr` instead. I simply wrote what I'd do in my own code, but if there is a sage convention to write things differently, I'll gladly comply.\n\n3. When to compute the minpoly.\n\n   I now do the computation on demand, hoping that expressions cannot be legally modified after their creation. If they can be modified, we might want to do the minpoly computation when we do the conversion from symbolic to algebraic. Likewise if you have reasons to believe that minpoly computation should not make that conversion noticably slower than it already is.\n\n4. Test case dependency.\n\n   I just noticed that my current doctest depends on ticket:14239, but that should be easy to fix. If you have a favorite example, let me know, but otherwise I'll come up with something.\n\n5. The `_more_precision` loop.\n\n   I have more trouble testing that `_more_precision` codepath inside `_exact_symbolic`. Can anyone come up with a situation where identifying the correct root would need more than default precision but where numeric minpoly computation would still succeed? If not, what shall we do? Shall we deactivate that whole loop, and simply return without taking action if we find more than one root which could match?\n\n6. How to obtain the roots.\n\n   Is `p.roots(self.parent(), False)` the right thing to do, or is this too high-level, should I use something more basic here?\n  \n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/bb01e0b15e1a4e7e1e112faa25abbcd6d6c06238\">bb01e0b</a></td><td><code>Drive-by fix to minpoly documentation.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/491dcc4884823cab627f318ea63f0e2504f9f7c2\">491dcc4</a></td><td><code>Faster exactification using numeric minpoly.</code></td></tr></table>\n",
    "created_at": "2014-04-23T19:58:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16222#issuecomment-209874",
    "user": "https://github.com/gagern"
}
```

<div id="comment:2" align="right">Comment 2</div>

These modifications are not ready for inclusion yet, they need some discussion first.

1. How to get the symbolic expression from `expression_conversion` into `AlgebraicNumber_base`.

   Right now, I use direct access to an underscore-prefixed variable called `_symbolic_expression`. I'm not sure whether that is acceptable. If cross-module interfaces should avoid using underscores, we'll have to look for alternatives. But I rely on the fact that the expression really describes the number in question, so this certainly isn't a variable the user should be allowed to tweak. So I don't know a better way yet.

2. Default state of that variable.

   Right now, I have a class variable which defaults to `None` and instance variables which will override this when required. Alternatives would be setting an instance variable in some (as of now non-existant) constructor, or avoding a default value altogether and using `hasattr` instead. I simply wrote what I'd do in my own code, but if there is a sage convention to write things differently, I'll gladly comply.

3. When to compute the minpoly.

   I now do the computation on demand, hoping that expressions cannot be legally modified after their creation. If they can be modified, we might want to do the minpoly computation when we do the conversion from symbolic to algebraic. Likewise if you have reasons to believe that minpoly computation should not make that conversion noticably slower than it already is.

4. Test case dependency.

   I just noticed that my current doctest depends on ticket:14239, but that should be easy to fix. If you have a favorite example, let me know, but otherwise I'll come up with something.

5. The `_more_precision` loop.

   I have more trouble testing that `_more_precision` codepath inside `_exact_symbolic`. Can anyone come up with a situation where identifying the correct root would need more than default precision but where numeric minpoly computation would still succeed? If not, what shall we do? Shall we deactivate that whole loop, and simply return without taking action if we find more than one root which could match?

6. How to obtain the roots.

   Is `p.roots(self.parent(), False)` the right thing to do, or is this too high-level, should I use something more basic here?
  
---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/bb01e0b15e1a4e7e1e112faa25abbcd6d6c06238">bb01e0b</a></td><td><code>Drive-by fix to minpoly documentation.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/491dcc4884823cab627f318ea63f0e2504f9f7c2">491dcc4</a></td><td><code>Faster exactification using numeric minpoly.</code></td></tr></table>




---

archive/issue_comments_209875.json:
```json
{
    "body": "Commit: **[491dcc4](https://github.com/sagemath/sagetrac-mirror/commit/491dcc4884823cab627f318ea63f0e2504f9f7c2)**",
    "created_at": "2014-04-23T19:58:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16222#issuecomment-209875",
    "user": "https://github.com/gagern"
}
```

Commit: **[491dcc4](https://github.com/sagemath/sagetrac-mirror/commit/491dcc4884823cab627f318ea63f0e2504f9f7c2)**



---

archive/issue_comments_209876.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1b703c0cbe4714ffbac867c47c4fdaca9434b616\">1b703c0</a></td><td><code>Drive-by fix to minpoly documentation.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8597313c855ce7a413cfcabead498059d2cfcbf6\">8597313</a></td><td><code>Faster exactification using numeric minpoly.</code></td></tr></table>\n",
    "created_at": "2014-07-13T03:00:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16222#issuecomment-209876",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1b703c0cbe4714ffbac867c47c4fdaca9434b616">1b703c0</a></td><td><code>Drive-by fix to minpoly documentation.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8597313c855ce7a413cfcabead498059d2cfcbf6">8597313</a></td><td><code>Faster exactification using numeric minpoly.</code></td></tr></table>




---

archive/issue_comments_209877.json:
```json
{
    "body": "Changed commit from **[491dcc4](https://github.com/sagemath/sagetrac-mirror/commit/491dcc4884823cab627f318ea63f0e2504f9f7c2)** to **[8597313](https://github.com/sagemath/sagetrac-mirror/commit/8597313c855ce7a413cfcabead498059d2cfcbf6)**",
    "created_at": "2014-07-13T03:00:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16222#issuecomment-209877",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[491dcc4](https://github.com/sagemath/sagetrac-mirror/commit/491dcc4884823cab627f318ea63f0e2504f9f7c2)** to **[8597313](https://github.com/sagemath/sagetrac-mirror/commit/8597313c855ce7a413cfcabead498059d2cfcbf6)**



---

archive/issue_events_214073.json:
```json
{
    "actor": "https://github.com/gagern",
    "created_at": "2014-07-13T03:21:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16222#event-214073"
}
```



---

archive/issue_comments_209878.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nRebased to current develop, and changed test case to avoid dependency on #14239. I'm now requesting review since that's what I need even if I consider it likely that the reviewer will not agree with all the choices I made as outlined above.",
    "created_at": "2014-07-13T03:21:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16222#issuecomment-209878",
    "user": "https://github.com/gagern"
}
```

<div id="comment:4" align="right">Comment 4</div>

Rebased to current develop, and changed test case to avoid dependency on #14239. I'm now requesting review since that's what I need even if I consider it likely that the reviewer will not agree with all the choices I made as outlined above.



---

archive/issue_events_214074.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16222#event-214074"
}
```



---

archive/issue_events_214075.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16222#event-214075"
}
```



---

archive/issue_comments_209879.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nHello,\n\nDid you try using the magic `algdep` from pari? It perform (very quickly) some LLL to find the polynomial with smallest coefficient for which `x` is almost a root. With your example\n\n```\nsage: a = (443/96*I*sqrt(443)*sqrt(3) + 833939/1728)^(1/3)\nsage: b = sqrt(144*a + 9205/a + 1176)/12\nsage: c = QQbar(b)\nsage: gp.algdep(c.n(prec=150).real(), 6)\n16*x^6 - 392*x^4 + 133*x^2 + 900\n```\nThe advantage is that it would potentially speed up non symbolic cases.\n\nVincent",
    "created_at": "2015-02-01T11:54:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16222#issuecomment-209879",
    "user": "https://github.com/videlec"
}
```

<div id="comment:6" align="right">Comment 6</div>

Hello,

Did you try using the magic `algdep` from pari? It perform (very quickly) some LLL to find the polynomial with smallest coefficient for which `x` is almost a root. With your example

```
sage: a = (443/96*I*sqrt(443)*sqrt(3) + 833939/1728)^(1/3)
sage: b = sqrt(144*a + 9205/a + 1176)/12
sage: c = QQbar(b)
sage: gp.algdep(c.n(prec=150).real(), 6)
16*x^6 - 392*x^4 + 133*x^2 + 900
```
The advantage is that it would potentially speed up non symbolic cases.

Vincent



---

archive/issue_comments_209880.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nReplying to [@videlec](#comment%3A6):\n> Did you try using the magic `algdep` from pari?\n\nMy code builds on that, since [expression.minpoly calls calculus.minpoly](https://github.com/sagemath/sagetrac-mirror/blob/develop/src/sage/symbolic/expression.pyx?h=develop&id=d27f8497dcd19d70ec08155888e6fec9c74b839a#n10602) which in turn [calls algdep](https://github.com/sagemath/sagetrac-mirror/blob/develop/src/sage/calculus/calculus.py?h=develop&id=d27f8497dcd19d70ec08155888e6fec9c74b839a#n957). But it does come with a verification step, ensuring that the found polynomial is in fact the one we need. I'm not sure I'd trust the output from `algdep` directly, particularly not unless I had some very good ideas of what degree to expect.\n\n> The advantage is that it would potentially speed up non symbolic cases.\n\nWell, one alternative would be to directly feed the descriptor DAG into `calculus.minpoly`. Computing a `numerical_approx` from these would be easy enough. The hard part would be the symbolic verification step, `g(ex).simplify_trig().canonicalize_radical() == 0`. I guess that's done in Maxima. Do you have an idea how we might either implement this for the non-symbolic case, or find a rigorous argument why we don't need such a proof?",
    "created_at": "2015-02-01T13:18:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16222#issuecomment-209880",
    "user": "https://github.com/gagern"
}
```

<div id="comment:7" align="right">Comment 7</div>

Replying to [@videlec](#comment%3A6):
> Did you try using the magic `algdep` from pari?

My code builds on that, since [expression.minpoly calls calculus.minpoly](https://github.com/sagemath/sagetrac-mirror/blob/develop/src/sage/symbolic/expression.pyx?h=develop&id=d27f8497dcd19d70ec08155888e6fec9c74b839a#n10602) which in turn [calls algdep](https://github.com/sagemath/sagetrac-mirror/blob/develop/src/sage/calculus/calculus.py?h=develop&id=d27f8497dcd19d70ec08155888e6fec9c74b839a#n957). But it does come with a verification step, ensuring that the found polynomial is in fact the one we need. I'm not sure I'd trust the output from `algdep` directly, particularly not unless I had some very good ideas of what degree to expect.

> The advantage is that it would potentially speed up non symbolic cases.

Well, one alternative would be to directly feed the descriptor DAG into `calculus.minpoly`. Computing a `numerical_approx` from these would be easy enough. The hard part would be the symbolic verification step, `g(ex).simplify_trig().canonicalize_radical() == 0`. I guess that's done in Maxima. Do you have an idea how we might either implement this for the non-symbolic case, or find a rigorous argument why we don't need such a proof?



---

archive/issue_comments_209881.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nReplying to [@gagern](#comment%3A7):\n> Replying to [@videlec](#comment%3A6):\n> > Did you try using the magic `algdep` from pari?\n\n> \n> My code builds on that, since [expression.minpoly calls calculus.minpoly](https://github.com/sagemath/sagetrac-mirror/blob/develop/src/sage/symbolic/expression.pyx?h=develop&id=d27f8497dcd19d70ec08155888e6fec9c74b839a#n10602) which in turn [calls algdep](https://github.com/sagemath/sagetrac-mirror/blob/develop/src/sage/calculus/calculus.py?h=develop&id=d27f8497dcd19d70ec08155888e6fec9c74b839a#n957). But it does come with a verification step, ensuring that the found polynomial is in fact the one we need. I'm not sure I'd trust the output from `algdep` directly, particularly not unless I had some very good ideas of what degree to expect.\n\nOf course we can not believe the output of `algdep`. But at least we can bound the degree. And once a polynomial is given we can check irreducibility. So I guess we can produce a candidate quite reliably.\n\n> > The advantage is that it would potentially speed up non symbolic cases.\n\n> \n> Well, one alternative would be to directly feed the descriptor DAG into `calculus.minpoly`. Computing a `numerical_approx` from these would be easy enough. The hard part would be the symbolic verification step, `g(ex).simplify_trig().canonicalize_radical() == 0`. I guess that's done in Maxima. Do you have an idea how we might either implement this for the non-symbolic case, or find a rigorous argument why we don't need such a proof?\n\nRight, checking that it is actually an annihilator polynomial is harder... I do not see anything different from calling `__nonzero__` which indeed calls `.exactify()`.",
    "created_at": "2015-02-01T16:35:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16222#issuecomment-209881",
    "user": "https://github.com/videlec"
}
```

<div id="comment:8" align="right">Comment 8</div>

Replying to [@gagern](#comment%3A7):
> Replying to [@videlec](#comment%3A6):
> > Did you try using the magic `algdep` from pari?

> 
> My code builds on that, since [expression.minpoly calls calculus.minpoly](https://github.com/sagemath/sagetrac-mirror/blob/develop/src/sage/symbolic/expression.pyx?h=develop&id=d27f8497dcd19d70ec08155888e6fec9c74b839a#n10602) which in turn [calls algdep](https://github.com/sagemath/sagetrac-mirror/blob/develop/src/sage/calculus/calculus.py?h=develop&id=d27f8497dcd19d70ec08155888e6fec9c74b839a#n957). But it does come with a verification step, ensuring that the found polynomial is in fact the one we need. I'm not sure I'd trust the output from `algdep` directly, particularly not unless I had some very good ideas of what degree to expect.

Of course we can not believe the output of `algdep`. But at least we can bound the degree. And once a polynomial is given we can check irreducibility. So I guess we can produce a candidate quite reliably.

> > The advantage is that it would potentially speed up non symbolic cases.

> 
> Well, one alternative would be to directly feed the descriptor DAG into `calculus.minpoly`. Computing a `numerical_approx` from these would be easy enough. The hard part would be the symbolic verification step, `g(ex).simplify_trig().canonicalize_radical() == 0`. I guess that's done in Maxima. Do you have an idea how we might either implement this for the non-symbolic case, or find a rigorous argument why we don't need such a proof?

Right, checking that it is actually an annihilator polynomial is harder... I do not see anything different from calling `__nonzero__` which indeed calls `.exactify()`.



---

archive/issue_comments_209882.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nReplying to [@videlec](#comment%3A8):\n> So I guess we can produce a candidate quite reliably.\n\nI'm unsure about the relation between \u201ccandidate\u201d and \u201creliable\u201d. The former implies something that needs to be verified, the latter like I could rely on it without verification. But I guess what you mean is that we could construct a candidate which would *very likely* be the correct one. But \u201cvery likely\u201d isn't enough in my opinion.\n\n> Right, checking that it is actually an annihilator polynomial is harder... I do not see anything different from calling `__nonzero__` which indeed calls `.exactify()`.\n\nIn theory, we could go the reverse route: take the descriptor dag and try building a symbolic expression from this which in turn can be fed to Maxima for verification. But I'm far from certain that this is a good idea for those cases which didn't originate in a symbolic expression in the first place. Which brings us back to the request as it currently stands, right?",
    "created_at": "2015-02-01T17:18:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16222#issuecomment-209882",
    "user": "https://github.com/gagern"
}
```

<div id="comment:9" align="right">Comment 9</div>

Replying to [@videlec](#comment%3A8):
> So I guess we can produce a candidate quite reliably.

I'm unsure about the relation between “candidate” and “reliable”. The former implies something that needs to be verified, the latter like I could rely on it without verification. But I guess what you mean is that we could construct a candidate which would *very likely* be the correct one. But “very likely” isn't enough in my opinion.

> Right, checking that it is actually an annihilator polynomial is harder... I do not see anything different from calling `__nonzero__` which indeed calls `.exactify()`.

In theory, we could go the reverse route: take the descriptor dag and try building a symbolic expression from this which in turn can be fed to Maxima for verification. But I'm far from certain that this is a good idea for those cases which didn't originate in a symbolic expression in the first place. Which brings us back to the request as it currently stands, right?



---

archive/issue_comments_209883.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nI just notice that your ticket is based on a very old version of Sage (6.3.beta5)... it would be nice to upgrade to the current 6.5.rc0.\n\nMore about the ticket, I am worried because `self._symbolic_expression` is completely ignored in the following cases\n\n```\nsage: a = QQbar(sqrt(2)) + 1\nsage: b = QQbar(sqrt(2)) + QQbar(sqrt(3))\n```",
    "created_at": "2015-02-08T11:59:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16222#issuecomment-209883",
    "user": "https://github.com/videlec"
}
```

<div id="comment:10" align="right">Comment 10</div>

I just notice that your ticket is based on a very old version of Sage (6.3.beta5)... it would be nice to upgrade to the current 6.5.rc0.

More about the ticket, I am worried because `self._symbolic_expression` is completely ignored in the following cases

```
sage: a = QQbar(sqrt(2)) + 1
sage: b = QQbar(sqrt(2)) + QQbar(sqrt(3))
```



---

archive/issue_comments_209884.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nReplying to [@videlec](#comment%3A10):\n> More about the ticket, I am worried because `self._symbolic_expression` is completely ignored in the following cases [\u2026]\n\nSo what do you propose? That we always try to build a symbolic expression from the descriptor dag?",
    "created_at": "2015-02-08T13:17:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16222#issuecomment-209884",
    "user": "https://github.com/gagern"
}
```

<div id="comment:11" align="right">Comment 11</div>

Replying to [@videlec](#comment%3A10):
> More about the ticket, I am worried because `self._symbolic_expression` is completely ignored in the following cases […]

So what do you propose? That we always try to build a symbolic expression from the descriptor dag?



---

archive/issue_comments_209885.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nReplying to [@gagern](#comment%3A11):\n> Replying to [@videlec](#comment%3A10):\n> > More about the ticket, I am worried because `self._symbolic_expression` is completely ignored in the following cases [\u2026]\n\n> \n> So what do you propose? That we always try to build a symbolic expression from the descriptor dag?\n\nNo. But at least when performing arithmetic operations on `QQbar` or `AA` elements the operation are also done on the corresponding symbolic expressions.",
    "created_at": "2015-02-08T13:39:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16222#issuecomment-209885",
    "user": "https://github.com/videlec"
}
```

<div id="comment:12" align="right">Comment 12</div>

Replying to [@gagern](#comment%3A11):
> Replying to [@videlec](#comment%3A10):
> > More about the ticket, I am worried because `self._symbolic_expression` is completely ignored in the following cases […]

> 
> So what do you propose? That we always try to build a symbolic expression from the descriptor dag?

No. But at least when performing arithmetic operations on `QQbar` or `AA` elements the operation are also done on the corresponding symbolic expressions.



---

archive/issue_comments_209886.json:
```json
{
    "body": "Changed commit from **[8597313](https://github.com/sagemath/sagetrac-mirror/commit/8597313c855ce7a413cfcabead498059d2cfcbf6)** to **[998534a](https://github.com/sagemath/sagetrac-mirror/commit/998534a33a502a86518f6c541f99cac1ffacdaa5)**",
    "created_at": "2015-02-08T20:46:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16222#issuecomment-209886",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[8597313](https://github.com/sagemath/sagetrac-mirror/commit/8597313c855ce7a413cfcabead498059d2cfcbf6)** to **[998534a](https://github.com/sagemath/sagetrac-mirror/commit/998534a33a502a86518f6c541f99cac1ffacdaa5)**



---

archive/issue_comments_209887.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5c818209dcd49ac71df7951d3e393bf03002f8cb\">5c81820</a></td><td><code>Drive-by fix to minpoly documentation.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f1bd97040a4399ea2d30a144331234f35eb8932c\">f1bd970</a></td><td><code>Faster exactification using numeric minpoly.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/998534a33a502a86518f6c541f99cac1ffacdaa5\">998534a</a></td><td><code>Build symbolic expression from descriptor graph of algebraic number.</code></td></tr></table>\n",
    "created_at": "2015-02-08T20:46:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16222#issuecomment-209887",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:13" align="right">Comment 13</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5c818209dcd49ac71df7951d3e393bf03002f8cb">5c81820</a></td><td><code>Drive-by fix to minpoly documentation.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f1bd97040a4399ea2d30a144331234f35eb8932c">f1bd970</a></td><td><code>Faster exactification using numeric minpoly.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/998534a33a502a86518f6c541f99cac1ffacdaa5">998534a</a></td><td><code>Build symbolic expression from descriptor graph of algebraic number.</code></td></tr></table>




---

archive/issue_events_214076.json:
```json
{
    "actor": "https://github.com/gagern",
    "created_at": "2015-02-08T21:07:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16222#event-214076"
}
```



---

archive/issue_events_214077.json:
```json
{
    "actor": "https://github.com/gagern",
    "created_at": "2015-02-08T21:07:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16222#event-214077"
}
```



---

archive/issue_comments_209888.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nI find that it's hard to draw the line. Your example of `QQbar(sqrt(2)) + 1` shows that we want symbolic expressions for minpoly construction even if some arguments were not explicitely constructed from symbolic expressions. But where does it stop? In my demo implementation, I decided that caching the symbolic expression after every elementary operation would duplicate a lot of information. So I decided to recursively construct such symbolic expressions. The result is pretty much a construction of symbolic expressions from the descriptor dag, even though I didn't think of it that way when I started coding. It can generate symbolic expression for a lot of things.\n\nAnd I'm using it unconditionally, which leads to a lot of doctests failing. Some of them for better:\n\n```\nline 3653, rt3b.as_number_field_element():\nExpected: \u2026 defining polynomial y^4 - 4*y^2 + 1, -a^2 + 2 \u2026\nGot:      \u2026 defining polynomial y^2 - 3, a \u2026\n\nline 3755, rt2b._exact_value():\nExpected: a^3 - 3*a where a^4 - 4*a^2 + 1 = 0 and a in 1.931851652578137?\nGot:      a where a^2 - 2 = 0 and a in 1.414213562373095?\n\nline 7171, y.is_complex():\nExpected: True\nGot:      False\n```\n\nBut some of them for worse:\n\n```\nline 360, sage_input(n, verify=True):\nExpected: AA(2)\nGot:      v = sqrt(AA(2))\n          v*v\n\nline 7881, sage_input(n):\nExpected: QQbar(sqrt(AA(2))) + 1\nGot:      v = sqrt(QQbar(3))\n          QQbar(sqrt(AA(2))) + v/v\n```\n\nI don't have any performance comparisons yet, but I guess that there might well be cases where the current approach was fast enough, but the whole detour via symbolic expressions, PARI's algdep and Maxima for verification might have a severe impact.\n\nSo my key concern right now is, when do we want to use this approach, and when not to? If multithreading in Python were a more natural concept, I'd be so bold as to suggest that we start both approaches, and the one which terminates first wins, aborting the other. This might make results somewhat unpredictable, though. And given the state of threading support in CPython, I doubt this is realistic in any case.\n\nHere are a few more realistic ways how we could make that case distinctions in a way that might make sense. I'm not completely happy with any of them.\n\n1. Only do the algdep approach if at least one number in the descriptor dag originated from a symbolic expression\n2. Limit the reconstruction of symbolic expressions to fewer operations, mainly elementary arithmetic operations\n3. Give control to the user: make exactification using algdep not part of the normal exactification procedure, but instead the result of an explicit method invocation\n4. Try to detect exactifications which are likely very slow using the standard approach, e.g. indicated by the expected size of the minpoly for some union field, and try algdep only on those",
    "created_at": "2015-02-08T21:07:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16222#issuecomment-209888",
    "user": "https://github.com/gagern"
}
```

<div id="comment:14" align="right">Comment 14</div>

I find that it's hard to draw the line. Your example of `QQbar(sqrt(2)) + 1` shows that we want symbolic expressions for minpoly construction even if some arguments were not explicitely constructed from symbolic expressions. But where does it stop? In my demo implementation, I decided that caching the symbolic expression after every elementary operation would duplicate a lot of information. So I decided to recursively construct such symbolic expressions. The result is pretty much a construction of symbolic expressions from the descriptor dag, even though I didn't think of it that way when I started coding. It can generate symbolic expression for a lot of things.

And I'm using it unconditionally, which leads to a lot of doctests failing. Some of them for better:

```
line 3653, rt3b.as_number_field_element():
Expected: … defining polynomial y^4 - 4*y^2 + 1, -a^2 + 2 …
Got:      … defining polynomial y^2 - 3, a …

line 3755, rt2b._exact_value():
Expected: a^3 - 3*a where a^4 - 4*a^2 + 1 = 0 and a in 1.931851652578137?
Got:      a where a^2 - 2 = 0 and a in 1.414213562373095?

line 7171, y.is_complex():
Expected: True
Got:      False
```

But some of them for worse:

```
line 360, sage_input(n, verify=True):
Expected: AA(2)
Got:      v = sqrt(AA(2))
          v*v

line 7881, sage_input(n):
Expected: QQbar(sqrt(AA(2))) + 1
Got:      v = sqrt(QQbar(3))
          QQbar(sqrt(AA(2))) + v/v
```

I don't have any performance comparisons yet, but I guess that there might well be cases where the current approach was fast enough, but the whole detour via symbolic expressions, PARI's algdep and Maxima for verification might have a severe impact.

So my key concern right now is, when do we want to use this approach, and when not to? If multithreading in Python were a more natural concept, I'd be so bold as to suggest that we start both approaches, and the one which terminates first wins, aborting the other. This might make results somewhat unpredictable, though. And given the state of threading support in CPython, I doubt this is realistic in any case.

Here are a few more realistic ways how we could make that case distinctions in a way that might make sense. I'm not completely happy with any of them.

1. Only do the algdep approach if at least one number in the descriptor dag originated from a symbolic expression
2. Limit the reconstruction of symbolic expressions to fewer operations, mainly elementary arithmetic operations
3. Give control to the user: make exactification using algdep not part of the normal exactification procedure, but instead the result of an explicit method invocation
4. Try to detect exactifications which are likely very slow using the standard approach, e.g. indicated by the expected size of the minpoly for some union field, and try algdep only on those



---

archive/issue_comments_209889.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">Comment 15</div>\n\nReplying to [@gagern](#comment%3A14):\n> But some of them for worse:\n> \n> ```\n> line 360, sage_input(n, verify=True):\n> Expected: AA(2)\n> Got:      v = sqrt(AA(2))\n>           v*v\n> \n> line 7881, sage_input(n):\n> Expected: QQbar(sqrt(AA(2))) + 1\n> Got:      v = sqrt(QQbar(3))\n>           QQbar(sqrt(AA(2))) + v/v\n> ```\n\nNot really for worse. The previous doctests assumed that at some point in the code `.exactify()` would have been called for all nodes of the description tree... Right now, the symbolic code ignore the exactification of the nodes.\n\nEDIT: I removed the non-example I previously wrote here.\n\n> Here are a few more realistic ways how we could make that case distinctions in a way that might make sense. I'm not completely happy with any of them.\n> \n> 1. Only do the algdep approach if at least one number in the descriptor dag originated from a symbolic expression\n> 2. Limit the reconstruction of symbolic expressions to fewer operations, mainly elementary arithmetic operations\n> 3. Give control to the user: make exactification using algdep not part of the normal exactification procedure, but instead the result of an explicit method invocation\n> 4. Try to detect exactifications which are likely very slow using the standard approach, e.g. indicated by the expected size of the minpoly for some union field, and try algdep only on those\n\nOption 3 is definitely needed.\n\nI am against option 1 as we do not want to keep the origin of the objects. By the way, do you have an idea to have quick symbolic representation for\n\n```\nsage: QQbar(sqrt2).sqrt() + QQbar(3).sqrt()\n```\nI.e. roots of `x^n - a` have a simple symbolic counterpart.\n\nFor options 2 and 4 there is a right balance to find.",
    "created_at": "2015-02-09T00:08:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16222#issuecomment-209889",
    "user": "https://github.com/videlec"
}
```

<div id="comment:15" align="right">Comment 15</div>

Replying to [@gagern](#comment%3A14):
> But some of them for worse:
> 
> ```
> line 360, sage_input(n, verify=True):
> Expected: AA(2)
> Got:      v = sqrt(AA(2))
>           v*v
> 
> line 7881, sage_input(n):
> Expected: QQbar(sqrt(AA(2))) + 1
> Got:      v = sqrt(QQbar(3))
>           QQbar(sqrt(AA(2))) + v/v
> ```

Not really for worse. The previous doctests assumed that at some point in the code `.exactify()` would have been called for all nodes of the description tree... Right now, the symbolic code ignore the exactification of the nodes.

EDIT: I removed the non-example I previously wrote here.

> Here are a few more realistic ways how we could make that case distinctions in a way that might make sense. I'm not completely happy with any of them.
> 
> 1. Only do the algdep approach if at least one number in the descriptor dag originated from a symbolic expression
> 2. Limit the reconstruction of symbolic expressions to fewer operations, mainly elementary arithmetic operations
> 3. Give control to the user: make exactification using algdep not part of the normal exactification procedure, but instead the result of an explicit method invocation
> 4. Try to detect exactifications which are likely very slow using the standard approach, e.g. indicated by the expected size of the minpoly for some union field, and try algdep only on those

Option 3 is definitely needed.

I am against option 1 as we do not want to keep the origin of the objects. By the way, do you have an idea to have quick symbolic representation for

```
sage: QQbar(sqrt2).sqrt() + QQbar(3).sqrt()
```
I.e. roots of `x^n - a` have a simple symbolic counterpart.

For options 2 and 4 there is a right balance to find.



---

archive/issue_comments_209890.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">Comment 16</div>\n\nNot exactly related to this ticket: I just found out some maxima magic to get the polynomial of symbolic expressions involving square root and such (p. 1041 of the [maxima manual](http://maxima.sourceforge.net/docs/manual/maxima.pdf)):\n\n```\n(%i1) load(to_poly_solve)$\n(%i2) first(elim_allbut(first(to_poly(eq,[1,x])),[x]));\n                                 4       2\n(%o2)                          [x  - 24 x  + 4]\n(%i3) eq: x = sqrt(sqrt(3) + sqrt(5) + 1) + sqrt(7);\n(%o3)             x = sqrt(7) + sqrt(sqrt(5) + sqrt(3) + 1)\n(%i3) first(elim_allbut(first(to_poly(eq, [1,x])), [x]));\n         16       14         12          10           8            6\n(%o3) [x   - 64 x   + 1648 x   - 23552 x   + 213480 x  - 1191424 x\n                                                       4            2\n                                            + 3340480 x  - 3562496 x  + 524176]\n```\nIt is way much much faster than what we have in Sage for minpoly:\n\n```\nsage: maxima.load(\"to_poly_solve\")\n\"/opt/sage_flatsurf/local/share/maxima/5.35.1/share/to_poly_solve/to_poly_solve.mac\"\nsage: maxima(\"eq: x = sqrt(sqrt(3) + sqrt(5) + 1) + sqrt(7);\")\nx=sqrt(7)+sqrt(sqrt(5)+sqrt(3)+1)\nsage: maxima(\"first(elim_allbut(first(to_poly(eq, [1,x])), [x]));\")\n[x^16-64*x^14+1648*x^12-23552*x^10+213480*x^8-1191424*x^6+3340480*x^4-3562496*x^2+524176]\nsage: %timeit maxima(\"first(elim_allbut(first(to_poly(eq, [1,x])), [x]));\")\n10 loops, best of 3: 18 ms per loop\nsage: a = sqrt(7) + sqrt(sqrt(5) + sqrt(3) + 1)\nsage: %timeit a.minpoly()\n1 loops, best of 3: 449 ms per loop\n```",
    "created_at": "2015-02-26T16:52:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16222#issuecomment-209890",
    "user": "https://github.com/videlec"
}
```

<div id="comment:16" align="right">Comment 16</div>

Not exactly related to this ticket: I just found out some maxima magic to get the polynomial of symbolic expressions involving square root and such (p. 1041 of the [maxima manual](http://maxima.sourceforge.net/docs/manual/maxima.pdf)):

```
(%i1) load(to_poly_solve)$
(%i2) first(elim_allbut(first(to_poly(eq,[1,x])),[x]));
                                 4       2
(%o2)                          [x  - 24 x  + 4]
(%i3) eq: x = sqrt(sqrt(3) + sqrt(5) + 1) + sqrt(7);
(%o3)             x = sqrt(7) + sqrt(sqrt(5) + sqrt(3) + 1)
(%i3) first(elim_allbut(first(to_poly(eq, [1,x])), [x]));
         16       14         12          10           8            6
(%o3) [x   - 64 x   + 1648 x   - 23552 x   + 213480 x  - 1191424 x
                                                       4            2
                                            + 3340480 x  - 3562496 x  + 524176]
```
It is way much much faster than what we have in Sage for minpoly:

```
sage: maxima.load("to_poly_solve")
"/opt/sage_flatsurf/local/share/maxima/5.35.1/share/to_poly_solve/to_poly_solve.mac"
sage: maxima("eq: x = sqrt(sqrt(3) + sqrt(5) + 1) + sqrt(7);")
x=sqrt(7)+sqrt(sqrt(5)+sqrt(3)+1)
sage: maxima("first(elim_allbut(first(to_poly(eq, [1,x])), [x]));")
[x^16-64*x^14+1648*x^12-23552*x^10+213480*x^8-1191424*x^6+3340480*x^4-3562496*x^2+524176]
sage: %timeit maxima("first(elim_allbut(first(to_poly(eq, [1,x])), [x]));")
10 loops, best of 3: 18 ms per loop
sage: a = sqrt(7) + sqrt(sqrt(5) + sqrt(3) + 1)
sage: %timeit a.minpoly()
1 loops, best of 3: 449 ms per loop
```



---

archive/issue_comments_209891.json:
```json
{
    "body": "<div id=\"comment:17\" align=\"right\">Comment 17</div>\n\nReplying to [@videlec](#comment%3A16):\n> Not exactly related to this ticket: I just found out some maxima magic to get the polynomial of symbolic expressions involving square root and such\n\nNice! Do you want to open a spin-off ticket, asking for a way to exploit this functionality in Sage? As you said, it's not exactly on topic here, but it would be a shame if that idea got lost once we have this issue here wrapped up.",
    "created_at": "2015-03-02T16:28:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16222#issuecomment-209891",
    "user": "https://github.com/gagern"
}
```

<div id="comment:17" align="right">Comment 17</div>

Replying to [@videlec](#comment%3A16):
> Not exactly related to this ticket: I just found out some maxima magic to get the polynomial of symbolic expressions involving square root and such

Nice! Do you want to open a spin-off ticket, asking for a way to exploit this functionality in Sage? As you said, it's not exactly on topic here, but it would be a shame if that idea got lost once we have this issue here wrapped up.



---

archive/issue_comments_209892.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">Comment 18</div>\n\nReplying to [@videlec](#comment%3A15):\n> Option 3 is definitely needed.\n\n>\n> For options 2 and 4 there is a right balance to find.\n\nI guess once we have a method which can be called by hand, we can use that to experiment a bit, in our daily work, and can ask others to do the same. That might give us some intuition about when to use which. So I'd keep this out of this ticket for now, and instead post a follow-up ticket to investigate that.",
    "created_at": "2015-04-03T13:00:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16222#issuecomment-209892",
    "user": "https://github.com/gagern"
}
```

<div id="comment:18" align="right">Comment 18</div>

Replying to [@videlec](#comment%3A15):
> Option 3 is definitely needed.

>
> For options 2 and 4 there is a right balance to find.

I guess once we have a method which can be called by hand, we can use that to experiment a bit, in our daily work, and can ask others to do the same. That might give us some intuition about when to use which. So I'd keep this out of this ticket for now, and instead post a follow-up ticket to investigate that.



---

archive/issue_comments_209893.json:
```json
{
    "body": "<div id=\"comment:19\" align=\"right\">Comment 19</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4b1d01e0932c953ee66e57f27d19b03b8341fba8\">4b1d01e</a></td><td><code>Drive-by fix to minpoly documentation.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f065ca4b905cb414e24aa00a751d696f8ee5b5e6\">f065ca4</a></td><td><code>Faster exactification using numeric minpoly.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8cc7bc7025f21296e5942b0120804d49fab9a24b\">8cc7bc7</a></td><td><code>Build symbolic expression from descriptor graph of algebraic number.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/96942f09ea8a9cddf2b2dc8fece352935ddfe3b2\">96942f0</a></td><td><code>Trac #16222: Add argument \u201csymbolic\u201d to method \u201cexactify\u201d</code></td></tr></table>\n",
    "created_at": "2015-04-03T15:01:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16222#issuecomment-209893",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:19" align="right">Comment 19</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4b1d01e0932c953ee66e57f27d19b03b8341fba8">4b1d01e</a></td><td><code>Drive-by fix to minpoly documentation.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f065ca4b905cb414e24aa00a751d696f8ee5b5e6">f065ca4</a></td><td><code>Faster exactification using numeric minpoly.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8cc7bc7025f21296e5942b0120804d49fab9a24b">8cc7bc7</a></td><td><code>Build symbolic expression from descriptor graph of algebraic number.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/96942f09ea8a9cddf2b2dc8fece352935ddfe3b2">96942f0</a></td><td><code>Trac #16222: Add argument “symbolic” to method “exactify”</code></td></tr></table>




---

archive/issue_comments_209894.json:
```json
{
    "body": "Changed commit from **[998534a](https://github.com/sagemath/sagetrac-mirror/commit/998534a33a502a86518f6c541f99cac1ffacdaa5)** to **[96942f0](https://github.com/sagemath/sagetrac-mirror/commit/96942f09ea8a9cddf2b2dc8fece352935ddfe3b2)**",
    "created_at": "2015-04-03T15:01:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16222#issuecomment-209894",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[998534a](https://github.com/sagemath/sagetrac-mirror/commit/998534a33a502a86518f6c541f99cac1ffacdaa5)** to **[96942f0](https://github.com/sagemath/sagetrac-mirror/commit/96942f09ea8a9cddf2b2dc8fece352935ddfe3b2)**



---

archive/issue_comments_209895.json:
```json
{
    "body": "<div id=\"comment:20\" align=\"right\">Comment 20</div>\n\nRebased to 6.6.rc2 and added public interface `z.exactify(symbolic=True)`.\n\nI considered \u201cnumeric\u201d instead of \u201csymbolic\u201d, but I at least tend to associate \u201cnumeric\u201d with \u201cinexact\u201d, so I felt that term might be more confusing. Also considered \u201calgoiritm='symbolic'\u201d but I think in the long run we might have various alternatives which could be tried one after the other, so a flag to enable or disable each sounds more useful. The same holds for a new method as opposed to an argument to `exactify`. So that's why I chose the interface the way I did.\n\nNow that my new code is not called by default, it shouldn't have any adverse effects on other parts of the code, which might make this easier to review. I hope this can be merged, to enable some evaluation in preparation for #18122 which I filed for automatically choosing between algorithms. I fear that might be hard to get right.",
    "created_at": "2015-04-03T15:09:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16222#issuecomment-209895",
    "user": "https://github.com/gagern"
}
```

<div id="comment:20" align="right">Comment 20</div>

Rebased to 6.6.rc2 and added public interface `z.exactify(symbolic=True)`.

I considered “numeric” instead of “symbolic”, but I at least tend to associate “numeric” with “inexact”, so I felt that term might be more confusing. Also considered “algoiritm='symbolic'” but I think in the long run we might have various alternatives which could be tried one after the other, so a flag to enable or disable each sounds more useful. The same holds for a new method as opposed to an argument to `exactify`. So that's why I chose the interface the way I did.

Now that my new code is not called by default, it shouldn't have any adverse effects on other parts of the code, which might make this easier to review. I hope this can be merged, to enable some evaluation in preparation for #18122 which I filed for automatically choosing between algorithms. I fear that might be hard to get right.



---

archive/issue_events_214078.json:
```json
{
    "actor": "https://github.com/gagern",
    "created_at": "2015-04-03T15:09:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16222#event-214078"
}
```



---

archive/issue_events_214079.json:
```json
{
    "actor": "https://github.com/gagern",
    "created_at": "2015-04-03T15:09:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16222#event-214079"
}
```



---

archive/issue_comments_209896.json:
```json
{
    "body": "Changed commit from **[96942f0](https://github.com/sagemath/sagetrac-mirror/commit/96942f09ea8a9cddf2b2dc8fece352935ddfe3b2)** to **[8773468](https://github.com/sagemath/sagetrac-mirror/commit/8773468739544ed9f12b836c7f28a06be37ffd94)**",
    "created_at": "2015-04-06T16:49:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16222#issuecomment-209896",
    "user": "https://github.com/videlec"
}
```

Changed commit from **[96942f0](https://github.com/sagemath/sagetrac-mirror/commit/96942f09ea8a9cddf2b2dc8fece352935ddfe3b2)** to **[8773468](https://github.com/sagemath/sagetrac-mirror/commit/8773468739544ed9f12b836c7f28a06be37ffd94)**



---

archive/issue_comments_209897.json:
```json
{
    "body": "<div id=\"comment:21\" align=\"right\">Comment 21</div>\n\nHello,\n\nIt's a pity that we got `None` in\n\n```\nsage: AA(2).sqrt()._descr.quick_symbolic()\n```\nI added a new commit to handle that.\n\nOn the other hand, did you know that the symbolic ring is **very** broken to deal with n-th root\n\n```\nsage: a = (-1)^(1/3)\nsage: a.n()\n0.500000000000000 + 0.866025403784439*I\nsage: a**2\n1\n```\nAs `bool(I^(2/3) == (-1)^(1/3))` is `True` we might get into troubles with subtle examples. I am **very** worried by that.\n\nVincent\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8773468739544ed9f12b836c7f28a06be37ffd94\">8773468</a></td><td><code>Trac 16222: support quick symbolic for roots of x^n -a</code></td></tr></table>\n",
    "created_at": "2015-04-06T16:49:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16222#issuecomment-209897",
    "user": "https://github.com/videlec"
}
```

<div id="comment:21" align="right">Comment 21</div>

Hello,

It's a pity that we got `None` in

```
sage: AA(2).sqrt()._descr.quick_symbolic()
```
I added a new commit to handle that.

On the other hand, did you know that the symbolic ring is **very** broken to deal with n-th root

```
sage: a = (-1)^(1/3)
sage: a.n()
0.500000000000000 + 0.866025403784439*I
sage: a**2
1
```
As `bool(I^(2/3) == (-1)^(1/3))` is `True` we might get into troubles with subtle examples. I am **very** worried by that.

Vincent

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8773468739544ed9f12b836c7f28a06be37ffd94">8773468</a></td><td><code>Trac 16222: support quick symbolic for roots of x^n -a</code></td></tr></table>




---

archive/issue_comments_209898.json:
```json
{
    "body": "Changed branch from **[u/gagern/ticket/16222](https://github.com/sagemath/sagetrac-mirror/tree/u/gagern/ticket/16222)** to **[u/vdelecroix/16222](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/16222)**",
    "created_at": "2015-04-06T16:49:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16222#issuecomment-209898",
    "user": "https://github.com/videlec"
}
```

Changed branch from **[u/gagern/ticket/16222](https://github.com/sagemath/sagetrac-mirror/tree/u/gagern/ticket/16222)** to **[u/vdelecroix/16222](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/16222)**



---

archive/issue_events_214080.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-04-06T16:50:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16222#event-214080"
}
```



---

archive/issue_events_214081.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-04-06T16:50:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16222#event-214081"
}
```



---

archive/issue_comments_209899.json:
```json
{
    "body": "Reviewer: **Vincent Delecroix**",
    "created_at": "2015-04-06T16:50:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16222#issuecomment-209899",
    "user": "https://github.com/videlec"
}
```

Reviewer: **Vincent Delecroix**



---

archive/issue_comments_209900.json:
```json
{
    "body": "<div id=\"comment:23\" align=\"right\">Comment 23</div>\n\nAre there any link/dependency with #14239 or/and #17516?",
    "created_at": "2015-04-06T16:54:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16222#issuecomment-209900",
    "user": "https://github.com/videlec"
}
```

<div id="comment:23" align="right">Comment 23</div>

Are there any link/dependency with #14239 or/and #17516?



---

archive/issue_comments_209901.json:
```json
{
    "body": "<div id=\"comment:24\" align=\"right\">Comment 24</div>\n\nReplying to [@videlec](#comment%3A23):\n> Are there any link/dependency with #14239 or/and #17516?\n\nNo, since in general they are not what I'd consider fast. To obtain an exact symbolic number using #14239, we need to have a minimal polynomial, while this one here is aiming to obtain that polynomial more quickly. #17516 would extend #14239 to give results in a larger number of cases, but won't affect this here either. If at all, there is some overlap with #17886, since if that works out relly well, it might make this one here superfluous. I don't think so, though, and since we are only offering this here as an alternative for now, we can use #18122 to evaluate those alternatives once we get there.\n\n> \u2026 did you know that the symbolic ring is **very** broken \u2026\n\nNo, I did not, but I share your deep concerns. Have you filed a ticket for this yet?",
    "created_at": "2015-04-06T17:37:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16222#issuecomment-209901",
    "user": "https://github.com/gagern"
}
```

<div id="comment:24" align="right">Comment 24</div>

Replying to [@videlec](#comment%3A23):
> Are there any link/dependency with #14239 or/and #17516?

No, since in general they are not what I'd consider fast. To obtain an exact symbolic number using #14239, we need to have a minimal polynomial, while this one here is aiming to obtain that polynomial more quickly. #17516 would extend #14239 to give results in a larger number of cases, but won't affect this here either. If at all, there is some overlap with #17886, since if that works out relly well, it might make this one here superfluous. I don't think so, though, and since we are only offering this here as an alternative for now, we can use #18122 to evaluate those alternatives once we get there.

> … did you know that the symbolic ring is **very** broken …

No, I did not, but I share your deep concerns. Have you filed a ticket for this yet?



---

archive/issue_comments_209902.json:
```json
{
    "body": "<div id=\"comment:25\" align=\"right\">Comment 25</div>\n\nReplying to [@gagern](#comment%3A24):\n> > \u2026 did you know that the symbolic ring is **very** broken \u2026\n\n> \n> No, I did not, but I share your deep concerns. Have you filed a ticket for this yet?\n\n#18129",
    "created_at": "2015-04-06T17:49:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16222#issuecomment-209902",
    "user": "https://github.com/videlec"
}
```

<div id="comment:25" align="right">Comment 25</div>

Replying to [@gagern](#comment%3A24):
> > … did you know that the symbolic ring is **very** broken …

> 
> No, I did not, but I share your deep concerns. Have you filed a ticket for this yet?

#18129



---

archive/issue_comments_209903.json:
```json
{
    "body": "Dependencies: **#15605**",
    "created_at": "2015-04-08T08:50:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16222#issuecomment-209903",
    "user": "https://github.com/gagern"
}
```

Dependencies: **#15605**



---

archive/issue_comments_209904.json:
```json
{
    "body": "<div id=\"comment:26\" align=\"right\">Comment 26</div>\n\nI fear we'll have to wait for #15605, the ticket #18129 got duplicated to, before we can trust SR to get this right\u2026",
    "created_at": "2015-04-08T08:50:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16222#issuecomment-209904",
    "user": "https://github.com/gagern"
}
```

<div id="comment:26" align="right">Comment 26</div>

I fear we'll have to wait for #15605, the ticket #18129 got duplicated to, before we can trust SR to get this right…



---

archive/issue_events_214082.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16222",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16222#event-214082"
}
```
