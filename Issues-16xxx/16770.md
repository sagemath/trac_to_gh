# Issue 16770: incorrect type of falling_factorial and rising_factorial with zero factors

archive/issues_016533.json:
```json
{
    "assignees": [],
    "body": "We have\n\n```\nsage: type(falling_factorial(x,0))\n<type 'sage.rings.integer.Integer'>\nsage: type(falling_factorial(x,1))\n<type 'sage.symbolic.expression.Expression'>\n```\nThis is inconsistent and we want\n\n```\nsage: type(falling_factorial(x,0))\n<type 'sage.symbolic.expression.Expression'>\n```\n\n\nKeywords: **incorrect type, factors**\n\nBranch/Commit: **[dfc59bf](https://github.com/sagemath/sagetrac-mirror/commit/dfc59bf1dd299aff360ff9d1e6171639c29f3947)**\n\nReviewer: **Ralf Stephan**\n\nAuthor: **Daniel Krenn**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16770_\n\n",
    "closed_at": "2015-01-02T15:46:24Z",
    "created_at": "2014-08-06T08:52:45Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "incorrect type of falling_factorial and rising_factorial with zero factors",
    "type": "issue",
    "updated_at": "2015-01-02T15:46:24Z",
    "url": "https://github.com/sagemath/sage/issues/16770",
    "user": "https://github.com/dkrenn"
}
```
We have

```
sage: type(falling_factorial(x,0))
<type 'sage.rings.integer.Integer'>
sage: type(falling_factorial(x,1))
<type 'sage.symbolic.expression.Expression'>
```
This is inconsistent and we want

```
sage: type(falling_factorial(x,0))
<type 'sage.symbolic.expression.Expression'>
```


Keywords: **incorrect type, factors**

Branch/Commit: **[dfc59bf](https://github.com/sagemath/sagetrac-mirror/commit/dfc59bf1dd299aff360ff9d1e6171639c29f3947)**

Reviewer: **Ralf Stephan**

Author: **Daniel Krenn**

_Issue created by migration from https://trac.sagemath.org/ticket/16770_





---

archive/issue_events_222060.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2014-08-06T08:52:45Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16770",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16770#event-222060"
}
```



---

archive/issue_events_222061.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2014-08-06T08:52:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16770",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16770#event-222061"
}
```



---

archive/issue_events_222062.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2014-08-06T08:52:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16770",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16770#event-222062"
}
```



---

archive/issue_comments_222185.json:
```json
{
    "body": "Branch: **[u/dkrenn/falling_factorials_type](https://github.com/sagemath/sagetrac-mirror/tree/u/dkrenn/falling_factorials_type)**",
    "created_at": "2014-08-06T09:00:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16770",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16770#issuecomment-222185",
    "user": "https://github.com/dkrenn"
}
```

Branch: **[u/dkrenn/falling_factorials_type](https://github.com/sagemath/sagetrac-mirror/tree/u/dkrenn/falling_factorials_type)**



---

archive/issue_comments_222186.json:
```json
{
    "body": "Author: **Daniel Krenn**",
    "created_at": "2014-08-06T09:01:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16770",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16770#issuecomment-222186",
    "user": "https://github.com/dkrenn"
}
```

Author: **Daniel Krenn**



---

archive/issue_comments_222187.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nPatch prepared (still running make ptestlong on my machine, so not ready for reviewing...)\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/dfc59bf1dd299aff360ff9d1e6171639c29f3947\">dfc59bf</a></td><td><code>trac #16770: fixed type of falling_factorial with zero factors</code></td></tr></table>\n",
    "created_at": "2014-08-06T09:01:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16770",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16770#issuecomment-222187",
    "user": "https://github.com/dkrenn"
}
```

<a id='comment:2'>Comment 2:</a>
Patch prepared (still running make ptestlong on my machine, so not ready for reviewing...)

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/dfc59bf1dd299aff360ff9d1e6171639c29f3947">dfc59bf</a></td><td><code>trac #16770: fixed type of falling_factorial with zero factors</code></td></tr></table>




---

archive/issue_comments_222188.json:
```json
{
    "body": "Commit: **[dfc59bf](https://github.com/sagemath/sagetrac-mirror/commit/dfc59bf1dd299aff360ff9d1e6171639c29f3947)**",
    "created_at": "2014-08-06T09:01:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16770",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16770#issuecomment-222188",
    "user": "https://github.com/dkrenn"
}
```

Commit: **[dfc59bf](https://github.com/sagemath/sagetrac-mirror/commit/dfc59bf1dd299aff360ff9d1e6171639c29f3947)**



---

archive/issue_events_222063.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2014-08-06T11:44:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16770",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16770#event-222063"
}
```



---

archive/issue_comments_222189.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nAll tests passed, so please review.",
    "created_at": "2014-08-06T11:44:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16770",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16770#issuecomment-222189",
    "user": "https://github.com/dkrenn"
}
```

<a id='comment:4'>Comment 4:</a>
All tests passed, so please review.



---

archive/issue_events_222064.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16770",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16770#event-222064"
}
```



---

archive/issue_events_222065.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16770",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16770#event-222065"
}
```



---

archive/issue_comments_222190.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nCode is straightforward, passes doctests in all files using it.",
    "created_at": "2014-12-29T16:44:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16770",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16770#issuecomment-222190",
    "user": "https://github.com/rwst"
}
```

<a id='comment:6'>Comment 6:</a>
Code is straightforward, passes doctests in all files using it.



---

archive/issue_comments_222191.json:
```json
{
    "body": "Reviewer: **Ralf Stephan**",
    "created_at": "2014-12-29T16:44:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16770",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16770#issuecomment-222191",
    "user": "https://github.com/rwst"
}
```

Reviewer: **Ralf Stephan**



---

archive/issue_events_222066.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-12-29T16:44:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16770",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16770#event-222066"
}
```



---

archive/issue_events_222067.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-12-29T16:44:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16770",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16770#event-222067"
}
```



---

archive/issue_comments_222192.json:
```json
{
    "body": "Changed branch from **[u/dkrenn/falling_factorials_type](https://github.com/sagemath/sagetrac-mirror/tree/u/dkrenn/falling_factorials_type)** to **[dfc59bf](https://github.com/sagemath/sagetrac-mirror/commit/dfc59bf1dd299aff360ff9d1e6171639c29f3947)**",
    "created_at": "2015-01-02T15:46:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16770",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16770#issuecomment-222192",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/dkrenn/falling_factorials_type](https://github.com/sagemath/sagetrac-mirror/tree/u/dkrenn/falling_factorials_type)** to **[dfc59bf](https://github.com/sagemath/sagetrac-mirror/commit/dfc59bf1dd299aff360ff9d1e6171639c29f3947)**



---

archive/issue_events_222068.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-01-02T15:46:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16770",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16770#event-222068"
}
```



---

archive/issue_events_222069.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "2f6ac0c28508edd4220242b3e60f48f3c6d85ab1",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-01-02T15:46:24Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16770",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16770#event-222069"
}
```
