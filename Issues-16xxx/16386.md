# Issue 16386: matching_polynomial of Grid2dGraph bombs

archive/issues_016149.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: g = graphs.Grid2dGraph(4,4)\nsage: g.matching_polynomial()\n---------------------------------------------------------------------------\nTypeError                                 Traceback (most recent call last)\n<ipython-input-51-60f79320a776> in <module>()\n----> 1 g.matching_polynomial()\n\n/home/ralf/sage-6.2-x86_64-Linux/local/lib/python2.7/site-packages/sage/graphs/matchpoly.so in sage.graphs.matchpoly.matching_polynomial (sage/graphs/matchpoly.c:2718)()\n\nTypeError: an integer is required\n```\nThe same with `matching_polynomial(graphs.Grid2dGraph(3,3))`.\n\n\nKeywords: **grid graph**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16386_\n\n",
    "closed_at": "2014-05-23T16:04:58Z",
    "created_at": "2014-05-21T16:48:04Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "matching_polynomial of Grid2dGraph bombs",
    "type": "issue",
    "updated_at": "2014-05-23T16:04:58Z",
    "url": "https://github.com/sagemath/sage/issues/16386",
    "user": "https://github.com/rwst"
}
```

```
sage: g = graphs.Grid2dGraph(4,4)
sage: g.matching_polynomial()
---------------------------------------------------------------------------
TypeError                                 Traceback (most recent call last)
<ipython-input-51-60f79320a776> in <module>()
----> 1 g.matching_polynomial()

/home/ralf/sage-6.2-x86_64-Linux/local/lib/python2.7/site-packages/sage/graphs/matchpoly.so in sage.graphs.matchpoly.matching_polynomial (sage/graphs/matchpoly.c:2718)()

TypeError: an integer is required
```
The same with `matching_polynomial(graphs.Grid2dGraph(3,3))`.


Keywords: **grid graph**

_Issue created by migration from https://trac.sagemath.org/ticket/16386_





---

archive/issue_events_216503.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-05-21T16:48:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16386",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16386#event-216503"
}
```



---

archive/issue_events_216504.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-05-21T16:48:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16386",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16386#event-216504"
}
```



---

archive/issue_events_216505.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-05-21T16:48:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16386",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16386#event-216505"
}
```



---

archive/issue_events_216506.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-05-21T16:48:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16386",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16386#event-216506"
}
```



---

archive/issue_comments_213925.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nYou need first to rebabel the graph\n\n```\nsage: g = graphs.Grid2dGraph(4,4)\nsage: g.relabel()\nsage: g.matching_polynomial()\nx^16 - 24*x^14 + 224*x^12 - 1044*x^10 + 2593*x^8 - 3388*x^6 + 2150*x^4 - 552*x^2 + 36\n```\nIts probably because the matching polynomial assumes that vertices are integers.",
    "created_at": "2014-05-23T15:33:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16386",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16386#issuecomment-213925",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:1'>Comment 1:</a>
You need first to rebabel the graph

```
sage: g = graphs.Grid2dGraph(4,4)
sage: g.relabel()
sage: g.matching_polynomial()
x^16 - 24*x^14 + 224*x^12 - 1044*x^10 + 2593*x^8 - 3388*x^6 + 2150*x^4 - 552*x^2 + 36
```
Its probably because the matching polynomial assumes that vertices are integers.



---

archive/issue_comments_213926.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nduplicate of #15545",
    "created_at": "2014-05-23T15:36:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16386",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16386#issuecomment-213926",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:2'>Comment 2:</a>
duplicate of #15545



---

archive/issue_events_216507.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-05-23T15:36:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16386",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16386#event-216507"
}
```



---

archive/issue_events_216508.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-05-23T15:36:34Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16386",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16386#event-216508"
}
```



---

archive/issue_events_216509.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-05-23T15:36:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16386",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16386#event-216509"
}
```



---

archive/issue_events_216510.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-05-23T15:36:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16386",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16386#event-216510"
}
```



---

archive/issue_events_216511.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-05-23T15:53:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16386",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16386#event-216511"
}
```



---

archive/issue_events_216512.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-05-23T15:53:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16386",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16386#event-216512"
}
```



---

archive/issue_events_216513.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-05-23T16:04:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16386",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16386#event-216513"
}
```



---

archive/issue_events_216514.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-05-23T16:04:58Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16386",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16386#event-216514"
}
```



---

archive/issue_events_216515.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-05-23T16:04:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16386",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16386#event-216515"
}
```
