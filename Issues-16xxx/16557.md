# Issue 16557: FiniteStateMachine - LaTeX output in notebook

archive/issues_016320.json:
```json
{
    "assignees": [],
    "body": "In #16020, `graphs.setup_latex_preamble` has been modified to include tikz\ninto the mathjax avoid list so that graphs can be rendered more easily in\nthe sage notebook.\n    \nThis ticket does the same for `finite_state_machine.setup_latex_preamble`.\n\nMoreover, `finite_state_machine.setup_latex_preamble` is now loaded automatically (as it is the case in the graphs module).\n\nCC:  @sagetrac-skropf @dkrenn @sagetrac-elixyre @nathanncohen\n\nKeywords: **finite_state_machine, latex**\n\nBranch/Commit: **[1bfb513](https://github.com/sagemath/sagetrac-mirror/commit/1bfb513ca3a9dc11a232bdd6ee31625fe5822572)**\n\nReviewer: **Jean-Baptiste Priez**\n\nAuthor: **Clemens Heuberger**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16557_\n\n",
    "closed_at": "2014-07-17T00:01:14Z",
    "created_at": "2014-06-26T18:13:25Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20finite%20state%20machines",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "FiniteStateMachine - LaTeX output in notebook",
    "type": "issue",
    "updated_at": "2014-07-17T00:01:14Z",
    "url": "https://github.com/sagemath/sage/issues/16557",
    "user": "https://github.com/cheuberg"
}
```
In #16020, `graphs.setup_latex_preamble` has been modified to include tikz
into the mathjax avoid list so that graphs can be rendered more easily in
the sage notebook.
    
This ticket does the same for `finite_state_machine.setup_latex_preamble`.

Moreover, `finite_state_machine.setup_latex_preamble` is now loaded automatically (as it is the case in the graphs module).

CC:  @sagetrac-skropf @dkrenn @sagetrac-elixyre @nathanncohen

Keywords: **finite_state_machine, latex**

Branch/Commit: **[1bfb513](https://github.com/sagemath/sagetrac-mirror/commit/1bfb513ca3a9dc11a232bdd6ee31625fe5822572)**

Reviewer: **Jean-Baptiste Priez**

Author: **Clemens Heuberger**

_Issue created by migration from https://trac.sagemath.org/ticket/16557_





---

archive/issue_events_218918.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-06-26T18:13:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16557",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16557#event-218918"
}
```



---

archive/issue_events_218919.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-06-26T18:13:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16557",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20finite%20state%20machines",
    "label_color": "0000ff",
    "label_name": "component: finite state machines",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16557#event-218919"
}
```



---

archive/issue_events_218920.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-06-26T18:13:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16557",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16557#event-218920"
}
```



---

archive/issue_events_218921.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-06-26T18:13:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16557",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16557#event-218921"
}
```



---

archive/issue_events_218922.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-06-26T18:13:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16557",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16557#event-218922"
}
```



---

archive/issue_comments_217529.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nIn #16020, we don't need to call:\n\n```\nsage.combinat.finite_state_machine.setup_latex_preamble()\n```\nto see some multi-graphs.\nIt could be convenient to directly load these lines:\n\n```\nlatex.add_package_to_preamble_if_available('tikz')\nlatex.add_to_mathjax_avoid_list(\"tikz\")\nlatex.add_to_preamble('\\\\usetikzlibrary{automata}')\n```\nwhen we load the file.\n\nIt seems to me, that is not disturbing to load these lines... but it is disturbing to have to call this method to draw some automata.\n\nCheers,\n\nJean-Baptiste Priez",
    "created_at": "2014-07-03T03:00:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16557",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16557#issuecomment-217529",
    "user": "https://github.com/sagetrac-elixyre"
}
```

<div id="comment:2" align="right">Comment 2</div>

In #16020, we don't need to call:

```
sage.combinat.finite_state_machine.setup_latex_preamble()
```
to see some multi-graphs.
It could be convenient to directly load these lines:

```
latex.add_package_to_preamble_if_available('tikz')
latex.add_to_mathjax_avoid_list("tikz")
latex.add_to_preamble('\\usetikzlibrary{automata}')
```
when we load the file.

It seems to me, that is not disturbing to load these lines... but it is disturbing to have to call this method to draw some automata.

Cheers,

Jean-Baptiste Priez



---

archive/issue_events_218923.json:
```json
{
    "actor": "https://github.com/sagetrac-elixyre",
    "created_at": "2014-07-03T03:00:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16557",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16557#event-218923"
}
```



---

archive/issue_events_218924.json:
```json
{
    "actor": "https://github.com/sagetrac-elixyre",
    "created_at": "2014-07-03T03:00:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16557",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16557#event-218924"
}
```



---

archive/issue_comments_217530.json:
```json
{
    "body": "Reviewer: **Jean-Baptiste Priez**",
    "created_at": "2014-07-03T03:00:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16557",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16557#issuecomment-217530",
    "user": "https://github.com/sagetrac-elixyre"
}
```

Reviewer: **Jean-Baptiste Priez**



---

archive/issue_comments_217531.json:
```json
{
    "body": "Changed commit from **[2ac09bc](https://github.com/sagemath/sagetrac-mirror/commit/2ac09bc134bde5ae44d326bd9ad2a4266fb7f2c6)** to **[1bfb513](https://github.com/sagemath/sagetrac-mirror/commit/1bfb513ca3a9dc11a232bdd6ee31625fe5822572)**",
    "created_at": "2014-07-06T17:04:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16557",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16557#issuecomment-217531",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[2ac09bc](https://github.com/sagemath/sagetrac-mirror/commit/2ac09bc134bde5ae44d326bd9ad2a4266fb7f2c6)** to **[1bfb513](https://github.com/sagemath/sagetrac-mirror/commit/1bfb513ca3a9dc11a232bdd6ee31625fe5822572)**



---

archive/issue_comments_217532.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/dbc80a0ad1198a4a17746497b8893a8662109076\">dbc80a0</a></td><td><code>Automatically call setup_latex_preamble</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1bfb513ca3a9dc11a232bdd6ee31625fe5822572\">1bfb513</a></td><td><code>Forgot to import cached_function</code></td></tr></table>\n",
    "created_at": "2014-07-06T17:04:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16557",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16557#issuecomment-217532",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/dbc80a0ad1198a4a17746497b8893a8662109076">dbc80a0</a></td><td><code>Automatically call setup_latex_preamble</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1bfb513ca3a9dc11a232bdd6ee31625fe5822572">1bfb513</a></td><td><code>Forgot to import cached_function</code></td></tr></table>




---

archive/issue_comments_217533.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -4,3 +4,4 @@\n     \n This ticket does the same for `finite_state_machine.setup_latex_preamble`.\n \n+Moreover, `finite_state_machine.setup_latex_preamble` is now loaded automatically (as it is the case in the graphs module).\n``````\n",
    "created_at": "2014-07-06T17:09:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16557",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16557#issuecomment-217533",
    "user": "https://github.com/cheuberg"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -4,3 +4,4 @@
     
 This ticket does the same for `finite_state_machine.setup_latex_preamble`.
 
+Moreover, `finite_state_machine.setup_latex_preamble` is now loaded automatically (as it is the case in the graphs module).
``````




---

archive/issue_comments_217534.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThanks for pointing this out, I was not aware of the fact that the graphs module calls `setup_latex_preamble` automatically. Therefore, I submitted a patch for adapting the documentation of the graphs module (#16620).\n\nSo, we now call `setup_latex_preamble` automatically (not when first loading the file as suggested, but when `_latex_` is called as the graphs module does). To reduce overhead,\n`setup_latex_preamble` is now a cached function (as in the graphs module). Finally, the documentation has been adapted to reflect the change.",
    "created_at": "2014-07-06T17:09:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16557",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16557#issuecomment-217534",
    "user": "https://github.com/cheuberg"
}
```

<div id="comment:4" align="right">Comment 4</div>

Thanks for pointing this out, I was not aware of the fact that the graphs module calls `setup_latex_preamble` automatically. Therefore, I submitted a patch for adapting the documentation of the graphs module (#16620).

So, we now call `setup_latex_preamble` automatically (not when first loading the file as suggested, but when `_latex_` is called as the graphs module does). To reduce overhead,
`setup_latex_preamble` is now a cached function (as in the graphs module). Finally, the documentation has been adapted to reflect the change.



---

archive/issue_events_218925.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-07-06T17:09:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16557",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16557#event-218925"
}
```



---

archive/issue_events_218926.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-07-06T17:09:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16557",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16557#event-218926"
}
```



---

archive/issue_events_218927.json:
```json
{
    "actor": "https://github.com/sagetrac-elixyre",
    "created_at": "2014-07-16T03:35:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16557",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16557#event-218927"
}
```



---

archive/issue_events_218928.json:
```json
{
    "actor": "https://github.com/sagetrac-elixyre",
    "created_at": "2014-07-16T03:35:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16557",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16557#event-218928"
}
```



---

archive/issue_events_218929.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-07-17T00:01:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16557",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16557#event-218929"
}
```



---

archive/issue_events_218930.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "04bdc5b939a2b97f57dd9569ee15deca32895d3f",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-07-17T00:01:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16557",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16557#event-218930"
}
```



---

archive/issue_comments_217535.json:
```json
{
    "body": "Changed branch from **[u/cheuberg/fsm/mathjax-avoid-tikz](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/mathjax-avoid-tikz)** to **[1bfb513](https://github.com/sagemath/sagetrac-mirror/commit/1bfb513ca3a9dc11a232bdd6ee31625fe5822572)**",
    "created_at": "2014-07-17T00:01:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16557",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16557#issuecomment-217535",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/cheuberg/fsm/mathjax-avoid-tikz](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/mathjax-avoid-tikz)** to **[1bfb513](https://github.com/sagemath/sagetrac-mirror/commit/1bfb513ca3a9dc11a232bdd6ee31625fe5822572)**
