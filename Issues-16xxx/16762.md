# Issue 16762: Extend FiniteStateMachine.product_FiniteStateMachine: keep isolated states

archive/issues_016525.json:
```json
{
    "assignees": [],
    "body": "Isolated states are currently not created in `FiniteStateMachine.product_FiniteStateMachine`. This may lead to products without any states. This is not desirable: there might be final output words, hooks, etc.\n\nPlanned behaviour:\n\n```\nsage: F = Transducer(initial_states=[0])\nsage: F.add_state(1)\n1\nsage: G = Transducer(initial_states=['A'])\nsage: F.product_FiniteStateMachine(G, None).states()\n[(0, 'A')]\nsage: F.product_FiniteStateMachine(\n....:     G, None, only_accessible_components=False).states()\n[(0, 'A'), (1, 'A')]\n```\n\nCurrent behaviour is that the two last tests yield `[]`.\n\nDepends on #16758\n\nCC:  @sagetrac-skropf @dkrenn\n\nKeywords: **product_FiniteStateMachine, isolated state**\n\nBranch/Commit: **[5b75691](https://github.com/sagemath/sagetrac-mirror/commit/5b75691cfea85d2da9c47aa66b2c5f64e9ad31a7)**\n\nReviewer: **Daniel Krenn, Sara Kropf**\n\nAuthor: **Clemens Heuberger**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16762_\n\n",
    "closed_at": "2014-08-11T12:07:40Z",
    "created_at": "2014-08-05T04:55:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20finite%20state%20machines",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Extend FiniteStateMachine.product_FiniteStateMachine: keep isolated states",
    "type": "issue",
    "updated_at": "2014-08-11T12:07:40Z",
    "url": "https://github.com/sagemath/sage/issues/16762",
    "user": "https://github.com/cheuberg"
}
```
Isolated states are currently not created in `FiniteStateMachine.product_FiniteStateMachine`. This may lead to products without any states. This is not desirable: there might be final output words, hooks, etc.

Planned behaviour:

```
sage: F = Transducer(initial_states=[0])
sage: F.add_state(1)
1
sage: G = Transducer(initial_states=['A'])
sage: F.product_FiniteStateMachine(G, None).states()
[(0, 'A')]
sage: F.product_FiniteStateMachine(
....:     G, None, only_accessible_components=False).states()
[(0, 'A'), (1, 'A')]
```

Current behaviour is that the two last tests yield `[]`.

Depends on #16758

CC:  @sagetrac-skropf @dkrenn

Keywords: **product_FiniteStateMachine, isolated state**

Branch/Commit: **[5b75691](https://github.com/sagemath/sagetrac-mirror/commit/5b75691cfea85d2da9c47aa66b2c5f64e9ad31a7)**

Reviewer: **Daniel Krenn, Sara Kropf**

Author: **Clemens Heuberger**

_Issue created by migration from https://trac.sagemath.org/ticket/16762_





---

archive/issue_events_221986.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-08-05T04:55:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16762",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16762#event-221986"
}
```



---

archive/issue_events_221987.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-08-05T04:55:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16762",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20finite%20state%20machines",
    "label_color": "0000ff",
    "label_name": "component: finite state machines",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16762#event-221987"
}
```



---

archive/issue_events_221988.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-08-05T04:55:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16762",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16762#event-221988"
}
```



---

archive/issue_events_221989.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-08-05T04:55:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16762",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16762#event-221989"
}
```



---

archive/issue_comments_222083.json:
```json
{
    "body": "Commit: **[5b75691](https://github.com/sagemath/sagetrac-mirror/commit/5b75691cfea85d2da9c47aa66b2c5f64e9ad31a7)**",
    "created_at": "2014-08-05T05:16:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16762",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16762#issuecomment-222083",
    "user": "https://github.com/cheuberg"
}
```

Commit: **[5b75691](https://github.com/sagemath/sagetrac-mirror/commit/5b75691cfea85d2da9c47aa66b2c5f64e9ad31a7)**



---

archive/issue_comments_222084.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nWrote a fix and implemented the above doctests. Merged #16758 to resolve conflict.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/76277a113087393531022604b44c97d13ebba436\">76277a1</a></td><td><code>trac #16762: failing doctests</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d33f453ec2c0ac94651e5f21c99b34cb2b3512e8\">d33f453</a></td><td><code>trac #16762: keep isolated states</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5704b119ea88d6aca50f2184bdd2e98b9b68708d\">5704b11</a></td><td><code>trac #16758: Add failing doctests</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6dc0263a933affbc89820c328a13606f47db70ae\">6dc0263</a></td><td><code>trac #16758: Disallow composition with automaton</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/61f6acf619b2c13010ea4c420743fc05fededc8b\">61f6acf</a></td><td><code>trac #16758: Use class of outer machine in composition</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5b75691cfea85d2da9c47aa66b2c5f64e9ad31a7\">5b75691</a></td><td><code>Merge remote-tracking branch 'fsm/composition-types' (#16758) into fsm/product_isolated</code></td></tr></table>\n",
    "created_at": "2014-08-05T05:16:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16762",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16762#issuecomment-222084",
    "user": "https://github.com/cheuberg"
}
```

<div id="comment:1" align="right">Comment 1</div>

Wrote a fix and implemented the above doctests. Merged #16758 to resolve conflict.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/76277a113087393531022604b44c97d13ebba436">76277a1</a></td><td><code>trac #16762: failing doctests</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d33f453ec2c0ac94651e5f21c99b34cb2b3512e8">d33f453</a></td><td><code>trac #16762: keep isolated states</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5704b119ea88d6aca50f2184bdd2e98b9b68708d">5704b11</a></td><td><code>trac #16758: Add failing doctests</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6dc0263a933affbc89820c328a13606f47db70ae">6dc0263</a></td><td><code>trac #16758: Disallow composition with automaton</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/61f6acf619b2c13010ea4c420743fc05fededc8b">61f6acf</a></td><td><code>trac #16758: Use class of outer machine in composition</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5b75691cfea85d2da9c47aa66b2c5f64e9ad31a7">5b75691</a></td><td><code>Merge remote-tracking branch 'fsm/composition-types' (#16758) into fsm/product_isolated</code></td></tr></table>




---

archive/issue_events_221990.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-08-05T05:16:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16762",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16762#event-221990"
}
```



---

archive/issue_comments_222085.json:
```json
{
    "body": "Author: **Clemens Heuberger**",
    "created_at": "2014-08-05T05:16:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16762",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16762#issuecomment-222085",
    "user": "https://github.com/cheuberg"
}
```

Author: **Clemens Heuberger**



---

archive/issue_comments_222086.json:
```json
{
    "body": "Dependencies: **#16758**",
    "created_at": "2014-08-05T05:16:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16762",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16762#issuecomment-222086",
    "user": "https://github.com/cheuberg"
}
```

Dependencies: **#16758**



---

archive/issue_comments_222087.json:
```json
{
    "body": "Branch: **[u/cheuberg/fsm/product_isolated](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/product_isolated)**",
    "created_at": "2014-08-05T05:16:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16762",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16762#issuecomment-222087",
    "user": "https://github.com/cheuberg"
}
```

Branch: **[u/cheuberg/fsm/product_isolated](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/product_isolated)**



---

archive/issue_comments_222088.json:
```json
{
    "body": "Reviewer: **Sara Kropf**",
    "created_at": "2014-08-05T08:46:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16762",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16762#issuecomment-222088",
    "user": "https://github.com/sagetrac-skropf"
}
```

Reviewer: **Sara Kropf**



---

archive/issue_comments_222089.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nThis looks fine for me.",
    "created_at": "2014-08-05T08:46:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16762",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16762#issuecomment-222089",
    "user": "https://github.com/sagetrac-skropf"
}
```

<div id="comment:2" align="right">Comment 2</div>

This looks fine for me.



---

archive/issue_events_221991.json:
```json
{
    "actor": "https://github.com/sagetrac-skropf",
    "created_at": "2014-08-05T08:46:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16762",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16762#event-221991"
}
```



---

archive/issue_events_221992.json:
```json
{
    "actor": "https://github.com/sagetrac-skropf",
    "created_at": "2014-08-05T08:46:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16762",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16762#event-221992"
}
```



---

archive/issue_comments_222090.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLooks also good to me.",
    "created_at": "2014-08-05T09:31:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16762",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16762#issuecomment-222090",
    "user": "https://github.com/dkrenn"
}
```

<div id="comment:3" align="right">Comment 3</div>

Looks also good to me.



---

archive/issue_comments_222091.json:
```json
{
    "body": "Changed reviewer from **Sara Kropf** to **Daniel Krenn, Sara Kropf**",
    "created_at": "2014-08-05T09:31:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16762",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16762#issuecomment-222091",
    "user": "https://github.com/dkrenn"
}
```

Changed reviewer from **Sara Kropf** to **Daniel Krenn, Sara Kropf**



---

archive/issue_events_221993.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16762",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16762#event-221993"
}
```



---

archive/issue_events_221994.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16762",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16762#event-221994"
}
```



---

archive/issue_comments_222092.json:
```json
{
    "body": "Changed branch from **[u/cheuberg/fsm/product_isolated](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/product_isolated)** to **[5b75691](https://github.com/sagemath/sagetrac-mirror/commit/5b75691cfea85d2da9c47aa66b2c5f64e9ad31a7)**",
    "created_at": "2014-08-11T12:07:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16762",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16762#issuecomment-222092",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/cheuberg/fsm/product_isolated](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/product_isolated)** to **[5b75691](https://github.com/sagemath/sagetrac-mirror/commit/5b75691cfea85d2da9c47aa66b2c5f64e9ad31a7)**



---

archive/issue_events_221995.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-08-11T12:07:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16762",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16762#event-221995"
}
```



---

archive/issue_events_221996.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "2c184fe5d46068304c60660f367153639ca470a3",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-08-11T12:07:40Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16762",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16762#event-221996"
}
```
