# Issue 16796: OSX App fails on 10.6

archive/issues_016559.json:
```json
{
    "body": "From the buildbot (this works on 10.9):\n\n```\nAnalyze AppController.m\n    cd /Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app\n    setenv LANG en_US.US-ASCII\n    /Developer/usr/bin/clang -x objective-c -arch i386 -fmessage-length=0 -pipe -fdiagnostics-print-source-range-info -std=gnu99 -Wno-trigraphs -fpascal-strings -O0 -Wreturn-type -Wunused-variable -fasm-blocks -mfix-and-continue -mmacosx-version-min=10.4 -gdwarf-2 -iquote /Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Sage.build/Debug/Sage.build/Sage-generated-files.hmap -I/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Sage.build/Debug/Sage.build/Sage-own-target-headers.hmap -I/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Sage.build/Debug/Sage.build/Sage-all-target-headers.hmap -iquote /Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Sage.build/Debug/Sage.build/Sage-project-headers.hmap -F/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Debug -I/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Debug/include -I/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Sage.build/Debug/Sage.build/DerivedSources/i386 -I/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Sage.build/Debug/Sage.build/DerivedSources -include /var/folders/Sq/SqJYMP6AF4KcJqFvHjeBeU+++VY/-Caches-/com.apple.Xcode.537/SharedPrecompiledHeaders/Sage_Prefix-dokpneuepjvknfdgtpkznyzrxbub/Sage_Prefix.pch --analyze /Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/AppController.m -o /Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Sage.build/Debug/Sage.build/StaticAnalyzer/normal/i386/AppController.plist\n\n/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/AppController.m:199:13: error: use of undeclared identifier 'NSModalResponse'\nAnalyzer skipped this file due to parse errors\n             NSModalResponse resp = [alert runModal];\n             ^\n/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/AppController.m:200:17: error: use of undeclared identifier 'resp'\nuse of undeclared identifier 'resp'\n             if (resp == NSModalResponseOK) {\n                 ^\n/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/AppController.m:200:25: error: use of undeclared identifier 'NSModalResponseOK'\nuse of undeclared identifier 'NSModalResponseOK'\n             if (resp == NSModalResponseOK) {\n                         ^\n/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/AppController.m:362:9: error: use of undeclared identifier 'NSModalResponse'\nuse of undeclared identifier 'NSModalResponse'\n         NSModalResponse resp = [alert runModal];\n         ^\n/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/AppController.m:363:13: error: use of undeclared identifier 'resp'\nuse of undeclared identifier 'resp'\n         if (resp == NSModalResponseOK) {// Quit\n             ^\n/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/AppController.m:363:21: error: use of undeclared identifier 'NSModalResponseOK'\nuse of undeclared identifier 'NSModalResponseOK'\n         if (resp == NSModalResponseOK) {// Quit\n                     ^\n/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/AppController.m:366:21: error: use of undeclared identifier 'resp'\nuse of undeclared identifier 'resp'\n         } else if ( resp == NSModalResponseCancel ) { // Continue\n                     ^\n/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/AppController.m:366:29: error: use of undeclared identifier 'NSModalResponseCancel'\nuse of undeclared identifier 'NSModalResponseCancel'\n         } else if ( resp == NSModalResponseCancel ) { // Continue\n                             ^\n8 errors generated.\nCompileC build/Sage.build/Debug/Sage.build/Objects-normal/i386/AppDelegate.o AppDelegate.m normal i386 objective-c com.apple.compilers.gcc.4_2\n    cd /Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app\n    setenv LANG en_US.US-ASCII\n    /Developer/usr/bin/gcc-4.2 -x objective-c -arch i386 -fmessage-length=0 -pipe -std=gnu99 -Wno-trigraphs -fpascal-strings -fasm-blocks -O0 -Wreturn-type -Wunused-variable -mfix-and-continue -mmacosx-version-min=10.4 -gdwarf-2 -iquote /Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Sage.build/Debug/Sage.build/Sage-generated-files.hmap -I/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Sage.build/Debug/Sage.build/Sage-own-target-headers.hmap -I/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Sage.build/Debug/Sage.build/Sage-all-target-headers.hmap -iquote /Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Sage.build/Debug/Sage.build/Sage-project-headers.hmap -F/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Debug -I/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Debug/include -I/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Sage.build/Debug/Sage.build/DerivedSources/i386 -I/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Sage.build/Debug/Sage.build/DerivedSources -include /var/folders/Sq/SqJYMP6AF4KcJqFvHjeBeU+++VY/-Caches-/com.apple.Xcode.537/SharedPrecompiledHeaders/Sage_Prefix-gxeuizjduvirzagieqssinllffch/Sage_Prefix.pch -c /Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/AppDelegate.m -o /Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Sage.build/Debug/Sage.build/Objects-normal/i386/AppDelegate.o\n\nPhaseScriptExecution \"Run Script\" build/Sage.build/Debug/Sage.build/Script-1967A8B911D6696A00ABC39D.sh\n    cd /Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app\n    /bin/sh -c /Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Sage.build/Debug/Sage.build/Script-1967A8B911D6696A00ABC39D.sh\n\n** BUILD FAILED **\n\n\nThe following build commands failed:\nSage:\nCompileC build/Sage.build/Debug/Sage.build/Objects-normal/i386/AppController.o /Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/AppController.m normal i386 objective-c com.apple.compilers.gcc.4_2\n(1 failure)\n```\n\nCC:  @gvol\n\nReviewer: Karl-Dieter Crisman, Volker Braun\n\nAuthor: Ivan Andrus\n\nBranch: 70f18895f3a8d310e7d6cf19daede7a0726cd41e\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/16796\n\n",
    "closed_at": "2014-08-20T20:21:53Z",
    "created_at": "2014-08-11T12:20:07Z",
    "labels": [
        "component: packages: standard",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.4",
    "title": "OSX App fails on 10.6",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/16796",
    "user": "https://github.com/vbraun"
}
```
From the buildbot (this works on 10.9):

```
Analyze AppController.m
    cd /Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app
    setenv LANG en_US.US-ASCII
    /Developer/usr/bin/clang -x objective-c -arch i386 -fmessage-length=0 -pipe -fdiagnostics-print-source-range-info -std=gnu99 -Wno-trigraphs -fpascal-strings -O0 -Wreturn-type -Wunused-variable -fasm-blocks -mfix-and-continue -mmacosx-version-min=10.4 -gdwarf-2 -iquote /Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Sage.build/Debug/Sage.build/Sage-generated-files.hmap -I/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Sage.build/Debug/Sage.build/Sage-own-target-headers.hmap -I/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Sage.build/Debug/Sage.build/Sage-all-target-headers.hmap -iquote /Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Sage.build/Debug/Sage.build/Sage-project-headers.hmap -F/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Debug -I/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Debug/include -I/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Sage.build/Debug/Sage.build/DerivedSources/i386 -I/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Sage.build/Debug/Sage.build/DerivedSources -include /var/folders/Sq/SqJYMP6AF4KcJqFvHjeBeU+++VY/-Caches-/com.apple.Xcode.537/SharedPrecompiledHeaders/Sage_Prefix-dokpneuepjvknfdgtpkznyzrxbub/Sage_Prefix.pch --analyze /Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/AppController.m -o /Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Sage.build/Debug/Sage.build/StaticAnalyzer/normal/i386/AppController.plist

/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/AppController.m:199:13: error: use of undeclared identifier 'NSModalResponse'
Analyzer skipped this file due to parse errors
             NSModalResponse resp = [alert runModal];
             ^
/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/AppController.m:200:17: error: use of undeclared identifier 'resp'
use of undeclared identifier 'resp'
             if (resp == NSModalResponseOK) {
                 ^
/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/AppController.m:200:25: error: use of undeclared identifier 'NSModalResponseOK'
use of undeclared identifier 'NSModalResponseOK'
             if (resp == NSModalResponseOK) {
                         ^
/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/AppController.m:362:9: error: use of undeclared identifier 'NSModalResponse'
use of undeclared identifier 'NSModalResponse'
         NSModalResponse resp = [alert runModal];
         ^
/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/AppController.m:363:13: error: use of undeclared identifier 'resp'
use of undeclared identifier 'resp'
         if (resp == NSModalResponseOK) {// Quit
             ^
/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/AppController.m:363:21: error: use of undeclared identifier 'NSModalResponseOK'
use of undeclared identifier 'NSModalResponseOK'
         if (resp == NSModalResponseOK) {// Quit
                     ^
/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/AppController.m:366:21: error: use of undeclared identifier 'resp'
use of undeclared identifier 'resp'
         } else if ( resp == NSModalResponseCancel ) { // Continue
                     ^
/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/AppController.m:366:29: error: use of undeclared identifier 'NSModalResponseCancel'
use of undeclared identifier 'NSModalResponseCancel'
         } else if ( resp == NSModalResponseCancel ) { // Continue
                             ^
8 errors generated.
CompileC build/Sage.build/Debug/Sage.build/Objects-normal/i386/AppDelegate.o AppDelegate.m normal i386 objective-c com.apple.compilers.gcc.4_2
    cd /Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app
    setenv LANG en_US.US-ASCII
    /Developer/usr/bin/gcc-4.2 -x objective-c -arch i386 -fmessage-length=0 -pipe -std=gnu99 -Wno-trigraphs -fpascal-strings -fasm-blocks -O0 -Wreturn-type -Wunused-variable -mfix-and-continue -mmacosx-version-min=10.4 -gdwarf-2 -iquote /Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Sage.build/Debug/Sage.build/Sage-generated-files.hmap -I/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Sage.build/Debug/Sage.build/Sage-own-target-headers.hmap -I/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Sage.build/Debug/Sage.build/Sage-all-target-headers.hmap -iquote /Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Sage.build/Debug/Sage.build/Sage-project-headers.hmap -F/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Debug -I/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Debug/include -I/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Sage.build/Debug/Sage.build/DerivedSources/i386 -I/Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Sage.build/Debug/Sage.build/DerivedSources -include /var/folders/Sq/SqJYMP6AF4KcJqFvHjeBeU+++VY/-Caches-/com.apple.Xcode.537/SharedPrecompiledHeaders/Sage_Prefix-gxeuizjduvirzagieqssinllffch/Sage_Prefix.pch -c /Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/AppDelegate.m -o /Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Sage.build/Debug/Sage.build/Objects-normal/i386/AppDelegate.o

PhaseScriptExecution "Run Script" build/Sage.build/Debug/Sage.build/Script-1967A8B911D6696A00ABC39D.sh
    cd /Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app
    /bin/sh -c /Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/build/Sage.build/Debug/Sage.build/Script-1967A8B911D6696A00ABC39D.sh

** BUILD FAILED **


The following build commands failed:
Sage:
CompileC build/Sage.build/Debug/Sage.build/Objects-normal/i386/AppController.o /Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/AppController.m normal i386 objective-c com.apple.compilers.gcc.4_2
(1 failure)
```

CC:  @gvol

Reviewer: Karl-Dieter Crisman, Volker Braun

Author: Ivan Andrus

Branch: 70f18895f3a8d310e7d6cf19daede7a0726cd41e

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/16796





---

archive/issue_comments_279806.json:
```json
{
    "body": "<a id='comment:1'></a>I have an error in building on OS X 10.7 as well.  It's similar but not identical - see [here](http://boxen.math.washington.edu/home/kcrisman/MacAppBuildError.txt).  In particular, it seems to have a slightly better error message about what is wrong.\n\n```\n/Users/.../Downloads/sage/src/mac-app/AppController.m:199:13: error: use of undeclared identifier 'NSModalResponse'; did you mean 'NSURLResponse'?\n            NSModalResponse resp = [alert runModal];\n            ^\n```\nLooks like [this commit](http://git.sagemath.org/sage.git/commit/?h=eb134581fc1b354948e99e150e6c5fdd63cd9120) is the \"problem\".  In particular, `NSModalResponse` is [\"available in OS X v10.9 and later\"](https://developer.apple.com/library/mac/documentation/Cocoa/Reference/ApplicationKit/Classes/NSApplication_Class/Reference/Reference.html#//apple_ref/c/tdef/NSModalResponse).",
    "created_at": "2014-08-12T02:34:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16796#issuecomment-279806",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:1'></a>I have an error in building on OS X 10.7 as well.  It's similar but not identical - see [here](http://boxen.math.washington.edu/home/kcrisman/MacAppBuildError.txt).  In particular, it seems to have a slightly better error message about what is wrong.

```
/Users/.../Downloads/sage/src/mac-app/AppController.m:199:13: error: use of undeclared identifier 'NSModalResponse'; did you mean 'NSURLResponse'?
            NSModalResponse resp = [alert runModal];
            ^
```
Looks like [this commit](http://git.sagemath.org/sage.git/commit/?h=eb134581fc1b354948e99e150e6c5fdd63cd9120) is the "problem".  In particular, `NSModalResponse` is ["available in OS X v10.9 and later"](https://developer.apple.com/library/mac/documentation/Cocoa/Reference/ApplicationKit/Classes/NSApplication_Class/Reference/Reference.html#//apple_ref/c/tdef/NSModalResponse).



---

archive/issue_comments_279807.json:
```json
{
    "body": "<a id='comment:2'></a>Clearly a blocker.  If Ivan and/or others can check this out soon enough, maybe we can even release 6.3.1 or something.  I don't know if there is an easy way to replicate this functionality without it, though.  So did no one test this #15732 on anything except Mavericks?   I specifically said that I didn't test it, and assumed someone else did on something else - trust but verify, I guess :(",
    "created_at": "2014-08-12T02:38:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16796#issuecomment-279807",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:2'></a>Clearly a blocker.  If Ivan and/or others can check this out soon enough, maybe we can even release 6.3.1 or something.  I don't know if there is an easy way to replicate this functionality without it, though.  So did no one test this #15732 on anything except Mavericks?   I specifically said that I didn't test it, and assumed someone else did on something else - trust but verify, I guess :(



---

archive/issue_events_049162.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-08-12T02:38:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16796",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16796#event-49162"
}
```



---

archive/issue_comments_279808.json:
```json
{
    "body": "Changing priority from major to blocker.",
    "created_at": "2014-08-12T02:38:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16796#issuecomment-279808",
    "user": "https://github.com/kcrisman"
}
```

Changing priority from major to blocker.



---

archive/issue_comments_279809.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2014-08-12T03:33:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16796#issuecomment-279809",
    "user": "https://github.com/gvol"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_279810.json:
```json
{
    "body": "Changing commit from \"\" to \"70f18895f3a8d310e7d6cf19daede7a0726cd41e\"",
    "created_at": "2014-08-12T03:33:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16796#issuecomment-279810",
    "user": "https://github.com/gvol"
}
```

Changing commit from "" to "70f18895f3a8d310e7d6cf19daede7a0726cd41e"



---

archive/issue_comments_279811.json:
```json
{
    "body": "Changing branch from \"\" to \"u/iandrus/trac-16796\"",
    "created_at": "2014-08-12T03:33:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16796#issuecomment-279811",
    "user": "https://github.com/gvol"
}
```

Changing branch from "" to "u/iandrus/trac-16796"



---

archive/issue_comments_279812.json:
```json
{
    "body": "Changing author from \"\" to \"Ivan Andrus\"",
    "created_at": "2014-08-12T03:33:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16796#issuecomment-279812",
    "user": "https://github.com/gvol"
}
```

Changing author from "" to "Ivan Andrus"



---

archive/issue_comments_279813.json:
```json
{
    "body": "<a id='comment:3'></a>New commits:",
    "created_at": "2014-08-12T03:33:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16796#issuecomment-279813",
    "user": "https://github.com/gvol"
}
```

<a id='comment:3'></a>New commits:



---

archive/issue_comments_279814.json:
```json
{
    "body": "<a id='comment:4'></a>Hopefully this fixes it.  Sadly I can't test on anything but Mavericks.",
    "created_at": "2014-08-12T03:36:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16796#issuecomment-279814",
    "user": "https://github.com/gvol"
}
```

<a id='comment:4'></a>Hopefully this fixes it.  Sadly I can't test on anything but Mavericks.



---

archive/issue_comments_279815.json:
```json
{
    "body": "<a id='comment:5'></a>Thanks, I'll try that right now.",
    "created_at": "2014-08-12T03:46:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16796#issuecomment-279815",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:5'></a>Thanks, I'll try that right now.



---

archive/issue_comments_279816.json:
```json
{
    "body": "<a id='comment:6'></a>\n```\n** BUILD SUCCEEDED **\n\nCopying Sage.app\n/Users/.../Downloads/sage/src/bin/sage-bdist: line 106: cd: 6.3: No such file or directory\n```\nSo good news, but I don't understand why this `cd` error is happening.  I suppose I am inside of 6.3 already (`./sage -bdist 6.3` and see `TMP_DIR` in `src/bin/sage-bdist`) but this never happened before.  Anyway, I'm making the dmg \"by hand\", hopefully all will be well.",
    "created_at": "2014-08-12T05:38:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16796#issuecomment-279816",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:6'></a>
```
** BUILD SUCCEEDED **

Copying Sage.app
/Users/.../Downloads/sage/src/bin/sage-bdist: line 106: cd: 6.3: No such file or directory
```
So good news, but I don't understand why this `cd` error is happening.  I suppose I am inside of 6.3 already (`./sage -bdist 6.3` and see `TMP_DIR` in `src/bin/sage-bdist`) but this never happened before.  Anyway, I'm making the dmg "by hand", hopefully all will be well.



---

archive/issue_comments_279817.json:
```json
{
    "body": "<a id='comment:7'></a>Oh, I see.  Yet another not-well-advertised change, and of course with no Trac # in the git log.  Guess I haven't `bdist`ed in a while.\n\nAnd indeed `SAGE_VERSION` is gotten from elsewhere now.  But NO CHANGES TO THE DOCUMENTATION and even http://sagemath.org/doc/installation/source.html?highlight=bdist still has the old syntax.  **Not acceptable.**",
    "created_at": "2014-08-12T05:56:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16796#issuecomment-279817",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:7'></a>Oh, I see.  Yet another not-well-advertised change, and of course with no Trac # in the git log.  Guess I haven't `bdist`ed in a while.

And indeed `SAGE_VERSION` is gotten from elsewhere now.  But NO CHANGES TO THE DOCUMENTATION and even http://sagemath.org/doc/installation/source.html?highlight=bdist still has the old syntax.  **Not acceptable.**



---

archive/issue_comments_279818.json:
```json
{
    "body": "<a id='comment:8'></a>The fix seems to work, modulo my garbled input to `sage -bdist`, and at least on 10.7.  Ivan, it works correctly for 10.9?  Volker, if this also works for you (and I would check that the bdisted app actually runs, just in case - we had some examples of an app that built but didn't run a while back, I think) then this can have positive review.\n\nIn which event either I can upload my 10.7 binary or you can bdist one from the buildbot.  I guess we wouldn't really need a big announcement, since this only affects building binaries; in a pinch we could even upload these as 6.3, though I guess 6.3.1 won't really say much to most people.",
    "created_at": "2014-08-12T08:59:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16796#issuecomment-279818",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:8'></a>The fix seems to work, modulo my garbled input to `sage -bdist`, and at least on 10.7.  Ivan, it works correctly for 10.9?  Volker, if this also works for you (and I would check that the bdisted app actually runs, just in case - we had some examples of an app that built but didn't run a while back, I think) then this can have positive review.

In which event either I can upload my 10.7 binary or you can bdist one from the buildbot.  I guess we wouldn't really need a big announcement, since this only affects building binaries; in a pinch we could even upload these as 6.3, though I guess 6.3.1 won't really say much to most people.



---

archive/issue_comments_279819.json:
```json
{
    "body": "<a id='comment:9'></a>Hello!  I'm offering a 10.7 binary :-) but I don't want to upload it unless I have to as it might take a while.  If Volker can just quick check on his buildbot as he has time, then this is ready to go, I think.",
    "created_at": "2014-08-14T13:36:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16796#issuecomment-279819",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:9'></a>Hello!  I'm offering a 10.7 binary :-) but I don't want to upload it unless I have to as it might take a while.  If Volker can just quick check on his buildbot as he has time, then this is ready to go, I think.



---

archive/issue_comments_279820.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Karl-Dieter Crisman\"",
    "created_at": "2014-08-14T13:36:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16796#issuecomment-279820",
    "user": "https://github.com/kcrisman"
}
```

Changing reviewer from "" to "Karl-Dieter Crisman"



---

archive/issue_comments_279821.json:
```json
{
    "body": "<a id='comment:10'></a>Sorry, I forgot to say that I have tested on 10.9 including `sage -bdist` and things are working.",
    "created_at": "2014-08-15T18:14:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16796#issuecomment-279821",
    "user": "https://github.com/gvol"
}
```

<a id='comment:10'></a>Sorry, I forgot to say that I have tested on 10.9 including `sage -bdist` and things are working.



---

archive/issue_comments_279822.json:
```json
{
    "body": "<a id='comment:11'></a>Sounds like bsd.math is down for a while, so I say this will have to be enough for positive review.  What do you all think?  (I will eventually be able to try on 10.4 but it may be several days, and that has other issues anyway; I don't have access to 10.6.)\n\n[Another take:](http://www.computerworld.com/s/article/9246609/Apple_retires_Snow_Leopard_from_support_leaves_1_in_5_Macs_vulnerable_to_attacks)\n> \"Let's face it, Apple doesn't go out of their way to ensure users are aware when products are going end of life,\" said Andrew Storms, director of DevOps at security company CloudPassage, in a December interview.",
    "created_at": "2014-08-20T13:36:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16796#issuecomment-279822",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:11'></a>Sounds like bsd.math is down for a while, so I say this will have to be enough for positive review.  What do you all think?  (I will eventually be able to try on 10.4 but it may be several days, and that has other issues anyway; I don't have access to 10.6.)

[Another take:](http://www.computerworld.com/s/article/9246609/Apple_retires_Snow_Leopard_from_support_leaves_1_in_5_Macs_vulnerable_to_attacks)
> "Let's face it, Apple doesn't go out of their way to ensure users are aware when products are going end of life," said Andrew Storms, director of DevOps at security company CloudPassage, in a December interview.



---

archive/issue_comments_279823.json:
```json
{
    "body": "<a id='comment:12'></a>lgtm",
    "created_at": "2014-08-20T15:01:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16796#issuecomment-279823",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:12'></a>lgtm



---

archive/issue_comments_279824.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2014-08-20T15:01:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16796#issuecomment-279824",
    "user": "https://github.com/vbraun"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_279825.json:
```json
{
    "body": "Changing reviewer from \"Karl-Dieter Crisman\" to \"Karl-Dieter Crisman, Volker Braun\"",
    "created_at": "2014-08-20T15:01:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16796#issuecomment-279825",
    "user": "https://github.com/vbraun"
}
```

Changing reviewer from "Karl-Dieter Crisman" to "Karl-Dieter Crisman, Volker Braun"



---

archive/issue_events_049163.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-08-20T20:21:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/16796",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16796#event-49163"
}
```



---

archive/issue_comments_279826.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2014-08-20T20:21:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16796#issuecomment-279826",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_comments_279827.json:
```json
{
    "body": "Changing branch from \"u/iandrus/trac-16796\" to \"70f18895f3a8d310e7d6cf19daede7a0726cd41e\"",
    "created_at": "2014-08-20T20:21:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16796#issuecomment-279827",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/iandrus/trac-16796" to "70f18895f3a8d310e7d6cf19daede7a0726cd41e"



---

archive/issue_comments_279828.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -53,7 +53,7 @@\n \n The following build commands failed:\n Sage:\n-\tCompileC build/Sage.build/Debug/Sage.build/Objects-normal/i386/AppController.o /Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/AppController.m normal i386 objective-c com.apple.compilers.gcc.4_2\n+CompileC build/Sage.build/Debug/Sage.build/Objects-normal/i386/AppController.o /Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/AppController.m normal i386 objective-c com.apple.compilers.gcc.4_2\n (1 failure)\n ```\n \n``````\n",
    "created_at": "2015-09-06T15:41:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16796#issuecomment-279828",
    "user": "https://github.com/fchapoton"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -53,7 +53,7 @@
 
 The following build commands failed:
 Sage:
-	CompileC build/Sage.build/Debug/Sage.build/Objects-normal/i386/AppController.o /Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/AppController.m normal i386 objective-c com.apple.compilers.gcc.4_2
+CompileC build/Sage.build/Debug/Sage.build/Objects-normal/i386/AppController.o /Users/buildbot/build/sage/bsd-1/sage_git/build/src/mac-app/AppController.m normal i386 objective-c com.apple.compilers.gcc.4_2
 (1 failure)
 ```
 
``````




---

archive/issue_comments_279829.json:
```json
{
    "body": "Changing commit from \"70f18895f3a8d310e7d6cf19daede7a0726cd41e\" to \"\"",
    "created_at": "2015-09-06T15:41:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16796",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16796#issuecomment-279829",
    "user": "https://github.com/fchapoton"
}
```

Changing commit from "70f18895f3a8d310e7d6cf19daede7a0726cd41e" to ""
