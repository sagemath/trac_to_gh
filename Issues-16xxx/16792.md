...
```json
{
    "body": "<a id='comment:1'></a>Is there a reason why not using `self.cache.clear()`?",
    "created_at": "2014-08-10T12:40:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16792",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16792#issuecomment-231869",
    "user": "https://github.com/dkrenn"
}
```

<a id='comment:1'></a>Is there a reason why not using `self.cache.clear()`?
...
```json
{
    "body": "<a id='comment:2'></a>Your point is probably correct, but your code timing doesn't really corroborate it. Timeit, as used, it still repeating the command thrice. It's a mutating command, so the second time around, the original length of the dict doesn't come into play at all.\n\n```\nsage: %time E.clear()\nCPU times: user 11 ms, sys: 6 ms, total: 17 ms\nsage: %time noop.clear_cache()\nCPU times: user 59 ms, sys: 5 ms, total: 64 ms\nWall time: 64 ms\n```\nprobably gives a fairer picture. Your original numbers would have meant that `E.clear()` would have visited al 1000000 entries (they all have to be decreffed!) in less than 1 microsecond, meaning your computer would be performing more than 10<sup>12</sup> operations per second. Terahertz performance is usually not so easily attained.",
    "created_at": "2014-08-10T15:57:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16792",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16792#issuecomment-231870",
    "user": "https://github.com/nbruin"
}
```

<a id='comment:2'></a>Your point is probably correct, but your code timing doesn't really corroborate it. Timeit, as used, it still repeating the command thrice. It's a mutating command, so the second time around, the original length of the dict doesn't come into play at all.
...
```json
{
    ...
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16792#event-49158"
}
```
