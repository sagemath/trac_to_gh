# Issue 16278: MPFI's spkg-install overwrites CFLAGS

archive/issues_016041.json:
```json
{
    "assignees": [],
    "body": "\n```sh\nif [ \"$SAGE64\" = \"yes\" ]; then\n   echo \"64 bit build\"\n   CFLAGS=\"-O2 -g -m64 -fPIC\"; export CFLAGS\n   CXXLAGS=\"-O2 -g -m64 -fPIC\"; export CXXFLAGS\n   LDFLAGS=\"-m64 \"; export LDFLAGS\nelse\n   CFLAGS=\"-O2 -g -fPIC\"; export CFLAGS\nfi\n```\n\n\nBranch/Commit: **[99b4712](https://github.com/sagemath/sagetrac-mirror/commit/99b471232d51219bd1eb13f529414c019212db04)**\n\nReviewer: **Volker Braun**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16278_\n\n",
    "closed_at": "2014-10-26T19:22:20Z",
    "created_at": "2014-05-02T13:41:45Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "MPFI's spkg-install overwrites CFLAGS",
    "type": "issue",
    "updated_at": "2014-10-26T19:22:20Z",
    "url": "https://github.com/sagemath/sage/issues/16278",
    "user": "https://github.com/nexttime"
}
```

```sh
if [ "$SAGE64" = "yes" ]; then
   echo "64 bit build"
   CFLAGS="-O2 -g -m64 -fPIC"; export CFLAGS
   CXXLAGS="-O2 -g -m64 -fPIC"; export CXXFLAGS
   LDFLAGS="-m64 "; export LDFLAGS
else
   CFLAGS="-O2 -g -fPIC"; export CFLAGS
fi
```


Branch/Commit: **[99b4712](https://github.com/sagemath/sagetrac-mirror/commit/99b471232d51219bd1eb13f529414c019212db04)**

Reviewer: **Volker Braun**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/16278_





---

archive/issue_events_215007.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2014-05-02T13:41:45Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16278",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16278#event-215007"
}
```



---

archive/issue_events_215008.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2014-05-02T13:41:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16278",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16278#event-215008"
}
```



---

archive/issue_events_215009.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2014-05-02T13:41:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16278",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16278#event-215009"
}
```



---

archive/issue_events_215010.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2014-05-02T13:41:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16278",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16278#event-215010"
}
```



---

archive/issue_comments_211436.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nBesides setting `CXXLAGS`[sic] if `SAGE64=yes`... 8)",
    "created_at": "2014-05-02T13:46:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16278#issuecomment-211436",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:1" align="right">Comment 1</div>

Besides setting `CXXLAGS`[sic] if `SAGE64=yes`... 8)



---

archive/issue_events_215011.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16278",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16278#event-215011"
}
```



---

archive/issue_events_215012.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16278",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16278#event-215012"
}
```



---

archive/issue_comments_211437.json:
```json
{
    "body": "Branch: **[u/chapoton/16278](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/16278)**",
    "created_at": "2014-10-21T14:10:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16278#issuecomment-211437",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/16278](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/16278)**



---

archive/issue_events_215013.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-10-21T14:10:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16278",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16278#event-215013"
}
```



---

archive/issue_comments_211438.json:
```json
{
    "body": "Commit: **[99b4712](https://github.com/sagemath/sagetrac-mirror/commit/99b471232d51219bd1eb13f529414c019212db04)**",
    "created_at": "2014-10-21T14:10:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16278#issuecomment-211438",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[99b4712](https://github.com/sagemath/sagetrac-mirror/commit/99b471232d51219bd1eb13f529414c019212db04)**



---

archive/issue_comments_211439.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/99b471232d51219bd1eb13f529414c019212db04\">99b4712</a></td><td><code>trac #16728 first proposal</code></td></tr></table>\n",
    "created_at": "2014-10-21T14:10:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16278#issuecomment-211439",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:3" align="right">Comment 3</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/99b471232d51219bd1eb13f529414c019212db04">99b4712</a></td><td><code>trac #16728 first proposal</code></td></tr></table>




---

archive/issue_comments_211440.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2014-10-26T13:14:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16278#issuecomment-211440",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_events_215014.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-10-26T13:14:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16278",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16278#event-215014"
}
```



---

archive/issue_events_215015.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-10-26T13:14:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16278",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16278#event-215015"
}
```



---

archive/issue_comments_211441.json:
```json
{
    "body": "Author: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2014-10-26T13:14:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16278#issuecomment-211441",
    "user": "https://github.com/vbraun"
}
```

Author: **Frédéric Chapoton**



---

archive/issue_events_215016.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-10-26T19:22:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16278",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16278#event-215016"
}
```



---

archive/issue_events_215017.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "1e1cc3bd9b0cd5b88193072655508bc56825aa51",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-10-26T19:22:20Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16278",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16278#event-215017"
}
```



---

archive/issue_comments_211442.json:
```json
{
    "body": "Changed branch from **[u/chapoton/16278](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/16278)** to **[99b4712](https://github.com/sagemath/sagetrac-mirror/commit/99b471232d51219bd1eb13f529414c019212db04)**",
    "created_at": "2014-10-26T19:22:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16278#issuecomment-211442",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/chapoton/16278](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/16278)** to **[99b4712](https://github.com/sagemath/sagetrac-mirror/commit/99b471232d51219bd1eb13f529414c019212db04)**
