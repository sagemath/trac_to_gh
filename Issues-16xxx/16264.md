...
```json
{
    "body": "I'd suggest 2; why do you consider it a bad idea? I don't like throwing exceptions when the user is not at fault. I think 1 is too confusing.\n\nGood catch; the two methods will be considerably more useful if they return reasonable data when the rotation cannot be performed.",
...
```json
{
    "body": "Well, it seems odd to me to return None on such an operation. The user expects a binary tree, I don't see why the result should be None, it might be confusing.\n\nMaybe we should have a look on other combinatorial objects to see how this question is handled.\n\nBy the way, I didn't catch this error, FindStat did ;)",
...
```json
{
    "body": "I don't think it is a good idea. A rotation is really a rotation not a reflection!",
...
```json
{
    "body": "The problem with doing nothing is that it is expensive to check if the result is the result of a rotation or the result of doing nothing. If you add an additional variable that causes this behavior, OK, but I'd really not like seeing it as default behavior.",
