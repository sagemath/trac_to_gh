# Issue 16006: gengtype-lex.c:1:21: fatal error: bconfig.h: No such file or directory

archive/issues_015769.json:
```json
{
    "assignees": [],
    "body": "There is a race in GCC parallel build. Failed at least once on iras with\n\n```\n../../src/gcc/gengtype-lex.c:1:21: fatal error: bconfig.h: No such file or directory\ncompilation terminated.\nmake[6]: *** [gengtype-lex.o] Error 1\n```\nUpstream bug: http://gcc.gnu.org/bugzilla/show_bug.cgi?id=57125\n\nCC:  @jdemeyer\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16006_\n\n",
    "closed_at": "2017-09-22T13:36:38Z",
    "created_at": "2014-03-24T17:52:46Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "gengtype-lex.c:1:21: fatal error: bconfig.h: No such file or directory",
    "type": "issue",
    "updated_at": "2017-09-22T13:36:38Z",
    "url": "https://github.com/sagemath/sage/issues/16006",
    "user": "https://github.com/vbraun"
}
```
There is a race in GCC parallel build. Failed at least once on iras with

```
../../src/gcc/gengtype-lex.c:1:21: fatal error: bconfig.h: No such file or directory
compilation terminated.
make[6]: *** [gengtype-lex.o] Error 1
```
Upstream bug: http://gcc.gnu.org/bugzilla/show_bug.cgi?id=57125

CC:  @jdemeyer

_Issue created by migration from https://trac.sagemath.org/ticket/16006_





---

archive/issue_events_210962.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-03-24T17:52:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16006",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16006#event-210962"
}
```



---

archive/issue_events_210963.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-03-24T17:52:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16006",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16006#event-210963"
}
```



---

archive/issue_events_210964.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-03-24T17:52:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16006",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16006#event-210964"
}
```



---

archive/issue_events_210965.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-03-24T17:52:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16006",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16006#event-210965"
}
```



---

archive/issue_events_210966.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16006",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16006#event-210966"
}
```



---

archive/issue_events_210967.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16006",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16006#event-210967"
}
```



---

archive/issue_events_210968.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16006",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16006#event-210968"
}
```



---

archive/issue_events_210969.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16006",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16006#event-210969"
}
```



---

archive/issue_events_210970.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-09-22T13:36:38Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16006",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16006#event-210970"
}
```



---

archive/issue_events_210971.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-09-22T13:36:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16006",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16006#event-210971"
}
```



---

archive/issue_events_210972.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-09-22T13:36:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16006",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16006#event-210972"
}
```



---

archive/issue_comments_205884.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nI'm assuming that this bug is obsolete.",
    "created_at": "2017-09-22T13:36:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16006#issuecomment-205884",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:3" align="right">Comment 3</div>

I'm assuming that this bug is obsolete.



---

archive/issue_events_210973.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-09-22T13:36:38Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16006",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16006#event-210973"
}
```
