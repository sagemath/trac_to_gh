# Issue 16006: gengtype-lex.c:1:21: fatal error: bconfig.h: No such file or directory

archive/issues_015769.json:
```json
{
    "body": "There is a race in GCC parallel build. Failed at least once on iras with\n\n```\n../../src/gcc/gengtype-lex.c:1:21: fatal error: bconfig.h: No such file or directory\ncompilation terminated.\nmake[6]: *** [gengtype-lex.o] Error 1\n```\nUpstream bug: http://gcc.gnu.org/bugzilla/show_bug.cgi?id=57125\n\n**CC:**  @jdemeyer\n\n**Resolution:** invalid\n\nIssue created by migration from https://trac.sagemath.org/ticket/16006\n\n",
    "closed_at": "2017-09-22T13:36:38Z",
    "created_at": "2014-03-24T17:52:46Z",
    "labels": [
        "component: packages: standard",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "gengtype-lex.c:1:21: fatal error: bconfig.h: No such file or directory",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/16006",
    "user": "https://github.com/vbraun"
}
```
There is a race in GCC parallel build. Failed at least once on iras with

```
../../src/gcc/gengtype-lex.c:1:21: fatal error: bconfig.h: No such file or directory
compilation terminated.
make[6]: *** [gengtype-lex.o] Error 1
```
Upstream bug: http://gcc.gnu.org/bugzilla/show_bug.cgi?id=57125

**CC:**  @jdemeyer

**Resolution:** invalid

Issue created by migration from https://trac.sagemath.org/ticket/16006





---

archive/issue_events_053646.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16006",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16006#event-53646"
}
```



---

archive/issue_events_053647.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16006",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16006#event-53647"
}
```



---

archive/issue_events_053648.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16006",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16006#event-53648"
}
```



---

archive/issue_events_053649.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-09-22T13:36:38Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16006",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16006#event-53649"
}
```



---

archive/issue_events_053650.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-09-22T13:36:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16006",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16006#event-53650"
}
```



---

archive/issue_comments_264305.json:
```json
{
    "body": "<a id='comment:3'></a>\nI'm assuming that this bug is obsolete.",
    "created_at": "2017-09-22T13:36:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16006#issuecomment-264305",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:3'></a>
I'm assuming that this bug is obsolete.



---

archive/issue_comments_264306.json:
```json
{
    "body": "**Resolution:** invalid",
    "created_at": "2017-09-22T13:36:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16006#issuecomment-264306",
    "user": "https://github.com/jdemeyer"
}
```

**Resolution:** invalid



---

archive/issue_events_053651.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-09-22T13:36:38Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/16006",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16006#event-53651"
}
```
