# Issue 16171: Sort cone facets

archive/issues_015934.json:
```json
{
    "assignees": [],
    "body": "Cones already make sure that 1-d faces are sorted in the same way as rays. Let's also make facets listed in the same order as facet normals: the first of these lists is stored as is and the second one is sorted, no matter in which order they are computed (facets can be determined from the ambient structure without normals). The corner case when facets are rays is handled as well by sorting facet normals immediately.\n\nDepends on #16180\n\nCC:  @vbraun @sagetrac-jkeitel\n\nKeywords: **toric**\n\nBranch/Commit: **[9688ff1](https://github.com/sagemath/sagetrac-mirror/commit/9688ff146df01e1838f3734e193ade5531bc0886)**\n\nReviewer: **Jan Keitel**\n\nAuthor: **Andrey Novoseltsev**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16171_\n\n",
    "closed_at": "2014-07-25T22:19:02Z",
    "created_at": "2014-04-15T22:18:55Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Sort cone facets",
    "type": "issue",
    "updated_at": "2014-07-25T22:19:02Z",
    "url": "https://github.com/sagemath/sage/issues/16171",
    "user": "https://github.com/novoselt"
}
```
Cones already make sure that 1-d faces are sorted in the same way as rays. Let's also make facets listed in the same order as facet normals: the first of these lists is stored as is and the second one is sorted, no matter in which order they are computed (facets can be determined from the ambient structure without normals). The corner case when facets are rays is handled as well by sorting facet normals immediately.

Depends on #16180

CC:  @vbraun @sagetrac-jkeitel

Keywords: **toric**

Branch/Commit: **[9688ff1](https://github.com/sagemath/sagetrac-mirror/commit/9688ff146df01e1838f3734e193ade5531bc0886)**

Reviewer: **Jan Keitel**

Author: **Andrey Novoseltsev**

_Issue created by migration from https://trac.sagemath.org/ticket/16171_





---

archive/issue_events_213335.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2014-04-15T22:18:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16171#event-213335"
}
```



---

archive/issue_events_213336.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2014-04-15T22:18:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16171#event-213336"
}
```



---

archive/issue_comments_209089.json:
```json
{
    "body": "Branch: **[u/novoselt/sort_cone_facets](https://github.com/sagemath/sagetrac-mirror/tree/u/novoselt/sort_cone_facets)**",
    "created_at": "2014-04-16T00:06:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16171#issuecomment-209089",
    "user": "https://github.com/novoselt"
}
```

Branch: **[u/novoselt/sort_cone_facets](https://github.com/sagemath/sagetrac-mirror/tree/u/novoselt/sort_cone_facets)**



---

archive/issue_comments_209090.json:
```json
{
    "body": "Changed keywords from none to **toric**",
    "created_at": "2014-04-16T00:22:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16171#issuecomment-209090",
    "user": "https://github.com/novoselt"
}
```

Changed keywords from none to **toric**



---

archive/issue_comments_209091.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5cc237f55cc7533c176dacf179a8dcffbf0ed74b\">5cc237f</a></td><td><code>Keep the order of facets and their normals consistent for cones.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/01a6311e62e469b3cabc9419486363309da4f6a4\">01a6311</a></td><td><code>Fix doctests.</code></td></tr></table>\n",
    "created_at": "2014-04-16T00:22:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16171#issuecomment-209091",
    "user": "https://github.com/novoselt"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5cc237f55cc7533c176dacf179a8dcffbf0ed74b">5cc237f</a></td><td><code>Keep the order of facets and their normals consistent for cones.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/01a6311e62e469b3cabc9419486363309da4f6a4">01a6311</a></td><td><code>Fix doctests.</code></td></tr></table>




---

archive/issue_comments_209092.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-\n+Cones already make sure that 1-d faces are sorted in the same way as rays. Let's also make facets listed in the same order as facet normals: the first of these lists is stored as is and the second one is sorted, no matter in which order they are computed (facets can be determined from the ambient structure without normals). The corner case when facets are rays is handled as well by sorting facet normals immediately.\n``````\n",
    "created_at": "2014-04-16T00:22:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16171#issuecomment-209092",
    "user": "https://github.com/novoselt"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-
+Cones already make sure that 1-d faces are sorted in the same way as rays. Let's also make facets listed in the same order as facet normals: the first of these lists is stored as is and the second one is sorted, no matter in which order they are computed (facets can be determined from the ambient structure without normals). The corner case when facets are rays is handled as well by sorting facet normals immediately.
``````




---

archive/issue_comments_209093.json:
```json
{
    "body": "Author: **Andrey Novoseltsev**",
    "created_at": "2014-04-16T00:22:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16171#issuecomment-209093",
    "user": "https://github.com/novoselt"
}
```

Author: **Andrey Novoseltsev**



---

archive/issue_events_213337.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2014-04-16T00:22:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16171#event-213337"
}
```



---

archive/issue_events_213338.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2014-04-16T00:22:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16171#event-213338"
}
```



---

archive/issue_events_213339.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2014-04-16T00:22:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16171#event-213339"
}
```



---

archive/issue_events_213340.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2014-04-16T00:22:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16171#event-213340"
}
```



---

archive/issue_events_213341.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2014-04-16T00:22:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "component: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16171#event-213341"
}
```



---

archive/issue_comments_209094.json:
```json
{
    "body": "Commit: **[01a6311](https://github.com/sagemath/sagetrac-mirror/commit/01a6311e62e469b3cabc9419486363309da4f6a4)**",
    "created_at": "2014-04-16T00:22:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16171#issuecomment-209094",
    "user": "https://github.com/novoselt"
}
```

Commit: **[01a6311](https://github.com/sagemath/sagetrac-mirror/commit/01a6311e62e469b3cabc9419486363309da4f6a4)**



---

archive/issue_events_213342.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16171#event-213342"
}
```



---

archive/issue_events_213343.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16171#event-213343"
}
```



---

archive/issue_comments_209095.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nHi Jan - in case you have nothing else to do, there are a few toric-related tickets ready for review ;-)",
    "created_at": "2014-06-21T19:13:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16171#issuecomment-209095",
    "user": "https://github.com/novoselt"
}
```

<div id="comment:4" align="right">Comment 4</div>

Hi Jan - in case you have nothing else to do, there are a few toric-related tickets ready for review ;-)



---

archive/issue_comments_209096.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nThe worldcup has started! ;-) No, on a more serious note: I'm busy for the next two weeks, but I promise to take a look after that.",
    "created_at": "2014-06-21T22:55:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16171#issuecomment-209096",
    "user": "https://github.com/sagetrac-jkeitel"
}
```

<div id="comment:5" align="right">Comment 5</div>

The worldcup has started! ;-) No, on a more serious note: I'm busy for the next two weeks, but I promise to take a look after that.



---

archive/issue_comments_209097.json:
```json
{
    "body": "Reviewer: **Jan Keitel**",
    "created_at": "2014-07-24T12:03:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16171#issuecomment-209097",
    "user": "https://github.com/sagetrac-jkeitel"
}
```

Reviewer: **Jan Keitel**



---

archive/issue_events_213344.json:
```json
{
    "actor": "https://github.com/sagetrac-jkeitel",
    "created_at": "2014-07-24T12:03:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16171#event-213344"
}
```



---

archive/issue_events_213345.json:
```json
{
    "actor": "https://github.com/sagetrac-jkeitel",
    "created_at": "2014-07-24T12:03:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16171#event-213345"
}
```



---

archive/issue_comments_209098.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nI'm not entirely sure what this is needed for, but at least it makes things more consistent. Since the code looks good and works, I'm putting this to positive review.",
    "created_at": "2014-07-24T12:03:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16171#issuecomment-209098",
    "user": "https://github.com/sagetrac-jkeitel"
}
```

<div id="comment:6" align="right">Comment 6</div>

I'm not entirely sure what this is needed for, but at least it makes things more consistent. Since the code looks good and works, I'm putting this to positive review.



---

archive/issue_comments_209099.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nThat's exactly what this is needed for - consistency. My plan is to rewrite faces of `LatticePolytope` in the same fashion as it is done for cones and while I was looking at details I decided to change things here first. Thanks for taking a look!",
    "created_at": "2014-07-24T15:41:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16171#issuecomment-209099",
    "user": "https://github.com/novoselt"
}
```

<div id="comment:7" align="right">Comment 7</div>

That's exactly what this is needed for - consistency. My plan is to rewrite faces of `LatticePolytope` in the same fashion as it is done for cones and while I was looking at details I decided to change things here first. Thanks for taking a look!



---

archive/issue_events_213346.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-07-25T02:35:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16171#event-213346"
}
```



---

archive/issue_events_213347.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-07-25T02:35:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16171#event-213347"
}
```



---

archive/issue_comments_209100.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\n\n```\nsage -t --long src/sage/geometry/fan.py\n**********************************************************************\nFile \"src/sage/geometry/fan.py\", line 1716, in sage.geometry.fan.?._subdivide_stellar\nFailed example:\n    [cone.ambient_ray_indices() for cone in F]\nExpected:\n    [(0, 1, 3), (0, 2, 3)]\nGot:\n    [(0, 2, 3), (0, 1, 3)]\n**********************************************************************\n1 item had failures:\n   1 of  13 in sage.geometry.fan.?._subdivide_stellar\n```",
    "created_at": "2014-07-25T02:35:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16171#issuecomment-209100",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:8" align="right">Comment 8</div>


```
sage -t --long src/sage/geometry/fan.py
**********************************************************************
File "src/sage/geometry/fan.py", line 1716, in sage.geometry.fan.?._subdivide_stellar
Failed example:
    [cone.ambient_ray_indices() for cone in F]
Expected:
    [(0, 1, 3), (0, 2, 3)]
Got:
    [(0, 2, 3), (0, 1, 3)]
**********************************************************************
1 item had failures:
   1 of  13 in sage.geometry.fan.?._subdivide_stellar
```



---

archive/issue_events_213348.json:
```json
{
    "actor": "https://github.com/sagetrac-jkeitel",
    "created_at": "2014-07-25T07:27:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16171#event-213348"
}
```



---

archive/issue_events_213349.json:
```json
{
    "actor": "https://github.com/sagetrac-jkeitel",
    "created_at": "2014-07-25T07:27:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16171#event-213349"
}
```



---

archive/issue_comments_209101.json:
```json
{
    "body": "Changed branch from **[u/novoselt/sort_cone_facets](https://github.com/sagemath/sagetrac-mirror/tree/u/novoselt/sort_cone_facets)** to **[u/jkeitel/sort_cone_facets](https://github.com/sagemath/sagetrac-mirror/tree/u/jkeitel/sort_cone_facets)**",
    "created_at": "2014-07-25T07:27:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16171#issuecomment-209101",
    "user": "https://github.com/sagetrac-jkeitel"
}
```

Changed branch from **[u/novoselt/sort_cone_facets](https://github.com/sagemath/sagetrac-mirror/tree/u/novoselt/sort_cone_facets)** to **[u/jkeitel/sort_cone_facets](https://github.com/sagemath/sagetrac-mirror/tree/u/jkeitel/sort_cone_facets)**



---

archive/issue_comments_209102.json:
```json
{
    "body": "Dependencies: **#16180**",
    "created_at": "2014-07-25T07:27:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16171#issuecomment-209102",
    "user": "https://github.com/sagetrac-jkeitel"
}
```

Dependencies: **#16180**



---

archive/issue_comments_209103.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nSorry Volker, that one obviously came from #16180. I've merged in #16180 and fixed the doctest.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a6929591fdf4f797d7c89a79ba8723d82a58e380\">a692959</a></td><td><code>Use PPL for fan subdivision.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/17cb30b4b18f4422fc976039894bc7568782220d\">17cb30b</a></td><td><code>Fix doctests due to different facet ordering in PPL and PALP.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/dc35d9d1dd0d10678213344f59e38481f934c338\">dc35d9d</a></td><td><code>Deprecate Cone.lattice_polytope() and drop cache for Cone.polyhedron().</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/bd66bcd54e5a934547cae5f7e848612796a29c86\">bd66bcd</a></td><td><code>Merge branch 'develop' into u/novoselt/subdivide_fans_using_ppl</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f7f0dd54706d14d4d909715903581604ad133dc9\">f7f0dd5</a></td><td><code>Ticket 16180: Two minor formatting changes.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/59c8c9475223e98aeb604a79d22427605f20b5fb\">59c8c94</a></td><td><code>Change two Trac occurences to proper syntax.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5f542e07c8a0741d0209167047a9e0bb372c0335\">5f542e0</a></td><td><code>Merge branch 'develop' into u/novoselt/sort_cone_facets</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d676f9a4f58c49b6d3617757df352d89c21bafe6\">d676f9a</a></td><td><code>Merge branch 'u/novoselt/sort_cone_facets' into sort_cone_facets</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9688ff146df01e1838f3734e193ade5531bc0886\">9688ff1</a></td><td><code>Fix doctest that failed because of sorted facets.</code></td></tr></table>\n",
    "created_at": "2014-07-25T07:27:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16171#issuecomment-209103",
    "user": "https://github.com/sagetrac-jkeitel"
}
```

<div id="comment:9" align="right">Comment 9</div>

Sorry Volker, that one obviously came from #16180. I've merged in #16180 and fixed the doctest.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a6929591fdf4f797d7c89a79ba8723d82a58e380">a692959</a></td><td><code>Use PPL for fan subdivision.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/17cb30b4b18f4422fc976039894bc7568782220d">17cb30b</a></td><td><code>Fix doctests due to different facet ordering in PPL and PALP.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/dc35d9d1dd0d10678213344f59e38481f934c338">dc35d9d</a></td><td><code>Deprecate Cone.lattice_polytope() and drop cache for Cone.polyhedron().</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/bd66bcd54e5a934547cae5f7e848612796a29c86">bd66bcd</a></td><td><code>Merge branch 'develop' into u/novoselt/subdivide_fans_using_ppl</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f7f0dd54706d14d4d909715903581604ad133dc9">f7f0dd5</a></td><td><code>Ticket 16180: Two minor formatting changes.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/59c8c9475223e98aeb604a79d22427605f20b5fb">59c8c94</a></td><td><code>Change two Trac occurences to proper syntax.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5f542e07c8a0741d0209167047a9e0bb372c0335">5f542e0</a></td><td><code>Merge branch 'develop' into u/novoselt/sort_cone_facets</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d676f9a4f58c49b6d3617757df352d89c21bafe6">d676f9a</a></td><td><code>Merge branch 'u/novoselt/sort_cone_facets' into sort_cone_facets</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9688ff146df01e1838f3734e193ade5531bc0886">9688ff1</a></td><td><code>Fix doctest that failed because of sorted facets.</code></td></tr></table>




---

archive/issue_comments_209104.json:
```json
{
    "body": "Changed commit from **[01a6311](https://github.com/sagemath/sagetrac-mirror/commit/01a6311e62e469b3cabc9419486363309da4f6a4)** to **[9688ff1](https://github.com/sagemath/sagetrac-mirror/commit/9688ff146df01e1838f3734e193ade5531bc0886)**",
    "created_at": "2014-07-25T07:27:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16171#issuecomment-209104",
    "user": "https://github.com/sagetrac-jkeitel"
}
```

Changed commit from **[01a6311](https://github.com/sagemath/sagetrac-mirror/commit/01a6311e62e469b3cabc9419486363309da4f6a4)** to **[9688ff1](https://github.com/sagemath/sagetrac-mirror/commit/9688ff146df01e1838f3734e193ade5531bc0886)**



---

archive/issue_events_213350.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-07-25T22:19:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16171#event-213350"
}
```



---

archive/issue_events_213351.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "5cfa3a492ee2af74aba13819328db35318ae8704",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-07-25T22:19:02Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16171#event-213351"
}
```



---

archive/issue_comments_209105.json:
```json
{
    "body": "Changed branch from **[u/jkeitel/sort_cone_facets](https://github.com/sagemath/sagetrac-mirror/tree/u/jkeitel/sort_cone_facets)** to **[9688ff1](https://github.com/sagemath/sagetrac-mirror/commit/9688ff146df01e1838f3734e193ade5531bc0886)**",
    "created_at": "2014-07-25T22:19:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16171",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16171#issuecomment-209105",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jkeitel/sort_cone_facets](https://github.com/sagemath/sagetrac-mirror/tree/u/jkeitel/sort_cone_facets)** to **[9688ff1](https://github.com/sagemath/sagetrac-mirror/commit/9688ff146df01e1838f3734e193ade5531bc0886)**
