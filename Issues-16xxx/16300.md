# Issue 16300: upgrade setuptools to 2.2b1

archive/issues_016063.json:
```json
{
    "assignees": [],
    "body": "This is critical for patchbot runs.\n\nFrequently patchbot runs fail, see https://groups.google.com/forum/?hl=en#!topic/sage-devel/7acENsAQeXo . It is probable this was introduced with setuptools-2.0.1 because only those runs based on versions >6.1.1 show this behaviour. Also first tests with 2.2b1 are encouraging, so it seems right to open this upgrade ticket.\n\n\nKeywords: **setuptools**\n\nBranch/Commit: **[77d1e8f](https://github.com/sagemath/sagetrac-mirror/commit/77d1e8f241e77613dbbdc2f2d354ff9cdfb320fb)**\n\nReviewer: **Peter Bruin**\n\nAuthor: **Ralf Stephan**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16300_\n\n",
    "closed_at": "2014-05-12T09:48:16Z",
    "created_at": "2014-05-07T08:58:21Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "upgrade setuptools to 2.2b1",
    "type": "issue",
    "updated_at": "2014-05-12T09:48:16Z",
    "url": "https://github.com/sagemath/sage/issues/16300",
    "user": "https://github.com/rwst"
}
```
This is critical for patchbot runs.

Frequently patchbot runs fail, see https://groups.google.com/forum/?hl=en#!topic/sage-devel/7acENsAQeXo . It is probable this was introduced with setuptools-2.0.1 because only those runs based on versions >6.1.1 show this behaviour. Also first tests with 2.2b1 are encouraging, so it seems right to open this upgrade ticket.


Keywords: **setuptools**

Branch/Commit: **[77d1e8f](https://github.com/sagemath/sagetrac-mirror/commit/77d1e8f241e77613dbbdc2f2d354ff9cdfb320fb)**

Reviewer: **Peter Bruin**

Author: **Ralf Stephan**

_Issue created by migration from https://trac.sagemath.org/ticket/16300_





---

archive/issue_events_207576.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-05-07T08:58:21Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16300",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16300#event-207576"
}
```



---

archive/issue_events_207577.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-05-07T08:58:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16300",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16300#event-207577"
}
```



---

archive/issue_events_207578.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-05-07T08:58:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16300",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16300#event-207578"
}
```



---

archive/issue_events_207579.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-05-07T08:58:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16300",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16300#event-207579"
}
```



---

archive/issue_comments_211874.json:
```json
{
    "body": "Branch: **[u/rws/upgrade_setuptools_to_2_2b1](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/upgrade_setuptools_to_2_2b1)**",
    "created_at": "2014-05-07T14:37:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16300",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16300#issuecomment-211874",
    "user": "https://github.com/rwst"
}
```

Branch: **[u/rws/upgrade_setuptools_to_2_2b1](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/upgrade_setuptools_to_2_2b1)**



---

archive/issue_comments_211875.json:
```json
{
    "body": "Attachment: **[setuptools-2.2b1.tar.gz](https://github.com/sagemath/sage/files/ticket16300/setuptools-2.2b1.tar.gz)**",
    "created_at": "2014-05-07T14:37:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16300",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16300#issuecomment-211875",
    "user": "https://github.com/rwst"
}
```

Attachment: **[setuptools-2.2b1.tar.gz](https://github.com/sagemath/sage/files/ticket16300/setuptools-2.2b1.tar.gz)**



---

archive/issue_comments_211876.json:
```json
{
    "body": "Commit: **[77d1e8f](https://github.com/sagemath/sagetrac-mirror/commit/77d1e8f241e77613dbbdc2f2d354ff9cdfb320fb)**",
    "created_at": "2014-05-07T14:38:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16300",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16300#issuecomment-211876",
    "user": "https://github.com/rwst"
}
```

Commit: **[77d1e8f](https://github.com/sagemath/sagetrac-mirror/commit/77d1e8f241e77613dbbdc2f2d354ff9cdfb320fb)**



---

archive/issue_comments_211877.json:
```json
{
    "body": "Author: **Ralf Stephan**",
    "created_at": "2014-05-07T14:38:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16300",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16300#issuecomment-211877",
    "user": "https://github.com/rwst"
}
```

Author: **Ralf Stephan**



---

archive/issue_comments_211878.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/77d1e8f241e77613dbbdc2f2d354ff9cdfb320fb\">77d1e8f</a></td><td><code>use setuptools-2.2b1</code></td></tr></table>\n",
    "created_at": "2014-05-07T14:38:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16300",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16300#issuecomment-211878",
    "user": "https://github.com/rwst"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/77d1e8f241e77613dbbdc2f2d354ff9cdfb320fb">77d1e8f</a></td><td><code>use setuptools-2.2b1</code></td></tr></table>




---

archive/issue_events_207580.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-05-07T14:38:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16300",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16300#event-207580"
}
```



---

archive/issue_comments_211879.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nThis upgrade indeed seems to solve the problem; my patchbot failed more than half of the time before and not at all after making the same changes as in this patch.",
    "created_at": "2014-05-07T14:59:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16300",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16300#issuecomment-211879",
    "user": "https://github.com/pjbruin"
}
```

<a id='comment:3'>Comment 3:</a>
This upgrade indeed seems to solve the problem; my patchbot failed more than half of the time before and not at all after making the same changes as in this patch.



---

archive/issue_events_207581.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2014-05-07T14:59:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16300",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16300#event-207581"
}
```



---

archive/issue_events_207582.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2014-05-07T14:59:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16300",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16300#event-207582"
}
```



---

archive/issue_comments_211880.json:
```json
{
    "body": "Reviewer: **Peter Bruin**",
    "created_at": "2014-05-07T14:59:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16300",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16300#issuecomment-211880",
    "user": "https://github.com/pjbruin"
}
```

Reviewer: **Peter Bruin**



---

archive/issue_comments_211881.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nPlease don't attach tarballs in the future, we'd rather not store binary blobs in the trac database forever.",
    "created_at": "2014-05-12T09:20:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16300",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16300#issuecomment-211881",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:5'>Comment 5:</a>
Please don't attach tarballs in the future, we'd rather not store binary blobs in the trac database forever.



---

archive/issue_events_207583.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-05-12T09:48:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16300",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16300#event-207583"
}
```



---

archive/issue_events_207584.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "d54a9912e19aae0908cdd06c85e7bd62c1cdb478",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-05-12T09:48:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16300",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16300#event-207584"
}
```



---

archive/issue_comments_211882.json:
```json
{
    "body": "Changed branch from **[u/rws/upgrade_setuptools_to_2_2b1](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/upgrade_setuptools_to_2_2b1)** to **[77d1e8f](https://github.com/sagemath/sagetrac-mirror/commit/77d1e8f241e77613dbbdc2f2d354ff9cdfb320fb)**",
    "created_at": "2014-05-12T09:48:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16300",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16300#issuecomment-211882",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/rws/upgrade_setuptools_to_2_2b1](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/upgrade_setuptools_to_2_2b1)** to **[77d1e8f](https://github.com/sagemath/sagetrac-mirror/commit/77d1e8f241e77613dbbdc2f2d354ff9cdfb320fb)**
