# Issue 16823: Implement the free Lie algebra

archive/issues_016586.json:
```json
{
    "assignees": [],
    "body": "Part of #14901. Implements the free Lie algebra in two different bases.\n\nAssignee: **@tscrim**\n\nCC:  sd67 @bsalisbury1\n\nKeywords: **lie algebras**\n\nBranch/Commit: **[e27b98b](https://github.com/sagemath/sagetrac-mirror/commit/e27b98b282a0c86945fc155519169b77fa0b2d41)**\n\nReviewer: **Darij Grinberg**\n\nAuthor: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16823_\n\n",
    "closed_at": "2018-05-09T09:50:01Z",
    "created_at": "2014-08-14T10:53:56Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Implement the free Lie algebra",
    "type": "issue",
    "updated_at": "2018-05-09T09:50:01Z",
    "url": "https://github.com/sagemath/sage/issues/16823",
    "user": "https://github.com/tscrim"
}
```
Part of #14901. Implements the free Lie algebra in two different bases.

Assignee: **@tscrim**

CC:  sd67 @bsalisbury1

Keywords: **lie algebras**

Branch/Commit: **[e27b98b](https://github.com/sagemath/sagetrac-mirror/commit/e27b98b282a0c86945fc155519169b77fa0b2d41)**

Reviewer: **Darij Grinberg**

Author: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/16823_





---

archive/issue_events_214938.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-08-14T10:53:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214938"
}
```



---

archive/issue_events_214939.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-08-14T10:53:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214939"
}
```



---

archive/issue_events_214940.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-08-14T10:53:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214940"
}
```



---

archive/issue_events_214941.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-08-14T10:53:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214941"
}
```



---

archive/issue_comments_223607.json:
```json
{
    "body": "Commit: **[ef1741e](https://github.com/sagemath/sagetrac-mirror/commit/ef1741e5cc8d72692fcafcaf947be08ff6adce67)**",
    "created_at": "2015-04-14T15:18:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223607",
    "user": "https://github.com/tscrim"
}
```

Commit: **[ef1741e](https://github.com/sagemath/sagetrac-mirror/commit/ef1741e5cc8d72692fcafcaf947be08ff6adce67)**



---

archive/issue_comments_223608.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nLast 10 new commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e95d67cbdda81ca0dc127bb10c1175fb0535dfa4\">e95d67c</a></td><td><code>More fixes to fin-dim w/ basis category.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c800d6c26c79c9b0af3db68dc5edf656d2f21c24\">c800d6c</a></td><td><code>Merge branch 'public/categories/lie_algebras-16819' into public/lie_algebras/fd_structure_coeff-16820</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a491bb07db52cc51a790dda75de9cccbe8746bfa\">a491bb0</a></td><td><code>Some more cleanup and fixes.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/aabf5405d5472a0fad8782e506e96e382596f96f\">aabf540</a></td><td><code>Fixing coefficient division.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/12074e86295e2848a520c5962678c1c9e75b7505\">12074e8</a></td><td><code>Merge branch 'public/categories/lie_algebras-16819' into public/lie_algebras/fd_structure_coeff-16820</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7b50b7c63f66845a0825010d1ae31aaa7bd6a809\">7b50b7c</a></td><td><code>Merge branch 'public/lie_algebras/fd_structure_coeff-16820' of trac.sagemath.org:sage into public/lie_algebras/fd_structure_coeff-16820</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a9c57e2c9db90c41a9d5f872026351fcf4bc5f45\">a9c57e2</a></td><td><code>Merge branch 'public/lie_algebras/fd_structure_coeff-16820' into public/lie_algebras/free-16823</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b35064d63f6390ec950c65c78aaa6f6de66b372a\">b35064d</a></td><td><code>Putting out fires from refactoring of ABC's.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1c21f1be1631b8b1d1b1ad9cdf26c281784f3daa\">1c21f1b</a></td><td><code>Adding doctests to most methods.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ef1741e5cc8d72692fcafcaf947be08ff6adce67\">ef1741e</a></td><td><code>Finishing adding doctests and getting bugs out.</code></td></tr></table>\n",
    "created_at": "2015-04-14T15:18:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223608",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:1'>Comment 1:</a>
Last 10 new commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e95d67cbdda81ca0dc127bb10c1175fb0535dfa4">e95d67c</a></td><td><code>More fixes to fin-dim w/ basis category.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c800d6c26c79c9b0af3db68dc5edf656d2f21c24">c800d6c</a></td><td><code>Merge branch 'public/categories/lie_algebras-16819' into public/lie_algebras/fd_structure_coeff-16820</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a491bb07db52cc51a790dda75de9cccbe8746bfa">a491bb0</a></td><td><code>Some more cleanup and fixes.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/aabf5405d5472a0fad8782e506e96e382596f96f">aabf540</a></td><td><code>Fixing coefficient division.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/12074e86295e2848a520c5962678c1c9e75b7505">12074e8</a></td><td><code>Merge branch 'public/categories/lie_algebras-16819' into public/lie_algebras/fd_structure_coeff-16820</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7b50b7c63f66845a0825010d1ae31aaa7bd6a809">7b50b7c</a></td><td><code>Merge branch 'public/lie_algebras/fd_structure_coeff-16820' of trac.sagemath.org:sage into public/lie_algebras/fd_structure_coeff-16820</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a9c57e2c9db90c41a9d5f872026351fcf4bc5f45">a9c57e2</a></td><td><code>Merge branch 'public/lie_algebras/fd_structure_coeff-16820' into public/lie_algebras/free-16823</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b35064d63f6390ec950c65c78aaa6f6de66b372a">b35064d</a></td><td><code>Putting out fires from refactoring of ABC's.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1c21f1be1631b8b1d1b1ad9cdf26c281784f3daa">1c21f1b</a></td><td><code>Adding doctests to most methods.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ef1741e5cc8d72692fcafcaf947be08ff6adce67">ef1741e</a></td><td><code>Finishing adding doctests and getting bugs out.</code></td></tr></table>




---

archive/issue_comments_223609.json:
```json
{
    "body": "Branch: **[public/lie_algebras/free-16823](https://github.com/sagemath/sagetrac-mirror/tree/public/lie_algebras/free-16823)**",
    "created_at": "2015-04-14T15:18:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223609",
    "user": "https://github.com/tscrim"
}
```

Branch: **[public/lie_algebras/free-16823](https://github.com/sagemath/sagetrac-mirror/tree/public/lie_algebras/free-16823)**



---

archive/issue_comments_223610.json:
```json
{
    "body": "Changed dependencies from **#16819** to **#16820**",
    "created_at": "2015-04-14T15:18:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223610",
    "user": "https://github.com/tscrim"
}
```

Changed dependencies from **#16819** to **#16820**



---

archive/issue_events_214942.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-04-14T15:18:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214942"
}
```



---

archive/issue_comments_223611.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9d0e2114af5cd39c93387d7671e4f537d7342e6b\">9d0e211</a></td><td><code>fixed merge conflicts</code></td></tr></table>\n",
    "created_at": "2015-10-11T19:18:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223611",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:2'>Comment 2:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9d0e2114af5cd39c93387d7671e4f537d7342e6b">9d0e211</a></td><td><code>fixed merge conflicts</code></td></tr></table>




---

archive/issue_comments_223612.json:
```json
{
    "body": "Changed commit from **[ef1741e](https://github.com/sagemath/sagetrac-mirror/commit/ef1741e5cc8d72692fcafcaf947be08ff6adce67)** to **[9d0e211](https://github.com/sagemath/sagetrac-mirror/commit/9d0e2114af5cd39c93387d7671e4f537d7342e6b)**",
    "created_at": "2015-10-11T19:18:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223612",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[ef1741e](https://github.com/sagemath/sagetrac-mirror/commit/ef1741e5cc8d72692fcafcaf947be08ff6adce67)** to **[9d0e211](https://github.com/sagemath/sagetrac-mirror/commit/9d0e2114af5cd39c93387d7671e4f537d7342e6b)**



---

archive/issue_events_214943.json:
```json
{
    "actor": "https://github.com/kevindilks",
    "created_at": "2015-10-11T19:21:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214943"
}
```



---

archive/issue_events_214944.json:
```json
{
    "actor": "https://github.com/kevindilks",
    "created_at": "2015-10-11T19:21:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "milestone_number": null,
    "milestone_title": "sage-6.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214944"
}
```



---

archive/issue_comments_223613.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9d0e2114af5cd39c93387d7671e4f537d7342e6b\">9d0e211</a></td><td><code>fixed merge conflicts</code></td></tr></table>\n",
    "created_at": "2015-10-11T19:21:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223613",
    "user": "https://github.com/kevindilks"
}
```

<a id='comment:3'>Comment 3:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9d0e2114af5cd39c93387d7671e4f537d7342e6b">9d0e211</a></td><td><code>fixed merge conflicts</code></td></tr></table>




---

archive/issue_comments_223614.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nBranch pushed to git repo; I updated commit sha1. Last 10 new commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6815f1c02c6a13cef96d542328d3248e06ca9fb9\">6815f1c</a></td><td><code>review heisenberg.py again; coercion still relies on unstaged changes to module morphisms</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/573e0748daeb844cb79792c08a5c851ea047e7f1\">573e074</a></td><td><code>Merge branch 'public/lie_algebras/fd_structure_coeff-16820' of trac.sagemath.org:sage into public/lie_algebras/fd_structure_coeff-16820</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3f9a4a19a30d6749ab046160bc43988958c9490f\">3f9a4a1</a></td><td><code>Fixing the failing doctests and making the doc build.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b9930e6c4f3f099baf341b58dc81d50a00df4986\">b9930e6</a></td><td><code>some changes to standardization of names</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/60f743a2d8f8d684e4e636d6bdf329fae9967cb2\">60f743a</a></td><td><code>Parsing the input so that it uses the indexing set.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a02380fd7888a50f85218635f99bb92cab8426d4\">a02380f</a></td><td><code>Merge branch 'public/lie_algebras/fd_structure_coeff-16820' in 7.0.rc1</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/50f4441019eeb080c7e700e2853f9b6aa564cf7c\">50f4441</a></td><td><code>trac #16820 correct the spelling of Cartesian</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f1514bd21164a21c8ff90adf91bb036605fc1d85\">f1514bd</a></td><td><code>Merge branch 'public/lie_algebras/fd_structure_coeff-16820' into 7.3.b2</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7d155ecae295b68d22eee62f07438af2111641fb\">7d155ec</a></td><td><code>Fixing trivial doctest failures.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/29b3b23c4c8ff89c145abea50f2d4329bbc6a12b\">29b3b23</a></td><td><code>Merge branch 'public/lie_algebras/free-16823' of trac.sagemath.org:sage into public/lie_algebras/free-16823</code></td></tr></table>\n",
    "created_at": "2016-06-02T13:35:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223614",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:4'>Comment 4:</a>
Branch pushed to git repo; I updated commit sha1. Last 10 new commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6815f1c02c6a13cef96d542328d3248e06ca9fb9">6815f1c</a></td><td><code>review heisenberg.py again; coercion still relies on unstaged changes to module morphisms</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/573e0748daeb844cb79792c08a5c851ea047e7f1">573e074</a></td><td><code>Merge branch 'public/lie_algebras/fd_structure_coeff-16820' of trac.sagemath.org:sage into public/lie_algebras/fd_structure_coeff-16820</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3f9a4a19a30d6749ab046160bc43988958c9490f">3f9a4a1</a></td><td><code>Fixing the failing doctests and making the doc build.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b9930e6c4f3f099baf341b58dc81d50a00df4986">b9930e6</a></td><td><code>some changes to standardization of names</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/60f743a2d8f8d684e4e636d6bdf329fae9967cb2">60f743a</a></td><td><code>Parsing the input so that it uses the indexing set.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a02380fd7888a50f85218635f99bb92cab8426d4">a02380f</a></td><td><code>Merge branch 'public/lie_algebras/fd_structure_coeff-16820' in 7.0.rc1</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/50f4441019eeb080c7e700e2853f9b6aa564cf7c">50f4441</a></td><td><code>trac #16820 correct the spelling of Cartesian</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f1514bd21164a21c8ff90adf91bb036605fc1d85">f1514bd</a></td><td><code>Merge branch 'public/lie_algebras/fd_structure_coeff-16820' into 7.3.b2</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7d155ecae295b68d22eee62f07438af2111641fb">7d155ec</a></td><td><code>Fixing trivial doctest failures.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/29b3b23c4c8ff89c145abea50f2d4329bbc6a12b">29b3b23</a></td><td><code>Merge branch 'public/lie_algebras/free-16823' of trac.sagemath.org:sage into public/lie_algebras/free-16823</code></td></tr></table>




---

archive/issue_comments_223615.json:
```json
{
    "body": "Changed commit from **[9d0e211](https://github.com/sagemath/sagetrac-mirror/commit/9d0e2114af5cd39c93387d7671e4f537d7342e6b)** to **[29b3b23](https://github.com/sagemath/sagetrac-mirror/commit/29b3b23c4c8ff89c145abea50f2d4329bbc6a12b)**",
    "created_at": "2016-06-02T13:35:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223615",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[9d0e211](https://github.com/sagemath/sagetrac-mirror/commit/9d0e2114af5cd39c93387d7671e4f537d7342e6b)** to **[29b3b23](https://github.com/sagemath/sagetrac-mirror/commit/29b3b23c4c8ff89c145abea50f2d4329bbc6a12b)**



---

archive/issue_comments_223616.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nBranch pushed to git repo; I updated commit sha1. Last 10 new commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3e2c5478f9d81ffde9c219bb9e32407096c7f5df\">3e2c547</a></td><td><code>Merge branch 'public/lie_algebras/fd_structure_coeff-16820' of git://trac.sagemath.org/sage into public/lie_algebras/fd_structure_coeff-16820</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/cf88fee19593c1dc07b7e11a64a1b7cfa1c0f148\">cf88fee</a></td><td><code>Changes for the reviewer and using standard copyright.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/42666d4c3685d9ec2d9851ae9618d3a9e809449f\">42666d4</a></td><td><code>Adding centralizer_basis method, improving to/from_vector for finite-dim Lie algebras, other fixes.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/59df7ce401046c9bc1df6763d8ab80da594068a3\">59df7ce</a></td><td><code>Fixing pdf docbuild in finite-dim Lie algebras w/ basis category.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ed587af6769afaae0dcd0ec9edd990f38c347cc0\">ed587af</a></td><td><code>Implementing ideal checks and fixing product_space.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3baf33f76251fbec369c86a03db8eac488183c37\">3baf33f</a></td><td><code>Merge branch 'develop' into public/lie_algebras/fd_structure_coeff-16820</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c0f01df449f2e40c26d6e8002c42f08fec59e589\">c0f01df</a></td><td><code>Cache is_ideal and explaining doc what is clear from the code.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/fc395c8a0c03c608e2987b13dc0b8568e772046d\">fc395c8</a></td><td><code>Fixing typo.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/91f46886dcd55f0a757820768304c37dcd2a43a9\">91f4688</a></td><td><code>Merge branch 'public/lie_algebras/free-16823' of git://trac.sagemath.org/sage into public/lie_algebras/free-16823</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/772391b350972e63c34257e20ccb406526d2b0ba\">772391b</a></td><td><code>Improvements, fixes, and rebasing.</code></td></tr></table>\n",
    "created_at": "2017-04-01T23:36:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223616",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:5'>Comment 5:</a>
Branch pushed to git repo; I updated commit sha1. Last 10 new commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3e2c5478f9d81ffde9c219bb9e32407096c7f5df">3e2c547</a></td><td><code>Merge branch 'public/lie_algebras/fd_structure_coeff-16820' of git://trac.sagemath.org/sage into public/lie_algebras/fd_structure_coeff-16820</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/cf88fee19593c1dc07b7e11a64a1b7cfa1c0f148">cf88fee</a></td><td><code>Changes for the reviewer and using standard copyright.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/42666d4c3685d9ec2d9851ae9618d3a9e809449f">42666d4</a></td><td><code>Adding centralizer_basis method, improving to/from_vector for finite-dim Lie algebras, other fixes.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/59df7ce401046c9bc1df6763d8ab80da594068a3">59df7ce</a></td><td><code>Fixing pdf docbuild in finite-dim Lie algebras w/ basis category.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ed587af6769afaae0dcd0ec9edd990f38c347cc0">ed587af</a></td><td><code>Implementing ideal checks and fixing product_space.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3baf33f76251fbec369c86a03db8eac488183c37">3baf33f</a></td><td><code>Merge branch 'develop' into public/lie_algebras/fd_structure_coeff-16820</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c0f01df449f2e40c26d6e8002c42f08fec59e589">c0f01df</a></td><td><code>Cache is_ideal and explaining doc what is clear from the code.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/fc395c8a0c03c608e2987b13dc0b8568e772046d">fc395c8</a></td><td><code>Fixing typo.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/91f46886dcd55f0a757820768304c37dcd2a43a9">91f4688</a></td><td><code>Merge branch 'public/lie_algebras/free-16823' of git://trac.sagemath.org/sage into public/lie_algebras/free-16823</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/772391b350972e63c34257e20ccb406526d2b0ba">772391b</a></td><td><code>Improvements, fixes, and rebasing.</code></td></tr></table>




---

archive/issue_comments_223617.json:
```json
{
    "body": "Changed commit from **[29b3b23](https://github.com/sagemath/sagetrac-mirror/commit/29b3b23c4c8ff89c145abea50f2d4329bbc6a12b)** to **[772391b](https://github.com/sagemath/sagetrac-mirror/commit/772391b350972e63c34257e20ccb406526d2b0ba)**",
    "created_at": "2017-04-01T23:36:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223617",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[29b3b23](https://github.com/sagemath/sagetrac-mirror/commit/29b3b23c4c8ff89c145abea50f2d4329bbc6a12b)** to **[772391b](https://github.com/sagemath/sagetrac-mirror/commit/772391b350972e63c34257e20ccb406526d2b0ba)**



---

archive/issue_events_214945.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-04-01T23:37:34Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "milestone_number": null,
    "milestone_title": "sage-6.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214945"
}
```



---

archive/issue_events_214946.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-04-01T23:37:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214946"
}
```



---

archive/issue_comments_223618.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/23071804eae2c96c8bb186e24e326772a01b14b2\">2307180</a></td><td><code>Merge branch 'public/lie_algebras/free-16823' of git://trac.sagemath.org/sage into lie</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1297bd5f5006ca9c5edf0413e030a6d857d98b22\">1297bd5</a></td><td><code>doc fixes (deadlink for Munthe-Kaas in particular)</code></td></tr></table>\n",
    "created_at": "2017-04-13T21:41:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223618",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:7'>Comment 7:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/23071804eae2c96c8bb186e24e326772a01b14b2">2307180</a></td><td><code>Merge branch 'public/lie_algebras/free-16823' of git://trac.sagemath.org/sage into lie</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1297bd5f5006ca9c5edf0413e030a6d857d98b22">1297bd5</a></td><td><code>doc fixes (deadlink for Munthe-Kaas in particular)</code></td></tr></table>




---

archive/issue_comments_223619.json:
```json
{
    "body": "Changed commit from **[772391b](https://github.com/sagemath/sagetrac-mirror/commit/772391b350972e63c34257e20ccb406526d2b0ba)** to **[1297bd5](https://github.com/sagemath/sagetrac-mirror/commit/1297bd5f5006ca9c5edf0413e030a6d857d98b22)**",
    "created_at": "2017-04-13T21:41:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223619",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[772391b](https://github.com/sagemath/sagetrac-mirror/commit/772391b350972e63c34257e20ccb406526d2b0ba)** to **[1297bd5](https://github.com/sagemath/sagetrac-mirror/commit/1297bd5f5006ca9c5edf0413e030a6d857d98b22)**



---

archive/issue_comments_223620.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nTravis: What Hall basis is this? Are you following a specific source? There are many different Hall bases, and the simplest one (I believe using the lexicographic order) is just the Lyndon basis IIRC; hence the question.",
    "created_at": "2017-04-13T23:13:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223620",
    "user": "https://github.com/darijgr"
}
```

<a id='comment:8'>Comment 8:</a>
Travis: What Hall basis is this? Are you following a specific source? There are many different Hall bases, and the simplest one (I believe using the lexicographic order) is just the Lyndon basis IIRC; hence the question.



---

archive/issue_comments_223621.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9ec41ba84a72103188d6c7ff02dd26883f6401d9\">9ec41ba</a></td><td><code>more trivial fixes</code></td></tr></table>\n",
    "created_at": "2017-04-13T23:14:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223621",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:9'>Comment 9:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9ec41ba84a72103188d6c7ff02dd26883f6401d9">9ec41ba</a></td><td><code>more trivial fixes</code></td></tr></table>




---

archive/issue_comments_223622.json:
```json
{
    "body": "Changed commit from **[1297bd5](https://github.com/sagemath/sagetrac-mirror/commit/1297bd5f5006ca9c5edf0413e030a6d857d98b22)** to **[9ec41ba](https://github.com/sagemath/sagetrac-mirror/commit/9ec41ba84a72103188d6c7ff02dd26883f6401d9)**",
    "created_at": "2017-04-13T23:14:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223622",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[1297bd5](https://github.com/sagemath/sagetrac-mirror/commit/1297bd5f5006ca9c5edf0413e030a6d857d98b22)** to **[9ec41ba](https://github.com/sagemath/sagetrac-mirror/commit/9ec41ba84a72103188d6c7ff02dd26883f6401d9)**



---

archive/issue_comments_223623.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nIIRC, I followed [MKO1998], which I thought was based on Hall's original construction, but I can't seem to find that reference. I am not tied to the name, and another name might be better if someone wants to implement the other 2 \"natural\" Hall set based bases. It's up to you.\n\nAlso, we should get rid of this filter:\n\n```diff\n-cf = filter(lambda x: x != 0, c)\n+cf = [x for x in c if x != 0]\n```\nJust below that, we could also do:\n\n```diff\n-                nonzero_indices = []\n-                for i in range(len(c)):\n-                    if c[i] != 0:\n-                        nonzero_indices.append(i)\n+                nonzero_indices = [i for i,val in enumerate(c) if val != 0]\n```\nI can make these changes if you want.",
    "created_at": "2017-04-14T00:28:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223623",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:10'>Comment 10:</a>
IIRC, I followed [MKO1998], which I thought was based on Hall's original construction, but I can't seem to find that reference. I am not tied to the name, and another name might be better if someone wants to implement the other 2 "natural" Hall set based bases. It's up to you.

Also, we should get rid of this filter:

```diff
-cf = filter(lambda x: x != 0, c)
+cf = [x for x in c if x != 0]
```
Just below that, we could also do:

```diff
-                nonzero_indices = []
-                for i in range(len(c)):
-                    if c[i] != 0:
-                        nonzero_indices.append(i)
+                nonzero_indices = [i for i,val in enumerate(c) if val != 0]
```
I can make these changes if you want.



---

archive/issue_comments_223624.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nWould be great if you did them. I squinted with some unease at the filter as well, but I thought you had a rationale for using it :)",
    "created_at": "2017-04-14T00:53:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223624",
    "user": "https://github.com/darijgr"
}
```

<a id='comment:11'>Comment 11:</a>
Would be great if you did them. I squinted with some unease at the filter as well, but I thought you had a rationale for using it :)



---

archive/issue_comments_223625.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nOld code, back before I knew as much as I do now.",
    "created_at": "2017-04-14T00:54:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223625",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:12'>Comment 12:</a>
Old code, back before I knew as much as I do now.



---

archive/issue_comments_223626.json:
```json
{
    "body": "Changed commit from **[9ec41ba](https://github.com/sagemath/sagetrac-mirror/commit/9ec41ba84a72103188d6c7ff02dd26883f6401d9)** to **[8efccc0](https://github.com/sagemath/sagetrac-mirror/commit/8efccc018244b2f0b01a5f12af0c07f24ae3775c)**",
    "created_at": "2017-04-23T22:30:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223626",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[9ec41ba](https://github.com/sagemath/sagetrac-mirror/commit/9ec41ba84a72103188d6c7ff02dd26883f6401d9)** to **[8efccc0](https://github.com/sagemath/sagetrac-mirror/commit/8efccc018244b2f0b01a5f12af0c07f24ae3775c)**



---

archive/issue_comments_223627.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8efccc018244b2f0b01a5f12af0c07f24ae3775c\">8efccc0</a></td><td><code>Adding missing doctest and Python3 compatibility.</code></td></tr></table>\n",
    "created_at": "2017-04-23T22:30:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223627",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:13'>Comment 13:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8efccc018244b2f0b01a5f12af0c07f24ae3775c">8efccc0</a></td><td><code>Adding missing doctest and Python3 compatibility.</code></td></tr></table>




---

archive/issue_comments_223628.json:
```json
{
    "body": "Changed commit from **[8efccc0](https://github.com/sagemath/sagetrac-mirror/commit/8efccc018244b2f0b01a5f12af0c07f24ae3775c)** to **[ca5b637](https://github.com/sagemath/sagetrac-mirror/commit/ca5b6371bee902a2747953d15483a61cbcebf2db)**",
    "created_at": "2017-04-23T22:33:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223628",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[8efccc0](https://github.com/sagemath/sagetrac-mirror/commit/8efccc018244b2f0b01a5f12af0c07f24ae3775c)** to **[ca5b637](https://github.com/sagemath/sagetrac-mirror/commit/ca5b6371bee902a2747953d15483a61cbcebf2db)**



---

archive/issue_comments_223629.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ca5b6371bee902a2747953d15483a61cbcebf2db\">ca5b637</a></td><td><code>Some minor code improvements as in [comment:10](#comment%3A10).</code></td></tr></table>\n",
    "created_at": "2017-04-23T22:33:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223629",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:14'>Comment 14:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ca5b6371bee902a2747953d15483a61cbcebf2db">ca5b637</a></td><td><code>Some minor code improvements as in [comment:10](#comment%3A10).</code></td></tr></table>




---

archive/issue_comments_223630.json:
```json
{
    "body": "<a id='comment:15'>Comment 15:</a>\nThis makes the changes I noted in [comment:10](#comment%3A10), as well as making it Python3 compatible and full doctest coverage (I missed one hidden method).",
    "created_at": "2017-04-23T22:34:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223630",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:15'>Comment 15:</a>
This makes the changes I noted in [comment:10](#comment%3A10), as well as making it Python3 compatible and full doctest coverage (I missed one hidden method).



---

archive/issue_comments_223631.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9de28bf9b286ee502cd847f943e4e92c38a0d426\">9de28bf</a></td><td><code>Merge branch 'public/lie_algebras/free-16823' in 8.0.b9</code></td></tr></table>\n",
    "created_at": "2017-06-05T13:57:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223631",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:16'>Comment 16:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9de28bf9b286ee502cd847f943e4e92c38a0d426">9de28bf</a></td><td><code>Merge branch 'public/lie_algebras/free-16823' in 8.0.b9</code></td></tr></table>




---

archive/issue_comments_223632.json:
```json
{
    "body": "Changed commit from **[ca5b637](https://github.com/sagemath/sagetrac-mirror/commit/ca5b6371bee902a2747953d15483a61cbcebf2db)** to **[9de28bf](https://github.com/sagemath/sagetrac-mirror/commit/9de28bf9b286ee502cd847f943e4e92c38a0d426)**",
    "created_at": "2017-06-05T13:57:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223632",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[ca5b637](https://github.com/sagemath/sagetrac-mirror/commit/ca5b6371bee902a2747953d15483a61cbcebf2db)** to **[9de28bf](https://github.com/sagemath/sagetrac-mirror/commit/9de28bf9b286ee502cd847f943e4e92c38a0d426)**



---

archive/issue_comments_223633.json:
```json
{
    "body": "Changed commit from **[9de28bf](https://github.com/sagemath/sagetrac-mirror/commit/9de28bf9b286ee502cd847f943e4e92c38a0d426)** to **[c7b203d](https://github.com/sagemath/sagetrac-mirror/commit/c7b203d9e9558b3869604962d04878c02d37a2d0)**",
    "created_at": "2017-06-05T18:34:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223633",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[9de28bf](https://github.com/sagemath/sagetrac-mirror/commit/9de28bf9b286ee502cd847f943e4e92c38a0d426)** to **[c7b203d](https://github.com/sagemath/sagetrac-mirror/commit/c7b203d9e9558b3869604962d04878c02d37a2d0)**



---

archive/issue_comments_223634.json:
```json
{
    "body": "<a id='comment:17'>Comment 17:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c7b203d9e9558b3869604962d04878c02d37a2d0\">c7b203d</a></td><td><code>trac 16823 py3 iteritems</code></td></tr></table>\n",
    "created_at": "2017-06-05T18:34:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223634",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:17'>Comment 17:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c7b203d9e9558b3869604962d04878c02d37a2d0">c7b203d</a></td><td><code>trac 16823 py3 iteritems</code></td></tr></table>




---

archive/issue_comments_223635.json:
```json
{
    "body": "Changed dependencies from **#16820** to none",
    "created_at": "2017-06-05T18:35:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223635",
    "user": "https://github.com/fchapoton"
}
```

Changed dependencies from **#16820** to none



---

archive/issue_comments_223636.json:
```json
{
    "body": "<a id='comment:19'>Comment 19:</a>\ndoes not apply",
    "created_at": "2017-06-17T06:00:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223636",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:19'>Comment 19:</a>
does not apply



---

archive/issue_events_214947.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-06-17T06:00:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214947"
}
```



---

archive/issue_events_214948.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-06-17T06:00:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214948"
}
```



---

archive/issue_comments_223637.json:
```json
{
    "body": "Changed commit from **[c7b203d](https://github.com/sagemath/sagetrac-mirror/commit/c7b203d9e9558b3869604962d04878c02d37a2d0)** to **[394d58f](https://github.com/sagemath/sagetrac-mirror/commit/394d58f54c67a3737f31d4cb6095d251f8652899)**",
    "created_at": "2017-06-17T06:13:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223637",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[c7b203d](https://github.com/sagemath/sagetrac-mirror/commit/c7b203d9e9558b3869604962d04878c02d37a2d0)** to **[394d58f](https://github.com/sagemath/sagetrac-mirror/commit/394d58f54c67a3737f31d4cb6095d251f8652899)**



---

archive/issue_comments_223638.json:
```json
{
    "body": "<a id='comment:20'>Comment 20:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/394d58f54c67a3737f31d4cb6095d251f8652899\">394d58f</a></td><td><code>Merge branch 'public/lie_algebras/free-16823' of git://trac.sagemath.org/sage into public/lie_algebras/free-16823</code></td></tr></table>\n",
    "created_at": "2017-06-17T06:13:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223638",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:20'>Comment 20:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/394d58f54c67a3737f31d4cb6095d251f8652899">394d58f</a></td><td><code>Merge branch 'public/lie_algebras/free-16823' of git://trac.sagemath.org/sage into public/lie_algebras/free-16823</code></td></tr></table>




---

archive/issue_events_214949.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-06-17T06:13:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214949"
}
```



---

archive/issue_events_214950.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-06-17T06:13:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214950"
}
```



---

archive/issue_comments_223639.json:
```json
{
    "body": "<a id='comment:23'>Comment 23:</a>\ndoc does not build\n\n```\n+[dochtml] OSError: [groups   ] /home/chapoton/sage/local/lib/python2.7/site-packages/sage/groups/matrix_gps/finitely_generated.py:docstring of sage.groups.matrix_gps.finitely_generated.FinitelyGeneratedMatrixGroup_gap.invariant_generators:59: WARNING: citation not found: Stu1993\n```",
    "created_at": "2017-06-20T06:15:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223639",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:23'>Comment 23:</a>
doc does not build

```
+[dochtml] OSError: [groups   ] /home/chapoton/sage/local/lib/python2.7/site-packages/sage/groups/matrix_gps/finitely_generated.py:docstring of sage.groups.matrix_gps.finitely_generated.FinitelyGeneratedMatrixGroup_gap.invariant_generators:59: WARNING: citation not found: Stu1993
```



---

archive/issue_events_214951.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-06-21T07:00:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214951"
}
```



---

archive/issue_events_214952.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-06-21T07:00:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214952"
}
```



---

archive/issue_comments_223640.json:
```json
{
    "body": "<a id='comment:25'>Comment 25:</a>\nYeah, not sure why the Stu1993 reference was removed. Merge gone wrong?\n\nDoc of `graded_dimension` method: replace `n^(k/d)` by `n^{k/d}` (unless our parser somehow handles parentheses here).\n\nI'm a bit surprised that `_rewrite_bracket` is an abstract method on class `FreeLieBasis_abstract` and not on some higher-up class for arbitrary Lie algebras with ordered family bases. But you know better how the class hierarchy should work.\n\nBased on my arguably naive read of the `lie_algebra_generators` method, it constructs the generators in the Lyndon bases, even if `self` is a Hall basis. Is this really what the rest of the code expects? Same for `gens`.\n\nWhere did you get the definition of a Hall set from? [MKO...]? I cannot really match it with Reutenauer's (Section 5 in http://www.lacim.uqam.ca/~christo/Publi%C3%A9s/2003/free%20Lie%20algebras.pdf ); for example, your `b._left <= a` seems to contradict Reutenauer's rule, which is more like `a._right >= b`. More importantly, what is the ordering on the free magma that you are using? Even [MKO...] leaves this undecided (\"Elements of the same length are ordered internally as we please\"). At the very least it seems to be a degree-first ordering, since otherwise it's probably illegal to restrict yourself to `i in range(1, (k+1) // 2)`.\n\nAlso, what is the mathematical significance of the `GradedLieBracket` class? Are they something like the free magma? Things like this should probably be a lot more transparent, ideally with a way for the user to specify the ordering on the free magma.",
    "created_at": "2017-06-21T20:35:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223640",
    "user": "https://github.com/darijgr"
}
```

<a id='comment:25'>Comment 25:</a>
Yeah, not sure why the Stu1993 reference was removed. Merge gone wrong?

Doc of `graded_dimension` method: replace `n^(k/d)` by `n^{k/d}` (unless our parser somehow handles parentheses here).

I'm a bit surprised that `_rewrite_bracket` is an abstract method on class `FreeLieBasis_abstract` and not on some higher-up class for arbitrary Lie algebras with ordered family bases. But you know better how the class hierarchy should work.

Based on my arguably naive read of the `lie_algebra_generators` method, it constructs the generators in the Lyndon bases, even if `self` is a Hall basis. Is this really what the rest of the code expects? Same for `gens`.

Where did you get the definition of a Hall set from? [MKO...]? I cannot really match it with Reutenauer's (Section 5 in http://www.lacim.uqam.ca/~christo/Publi%C3%A9s/2003/free%20Lie%20algebras.pdf ); for example, your `b._left <= a` seems to contradict Reutenauer's rule, which is more like `a._right >= b`. More importantly, what is the ordering on the free magma that you are using? Even [MKO...] leaves this undecided ("Elements of the same length are ordered internally as we please"). At the very least it seems to be a degree-first ordering, since otherwise it's probably illegal to restrict yourself to `i in range(1, (k+1) // 2)`.

Also, what is the mathematical significance of the `GradedLieBracket` class? Are they something like the free magma? Things like this should probably be a lot more transparent, ideally with a way for the user to specify the ordering on the free magma.



---

archive/issue_comments_223641.json:
```json
{
    "body": "<a id='comment:26'>Comment 26:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1c32cd9b2ea89f79685517e0ed2dfaf36b0a404c\">1c32cd9</a></td><td><code>Adding back reference lost in a merge.</code></td></tr></table>\n",
    "created_at": "2017-06-23T00:23:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223641",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:26'>Comment 26:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1c32cd9b2ea89f79685517e0ed2dfaf36b0a404c">1c32cd9</a></td><td><code>Adding back reference lost in a merge.</code></td></tr></table>




---

archive/issue_comments_223642.json:
```json
{
    "body": "Changed commit from **[394d58f](https://github.com/sagemath/sagetrac-mirror/commit/394d58f54c67a3737f31d4cb6095d251f8652899)** to **[1c32cd9](https://github.com/sagemath/sagetrac-mirror/commit/1c32cd9b2ea89f79685517e0ed2dfaf36b0a404c)**",
    "created_at": "2017-06-23T00:23:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223642",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[394d58f](https://github.com/sagemath/sagetrac-mirror/commit/394d58f54c67a3737f31d4cb6095d251f8652899)** to **[1c32cd9](https://github.com/sagemath/sagetrac-mirror/commit/1c32cd9b2ea89f79685517e0ed2dfaf36b0a404c)**



---

archive/issue_comments_223643.json:
```json
{
    "body": "Changed commit from **[1c32cd9](https://github.com/sagemath/sagetrac-mirror/commit/1c32cd9b2ea89f79685517e0ed2dfaf36b0a404c)** to **[a8ca231](https://github.com/sagemath/sagetrac-mirror/commit/a8ca231ac65bbf9ccf8b4cc316da23390d94f940)**",
    "created_at": "2017-06-23T00:28:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223643",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[1c32cd9](https://github.com/sagemath/sagetrac-mirror/commit/1c32cd9b2ea89f79685517e0ed2dfaf36b0a404c)** to **[a8ca231](https://github.com/sagemath/sagetrac-mirror/commit/a8ca231ac65bbf9ccf8b4cc316da23390d94f940)**



---

archive/issue_comments_223644.json:
```json
{
    "body": "<a id='comment:27'>Comment 27:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a8ca231ac65bbf9ccf8b4cc316da23390d94f940\">a8ca231</a></td><td><code>Some doc improvements.</code></td></tr></table>\n",
    "created_at": "2017-06-23T00:28:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223644",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:27'>Comment 27:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a8ca231ac65bbf9ccf8b4cc316da23390d94f940">a8ca231</a></td><td><code>Some doc improvements.</code></td></tr></table>




---

archive/issue_comments_223645.json:
```json
{
    "body": "<a id='comment:28'>Comment 28:</a>\nReplying to [@darijgr](#comment%3A25):\n> Yeah, not sure why the Stu1993 reference was removed. Merge gone wrong?\n\nYep, bad merge.\n\n> Doc of `graded_dimension` method: replace `n^(k/d)` by `n^{k/d}` (unless our parser somehow handles parentheses here).\n\nDone.\n\n> I'm a bit surprised that `_rewrite_bracket` is an abstract method on class `FreeLieBasis_abstract` and not on some higher-up class for arbitrary Lie algebras with ordered family bases. But you know better how the class hierarchy should work.\n\nIt makes no sense anywhere else as this is the only class that has elements stored as trees of Lie brackets.\n\n> Based on my arguably naive read of the `lie_algebra_generators` method, it constructs the generators in the Lyndon bases, even if `self` is a Hall basis. Is this really what the rest of the code expects? Same for `gens`.\n\nNot quite. This is a shortcut for creating elements of the free Lie algebra without explicitly specifying the basis (think `SymmetricFunctions`). If you create the Hall basis, which is a separate object, then you have a different `lie_algebra_generators`.\n\n> Where did you get the definition of a Hall set from? [MKO...]? I cannot really match it with Reutenauer's (Section 5 in http://www.lacim.uqam.ca/~christo/Publi%C3%A9s/2003/free%20Lie%20algebras.pdf ); for example, your `b._left <= a` seems to contradict Reutenauer's rule, which is more like `a._right >= b`. More importantly, what is the ordering on the free magma that you are using? Even [MKO...] leaves this undecided (\"Elements of the same length are ordered internally as we please\"). At the very least it seems to be a degree-first ordering, since otherwise it's probably illegal to restrict yourself to `i in range(1, (k+1) // 2)`.\n\nFor the most part, it was far too long ago that I wrote this that I do not really remember. I am pretty sure I used [MKO...], but I can't say for certain. However, length := degree in this construction. Feel free to change what you want, but I know this works. IIRC, it agrees with the monomials in [MKO...]. As you point out, there are a number of different Hall sets (well, really I think there are 4 by fundamentally playing two types of left vs right games), and the code seems very sensitive when things are not correct (infinite recursions).\n\n> Also, what is the mathematical significance of the `GradedLieBracket` class? Are they something like the free magma? Things like this should probably be a lot more transparent, ideally with a way for the user to specify the ordering on the free magma.\n\nIt is precisely what it says it is: it is a Lie bracket with a grading. In particular, we use the natural grading of generators being degree 1. However, it is basically a helper class used to construct the basis elements and not really meant to be manipulated by the user.",
    "created_at": "2017-06-23T00:45:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223645",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:28'>Comment 28:</a>
Replying to [@darijgr](#comment%3A25):
> Yeah, not sure why the Stu1993 reference was removed. Merge gone wrong?

Yep, bad merge.

> Doc of `graded_dimension` method: replace `n^(k/d)` by `n^{k/d}` (unless our parser somehow handles parentheses here).

Done.

> I'm a bit surprised that `_rewrite_bracket` is an abstract method on class `FreeLieBasis_abstract` and not on some higher-up class for arbitrary Lie algebras with ordered family bases. But you know better how the class hierarchy should work.

It makes no sense anywhere else as this is the only class that has elements stored as trees of Lie brackets.

> Based on my arguably naive read of the `lie_algebra_generators` method, it constructs the generators in the Lyndon bases, even if `self` is a Hall basis. Is this really what the rest of the code expects? Same for `gens`.

Not quite. This is a shortcut for creating elements of the free Lie algebra without explicitly specifying the basis (think `SymmetricFunctions`). If you create the Hall basis, which is a separate object, then you have a different `lie_algebra_generators`.

> Where did you get the definition of a Hall set from? [MKO...]? I cannot really match it with Reutenauer's (Section 5 in http://www.lacim.uqam.ca/~christo/Publi%C3%A9s/2003/free%20Lie%20algebras.pdf ); for example, your `b._left <= a` seems to contradict Reutenauer's rule, which is more like `a._right >= b`. More importantly, what is the ordering on the free magma that you are using? Even [MKO...] leaves this undecided ("Elements of the same length are ordered internally as we please"). At the very least it seems to be a degree-first ordering, since otherwise it's probably illegal to restrict yourself to `i in range(1, (k+1) // 2)`.

For the most part, it was far too long ago that I wrote this that I do not really remember. I am pretty sure I used [MKO...], but I can't say for certain. However, length := degree in this construction. Feel free to change what you want, but I know this works. IIRC, it agrees with the monomials in [MKO...]. As you point out, there are a number of different Hall sets (well, really I think there are 4 by fundamentally playing two types of left vs right games), and the code seems very sensitive when things are not correct (infinite recursions).

> Also, what is the mathematical significance of the `GradedLieBracket` class? Are they something like the free magma? Things like this should probably be a lot more transparent, ideally with a way for the user to specify the ordering on the free magma.

It is precisely what it says it is: it is a Lie bracket with a grading. In particular, we use the natural grading of generators being degree 1. However, it is basically a helper class used to construct the basis elements and not really meant to be manipulated by the user.



---

archive/issue_comments_223646.json:
```json
{
    "body": "<a id='comment:29'>Comment 29:</a>\nBringing over the side discussion from #24144: If you're worried about the Hall basis, then how about I mark it as `experimental` (yes, there something for that similar to `deprecation`). I recall doing a fair bit of testing with it, so I have some confidence it is solid. However, I am willing to give somewhat to get some implementation of the free Lie algebra into Sage. Moreover, if there are serious problems, we can always fix them on later tickets (Sage reviews do not have to be as thorough as math reviews `:)`).",
    "created_at": "2017-11-04T05:42:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223646",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:29'>Comment 29:</a>
Bringing over the side discussion from #24144: If you're worried about the Hall basis, then how about I mark it as `experimental` (yes, there something for that similar to `deprecation`). I recall doing a fair bit of testing with it, so I have some confidence it is solid. However, I am willing to give somewhat to get some implementation of the free Lie algebra into Sage. Moreover, if there are serious problems, we can always fix them on later tickets (Sage reviews do not have to be as thorough as math reviews `:)`).



---

archive/issue_comments_223647.json:
```json
{
    "body": "<a id='comment:30'>Comment 30:</a>\nSo if you're willing to review everything else now and simply mark the Hall as experimental (unless you find a bug if you do look at it), then I can rebase the ticket with that added in.",
    "created_at": "2017-11-04T05:43:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223647",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:30'>Comment 30:</a>
So if you're willing to review everything else now and simply mark the Hall as experimental (unless you find a bug if you do look at it), then I can rebase the ticket with that added in.



---

archive/issue_comments_223648.json:
```json
{
    "body": "<a id='comment:31'>Comment 31:</a>\nPlease do so!",
    "created_at": "2017-11-04T05:43:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223648",
    "user": "https://github.com/darijgr"
}
```

<a id='comment:31'>Comment 31:</a>
Please do so!



---

archive/issue_comments_223649.json:
```json
{
    "body": "<a id='comment:32'>Comment 32:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/48916e08332e40cf44acaaf8005ca8e293dc41d6\">48916e0</a></td><td><code>Merge branch 'public/lie_algebras/free-16823' of git://trac.sagemath.org/sage into public/lie_algebras/free-16823</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/63ea416948074f61516d8c93f3df1cf0ab82898c\">63ea416</a></td><td><code>Added experimental warning to Hall basis.</code></td></tr></table>\n",
    "created_at": "2017-11-04T06:13:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223649",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:32'>Comment 32:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/48916e08332e40cf44acaaf8005ca8e293dc41d6">48916e0</a></td><td><code>Merge branch 'public/lie_algebras/free-16823' of git://trac.sagemath.org/sage into public/lie_algebras/free-16823</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/63ea416948074f61516d8c93f3df1cf0ab82898c">63ea416</a></td><td><code>Added experimental warning to Hall basis.</code></td></tr></table>




---

archive/issue_comments_223650.json:
```json
{
    "body": "Changed commit from **[a8ca231](https://github.com/sagemath/sagetrac-mirror/commit/a8ca231ac65bbf9ccf8b4cc316da23390d94f940)** to **[63ea416](https://github.com/sagemath/sagetrac-mirror/commit/63ea416948074f61516d8c93f3df1cf0ab82898c)**",
    "created_at": "2017-11-04T06:13:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223650",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[a8ca231](https://github.com/sagemath/sagetrac-mirror/commit/a8ca231ac65bbf9ccf8b4cc316da23390d94f940)** to **[63ea416](https://github.com/sagemath/sagetrac-mirror/commit/63ea416948074f61516d8c93f3df1cf0ab82898c)**



---

archive/issue_comments_223651.json:
```json
{
    "body": "<a id='comment:33'>Comment 33:</a>\nDone.\n\nNote that this will trivially conflict with #24134. So if this is positively reviewed before #24134 is merged, then we will merge it in here. However, we will not do it beforehand.\n\nEdit - Need the right ticket number.",
    "created_at": "2017-11-04T06:13:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223651",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:33'>Comment 33:</a>
Done.

Note that this will trivially conflict with #24134. So if this is positively reviewed before #24134 is merged, then we will merge it in here. However, we will not do it beforehand.

Edit - Need the right ticket number.



---

archive/issue_events_214953.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-11-04T06:13:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214953"
}
```



---

archive/issue_events_214954.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-11-04T06:13:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214954"
}
```



---

archive/issue_comments_223652.json:
```json
{
    "body": "Changed commit from **[63ea416](https://github.com/sagemath/sagetrac-mirror/commit/63ea416948074f61516d8c93f3df1cf0ab82898c)** to **[b703250](https://github.com/sagemath/sagetrac-mirror/commit/b703250b8fbf324594a8b7f462a564cc429fc8a5)**",
    "created_at": "2017-11-21T11:01:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223652",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[63ea416](https://github.com/sagemath/sagetrac-mirror/commit/63ea416948074f61516d8c93f3df1cf0ab82898c)** to **[b703250](https://github.com/sagemath/sagetrac-mirror/commit/b703250b8fbf324594a8b7f462a564cc429fc8a5)**



---

archive/issue_comments_223653.json:
```json
{
    "body": "<a id='comment:34'>Comment 34:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/493d37caea18f01c77286e3a8f386ae45e58280f\">493d37c</a></td><td><code>Merge branch 'public/lie_algebras/free-16823' in 8.1.rc2</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b703250b8fbf324594a8b7f462a564cc429fc8a5\">b703250</a></td><td><code>trac 16823 fix doctests</code></td></tr></table>\n",
    "created_at": "2017-11-21T11:01:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223653",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:34'>Comment 34:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/493d37caea18f01c77286e3a8f386ae45e58280f">493d37c</a></td><td><code>Merge branch 'public/lie_algebras/free-16823' in 8.1.rc2</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b703250b8fbf324594a8b7f462a564cc429fc8a5">b703250</a></td><td><code>trac 16823 fix doctests</code></td></tr></table>




---

archive/issue_comments_223654.json:
```json
{
    "body": "<a id='comment:35'>Comment 35:</a>\nThis needs a non-fully-trivial rebase, that I did not know how to do myself.",
    "created_at": "2017-12-20T09:24:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223654",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:35'>Comment 35:</a>
This needs a non-fully-trivial rebase, that I did not know how to do myself.



---

archive/issue_events_214955.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-12-20T09:24:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214955"
}
```



---

archive/issue_events_214956.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-12-20T09:24:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214956"
}
```



---

archive/issue_comments_223655.json:
```json
{
    "body": "Changed commit from **[b703250](https://github.com/sagemath/sagetrac-mirror/commit/b703250b8fbf324594a8b7f462a564cc429fc8a5)** to **[b18c900](https://github.com/sagemath/sagetrac-mirror/commit/b18c900ade34cff2e0ef7b358f8ba294a7bc24ab)**",
    "created_at": "2017-12-20T11:09:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223655",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[b703250](https://github.com/sagemath/sagetrac-mirror/commit/b703250b8fbf324594a8b7f462a564cc429fc8a5)** to **[b18c900](https://github.com/sagemath/sagetrac-mirror/commit/b18c900ade34cff2e0ef7b358f8ba294a7bc24ab)**



---

archive/issue_comments_223656.json:
```json
{
    "body": "<a id='comment:36'>Comment 36:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b18c900ade34cff2e0ef7b358f8ba294a7bc24ab\">b18c900</a></td><td><code>Merge branch 'public/lie_algebras/free-16823' of git://trac.sagemath.org/sage into public/lie_algebras/free-16823</code></td></tr></table>\n",
    "created_at": "2017-12-20T11:09:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223656",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:36'>Comment 36:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b18c900ade34cff2e0ef7b358f8ba294a7bc24ab">b18c900</a></td><td><code>Merge branch 'public/lie_algebras/free-16823' of git://trac.sagemath.org/sage into public/lie_algebras/free-16823</code></td></tr></table>




---

archive/issue_events_214957.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-12-20T11:10:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214957"
}
```



---

archive/issue_events_214958.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-12-20T11:10:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214958"
}
```



---

archive/issue_events_214959.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-12-20T11:10:14Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214959"
}
```



---

archive/issue_events_214960.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-12-20T11:10:14Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214960"
}
```



---

archive/issue_comments_223657.json:
```json
{
    "body": "<a id='comment:37'>Comment 37:</a>\nWasn't too bad.",
    "created_at": "2017-12-20T11:10:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223657",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:37'>Comment 37:</a>
Wasn't too bad.



---

archive/issue_comments_223658.json:
```json
{
    "body": "Changed commit from **[b18c900](https://github.com/sagemath/sagetrac-mirror/commit/b18c900ade34cff2e0ef7b358f8ba294a7bc24ab)** to **[bd22ffa](https://github.com/sagemath/sagetrac-mirror/commit/bd22ffa849c1105859eb563f94a89a798949028b)**",
    "created_at": "2017-12-28T01:22:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223658",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[b18c900](https://github.com/sagemath/sagetrac-mirror/commit/b18c900ade34cff2e0ef7b358f8ba294a7bc24ab)** to **[bd22ffa](https://github.com/sagemath/sagetrac-mirror/commit/bd22ffa849c1105859eb563f94a89a798949028b)**



---

archive/issue_comments_223659.json:
```json
{
    "body": "<a id='comment:38'>Comment 38:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/bd22ffa849c1105859eb563f94a89a798949028b\">bd22ffa</a></td><td><code>doc changes; why do doctests fail?</code></td></tr></table>\n",
    "created_at": "2017-12-28T01:22:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223659",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:38'>Comment 38:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/bd22ffa849c1105859eb563f94a89a798949028b">bd22ffa</a></td><td><code>doc changes; why do doctests fail?</code></td></tr></table>




---

archive/issue_comments_223660.json:
```json
{
    "body": "<a id='comment:39'>Comment 39:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0596991a54d5530bf48870c0ec824c9a721c49a1\">0596991</a></td><td><code>revert a wrong edit and document the pitfall</code></td></tr></table>\n",
    "created_at": "2017-12-28T02:09:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223660",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:39'>Comment 39:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0596991a54d5530bf48870c0ec824c9a721c49a1">0596991</a></td><td><code>revert a wrong edit and document the pitfall</code></td></tr></table>




---

archive/issue_comments_223661.json:
```json
{
    "body": "Changed commit from **[bd22ffa](https://github.com/sagemath/sagetrac-mirror/commit/bd22ffa849c1105859eb563f94a89a798949028b)** to **[0596991](https://github.com/sagemath/sagetrac-mirror/commit/0596991a54d5530bf48870c0ec824c9a721c49a1)**",
    "created_at": "2017-12-28T02:09:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223661",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[bd22ffa](https://github.com/sagemath/sagetrac-mirror/commit/bd22ffa849c1105859eb563f94a89a798949028b)** to **[0596991](https://github.com/sagemath/sagetrac-mirror/commit/0596991a54d5530bf48870c0ec824c9a721c49a1)**



---

archive/issue_comments_223662.json:
```json
{
    "body": "<a id='comment:40'>Comment 40:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/04117265e47325f78c4ff62ea9b77af88376cc3c\">0411726</a></td><td><code>Merge branch 'public/lie_algebras/free-16823' of git://trac.sagemath.org/sage into freelie</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3fe36e75f4de330d81d09755f22b903984471fd9\">3fe36e7</a></td><td><code>maybe improvements</code></td></tr></table>\n",
    "created_at": "2017-12-29T02:18:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223662",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:40'>Comment 40:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/04117265e47325f78c4ff62ea9b77af88376cc3c">0411726</a></td><td><code>Merge branch 'public/lie_algebras/free-16823' of git://trac.sagemath.org/sage into freelie</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3fe36e75f4de330d81d09755f22b903984471fd9">3fe36e7</a></td><td><code>maybe improvements</code></td></tr></table>




---

archive/issue_comments_223663.json:
```json
{
    "body": "Changed commit from **[0596991](https://github.com/sagemath/sagetrac-mirror/commit/0596991a54d5530bf48870c0ec824c9a721c49a1)** to **[3fe36e7](https://github.com/sagemath/sagetrac-mirror/commit/3fe36e75f4de330d81d09755f22b903984471fd9)**",
    "created_at": "2017-12-29T02:18:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223663",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[0596991](https://github.com/sagemath/sagetrac-mirror/commit/0596991a54d5530bf48870c0ec824c9a721c49a1)** to **[3fe36e7](https://github.com/sagemath/sagetrac-mirror/commit/3fe36e75f4de330d81d09755f22b903984471fd9)**



---

archive/issue_comments_223664.json:
```json
{
    "body": "<a id='comment:41'>Comment 41:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/01c9dca23deeb2a4206914ec6f64d5e74ba3c2fd\">01c9dca</a></td><td><code>review, minus Hall basis and some technical stuff</code></td></tr></table>\n",
    "created_at": "2017-12-30T19:37:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223664",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:41'>Comment 41:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/01c9dca23deeb2a4206914ec6f64d5e74ba3c2fd">01c9dca</a></td><td><code>review, minus Hall basis and some technical stuff</code></td></tr></table>




---

archive/issue_comments_223665.json:
```json
{
    "body": "Changed commit from **[3fe36e7](https://github.com/sagemath/sagetrac-mirror/commit/3fe36e75f4de330d81d09755f22b903984471fd9)** to **[01c9dca](https://github.com/sagemath/sagetrac-mirror/commit/01c9dca23deeb2a4206914ec6f64d5e74ba3c2fd)**",
    "created_at": "2017-12-30T19:37:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223665",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[3fe36e7](https://github.com/sagemath/sagetrac-mirror/commit/3fe36e75f4de330d81d09755f22b903984471fd9)** to **[01c9dca](https://github.com/sagemath/sagetrac-mirror/commit/01c9dca23deeb2a4206914ec6f64d5e74ba3c2fd)**



---

archive/issue_comments_223666.json:
```json
{
    "body": "<a id='comment:42'>Comment 42:</a>\nTODO:\n\n* Review Hall basis. (Which Hall set and which ordering does it use?)\n\n* Extend Lyndon and Hall basis to arbitrary orderings perhaps. (Next ticket.)\n\n* If you don't like my doc for the helper classes, please suggest better alternatives. Doc is highly necessary there. These classes *are* exposed: they are the parents for basis indices, and so will be relevant to anyone defining e.g. maps out of a free Lie algebra. And their use is sufficiently complex that a grep doesn't answer the questions (this is where I got stuck trying to review the ticket months ago).",
    "created_at": "2017-12-30T19:39:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223666",
    "user": "https://github.com/darijgr"
}
```

<a id='comment:42'>Comment 42:</a>
TODO:

* Review Hall basis. (Which Hall set and which ordering does it use?)

* Extend Lyndon and Hall basis to arbitrary orderings perhaps. (Next ticket.)

* If you don't like my doc for the helper classes, please suggest better alternatives. Doc is highly necessary there. These classes *are* exposed: they are the parents for basis indices, and so will be relevant to anyone defining e.g. maps out of a free Lie algebra. And their use is sufficiently complex that a grep doesn't answer the questions (this is where I got stuck trying to review the ticket months ago).



---

archive/issue_comments_223667.json:
```json
{
    "body": "<a id='comment:43'>Comment 43:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d9e54f1472164256841acad5b54d86221b51f998\">d9e54f1</a></td><td><code>document basis key classes in free Lie algebra</code></td></tr></table>\n",
    "created_at": "2017-12-31T14:24:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223667",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:43'>Comment 43:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d9e54f1472164256841acad5b54d86221b51f998">d9e54f1</a></td><td><code>document basis key classes in free Lie algebra</code></td></tr></table>




---

archive/issue_comments_223668.json:
```json
{
    "body": "Changed commit from **[01c9dca](https://github.com/sagemath/sagetrac-mirror/commit/01c9dca23deeb2a4206914ec6f64d5e74ba3c2fd)** to **[d9e54f1](https://github.com/sagemath/sagetrac-mirror/commit/d9e54f1472164256841acad5b54d86221b51f998)**",
    "created_at": "2017-12-31T14:24:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223668",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[01c9dca](https://github.com/sagemath/sagetrac-mirror/commit/01c9dca23deeb2a4206914ec6f64d5e74ba3c2fd)** to **[d9e54f1](https://github.com/sagemath/sagetrac-mirror/commit/d9e54f1472164256841acad5b54d86221b51f998)**



---

archive/issue_comments_223669.json:
```json
{
    "body": "<a id='comment:44'>Comment 44:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d1ed81d4f698ad9d9b098c3b0c2c9bb199c1c9c9\">d1ed81d</a></td><td><code>Merge branch 'public/lie_algebras/free-16823' of trac.sagemath.org:sage into freelie</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/882c861232abf0f6c50aa09effe8f4f32876dea3\">882c861</a></td><td><code>improve the doc</code></td></tr></table>\n",
    "created_at": "2018-04-13T19:57:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223669",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:44'>Comment 44:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d1ed81d4f698ad9d9b098c3b0c2c9bb199c1c9c9">d1ed81d</a></td><td><code>Merge branch 'public/lie_algebras/free-16823' of trac.sagemath.org:sage into freelie</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/882c861232abf0f6c50aa09effe8f4f32876dea3">882c861</a></td><td><code>improve the doc</code></td></tr></table>




---

archive/issue_comments_223670.json:
```json
{
    "body": "Changed commit from **[d9e54f1](https://github.com/sagemath/sagetrac-mirror/commit/d9e54f1472164256841acad5b54d86221b51f998)** to **[882c861](https://github.com/sagemath/sagetrac-mirror/commit/882c861232abf0f6c50aa09effe8f4f32876dea3)**",
    "created_at": "2018-04-13T19:57:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223670",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[d9e54f1](https://github.com/sagemath/sagetrac-mirror/commit/d9e54f1472164256841acad5b54d86221b51f998)** to **[882c861](https://github.com/sagemath/sagetrac-mirror/commit/882c861232abf0f6c50aa09effe8f4f32876dea3)**



---

archive/issue_comments_223671.json:
```json
{
    "body": "Reviewer: **Darij Grinberg**",
    "created_at": "2018-04-13T20:02:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223671",
    "user": "https://github.com/darijgr"
}
```

Reviewer: **Darij Grinberg**



---

archive/issue_events_214961.json:
```json
{
    "actor": "https://github.com/darijgr",
    "created_at": "2018-04-13T20:02:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214961"
}
```



---

archive/issue_events_214962.json:
```json
{
    "actor": "https://github.com/darijgr",
    "created_at": "2018-04-13T20:02:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214962"
}
```



---

archive/issue_events_214963.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-04-13T20:04:42Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214963"
}
```



---

archive/issue_events_214964.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-04-13T20:04:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214964"
}
```



---

archive/issue_comments_223672.json:
```json
{
    "body": "<a id='comment:46'>Comment 46:</a>\nThank you. See #25166 for a follow-up.",
    "created_at": "2018-04-13T20:04:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223672",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:46'>Comment 46:</a>
Thank you. See #25166 for a follow-up.



---

archive/issue_comments_223673.json:
```json
{
    "body": "<a id='comment:47'>Comment 47:</a>\nPDF docs don't build",
    "created_at": "2018-05-06T10:47:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223673",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:47'>Comment 47:</a>
PDF docs don't build



---

archive/issue_events_214965.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-05-06T10:47:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214965"
}
```



---

archive/issue_events_214966.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-05-06T10:47:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214966"
}
```



---

archive/issue_comments_223674.json:
```json
{
    "body": "Changed commit from **[882c861](https://github.com/sagemath/sagetrac-mirror/commit/882c861232abf0f6c50aa09effe8f4f32876dea3)** to **[70cb2b7](https://github.com/sagemath/sagetrac-mirror/commit/70cb2b75347945d9f75ac63189937bb85eda0b9b)**",
    "created_at": "2018-05-06T21:34:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223674",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[882c861](https://github.com/sagemath/sagetrac-mirror/commit/882c861232abf0f6c50aa09effe8f4f32876dea3)** to **[70cb2b7](https://github.com/sagemath/sagetrac-mirror/commit/70cb2b75347945d9f75ac63189937bb85eda0b9b)**



---

archive/issue_comments_223675.json:
```json
{
    "body": "<a id='comment:48'>Comment 48:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b452a38ef70f514abad67e53a479a8a4ab4d2bd9\">b452a38</a></td><td><code>Merge branch 'public/lie_algebras/free-16823' of trac.sagemath.org:sage into lie</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/70cb2b75347945d9f75ac63189937bb85eda0b9b\">70cb2b7</a></td><td><code>fixes to doc</code></td></tr></table>\n",
    "created_at": "2018-05-06T21:34:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223675",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:48'>Comment 48:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b452a38ef70f514abad67e53a479a8a4ab4d2bd9">b452a38</a></td><td><code>Merge branch 'public/lie_algebras/free-16823' of trac.sagemath.org:sage into lie</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/70cb2b75347945d9f75ac63189937bb85eda0b9b">70cb2b7</a></td><td><code>fixes to doc</code></td></tr></table>




---

archive/issue_comments_223676.json:
```json
{
    "body": "<a id='comment:49'>Comment 49:</a>\nBetter now? I regret I can't test PDF output on the university machine where I have sage installed (latexmk is missing).",
    "created_at": "2018-05-06T21:41:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223676",
    "user": "https://github.com/darijgr"
}
```

<a id='comment:49'>Comment 49:</a>
Better now? I regret I can't test PDF output on the university machine where I have sage installed (latexmk is missing).



---

archive/issue_events_214967.json:
```json
{
    "actor": "https://github.com/darijgr",
    "created_at": "2018-05-06T21:41:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214967"
}
```



---

archive/issue_events_214968.json:
```json
{
    "actor": "https://github.com/darijgr",
    "created_at": "2018-05-06T21:41:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214968"
}
```



---

archive/issue_comments_223677.json:
```json
{
    "body": "<a id='comment:50'>Comment 50:</a>\nI am testing it right now.",
    "created_at": "2018-05-06T22:20:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223677",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:50'>Comment 50:</a>
I am testing it right now.



---

archive/issue_events_214969.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-05-06T22:20:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214969"
}
```



---

archive/issue_events_214970.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-05-06T22:20:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214970"
}
```



---

archive/issue_comments_223678.json:
```json
{
    "body": "Changed commit from **[70cb2b7](https://github.com/sagemath/sagetrac-mirror/commit/70cb2b75347945d9f75ac63189937bb85eda0b9b)** to **[e27b98b](https://github.com/sagemath/sagetrac-mirror/commit/e27b98b282a0c86945fc155519169b77fa0b2d41)**",
    "created_at": "2018-05-06T22:45:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223678",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[70cb2b7](https://github.com/sagemath/sagetrac-mirror/commit/70cb2b75347945d9f75ac63189937bb85eda0b9b)** to **[e27b98b](https://github.com/sagemath/sagetrac-mirror/commit/e27b98b282a0c86945fc155519169b77fa0b2d41)**



---

archive/issue_comments_223679.json:
```json
{
    "body": "<a id='comment:51'>Comment 51:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4465d037e9c9041160e757bccab65b7bfe5d2606\">4465d03</a></td><td><code>Merge branch 'public/lie_algebras/free-16823' of git://trac.sagemath.org/sage into public/lie_algebras/free-16823</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e27b98b282a0c86945fc155519169b77fa0b2d41\">e27b98b</a></td><td><code>Fixing docbuild.</code></td></tr></table>\n",
    "created_at": "2018-05-06T22:45:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223679",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:51'>Comment 51:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4465d037e9c9041160e757bccab65b7bfe5d2606">4465d03</a></td><td><code>Merge branch 'public/lie_algebras/free-16823' of git://trac.sagemath.org/sage into public/lie_algebras/free-16823</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e27b98b282a0c86945fc155519169b77fa0b2d41">e27b98b</a></td><td><code>Fixing docbuild.</code></td></tr></table>




---

archive/issue_events_214971.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-05-06T22:45:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214971"
}
```



---

archive/issue_events_214972.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-05-06T22:45:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214972"
}
```



---

archive/issue_comments_223680.json:
```json
{
    "body": "<a id='comment:52'>Comment 52:</a>\nI was able to build the pdf with these changes.",
    "created_at": "2018-05-06T22:45:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223680",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:52'>Comment 52:</a>
I was able to build the pdf with these changes.



---

archive/issue_events_214973.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-05-09T09:50:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214973"
}
```



---

archive/issue_events_214974.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "59c99eca6a8187b0aa273dde98abcddec8ca114b",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-05-09T09:50:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16823#event-214974"
}
```



---

archive/issue_comments_223681.json:
```json
{
    "body": "Changed branch from **[public/lie_algebras/free-16823](https://github.com/sagemath/sagetrac-mirror/tree/public/lie_algebras/free-16823)** to **[e27b98b](https://github.com/sagemath/sagetrac-mirror/commit/e27b98b282a0c86945fc155519169b77fa0b2d41)**",
    "created_at": "2018-05-09T09:50:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16823#issuecomment-223681",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/lie_algebras/free-16823](https://github.com/sagemath/sagetrac-mirror/tree/public/lie_algebras/free-16823)** to **[e27b98b](https://github.com/sagemath/sagetrac-mirror/commit/e27b98b282a0c86945fc155519169b77fa0b2d41)**
