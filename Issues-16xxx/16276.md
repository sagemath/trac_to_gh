...
```json
{
    "body": "Question:  are we 100% certain that every instance of the class which has this method does have a valid quo_rem method?  If not (and may even if so) it would be good to wrap the call to quo_rem in a try...except block with a failsafe default to just return self.",
...
```json
{
    "body": "No we are not. As you pointed out, in ZZ[X], quo_rem cannot work reliably in all cases. By design, it does \"something\" when b is not monic and does not divides a exactly (it tries to reduce \"as much as possible\").\n\nDo you think we should also patch quo_rem to return an `ArithmeticError` exception in this case where the result is not well-defined?",
...
```json
{
    "body": "No, I don't think that would be helpful.  We agreed (I think) that for the reduction of `X^100 mod 2*X+1` in ZZ[X] there was no better result than leaving it as `X^100`, so would we really want to raise an error when someone types \n\n```\nsage: x = polygen(ZZ)\nsage: (x^100) % (2*x+1)\n```\ninstead of just returning\n\n```\nx^100\n```\nas happens now?  Even this\n\n```\nsage: x = polygen(QQ)\nsage: (x^100) % (2*x+1)\n1/1267650600228229401496703205376\n```\nwhile being quite valid mathematically, is slightly surprising...",
...
```json
{
    "body": "`@`jpflori : I implemented your suggestion.",
...
```json
{
    "body": "Is `mod = __mod__` the correct way to say that these two functions should behave identically?  If `C` is a subclass of `Polynomial` implementing `__mod__` but not `mod`, then `C.mod` will still be equal to `Polynomial.__mod__`, not to `C.__mod__`, if I'm not mistaken.\n\nThis is currently the case for `Polynomial_dense_modn_ntl_zz`, `Polynomial_dense_modn_ntl_ZZ` and `Polynomial_rational_flint`.  To automatically make `mod == __mod__` for these classes, I think `Polynomial.mod` would have to be defined as\n\n```\ndef mod(self, other):\n    return self.__mod__(other)\n```",
...
```json
{
    "body": "Thanks Peter, you might be right, my comment was just a quick remark about code duplication.",
...
```json
{
    "body": "And I can confirm that just putting `mod = __mod__` gives the behavior you mention.",
...
```json
{
    "body": "Duly noted. I have made the change. I can't see how to avoid doctest duplication though.",
...
```json
{
    "body": "I also suggest to close this one to get `mod` fixed and open a follow up ticket to unify `quo_rem` behavior.",
...
```json
{
    "body": "Thanks!  I think we all agree that this neatly solves the original problem.  The patchbot doesn't seem to have tested the current state of affairs yet; now running doctests.",
