# Issue 16829: Cython profiling does should include absolute path

archive/issues_016592.json:
```json
{
    "body": "For profiling purposes and even more for citation management (see #16777) it is desirable to get a full file path when profiling pyx-files. Cython generates this information in Cython/Compiler/ModuleNode.py at line 688 (as of Sage 6.3). Putting os.path.abspath there yields the desired effect.\n\nUpstream has been provided with a pull request. See http://github.com/cython/cython/pull/315.\n\nCC:  @robertwb @vbraun\n\nUpstream: Completely fixed; Fix reported upstream\n\nReviewer: Jeroen Demeyer\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/16829\n\n",
    "closed_at": "2015-02-16T23:09:27Z",
    "created_at": "2014-08-14T15:29:30Z",
    "labels": [
        "component: build",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "Cython profiling does should include absolute path",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/16829",
    "user": "https://trac.sagemath.org/admin/accounts/users/mraum"
}
```
For profiling purposes and even more for citation management (see #16777) it is desirable to get a full file path when profiling pyx-files. Cython generates this information in Cython/Compiler/ModuleNode.py at line 688 (as of Sage 6.3). Putting os.path.abspath there yields the desired effect.

Upstream has been provided with a pull request. See http://github.com/cython/cython/pull/315.

CC:  @robertwb @vbraun

Upstream: Completely fixed; Fix reported upstream

Reviewer: Jeroen Demeyer

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/16829





---

archive/issue_comments_281231.json:
```json
{
    "body": "<a id='comment:1'></a>Cython has merged 315. I hope this will be part of Cython 0.21, and I suppose then this can be closed as soon as we have upgraded Cython.",
    "created_at": "2014-08-15T06:04:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16829",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16829#issuecomment-281231",
    "user": "https://trac.sagemath.org/admin/accounts/users/mraum"
}
```

<a id='comment:1'></a>Cython has merged 315. I hope this will be part of Cython 0.21, and I suppose then this can be closed as soon as we have upgraded Cython.



---

archive/issue_comments_281232.json:
```json
{
    "body": "Changing upstream from \"Reported upstream. No feedback yet.\" to \"Fixed upstream, but not in a stable release.\"",
    "created_at": "2014-08-15T06:04:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16829",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16829#issuecomment-281232",
    "user": "https://trac.sagemath.org/admin/accounts/users/mraum"
}
```

Changing upstream from "Reported upstream. No feedback yet." to "Fixed upstream, but not in a stable release."



---

archive/issue_comments_281233.json:
```json
{
    "body": "Changing status from new to needs_info.",
    "created_at": "2014-08-15T06:04:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16829",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16829#issuecomment-281233",
    "user": "https://trac.sagemath.org/admin/accounts/users/mraum"
}
```

Changing status from new to needs_info.



---

archive/issue_events_049264.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-02-13T20:34:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16829",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16829#event-49264"
}
```



---

archive/issue_comments_281234.json:
```json
{
    "body": "Changing upstream from \"Fixed upstream, but not in a stable release.\" to \"Completely fixed; Fix reported upstream\"",
    "created_at": "2015-02-13T20:34:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16829",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16829#issuecomment-281234",
    "user": "https://github.com/jdemeyer"
}
```

Changing upstream from "Fixed upstream, but not in a stable release." to "Completely fixed; Fix reported upstream"



---

archive/issue_comments_281235.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Jeroen Demeyer\"",
    "created_at": "2015-02-13T20:34:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16829",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16829#issuecomment-281235",
    "user": "https://github.com/jdemeyer"
}
```

Changing reviewer from "" to "Jeroen Demeyer"



---

archive/issue_comments_281236.json:
```json
{
    "body": "Changing status from needs_info to positive_review.",
    "created_at": "2015-02-13T20:34:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16829",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16829#issuecomment-281236",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_info to positive_review.



---

archive/issue_events_049265.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-02-16T23:09:27Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/16829",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16829#event-49265"
}
```



---

archive/issue_comments_281237.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2015-02-16T23:09:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16829",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16829#issuecomment-281237",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed
