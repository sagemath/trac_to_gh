# Issue 16829: Cython profiling does should include absolute path

archive/issues_016592.json:
```json
{
    "assignees": [],
    "body": "For profiling purposes and even more for citation management (see #16777) it is desirable to get a full file path when profiling pyx-files. Cython generates this information in Cython/Compiler/ModuleNode.py at line 688 (as of Sage 6.3). Putting os.path.abspath there yields the desired effect.\n\nUpstream has been provided with a pull request. See http://github.com/cython/cython/pull/315.\n\nCC:  @robertwb @vbraun\n\nUpstream: **Completely fixed; Fix reported upstream**\n\nReviewer: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16829_\n\n",
    "closed_at": "2015-02-16T23:09:27Z",
    "created_at": "2014-08-14T15:29:30Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Cython profiling does should include absolute path",
    "type": "issue",
    "updated_at": "2015-02-16T23:09:27Z",
    "url": "https://github.com/sagemath/sage/issues/16829",
    "user": "https://github.com/sagetrac-mraum"
}
```
For profiling purposes and even more for citation management (see #16777) it is desirable to get a full file path when profiling pyx-files. Cython generates this information in Cython/Compiler/ModuleNode.py at line 688 (as of Sage 6.3). Putting os.path.abspath there yields the desired effect.

Upstream has been provided with a pull request. See http://github.com/cython/cython/pull/315.

CC:  @robertwb @vbraun

Upstream: **Completely fixed; Fix reported upstream**

Reviewer: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/16829_





---

archive/issue_events_223039.json:
```json
{
    "actor": "https://github.com/sagetrac-mraum",
    "created_at": "2014-08-14T15:29:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16829",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16829#event-223039"
}
```



---

archive/issue_events_223040.json:
```json
{
    "actor": "https://github.com/sagetrac-mraum",
    "created_at": "2014-08-14T15:29:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16829",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16829#event-223040"
}
```



---

archive/issue_events_223041.json:
```json
{
    "actor": "https://github.com/sagetrac-mraum",
    "created_at": "2014-08-14T15:29:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16829",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16829#event-223041"
}
```



---

archive/issue_events_223042.json:
```json
{
    "actor": "https://github.com/sagetrac-mraum",
    "created_at": "2014-08-14T15:29:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16829",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16829#event-223042"
}
```



---

archive/issue_comments_223726.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nCython has merged 315. I hope this will be part of Cython 0.21, and I suppose then this can be closed as soon as we have upgraded Cython.",
    "created_at": "2014-08-15T06:04:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16829",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16829#issuecomment-223726",
    "user": "https://github.com/sagetrac-mraum"
}
```

<div id="comment:1" align="right">Comment 1</div>

Cython has merged 315. I hope this will be part of Cython 0.21, and I suppose then this can be closed as soon as we have upgraded Cython.



---

archive/issue_comments_223727.json:
```json
{
    "body": "Changed upstream from **Reported upstream. No feedback yet.** to **Fixed upstream, but not in a stable release.**",
    "created_at": "2014-08-15T06:04:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16829",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16829#issuecomment-223727",
    "user": "https://github.com/sagetrac-mraum"
}
```

Changed upstream from **Reported upstream. No feedback yet.** to **Fixed upstream, but not in a stable release.**



---

archive/issue_events_223043.json:
```json
{
    "actor": "https://github.com/sagetrac-mraum",
    "created_at": "2014-08-15T06:04:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16829",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16829#event-223043"
}
```



---

archive/issue_events_223044.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-02-13T20:34:16Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16829",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16829#event-223044"
}
```



---

archive/issue_events_223045.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-02-13T20:34:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16829",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16829#event-223045"
}
```



---

archive/issue_events_223046.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-02-13T20:34:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16829",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16829#event-223046"
}
```



---

archive/issue_comments_223728.json:
```json
{
    "body": "Changed upstream from **Fixed upstream, but not in a stable release.** to **Completely fixed; Fix reported upstream**",
    "created_at": "2015-02-13T20:34:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16829",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16829#issuecomment-223728",
    "user": "https://github.com/jdemeyer"
}
```

Changed upstream from **Fixed upstream, but not in a stable release.** to **Completely fixed; Fix reported upstream**



---

archive/issue_comments_223729.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2015-02-13T20:34:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16829",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16829#issuecomment-223729",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_223047.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-02-13T20:34:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16829",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16829#event-223047"
}
```



---

archive/issue_events_223048.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-02-13T20:34:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16829",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16829#event-223048"
}
```



---

archive/issue_events_223049.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-02-16T23:09:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16829",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16829#event-223049"
}
```



---

archive/issue_events_223050.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-02-16T23:09:27Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16829",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16829#event-223050"
}
```
