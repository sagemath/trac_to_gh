# Issue 16154: paste line continuation

archive/issues_015917.json:
```json
{
    "assignees": [],
    "body": "Pasting new-style line continuations does not work, possibly due to the last IPython update:\n\n```\nsage: sage: a = [1, \n....: ....:      2]\n```\nThis is because IPythons own prompt stripper takes out the first three periods. The patch lets our own prompt stripper run first.\n\nCC:  @ohanar\n\nBranch/Commit: **[u/vbraun/paste_line_continuation](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/paste_line_continuation) @ [b367950](https://github.com/sagemath/sagetrac-mirror/commit/b367950d85a278884c1f0c4806f58f2f423a50c1)**\n\nAuthor: **Volker Braun**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16154_\n\n",
    "closed_at": "2014-04-14T08:38:11Z",
    "created_at": "2014-04-13T21:24:15Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "paste line continuation",
    "type": "issue",
    "updated_at": "2014-04-14T08:38:11Z",
    "url": "https://github.com/sagemath/sage/issues/16154",
    "user": "https://github.com/vbraun"
}
```
Pasting new-style line continuations does not work, possibly due to the last IPython update:

```
sage: sage: a = [1, 
....: ....:      2]
```
This is because IPythons own prompt stripper takes out the first three periods. The patch lets our own prompt stripper run first.

CC:  @ohanar

Branch/Commit: **[u/vbraun/paste_line_continuation](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/paste_line_continuation) @ [b367950](https://github.com/sagemath/sagetrac-mirror/commit/b367950d85a278884c1f0c4806f58f2f423a50c1)**

Author: **Volker Braun**

_Issue created by migration from https://trac.sagemath.org/ticket/16154_





---

archive/issue_events_213116.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-04-13T21:24:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16154",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16154#event-213116"
}
```



---

archive/issue_events_213117.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-04-13T21:24:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16154",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16154#event-213117"
}
```



---

archive/issue_events_213118.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-04-13T21:26:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16154",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16154#event-213118"
}
```



---

archive/issue_comments_208899.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,6 @@\n+Pasting new-style line continuations does not work, possibly due to the last IPython update:\n \n+```\n+sage: sage: a = [1, \n+....: ....:      2]\n+```\n``````\n",
    "created_at": "2014-04-13T21:26:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16154",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16154#issuecomment-208899",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,6 @@
+Pasting new-style line continuations does not work, possibly due to the last IPython update:
 
+```
+sage: sage: a = [1, 
+....: ....:      2]
+```
``````




---

archive/issue_comments_208900.json:
```json
{
    "body": "Branch: **[u/vbraun/paste_line_continuation](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/paste_line_continuation)**",
    "created_at": "2014-04-13T21:36:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16154",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16154#issuecomment-208900",
    "user": "https://github.com/vbraun"
}
```

Branch: **[u/vbraun/paste_line_continuation](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/paste_line_continuation)**



---

archive/issue_comments_208901.json:
```json
{
    "body": "Commit: **[b367950](https://github.com/sagemath/sagetrac-mirror/commit/b367950d85a278884c1f0c4806f58f2f423a50c1)**",
    "created_at": "2014-04-13T21:46:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16154",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16154#issuecomment-208901",
    "user": "https://github.com/vbraun"
}
```

Commit: **[b367950](https://github.com/sagemath/sagetrac-mirror/commit/b367950d85a278884c1f0c4806f58f2f423a50c1)**



---

archive/issue_comments_208902.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nMight be best to deal with this in #16050\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b367950d85a278884c1f0c4806f58f2f423a50c1\">b367950</a></td><td><code>make our own prompt stripper run before the IPython one</code></td></tr></table>\n",
    "created_at": "2014-04-13T21:46:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16154",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16154#issuecomment-208902",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:3" align="right">Comment 3</div>

Might be best to deal with this in #16050

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b367950d85a278884c1f0c4806f58f2f423a50c1">b367950</a></td><td><code>make our own prompt stripper run before the IPython one</code></td></tr></table>




---

archive/issue_events_213119.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-04-13T21:46:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16154",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16154#event-213119"
}
```



---

archive/issue_comments_208903.json:
```json
{
    "body": "Author: **Volker Braun**",
    "created_at": "2014-04-13T21:46:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16154",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16154#issuecomment-208903",
    "user": "https://github.com/vbraun"
}
```

Author: **Volker Braun**



---

archive/issue_comments_208904.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -4,3 +4,4 @@\n sage: sage: a = [1, \n ....: ....:      2]\n ```\n+This is because IPythons own prompt stripper takes out the first three periods. The patch lets our own prompt stripper run first.\n``````\n",
    "created_at": "2014-04-13T21:46:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16154",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16154#issuecomment-208904",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -4,3 +4,4 @@
 sage: sage: a = [1, 
 ....: ....:      2]
 ```
+This is because IPythons own prompt stripper takes out the first three periods. The patch lets our own prompt stripper run first.
``````




---

archive/issue_events_213120.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-04-14T08:38:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16154",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16154#event-213120"
}
```



---

archive/issue_events_213121.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-04-14T08:38:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16154",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16154#event-213121"
}
```



---

archive/issue_events_213122.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-04-14T08:38:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16154",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16154#event-213122"
}
```



---

archive/issue_events_213123.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-04-14T08:38:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16154",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16154#event-213123"
}
```



---

archive/issue_comments_208905.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nIncorporated into #16050",
    "created_at": "2014-04-14T08:38:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16154",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16154#issuecomment-208905",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:4" align="right">Comment 4</div>

Incorporated into #16050
