# Issue 16890: Wrong argument name in class FreeQuadraticModule_submodule_with_basis_field

archive/issues_016653.json:
```json
{
    "assignees": [],
    "body": "There is a wrong argument name in the class FreeQuadraticModule_submodule_with_basis_field in sage/modules/free_quadratic_module.py.\n\nThis code illustrates it.\n\n```\nsage: from sage.modules.free_quadratic_module import FreeQuadraticModule_submodule_with_basis_field\nsage: FreeQuadraticModule_submodule_with_basis_field(ZZ**1,[[1]],inner_product_basis=1)\n...\nNameError: global name 'inner_product_matrix' is not defined\n\n```\n\nKeywords: **FreeQuadraticModule**\n\nBranch/Commit: **[1c098e2](https://github.com/sagemath/sagetrac-mirror/commit/1c098e24989de210f8f72570fba8e38a47268063)**\n\nReviewer: **Marc Masdeu**\n\nAuthor: **Nicol\u00e1s Sirolli**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16890_\n\n",
    "closed_at": "2014-08-28T18:42:00Z",
    "created_at": "2014-08-27T20:48:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Wrong argument name in class FreeQuadraticModule_submodule_with_basis_field",
    "type": "issue",
    "updated_at": "2014-08-28T18:42:00Z",
    "url": "https://github.com/sagemath/sage/issues/16890",
    "user": "https://github.com/nsirolli"
}
```
There is a wrong argument name in the class FreeQuadraticModule_submodule_with_basis_field in sage/modules/free_quadratic_module.py.

This code illustrates it.

```
sage: from sage.modules.free_quadratic_module import FreeQuadraticModule_submodule_with_basis_field
sage: FreeQuadraticModule_submodule_with_basis_field(ZZ**1,[[1]],inner_product_basis=1)
...
NameError: global name 'inner_product_matrix' is not defined

```

Keywords: **FreeQuadraticModule**

Branch/Commit: **[1c098e2](https://github.com/sagemath/sagetrac-mirror/commit/1c098e24989de210f8f72570fba8e38a47268063)**

Reviewer: **Marc Masdeu**

Author: **Nicolás Sirolli**

_Issue created by migration from https://trac.sagemath.org/ticket/16890_





---

archive/issue_events_223840.json:
```json
{
    "actor": "https://github.com/nsirolli",
    "created_at": "2014-08-27T20:48:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16890",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16890#event-223840"
}
```



---

archive/issue_events_223841.json:
```json
{
    "actor": "https://github.com/nsirolli",
    "created_at": "2014-08-27T20:48:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16890",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "component: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16890#event-223841"
}
```



---

archive/issue_events_223842.json:
```json
{
    "actor": "https://github.com/nsirolli",
    "created_at": "2014-08-27T20:48:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16890",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16890#event-223842"
}
```



---

archive/issue_events_223843.json:
```json
{
    "actor": "https://github.com/nsirolli",
    "created_at": "2014-08-27T20:48:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16890",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16890#event-223843"
}
```



---

archive/issue_events_223844.json:
```json
{
    "actor": "https://github.com/nsirolli",
    "created_at": "2014-08-27T21:07:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16890",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16890#event-223844"
}
```



---

archive/issue_comments_225009.json:
```json
{
    "body": "Commit: **[b39aa46](https://github.com/sagemath/sagetrac-mirror/commit/b39aa462bd79adf74741f4794a275a9687c20779)**",
    "created_at": "2014-08-27T21:07:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16890",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16890#issuecomment-225009",
    "user": "https://github.com/nsirolli"
}
```

Commit: **[b39aa46](https://github.com/sagemath/sagetrac-mirror/commit/b39aa462bd79adf74741f4794a275a9687c20779)**



---

archive/issue_comments_225010.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b39aa462bd79adf74741f4794a275a9687c20779\">b39aa46</a></td><td><code>trac #16890 - Fixed wrong argument name in FreeQuadraticModule_submodule_with_basis_field</code></td></tr></table>\n",
    "created_at": "2014-08-27T21:07:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16890",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16890#issuecomment-225010",
    "user": "https://github.com/nsirolli"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b39aa462bd79adf74741f4794a275a9687c20779">b39aa46</a></td><td><code>trac #16890 - Fixed wrong argument name in FreeQuadraticModule_submodule_with_basis_field</code></td></tr></table>




---

archive/issue_comments_225011.json:
```json
{
    "body": "Branch: **[u/nmsirolli/16890](https://github.com/sagemath/sagetrac-mirror/tree/u/nmsirolli/16890)**",
    "created_at": "2014-08-27T21:07:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16890",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16890#issuecomment-225011",
    "user": "https://github.com/nsirolli"
}
```

Branch: **[u/nmsirolli/16890](https://github.com/sagemath/sagetrac-mirror/tree/u/nmsirolli/16890)**



---

archive/issue_comments_225012.json:
```json
{
    "body": "Changed commit from **[b39aa46](https://github.com/sagemath/sagetrac-mirror/commit/b39aa462bd79adf74741f4794a275a9687c20779)** to **[1c098e2](https://github.com/sagemath/sagetrac-mirror/commit/1c098e24989de210f8f72570fba8e38a47268063)**",
    "created_at": "2014-08-27T21:26:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16890",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16890#issuecomment-225012",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[b39aa46](https://github.com/sagemath/sagetrac-mirror/commit/b39aa462bd79adf74741f4794a275a9687c20779)** to **[1c098e2](https://github.com/sagemath/sagetrac-mirror/commit/1c098e24989de210f8f72570fba8e38a47268063)**



---

archive/issue_comments_225013.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1c098e24989de210f8f72570fba8e38a47268063\">1c098e2</a></td><td><code>trac #16890 added doctest</code></td></tr></table>\n",
    "created_at": "2014-08-27T21:26:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16890",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16890#issuecomment-225013",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1c098e24989de210f8f72570fba8e38a47268063">1c098e2</a></td><td><code>trac #16890 added doctest</code></td></tr></table>




---

archive/issue_events_223845.json:
```json
{
    "actor": "https://github.com/mmasdeu",
    "created_at": "2014-08-28T06:14:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16890",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16890#event-223845"
}
```



---

archive/issue_events_223846.json:
```json
{
    "actor": "https://github.com/mmasdeu",
    "created_at": "2014-08-28T06:14:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16890",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16890#event-223846"
}
```



---

archive/issue_comments_225014.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLooks good to me.",
    "created_at": "2014-08-28T06:14:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16890",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16890#issuecomment-225014",
    "user": "https://github.com/mmasdeu"
}
```

<div id="comment:3" align="right">Comment 3</div>

Looks good to me.



---

archive/issue_comments_225015.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nauthor/reviewer name must be filled out and real names.",
    "created_at": "2014-08-28T10:31:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16890",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16890#issuecomment-225015",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:4" align="right">Comment 4</div>

author/reviewer name must be filled out and real names.



---

archive/issue_events_223847.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-08-28T10:31:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16890",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16890#event-223847"
}
```



---

archive/issue_events_223848.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-08-28T10:31:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16890",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16890#event-223848"
}
```



---

archive/issue_comments_225016.json:
```json
{
    "body": "Author: **Nicol\u00e1s Sirolli**",
    "created_at": "2014-08-28T11:41:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16890",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16890#issuecomment-225016",
    "user": "https://github.com/nsirolli"
}
```

Author: **Nicolás Sirolli**



---

archive/issue_comments_225017.json:
```json
{
    "body": "Reviewer: **Marc Masdeu**",
    "created_at": "2014-08-28T13:22:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16890",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16890#issuecomment-225017",
    "user": "https://github.com/mmasdeu"
}
```

Reviewer: **Marc Masdeu**



---

archive/issue_events_223849.json:
```json
{
    "actor": "https://github.com/mmasdeu",
    "created_at": "2014-08-28T13:22:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16890",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16890#event-223849"
}
```



---

archive/issue_events_223850.json:
```json
{
    "actor": "https://github.com/mmasdeu",
    "created_at": "2014-08-28T13:22:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16890",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16890#event-223850"
}
```



---

archive/issue_events_223851.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-08-28T13:23:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16890",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16890#event-223851"
}
```



---

archive/issue_events_223852.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-08-28T13:23:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16890",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16890#event-223852"
}
```



---

archive/issue_events_223853.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-08-28T18:42:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16890",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16890#event-223853"
}
```



---

archive/issue_events_223854.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "95c103088ad063649cd36967e5ae1b07b9f4ffe4",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-08-28T18:42:00Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16890",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16890#event-223854"
}
```



---

archive/issue_comments_225018.json:
```json
{
    "body": "Changed branch from **[u/nmsirolli/16890](https://github.com/sagemath/sagetrac-mirror/tree/u/nmsirolli/16890)** to **[1c098e2](https://github.com/sagemath/sagetrac-mirror/commit/1c098e24989de210f8f72570fba8e38a47268063)**",
    "created_at": "2014-08-28T18:42:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16890",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16890#issuecomment-225018",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/nmsirolli/16890](https://github.com/sagemath/sagetrac-mirror/tree/u/nmsirolli/16890)** to **[1c098e2](https://github.com/sagemath/sagetrac-mirror/commit/1c098e24989de210f8f72570fba8e38a47268063)**
