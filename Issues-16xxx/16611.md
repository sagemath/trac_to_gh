...
```json
{
    "body": "Not sure why the changes in the doctests.  Shouldn't that command still work, perhaps with a minor tweak?  Not that it really matters so much, but I hope it isn't a symptom of something else.",
...
```json
{
    "body": "Patches look good. I guess the source tarball is incorrect (capital-R). Please fix.",
    "created_at": "2014-07-06T00:14:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16611",
    "type": "issue_comment",
...
```json
{
    "body": "The spkg-src file doesn't have any strict rules. Let me ask again, is it ok to have the tarball filename with a capital R? The tarball in the description has a capital R. If thats wrong then you need to provide a tarball with the correct name.",
...
```json
{
    "body": "The tarball must match whats in `checksums.ini` including case and bz2 vs gz extension. Current branch doesn't work.",
...
```json
{
    "body": "I.e. if you put the R tarball into your upstream/ folder and run `sage -f r` then it needs to work. On a case-sensitive filesystem.",
...
```json
{
    "body": "It seems that, at least for *this* package, sage-fix-pkg-checksums creates the wrong \"tarball=\" line in checksum.ini\n\nThe 0f89d3e... commit has a hand-edited checksum.ini.\n\nShould we open a ticket against sage-fix-pkg-checksums ?",
...
```json
{
    "body": "IMHO the easiest solution is to lower-case the tarball. Which would then also match the name of the package. There are various annoyances with how we handle tarball names.",
...
```json
{
    "body": "IMHO our strategic goal ought to get rid of the current tarball handling, beautifying sage-fix-pkg-checksums is pretty low on my list of priorities.\n\nThis ticket needs a lower-case r tarball.",
