# Issue 16668: Enhance FiniteStateMachine.is_Markov_chain to handle symbolic probabilities

archive/issues_016431.json:
```json
{
    "assignees": [],
    "body": "When the probabilities of a Markov chain are variables of a polynomial ring, they may not sum up to one without additional information. Therefore, we now provide a parameter `is_zero`.\n\nCC:  skropf @dkrenn\n\nKeywords: **finite_state_machine, Markov chain**\n\nBranch/Commit: **[71246e4](https://github.com/sagemath/sagetrac-mirror/commit/71246e4429e7454d81ba32217da0a6300b032e45)**\n\nReviewer: **Daniel Krenn**\n\nAuthor: **Clemens Heuberger**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16668_\n\n",
    "closed_at": "2014-07-19T04:57:46Z",
    "created_at": "2014-07-17T11:38:44Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20finite%20state%20machines",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Enhance FiniteStateMachine.is_Markov_chain to handle symbolic probabilities",
    "type": "issue",
    "updated_at": "2014-07-19T04:57:46Z",
    "url": "https://github.com/sagemath/sage/issues/16668",
    "user": "https://github.com/cheuberg"
}
```
When the probabilities of a Markov chain are variables of a polynomial ring, they may not sum up to one without additional information. Therefore, we now provide a parameter `is_zero`.

CC:  skropf @dkrenn

Keywords: **finite_state_machine, Markov chain**

Branch/Commit: **[71246e4](https://github.com/sagemath/sagetrac-mirror/commit/71246e4429e7454d81ba32217da0a6300b032e45)**

Reviewer: **Daniel Krenn**

Author: **Clemens Heuberger**

_Issue created by migration from https://trac.sagemath.org/ticket/16668_





---

archive/issue_events_220705.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-07-17T11:38:44Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16668",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16668#event-220705"
}
```



---

archive/issue_events_220706.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-07-17T11:38:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16668",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20finite%20state%20machines",
    "label_color": "0000ff",
    "label_name": "component: finite state machines",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16668#event-220706"
}
```



---

archive/issue_events_220707.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-07-17T11:38:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16668",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16668#event-220707"
}
```



---

archive/issue_events_220708.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-07-17T11:38:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16668",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16668#event-220708"
}
```



---

archive/issue_events_220709.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-07-17T11:39:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16668",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16668#event-220709"
}
```



---

archive/issue_comments_220213.json:
```json
{
    "body": "Changed branch from **[u/cheuberg/fsm/is_markov_chain_extended](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/is_markov_chain_extended)** to **[u/dkrenn/fsm/is_markov_chain_extended](https://github.com/sagemath/sagetrac-mirror/tree/u/dkrenn/fsm/is_markov_chain_extended)**",
    "created_at": "2014-07-18T10:19:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16668",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16668#issuecomment-220213",
    "user": "https://github.com/dkrenn"
}
```

Changed branch from **[u/cheuberg/fsm/is_markov_chain_extended](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/is_markov_chain_extended)** to **[u/dkrenn/fsm/is_markov_chain_extended](https://github.com/sagemath/sagetrac-mirror/tree/u/dkrenn/fsm/is_markov_chain_extended)**



---

archive/issue_comments_220214.json:
```json
{
    "body": "Changed commit from **[ef5ab0e](https://github.com/sagemath/sagetrac-mirror/commit/ef5ab0e906bbfc71421e3a6b6f0319e35d473fa1)** to **[71246e4](https://github.com/sagemath/sagetrac-mirror/commit/71246e4429e7454d81ba32217da0a6300b032e45)**",
    "created_at": "2014-07-18T10:22:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16668",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16668#issuecomment-220214",
    "user": "https://github.com/dkrenn"
}
```

Changed commit from **[ef5ab0e](https://github.com/sagemath/sagetrac-mirror/commit/ef5ab0e906bbfc71421e3a6b6f0319e35d473fa1)** to **[71246e4](https://github.com/sagemath/sagetrac-mirror/commit/71246e4429e7454d81ba32217da0a6300b032e45)**



---

archive/issue_comments_220215.json:
```json
{
    "body": "Reviewer: **Daniel Krenn**",
    "created_at": "2014-07-18T10:22:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16668",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16668#issuecomment-220215",
    "user": "https://github.com/dkrenn"
}
```

Reviewer: **Daniel Krenn**



---

archive/issue_comments_220216.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nPatch is fine, tests pass. Corrected a `==` to a `=` in the docu. Please cross-review and set to positive if okay for you.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/71246e4429e7454d81ba32217da0a6300b032e45\">71246e4</a></td><td><code>in doc of input-parameter: is_zero=None (= instead of ==)</code></td></tr></table>\n",
    "created_at": "2014-07-18T10:22:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16668",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16668#issuecomment-220216",
    "user": "https://github.com/dkrenn"
}
```

<a id='comment:3'>Comment 3:</a>
Patch is fine, tests pass. Corrected a `==` to a `=` in the docu. Please cross-review and set to positive if okay for you.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/71246e4429e7454d81ba32217da0a6300b032e45">71246e4</a></td><td><code>in doc of input-parameter: is_zero=None (= instead of ==)</code></td></tr></table>




---

archive/issue_events_220710.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-07-18T10:34:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16668",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16668#event-220710"
}
```



---

archive/issue_events_220711.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-07-18T10:34:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16668",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16668#event-220711"
}
```



---

archive/issue_comments_220217.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nfine for me.",
    "created_at": "2014-07-18T10:34:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16668",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16668#issuecomment-220217",
    "user": "https://github.com/cheuberg"
}
```

<a id='comment:4'>Comment 4:</a>
fine for me.



---

archive/issue_events_220712.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-07-19T04:57:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16668",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16668#event-220712"
}
```



---

archive/issue_events_220713.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "b5dd91bc2954fd8c836d31a7d7194182e0101a92",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-07-19T04:57:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16668",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16668#event-220713"
}
```



---

archive/issue_comments_220218.json:
```json
{
    "body": "Changed branch from **[u/dkrenn/fsm/is_markov_chain_extended](https://github.com/sagemath/sagetrac-mirror/tree/u/dkrenn/fsm/is_markov_chain_extended)** to **[71246e4](https://github.com/sagemath/sagetrac-mirror/commit/71246e4429e7454d81ba32217da0a6300b032e45)**",
    "created_at": "2014-07-19T04:57:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16668",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16668#issuecomment-220218",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/dkrenn/fsm/is_markov_chain_extended](https://github.com/sagemath/sagetrac-mirror/tree/u/dkrenn/fsm/is_markov_chain_extended)** to **[71246e4](https://github.com/sagemath/sagetrac-mirror/commit/71246e4429e7454d81ba32217da0a6300b032e45)**
