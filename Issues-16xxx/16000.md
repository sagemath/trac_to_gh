# Issue 16000: Refactor expression_conversion.py:InterfaceInit.derivative()

archive/issues_015763.json:
```json
{
    "body": "In `src/sage/symbolic/expression_conversions.py`, the code\nfor the `derivative` method of the `InterfaceInit` class starts\nwith [this comment](https://github.com/sagemath/sage/blob/9.3.rc1/src/sage/symbolic/expression_conversions.py#L586):\n\n```\n        #This code should probably be moved into the interface\n        #object in a nice way.\n```\n\n\n**CC:**  @slel\n\n**Status:** new\n\nIssue created by migration from https://trac.sagemath.org/ticket/16000\n\n",
    "created_at": "2014-03-23T09:25:17Z",
    "labels": [
        "component: interfaces",
        "minor",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-wishlist",
    "title": "Refactor expression_conversion.py:InterfaceInit.derivative()",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/16000",
    "user": "https://github.com/rwst"
}
```
In `src/sage/symbolic/expression_conversions.py`, the code
for the `derivative` method of the `InterfaceInit` class starts
with [this comment](https://github.com/sagemath/sage/blob/9.3.rc1/src/sage/symbolic/expression_conversions.py#L586):

```
        #This code should probably be moved into the interface
        #object in a nice way.
```


**CC:**  @slel

**Status:** new

Issue created by migration from https://trac.sagemath.org/ticket/16000





---

archive/issue_events_053212.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16000",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16000#event-53212"
}
```



---

archive/issue_events_053213.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16000",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16000#event-53213"
}
```



---

archive/issue_events_053214.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16000",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16000#event-53214"
}
```



---

archive/issue_comments_249217.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,4 +1,6 @@\n-In `expression_conversion.py:494`\n+In `src/sage/symbolic/expression_conversions.py`, the code\n+for the `derivative` method of the `InterfaceInit` class starts\n+with [this comment](https://github.com/sagemath/sage/blob/9.3.rc1/src/sage/symbolic/expression_conversions.py#L586):\n \n ```\n         #This code should probably be moved into the interface\n``````\n",
    "created_at": "2021-04-03T10:38:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16000",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16000#issuecomment-249217",
    "user": "https://github.com/slel"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,4 +1,6 @@
-In `expression_conversion.py:494`
+In `src/sage/symbolic/expression_conversions.py`, the code
+for the `derivative` method of the `InterfaceInit` class starts
+with [this comment](https://github.com/sagemath/sage/blob/9.3.rc1/src/sage/symbolic/expression_conversions.py#L586):
 
 ```
         #This code should probably be moved into the interface
``````




---

archive/issue_comments_249218.json:
```json
{
    "body": "<a id='comment:4'></a>\nCould you elaborate on what would need to be done here?",
    "created_at": "2021-04-03T10:38:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16000",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16000#issuecomment-249218",
    "user": "https://github.com/slel"
}
```

<a id='comment:4'></a>
Could you elaborate on what would need to be done here?



---

archive/issue_events_053215.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/16000",
    "rename": {
        "from": "refactoring of expression_conversion.py:InterfaceInit.derivative()",
        "to": "Refactor expression_conversion.py:InterfaceInit.derivative()"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16000#event-53215"
}
```



---

archive/issue_events_053216.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-04-03T10:38:46Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16000",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16000#event-53216"
}
```



---

archive/issue_events_053217.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-04-03T10:38:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16000",
    "milestone": "sage-wishlist",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16000#event-53217"
}
```
