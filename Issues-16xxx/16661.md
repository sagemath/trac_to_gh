# Issue 16661: New common transducer Wait

archive/issues_016424.json:
```json
{
    "assignees": [],
    "body": "Introduction of a new transducer `Wait` which writes `False` until reading the threshold-th occurrence of a true input letter and which then writes `True`.\n\nThis might be useful together with the subblock counting transducer.\n\nCC:  @sagetrac-skropf @dkrenn\n\nKeywords: **finite_state_machine**\n\nBranch/Commit: **[737041a](https://github.com/sagemath/sagetrac-mirror/commit/737041aacd4d8df0a6f7965e72c11a192275407a)**\n\nReviewer: **Daniel Krenn**\n\nAuthor: **Clemens Heuberger**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16661_\n\n",
    "closed_at": "2014-07-19T04:57:48Z",
    "created_at": "2014-07-16T11:29:44Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20finite%20state%20machines",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "New common transducer Wait",
    "type": "issue",
    "updated_at": "2014-07-19T04:57:48Z",
    "url": "https://github.com/sagemath/sage/issues/16661",
    "user": "https://github.com/cheuberg"
}
```
Introduction of a new transducer `Wait` which writes `False` until reading the threshold-th occurrence of a true input letter and which then writes `True`.

This might be useful together with the subblock counting transducer.

CC:  @sagetrac-skropf @dkrenn

Keywords: **finite_state_machine**

Branch/Commit: **[737041a](https://github.com/sagemath/sagetrac-mirror/commit/737041aacd4d8df0a6f7965e72c11a192275407a)**

Reviewer: **Daniel Krenn**

Author: **Clemens Heuberger**

_Issue created by migration from https://trac.sagemath.org/ticket/16661_





---

archive/issue_events_220609.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-07-16T11:29:44Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16661",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16661#event-220609"
}
```



---

archive/issue_events_220610.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-07-16T11:29:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16661",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20finite%20state%20machines",
    "label_color": "0000ff",
    "label_name": "component: finite state machines",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16661#event-220610"
}
```



---

archive/issue_events_220611.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-07-16T11:29:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16661",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16661#event-220611"
}
```



---

archive/issue_events_220612.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-07-16T11:29:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16661",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16661#event-220612"
}
```



---

archive/issue_events_220613.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-07-16T11:29:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16661",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16661#event-220613"
}
```



---

archive/issue_comments_219991.json:
```json
{
    "body": "Changed branch from **[u/cheuberg/fsm/generator-wait-threshold](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/generator-wait-threshold)** to **[u/dkrenn/fsm/generator-wait-threshold](https://github.com/sagemath/sagetrac-mirror/tree/u/dkrenn/fsm/generator-wait-threshold)**",
    "created_at": "2014-07-18T09:31:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16661",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16661#issuecomment-219991",
    "user": "https://github.com/dkrenn"
}
```

Changed branch from **[u/cheuberg/fsm/generator-wait-threshold](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/generator-wait-threshold)** to **[u/dkrenn/fsm/generator-wait-threshold](https://github.com/sagemath/sagetrac-mirror/tree/u/dkrenn/fsm/generator-wait-threshold)**



---

archive/issue_events_220614.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2014-07-18T09:32:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16661",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16661#event-220614"
}
```



---

archive/issue_events_220615.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2014-07-18T09:32:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16661",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16661#event-220615"
}
```



---

archive/issue_comments_219992.json:
```json
{
    "body": "Reviewer: **Daniel Krenn**",
    "created_at": "2014-07-18T09:32:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16661",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16661#issuecomment-219992",
    "user": "https://github.com/dkrenn"
}
```

Reviewer: **Daniel Krenn**



---

archive/issue_comments_219993.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nMade very small/minor changes. Code and Docs look fine; tests pass.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/737041aacd4d8df0a6f7965e72c11a192275407a\">737041a</a></td><td><code>one Type, one ``...```, one PEP8</code></td></tr></table>\n",
    "created_at": "2014-07-18T09:32:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16661",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16661#issuecomment-219993",
    "user": "https://github.com/dkrenn"
}
```

<div id="comment:3" align="right">Comment 3</div>

Made very small/minor changes. Code and Docs look fine; tests pass.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/737041aacd4d8df0a6f7965e72c11a192275407a">737041a</a></td><td><code>one Type, one ``...```, one PEP8</code></td></tr></table>




---

archive/issue_comments_219994.json:
```json
{
    "body": "Changed commit from **[44321ff](https://github.com/sagemath/sagetrac-mirror/commit/44321ffcf5f2017adb12c306741e8062aa0da6e1)** to **[737041a](https://github.com/sagemath/sagetrac-mirror/commit/737041aacd4d8df0a6f7965e72c11a192275407a)**",
    "created_at": "2014-07-18T09:32:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16661",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16661#issuecomment-219994",
    "user": "https://github.com/dkrenn"
}
```

Changed commit from **[44321ff](https://github.com/sagemath/sagetrac-mirror/commit/44321ffcf5f2017adb12c306741e8062aa0da6e1)** to **[737041a](https://github.com/sagemath/sagetrac-mirror/commit/737041aacd4d8df0a6f7965e72c11a192275407a)**



---

archive/issue_comments_219995.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nIn the description of 737041a there is a typo in \"Type\" ;)",
    "created_at": "2014-07-18T09:33:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16661",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16661#issuecomment-219995",
    "user": "https://github.com/dkrenn"
}
```

<div id="comment:4" align="right">Comment 4</div>

In the description of 737041a there is a typo in "Type" ;)



---

archive/issue_events_220616.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-07-19T04:57:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16661",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16661#event-220616"
}
```



---

archive/issue_events_220617.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "5491e620efc9c7806ab2e51a60873f8da609ddfa",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-07-19T04:57:48Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16661",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16661#event-220617"
}
```



---

archive/issue_comments_219996.json:
```json
{
    "body": "Changed branch from **[u/dkrenn/fsm/generator-wait-threshold](https://github.com/sagemath/sagetrac-mirror/tree/u/dkrenn/fsm/generator-wait-threshold)** to **[737041a](https://github.com/sagemath/sagetrac-mirror/commit/737041aacd4d8df0a6f7965e72c11a192275407a)**",
    "created_at": "2014-07-19T04:57:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16661",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16661#issuecomment-219996",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/dkrenn/fsm/generator-wait-threshold](https://github.com/sagemath/sagetrac-mirror/tree/u/dkrenn/fsm/generator-wait-threshold)** to **[737041a](https://github.com/sagemath/sagetrac-mirror/commit/737041aacd4d8df0a6f7965e72c11a192275407a)**
