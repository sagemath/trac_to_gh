# Issue 16988: Implement type D_4^{(3)} rigged configuration bijection

archive/issues_016751.json:
```json
{
    "body": "Adds the type D<sub>4</sub><sup>(3)</sup> bijection between rigged configurations and tensor products of KR tableaux and the resulting KR tableaux crystal.\n\nAssignee: sage-combinat\n\nCC:  sage-combinat @anneschilling @bsalisbury1\n\nKeywords: rigged configurations, KR tableaux\n\nBranch/Commit: 500628b8fa311267c900ec34126762e5d029d038\n\nReviewer: Ben Salisbury\n\nAuthor: Travis Scrimshaw\n\nDependencies: #16876 #16974\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/16988\n\n",
    "closed_at": "2015-10-29T16:35:04Z",
    "created_at": "2014-09-15T16:20:01Z",
    "labels": [
        "component: combinatorics"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.10",
    "title": "Implement type D_4^{(3)} rigged configuration bijection",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/16988",
    "user": "https://github.com/tscrim"
}
```
Adds the type D<sub>4</sub><sup>(3)</sup> bijection between rigged configurations and tensor products of KR tableaux and the resulting KR tableaux crystal.

Assignee: sage-combinat

CC:  sage-combinat @anneschilling @bsalisbury1

Keywords: rigged configurations, KR tableaux

Branch/Commit: 500628b8fa311267c900ec34126762e5d029d038

Reviewer: Ben Salisbury

Author: Travis Scrimshaw

Dependencies: #16876 #16974

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/16988





---

archive/issue_comments_285218.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2014-09-15T16:21:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285218",
    "user": "https://github.com/tscrim"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_285219.json:
```json
{
    "body": "Changing commit from \"\" to \"c267f88ed12beef7e56823e6ee62c946ef68ec68\"",
    "created_at": "2014-09-15T16:21:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285219",
    "user": "https://github.com/tscrim"
}
```

Changing commit from "" to "c267f88ed12beef7e56823e6ee62c946ef68ec68"



---

archive/issue_comments_285220.json:
```json
{
    "body": "Changing branch from \"\" to \"public/rigged_configurations/type_D_tri_bijection-16988\"",
    "created_at": "2014-09-15T16:21:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285220",
    "user": "https://github.com/tscrim"
}
```

Changing branch from "" to "public/rigged_configurations/type_D_tri_bijection-16988"



---

archive/issue_comments_285221.json:
```json
{
    "body": "<a id='comment:1'></a>Last 10 new commits:",
    "created_at": "2014-09-15T16:21:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285221",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:1'></a>Last 10 new commits:



---

archive/issue_comments_285222.json:
```json
{
    "body": "<a id='comment:2'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2015-03-17T18:42:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285222",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:2'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_285223.json:
```json
{
    "body": "Changing commit from \"c267f88ed12beef7e56823e6ee62c946ef68ec68\" to \"3c1a42eac22e99894eaac585aa67b722b82c4acd\"",
    "created_at": "2015-03-17T18:42:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285223",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "c267f88ed12beef7e56823e6ee62c946ef68ec68" to "3c1a42eac22e99894eaac585aa67b722b82c4acd"



---

archive/issue_comments_285224.json:
```json
{
    "body": "Changing commit from \"3c1a42eac22e99894eaac585aa67b722b82c4acd\" to \"8053f0c833809ff45347aadf7e66d93035064488\"",
    "created_at": "2015-05-13T15:47:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285224",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "3c1a42eac22e99894eaac585aa67b722b82c4acd" to "8053f0c833809ff45347aadf7e66d93035064488"



---

archive/issue_comments_285225.json:
```json
{
    "body": "<a id='comment:3'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2015-05-13T15:47:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285225",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:3'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_285226.json:
```json
{
    "body": "Changing commit from \"8053f0c833809ff45347aadf7e66d93035064488\" to \"21a142bd6727a1642aa14b135a82273481a59ac6\"",
    "created_at": "2015-06-15T20:54:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285226",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "8053f0c833809ff45347aadf7e66d93035064488" to "21a142bd6727a1642aa14b135a82273481a59ac6"



---

archive/issue_comments_285227.json:
```json
{
    "body": "<a id='comment:4'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2015-06-15T20:54:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285227",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:4'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_285228.json:
```json
{
    "body": "Changing commit from \"21a142bd6727a1642aa14b135a82273481a59ac6\" to \"95c661149f175ea28aec55ed101f16f4603c2b8d\"",
    "created_at": "2015-07-13T10:32:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285228",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "21a142bd6727a1642aa14b135a82273481a59ac6" to "95c661149f175ea28aec55ed101f16f4603c2b8d"



---

archive/issue_comments_285229.json:
```json
{
    "body": "<a id='comment:5'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2015-07-13T10:32:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285229",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:5'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_events_049497.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-09-02T14:44:57Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "milestone": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16988#event-49497"
}
```



---

archive/issue_comments_285230.json:
```json
{
    "body": "<a id='comment:8'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2015-10-22T21:00:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285230",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:8'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_285231.json:
```json
{
    "body": "Changing commit from \"95c661149f175ea28aec55ed101f16f4603c2b8d\" to \"68a8208d9def8674dfe9711495f171b98349dbdc\"",
    "created_at": "2015-10-22T21:00:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285231",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "95c661149f175ea28aec55ed101f16f4603c2b8d" to "68a8208d9def8674dfe9711495f171b98349dbdc"



---

archive/issue_comments_285232.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Ben Salisbury\"",
    "created_at": "2015-10-22T21:01:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285232",
    "user": "https://github.com/bsalisbury1"
}
```

Changing reviewer from "" to "Ben Salisbury"



---

archive/issue_comments_285233.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2015-10-22T21:01:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285233",
    "user": "https://github.com/bsalisbury1"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_285234.json:
```json
{
    "body": "<a id='comment:9'></a>Thanks, Travis!",
    "created_at": "2015-10-22T21:01:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285234",
    "user": "https://github.com/bsalisbury1"
}
```

<a id='comment:9'></a>Thanks, Travis!



---

archive/issue_events_049498.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-10-22T21:01:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "milestone": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16988#event-49498"
}
```



---

archive/issue_events_049499.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-10-22T21:01:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "milestone": "sage-6.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16988#event-49499"
}
```



---

archive/issue_comments_285235.json:
```json
{
    "body": "<a id='comment:10'></a>Thanks for doing the review!",
    "created_at": "2015-10-22T21:01:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285235",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:10'></a>Thanks for doing the review!



---

archive/issue_comments_285236.json:
```json
{
    "body": "Changing branch from \"public/rigged_configurations/type_D_tri_bijection-16988\" to \"68a8208d9def8674dfe9711495f171b98349dbdc\"",
    "created_at": "2015-10-23T17:28:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285236",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "public/rigged_configurations/type_D_tri_bijection-16988" to "68a8208d9def8674dfe9711495f171b98349dbdc"



---

archive/issue_events_049500.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-10-23T17:28:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16988#event-49500"
}
```



---

archive/issue_comments_285237.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2015-10-23T17:28:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285237",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_comments_285238.json:
```json
{
    "body": "Resolution changed from fixed to ",
    "created_at": "2015-10-24T07:54:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285238",
    "user": "https://github.com/vbraun"
}
```

Resolution changed from fixed to 



---

archive/issue_comments_285239.json:
```json
{
    "body": "Changing status from closed to new.",
    "created_at": "2015-10-24T07:54:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285239",
    "user": "https://github.com/vbraun"
}
```

Changing status from closed to new.



---

archive/issue_events_049501.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-10-24T07:54:11Z",
    "event": "reopened",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16988#event-49501"
}
```



---

archive/issue_comments_285240.json:
```json
{
    "body": "<a id='comment:12'></a>I'm getting random failures that point to this ticket:\n\n```\nsage -t --long src/sage/combinat/rigged_configurations/kr_tableaux.py\n**********************************************************************\nFile \"src/sage/combinat/rigged_configurations/kr_tableaux.py\", line 1671, in sage.combinat.rigged_configurations.kr_tableaux.KRTableauxTypeDTri2Element.e\nFailed example:\n    KRT.module_generators[0].e(0)\nException raised:\n    Traceback (most recent call last):\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/doctest/forker.py\", line 496, in _run\n        self.compile_and_execute(example, compiler, test.globs)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/doctest/forker.py\", line 858, in compile_and_execute\n        exec(compiled, globs)\n      File \"<doctest sage.combinat.rigged_configurations.kr_tableaux.KRTableauxTypeDTri2Element.e[1]>\", line 1, in <module>\n        KRT.module_generators[Integer(0)].e(Integer(0))\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py\", line 1680, in e\n        ret = ret.to_virtual_configuration().e(0)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/rigged_configuration_element.py\", line 1327, in e\n        ret = self.to_tensor_product_of_kirillov_reshetikhin_tableaux().e(a)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/tensor_product.py\", line 1258, in e\n        position = self.positions_of_unmatched_plus(i)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/tensor_product.py\", line 1400, in positions_of_unmatched_plus\n        l = self.positions_of_unmatched_minus(i, dual=True, reverse=True)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/tensor_product.py\", line 1380, in positions_of_unmatched_minus\n        plus = self[j].epsilon(i)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py\", line 1532, in epsilon\n        return KirillovReshetikhinTableauxElement.epsilon(self, i)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py\", line 1373, in epsilon\n        return self.to_kirillov_reshetikhin_crystal().epsilon0()\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py\", line 1160, in to_kirillov_reshetikhin_crystal\n        return self.parent().kirillov_reshetikhin_crystal()(self)\n      File \"sage/structure/parent.pyx\", line 1098, in sage.structure.parent.Parent.__call__ (/Users/buildslave-sage/slave/sage_git/build/src/build/cythonized/sage/structure/parent.c:9821)\n        return mor._call_(x)\n      File \"sage/structure/coerce_maps.pyx\", line 109, in sage.structure.coerce_maps.DefaultConvertMap_unique._call_ (/Users/buildslave-sage/slave/sage_git/build/src/build/cythonized/sage/structure/coerce_maps.c:4543)\n        raise\n      File \"sage/structure/coerce_maps.pyx\", line 104, in sage.structure.coerce_maps.DefaultConvertMap_unique._call_ (/Users/buildslave-sage/slave/sage_git/build/src/build/cythonized/sage/structure/coerce_maps.c:4436)\n        return C._element_constructor(x)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/kirillov_reshetikhin.py\", line 3024, in _element_constructor_\n        return self.module_generator().f_string(f_str)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/kirillov_reshetikhin.py\", line 543, in module_generator\n        return [ b for b in self.module_generators if b.weight() == weight][0]\n    IndexError: list index out of range\n**********************************************************************\nFile \"src/sage/combinat/rigged_configurations/kr_tableaux.py\", line 1699, in sage.combinat.rigged_configurations.kr_tableaux.KRTableauxTypeDTri2Element.f\nFailed example:\n    KRT.module_generators[0].f(0)\nException raised:\n    Traceback (most recent call last):\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/doctest/forker.py\", line 496, in _run\n        self.compile_and_execute(example, compiler, test.globs)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/doctest/forker.py\", line 858, in compile_and_execute\n        exec(compiled, globs)\n      File \"<doctest sage.combinat.rigged_configurations.kr_tableaux.KRTableauxTypeDTri2Element.f[1]>\", line 1, in <module>\n        KRT.module_generators[Integer(0)].f(Integer(0))\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py\", line 1709, in f\n        ret = ret.to_virtual_configuration().f(0)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/rigged_configuration_element.py\", line 1383, in f\n        ret = self.to_tensor_product_of_kirillov_reshetikhin_tableaux().f(a)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/tensor_product.py\", line 1297, in f\n        position = self.positions_of_unmatched_minus(i)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/tensor_product.py\", line 1371, in positions_of_unmatched_minus\n        minus = self[j].phi(i)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py\", line 1549, in phi\n        return KirillovReshetikhinTableauxElement.phi(self, i)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py\", line 1391, in phi\n        return self.to_kirillov_reshetikhin_crystal().phi0()\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py\", line 1160, in to_kirillov_reshetikhin_crystal\n        return self.parent().kirillov_reshetikhin_crystal()(self)\n      File \"sage/structure/parent.pyx\", line 1098, in sage.structure.parent.Parent.__call__ (/Users/buildslave-sage/slave/sage_git/build/src/build/cythonized/sage/structure/parent.c:9821)\n        return mor._call_(x)\n      File \"sage/structure/coerce_maps.pyx\", line 109, in sage.structure.coerce_maps.DefaultConvertMap_unique._call_ (/Users/buildslave-sage/slave/sage_git/build/src/build/cythonized/sage/structure/coerce_maps.c:4543)\n        raise\n      File \"sage/structure/coerce_maps.pyx\", line 104, in sage.structure.coerce_maps.DefaultConvertMap_unique._call_ (/Users/buildslave-sage/slave/sage_git/build/src/build/cythonized/sage/structure/coerce_maps.c:4436)\n        return C._element_constructor(x)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/kirillov_reshetikhin.py\", line 3024, in _element_constructor_\n        return self.module_generator().f_string(f_str)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/kirillov_reshetikhin.py\", line 543, in module_generator\n        return [ b for b in self.module_generators if b.weight() == weight][0]\n    IndexError: list index out of range\n**********************************************************************\nFile \"src/sage/combinat/rigged_configurations/kr_tableaux.py\", line 1700, in sage.combinat.rigged_configurations.kr_tableaux.KRTableauxTypeDTri2Element.f\nFailed example:\n    KRT.module_generators[3].f(0)\nException raised:\n    Traceback (most recent call last):\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/doctest/forker.py\", line 496, in _run\n        self.compile_and_execute(example, compiler, test.globs)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/doctest/forker.py\", line 858, in compile_and_execute\n        exec(compiled, globs)\n      File \"<doctest sage.combinat.rigged_configurations.kr_tableaux.KRTableauxTypeDTri2Element.f[2]>\", line 1, in <module>\n        KRT.module_generators[Integer(3)].f(Integer(0))\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py\", line 1709, in f\n        ret = ret.to_virtual_configuration().f(0)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/rigged_configuration_element.py\", line 1383, in f\n        ret = self.to_tensor_product_of_kirillov_reshetikhin_tableaux().f(a)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/tensor_product.py\", line 1297, in f\n        position = self.positions_of_unmatched_minus(i)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/tensor_product.py\", line 1371, in positions_of_unmatched_minus\n        minus = self[j].phi(i)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py\", line 1549, in phi\n        return KirillovReshetikhinTableauxElement.phi(self, i)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py\", line 1391, in phi\n        return self.to_kirillov_reshetikhin_crystal().phi0()\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py\", line 1160, in to_kirillov_reshetikhin_crystal\n        return self.parent().kirillov_reshetikhin_crystal()(self)\n      File \"sage/structure/parent.pyx\", line 1098, in sage.structure.parent.Parent.__call__ (/Users/buildslave-sage/slave/sage_git/build/src/build/cythonized/sage/structure/parent.c:9821)\n        return mor._call_(x)\n      File \"sage/structure/coerce_maps.pyx\", line 109, in sage.structure.coerce_maps.DefaultConvertMap_unique._call_ (/Users/buildslave-sage/slave/sage_git/build/src/build/cythonized/sage/structure/coerce_maps.c:4543)\n        raise\n      File \"sage/structure/coerce_maps.pyx\", line 104, in sage.structure.coerce_maps.DefaultConvertMap_unique._call_ (/Users/buildslave-sage/slave/sage_git/build/src/build/cythonized/sage/structure/coerce_maps.c:4436)\n        return C._element_constructor(x)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/kirillov_reshetikhin.py\", line 3024, in _element_constructor_\n        return self.module_generator().f_string(f_str)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/kirillov_reshetikhin.py\", line 543, in module_generator\n        return [ b for b in self.module_generators if b.weight() == weight][0]\n    IndexError: list index out of range\n**********************************************************************\nFile \"src/sage/combinat/rigged_configurations/kr_tableaux.py\", line 1778, in sage.combinat.rigged_configurations.kr_tableaux.KRTableauxTypeDTri2.__init__\nFailed example:\n    TestSuite(KRT).run() # long time\nExpected nothing\nGot:\n      Failure in _test_stembridge_local_axioms:\n      Traceback (most recent call last):\n        File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/misc/sage_unittest.py\", line 283, in run\n          test_method(tester = tester)\n        File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/categories/regular_crystals.py\", line 760, in _test_stembridge_local_axioms\n          if self.e(i) is not None and self.e(j) is not None:\n        File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py\", line 1680, in e\n          ret = ret.to_virtual_configuration().e(0)\n        File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/rigged_configuration_element.py\", line 1327, in e\n          ret = self.to_tensor_product_of_kirillov_reshetikhin_tableaux().e(a)\n        File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/tensor_product.py\", line 1258, in e\n          position = self.positions_of_unmatched_plus(i)\n        File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/tensor_product.py\", line 1400, in positions_of_unmatched_plus\n          l = self.positions_of_unmatched_minus(i, dual=True, reverse=True)\n        File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/tensor_product.py\", line 1380, in positions_of_unmatched_minus\n          plus = self[j].epsilon(i)\n        File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py\", line 1532, in epsilon\n          return KirillovReshetikhinTableauxElement.epsilon(self, i)\n        File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py\", line 1373, in epsilon\n          return self.to_kirillov_reshetikhin_crystal().epsilon0()\n        File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py\", line 1160, in to_kirillov_reshetikhin_crystal\n          return self.parent().kirillov_reshetikhin_crystal()(self)\n        File \"sage/structure/parent.pyx\", line 1098, in sage.structure.parent.Parent.__call__ (/Users/buildslave-sage/slave/sage_git/build/src/build/cythonized/sage/structure/parent.c:9821)\n          return mor._call_(x)\n        File \"sage/structure/coerce_maps.pyx\", line 109, in sage.structure.coerce_maps.DefaultConvertMap_unique._call_ (/Users/buildslave-sage/slave/sage_git/build/src/build/cythonized/sage/structure/coerce_maps.c:4543)\n          raise\n        File \"sage/structure/coerce_maps.pyx\", line 104, in sage.structure.coerce_maps.DefaultConvertMap_unique._call_ (/Users/buildslave-sage/slave/sage_git/build/src/build/cythonized/sage/structure/coerce_maps.c:4436)\n          return C._element_constructor(x)\n        File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/kirillov_reshetikhin.py\", line 3024, in _element_constructor_\n          return self.module_generator().f_string(f_str)\n        File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/kirillov_reshetikhin.py\", line 543, in module_generator\n          return [ b for b in self.module_generators if b.weight() == weight][0]\n      IndexError: list index out of range\n      ------------------------------------------------------------\n      The following tests failed: _test_stembridge_local_axioms\n    Failure in _test_elements\n    Failure in _test_stembridge_local_axioms:\n    Traceback (most recent call last):\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/misc/sage_unittest.py\", line 283, in run\n        test_method(tester = tester)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/categories/regular_crystals.py\", line 310, in _test_stembridge_local_axioms\n        goodness = x._test_stembridge_local_axioms(index_set, verbose)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/categories/regular_crystals.py\", line 760, in _test_stembridge_local_axioms\n        if self.e(i) is not None and self.e(j) is not None:\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py\", line 1680, in e\n        ret = ret.to_virtual_configuration().e(0)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/rigged_configuration_element.py\", line 1327, in e\n        ret = self.to_tensor_product_of_kirillov_reshetikhin_tableaux().e(a)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/tensor_product.py\", line 1258, in e\n        position = self.positions_of_unmatched_plus(i)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/tensor_product.py\", line 1400, in positions_of_unmatched_plus\n        l = self.positions_of_unmatched_minus(i, dual=True, reverse=True)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/tensor_product.py\", line 1380, in positions_of_unmatched_minus\n        plus = self[j].epsilon(i)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py\", line 1532, in epsilon\n        return KirillovReshetikhinTableauxElement.epsilon(self, i)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py\", line 1373, in epsilon\n        return self.to_kirillov_reshetikhin_crystal().epsilon0()\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py\", line 1160, in to_kirillov_reshetikhin_crystal\n        return self.parent().kirillov_reshetikhin_crystal()(self)\n      File \"sage/structure/parent.pyx\", line 1098, in sage.structure.parent.Parent.__call__ (/Users/buildslave-sage/slave/sage_git/build/src/build/cythonized/sage/structure/parent.c:9821)\n        return mor._call_(x)\n      File \"sage/structure/coerce_maps.pyx\", line 109, in sage.structure.coerce_maps.DefaultConvertMap_unique._call_ (/Users/buildslave-sage/slave/sage_git/build/src/build/cythonized/sage/structure/coerce_maps.c:4543)\n        raise\n      File \"sage/structure/coerce_maps.pyx\", line 104, in sage.structure.coerce_maps.DefaultConvertMap_unique._call_ (/Users/buildslave-sage/slave/sage_git/build/src/build/cythonized/sage/structure/coerce_maps.c:4436)\n        return C._element_constructor(x)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/kirillov_reshetikhin.py\", line 3024, in _element_constructor_\n        return self.module_generator().f_string(f_str)\n      File \"/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/kirillov_reshetikhin.py\", line 543, in module_generator\n        return [ b for b in self.module_generators if b.weight() == weight][0]\n    IndexError: list index out of range\n    ------------------------------------------------------------\n    The following tests failed: _test_elements, _test_stembridge_local_axioms\n**********************************************************************\n3 items had failures:\n   1 of   3 in sage.combinat.rigged_configurations.kr_tableaux.KRTableauxTypeDTri2.__init__\n   1 of   3 in sage.combinat.rigged_configurations.kr_tableaux.KRTableauxTypeDTri2Element.e\n   2 of   4 in sage.combinat.rigged_configurations.kr_tableaux.KRTableauxTypeDTri2Element.f\n    [247 tests, 4 failures, 130.41 s]\n```",
    "created_at": "2015-10-24T07:54:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285240",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:12'></a>I'm getting random failures that point to this ticket:

```
sage -t --long src/sage/combinat/rigged_configurations/kr_tableaux.py
**********************************************************************
File "src/sage/combinat/rigged_configurations/kr_tableaux.py", line 1671, in sage.combinat.rigged_configurations.kr_tableaux.KRTableauxTypeDTri2Element.e
Failed example:
    KRT.module_generators[0].e(0)
Exception raised:
    Traceback (most recent call last):
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/doctest/forker.py", line 496, in _run
        self.compile_and_execute(example, compiler, test.globs)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/doctest/forker.py", line 858, in compile_and_execute
        exec(compiled, globs)
      File "<doctest sage.combinat.rigged_configurations.kr_tableaux.KRTableauxTypeDTri2Element.e[1]>", line 1, in <module>
        KRT.module_generators[Integer(0)].e(Integer(0))
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py", line 1680, in e
        ret = ret.to_virtual_configuration().e(0)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/rigged_configuration_element.py", line 1327, in e
        ret = self.to_tensor_product_of_kirillov_reshetikhin_tableaux().e(a)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/tensor_product.py", line 1258, in e
        position = self.positions_of_unmatched_plus(i)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/tensor_product.py", line 1400, in positions_of_unmatched_plus
        l = self.positions_of_unmatched_minus(i, dual=True, reverse=True)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/tensor_product.py", line 1380, in positions_of_unmatched_minus
        plus = self[j].epsilon(i)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py", line 1532, in epsilon
        return KirillovReshetikhinTableauxElement.epsilon(self, i)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py", line 1373, in epsilon
        return self.to_kirillov_reshetikhin_crystal().epsilon0()
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py", line 1160, in to_kirillov_reshetikhin_crystal
        return self.parent().kirillov_reshetikhin_crystal()(self)
      File "sage/structure/parent.pyx", line 1098, in sage.structure.parent.Parent.__call__ (/Users/buildslave-sage/slave/sage_git/build/src/build/cythonized/sage/structure/parent.c:9821)
        return mor._call_(x)
      File "sage/structure/coerce_maps.pyx", line 109, in sage.structure.coerce_maps.DefaultConvertMap_unique._call_ (/Users/buildslave-sage/slave/sage_git/build/src/build/cythonized/sage/structure/coerce_maps.c:4543)
        raise
      File "sage/structure/coerce_maps.pyx", line 104, in sage.structure.coerce_maps.DefaultConvertMap_unique._call_ (/Users/buildslave-sage/slave/sage_git/build/src/build/cythonized/sage/structure/coerce_maps.c:4436)
        return C._element_constructor(x)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/kirillov_reshetikhin.py", line 3024, in _element_constructor_
        return self.module_generator().f_string(f_str)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/kirillov_reshetikhin.py", line 543, in module_generator
        return [ b for b in self.module_generators if b.weight() == weight][0]
    IndexError: list index out of range
**********************************************************************
File "src/sage/combinat/rigged_configurations/kr_tableaux.py", line 1699, in sage.combinat.rigged_configurations.kr_tableaux.KRTableauxTypeDTri2Element.f
Failed example:
    KRT.module_generators[0].f(0)
Exception raised:
    Traceback (most recent call last):
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/doctest/forker.py", line 496, in _run
        self.compile_and_execute(example, compiler, test.globs)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/doctest/forker.py", line 858, in compile_and_execute
        exec(compiled, globs)
      File "<doctest sage.combinat.rigged_configurations.kr_tableaux.KRTableauxTypeDTri2Element.f[1]>", line 1, in <module>
        KRT.module_generators[Integer(0)].f(Integer(0))
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py", line 1709, in f
        ret = ret.to_virtual_configuration().f(0)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/rigged_configuration_element.py", line 1383, in f
        ret = self.to_tensor_product_of_kirillov_reshetikhin_tableaux().f(a)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/tensor_product.py", line 1297, in f
        position = self.positions_of_unmatched_minus(i)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/tensor_product.py", line 1371, in positions_of_unmatched_minus
        minus = self[j].phi(i)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py", line 1549, in phi
        return KirillovReshetikhinTableauxElement.phi(self, i)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py", line 1391, in phi
        return self.to_kirillov_reshetikhin_crystal().phi0()
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py", line 1160, in to_kirillov_reshetikhin_crystal
        return self.parent().kirillov_reshetikhin_crystal()(self)
      File "sage/structure/parent.pyx", line 1098, in sage.structure.parent.Parent.__call__ (/Users/buildslave-sage/slave/sage_git/build/src/build/cythonized/sage/structure/parent.c:9821)
        return mor._call_(x)
      File "sage/structure/coerce_maps.pyx", line 109, in sage.structure.coerce_maps.DefaultConvertMap_unique._call_ (/Users/buildslave-sage/slave/sage_git/build/src/build/cythonized/sage/structure/coerce_maps.c:4543)
        raise
      File "sage/structure/coerce_maps.pyx", line 104, in sage.structure.coerce_maps.DefaultConvertMap_unique._call_ (/Users/buildslave-sage/slave/sage_git/build/src/build/cythonized/sage/structure/coerce_maps.c:4436)
        return C._element_constructor(x)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/kirillov_reshetikhin.py", line 3024, in _element_constructor_
        return self.module_generator().f_string(f_str)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/kirillov_reshetikhin.py", line 543, in module_generator
        return [ b for b in self.module_generators if b.weight() == weight][0]
    IndexError: list index out of range
**********************************************************************
File "src/sage/combinat/rigged_configurations/kr_tableaux.py", line 1700, in sage.combinat.rigged_configurations.kr_tableaux.KRTableauxTypeDTri2Element.f
Failed example:
    KRT.module_generators[3].f(0)
Exception raised:
    Traceback (most recent call last):
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/doctest/forker.py", line 496, in _run
        self.compile_and_execute(example, compiler, test.globs)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/doctest/forker.py", line 858, in compile_and_execute
        exec(compiled, globs)
      File "<doctest sage.combinat.rigged_configurations.kr_tableaux.KRTableauxTypeDTri2Element.f[2]>", line 1, in <module>
        KRT.module_generators[Integer(3)].f(Integer(0))
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py", line 1709, in f
        ret = ret.to_virtual_configuration().f(0)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/rigged_configuration_element.py", line 1383, in f
        ret = self.to_tensor_product_of_kirillov_reshetikhin_tableaux().f(a)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/tensor_product.py", line 1297, in f
        position = self.positions_of_unmatched_minus(i)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/tensor_product.py", line 1371, in positions_of_unmatched_minus
        minus = self[j].phi(i)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py", line 1549, in phi
        return KirillovReshetikhinTableauxElement.phi(self, i)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py", line 1391, in phi
        return self.to_kirillov_reshetikhin_crystal().phi0()
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py", line 1160, in to_kirillov_reshetikhin_crystal
        return self.parent().kirillov_reshetikhin_crystal()(self)
      File "sage/structure/parent.pyx", line 1098, in sage.structure.parent.Parent.__call__ (/Users/buildslave-sage/slave/sage_git/build/src/build/cythonized/sage/structure/parent.c:9821)
        return mor._call_(x)
      File "sage/structure/coerce_maps.pyx", line 109, in sage.structure.coerce_maps.DefaultConvertMap_unique._call_ (/Users/buildslave-sage/slave/sage_git/build/src/build/cythonized/sage/structure/coerce_maps.c:4543)
        raise
      File "sage/structure/coerce_maps.pyx", line 104, in sage.structure.coerce_maps.DefaultConvertMap_unique._call_ (/Users/buildslave-sage/slave/sage_git/build/src/build/cythonized/sage/structure/coerce_maps.c:4436)
        return C._element_constructor(x)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/kirillov_reshetikhin.py", line 3024, in _element_constructor_
        return self.module_generator().f_string(f_str)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/kirillov_reshetikhin.py", line 543, in module_generator
        return [ b for b in self.module_generators if b.weight() == weight][0]
    IndexError: list index out of range
**********************************************************************
File "src/sage/combinat/rigged_configurations/kr_tableaux.py", line 1778, in sage.combinat.rigged_configurations.kr_tableaux.KRTableauxTypeDTri2.__init__
Failed example:
    TestSuite(KRT).run() # long time
Expected nothing
Got:
      Failure in _test_stembridge_local_axioms:
      Traceback (most recent call last):
        File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/misc/sage_unittest.py", line 283, in run
          test_method(tester = tester)
        File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/categories/regular_crystals.py", line 760, in _test_stembridge_local_axioms
          if self.e(i) is not None and self.e(j) is not None:
        File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py", line 1680, in e
          ret = ret.to_virtual_configuration().e(0)
        File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/rigged_configuration_element.py", line 1327, in e
          ret = self.to_tensor_product_of_kirillov_reshetikhin_tableaux().e(a)
        File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/tensor_product.py", line 1258, in e
          position = self.positions_of_unmatched_plus(i)
        File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/tensor_product.py", line 1400, in positions_of_unmatched_plus
          l = self.positions_of_unmatched_minus(i, dual=True, reverse=True)
        File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/tensor_product.py", line 1380, in positions_of_unmatched_minus
          plus = self[j].epsilon(i)
        File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py", line 1532, in epsilon
          return KirillovReshetikhinTableauxElement.epsilon(self, i)
        File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py", line 1373, in epsilon
          return self.to_kirillov_reshetikhin_crystal().epsilon0()
        File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py", line 1160, in to_kirillov_reshetikhin_crystal
          return self.parent().kirillov_reshetikhin_crystal()(self)
        File "sage/structure/parent.pyx", line 1098, in sage.structure.parent.Parent.__call__ (/Users/buildslave-sage/slave/sage_git/build/src/build/cythonized/sage/structure/parent.c:9821)
          return mor._call_(x)
        File "sage/structure/coerce_maps.pyx", line 109, in sage.structure.coerce_maps.DefaultConvertMap_unique._call_ (/Users/buildslave-sage/slave/sage_git/build/src/build/cythonized/sage/structure/coerce_maps.c:4543)
          raise
        File "sage/structure/coerce_maps.pyx", line 104, in sage.structure.coerce_maps.DefaultConvertMap_unique._call_ (/Users/buildslave-sage/slave/sage_git/build/src/build/cythonized/sage/structure/coerce_maps.c:4436)
          return C._element_constructor(x)
        File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/kirillov_reshetikhin.py", line 3024, in _element_constructor_
          return self.module_generator().f_string(f_str)
        File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/kirillov_reshetikhin.py", line 543, in module_generator
          return [ b for b in self.module_generators if b.weight() == weight][0]
      IndexError: list index out of range
      ------------------------------------------------------------
      The following tests failed: _test_stembridge_local_axioms
    Failure in _test_elements
    Failure in _test_stembridge_local_axioms:
    Traceback (most recent call last):
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/misc/sage_unittest.py", line 283, in run
        test_method(tester = tester)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/categories/regular_crystals.py", line 310, in _test_stembridge_local_axioms
        goodness = x._test_stembridge_local_axioms(index_set, verbose)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/categories/regular_crystals.py", line 760, in _test_stembridge_local_axioms
        if self.e(i) is not None and self.e(j) is not None:
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py", line 1680, in e
        ret = ret.to_virtual_configuration().e(0)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/rigged_configuration_element.py", line 1327, in e
        ret = self.to_tensor_product_of_kirillov_reshetikhin_tableaux().e(a)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/tensor_product.py", line 1258, in e
        position = self.positions_of_unmatched_plus(i)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/tensor_product.py", line 1400, in positions_of_unmatched_plus
        l = self.positions_of_unmatched_minus(i, dual=True, reverse=True)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/tensor_product.py", line 1380, in positions_of_unmatched_minus
        plus = self[j].epsilon(i)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py", line 1532, in epsilon
        return KirillovReshetikhinTableauxElement.epsilon(self, i)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py", line 1373, in epsilon
        return self.to_kirillov_reshetikhin_crystal().epsilon0()
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/rigged_configurations/kr_tableaux.py", line 1160, in to_kirillov_reshetikhin_crystal
        return self.parent().kirillov_reshetikhin_crystal()(self)
      File "sage/structure/parent.pyx", line 1098, in sage.structure.parent.Parent.__call__ (/Users/buildslave-sage/slave/sage_git/build/src/build/cythonized/sage/structure/parent.c:9821)
        return mor._call_(x)
      File "sage/structure/coerce_maps.pyx", line 109, in sage.structure.coerce_maps.DefaultConvertMap_unique._call_ (/Users/buildslave-sage/slave/sage_git/build/src/build/cythonized/sage/structure/coerce_maps.c:4543)
        raise
      File "sage/structure/coerce_maps.pyx", line 104, in sage.structure.coerce_maps.DefaultConvertMap_unique._call_ (/Users/buildslave-sage/slave/sage_git/build/src/build/cythonized/sage/structure/coerce_maps.c:4436)
        return C._element_constructor(x)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/kirillov_reshetikhin.py", line 3024, in _element_constructor_
        return self.module_generator().f_string(f_str)
      File "/Users/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/crystals/kirillov_reshetikhin.py", line 543, in module_generator
        return [ b for b in self.module_generators if b.weight() == weight][0]
    IndexError: list index out of range
    ------------------------------------------------------------
    The following tests failed: _test_elements, _test_stembridge_local_axioms
**********************************************************************
3 items had failures:
   1 of   3 in sage.combinat.rigged_configurations.kr_tableaux.KRTableauxTypeDTri2.__init__
   1 of   3 in sage.combinat.rigged_configurations.kr_tableaux.KRTableauxTypeDTri2Element.e
   2 of   4 in sage.combinat.rigged_configurations.kr_tableaux.KRTableauxTypeDTri2Element.f
    [247 tests, 4 failures, 130.41 s]
```



---

archive/issue_comments_285241.json:
```json
{
    "body": "Changing commit from \"68a8208d9def8674dfe9711495f171b98349dbdc\" to \"\"",
    "created_at": "2015-10-24T07:54:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285241",
    "user": "https://github.com/vbraun"
}
```

Changing commit from "68a8208d9def8674dfe9711495f171b98349dbdc" to ""



---

archive/issue_comments_285242.json:
```json
{
    "body": "Changing commit from \"\" to \"68a8208d9def8674dfe9711495f171b98349dbdc\"",
    "created_at": "2015-10-24T07:54:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285242",
    "user": "https://github.com/vbraun"
}
```

Changing commit from "" to "68a8208d9def8674dfe9711495f171b98349dbdc"



---

archive/issue_comments_285243.json:
```json
{
    "body": "Changing branch from \"68a8208d9def8674dfe9711495f171b98349dbdc\" to \"public/rigged_configurations/type_D_tri_bijection-16988\"",
    "created_at": "2015-10-24T07:54:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285243",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "68a8208d9def8674dfe9711495f171b98349dbdc" to "public/rigged_configurations/type_D_tri_bijection-16988"



---

archive/issue_comments_285244.json:
```json
{
    "body": "<a id='comment:13'></a>Last 10 new commits:",
    "created_at": "2015-10-24T07:54:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285244",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:13'></a>Last 10 new commits:



---

archive/issue_comments_285245.json:
```json
{
    "body": "Changing commit from \"68a8208d9def8674dfe9711495f171b98349dbdc\" to \"500628b8fa311267c900ec34126762e5d029d038\"",
    "created_at": "2015-10-24T20:37:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285245",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "68a8208d9def8674dfe9711495f171b98349dbdc" to "500628b8fa311267c900ec34126762e5d029d038"



---

archive/issue_comments_285246.json:
```json
{
    "body": "<a id='comment:14'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2015-10-24T20:37:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285246",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:14'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_285247.json:
```json
{
    "body": "<a id='comment:15'></a>It's not a Heisenbug as I can regularly reproduce this when I run long tests on just `kr_tableaux.py`. I have traced this down to somewhere there is an `s` for a KR crystal that is being created as a python `int`:\n\n```\nsage: K = crystals.KirillovReshetikhin(['D',4,1], 4, int(1))\nsage: K.classical_crystal\nThe crystal of tableaux of type ['D', 4] and shape(s) [[0, 0, 0, 0]]\n```\nThis patch just exposed this issue that `self.s()` needs to be explicitly converted to a rational number for the `s/2` part of the classical decomposition.",
    "created_at": "2015-10-24T20:37:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285247",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:15'></a>It's not a Heisenbug as I can regularly reproduce this when I run long tests on just `kr_tableaux.py`. I have traced this down to somewhere there is an `s` for a KR crystal that is being created as a python `int`:

```
sage: K = crystals.KirillovReshetikhin(['D',4,1], 4, int(1))
sage: K.classical_crystal
The crystal of tableaux of type ['D', 4] and shape(s) [[0, 0, 0, 0]]
```
This patch just exposed this issue that `self.s()` needs to be explicitly converted to a rational number for the `s/2` part of the classical decomposition.



---

archive/issue_comments_285248.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2015-10-24T20:37:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285248",
    "user": "https://github.com/tscrim"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_285249.json:
```json
{
    "body": "<a id='comment:17'></a>\n```\n$ sage -t --long src/sage/combinat/rigged_configurations/kr_tableaux.py\nRunning doctests with ID 2015-10-28-15-21-54-c356bd2b.\nGit branch: t/16988/public/rigged_configurations/type_D_tri_bijection-16988\nUsing --optional=gcc,mpir,python2,sage\nDoctesting 1 file.\nsage -t --long --warn-long 83.6 src/sage/combinat/rigged_configurations/kr_tableaux.py\n    [247 tests, 71.36 s]\n----------------------------------------------------------------------\nAll tests passed!\n----------------------------------------------------------------------\nTotal time for all tests: 71.5 seconds\n    cpu time: 71.1 seconds\n    cumulative wall time: 71.4 seconds\n```",
    "created_at": "2015-10-28T19:24:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285249",
    "user": "https://github.com/bsalisbury1"
}
```

<a id='comment:17'></a>
```
$ sage -t --long src/sage/combinat/rigged_configurations/kr_tableaux.py
Running doctests with ID 2015-10-28-15-21-54-c356bd2b.
Git branch: t/16988/public/rigged_configurations/type_D_tri_bijection-16988
Using --optional=gcc,mpir,python2,sage
Doctesting 1 file.
sage -t --long --warn-long 83.6 src/sage/combinat/rigged_configurations/kr_tableaux.py
    [247 tests, 71.36 s]
----------------------------------------------------------------------
All tests passed!
----------------------------------------------------------------------
Total time for all tests: 71.5 seconds
    cpu time: 71.1 seconds
    cumulative wall time: 71.4 seconds
```



---

archive/issue_comments_285250.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2015-10-28T19:24:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285250",
    "user": "https://github.com/bsalisbury1"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_events_049502.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-10-29T16:35:04Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16988#event-49502"
}
```



---

archive/issue_comments_285251.json:
```json
{
    "body": "Changing branch from \"public/rigged_configurations/type_D_tri_bijection-16988\" to \"500628b8fa311267c900ec34126762e5d029d038\"",
    "created_at": "2015-10-29T16:35:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285251",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "public/rigged_configurations/type_D_tri_bijection-16988" to "500628b8fa311267c900ec34126762e5d029d038"



---

archive/issue_comments_285252.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2015-10-29T16:35:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16988",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16988#issuecomment-285252",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed
