# Issue 16910: Get rid of mpz_t_offset hacks in integer.pyx

archive/issues_016673.json:
```json
{
    "assignees": [],
    "body": "The comment \"Accessing the `.value` attribute of an `Integer` object causes Cython to refcount it.\" is no longer true, so we don't need this hack anymore.\n\nBranch/Commit: **[6fc03e3](https://github.com/sagemath/sagetrac-mirror/commit/6fc03e3ea73f1a472b37661ae9ba9a72b4579dcc)**\n\nReviewer: **Volker Braun**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16910_\n\n",
    "closed_at": "2014-09-06T11:02:45Z",
    "created_at": "2014-08-30T12:00:41Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20cython",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Get rid of mpz_t_offset hacks in integer.pyx",
    "type": "issue",
    "updated_at": "2014-09-06T11:02:45Z",
    "url": "https://github.com/sagemath/sage/issues/16910",
    "user": "https://github.com/jdemeyer"
}
```
The comment "Accessing the `.value` attribute of an `Integer` object causes Cython to refcount it." is no longer true, so we don't need this hack anymore.

Branch/Commit: **[6fc03e3](https://github.com/sagemath/sagetrac-mirror/commit/6fc03e3ea73f1a472b37661ae9ba9a72b4579dcc)**

Reviewer: **Volker Braun**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/16910_





---

archive/issue_events_224131.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-08-30T12:00:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16910",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16910#event-224131"
}
```



---

archive/issue_events_224132.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-08-30T12:00:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16910",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20cython",
    "label_color": "000080",
    "label_name": "component: cython",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16910#event-224132"
}
```



---

archive/issue_events_224133.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-08-30T12:00:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16910",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16910#event-224133"
}
```



---

archive/issue_events_224134.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-08-30T12:00:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16910",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16910#event-224134"
}
```



---

archive/issue_comments_225375.json:
```json
{
    "body": "Branch: **[u/jdemeyer/ticket/16910](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/16910)**",
    "created_at": "2014-08-30T13:28:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16910#issuecomment-225375",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/ticket/16910](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/16910)**



---

archive/issue_comments_225376.json:
```json
{
    "body": "Commit: **[6fc03e3](https://github.com/sagemath/sagetrac-mirror/commit/6fc03e3ea73f1a472b37661ae9ba9a72b4579dcc)**",
    "created_at": "2014-08-30T13:28:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16910#issuecomment-225376",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[6fc03e3](https://github.com/sagemath/sagetrac-mirror/commit/6fc03e3ea73f1a472b37661ae9ba9a72b4579dcc)**



---

archive/issue_comments_225377.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6fc03e3ea73f1a472b37661ae9ba9a72b4579dcc\">6fc03e3</a></td><td><code>Get rid of mpz_t_offset hacks</code></td></tr></table>\n",
    "created_at": "2014-08-30T13:28:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16910#issuecomment-225377",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6fc03e3ea73f1a472b37661ae9ba9a72b4579dcc">6fc03e3</a></td><td><code>Get rid of mpz_t_offset hacks</code></td></tr></table>




---

archive/issue_events_224135.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-08-30T13:28:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16910",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16910#event-224135"
}
```



---

archive/issue_comments_225378.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-\n+The comment \"Accessing the `.value` attribute of an `Integer` object causes Cython to refcount it.\" is no longer true, so we don't need this hack anymore.\n``````\n",
    "created_at": "2014-08-30T13:29:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16910#issuecomment-225378",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-
+The comment "Accessing the `.value` attribute of an `Integer` object causes Cython to refcount it." is no longer true, so we don't need this hack anymore.
``````




---

archive/issue_events_224136.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-09-03T10:28:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16910",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16910#event-224136"
}
```



---

archive/issue_events_224137.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-09-03T10:28:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16910",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16910#event-224137"
}
```



---

archive/issue_comments_225379.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2014-09-03T10:28:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16910#issuecomment-225379",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_events_224138.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-09-06T11:02:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16910",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16910#event-224138"
}
```



---

archive/issue_events_224139.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "11c3badc5116d0b755347abe9b7a356f3fb01bcd",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-09-06T11:02:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16910",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16910#event-224139"
}
```



---

archive/issue_comments_225380.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/ticket/16910](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/16910)** to **[6fc03e3](https://github.com/sagemath/sagetrac-mirror/commit/6fc03e3ea73f1a472b37661ae9ba9a72b4579dcc)**",
    "created_at": "2014-09-06T11:02:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16910",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16910#issuecomment-225380",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/ticket/16910](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/16910)** to **[6fc03e3](https://github.com/sagemath/sagetrac-mirror/commit/6fc03e3ea73f1a472b37661ae9ba9a72b4579dcc)**
