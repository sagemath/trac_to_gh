# Issue 16357: FiniteStateMachine.default_format_transition_label: accept iterable

archive/issues_016120.json:
```json
{
    "assignees": [],
    "body": "Current inconsistent behaviour:\n\n```\nsage: T = Transducer()\nsage: T.default_format_transition_label([])\n'\\\\varepsilon'\nsage: T.default_format_transition_label(iter([]))\n''\nsage: T.format_transition_label_reversed([])\n''\n```\n\nChange code such that `\\varepsilon` is returned in all three instances.\n\n\nCC:  @dkrenn @sagetrac-skropf\n\nKeywords: **finite_state_machine**\n\nBranch/Commit: **[f941bbd](https://github.com/sagemath/sagetrac-mirror/commit/f941bbd0598c3c57a39b732462776392c5fb01c4)**\n\nReviewer: **Daniel Krenn**\n\nAuthor: **Clemens Heuberger**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16357_\n\n",
    "closed_at": "2014-05-21T20:56:09Z",
    "created_at": "2014-05-16T05:42:39Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "FiniteStateMachine.default_format_transition_label: accept iterable",
    "type": "issue",
    "updated_at": "2014-05-21T20:56:09Z",
    "url": "https://github.com/sagemath/sage/issues/16357",
    "user": "https://github.com/cheuberg"
}
```
Current inconsistent behaviour:

```
sage: T = Transducer()
sage: T.default_format_transition_label([])
'\\varepsilon'
sage: T.default_format_transition_label(iter([]))
''
sage: T.format_transition_label_reversed([])
''
```

Change code such that `\varepsilon` is returned in all three instances.


CC:  @dkrenn @sagetrac-skropf

Keywords: **finite_state_machine**

Branch/Commit: **[f941bbd](https://github.com/sagemath/sagetrac-mirror/commit/f941bbd0598c3c57a39b732462776392c5fb01c4)**

Reviewer: **Daniel Krenn**

Author: **Clemens Heuberger**

_Issue created by migration from https://trac.sagemath.org/ticket/16357_





---

archive/issue_events_216150.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-05-16T05:42:39Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16357",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16357#event-216150"
}
```



---

archive/issue_events_216151.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-05-16T05:42:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16357",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16357#event-216151"
}
```



---

archive/issue_events_216152.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-05-16T05:42:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16357",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16357#event-216152"
}
```



---

archive/issue_events_216153.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-05-16T05:42:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16357",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16357#event-216153"
}
```



---

archive/issue_comments_213434.json:
```json
{
    "body": "Branch: **[u/cheuberg/fsm/format_transition_iterable](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/format_transition_iterable)**",
    "created_at": "2014-05-17T07:42:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16357#issuecomment-213434",
    "user": "https://github.com/cheuberg"
}
```

Branch: **[u/cheuberg/fsm/format_transition_iterable](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/format_transition_iterable)**



---

archive/issue_comments_213435.json:
```json
{
    "body": "Commit: **[f941bbd](https://github.com/sagemath/sagetrac-mirror/commit/f941bbd0598c3c57a39b732462776392c5fb01c4)**",
    "created_at": "2014-05-17T07:43:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16357#issuecomment-213435",
    "user": "https://github.com/cheuberg"
}
```

Commit: **[f941bbd](https://github.com/sagemath/sagetrac-mirror/commit/f941bbd0598c3c57a39b732462776392c5fb01c4)**



---

archive/issue_events_216154.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-05-17T07:43:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16357",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16357#event-216154"
}
```



---

archive/issue_comments_213436.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f941bbd0598c3c57a39b732462776392c5fb01c4\">f941bbd</a></td><td><code>trac #16357: FiniteStateMachine.default_format_transition_label: accept iterable</code></td></tr></table>\n",
    "created_at": "2014-05-17T07:43:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16357#issuecomment-213436",
    "user": "https://github.com/cheuberg"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f941bbd0598c3c57a39b732462776392c5fb01c4">f941bbd</a></td><td><code>trac #16357: FiniteStateMachine.default_format_transition_label: accept iterable</code></td></tr></table>




---

archive/issue_comments_213437.json:
```json
{
    "body": "Reviewer: **Daniel Krenn**",
    "created_at": "2014-05-21T14:17:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16357#issuecomment-213437",
    "user": "https://github.com/dkrenn"
}
```

Reviewer: **Daniel Krenn**



---

archive/issue_comments_213438.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nPatch looks fine.",
    "created_at": "2014-05-21T14:17:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16357#issuecomment-213438",
    "user": "https://github.com/dkrenn"
}
```

<div id="comment:3" align="right">Comment 3</div>

Patch looks fine.



---

archive/issue_events_216155.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2014-05-21T14:17:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16357",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16357#event-216155"
}
```



---

archive/issue_events_216156.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2014-05-21T14:17:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16357",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16357#event-216156"
}
```



---

archive/issue_comments_213439.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nConflicts, please merge in the next beta.",
    "created_at": "2014-05-21T15:36:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16357#issuecomment-213439",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:4" align="right">Comment 4</div>

Conflicts, please merge in the next beta.



---

archive/issue_comments_213440.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nIgnore, wrong ticket..",
    "created_at": "2014-05-21T15:37:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16357#issuecomment-213440",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:5" align="right">Comment 5</div>

Ignore, wrong ticket..



---

archive/issue_comments_213441.json:
```json
{
    "body": "Changed branch from **[u/cheuberg/fsm/format_transition_iterable](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/format_transition_iterable)** to **[f941bbd](https://github.com/sagemath/sagetrac-mirror/commit/f941bbd0598c3c57a39b732462776392c5fb01c4)**",
    "created_at": "2014-05-21T20:56:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16357#issuecomment-213441",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/cheuberg/fsm/format_transition_iterable](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/format_transition_iterable)** to **[f941bbd](https://github.com/sagemath/sagetrac-mirror/commit/f941bbd0598c3c57a39b732462776392c5fb01c4)**



---

archive/issue_events_216157.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-05-21T20:56:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16357",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16357#event-216157"
}
```



---

archive/issue_events_216158.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "3f6e0643175a26c4870adb7cb56e24b398b82c54",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-05-21T20:56:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16357",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16357#event-216158"
}
```
