# Issue 16686: parametric_plot ignores xmin and xmax axis limits

archive/issues_016449.json:
```json
{
    "assignees": [],
    "body": "Try for instance:\n\n```\nsage: parametric_plot((sin, cos), (0, 2*pi), xmin = -2, xmax = 2, ymin = -2, ymax = 2)\n```\n\nThe xmin and xmax limits are ignored, while the ymin and ymax limits are not.\n\nKeywords: **parametric plot axes options**\n\nReviewer: **Punarbasu Purkayastha**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16686_\n\n",
    "closed_at": "2014-07-20T15:15:28Z",
    "created_at": "2014-07-20T01:19:09Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20graphics",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "parametric_plot ignores xmin and xmax axis limits",
    "type": "issue",
    "updated_at": "2014-07-20T15:15:28Z",
    "url": "https://github.com/sagemath/sage/issues/16686",
    "user": "https://github.com/sagetrac-tcoffee"
}
```
Try for instance:

```
sage: parametric_plot((sin, cos), (0, 2*pi), xmin = -2, xmax = 2, ymin = -2, ymax = 2)
```

The xmin and xmax limits are ignored, while the ymin and ymax limits are not.

Keywords: **parametric plot axes options**

Reviewer: **Punarbasu Purkayastha**

_Issue created by migration from https://trac.sagemath.org/ticket/16686_





---

archive/issue_events_220982.json:
```json
{
    "actor": "https://github.com/sagetrac-tcoffee",
    "created_at": "2014-07-20T01:19:09Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16686",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16686#event-220982"
}
```



---

archive/issue_events_220983.json:
```json
{
    "actor": "https://github.com/sagetrac-tcoffee",
    "created_at": "2014-07-20T01:19:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16686",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graphics",
    "label_color": "0000ff",
    "label_name": "component: graphics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16686#event-220983"
}
```



---

archive/issue_events_220984.json:
```json
{
    "actor": "https://github.com/sagetrac-tcoffee",
    "created_at": "2014-07-20T01:19:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16686",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16686#event-220984"
}
```



---

archive/issue_events_220985.json:
```json
{
    "actor": "https://github.com/sagetrac-tcoffee",
    "created_at": "2014-07-20T01:19:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16686",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16686#event-220985"
}
```



---

archive/issue_comments_220588.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nThis is not a bug in `parametric_plot`. It *was* a deliberate decision made in `plot`; I don't know why it was done this way. See #13368",
    "created_at": "2014-07-20T03:57:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16686",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16686#issuecomment-220588",
    "user": "https://github.com/ppurka"
}
```

<div id="comment:1" align="right">Comment 1</div>

This is not a bug in `parametric_plot`. It *was* a deliberate decision made in `plot`; I don't know why it was done this way. See #13368



---

archive/issue_events_220986.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2014-07-20T03:57:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16686",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16686#event-220986"
}
```



---

archive/issue_events_220987.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2014-07-20T03:57:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16686",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16686#event-220987"
}
```



---

archive/issue_events_220988.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2014-07-20T03:57:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16686",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16686#event-220988"
}
```



---

archive/issue_events_220989.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2014-07-20T03:57:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16686",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16686#event-220989"
}
```



---

archive/issue_events_220990.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2014-07-20T03:58:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16686",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16686#event-220990"
}
```



---

archive/issue_events_220991.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2014-07-20T03:58:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16686",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16686#event-220991"
}
```



---

archive/issue_comments_220589.json:
```json
{
    "body": "Reviewer: **Punarbasu Purkayastha**",
    "created_at": "2014-07-20T03:58:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16686",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16686#issuecomment-220589",
    "user": "https://github.com/ppurka"
}
```

Reviewer: **Punarbasu Purkayastha**



---

archive/issue_events_220992.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-07-20T15:15:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16686",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16686#event-220992"
}
```



---

archive/issue_events_220993.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-07-20T15:15:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16686",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16686#event-220993"
}
```



---

archive/issue_events_220994.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-07-20T15:15:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16686",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "a6a6a6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16686#event-220994"
}
```
