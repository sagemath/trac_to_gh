# Issue 16140: FiniteStateMachine.final_components: New method

archive/issues_015903.json:
```json
{
    "assignees": [],
    "body": "Introduced a new method final_components: a final component of a\ntransducer T is a strongly connected componen C such that there are no\ntransitions of T leaving C.\n    \nThose are the only parts of a transducer which influence the main\nterms of the asympotic behaviour of the sum of output labels of a\ntransducer.\n\n\nDepends on #15849\n\nCC:  skropf\n\nKeywords: **finite_state_machine**\n\nBranch/Commit: **[e5f4b5b](https://github.com/sagemath/sagetrac-mirror/commit/e5f4b5b06c8936ba80c86b5431f7cce194c90b94)**\n\nReviewer: **Daniel Krenn**\n\nAuthor: **Clemens Heuberger**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16140_\n\n",
    "closed_at": "2014-04-16T09:10:04Z",
    "created_at": "2014-04-12T03:20:05Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "FiniteStateMachine.final_components: New method",
    "type": "issue",
    "updated_at": "2014-04-16T09:10:04Z",
    "url": "https://github.com/sagemath/sage/issues/16140",
    "user": "https://github.com/cheuberg"
}
```
Introduced a new method final_components: a final component of a
transducer T is a strongly connected componen C such that there are no
transitions of T leaving C.
    
Those are the only parts of a transducer which influence the main
terms of the asympotic behaviour of the sum of output labels of a
transducer.


Depends on #15849

CC:  skropf

Keywords: **finite_state_machine**

Branch/Commit: **[e5f4b5b](https://github.com/sagemath/sagetrac-mirror/commit/e5f4b5b06c8936ba80c86b5431f7cce194c90b94)**

Reviewer: **Daniel Krenn**

Author: **Clemens Heuberger**

_Issue created by migration from https://trac.sagemath.org/ticket/16140_





---

archive/issue_events_212961.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-04-12T03:20:05Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16140",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16140#event-212961"
}
```



---

archive/issue_events_212962.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-04-12T03:20:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16140",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16140#event-212962"
}
```



---

archive/issue_events_212963.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-04-12T03:20:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16140",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16140#event-212963"
}
```



---

archive/issue_events_212964.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-04-12T03:20:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16140",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16140#event-212964"
}
```



---

archive/issue_events_212965.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-04-12T03:20:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16140",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16140#event-212965"
}
```



---

archive/issue_comments_208683.json:
```json
{
    "body": "Changed keywords from **** to **finite_state_machine**",
    "created_at": "2014-04-12T05:21:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16140",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16140#issuecomment-208683",
    "user": "https://github.com/cheuberg"
}
```

Changed keywords from **** to **finite_state_machine**



---

archive/issue_comments_208684.json:
```json
{
    "body": "Changed branch from **[u/cheuberg/fsm/final_components](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/final_components)** to **[u/cheuberg/ticket/16140](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/ticket/16140)**",
    "created_at": "2014-04-15T03:16:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16140",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16140#issuecomment-208684",
    "user": "https://github.com/cheuberg"
}
```

Changed branch from **[u/cheuberg/fsm/final_components](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/final_components)** to **[u/cheuberg/ticket/16140](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/ticket/16140)**



---

archive/issue_comments_208685.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nUpdated reference HKW2014 to point to arxiv:1404.3680.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2a2f753a8627356fc483a99b75aef0337466e5d9\">2a2f753</a></td><td><code>FiniteStateMachine.final_components: Link [HKW2014] -> arxiv:1404.3680</code></td></tr></table>\n",
    "created_at": "2014-04-15T03:28:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16140",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16140#issuecomment-208685",
    "user": "https://github.com/cheuberg"
}
```

<a id='comment:4'>Comment 4:</a>
Updated reference HKW2014 to point to arxiv:1404.3680.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2a2f753a8627356fc483a99b75aef0337466e5d9">2a2f753</a></td><td><code>FiniteStateMachine.final_components: Link [HKW2014] -> arxiv:1404.3680</code></td></tr></table>




---

archive/issue_comments_208686.json:
```json
{
    "body": "Changed commit from **[d39af91](https://github.com/sagemath/sagetrac-mirror/commit/d39af9188a62f06fe668b6e625355e172f21c5a3)** to **[2a2f753](https://github.com/sagemath/sagetrac-mirror/commit/2a2f753a8627356fc483a99b75aef0337466e5d9)**",
    "created_at": "2014-04-15T03:28:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16140",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16140#issuecomment-208686",
    "user": "https://github.com/cheuberg"
}
```

Changed commit from **[d39af91](https://github.com/sagemath/sagetrac-mirror/commit/d39af9188a62f06fe668b6e625355e172f21c5a3)** to **[2a2f753](https://github.com/sagemath/sagetrac-mirror/commit/2a2f753a8627356fc483a99b75aef0337466e5d9)**



---

archive/issue_events_212966.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2014-04-15T14:22:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16140",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16140#event-212966"
}
```



---

archive/issue_events_212967.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2014-04-15T14:22:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16140",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16140#event-212967"
}
```



---

archive/issue_comments_208687.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nadded one spaceing and two commas in docstring. Code looks also good. Tests pass, docu builds.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3fcf289ad7311fd211da3d48feec624fbdd6fe25\">3fcf289</a></td><td><code>FiniteStateMachine.induced_sub_finite_state_machine: new method</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/df42a941f86b9bcc826119ce926e104791f644a9\">df42a94</a></td><td><code>Merge #15849 into 'fsm/final_components'</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5fb22c3b438fcb0740f8f0badf048390bc7d6f53\">5fb22c3</a></td><td><code>FiniteStateMachine.final_components: New method</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3fad1845e8b52457ba5266d456da0c9f493ba0e7\">3fad184</a></td><td><code>FiniteStateMachine.final_components: another Doctest</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d39af9188a62f06fe668b6e625355e172f21c5a3\">d39af91</a></td><td><code>fixed docstring formatting</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2a2f753a8627356fc483a99b75aef0337466e5d9\">2a2f753</a></td><td><code>FiniteStateMachine.final_components: Link [HKW2014] -> arxiv:1404.3680</code></td></tr></table>\n",
    "created_at": "2014-04-15T14:22:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16140",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16140#issuecomment-208687",
    "user": "https://github.com/dkrenn"
}
```

<a id='comment:5'>Comment 5:</a>
added one spaceing and two commas in docstring. Code looks also good. Tests pass, docu builds.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3fcf289ad7311fd211da3d48feec624fbdd6fe25">3fcf289</a></td><td><code>FiniteStateMachine.induced_sub_finite_state_machine: new method</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/df42a941f86b9bcc826119ce926e104791f644a9">df42a94</a></td><td><code>Merge #15849 into 'fsm/final_components'</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5fb22c3b438fcb0740f8f0badf048390bc7d6f53">5fb22c3</a></td><td><code>FiniteStateMachine.final_components: New method</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3fad1845e8b52457ba5266d456da0c9f493ba0e7">3fad184</a></td><td><code>FiniteStateMachine.final_components: another Doctest</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d39af9188a62f06fe668b6e625355e172f21c5a3">d39af91</a></td><td><code>fixed docstring formatting</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2a2f753a8627356fc483a99b75aef0337466e5d9">2a2f753</a></td><td><code>FiniteStateMachine.final_components: Link [HKW2014] -> arxiv:1404.3680</code></td></tr></table>




---

archive/issue_comments_208688.json:
```json
{
    "body": "Changed branch from **[u/cheuberg/ticket/16140](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/ticket/16140)** to **[u/dkrenn/ticket/16140](https://github.com/sagemath/sagetrac-mirror/tree/u/dkrenn/ticket/16140)**",
    "created_at": "2014-04-15T14:22:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16140",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16140#issuecomment-208688",
    "user": "https://github.com/dkrenn"
}
```

Changed branch from **[u/cheuberg/ticket/16140](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/ticket/16140)** to **[u/dkrenn/ticket/16140](https://github.com/sagemath/sagetrac-mirror/tree/u/dkrenn/ticket/16140)**



---

archive/issue_comments_208689.json:
```json
{
    "body": "Reviewer: **Daniel Krenn**",
    "created_at": "2014-04-15T14:22:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16140",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16140#issuecomment-208689",
    "user": "https://github.com/dkrenn"
}
```

Reviewer: **Daniel Krenn**



---

archive/issue_comments_208690.json:
```json
{
    "body": "Changed commit from **[2a2f753](https://github.com/sagemath/sagetrac-mirror/commit/2a2f753a8627356fc483a99b75aef0337466e5d9)** to **[e5f4b5b](https://github.com/sagemath/sagetrac-mirror/commit/e5f4b5b06c8936ba80c86b5431f7cce194c90b94)**",
    "created_at": "2014-04-15T14:23:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16140",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16140#issuecomment-208690",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[2a2f753](https://github.com/sagemath/sagetrac-mirror/commit/2a2f753a8627356fc483a99b75aef0337466e5d9)** to **[e5f4b5b](https://github.com/sagemath/sagetrac-mirror/commit/e5f4b5b06c8936ba80c86b5431f7cce194c90b94)**



---

archive/issue_comments_208691.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nBranch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e5f4b5b06c8936ba80c86b5431f7cce194c90b94\">e5f4b5b</a></td><td><code>added one spacing and two commas in docstring of .final_components</code></td></tr></table>\n",
    "created_at": "2014-04-15T14:23:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16140",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16140#issuecomment-208691",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:6'>Comment 6:</a>
Branch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e5f4b5b06c8936ba80c86b5431f7cce194c90b94">e5f4b5b</a></td><td><code>added one spacing and two commas in docstring of .final_components</code></td></tr></table>




---

archive/issue_events_212968.json:
```json
{
    "actor": "https://github.com/sagetrac-git",
    "created_at": "2014-04-15T14:23:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16140",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16140#event-212968"
}
```



---

archive/issue_events_212969.json:
```json
{
    "actor": "https://github.com/sagetrac-git",
    "created_at": "2014-04-15T14:23:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16140",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16140#event-212969"
}
```



---

archive/issue_comments_208692.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3fcf289ad7311fd211da3d48feec624fbdd6fe25\">3fcf289</a></td><td><code>FiniteStateMachine.induced_sub_finite_state_machine: new method</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/df42a941f86b9bcc826119ce926e104791f644a9\">df42a94</a></td><td><code>Merge #15849 into 'fsm/final_components'</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5fb22c3b438fcb0740f8f0badf048390bc7d6f53\">5fb22c3</a></td><td><code>FiniteStateMachine.final_components: New method</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3fad1845e8b52457ba5266d456da0c9f493ba0e7\">3fad184</a></td><td><code>FiniteStateMachine.final_components: another Doctest</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d39af9188a62f06fe668b6e625355e172f21c5a3\">d39af91</a></td><td><code>fixed docstring formatting</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2a2f753a8627356fc483a99b75aef0337466e5d9\">2a2f753</a></td><td><code>FiniteStateMachine.final_components: Link [HKW2014] -> arxiv:1404.3680</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e5f4b5b06c8936ba80c86b5431f7cce194c90b94\">e5f4b5b</a></td><td><code>added one spacing and two commas in docstring of .final_components</code></td></tr></table>\n",
    "created_at": "2014-04-15T14:23:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16140",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16140#issuecomment-208692",
    "user": "https://github.com/dkrenn"
}
```

<a id='comment:7'>Comment 7:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3fcf289ad7311fd211da3d48feec624fbdd6fe25">3fcf289</a></td><td><code>FiniteStateMachine.induced_sub_finite_state_machine: new method</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/df42a941f86b9bcc826119ce926e104791f644a9">df42a94</a></td><td><code>Merge #15849 into 'fsm/final_components'</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5fb22c3b438fcb0740f8f0badf048390bc7d6f53">5fb22c3</a></td><td><code>FiniteStateMachine.final_components: New method</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3fad1845e8b52457ba5266d456da0c9f493ba0e7">3fad184</a></td><td><code>FiniteStateMachine.final_components: another Doctest</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d39af9188a62f06fe668b6e625355e172f21c5a3">d39af91</a></td><td><code>fixed docstring formatting</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2a2f753a8627356fc483a99b75aef0337466e5d9">2a2f753</a></td><td><code>FiniteStateMachine.final_components: Link [HKW2014] -> arxiv:1404.3680</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e5f4b5b06c8936ba80c86b5431f7cce194c90b94">e5f4b5b</a></td><td><code>added one spacing and two commas in docstring of .final_components</code></td></tr></table>




---

archive/issue_events_212970.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2014-04-15T14:24:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16140",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16140#event-212970"
}
```



---

archive/issue_events_212971.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2014-04-15T14:24:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16140",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16140#event-212971"
}
```



---

archive/issue_comments_208693.json:
```json
{
    "body": "Changed branch from **[u/dkrenn/ticket/16140](https://github.com/sagemath/sagetrac-mirror/tree/u/dkrenn/ticket/16140)** to **[e5f4b5b](https://github.com/sagemath/sagetrac-mirror/commit/e5f4b5b06c8936ba80c86b5431f7cce194c90b94)**",
    "created_at": "2014-04-16T09:10:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16140",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16140#issuecomment-208693",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/dkrenn/ticket/16140](https://github.com/sagemath/sagetrac-mirror/tree/u/dkrenn/ticket/16140)** to **[e5f4b5b](https://github.com/sagemath/sagetrac-mirror/commit/e5f4b5b06c8936ba80c86b5431f7cce194c90b94)**



---

archive/issue_events_212972.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-04-16T09:10:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16140",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16140#event-212972"
}
```



---

archive/issue_events_212973.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "d6958c0ea85b03de62beb47c9a9e951448d542b3",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-04-16T09:10:04Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16140",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16140#event-212973"
}
```
