# Issue 16442: Replace os.tmpfile by tempfile module

archive/issues_016205.json:
```json
{
    "assignees": [],
    "body": "os.tmpfile() is not available in python3. Use tempfile module which exists in both versions.\n\nThe relevant reference in official Python documentation is https://docs.python.org/3.0/whatsnew/3.0.html#library-changes:\n//The functions os.tmpnam(), os.tempnam() and os.tmpfile() have been removed in favor of the tempfile module.//\n\nKeywords: **python3**\n\nBranch: **[5661567](https://github.com/sagemath/sagetrac-mirror/commit/566156710124d51851fc9045335dada96c06396f)**\n\nReviewer: **Jeroen Demeyer**\n\nAuthor: **Andr\u00e9 Apitzsch**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16442_\n\n",
    "closed_at": "2014-10-02T16:21:54Z",
    "created_at": "2014-06-04T20:22:35Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20doctest%20framework",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Replace os.tmpfile by tempfile module",
    "type": "issue",
    "updated_at": "2014-11-25T19:01:08Z",
    "url": "https://github.com/sagemath/sage/issues/16442",
    "user": "https://github.com/a-andre"
}
```
os.tmpfile() is not available in python3. Use tempfile module which exists in both versions.

The relevant reference in official Python documentation is https://docs.python.org/3.0/whatsnew/3.0.html#library-changes:
//The functions os.tmpnam(), os.tempnam() and os.tmpfile() have been removed in favor of the tempfile module.//

Keywords: **python3**

Branch: **[5661567](https://github.com/sagemath/sagetrac-mirror/commit/566156710124d51851fc9045335dada96c06396f)**

Reviewer: **Jeroen Demeyer**

Author: **André Apitzsch**

_Issue created by migration from https://trac.sagemath.org/ticket/16442_





---

archive/issue_events_217235.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2014-06-04T20:22:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16442",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16442#event-217235"
}
```



---

archive/issue_events_217236.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2014-06-04T20:22:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16442",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20doctest%20framework",
    "label_color": "000080",
    "label_name": "component: doctest framework",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16442#event-217236"
}
```



---

archive/issue_events_217237.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2014-06-04T20:22:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16442",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16442#event-217237"
}
```



---

archive/issue_events_217238.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2014-06-04T20:22:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16442",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16442#event-217238"
}
```



---

archive/issue_comments_214856.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6059f150620343b6cfa1c1681a57de47de843f25\">6059f15</a></td><td><code>replace os.tmpfile() by tempfile.TemporaryFile()</code></td></tr></table>\n",
    "created_at": "2014-06-04T20:27:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16442#issuecomment-214856",
    "user": "https://github.com/a-andre"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6059f150620343b6cfa1c1681a57de47de843f25">6059f15</a></td><td><code>replace os.tmpfile() by tempfile.TemporaryFile()</code></td></tr></table>




---

archive/issue_comments_214857.json:
```json
{
    "body": "Commit: **[6059f15](https://github.com/sagemath/sagetrac-mirror/commit/6059f150620343b6cfa1c1681a57de47de843f25)**",
    "created_at": "2014-06-04T20:27:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16442#issuecomment-214857",
    "user": "https://github.com/a-andre"
}
```

Commit: **[6059f15](https://github.com/sagemath/sagetrac-mirror/commit/6059f150620343b6cfa1c1681a57de47de843f25)**



---

archive/issue_events_217239.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2014-06-04T20:27:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16442",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16442#event-217239"
}
```



---

archive/issue_comments_214858.json:
```json
{
    "body": "Branch: **[u/aapitzsch/ticket/16442](https://github.com/sagemath/sagetrac-mirror/tree/u/aapitzsch/ticket/16442)**",
    "created_at": "2014-06-04T20:27:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16442#issuecomment-214858",
    "user": "https://github.com/a-andre"
}
```

Branch: **[u/aapitzsch/ticket/16442](https://github.com/sagemath/sagetrac-mirror/tree/u/aapitzsch/ticket/16442)**



---

archive/issue_comments_214859.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nHello,\n\nWhat is the point of modifying `src/doc/en/thematic_tutorials/linear_programming.rst`, `src/sage/numerical/backends/glpk_backend.pyx`, `src/sage/numerical/linear_functions.pyx` and `src/sage/numerical/mip.pyx` in the same commit?\n\nVincent",
    "created_at": "2014-06-05T13:31:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16442#issuecomment-214859",
    "user": "https://github.com/videlec"
}
```

<div id="comment:2" align="right">Comment 2</div>

Hello,

What is the point of modifying `src/doc/en/thematic_tutorials/linear_programming.rst`, `src/sage/numerical/backends/glpk_backend.pyx`, `src/sage/numerical/linear_functions.pyx` and `src/sage/numerical/mip.pyx` in the same commit?

Vincent



---

archive/issue_comments_214860.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nReplying to [@videlec](#comment%3A2):\n> What is the point of modifying `src/doc/en/thematic_tutorials/linear_programming.rst`, `src/sage/numerical/backends/glpk_backend.pyx`, `src/sage/numerical/linear_functions.pyx` and `src/sage/numerical/mip.pyx` in the same commit?\n\nWithout these changes some doctests will fail because the line number changed from 839 to 840.\n\nIt's common to write `...` instead of the line number.",
    "created_at": "2014-06-05T13:59:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16442#issuecomment-214860",
    "user": "https://github.com/a-andre"
}
```

<div id="comment:3" align="right">Comment 3</div>

Replying to [@videlec](#comment%3A2):
> What is the point of modifying `src/doc/en/thematic_tutorials/linear_programming.rst`, `src/sage/numerical/backends/glpk_backend.pyx`, `src/sage/numerical/linear_functions.pyx` and `src/sage/numerical/mip.pyx` in the same commit?

Without these changes some doctests will fail because the line number changed from 839 to 840.

It's common to write `...` instead of the line number.



---

archive/issue_comments_214861.json:
```json
{
    "body": "Changed commit from **[6059f15](https://github.com/sagemath/sagetrac-mirror/commit/6059f150620343b6cfa1c1681a57de47de843f25)** to **[5661567](https://github.com/sagemath/sagetrac-mirror/commit/566156710124d51851fc9045335dada96c06396f)**",
    "created_at": "2014-07-19T13:41:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16442#issuecomment-214861",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[6059f15](https://github.com/sagemath/sagetrac-mirror/commit/6059f150620343b6cfa1c1681a57de47de843f25)** to **[5661567](https://github.com/sagemath/sagetrac-mirror/commit/566156710124d51851fc9045335dada96c06396f)**



---

archive/issue_comments_214862.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/566156710124d51851fc9045335dada96c06396f\">5661567</a></td><td><code>Merge remote-tracking branch 'origin/develop' into py3_tempfile</code></td></tr></table>\n",
    "created_at": "2014-07-19T13:41:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16442#issuecomment-214862",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/566156710124d51851fc9045335dada96c06396f">5661567</a></td><td><code>Merge remote-tracking branch 'origin/develop' into py3_tempfile</code></td></tr></table>




---

archive/issue_events_217240.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16442",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16442#event-217240"
}
```



---

archive/issue_events_217241.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16442",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16442#event-217241"
}
```



---

archive/issue_comments_214863.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2014-10-01T13:49:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16442#issuecomment-214863",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_217242.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-10-01T13:49:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16442",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16442#event-217242"
}
```



---

archive/issue_events_217243.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-10-01T13:49:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16442",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16442#event-217243"
}
```



---

archive/issue_events_217244.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-10-02T16:21:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16442",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16442#event-217244"
}
```



---

archive/issue_events_217245.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "6e79db477e48b678a9bffc02b6cccc3fdf7d551b",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-10-02T16:21:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16442",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16442#event-217245"
}
```



---

archive/issue_comments_214864.json:
```json
{
    "body": "Changed branch from **[u/aapitzsch/ticket/16442](https://github.com/sagemath/sagetrac-mirror/tree/u/aapitzsch/ticket/16442)** to **[5661567](https://github.com/sagemath/sagetrac-mirror/commit/566156710124d51851fc9045335dada96c06396f)**",
    "created_at": "2014-10-02T16:21:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16442#issuecomment-214864",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/aapitzsch/ticket/16442](https://github.com/sagemath/sagetrac-mirror/tree/u/aapitzsch/ticket/16442)** to **[5661567](https://github.com/sagemath/sagetrac-mirror/commit/566156710124d51851fc9045335dada96c06396f)**



---

archive/issue_comments_214865.json:
```json
{
    "body": "Changed commit from **[5661567](https://github.com/sagemath/sagetrac-mirror/commit/566156710124d51851fc9045335dada96c06396f)** to none",
    "created_at": "2014-11-25T19:01:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16442#issuecomment-214865",
    "user": "https://github.com/wluebbe"
}
```

Changed commit from **[5661567](https://github.com/sagemath/sagetrac-mirror/commit/566156710124d51851fc9045335dada96c06396f)** to none



---

archive/issue_comments_214866.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,4 @@\n os.tmpfile() is not available in python3. Use tempfile module which exists in both versions.\n+\n+The relevant reference in official Python documentation is https://docs.python.org/3.0/whatsnew/3.0.html#library-changes:\n+//The functions os.tmpnam(), os.tempnam() and os.tmpfile() have been removed in favor of the tempfile module.//\n``````\n",
    "created_at": "2014-11-25T19:01:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16442#issuecomment-214866",
    "user": "https://github.com/wluebbe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,4 @@
 os.tmpfile() is not available in python3. Use tempfile module which exists in both versions.
+
+The relevant reference in official Python documentation is https://docs.python.org/3.0/whatsnew/3.0.html#library-changes:
+//The functions os.tmpnam(), os.tempnam() and os.tmpfile() have been removed in favor of the tempfile module.//
``````

