# Issue 16126: Introduce a class for generalized Coxeter graphs

archive/issues_015889.json:
```json
{
    "body": "Coxeter graphs are undirected graph without multiedges with possibly labeled edges within the set {4,5,...,oo}. Coxeter graphs encode Coxeter systems. A Coxeter system is a group presented with a special set of generators.\n\nTo represent a Coxeter group, one can use the canonical geometric representation, see Chapter 5 of Humphreys for instance. In this representation, the group acts faithfully on a vector space and preserves a canonical bilinear form.\n\nFor infinite Coxeter groups, it is useful to allow a more general bilinear form, which still gives a faithful representation and has the advantage that every reflection subgroup is represented as a general geometric representation.\n\nThese more general representations can be encoded into a generalized Coxeter graph where labels \"oo\" can be replaced by any real number $c <= -1$. The case $c=-1$ represents the canonical choice, in this case one can keep the label \"oo\".\n\nThe class for generalized Coxeter graphs will be based on the class DynkinDiagram and adapted for its purposes.\n\nAssignee: @jplab\n\nCC:  vripoll @nthiery @fchapoton sage-combinat @KPanComputes\n\nKeywords: coxeter, graphs, days57\n\nAuthor: Jean-Philippe Labb\u00e9\n\nBranch: u/jipilab/ticket16126\n\nStatus: new\n\nDependencies: #17798\n\nCommit: 8cf2af9e2363366692857eeb91c4f4c14eabe46f\n\nIssue created by migration from https://trac.sagemath.org/ticket/16126\n\n",
    "created_at": "2014-04-10T17:45:21Z",
    "labels": [
        "component: combinatorics"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.10",
    "title": "Introduce a class for generalized Coxeter graphs",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/16126",
    "user": "https://github.com/jplab"
}
```
Coxeter graphs are undirected graph without multiedges with possibly labeled edges within the set {4,5,...,oo}. Coxeter graphs encode Coxeter systems. A Coxeter system is a group presented with a special set of generators.

To represent a Coxeter group, one can use the canonical geometric representation, see Chapter 5 of Humphreys for instance. In this representation, the group acts faithfully on a vector space and preserves a canonical bilinear form.

For infinite Coxeter groups, it is useful to allow a more general bilinear form, which still gives a faithful representation and has the advantage that every reflection subgroup is represented as a general geometric representation.

These more general representations can be encoded into a generalized Coxeter graph where labels "oo" can be replaced by any real number $c <= -1$. The case $c=-1$ represents the canonical choice, in this case one can keep the label "oo".

The class for generalized Coxeter graphs will be based on the class DynkinDiagram and adapted for its purposes.

Assignee: @jplab

CC:  vripoll @nthiery @fchapoton sage-combinat @KPanComputes

Keywords: coxeter, graphs, days57

Author: Jean-Philippe Labbé

Branch: u/jipilab/ticket16126

Status: new

Dependencies: #17798

Commit: 8cf2af9e2363366692857eeb91c4f4c14eabe46f

Issue created by migration from https://trac.sagemath.org/ticket/16126





---

archive/issue_events_047522.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16126",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16126#event-47522"
}
```



---

archive/issue_events_047523.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16126",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16126#event-47523"
}
```



---

archive/issue_events_047524.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16126",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16126#event-47524"
}
```



---

archive/issue_comments_262590.json:
```json
{
    "body": "Changing dependencies from \"\" to \"#17798\"",
    "created_at": "2015-04-30T09:01:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16126",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16126#issuecomment-262590",
    "user": "https://github.com/jplab"
}
```

Changing dependencies from "" to "#17798"



---

archive/issue_events_047525.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-10-31T15:59:07Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16126",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16126#event-47525"
}
```



---

archive/issue_events_047526.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-10-31T15:59:07Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16126",
    "milestone": "sage-6.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16126#event-47526"
}
```



---

archive/issue_comments_262591.json:
```json
{
    "body": "<a id='comment:5'></a>\nNow that #17798 is merged, we should get to work on this next. Jean-Philippe, do you have any (partial) code for this?",
    "created_at": "2015-10-31T15:59:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16126",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16126#issuecomment-262591",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:5'></a>
Now that #17798 is merged, we should get to work on this next. Jean-Philippe, do you have any (partial) code for this?



---

archive/issue_comments_262592.json:
```json
{
    "body": "<a id='comment:6'></a>\nHi, Yes, for sure!\n\nI already have some old initial code. I haven't looked at it for a while, but I will push it in the next few days.",
    "created_at": "2015-10-31T16:13:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16126",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16126#issuecomment-262592",
    "user": "https://github.com/jplab"
}
```

<a id='comment:6'></a>
Hi, Yes, for sure!

I already have some old initial code. I haven't looked at it for a while, but I will push it in the next few days.



---

archive/issue_comments_262593.json:
```json
{
    "body": "Changing branch from \"\" to \"u/jipilab/ticket16126\"",
    "created_at": "2015-11-05T14:35:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16126",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16126#issuecomment-262593",
    "user": "https://github.com/jplab"
}
```

Changing branch from "" to "u/jipilab/ticket16126"



---

archive/issue_comments_262594.json:
```json
{
    "body": "<a id='comment:8'></a>\nHere is a first \"very dirty version\" of the patch.\n\nBasically, everything is still to be looked over...\n\nThis is just for you to have a look. I will continue in more detail soon enough.\n\n---\nNew commits:\n|                                                                                                                                          |                                                 |\n|------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------|\n|[f2f84eb](http://git.sagemath.org/sage.git/commit/?id=f2f84eb39661876e7e32094f6038181fa139875c)|`Initial commit: added the file coxeter_graph.py`|\n|[6a2e20c](http://git.sagemath.org/sage.git/commit/?id=6a2e20c55168350a53efbd218adb913cfe91f0a0)|`Some minor changes to start the patch`|\n|[987aefc](http://git.sagemath.org/sage.git/commit/?id=987aefc4037d7eb7a847338d29c7b22b2b0ca3ad)|`Merge branch 'ticket16126' into newticket16126`|\n|[afd5e59](http://git.sagemath.org/sage.git/commit/?id=afd5e59d7d65efbb48190e9671752352a32a7ae4)|`Added necessary startup files`|\n|[9b135a5](http://git.sagemath.org/sage.git/commit/?id=9b135a5f3cbb5e6e190800880e61fbbd6387f46a)|`Changed the occurences of Dynkin to Coxeter`|\n|[c0ac5a4](http://git.sagemath.org/sage.git/commit/?id=c0ac5a4c0669fdb7e16338048dbc98568facb49f)|`Made some Cartan to Coxeter`|",
    "created_at": "2015-11-05T14:38:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16126",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16126#issuecomment-262594",
    "user": "https://github.com/jplab"
}
```

<a id='comment:8'></a>
Here is a first "very dirty version" of the patch.

Basically, everything is still to be looked over...

This is just for you to have a look. I will continue in more detail soon enough.

---
New commits:
|                                                                                                                                          |                                                 |
|------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------|
|[f2f84eb](http://git.sagemath.org/sage.git/commit/?id=f2f84eb39661876e7e32094f6038181fa139875c)|`Initial commit: added the file coxeter_graph.py`|
|[6a2e20c](http://git.sagemath.org/sage.git/commit/?id=6a2e20c55168350a53efbd218adb913cfe91f0a0)|`Some minor changes to start the patch`|
|[987aefc](http://git.sagemath.org/sage.git/commit/?id=987aefc4037d7eb7a847338d29c7b22b2b0ca3ad)|`Merge branch 'ticket16126' into newticket16126`|
|[afd5e59](http://git.sagemath.org/sage.git/commit/?id=afd5e59d7d65efbb48190e9671752352a32a7ae4)|`Added necessary startup files`|
|[9b135a5](http://git.sagemath.org/sage.git/commit/?id=9b135a5f3cbb5e6e190800880e61fbbd6387f46a)|`Changed the occurences of Dynkin to Coxeter`|
|[c0ac5a4](http://git.sagemath.org/sage.git/commit/?id=c0ac5a4c0669fdb7e16338048dbc98568facb49f)|`Made some Cartan to Coxeter`|



---

archive/issue_comments_262595.json:
```json
{
    "body": "Changing commit from \"\" to \"c0ac5a4c0669fdb7e16338048dbc98568facb49f\"",
    "created_at": "2015-11-05T14:38:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16126",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16126#issuecomment-262595",
    "user": "https://github.com/jplab"
}
```

Changing commit from "" to "c0ac5a4c0669fdb7e16338048dbc98568facb49f"



---

archive/issue_comments_262596.json:
```json
{
    "body": "Set assignee to @jplab.",
    "created_at": "2015-11-05T14:38:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16126",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16126#issuecomment-262596",
    "user": "https://github.com/jplab"
}
```

Set assignee to @jplab.



---

archive/issue_comments_262597.json:
```json
{
    "body": "Changing author from \"\" to \"Jean-Philippe Labb\u00e9\"",
    "created_at": "2015-11-14T19:41:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16126",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16126#issuecomment-262597",
    "user": "https://github.com/fchapoton"
}
```

Changing author from "" to "Jean-Philippe Labbé"



---

archive/issue_comments_262598.json:
```json
{
    "body": "Changing commit from \"c0ac5a4c0669fdb7e16338048dbc98568facb49f\" to \"8cf2af9e2363366692857eeb91c4f4c14eabe46f\"",
    "created_at": "2015-11-26T16:52:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16126",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16126#issuecomment-262598",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "c0ac5a4c0669fdb7e16338048dbc98568facb49f" to "8cf2af9e2363366692857eeb91c4f4c14eabe46f"



---

archive/issue_comments_262599.json:
```json
{
    "body": "<a id='comment:10'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                          |                           |\n|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------|\n|[2975f1c](http://git.sagemath.org/sage.git/commit/?id=2975f1cb65f75cedfcb5376b30c5c92aed57a3f9)|`Some partial broken edits`|\n|[8cf2af9](http://git.sagemath.org/sage.git/commit/?id=8cf2af9e2363366692857eeb91c4f4c14eabe46f)|`Changed each type to construct a Coxeter graph`|",
    "created_at": "2015-11-26T16:52:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16126",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16126#issuecomment-262599",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:10'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                          |                           |
|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
|[2975f1c](http://git.sagemath.org/sage.git/commit/?id=2975f1cb65f75cedfcb5376b30c5c92aed57a3f9)|`Some partial broken edits`|
|[8cf2af9](http://git.sagemath.org/sage.git/commit/?id=8cf2af9e2363366692857eeb91c4f4c14eabe46f)|`Changed each type to construct a Coxeter graph`|



---

archive/issue_comments_262600.json:
```json
{
    "body": "<a id='comment:11'></a>\nSome more broken edits.\n\nNo need to give comments yet... There is still too many things to look after. I just push so you have an overview.\n\nI will come up with a DOTO list for the ticket once things are a bit less messy.\n\nBasically, I decided to follow the structure of CoxeterMatrix instead of DynkinDiagrams... This requires a lot a adaptation and overall modification of everything...\n\nBasically, it seems to me that hardcoding the graphs is the best way to go. From there... everything should go through...\n\nI'm still learning a lot how the structure goes. But it seems that DynkinDiagrams should also be somehow re-engineered so that it fits better?!\n\nBasically, once we have CoxeterGraph and CoxeterMatrix up and running, the next logic step would be to have a parent type for CoxeterGraph-DynkinDiagram and then for CoxeterMatrix, CartanMatrix. But this is for later...",
    "created_at": "2015-11-26T16:57:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16126",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16126#issuecomment-262600",
    "user": "https://github.com/jplab"
}
```

<a id='comment:11'></a>
Some more broken edits.

No need to give comments yet... There is still too many things to look after. I just push so you have an overview.

I will come up with a DOTO list for the ticket once things are a bit less messy.

Basically, I decided to follow the structure of CoxeterMatrix instead of DynkinDiagrams... This requires a lot a adaptation and overall modification of everything...

Basically, it seems to me that hardcoding the graphs is the best way to go. From there... everything should go through...

I'm still learning a lot how the structure goes. But it seems that DynkinDiagrams should also be somehow re-engineered so that it fits better?!

Basically, once we have CoxeterGraph and CoxeterMatrix up and running, the next logic step would be to have a parent type for CoxeterGraph-DynkinDiagram and then for CoxeterMatrix, CartanMatrix. But this is for later...
