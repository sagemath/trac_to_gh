# Issue 16511: Fix solve_right for matrices over IntegerModRing

archive/issues_016274.json:
```json
{
    "assignees": [],
    "body": "Here are two bugs in `Matrix.solve_right()`:\n\n```\nsage: A = Matrix.identity(2)\nsage: B = Matrix.identity(Zmod(8), 2)\nsage: B.solve_right(A)\n...\nAttributeError: 'sage.rings.integer_ring.IntegerRing_class' object has no attribute 'cover_ring'\nsage: B.solve_right(B)\n...\nPariError: syntax error, unexpected ')', expecting )-> or ',': (1,0)\n```\nIn the first case, there is no solution due to the different base rings; we should at least return a more informative error message.  In the second case, `B` itself is the unique solution.\n\nCC:  jakobkroeker\n\nKeywords: **matrix integer_mod solve_right**\n\nBranch/Commit: **[00861a4](https://github.com/sagemath/sagetrac-mirror/commit/00861a43d783a56a1b52fb98a33887dc156648c9)**\n\nStopgaps: **wrongBehaviourMarker**\n\nReviewer: **David Roe**\n\nAuthor: **Simon Brandhorst**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16511_\n\n",
    "closed_at": "2018-05-19T15:56:41Z",
    "created_at": "2014-06-21T14:16:49Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix solve_right for matrices over IntegerModRing",
    "type": "issue",
    "updated_at": "2018-05-19T15:56:41Z",
    "url": "https://github.com/sagemath/sage/issues/16511",
    "user": "https://github.com/pjbruin"
}
```
Here are two bugs in `Matrix.solve_right()`:

```
sage: A = Matrix.identity(2)
sage: B = Matrix.identity(Zmod(8), 2)
sage: B.solve_right(A)
...
AttributeError: 'sage.rings.integer_ring.IntegerRing_class' object has no attribute 'cover_ring'
sage: B.solve_right(B)
...
PariError: syntax error, unexpected ')', expecting )-> or ',': (1,0)
```
In the first case, there is no solution due to the different base rings; we should at least return a more informative error message.  In the second case, `B` itself is the unique solution.

CC:  jakobkroeker

Keywords: **matrix integer_mod solve_right**

Branch/Commit: **[00861a4](https://github.com/sagemath/sagetrac-mirror/commit/00861a43d783a56a1b52fb98a33887dc156648c9)**

Stopgaps: **wrongBehaviourMarker**

Reviewer: **David Roe**

Author: **Simon Brandhorst**

_Issue created by migration from https://trac.sagemath.org/ticket/16511_





---

archive/issue_events_218228.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2014-06-21T14:16:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16511",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16511#event-218228"
}
```



---

archive/issue_events_218229.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2014-06-21T14:16:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16511",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "component: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16511#event-218229"
}
```



---

archive/issue_events_218230.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2014-06-21T14:16:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16511",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16511#event-218230"
}
```



---

archive/issue_events_218231.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2014-06-21T14:16:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16511",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16511#event-218231"
}
```



---

archive/issue_events_218232.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16511",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16511#event-218232"
}
```



---

archive/issue_events_218233.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16511",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16511#event-218233"
}
```



---

archive/issue_comments_216543.json:
```json
{
    "body": "Stopgaps: **wrongBehaviourMarker**",
    "created_at": "2017-03-04T00:51:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16511",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16511#issuecomment-216543",
    "user": "https://github.com/sagetrac-jakobkroeker"
}
```

Stopgaps: **wrongBehaviourMarker**



---

archive/issue_comments_216544.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNow we get \n\n```\nsage: A = Matrix.identity(2)\nsage: B = Matrix.identity(Zmod(8), 2)\nsage: B.solve_right(A)\n\n[1]\n[0]\nsage: B.solve_right(B)\n\n[1]\n[0]\n\n```",
    "created_at": "2017-03-04T00:51:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16511",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16511#issuecomment-216544",
    "user": "https://github.com/sagetrac-jakobkroeker"
}
```

<a id='comment:2'>Comment 2:</a>
Now we get 

```
sage: A = Matrix.identity(2)
sage: B = Matrix.identity(Zmod(8), 2)
sage: B.solve_right(A)

[1]
[0]
sage: B.solve_right(B)

[1]
[0]

```



---

archive/issue_comments_216545.json:
```json
{
    "body": "Branch: **[u/sbrandhorst/fix_solve_right_for_matrices_over_integermodring](https://github.com/sagemath/sagetrac-mirror/tree/u/sbrandhorst/fix_solve_right_for_matrices_over_integermodring)**",
    "created_at": "2018-02-03T10:34:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16511",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16511#issuecomment-216545",
    "user": "https://github.com/simonbrandhorst"
}
```

Branch: **[u/sbrandhorst/fix_solve_right_for_matrices_over_integermodring](https://github.com/sagemath/sagetrac-mirror/tree/u/sbrandhorst/fix_solve_right_for_matrices_over_integermodring)**



---

archive/issue_comments_216546.json:
```json
{
    "body": "Commit: **[677cd07](https://github.com/sagemath/sagetrac-mirror/commit/677cd07a2d2ab44fa28ef4fe31ed39b3d4bee2a9)**",
    "created_at": "2018-02-03T10:34:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16511",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16511#issuecomment-216546",
    "user": "https://github.com/simonbrandhorst"
}
```

Commit: **[677cd07](https://github.com/sagemath/sagetrac-mirror/commit/677cd07a2d2ab44fa28ef4fe31ed39b3d4bee2a9)**



---

archive/issue_comments_216547.json:
```json
{
    "body": "Author: **Simon Brandhorst**",
    "created_at": "2018-02-03T10:34:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16511",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16511#issuecomment-216547",
    "user": "https://github.com/simonbrandhorst"
}
```

Author: **Simon Brandhorst**



---

archive/issue_events_218234.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2018-02-03T10:34:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16511",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16511#event-218234"
}
```



---

archive/issue_comments_216548.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/677cd07a2d2ab44fa28ef4fe31ed39b3d4bee2a9\">677cd07</a></td><td><code>Output a matrix if the input is a matrix</code></td></tr></table>\n",
    "created_at": "2018-02-03T10:34:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16511",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16511#issuecomment-216548",
    "user": "https://github.com/simonbrandhorst"
}
```

<a id='comment:4'>Comment 4:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/677cd07a2d2ab44fa28ef4fe31ed39b3d4bee2a9">677cd07</a></td><td><code>Output a matrix if the input is a matrix</code></td></tr></table>




---

archive/issue_comments_216549.json:
```json
{
    "body": "Reviewer: **David Roe**",
    "created_at": "2018-05-17T17:17:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16511",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16511#issuecomment-216549",
    "user": "https://github.com/roed314"
}
```

Reviewer: **David Roe**



---

archive/issue_comments_216550.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nThe line `ret = A.matsolvemod(K.cardinality(), b)` is unnecessarily repeated.  Also, why is the final return statement removed?",
    "created_at": "2018-05-17T17:17:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16511",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16511#issuecomment-216550",
    "user": "https://github.com/roed314"
}
```

<a id='comment:5'>Comment 5:</a>
The line `ret = A.matsolvemod(K.cardinality(), b)` is unnecessarily repeated.  Also, why is the final return statement removed?



---

archive/issue_events_218235.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2018-05-17T17:17:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16511",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16511#event-218235"
}
```



---

archive/issue_events_218236.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2018-05-17T17:17:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16511",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16511#event-218236"
}
```



---

archive/issue_comments_216551.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c413ed149198e8bb91df98e3fc2981d2c7394d68\">c413ed1</a></td><td><code>Merge branch 'develop' into t/16511/fix_solve_right_for_matrices_over_integermodring</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/00861a43d783a56a1b52fb98a33887dc156648c9\">00861a4</a></td><td><code>removed repeated line and unnecessary if statement</code></td></tr></table>\n",
    "created_at": "2018-05-18T05:55:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16511",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16511#issuecomment-216551",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:6'>Comment 6:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c413ed149198e8bb91df98e3fc2981d2c7394d68">c413ed1</a></td><td><code>Merge branch 'develop' into t/16511/fix_solve_right_for_matrices_over_integermodring</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/00861a43d783a56a1b52fb98a33887dc156648c9">00861a4</a></td><td><code>removed repeated line and unnecessary if statement</code></td></tr></table>




---

archive/issue_comments_216552.json:
```json
{
    "body": "Changed commit from **[677cd07](https://github.com/sagemath/sagetrac-mirror/commit/677cd07a2d2ab44fa28ef4fe31ed39b3d4bee2a9)** to **[00861a4](https://github.com/sagemath/sagetrac-mirror/commit/00861a43d783a56a1b52fb98a33887dc156648c9)**",
    "created_at": "2018-05-18T05:55:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16511",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16511#issuecomment-216552",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[677cd07](https://github.com/sagemath/sagetrac-mirror/commit/677cd07a2d2ab44fa28ef4fe31ed39b3d4bee2a9)** to **[00861a4](https://github.com/sagemath/sagetrac-mirror/commit/00861a43d783a56a1b52fb98a33887dc156648c9)**



---

archive/issue_comments_216553.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nindeed",
    "created_at": "2018-05-18T05:57:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16511",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16511#issuecomment-216553",
    "user": "https://github.com/simonbrandhorst"
}
```

<a id='comment:7'>Comment 7:</a>
indeed



---

archive/issue_events_218237.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2018-05-18T05:57:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16511",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16511#event-218237"
}
```



---

archive/issue_events_218238.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2018-05-18T05:57:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16511",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16511#event-218238"
}
```



---

archive/issue_events_218239.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2018-05-18T16:34:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16511",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16511#event-218239"
}
```



---

archive/issue_events_218240.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2018-05-18T16:34:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16511",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16511#event-218240"
}
```



---

archive/issue_comments_216554.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nLooks good!",
    "created_at": "2018-05-18T16:34:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16511",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16511#issuecomment-216554",
    "user": "https://github.com/roed314"
}
```

<a id='comment:8'>Comment 8:</a>
Looks good!



---

archive/issue_events_218241.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-05-18T18:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16511",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16511#event-218241"
}
```



---

archive/issue_events_218242.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-05-18T18:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16511",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16511#event-218242"
}
```



---

archive/issue_events_218243.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-05-19T15:56:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16511",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16511#event-218243"
}
```



---

archive/issue_events_218244.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "5ccb0a295f4b89a236a193ec1ae8c3ed691e05bf",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-05-19T15:56:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16511",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16511#event-218244"
}
```



---

archive/issue_comments_216555.json:
```json
{
    "body": "Changed branch from **[u/sbrandhorst/fix_solve_right_for_matrices_over_integermodring](https://github.com/sagemath/sagetrac-mirror/tree/u/sbrandhorst/fix_solve_right_for_matrices_over_integermodring)** to **[00861a4](https://github.com/sagemath/sagetrac-mirror/commit/00861a43d783a56a1b52fb98a33887dc156648c9)**",
    "created_at": "2018-05-19T15:56:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16511",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16511#issuecomment-216555",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/sbrandhorst/fix_solve_right_for_matrices_over_integermodring](https://github.com/sagemath/sagetrac-mirror/tree/u/sbrandhorst/fix_solve_right_for_matrices_over_integermodring)** to **[00861a4](https://github.com/sagemath/sagetrac-mirror/commit/00861a43d783a56a1b52fb98a33887dc156648c9)**
