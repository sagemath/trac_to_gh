# Issue 16290: UnboundLocalError in jordan_form

archive/issues_016053.json:
```json
{
    "assignees": [],
    "body": "Using sage-6.0:\n\n```\nsage: m = identity_matrix(2)\nsage: m.jordan_form()\n[1|0]\n[-+-]\n[0|1]\nsage: m.jordan_form(base_ring=QQ)\nTraceback (most recent call last):\n...\nUnboundLocalError: local variable 'A' referenced before assignment\n```\n\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16290_\n\n",
    "closed_at": "2014-05-19T10:29:36Z",
    "created_at": "2014-05-05T14:10:07Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "UnboundLocalError in jordan_form",
    "type": "issue",
    "updated_at": "2014-05-19T10:29:36Z",
    "url": "https://github.com/sagemath/sage/issues/16290",
    "user": "https://github.com/seblabbe"
}
```
Using sage-6.0:

```
sage: m = identity_matrix(2)
sage: m.jordan_form()
[1|0]
[-+-]
[0|1]
sage: m.jordan_form(base_ring=QQ)
Traceback (most recent call last):
...
UnboundLocalError: local variable 'A' referenced before assignment
```


_Issue created by migration from https://trac.sagemath.org/ticket/16290_





---

archive/issue_events_215171.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2014-05-05T14:10:07Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16290",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16290#event-215171"
}
```



---

archive/issue_events_215172.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2014-05-05T14:10:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16290",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "component: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16290#event-215172"
}
```



---

archive/issue_events_215173.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2014-05-05T14:10:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16290",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16290#event-215173"
}
```



---

archive/issue_events_215174.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2014-05-05T14:10:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16290",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16290#event-215174"
}
```



---

archive/issue_events_215175.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2014-05-05T19:56:50Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16290",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16290#event-215175"
}
```



---

archive/issue_events_215176.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2014-05-05T19:56:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16290",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16290#event-215176"
}
```



---

archive/issue_events_215177.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2014-05-05T19:56:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16290",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16290#event-215177"
}
```



---

archive/issue_comments_211625.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nSorry, this is a duplicate of #14508.",
    "created_at": "2014-05-05T19:56:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16290",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16290#issuecomment-211625",
    "user": "https://github.com/seblabbe"
}
```

<div id="comment:1" align="right">Comment 1</div>

Sorry, this is a duplicate of #14508.



---

archive/issue_events_215178.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2014-05-15T20:39:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16290",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16290#event-215178"
}
```



---

archive/issue_events_215179.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2014-05-15T20:39:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16290",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16290#event-215179"
}
```



---

archive/issue_events_215180.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2014-05-15T20:39:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16290",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16290#event-215180"
}
```



---

archive/issue_events_215181.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-05-19T10:29:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16290",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16290#event-215181"
}
```



---

archive/issue_events_215182.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-05-19T10:29:36Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16290",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16290#event-215182"
}
```



---

archive/issue_events_215183.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-05-19T10:29:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16290",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16290#event-215183"
}
```
