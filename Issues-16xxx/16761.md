# Issue 16761: symbolic_expression: Better handling of vectors and matrices

archive/issues_016524.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: symbolic_expression(vector([1,2]))\n<boom>\nTypeError:\n```\nThis is pretty uninformative, esp. when it is in [the midst of a larger computation](http://ask.sagemath.org/question/23690/how-to-fit-data-to-an-arrhenius-equation/).\n\nInstead of working on the error message, we add some code to handle vector and matrix inputs.\n\n\nDepends on #14270\n\nCC:  @tscrim\n\nBranch/Commit: **[0623d79](https://github.com/sagemath/sagetrac-mirror/commit/0623d795c7e1d8a90cc06d9fa0b618f4ee4e9348)**\n\nReviewer: **Karl-Dieter Crisman, Travis Scrimshaw**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16761_\n\n",
    "closed_at": "2021-08-29T09:37:59Z",
    "created_at": "2014-08-05T01:36:04Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "symbolic_expression: Better handling of vectors and matrices",
    "type": "issue",
    "updated_at": "2021-08-29T09:37:59Z",
    "url": "https://github.com/sagemath/sage/issues/16761",
    "user": "https://github.com/kcrisman"
}
```

```
sage: symbolic_expression(vector([1,2]))
<boom>
TypeError:
```
This is pretty uninformative, esp. when it is in [the midst of a larger computation](http://ask.sagemath.org/question/23690/how-to-fit-data-to-an-arrhenius-equation/).

Instead of working on the error message, we add some code to handle vector and matrix inputs.


Depends on #14270

CC:  @tscrim

Branch/Commit: **[0623d79](https://github.com/sagemath/sagetrac-mirror/commit/0623d795c7e1d8a90cc06d9fa0b618f4ee4e9348)**

Reviewer: **Karl-Dieter Crisman, Travis Scrimshaw**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/16761_





---

archive/issue_events_221966.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-08-05T01:36:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16761#event-221966"
}
```



---

archive/issue_events_221967.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-08-05T01:36:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16761#event-221967"
}
```



---

archive/issue_events_221968.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-08-05T01:36:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16761#event-221968"
}
```



---

archive/issue_events_221969.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-08-05T01:36:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16761#event-221969"
}
```



---

archive/issue_events_221970.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16761#event-221970"
}
```



---

archive/issue_events_221971.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16761#event-221971"
}
```



---

archive/issue_events_221972.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-06T03:31:19Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16761#event-221972"
}
```



---

archive/issue_events_221973.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-06T03:31:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16761#event-221973"
}
```



---

archive/issue_comments_222049.json:
```json
{
    "body": "Dependencies: **#32103**",
    "created_at": "2021-07-06T03:31:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222049",
    "user": "https://github.com/mkoeppe"
}
```

Dependencies: **#32103**



---

archive/issue_comments_222050.json:
```json
{
    "body": "Branch: **[u/mkoeppe/error_message_bad_when_trying_to_make_vector_symbolic_expression](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/error_message_bad_when_trying_to_make_vector_symbolic_expression)**",
    "created_at": "2021-07-06T05:02:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222050",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/error_message_bad_when_trying_to_make_vector_symbolic_expression](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/error_message_bad_when_trying_to_make_vector_symbolic_expression)**



---

archive/issue_events_221974.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-06T05:03:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16761#event-221974"
}
```



---

archive/issue_comments_222051.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e523ac00d9c89e36a6341a731633b12e87ee1bd7\">e523ac0</a></td><td><code>sage.calculus.all.symbolic_expression: Handle callables</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d2e8c8f402167c5ecc0fe0ee6514ce9e21fa6bd1\">d2e8c8f</a></td><td><code>symbolic_expression: Update doctest output</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0b9408efe48d3344ef39113ba2097cefb85704eb\">0b9408e</a></td><td><code>Merge #32103</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5e3e34fab35aa537d978b11df4bb2666040f7071\">5e3e34f</a></td><td><code>symbolic_expression: Handle vectors and matrices; fix result parent for list/tuple of length 0</code></td></tr></table>\n",
    "created_at": "2021-07-06T05:03:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222051",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:4" align="right">Comment 4</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e523ac00d9c89e36a6341a731633b12e87ee1bd7">e523ac0</a></td><td><code>sage.calculus.all.symbolic_expression: Handle callables</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d2e8c8f402167c5ecc0fe0ee6514ce9e21fa6bd1">d2e8c8f</a></td><td><code>symbolic_expression: Update doctest output</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0b9408efe48d3344ef39113ba2097cefb85704eb">0b9408e</a></td><td><code>Merge #32103</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5e3e34fab35aa537d978b11df4bb2666040f7071">5e3e34f</a></td><td><code>symbolic_expression: Handle vectors and matrices; fix result parent for list/tuple of length 0</code></td></tr></table>




---

archive/issue_comments_222052.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -6,9 +6,5 @@\n ```\n This is pretty uninformative, esp. when it is in [the midst of a larger computation](http://ask.sagemath.org/question/23690/how-to-fit-data-to-an-arrhenius-equation/).\n \n-I'm not suggesting we make a vector a symbolic expression, though of course \n+Instead of working on the error message, we add some code to handle vector and matrix inputs.\n \n-```\n-sage: vector([x,2*x])\n-```\n-works so we could go that way.\n``````\n",
    "created_at": "2021-07-06T05:03:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222052",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -6,9 +6,5 @@
 ```
 This is pretty uninformative, esp. when it is in [the midst of a larger computation](http://ask.sagemath.org/question/23690/how-to-fit-data-to-an-arrhenius-equation/).
 
-I'm not suggesting we make a vector a symbolic expression, though of course 
+Instead of working on the error message, we add some code to handle vector and matrix inputs.
 
-```
-sage: vector([x,2*x])
-```
-works so we could go that way.
``````




---

archive/issue_comments_222053.json:
```json
{
    "body": "Commit: **[5e3e34f](https://github.com/sagemath/sagetrac-mirror/commit/5e3e34fab35aa537d978b11df4bb2666040f7071)**",
    "created_at": "2021-07-06T05:03:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222053",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[5e3e34f](https://github.com/sagemath/sagetrac-mirror/commit/5e3e34fab35aa537d978b11df4bb2666040f7071)**



---

archive/issue_comments_222054.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2021-07-06T05:03:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222054",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_comments_222055.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nIt's always so great to wake up to someone working on a ticket from seven years ago :-)\n\nI agree that this is the preferred way to handle it, and on a first pass the code looks good for it.  I do have two usage questions.\n\n```\nsage: f = symbolic_expression(lambda z: z^2 + 1); f\n```\nAm I correct that this does *not* inject `z` into the global namespace?  This seems to be how you constructed it with `SR.var`.\n\nAlso, I'm not familiar with `SR.subring(no_variables=True)`, which is presumably what generates the \"Symbolic Constants Subring\".  If someone later wanted to take something like `symbolic_expression(vector(QQ, 0))` and then add it to another (zero-dimensional) vector, how would that behave?  Would it complain because it has a different base ring?  Or would it do the \"obvious\" thing in any of these degenerate cases?  I don't know if there is a \"right\" answer here (for degenerate cases), but I wanted to ask.\n\nFinally, it could be fun to exemplify the recursive nature of the code by having a vector composed of vector functions or something.  Though I doubt that would be a real use case; this ticket is more to make sure that if people insist on making everything \"symbolic\" that nothing goes boom.",
    "created_at": "2021-07-06T12:05:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222055",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:5" align="right">Comment 5</div>

It's always so great to wake up to someone working on a ticket from seven years ago :-)

I agree that this is the preferred way to handle it, and on a first pass the code looks good for it.  I do have two usage questions.

```
sage: f = symbolic_expression(lambda z: z^2 + 1); f
```
Am I correct that this does *not* inject `z` into the global namespace?  This seems to be how you constructed it with `SR.var`.

Also, I'm not familiar with `SR.subring(no_variables=True)`, which is presumably what generates the "Symbolic Constants Subring".  If someone later wanted to take something like `symbolic_expression(vector(QQ, 0))` and then add it to another (zero-dimensional) vector, how would that behave?  Would it complain because it has a different base ring?  Or would it do the "obvious" thing in any of these degenerate cases?  I don't know if there is a "right" answer here (for degenerate cases), but I wanted to ask.

Finally, it could be fun to exemplify the recursive nature of the code by having a vector composed of vector functions or something.  Though I doubt that would be a real use case; this ticket is more to make sure that if people insist on making everything "symbolic" that nothing goes boom.



---

archive/issue_comments_222056.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nReplying to [@kcrisman](#comment%3A5):\n> \n> ```\n> sage: f = symbolic_expression(lambda z: z^2 + 1); f\n> ```\n> Am I correct that this does *not* inject `z` into the global namespace?  This seems to be how you constructed it with `SR.var`.\n\nThat's right. (Note this is from #32103, not the present ticket.)",
    "created_at": "2021-07-06T12:34:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222056",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:6" align="right">Comment 6</div>

Replying to [@kcrisman](#comment%3A5):
> 
> ```
> sage: f = symbolic_expression(lambda z: z^2 + 1); f
> ```
> Am I correct that this does *not* inject `z` into the global namespace?  This seems to be how you constructed it with `SR.var`.

That's right. (Note this is from #32103, not the present ticket.)



---

archive/issue_comments_222057.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nReplying to [@kcrisman](#comment%3A5):\n> Also, I'm not familiar with `SR.subring(no_variables=True)`, which is presumably what generates the \"Symbolic Constants Subring\".  If someone later wanted to take something like `symbolic_expression(vector(QQ, 0))` and then add it to another (zero-dimensional) vector, how would that behave?\n\nThe coercion system takes care of it:\n\n```\nsage: vector(SR.subring(no_variables=True), 0) + vector(SR, 0) + vector(ZZ, 0)\n()\nsage: _.parent()\nVector space of dimension 0 over Symbolic Ring\n```",
    "created_at": "2021-07-06T12:36:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222057",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:7" align="right">Comment 7</div>

Replying to [@kcrisman](#comment%3A5):
> Also, I'm not familiar with `SR.subring(no_variables=True)`, which is presumably what generates the "Symbolic Constants Subring".  If someone later wanted to take something like `symbolic_expression(vector(QQ, 0))` and then add it to another (zero-dimensional) vector, how would that behave?

The coercion system takes care of it:

```
sage: vector(SR.subring(no_variables=True), 0) + vector(SR, 0) + vector(ZZ, 0)
()
sage: _.parent()
Vector space of dimension 0 over Symbolic Ring
```



---

archive/issue_comments_222058.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nReplying to [@kcrisman](#comment%3A5):\n > it could be fun to exemplify the recursive nature of the code by having a vector composed of vector functions or something.  Though I doubt that would be a real use case; this ticket is more to make sure that if people insist on making everything \"symbolic\" that nothing goes boom.\n\nI think I'd rather not advertise more complicated possible uses of this function at this point. I only became aware of its existence when I was looking for a preparser-less idiom for creating callable symbolic expressions (#32103).",
    "created_at": "2021-07-06T12:43:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222058",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:8" align="right">Comment 8</div>

Replying to [@kcrisman](#comment%3A5):
 > it could be fun to exemplify the recursive nature of the code by having a vector composed of vector functions or something.  Though I doubt that would be a real use case; this ticket is more to make sure that if people insist on making everything "symbolic" that nothing goes boom.

I think I'd rather not advertise more complicated possible uses of this function at this point. I only became aware of its existence when I was looking for a preparser-less idiom for creating callable symbolic expressions (#32103).



---

archive/issue_comments_222059.json:
```json
{
    "body": "Reviewer: **Karl-Dieter Crisman**",
    "created_at": "2021-07-06T12:49:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222059",
    "user": "https://github.com/kcrisman"
}
```

Reviewer: **Karl-Dieter Crisman**



---

archive/issue_comments_222060.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\n> I only became aware of its existence when I was looking for a preparser-less idiom for creating callable symbolic expressions (#32103). \n\nI see now.  I certainly can give positive review to the changes for this ticket (modulo green patchbot).  I'll make one or two other comments there.  Thanks for fixing this!",
    "created_at": "2021-07-06T12:49:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222060",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:9" align="right">Comment 9</div>

> I only became aware of its existence when I was looking for a preparser-less idiom for creating callable symbolic expressions (#32103). 

I see now.  I certainly can give positive review to the changes for this ticket (modulo green patchbot).  I'll make one or two other comments there.  Thanks for fixing this!



---

archive/issue_comments_222061.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2b5535ff600251430fee311881d319fea5e65082\">2b5535f</a></td><td><code>sage.calculus.all.symbolic_expression: Add more tests</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b2c8b8ec7df50c65a739f262cf7adee5baada8b3\">b2c8b8e</a></td><td><code>Merge #32103</code></td></tr></table>\n",
    "created_at": "2021-07-06T13:41:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222061",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:10" align="right">Comment 10</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2b5535ff600251430fee311881d319fea5e65082">2b5535f</a></td><td><code>sage.calculus.all.symbolic_expression: Add more tests</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b2c8b8ec7df50c65a739f262cf7adee5baada8b3">b2c8b8e</a></td><td><code>Merge #32103</code></td></tr></table>




---

archive/issue_comments_222062.json:
```json
{
    "body": "Changed commit from **[5e3e34f](https://github.com/sagemath/sagetrac-mirror/commit/5e3e34fab35aa537d978b11df4bb2666040f7071)** to **[b2c8b8e](https://github.com/sagemath/sagetrac-mirror/commit/b2c8b8ec7df50c65a739f262cf7adee5baada8b3)**",
    "created_at": "2021-07-06T13:41:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222062",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[5e3e34f](https://github.com/sagemath/sagetrac-mirror/commit/5e3e34fab35aa537d978b11df4bb2666040f7071)** to **[b2c8b8e](https://github.com/sagemath/sagetrac-mirror/commit/b2c8b8ec7df50c65a739f262cf7adee5baada8b3)**



---

archive/issue_events_221975.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-26T17:55:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16761#event-221975"
}
```



---

archive/issue_events_221976.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-26T17:55:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16761#event-221976"
}
```



---

archive/issue_comments_222063.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nDoctest failures",
    "created_at": "2021-07-26T17:55:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222063",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:11" align="right">Comment 11</div>

Doctest failures



---

archive/issue_comments_222064.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a4ace52bfddee6a937458553025bdbd195ba9236\">a4ace52</a></td><td><code>Merge tag '9.4.beta6' into t/16761/error_message_bad_when_trying_to_make_vector_symbolic_expression</code></td></tr></table>\n",
    "created_at": "2021-07-26T20:14:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222064",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:12" align="right">Comment 12</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a4ace52bfddee6a937458553025bdbd195ba9236">a4ace52</a></td><td><code>Merge tag '9.4.beta6' into t/16761/error_message_bad_when_trying_to_make_vector_symbolic_expression</code></td></tr></table>




---

archive/issue_comments_222065.json:
```json
{
    "body": "Changed commit from **[b2c8b8e](https://github.com/sagemath/sagetrac-mirror/commit/b2c8b8ec7df50c65a739f262cf7adee5baada8b3)** to **[a4ace52](https://github.com/sagemath/sagetrac-mirror/commit/a4ace52bfddee6a937458553025bdbd195ba9236)**",
    "created_at": "2021-07-26T20:14:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222065",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[b2c8b8e](https://github.com/sagemath/sagetrac-mirror/commit/b2c8b8ec7df50c65a739f262cf7adee5baada8b3)** to **[a4ace52](https://github.com/sagemath/sagetrac-mirror/commit/a4ace52bfddee6a937458553025bdbd195ba9236)**



---

archive/issue_events_221977.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-26T20:15:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16761#event-221977"
}
```



---

archive/issue_events_221978.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-26T20:15:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16761#event-221978"
}
```



---

archive/issue_comments_222066.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nDo we also want to support the case when a list of lists is passed to create a symbolic matrix? It could be done by\n\n```diff\n     elif isinstance(x, (tuple, list)) or is_FreeModuleElement(x):\n         expressions = [symbolic_expression(item) for item in x]\n         if not expressions:\n             # Make sure it is symbolic also when length is 0\n             return vector(SR.subring(no_variables=True), 0)\n+        if is_FreeModuleElement(expressions[0]):\n+            return matrix(expressions)\n         return vector(expressions)\n```",
    "created_at": "2021-07-26T22:43:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222066",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:14" align="right">Comment 14</div>

Do we also want to support the case when a list of lists is passed to create a symbolic matrix? It could be done by

```diff
     elif isinstance(x, (tuple, list)) or is_FreeModuleElement(x):
         expressions = [symbolic_expression(item) for item in x]
         if not expressions:
             # Make sure it is symbolic also when length is 0
             return vector(SR.subring(no_variables=True), 0)
+        if is_FreeModuleElement(expressions[0]):
+            return matrix(expressions)
         return vector(expressions)
```



---

archive/issue_comments_222067.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">Comment 15</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/eee0def9032500f85de574d82ba51f1e904af071\">eee0def</a></td><td><code>symbolic_expression: Make matrices from lists of lists etc.</code></td></tr></table>\n",
    "created_at": "2021-07-26T23:45:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222067",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:15" align="right">Comment 15</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/eee0def9032500f85de574d82ba51f1e904af071">eee0def</a></td><td><code>symbolic_expression: Make matrices from lists of lists etc.</code></td></tr></table>




---

archive/issue_comments_222068.json:
```json
{
    "body": "Changed commit from **[a4ace52](https://github.com/sagemath/sagetrac-mirror/commit/a4ace52bfddee6a937458553025bdbd195ba9236)** to **[eee0def](https://github.com/sagemath/sagetrac-mirror/commit/eee0def9032500f85de574d82ba51f1e904af071)**",
    "created_at": "2021-07-26T23:45:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222068",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[a4ace52](https://github.com/sagemath/sagetrac-mirror/commit/a4ace52bfddee6a937458553025bdbd195ba9236)** to **[eee0def](https://github.com/sagemath/sagetrac-mirror/commit/eee0def9032500f85de574d82ba51f1e904af071)**



---

archive/issue_comments_222069.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">Comment 16</div>\n\nGood idea, done",
    "created_at": "2021-07-26T23:46:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222069",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:16" align="right">Comment 16</div>

Good idea, done



---

archive/issue_comments_222070.json:
```json
{
    "body": "<div id=\"comment:17\" align=\"right\">Comment 17</div>\n\nThanks. Green bot => positive review.",
    "created_at": "2021-07-27T01:53:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222070",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:17" align="right">Comment 17</div>

Thanks. Green bot => positive review.



---

archive/issue_comments_222071.json:
```json
{
    "body": "Changed reviewer from **Karl-Dieter Crisman** to **Karl-Dieter Crisman, Travis Scrimshaw**",
    "created_at": "2021-07-27T01:53:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222071",
    "user": "https://github.com/tscrim"
}
```

Changed reviewer from **Karl-Dieter Crisman** to **Karl-Dieter Crisman, Travis Scrimshaw**



---

archive/issue_comments_222072.json:
```json
{
    "body": "Changed dependencies from **#32103** to **#14270**",
    "created_at": "2021-08-03T18:54:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222072",
    "user": "https://github.com/mkoeppe"
}
```

Changed dependencies from **#32103** to **#14270**



---

archive/issue_comments_222073.json:
```json
{
    "body": "<div id=\"comment:19\" align=\"right\">Comment 19</div>\n\nUsing the symbolic constants ring actually depends on #31999, #31988",
    "created_at": "2021-08-03T19:13:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222073",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:19" align="right">Comment 19</div>

Using the symbolic constants ring actually depends on #31999, #31988



---

archive/issue_comments_222074.json:
```json
{
    "body": "Changed commit from **[eee0def](https://github.com/sagemath/sagetrac-mirror/commit/eee0def9032500f85de574d82ba51f1e904af071)** to **[9666263](https://github.com/sagemath/sagetrac-mirror/commit/9666263fa7e56a0dccead48fcae3b6555ec539ac)**",
    "created_at": "2021-08-03T19:32:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222074",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[eee0def](https://github.com/sagemath/sagetrac-mirror/commit/eee0def9032500f85de574d82ba51f1e904af071)** to **[9666263](https://github.com/sagemath/sagetrac-mirror/commit/9666263fa7e56a0dccead48fcae3b6555ec539ac)**



---

archive/issue_comments_222075.json:
```json
{
    "body": "<div id=\"comment:20\" align=\"right\">Comment 20</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9666263fa7e56a0dccead48fcae3b6555ec539ac\">9666263</a></td><td><code>symbolic_expression: Do not try to use the Symbolic Constants Subring when creating matrices/vectors with dim 0</code></td></tr></table>\n",
    "created_at": "2021-08-03T19:32:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222075",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:20" align="right">Comment 20</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9666263fa7e56a0dccead48fcae3b6555ec539ac">9666263</a></td><td><code>symbolic_expression: Do not try to use the Symbolic Constants Subring when creating matrices/vectors with dim 0</code></td></tr></table>




---

archive/issue_comments_222076.json:
```json
{
    "body": "<div id=\"comment:21\" align=\"right\">Comment 21</div>\n\nAnother issue remains - symbolic matrices have no `function` method\n\n```\nsage -t --random-seed=0 src/sage/symbolic/expression.pyx\n**********************************************************************\nFile \"src/sage/symbolic/expression.pyx\", line 3186, in sage.symbolic.expression.Expression.__nonzero__\nFailed example:\n    f(x) = matrix()\nException raised:\n    Traceback (most recent call last):\n      File \"/Users/mkoeppe/s/sage/sage-rebasing/worktree-gcc11/src/sage/doctest/forker.py\", line 718, in _run\n        self.compile_and_execute(example, compiler, test.globs)\n      File \"/Users/mkoeppe/s/sage/sage-rebasing/worktree-gcc11/src/sage/doctest/forker.py\", line 1137, in compile_and_execute\n        exec(compiled, globs)\n      File \"<doctest sage.symbolic.expression.Expression.__nonzero__[87]>\", line 1, in <module>\n        __tmp__=var(\"x\"); f = symbolic_expression(matrix()).function(x)\n      File \"sage/structure/element.pyx\", line 493, in sage.structure.element.Element.__getattr__\n        return self.getattr_from_category(name)\n      File \"sage/structure/element.pyx\", line 506, in sage.structure.element.Element.getattr_from_category\n        return getattr_from_other_class(self, cls, name)\n      File \"sage/cpython/getattr.pyx\", line 372, in sage.cpython.getattr.getattr_from_other_class\n        raise AttributeError(dummy_error_message)\n    AttributeError: 'sage.matrix.matrix_symbolic_dense.Matrix_symbolic_dense' object has no attribute 'function'\n```",
    "created_at": "2021-08-03T19:37:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222076",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:21" align="right">Comment 21</div>

Another issue remains - symbolic matrices have no `function` method

```
sage -t --random-seed=0 src/sage/symbolic/expression.pyx
**********************************************************************
File "src/sage/symbolic/expression.pyx", line 3186, in sage.symbolic.expression.Expression.__nonzero__
Failed example:
    f(x) = matrix()
Exception raised:
    Traceback (most recent call last):
      File "/Users/mkoeppe/s/sage/sage-rebasing/worktree-gcc11/src/sage/doctest/forker.py", line 718, in _run
        self.compile_and_execute(example, compiler, test.globs)
      File "/Users/mkoeppe/s/sage/sage-rebasing/worktree-gcc11/src/sage/doctest/forker.py", line 1137, in compile_and_execute
        exec(compiled, globs)
      File "<doctest sage.symbolic.expression.Expression.__nonzero__[87]>", line 1, in <module>
        __tmp__=var("x"); f = symbolic_expression(matrix()).function(x)
      File "sage/structure/element.pyx", line 493, in sage.structure.element.Element.__getattr__
        return self.getattr_from_category(name)
      File "sage/structure/element.pyx", line 506, in sage.structure.element.Element.getattr_from_category
        return getattr_from_other_class(self, cls, name)
      File "sage/cpython/getattr.pyx", line 372, in sage.cpython.getattr.getattr_from_other_class
        raise AttributeError(dummy_error_message)
    AttributeError: 'sage.matrix.matrix_symbolic_dense.Matrix_symbolic_dense' object has no attribute 'function'
```



---

archive/issue_comments_222077.json:
```json
{
    "body": "<div id=\"comment:22\" align=\"right\">Comment 22</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0623d795c7e1d8a90cc06d9fa0b618f4ee4e9348\">0623d79</a></td><td><code>Matrix_symbolic_dense.function: New</code></td></tr></table>\n",
    "created_at": "2021-08-03T19:49:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222077",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:22" align="right">Comment 22</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0623d795c7e1d8a90cc06d9fa0b618f4ee4e9348">0623d79</a></td><td><code>Matrix_symbolic_dense.function: New</code></td></tr></table>




---

archive/issue_comments_222078.json:
```json
{
    "body": "Changed commit from **[9666263](https://github.com/sagemath/sagetrac-mirror/commit/9666263fa7e56a0dccead48fcae3b6555ec539ac)** to **[0623d79](https://github.com/sagemath/sagetrac-mirror/commit/0623d795c7e1d8a90cc06d9fa0b618f4ee4e9348)**",
    "created_at": "2021-08-03T19:49:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222078",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[9666263](https://github.com/sagemath/sagetrac-mirror/commit/9666263fa7e56a0dccead48fcae3b6555ec539ac)** to **[0623d79](https://github.com/sagemath/sagetrac-mirror/commit/0623d795c7e1d8a90cc06d9fa0b618f4ee4e9348)**



---

archive/issue_comments_222079.json:
```json
{
    "body": "<div id=\"comment:23\" align=\"right\">Comment 23</div>\n\nThis works now but it is not perfect yet because we do not have a matrix element class in analogy to `Vector_callable_symbolic_dense`, so the repr of such callable symbolic matrices does not look nice. But this is a cosmetic wishlist item that is already noted in #31988.",
    "created_at": "2021-08-03T20:03:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222079",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:23" align="right">Comment 23</div>

This works now but it is not perfect yet because we do not have a matrix element class in analogy to `Vector_callable_symbolic_dense`, so the repr of such callable symbolic matrices does not look nice. But this is a cosmetic wishlist item that is already noted in #31988.



---

archive/issue_events_221979.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-03T20:05:05Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "title_is": "symbolic_expression: Better handling of vectors and matrices",
    "title_was": "Error message bad when trying to make vector symbolic expression",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16761#event-221979"
}
```



---

archive/issue_comments_222080.json:
```json
{
    "body": "<div id=\"comment:25\" align=\"right\">Comment 25</div>\n\nI have noticed in the past other inconsistencies between matrices and vectors over symbolic rings with elements in SR. I just can't remember them now. We can just add any missing functionality as we come across it.\n\nAnyways, the current branch LGTM.",
    "created_at": "2021-08-05T22:40:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222080",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:25" align="right">Comment 25</div>

I have noticed in the past other inconsistencies between matrices and vectors over symbolic rings with elements in SR. I just can't remember them now. We can just add any missing functionality as we come across it.

Anyways, the current branch LGTM.



---

archive/issue_events_221980.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2021-08-05T22:40:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16761#event-221980"
}
```



---

archive/issue_events_221981.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2021-08-05T22:40:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16761#event-221981"
}
```



---

archive/issue_comments_222081.json:
```json
{
    "body": "<div id=\"comment:26\" align=\"right\">Comment 26</div>\n\nThanks!",
    "created_at": "2021-08-09T20:01:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222081",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:26" align="right">Comment 26</div>

Thanks!



---

archive/issue_events_221982.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-09T21:07:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16761#event-221982"
}
```



---

archive/issue_events_221983.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-09T21:07:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16761#event-221983"
}
```



---

archive/issue_comments_222082.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/error_message_bad_when_trying_to_make_vector_symbolic_expression](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/error_message_bad_when_trying_to_make_vector_symbolic_expression)** to **[0623d79](https://github.com/sagemath/sagetrac-mirror/commit/0623d795c7e1d8a90cc06d9fa0b618f4ee4e9348)**",
    "created_at": "2021-08-29T09:37:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16761#issuecomment-222082",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/error_message_bad_when_trying_to_make_vector_symbolic_expression](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/error_message_bad_when_trying_to_make_vector_symbolic_expression)** to **[0623d79](https://github.com/sagemath/sagetrac-mirror/commit/0623d795c7e1d8a90cc06d9fa0b618f4ee4e9348)**



---

archive/issue_events_221984.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-08-29T09:37:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16761#event-221984"
}
```



---

archive/issue_events_221985.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "3ecf4ba9042ea49a79ab7222d429776039a5b168",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-08-29T09:37:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16761",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16761#event-221985"
}
```
