# Issue 16897: use pip for some optional/experimental python packages

archive/issues_016660.json:
```json
{
    "body": "Instead of using the home made `sage-spkg` for unpatched Python packages it would be simpler/better to use pip (no worries about upgrades, etc). As examples, we have the following optional packages that can be used to test the switch\n- beautiful soup \n- pyopenssl\n- pyqtx\n- pyzmq\n\nReviewer: Jeroen Demeyer\n\nAuthor: Vincent Delecroix\n\nResolution: wontfix\n\nIssue created by migration from https://trac.sagemath.org/ticket/16897\n\n",
    "closed_at": "2017-07-13T07:54:31Z",
    "created_at": "2014-08-28T14:08:01Z",
    "labels": [
        "component: build"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "use pip for some optional/experimental python packages",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/16897",
    "user": "https://github.com/videlec"
}
```
Instead of using the home made `sage-spkg` for unpatched Python packages it would be simpler/better to use pip (no worries about upgrades, etc). As examples, we have the following optional packages that can be used to test the switch
- beautiful soup 
- pyopenssl
- pyqtx
- pyzmq

Reviewer: Jeroen Demeyer

Author: Vincent Delecroix

Resolution: wontfix

Issue created by migration from https://trac.sagemath.org/ticket/16897





---

archive/issue_comments_283017.json:
```json
{
    "body": "<a id='comment:1'></a>I guess the simplest would be to make `pip` a standard package and create the command `sage -pip`.\n\nThen for each of the listed package in the description, the `sage-spkg` script will just raises an error explaining the new procedure\n\n```\nThe packages XYZ is now handled with pip (a package manager for Python).\nTo install it, just do\n   $ sage -pip install XYZ [extra_options]\nFor further informations, refer to the documentation at\nhttp://pip.readthedocs.org\n```\n\nAnother option is to redirect `sage-spkg` input to `pip` if everything else failed but I do not like that so much.\n\nVincent",
    "created_at": "2014-08-28T14:16:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16897#issuecomment-283017",
    "user": "https://github.com/videlec"
}
```

<a id='comment:1'></a>I guess the simplest would be to make `pip` a standard package and create the command `sage -pip`.

Then for each of the listed package in the description, the `sage-spkg` script will just raises an error explaining the new procedure

```
The packages XYZ is now handled with pip (a package manager for Python).
To install it, just do
   $ sage -pip install XYZ [extra_options]
For further informations, refer to the documentation at
http://pip.readthedocs.org
```

Another option is to redirect `sage-spkg` input to `pip` if everything else failed but I do not like that so much.

Vincent



---

archive/issue_comments_283018.json:
```json
{
    "body": "<a id='comment:2'></a>#20218 offers a middle ground--it keeps `spkg-install` (in particular since at least some packages need a little more work before it can run `pip`).  But for most Python packages the sole function of `spkg-install` is to cd into its source and run `pip`.\n\nI think having a proper `spkg-install` script for each package, even if it's trivial, is ultimately simpler and less confusing than having different build/install paths for different kinds of packages that meet a very specific set up conditions.  Better that all packages have an `spkg-install` documenting \"this is how you install this package\", and not require a priori knowledge that some arbitrary package happens to be a Python module distribution.",
    "created_at": "2016-05-27T12:00:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16897#issuecomment-283018",
    "user": "https://github.com/embray"
}
```

<a id='comment:2'></a>#20218 offers a middle ground--it keeps `spkg-install` (in particular since at least some packages need a little more work before it can run `pip`).  But for most Python packages the sole function of `spkg-install` is to cd into its source and run `pip`.

I think having a proper `spkg-install` script for each package, even if it's trivial, is ultimately simpler and less confusing than having different build/install paths for different kinds of packages that meet a very specific set up conditions.  Better that all packages have an `spkg-install` documenting "this is how you install this package", and not require a priori knowledge that some arbitrary package happens to be a Python module distribution.



---

archive/issue_comments_283019.json:
```json
{
    "body": "<a id='comment:3'></a>We have `pip` packages now.",
    "created_at": "2017-04-19T13:16:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16897#issuecomment-283019",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:3'></a>We have `pip` packages now.



---

archive/issue_comments_283020.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Jeroen Demeyer\"",
    "created_at": "2017-04-19T13:16:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16897#issuecomment-283020",
    "user": "https://github.com/jdemeyer"
}
```

Changing reviewer from "" to "Jeroen Demeyer"



---

archive/issue_events_049362.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-19T13:16:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16897",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16897#event-49362"
}
```



---

archive/issue_comments_283021.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2017-04-19T13:16:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16897#issuecomment-283021",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_283022.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2017-04-19T13:16:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16897#issuecomment-283022",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_events_049363.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-07-13T07:54:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/16897",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16897#event-49363"
}
```



---

archive/issue_comments_283023.json:
```json
{
    "body": "Resolution: wontfix",
    "created_at": "2017-07-13T07:54:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16897#issuecomment-283023",
    "user": "https://github.com/embray"
}
```

Resolution: wontfix



---

archive/issue_comments_283024.json:
```json
{
    "body": "<a id='comment:5'></a>Closing tickets in the sage-duplicate/invalid/wontfix module with positive_review (i.e. someone has confirmed they should be closed).",
    "created_at": "2017-07-13T07:54:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16897#issuecomment-283024",
    "user": "https://github.com/embray"
}
```

<a id='comment:5'></a>Closing tickets in the sage-duplicate/invalid/wontfix module with positive_review (i.e. someone has confirmed they should be closed).
