# Issue 16249: dev.abandon() fails

archive/issues_016012.json:
```json
{
    "assignees": [],
    "body": "The docstring says that the default parameter `ticket_or_branch=None` refers to the current branch. However, deleting the current branch is not possible. Currently, the error message is not very helpful:\n\n```\nsage: dev.abandon()\nValueError: name must be a string\n```\nNow it prints:\n\n```\nsage: dev.abandon()\nTypeError: abandon() takes at least 2 arguments (1 given)\n```\n\nBranch/Commit: **[807f707](https://github.com/sagemath/sagetrac-mirror/commit/807f7070618f351b78e918a99b756cd86bb11ffb)**\n\nReviewer: **Volker Braun**\n\nAuthor: **Julian R\u00fcth**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16249_\n\n",
    "closed_at": "2014-04-29T12:03:14Z",
    "created_at": "2014-04-27T11:38:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/good%20first%20issue"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "dev.abandon() fails",
    "type": "issue",
    "updated_at": "2014-04-29T12:03:14Z",
    "url": "https://github.com/sagemath/sage/issues/16249",
    "user": "https://github.com/saraedum"
}
```
The docstring says that the default parameter `ticket_or_branch=None` refers to the current branch. However, deleting the current branch is not possible. Currently, the error message is not very helpful:

```
sage: dev.abandon()
ValueError: name must be a string
```
Now it prints:

```
sage: dev.abandon()
TypeError: abandon() takes at least 2 arguments (1 given)
```

Branch/Commit: **[807f707](https://github.com/sagemath/sagetrac-mirror/commit/807f7070618f351b78e918a99b756cd86bb11ffb)**

Reviewer: **Volker Braun**

Author: **Julian Rüth**

_Issue created by migration from https://trac.sagemath.org/ticket/16249_





---

archive/issue_events_214489.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2014-04-27T11:38:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16249",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16249#event-214489"
}
```



---

archive/issue_events_214490.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2014-04-27T11:38:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16249",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16249#event-214490"
}
```



---

archive/issue_events_214491.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2014-04-27T11:38:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16249",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16249#event-214491"
}
```



---

archive/issue_comments_210391.json:
```json
{
    "body": "Branch: **[u/saraedum/ticket/16249](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/16249)**",
    "created_at": "2014-04-27T11:51:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16249",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16249#issuecomment-210391",
    "user": "https://github.com/saraedum"
}
```

Branch: **[u/saraedum/ticket/16249](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/16249)**



---

archive/issue_events_214492.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2014-04-27T11:51:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16249",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16249#event-214492"
}
```



---

archive/issue_events_214493.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2014-04-27T11:52:54Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/16249",
    "title_is": "dev.abandon() fails",
    "title_was": "dev.abandon(None) fails",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16249#event-214493"
}
```



---

archive/issue_events_214494.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2014-04-27T11:52:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16249",
    "label": "https://github.com/sagemath/sage/labels/good%20first%20issue",
    "label_color": "7057ff",
    "label_name": "good first issue",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16249#event-214494"
}
```



---

archive/issue_comments_210392.json:
```json
{
    "body": "Author: **Julian R\u00fcth**",
    "created_at": "2014-04-27T11:52:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16249",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16249#issuecomment-210392",
    "user": "https://github.com/saraedum"
}
```

Author: **Julian Rüth**



---

archive/issue_comments_210393.json:
```json
{
    "body": "Changed branch from **[u/saraedum/ticket/16249](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/16249)** to none",
    "created_at": "2014-04-27T11:52:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16249",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16249#issuecomment-210393",
    "user": "https://github.com/saraedum"
}
```

Changed branch from **[u/saraedum/ticket/16249](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/16249)** to none



---

archive/issue_comments_210394.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,12 @@\n-`sage --dev abandon` with no further arguments fails because the default parameter `None` is not replaced with the current branch.\n+The docstring says that the default parameter `ticket_or_branch=None` refers to the current branch. However, deleting the current branch is not possible. Currently, the error message is not very helpful:\n+\n+```\n+sage: dev.abandon()\n+ValueError: name must be a string\n+```\n+Now it prints:\n+\n+```\n+sage: dev.abandon()\n+TypeError: abandon() takes at least 2 arguments (1 given)\n+```\n``````\n",
    "created_at": "2014-04-27T11:52:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16249",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16249#issuecomment-210394",
    "user": "https://github.com/saraedum"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,12 @@
-`sage --dev abandon` with no further arguments fails because the default parameter `None` is not replaced with the current branch.
+The docstring says that the default parameter `ticket_or_branch=None` refers to the current branch. However, deleting the current branch is not possible. Currently, the error message is not very helpful:
+
+```
+sage: dev.abandon()
+ValueError: name must be a string
+```
+Now it prints:
+
+```
+sage: dev.abandon()
+TypeError: abandon() takes at least 2 arguments (1 given)
+```
``````




---

archive/issue_comments_210395.json:
```json
{
    "body": "Branch: **[u/saraedum/ticket/16249](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/16249)**",
    "created_at": "2014-04-27T11:54:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16249",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16249#issuecomment-210395",
    "user": "https://github.com/saraedum"
}
```

Branch: **[u/saraedum/ticket/16249](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/16249)**



---

archive/issue_events_214495.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-04-28T20:08:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16249",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16249#event-214495"
}
```



---

archive/issue_events_214496.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-04-28T20:08:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16249",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16249#event-214496"
}
```



---

archive/issue_comments_210396.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/807f7070618f351b78e918a99b756cd86bb11ffb\">807f707</a></td><td><code>dev.abandon takes a mandatory argument</code></td></tr></table>\n",
    "created_at": "2014-04-28T20:08:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16249",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16249#issuecomment-210396",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:5" align="right">Comment 5</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/807f7070618f351b78e918a99b756cd86bb11ffb">807f707</a></td><td><code>dev.abandon takes a mandatory argument</code></td></tr></table>




---

archive/issue_comments_210397.json:
```json
{
    "body": "Commit: **[807f707](https://github.com/sagemath/sagetrac-mirror/commit/807f7070618f351b78e918a99b756cd86bb11ffb)**",
    "created_at": "2014-04-28T20:08:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16249",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16249#issuecomment-210397",
    "user": "https://github.com/vbraun"
}
```

Commit: **[807f707](https://github.com/sagemath/sagetrac-mirror/commit/807f7070618f351b78e918a99b756cd86bb11ffb)**



---

archive/issue_comments_210398.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2014-04-28T20:08:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16249",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16249#issuecomment-210398",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_events_214497.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-04-29T12:03:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16249",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16249#event-214497"
}
```



---

archive/issue_events_214498.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "04610f029cb1ac6f98b6ce2ff8864407d55e3621",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-04-29T12:03:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16249",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16249#event-214498"
}
```



---

archive/issue_comments_210399.json:
```json
{
    "body": "Changed branch from **[u/saraedum/ticket/16249](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/16249)** to **[807f707](https://github.com/sagemath/sagetrac-mirror/commit/807f7070618f351b78e918a99b756cd86bb11ffb)**",
    "created_at": "2014-04-29T12:03:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16249",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16249#issuecomment-210399",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/saraedum/ticket/16249](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/16249)** to **[807f707](https://github.com/sagemath/sagetrac-mirror/commit/807f7070618f351b78e918a99b756cd86bb11ffb)**
