# Issue 16799: clique polynomial of a graph

archive/issues_016562.json:
```json
{
    "assignees": [],
    "body": "Add a method for computing the [clique polynomial](http://mathworld.wolfram.com/CliquePolynomial.html) of a graph.\n\nCC:  @nathanncohen @ayyer @kcrisman\n\nKeywords: **days60**\n\nBranch/Commit: **[c1d13e4](https://github.com/sagemath/sagetrac-mirror/commit/c1d13e47410ce1f0a1c449460e81fb87c5ddf30a)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton, Nathann Cohen**\n\nAuthor: **Amritanshu Prasad**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16799_\n\n",
    "closed_at": "2014-09-06T11:02:38Z",
    "created_at": "2014-08-12T04:03:33Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "clique polynomial of a graph",
    "type": "issue",
    "updated_at": "2014-09-06T11:02:38Z",
    "url": "https://github.com/sagemath/sage/issues/16799",
    "user": "https://github.com/amritanshu-prasad"
}
```
Add a method for computing the [clique polynomial](http://mathworld.wolfram.com/CliquePolynomial.html) of a graph.

CC:  @nathanncohen @ayyer @kcrisman

Keywords: **days60**

Branch/Commit: **[c1d13e4](https://github.com/sagemath/sagetrac-mirror/commit/c1d13e47410ce1f0a1c449460e81fb87c5ddf30a)**

Reviewer: **Frédéric Chapoton, Nathann Cohen**

Author: **Amritanshu Prasad**

_Issue created by migration from https://trac.sagemath.org/ticket/16799_





---

archive/issue_events_214449.json:
```json
{
    "actor": "https://github.com/amritanshu-prasad",
    "created_at": "2014-08-12T04:03:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16799#event-214449"
}
```



---

archive/issue_events_214450.json:
```json
{
    "actor": "https://github.com/amritanshu-prasad",
    "created_at": "2014-08-12T04:03:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16799#event-214450"
}
```



---

archive/issue_events_214451.json:
```json
{
    "actor": "https://github.com/amritanshu-prasad",
    "created_at": "2014-08-12T04:03:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16799#event-214451"
}
```



---

archive/issue_events_214452.json:
```json
{
    "actor": "https://github.com/amritanshu-prasad",
    "created_at": "2014-08-12T04:03:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16799#event-214452"
}
```



---

archive/issue_comments_222599.json:
```json
{
    "body": "Commit: **[757e9c7](https://github.com/sagemath/sagetrac-mirror/commit/757e9c7d9e11bea0cef7a8a5e1eacdfff88f5e12)**",
    "created_at": "2014-08-18T16:37:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16799#issuecomment-222599",
    "user": "https://github.com/amritanshu-prasad"
}
```

Commit: **[757e9c7](https://github.com/sagemath/sagetrac-mirror/commit/757e9c7d9e11bea0cef7a8a5e1eacdfff88f5e12)**



---

archive/issue_events_214453.json:
```json
{
    "actor": "https://github.com/amritanshu-prasad",
    "created_at": "2014-08-18T16:37:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16799#event-214453"
}
```



---

archive/issue_comments_222600.json:
```json
{
    "body": "Changed keywords from **** to **days60**",
    "created_at": "2014-08-18T16:37:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16799#issuecomment-222600",
    "user": "https://github.com/amritanshu-prasad"
}
```

Changed keywords from **** to **days60**



---

archive/issue_comments_222601.json:
```json
{
    "body": "Branch: **[public/amri/graphs/16799-clique_poly](https://github.com/sagemath/sagetrac-mirror/tree/public/amri/graphs/16799-clique_poly)**",
    "created_at": "2014-08-18T16:37:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16799#issuecomment-222601",
    "user": "https://github.com/amritanshu-prasad"
}
```

Branch: **[public/amri/graphs/16799-clique_poly](https://github.com/sagemath/sagetrac-mirror/tree/public/amri/graphs/16799-clique_poly)**



---

archive/issue_comments_222602.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nI suspect that you would get a better performance by using this code, since you want to count all cliques.\n\nhttp://www.sagemath.org/doc/reference/graphs/sage/graphs/independent_sets.html\n\nNathann",
    "created_at": "2014-08-19T09:52:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16799#issuecomment-222602",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:2'>Comment 2:</a>
I suspect that you would get a better performance by using this code, since you want to count all cliques.

http://www.sagemath.org/doc/reference/graphs/sage/graphs/independent_sets.html

Nathann



---

archive/issue_comments_222603.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/08c42f18d700feeb425dc0d9c1f1cb2bfa0d453d\">08c42f1</a></td><td><code>changed code to use IndependentSets instead of clique_complex()</code></td></tr></table>\n",
    "created_at": "2014-08-19T10:27:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16799#issuecomment-222603",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:3'>Comment 3:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/08c42f18d700feeb425dc0d9c1f1cb2bfa0d453d">08c42f1</a></td><td><code>changed code to use IndependentSets instead of clique_complex()</code></td></tr></table>




---

archive/issue_comments_222604.json:
```json
{
    "body": "Changed commit from **[757e9c7](https://github.com/sagemath/sagetrac-mirror/commit/757e9c7d9e11bea0cef7a8a5e1eacdfff88f5e12)** to **[08c42f1](https://github.com/sagemath/sagetrac-mirror/commit/08c42f18d700feeb425dc0d9c1f1cb2bfa0d453d)**",
    "created_at": "2014-08-19T10:27:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16799#issuecomment-222604",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[757e9c7](https://github.com/sagemath/sagetrac-mirror/commit/757e9c7d9e11bea0cef7a8a5e1eacdfff88f5e12)** to **[08c42f1](https://github.com/sagemath/sagetrac-mirror/commit/08c42f18d700feeb425dc0d9c1f1cb2bfa0d453d)**



---

archive/issue_comments_222605.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nThanks for the suggestion. Done.",
    "created_at": "2014-08-19T10:29:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16799#issuecomment-222605",
    "user": "https://github.com/amritanshu-prasad"
}
```

<a id='comment:4'>Comment 4:</a>
Thanks for the suggestion. Done.



---

archive/issue_events_214454.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-08-19T10:51:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16799#event-214454"
}
```



---

archive/issue_events_214455.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-08-19T10:51:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16799#event-214455"
}
```



---

archive/issue_comments_222606.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nHello again !\n\n* You say that the constant term is expected to be 0 in the doc, but in the examples it is equal to 1\n\n* You can use `IndependentSets(g,complement=True)` instead of `IndependentSets(g.complement())`. Computing the complement of a sparse graph is rather slow.\n\n* The documentation of a function should begin with a short one-line description. Anything can follow, it can be as long as you want, though. It can be the line that appears right next to the function, in the index at the top of the file\n\n* If you change \n  `return sum([number_of[i]*t**i for i in range(self.order()+1)])`\n\n  into\n\n  `return sum([coeff*t**i for i,coeff in enumerate(number_of) if coeff])`\n\n  You avoid computing the `t**i` that will be multiplied by 0. It does not bring any detectable speedup in this case, however.\nNathann",
    "created_at": "2014-08-19T10:51:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16799#issuecomment-222606",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:5'>Comment 5:</a>
Hello again !

* You say that the constant term is expected to be 0 in the doc, but in the examples it is equal to 1

* You can use `IndependentSets(g,complement=True)` instead of `IndependentSets(g.complement())`. Computing the complement of a sparse graph is rather slow.

* The documentation of a function should begin with a short one-line description. Anything can follow, it can be as long as you want, though. It can be the line that appears right next to the function, in the index at the top of the file

* If you change 
  `return sum([number_of[i]*t**i for i in range(self.order()+1)])`

  into

  `return sum([coeff*t**i for i,coeff in enumerate(number_of) if coeff])`

  You avoid computing the `t**i` that will be multiplied by 0. It does not bring any detectable speedup in this case, however.
Nathann



---

archive/issue_comments_222607.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/438ffde02cc3bb1c33064a09e96e50869b2692e9\">438ffde</a></td><td><code>corrected documentation and other minor changes to code</code></td></tr></table>\n",
    "created_at": "2014-08-19T11:36:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16799#issuecomment-222607",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:6'>Comment 6:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/438ffde02cc3bb1c33064a09e96e50869b2692e9">438ffde</a></td><td><code>corrected documentation and other minor changes to code</code></td></tr></table>




---

archive/issue_comments_222608.json:
```json
{
    "body": "Changed commit from **[08c42f1](https://github.com/sagemath/sagetrac-mirror/commit/08c42f18d700feeb425dc0d9c1f1cb2bfa0d453d)** to **[438ffde](https://github.com/sagemath/sagetrac-mirror/commit/438ffde02cc3bb1c33064a09e96e50869b2692e9)**",
    "created_at": "2014-08-19T11:36:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16799#issuecomment-222608",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[08c42f1](https://github.com/sagemath/sagetrac-mirror/commit/08c42f18d700feeb425dc0d9c1f1cb2bfa0d453d)** to **[438ffde](https://github.com/sagemath/sagetrac-mirror/commit/438ffde02cc3bb1c33064a09e96e50869b2692e9)**



---

archive/issue_comments_222609.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nHope this is ok.",
    "created_at": "2014-08-19T11:38:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16799#issuecomment-222609",
    "user": "https://github.com/amritanshu-prasad"
}
```

<a id='comment:7'>Comment 7:</a>
Hope this is ok.



---

archive/issue_events_214456.json:
```json
{
    "actor": "https://github.com/amritanshu-prasad",
    "created_at": "2014-08-19T11:38:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16799#event-214456"
}
```



---

archive/issue_events_214457.json:
```json
{
    "actor": "https://github.com/amritanshu-prasad",
    "created_at": "2014-08-19T11:38:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16799#event-214457"
}
```



---

archive/issue_events_214458.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-08-19T11:42:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16799#event-214458"
}
```



---

archive/issue_events_214459.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-08-19T11:42:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16799#event-214459"
}
```



---

archive/issue_comments_222610.json:
```json
{
    "body": "Reviewer: **Nathann Cohen**",
    "created_at": "2014-08-19T11:42:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16799#issuecomment-222610",
    "user": "https://github.com/nathanncohen"
}
```

Reviewer: **Nathann Cohen**



---

archive/issue_comments_222611.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\n> Hope this is ok.\n\nWell, I don't see anything wrong with it. Thanks for this patch !\n\nNathann",
    "created_at": "2014-08-19T11:42:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16799#issuecomment-222611",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:8'>Comment 8:</a>
> Hope this is ok.

Well, I don't see anything wrong with it. Thanks for this patch !

Nathann



---

archive/issue_comments_222612.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nThank you.",
    "created_at": "2014-08-19T11:50:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16799#issuecomment-222612",
    "user": "https://github.com/amritanshu-prasad"
}
```

<a id='comment:9'>Comment 9:</a>
Thank you.



---

archive/issue_comments_222613.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nauthor name",
    "created_at": "2014-08-19T22:14:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16799#issuecomment-222613",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:10'>Comment 10:</a>
author name



---

archive/issue_events_214460.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-08-19T22:14:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16799#event-214460"
}
```



---

archive/issue_events_214461.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-08-19T22:14:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16799#event-214461"
}
```



---

archive/issue_comments_222614.json:
```json
{
    "body": "Changed commit from **[438ffde](https://github.com/sagemath/sagetrac-mirror/commit/438ffde02cc3bb1c33064a09e96e50869b2692e9)** to **[fdd7084](https://github.com/sagemath/sagetrac-mirror/commit/fdd7084c6e940ba65d1e2dc346f22ae1d8c326c4)**",
    "created_at": "2014-08-20T05:51:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16799#issuecomment-222614",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[438ffde](https://github.com/sagemath/sagetrac-mirror/commit/438ffde02cc3bb1c33064a09e96e50869b2692e9)** to **[fdd7084](https://github.com/sagemath/sagetrac-mirror/commit/fdd7084c6e940ba65d1e2dc346f22ae1d8c326c4)**



---

archive/issue_comments_222615.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/fdd7084c6e940ba65d1e2dc346f22ae1d8c326c4\">fdd7084</a></td><td><code>Added author</code></td></tr></table>\n",
    "created_at": "2014-08-20T05:51:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16799#issuecomment-222615",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:11'>Comment 11:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/fdd7084c6e940ba65d1e2dc346f22ae1d8c326c4">fdd7084</a></td><td><code>Added author</code></td></tr></table>




---

archive/issue_events_214462.json:
```json
{
    "actor": "https://github.com/amritanshu-prasad",
    "created_at": "2014-08-20T05:53:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16799#event-214462"
}
```



---

archive/issue_events_214463.json:
```json
{
    "actor": "https://github.com/amritanshu-prasad",
    "created_at": "2014-08-20T05:53:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16799#event-214463"
}
```



---

archive/issue_comments_222616.json:
```json
{
    "body": "Author: **Amritanshu Prasad**",
    "created_at": "2014-08-20T07:34:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16799#issuecomment-222616",
    "user": "https://github.com/fchapoton"
}
```

Author: **Amritanshu Prasad**



---

archive/issue_comments_222617.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nVolker was requiring the author name here in the author field.",
    "created_at": "2014-08-20T07:34:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16799#issuecomment-222617",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:13'>Comment 13:</a>
Volker was requiring the author name here in the author field.



---

archive/issue_events_214464.json:
```json
{
    "actor": "https://github.com/amritanshu-prasad",
    "created_at": "2014-08-27T17:44:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16799#event-214464"
}
```



---

archive/issue_events_214465.json:
```json
{
    "actor": "https://github.com/amritanshu-prasad",
    "created_at": "2014-08-27T17:44:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16799#event-214465"
}
```



---

archive/issue_comments_222618.json:
```json
{
    "body": "<a id='comment:15'>Comment 15:</a>\nDocumentation build fails with:\n\n```\n[graphs   ] /home/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/graphs/graph.py:docstring of sage.graphs.graph:209: WARNING: Bullet list ends without a blank line; unexpected unindent.\nError building the documentation.\nTraceback (most recent call last):\n  File \"/home/buildslave-sage/slave/sage_git/build/src/doc/common/builder.py\", line 1490, in <module>\n    getattr(get_builder(name), type)()\n  File \"/home/buildslave-sage/slave/sage_git/build/src/doc/common/builder.py\", line 291, in _wrapper\n    getattr(get_builder(document), 'inventory')(*args, **kwds)\n  File \"/home/buildslave-sage/slave/sage_git/build/src/doc/common/builder.py\", line 502, in _wrapper\n    x.get(99999)\n  File \"/home/buildslave-sage/slave/sage_git/build/local/lib/python/multiprocessing/pool.py\", line 558, in get\n    raise self._value\nOSError: [graphs   ] /home/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/graphs/graph.py:docstring of sage.graphs.graph:209: WARNING: Bullet list ends without a blank line; unexpected unindent.\n```",
    "created_at": "2014-08-28T09:40:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16799#issuecomment-222618",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:15'>Comment 15:</a>
Documentation build fails with:

```
[graphs   ] /home/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/graphs/graph.py:docstring of sage.graphs.graph:209: WARNING: Bullet list ends without a blank line; unexpected unindent.
Error building the documentation.
Traceback (most recent call last):
  File "/home/buildslave-sage/slave/sage_git/build/src/doc/common/builder.py", line 1490, in <module>
    getattr(get_builder(name), type)()
  File "/home/buildslave-sage/slave/sage_git/build/src/doc/common/builder.py", line 291, in _wrapper
    getattr(get_builder(document), 'inventory')(*args, **kwds)
  File "/home/buildslave-sage/slave/sage_git/build/src/doc/common/builder.py", line 502, in _wrapper
    x.get(99999)
  File "/home/buildslave-sage/slave/sage_git/build/local/lib/python/multiprocessing/pool.py", line 558, in get
    raise self._value
OSError: [graphs   ] /home/buildslave-sage/slave/sage_git/build/local/lib/python2.7/site-packages/sage/graphs/graph.py:docstring of sage.graphs.graph:209: WARNING: Bullet list ends without a blank line; unexpected unindent.
```



---

archive/issue_events_214466.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-08-28T09:40:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16799#event-214466"
}
```



---

archive/issue_events_214467.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-08-28T09:40:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16799#event-214467"
}
```



---

archive/issue_comments_222619.json:
```json
{
    "body": "Changed commit from **[fdd7084](https://github.com/sagemath/sagetrac-mirror/commit/fdd7084c6e940ba65d1e2dc346f22ae1d8c326c4)** to **[c1d13e4](https://github.com/sagemath/sagetrac-mirror/commit/c1d13e47410ce1f0a1c449460e81fb87c5ddf30a)**",
    "created_at": "2014-08-31T15:03:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16799#issuecomment-222619",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[fdd7084](https://github.com/sagemath/sagetrac-mirror/commit/fdd7084c6e940ba65d1e2dc346f22ae1d8c326c4)** to **[c1d13e4](https://github.com/sagemath/sagetrac-mirror/commit/c1d13e47410ce1f0a1c449460e81fb87c5ddf30a)**



---

archive/issue_comments_222620.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/339e7ac11f0cbeef6bbf2e467029d05ea5ae2ac0\">339e7ac</a></td><td><code>Merge branch 'public/amri/graphs/16799-clique_poly' of ssh://trac.sagemath.org:22/sage into 16799</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c1d13e47410ce1f0a1c449460e81fb87c5ddf30a\">c1d13e4</a></td><td><code>trac #16799 correcting doc</code></td></tr></table>\n",
    "created_at": "2014-08-31T15:03:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16799#issuecomment-222620",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:16'>Comment 16:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/339e7ac11f0cbeef6bbf2e467029d05ea5ae2ac0">339e7ac</a></td><td><code>Merge branch 'public/amri/graphs/16799-clique_poly' of ssh://trac.sagemath.org:22/sage into 16799</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c1d13e47410ce1f0a1c449460e81fb87c5ddf30a">c1d13e4</a></td><td><code>trac #16799 correcting doc</code></td></tr></table>




---

archive/issue_events_214468.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-08-31T15:04:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16799#event-214468"
}
```



---

archive/issue_events_214469.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-08-31T15:04:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16799#event-214469"
}
```



---

archive/issue_events_214470.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-09-01T08:44:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16799#event-214470"
}
```



---

archive/issue_events_214471.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-09-01T08:44:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16799#event-214471"
}
```



---

archive/issue_comments_222621.json:
```json
{
    "body": "Changed reviewer from **Nathann Cohen** to **Fr\u00e9d\u00e9ric Chapoton, Nathann Cohen**",
    "created_at": "2014-09-01T08:44:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16799#issuecomment-222621",
    "user": "https://github.com/nathanncohen"
}
```

Changed reviewer from **Nathann Cohen** to **Frédéric Chapoton, Nathann Cohen**



---

archive/issue_comments_222622.json:
```json
{
    "body": "Changed branch from **[public/amri/graphs/16799-clique_poly](https://github.com/sagemath/sagetrac-mirror/tree/public/amri/graphs/16799-clique_poly)** to **[c1d13e4](https://github.com/sagemath/sagetrac-mirror/commit/c1d13e47410ce1f0a1c449460e81fb87c5ddf30a)**",
    "created_at": "2014-09-06T11:02:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16799#issuecomment-222622",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/amri/graphs/16799-clique_poly](https://github.com/sagemath/sagetrac-mirror/tree/public/amri/graphs/16799-clique_poly)** to **[c1d13e4](https://github.com/sagemath/sagetrac-mirror/commit/c1d13e47410ce1f0a1c449460e81fb87c5ddf30a)**



---

archive/issue_events_214472.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-09-06T11:02:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16799#event-214472"
}
```



---

archive/issue_events_214473.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "0eb8bfb230ef434d1400d697c20ce8bc8cda4369",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-09-06T11:02:38Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16799",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16799#event-214473"
}
```
