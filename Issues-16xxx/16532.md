...
```json
{
    "body": "Or, if calling `register_coercion` during creation of B rather than A is no option, you should/could rely on `_coerce_map_from_` (on B, of course).",
...
```json
{
    "body": "Okay, then we need to get back on #15303 as setting up `_coerce_map_from_` can be somewhat painful for things with multiple realizations (ex. symmetric functions). Also, do you think it's worthwhile documenting this somewhere and if so, where? The coercion thematic tutorial(s) or somewhere in `coerce.pyx`?",
