# Issue 16585: improved PolynomialSequence

archive/issues_016348.json:
```json
{
    "body": "This now works:\n\n```python\nsage: sr = mq.SR(1,1,2,4, gf2=True)\nsage: F,s = sr.polynomial_system()\nsage: I = F.ideal()\nsage: I.random_element(degree=3) # new: return some funny distribution, #11850\nk100*k102*x102 + k100*k102*x110 + ...\n\nsage: I.random_element(degree=3, compute_gb=True, terms=True) # new: uniformly random up to degree 3\nk100*k101*w112 + k100*k102*k110 + k100*k102*w102 + k100*k103*w111 + ...\n\nsage: F.is_groebner() # new: PolynomialSequence.is_groebner, cf. #10856\nFalse\n\nsage: F.maximal_degree() # new: PolynomialSequence.maximal_degree, cf. \n2\n\nsage: gb = F.groebner_basis(algorithm='magma') # new: this used to fail\nsage: gb.is_groebner()\nTrue\n\nsage: gb.maximal_degree() \n1\n\nsage: M = Sequence([f.lm() for f in F]) # new: this used to fail cf. #10680\nsage: M.maximal_degree()\n2\n\nsage: F.reduced() # new: interreduced_basis() moved to Sequence\nPolynomial Sequence with 60 Polynomials in 36 Variables\n```\n\nThis ticket replaces #11850, #10856, #10680\n\nCC:  @miguelmarco\n\nKeywords: sd59\n\nBranch/Commit: 1b208b5a2d4722637f82b14ef92ab46d4ab4d0e9\n\nReviewer: Jakob Kroeker\n\nAuthor: Martin Albrecht\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/16585\n\n",
    "closed_at": "2015-01-29T13:26:14Z",
    "created_at": "2014-06-29T04:17:13Z",
    "labels": [
        "component: commutative algebra"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.4",
    "title": "improved PolynomialSequence",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/16585",
    "user": "https://github.com/malb"
}
```
This now works:

```python
sage: sr = mq.SR(1,1,2,4, gf2=True)
sage: F,s = sr.polynomial_system()
sage: I = F.ideal()
sage: I.random_element(degree=3) # new: return some funny distribution, #11850
k100*k102*x102 + k100*k102*x110 + ...

sage: I.random_element(degree=3, compute_gb=True, terms=True) # new: uniformly random up to degree 3
k100*k101*w112 + k100*k102*k110 + k100*k102*w102 + k100*k103*w111 + ...

sage: F.is_groebner() # new: PolynomialSequence.is_groebner, cf. #10856
False

sage: F.maximal_degree() # new: PolynomialSequence.maximal_degree, cf. 
2

sage: gb = F.groebner_basis(algorithm='magma') # new: this used to fail
sage: gb.is_groebner()
True

sage: gb.maximal_degree() 
1

sage: M = Sequence([f.lm() for f in F]) # new: this used to fail cf. #10680
sage: M.maximal_degree()
2

sage: F.reduced() # new: interreduced_basis() moved to Sequence
Polynomial Sequence with 60 Polynomials in 36 Variables
```

This ticket replaces #11850, #10856, #10680

CC:  @miguelmarco

Keywords: sd59

Branch/Commit: 1b208b5a2d4722637f82b14ef92ab46d4ab4d0e9

Reviewer: Jakob Kroeker

Author: Martin Albrecht

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/16585





---

archive/issue_comments_274744.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -30,3 +30,5 @@\n sage: F.reduced() # new: interreduced_basis() moved to Sequence\n Polynomial Sequence with 60 Polynomials in 36 Variables\n ```\n+\n+This ticket replaces #11850, #10856, #10680\n``````\n",
    "created_at": "2014-06-29T04:39:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274744",
    "user": "https://github.com/malb"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -30,3 +30,5 @@
 sage: F.reduced() # new: interreduced_basis() moved to Sequence
 Polynomial Sequence with 60 Polynomials in 36 Variables
 ```
+
+This ticket replaces #11850, #10856, #10680
``````




---

archive/issue_comments_274745.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2014-06-29T04:39:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274745",
    "user": "https://github.com/malb"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_274746.json:
```json
{
    "body": "Changing commit from \"\" to \"fef7d3bf43c0a2222fa9ddaf2809fd74603577fd\"",
    "created_at": "2014-06-29T04:39:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274746",
    "user": "https://github.com/malb"
}
```

Changing commit from "" to "fef7d3bf43c0a2222fa9ddaf2809fd74603577fd"



---

archive/issue_comments_274747.json:
```json
{
    "body": "Changing branch from \"\" to \"u/malb/t16585_mpolynomial_sequence\"",
    "created_at": "2014-06-29T04:39:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274747",
    "user": "https://github.com/malb"
}
```

Changing branch from "" to "u/malb/t16585_mpolynomial_sequence"



---

archive/issue_comments_274748.json:
```json
{
    "body": "<a id='comment:2'></a>Anyone up for reviewing this?",
    "created_at": "2014-08-04T17:23:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274748",
    "user": "https://github.com/malb"
}
```

<a id='comment:2'></a>Anyone up for reviewing this?



---

archive/issue_comments_274749.json:
```json
{
    "body": "<a id='comment:3'></a>This obviously needs to be merged with the latest development version.\n\nApart from that, this ticket seems to do a lot of things (not only fixing several independent bugs, but also refactoring code), so it could be seen as a patch bomb which is a bad thing. Since this patch touches Magma-related stuff, only people who have Magma could review this.",
    "created_at": "2014-08-04T17:46:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274749",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:3'></a>This obviously needs to be merged with the latest development version.

Apart from that, this ticket seems to do a lot of things (not only fixing several independent bugs, but also refactoring code), so it could be seen as a patch bomb which is a bad thing. Since this patch touches Magma-related stuff, only people who have Magma could review this.



---

archive/issue_comments_274750.json:
```json
{
    "body": "Changing commit from \"fef7d3bf43c0a2222fa9ddaf2809fd74603577fd\" to \"6e9ef02bae16c9ec564b901d2a573bcb8f65ee6d\"",
    "created_at": "2014-08-05T09:52:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274750",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "fef7d3bf43c0a2222fa9ddaf2809fd74603577fd" to "6e9ef02bae16c9ec564b901d2a573bcb8f65ee6d"



---

archive/issue_comments_274751.json:
```json
{
    "body": "<a id='comment:4'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2014-08-05T09:52:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274751",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:4'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_events_048660.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16585#event-48660"
}
```



---

archive/issue_comments_274752.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2014-10-05T12:05:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274752",
    "user": "https://trac.sagemath.org/admin/accounts/users/jakobkroeker"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_274753.json:
```json
{
    "body": "<a id='comment:6'></a>First remark:\n\nin the commit\t1b7927cb54bb2943ad2ae1a43c8e3befdf6703b6\nin file src/sage/rings/polynomial/multi_polynomial_ideal.py\nat line 4139 I think it should be \n`if d >= 0` instead of `if d > 0`\n\nSecond remark:\n\nin the commit \tfffdbe0c8f9a248b3436d9c846bcd7b04a036d88\nyou introduce option terms=True to choose maximum number of terms.\nI don't like it, because the option is not self-descriptive,\nbut try to convince me.\n\nThird remark: \nin \t81c951fa9d428816a44dd8855d5a00ec4c3cf2b3\n\nyou change the default option 'choose_degree=False' \nChanging default options is dangerous since it may break\nexisting code. Please give reasons why this change is still ok\n\ncommits\n \t3b6c11a13eb300f0a2bf6acb56d7b9001b03e1f8\n\tc2a8506eed5d9db0106b38d1c643f503d19ab54a\n        37fb67cce683927cd248b518602e81a2e33ac134\n\nlooks ok for me\n\nI will continue the review tomorrow evening.\n\nJakob",
    "created_at": "2014-10-05T12:05:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274753",
    "user": "https://trac.sagemath.org/admin/accounts/users/jakobkroeker"
}
```

<a id='comment:6'></a>First remark:

in the commit	1b7927cb54bb2943ad2ae1a43c8e3befdf6703b6
in file src/sage/rings/polynomial/multi_polynomial_ideal.py
at line 4139 I think it should be 
`if d >= 0` instead of `if d > 0`

Second remark:

in the commit 	fffdbe0c8f9a248b3436d9c846bcd7b04a036d88
you introduce option terms=True to choose maximum number of terms.
I don't like it, because the option is not self-descriptive,
but try to convince me.

Third remark: 
in 	81c951fa9d428816a44dd8855d5a00ec4c3cf2b3

you change the default option 'choose_degree=False' 
Changing default options is dangerous since it may break
existing code. Please give reasons why this change is still ok

commits
 	3b6c11a13eb300f0a2bf6acb56d7b9001b03e1f8
	c2a8506eed5d9db0106b38d1c643f503d19ab54a
        37fb67cce683927cd248b518602e81a2e33ac134

looks ok for me

I will continue the review tomorrow evening.

Jakob



---

archive/issue_comments_274754.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Jakob Kroeker\"",
    "created_at": "2014-10-05T12:05:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274754",
    "user": "https://trac.sagemath.org/admin/accounts/users/jakobkroeker"
}
```

Changing reviewer from "" to "Jakob Kroeker"



---

archive/issue_comments_274755.json:
```json
{
    "body": "<a id='comment:7'></a>Ok, here the missing review comments:\n\n##############################################################################\n1.\n\"make sure we use the polynomial ring as ring not the monoid\", in\nfile `./src/sage/rings/polynomial/multi_polynomial_sequence.py`\n\n\nI think there is a failing example, if the input below is legal:\n\n```\nR.<x,y,z> = PolynomialRing(QQ)\nI = Ideal(5000*x*R,y*x+5) #error\n# Ring is Monoid of ideals of Multivariate Polynomial Ring in x, y, z over Rational Field =>error:\n# R must be a commutative ring\n```\n\nupdate: something is fishy with the next failing example:\na. what was K ?\nb. how to reproduce \"not a constant polynomial\" error\n\n\n\nsecond failing example (different error):\n\n```\nR.<x,y,z> = PolynomialRing(QQ)\nI = Ideal(5000*x*K+y*x,y*x+5) \n# TypeError: not a constant polynomial\n```\n\n#######################################################\n2.\nin  commit  [9ee750ef52d6d6ba43254a07ffbf9cea1160315f](http://git.sagemath.org/sage.git/commit/?h=9ee750ef52d6d6ba43254a07ffbf9cea1160315f&id=9ee750ef52d6d6ba43254a07ffbf9cea1160315f)\nsnippet:\n\n```\n+ try:\n+ c = K.characteristic()\n+ except NotImplementedError:\n+ c = 0\n```\n\nWhy is c set to zero?  At least this should be documented. \nIs it not possible to have a different characteristic in case of an NotImplementedError? Do you have an example which hits this issue?\n\n####################################################################\n3.\nrefactored (lib)Singular GB code,\nin commit    [e60f83a9e1584a899e8c79171ce177474b9fed06](http://git.sagemath.org/sage.git/commit/?h=6e9ef02bae16c9ec564b901d2a573bcb8f65ee6d&id=e60f83a9e1584a899e8c79171ce177474b9fed06)\nin src/sage/rings/polynomial/multi_polynomial_ideal_libsingular.pyx ,\nline 114, ff.`sage_ideal_to_singular_ideal`\nIs there a corresponding test for a conversion from \"`or a list of generators`\"? \nIf not, could you add one?\n\nSame commit, \nin file 'src/sage/rings/polynomial/polynomial_singular_interface.py\nrenaming `singular_default` to `singular`; why? \nAt other places there are singular_default usages.\nIs there a difference?\n\n####################################################################\n4.\nSame commit,\nIt seems that reduce examples show no examples for explicitly reducing leading coefficient,\nCould you add an example or a test which hits \"`ret.append(f.lc()**(-1)*f)`\"?\n\n####################################################################\n5.\nSequence has no 'reduced' function\nIs that ok?\nExample:\n\n```\nR.<x> = PolynomialRing(QQ)\nF = Sequence([x,x+1])\nF.reduced() # fails\n```\n\n\n\n####################################################################\n6.\ncommit  [a88b44703751c13e451252ca2da671679077711e](http://git.sagemath.org/sage.git/commit/?h=a88b44703751c13e451252ca2da671679077711e&id=a88b44703751c13e451252ca2da671679077711e) \n(fix Magma interface broken when moving decorators around):\n\nCould you explain what may get wrong and add an example?",
    "created_at": "2014-10-27T18:21:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274755",
    "user": "https://trac.sagemath.org/admin/accounts/users/jakobkroeker"
}
```

<a id='comment:7'></a>Ok, here the missing review comments:

##############################################################################
1.
"make sure we use the polynomial ring as ring not the monoid", in
file `./src/sage/rings/polynomial/multi_polynomial_sequence.py`


I think there is a failing example, if the input below is legal:

```
R.<x,y,z> = PolynomialRing(QQ)
I = Ideal(5000*x*R,y*x+5) #error
# Ring is Monoid of ideals of Multivariate Polynomial Ring in x, y, z over Rational Field =>error:
# R must be a commutative ring
```

update: something is fishy with the next failing example:
a. what was K ?
b. how to reproduce "not a constant polynomial" error



second failing example (different error):

```
R.<x,y,z> = PolynomialRing(QQ)
I = Ideal(5000*x*K+y*x,y*x+5) 
# TypeError: not a constant polynomial
```

#######################################################
2.
in  commit  [9ee750ef52d6d6ba43254a07ffbf9cea1160315f](http://git.sagemath.org/sage.git/commit/?h=9ee750ef52d6d6ba43254a07ffbf9cea1160315f&id=9ee750ef52d6d6ba43254a07ffbf9cea1160315f)
snippet:

```
+ try:
+ c = K.characteristic()
+ except NotImplementedError:
+ c = 0
```

Why is c set to zero?  At least this should be documented. 
Is it not possible to have a different characteristic in case of an NotImplementedError? Do you have an example which hits this issue?

####################################################################
3.
refactored (lib)Singular GB code,
in commit    [e60f83a9e1584a899e8c79171ce177474b9fed06](http://git.sagemath.org/sage.git/commit/?h=6e9ef02bae16c9ec564b901d2a573bcb8f65ee6d&id=e60f83a9e1584a899e8c79171ce177474b9fed06)
in src/sage/rings/polynomial/multi_polynomial_ideal_libsingular.pyx ,
line 114, ff.`sage_ideal_to_singular_ideal`
Is there a corresponding test for a conversion from "`or a list of generators`"? 
If not, could you add one?

Same commit, 
in file 'src/sage/rings/polynomial/polynomial_singular_interface.py
renaming `singular_default` to `singular`; why? 
At other places there are singular_default usages.
Is there a difference?

####################################################################
4.
Same commit,
It seems that reduce examples show no examples for explicitly reducing leading coefficient,
Could you add an example or a test which hits "`ret.append(f.lc()**(-1)*f)`"?

####################################################################
5.
Sequence has no 'reduced' function
Is that ok?
Example:

```
R.<x> = PolynomialRing(QQ)
F = Sequence([x,x+1])
F.reduced() # fails
```



####################################################################
6.
commit  [a88b44703751c13e451252ca2da671679077711e](http://git.sagemath.org/sage.git/commit/?h=a88b44703751c13e451252ca2da671679077711e&id=a88b44703751c13e451252ca2da671679077711e) 
(fix Magma interface broken when moving decorators around):

Could you explain what may get wrong and add an example?



---

archive/issue_comments_274756.json:
```json
{
    "body": "<a id='comment:8'></a>Branch pushed to git repo; I updated commit sha1. Last 10 new commits:",
    "created_at": "2014-11-03T22:04:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274756",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:8'></a>Branch pushed to git repo; I updated commit sha1. Last 10 new commits:



---

archive/issue_comments_274757.json:
```json
{
    "body": "Changing commit from \"6e9ef02bae16c9ec564b901d2a573bcb8f65ee6d\" to \"c0241972230aec8d6ca71dffeca933df5ef9b963\"",
    "created_at": "2014-11-03T22:04:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274757",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "6e9ef02bae16c9ec564b901d2a573bcb8f65ee6d" to "c0241972230aec8d6ca71dffeca933df5ef9b963"



---

archive/issue_comments_274758.json:
```json
{
    "body": "<a id='comment:9'></a>Hi Jakob, thanks for your review. I started addressing the issues you identified.\n\n> in file src/sage/rings/polynomial/multi_polynomial_ideal.py at line 4139 I\n> think it should be if d >= 0 instead of if d > 0\n\n\nFixed: `0fa7a24`\n\n> you introduce option terms=True to choose maximum number of terms.  I don't\n> like it, because the option is not self-descriptive, but try to convince me.\n\n\nI agree, so I changed it. I changed the `True` to `Infinity` and changed the behaviour such that when requesting more terms than exist the number is silently reduced to the maximum number of available terms: `563baf0`\n\nPS: Because I am stupid, though, I rebased instead of merged. Hence, I believe you'll have to check out the code again, i.e. you can't just git pull again in your current branch. Sorry for that!",
    "created_at": "2014-11-03T22:07:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274758",
    "user": "https://github.com/malb"
}
```

<a id='comment:9'></a>Hi Jakob, thanks for your review. I started addressing the issues you identified.

> in file src/sage/rings/polynomial/multi_polynomial_ideal.py at line 4139 I
> think it should be if d >= 0 instead of if d > 0


Fixed: `0fa7a24`

> you introduce option terms=True to choose maximum number of terms.  I don't
> like it, because the option is not self-descriptive, but try to convince me.


I agree, so I changed it. I changed the `True` to `Infinity` and changed the behaviour such that when requesting more terms than exist the number is silently reduced to the maximum number of available terms: `563baf0`

PS: Because I am stupid, though, I rebased instead of merged. Hence, I believe you'll have to check out the code again, i.e. you can't just git pull again in your current branch. Sorry for that!



---

archive/issue_comments_274759.json:
```json
{
    "body": "<a id='comment:10'></a>> you can't just git pull again in your current branch\n> I believe you'll have to check out the code again\n\n\nThanks for the hint! \nIts likely that otherwise I  would get confused by the merge conflicts.",
    "created_at": "2014-11-18T14:36:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274759",
    "user": "https://trac.sagemath.org/admin/accounts/users/jakobkroeker"
}
```

<a id='comment:10'></a>> you can't just git pull again in your current branch
> I believe you'll have to check out the code again


Thanks for the hint! 
Its likely that otherwise I  would get confused by the merge conflicts.



---

archive/issue_comments_274760.json:
```json
{
    "body": "Changing commit from \"c0241972230aec8d6ca71dffeca933df5ef9b963\" to \"620322d9c80b1b30e71ce89d09e5ee42e079163d\"",
    "created_at": "2015-01-02T12:46:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274760",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "c0241972230aec8d6ca71dffeca933df5ef9b963" to "620322d9c80b1b30e71ce89d09e5ee42e079163d"



---

archive/issue_comments_274761.json:
```json
{
    "body": "<a id='comment:11'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2015-01-02T12:46:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274761",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:11'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_274762.json:
```json
{
    "body": "<a id='comment:12'></a>> you change the default option 'choose_degree=False' Changing default options\n> is dangerous since it may break existing code. Please give reasons why this\n> change is still ok\n\n\nIt changes the behaviour of a random sampler so the output is not deterministic anyway. Also, ``choose_degree=True``does not return what I\u2019d expect from a random sampler: something uniform if possible. Mod p the new default lives up to this expectation if we consider uniform to mean: uniform of a given degree.\n\n> \"make sure we use the polynomial ring as ring not the monoid\", in file\n> ./src/sage/rings/polynomial/multi_polynomial_sequence.py\n> \n> I think there is a failing example, if the input below is legal:\n\n\n```\nsage: R.<x,y,z> = PolynomialRing(QQ)\nsage: I = Ideal(5000*x*R,y*x+5) #error\n# Ring is Monoid of ideals of Multivariate Polynomial Ring in x, y, z over Rational Field =>error:\n# R must be a commutative ring\n```\n\nI don\u2019t think this is connected. I\u2019m getting the same error with sage 6.4.1. Also, The business of monoid vs ring only occurs when PolyBoRi implements the ring.\n\n> a second failing example (different error):\n\n\n```\nsage: R.<x,y,z> = PolynomialRing(QQ)\nsage: I = Ideal(5000*x*K+y*x,y*x+5) \n# TypeError: not a constant polynomial\n```\n\nSee above.\n\n> Why is c set to zero? At least this should be documented.  Is it not possible\n> to have a different characteristic in case of an NotImplementedError?? Do you\n> have an example which hits this issue?\n\n\nI added a comment:\n\n# We assume that our ring has characteristic zero if it does not implement a\n# characteristic(). For example, generic quotient rings do not have a characteristic()\n# method implemented. It is okay to set c = 0 here because we're only using the\n# characteristic to pick a more specialized implementation for c = 2.\n\n> in src/sage/rings/polynomial/multi_polynomial_ideal_libsingular.pyx , line\n> 114, ff.sage_ideal_to_singular_ideal Is there a corresponding test for a\n> conversion from \"or a list of generators\"?  If not, could you add one?\n\n\nI added a test.\n\n> in file 'src/sage/rings/polynomial/polynomial_singular_interface.py renaming\n> singular_default to singular; why?  At other places there are singular_default\n> usages. Is there a difference?\n\n\nThere is no need to rename singular to singular_default so I dropped it. It makes no difference either way.\n\n> It seems that reduce examples show no examples for explicitly reducing leading\n> coefficient, Could you add an example or a test which hits\n> \"ret.append(f.lc()**(-1)*f)\"?\n\n\nI added one.\n\n> Sequence has no 'reduced' function Is that ok? Example:\n\n\nI think this is fine as Sequence is not only for polynomials but for anything. However, reduced() only seems to make sense for polynomials.\n\n> (fix Magma interface broken when moving decorators around): Could you explain\n> what may get wrong and add an example?\n\n\nUnfortunately, I forgot. I could investigate if you insist.",
    "created_at": "2015-01-02T12:54:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274762",
    "user": "https://github.com/malb"
}
```

<a id='comment:12'></a>> you change the default option 'choose_degree=False' Changing default options
> is dangerous since it may break existing code. Please give reasons why this
> change is still ok


It changes the behaviour of a random sampler so the output is not deterministic anyway. Also, ``choose_degree=True``does not return what I’d expect from a random sampler: something uniform if possible. Mod p the new default lives up to this expectation if we consider uniform to mean: uniform of a given degree.

> "make sure we use the polynomial ring as ring not the monoid", in file
> ./src/sage/rings/polynomial/multi_polynomial_sequence.py
> 
> I think there is a failing example, if the input below is legal:


```
sage: R.<x,y,z> = PolynomialRing(QQ)
sage: I = Ideal(5000*x*R,y*x+5) #error
# Ring is Monoid of ideals of Multivariate Polynomial Ring in x, y, z over Rational Field =>error:
# R must be a commutative ring
```

I don’t think this is connected. I’m getting the same error with sage 6.4.1. Also, The business of monoid vs ring only occurs when PolyBoRi implements the ring.

> a second failing example (different error):


```
sage: R.<x,y,z> = PolynomialRing(QQ)
sage: I = Ideal(5000*x*K+y*x,y*x+5) 
# TypeError: not a constant polynomial
```

See above.

> Why is c set to zero? At least this should be documented.  Is it not possible
> to have a different characteristic in case of an NotImplementedError?? Do you
> have an example which hits this issue?


I added a comment:

# We assume that our ring has characteristic zero if it does not implement a
# characteristic(). For example, generic quotient rings do not have a characteristic()
# method implemented. It is okay to set c = 0 here because we're only using the
# characteristic to pick a more specialized implementation for c = 2.

> in src/sage/rings/polynomial/multi_polynomial_ideal_libsingular.pyx , line
> 114, ff.sage_ideal_to_singular_ideal Is there a corresponding test for a
> conversion from "or a list of generators"?  If not, could you add one?


I added a test.

> in file 'src/sage/rings/polynomial/polynomial_singular_interface.py renaming
> singular_default to singular; why?  At other places there are singular_default
> usages. Is there a difference?


There is no need to rename singular to singular_default so I dropped it. It makes no difference either way.

> It seems that reduce examples show no examples for explicitly reducing leading
> coefficient, Could you add an example or a test which hits
> "ret.append(f.lc()**(-1)*f)"?


I added one.

> Sequence has no 'reduced' function Is that ok? Example:


I think this is fine as Sequence is not only for polynomials but for anything. However, reduced() only seems to make sense for polynomials.

> (fix Magma interface broken when moving decorators around): Could you explain
> what may get wrong and add an example?


Unfortunately, I forgot. I could investigate if you insist.



---

archive/issue_comments_274763.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2015-01-02T12:54:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274763",
    "user": "https://github.com/malb"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_274764.json:
```json
{
    "body": "<a id='comment:13'></a>PS: Thanks again for your comments and sorry for taking so long to reply!",
    "created_at": "2015-01-02T12:54:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274764",
    "user": "https://github.com/malb"
}
```

<a id='comment:13'></a>PS: Thanks again for your comments and sorry for taking so long to reply!



---

archive/issue_comments_274765.json:
```json
{
    "body": "<a id='comment:14'></a>> Thanks again for your comments and sorry for taking so long to reply!\n\n\nYou are welcome!\n\nPS: I'm currently busy so I can't promise to look \nat the patch earlier than next Thursday",
    "created_at": "2015-01-10T18:26:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274765",
    "user": "https://trac.sagemath.org/admin/accounts/users/jakobkroeker"
}
```

<a id='comment:14'></a>> Thanks again for your comments and sorry for taking so long to reply!


You are welcome!

PS: I'm currently busy so I can't promise to look 
at the patch earlier than next Thursday



---

archive/issue_comments_274766.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2015-01-24T18:16:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274766",
    "user": "https://trac.sagemath.org/admin/accounts/users/jakobkroeker"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_274767.json:
```json
{
    "body": "<a id='comment:15'></a>positive review.\n\nI rebased the commits on recent develop branch\n\nand ran all the tests again.",
    "created_at": "2015-01-24T18:16:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274767",
    "user": "https://trac.sagemath.org/admin/accounts/users/jakobkroeker"
}
```

<a id='comment:15'></a>positive review.

I rebased the commits on recent develop branch

and ran all the tests again.



---

archive/issue_comments_274768.json:
```json
{
    "body": "Changing branch from \"u/malb/t16585_mpolynomial_sequence\" to \"u/jakobkroeker/t16585_mpolynomial_sequence.rebased\"",
    "created_at": "2015-01-24T18:16:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274768",
    "user": "https://trac.sagemath.org/admin/accounts/users/jakobkroeker"
}
```

Changing branch from "u/malb/t16585_mpolynomial_sequence" to "u/jakobkroeker/t16585_mpolynomial_sequence.rebased"



---

archive/issue_comments_274769.json:
```json
{
    "body": "Changing commit from \"620322d9c80b1b30e71ce89d09e5ee42e079163d\" to \"68c684cb7ddea420460ac12797f59b2ba6457b37\"",
    "created_at": "2015-01-24T18:16:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274769",
    "user": "https://trac.sagemath.org/admin/accounts/users/jakobkroeker"
}
```

Changing commit from "620322d9c80b1b30e71ce89d09e5ee42e079163d" to "68c684cb7ddea420460ac12797f59b2ba6457b37"



---

archive/issue_comments_274770.json:
```json
{
    "body": "Changing status from positive_review to needs_work.",
    "created_at": "2015-01-25T18:24:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274770",
    "user": "https://github.com/vbraun"
}
```

Changing status from positive_review to needs_work.



---

archive/issue_comments_274771.json:
```json
{
    "body": "<a id='comment:16'></a>Breaks PDF docs:\n\n```\n! Package inputenc Error: Unicode char \\u8:\u1d62 not set up for use with LaTeX.\n\nSee the inputenc package documentation for explanation.\nType  H <return>  for immediate help.\n ...                                              \n                                                  \nl.22080 ...element in this ideal as $r = \\sum h\u1d62\n                                                  \u00b7f\u1d62$.\n? \n```",
    "created_at": "2015-01-25T18:24:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274771",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:16'></a>Breaks PDF docs:

```
! Package inputenc Error: Unicode char \u8:ᵢ not set up for use with LaTeX.

See the inputenc package documentation for explanation.
Type  H <return>  for immediate help.
 ...                                              
                                                  
l.22080 ...element in this ideal as $r = \sum hᵢ
                                                  ·fᵢ$.
? 
```



---

archive/issue_comments_274772.json:
```json
{
    "body": "Changing commit from \"68c684cb7ddea420460ac12797f59b2ba6457b37\" to \"1b208b5a2d4722637f82b14ef92ab46d4ab4d0e9\"",
    "created_at": "2015-01-25T19:49:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274772",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "68c684cb7ddea420460ac12797f59b2ba6457b37" to "1b208b5a2d4722637f82b14ef92ab46d4ab4d0e9"



---

archive/issue_comments_274773.json:
```json
{
    "body": "<a id='comment:17'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2015-01-25T19:49:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274773",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:17'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_274774.json:
```json
{
    "body": "<a id='comment:18'></a>`./sage --docbuild reference pdf` went through without errors with Jakob's fix.",
    "created_at": "2015-01-26T10:16:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274774",
    "user": "https://github.com/malb"
}
```

<a id='comment:18'></a>`./sage --docbuild reference pdf` went through without errors with Jakob's fix.



---

archive/issue_comments_274775.json:
```json
{
    "body": "Changing status from needs_work to positive_review.",
    "created_at": "2015-01-26T10:16:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274775",
    "user": "https://github.com/malb"
}
```

Changing status from needs_work to positive_review.



---

archive/issue_comments_274776.json:
```json
{
    "body": "<a id='comment:19'></a>the reference builds,\nbut building all pdf documentation fails\n\n```\n./sage --nodotsage --docbuild all pdf\n```",
    "created_at": "2015-01-26T10:43:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274776",
    "user": "https://trac.sagemath.org/admin/accounts/users/jakobkroeker"
}
```

<a id='comment:19'></a>the reference builds,
but building all pdf documentation fails

```
./sage --nodotsage --docbuild all pdf
```



---

archive/issue_comments_274777.json:
```json
{
    "body": "Changing status from positive_review to needs_work.",
    "created_at": "2015-01-26T10:43:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274777",
    "user": "https://trac.sagemath.org/admin/accounts/users/jakobkroeker"
}
```

Changing status from positive_review to needs_work.



---

archive/issue_comments_274778.json:
```json
{
    "body": "<a id='comment:20'></a>What is the error message?",
    "created_at": "2015-01-26T14:08:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274778",
    "user": "https://github.com/malb"
}
```

<a id='comment:20'></a>What is the error message?



---

archive/issue_comments_274779.json:
```json
{
    "body": "<a id='comment:21'></a>>What is the error message?\n\n\nthe failure was likely related to incremental docbuild (I didn' keep the error message)\n\nRunning 'make doc-clean' first did the trick ( the docbuild succeeds with warnings)\n\n1. Can you confirm that './sage --nodotsage --docbuild all pdf' succeeds`?\n\n2. Did I forget to review some other important aspects ? I must admit that there is a hint in the review checklist http://www.sagemath.org/doc/developer/trac.html?highlight=review to test if the reference manual builds or not.",
    "created_at": "2015-01-26T14:21:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274779",
    "user": "https://trac.sagemath.org/admin/accounts/users/jakobkroeker"
}
```

<a id='comment:21'></a>>What is the error message?


the failure was likely related to incremental docbuild (I didn' keep the error message)

Running 'make doc-clean' first did the trick ( the docbuild succeeds with warnings)

1. Can you confirm that './sage --nodotsage --docbuild all pdf' succeeds`?

2. Did I forget to review some other important aspects ? I must admit that there is a hint in the review checklist http://www.sagemath.org/doc/developer/trac.html?highlight=review to test if the reference manual builds or not.



---

archive/issue_comments_274780.json:
```json
{
    "body": "<a id='comment:22'></a>I can confirm than `make doc-pdf` succeeds. I think this should be all :).",
    "created_at": "2015-01-26T14:35:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274780",
    "user": "https://github.com/malb"
}
```

<a id='comment:22'></a>I can confirm than `make doc-pdf` succeeds. I think this should be all :).



---

archive/issue_comments_274781.json:
```json
{
    "body": "Changing status from needs_work to positive_review.",
    "created_at": "2015-01-26T14:35:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274781",
    "user": "https://github.com/malb"
}
```

Changing status from needs_work to positive_review.



---

archive/issue_comments_274782.json:
```json
{
    "body": "Changing branch from \"u/jakobkroeker/t16585_mpolynomial_sequence.rebased\" to \"1b208b5a2d4722637f82b14ef92ab46d4ab4d0e9\"",
    "created_at": "2015-01-29T13:26:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274782",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/jakobkroeker/t16585_mpolynomial_sequence.rebased" to "1b208b5a2d4722637f82b14ef92ab46d4ab4d0e9"



---

archive/issue_events_048661.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-01-29T13:26:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/16585#event-48661"
}
```



---

archive/issue_comments_274783.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2015-01-29T13:26:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/16585",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/16585#issuecomment-274783",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed
