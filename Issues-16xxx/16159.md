# Issue 16159: Moduli space of quadratic differentials

archive/issues_015922.json:
```json
{
    "assignees": [],
    "body": "**It was decided that flatsurf would be developed as an independent package**\n\n---\n\nOld description\n\nThe ticket introduce improvements to the current code in `sage.dynamics.flat_surfaces.quadratic_strata` especially:\n- classification of connected components (from Kontsevich-Zorich)\n- explicit permutation representative for each connected components (from Zorich)\n- recognize the connected component of a given permutation\n\nIt also refactors the code for Abelian differentials.\n\nAssignee: **@videlec**\n\nCC:  Fougeroc\n\nKeywords: **flat surfaces, Abelian differentials, quadratic differentials**\n\nReviewer: **Samuel Leli\u00e8vre**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16159_\n\n",
    "closed_at": "2016-06-12T12:02:30Z",
    "created_at": "2014-04-14T14:05:21Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20geometry",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Moduli space of quadratic differentials",
    "type": "issue",
    "updated_at": "2016-06-12T12:02:30Z",
    "url": "https://github.com/sagemath/sage/issues/16159",
    "user": "https://github.com/videlec"
}
```
**It was decided that flatsurf would be developed as an independent package**

---

Old description

The ticket introduce improvements to the current code in `sage.dynamics.flat_surfaces.quadratic_strata` especially:
- classification of connected components (from Kontsevich-Zorich)
- explicit permutation representative for each connected components (from Zorich)
- recognize the connected component of a given permutation

It also refactors the code for Abelian differentials.

Assignee: **@videlec**

CC:  Fougeroc

Keywords: **flat surfaces, Abelian differentials, quadratic differentials**

Reviewer: **Samuel Lelièvre**

_Issue created by migration from https://trac.sagemath.org/ticket/16159_





---

archive/issue_events_213162.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2014-04-14T14:05:21Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16159",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16159#event-213162"
}
```



---

archive/issue_events_213163.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2014-04-14T14:05:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16159",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "component: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16159#event-213163"
}
```



---

archive/issue_events_213164.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2014-04-14T14:05:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16159",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16159#event-213164"
}
```



---

archive/issue_events_213165.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2014-04-14T14:05:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16159",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16159#event-213165"
}
```



---

archive/issue_comments_208979.json:
```json
{
    "body": "Branch: **[u/vdelecroix/16159-strata](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/16159-strata)**",
    "created_at": "2014-04-14T14:06:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16159",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16159#issuecomment-208979",
    "user": "https://github.com/videlec"
}
```

Branch: **[u/vdelecroix/16159-strata](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/16159-strata)**



---

archive/issue_comments_208980.json:
```json
{
    "body": "Commit: **[1fc5d9b](https://github.com/sagemath/sagetrac-mirror/commit/1fc5d9ba75864cd633f880cc25d71abfd5804dfc)**",
    "created_at": "2014-04-14T14:06:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16159",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16159#issuecomment-208980",
    "user": "https://github.com/videlec"
}
```

Commit: **[1fc5d9b](https://github.com/sagemath/sagetrac-mirror/commit/1fc5d9ba75864cd633f880cc25d71abfd5804dfc)**



---

archive/issue_comments_208981.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/530fe858e831dc986235125a0baa3630bbc58505\">530fe85</a></td><td><code>Initial commit: new datastructure for iet.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/19f4edea2cc44264cae42c34c160b517268c5465\">19f4ede</a></td><td><code>No Subject. Modified: abelian_strata.py, strata.py</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d2577d4303585bcc36a30500a4834c66cbe4db49\">d2577d4</a></td><td><code>* * *</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e613180733a82efc8123e7e409a08a35096e6a6e\">e613180</a></td><td><code>add a database for irregular components</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/10e845a292f7da4d7192c43ece639fd1247cfa1b\">10e845a</a></td><td><code>try to fix documentation in quadratic_strata</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/18998a172d9f7d3c7273f736d7486778d936ac32\">18998a1</a></td><td><code>try to fix documentation...</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/356e03e9adf942efec59dba6f89666f3593a9327\">356e03e</a></td><td><code>fixing doc for strata</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1fc5d9ba75864cd633f880cc25d71abfd5804dfc\">1fc5d9b</a></td><td><code>fixed doc issue</code></td></tr></table>\n",
    "created_at": "2014-04-14T14:06:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16159",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16159#issuecomment-208981",
    "user": "https://github.com/videlec"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/530fe858e831dc986235125a0baa3630bbc58505">530fe85</a></td><td><code>Initial commit: new datastructure for iet.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/19f4edea2cc44264cae42c34c160b517268c5465">19f4ede</a></td><td><code>No Subject. Modified: abelian_strata.py, strata.py</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d2577d4303585bcc36a30500a4834c66cbe4db49">d2577d4</a></td><td><code>* * *</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e613180733a82efc8123e7e409a08a35096e6a6e">e613180</a></td><td><code>add a database for irregular components</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/10e845a292f7da4d7192c43ece639fd1247cfa1b">10e845a</a></td><td><code>try to fix documentation in quadratic_strata</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/18998a172d9f7d3c7273f736d7486778d936ac32">18998a1</a></td><td><code>try to fix documentation...</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/356e03e9adf942efec59dba6f89666f3593a9327">356e03e</a></td><td><code>fixing doc for strata</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1fc5d9ba75864cd633f880cc25d71abfd5804dfc">1fc5d9b</a></td><td><code>fixed doc issue</code></td></tr></table>




---

archive/issue_events_213166.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16159",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16159#event-213166"
}
```



---

archive/issue_events_213167.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16159",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16159#event-213167"
}
```



---

archive/issue_events_213168.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16159",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16159#event-213168"
}
```



---

archive/issue_events_213169.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16159",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16159#event-213169"
}
```



---

archive/issue_comments_208982.json:
```json
{
    "body": "Changed author from **Vincent Delecroix, Samuel Leli\u00e8vre** to none",
    "created_at": "2016-05-27T17:31:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16159",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16159#issuecomment-208982",
    "user": "https://github.com/videlec"
}
```

Changed author from **Vincent Delecroix, Samuel Lelièvre** to none



---

archive/issue_comments_208983.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,9 @@\n+**It was decided that flatsurf would be developed as an independent package**\n+\n+---\n+\n+Old description\n+\n The ticket introduce improvements to the current code in `sage.dynamics.flat_surfaces.quadratic_strata` especially:\n - classification of connected components (from Kontsevich-Zorich)\n - explicit permutation representative for each connected components (from Zorich)\n``````\n",
    "created_at": "2016-05-27T17:31:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16159",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16159#issuecomment-208983",
    "user": "https://github.com/videlec"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,9 @@
+**It was decided that flatsurf would be developed as an independent package**
+
+---
+
+Old description
+
 The ticket introduce improvements to the current code in `sage.dynamics.flat_surfaces.quadratic_strata` especially:
 - classification of connected components (from Kontsevich-Zorich)
 - explicit permutation representative for each connected components (from Zorich)
``````




---

archive/issue_comments_208984.json:
```json
{
    "body": "Changed branch from **[u/vdelecroix/16159-strata](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/16159-strata)** to none",
    "created_at": "2016-05-27T17:31:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16159",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16159#issuecomment-208984",
    "user": "https://github.com/videlec"
}
```

Changed branch from **[u/vdelecroix/16159-strata](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/16159-strata)** to none



---

archive/issue_comments_208985.json:
```json
{
    "body": "Changed dependencies from **#14683** to none",
    "created_at": "2016-05-27T17:31:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16159",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16159#issuecomment-208985",
    "user": "https://github.com/videlec"
}
```

Changed dependencies from **#14683** to none



---

archive/issue_events_213170.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2016-05-27T17:31:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16159",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16159#event-213170"
}
```



---

archive/issue_events_213171.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2016-05-27T17:31:43Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/16159",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16159#event-213171"
}
```



---

archive/issue_events_213172.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2016-05-27T17:31:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16159",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16159#event-213172"
}
```



---

archive/issue_events_213173.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2016-05-27T17:31:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16159",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16159#event-213173"
}
```



---

archive/issue_comments_208986.json:
```json
{
    "body": "Changed commit from **[1fc5d9b](https://github.com/sagemath/sagetrac-mirror/commit/1fc5d9ba75864cd633f880cc25d71abfd5804dfc)** to none",
    "created_at": "2016-05-27T17:31:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16159",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16159#issuecomment-208986",
    "user": "https://github.com/videlec"
}
```

Changed commit from **[1fc5d9b](https://github.com/sagemath/sagetrac-mirror/commit/1fc5d9ba75864cd633f880cc25d71abfd5804dfc)** to none



---

archive/issue_events_213174.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2016-05-27T23:08:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16159",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16159#event-213174"
}
```



---

archive/issue_events_213175.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2016-05-27T23:08:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16159",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16159#event-213175"
}
```



---

archive/issue_comments_208987.json:
```json
{
    "body": "Reviewer: **Samuel Leli\u00e8vre**",
    "created_at": "2016-05-27T23:08:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16159",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16159#issuecomment-208987",
    "user": "https://github.com/slel"
}
```

Reviewer: **Samuel Lelièvre**



---

archive/issue_events_213176.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-06-12T12:02:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16159",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16159#event-213176"
}
```



---

archive/issue_events_213177.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-06-12T12:02:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16159",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16159#event-213177"
}
```
