# Issue 16943: FiniteStateMachine._latex_: convert angles to floats where necessary.

archive/issues_016706.json:
```json
{
    "assignees": [],
    "body": "Fix the following bug:\n\n```\nsage: T = Transducer([(0,1),(1,1),(2,2),(3,3),(4,4)])\nsage: latex(T)\nTraceback (most recent call last)\n...\nTypeError: float argument required, not sage.symbolic.expression.Expression\n```\n\nCC:  skropf @dkrenn\n\nKeywords: **finite_state_machine, latex**\n\nBranch/Commit: **[13a2977](https://github.com/sagemath/sagetrac-mirror/commit/13a2977eecdb0f595d6e47abac801164459b6cd9)**\n\nReviewer: **Sara Kropf**\n\nAuthor: **Clemens Heuberger**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16943_\n\n",
    "closed_at": "2014-09-09T09:32:28Z",
    "created_at": "2014-09-08T09:48:50Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20finite%20state%20machines",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "FiniteStateMachine._latex_: convert angles to floats where necessary.",
    "type": "issue",
    "updated_at": "2014-09-09T09:32:28Z",
    "url": "https://github.com/sagemath/sage/issues/16943",
    "user": "https://github.com/cheuberg"
}
```
Fix the following bug:

```
sage: T = Transducer([(0,1),(1,1),(2,2),(3,3),(4,4)])
sage: latex(T)
Traceback (most recent call last)
...
TypeError: float argument required, not sage.symbolic.expression.Expression
```

CC:  skropf @dkrenn

Keywords: **finite_state_machine, latex**

Branch/Commit: **[13a2977](https://github.com/sagemath/sagetrac-mirror/commit/13a2977eecdb0f595d6e47abac801164459b6cd9)**

Reviewer: **Sara Kropf**

Author: **Clemens Heuberger**

_Issue created by migration from https://trac.sagemath.org/ticket/16943_





---

archive/issue_events_224583.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-09-08T09:48:50Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/16943",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16943#event-224583"
}
```



---

archive/issue_events_224584.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-09-08T09:48:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16943",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20finite%20state%20machines",
    "label_color": "0000ff",
    "label_name": "component: finite state machines",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16943#event-224584"
}
```



---

archive/issue_events_224585.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-09-08T09:48:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16943",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16943#event-224585"
}
```



---

archive/issue_events_224586.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-09-08T09:48:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16943",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16943#event-224586"
}
```



---

archive/issue_comments_226261.json:
```json
{
    "body": "Author: **Clemens Heuberger**",
    "created_at": "2014-09-08T10:07:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16943",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16943#issuecomment-226261",
    "user": "https://github.com/cheuberg"
}
```

Author: **Clemens Heuberger**



---

archive/issue_events_224587.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-09-08T10:07:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16943",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16943#event-224587"
}
```



---

archive/issue_comments_226262.json:
```json
{
    "body": "Branch: **[u/cheuberg/fsm/latex-coordinates](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/latex-coordinates)**",
    "created_at": "2014-09-08T10:07:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16943",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16943#issuecomment-226262",
    "user": "https://github.com/cheuberg"
}
```

Branch: **[u/cheuberg/fsm/latex-coordinates](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/latex-coordinates)**



---

archive/issue_events_224588.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-09-08T10:07:13Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/16943",
    "title_is": "FiniteStateMachine._latex_: convert angles to floats where necessary.",
    "title_was": "FiniteStateMachine.__latex__: convert angles to floats where necessary.",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16943#event-224588"
}
```



---

archive/issue_comments_226263.json:
```json
{
    "body": "Commit: **[13a2977](https://github.com/sagemath/sagetrac-mirror/commit/13a2977eecdb0f595d6e47abac801164459b6cd9)**",
    "created_at": "2014-09-08T10:07:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16943",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16943#issuecomment-226263",
    "user": "https://github.com/cheuberg"
}
```

Commit: **[13a2977](https://github.com/sagemath/sagetrac-mirror/commit/13a2977eecdb0f595d6e47abac801164459b6cd9)**



---

archive/issue_comments_226264.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/13a2977eecdb0f595d6e47abac801164459b6cd9\">13a2977</a></td><td><code>Trac #16943: convert angle_label to float if possible</code></td></tr></table>\n",
    "created_at": "2014-09-08T10:07:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16943",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16943#issuecomment-226264",
    "user": "https://github.com/cheuberg"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/13a2977eecdb0f595d6e47abac801164459b6cd9">13a2977</a></td><td><code>Trac #16943: convert angle_label to float if possible</code></td></tr></table>




---

archive/issue_events_224589.json:
```json
{
    "actor": "https://github.com/sagetrac-skropf",
    "created_at": "2014-09-08T11:54:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16943",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16943#event-224589"
}
```



---

archive/issue_events_224590.json:
```json
{
    "actor": "https://github.com/sagetrac-skropf",
    "created_at": "2014-09-08T11:54:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16943",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16943#event-224590"
}
```



---

archive/issue_comments_226265.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nThis is fine for me. I set it to positive review.",
    "created_at": "2014-09-08T11:54:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16943",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16943#issuecomment-226265",
    "user": "https://github.com/sagetrac-skropf"
}
```

<a id='comment:2'>Comment 2:</a>
This is fine for me. I set it to positive review.



---

archive/issue_comments_226266.json:
```json
{
    "body": "Reviewer: **Sara Kropf**",
    "created_at": "2014-09-08T14:28:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16943",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16943#issuecomment-226266",
    "user": "https://github.com/cheuberg"
}
```

Reviewer: **Sara Kropf**



---

archive/issue_comments_226267.json:
```json
{
    "body": "Changed branch from **[u/cheuberg/fsm/latex-coordinates](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/latex-coordinates)** to **[13a2977](https://github.com/sagemath/sagetrac-mirror/commit/13a2977eecdb0f595d6e47abac801164459b6cd9)**",
    "created_at": "2014-09-09T09:32:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16943",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16943#issuecomment-226267",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/cheuberg/fsm/latex-coordinates](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/latex-coordinates)** to **[13a2977](https://github.com/sagemath/sagetrac-mirror/commit/13a2977eecdb0f595d6e47abac801164459b6cd9)**



---

archive/issue_events_224591.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-09-09T09:32:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16943",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16943#event-224591"
}
```



---

archive/issue_events_224592.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "f497c5761d0e694d19bf4664bcb19885295d0dec",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-09-09T09:32:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16943",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16943#event-224592"
}
```
