# Issue 16818: Installing kash3 gives error message

archive/issues_016581.json:
```json
{
    "assignees": [],
    "body": "sage -i kash3 works, but gives\n\n  rm: cannot remove `kash': No such file or directory\n\nafter installing. This happens on Ubuntu 12.04.5 LTS, Sage version 6.3. Maybe just \"3\" forgotten on directory name?\n\n\nComponent: **packages: optional**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/16818_\n\n",
    "closed_at": "2015-09-30T21:53:54Z",
    "created_at": "2014-08-14T09:22:55Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20optional",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Installing kash3 gives error message",
    "type": "issue",
    "updated_at": "2015-09-30T21:53:54Z",
    "url": "https://github.com/sagemath/sage/issues/16818",
    "user": "https://github.com/jm58660"
}
```
sage -i kash3 works, but gives

  rm: cannot remove `kash': No such file or directory

after installing. This happens on Ubuntu 12.04.5 LTS, Sage version 6.3. Maybe just "3" forgotten on directory name?


Component: **packages: optional**

_Issue created by migration from https://trac.sagemath.org/ticket/16818_





---

archive/issue_events_240426.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2014-08-14T09:22:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16818",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20optional",
    "label_color": "000080",
    "label_name": "c: packages: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16818#event-240426"
}
```



---

archive/issue_events_240427.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2014-08-14T09:22:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16818",
    "label": "https://github.com/sagemath/sage/labels/p%3A%205%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p: 5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16818#event-240427"
}
```



---

archive/issue_events_240428.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2014-08-14T09:22:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16818",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16818#event-240428"
}
```



---

archive/issue_comments_220741.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nThis is due to `rm kash` after `cd \"$SAGE_LOCAL/bin/\"`. It is unneeded, as `f` in next command `ln -sf kash3 kash` means that existing destination will be removed.",
    "created_at": "2015-07-27T13:31:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16818",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16818#issuecomment-220741",
    "user": "https://github.com/jm58660"
}
```

<div id="comment:1" align="right">comment:1</div>

This is due to `rm kash` after `cd "$SAGE_LOCAL/bin/"`. It is unneeded, as `f` in next command `ln -sf kash3 kash` means that existing destination will be removed.



---

archive/issue_events_240429.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2015-09-28T05:59:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16818",
    "label": "https://github.com/sagemath/sage/labels/p%3A%205%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p: 5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16818#event-240429"
}
```



---

archive/issue_events_240430.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2015-09-28T05:59:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16818",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16818#event-240430"
}
```



---

archive/issue_comments_220742.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nAs the whole packaging system changes, this is now obsolete.",
    "created_at": "2015-09-28T05:59:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/16818",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/16818#issuecomment-220742",
    "user": "https://github.com/jm58660"
}
```

<div id="comment:2" align="right">comment:2</div>

As the whole packaging system changes, this is now obsolete.



---

archive/issue_events_240431.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2015-09-28T05:59:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16818",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16818#event-240431"
}
```



---

archive/issue_events_240432.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2015-09-28T05:59:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16818",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16818#event-240432"
}
```



---

archive/issue_events_240433.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2015-09-28T05:59:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16818",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16818#event-240433"
}
```



---

archive/issue_events_240434.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-09-30T21:53:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/16818",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16818#event-240434"
}
```



---

archive/issue_events_240435.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-09-30T21:53:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/16818",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16818#event-240435"
}
```



---

archive/issue_events_240436.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-09-30T21:53:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/16818",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "a6a6a6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/16818#event-240436"
}
```
