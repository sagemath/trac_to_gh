# Issue 15747: bus or segmentation fault in integral computation

archive/issues_015510.json:
```json
{
    "assignees": [],
    "body": "From [this ask question](http://ask.sagemath.org/question/3464/integral-failing-with-segmentation-fault):\n\n```\nsage: ys=0.06+2*(x-0.0275)\nsage: xs=ys/1.516\nsage: i=1/(xs/(1+xs)-x/(1+x))\nsage: integral(i,x)\n250/121*x^2 + 423541/58564*x + 86958139/28344976*log(484*x + 5)\n```\n\nBut, the same integral with boundaries:\n\n```\nsage: integral(i,x,0,0.275)\n```\n\nleads to a series of lines of the form:\n\n```\n;;;\n;;; Detected access to protected memory, also kwown as 'bus or segmentation fault'.\n;;; Jumping to the outermost toplevel prompt\n;;;\n```\n\n\nKeywords: **integral, symbolic**\n\nReviewer: **Dima Pasechnik**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15747_\n\n",
    "created_at": "2014-01-27T16:07:23Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/invalid",
        "https://github.com/sagemath/sage/labels/needs%20work"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "bus or segmentation fault in integral computation",
    "type": "issue",
    "updated_at": "2021-08-27T19:24:18Z",
    "url": "https://github.com/sagemath/sage/issues/15747",
    "user": "https://github.com/sagetrac-tmonteil"
}
```
From [this ask question](http://ask.sagemath.org/question/3464/integral-failing-with-segmentation-fault):

```
sage: ys=0.06+2*(x-0.0275)
sage: xs=ys/1.516
sage: i=1/(xs/(1+xs)-x/(1+x))
sage: integral(i,x)
250/121*x^2 + 423541/58564*x + 86958139/28344976*log(484*x + 5)
```

But, the same integral with boundaries:

```
sage: integral(i,x,0,0.275)
```

leads to a series of lines of the form:

```
;;;
;;; Detected access to protected memory, also kwown as 'bus or segmentation fault'.
;;; Jumping to the outermost toplevel prompt
;;;
```


Keywords: **integral, symbolic**

Reviewer: **Dima Pasechnik**

_Issue created by migration from https://trac.sagemath.org/ticket/15747_





---

archive/issue_events_207328.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2014-01-27T16:07:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15747",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15747#event-207328"
}
```



---

archive/issue_events_207329.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2014-01-27T16:07:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15747",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15747#event-207329"
}
```



---

archive/issue_events_207330.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2014-01-27T16:07:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15747",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15747#event-207330"
}
```



---

archive/issue_events_207331.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2014-01-27T16:07:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15747",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15747#event-207331"
}
```



---

archive/issue_comments_200919.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nI can confirm that this problem also occurs with the integral to 11/40.\n\nThat said, \n\n```\nsage: i\n1/((1.31926121372032*x + 0.00329815303430079)/(1.31926121372032*x + 1.00329815303430) - x/(x + 1))\n```\nand usually Maxima does not particularly like integrating floating point things exactly, because it sort of doesn't make sense.\n\n```\nsage: numerical_integral(i,0,0.275)\n(12.325862793647284, 4.191806193893343e-11)\n```",
    "created_at": "2014-01-28T02:46:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15747#issuecomment-200919",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:1" align="right">Comment 1</div>

I can confirm that this problem also occurs with the integral to 11/40.

That said, 

```
sage: i
1/((1.31926121372032*x + 0.00329815303430079)/(1.31926121372032*x + 1.00329815303430) - x/(x + 1))
```
and usually Maxima does not particularly like integrating floating point things exactly, because it sort of doesn't make sense.

```
sage: numerical_integral(i,0,0.275)
(12.325862793647284, 4.191806193893343e-11)
```



---

archive/issue_events_207332.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15747",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15747#event-207332"
}
```



---

archive/issue_events_207333.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15747",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15747#event-207333"
}
```



---

archive/issue_events_207334.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15747",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15747#event-207334"
}
```



---

archive/issue_events_207335.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15747",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15747#event-207335"
}
```



---

archive/issue_events_207336.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15747",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15747#event-207336"
}
```



---

archive/issue_events_207337.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15747",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15747#event-207337"
}
```



---

archive/issue_events_207338.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-01-31T16:29:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15747",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15747#event-207338"
}
```



---

archive/issue_events_207339.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-01-31T16:29:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15747",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15747#event-207339"
}
```



---

archive/issue_events_207340.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-01-31T16:29:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15747",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15747#event-207340"
}
```



---

archive/issue_events_207341.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-01-31T16:29:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15747",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15747#event-207341"
}
```



---

archive/issue_comments_200920.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nStill an issue",
    "created_at": "2017-03-03T23:36:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15747#issuecomment-200920",
    "user": "https://github.com/sagetrac-jakobkroeker"
}
```

<div id="comment:6" align="right">Comment 6</div>

Still an issue



---

archive/issue_events_207342.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-08-27T18:38:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15747",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15747#event-207342"
}
```



---

archive/issue_comments_200921.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nappears to be fixed in Sage 9.4",
    "created_at": "2021-08-27T18:38:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15747#issuecomment-200921",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:7" align="right">Comment 7</div>

appears to be fixed in Sage 9.4



---

archive/issue_events_207343.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-08-27T18:38:28Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15747",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15747#event-207343"
}
```



---

archive/issue_events_207344.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-08-27T18:38:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15747",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15747#event-207344"
}
```



---

archive/issue_events_207345.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-08-27T18:38:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15747",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15747#event-207345"
}
```



---

archive/issue_comments_200922.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2021-08-27T18:38:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15747#issuecomment-200922",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_events_207346.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-08-27T18:38:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15747",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15747#event-207346"
}
```



---

archive/issue_events_207347.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-08-27T18:38:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15747",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15747#event-207347"
}
```



---

archive/issue_events_207348.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2021-08-27T19:24:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15747",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15747#event-207348"
}
```



---

archive/issue_events_207349.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2021-08-27T19:24:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15747",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15747#event-207349"
}
```



---

archive/issue_comments_200923.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nIt is still broken on my computer (running 9.4) :(",
    "created_at": "2021-08-27T19:24:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15747#issuecomment-200923",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

<div id="comment:9" align="right">Comment 9</div>

It is still broken on my computer (running 9.4) :(
