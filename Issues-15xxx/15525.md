# Issue 15525: More partition parents and fixes to global options

archive/issues_015288.json:
```json
{
    "assignees": [],
    "body": "Currently the following breaks:\n\n```\nsage: P = Partitions(5, max_length=4)\nsage: loads(dumps(P))\n```\nsince `PartitionOptions` does not pickle, hence the `IntegerListsLex`, which takes it as an (optional) input, does not.\n\nThe proposed solution is two-fold:\n\n- Add a class for partitions that inherit from `IntegerListsLex` (or use it as an iterator).\n- Add a warning when using the `global_options` parameter that the resulting class might not pickle.\n\nHere's also a discussion on sage-combinat-devel:\n\nhttps://groups.google.com/forum/?fromgroups=#!topic/sage-combinat-devel/Hyak8ndRi4o\n\nI'm also going to add parent classes for k-regular partitions.\n\nAssignee: **sage-combinat**\n\nCC:  sage-combinat @nthiery @AndrewAtLarge @kevindilks\n\nKeywords: **partitions, global options**\n\nBranch/Commit: **[1ec481d](https://github.com/sagemath/sagetrac-mirror/commit/1ec481d803e3fa7d8c3d0e4038f4c476a73c0fd0)**\n\nReviewer: **Kevin Dilks**\n\nAuthor: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15525_\n\n",
    "closed_at": "2015-10-12T07:16:27Z",
    "created_at": "2013-12-16T16:51:59Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.10",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "More partition parents and fixes to global options",
    "type": "issue",
    "updated_at": "2015-10-12T07:16:27Z",
    "url": "https://github.com/sagemath/sage/issues/15525",
    "user": "https://github.com/tscrim"
}
```
Currently the following breaks:

```
sage: P = Partitions(5, max_length=4)
sage: loads(dumps(P))
```
since `PartitionOptions` does not pickle, hence the `IntegerListsLex`, which takes it as an (optional) input, does not.

The proposed solution is two-fold:

- Add a class for partitions that inherit from `IntegerListsLex` (or use it as an iterator).
- Add a warning when using the `global_options` parameter that the resulting class might not pickle.

Here's also a discussion on sage-combinat-devel:

https://groups.google.com/forum/?fromgroups=#!topic/sage-combinat-devel/Hyak8ndRi4o

I'm also going to add parent classes for k-regular partitions.

Assignee: **sage-combinat**

CC:  sage-combinat @nthiery @AndrewAtLarge @kevindilks

Keywords: **partitions, global options**

Branch/Commit: **[1ec481d](https://github.com/sagemath/sagetrac-mirror/commit/1ec481d803e3fa7d8c3d0e4038f4c476a73c0fd0)**

Reviewer: **Kevin Dilks**

Author: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/15525_





---

archive/issue_events_203795.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-12-16T16:51:59Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15525#event-203795"
}
```



---

archive/issue_events_203796.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-12-16T16:51:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15525#event-203796"
}
```



---

archive/issue_events_203797.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-12-16T16:51:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15525#event-203797"
}
```



---

archive/issue_events_203798.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-12-16T16:51:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15525#event-203798"
}
```



---

archive/issue_comments_196329.json:
```json
{
    "body": "Branch: **[public/combinat/partitions_constraints](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/partitions_constraints)**",
    "created_at": "2013-12-16T16:52:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15525#issuecomment-196329",
    "user": "https://github.com/tscrim"
}
```

Branch: **[public/combinat/partitions_constraints](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/partitions_constraints)**



---

archive/issue_comments_196330.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nHere's the initial branch, still a little more work to be done.",
    "created_at": "2013-12-16T16:52:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15525#issuecomment-196330",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:1'>Comment 1:</a>
Here's the initial branch, still a little more work to be done.



---

archive/issue_comments_196331.json:
```json
{
    "body": "Commit: **[7982d3a](https://github.com/sagemath/sagetrac-mirror/commit/7982d3ad9815b6313ac93880467aaf71ddb096ca)**",
    "created_at": "2013-12-16T16:53:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15525#issuecomment-196331",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[7982d3a](https://github.com/sagemath/sagetrac-mirror/commit/7982d3ad9815b6313ac93880467aaf71ddb096ca)**



---

archive/issue_comments_196332.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td>[7982d3a](https://github.com/sagemath/sagetrac-mirror/commit/7982d3a)</td><td><code>Iniital fix and added regular partitions.</code></td></tr></table>\n",
    "created_at": "2013-12-16T16:53:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15525#issuecomment-196332",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:2'>Comment 2:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td>[7982d3a](https://github.com/sagemath/sagetrac-mirror/commit/7982d3a)</td><td><code>Iniital fix and added regular partitions.</code></td></tr></table>




---

archive/issue_comments_196333.json:
```json
{
    "body": "Changed branch from **[public/combinat/partitions_constraints](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/partitions_constraints)** to **[public/combinat/partitions_constraints-15525](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/partitions_constraints-15525)**",
    "created_at": "2013-12-16T16:53:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15525#issuecomment-196333",
    "user": "https://github.com/tscrim"
}
```

Changed branch from **[public/combinat/partitions_constraints](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/partitions_constraints)** to **[public/combinat/partitions_constraints-15525](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/partitions_constraints-15525)**



---

archive/issue_comments_196334.json:
```json
{
    "body": "Changed commit from **[7982d3a](https://github.com/sagemath/sagetrac-mirror/commit/7982d3ad9815b6313ac93880467aaf71ddb096ca)** to none",
    "created_at": "2013-12-16T16:53:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15525#issuecomment-196334",
    "user": "https://github.com/tscrim"
}
```

Changed commit from **[7982d3a](https://github.com/sagemath/sagetrac-mirror/commit/7982d3ad9815b6313ac93880467aaf71ddb096ca)** to none



---

archive/issue_comments_196335.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td>[7982d3a](https://github.com/sagemath/sagetrac-mirror/commit/7982d3a)</td><td><code>Iniital fix and added regular partitions.</code></td></tr></table>\n",
    "created_at": "2013-12-16T16:54:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15525#issuecomment-196335",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:4'>Comment 4:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td>[7982d3a](https://github.com/sagemath/sagetrac-mirror/commit/7982d3a)</td><td><code>Iniital fix and added regular partitions.</code></td></tr></table>




---

archive/issue_comments_196336.json:
```json
{
    "body": "Commit: **[7982d3a](https://github.com/sagemath/sagetrac-mirror/commit/7982d3ad9815b6313ac93880467aaf71ddb096ca)**",
    "created_at": "2013-12-16T16:54:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15525#issuecomment-196336",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[7982d3a](https://github.com/sagemath/sagetrac-mirror/commit/7982d3ad9815b6313ac93880467aaf71ddb096ca)**



---

archive/issue_comments_196337.json:
```json
{
    "body": "Changed commit from **[7982d3a](https://github.com/sagemath/sagetrac-mirror/commit/7982d3ad9815b6313ac93880467aaf71ddb096ca)** to **[1adb368](https://github.com/sagemath/sagetrac-mirror/commit/1adb368c00b4b8e1eb49186af47ee1e86dafd94f)**",
    "created_at": "2013-12-27T03:02:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15525#issuecomment-196337",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[7982d3a](https://github.com/sagemath/sagetrac-mirror/commit/7982d3ad9815b6313ac93880467aaf71ddb096ca)** to **[1adb368](https://github.com/sagemath/sagetrac-mirror/commit/1adb368c00b4b8e1eb49186af47ee1e86dafd94f)**



---

archive/issue_comments_196338.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1adb368\">1adb368</a></td><td><code>Added deprecation to IntegerListLex global_options arg. Fixed doctests.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4143c96\">4143c96</a></td><td><code>Merge branch 'develop' into public/combinat/partitions_constraints-15525</code></td></tr></table>\n",
    "created_at": "2013-12-27T03:02:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15525#issuecomment-196338",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:5'>Comment 5:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1adb368">1adb368</a></td><td><code>Added deprecation to IntegerListLex global_options arg. Fixed doctests.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4143c96">4143c96</a></td><td><code>Merge branch 'develop' into public/combinat/partitions_constraints-15525</code></td></tr></table>




---

archive/issue_comments_196339.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nOkay, ready for consumption.",
    "created_at": "2013-12-27T03:03:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15525#issuecomment-196339",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:6'>Comment 6:</a>
Okay, ready for consumption.



---

archive/issue_events_203799.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-12-27T03:03:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15525#event-203799"
}
```



---

archive/issue_events_203800.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15525#event-203800"
}
```



---

archive/issue_events_203801.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15525#event-203801"
}
```



---

archive/issue_events_203802.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15525#event-203802"
}
```



---

archive/issue_events_203803.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15525#event-203803"
}
```



---

archive/issue_events_203804.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15525#event-203804"
}
```



---

archive/issue_events_203805.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15525#event-203805"
}
```



---

archive/issue_comments_196340.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c477d6cf7d5ac2143f700dc83123f7d2d8d919ac\">c477d6c</a></td><td><code>Merge branch 'public/combinat/partitions_constraints-15525' of trac.sagemath.org:sage into public/combinat/partitions_constraints-15525</code></td></tr></table>\n",
    "created_at": "2015-01-04T03:58:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15525#issuecomment-196340",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:10'>Comment 10:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c477d6cf7d5ac2143f700dc83123f7d2d8d919ac">c477d6c</a></td><td><code>Merge branch 'public/combinat/partitions_constraints-15525' of trac.sagemath.org:sage into public/combinat/partitions_constraints-15525</code></td></tr></table>




---

archive/issue_comments_196341.json:
```json
{
    "body": "Changed commit from **[1adb368](https://github.com/sagemath/sagetrac-mirror/commit/1adb368c00b4b8e1eb49186af47ee1e86dafd94f)** to **[c477d6c](https://github.com/sagemath/sagetrac-mirror/commit/c477d6cf7d5ac2143f700dc83123f7d2d8d919ac)**",
    "created_at": "2015-01-04T03:58:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15525#issuecomment-196341",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[1adb368](https://github.com/sagemath/sagetrac-mirror/commit/1adb368c00b4b8e1eb49186af47ee1e86dafd94f)** to **[c477d6c](https://github.com/sagemath/sagetrac-mirror/commit/c477d6cf7d5ac2143f700dc83123f7d2d8d919ac)**



---

archive/issue_comments_196342.json:
```json
{
    "body": "Changed commit from **[c477d6c](https://github.com/sagemath/sagetrac-mirror/commit/c477d6cf7d5ac2143f700dc83123f7d2d8d919ac)** to **[03dcda9](https://github.com/sagemath/sagetrac-mirror/commit/03dcda9bd81cc42c2c61439ea11be485e15f6142)**",
    "created_at": "2015-05-07T17:46:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15525#issuecomment-196342",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[c477d6c](https://github.com/sagemath/sagetrac-mirror/commit/c477d6cf7d5ac2143f700dc83123f7d2d8d919ac)** to **[03dcda9](https://github.com/sagemath/sagetrac-mirror/commit/03dcda9bd81cc42c2c61439ea11be485e15f6142)**



---

archive/issue_comments_196343.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/03dcda9bd81cc42c2c61439ea11be485e15f6142\">03dcda9</a></td><td><code>Merge branch 'public/combinat/partitions_constraints-15525' of trac.sagemath.org:sage into public/combinat/partitions_constraints-15525</code></td></tr></table>\n",
    "created_at": "2015-05-07T17:46:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15525#issuecomment-196343",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:11'>Comment 11:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/03dcda9bd81cc42c2c61439ea11be485e15f6142">03dcda9</a></td><td><code>Merge branch 'public/combinat/partitions_constraints-15525' of trac.sagemath.org:sage into public/combinat/partitions_constraints-15525</code></td></tr></table>




---

archive/issue_events_203806.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-08-23T16:38:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15525#event-203806"
}
```



---

archive/issue_events_203807.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-08-23T16:38:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15525#event-203807"
}
```



---

archive/issue_comments_196344.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\none failing doctest, see patchbot report",
    "created_at": "2015-08-23T16:38:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15525#issuecomment-196344",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:12'>Comment 12:</a>
one failing doctest, see patchbot report



---

archive/issue_comments_196345.json:
```json
{
    "body": "Changed commit from **[03dcda9](https://github.com/sagemath/sagetrac-mirror/commit/03dcda9bd81cc42c2c61439ea11be485e15f6142)** to **[0cc28a0](https://github.com/sagemath/sagetrac-mirror/commit/0cc28a0202ba183faec750fa4e02083d1500e52d)**",
    "created_at": "2015-10-09T21:47:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15525#issuecomment-196345",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[03dcda9](https://github.com/sagemath/sagetrac-mirror/commit/03dcda9bd81cc42c2c61439ea11be485e15f6142)** to **[0cc28a0](https://github.com/sagemath/sagetrac-mirror/commit/0cc28a0202ba183faec750fa4e02083d1500e52d)**



---

archive/issue_comments_196346.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/82f4fd5e98de8bf7290f54aa39155d25f88c649c\">82f4fd5</a></td><td><code>Merge branch 'public/combinat/partitions_constraints-15525' of trac.sagemath.org:sage into public/combinat/partitions_constraints-15525</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0cc28a0202ba183faec750fa4e02083d1500e52d\">0cc28a0</a></td><td><code>Fixing some things.</code></td></tr></table>\n",
    "created_at": "2015-10-09T21:47:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15525#issuecomment-196346",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:13'>Comment 13:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/82f4fd5e98de8bf7290f54aa39155d25f88c649c">82f4fd5</a></td><td><code>Merge branch 'public/combinat/partitions_constraints-15525' of trac.sagemath.org:sage into public/combinat/partitions_constraints-15525</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0cc28a0202ba183faec750fa4e02083d1500e52d">0cc28a0</a></td><td><code>Fixing some things.</code></td></tr></table>




---

archive/issue_events_203808.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-10-09T21:49:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15525#event-203808"
}
```



---

archive/issue_events_203809.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-10-09T21:49:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15525#event-203809"
}
```



---

archive/issue_events_203810.json:
```json
{
    "actor": "https://github.com/kevindilks",
    "created_at": "2015-10-09T22:25:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15525#event-203810"
}
```



---

archive/issue_events_203811.json:
```json
{
    "actor": "https://github.com/kevindilks",
    "created_at": "2015-10-09T22:25:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "milestone_number": null,
    "milestone_title": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15525#event-203811"
}
```



---

archive/issue_comments_196347.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nSo far only noticing very minor issues.\n\n* In the definition of being \\ell-regular, it should mention that m_i is the multiplicity of i in the partition. The first time I looked at this, I thought that m_i was maybe referring to the part sizes. In the source code you can just search for 'm_i' to see what it means, but in the HTML doc 'm_i' is always going to be Tex'd and harder to search for. Also, `character_polynomial()` doc used a_i to refer to the multiplicity of i in a permutation alpha, so maybe that should be changed to m_i to be consistent.\n* Does the code ever check that \\ell is an integer, and preferably a nonnegative one?",
    "created_at": "2015-10-10T21:57:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15525#issuecomment-196347",
    "user": "https://github.com/kevindilks"
}
```

<a id='comment:16'>Comment 16:</a>
So far only noticing very minor issues.

* In the definition of being \ell-regular, it should mention that m_i is the multiplicity of i in the partition. The first time I looked at this, I thought that m_i was maybe referring to the part sizes. In the source code you can just search for 'm_i' to see what it means, but in the HTML doc 'm_i' is always going to be Tex'd and harder to search for. Also, `character_polynomial()` doc used a_i to refer to the multiplicity of i in a permutation alpha, so maybe that should be changed to m_i to be consistent.
* Does the code ever check that \ell is an integer, and preferably a nonnegative one?



---

archive/issue_comments_196348.json:
```json
{
    "body": "Changed commit from **[0cc28a0](https://github.com/sagemath/sagetrac-mirror/commit/0cc28a0202ba183faec750fa4e02083d1500e52d)** to **[1ec481d](https://github.com/sagemath/sagetrac-mirror/commit/1ec481d803e3fa7d8c3d0e4038f4c476a73c0fd0)**",
    "created_at": "2015-10-10T23:06:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15525#issuecomment-196348",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[0cc28a0](https://github.com/sagemath/sagetrac-mirror/commit/0cc28a0202ba183faec750fa4e02083d1500e52d)** to **[1ec481d](https://github.com/sagemath/sagetrac-mirror/commit/1ec481d803e3fa7d8c3d0e4038f4c476a73c0fd0)**



---

archive/issue_comments_196349.json:
```json
{
    "body": "<a id='comment:17'>Comment 17:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1ec481d803e3fa7d8c3d0e4038f4c476a73c0fd0\">1ec481d</a></td><td><code>Addressing Kevin's comments.</code></td></tr></table>\n",
    "created_at": "2015-10-10T23:06:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15525#issuecomment-196349",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:17'>Comment 17:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1ec481d803e3fa7d8c3d0e4038f4c476a73c0fd0">1ec481d</a></td><td><code>Addressing Kevin's comments.</code></td></tr></table>




---

archive/issue_events_203812.json:
```json
{
    "actor": "https://github.com/kevindilks",
    "created_at": "2015-10-10T23:08:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15525#event-203812"
}
```



---

archive/issue_events_203813.json:
```json
{
    "actor": "https://github.com/kevindilks",
    "created_at": "2015-10-10T23:08:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15525#event-203813"
}
```



---

archive/issue_events_203814.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-10-11T03:37:38Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "milestone_number": null,
    "milestone_title": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15525#event-203814"
}
```



---

archive/issue_events_203815.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-10-11T03:37:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "milestone_number": null,
    "milestone_title": "sage-6.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15525#event-203815"
}
```



---

archive/issue_comments_196350.json:
```json
{
    "body": "Reviewer: **Kevin Dilks**",
    "created_at": "2015-10-11T03:37:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15525#issuecomment-196350",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Kevin Dilks**



---

archive/issue_comments_196351.json:
```json
{
    "body": "Changed branch from **[public/combinat/partitions_constraints-15525](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/partitions_constraints-15525)** to **[1ec481d](https://github.com/sagemath/sagetrac-mirror/commit/1ec481d803e3fa7d8c3d0e4038f4c476a73c0fd0)**",
    "created_at": "2015-10-12T07:16:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15525#issuecomment-196351",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/combinat/partitions_constraints-15525](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/partitions_constraints-15525)** to **[1ec481d](https://github.com/sagemath/sagetrac-mirror/commit/1ec481d803e3fa7d8c3d0e4038f4c476a73c0fd0)**



---

archive/issue_events_203816.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-10-12T07:16:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15525#event-203816"
}
```



---

archive/issue_events_203817.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "d56ab8fbf9963bdc3c47ed22be64d0d2cea3ceb2",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-10-12T07:16:27Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15525",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15525#event-203817"
}
```
