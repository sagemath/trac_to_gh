# Issue 15081: fix ffmpeg command for creating gif in animate.py

archive/issues_014844.json:
```json
{
    "assignees": [],
    "body": "See [this ask.sagemath](http://ask.sagemath.org/question/2927/update-animatepy-for-gif-creation-using-ffmpeg) question. The option `-loop_output` for ffmpeg is deprecated and instead one should use `-loop`.\n\nCC:  @kcrisman\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15081_\n\n",
    "closed_at": "2014-07-03T02:04:42Z",
    "created_at": "2013-08-22T16:20:49Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20graphics",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fix ffmpeg command for creating gif in animate.py",
    "type": "issue",
    "updated_at": "2014-07-03T02:04:42Z",
    "url": "https://github.com/sagemath/sage/issues/15081",
    "user": "https://github.com/ppurka"
}
```
See [this ask.sagemath](http://ask.sagemath.org/question/2927/update-animatepy-for-gif-creation-using-ffmpeg) question. The option `-loop_output` for ffmpeg is deprecated and instead one should use `-loop`.

CC:  @kcrisman

_Issue created by migration from https://trac.sagemath.org/ticket/15081_





---

archive/issue_events_196741.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2013-08-22T16:20:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15081",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15081#event-196741"
}
```



---

archive/issue_events_196742.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2013-08-22T16:20:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15081",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graphics",
    "label_color": "0000ff",
    "label_name": "component: graphics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15081#event-196742"
}
```



---

archive/issue_events_196743.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2013-08-22T16:20:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15081",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15081#event-196743"
}
```



---

archive/issue_events_196744.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2013-08-22T16:20:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15081",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15081#event-196744"
}
```



---

archive/issue_events_196745.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15081",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15081#event-196745"
}
```



---

archive/issue_events_196746.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15081",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15081#event-196746"
}
```



---

archive/issue_events_196747.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15081",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15081#event-196747"
}
```



---

archive/issue_events_196748.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15081",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15081#event-196748"
}
```



---

archive/issue_comments_187723.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nSee also #16533 and its followups.",
    "created_at": "2014-06-30T13:53:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15081",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15081#issuecomment-187723",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:4" align="right">Comment 4</div>

See also #16533 and its followups.



---

archive/issue_comments_187724.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nOr, is this possibly fixed in the doc fixes for #12827?",
    "created_at": "2014-06-30T13:55:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15081",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15081#issuecomment-187724",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:5" align="right">Comment 5</div>

Or, is this possibly fixed in the doc fixes for #12827?



---

archive/issue_events_196749.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2014-07-02T20:55:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15081",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15081#event-196749"
}
```



---

archive/issue_comments_187725.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nI think this has already been fixed. According to the `search_src` function, \"loop_output\" only appears in the file animate.py, and in that file it only appears in comments.",
    "created_at": "2014-07-02T20:55:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15081",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15081#issuecomment-187725",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:6" align="right">Comment 6</div>

I think this has already been fixed. According to the `search_src` function, "loop_output" only appears in the file animate.py, and in that file it only appears in comments.



---

archive/issue_events_196750.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2014-07-02T20:56:10Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15081",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15081#event-196750"
}
```



---

archive/issue_events_196751.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2014-07-02T20:56:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15081",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15081#event-196751"
}
```



---

archive/issue_events_196752.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2014-07-02T20:56:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15081",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15081#event-196752"
}
```



---

archive/issue_events_196753.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2014-07-02T20:56:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15081",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15081#event-196753"
}
```



---

archive/issue_events_196754.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2014-07-02T20:56:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15081",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15081#event-196754"
}
```



---

archive/issue_events_196755.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-07-03T02:04:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15081",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15081#event-196755"
}
```



---

archive/issue_events_196756.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-07-03T02:04:42Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15081",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15081#event-196756"
}
```
