# Issue 15451: symbolic.limit wrongly gives ZeroDivisionError

archive/issues_015214.json:
```json
{
    "assignees": [],
    "body": "We have\n\n```\nsage: (4-x^2)/(3-sqrt(x^2+5)).limit(x=2)\nTraceback (most recent call last):\n...\nZeroDivisionError: Symbolic division by zero\n```\nbut\n\n```\nsage: limit((4-x^2)/(3-sqrt(x^2+5)), x=2)\n6\n```\nThose two should give the same result (at least the docstring of `x.limit` says `See \"sage.calculus.calculus.limit\"`, so it has the appearence that the same functions are called).\n\n\nCC:  @kcrisman\n\nKeywords: **symbolic, limit, wrong, zero division**\n\nReviewer: **Daniel Krenn, Akshay Ajagekar**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15451_\n\n",
    "closed_at": "2016-03-26T12:02:31Z",
    "created_at": "2013-11-25T15:17:10Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "symbolic.limit wrongly gives ZeroDivisionError",
    "type": "issue",
    "updated_at": "2016-03-26T12:02:31Z",
    "url": "https://github.com/sagemath/sage/issues/15451",
    "user": "https://github.com/dkrenn"
}
```
We have

```
sage: (4-x^2)/(3-sqrt(x^2+5)).limit(x=2)
Traceback (most recent call last):
...
ZeroDivisionError: Symbolic division by zero
```
but

```
sage: limit((4-x^2)/(3-sqrt(x^2+5)), x=2)
6
```
Those two should give the same result (at least the docstring of `x.limit` says `See "sage.calculus.calculus.limit"`, so it has the appearence that the same functions are called).


CC:  @kcrisman

Keywords: **symbolic, limit, wrong, zero division**

Reviewer: **Daniel Krenn, Akshay Ajagekar**

_Issue created by migration from https://trac.sagemath.org/ticket/15451_





---

archive/issue_events_202627.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2013-11-25T15:17:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15451",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15451#event-202627"
}
```



---

archive/issue_events_202628.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2013-11-25T15:17:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15451",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15451#event-202628"
}
```



---

archive/issue_events_202629.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2013-11-25T15:17:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15451",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15451#event-202629"
}
```



---

archive/issue_events_202630.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2013-11-25T15:17:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15451",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15451#event-202630"
}
```



---

archive/issue_events_202631.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15451",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15451#event-202631"
}
```



---

archive/issue_events_202632.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15451",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15451#event-202632"
}
```



---

archive/issue_events_202633.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15451",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15451#event-202633"
}
```



---

archive/issue_events_202634.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15451",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15451#event-202634"
}
```



---

archive/issue_events_202635.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15451",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15451#event-202635"
}
```



---

archive/issue_events_202636.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15451",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15451#event-202636"
}
```



---

archive/issue_comments_194881.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nStill there in 6.6.",
    "created_at": "2015-04-16T20:15:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15451",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15451#issuecomment-194881",
    "user": "https://github.com/dkrenn"
}
```

<div id="comment:5" align="right">Comment 5</div>

Still there in 6.6.



---

archive/issue_comments_194882.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nI don't understand the issue. In case of \n\n```\nsage: (4-x^2)/(3-sqrt(x^2+5)).limit(x=2)\n```\n`4-x^2` gets divided by ` (3-sqrt(x^2+5)).limit(x=2)` which is 0, so the ZeroDivisionError. But this works\n\n```\nsage: ((4-x^2)/(3-sqrt(x^2+5))).limit(x=2)\n6\n```",
    "created_at": "2016-03-05T13:11:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15451",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15451#issuecomment-194882",
    "user": "https://github.com/sagetrac-ajagekar-akshay"
}
```

<div id="comment:6" align="right">Comment 6</div>

I don't understand the issue. In case of 

```
sage: (4-x^2)/(3-sqrt(x^2+5)).limit(x=2)
```
`4-x^2` gets divided by ` (3-sqrt(x^2+5)).limit(x=2)` which is 0, so the ZeroDivisionError. But this works

```
sage: ((4-x^2)/(3-sqrt(x^2+5))).limit(x=2)
6
```



---

archive/issue_comments_194883.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nWell, it's similar to `(sin(x)/x).limit(x=0)`, isn't it?\n\nhttps://en.wikipedia.org/wiki/Indeterminate_form",
    "created_at": "2016-03-05T15:43:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15451",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15451#issuecomment-194883",
    "user": "https://github.com/rwst"
}
```

<div id="comment:7" align="right">Comment 7</div>

Well, it's similar to `(sin(x)/x).limit(x=0)`, isn't it?

https://en.wikipedia.org/wiki/Indeterminate_form



---

archive/issue_comments_194884.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nThe expression of which limit is evaluated in this case is ` (3-sqrt(x^2+5))` at x=2 and not the whole expression ` (4-x<sup>2)/(3-sqrt(x</sup>2+5))` as for the second case ` limit((4-x<sup>2)/(3-sqrt(x</sup>2+5)), x=2)` limit is evaluated for whole expression. proper parenthesis would give correct answer.",
    "created_at": "2016-03-05T16:11:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15451",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15451#issuecomment-194884",
    "user": "https://github.com/sagetrac-ajagekar-akshay"
}
```

<div id="comment:8" align="right">Comment 8</div>

The expression of which limit is evaluated in this case is ` (3-sqrt(x^2+5))` at x=2 and not the whole expression ` (4-x<sup>2)/(3-sqrt(x</sup>2+5))` as for the second case ` limit((4-x<sup>2)/(3-sqrt(x</sup>2+5)), x=2)` limit is evaluated for whole expression. proper parenthesis would give correct answer.



---

archive/issue_events_202637.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2016-03-05T16:22:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15451",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15451#event-202637"
}
```



---

archive/issue_events_202638.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2016-03-05T16:22:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15451",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15451#event-202638"
}
```



---

archive/issue_events_202639.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2016-03-05T16:22:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15451",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15451#event-202639"
}
```



---

archive/issue_comments_194885.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nSo you agree it's user error?",
    "created_at": "2016-03-05T16:22:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15451",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15451#issuecomment-194885",
    "user": "https://github.com/rwst"
}
```

<div id="comment:9" align="right">Comment 9</div>

So you agree it's user error?



---

archive/issue_events_202640.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2016-03-05T16:22:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15451",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15451#event-202640"
}
```



---

archive/issue_comments_194886.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nReplying to [@rwst](#comment%3A9):\n> So you agree it's user error?\n\nYes.",
    "created_at": "2016-03-05T16:34:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15451",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15451#issuecomment-194886",
    "user": "https://github.com/sagetrac-ajagekar-akshay"
}
```

<div id="comment:10" align="right">Comment 10</div>

Replying to [@rwst](#comment%3A9):
> So you agree it's user error?

Yes.



---

archive/issue_comments_194887.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nI agree as well that it is a user error (sorry, I didn't see this (the wrong parentheses) when opening the ticket).\n\nFeel free to insert your name(s) in the author field.",
    "created_at": "2016-03-05T16:37:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15451",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15451#issuecomment-194887",
    "user": "https://github.com/dkrenn"
}
```

<div id="comment:11" align="right">Comment 11</div>

I agree as well that it is a user error (sorry, I didn't see this (the wrong parentheses) when opening the ticket).

Feel free to insert your name(s) in the author field.



---

archive/issue_comments_194888.json:
```json
{
    "body": "Reviewer: **Daniel Krenn**",
    "created_at": "2016-03-05T16:37:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15451",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15451#issuecomment-194888",
    "user": "https://github.com/dkrenn"
}
```

Reviewer: **Daniel Krenn**



---

archive/issue_events_202641.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2016-03-05T16:37:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15451",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15451#event-202641"
}
```



---

archive/issue_events_202642.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2016-03-05T16:37:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15451",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15451#event-202642"
}
```



---

archive/issue_comments_194889.json:
```json
{
    "body": "Author: **Akshay Ajagekar**",
    "created_at": "2016-03-05T16:39:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15451",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15451#issuecomment-194889",
    "user": "https://github.com/sagetrac-ajagekar-akshay"
}
```

Author: **Akshay Ajagekar**



---

archive/issue_comments_194890.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nActually, there are no authors if there is no patch, but definitely this is a contribution so I put it in reviewers.",
    "created_at": "2016-03-07T19:01:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15451",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15451#issuecomment-194890",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:13" align="right">Comment 13</div>

Actually, there are no authors if there is no patch, but definitely this is a contribution so I put it in reviewers.



---

archive/issue_comments_194891.json:
```json
{
    "body": "Changed reviewer from **Daniel Krenn** to **Daniel Krenn, Akshay Ajagekar**",
    "created_at": "2016-03-07T19:01:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15451",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15451#issuecomment-194891",
    "user": "https://github.com/kcrisman"
}
```

Changed reviewer from **Daniel Krenn** to **Daniel Krenn, Akshay Ajagekar**



---

archive/issue_comments_194892.json:
```json
{
    "body": "Changed author from **Akshay Ajagekar** to none",
    "created_at": "2016-03-07T19:01:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15451",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15451#issuecomment-194892",
    "user": "https://github.com/kcrisman"
}
```

Changed author from **Akshay Ajagekar** to none



---

archive/issue_events_202643.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-03-26T12:02:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15451",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15451#event-202643"
}
```



---

archive/issue_events_202644.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-03-26T12:02:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15451",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15451#event-202644"
}
```
