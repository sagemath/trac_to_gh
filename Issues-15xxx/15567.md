...
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\n[@Vincent](#comment:6): I've (unintentionally) made that work with this branch, but I will keep the `names` argument around.\n\n[@Nathann](#comment:5): I've made it so that `names` and `data` switch if necessary, as well as addressed some of your other concerns. I'm not going to fix the slices here (but I do agree it should be fixed, I'll post a fix to that as soon as I can). There's more detail and examples about the hidden arguments for `Family` further down in it's documentation. I've expanded upon the `name` argument documentation for `Family`.",
