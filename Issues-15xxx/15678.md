# Issue 15678: R fails to build on Cygwin64 passing --large-address-aware to ld.

archive/issues_015441.json:
```json
{
    "assignees": [],
    "body": "This is triggered on cygwin* but makes ld fail on cygwin64.\n\nCC:  @kcrisman @dimpase @tscrim @vbraun\n\nKeywords: **r cygwin**\n\nBranch/Commit: **[7015c53](https://github.com/sagemath/sagetrac-mirror/commit/7015c53f0d853e8708ae0f4c186cddc9920a2f5b)**\n\nUpstream: **Reported upstream. Developers deny it's a bug.**\n\nReviewer: **Volker Braun**\n\nAuthor: **Jean-Pierre Flori**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15678_\n\n",
    "closed_at": "2014-04-15T23:19:34Z",
    "created_at": "2014-01-15T10:02:00Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "R fails to build on Cygwin64 passing --large-address-aware to ld.",
    "type": "issue",
    "updated_at": "2014-04-15T23:19:34Z",
    "url": "https://github.com/sagemath/sage/issues/15678",
    "user": "https://github.com/sagetrac-jpflori"
}
```
This is triggered on cygwin* but makes ld fail on cygwin64.

CC:  @kcrisman @dimpase @tscrim @vbraun

Keywords: **r cygwin**

Branch/Commit: **[7015c53](https://github.com/sagemath/sagetrac-mirror/commit/7015c53f0d853e8708ae0f4c186cddc9920a2f5b)**

Upstream: **Reported upstream. Developers deny it's a bug.**

Reviewer: **Volker Braun**

Author: **Jean-Pierre Flori**

_Issue created by migration from https://trac.sagemath.org/ticket/15678_





---

archive/issue_events_206254.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2014-01-15T10:02:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15678",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15678#event-206254"
}
```



---

archive/issue_events_206255.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2014-01-15T10:02:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15678",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
    "label_color": "000080",
    "label_name": "component: porting: cygwin",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15678#event-206255"
}
```



---

archive/issue_events_206256.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2014-01-15T10:02:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15678",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15678#event-206256"
}
```



---

archive/issue_events_206257.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2014-01-15T10:02:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15678",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15678#event-206257"
}
```



---

archive/issue_comments_199700.json:
```json
{
    "body": "Upstream: **Reported upstream. No feedback yet.**",
    "created_at": "2014-01-18T18:01:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15678",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15678#issuecomment-199700",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Upstream: **Reported upstream. No feedback yet.**



---

archive/issue_comments_199701.json:
```json
{
    "body": "Author: **Jean-Pierre Flori**",
    "created_at": "2014-01-18T18:01:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15678",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15678#issuecomment-199701",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Author: **Jean-Pierre Flori**



---

archive/issue_comments_199702.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nPatch to configure.ac and configure (not automagically autoreconf, but made by hand, as the former solution produces far too much).",
    "created_at": "2014-01-18T18:01:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15678",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15678#issuecomment-199702",
    "user": "https://github.com/sagetrac-jpflori"
}
```

<div id="comment:1" align="right">Comment 1</div>

Patch to configure.ac and configure (not automagically autoreconf, but made by hand, as the former solution produces far too much).



---

archive/issue_comments_199703.json:
```json
{
    "body": "Branch: **[u/jpflori/ticket/15678](https://github.com/sagemath/sagetrac-mirror/tree/u/jpflori/ticket/15678)**",
    "created_at": "2014-01-18T18:01:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15678",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15678#issuecomment-199703",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Branch: **[u/jpflori/ticket/15678](https://github.com/sagemath/sagetrac-mirror/tree/u/jpflori/ticket/15678)**



---

archive/issue_comments_199704.json:
```json
{
    "body": "Changed keywords from **** to **r cygwin**",
    "created_at": "2014-01-18T18:01:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15678",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15678#issuecomment-199704",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Changed keywords from **** to **r cygwin**



---

archive/issue_events_206258.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2014-01-18T18:01:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15678",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15678#event-206258"
}
```



---

archive/issue_comments_199705.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nUpstream report at:\n* https://bugs.r-project.org/bugzilla3/show_bug.cgi?id=15643",
    "created_at": "2014-01-18T18:01:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15678",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15678#issuecomment-199705",
    "user": "https://github.com/sagetrac-jpflori"
}
```

<div id="comment:2" align="right">Comment 2</div>

Upstream report at:
* https://bugs.r-project.org/bugzilla3/show_bug.cgi?id=15643



---

archive/issue_comments_199706.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7015c53f0d853e8708ae0f4c186cddc9920a2f5b\">7015c53</a></td><td><code>Don't pass large-address-aware flag to ld on Cygwin64.</code></td></tr></table>\n",
    "created_at": "2014-01-18T18:04:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15678",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15678#issuecomment-199706",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7015c53f0d853e8708ae0f4c186cddc9920a2f5b">7015c53</a></td><td><code>Don't pass large-address-aware flag to ld on Cygwin64.</code></td></tr></table>




---

archive/issue_comments_199707.json:
```json
{
    "body": "Commit: **[7015c53](https://github.com/sagemath/sagetrac-mirror/commit/7015c53f0d853e8708ae0f4c186cddc9920a2f5b)**",
    "created_at": "2014-01-18T18:04:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15678",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15678#issuecomment-199707",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[7015c53](https://github.com/sagemath/sagetrac-mirror/commit/7015c53f0d853e8708ae0f4c186cddc9920a2f5b)**



---

archive/issue_comments_199708.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nDoing my homework.",
    "created_at": "2014-01-18T19:44:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15678",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15678#issuecomment-199708",
    "user": "https://github.com/sagetrac-jpflori"
}
```

<div id="comment:4" align="right">Comment 4</div>

Doing my homework.



---

archive/issue_comments_199709.json:
```json
{
    "body": "Changed upstream from **Reported upstream. No feedback yet.** to **Reported upstream. Developers deny it's a bug.**",
    "created_at": "2014-01-18T19:44:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15678",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15678#issuecomment-199709",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Changed upstream from **Reported upstream. No feedback yet.** to **Reported upstream. Developers deny it's a bug.**



---

archive/issue_events_206259.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15678",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15678#event-206259"
}
```



---

archive/issue_events_206260.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15678",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15678#event-206260"
}
```



---

archive/issue_comments_199710.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\n> Doing my homework.\n\nIf the word \"cygwin\" occurs anywhere in their source code (and the configure has that whole section), they have no right to treat you like this.  Vewy wude.   And why wouldn't they want to support Cygwin?\n\nAlso, I don't understand why they don't have a web-browsable source.  Or actually, [they do](https://github.com/wch/r-source), but it's not apparently linked from the page...",
    "created_at": "2014-03-18T15:26:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15678",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15678#issuecomment-199710",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:6" align="right">Comment 6</div>

> Doing my homework.

If the word "cygwin" occurs anywhere in their source code (and the configure has that whole section), they have no right to treat you like this.  Vewy wude.   And why wouldn't they want to support Cygwin?

Also, I don't understand why they don't have a web-browsable source.  Or actually, [they do](https://github.com/wch/r-source), but it's not apparently linked from the page...



---

archive/issue_comments_199711.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nSure, that was rude.\nI don't plan on fighting for that.",
    "created_at": "2014-03-18T15:28:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15678",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15678#issuecomment-199711",
    "user": "https://github.com/sagetrac-jpflori"
}
```

<div id="comment:7" align="right">Comment 7</div>

Sure, that was rude.
I don't plan on fighting for that.



---

archive/issue_comments_199712.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nVolker: in case you're in a reviewing mood, this one is awfully simple...",
    "created_at": "2014-04-15T08:52:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15678",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15678#issuecomment-199712",
    "user": "https://github.com/sagetrac-jpflori"
}
```

<div id="comment:8" align="right">Comment 8</div>

Volker: in case you're in a reviewing mood, this one is awfully simple...



---

archive/issue_comments_199713.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2014-04-15T10:10:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15678",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15678#issuecomment-199713",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_comments_199714.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nyou found the Grinch on the R bugzilla ;-)",
    "created_at": "2014-04-15T10:21:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15678",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15678#issuecomment-199714",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:10" align="right">Comment 10</div>

you found the Grinch on the R bugzilla ;-)



---

archive/issue_comments_199715.json:
```json
{
    "body": "Changed branch from **[u/jpflori/ticket/15678](https://github.com/sagemath/sagetrac-mirror/tree/u/jpflori/ticket/15678)** to **[7015c53](https://github.com/sagemath/sagetrac-mirror/commit/7015c53f0d853e8708ae0f4c186cddc9920a2f5b)**",
    "created_at": "2014-04-15T23:19:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15678",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15678#issuecomment-199715",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jpflori/ticket/15678](https://github.com/sagemath/sagetrac-mirror/tree/u/jpflori/ticket/15678)** to **[7015c53](https://github.com/sagemath/sagetrac-mirror/commit/7015c53f0d853e8708ae0f4c186cddc9920a2f5b)**



---

archive/issue_events_206261.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-04-15T23:19:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15678",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15678#event-206261"
}
```



---

archive/issue_events_206262.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "02a0f73528656366c6030844a1f79d788b170b84",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-04-15T23:19:34Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15678",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15678#event-206262"
}
```
