# Issue 15238: Wrapper around Pari's matsolvemod to solve systems of linear congruences

archive/issues_015001.json:
```json
{
    "assignees": [],
    "body": "Add a wrapper around Pari's matsolvemod. This returns a small solution to a system of linear congruences of the form A*X = B mod D, where A is an integral matrix, B is an integral vector and D is either a nonnegative integral vector or a nonnegative integer. \n\nCC:  @pjbruin\n\nBranch/Commit: **[u/smueller/ticket/15238](https://github.com/sagemath/sagetrac-mirror/tree/u/smueller/ticket/15238) @ [fbf86ec](https://github.com/sagemath/sagetrac-mirror/commit/fbf86ecf3c040d6414f1b5afb4d367b521695dd9)**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15238_\n\n",
    "created_at": "2013-09-28T16:11:49Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Wrapper around Pari's matsolvemod to solve systems of linear congruences",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/15238",
    "user": "https://github.com/sagetrac-smueller"
}
```
Add a wrapper around Pari's matsolvemod. This returns a small solution to a system of linear congruences of the form A*X = B mod D, where A is an integral matrix, B is an integral vector and D is either a nonnegative integral vector or a nonnegative integer. 

CC:  @pjbruin

Branch/Commit: **[u/smueller/ticket/15238](https://github.com/sagemath/sagetrac-mirror/tree/u/smueller/ticket/15238) @ [fbf86ec](https://github.com/sagemath/sagetrac-mirror/commit/fbf86ecf3c040d6414f1b5afb4d367b521695dd9)**

_Issue created by migration from https://trac.sagemath.org/ticket/15238_





---

archive/issue_events_199223.json:
```json
{
    "actor": "https://github.com/sagetrac-smueller",
    "created_at": "2013-09-28T16:11:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "milestone_number": null,
    "milestone_title": "sage-5.13",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15238#event-199223"
}
```



---

archive/issue_events_199224.json:
```json
{
    "actor": "https://github.com/sagetrac-smueller",
    "created_at": "2013-09-28T16:11:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "component: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15238#event-199224"
}
```



---

archive/issue_events_199225.json:
```json
{
    "actor": "https://github.com/sagetrac-smueller",
    "created_at": "2013-09-28T16:11:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15238#event-199225"
}
```



---

archive/issue_events_199226.json:
```json
{
    "actor": "https://github.com/sagetrac-smueller",
    "created_at": "2013-09-28T16:11:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15238#event-199226"
}
```



---

archive/issue_comments_190393.json:
```json
{
    "body": "Branch: **[u/smueller/ticket/15238](https://github.com/sagemath/sagetrac-mirror/tree/u/smueller/ticket/15238)**",
    "created_at": "2013-09-28T16:13:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15238#issuecomment-190393",
    "user": "https://github.com/sagetrac-smueller"
}
```

Branch: **[u/smueller/ticket/15238](https://github.com/sagemath/sagetrac-mirror/tree/u/smueller/ticket/15238)**



---

archive/issue_comments_190394.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td>[changeset:fbf86ec]</td><td><code>changed doctests</code></td></tr></table>\n",
    "created_at": "2013-09-28T16:50:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15238#issuecomment-190394",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td>[changeset:fbf86ec]</td><td><code>changed doctests</code></td></tr></table>




---

archive/issue_comments_190395.json:
```json
{
    "body": "Commit: **[fbf86ec](https://github.com/sagemath/sagetrac-mirror/commit/fbf86ecf3c040d6414f1b5afb4d367b521695dd9)**",
    "created_at": "2013-09-28T16:50:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15238#issuecomment-190395",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[fbf86ec](https://github.com/sagemath/sagetrac-mirror/commit/fbf86ecf3c040d6414f1b5afb4d367b521695dd9)**



---

archive/issue_comments_190396.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-ADD DESCRIPTION\n+Add a wrapper around Pari's matsolvemod. This returns a small solution to a system of linear congruences of the form A*X = B mod D, where A is an integral matrix, B is an integral vector and D is either a nonnegative integral vector or a nonnegative integer. \n``````\n",
    "created_at": "2013-09-29T09:58:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15238#issuecomment-190396",
    "user": "https://github.com/sagetrac-smueller"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-ADD DESCRIPTION
+Add a wrapper around Pari's matsolvemod. This returns a small solution to a system of linear congruences of the form A*X = B mod D, where A is an integral matrix, B is an integral vector and D is either a nonnegative integral vector or a nonnegative integer. 
``````




---

archive/issue_events_199227.json:
```json
{
    "actor": "https://github.com/sagetrac-smueller",
    "created_at": "2013-09-29T09:58:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15238#event-199227"
}
```



---

archive/issue_events_199228.json:
```json
{
    "actor": "https://github.com/sagetrac-smueller",
    "created_at": "2013-09-29T09:58:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15238#event-199228"
}
```



---

archive/issue_events_199229.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-03T10:21:08Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "milestone_number": null,
    "milestone_title": "sage-5.13",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15238#event-199229"
}
```



---

archive/issue_events_199230.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-03T10:21:08Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "milestone_number": null,
    "milestone_title": "sage-6.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15238#event-199230"
}
```



---

archive/issue_events_199231.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2013-12-17T18:39:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "milestone_number": null,
    "milestone_title": "sage-6.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15238#event-199231"
}
```



---

archive/issue_events_199232.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2013-12-17T18:39:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15238#event-199232"
}
```



---

archive/issue_events_199233.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15238#event-199233"
}
```



---

archive/issue_events_199234.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15238#event-199234"
}
```



---

archive/issue_events_199235.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15238#event-199235"
}
```



---

archive/issue_events_199236.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15238#event-199236"
}
```



---

archive/issue_events_199237.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15238#event-199237"
}
```



---

archive/issue_events_199238.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15238#event-199238"
}
```



---

archive/issue_events_199239.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15238",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15238#event-199239"
}
```
