# Issue 15182: duplicate declaration of stackdummy() in sage/libs/pari/decl.pxi

archive/issues_014945.json:
```json
{
    "assignees": [],
    "body": "After #15125, the file `sage/libs/pari/decl.pxi` contains two declarations of the function `stackdummy()`.  The second one is obsolete.\n\n\nKeywords: **pari**\n\nReviewer: **Peter Bruin, Jeroen Demeyer**\n\nComponent: **packages: standard**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15182_\n\n",
    "closed_at": "2013-10-05T09:38:29Z",
    "created_at": "2013-09-09T17:33:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "duplicate declaration of stackdummy() in sage/libs/pari/decl.pxi",
    "type": "issue",
    "updated_at": "2013-10-05T09:38:29Z",
    "url": "https://github.com/sagemath/sage/issues/15182",
    "user": "https://github.com/pjbruin"
}
```
After #15125, the file `sage/libs/pari/decl.pxi` contains two declarations of the function `stackdummy()`.  The second one is obsolete.


Keywords: **pari**

Reviewer: **Peter Bruin, Jeroen Demeyer**

Component: **packages: standard**

_Issue created by migration from https://trac.sagemath.org/ticket/15182_





---

archive/issue_events_215723.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2013-09-09T17:33:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15182#event-215723"
}
```



---

archive/issue_events_215724.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2013-09-09T17:33:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15182#event-215724"
}
```



---

archive/issue_events_215725.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2013-09-09T17:33:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "label": "https://github.com/sagemath/sage/labels/p%3A%205%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p: 5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15182#event-215725"
}
```



---

archive/issue_events_215726.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2013-09-09T17:33:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15182#event-215726"
}
```



---

archive/issue_comments_186918.json:
```json
{
    "body": "Attachment: **[15182-decl_stackdummy.patch.gz](https://github.com/sagemath/sage/files/ticket15182/15182-decl_stackdummy.patch.gz)**\n\nbased on 5.12.beta4 + #15124 + #15125",
    "created_at": "2013-09-09T17:35:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15182#issuecomment-186918",
    "user": "https://github.com/pjbruin"
}
```

Attachment: **[15182-decl_stackdummy.patch.gz](https://github.com/sagemath/sage/files/ticket15182/15182-decl_stackdummy.patch.gz)**

based on 5.12.beta4 + #15124 + #15125



---

archive/issue_events_215727.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2013-09-09T17:36:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15182#event-215727"
}
```



---

archive/issue_comments_186919.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2013-10-02T09:32:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15182#issuecomment-186919",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_comments_186920.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nI made the same change at #15243. If that gets merged soon, I will close this ticket as duplicate.",
    "created_at": "2013-10-02T09:32:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15182#issuecomment-186920",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">comment:2</div>

I made the same change at #15243. If that gets merged soon, I will close this ticket as duplicate.



---

archive/issue_events_215728.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-02T09:32:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15182#event-215728"
}
```



---

archive/issue_events_215729.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-02T09:32:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15182#event-215729"
}
```



---

archive/issue_events_215730.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-02T09:32:45Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15182#event-215730"
}
```



---

archive/issue_events_215731.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-02T09:32:45Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "milestone_number": null,
    "milestone_title": "sage-5.13",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15182#event-215731"
}
```



---

archive/issue_comments_186921.json:
```json
{
    "body": "Changed reviewer from **Jeroen Demeyer** to **Peter Bruin, Jeroen Demeyer**",
    "created_at": "2013-10-02T12:02:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15182#issuecomment-186921",
    "user": "https://github.com/jdemeyer"
}
```

Changed reviewer from **Jeroen Demeyer** to **Peter Bruin, Jeroen Demeyer**



---

archive/issue_events_215732.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-02T12:02:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "milestone_number": null,
    "milestone_title": "sage-5.13",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15182#event-215732"
}
```



---

archive/issue_events_215733.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-02T12:02:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "label": "https://github.com/sagemath/sage/labels/p%3A%205%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p: 5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15182#event-215733"
}
```



---

archive/issue_events_215734.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-02T12:02:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15182#event-215734"
}
```



---

archive/issue_comments_186922.json:
```json
{
    "body": "Changed author from **Peter Bruin** to none",
    "created_at": "2013-10-02T12:02:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15182#issuecomment-186922",
    "user": "https://github.com/jdemeyer"
}
```

Changed author from **Peter Bruin** to none



---

archive/issue_events_215735.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-05T09:38:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15182#event-215735"
}
```



---

archive/issue_events_215736.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-05T09:38:29Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15182#event-215736"
}
```



---

archive/issue_events_215737.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-05T09:38:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15182#event-215737"
}
```
