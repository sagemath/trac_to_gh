# Issue 15182: duplicate declaration of stackdummy() in sage/libs/pari/decl.pxi

archive/issues_014945.json:
```json
{
    "assignees": [],
    "body": "After #15125, the file `sage/libs/pari/decl.pxi` contains two declarations of the function `stackdummy()`.  The second one is obsolete.\n\n\nKeywords: **pari**\n\nReviewer: **Peter Bruin, Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15182_\n\n",
    "closed_at": "2013-10-05T09:38:29Z",
    "created_at": "2013-09-09T17:33:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "duplicate declaration of stackdummy() in sage/libs/pari/decl.pxi",
    "type": "issue",
    "updated_at": "2013-10-05T09:38:29Z",
    "url": "https://github.com/sagemath/sage/issues/15182",
    "user": "https://github.com/pjbruin"
}
```
After #15125, the file `sage/libs/pari/decl.pxi` contains two declarations of the function `stackdummy()`.  The second one is obsolete.


Keywords: **pari**

Reviewer: **Peter Bruin, Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/15182_





---

archive/issue_events_198301.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2013-09-09T17:33:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15182#event-198301"
}
```



---

archive/issue_events_198302.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2013-09-09T17:33:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15182#event-198302"
}
```



---

archive/issue_events_198303.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2013-09-09T17:33:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15182#event-198303"
}
```



---

archive/issue_events_198304.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2013-09-09T17:33:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15182#event-198304"
}
```



---

archive/issue_comments_189343.json:
```json
{
    "body": "Attachment: **[15182-decl_stackdummy.patch.gz](https://github.com/sagemath/sage/files/ticket15182/15182-decl_stackdummy.patch.gz)**\n\nbased on 5.12.beta4 + #15124 + #15125",
    "created_at": "2013-09-09T17:35:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15182#issuecomment-189343",
    "user": "https://github.com/pjbruin"
}
```

Attachment: **[15182-decl_stackdummy.patch.gz](https://github.com/sagemath/sage/files/ticket15182/15182-decl_stackdummy.patch.gz)**

based on 5.12.beta4 + #15124 + #15125



---

archive/issue_events_198305.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2013-09-09T17:36:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15182#event-198305"
}
```



---

archive/issue_comments_189344.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2013-10-02T09:32:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15182#issuecomment-189344",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_comments_189345.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nI made the same change at #15243. If that gets merged soon, I will close this ticket as duplicate.",
    "created_at": "2013-10-02T09:32:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15182#issuecomment-189345",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

I made the same change at #15243. If that gets merged soon, I will close this ticket as duplicate.



---

archive/issue_events_198306.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-02T09:32:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15182#event-198306"
}
```



---

archive/issue_events_198307.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-02T09:32:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15182#event-198307"
}
```



---

archive/issue_events_198308.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-02T09:32:45Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15182#event-198308"
}
```



---

archive/issue_events_198309.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-02T09:32:45Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "milestone_number": null,
    "milestone_title": "sage-5.13",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15182#event-198309"
}
```



---

archive/issue_comments_189346.json:
```json
{
    "body": "Changed reviewer from **Jeroen Demeyer** to **Peter Bruin, Jeroen Demeyer**",
    "created_at": "2013-10-02T12:02:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15182#issuecomment-189346",
    "user": "https://github.com/jdemeyer"
}
```

Changed reviewer from **Jeroen Demeyer** to **Peter Bruin, Jeroen Demeyer**



---

archive/issue_events_198310.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-02T12:02:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "milestone_number": null,
    "milestone_title": "sage-5.13",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15182#event-198310"
}
```



---

archive/issue_events_198311.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-02T12:02:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15182#event-198311"
}
```



---

archive/issue_events_198312.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-02T12:02:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15182#event-198312"
}
```



---

archive/issue_comments_189347.json:
```json
{
    "body": "Changed author from **Peter Bruin** to none",
    "created_at": "2013-10-02T12:02:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15182#issuecomment-189347",
    "user": "https://github.com/jdemeyer"
}
```

Changed author from **Peter Bruin** to none



---

archive/issue_events_198313.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-05T09:38:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15182#event-198313"
}
```



---

archive/issue_events_198314.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-05T09:38:29Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15182#event-198314"
}
```



---

archive/issue_events_198315.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-05T09:38:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15182",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15182#event-198315"
}
```
