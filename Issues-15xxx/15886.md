# Issue 15886: Adding a list setter in ClonableArray and ClonableList

archive/issues_015886.json:
```json
{
    "body": "Assignee: patxiku\n\nCC:  @hivert\n\nKeywords: setter ClonableList, days57\n\nWhen a ClonableList instance is in mutable mode.\nWe need to change the list of the instance.\nThat's why we need to implement a list setter. \n\nThe same thing occurs for a ClonableArray.\n\nIssue created by migration from https://trac.sagemath.org/ticket/16123\n\n",
    "created_at": "2014-04-10T16:31:19Z",
    "labels": [
        "component: combinatorics",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.4",
    "title": "Adding a list setter in ClonableArray and ClonableList",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/15886",
    "user": "https://trac.sagemath.org/admin/accounts/users/patxiku"
}
```
Assignee: patxiku

CC:  @hivert

Keywords: setter ClonableList, days57

When a ClonableList instance is in mutable mode.
We need to change the list of the instance.
That's why we need to implement a list setter. 

The same thing occurs for a ClonableArray.

Issue created by migration from https://trac.sagemath.org/ticket/16123





---

archive/issue_comments_206054.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2014-04-10T18:13:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15886",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15886#issuecomment-206054",
    "user": "https://trac.sagemath.org/admin/accounts/users/patxiku"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_206055.json:
```json
{
    "body": "New commits:",
    "created_at": "2014-04-10T18:13:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15886",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15886#issuecomment-206055",
    "user": "https://trac.sagemath.org/admin/accounts/users/patxiku"
}
```

New commits:



---

archive/issue_comments_206056.json:
```json
{
    "body": "Changing keywords from \"setter ClonableList\" to \"setter ClonableList, days57\".",
    "created_at": "2014-04-10T23:36:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15886",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15886#issuecomment-206056",
    "user": "https://trac.sagemath.org/admin/accounts/users/boussica"
}
```

Changing keywords from "setter ClonableList" to "setter ClonableList, days57".



---

archive/issue_events_047510.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15886",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15886#event-47510"
}
```



---

archive/issue_events_047511.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15886",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15886#event-47511"
}
```



---

archive/issue_events_047512.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15886",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15886#event-47512"
}
```



---

archive/issue_comments_206057.json:
```json
{
    "body": "Hi,\n\nThere is already a `._set_list` method (with an underscore). Why do you need this `.set_list`?\n\nVincent",
    "created_at": "2014-10-27T18:14:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15886",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15886#issuecomment-206057",
    "user": "https://github.com/videlec"
}
```

Hi,

There is already a `._set_list` method (with an underscore). Why do you need this `.set_list`?

Vincent



---

archive/issue_comments_206058.json:
```json
{
    "body": "Changing status from needs_review to needs_info.",
    "created_at": "2014-10-27T18:14:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15886",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15886#issuecomment-206058",
    "user": "https://github.com/videlec"
}
```

Changing status from needs_review to needs_info.
