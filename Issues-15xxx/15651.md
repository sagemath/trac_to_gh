# Issue 15651: Let MPIR build correctly on Cygwin64

archive/issues_015414.json:
```json
{
    "assignees": [],
    "body": "We have to modify the ABI tricks in our spkg-instal script, but also patch MPIR so that it uses correct assembly (see upstream commits at https://github.com/wbhart/mpir/commits/master/configure.ac).\n\n\nDepends on #15365\nDepends on #14648\n\nCC:  @kcrisman @dimpase @tscrim\n\nKeywords: **cygwin spkg mpir**\n\nBranch/Commit: **[u/jpflori/ticket/15651](https://github.com/sagemath/sagetrac-mirror/tree/u/jpflori/ticket/15651) @ [0286624](https://github.com/sagemath/sagetrac-mirror/commit/02866244b8b7ec0d9abb478ff9ba4a7a8b39cbed)**\n\nUpstream: **Fixed upstream, but not in a stable release.**\n\nReviewer: **Volker Braun**\n\nAuthor: **Jean-Pierre Flori**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15651_\n\n",
    "closed_at": "2014-10-25T21:46:59Z",
    "created_at": "2014-01-09T08:53:51Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Let MPIR build correctly on Cygwin64",
    "type": "issue",
    "updated_at": "2014-10-25T21:46:59Z",
    "url": "https://github.com/sagemath/sage/issues/15651",
    "user": "https://github.com/sagetrac-jpflori"
}
```
We have to modify the ABI tricks in our spkg-instal script, but also patch MPIR so that it uses correct assembly (see upstream commits at https://github.com/wbhart/mpir/commits/master/configure.ac).


Depends on #15365
Depends on #14648

CC:  @kcrisman @dimpase @tscrim

Keywords: **cygwin spkg mpir**

Branch/Commit: **[u/jpflori/ticket/15651](https://github.com/sagemath/sagetrac-mirror/tree/u/jpflori/ticket/15651) @ [0286624](https://github.com/sagemath/sagetrac-mirror/commit/02866244b8b7ec0d9abb478ff9ba4a7a8b39cbed)**

Upstream: **Fixed upstream, but not in a stable release.**

Reviewer: **Volker Braun**

Author: **Jean-Pierre Flori**

_Issue created by migration from https://trac.sagemath.org/ticket/15651_





---

archive/issue_events_205828.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2014-01-09T08:53:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15651",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15651#event-205828"
}
```



---

archive/issue_events_205829.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2014-01-09T08:53:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15651",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
    "label_color": "000080",
    "label_name": "component: porting: cygwin",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15651#event-205829"
}
```



---

archive/issue_events_205830.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2014-01-09T08:53:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15651",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15651#event-205830"
}
```



---

archive/issue_events_205831.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2014-01-09T08:53:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15651",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15651#event-205831"
}
```



---

archive/issue_comments_199250.json:
```json
{
    "body": "Branch: **[u/jpflori/ticket/15651](https://github.com/sagemath/sagetrac-mirror/tree/u/jpflori/ticket/15651)**",
    "created_at": "2014-01-09T14:11:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15651#issuecomment-199250",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Branch: **[u/jpflori/ticket/15651](https://github.com/sagemath/sagetrac-mirror/tree/u/jpflori/ticket/15651)**



---

archive/issue_comments_199251.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f09bc424bc19d6f6cc1b1d1c5718d99ce6d739eb\">f09bc42</a></td><td><code>Let MPIR build without C++ support on Cygwin.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c81e4b9044197174e5c043a1cc39459a6673497c\">c81e4b9</a></td><td><code>Merge remote-tracking branch 'trac/u/jpflori/ticket/15365' into ticket/14648</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9ba3d7fb03b7e55aa3f116a82840e5b07e472ea1\">9ba3d7f</a></td><td><code>Update config.(guess|sub) files to recognize Cygwin64.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/09276715c5826609a87d21ae0947b30f6e5c1708\">0927671</a></td><td><code>Let MPIR build correctly on Cygwin64.</code></td></tr></table>\n",
    "created_at": "2014-01-09T14:11:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15651#issuecomment-199251",
    "user": "https://github.com/sagetrac-jpflori"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f09bc424bc19d6f6cc1b1d1c5718d99ce6d739eb">f09bc42</a></td><td><code>Let MPIR build without C++ support on Cygwin.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c81e4b9044197174e5c043a1cc39459a6673497c">c81e4b9</a></td><td><code>Merge remote-tracking branch 'trac/u/jpflori/ticket/15365' into ticket/14648</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9ba3d7fb03b7e55aa3f116a82840e5b07e472ea1">9ba3d7f</a></td><td><code>Update config.(guess|sub) files to recognize Cygwin64.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/09276715c5826609a87d21ae0947b30f6e5c1708">0927671</a></td><td><code>Let MPIR build correctly on Cygwin64.</code></td></tr></table>




---

archive/issue_comments_199252.json:
```json
{
    "body": "Author: **Jean-Pierre Flori**",
    "created_at": "2014-01-09T14:11:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15651#issuecomment-199252",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Author: **Jean-Pierre Flori**



---

archive/issue_comments_199253.json:
```json
{
    "body": "Commit: **[0927671](https://github.com/sagemath/sagetrac-mirror/commit/09276715c5826609a87d21ae0947b30f6e5c1708)**",
    "created_at": "2014-01-09T14:11:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15651#issuecomment-199253",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Commit: **[0927671](https://github.com/sagemath/sagetrac-mirror/commit/09276715c5826609a87d21ae0947b30f6e5c1708)**



---

archive/issue_comments_199254.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f169eaa88c7b9dce933cfc90ae160dfaaf24aead\">f169eaa</a></td><td><code>Missing commit to fix asm files for Cygwin64.</code></td></tr></table>\n",
    "created_at": "2014-01-09T14:48:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15651#issuecomment-199254",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:2'>Comment 2:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f169eaa88c7b9dce933cfc90ae160dfaaf24aead">f169eaa</a></td><td><code>Missing commit to fix asm files for Cygwin64.</code></td></tr></table>




---

archive/issue_comments_199255.json:
```json
{
    "body": "Changed commit from **[0927671](https://github.com/sagemath/sagetrac-mirror/commit/09276715c5826609a87d21ae0947b30f6e5c1708)** to **[f169eaa](https://github.com/sagemath/sagetrac-mirror/commit/f169eaa88c7b9dce933cfc90ae160dfaaf24aead)**",
    "created_at": "2014-01-09T14:48:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15651#issuecomment-199255",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[0927671](https://github.com/sagemath/sagetrac-mirror/commit/09276715c5826609a87d21ae0947b30f6e5c1708)** to **[f169eaa](https://github.com/sagemath/sagetrac-mirror/commit/f169eaa88c7b9dce933cfc90ae160dfaaf24aead)**



---

archive/issue_comments_199256.json:
```json
{
    "body": "Changed commit from **[f169eaa](https://github.com/sagemath/sagetrac-mirror/commit/f169eaa88c7b9dce933cfc90ae160dfaaf24aead)** to **[0286624](https://github.com/sagemath/sagetrac-mirror/commit/02866244b8b7ec0d9abb478ff9ba4a7a8b39cbed)**",
    "created_at": "2014-01-09T15:08:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15651#issuecomment-199256",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[f169eaa](https://github.com/sagemath/sagetrac-mirror/commit/f169eaa88c7b9dce933cfc90ae160dfaaf24aead)** to **[0286624](https://github.com/sagemath/sagetrac-mirror/commit/02866244b8b7ec0d9abb478ff9ba4a7a8b39cbed)**



---

archive/issue_comments_199257.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/02866244b8b7ec0d9abb478ff9ba4a7a8b39cbed\">0286624</a></td><td><code>Prevent MPIR spkg-install script to force 32 bit on Cygwin*.</code></td></tr></table>\n",
    "created_at": "2014-01-09T15:08:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15651#issuecomment-199257",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:3'>Comment 3:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/02866244b8b7ec0d9abb478ff9ba4a7a8b39cbed">0286624</a></td><td><code>Prevent MPIR spkg-install script to force 32 bit on Cygwin*.</code></td></tr></table>




---

archive/issue_events_205832.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2014-01-15T21:09:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15651",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15651#event-205832"
}
```



---

archive/issue_comments_199258.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nI forgot to mention this here, but I was able to install MPIR on Cygwin64. Yet with #14648 failing on my other machines, the positive review will have to wait until that's done.",
    "created_at": "2014-01-17T06:15:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15651#issuecomment-199258",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:5'>Comment 5:</a>
I forgot to mention this here, but I was able to install MPIR on Cygwin64. Yet with #14648 failing on my other machines, the positive review will have to wait until that's done.



---

archive/issue_events_205833.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15651",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15651#event-205833"
}
```



---

archive/issue_events_205834.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15651",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15651#event-205834"
}
```



---

archive/issue_comments_199259.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nAnd MPIR 2.7 is on the way (which hopefully includes all fixes necessary, except those to `spkg-install` of course).",
    "created_at": "2014-04-03T19:14:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15651#issuecomment-199259",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:7'>Comment 7:</a>
And MPIR 2.7 is on the way (which hopefully includes all fixes necessary, except those to `spkg-install` of course).



---

archive/issue_comments_199260.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nyes indeed...",
    "created_at": "2014-04-03T22:59:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15651#issuecomment-199260",
    "user": "https://github.com/sagetrac-jpflori"
}
```

<a id='comment:8'>Comment 8:</a>
yes indeed...



---

archive/issue_comments_199261.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\n#15015 should supercede this ticket.",
    "created_at": "2014-04-06T19:02:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15651#issuecomment-199261",
    "user": "https://github.com/sagetrac-jpflori"
}
```

<a id='comment:9'>Comment 9:</a>
#15015 should supercede this ticket.



---

archive/issue_events_205835.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15651",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15651#event-205835"
}
```



---

archive/issue_events_205836.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15651",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15651#event-205836"
}
```



---

archive/issue_events_205837.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15651",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15651#event-205837"
}
```



---

archive/issue_events_205838.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15651",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15651#event-205838"
}
```



---

archive/issue_comments_199262.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\n> #15015 should supercede this ticket.\n\nSo, this ticket should be closed now?",
    "created_at": "2014-08-25T14:55:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15651#issuecomment-199262",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:12'>Comment 12:</a>
> #15015 should supercede this ticket.

So, this ticket should be closed now?



---

archive/issue_events_205839.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-08-25T14:55:15Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15651",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15651#event-205839"
}
```



---

archive/issue_events_205840.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-08-25T14:55:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15651",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15651#event-205840"
}
```



---

archive/issue_events_205841.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-08-25T14:55:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15651",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15651#event-205841"
}
```



---

archive/issue_comments_199263.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nI'd say we shoudl wait for MPIR 2.7.0 to be released and #15015 to be merged.",
    "created_at": "2014-08-25T14:57:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15651#issuecomment-199263",
    "user": "https://github.com/sagetrac-jpflori"
}
```

<a id='comment:13'>Comment 13:</a>
I'd say we shoudl wait for MPIR 2.7.0 to be released and #15015 to be merged.



---

archive/issue_comments_199264.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2014-10-25T21:46:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15651#issuecomment-199264",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_events_205842.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-10-25T21:46:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15651",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15651#event-205842"
}
```



---

archive/issue_events_205843.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-10-25T21:46:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15651",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15651#event-205843"
}
```



---

archive/issue_events_205844.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-10-25T21:46:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15651",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15651#event-205844"
}
```
