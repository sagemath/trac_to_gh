# Issue 15292: matroids.Uniform(r,r) returns wrong matroid

archive/issues_015055.json:
```json
{
    "assignees": [],
    "body": "The following is wrong:\n\n```\nsage: M = matroids.Uniform(4,4)\nsage: len(M.circuit_closures())\n1\nsage: N = matroids.Uniform(4,5).delete(4)\nsage: len(N.circuit_closures())\n0\nsage: N.is_isomorphic(M)\nTrue\n```\n\n\nBranch/Commit: **[u/Rudi/ticket/15292](https://github.com/sagemath/sagetrac-mirror/tree/u/Rudi/ticket/15292) @ [a8a28c7](https://github.com/sagemath/sagetrac-mirror/commit/a8a28c71bcf525845acd73620839e86549e4b2f2)**\n\nReviewer: **Stefan van Zwam**\n\nAuthor: **Rudi Pendavingh**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15292_\n\n",
    "closed_at": "2014-01-05T00:32:18Z",
    "created_at": "2013-10-16T16:41:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20matroid%20theory",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "matroids.Uniform(r,r) returns wrong matroid",
    "type": "issue",
    "updated_at": "2014-01-05T00:32:18Z",
    "url": "https://github.com/sagemath/sage/issues/15292",
    "user": "https://github.com/sagetrac-Stefan"
}
```
The following is wrong:

```
sage: M = matroids.Uniform(4,4)
sage: len(M.circuit_closures())
1
sage: N = matroids.Uniform(4,5).delete(4)
sage: len(N.circuit_closures())
0
sage: N.is_isomorphic(M)
True
```


Branch/Commit: **[u/Rudi/ticket/15292](https://github.com/sagemath/sagetrac-mirror/tree/u/Rudi/ticket/15292) @ [a8a28c7](https://github.com/sagemath/sagetrac-mirror/commit/a8a28c71bcf525845acd73620839e86549e4b2f2)**

Reviewer: **Stefan van Zwam**

Author: **Rudi Pendavingh**

_Issue created by migration from https://trac.sagemath.org/ticket/15292_





---

archive/issue_events_200047.json:
```json
{
    "actor": "https://github.com/sagetrac-Stefan",
    "created_at": "2013-10-16T16:41:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15292",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15292#event-200047"
}
```



---

archive/issue_events_200048.json:
```json
{
    "actor": "https://github.com/sagetrac-Stefan",
    "created_at": "2013-10-16T16:41:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15292",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20matroid%20theory",
    "label_color": "0000ff",
    "label_name": "component: matroid theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15292#event-200048"
}
```



---

archive/issue_events_200049.json:
```json
{
    "actor": "https://github.com/sagetrac-Stefan",
    "created_at": "2013-10-16T16:41:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15292",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15292#event-200049"
}
```



---

archive/issue_events_200050.json:
```json
{
    "actor": "https://github.com/sagetrac-Stefan",
    "created_at": "2013-10-16T16:41:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15292",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15292#event-200050"
}
```



---

archive/issue_comments_191438.json:
```json
{
    "body": "Branch: **[u/Rudi/ticket/15292](https://github.com/sagemath/sagetrac-mirror/tree/u/Rudi/ticket/15292)**",
    "created_at": "2014-01-03T14:54:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15292",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15292#issuecomment-191438",
    "user": "https://github.com/sagetrac-Rudi"
}
```

Branch: **[u/Rudi/ticket/15292](https://github.com/sagemath/sagetrac-mirror/tree/u/Rudi/ticket/15292)**



---

archive/issue_events_200051.json:
```json
{
    "actor": "https://github.com/sagetrac-Rudi",
    "created_at": "2014-01-03T14:56:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15292",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15292#event-200051"
}
```



---

archive/issue_comments_191439.json:
```json
{
    "body": "Commit: **[a8a28c7](https://github.com/sagemath/sagetrac-mirror/commit/a8a28c71bcf525845acd73620839e86549e4b2f2)**",
    "created_at": "2014-01-03T14:56:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15292",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15292#issuecomment-191439",
    "user": "https://github.com/sagetrac-Rudi"
}
```

Commit: **[a8a28c7](https://github.com/sagemath/sagetrac-mirror/commit/a8a28c71bcf525845acd73620839e86549e4b2f2)**



---

archive/issue_comments_191440.json:
```json
{
    "body": "Author: **Rudi Pendavingh**",
    "created_at": "2014-01-03T14:56:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15292",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15292#issuecomment-191440",
    "user": "https://github.com/sagetrac-Rudi"
}
```

Author: **Rudi Pendavingh**



---

archive/issue_comments_191441.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a8a28c7\">a8a28c7</a></td><td><code>Fixed bug in uniform matroid.</code></td></tr></table>\n",
    "created_at": "2014-01-03T14:56:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15292",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15292#issuecomment-191441",
    "user": "https://github.com/sagetrac-Rudi"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a8a28c7">a8a28c7</a></td><td><code>Fixed bug in uniform matroid.</code></td></tr></table>




---

archive/issue_comments_191442.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nThe code looks solid to me, a simple bug fix. Positive review!",
    "created_at": "2014-01-03T20:13:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15292",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15292#issuecomment-191442",
    "user": "https://github.com/sagetrac-Stefan"
}
```

<div id="comment:3" align="right">Comment 3</div>

The code looks solid to me, a simple bug fix. Positive review!



---

archive/issue_events_200052.json:
```json
{
    "actor": "https://github.com/sagetrac-Stefan",
    "created_at": "2014-01-03T20:13:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15292",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15292#event-200052"
}
```



---

archive/issue_events_200053.json:
```json
{
    "actor": "https://github.com/sagetrac-Stefan",
    "created_at": "2014-01-03T20:13:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15292",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15292#event-200053"
}
```



---

archive/issue_comments_191443.json:
```json
{
    "body": "Reviewer: **Stefan van Zwam**",
    "created_at": "2014-01-04T04:21:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15292",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15292#issuecomment-191443",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Stefan van Zwam**



---

archive/issue_comments_191444.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nPlease fill in reviewer name next time...",
    "created_at": "2014-01-04T04:21:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15292",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15292#issuecomment-191444",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:4" align="right">Comment 4</div>

Please fill in reviewer name next time...



---

archive/issue_events_200054.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-01-05T00:32:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15292",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15292#event-200054"
}
```



---

archive/issue_events_200055.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "9c9a8c6b96c0b8b6c0dd88457138c93857328791",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-01-05T00:32:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15292",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15292#event-200055"
}
```
