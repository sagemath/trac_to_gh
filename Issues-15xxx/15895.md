# Issue 15895: Bug in Graph.subdivide_edge

archive/issues_015658.json:
```json
{
    "assignees": [],
    "body": "As reported by Georgi Guninski:\n\n```\nsage: F=graphs.PathGraph(3);\nsage: S='S';F.add_vertex(S);\nsage: F.add_edges([(S,0)]);\nsage: F2=Graph(F);\nsage: F2.subdivide_edges(list(F2.edges(labels=False)),2);\nsage: F2.degree()\n[2, 2, 1, 0, 2, 0, 2, 0, 2, 5]\n```\n\nafter relabel:\n\n```\nsage: F=graphs.PathGraph(3);\nsage: S='S';\nsage: F.add_vertex(S);\nsage: F.add_edges([(S,0)]);\nsage: F2=Graph(F);\nsage: F2.relabel();\nsage: F2.subdivide_edges(list(F2.edges(labels=False)),2);\nsage: F2.degree()\n[2, 2, 1, 1, 2, 2, 2, 2, 2, 2]\n```\n\nThis bug, as very often, happened because I am an idiot.\n\nNathann\n\nBranch/Commit: **[0d8480d](https://github.com/sagemath/sagetrac-mirror/commit/0d8480dd91233fb3a64242177ba54237dfffed65)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Nathann Cohen**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15895_\n\n",
    "closed_at": "2014-03-13T21:05:43Z",
    "created_at": "2014-03-06T10:24:43Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Bug in Graph.subdivide_edge",
    "type": "issue",
    "updated_at": "2014-03-13T21:05:43Z",
    "url": "https://github.com/sagemath/sage/issues/15895",
    "user": "https://github.com/nathanncohen"
}
```
As reported by Georgi Guninski:

```
sage: F=graphs.PathGraph(3);
sage: S='S';F.add_vertex(S);
sage: F.add_edges([(S,0)]);
sage: F2=Graph(F);
sage: F2.subdivide_edges(list(F2.edges(labels=False)),2);
sage: F2.degree()
[2, 2, 1, 0, 2, 0, 2, 0, 2, 5]
```

after relabel:

```
sage: F=graphs.PathGraph(3);
sage: S='S';
sage: F.add_vertex(S);
sage: F.add_edges([(S,0)]);
sage: F2=Graph(F);
sage: F2.relabel();
sage: F2.subdivide_edges(list(F2.edges(labels=False)),2);
sage: F2.degree()
[2, 2, 1, 1, 2, 2, 2, 2, 2, 2]
```

This bug, as very often, happened because I am an idiot.

Nathann

Branch/Commit: **[0d8480d](https://github.com/sagemath/sagetrac-mirror/commit/0d8480dd91233fb3a64242177ba54237dfffed65)**

Reviewer: **Frédéric Chapoton**

Author: **Nathann Cohen**

_Issue created by migration from https://trac.sagemath.org/ticket/15895_





---

archive/issue_events_209434.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-03-06T10:24:43Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15895",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15895#event-209434"
}
```



---

archive/issue_events_209435.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-03-06T10:24:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15895",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15895#event-209435"
}
```



---

archive/issue_events_209436.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-03-06T10:24:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15895",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15895#event-209436"
}
```



---

archive/issue_events_209437.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-03-06T10:24:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15895",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15895#event-209437"
}
```



---

archive/issue_comments_203834.json:
```json
{
    "body": "Branch: **[u/ncohen/15895](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/15895)**",
    "created_at": "2014-03-06T10:27:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15895",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15895#issuecomment-203834",
    "user": "https://github.com/nathanncohen"
}
```

Branch: **[u/ncohen/15895](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/15895)**



---

archive/issue_events_209438.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-03-06T10:27:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15895",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15895#event-209438"
}
```



---

archive/issue_comments_203835.json:
```json
{
    "body": "Commit: **[b27adaa](https://github.com/sagemath/sagetrac-mirror/commit/b27adaac3f98745fe99839d493324ebd32b9cec2)**",
    "created_at": "2014-03-06T10:30:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15895",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15895#issuecomment-203835",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[b27adaa](https://github.com/sagemath/sagetrac-mirror/commit/b27adaac3f98745fe99839d493324ebd32b9cec2)**



---

archive/issue_comments_203836.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b27adaac3f98745fe99839d493324ebd32b9cec2\">b27adaa</a></td><td><code>trac #15895: Bug in Graph.subdivide_edge</code></td></tr></table>\n",
    "created_at": "2014-03-06T10:30:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15895",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15895#issuecomment-203836",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b27adaac3f98745fe99839d493324ebd32b9cec2">b27adaa</a></td><td><code>trac #15895: Bug in Graph.subdivide_edge</code></td></tr></table>




---

archive/issue_comments_203837.json:
```json
{
    "body": "Changed commit from **[b27adaa](https://github.com/sagemath/sagetrac-mirror/commit/b27adaac3f98745fe99839d493324ebd32b9cec2)** to **[0d8480d](https://github.com/sagemath/sagetrac-mirror/commit/0d8480dd91233fb3a64242177ba54237dfffed65)**",
    "created_at": "2014-03-09T12:45:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15895",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15895#issuecomment-203837",
    "user": "https://github.com/fchapoton"
}
```

Changed commit from **[b27adaa](https://github.com/sagemath/sagetrac-mirror/commit/b27adaac3f98745fe99839d493324ebd32b9cec2)** to **[0d8480d](https://github.com/sagemath/sagetrac-mirror/commit/0d8480dd91233fb3a64242177ba54237dfffed65)**



---

archive/issue_comments_203838.json:
```json
{
    "body": "Changed branch from **[u/ncohen/15895](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/15895)** to **[public/ticket/15895](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/15895)**",
    "created_at": "2014-03-09T12:45:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15895",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15895#issuecomment-203838",
    "user": "https://github.com/fchapoton"
}
```

Changed branch from **[u/ncohen/15895](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/15895)** to **[public/ticket/15895](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/15895)**



---

archive/issue_comments_203839.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLooks good to me. I have made a cosmetic review patch. If you agree with my changes, you can set to positive review.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0d8480dd91233fb3a64242177ba54237dfffed65\">0d8480d</a></td><td><code>trac #15895 review patch</code></td></tr></table>\n",
    "created_at": "2014-03-09T12:45:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15895",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15895#issuecomment-203839",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:3" align="right">Comment 3</div>

Looks good to me. I have made a cosmetic review patch. If you agree with my changes, you can set to positive review.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0d8480dd91233fb3a64242177ba54237dfffed65">0d8480d</a></td><td><code>trac #15895 review patch</code></td></tr></table>




---

archive/issue_events_209439.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-03-09T12:48:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15895",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15895#event-209439"
}
```



---

archive/issue_events_209440.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-03-09T12:48:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15895",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15895#event-209440"
}
```



---

archive/issue_comments_203840.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2014-03-09T12:48:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15895",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15895#issuecomment-203840",
    "user": "https://github.com/nathanncohen"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_203841.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nOkayyyy, then it is good to go ! Thanks :-)\n\nNathann",
    "created_at": "2014-03-09T12:48:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15895",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15895#issuecomment-203841",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:4" align="right">Comment 4</div>

Okayyyy, then it is good to go ! Thanks :-)

Nathann



---

archive/issue_comments_203842.json:
```json
{
    "body": "Changed branch from **[public/ticket/15895](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/15895)** to **[0d8480d](https://github.com/sagemath/sagetrac-mirror/commit/0d8480dd91233fb3a64242177ba54237dfffed65)**",
    "created_at": "2014-03-13T21:05:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15895",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15895#issuecomment-203842",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/ticket/15895](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/15895)** to **[0d8480d](https://github.com/sagemath/sagetrac-mirror/commit/0d8480dd91233fb3a64242177ba54237dfffed65)**



---

archive/issue_events_209441.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-03-13T21:05:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15895",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15895#event-209441"
}
```



---

archive/issue_events_209442.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "4d888e6dea6590de515b62df9d3c8268a6fbb270",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-03-13T21:05:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15895",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15895#event-209442"
}
```
