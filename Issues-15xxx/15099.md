# Issue 15099: Implement numeric evaluation of zetaderiv

archive/issues_014862.json:
```json
{
    "assignees": [],
    "body": "Numeric evaluation of `zetaderiv` can easily be implemented with mpmath.\n\nCC:  @burcin\n\nBranch/Commit: **[be67fb0](https://github.com/sagemath/sagetrac-mirror/commit/be67fb0ccad269ac6c1772c00d6d010c39c636ae)**\n\nReviewer: **Ralf Stephan**\n\nAuthor: **Eviatar Bach**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15099_\n\n",
    "closed_at": "2014-05-13T13:48:53Z",
    "created_at": "2013-08-25T15:02:50Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Implement numeric evaluation of zetaderiv",
    "type": "issue",
    "updated_at": "2014-05-13T13:48:53Z",
    "url": "https://github.com/sagemath/sage/issues/15099",
    "user": "https://github.com/eviatarbach"
}
```
Numeric evaluation of `zetaderiv` can easily be implemented with mpmath.

CC:  @burcin

Branch/Commit: **[be67fb0](https://github.com/sagemath/sagetrac-mirror/commit/be67fb0ccad269ac6c1772c00d6d010c39c636ae)**

Reviewer: **Ralf Stephan**

Author: **Eviatar Bach**

_Issue created by migration from https://trac.sagemath.org/ticket/15099_





---

archive/issue_events_196996.json:
```json
{
    "actor": "https://github.com/eviatarbach",
    "created_at": "2013-08-25T15:02:50Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15099",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15099#event-196996"
}
```



---

archive/issue_events_196997.json:
```json
{
    "actor": "https://github.com/eviatarbach",
    "created_at": "2013-08-25T15:02:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15099",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15099#event-196997"
}
```



---

archive/issue_events_196998.json:
```json
{
    "actor": "https://github.com/eviatarbach",
    "created_at": "2013-08-25T15:02:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15099",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15099#event-196998"
}
```



---

archive/issue_events_196999.json:
```json
{
    "actor": "https://github.com/eviatarbach",
    "created_at": "2013-08-25T15:02:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15099",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15099#event-196999"
}
```



---

archive/issue_events_197000.json:
```json
{
    "actor": "https://github.com/eviatarbach",
    "created_at": "2013-08-25T15:39:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15099",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15099#event-197000"
}
```



---

archive/issue_comments_187896.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nAttachment: **[trac15099.patch.gz](https://github.com/sagemath/sage/files/ticket15099/trac15099.patch.gz)**",
    "created_at": "2013-08-25T15:39:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15099",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15099#issuecomment-187896",
    "user": "https://github.com/eviatarbach"
}
```

<div id="comment:1" align="right">Comment 1</div>

Attachment: **[trac15099.patch.gz](https://github.com/sagemath/sage/files/ticket15099/trac15099.patch.gz)**



---

archive/issue_comments_187897.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nAttachment: **[trac15099_2.patch.gz](https://github.com/sagemath/sage/files/ticket15099/trac15099_2.patch.gz)**\n\nDoctests in `_evalf_` for complete coverage.\n\nPatchbot apply trac15099_2.patch",
    "created_at": "2013-08-30T05:28:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15099",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15099#issuecomment-187897",
    "user": "https://github.com/eviatarbach"
}
```

<div id="comment:2" align="right">Comment 2</div>

Attachment: **[trac15099_2.patch.gz](https://github.com/sagemath/sage/files/ticket15099/trac15099_2.patch.gz)**

Doctests in `_evalf_` for complete coverage.

Patchbot apply trac15099_2.patch



---

archive/issue_events_197001.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15099",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15099#event-197001"
}
```



---

archive/issue_events_197002.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15099",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15099#event-197002"
}
```



---

archive/issue_events_197003.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15099",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15099#event-197003"
}
```



---

archive/issue_events_197004.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15099",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15099#event-197004"
}
```



---

archive/issue_comments_187898.json:
```json
{
    "body": "Branch: **[u/rws/implement_numeric_evaluation_of_zetaderiv](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/implement_numeric_evaluation_of_zetaderiv)**",
    "created_at": "2014-05-12T09:08:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15099",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15099#issuecomment-187898",
    "user": "https://github.com/rwst"
}
```

Branch: **[u/rws/implement_numeric_evaluation_of_zetaderiv](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/implement_numeric_evaluation_of_zetaderiv)**



---

archive/issue_events_197005.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-05-12T09:10:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15099",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15099#event-197005"
}
```



---

archive/issue_events_197006.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-05-12T09:10:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15099",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15099#event-197006"
}
```



---

archive/issue_comments_187899.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nRebased. Most of the original patch was already included with #15095, so this ticket only adds two doctests which pass.\n  \n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/be67fb0ccad269ac6c1772c00d6d010c39c636ae\">be67fb0</a></td><td><code>Trac 15099: numeric evaluation of zetaderiv</code></td></tr></table>\n",
    "created_at": "2014-05-12T09:10:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15099",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15099#issuecomment-187899",
    "user": "https://github.com/rwst"
}
```

<div id="comment:6" align="right">Comment 6</div>

Rebased. Most of the original patch was already included with #15095, so this ticket only adds two doctests which pass.
  
---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/be67fb0ccad269ac6c1772c00d6d010c39c636ae">be67fb0</a></td><td><code>Trac 15099: numeric evaluation of zetaderiv</code></td></tr></table>




---

archive/issue_comments_187900.json:
```json
{
    "body": "Commit: **[be67fb0](https://github.com/sagemath/sagetrac-mirror/commit/be67fb0ccad269ac6c1772c00d6d010c39c636ae)**",
    "created_at": "2014-05-12T09:10:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15099",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15099#issuecomment-187900",
    "user": "https://github.com/rwst"
}
```

Commit: **[be67fb0](https://github.com/sagemath/sagetrac-mirror/commit/be67fb0ccad269ac6c1772c00d6d010c39c636ae)**



---

archive/issue_comments_187901.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nReviewer name",
    "created_at": "2014-05-13T08:44:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15099",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15099#issuecomment-187901",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:7" align="right">Comment 7</div>

Reviewer name



---

archive/issue_comments_187902.json:
```json
{
    "body": "Reviewer: **Ralf Stephan**",
    "created_at": "2014-05-13T08:46:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15099",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15099#issuecomment-187902",
    "user": "https://github.com/rwst"
}
```

Reviewer: **Ralf Stephan**



---

archive/issue_comments_187903.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nThanks! I forgot this was included in the other patch too.",
    "created_at": "2014-05-13T08:50:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15099",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15099#issuecomment-187903",
    "user": "https://github.com/eviatarbach"
}
```

<div id="comment:9" align="right">Comment 9</div>

Thanks! I forgot this was included in the other patch too.



---

archive/issue_comments_187904.json:
```json
{
    "body": "Author: **Eviatar Bach**",
    "created_at": "2014-05-13T08:50:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15099",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15099#issuecomment-187904",
    "user": "https://github.com/eviatarbach"
}
```

Author: **Eviatar Bach**



---

archive/issue_events_197007.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-05-13T13:48:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15099",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15099#event-197007"
}
```



---

archive/issue_events_197008.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "bbc23e19fba792edb05c4c015c05d3401b578da5",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-05-13T13:48:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15099",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15099#event-197008"
}
```



---

archive/issue_comments_187905.json:
```json
{
    "body": "Changed branch from **[u/rws/implement_numeric_evaluation_of_zetaderiv](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/implement_numeric_evaluation_of_zetaderiv)** to **[be67fb0](https://github.com/sagemath/sagetrac-mirror/commit/be67fb0ccad269ac6c1772c00d6d010c39c636ae)**",
    "created_at": "2014-05-13T13:48:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15099",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15099#issuecomment-187905",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/rws/implement_numeric_evaluation_of_zetaderiv](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/implement_numeric_evaluation_of_zetaderiv)** to **[be67fb0](https://github.com/sagemath/sagetrac-mirror/commit/be67fb0ccad269ac6c1772c00d6d010c39c636ae)**
