# Issue 15909: Spurious gap processes when running tests

archive/issues_015672.json:
```json
{
    "assignees": [],
    "body": "When I run tests, I often remain with spurious gap processes in the background. For example running\n\n```\nsage -t /src/sage/combinat/root_system/coxeter_group.py\n```\nI end up with\n\n```\n84121- gap          98.0  07:30.09 1/1  0    14   30     155M   240K   156M   266M   1099M  84121 1     running  501  40753\n```\nThis is on a MacOS 10.6.8 with coxeter3 installed.\n\nCC:  @mkoeppe @slel\n\nKeywords: **gap, doctests**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton, Samuel Leli\u00e8vre**\n\nComponent: **packages: optional**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15909_\n\n",
    "closed_at": "2022-03-04T12:18:18Z",
    "created_at": "2014-03-08T03:05:05Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20optional",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Spurious gap processes when running tests",
    "type": "issue",
    "updated_at": "2022-03-04T12:18:18Z",
    "url": "https://github.com/sagemath/sage/issues/15909",
    "user": "https://github.com/anneschilling"
}
```
When I run tests, I often remain with spurious gap processes in the background. For example running

```
sage -t /src/sage/combinat/root_system/coxeter_group.py
```
I end up with

```
84121- gap          98.0  07:30.09 1/1  0    14   30     155M   240K   156M   266M   1099M  84121 1     running  501  40753
```
This is on a MacOS 10.6.8 with coxeter3 installed.

CC:  @mkoeppe @slel

Keywords: **gap, doctests**

Reviewer: **Frédéric Chapoton, Samuel Lelièvre**

Component: **packages: optional**

_Issue created by migration from https://trac.sagemath.org/ticket/15909_





---

archive/issue_events_227109.json:
```json
{
    "actor": "https://github.com/anneschilling",
    "created_at": "2014-03-08T03:05:05Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15909",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15909#event-227109"
}
```



---

archive/issue_events_227110.json:
```json
{
    "actor": "https://github.com/anneschilling",
    "created_at": "2014-03-08T03:05:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15909",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15909#event-227110"
}
```



---

archive/issue_events_227111.json:
```json
{
    "actor": "https://github.com/anneschilling",
    "created_at": "2014-03-08T03:05:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15909",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15909#event-227111"
}
```



---

archive/issue_comments_201537.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -8,3 +8,4 @@\n ```\n 84121- gap          98.0  07:30.09 1/1  0    14   30     155M   240K   156M   266M   1099M  84121 1     running  501  40753\n ```\n+This is on a MacOS 10.6.8 with coxeter3 installed.\n``````\n",
    "created_at": "2014-03-08T03:05:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15909#issuecomment-201537",
    "user": "https://github.com/anneschilling"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -8,3 +8,4 @@
 ```
 84121- gap          98.0  07:30.09 1/1  0    14   30     155M   240K   156M   266M   1099M  84121 1     running  501  40753
 ```
+This is on a MacOS 10.6.8 with coxeter3 installed.
``````




---

archive/issue_events_227112.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15909",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15909#event-227112"
}
```



---

archive/issue_events_227113.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15909",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15909#event-227113"
}
```



---

archive/issue_events_227114.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15909",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15909#event-227114"
}
```



---

archive/issue_events_227115.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15909",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15909#event-227115"
}
```



---

archive/issue_events_227116.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-03-03T16:21:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15909",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15909#event-227116"
}
```



---

archive/issue_comments_201538.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nclose as obsolete ?",
    "created_at": "2022-03-03T16:21:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15909#issuecomment-201538",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:4" align="right">comment:4</div>

close as obsolete ?



---

archive/issue_events_227117.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-03-03T16:21:06Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15909",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15909#event-227117"
}
```



---

archive/issue_events_227118.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-03-03T16:21:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15909",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15909#event-227118"
}
```



---

archive/issue_events_227119.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-03-03T16:21:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15909",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15909#event-227119"
}
```



---

archive/issue_comments_201539.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nThere have been several GAP updates in the meantime so\nit seems reasonable to close.\n\nReopen if this can still be observed.",
    "created_at": "2022-03-04T11:28:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15909#issuecomment-201539",
    "user": "https://github.com/slel"
}
```

<div id="comment:5" align="right">comment:5</div>

There have been several GAP updates in the meantime so
it seems reasonable to close.

Reopen if this can still be observed.



---

archive/issue_events_227120.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2022-03-04T11:28:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15909",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20optional",
    "label_color": "000080",
    "label_name": "c: packages: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15909#event-227120"
}
```



---

archive/issue_comments_201540.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton, Samuel Leli\u00e8vre**",
    "created_at": "2022-03-04T11:28:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15909#issuecomment-201540",
    "user": "https://github.com/slel"
}
```

Reviewer: **Frédéric Chapoton, Samuel Lelièvre**



---

archive/issue_events_227121.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2022-03-04T11:28:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15909",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15909#event-227121"
}
```



---

archive/issue_events_227122.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2022-03-04T11:28:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15909",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15909#event-227122"
}
```



---

archive/issue_events_227123.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-03-04T12:18:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15909",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15909#event-227123"
}
```



---

archive/issue_events_227124.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-03-04T12:18:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15909",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15909#event-227124"
}
```
