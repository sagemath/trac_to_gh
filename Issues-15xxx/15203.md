# Issue 15203: error in LLL method with delta=1

archive/issues_014966.json:
```json
{
    "assignees": [],
    "body": "Acording to the documentation, the method LLL for integer matrices allows the parameter delta to take a value <=1, however, for delta=1 the method always fail. It did not failed with previous version of sage.\n\n```\nsage: m=random_matrix(ZZ,2,2)\nsage: m.LLL(delta=1)\n...\nRuntimeError: fpLLL returned -1 != 0\n```\n\nEither the documentation should be updated or the code should be fixed.\n\nKeywords: **fpLLL**\n\nBranch/Commit: **[fc01a01](https://github.com/sagemath/sagetrac-mirror/commit/fc01a01ab91e112c29f9507aa6aa0c44489e6500)**\n\nReviewer: **Martin Albrecht**\n\nAuthor: **Luis Felipe Tabera Alonso**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15203_\n\n",
    "closed_at": "2014-10-16T21:10:28Z",
    "created_at": "2013-09-17T13:50:10Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "error in LLL method with delta=1",
    "type": "issue",
    "updated_at": "2014-10-16T21:10:28Z",
    "url": "https://github.com/sagemath/sage/issues/15203",
    "user": "https://github.com/lftabera"
}
```
Acording to the documentation, the method LLL for integer matrices allows the parameter delta to take a value <=1, however, for delta=1 the method always fail. It did not failed with previous version of sage.

```
sage: m=random_matrix(ZZ,2,2)
sage: m.LLL(delta=1)
...
RuntimeError: fpLLL returned -1 != 0
```

Either the documentation should be updated or the code should be fixed.

Keywords: **fpLLL**

Branch/Commit: **[fc01a01](https://github.com/sagemath/sagetrac-mirror/commit/fc01a01ab91e112c29f9507aa6aa0c44489e6500)**

Reviewer: **Martin Albrecht**

Author: **Luis Felipe Tabera Alonso**

_Issue created by migration from https://trac.sagemath.org/ticket/15203_





---

archive/issue_events_198636.json:
```json
{
    "actor": "https://github.com/lftabera",
    "created_at": "2013-09-17T13:50:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15203",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15203#event-198636"
}
```



---

archive/issue_events_198637.json:
```json
{
    "actor": "https://github.com/lftabera",
    "created_at": "2013-09-17T13:50:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15203",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15203#event-198637"
}
```



---

archive/issue_events_198638.json:
```json
{
    "actor": "https://github.com/lftabera",
    "created_at": "2013-09-17T13:50:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15203",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15203#event-198638"
}
```



---

archive/issue_events_198639.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15203",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15203#event-198639"
}
```



---

archive/issue_events_198640.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15203",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15203#event-198640"
}
```



---

archive/issue_events_198641.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15203",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15203#event-198641"
}
```



---

archive/issue_events_198642.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15203",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15203#event-198642"
}
```



---

archive/issue_events_198643.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15203",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15203#event-198643"
}
```



---

archive/issue_events_198644.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15203",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15203#event-198644"
}
```



---

archive/issue_comments_189694.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nIt's a documentation error, delta must be < 1 for polynomial time.",
    "created_at": "2014-10-15T09:33:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15203",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15203#issuecomment-189694",
    "user": "https://github.com/malb"
}
```

<div id="comment:4" align="right">Comment 4</div>

It's a documentation error, delta must be < 1 for polynomial time.



---

archive/issue_comments_189695.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\ndelta must be < 1 for polynomial time. But not for finishing or not. Apparently, fplll does not allow delta=1, while NTL does. I have updated the documentation.",
    "created_at": "2014-10-15T14:06:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15203",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15203#issuecomment-189695",
    "user": "https://github.com/lftabera"
}
```

<div id="comment:5" align="right">Comment 5</div>

delta must be < 1 for polynomial time. But not for finishing or not. Apparently, fplll does not allow delta=1, while NTL does. I have updated the documentation.



---

archive/issue_comments_189696.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/fc01a01ab91e112c29f9507aa6aa0c44489e6500\">fc01a01</a></td><td><code>Fix documentation of LLL method for delta=1, Track #15203</code></td></tr></table>\n",
    "created_at": "2014-10-15T14:07:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15203",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15203#issuecomment-189696",
    "user": "https://github.com/lftabera"
}
```

<div id="comment:6" align="right">Comment 6</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/fc01a01ab91e112c29f9507aa6aa0c44489e6500">fc01a01</a></td><td><code>Fix documentation of LLL method for delta=1, Track #15203</code></td></tr></table>




---

archive/issue_events_198645.json:
```json
{
    "actor": "https://github.com/lftabera",
    "created_at": "2014-10-15T14:07:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15203",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15203#event-198645"
}
```



---

archive/issue_comments_189697.json:
```json
{
    "body": "Branch: **[u/lftabera/ticket/152203](https://github.com/sagemath/sagetrac-mirror/tree/u/lftabera/ticket/152203)**",
    "created_at": "2014-10-15T14:07:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15203",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15203#issuecomment-189697",
    "user": "https://github.com/lftabera"
}
```

Branch: **[u/lftabera/ticket/152203](https://github.com/sagemath/sagetrac-mirror/tree/u/lftabera/ticket/152203)**



---

archive/issue_events_198646.json:
```json
{
    "actor": "https://github.com/lftabera",
    "created_at": "2014-10-15T14:07:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15203",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15203#event-198646"
}
```



---

archive/issue_events_198647.json:
```json
{
    "actor": "https://github.com/lftabera",
    "created_at": "2014-10-15T14:07:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15203",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15203#event-198647"
}
```



---

archive/issue_comments_189698.json:
```json
{
    "body": "Commit: **[fc01a01](https://github.com/sagemath/sagetrac-mirror/commit/fc01a01ab91e112c29f9507aa6aa0c44489e6500)**",
    "created_at": "2014-10-15T14:07:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15203",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15203#issuecomment-189698",
    "user": "https://github.com/lftabera"
}
```

Commit: **[fc01a01](https://github.com/sagemath/sagetrac-mirror/commit/fc01a01ab91e112c29f9507aa6aa0c44489e6500)**



---

archive/issue_comments_189699.json:
```json
{
    "body": "Author: **Luis Felipe Tabera Alonso**",
    "created_at": "2014-10-15T14:07:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15203",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15203#issuecomment-189699",
    "user": "https://github.com/lftabera"
}
```

Author: **Luis Felipe Tabera Alonso**



---

archive/issue_events_198648.json:
```json
{
    "actor": "https://github.com/lftabera",
    "created_at": "2014-10-15T14:07:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15203",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15203#event-198648"
}
```



---

archive/issue_comments_189700.json:
```json
{
    "body": "Reviewer: **Martin Albrecht**",
    "created_at": "2014-10-15T14:15:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15203",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15203#issuecomment-189700",
    "user": "https://github.com/malb"
}
```

Reviewer: **Martin Albrecht**



---

archive/issue_events_198649.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2014-10-15T14:15:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15203",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15203#event-198649"
}
```



---

archive/issue_events_198650.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2014-10-15T14:15:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15203",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15203#event-198650"
}
```



---

archive/issue_comments_189701.json:
```json
{
    "body": "Changed branch from **[u/lftabera/ticket/152203](https://github.com/sagemath/sagetrac-mirror/tree/u/lftabera/ticket/152203)** to **[fc01a01](https://github.com/sagemath/sagetrac-mirror/commit/fc01a01ab91e112c29f9507aa6aa0c44489e6500)**",
    "created_at": "2014-10-16T21:10:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15203",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15203#issuecomment-189701",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/lftabera/ticket/152203](https://github.com/sagemath/sagetrac-mirror/tree/u/lftabera/ticket/152203)** to **[fc01a01](https://github.com/sagemath/sagetrac-mirror/commit/fc01a01ab91e112c29f9507aa6aa0c44489e6500)**



---

archive/issue_events_198651.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-10-16T21:10:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15203",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15203#event-198651"
}
```



---

archive/issue_events_198652.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "f9f642c334b77116a062d940300de7f1f58f3b2a",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-10-16T21:10:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15203",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15203#event-198652"
}
```
