...
```json
{
    "body": "An amazing amount of time is spent for the initialisation of free modules when computing `E.gens()`. So, perhaps this is the culprit.",
...
```json
{
    "body": "So, the plan is: Remove the ugly hack and finally do a full initialisation of matrix spaces. And then see why everything is so slow.",
...
```json
{
    "body": "Adding John to Cc, since the examples seemingly showing a regression since #11900 belong (I think) to his field of expertise.",
...
```json
{
    "body": "There's another open ticket on integer points at #1-973 which I am working on when I have time.  I don't have time to look at this now but it seems fomr the above comments that something has changed outside the ellipticv curve code, is that right?",
...
```json
{
    "body": "That looks completely crazily over the top for an integer points computation.  Why qqbar?  Why abelian groups?  I will look but it will take time and I have less than none for a while.",
...
```json
{
    "body": "There also is a regression in a different example (unrelated with elliptic curves, but part of #11900).",
...
```json
{
    "body": "Note that in the second example from the ticket description, finite fields are involved. But their creation (in a freshly started Sage session, of course) is fast:\n\n```\nsage: def test():\n....:     for p in prime_range(10000):\n....:         P = GF(p)\n....:         \nsage: %time test()\nCPU times: user 0.52 s, sys: 0.01 s, total: 0.53 s\nWall time: 0.53 s\n```",
...
```json
{
    "body": "This one might be slower than in the good old times:\n\n```\nsage: def test():\n....:     for i in xrange(5000):\n....:         MS = MatrixSpace(QQ,i)\n....:         \nsage: %time test()\nCPU times: user 0.49 s, sys: 0.02 s, total: 0.51 s\nWall time: 0.49 s\n```",
...
```json
{
    "body": "#10963 makes the situation even worse. I am commenting there.",
