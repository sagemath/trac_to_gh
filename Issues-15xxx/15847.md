# Issue 15847: Do deepcopy + reset label in FiniteStateMachine.quotient instead of creating a new state

archive/issues_015610.json:
```json
{
    "assignees": [],
    "body": "In\n`sage.combinat.finite_state_machine.FiniteStateMachine.quotient`,\ndo not create a new state, but make deepcopy + reset label in order to\npreserve possible additional information of the state.\n\nKeywords: **finite_state_machine**\n\nBranch/Commit: **[c518d08](https://github.com/sagemath/sagetrac-mirror/commit/c518d0808336d148454d290c41ffefc3d5467bac)**\n\nReviewer: **Daniel Krenn**\n\nAuthor: **Clemens Heuberger, Daniel Krenn**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15847_\n\n",
    "closed_at": "2014-03-20T20:07:56Z",
    "created_at": "2014-02-23T13:38:15Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Do deepcopy + reset label in FiniteStateMachine.quotient instead of creating a new state",
    "type": "issue",
    "updated_at": "2014-03-20T20:07:56Z",
    "url": "https://github.com/sagemath/sage/issues/15847",
    "user": "https://github.com/cheuberg"
}
```
In
`sage.combinat.finite_state_machine.FiniteStateMachine.quotient`,
do not create a new state, but make deepcopy + reset label in order to
preserve possible additional information of the state.

Keywords: **finite_state_machine**

Branch/Commit: **[c518d08](https://github.com/sagemath/sagetrac-mirror/commit/c518d0808336d148454d290c41ffefc3d5467bac)**

Reviewer: **Daniel Krenn**

Author: **Clemens Heuberger, Daniel Krenn**

_Issue created by migration from https://trac.sagemath.org/ticket/15847_





---

archive/issue_events_208803.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-02-23T13:38:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15847",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15847#event-208803"
}
```



---

archive/issue_events_208804.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-02-23T13:38:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15847",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15847#event-208804"
}
```



---

archive/issue_events_208805.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-02-23T13:38:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15847",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15847#event-208805"
}
```



---

archive/issue_events_208806.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-02-23T13:38:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15847",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15847#event-208806"
}
```



---

archive/issue_comments_203054.json:
```json
{
    "body": "Commit: **[c518d08](https://github.com/sagemath/sagetrac-mirror/commit/c518d0808336d148454d290c41ffefc3d5467bac)**",
    "created_at": "2014-03-12T14:28:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15847",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15847#issuecomment-203054",
    "user": "https://github.com/cheuberg"
}
```

Commit: **[c518d08](https://github.com/sagemath/sagetrac-mirror/commit/c518d0808336d148454d290c41ffefc3d5467bac)**



---

archive/issue_comments_203055.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1bc867f07d6c0fe07fa7313da89007990aea959c\">1bc867f</a></td><td><code>deepcopy + reset label in FiniteStateMachine.quotient</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/33442a32e7ce145eddf77515180a3c266cc43cc2\">33442a3</a></td><td><code>Removed Whitespace according PEP 8</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/709d3a4903862db94e27415323423cb0ed7c9651\">709d3a4</a></td><td><code>corrected spacings (PEP8); added line-breaks in long lines</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c518d0808336d148454d290c41ffefc3d5467bac\">c518d08</a></td><td><code>using relabeled instead of deepcopy (former TODO)</code></td></tr></table>\n",
    "created_at": "2014-03-12T14:28:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15847",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15847#issuecomment-203055",
    "user": "https://github.com/cheuberg"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1bc867f07d6c0fe07fa7313da89007990aea959c">1bc867f</a></td><td><code>deepcopy + reset label in FiniteStateMachine.quotient</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/33442a32e7ce145eddf77515180a3c266cc43cc2">33442a3</a></td><td><code>Removed Whitespace according PEP 8</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/709d3a4903862db94e27415323423cb0ed7c9651">709d3a4</a></td><td><code>corrected spacings (PEP8); added line-breaks in long lines</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c518d0808336d148454d290c41ffefc3d5467bac">c518d08</a></td><td><code>using relabeled instead of deepcopy (former TODO)</code></td></tr></table>




---

archive/issue_comments_203056.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,5 +2,3 @@\n `sage.combinat.finite_state_machine.FiniteStateMachine.quotient`,\n do not create a new state, but make deepcopy + reset label in order to\n preserve possible additional information of the state.\n-\n-I am preparing a patch, it will be pushed in a few days.\n``````\n",
    "created_at": "2014-03-12T14:28:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15847",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15847#issuecomment-203056",
    "user": "https://github.com/cheuberg"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,5 +2,3 @@
 `sage.combinat.finite_state_machine.FiniteStateMachine.quotient`,
 do not create a new state, but make deepcopy + reset label in order to
 preserve possible additional information of the state.
-
-I am preparing a patch, it will be pushed in a few days.
``````




---

archive/issue_comments_203057.json:
```json
{
    "body": "Branch: **[u/cheuberg/fsm/quotient-deepcopy-state](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/quotient-deepcopy-state)**",
    "created_at": "2014-03-12T14:28:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15847",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15847#issuecomment-203057",
    "user": "https://github.com/cheuberg"
}
```

Branch: **[u/cheuberg/fsm/quotient-deepcopy-state](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/quotient-deepcopy-state)**



---

archive/issue_comments_203058.json:
```json
{
    "body": "Changed author from **Clemens Heuberger** to **Clemens Heuberger, Daniel Krenn**",
    "created_at": "2014-03-12T14:28:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15847",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15847#issuecomment-203058",
    "user": "https://github.com/cheuberg"
}
```

Changed author from **Clemens Heuberger** to **Clemens Heuberger, Daniel Krenn**



---

archive/issue_events_208807.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2014-03-12T14:28:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15847",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15847#event-208807"
}
```



---

archive/issue_comments_203059.json:
```json
{
    "body": "Reviewer: **Daniel Krenn**",
    "created_at": "2014-03-13T16:42:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15847",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15847#issuecomment-203059",
    "user": "https://github.com/dkrenn"
}
```

Reviewer: **Daniel Krenn**



---

archive/issue_events_208808.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2014-03-17T13:02:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15847",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15847#event-208808"
}
```



---

archive/issue_events_208809.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2014-03-17T13:02:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15847",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15847#event-208809"
}
```



---

archive/issue_comments_203060.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\ncode looks fine, doctests pass, docu checked, coverage checked",
    "created_at": "2014-03-18T09:48:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15847",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15847#issuecomment-203060",
    "user": "https://github.com/dkrenn"
}
```

<div id="comment:4" align="right">Comment 4</div>

code looks fine, doctests pass, docu checked, coverage checked



---

archive/issue_events_208810.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-03-20T20:07:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15847",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15847#event-208810"
}
```



---

archive/issue_events_208811.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "20c4867fb3b636e1630760b055c5f91ba748c06c",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-03-20T20:07:56Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15847",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15847#event-208811"
}
```



---

archive/issue_comments_203061.json:
```json
{
    "body": "Changed branch from **[u/cheuberg/fsm/quotient-deepcopy-state](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/quotient-deepcopy-state)** to **[c518d08](https://github.com/sagemath/sagetrac-mirror/commit/c518d0808336d148454d290c41ffefc3d5467bac)**",
    "created_at": "2014-03-20T20:07:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15847",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15847#issuecomment-203061",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/cheuberg/fsm/quotient-deepcopy-state](https://github.com/sagemath/sagetrac-mirror/tree/u/cheuberg/fsm/quotient-deepcopy-state)** to **[c518d08](https://github.com/sagemath/sagetrac-mirror/commit/c518d0808336d148454d290c41ffefc3d5467bac)**
