# Issue 15464: PIL still wants to use libjpeg when it cannot

archive/issues_015227.json:
```json
{
    "assignees": [],
    "body": "As in #7273, trying to install PIL on an ARM board failed because PIL found the jpeg library but there were no headers installed yielding an error:\n\n```\nlibImaging/Jpeg.h:11:21: error: jpeglib.h: No such file or directory\n```\n\nReviewer: **Volker Braun**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15464_\n\n",
    "closed_at": "2014-05-10T20:14:33Z",
    "created_at": "2013-11-28T13:54:01Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "PIL still wants to use libjpeg when it cannot",
    "type": "issue",
    "updated_at": "2014-05-10T20:14:33Z",
    "url": "https://github.com/sagemath/sage/issues/15464",
    "user": "https://github.com/sagetrac-jpflori"
}
```
As in #7273, trying to install PIL on an ARM board failed because PIL found the jpeg library but there were no headers installed yielding an error:

```
libImaging/Jpeg.h:11:21: error: jpeglib.h: No such file or directory
```

Reviewer: **Volker Braun**

_Issue created by migration from https://trac.sagemath.org/ticket/15464_





---

archive/issue_events_202853.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2013-11-28T13:54:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15464",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15464#event-202853"
}
```



---

archive/issue_events_202854.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2013-11-28T13:54:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15464",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15464#event-202854"
}
```



---

archive/issue_events_202855.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2013-11-28T13:54:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15464",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15464#event-202855"
}
```



---

archive/issue_events_202856.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2013-11-28T13:54:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15464",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15464#event-202856"
}
```



---

archive/issue_comments_195181.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nBy the way, we ship 1.1.6 and 1.1.7 is released:\n* http://www.pythonware.com/products/pil/#pil117",
    "created_at": "2013-11-28T13:55:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15464",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15464#issuecomment-195181",
    "user": "https://github.com/sagetrac-jpflori"
}
```

<a id='comment:1'>Comment 1:</a>
By the way, we ship 1.1.6 and 1.1.7 is released:
* http://www.pythonware.com/products/pil/#pil117



---

archive/issue_comments_195182.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nThis may be fixed by #15539.",
    "created_at": "2013-12-25T15:54:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15464",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15464#issuecomment-195182",
    "user": "https://github.com/sagetrac-jpflori"
}
```

<a id='comment:2'>Comment 2:</a>
This may be fixed by #15539.



---

archive/issue_events_202857.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15464",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15464#event-202857"
}
```



---

archive/issue_events_202858.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15464",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15464#event-202858"
}
```



---

archive/issue_events_202859.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-04-28T20:06:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15464",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15464#event-202859"
}
```



---

archive/issue_comments_195183.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2014-04-28T20:06:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15464",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15464#issuecomment-195183",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_events_202860.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-04-28T20:06:55Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15464",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15464#event-202860"
}
```



---

archive/issue_events_202861.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-04-28T20:06:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15464",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15464#event-202861"
}
```



---

archive/issue_events_202862.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-04-28T20:06:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15464",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15464#event-202862"
}
```



---

archive/issue_comments_195184.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nWe don't use PIL any more",
    "created_at": "2014-04-28T20:06:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15464",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15464#issuecomment-195184",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:4'>Comment 4:</a>
We don't use PIL any more



---

archive/issue_events_202863.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-05-09T14:19:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15464",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15464#event-202863"
}
```



---

archive/issue_events_202864.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-05-09T14:19:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15464",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15464#event-202864"
}
```



---

archive/issue_events_202865.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-05-10T20:14:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15464",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "a6a6a6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15464#event-202865"
}
```



---

archive/issue_events_202866.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-05-10T20:14:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15464",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15464#event-202866"
}
```



---

archive/issue_events_202867.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-05-10T20:14:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15464",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15464#event-202867"
}
```
