# Issue 15763: Implement an inverse entropy function

archive/issues_015526.json:
```json
{
    "assignees": [],
    "body": "This patch implements an inverse entropy function.\n\nThere is no nice expression for the inverse, so we resort to root finding. For a value of `q` (the base of the logarithm), the function is strictly increasing in the interval `[0, 1-1/q]` and hence has a unique value in this interval.\n\nCC:  @dimpase\n\nBranch/Commit: **[u/ppurka/ticket/15763](https://github.com/sagemath/sagetrac-mirror/tree/u/ppurka/ticket/15763) @ [94ed2d7](https://github.com/sagemath/sagetrac-mirror/commit/94ed2d7329ed637532478778756047175cc9e51e)**\n\nReviewer: **Dmitrii Pasechnik**\n\nAuthor: **Punarbasu Purkayastha**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15763_\n\n",
    "closed_at": "2014-02-08T14:02:33Z",
    "created_at": "2014-01-29T17:08:06Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20coding%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Implement an inverse entropy function",
    "type": "issue",
    "updated_at": "2014-02-08T14:02:33Z",
    "url": "https://github.com/sagemath/sage/issues/15763",
    "user": "https://github.com/ppurka"
}
```
This patch implements an inverse entropy function.

There is no nice expression for the inverse, so we resort to root finding. For a value of `q` (the base of the logarithm), the function is strictly increasing in the interval `[0, 1-1/q]` and hence has a unique value in this interval.

CC:  @dimpase

Branch/Commit: **[u/ppurka/ticket/15763](https://github.com/sagemath/sagetrac-mirror/tree/u/ppurka/ticket/15763) @ [94ed2d7](https://github.com/sagemath/sagetrac-mirror/commit/94ed2d7329ed637532478778756047175cc9e51e)**

Reviewer: **Dmitrii Pasechnik**

Author: **Punarbasu Purkayastha**

_Issue created by migration from https://trac.sagemath.org/ticket/15763_





---

archive/issue_events_207551.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2014-01-29T17:08:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15763",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15763#event-207551"
}
```



---

archive/issue_events_207552.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2014-01-29T17:08:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15763",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20coding%20theory",
    "label_color": "0000ff",
    "label_name": "component: coding theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15763#event-207552"
}
```



---

archive/issue_events_207553.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2014-01-29T17:08:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15763",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15763#event-207553"
}
```



---

archive/issue_events_207554.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2014-01-29T17:08:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15763",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15763#event-207554"
}
```



---

archive/issue_comments_201122.json:
```json
{
    "body": "Branch: **[u/ppurka/ticket/15763](https://github.com/sagemath/sagetrac-mirror/tree/u/ppurka/ticket/15763)**",
    "created_at": "2014-01-29T17:10:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15763#issuecomment-201122",
    "user": "https://github.com/ppurka"
}
```

Branch: **[u/ppurka/ticket/15763](https://github.com/sagemath/sagetrac-mirror/tree/u/ppurka/ticket/15763)**



---

archive/issue_comments_201123.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nany reason for it not being ready for review?\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/94ed2d7329ed637532478778756047175cc9e51e\">94ed2d7</a></td><td><code>define an inverse (Shannon) entropy function</code></td></tr></table>\n",
    "created_at": "2014-01-30T09:48:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15763#issuecomment-201123",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:2" align="right">Comment 2</div>

any reason for it not being ready for review?

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/94ed2d7329ed637532478778756047175cc9e51e">94ed2d7</a></td><td><code>define an inverse (Shannon) entropy function</code></td></tr></table>




---

archive/issue_comments_201124.json:
```json
{
    "body": "Commit: **[94ed2d7](https://github.com/sagemath/sagetrac-mirror/commit/94ed2d7329ed637532478778756047175cc9e51e)**",
    "created_at": "2014-01-30T09:48:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15763#issuecomment-201124",
    "user": "https://github.com/dimpase"
}
```

Commit: **[94ed2d7](https://github.com/sagemath/sagetrac-mirror/commit/94ed2d7329ed637532478778756047175cc9e51e)**



---

archive/issue_comments_201125.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nReplying to [@dimpase](#comment%3A2):\n> any reason for it not being ready for review?\n\nI forgot to set it for review! Busy with other stuff..",
    "created_at": "2014-01-30T10:11:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15763#issuecomment-201125",
    "user": "https://github.com/ppurka"
}
```

<div id="comment:3" align="right">Comment 3</div>

Replying to [@dimpase](#comment%3A2):
> any reason for it not being ready for review?

I forgot to set it for review! Busy with other stuff..



---

archive/issue_events_207555.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2014-01-30T10:11:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15763",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15763#event-207555"
}
```



---

archive/issue_events_207556.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15763",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15763#event-207556"
}
```



---

archive/issue_events_207557.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15763",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15763#event-207557"
}
```



---

archive/issue_comments_201126.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nlooks good to me.",
    "created_at": "2014-02-02T19:41:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15763#issuecomment-201126",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:5" align="right">Comment 5</div>

looks good to me.



---

archive/issue_events_207558.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2014-02-02T19:41:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15763",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15763#event-207558"
}
```



---

archive/issue_events_207559.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2014-02-02T19:41:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15763",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15763#event-207559"
}
```



---

archive/issue_comments_201127.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2014-02-02T22:18:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15763#issuecomment-201127",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_comments_201128.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nDima.. you should fix your git username. Your hg user name was \"Dmitrii Pasechnik\" whereas your git one is \"Dima Pasechnik\". It will come up as a new contributor in sage releases.",
    "created_at": "2014-02-03T06:16:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15763#issuecomment-201128",
    "user": "https://github.com/ppurka"
}
```

<div id="comment:7" align="right">Comment 7</div>

Dima.. you should fix your git username. Your hg user name was "Dmitrii Pasechnik" whereas your git one is "Dima Pasechnik". It will come up as a new contributor in sage releases.



---

archive/issue_comments_201129.json:
```json
{
    "body": "Changed reviewer from **Dima Pasechnik** to **Dmitrii Pasechnik**",
    "created_at": "2014-02-03T06:16:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15763#issuecomment-201129",
    "user": "https://github.com/ppurka"
}
```

Changed reviewer from **Dima Pasechnik** to **Dmitrii Pasechnik**



---

archive/issue_events_207560.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-02-08T14:02:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15763",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15763#event-207560"
}
```



---

archive/issue_events_207561.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "ed168402503349db46e2cbdc8c55bf40f42db44d",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-02-08T14:02:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15763",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15763#event-207561"
}
```
