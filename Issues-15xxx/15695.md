# Issue 15695: Coercion problems between numpy and sage floats

archive/issues_015458.json:
```json
{
    "assignees": [],
    "body": "See this [sage-devel thread](https://groups.google.com/forum/#!topic/sage-devel/0IalWN4NHig).\nThis problem arises (at least on a lot of machines)\n\n```\nimport numpy as np\nsage: isinstance(np.float64(1),float)\nTrue\nsage: isinstance(np.float32(1),float)\nFalse\nsage: isinstance(np.float128(1),float)\nFalse\nsage: isinstance(np.float(1),float)\nTrue\nsage: type(np.float(1))\n<type 'float'>\nsage: parent(np.float64(1))\n<type 'numpy.float64'>\n```\nAs you can see, numpy decides to map `numpy.float64` a subclass of `float`. Sage's coercion code wasn't prepared for subclassing. This leads to awkward coercion problems:\n\n```\nsage: 1j + np.float64(2)\n2.0\n```\n(the result is of type np.float64 rather than CC, i.e., the wrong parent is chosen).\nThe simplest choice seems to be to ensure that sage tests for subtypes in the relevant spot.\n\nReviewer: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15695_\n\n",
    "closed_at": "2015-04-23T14:51:24Z",
    "created_at": "2014-01-19T17:28:47Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20coercion",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Coercion problems between numpy and sage floats",
    "type": "issue",
    "updated_at": "2015-04-23T14:51:24Z",
    "url": "https://github.com/sagemath/sage/issues/15695",
    "user": "https://github.com/nbruin"
}
```
See this [sage-devel thread](https://groups.google.com/forum/#!topic/sage-devel/0IalWN4NHig).
This problem arises (at least on a lot of machines)

```
import numpy as np
sage: isinstance(np.float64(1),float)
True
sage: isinstance(np.float32(1),float)
False
sage: isinstance(np.float128(1),float)
False
sage: isinstance(np.float(1),float)
True
sage: type(np.float(1))
<type 'float'>
sage: parent(np.float64(1))
<type 'numpy.float64'>
```
As you can see, numpy decides to map `numpy.float64` a subclass of `float`. Sage's coercion code wasn't prepared for subclassing. This leads to awkward coercion problems:

```
sage: 1j + np.float64(2)
2.0
```
(the result is of type np.float64 rather than CC, i.e., the wrong parent is chosen).
The simplest choice seems to be to ensure that sage tests for subtypes in the relevant spot.

Reviewer: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/15695_





---

archive/issue_events_206525.json:
```json
{
    "actor": "https://github.com/nbruin",
    "created_at": "2014-01-19T17:28:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15695",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15695#event-206525"
}
```



---

archive/issue_events_206526.json:
```json
{
    "actor": "https://github.com/nbruin",
    "created_at": "2014-01-19T17:28:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15695",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20coercion",
    "label_color": "0000ff",
    "label_name": "component: coercion",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15695#event-206526"
}
```



---

archive/issue_events_206527.json:
```json
{
    "actor": "https://github.com/nbruin",
    "created_at": "2014-01-19T17:28:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15695",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15695#event-206527"
}
```



---

archive/issue_events_206528.json:
```json
{
    "actor": "https://github.com/nbruin",
    "created_at": "2014-01-19T17:28:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15695",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15695#event-206528"
}
```



---

archive/issue_comments_200065.json:
```json
{
    "body": "Branch: **[u/nbruin/ticket/15695](https://github.com/sagemath/sagetrac-mirror/tree/u/nbruin/ticket/15695)**",
    "created_at": "2014-01-19T17:48:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15695",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15695#issuecomment-200065",
    "user": "https://github.com/nbruin"
}
```

Branch: **[u/nbruin/ticket/15695](https://github.com/sagemath/sagetrac-mirror/tree/u/nbruin/ticket/15695)**



---

archive/issue_comments_200066.json:
```json
{
    "body": "Commit: **[f07008b](https://github.com/sagemath/sagetrac-mirror/commit/f07008bd6f1e9b5a37a4d3eae874a5eac2815234)**",
    "created_at": "2014-01-19T17:52:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15695",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15695#issuecomment-200066",
    "user": "https://github.com/nbruin"
}
```

Commit: **[f07008b](https://github.com/sagemath/sagetrac-mirror/commit/f07008bd6f1e9b5a37a4d3eae874a5eac2815234)**



---

archive/issue_comments_200067.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nvery basic fix. Feel free to adapt. Don't forget to test (including performance regression. Python should be able to do a `issubtype` really quickly, but it will be slower than an identity test)\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f07008bd6f1e9b5a37a4d3eae874a5eac2815234\">f07008b</a></td><td><code>trac #15695: check for subtypes in py_scalar_type</code></td></tr></table>\n",
    "created_at": "2014-01-19T17:52:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15695",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15695#issuecomment-200067",
    "user": "https://github.com/nbruin"
}
```

<a id='comment:2'>Comment 2:</a>
very basic fix. Feel free to adapt. Don't forget to test (including performance regression. Python should be able to do a `issubtype` really quickly, but it will be slower than an identity test)

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f07008bd6f1e9b5a37a4d3eae874a5eac2815234">f07008b</a></td><td><code>trac #15695: check for subtypes in py_scalar_type</code></td></tr></table>




---

archive/issue_events_206529.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15695",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15695#event-206529"
}
```



---

archive/issue_events_206530.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15695",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15695#event-206530"
}
```



---

archive/issue_comments_200068.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nThis might be the right place to deal with #8426 (which now works as requested in that ticket but needs a regression test if that's indeed the behaviour we want to support).",
    "created_at": "2014-03-15T18:56:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15695",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15695#issuecomment-200068",
    "user": "https://github.com/mezzarobba"
}
```

<a id='comment:4'>Comment 4:</a>
This might be the right place to deal with #8426 (which now works as requested in that ticket but needs a regression test if that's indeed the behaviour we want to support).



---

archive/issue_events_206531.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15695",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15695#event-206531"
}
```



---

archive/issue_events_206532.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15695",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15695#event-206532"
}
```



---

archive/issue_events_206533.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15695",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15695#event-206533"
}
```



---

archive/issue_events_206534.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15695",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15695#event-206534"
}
```



---

archive/issue_comments_200069.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nHello,\n\nI propose to close this as duplicates because of #18076. With the branch applied\n\n```\nsage: import numpy\nsage: 1j + numpy.float64(2)\n2.00000000000000 + 1.00000000000000*I\nsage: parent(_)\nComplex Field with 53 bits of precision\n```\nBut #8824 is still an issue\n\n```\nsage: numpy.float64(2) + 1j\n(2+1j)\nsage: parent(_)\n<type 'numpy.complex128'>\n```\n\nSince as I copied some of the suggestions from the branch, I propose to set you as an author in #18076.\n\nVincent",
    "created_at": "2015-03-28T12:00:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15695",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15695#issuecomment-200069",
    "user": "https://github.com/videlec"
}
```

<a id='comment:7'>Comment 7:</a>
Hello,

I propose to close this as duplicates because of #18076. With the branch applied

```
sage: import numpy
sage: 1j + numpy.float64(2)
2.00000000000000 + 1.00000000000000*I
sage: parent(_)
Complex Field with 53 bits of precision
```
But #8824 is still an issue

```
sage: numpy.float64(2) + 1j
(2+1j)
sage: parent(_)
<type 'numpy.complex128'>
```

Since as I copied some of the suggestions from the branch, I propose to set you as an author in #18076.

Vincent



---

archive/issue_events_206535.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-23T12:00:21Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15695",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15695#event-206535"
}
```



---

archive/issue_events_206536.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-23T12:00:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15695",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15695#event-206536"
}
```



---

archive/issue_events_206537.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-23T12:00:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15695",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15695#event-206537"
}
```



---

archive/issue_events_206538.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-23T12:00:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15695",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15695#event-206538"
}
```



---

archive/issue_comments_200070.json:
```json
{
    "body": "Changed branch from **[u/nbruin/ticket/15695](https://github.com/sagemath/sagetrac-mirror/tree/u/nbruin/ticket/15695)** to none",
    "created_at": "2015-04-23T12:00:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15695",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15695#issuecomment-200070",
    "user": "https://github.com/jdemeyer"
}
```

Changed branch from **[u/nbruin/ticket/15695](https://github.com/sagemath/sagetrac-mirror/tree/u/nbruin/ticket/15695)** to none



---

archive/issue_comments_200071.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2015-04-23T12:00:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15695",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15695#issuecomment-200071",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_comments_200072.json:
```json
{
    "body": "Changed commit from **[f07008b](https://github.com/sagemath/sagetrac-mirror/commit/f07008bd6f1e9b5a37a4d3eae874a5eac2815234)** to none",
    "created_at": "2015-04-23T12:00:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15695",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15695#issuecomment-200072",
    "user": "https://github.com/jdemeyer"
}
```

Changed commit from **[f07008b](https://github.com/sagemath/sagetrac-mirror/commit/f07008bd6f1e9b5a37a4d3eae874a5eac2815234)** to none



---

archive/issue_events_206539.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-23T12:00:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15695",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15695#event-206539"
}
```



---

archive/issue_events_206540.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-04-23T12:00:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15695",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15695#event-206540"
}
```



---

archive/issue_events_206541.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-23T14:51:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15695",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15695#event-206541"
}
```



---

archive/issue_events_206542.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-23T14:51:24Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15695",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15695#event-206542"
}
```



---

archive/issue_events_206543.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-23T14:51:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15695",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15695#event-206543"
}
```
