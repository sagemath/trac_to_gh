# Issue 15075: Allow SyntaxErrors from IPython InputTransformer

archive/issues_014838.json:
```json
{
    "body": "I've submitted a pull request to IPython that would allow us to raise SyntaxErrors properly instead of the hack at #14961. See https://github.com/ipython/ipython/pull/4089\n\n\nCC:  @tscrim @jasongrout\n\nUpstream: Workaround found; Bug reported upstream.\n\nDependencies: #14961\n\nIssue created by migration from https://trac.sagemath.org/ticket/15075\n\n",
    "created_at": "2013-08-21T21:15:50Z",
    "labels": [
        "component: user interface",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.4",
    "title": "Allow SyntaxErrors from IPython InputTransformer",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/15075",
    "user": "https://github.com/vbraun"
}
```
I've submitted a pull request to IPython that would allow us to raise SyntaxErrors properly instead of the hack at #14961. See https://github.com/ipython/ipython/pull/4089


CC:  @tscrim @jasongrout

Upstream: Workaround found; Bug reported upstream.

Dependencies: #14961

Issue created by migration from https://trac.sagemath.org/ticket/15075





---

archive/issue_events_043365.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15075",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15075#event-43365"
}
```



---

archive/issue_events_043366.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15075",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15075#event-43366"
}
```



---

archive/issue_events_043367.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15075",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15075#event-43367"
}
```



---

archive/issue_comments_189015.json:
```json
{
    "body": "<a id='comment:3'></a>Not sure if that is related, but the following syntax error is not caught by #14961 and makes sage 6.2 crash:\n\n```python\nR.<x> = #crashes when pressing <Enter>\n```\n\nThe crash does not happen if there is no space after the equal sign though.",
    "created_at": "2014-07-29T12:49:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15075",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15075#issuecomment-189015",
    "user": "https://trac.sagemath.org/admin/accounts/users/gmoroz"
}
```

<a id='comment:3'></a>Not sure if that is related, but the following syntax error is not caught by #14961 and makes sage 6.2 crash:

```python
R.<x> = #crashes when pressing <Enter>
```

The crash does not happen if there is no space after the equal sign though.



---

archive/issue_comments_189016.json:
```json
{
    "body": "<a id='comment:4'></a>I made #16731 for this issue.",
    "created_at": "2014-07-29T18:45:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15075",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15075#issuecomment-189016",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:4'></a>I made #16731 for this issue.



---

archive/issue_events_043368.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15075",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15075#event-43368"
}
```



---

archive/issue_events_043369.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15075",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15075#event-43369"
}
```
