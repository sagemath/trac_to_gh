# Issue 15534: Corrupt pickle in cluster_seed.py

archive/issues_015297.json:
```json
{
    "assignees": [],
    "body": "The following is an unlikely but occasional error on the buildbot (on \"mod\"), see http://build.sagemath.org/sage/builders/%20fast%20UW%20mod%20%28Ubuntu%208-64%29%20incremental/builds/0/steps/shell_3/logs/stdio\n\nThe problem seems to be that the pickle hasn't been flushed to disc before trying to read it, e.g. http://stackoverflow.com/questions/1746825/python-valueerror-insecure-string-pickle\n\n```\nsage -t --long src/sage/combinat/cluster_algebra_quiver/cluster_seed.py\n**********************************************************************\nFile \"src/sage/combinat/cluster_algebra_quiver/cluster_seed.py\", line 1847, in sage.combinat.cluster_algebra_quiver.cluster_seed.ClusterSeed.mutation_type\nFailed example:\n    S.mutation_type() # long time\nException raised:\n    Traceback (most recent call last):\n      File \"/mnt/SSD1/mod_buildslave/sage_git/build/local/lib/python2.7/site-packages/sage/doctest/forker.py\", line 480, in _run\n        self.execute(example, compiled, test.globs)\n      File \"/mnt/SSD1/mod_buildslave/sage_git/build/local/lib/python2.7/site-packages/sage/doctest/forker.py\", line 839, in execute\n        exec compiled in globs\n      File \"<doctest sage.combinat.cluster_algebra_quiver.cluster_seed.ClusterSeed.mutation_type[7]>\", line 1, in <module>\n        S.mutation_type() # long time\n      File \"/mnt/SSD1/mod_buildslave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/cluster_algebra_quiver/cluster_seed.py\", line 1874, in mutation_type\n        self._mutation_type = self._quiver.mutation_type()\n      File \"/mnt/SSD1/mod_buildslave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/cluster_algebra_quiver/quiver.py\", line 855, in mutation_type\n        mut_type_part = _mutation_type_from_data(dg_component.order(), dig6, compute_if_necessary=True)\n      File \"/mnt/SSD1/mod_buildslave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/cluster_algebra_quiver/mutation_type.py\", line 1243, in _mutation_type_from_data\n        data = load_data(n)\n      File \"cachefunc.pyx\", line 749, in sage.misc.cachefunc.CachedFunction.__call__ (sage/misc/cachefunc.c:3356)\n      File \"/mnt/SSD1/mod_buildslave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/cluster_algebra_quiver/mutation_type.py\", line 1214, in load_data\n        data_new = cPickle.load(f)\n    ValueError: insecure string pickle\n**********************************************************************\n```\n\nCC:  @simon-king-jena\n\nKeywords: **cluster algebras random_fail**\n\nBranch/Commit: **[7f98706](https://github.com/sagemath/sagetrac-mirror/commit/7f98706c1bdd93315276c60d6b7c275b3a21a213)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Volker Braun**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15534_\n\n",
    "closed_at": "2014-11-23T19:59:49Z",
    "created_at": "2013-12-17T18:51:44Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Corrupt pickle in cluster_seed.py",
    "type": "issue",
    "updated_at": "2014-11-23T19:59:49Z",
    "url": "https://github.com/sagemath/sage/issues/15534",
    "user": "https://github.com/vbraun"
}
```
The following is an unlikely but occasional error on the buildbot (on "mod"), see http://build.sagemath.org/sage/builders/%20fast%20UW%20mod%20%28Ubuntu%208-64%29%20incremental/builds/0/steps/shell_3/logs/stdio

The problem seems to be that the pickle hasn't been flushed to disc before trying to read it, e.g. http://stackoverflow.com/questions/1746825/python-valueerror-insecure-string-pickle

```
sage -t --long src/sage/combinat/cluster_algebra_quiver/cluster_seed.py
**********************************************************************
File "src/sage/combinat/cluster_algebra_quiver/cluster_seed.py", line 1847, in sage.combinat.cluster_algebra_quiver.cluster_seed.ClusterSeed.mutation_type
Failed example:
    S.mutation_type() # long time
Exception raised:
    Traceback (most recent call last):
      File "/mnt/SSD1/mod_buildslave/sage_git/build/local/lib/python2.7/site-packages/sage/doctest/forker.py", line 480, in _run
        self.execute(example, compiled, test.globs)
      File "/mnt/SSD1/mod_buildslave/sage_git/build/local/lib/python2.7/site-packages/sage/doctest/forker.py", line 839, in execute
        exec compiled in globs
      File "<doctest sage.combinat.cluster_algebra_quiver.cluster_seed.ClusterSeed.mutation_type[7]>", line 1, in <module>
        S.mutation_type() # long time
      File "/mnt/SSD1/mod_buildslave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/cluster_algebra_quiver/cluster_seed.py", line 1874, in mutation_type
        self._mutation_type = self._quiver.mutation_type()
      File "/mnt/SSD1/mod_buildslave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/cluster_algebra_quiver/quiver.py", line 855, in mutation_type
        mut_type_part = _mutation_type_from_data(dg_component.order(), dig6, compute_if_necessary=True)
      File "/mnt/SSD1/mod_buildslave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/cluster_algebra_quiver/mutation_type.py", line 1243, in _mutation_type_from_data
        data = load_data(n)
      File "cachefunc.pyx", line 749, in sage.misc.cachefunc.CachedFunction.__call__ (sage/misc/cachefunc.c:3356)
      File "/mnt/SSD1/mod_buildslave/sage_git/build/local/lib/python2.7/site-packages/sage/combinat/cluster_algebra_quiver/mutation_type.py", line 1214, in load_data
        data_new = cPickle.load(f)
    ValueError: insecure string pickle
**********************************************************************
```

CC:  @simon-king-jena

Keywords: **cluster algebras random_fail**

Branch/Commit: **[7f98706](https://github.com/sagemath/sagetrac-mirror/commit/7f98706c1bdd93315276c60d6b7c275b3a21a213)**

Reviewer: **Frédéric Chapoton**

Author: **Volker Braun**

_Issue created by migration from https://trac.sagemath.org/ticket/15534_





---

archive/issue_events_203937.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-12-17T18:51:44Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15534",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15534#event-203937"
}
```



---

archive/issue_events_203938.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-12-17T18:51:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15534",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20experimental",
    "label_color": "000080",
    "label_name": "component: packages: experimental",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15534#event-203938"
}
```



---

archive/issue_events_203939.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-12-17T18:51:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15534",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15534#event-203939"
}
```



---

archive/issue_events_203940.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-12-17T18:51:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15534",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15534#event-203940"
}
```



---

archive/issue_events_203941.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15534",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15534#event-203941"
}
```



---

archive/issue_events_203942.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15534",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15534#event-203942"
}
```



---

archive/issue_comments_196524.json:
```json
{
    "body": "Changed keywords from **** to **cluster algebras**",
    "created_at": "2014-03-07T21:24:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15534#issuecomment-196524",
    "user": "https://github.com/fchapoton"
}
```

Changed keywords from **** to **cluster algebras**



---

archive/issue_events_203943.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15534",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15534#event-203943"
}
```



---

archive/issue_events_203944.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15534",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15534#event-203944"
}
```



---

archive/issue_comments_196525.json:
```json
{
    "body": "Changed keywords from **cluster algebras** to **cluster algebras random_fail**",
    "created_at": "2014-07-25T05:00:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15534#issuecomment-196525",
    "user": "https://github.com/vbraun"
}
```

Changed keywords from **cluster algebras** to **cluster algebras random_fail**



---

archive/issue_events_203945.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15534",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15534#event-203945"
}
```



---

archive/issue_events_203946.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15534",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15534#event-203946"
}
```



---

archive/issue_events_203947.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-11-22T22:13:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15534",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20experimental",
    "label_color": "000080",
    "label_name": "component: packages: experimental",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15534#event-203947"
}
```



---

archive/issue_events_203948.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-11-22T22:13:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15534",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15534#event-203948"
}
```



---

archive/issue_comments_196526.json:
```json
{
    "body": "Branch: **[u/vbraun/corrupt_pickle_in_cluster_seed_py](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/corrupt_pickle_in_cluster_seed_py)**",
    "created_at": "2014-11-22T22:25:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15534#issuecomment-196526",
    "user": "https://github.com/vbraun"
}
```

Branch: **[u/vbraun/corrupt_pickle_in_cluster_seed_py](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/corrupt_pickle_in_cluster_seed_py)**



---

archive/issue_events_203949.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-11-22T22:28:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15534",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15534#event-203949"
}
```



---

archive/issue_comments_196527.json:
```json
{
    "body": "Author: **Volker Braun**",
    "created_at": "2014-11-22T22:28:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15534#issuecomment-196527",
    "user": "https://github.com/vbraun"
}
```

Author: **Volker Braun**



---

archive/issue_comments_196528.json:
```json
{
    "body": "Commit: **[7f98706](https://github.com/sagemath/sagetrac-mirror/commit/7f98706c1bdd93315276c60d6b7c275b3a21a213)**",
    "created_at": "2014-11-22T22:28:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15534#issuecomment-196528",
    "user": "https://github.com/vbraun"
}
```

Commit: **[7f98706](https://github.com/sagemath/sagetrac-mirror/commit/7f98706c1bdd93315276c60d6b7c275b3a21a213)**



---

archive/issue_comments_196529.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7f98706c1bdd93315276c60d6b7c275b3a21a213\">7f98706</a></td><td><code>Write file without race</code></td></tr></table>\n",
    "created_at": "2014-11-22T22:28:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15534#issuecomment-196529",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:8" align="right">Comment 8</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7f98706c1bdd93315276c60d6b7c275b3a21a213">7f98706</a></td><td><code>Write file without race</code></td></tr></table>




---

archive/issue_events_203950.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-11-23T07:39:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15534",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15534#event-203950"
}
```



---

archive/issue_events_203951.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-11-23T07:39:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15534",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15534#event-203951"
}
```



---

archive/issue_comments_196530.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2014-11-23T07:39:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15534#issuecomment-196530",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_196531.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nLooks good to me. Thanks, Volker.",
    "created_at": "2014-11-23T07:39:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15534#issuecomment-196531",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:9" align="right">Comment 9</div>

Looks good to me. Thanks, Volker.



---

archive/issue_comments_196532.json:
```json
{
    "body": "Changed branch from **[u/vbraun/corrupt_pickle_in_cluster_seed_py](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/corrupt_pickle_in_cluster_seed_py)** to **[7f98706](https://github.com/sagemath/sagetrac-mirror/commit/7f98706c1bdd93315276c60d6b7c275b3a21a213)**",
    "created_at": "2014-11-23T19:59:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15534#issuecomment-196532",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/vbraun/corrupt_pickle_in_cluster_seed_py](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/corrupt_pickle_in_cluster_seed_py)** to **[7f98706](https://github.com/sagemath/sagetrac-mirror/commit/7f98706c1bdd93315276c60d6b7c275b3a21a213)**



---

archive/issue_events_203952.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-11-23T19:59:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15534",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15534#event-203952"
}
```



---

archive/issue_events_203953.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "109a50cc921881097fd8adacf1fcb94568d2fa18",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-11-23T19:59:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15534",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15534#event-203953"
}
```
