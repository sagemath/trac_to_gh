# Issue 15985: Python 3 preparation: Fix implicit relative imports (from sibling modules)

archive/issues_015748.json:
```json
{
    "body": "Python 3 accepts only absolute imports and explicit relative imports. \nTo be Python 2 compatible all affected modules need `from __future__ import absolute_import`.\n\n`lib2to3/fixes/fix_import.py` does not add this __future__ import.\n\nTherefore we intend to use an enhanced fixer from `https://pypi.python.org/pypi/future/0.11.3`.\n\nThe latest development version also handles imports of pyx modules.\n\nChanges according to `libfuturize/fixes/fix_absolute_imports.py`:\n\n```\nIf spam is being imported from the local directory, this import:\n    from spam import eggs\nbecomes:\n    from .spam import eggs\n\nand this import:\n    import spam\nbecomes:\n    from . import spam\n```\n\nThis ticket is tracked as a dependency of meta-ticket ticket:15980.\n\nCC:  @tscrim @jm58660 @jdemeyer\n\nKeywords: python3\n\nReviewer: Jeroen Demeyer\n\nAuthor: Wilfried Luebbe\n\nBranch: e2f59c294fe0dd027b981e1945f21ae97bad6dfb\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/15985\n\n",
    "closed_at": "2016-07-27T20:24:57Z",
    "created_at": "2014-03-20T15:22:38Z",
    "labels": [
        "component: python3"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-7.3",
    "title": "Python 3 preparation: Fix implicit relative imports (from sibling modules)",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/15985",
    "user": "https://github.com/wluebbe"
}
```
Python 3 accepts only absolute imports and explicit relative imports. 
To be Python 2 compatible all affected modules need `from __future__ import absolute_import`.

`lib2to3/fixes/fix_import.py` does not add this __future__ import.

Therefore we intend to use an enhanced fixer from `https://pypi.python.org/pypi/future/0.11.3`.

The latest development version also handles imports of pyx modules.

Changes according to `libfuturize/fixes/fix_absolute_imports.py`:

```
If spam is being imported from the local directory, this import:
    from spam import eggs
becomes:
    from .spam import eggs

and this import:
    import spam
becomes:
    from . import spam
```

This ticket is tracked as a dependency of meta-ticket ticket:15980.

CC:  @tscrim @jm58660 @jdemeyer

Keywords: python3

Reviewer: Jeroen Demeyer

Author: Wilfried Luebbe

Branch: e2f59c294fe0dd027b981e1945f21ae97bad6dfb

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/15985





---

archive/issue_comments_258958.json:
```json
{
    "body": "Changing type from defect to enhancement.",
    "created_at": "2014-03-20T15:47:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258958",
    "user": "https://github.com/wluebbe"
}
```

Changing type from defect to enhancement.



---

archive/issue_comments_258959.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2014-04-01T09:32:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258959",
    "user": "https://github.com/wluebbe"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_258960.json:
```json
{
    "body": "Changing branch from \"\" to \"u/wluebbe/ticket/15985\"",
    "created_at": "2014-04-01T09:32:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258960",
    "user": "https://github.com/wluebbe"
}
```

Changing branch from "" to "u/wluebbe/ticket/15985"



---

archive/issue_comments_258961.json:
```json
{
    "body": "<a id='comment:2'></a>\nNew commits:\n|                                                                                                                                          |                                             |\n|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------|\n|[502ac5c](http://git.sagemath.org/sage.git/commit/?id=502ac5c98586116e74521bde30fb4c84edb507f5)|`local run of futurize -w -f absolute_import`|",
    "created_at": "2014-04-01T09:32:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258961",
    "user": "https://github.com/wluebbe"
}
```

<a id='comment:2'></a>
New commits:
|                                                                                                                                          |                                             |
|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------|
|[502ac5c](http://git.sagemath.org/sage.git/commit/?id=502ac5c98586116e74521bde30fb4c84edb507f5)|`local run of futurize -w -f absolute_import`|



---

archive/issue_comments_258962.json:
```json
{
    "body": "Changing commit from \"\" to \"502ac5c98586116e74521bde30fb4c84edb507f5\"",
    "created_at": "2014-04-01T09:32:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258962",
    "user": "https://github.com/wluebbe"
}
```

Changing commit from "" to "502ac5c98586116e74521bde30fb4c84edb507f5"



---

archive/issue_comments_258963.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2014-04-01T09:39:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258963",
    "user": "https://github.com/wluebbe"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_258964.json:
```json
{
    "body": "<a id='comment:3'></a>\nTicket needs work :-(\n\nmake failed while building html-doc:\n\n```\nbyte-compiling /home/wluebbe/sage-6.2.beta4/local/lib/python2.7/site-packages/sage/probability/random_variable.py to random_variable.pyc\nrunning install_egg_info\nRemoving /home/wluebbe/sage-6.2.beta4/local/lib/python2.7/site-packages/sage-6.2.beta5-py2.7.egg-info\nWriting /home/wluebbe/sage-6.2.beta4/local/lib/python2.7/site-packages/sage-6.2.beta5-py2.7.egg-info\n\nreal\t10m2.009s\nuser\t35m33.530s\nsys\t0m48.657s\n[ -f local/etc/sage-started.txt ] || local/bin/sage-starts\nbuild/pipestatus \"./sage --docbuild --no-pdf-links all html  2>&1\" \"tee -a logs/dochtml.log\"\nTraceback (most recent call last):\n  File \"/home/wluebbe/sage-6.2.beta4/src/doc/common/builder.py\", line 1465, in <module>\n    import sage.all\n  File \"/home/wluebbe/sage-6.2.beta4/local/lib/python2.7/site-packages/sage/all.py\", line 87, in <module>\n    from sage.misc.all       import *         # takes a while\n  File \"/home/wluebbe/sage-6.2.beta4/local/lib/python2.7/site-packages/sage/misc/all.py\", line 94, in <module>\n    from .functional import (additive_order,\n  File \"/home/wluebbe/sage-6.2.beta4/local/lib/python2.7/site-packages/sage/misc/functional.py\", line 37, in <module>\n    from sage.rings.complex_double import CDF\n  File \"integer.pxd\", line 9, in init sage.rings.complex_double (sage/rings/complex_double.c:17880)\n  File \"integer.pyx\", line 179, in init sage.rings.integer (sage/rings/integer.c:40148)\n  File \"/home/wluebbe/sage-6.2.beta4/local/lib/python2.7/site-packages/sage/rings/infinity.py\", line 203, in <module>\n    import sage.rings.rational\n  File \"fast_arith.pxd\", line 5, in init sage.rings.rational (sage/rings/rational.c:29096)\n  File \"fast_arith.pyx\", line 51, in init sage.rings.fast_arith (sage/rings/fast_arith.c:8036)\n  File \"integer_ring.pyx\", line 65, in init sage.rings.integer_ring (sage/rings/integer_ring.c:12457)\n  File \"/home/wluebbe/sage-6.2.beta4/local/lib/python2.7/site-packages/sage/rings/rational_field.py\", line 54, in <module>\n    from . import rational\nImportError: cannot import name rational\nmake: *** [doc-html] Error 1\n```\n\nThe problem occurs while trying to import the cython module `ration.pyx`. But `rational.so`is in the same subdir. So the explicit relative import should succeed.\n\n```\nwluebbe@Willis-Ubuntu:~/sage-6.2.beta4/local/lib/python2.7/site-packages/sage/rings$ ls -l ratio*\n-rw-r--r-- 1 wluebbe wluebbe   28433 Apr  1 10:58 rational_field.py\n-rw-r--r-- 1 wluebbe wluebbe   34240 Apr  1 11:08 rational_field.pyc\n-rwxr-xr-x 1 wluebbe wluebbe 1666692 Apr  1 11:06 rational.so\n```",
    "created_at": "2014-04-01T09:39:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258964",
    "user": "https://github.com/wluebbe"
}
```

<a id='comment:3'></a>
Ticket needs work :-(

make failed while building html-doc:

```
byte-compiling /home/wluebbe/sage-6.2.beta4/local/lib/python2.7/site-packages/sage/probability/random_variable.py to random_variable.pyc
running install_egg_info
Removing /home/wluebbe/sage-6.2.beta4/local/lib/python2.7/site-packages/sage-6.2.beta5-py2.7.egg-info
Writing /home/wluebbe/sage-6.2.beta4/local/lib/python2.7/site-packages/sage-6.2.beta5-py2.7.egg-info

real	10m2.009s
user	35m33.530s
sys	0m48.657s
[ -f local/etc/sage-started.txt ] || local/bin/sage-starts
build/pipestatus "./sage --docbuild --no-pdf-links all html  2>&1" "tee -a logs/dochtml.log"
Traceback (most recent call last):
  File "/home/wluebbe/sage-6.2.beta4/src/doc/common/builder.py", line 1465, in <module>
    import sage.all
  File "/home/wluebbe/sage-6.2.beta4/local/lib/python2.7/site-packages/sage/all.py", line 87, in <module>
    from sage.misc.all       import *         # takes a while
  File "/home/wluebbe/sage-6.2.beta4/local/lib/python2.7/site-packages/sage/misc/all.py", line 94, in <module>
    from .functional import (additive_order,
  File "/home/wluebbe/sage-6.2.beta4/local/lib/python2.7/site-packages/sage/misc/functional.py", line 37, in <module>
    from sage.rings.complex_double import CDF
  File "integer.pxd", line 9, in init sage.rings.complex_double (sage/rings/complex_double.c:17880)
  File "integer.pyx", line 179, in init sage.rings.integer (sage/rings/integer.c:40148)
  File "/home/wluebbe/sage-6.2.beta4/local/lib/python2.7/site-packages/sage/rings/infinity.py", line 203, in <module>
    import sage.rings.rational
  File "fast_arith.pxd", line 5, in init sage.rings.rational (sage/rings/rational.c:29096)
  File "fast_arith.pyx", line 51, in init sage.rings.fast_arith (sage/rings/fast_arith.c:8036)
  File "integer_ring.pyx", line 65, in init sage.rings.integer_ring (sage/rings/integer_ring.c:12457)
  File "/home/wluebbe/sage-6.2.beta4/local/lib/python2.7/site-packages/sage/rings/rational_field.py", line 54, in <module>
    from . import rational
ImportError: cannot import name rational
make: *** [doc-html] Error 1
```

The problem occurs while trying to import the cython module `ration.pyx`. But `rational.so`is in the same subdir. So the explicit relative import should succeed.

```
wluebbe@Willis-Ubuntu:~/sage-6.2.beta4/local/lib/python2.7/site-packages/sage/rings$ ls -l ratio*
-rw-r--r-- 1 wluebbe wluebbe   28433 Apr  1 10:58 rational_field.py
-rw-r--r-- 1 wluebbe wluebbe   34240 Apr  1 11:08 rational_field.pyc
-rwxr-xr-x 1 wluebbe wluebbe 1666692 Apr  1 11:06 rational.so
```



---

archive/issue_comments_258965.json:
```json
{
    "body": "<a id='comment:4'></a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n|                                                                                                                                          |                                             |\n|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------|\n|[cde9b0d](http://git.sagemath.org/sage.git/commit/?id=cde9b0dc8d749e32ce47ce34f3cc1131e4d3ef6e)|`local run of futurize -w -f absolute_import`|",
    "created_at": "2014-04-03T08:52:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258965",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:4'></a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
|                                                                                                                                          |                                             |
|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------|
|[cde9b0d](http://git.sagemath.org/sage.git/commit/?id=cde9b0dc8d749e32ce47ce34f3cc1131e4d3ef6e)|`local run of futurize -w -f absolute_import`|



---

archive/issue_comments_258966.json:
```json
{
    "body": "Changing commit from \"502ac5c98586116e74521bde30fb4c84edb507f5\" to \"cde9b0dc8d749e32ce47ce34f3cc1131e4d3ef6e\"",
    "created_at": "2014-04-03T08:52:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258966",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "502ac5c98586116e74521bde30fb4c84edb507f5" to "cde9b0dc8d749e32ce47ce34f3cc1131e4d3ef6e"



---

archive/issue_comments_258967.json:
```json
{
    "body": "<a id='comment:5'></a>\nRebased on 6.2.beta6 and resolved merge conflicts.",
    "created_at": "2014-04-03T08:53:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258967",
    "user": "https://github.com/wluebbe"
}
```

<a id='comment:5'></a>
Rebased on 6.2.beta6 and resolved merge conflicts.



---

archive/issue_comments_258968.json:
```json
{
    "body": "<a id='comment:6'></a>\nNew commits:\n|                                                                                                                                          |                                       |\n|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------|\n|[19692cb](http://git.sagemath.org/sage.git/commit/?id=19692cba6ef085f5563f4302f34d6cb5c9a281e5)|`imports: fix a few automated mistakes`|",
    "created_at": "2014-04-03T16:15:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258968",
    "user": "https://github.com/ohanar"
}
```

<a id='comment:6'></a>
New commits:
|                                                                                                                                          |                                       |
|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------|
|[19692cb](http://git.sagemath.org/sage.git/commit/?id=19692cba6ef085f5563f4302f34d6cb5c9a281e5)|`imports: fix a few automated mistakes`|



---

archive/issue_comments_258969.json:
```json
{
    "body": "Changing author from \"\" to \"Wilfried Luebbe\"",
    "created_at": "2014-04-03T16:15:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258969",
    "user": "https://github.com/ohanar"
}
```

Changing author from "" to "Wilfried Luebbe"



---

archive/issue_comments_258970.json:
```json
{
    "body": "Changing commit from \"cde9b0dc8d749e32ce47ce34f3cc1131e4d3ef6e\" to \"19692cba6ef085f5563f4302f34d6cb5c9a281e5\"",
    "created_at": "2014-04-03T16:15:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258970",
    "user": "https://github.com/ohanar"
}
```

Changing commit from "cde9b0dc8d749e32ce47ce34f3cc1131e4d3ef6e" to "19692cba6ef085f5563f4302f34d6cb5c9a281e5"



---

archive/issue_comments_258971.json:
```json
{
    "body": "Changing branch from \"u/wluebbe/ticket/15985\" to \"u/ohanar/relative_imports\"",
    "created_at": "2014-04-03T16:15:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258971",
    "user": "https://github.com/ohanar"
}
```

Changing branch from "u/wluebbe/ticket/15985" to "u/ohanar/relative_imports"



---

archive/issue_comments_258972.json:
```json
{
    "body": "<a id='comment:7'></a>\nSo it looks like the issue that you are getting into is the wonder that is affectionately referred to as Sage's import hell :).\n\nLooking at the traceback, it looks like the issue is a circular import that mysteriously works right now: `sage.rings.rational` imports something from `sage.rings.fast_arith` which imports something from `sage.rings.integer_ring` which imports something from `sage.rings.rational_field` which imports `sage.rings.rational`.\n\nThis is one of the big reasons why Sage is not truly usable as a python library, because the order in which you import Sage is incredibly delicate, and seems to magically work (and as you can see from this patch which shouldn't change anything, it truly is magical). There hasn't been much effort to fix this, but I would say that getting Python 3 working is a pretty big motivator.",
    "created_at": "2014-04-03T19:44:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258972",
    "user": "https://github.com/ohanar"
}
```

<a id='comment:7'></a>
So it looks like the issue that you are getting into is the wonder that is affectionately referred to as Sage's import hell :).

Looking at the traceback, it looks like the issue is a circular import that mysteriously works right now: `sage.rings.rational` imports something from `sage.rings.fast_arith` which imports something from `sage.rings.integer_ring` which imports something from `sage.rings.rational_field` which imports `sage.rings.rational`.

This is one of the big reasons why Sage is not truly usable as a python library, because the order in which you import Sage is incredibly delicate, and seems to magically work (and as you can see from this patch which shouldn't change anything, it truly is magical). There hasn't been much effort to fix this, but I would say that getting Python 3 working is a pretty big motivator.



---

archive/issue_comments_258973.json:
```json
{
    "body": "<a id='comment:8'></a>\nOne thing to try (which still might break) is removing all relative imports in favor of absolute imports, rather than just converting them to explicit relative imports.",
    "created_at": "2014-04-04T00:56:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258973",
    "user": "https://github.com/ohanar"
}
```

<a id='comment:8'></a>
One thing to try (which still might break) is removing all relative imports in favor of absolute imports, rather than just converting them to explicit relative imports.



---

archive/issue_comments_258974.json:
```json
{
    "body": "<a id='comment:9'></a>\nSome numbers:\nstandard library and implicit relative import\n(and 1 explicit relative import :-)|| about 5,000||\n|                   |        |\n|-------------------|--------|\n|lines with \"import\"| 26,3231|\n|- thereof lines with \"from sage\"| 19,581|\n|- thereof lines with \"import sage\"| 1,650|\n|leaving lines with imports from Python \n\nQuestions:\n* Is there agreement in the Sage community that absolute import is the way to go?\n* Can it help with the above `ImportError`? And similar ones?\n* Other benefits or drawbacks?",
    "created_at": "2014-04-04T08:29:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258974",
    "user": "https://github.com/wluebbe"
}
```

<a id='comment:9'></a>
Some numbers:
standard library and implicit relative import
(and 1 explicit relative import :-)|| about 5,000||
|                   |        |
|-------------------|--------|
|lines with "import"| 26,3231|
|- thereof lines with "from sage"| 19,581|
|- thereof lines with "import sage"| 1,650|
|leaving lines with imports from Python 

Questions:
* Is there agreement in the Sage community that absolute import is the way to go?
* Can it help with the above `ImportError`? And similar ones?
* Other benefits or drawbacks?



---

archive/issue_comments_258975.json:
```json
{
    "body": "<a id='comment:10'></a>\nAs your numbers indicate (and from my personal experience), there is definitely an unofficial convention to use absolute from..import statements in the sage library, i.e. `from sage.foo import bar`; however this is one of (if not the main) culprit behind the circular import issues of the Sage library. A lot of the circular import issues could be resolved by just importing a module, and not particular attributes of the module (i.e. using `import sage.foo` or `import sage.foo as bar` and using `sage.foo.baz`/`bar.baz` instead of `from sage.foo import baz`).",
    "created_at": "2014-04-04T13:57:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258975",
    "user": "https://github.com/ohanar"
}
```

<a id='comment:10'></a>
As your numbers indicate (and from my personal experience), there is definitely an unofficial convention to use absolute from..import statements in the sage library, i.e. `from sage.foo import bar`; however this is one of (if not the main) culprit behind the circular import issues of the Sage library. A lot of the circular import issues could be resolved by just importing a module, and not particular attributes of the module (i.e. using `import sage.foo` or `import sage.foo as bar` and using `sage.foo.baz`/`bar.baz` instead of `from sage.foo import baz`).



---

archive/issue_comments_258976.json:
```json
{
    "body": "<a id='comment:11'></a>\nCan I read this proposal as:\n\nDo the (absolute) imports as `import sage.somewhere.amodule as am` and then use it as `am.a_function(a_parm)`.\n\nPerhaps the module could recommend its \"standard\" abbreviation (here `am`). Such a convention might help while reading code that uses the module.",
    "created_at": "2014-04-04T14:42:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258976",
    "user": "https://github.com/wluebbe"
}
```

<a id='comment:11'></a>
Can I read this proposal as:

Do the (absolute) imports as `import sage.somewhere.amodule as am` and then use it as `am.a_function(a_parm)`.

Perhaps the module could recommend its "standard" abbreviation (here `am`). Such a convention might help while reading code that uses the module.



---

archive/issue_events_047010.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15985#event-47010"
}
```



---

archive/issue_events_047011.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15985#event-47011"
}
```



---

archive/issue_events_047012.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15985#event-47012"
}
```



---

archive/issue_events_047013.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-07-06T13:28:46Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15985#event-47013"
}
```



---

archive/issue_events_047014.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-07-06T13:28:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "milestone": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15985#event-47014"
}
```



---

archive/issue_comments_258977.json:
```json
{
    "body": "<a id='comment:14'></a>\nLet us do that chunk by chunk.\n\nSee #20958, #20960, #20955, #20945, #20947 for the first steps",
    "created_at": "2016-07-06T13:28:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258977",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:14'></a>
Let us do that chunk by chunk.

See #20958, #20960, #20955, #20945, #20947 for the first steps



---

archive/issue_comments_258978.json:
```json
{
    "body": "Changing component from distribution to python3.",
    "created_at": "2016-07-15T11:55:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258978",
    "user": "https://github.com/fchapoton"
}
```

Changing component from distribution to python3.



---

archive/issue_comments_258979.json:
```json
{
    "body": "<a id='comment:16'></a>\nAlmost done, last step is #21036\n\nthere just remains a small problem in src/sage/misc/sagedoc.py",
    "created_at": "2016-07-17T12:19:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258979",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:16'></a>
Almost done, last step is #21036

there just remains a small problem in src/sage/misc/sagedoc.py



---

archive/issue_comments_258980.json:
```json
{
    "body": "Changing branch from \"u/ohanar/relative_imports\" to \"public/15985\"",
    "created_at": "2016-07-19T12:14:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258980",
    "user": "https://github.com/fchapoton"
}
```

Changing branch from "u/ohanar/relative_imports" to "public/15985"



---

archive/issue_comments_258981.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2016-07-19T12:14:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258981",
    "user": "https://github.com/fchapoton"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_258982.json:
```json
{
    "body": "<a id='comment:17'></a>\nHere is the very small last step. Hopefully the doctests will pass.\n\nFor the reviewer: to check, see that (after #21036 is closed)\n\n`futurize -w -f absolute_import src/sage`\n\ndoes nothing.\n\n---\nNew commits:\n|                                                                                                                                          |                                          |\n|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------|\n|[e2f59c2](https://git.sagemath.org/sage.git/commit?id=e2f59c294fe0dd027b981e1945f21ae97bad6dfb)|`trac 15985 last old-style import in misc`|",
    "created_at": "2016-07-19T12:14:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258982",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:17'></a>
Here is the very small last step. Hopefully the doctests will pass.

For the reviewer: to check, see that (after #21036 is closed)

`futurize -w -f absolute_import src/sage`

does nothing.

---
New commits:
|                                                                                                                                          |                                          |
|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------|
|[e2f59c2](https://git.sagemath.org/sage.git/commit?id=e2f59c294fe0dd027b981e1945f21ae97bad6dfb)|`trac 15985 last old-style import in misc`|



---

archive/issue_comments_258983.json:
```json
{
    "body": "Changing commit from \"19692cba6ef085f5563f4302f34d6cb5c9a281e5\" to \"e2f59c294fe0dd027b981e1945f21ae97bad6dfb\"",
    "created_at": "2016-07-19T12:14:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258983",
    "user": "https://github.com/fchapoton"
}
```

Changing commit from "19692cba6ef085f5563f4302f34d6cb5c9a281e5" to "e2f59c294fe0dd027b981e1945f21ae97bad6dfb"



---

archive/issue_comments_258984.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2016-07-25T10:05:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258984",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_258985.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Jeroen Demeyer\"",
    "created_at": "2016-07-25T10:05:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258985",
    "user": "https://github.com/jdemeyer"
}
```

Changing reviewer from "" to "Jeroen Demeyer"



---

archive/issue_comments_258986.json:
```json
{
    "body": "<a id='comment:18'></a>\nIt's not really the last step, since there are still a lot of implicit relative imports in Cython files.",
    "created_at": "2016-07-25T10:05:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258986",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:18'></a>
It's not really the last step, since there are still a lot of implicit relative imports in Cython files.



---

archive/issue_events_047015.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-07-27T20:24:57Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15985#event-47015"
}
```



---

archive/issue_comments_258987.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2016-07-27T20:24:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258987",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_comments_258988.json:
```json
{
    "body": "Changing branch from \"public/15985\" to \"e2f59c294fe0dd027b981e1945f21ae97bad6dfb\"",
    "created_at": "2016-07-27T20:24:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258988",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "public/15985" to "e2f59c294fe0dd027b981e1945f21ae97bad6dfb"



---

archive/issue_comments_258989.json:
```json
{
    "body": "Changing commit from \"e2f59c294fe0dd027b981e1945f21ae97bad6dfb\" to \"\"",
    "created_at": "2017-04-14T09:28:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258989",
    "user": "https://github.com/jdemeyer"
}
```

Changing commit from "e2f59c294fe0dd027b981e1945f21ae97bad6dfb" to ""



---

archive/issue_comments_258990.json:
```json
{
    "body": "<a id='comment:20'></a>\nReplying to [jdemeyer](#comment%3A18):\n> It's not really the last step, since there are still a lot of implicit relative imports in Cython files.\n\n\nSee #22806 and #22808.",
    "created_at": "2017-04-14T09:28:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258990",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:20'></a>
Replying to [jdemeyer](#comment%3A18):
> It's not really the last step, since there are still a lot of implicit relative imports in Cython files.


See #22806 and #22808.



---

archive/issue_comments_258991.json:
```json
{
    "body": "<a id='comment:21'></a>\nReplying to [chapoton](#comment%3A17):\n> Here is the very small last step.\n\n\nHow did you determine that this is the \"last step\"? In other words, how did you determine which modules need to be fixed?\n\nThere are still plenty of `.py` and `.pyx` files which do not have `from __future__ import absolute_import`.",
    "created_at": "2017-04-14T09:38:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15985#issuecomment-258991",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:21'></a>
Replying to [chapoton](#comment%3A17):
> Here is the very small last step.


How did you determine that this is the "last step"? In other words, how did you determine which modules need to be fixed?

There are still plenty of `.py` and `.pyx` files which do not have `from __future__ import absolute_import`.
