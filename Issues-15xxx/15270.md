# Issue 15270: ATLAS build failure on Solaris

archive/issues_015033.json:
```json
{
    "assignees": [],
    "body": "We should just keep going if the upstream shared library build \"make shared\" fails. It uses hand-crafted makefiles, so it can't work in general.\n\nhttp://boxen.math.washington.edu/home/vbraun/spkg/atlas-3.10.1.p6.spkg\n\nKeywords: **atlas solaris spkg**\n\nUpstream: **Reported upstream. No feedback yet.**\n\nReviewer: **Jean-Pierre Flori**\n\nAuthor: **Volker Braun**\n\nMerged: **sage-5.13.beta1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15270_\n\n",
    "closed_at": "2013-10-19T09:21:16Z",
    "created_at": "2013-10-11T14:27:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.13",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "ATLAS build failure on Solaris",
    "type": "issue",
    "updated_at": "2013-10-19T09:21:16Z",
    "url": "https://github.com/sagemath/sage/issues/15270",
    "user": "https://github.com/vbraun"
}
```
We should just keep going if the upstream shared library build "make shared" fails. It uses hand-crafted makefiles, so it can't work in general.

http://boxen.math.washington.edu/home/vbraun/spkg/atlas-3.10.1.p6.spkg

Keywords: **atlas solaris spkg**

Upstream: **Reported upstream. No feedback yet.**

Reviewer: **Jean-Pierre Flori**

Author: **Volker Braun**

Merged: **sage-5.13.beta1**

_Issue created by migration from https://trac.sagemath.org/ticket/15270_





---

archive/issue_events_199707.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-10-11T14:27:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15270",
    "milestone_number": null,
    "milestone_title": "sage-5.13",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15270#event-199707"
}
```



---

archive/issue_events_199708.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-10-11T14:27:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15270",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "component: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15270#event-199708"
}
```



---

archive/issue_events_199709.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-10-11T14:27:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15270",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15270#event-199709"
}
```



---

archive/issue_events_199710.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-10-11T14:27:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15270",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15270#event-199710"
}
```



---

archive/issue_comments_190927.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n+We should just keep going if the upstream shared library build \"make shared\" fails. It uses hand-crafted makefiles, so it can't work in general.\n \n+http://boxen.math.washington.edu/home/vbraun/spkg/atlas-3.10.1.p6.spkg\n``````\n",
    "created_at": "2013-10-11T14:33:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15270",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15270#issuecomment-190927",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
+We should just keep going if the upstream shared library build "make shared" fails. It uses hand-crafted makefiles, so it can't work in general.
 
+http://boxen.math.washington.edu/home/vbraun/spkg/atlas-3.10.1.p6.spkg
``````




---

archive/issue_comments_190928.json:
```json
{
    "body": "Upstream: **Reported upstream. No feedback yet.**",
    "created_at": "2013-10-11T14:42:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15270",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15270#issuecomment-190928",
    "user": "https://github.com/vbraun"
}
```

Upstream: **Reported upstream. No feedback yet.**



---

archive/issue_comments_190929.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nUpstream report: https://sourceforge.net/p/math-atlas/support-requests/923/",
    "created_at": "2013-10-11T14:42:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15270",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15270#issuecomment-190929",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:2" align="right">Comment 2</div>

Upstream report: https://sourceforge.net/p/math-atlas/support-requests/923/



---

archive/issue_events_199711.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2013-10-11T17:25:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15270",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15270#event-199711"
}
```



---

archive/issue_comments_190930.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nWorks fine for me, looks good.\nThis is not yet needs_review, but I assume Volker just forgot to change the status.",
    "created_at": "2013-10-11T17:25:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15270",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15270#issuecomment-190930",
    "user": "https://github.com/sagetrac-jpflori"
}
```

<div id="comment:3" align="right">Comment 3</div>

Works fine for me, looks good.
This is not yet needs_review, but I assume Volker just forgot to change the status.



---

archive/issue_events_199712.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2013-10-11T17:25:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15270",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15270#event-199712"
}
```



---

archive/issue_events_199713.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2013-10-11T17:25:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15270",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15270#event-199713"
}
```



---

archive/issue_comments_190931.json:
```json
{
    "body": "Reviewer: **Jean-Pierre Flori**",
    "created_at": "2013-10-11T17:25:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15270",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15270#issuecomment-190931",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Reviewer: **Jean-Pierre Flori**



---

archive/issue_events_199714.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2013-10-11T17:25:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15270",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15270#event-199714"
}
```



---

archive/issue_events_199715.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2013-10-11T17:25:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15270",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15270#event-199715"
}
```



---

archive/issue_comments_190932.json:
```json
{
    "body": "Changed keywords from **** to **atlas solaris spkg**",
    "created_at": "2013-10-11T17:25:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15270",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15270#issuecomment-190932",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Changed keywords from **** to **atlas solaris spkg**



---

archive/issue_comments_190933.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nI was just building it to test, agree with positive review!",
    "created_at": "2013-10-11T18:16:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15270",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15270#issuecomment-190933",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:5" align="right">Comment 5</div>

I was just building it to test, agree with positive review!



---

archive/issue_comments_190934.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nUpdated spkg which fixes the solaris issue according to David Kirkby: https://groups.google.com/d/msg/sage-devel/AnvdhtSJ21E/k-SVmrzkuxsJ",
    "created_at": "2013-10-13T11:15:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15270",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15270#issuecomment-190934",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:6" align="right">Comment 6</div>

Updated spkg which fixes the solaris issue according to David Kirkby: https://groups.google.com/d/msg/sage-devel/AnvdhtSJ21E/k-SVmrzkuxsJ



---

archive/issue_comments_190935.json:
```json
{
    "body": "Merged: **sage-5.13.beta1**",
    "created_at": "2013-10-19T09:21:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15270",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15270#issuecomment-190935",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.13.beta1**



---

archive/issue_events_199716.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-19T09:21:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15270",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15270#event-199716"
}
```



---

archive/issue_events_199717.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-19T09:21:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15270",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15270#event-199717"
}
```
