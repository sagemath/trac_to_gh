...
```json
{
    "body": "<a id='comment:12'></a>\nI'm not so sure about UI issues here and elsewhere, either. I'm much in favor of those tickets proposing functions with keywords handling this, like `rewrite` or `expand(rules=\"sum\")`. Please see [symbolics](../wikis/symbolics)#simplifyexpandsubstituteandrelationequalitytickets for these.\n\nIf we stick to `expand` and `rewrite`, what about `expand_sum`?",
