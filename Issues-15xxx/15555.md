# Issue 15555: Implement the Murnaghan-Nakayama formula for character values of symmetric groups

archive/issues_015318.json:
```json
{
    "body": "CC:  @tscrim @darijgr sage-combinat @KPanComputes\n\nKeywords: partitions, character\n\nThe Murnaghan-Nakayama formula is a recursive formula for  a character value of the symmetric group (for the statement, look at the second paragraph of Mark Wildon's answer\u00a0http://mathoverflow.net/a/134620/9672\u00a0on mathoverflow, or Theorem 2.4.7 in *Representation Theory of the Symmetric Group* by James and Kerber).\n\nIssue created by migration from https://trac.sagemath.org/ticket/15555\n\n",
    "created_at": "2013-12-20T12:08:08Z",
    "labels": [
        "component: combinatorics"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.8",
    "title": "Implement the Murnaghan-Nakayama formula for character values of symmetric groups",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/15555",
    "user": "https://github.com/amritanshu-prasad"
}
```
CC:  @tscrim @darijgr sage-combinat @KPanComputes

Keywords: partitions, character

The Murnaghan-Nakayama formula is a recursive formula for  a character value of the symmetric group (for the statement, look at the second paragraph of Mark Wildon's answer http://mathoverflow.net/a/134620/9672 on mathoverflow, or Theorem 2.4.7 in *Representation Theory of the Symmetric Group* by James and Kerber).

Issue created by migration from https://trac.sagemath.org/ticket/15555





---

archive/issue_events_045450.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15555#event-45450"
}
```



---

archive/issue_events_045451.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15555#event-45451"
}
```



---

archive/issue_events_045452.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15555#event-45452"
}
```



---

archive/issue_events_045453.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15555#event-45453"
}
```



---

archive/issue_events_045454.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15555#event-45454"
}
```



---

archive/issue_comments_196856.json:
```json
{
    "body": "Changing keywords from \"partitions\" to \"partitions, days60\".",
    "created_at": "2014-08-17T11:41:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15555#issuecomment-196856",
    "user": "https://github.com/amritanshu-prasad"
}
```

Changing keywords from "partitions" to "partitions, days60".



---

archive/issue_comments_196857.json:
```json
{
    "body": "Changing keywords from \"partitions, days60\" to \"partitions, character\".",
    "created_at": "2019-11-18T16:12:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15555#issuecomment-196857",
    "user": "https://github.com/SmartestKen"
}
```

Changing keywords from "partitions, days60" to "partitions, character".



---

archive/issue_events_045455.json:
```json
{
    "actor": "https://github.com/SmartestKen",
    "created_at": "2019-11-18T16:12:11Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15555#event-45455"
}
```



---

archive/issue_comments_196858.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2019-11-18T16:12:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15555#issuecomment-196858",
    "user": "https://github.com/SmartestKen"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_196859.json:
```json
{
    "body": "New commits:",
    "created_at": "2019-11-18T16:12:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15555#issuecomment-196859",
    "user": "https://github.com/SmartestKen"
}
```

New commits:



---

archive/issue_comments_196860.json:
```json
{
    "body": "a couple of quick comments:\n\nthe 1st line of the docstring should be separated from the rest by an empty line, i.e.\nnot\n\n```\n+        r\"\"\"\n+        Return `\\chi^{\\lambda/\\mu}(\\alpha)` value in Murnaghan-Nakayama formula\n+        (Enumerative Combinatorics (Stanley) equation 7.75)\n...\n```\nbut\n\n```\n+        r\"\"\"\n+        Return `\\chi^{\\lambda/\\mu}(\\alpha)` value in Murnaghan-Nakayama formula\n+\n+        (Enumerative Combinatorics (Stanley) equation 7.75)\n...\n```\n\nThe reference should be made properly (by the way, which volume it is , I guess vol. 2?)\nwe already have it, so just use `[EnumComb2]_`\n\n---\n\nI am not sure that the recursive function should be public, and not an internal function in the non-recursive one. Also, the debugging flag should be a function parameter (default value `False`)",
    "created_at": "2019-11-21T00:59:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15555#issuecomment-196860",
    "user": "https://github.com/dimpase"
}
```

a couple of quick comments:

the 1st line of the docstring should be separated from the rest by an empty line, i.e.
not

```
+        r"""
+        Return `\chi^{\lambda/\mu}(\alpha)` value in Murnaghan-Nakayama formula
+        (Enumerative Combinatorics (Stanley) equation 7.75)
...
```
but

```
+        r"""
+        Return `\chi^{\lambda/\mu}(\alpha)` value in Murnaghan-Nakayama formula
+
+        (Enumerative Combinatorics (Stanley) equation 7.75)
...
```

The reference should be made properly (by the way, which volume it is , I guess vol. 2?)
we already have it, so just use `[EnumComb2]_`

---

I am not sure that the recursive function should be public, and not an internal function in the non-recursive one. Also, the debugging flag should be a function parameter (default value `False`)



---

archive/issue_comments_196861.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2019-11-21T01:00:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15555#issuecomment-196861",
    "user": "https://github.com/dimpase"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_events_045456.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2019-11-21T01:00:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "milestone": "sage-9.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15555#event-45456"
}
```



---

archive/issue_comments_196862.json:
```json
{
    "body": "Some additional comments:\n\nI don't see the point of making these static methods attached to `Partition`. That is not the natural place I would look for them. Furthermore, `get_chi` is a very non-descriptive name and needs to be changed. Actually, these might best as a normal method attached to `SkewPartition` (with a redirect in `Partition` with the skew being the empty partition).\n\nI agree with Dima that the `get_chi_recursive` should not be public since it is essentially an internal function.\n\nThe `ALGORITHM:` block is over-indented (this is because it only (as it should) has one colon).\n\nDo not return `None` if there is a check failure; raise an error message as intended.\n\nWhy are the keys for `refDict` strings and not the data as a tuple?\n\nPlease follow Python naming conventions (`lower_case` variable names). Also, try to avoid the extra blanklines.",
    "created_at": "2019-11-22T04:07:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15555#issuecomment-196862",
    "user": "https://github.com/tscrim"
}
```

Some additional comments:

I don't see the point of making these static methods attached to `Partition`. That is not the natural place I would look for them. Furthermore, `get_chi` is a very non-descriptive name and needs to be changed. Actually, these might best as a normal method attached to `SkewPartition` (with a redirect in `Partition` with the skew being the empty partition).

I agree with Dima that the `get_chi_recursive` should not be public since it is essentially an internal function.

The `ALGORITHM:` block is over-indented (this is because it only (as it should) has one colon).

Do not return `None` if there is a check failure; raise an error message as intended.

Why are the keys for `refDict` strings and not the data as a tuple?

Please follow Python naming conventions (`lower_case` variable names). Also, try to avoid the extra blanklines.



---

archive/issue_comments_196863.json:
```json
{
    "body": "Thanks for the comment.\n\nWorking on all issues Dima and tscrim mentioned. I have a few questions though.\n\n1. Are all the docstring rules the same for internal function? (i.e. Do I just copy/paste the entire docstring for the current get_chi_recursive())?\n2. The reason why I use static is \n    i. it seems unnatural to call something like lda.get_chi(mu, alpha). And user will have to make Partition Object first before they can call the function. \n    iii. I do not rely on any methods from Partition/Skewed-Partition Object and I cannot figure out how to extract the underlying list from a Partition Object (.get_part() does not look like a nice option...)\n3. By redirecting, you mean on the docstring or make a redirect in function call? Please teach me how to do if you mean docstring.",
    "created_at": "2019-11-22T05:25:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15555#issuecomment-196863",
    "user": "https://github.com/SmartestKen"
}
```

Thanks for the comment.

Working on all issues Dima and tscrim mentioned. I have a few questions though.

1. Are all the docstring rules the same for internal function? (i.e. Do I just copy/paste the entire docstring for the current get_chi_recursive())?
2. The reason why I use static is 
    i. it seems unnatural to call something like lda.get_chi(mu, alpha). And user will have to make Partition Object first before they can call the function. 
    iii. I do not rely on any methods from Partition/Skewed-Partition Object and I cannot figure out how to extract the underlying list from a Partition Object (.get_part() does not look like a nice option...)
3. By redirecting, you mean on the docstring or make a redirect in function call? Please teach me how to do if you mean docstring.



---

archive/issue_comments_196864.json:
```json
{
    "body": "by the way, did you look at ``@`CachedFunction` decorator, which provides memoization, so you don't need to carry around `refDict` at all?\n\nthis would make the code cleaner, and you could roll the two functions, recursive and non-recursive, into one.",
    "created_at": "2019-11-22T10:51:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15555#issuecomment-196864",
    "user": "https://github.com/dimpase"
}
```

by the way, did you look at ``@`CachedFunction` decorator, which provides memoization, so you don't need to carry around `refDict` at all?

this would make the code cleaner, and you could roll the two functions, recursive and non-recursive, into one.



---

archive/issue_events_045457.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2020-01-06T14:10:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "milestone": "sage-9.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15555#event-45457"
}
```



---

archive/issue_events_045458.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2020-01-06T14:10:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "milestone": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15555#event-45458"
}
```



---

archive/issue_comments_196865.json:
```json
{
    "body": "Ticket retargeted after milestone closed",
    "created_at": "2020-01-06T14:10:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15555#issuecomment-196865",
    "user": "https://github.com/embray"
}
```

Ticket retargeted after milestone closed



---

archive/issue_comments_196866.json:
```json
{
    "body": "Batch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.",
    "created_at": "2020-04-14T19:41:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15555#issuecomment-196866",
    "user": "https://github.com/mkoeppe"
}
```

Batch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.



---

archive/issue_events_045459.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "milestone": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15555#event-45459"
}
```



---

archive/issue_events_045460.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "milestone": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15555#event-45460"
}
```



---

archive/issue_events_045461.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-05T19:21:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "milestone": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15555#event-45461"
}
```



---

archive/issue_events_045462.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-05T19:21:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15555#event-45462"
}
```



---

archive/issue_events_045463.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15555#event-45463"
}
```



---

archive/issue_events_045464.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15555#event-45464"
}
```



---

archive/issue_comments_196867.json:
```json
{
    "body": "Setting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-02-13T20:51:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15555#issuecomment-196867",
    "user": "https://github.com/mkoeppe"
}
```

Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_045465.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T00:44:56Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15555#event-45465"
}
```



---

archive/issue_events_045466.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T00:44:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15555#event-45466"
}
```



---

archive/issue_comments_196868.json:
```json
{
    "body": "Setting a new milestone for this ticket based on a cursory review.",
    "created_at": "2021-07-19T00:44:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15555#issuecomment-196868",
    "user": "https://github.com/mkoeppe"
}
```

Setting a new milestone for this ticket based on a cursory review.



---

archive/issue_events_045467.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:24:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15555#event-45467"
}
```



---

archive/issue_events_045468.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:24:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15555#event-45468"
}
```



---

archive/issue_events_045469.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-02T20:27:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15555#event-45469"
}
```



---

archive/issue_events_045470.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-02T20:27:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15555#event-45470"
}
```



---

archive/issue_events_045471.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15555#event-45471"
}
```



---

archive/issue_events_045472.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15555",
    "milestone": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15555#event-45472"
}
```
