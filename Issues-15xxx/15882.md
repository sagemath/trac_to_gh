...
```json
{
    "body": "<a id='comment:1'></a>Starting to get there.\n\n---\nNew commits:",
    "created_at": "2014-03-01T18:51:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15882",
    "type": "issue_comment",
...
<a id='comment:1'></a>Starting to get there.
...
<a id='comment:2'></a>Thanks Travis! I haven't checked the code yet, but this will be a nice improvement.
...
<a id='comment:3'></a>How about we link them all to the category? For example, currently I do
...
```json
{
    "body": "<a id='comment:6'></a>Using #14275 makes deprecating things from the global namespace so much easier.",
    "created_at": "2014-03-17T15:08:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15882",
    "type": "issue_comment",
...
<a id='comment:6'></a>Using #14275 makes deprecating things from the global namespace so much easier.
...
```json
{
    "body": "<a id='comment:10'></a>Okay, now it should be ready.",
...
<a id='comment:10'></a>Okay, now it should be ready.
...
```json
{
    "body": "<a id='comment:11'></a>(being curious. I love catalogs)",
...
<a id='comment:11'></a>(being curious. I love catalogs)
...
```json
{
    "body": "<a id='comment:14'></a>Oh and I forgot something important : the new index of crystals must appear in the reference manual !\n\nNathann",
...
<a id='comment:14'></a>Oh and I forgot something important : the new index of crystals must appear in the reference manual !
...
```json
{
    "body": "<a id='comment:18'></a>A lot of the changes look like simple line breaks. I strongly recommend to take these out of this ticket since this pollutes the real changes that were made and it does not enhance the code in any way. This will make it much easier to review as well!\n\nThanks,\n\nAnne",
...
<a id='comment:18'></a>A lot of the changes look like simple line breaks. I strongly recommend to take these out of this ticket since this pollutes the real changes that were made and it does not enhance the code in any way. This will make it much easier to review as well!
...
<a id='comment:19'></a>Helloooooo Anne !
...
<a id='comment:20'></a>Yoooo Travis !
...
<a id='comment:25'></a>I'm using `crystals.KirillovReshetikhin` as an entry point for the user on the front-end. All of the models from `crystals.kirillov_reshetikhin.<tab>` take a Cartan type, an `r`, and an `s` as input.
...
<a id='comment:26'></a>I still get doctest failures
...
```json
{
    "body": "<a id='comment:29'></a>Ack, I had forgotten about the categories. Also I added `regular_crystals.py` to the doc because we had forgotten about doing that when creating the category which fixes broken links in `classical_crystals.py` (in which I fixed some docstring formatting).",
...
<a id='comment:29'></a>Ack, I had forgotten about the categories. Also I added `regular_crystals.py` to the doc because we had forgotten about doing that when creating the category which fixes broken links in `classical_crystals.py` (in which I fixed some docstring formatting).
...
<a id='comment:36'></a>I've fixed the first one. However tab completion, `?`, and `??` all work fine for me. From the error message, it looks like something is messed up with your ipython (or, less likely, sphinx).
...
```json
{
    "body": "<a id='comment:38'></a>After running `make` the problem with the documentation disappeared. I will look further at the changes!",
...
<a id='comment:38'></a>After running `make` the problem with the documentation disappeared. I will look further at the changes!
...
```json
{
    "body": "<a id='comment:45'></a>Well, that was the only bad link I could find with an extensive grep campaign.",
...
<a id='comment:45'></a>Well, that was the only bad link I could find with an extensive grep campaign.
...
```json
{
    "body": "<a id='comment:48'></a>Looks good to me. Thanks Anne and Nathann for doing the review!",
    "created_at": "2014-04-13T03:41:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15882",
    "type": "issue_comment",
...
<a id='comment:48'></a>Looks good to me. Thanks Anne and Nathann for doing the review!
...
```json
{
    "body": "<a id='comment:51'></a>Sorry about that.",
    "created_at": "2014-04-14T14:59:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15882",
    "type": "issue_comment",
...
<a id='comment:51'></a>Sorry about that.
...
<a id='comment:55'></a>> Let's see how this is done in the other catalogs. Hmm, we have `graphs.ChvatalGraph`, but `groups.permutation.Dihedral`. Not super consistent ...
