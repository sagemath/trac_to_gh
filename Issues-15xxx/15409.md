# Issue 15409: Simplify sage/symbolic cythonization.

archive/issues_015172.json:
```json
{
    "assignees": [],
    "body": "Glob-style module specification, inline library and language specifiers.\n\nBranch/Commit: **[u/robertwb/ticket/15409](https://github.com/sagemath/sagetrac-mirror/tree/u/robertwb/ticket/15409) @ [76ee864](https://github.com/sagemath/sagetrac-mirror/commit/76ee864428576db91a6421c88a86afd0f5436d0d)**\n\nReviewer: **Nathann Cohen**\n\nAuthor: **Robert Bradshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15409_\n\n",
    "closed_at": "2013-12-20T15:50:43Z",
    "created_at": "2013-11-13T08:14:58Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Simplify sage/symbolic cythonization.",
    "type": "issue",
    "updated_at": "2013-12-20T15:50:43Z",
    "url": "https://github.com/sagemath/sage/issues/15409",
    "user": "https://github.com/robertwb"
}
```
Glob-style module specification, inline library and language specifiers.

Branch/Commit: **[u/robertwb/ticket/15409](https://github.com/sagemath/sagetrac-mirror/tree/u/robertwb/ticket/15409) @ [76ee864](https://github.com/sagemath/sagetrac-mirror/commit/76ee864428576db91a6421c88a86afd0f5436d0d)**

Reviewer: **Nathann Cohen**

Author: **Robert Bradshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/15409_





---

archive/issue_events_201958.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2013-11-13T08:14:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15409",
    "milestone_number": null,
    "milestone_title": "sage-5.13",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15409#event-201958"
}
```



---

archive/issue_events_201959.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2013-11-13T08:14:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15409",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15409#event-201959"
}
```



---

archive/issue_events_201960.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2013-11-13T08:14:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15409",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15409#event-201960"
}
```



---

archive/issue_events_201961.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2013-11-13T08:14:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15409",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15409#event-201961"
}
```



---

archive/issue_comments_194139.json:
```json
{
    "body": "Branch: **[u/robertwb/ticket/15409](https://github.com/sagemath/sagetrac-mirror/tree/u/robertwb/ticket/15409)**",
    "created_at": "2013-11-13T08:16:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15409",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15409#issuecomment-194139",
    "user": "https://github.com/robertwb"
}
```

Branch: **[u/robertwb/ticket/15409](https://github.com/sagemath/sagetrac-mirror/tree/u/robertwb/ticket/15409)**



---

archive/issue_comments_194140.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td>[76ee864](https://github.com/sagemath/sagetrac-mirror/commit/76ee864)</td><td><code>Simplify sage/symbolic cythonization.</code></td></tr></table>\n",
    "created_at": "2013-11-13T08:16:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15409",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15409#issuecomment-194140",
    "user": "https://github.com/robertwb"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td>[76ee864](https://github.com/sagemath/sagetrac-mirror/commit/76ee864)</td><td><code>Simplify sage/symbolic cythonization.</code></td></tr></table>




---

archive/issue_comments_194141.json:
```json
{
    "body": "Commit: **[76ee864](https://github.com/sagemath/sagetrac-mirror/commit/76ee864428576db91a6421c88a86afd0f5436d0d)**",
    "created_at": "2013-11-13T08:16:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15409",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15409#issuecomment-194141",
    "user": "https://github.com/robertwb"
}
```

Commit: **[76ee864](https://github.com/sagemath/sagetrac-mirror/commit/76ee864428576db91a6421c88a86afd0f5436d0d)**



---

archive/issue_events_201962.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2013-11-13T08:16:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15409",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15409#event-201962"
}
```



---

archive/issue_comments_194142.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nI *LOVE* this patch. Even though it looks like it can't be used on the numerical/ folder, as some things there can only be compiled if some optional spkg have been installed first. *Sigh* `;-)`\n\nNathann",
    "created_at": "2013-11-13T15:26:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15409",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15409#issuecomment-194142",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:3" align="right">Comment 3</div>

I *LOVE* this patch. Even though it looks like it can't be used on the numerical/ folder, as some things there can only be compiled if some optional spkg have been installed first. *Sigh* `;-)`

Nathann



---

archive/issue_events_201963.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-11-13T15:26:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15409",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15409#event-201963"
}
```



---

archive/issue_events_201964.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-11-13T15:26:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15409",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15409#event-201964"
}
```



---

archive/issue_comments_194143.json:
```json
{
    "body": "Reviewer: **Nathann Cohen**",
    "created_at": "2013-11-13T15:26:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15409",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15409#issuecomment-194143",
    "user": "https://github.com/nathanncohen"
}
```

Reviewer: **Nathann Cohen**



---

archive/issue_events_201965.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-11-13T16:33:36Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15409",
    "milestone_number": null,
    "milestone_title": "sage-5.13",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15409#event-201965"
}
```



---

archive/issue_events_201966.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-11-13T16:33:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15409",
    "milestone_number": null,
    "milestone_title": "sage-6.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15409#event-201966"
}
```



---

archive/issue_comments_194144.json:
```json
{
    "body": "Author: **Robert Bradshaw**",
    "created_at": "2013-11-14T11:05:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15409",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15409#issuecomment-194144",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Robert Bradshaw**



---

archive/issue_events_201967.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2013-12-17T18:39:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15409",
    "milestone_number": null,
    "milestone_title": "sage-6.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15409#event-201967"
}
```



---

archive/issue_events_201968.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2013-12-17T18:39:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15409",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15409#event-201968"
}
```



---

archive/issue_events_201969.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-12-20T15:50:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15409",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15409#event-201969"
}
```



---

archive/issue_events_201970.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "ab32c070bf0045640ecf1732150a6d1e371ebf03",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2013-12-20T15:50:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15409",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15409#event-201970"
}
```
