# Issue 15944: Thematic tutorial about real and complex numbers in Sage

archive/issues_015707.json:
```json
{
    "body": "This tickets implements a thematic tutorial about the different representations of real numbers in Sage, and when it is convenient to use them.\n\nThe doc is not yet finished, but should be quite soon.\n\n\nCC:  @kcrisman\n\nKeywords: real number\n\nAuthor: Thierry Monteil\n\nIssue created by migration from https://trac.sagemath.org/ticket/15944\n\n",
    "created_at": "2014-03-15T12:36:06Z",
    "labels": [
        "component: documentation"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-wishlist",
    "title": "Thematic tutorial about real and complex numbers in Sage",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/15944",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```
This tickets implements a thematic tutorial about the different representations of real numbers in Sage, and when it is convenient to use them.

The doc is not yet finished, but should be quite soon.


CC:  @kcrisman

Keywords: real number

Author: Thierry Monteil

Issue created by migration from https://trac.sagemath.org/ticket/15944





---

archive/issue_events_046887.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15944",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15944#event-46887"
}
```



---

archive/issue_events_046888.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15944",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15944#event-46888"
}
```



---

archive/issue_events_046889.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15944",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15944#event-46889"
}
```



---

archive/issue_comments_203065.json:
```json
{
    "body": "Attachment [real_and_complex_numbers.rst](tarball://root/attachments/some-uuid/ticket15944/real_and_complex_numbers.rst) by tmonteil created at 2018-01-05 02:42:22",
    "created_at": "2018-01-05T02:42:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15944",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15944#issuecomment-203065",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```

Attachment [real_and_complex_numbers.rst](tarball://root/attachments/some-uuid/ticket15944/real_and_complex_numbers.rst) by tmonteil created at 2018-01-05 02:42:22



---

archive/issue_comments_203066.json:
```json
{
    "body": "Attachment [real_and_complex_numbers.ipynb](tarball://root/attachments/some-uuid/ticket15944/real_and_complex_numbers.ipynb) by tmonteil created at 2018-01-05 02:42:34",
    "created_at": "2018-01-05T02:42:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15944",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15944#issuecomment-203066",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```

Attachment [real_and_complex_numbers.ipynb](tarball://root/attachments/some-uuid/ticket15944/real_and_complex_numbers.ipynb) by tmonteil created at 2018-01-05 02:42:34



---

archive/issue_comments_203067.json:
```json
{
    "body": "<a id='comment:4'></a>Here is one of the versions of the tutorial. Let me attach files directly since they are not interacting with the rest of Sage source code, it will avoid useless rebase. There is still some work to do for an acceptable version. In particular, i would have liked to have a way to display exercises with solutions (see e.g. #22508). Also, Sage code within notes is not well rendered within ipynb files.",
    "created_at": "2018-01-05T02:48:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15944",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15944#issuecomment-203067",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```

<a id='comment:4'></a>Here is one of the versions of the tutorial. Let me attach files directly since they are not interacting with the rest of Sage source code, it will avoid useless rebase. There is still some work to do for an acceptable version. In particular, i would have liked to have a way to display exercises with solutions (see e.g. #22508). Also, Sage code within notes is not well rendered within ipynb files.



---

archive/issue_comments_203068.json:
```json
{
    "body": "<a id='comment:5'></a>You missed in your list `sage.rings.complex_mpc.MPComplexField(prec)` (that is using `mpc_t` from [the mpc library](http://www.multiprecision.org/index.php?prog=mpc).\n\nAnd I think that it would be good to list the upstream libraries (gmp, mpfr, mpir, arb, mpc, etc) and describe their data structures (`mpz_t`, `mpq_t`, `mpfr_t`, etc).",
    "created_at": "2018-01-10T10:02:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15944",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15944#issuecomment-203068",
    "user": "https://github.com/videlec"
}
```

<a id='comment:5'></a>You missed in your list `sage.rings.complex_mpc.MPComplexField(prec)` (that is using `mpc_t` from [the mpc library](http://www.multiprecision.org/index.php?prog=mpc).

And I think that it would be good to list the upstream libraries (gmp, mpfr, mpir, arb, mpc, etc) and describe their data structures (`mpz_t`, `mpq_t`, `mpfr_t`, etc).



---

archive/issue_comments_203069.json:
```json
{
    "body": "<a id='comment:6'></a>I think that a first version would already be more than welcome inside the list of thematic tutorials now! The tutorial can be improved in further tickets.",
    "created_at": "2018-01-10T10:04:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15944",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15944#issuecomment-203069",
    "user": "https://github.com/videlec"
}
```

<a id='comment:6'></a>I think that a first version would already be more than welcome inside the list of thematic tutorials now! The tutorial can be improved in further tickets.



---

archive/issue_comments_203070.json:
```json
{
    "body": "<a id='comment:7'></a>And in your table at the begining you do not mention `RealLazyField` (`RLF`). Is that on purpose?",
    "created_at": "2018-01-10T10:06:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15944",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15944#issuecomment-203070",
    "user": "https://github.com/videlec"
}
```

<a id='comment:7'></a>And in your table at the begining you do not mention `RealLazyField` (`RLF`). Is that on purpose?



---

archive/issue_comments_203071.json:
```json
{
    "body": "<a id='comment:8'></a>Replying to [comment:6 vdelecroix]:\n> I think that a first version would already be more than welcome inside the list of thematic tutorials now! The tutorial can be improved in further tickets.\n\n\nI agree, the problem is that the syntax for independent rst tutorial is not the same as for Sage thematic tutorial. Since we will have a workshop on experimental mathematics soon that might get some feedback by participants, let us plan the branch to the end of the month, so that there is no versionning issue. \n\nReplying to [comment:5 vdelecroix]:\n> You missed in your list `sage.rings.complex_mpc.MPComplexField(prec)` (that is using `mpc_t` from [the mpc library](http://www.multiprecision.org/index.php?prog=mpc).\n\n\nAs far as i understand, this interface is not used within Sage, since a custom representation with two MPFR is used instead. I am not sure whether it is for historical reasons, or if the Sage own implementation is faster, or what. Do you know the differences/advantages/drawbacks with Sage custom implementation ? It could be nice to explain that in the section about complex numbers, but i am not sure it should enter the table at the beginning.\n\n> And I think that it would be good to list the upstream libraries (gmp, mpfr, mpir, arb, mpc, etc) \n\n\nThanks for the suggestion, i will add a column about \"Upstream libraries\" in the table.\n\n> and describe their data structures (`mpz_t`, `mpq_t`, `mpfr_t`, etc).\n\n\nI fear that it is too much \"under the hood\" compared of the rest of the tutorial, since the end user is not supposed to touch that (or maybe i am wrong). While the main question of the tutorial is \"which representation to use for which purpose\", we could make a dedicated section about \"how to use them low-level\", but i do not want to mix that with the simpler examples, not to fear the newcomer.\n\nReplying to [comment:7 vdelecroix]:\n> And in your table at the begining you do not mention `RealLazyField` (`RLF`). Is that on purpose?\n\n\nYes, it was on purpose, like the real litterals, it is a \"transitional field\", but not something the user is supposed to define. This explains a dedicated section. Note that i did not mention cyclotomic fields either. I am not sure if the table is supposed to be exhaustive or if it should show the main representations.",
    "created_at": "2018-05-10T12:53:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15944",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15944#issuecomment-203071",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```

<a id='comment:8'></a>Replying to [comment:6 vdelecroix]:
> I think that a first version would already be more than welcome inside the list of thematic tutorials now! The tutorial can be improved in further tickets.


I agree, the problem is that the syntax for independent rst tutorial is not the same as for Sage thematic tutorial. Since we will have a workshop on experimental mathematics soon that might get some feedback by participants, let us plan the branch to the end of the month, so that there is no versionning issue. 

Replying to [comment:5 vdelecroix]:
> You missed in your list `sage.rings.complex_mpc.MPComplexField(prec)` (that is using `mpc_t` from [the mpc library](http://www.multiprecision.org/index.php?prog=mpc).


As far as i understand, this interface is not used within Sage, since a custom representation with two MPFR is used instead. I am not sure whether it is for historical reasons, or if the Sage own implementation is faster, or what. Do you know the differences/advantages/drawbacks with Sage custom implementation ? It could be nice to explain that in the section about complex numbers, but i am not sure it should enter the table at the beginning.

> And I think that it would be good to list the upstream libraries (gmp, mpfr, mpir, arb, mpc, etc) 


Thanks for the suggestion, i will add a column about "Upstream libraries" in the table.

> and describe their data structures (`mpz_t`, `mpq_t`, `mpfr_t`, etc).


I fear that it is too much "under the hood" compared of the rest of the tutorial, since the end user is not supposed to touch that (or maybe i am wrong). While the main question of the tutorial is "which representation to use for which purpose", we could make a dedicated section about "how to use them low-level", but i do not want to mix that with the simpler examples, not to fear the newcomer.

Replying to [comment:7 vdelecroix]:
> And in your table at the begining you do not mention `RealLazyField` (`RLF`). Is that on purpose?


Yes, it was on purpose, like the real litterals, it is a "transitional field", but not something the user is supposed to define. This explains a dedicated section. Note that i did not mention cyclotomic fields either. I am not sure if the table is supposed to be exhaustive or if it should show the main representations.



---

archive/issue_comments_203072.json:
```json
{
    "body": "<a id='comment:9'></a>Replying to [comment:8 tmonteil]:\n> Replying to [comment:6 vdelecroix]:\n> Replying to [comment:5 vdelecroix]:\n> > You missed in your list `sage.rings.complex_mpc.MPComplexField(prec)` (that is using `mpc_t` from [the mpc library](http://www.multiprecision.org/index.php?prog=mpc).\n\n> \n> As far as i understand, this interface is not used within Sage, since a custom representation with two MPFR is used instead. I am not sure whether it is for historical reasons, or if the Sage own implementation is faster, or what. Do you know the differences/advantages/drawbacks with Sage custom implementation ? It could be nice to explain that in the section about complex numbers, but i am not sure it should enter the table at the beginning.\n\n\nI think that using `mpc_t` (maintained by the MPFR team) is much reliable than an home made Cython version. If there is any missing feature we should just provide some patches to upstream. See in particular the on going discussion at #24489.\n\n> > and describe their data structures (`mpz_t`, `mpq_t`, `mpfr_t`, etc).\n\n> \n> I fear that it is too much \"under the hood\" compared of the rest of the tutorial, since the end user is not supposed to touch that (or maybe i am wrong). While the main question of the tutorial is \"which representation to use for which purpose\", we could make a dedicated section about \"how to use them low-level\", but i do not want to mix that with the simpler examples, not to fear the newcomer.\n\n\nYes and no. I think that it would make sense to have a section \"how to use C-functions on real numbers\". That can also be a \"more advanced\" tutorial on real numbers. That would also be the occasion to clean the various implementations.\n \n> Replying to [comment:7 vdelecroix]:\n> > And in your table at the begining you do not mention `RealLazyField` (`RLF`). Is that on purpose?\n\n> \n> Yes, it was on purpose, like the real litterals, it is a \"transitional field\", but not something the user is supposed to define. This explains a dedicated section. Note that i did not mention cyclotomic fields either. I am not sure if the table is supposed to be exhaustive or if it should show the main representations.\n\n\nIndeed, but from a technical perspective I think that RLF is actually something that should be used in constructions. For example, the QQ-module generated by 1 and pi should be something like\n\n```\nsage: QQ + QQ*pi\n```\nOr even more useful, number field + uniform random element in [0,1], etc.",
    "created_at": "2018-05-10T14:48:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15944",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15944#issuecomment-203072",
    "user": "https://github.com/videlec"
}
```

<a id='comment:9'></a>Replying to [comment:8 tmonteil]:
> Replying to [comment:6 vdelecroix]:
> Replying to [comment:5 vdelecroix]:
> > You missed in your list `sage.rings.complex_mpc.MPComplexField(prec)` (that is using `mpc_t` from [the mpc library](http://www.multiprecision.org/index.php?prog=mpc).

> 
> As far as i understand, this interface is not used within Sage, since a custom representation with two MPFR is used instead. I am not sure whether it is for historical reasons, or if the Sage own implementation is faster, or what. Do you know the differences/advantages/drawbacks with Sage custom implementation ? It could be nice to explain that in the section about complex numbers, but i am not sure it should enter the table at the beginning.


I think that using `mpc_t` (maintained by the MPFR team) is much reliable than an home made Cython version. If there is any missing feature we should just provide some patches to upstream. See in particular the on going discussion at #24489.

> > and describe their data structures (`mpz_t`, `mpq_t`, `mpfr_t`, etc).

> 
> I fear that it is too much "under the hood" compared of the rest of the tutorial, since the end user is not supposed to touch that (or maybe i am wrong). While the main question of the tutorial is "which representation to use for which purpose", we could make a dedicated section about "how to use them low-level", but i do not want to mix that with the simpler examples, not to fear the newcomer.


Yes and no. I think that it would make sense to have a section "how to use C-functions on real numbers". That can also be a "more advanced" tutorial on real numbers. That would also be the occasion to clean the various implementations.
 
> Replying to [comment:7 vdelecroix]:
> > And in your table at the begining you do not mention `RealLazyField` (`RLF`). Is that on purpose?

> 
> Yes, it was on purpose, like the real litterals, it is a "transitional field", but not something the user is supposed to define. This explains a dedicated section. Note that i did not mention cyclotomic fields either. I am not sure if the table is supposed to be exhaustive or if it should show the main representations.


Indeed, but from a technical perspective I think that RLF is actually something that should be used in constructions. For example, the QQ-module generated by 1 and pi should be something like

```
sage: QQ + QQ*pi
```
Or even more useful, number field + uniform random element in [0,1], etc.



---

archive/issue_events_046890.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-03T18:46:56Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15944",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15944#event-46890"
}
```



---

archive/issue_events_046891.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-03T18:46:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15944",
    "milestone": "sage-wishlist",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15944#event-46891"
}
```
