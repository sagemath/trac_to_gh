# Issue 15971: multivar polynomials over poly fraction fields don't simplify

archive/issues_015734.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: F=FractionField(PolynomialRing(QQ,'t'))\nsage: R=PolynomialRing(F,'x,y')\nsage: R.inject_variables() \nDefining x, y\nsage: x/x \nx/x\n```\nwhereas simpler fraction fields for F yield 1\n\nCC:  jakobkroeker\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15971_\n\n",
    "created_at": "2014-03-18T07:49:40Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "multivar polynomials over poly fraction fields don't simplify",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/15971",
    "user": "https://github.com/rwst"
}
```

```
sage: F=FractionField(PolynomialRing(QQ,'t'))
sage: R=PolynomialRing(F,'x,y')
sage: R.inject_variables() 
Defining x, y
sage: x/x 
x/x
```
whereas simpler fraction fields for F yield 1

CC:  jakobkroeker

_Issue created by migration from https://trac.sagemath.org/ticket/15971_





---

archive/issue_events_210391.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-03-18T07:49:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15971",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15971#event-210391"
}
```



---

archive/issue_events_210392.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-03-18T07:49:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15971",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
    "label_color": "0000ff",
    "label_name": "component: commutative algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15971#event-210392"
}
```



---

archive/issue_events_210393.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-03-18T07:49:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15971",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15971#event-210393"
}
```



---

archive/issue_events_210394.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-03-18T07:49:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15971",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15971#event-210394"
}
```



---

archive/issue_events_210395.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15971",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15971#event-210395"
}
```



---

archive/issue_events_210396.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15971",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15971#event-210396"
}
```



---

archive/issue_events_210397.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15971",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15971#event-210397"
}
```



---

archive/issue_events_210398.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15971",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15971#event-210398"
}
```



---

archive/issue_comments_204967.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nSeems fixed now. Needs a test, then review",
    "created_at": "2017-03-04T00:07:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15971",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15971#issuecomment-204967",
    "user": "https://github.com/sagetrac-jakobkroeker"
}
```

<a id='comment:3'>Comment 3:</a>
Seems fixed now. Needs a test, then review



---

archive/issue_events_210399.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15971",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15971#event-210399"
}
```
