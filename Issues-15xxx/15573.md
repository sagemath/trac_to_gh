...
```json
{
    "body": "<a id='comment:7'></a>I experimented a bit with running a patchbot; it chose this ticket and discovered a doctest failure in `hopf_algebras.py` due to an undefined name `left`.  Independently, Travis's comment:4 also seems to mean this needs work.",
...
<a id='comment:7'></a>I experimented a bit with running a patchbot; it chose this ticket and discovered a doctest failure in `hopf_algebras.py` due to an undefined name `left`.  Independently, Travis's comment:4 also seems to mean this needs work.
