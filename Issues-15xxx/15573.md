...
<a id='comment:4'></a>I think whatever is returned by `a_realization()` should have a (co)product/antipode/etc. methods implemented. If not, the code should call the "best" realization to do the computations in by overwriting the respective `*_by_coercion()` in either the realization category or in the parent directly (sometimes [often] the change-of-basis is nontrivial as well). Although this requirement that `a_realization()` has a concrete implementation of the operation methods (or it's elements) is not stated and it should be.
...
```json
{
    "body": "<a id='comment:7'></a>I experimented a bit with running a patchbot; it chose this ticket and discovered a doctest failure in `hopf_algebras.py` due to an undefined name `left`.  Independently, Travis's[comment:4](#comment%3A4) also seems to mean this needs work.",
...
<a id='comment:7'></a>I experimented a bit with running a patchbot; it chose this ticket and discovered a doctest failure in `hopf_algebras.py` due to an undefined name `left`.  Independently, Travis's[comment:4](#comment%3A4) also seems to mean this needs work.
...
<a id='comment:9'></a>> I think whatever is returned by `a_realization()` should have a (co)product/antipode/etc. methods implemented.
...
```json
{
    "body": "<a id='comment:11'></a>one failing doctest, see buildbot report",
...
<a id='comment:11'></a>one failing doctest, see buildbot report
...
<a id='comment:13'></a>Based on this comment in `hopf_algebra.py` inside antipode_by_coercion:
