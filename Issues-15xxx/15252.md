# Issue 15252: sage.tests.french_book.linsolve_doctest error on OS X 10.4

archive/issues_015015.json:
```json
{
    "assignees": [],
    "body": "On OS X 10.4 PPC G5:\n\n```\nsage -t --long devel/sage/sage/tests/french_book/linsolve_doctest.py\n**********************************************************************\nFile \"devel/sage/sage/tests/french_book/linsolve_doctest.py\", line 159, in sage.tests.french_book.linsolve_doctest\nFailed example:\n    racines = comp.eigenvalues(); racines\nExpected:\n    [0.347521510119 + 0.566550553398*I, 0.347521510119 - 0.566550553398*I,\n    0.345023776962 + 0.439908702386*I, 0.345023776962 - 0.439908702386*I,\n    -0.517257614325 + 0.512958206789*I, -0.517257614325 -\n    0.512958206789*I, -1.36699716455, -9.98357818097]\nGot:\n    [0.347521510119 + 0.566550553399*I, 0.347521510119 - 0.566550553399*I, 0.345023776962 + 0.439908702386*I, 0.345023776962 - 0.439908702386*I, -0.517257614325 + 0.512958206789*I, -0.517257614325 - 0.512958206789*I, -1.36699716455, -9.98357818097]\n**********************************************************************\n```\n\nCC:  @zimmermann6\n\nReviewer: **Volker Braun**\n\nAuthor: **Jeroen Demeyer**\n\nMerged: **sage-5.12.rc1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15252_\n\n",
    "closed_at": "2013-10-04T07:39:41Z",
    "created_at": "2013-10-03T08:42:14Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/doctest%20coverage",
        "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.12",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sage.tests.french_book.linsolve_doctest error on OS X 10.4",
    "type": "issue",
    "updated_at": "2013-10-04T07:39:41Z",
    "url": "https://github.com/sagemath/sage/issues/15252",
    "user": "https://github.com/jdemeyer"
}
```
On OS X 10.4 PPC G5:

```
sage -t --long devel/sage/sage/tests/french_book/linsolve_doctest.py
**********************************************************************
File "devel/sage/sage/tests/french_book/linsolve_doctest.py", line 159, in sage.tests.french_book.linsolve_doctest
Failed example:
    racines = comp.eigenvalues(); racines
Expected:
    [0.347521510119 + 0.566550553398*I, 0.347521510119 - 0.566550553398*I,
    0.345023776962 + 0.439908702386*I, 0.345023776962 - 0.439908702386*I,
    -0.517257614325 + 0.512958206789*I, -0.517257614325 -
    0.512958206789*I, -1.36699716455, -9.98357818097]
Got:
    [0.347521510119 + 0.566550553399*I, 0.347521510119 - 0.566550553399*I, 0.345023776962 + 0.439908702386*I, 0.345023776962 - 0.439908702386*I, -0.517257614325 + 0.512958206789*I, -0.517257614325 - 0.512958206789*I, -1.36699716455, -9.98357818097]
**********************************************************************
```

CC:  @zimmermann6

Reviewer: **Volker Braun**

Author: **Jeroen Demeyer**

Merged: **sage-5.12.rc1**

_Issue created by migration from https://trac.sagemath.org/ticket/15252_





---

archive/issue_events_199455.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-03T08:42:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15252",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15252#event-199455"
}
```



---

archive/issue_events_199456.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-03T08:42:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15252",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15252#event-199456"
}
```



---

archive/issue_events_199457.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-03T08:42:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15252",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15252#event-199457"
}
```



---

archive/issue_events_199458.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-03T08:42:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15252",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15252#event-199458"
}
```



---

archive/issue_comments_190677.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nAttachment: **[15252_french_book_linsolve.patch.gz](https://github.com/sagemath/sage/files/ticket15252/15252_french_book_linsolve.patch.gz)**",
    "created_at": "2013-10-03T08:46:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15252",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15252#issuecomment-190677",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

Attachment: **[15252_french_book_linsolve.patch.gz](https://github.com/sagemath/sage/files/ticket15252/15252_french_book_linsolve.patch.gz)**



---

archive/issue_events_199459.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-03T08:46:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15252",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15252#event-199459"
}
```



---

archive/issue_events_199460.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-10-03T12:32:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15252",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15252#event-199460"
}
```



---

archive/issue_events_199461.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-10-03T12:32:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15252",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15252#event-199461"
}
```



---

archive/issue_comments_190678.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2013-10-03T12:32:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15252",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15252#issuecomment-190678",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_comments_190679.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLooks good to me.",
    "created_at": "2013-10-03T12:32:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15252",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15252#issuecomment-190679",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:3" align="right">Comment 3</div>

Looks good to me.



---

archive/issue_comments_190680.json:
```json
{
    "body": "Merged: **sage-5.12.rc1**",
    "created_at": "2013-10-04T07:39:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15252",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15252#issuecomment-190680",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.12.rc1**



---

archive/issue_events_199462.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-04T07:39:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15252",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15252#event-199462"
}
```



---

archive/issue_events_199463.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-04T07:39:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15252",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15252#event-199463"
}
```
