# Issue 15603: "immutable=True" for Graph/Digraph __init__ and copy()

archive/issues_015366.json:
```json
{
    "body": "CC:  simonking\n\nAs promised on #15278, this ticket adds an 'immutable' keyword to the constructors of `Graph/Digraph`, and in 'copy()' too.\n\nWhile I was at it, I tried to clean a bit the 'copy()' method, which now systematically checks its input. It should deal with every situation `:-P`\n\nAnd I think I will have to clean the constructors of `Graph/Digraph` too at some point.\n\nNathann\n\nIssue created by migration from https://trac.sagemath.org/ticket/15603\n\n",
    "closed_at": "2014-01-05T00:32:08Z",
    "created_at": "2013-12-28T23:54:32Z",
    "labels": [
        "component: graph theory"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.1",
    "title": "\"immutable=True\" for Graph/Digraph __init__ and copy()",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/15603",
    "user": "https://github.com/nathanncohen"
}
```
CC:  simonking

As promised on #15278, this ticket adds an 'immutable' keyword to the constructors of `Graph/Digraph`, and in 'copy()' too.

While I was at it, I tried to clean a bit the 'copy()' method, which now systematically checks its input. It should deal with every situation `:-P`

And I think I will have to clean the constructors of `Graph/Digraph` too at some point.

Nathann

Issue created by migration from https://trac.sagemath.org/ticket/15603





---

archive/issue_comments_197897.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2013-12-28T23:59:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15603",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15603#issuecomment-197897",
    "user": "https://github.com/nathanncohen"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_197898.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2013-12-29T00:08:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15603",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15603#issuecomment-197898",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_197899.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2013-12-29T11:14:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15603",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15603#issuecomment-197899",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_197900.json:
```json
{
    "body": "What do you think ? `:-)`\n\nNathann",
    "created_at": "2013-12-29T15:43:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15603",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15603#issuecomment-197900",
    "user": "https://github.com/nathanncohen"
}
```

What do you think ? `:-)`

Nathann



---

archive/issue_comments_197901.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2013-12-29T15:44:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15603",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15603#issuecomment-197901",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_197902.json:
```json
{
    "body": "The commit looks good, except for the spaces in front of colons and question marks... Do you add this space in your smileys, too? ` :- ?` ` ; -)`\n\nI think we agreed that we'll use a *new* ticket to fix sage.combinat's hack with `_immutable=True` and our counter-hack with type-checking immutability. Of course, I first want to see if the tests pass before giving a positive review.",
    "created_at": "2013-12-29T16:08:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15603",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15603#issuecomment-197902",
    "user": "https://github.com/simon-king-jena"
}
```

The commit looks good, except for the spaces in front of colons and question marks... Do you add this space in your smileys, too? ` :- ?` ` ; -)`

I think we agreed that we'll use a *new* ticket to fix sage.combinat's hack with `_immutable=True` and our counter-hack with type-checking immutability. Of course, I first want to see if the tests pass before giving a positive review.



---

archive/issue_comments_197903.json:
```json
{
    "body": "Yoooooo !\n\n> The commit looks good, except for the spaces in front of colons and question marks... Do you add this space in your smileys, too? ` :- ?` ` ; -)`\n\n\nArgggggg `T_T`\n\nWell right now I'm trying to read a spanish book (even though I don't speak a word of it)... At least I don't put their weird reversed question marks in the code `:-PPP`\n\n> I think we agreed that we'll use a *new* ticket to fix sage.combinat's hack with `_immutable=True` and our counter-hack with type-checking immutability. Of course, I first want to see if the tests pass before giving a positive review.\n\n\nYepyepyep, sounds right.\n\nNathann",
    "created_at": "2013-12-29T16:13:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15603",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15603#issuecomment-197903",
    "user": "https://github.com/nathanncohen"
}
```

Yoooooo !

> The commit looks good, except for the spaces in front of colons and question marks... Do you add this space in your smileys, too? ` :- ?` ` ; -)`


Argggggg `T_T`

Well right now I'm trying to read a spanish book (even though I don't speak a word of it)... At least I don't put their weird reversed question marks in the code `:-PPP`

> I think we agreed that we'll use a *new* ticket to fix sage.combinat's hack with `_immutable=True` and our counter-hack with type-checking immutability. Of course, I first want to see if the tests pass before giving a positive review.


Yepyepyep, sounds right.

Nathann



---

archive/issue_comments_197904.json:
```json
{
    "body": "All tests pass, but I think I'll add a review commit, a bit later.",
    "created_at": "2013-12-29T19:15:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15603",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15603#issuecomment-197904",
    "user": "https://github.com/simon-king-jena"
}
```

All tests pass, but I think I'll add a review commit, a bit later.



---

archive/issue_comments_197905.json:
```json
{
    "body": "Things that need to be done (in a review commit): In some place you explain the `immutable=...` optional argument, but in the example you use `data_structure=...` only. In some other place, you do this change:\n\n```diff\n     If the ``data_structure`` is equal to ``\"static_sparse\"``, then an\n     immutable graph results. Note that this does not use the NetworkX data\n     structure::\n \n-          sage: G_imm = Graph(g, data_structure=\"static_sparse\")\n-          sage: H_imm = Graph(g, data_structure=\"static_sparse\")\n+          sage: G_imm = Graph(g, immutable=True)\n+          sage: H_imm = Graph(g, immutable=True)\n           sage: G_imm == H_imm == G == H\n           True\n```\nHence, the text is about data_structure, but you remove it from the test.",
    "created_at": "2014-01-03T10:05:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15603",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15603#issuecomment-197905",
    "user": "https://github.com/simon-king-jena"
}
```

Things that need to be done (in a review commit): In some place you explain the `immutable=...` optional argument, but in the example you use `data_structure=...` only. In some other place, you do this change:

```diff
     If the ``data_structure`` is equal to ``"static_sparse"``, then an
     immutable graph results. Note that this does not use the NetworkX data
     structure::
 
-          sage: G_imm = Graph(g, data_structure="static_sparse")
-          sage: H_imm = Graph(g, data_structure="static_sparse")
+          sage: G_imm = Graph(g, immutable=True)
+          sage: H_imm = Graph(g, immutable=True)
           sage: G_imm == H_imm == G == H
           True
```
Hence, the text is about data_structure, but you remove it from the test.



---

archive/issue_comments_197906.json:
```json
{
    "body": "\"that need to be done\" : will that be your review commit, or do you want me to do that ?\n\nNathann",
    "created_at": "2014-01-03T10:15:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15603",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15603#issuecomment-197906",
    "user": "https://github.com/nathanncohen"
}
```

"that need to be done" : will that be your review commit, or do you want me to do that ?

Nathann



---

archive/issue_comments_197907.json:
```json
{
    "body": "Replying to [comment:11 ncohen]:\n> \"that need to be done\" : will that be your review commit, or do you want me to do that ?\n\n\nI wrote \"that need to be done (in a review commit)\". Hence, I'll do it.",
    "created_at": "2014-01-03T10:20:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15603",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15603#issuecomment-197907",
    "user": "https://github.com/simon-king-jena"
}
```

Replying to [comment:11 ncohen]:
> "that need to be done" : will that be your review commit, or do you want me to do that ?


I wrote "that need to be done (in a review commit)". Hence, I'll do it.



---

archive/issue_comments_197908.json:
```json
{
    "body": "Oops sorry. I missed that, in my eagerness to see these patches in Sage `^^;`\n\nNathann",
    "created_at": "2014-01-03T10:22:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15603",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15603#issuecomment-197908",
    "user": "https://github.com/nathanncohen"
}
```

Oops sorry. I missed that, in my eagerness to see these patches in Sage `^^;`

Nathann



---

archive/issue_comments_197909.json:
```json
{
    "body": "There is a naked `NotImplementedError`:\n\n```\n            sage: g = DiGraph(graphs.PetersenGraph(), immutable=True)\n            sage: g.add_edge(\"Hey\", \"Heyyyyyyy\")\n            Traceback (most recent call last):\n            ...\n            NotImplementedError:\n```\nShould we care?",
    "created_at": "2014-01-03T10:28:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15603",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15603#issuecomment-197909",
    "user": "https://github.com/simon-king-jena"
}
```

There is a naked `NotImplementedError`:

```
            sage: g = DiGraph(graphs.PetersenGraph(), immutable=True)
            sage: g.add_edge("Hey", "Heyyyyyyy")
            Traceback (most recent call last):
            ...
            NotImplementedError:
```
Should we care?



---

archive/issue_comments_197910.json:
```json
{
    "body": "```\n            sage: g = graphs.PetersenGraph()\n            sage: g = DiGraph(g.edges(),immutable=False)\n            sage: g.add_edge(\"Hey\", \"Heyyyyyyy\")\n            sage: {g:1}[g]\n            TypeError: This graph is mutable, and thus not hashable. Create an immutable copy by `g.copy(data_structure='static_sparse')`\n```\nShould the error message mention the other (more intuitive) possibility `g.copy(immutable=True)`?",
    "created_at": "2014-01-03T10:32:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15603",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15603#issuecomment-197910",
    "user": "https://github.com/simon-king-jena"
}
```

```
            sage: g = graphs.PetersenGraph()
            sage: g = DiGraph(g.edges(),immutable=False)
            sage: g.add_edge("Hey", "Heyyyyyyy")
            sage: {g:1}[g]
            TypeError: This graph is mutable, and thus not hashable. Create an immutable copy by `g.copy(data_structure='static_sparse')`
```
Should the error message mention the other (more intuitive) possibility `g.copy(immutable=True)`?



---

archive/issue_comments_197911.json:
```json
{
    "body": "Hmmmm... We could add an empty function for `add_edge` in the backend indeed. Depending on where the immutable graph comes from, the user may not have any idea of why this function is not implemented. Something like what `add_vertex` does.\n\nYes to your other comment about the exception raised by `__hash__`.\n\nNathann",
    "created_at": "2014-01-03T10:37:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15603",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15603#issuecomment-197911",
    "user": "https://github.com/nathanncohen"
}
```

Hmmmm... We could add an empty function for `add_edge` in the backend indeed. Depending on where the immutable graph comes from, the user may not have any idea of why this function is not implemented. Something like what `add_vertex` does.

Yes to your other comment about the exception raised by `__hash__`.

Nathann



---

archive/issue_comments_197912.json:
```json
{
    "body": "Since I want this to get over with and since my family plans to have an excursion today, I prefer to only fix the error message raised by `__hash__`, but leave the `NotImplementedError` as it is.",
    "created_at": "2014-01-03T10:39:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15603",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15603#issuecomment-197912",
    "user": "https://github.com/simon-king-jena"
}
```

Since I want this to get over with and since my family plans to have an excursion today, I prefer to only fix the error message raised by `__hash__`, but leave the `NotImplementedError` as it is.



---

archive/issue_comments_197913.json:
```json
{
    "body": "Ahahahah. Okay no prob. I'll fix that later in another patch `;-)`\n\nNathann",
    "created_at": "2014-01-03T10:41:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15603",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15603#issuecomment-197913",
    "user": "https://github.com/nathanncohen"
}
```

Ahahahah. Okay no prob. I'll fix that later in another patch `;-)`

Nathann



---

archive/issue_comments_197914.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2014-01-03T11:20:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15603",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15603#issuecomment-197914",
    "user": "https://github.com/simon-king-jena"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_197915.json:
```json
{
    "body": "I have added a review commit and run all tests in src/sage/graphs without error. Positive review (unless your aren't happy with my review commit or find that surprisingly the new error message appears in other parts of Sage too).\n\n---\nNew commits:",
    "created_at": "2014-01-03T11:20:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15603",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15603#issuecomment-197915",
    "user": "https://github.com/simon-king-jena"
}
```

I have added a review commit and run all tests in src/sage/graphs without error. Positive review (unless your aren't happy with my review commit or find that surprisingly the new error message appears in other parts of Sage too).

---
New commits:



---

archive/issue_comments_197916.json:
```json
{
    "body": "NOnono that' s great ! Thanks  `:-)` \n\nNathann",
    "created_at": "2014-01-03T11:23:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15603",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15603#issuecomment-197916",
    "user": "https://github.com/nathanncohen"
}
```

NOnono that' s great ! Thanks  `:-)` 

Nathann



---

archive/issue_events_045675.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-01-05T00:32:08Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/15603",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15603#event-45675"
}
```



---

archive/issue_comments_197917.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2014-01-05T00:32:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15603",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15603#issuecomment-197917",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed
