# Issue 15173: Can not save graph plots to pdf file

archive/issues_014936.json:
```json
{
    "assignees": [],
    "body": "See this [sage-support post](https://groups.google.com/d/topic/sage-support/kog6xcb6mtQ/discussion). It is broken since sage-5.10. Example code\n\n```\nsage: g = graphs.PetersenGraph()\nsage: pic = g.plot()\nsage: pic.save('pete.pdf')\n...\nAssertionError:\n```\n\nReviewer: **Punarbasu Purkayastha**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15173_\n\n",
    "closed_at": "2013-10-05T09:36:12Z",
    "created_at": "2013-09-07T16:03:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Can not save graph plots to pdf file",
    "type": "issue",
    "updated_at": "2013-10-05T09:36:12Z",
    "url": "https://github.com/sagemath/sage/issues/15173",
    "user": "https://github.com/ppurka"
}
```
See this [sage-support post](https://groups.google.com/d/topic/sage-support/kog6xcb6mtQ/discussion). It is broken since sage-5.10. Example code

```
sage: g = graphs.PetersenGraph()
sage: pic = g.plot()
sage: pic.save('pete.pdf')
...
AssertionError:
```

Reviewer: **Punarbasu Purkayastha**

_Issue created by migration from https://trac.sagemath.org/ticket/15173_





---

archive/issue_events_198156.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2013-09-07T16:03:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15173",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15173#event-198156"
}
```



---

archive/issue_events_198157.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2013-09-07T16:03:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15173",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15173#event-198157"
}
```



---

archive/issue_events_198158.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2013-09-07T16:03:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15173",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15173#event-198158"
}
```



---

archive/issue_events_198159.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2013-09-07T16:03:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15173",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15173#event-198159"
}
```



---

archive/issue_events_198160.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2013-09-08T20:41:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15173",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15173#event-198160"
}
```



---

archive/issue_events_198161.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2013-09-08T20:41:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15173",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15173#event-198161"
}
```



---

archive/issue_events_198162.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2013-09-08T20:41:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15173",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15173#event-198162"
}
```



---

archive/issue_events_198163.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2013-09-08T20:42:24Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15173",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15173#event-198163"
}
```



---

archive/issue_events_198164.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2013-09-08T20:42:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15173",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15173#event-198164"
}
```



---

archive/issue_events_198165.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2013-09-08T20:42:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15173",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15173#event-198165"
}
```



---

archive/issue_comments_189227.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nDupe of #15116",
    "created_at": "2013-09-08T20:42:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15173",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15173#issuecomment-189227",
    "user": "https://github.com/ppurka"
}
```

<div id="comment:3" align="right">Comment 3</div>

Dupe of #15116



---

archive/issue_comments_189228.json:
```json
{
    "body": "Reviewer: **Punarbasu Purkayastha**",
    "created_at": "2013-09-08T20:42:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15173",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15173#issuecomment-189228",
    "user": "https://github.com/ppurka"
}
```

Reviewer: **Punarbasu Purkayastha**



---

archive/issue_events_198166.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-05T09:36:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15173",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15173#event-198166"
}
```



---

archive/issue_events_198167.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-05T09:36:12Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15173",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15173#event-198167"
}
```



---

archive/issue_events_198168.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-05T09:36:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15173",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15173#event-198168"
}
```
