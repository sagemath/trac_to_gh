# Issue 15297: Elements from a Field of Fractions that compare equal should have equal hashes

archive/issues_015060.json:
```json
{
    "assignees": [],
    "body": "Sage can't guarantee that `p == q` implies `hash(p) == hash(q)`, but it is not unreasonable to strive to make this work in case `p,q` belong to the same ring or field.\n\nThis ticket deals with Fields of Fractions. The proposed solution from this post appears to do the trick:\nhttps://groups.google.com/forum/#!topic/sage-devel/TOp_5LCBBR4\n\nExample:\n\n```\nsage: R.<x> = ZZ['x']\nsage: F = R.fraction_field()\nsage: p = 1/(1-x)\nsage: q = (-1)/(x-1)\nsage: p == q\nTrue\nsage: hash(p) == hash(q)\nFalse\n```\n\nCC:  @yuan-zhou @mkoeppe\n\nKeywords: **field of fractions, hashing**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15297_\n\n",
    "created_at": "2013-10-16T19:41:39Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Elements from a Field of Fractions that compare equal should have equal hashes",
    "type": "issue",
    "updated_at": "2022-09-19T18:58:47Z",
    "url": "https://github.com/sagemath/sage/issues/15297",
    "user": "https://github.com/sagetrac-Stefan"
}
```
Sage can't guarantee that `p == q` implies `hash(p) == hash(q)`, but it is not unreasonable to strive to make this work in case `p,q` belong to the same ring or field.

This ticket deals with Fields of Fractions. The proposed solution from this post appears to do the trick:
https://groups.google.com/forum/#!topic/sage-devel/TOp_5LCBBR4

Example:

```
sage: R.<x> = ZZ['x']
sage: F = R.fraction_field()
sage: p = 1/(1-x)
sage: q = (-1)/(x-1)
sage: p == q
True
sage: hash(p) == hash(q)
False
```

CC:  @yuan-zhou @mkoeppe

Keywords: **field of fractions, hashing**

_Issue created by migration from https://trac.sagemath.org/ticket/15297_





---

archive/issue_events_200099.json:
```json
{
    "actor": "https://github.com/sagetrac-Stefan",
    "created_at": "2013-10-16T19:41:39Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15297",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15297#event-200099"
}
```



---

archive/issue_events_200100.json:
```json
{
    "actor": "https://github.com/sagetrac-Stefan",
    "created_at": "2013-10-16T19:41:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15297",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15297#event-200100"
}
```



---

archive/issue_events_200101.json:
```json
{
    "actor": "https://github.com/sagetrac-Stefan",
    "created_at": "2013-10-16T19:41:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15297",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15297#event-200101"
}
```



---

archive/issue_events_200102.json:
```json
{
    "actor": "https://github.com/sagetrac-Stefan",
    "created_at": "2013-10-16T19:41:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15297",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15297#event-200102"
}
```



---

archive/issue_events_200103.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15297",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15297#event-200103"
}
```



---

archive/issue_events_200104.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15297",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15297#event-200104"
}
```



---

archive/issue_events_200105.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15297",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15297#event-200105"
}
```



---

archive/issue_events_200106.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15297",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15297#event-200106"
}
```



---

archive/issue_events_200107.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15297",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15297#event-200107"
}
```



---

archive/issue_events_200108.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15297",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15297#event-200108"
}
```



---

archive/issue_comments_191458.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nStill broken even with #16268; see #26339.",
    "created_at": "2018-09-28T21:12:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15297",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15297#issuecomment-191458",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:5" align="right">Comment 5</div>

Still broken even with #16268; see #26339.



---

archive/issue_events_200109.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2018-09-28T21:12:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15297",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15297#event-200109"
}
```



---

archive/issue_events_200110.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2018-09-28T21:12:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15297",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15297#event-200110"
}
```



---

archive/issue_comments_191459.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nStill broken in 9.1.rc2",
    "created_at": "2020-05-02T01:09:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15297",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15297#issuecomment-191459",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:6" align="right">Comment 6</div>

Still broken in 9.1.rc2



---

archive/issue_events_200111.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-05-02T01:09:05Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15297",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15297#event-200111"
}
```



---

archive/issue_events_200112.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-05-02T01:09:05Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15297",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15297#event-200112"
}
```



---

archive/issue_events_200113.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15297",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15297#event-200113"
}
```



---

archive/issue_events_200114.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15297",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15297#event-200114"
}
```



---

archive/issue_events_200115.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-27T02:13:38Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15297",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15297#event-200115"
}
```



---

archive/issue_events_200116.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-27T02:13:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15297",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15297#event-200116"
}
```



---

archive/issue_comments_191460.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nSage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-03-27T02:13:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15297",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15297#issuecomment-191460",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:8" align="right">Comment 8</div>

Sage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_200117.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-09T21:26:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15297",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15297#event-200117"
}
```



---

archive/issue_events_200118.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-09T21:26:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15297",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15297#event-200118"
}
```



---

archive/issue_events_200119.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15297",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15297#event-200119"
}
```



---

archive/issue_events_200120.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15297",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15297#event-200120"
}
```



---

archive/issue_events_200121.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-03T15:17:06Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15297",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15297#event-200121"
}
```



---

archive/issue_events_200122.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-03T15:17:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15297",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15297#event-200122"
}
```



---

archive/issue_events_200123.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15297",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15297#event-200123"
}
```



---

archive/issue_events_200124.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15297",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15297#event-200124"
}
```
