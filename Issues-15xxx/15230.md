# Issue 15230: Issue with static graph backend

archive/issues_014993.json:
```json
{
    "assignees": [],
    "body": "Given a graph G, the following error occurs when trying to compute its distance matrix\n\n```\nsage: G = Graph(G, data_structure='static_sparse')\nsage: G.distance_matrix()\n---------------------------------------------------------------------------\nAttributeError                            Traceback (most recent call last)\n<ipython-input-3-c91bd79088dd> in <module>()\n----> 1 G.distance_matrix()\n\n/home/azi/sage-5.12.beta4/local/lib/python2.7/site-packages/sage/graphs/generic_graph.pyc in distance_matrix(self)\n   1255         V = self.vertices()\n   1256\n-> 1257         dist = self.distance_all_pairs()\n   1258\n   1259         for i in xrange(n):\n\n/home/azi/sage-5.12.beta4/local/lib/python2.7/site-packages/sage/graphs/generic_graph.pyc in distance_all_pairs(self, algorithm)\n  11565         elif algorithm == \"Floyd-Warshall\":\n  11566             from sage.graphs.distances_all_pairs import floyd_warshall\n> 11567             return floyd_warshall(self,paths = False, distances = True)\n  11568\n  11569         else:\n\n/home/azi/sage-5.12.beta4/local/lib/python2.7/site-packages/sage/graphs/distances_all_pairs.so in sage.graphs.distances_all_pairs.floyd_warshall (sage/graphs/distances_all_pairs.c:9427)()\n\nAttributeError: 'StaticSparseBackend' object has no attribute 'vertex_ints'\n\n```\n\nCC:  @rbeezer @nathanncohen @dcoudert @lobiCode\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15230_\n\n",
    "closed_at": "2019-01-07T13:10:06Z",
    "created_at": "2013-09-26T17:35:17Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/invalid",
        "https://github.com/sagemath/sage/labels/worksforme"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Issue with static graph backend",
    "type": "issue",
    "updated_at": "2019-01-07T13:10:06Z",
    "url": "https://github.com/sagemath/sage/issues/15230",
    "user": "https://github.com/sagetrac-azi"
}
```
Given a graph G, the following error occurs when trying to compute its distance matrix

```
sage: G = Graph(G, data_structure='static_sparse')
sage: G.distance_matrix()
---------------------------------------------------------------------------
AttributeError                            Traceback (most recent call last)
<ipython-input-3-c91bd79088dd> in <module>()
----> 1 G.distance_matrix()

/home/azi/sage-5.12.beta4/local/lib/python2.7/site-packages/sage/graphs/generic_graph.pyc in distance_matrix(self)
   1255         V = self.vertices()
   1256
-> 1257         dist = self.distance_all_pairs()
   1258
   1259         for i in xrange(n):

/home/azi/sage-5.12.beta4/local/lib/python2.7/site-packages/sage/graphs/generic_graph.pyc in distance_all_pairs(self, algorithm)
  11565         elif algorithm == "Floyd-Warshall":
  11566             from sage.graphs.distances_all_pairs import floyd_warshall
> 11567             return floyd_warshall(self,paths = False, distances = True)
  11568
  11569         else:

/home/azi/sage-5.12.beta4/local/lib/python2.7/site-packages/sage/graphs/distances_all_pairs.so in sage.graphs.distances_all_pairs.floyd_warshall (sage/graphs/distances_all_pairs.c:9427)()

AttributeError: 'StaticSparseBackend' object has no attribute 'vertex_ints'

```

CC:  @rbeezer @nathanncohen @dcoudert @lobiCode

_Issue created by migration from https://trac.sagemath.org/ticket/15230_





---

archive/issue_events_199082.json:
```json
{
    "actor": "https://github.com/sagetrac-azi",
    "created_at": "2013-09-26T17:35:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15230",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15230#event-199082"
}
```



---

archive/issue_events_199083.json:
```json
{
    "actor": "https://github.com/sagetrac-azi",
    "created_at": "2013-09-26T17:35:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15230",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15230#event-199083"
}
```



---

archive/issue_events_199084.json:
```json
{
    "actor": "https://github.com/sagetrac-azi",
    "created_at": "2013-09-26T17:35:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15230",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15230#event-199084"
}
```



---

archive/issue_events_199085.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15230",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15230#event-199085"
}
```



---

archive/issue_events_199086.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15230",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15230#event-199086"
}
```



---

archive/issue_events_199087.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15230",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15230#event-199087"
}
```



---

archive/issue_events_199088.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15230",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15230#event-199088"
}
```



---

archive/issue_events_199089.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15230",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15230#event-199089"
}
```



---

archive/issue_events_199090.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15230",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15230#event-199090"
}
```



---

archive/issue_events_199091.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-01-07T13:10:06Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15230",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15230#event-199091"
}
```



---

archive/issue_events_199092.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-01-07T13:10:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15230",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15230#event-199092"
}
```



---

archive/issue_events_199093.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-01-07T13:10:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15230",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15230#event-199093"
}
```



---

archive/issue_comments_190295.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThis appears to have been fixed at some point; probably by #15623.",
    "created_at": "2019-01-07T13:10:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15230",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15230#issuecomment-190295",
    "user": "https://github.com/embray"
}
```

<div id="comment:4" align="right">Comment 4</div>

This appears to have been fixed at some point; probably by #15623.



---

archive/issue_events_199094.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-01-07T13:10:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15230",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15230#event-199094"
}
```



---

archive/issue_events_199095.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-01-07T13:10:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15230",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15230#event-199095"
}
```



---

archive/issue_events_199096.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-01-07T13:10:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15230",
    "label": "https://github.com/sagemath/sage/labels/worksforme",
    "label_color": "a6a6a6",
    "label_name": "worksforme",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15230#event-199096"
}
```
