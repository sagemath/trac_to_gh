# Issue 15638: Recompiling an extension every time we rebuild

archive/issues_015401.json:
```json
{
    "assignees": [],
    "body": "Everytime I rebuild sage with 6.1.beta4 (even with no changes), I have the following cython file being rebuilt:\n\n```\nbuilding 'sage.groups.perm_gps.partn_ref2.refinement_generic' extension\n```\nWhile this doesn't take a long time, it shouldn't happen.\n\nAssignee: **@tscrim**\n\nCC:  @vbraun @jdemeyer\n\nBranch/Commit: **[u/jdemeyer/ticket/15638](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/15638) @ [7695e6f](https://github.com/sagemath/sagetrac-mirror/commit/7695e6f937e2d81a65818db9c0077c105e772e70)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15638_\n\n",
    "closed_at": "2014-01-07T04:55:37Z",
    "created_at": "2014-01-06T16:06:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Recompiling an extension every time we rebuild",
    "type": "issue",
    "updated_at": "2014-01-07T04:55:37Z",
    "url": "https://github.com/sagemath/sage/issues/15638",
    "user": "https://github.com/tscrim"
}
```
Everytime I rebuild sage with 6.1.beta4 (even with no changes), I have the following cython file being rebuilt:

```
building 'sage.groups.perm_gps.partn_ref2.refinement_generic' extension
```
While this doesn't take a long time, it shouldn't happen.

Assignee: **@tscrim**

CC:  @vbraun @jdemeyer

Branch/Commit: **[u/jdemeyer/ticket/15638](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/15638) @ [7695e6f](https://github.com/sagemath/sagetrac-mirror/commit/7695e6f937e2d81a65818db9c0077c105e772e70)**

Reviewer: **Travis Scrimshaw**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/15638_





---

archive/issue_events_205628.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-01-06T16:06:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15638",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15638#event-205628"
}
```



---

archive/issue_events_205629.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-01-06T16:06:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15638",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15638#event-205629"
}
```



---

archive/issue_events_205630.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-01-06T16:06:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15638",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15638#event-205630"
}
```



---

archive/issue_events_205631.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-01-06T16:06:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15638",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15638#event-205631"
}
```



---

archive/issue_comments_199002.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nConfirmed.",
    "created_at": "2014-01-06T16:11:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15638",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15638#issuecomment-199002",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:1'>Comment 1:</a>
Confirmed.



---

archive/issue_comments_199003.json:
```json
{
    "body": "Author: **Jeroen Demeyer**",
    "created_at": "2014-01-06T16:16:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15638",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15638#issuecomment-199003",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Jeroen Demeyer**



---

archive/issue_comments_199004.json:
```json
{
    "body": "Branch: **[u/jdemeyer/ticket/15638](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/15638)**",
    "created_at": "2014-01-06T16:28:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15638",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15638#issuecomment-199004",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/ticket/15638](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/15638)**



---

archive/issue_events_205632.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-01-06T16:28:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15638",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15638#event-205632"
}
```



---

archive/issue_comments_199005.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2014-01-06T19:08:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15638",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15638#issuecomment-199005",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_199006.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nThanks Jeroen.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7695e6f937e2d81a65818db9c0077c105e772e70\">7695e6f</a></td><td><code>Fix flint dependencies after flint upgrade</code></td></tr></table>\n",
    "created_at": "2014-01-06T19:08:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15638",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15638#issuecomment-199006",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:5'>Comment 5:</a>
Thanks Jeroen.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7695e6f937e2d81a65818db9c0077c105e772e70">7695e6f</a></td><td><code>Fix flint dependencies after flint upgrade</code></td></tr></table>




---

archive/issue_events_205633.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-01-06T19:08:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15638",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15638#event-205633"
}
```



---

archive/issue_events_205634.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-01-06T19:08:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15638",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15638#event-205634"
}
```



---

archive/issue_comments_199007.json:
```json
{
    "body": "Commit: **[7695e6f](https://github.com/sagemath/sagetrac-mirror/commit/7695e6f937e2d81a65818db9c0077c105e772e70)**",
    "created_at": "2014-01-06T19:08:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15638",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15638#issuecomment-199007",
    "user": "https://github.com/tscrim"
}
```

Commit: **[7695e6f](https://github.com/sagemath/sagetrac-mirror/commit/7695e6f937e2d81a65818db9c0077c105e772e70)**



---

archive/issue_events_205635.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-01-07T04:55:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15638",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15638#event-205635"
}
```



---

archive/issue_events_205636.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "922d2903caba0b31de13a26c05dbbe39cce595f4",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-01-07T04:55:37Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15638",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15638#event-205636"
}
```
