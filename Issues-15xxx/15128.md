# Issue 15128: Add structure_description method to permutation groups

archive/issues_014891.json:
```json
{
    "assignees": [],
    "body": "Add a method called structure_description to permutation groups that wraps the educational GAP method StructureDescription. There is also an option for LaTeX formatted string.\n\nThe full GAP documentation for this method is at [http://www.gap-system.org/Manuals/doc/ref/chap39_mj.html#X87BF1B887C91CA2E](http://www.gap-system.org/Manuals/doc/ref/chap39_mj.html#X87BF1B887C91CA2E)\n\nBranch/Commit: **[u/mathzeta2/ticket/15128](https://github.com/sagemath/sagetrac-mirror/tree/u/mathzeta2/ticket/15128) @ [bdb1f2a](https://github.com/sagemath/sagetrac-mirror/commit/bdb1f2a55fd3b0099428e425dd8a74e8df0f9c0f)**\n\nReviewer: **Nathann Cohen**\n\nAuthor: **Tomer Bauer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15128_\n\n",
    "closed_at": "2013-12-21T22:18:23Z",
    "created_at": "2013-08-30T13:42:18Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20group%20theory",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add structure_description method to permutation groups",
    "type": "issue",
    "updated_at": "2013-12-21T22:18:23Z",
    "url": "https://github.com/sagemath/sage/issues/15128",
    "user": "https://github.com/mathzeta"
}
```
Add a method called structure_description to permutation groups that wraps the educational GAP method StructureDescription. There is also an option for LaTeX formatted string.

The full GAP documentation for this method is at [http://www.gap-system.org/Manuals/doc/ref/chap39_mj.html#X87BF1B887C91CA2E](http://www.gap-system.org/Manuals/doc/ref/chap39_mj.html#X87BF1B887C91CA2E)

Branch/Commit: **[u/mathzeta2/ticket/15128](https://github.com/sagemath/sagetrac-mirror/tree/u/mathzeta2/ticket/15128) @ [bdb1f2a](https://github.com/sagemath/sagetrac-mirror/commit/bdb1f2a55fd3b0099428e425dd8a74e8df0f9c0f)**

Reviewer: **Nathann Cohen**

Author: **Tomer Bauer**

_Issue created by migration from https://trac.sagemath.org/ticket/15128_





---

archive/issue_events_197481.json:
```json
{
    "actor": "https://github.com/mathzeta",
    "created_at": "2013-08-30T13:42:18Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15128",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15128#event-197481"
}
```



---

archive/issue_events_197482.json:
```json
{
    "actor": "https://github.com/mathzeta",
    "created_at": "2013-08-30T13:42:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15128",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20group%20theory",
    "label_color": "0000ff",
    "label_name": "component: group theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15128#event-197482"
}
```



---

archive/issue_events_197483.json:
```json
{
    "actor": "https://github.com/mathzeta",
    "created_at": "2013-08-30T13:42:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15128",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15128#event-197483"
}
```



---

archive/issue_events_197484.json:
```json
{
    "actor": "https://github.com/mathzeta",
    "created_at": "2013-08-30T13:42:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15128",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15128#event-197484"
}
```



---

archive/issue_comments_188468.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nThis requires the small groups library which is optional due to licensing problems. The doctests need to be marked as `# optional - database_gap`, I think.",
    "created_at": "2013-08-30T18:17:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15128#issuecomment-188468",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:1" align="right">Comment 1</div>

This requires the small groups library which is optional due to licensing problems. The doctests need to be marked as `# optional - database_gap`, I think.



---

archive/issue_comments_188469.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nAttachment: **[trac_15128_structure_description.patch.gz](https://github.com/sagemath/sage/files/ticket15128/trac_15128_structure_description.patch.gz)**\n\nReplying to [@vbraun](#comment%3A1):\n> This requires the small groups library which is optional due to licensing problems. The doctests need to be marked as `# optional - database_gap`, I think.\n\nYes, I think you are right. I attached a revised patch.",
    "created_at": "2013-08-30T22:37:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15128#issuecomment-188469",
    "user": "https://github.com/mathzeta"
}
```

<div id="comment:2" align="right">Comment 2</div>

Attachment: **[trac_15128_structure_description.patch.gz](https://github.com/sagemath/sage/files/ticket15128/trac_15128_structure_description.patch.gz)**

Replying to [@vbraun](#comment%3A1):
> This requires the small groups library which is optional due to licensing problems. The doctests need to be marked as `# optional - database_gap`, I think.

Yes, I think you are right. I attached a revised patch.



---

archive/issue_events_197485.json:
```json
{
    "actor": "https://github.com/mathzeta",
    "created_at": "2013-09-12T14:20:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15128",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15128#event-197485"
}
```



---

archive/issue_comments_188470.json:
```json
{
    "body": "Branch: **[u/ncohen/15128](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/15128)**",
    "created_at": "2013-12-01T14:19:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15128#issuecomment-188470",
    "user": "https://github.com/nathanncohen"
}
```

Branch: **[u/ncohen/15128](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/15128)**



---

archive/issue_comments_188471.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nHellooooooooo !!\n\nI converted this patch to a git patch, and appended some modifications for the doc. I also added a couple of line to handle the latex version of the different outputs as given on http://www.gap-system.org/Manuals/doc/ref/chap39.html. You can set the patch to `positive_review` if you agree with these modifications `:-)`\n\nNathann",
    "created_at": "2013-12-01T14:19:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15128#issuecomment-188471",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:4" align="right">Comment 4</div>

Hellooooooooo !!

I converted this patch to a git patch, and appended some modifications for the doc. I also added a couple of line to handle the latex version of the different outputs as given on http://www.gap-system.org/Manuals/doc/ref/chap39.html. You can set the patch to `positive_review` if you agree with these modifications `:-)`

Nathann



---

archive/issue_comments_188472.json:
```json
{
    "body": "Reviewer: **Nathann Cohen**",
    "created_at": "2013-12-01T14:19:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15128#issuecomment-188472",
    "user": "https://github.com/nathanncohen"
}
```

Reviewer: **Nathann Cohen**



---

archive/issue_comments_188473.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td>[bdb1f2a](https://github.com/sagemath/sagetrac-mirror/commit/bdb1f2a)</td><td><code>trac #15128: reviewer's patch -- some doc and support for other outputs</code></td></tr><tr><td>[df0f7c9](https://github.com/sagemath/sagetrac-mirror/commit/df0f7c9)</td><td><code>Add structure_description method to permutation groups</code></td></tr></table>\n",
    "created_at": "2013-12-01T14:19:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15128#issuecomment-188473",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:5" align="right">Comment 5</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td>[bdb1f2a](https://github.com/sagemath/sagetrac-mirror/commit/bdb1f2a)</td><td><code>trac #15128: reviewer's patch -- some doc and support for other outputs</code></td></tr><tr><td>[df0f7c9](https://github.com/sagemath/sagetrac-mirror/commit/df0f7c9)</td><td><code>Add structure_description method to permutation groups</code></td></tr></table>




---

archive/issue_comments_188474.json:
```json
{
    "body": "Commit: **[bdb1f2a](https://github.com/sagemath/sagetrac-mirror/commit/bdb1f2a55fd3b0099428e425dd8a74e8df0f9c0f)**",
    "created_at": "2013-12-01T14:19:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15128#issuecomment-188474",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[bdb1f2a](https://github.com/sagemath/sagetrac-mirror/commit/bdb1f2a55fd3b0099428e425dd8a74e8df0f9c0f)**



---

archive/issue_comments_188475.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nI have made two minor typography changes.\n\nThe adventures of the new git process... positive review it is!",
    "created_at": "2013-12-21T16:18:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15128#issuecomment-188475",
    "user": "https://github.com/mathzeta"
}
```

<div id="comment:6" align="right">Comment 6</div>

I have made two minor typography changes.

The adventures of the new git process... positive review it is!



---

archive/issue_comments_188476.json:
```json
{
    "body": "Changed branch from **[u/ncohen/15128](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/15128)** to **[u/mathzeta2/ticket/15128](https://github.com/sagemath/sagetrac-mirror/tree/u/mathzeta2/ticket/15128)**",
    "created_at": "2013-12-21T16:18:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15128#issuecomment-188476",
    "user": "https://github.com/mathzeta"
}
```

Changed branch from **[u/ncohen/15128](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/15128)** to **[u/mathzeta2/ticket/15128](https://github.com/sagemath/sagetrac-mirror/tree/u/mathzeta2/ticket/15128)**



---

archive/issue_events_197486.json:
```json
{
    "actor": "https://github.com/mathzeta",
    "created_at": "2013-12-21T16:26:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15128",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15128#event-197486"
}
```



---

archive/issue_events_197487.json:
```json
{
    "actor": "https://github.com/mathzeta",
    "created_at": "2013-12-21T16:26:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15128",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15128#event-197487"
}
```



---

archive/issue_events_197488.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-12-21T22:18:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15128",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15128#event-197488"
}
```



---

archive/issue_events_197489.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-12-21T22:18:23Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15128",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15128#event-197489"
}
```
