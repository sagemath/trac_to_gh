# Issue 15435: WeylGroup and CoxeterGroup to groups.<tab>

archive/issues_015198.json:
```json
{
    "assignees": [],
    "body": "As the title says. Those groups were still missing. I also add a couple of doctests to be sure that the feature remains.\n\nNathann\n\nDepends on #15369\n\nCC:  @vbraun @hivert @nthiery @videlec\n\nBranch/Commit: **[u/ncohen/15435](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/15435) @ [52a7626](https://github.com/sagemath/sagetrac-mirror/commit/52a76265a64fe5c091e7c56e859a28e8d1343a4c)**\n\nReviewer: **Volker Braun**\n\nAuthor: **Nathann Cohen**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15435_\n\n",
    "closed_at": "2013-12-26T14:50:11Z",
    "created_at": "2013-11-18T16:43:56Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20group%20theory",
        "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "WeylGroup and CoxeterGroup to groups.<tab>",
    "type": "issue",
    "updated_at": "2013-12-26T14:50:11Z",
    "url": "https://github.com/sagemath/sage/issues/15435",
    "user": "https://github.com/nathanncohen"
}
```
As the title says. Those groups were still missing. I also add a couple of doctests to be sure that the feature remains.

Nathann

Depends on #15369

CC:  @vbraun @hivert @nthiery @videlec

Branch/Commit: **[u/ncohen/15435](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/15435) @ [52a7626](https://github.com/sagemath/sagetrac-mirror/commit/52a76265a64fe5c091e7c56e859a28e8d1343a4c)**

Reviewer: **Volker Braun**

Author: **Nathann Cohen**

_Issue created by migration from https://trac.sagemath.org/ticket/15435_





---

archive/issue_events_202403.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-11-18T16:43:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15435",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15435#event-202403"
}
```



---

archive/issue_events_202404.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-11-18T16:43:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15435",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20group%20theory",
    "label_color": "0000ff",
    "label_name": "component: group theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15435#event-202404"
}
```



---

archive/issue_events_202405.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-11-18T16:43:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15435",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15435#event-202405"
}
```



---

archive/issue_events_202406.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-11-18T16:43:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15435",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15435#event-202406"
}
```



---

archive/issue_events_202407.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-11-18T17:00:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15435",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15435#event-202407"
}
```



---

archive/issue_comments_194617.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nFollowing Nicolas' will, those two groups will lie in \"groups.misc\", near `BraidGroup`.\n\nNathann",
    "created_at": "2013-11-18T17:00:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15435#issuecomment-194617",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:1'>Comment 1:</a>
Following Nicolas' will, those two groups will lie in "groups.misc", near `BraidGroup`.

Nathann



---

archive/issue_comments_194618.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n-As the title says. Those groups were still missing.\n+As the title says. Those groups were still missing. I also add a couple of doctests to be sure that the feature remains.\n \n Nathann\n``````\n",
    "created_at": "2013-11-18T17:00:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15435#issuecomment-194618",
    "user": "https://github.com/nathanncohen"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
-As the title says. Those groups were still missing.
+As the title says. Those groups were still missing. I also add a couple of doctests to be sure that the feature remains.
 
 Nathann
``````




---

archive/issue_comments_194619.json:
```json
{
    "body": "Branch: **[u/ncohen/15435](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/15435)**",
    "created_at": "2013-11-18T17:00:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15435#issuecomment-194619",
    "user": "https://github.com/nathanncohen"
}
```

Branch: **[u/ncohen/15435](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/15435)**



---

archive/issue_comments_194620.json:
```json
{
    "body": "Commit: **[69f2852](https://github.com/sagemath/sagetrac-mirror/commit/69f28524404278a53c73218127bcc7014696eeeb)**",
    "created_at": "2013-11-18T17:00:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15435#issuecomment-194620",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[69f2852](https://github.com/sagemath/sagetrac-mirror/commit/69f28524404278a53c73218127bcc7014696eeeb)**



---

archive/issue_comments_194621.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td>[69f2852](https://github.com/sagemath/sagetrac-mirror/commit/69f2852)</td><td><code>trac #15435: WeylGroup and CoxeterGroup to groups.<tab></code></td></tr><tr><td>[488029f](https://github.com/sagemath/sagetrac-mirror/commit/488029f)</td><td><code>trac #15435: depends on #15369</code></td></tr><tr><td>[16d141c](https://github.com/sagemath/sagetrac-mirror/commit/16d141c)</td><td><code>Alias groups.misc.AdditiveCyclic to IntegerModRing</code></td></tr><tr><td>[c6bb227](https://github.com/sagemath/sagetrac-mirror/commit/c6bb227)</td><td><code>Depends on #15368</code></td></tr><tr><td>[0ad88a6](https://github.com/sagemath/sagetrac-mirror/commit/0ad88a6)</td><td><code>Additions to groups.misc : AdditiveAbelian, Free, Braid, SemimonomialTransformation</code></td></tr><tr><td>[3d75bd3](https://github.com/sagemath/sagetrac-mirror/commit/3d75bd3)</td><td><code>Move QuaternionMatrixGroupGF3 from groups.misc to groups.matrix</code></td></tr><tr><td>[2a68051](https://github.com/sagemath/sagetrac-mirror/commit/2a68051)</td><td><code>Add affine groups to groups.<tab></code></td></tr><tr><td>[017084f](https://github.com/sagemath/sagetrac-mirror/commit/017084f)</td><td><code>Broken link in groups catalog doc</code></td></tr></table>\n",
    "created_at": "2013-11-18T17:00:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15435#issuecomment-194621",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:2'>Comment 2:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td>[69f2852](https://github.com/sagemath/sagetrac-mirror/commit/69f2852)</td><td><code>trac #15435: WeylGroup and CoxeterGroup to groups.<tab></code></td></tr><tr><td>[488029f](https://github.com/sagemath/sagetrac-mirror/commit/488029f)</td><td><code>trac #15435: depends on #15369</code></td></tr><tr><td>[16d141c](https://github.com/sagemath/sagetrac-mirror/commit/16d141c)</td><td><code>Alias groups.misc.AdditiveCyclic to IntegerModRing</code></td></tr><tr><td>[c6bb227](https://github.com/sagemath/sagetrac-mirror/commit/c6bb227)</td><td><code>Depends on #15368</code></td></tr><tr><td>[0ad88a6](https://github.com/sagemath/sagetrac-mirror/commit/0ad88a6)</td><td><code>Additions to groups.misc : AdditiveAbelian, Free, Braid, SemimonomialTransformation</code></td></tr><tr><td>[3d75bd3](https://github.com/sagemath/sagetrac-mirror/commit/3d75bd3)</td><td><code>Move QuaternionMatrixGroupGF3 from groups.misc to groups.matrix</code></td></tr><tr><td>[2a68051](https://github.com/sagemath/sagetrac-mirror/commit/2a68051)</td><td><code>Add affine groups to groups.<tab></code></td></tr><tr><td>[017084f](https://github.com/sagemath/sagetrac-mirror/commit/017084f)</td><td><code>Broken link in groups catalog doc</code></td></tr></table>




---

archive/issue_comments_194622.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nI will rebase this thing in a second to clean the dependencies.\n\nNathann",
    "created_at": "2013-12-25T14:07:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15435#issuecomment-194622",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:3'>Comment 3:</a>
I will rebase this thing in a second to clean the dependencies.

Nathann



---

archive/issue_comments_194623.json:
```json
{
    "body": "Changed commit from **[69f2852](https://github.com/sagemath/sagetrac-mirror/commit/69f28524404278a53c73218127bcc7014696eeeb)** to **[52a7626](https://github.com/sagemath/sagetrac-mirror/commit/52a76265a64fe5c091e7c56e859a28e8d1343a4c)**",
    "created_at": "2013-12-25T14:08:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15435#issuecomment-194623",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[69f2852](https://github.com/sagemath/sagetrac-mirror/commit/69f28524404278a53c73218127bcc7014696eeeb)** to **[52a7626](https://github.com/sagemath/sagetrac-mirror/commit/52a76265a64fe5c091e7c56e859a28e8d1343a4c)**



---

archive/issue_comments_194624.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/52a7626\">52a7626</a></td><td><code>trac #15435: WeylGroup and CoxeterGroup to groups.<tab></code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6c7e2f4\">6c7e2f4</a></td><td><code>trac #15369: Alias groups.misc.AdditiveCyclic to IntegerModRing</code></td></tr></table>\n",
    "created_at": "2013-12-25T14:08:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15435#issuecomment-194624",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:4'>Comment 4:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/52a7626">52a7626</a></td><td><code>trac #15435: WeylGroup and CoxeterGroup to groups.<tab></code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6c7e2f4">6c7e2f4</a></td><td><code>trac #15369: Alias groups.misc.AdditiveCyclic to IntegerModRing</code></td></tr></table>




---

archive/issue_events_202408.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-12-25T14:26:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15435",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15435#event-202408"
}
```



---

archive/issue_events_202409.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-12-25T14:26:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15435",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15435#event-202409"
}
```



---

archive/issue_events_202410.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-12-25T15:43:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15435",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15435#event-202410"
}
```



---

archive/issue_events_202411.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-12-25T15:43:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15435",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15435#event-202411"
}
```



---

archive/issue_comments_194625.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2013-12-25T15:43:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15435#issuecomment-194625",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_comments_194626.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nThaaaaaaaaaaaaaaaaaaaaanks !!\n\nNathann",
    "created_at": "2013-12-25T15:44:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15435#issuecomment-194626",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:7'>Comment 7:</a>
Thaaaaaaaaaaaaaaaaaaaaanks !!

Nathann



---

archive/issue_events_202412.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-12-26T14:50:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15435",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15435#event-202412"
}
```



---

archive/issue_events_202413.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "6e44eda5db24bd996f691f089c180fa8bdc1d418",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2013-12-26T14:50:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15435",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15435#event-202413"
}
```
