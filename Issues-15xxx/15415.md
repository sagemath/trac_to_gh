# Issue 15415: Add methods for plotting crystals and LS paths

archive/issues_015178.json:
```json
{
    "assignees": [],
    "body": "Improvements to the root systems plotting which will plot crystals automatically in a few different formats, as well as LS paths as paths in the root system.\n\nAssignee: **@sagetrac-sage-combinat**\n\nCC:  @sagetrac-sage-combinat @nthiery @bsalisbury1\n\nKeywords: **crystal, LS paths, plotting**\n\nBranch/Commit: **[bac38bf](https://github.com/sagemath/sagetrac-mirror/commit/bac38bf2e8641c9a5b92f457429ce4dc1bb90267)**\n\nReviewer: **Ben Salisbury**\n\nAuthor: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15415_\n\n",
    "closed_at": "2015-04-14T19:44:17Z",
    "created_at": "2013-11-14T00:13:10Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graphics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add methods for plotting crystals and LS paths",
    "type": "issue",
    "updated_at": "2015-04-14T19:44:17Z",
    "url": "https://github.com/sagemath/sage/issues/15415",
    "user": "https://github.com/tscrim"
}
```
Improvements to the root systems plotting which will plot crystals automatically in a few different formats, as well as LS paths as paths in the root system.

Assignee: **@sagetrac-sage-combinat**

CC:  @sagetrac-sage-combinat @nthiery @bsalisbury1

Keywords: **crystal, LS paths, plotting**

Branch/Commit: **[bac38bf](https://github.com/sagemath/sagetrac-mirror/commit/bac38bf2e8641c9a5b92f457429ce4dc1bb90267)**

Reviewer: **Ben Salisbury**

Author: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/15415_





---

archive/issue_events_202076.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-11-14T00:13:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15415#event-202076"
}
```



---

archive/issue_events_202077.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-11-14T00:13:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graphics",
    "label_color": "0000ff",
    "label_name": "component: graphics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15415#event-202077"
}
```



---

archive/issue_events_202078.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-11-14T00:13:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15415#event-202078"
}
```



---

archive/issue_events_202079.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-11-14T00:13:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15415#event-202079"
}
```



---

archive/issue_events_202080.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-11-14T00:14:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15415#event-202080"
}
```



---

archive/issue_comments_194213.json:
```json
{
    "body": "Changed commit from **[76afbd5](https://github.com/sagemath/sagetrac-mirror/commit/76afbd578d796483cb646a97eb14f88ad7522713)** to **[e7a558d](https://github.com/sagemath/sagetrac-mirror/commit/e7a558d466ad530df5c3beec5bf67a41ece35446)**",
    "created_at": "2013-11-17T23:23:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15415#issuecomment-194213",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[76afbd5](https://github.com/sagemath/sagetrac-mirror/commit/76afbd578d796483cb646a97eb14f88ad7522713)** to **[e7a558d](https://github.com/sagemath/sagetrac-mirror/commit/e7a558d466ad530df5c3beec5bf67a41ece35446)**



---

archive/issue_comments_194214.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td>[e7a558d](https://github.com/sagemath/sagetrac-mirror/commit/e7a558d)</td><td><code>Merge branch 'master' into public/combinat/crystals/crystal_plots</code></td></tr></table>\n",
    "created_at": "2013-11-17T23:23:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15415#issuecomment-194214",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td>[e7a558d](https://github.com/sagemath/sagetrac-mirror/commit/e7a558d)</td><td><code>Merge branch 'master' into public/combinat/crystals/crystal_plots</code></td></tr></table>




---

archive/issue_comments_194215.json:
```json
{
    "body": "Changed commit from **[e7a558d](https://github.com/sagemath/sagetrac-mirror/commit/e7a558d466ad530df5c3beec5bf67a41ece35446)** to **[1b48a12](https://github.com/sagemath/sagetrac-mirror/commit/1b48a12d43a11fe0fe7300b2715afc15d1942910)**",
    "created_at": "2013-11-18T07:30:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15415#issuecomment-194215",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[e7a558d](https://github.com/sagemath/sagetrac-mirror/commit/e7a558d466ad530df5c3beec5bf67a41ece35446)** to **[1b48a12](https://github.com/sagemath/sagetrac-mirror/commit/1b48a12d43a11fe0fe7300b2715afc15d1942910)**



---

archive/issue_comments_194216.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td>[1b48a12](https://github.com/sagemath/sagetrac-mirror/commit/1b48a12)</td><td><code>Merge branch 'public/combinat/crystals/crystal_plots' of ssh://trac.sagemath.org:2222/sage into public/combinat/crystals/crystal_plots</code></td></tr><tr><td>[667282c](https://github.com/sagemath/sagetrac-mirror/commit/667282c)</td><td><code>Merge branch 'master' into public/combinat/crystals/crystal_plots</code></td></tr></table>\n",
    "created_at": "2013-11-18T07:30:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15415#issuecomment-194216",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td>[1b48a12](https://github.com/sagemath/sagetrac-mirror/commit/1b48a12)</td><td><code>Merge branch 'public/combinat/crystals/crystal_plots' of ssh://trac.sagemath.org:2222/sage into public/combinat/crystals/crystal_plots</code></td></tr><tr><td>[667282c](https://github.com/sagemath/sagetrac-mirror/commit/667282c)</td><td><code>Merge branch 'master' into public/combinat/crystals/crystal_plots</code></td></tr></table>




---

archive/issue_comments_194217.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThe ticket title could be more explicit :-) In fact the current description would be a rather good one.\n\nCheers,",
    "created_at": "2013-11-18T08:33:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15415#issuecomment-194217",
    "user": "https://github.com/nthiery"
}
```

<div id="comment:4" align="right">Comment 4</div>

The ticket title could be more explicit :-) In fact the current description would be a rather good one.

Cheers,



---

archive/issue_events_202081.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-11-18T16:51:28Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "title_is": "Add methods for plotting crystals and LS paths",
    "title_was": "Improvements to plotting",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15415#event-202081"
}
```



---

archive/issue_comments_194218.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-Adds methods to plot crystals and LS paths.\n+Improvements to the root systems plotting which will plot crystals automatically in a few different formats, as well as LS paths as paths in the root system.\n``````\n",
    "created_at": "2013-11-18T16:51:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15415#issuecomment-194218",
    "user": "https://github.com/tscrim"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-Adds methods to plot crystals and LS paths.
+Improvements to the root systems plotting which will plot crystals automatically in a few different formats, as well as LS paths as paths in the root system.
``````




---

archive/issue_events_202082.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15415#event-202082"
}
```



---

archive/issue_events_202083.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15415#event-202083"
}
```



---

archive/issue_comments_194219.json:
```json
{
    "body": "Changed commit from **[1b48a12](https://github.com/sagemath/sagetrac-mirror/commit/1b48a12d43a11fe0fe7300b2715afc15d1942910)** to **[5145935](https://github.com/sagemath/sagetrac-mirror/commit/514593570d7c3f6fb521cfa3bca5117d9f7ac525)**",
    "created_at": "2014-04-16T02:04:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15415#issuecomment-194219",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[1b48a12](https://github.com/sagemath/sagetrac-mirror/commit/1b48a12d43a11fe0fe7300b2715afc15d1942910)** to **[5145935](https://github.com/sagemath/sagetrac-mirror/commit/514593570d7c3f6fb521cfa3bca5117d9f7ac525)**



---

archive/issue_comments_194220.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f17233ac74e13b4de958c93d59694339c8263a77\">f17233a</a></td><td><code>Merge branch 'public/combinat/crystals/crystal_plots' of trac.sagemath.org:sage into public/combinat/crystals/crystal_plots-15415</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/514593570d7c3f6fb521cfa3bca5117d9f7ac525\">5145935</a></td><td><code>Changed crystals to catalog.</code></td></tr></table>\n",
    "created_at": "2014-04-16T02:04:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15415#issuecomment-194220",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:7" align="right">Comment 7</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f17233ac74e13b4de958c93d59694339c8263a77">f17233a</a></td><td><code>Merge branch 'public/combinat/crystals/crystal_plots' of trac.sagemath.org:sage into public/combinat/crystals/crystal_plots-15415</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/514593570d7c3f6fb521cfa3bca5117d9f7ac525">5145935</a></td><td><code>Changed crystals to catalog.</code></td></tr></table>




---

archive/issue_events_202084.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15415#event-202084"
}
```



---

archive/issue_events_202085.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15415#event-202085"
}
```



---

archive/issue_events_202086.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15415#event-202086"
}
```



---

archive/issue_events_202087.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15415#event-202087"
}
```



---

archive/issue_comments_194221.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nFailing doctests, see patchbot report",
    "created_at": "2015-03-25T19:40:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15415#issuecomment-194221",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:10" align="right">Comment 10</div>

Failing doctests, see patchbot report



---

archive/issue_events_202088.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-03-25T19:40:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15415#event-202088"
}
```



---

archive/issue_events_202089.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-03-25T19:40:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15415#event-202089"
}
```



---

archive/issue_comments_194222.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b0fe20157229358d540929a08480da1a5a5abee0\">b0fe201</a></td><td><code>Merge branch 'public/combinat/crystals/crystal_plots' of trac.sagemath.org:sage into public/combinat/crystals/crystal_plots</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5ed4f84d2f500ecd6d4c7cc10e8f825beeae0b50\">5ed4f84</a></td><td><code>Updating doctests.</code></td></tr></table>\n",
    "created_at": "2015-03-25T20:38:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15415#issuecomment-194222",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:11" align="right">Comment 11</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b0fe20157229358d540929a08480da1a5a5abee0">b0fe201</a></td><td><code>Merge branch 'public/combinat/crystals/crystal_plots' of trac.sagemath.org:sage into public/combinat/crystals/crystal_plots</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5ed4f84d2f500ecd6d4c7cc10e8f825beeae0b50">5ed4f84</a></td><td><code>Updating doctests.</code></td></tr></table>




---

archive/issue_comments_194223.json:
```json
{
    "body": "Changed commit from **[5145935](https://github.com/sagemath/sagetrac-mirror/commit/514593570d7c3f6fb521cfa3bca5117d9f7ac525)** to **[5ed4f84](https://github.com/sagemath/sagetrac-mirror/commit/5ed4f84d2f500ecd6d4c7cc10e8f825beeae0b50)**",
    "created_at": "2015-03-25T20:38:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15415#issuecomment-194223",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[5145935](https://github.com/sagemath/sagetrac-mirror/commit/514593570d7c3f6fb521cfa3bca5117d9f7ac525)** to **[5ed4f84](https://github.com/sagemath/sagetrac-mirror/commit/5ed4f84d2f500ecd6d4c7cc10e8f825beeae0b50)**



---

archive/issue_events_202090.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-03-25T20:39:42Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15415#event-202090"
}
```



---

archive/issue_events_202091.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-03-25T20:39:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "milestone_number": null,
    "milestone_title": "sage-6.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15415#event-202091"
}
```



---

archive/issue_events_202092.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-03-25T20:39:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15415#event-202092"
}
```



---

archive/issue_events_202093.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-03-25T20:39:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15415#event-202093"
}
```



---

archive/issue_comments_194224.json:
```json
{
    "body": "Changed commit from **[5ed4f84](https://github.com/sagemath/sagetrac-mirror/commit/5ed4f84d2f500ecd6d4c7cc10e8f825beeae0b50)** to **[349a935](https://github.com/sagemath/sagetrac-mirror/commit/349a9357828bc6d6ad56ac2c7f5b7349d9083eb8)**",
    "created_at": "2015-03-26T03:40:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15415#issuecomment-194224",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[5ed4f84](https://github.com/sagemath/sagetrac-mirror/commit/5ed4f84d2f500ecd6d4c7cc10e8f825beeae0b50)** to **[349a935](https://github.com/sagemath/sagetrac-mirror/commit/349a9357828bc6d6ad56ac2c7f5b7349d9083eb8)**



---

archive/issue_comments_194225.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nBranch pushed to git repo; I updated commit sha1. Last 10 new commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c2e84e1367cbfc47eb7104de654e2ee55caea95d\">c2e84e1</a></td><td><code>remaining doctests for induced_graded_map</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/03bd4cfb53e24b24c65aada21f08850b40fb5c5c\">03bd4cf</a></td><td><code>possibly controversial: graded_algebra and the three methods forming its interface are now cached_methods</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8a747c073e706950d1b14e77458539e126ff0550\">8a747c0</a></td><td><code>further edits</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/831c26955993e11b4e11a9840643a7d4e91d77a4\">831c269</a></td><td><code>Merge branch 'public/categories/filtered_algebras-17096' of git://trac.sagemath.org/sage into filt</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/752df7798b2ae6b9afc60528227b4e88c745fae9\">752df77</a></td><td><code>Revert the cached_method decorators from commit \"possibly controversial: graded_algebra and the three methods forming its interface are now cached_methods\"</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/dfaaaeb6d9652e071ea89c9854da984bc8ada319\">dfaaaeb</a></td><td><code>associated-graded now also defined for filtered *modules*, although still named 'graded algebra' in this case</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/43a976abb934c0aa43efabec2ca891ec44c452df\">43a976a</a></td><td><code>Merge branch 'public/categories/filtered_algebras-17096' of trac.sagemath.org:sage into public/categories/filtered_algebras-17096</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d1d135f03b4bed898ae018b8fec03786c8ca4c1a\">d1d135f</a></td><td><code>Give Weyl/Clifford/Exterior super powers.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ea66981c563c6920fad24f0c4a0581adfb411d7b\">ea66981</a></td><td><code>Merge branch 'public/combinat/crystals/crystal_plots' of trac.sagemath.org:sage into public/combinat/crystals/crystal_plots</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/349a9357828bc6d6ad56ac2c7f5b7349d9083eb8\">349a935</a></td><td><code>Fixing doctest.</code></td></tr></table>\n",
    "created_at": "2015-03-26T03:40:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15415#issuecomment-194225",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:13" align="right">Comment 13</div>

Branch pushed to git repo; I updated commit sha1. Last 10 new commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c2e84e1367cbfc47eb7104de654e2ee55caea95d">c2e84e1</a></td><td><code>remaining doctests for induced_graded_map</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/03bd4cfb53e24b24c65aada21f08850b40fb5c5c">03bd4cf</a></td><td><code>possibly controversial: graded_algebra and the three methods forming its interface are now cached_methods</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8a747c073e706950d1b14e77458539e126ff0550">8a747c0</a></td><td><code>further edits</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/831c26955993e11b4e11a9840643a7d4e91d77a4">831c269</a></td><td><code>Merge branch 'public/categories/filtered_algebras-17096' of git://trac.sagemath.org/sage into filt</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/752df7798b2ae6b9afc60528227b4e88c745fae9">752df77</a></td><td><code>Revert the cached_method decorators from commit "possibly controversial: graded_algebra and the three methods forming its interface are now cached_methods"</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/dfaaaeb6d9652e071ea89c9854da984bc8ada319">dfaaaeb</a></td><td><code>associated-graded now also defined for filtered *modules*, although still named 'graded algebra' in this case</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/43a976abb934c0aa43efabec2ca891ec44c452df">43a976a</a></td><td><code>Merge branch 'public/categories/filtered_algebras-17096' of trac.sagemath.org:sage into public/categories/filtered_algebras-17096</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d1d135f03b4bed898ae018b8fec03786c8ca4c1a">d1d135f</a></td><td><code>Give Weyl/Clifford/Exterior super powers.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ea66981c563c6920fad24f0c4a0581adfb411d7b">ea66981</a></td><td><code>Merge branch 'public/combinat/crystals/crystal_plots' of trac.sagemath.org:sage into public/combinat/crystals/crystal_plots</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/349a9357828bc6d6ad56ac2c7f5b7349d9083eb8">349a935</a></td><td><code>Fixing doctest.</code></td></tr></table>




---

archive/issue_comments_194226.json:
```json
{
    "body": "Changed commit from **[349a935](https://github.com/sagemath/sagetrac-mirror/commit/349a9357828bc6d6ad56ac2c7f5b7349d9083eb8)** to **[bac38bf](https://github.com/sagemath/sagetrac-mirror/commit/bac38bf2e8641c9a5b92f457429ce4dc1bb90267)**",
    "created_at": "2015-03-26T03:51:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15415#issuecomment-194226",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[349a935](https://github.com/sagemath/sagetrac-mirror/commit/349a9357828bc6d6ad56ac2c7f5b7349d9083eb8)** to **[bac38bf](https://github.com/sagemath/sagetrac-mirror/commit/bac38bf2e8641c9a5b92f457429ce4dc1bb90267)**



---

archive/issue_comments_194227.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/bac38bf2e8641c9a5b92f457429ce4dc1bb90267\">bac38bf</a></td><td><code>Fixing doctest.</code></td></tr></table>\n",
    "created_at": "2015-03-26T03:51:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15415#issuecomment-194227",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:14" align="right">Comment 14</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/bac38bf2e8641c9a5b92f457429ce4dc1bb90267">bac38bf</a></td><td><code>Fixing doctest.</code></td></tr></table>




---

archive/issue_comments_194228.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">Comment 15</div>\n\nLooks good to me.  Thanks, Travis!",
    "created_at": "2015-03-26T22:26:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15415#issuecomment-194228",
    "user": "https://github.com/bsalisbury1"
}
```

<div id="comment:15" align="right">Comment 15</div>

Looks good to me.  Thanks, Travis!



---

archive/issue_comments_194229.json:
```json
{
    "body": "Reviewer: **Ben Salisbury**",
    "created_at": "2015-03-26T22:26:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15415#issuecomment-194229",
    "user": "https://github.com/bsalisbury1"
}
```

Reviewer: **Ben Salisbury**



---

archive/issue_events_202094.json:
```json
{
    "actor": "https://github.com/bsalisbury1",
    "created_at": "2015-03-26T22:26:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15415#event-202094"
}
```



---

archive/issue_events_202095.json:
```json
{
    "actor": "https://github.com/bsalisbury1",
    "created_at": "2015-03-26T22:26:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15415#event-202095"
}
```



---

archive/issue_comments_194230.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">Comment 16</div>\n\nThanks for doing the review Ben!",
    "created_at": "2015-03-26T23:48:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15415#issuecomment-194230",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:16" align="right">Comment 16</div>

Thanks for doing the review Ben!



---

archive/issue_comments_194231.json:
```json
{
    "body": "Changed branch from **[public/combinat/crystals/crystal_plots](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/crystals/crystal_plots)** to **[bac38bf](https://github.com/sagemath/sagetrac-mirror/commit/bac38bf2e8641c9a5b92f457429ce4dc1bb90267)**",
    "created_at": "2015-04-14T19:44:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15415#issuecomment-194231",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/combinat/crystals/crystal_plots](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/crystals/crystal_plots)** to **[bac38bf](https://github.com/sagemath/sagetrac-mirror/commit/bac38bf2e8641c9a5b92f457429ce4dc1bb90267)**



---

archive/issue_events_202096.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-04-14T19:44:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15415#event-202096"
}
```



---

archive/issue_events_202097.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "1afa79922715de6ce38f6338c88f72b9d87294c7",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-04-14T19:44:17Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15415",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15415#event-202097"
}
```
