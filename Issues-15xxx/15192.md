# Issue 15192: add is_unit() for residue fields

archive/issues_014955.json:
```json
{
    "body": "Currently, residue fields have no `is_unit`:\n\n```\nsage: K.<z> = CyclotomicField(7)\nsage: P = K.factor(17)[0][0]\nsage: ff = K.residue_field(P)\nsage: a = ff(z)\nsage: a.is_unit()\nNotImplementedError\n```\n\nThey are fields but the implementation from `Fields.ElementMethods` is not used because the generic ring class defines a trivial `is_unit()` already. This ticket moves that method to the category of rings which gives a working `is_unit()` for residue fields.\n\nReviewer: David Roe\n\nAuthor: Julian Rueth\n\nBranch: u/saraedum/ticket/15192\n\nCommit: 953cf953cf7fa5a75cea895a7607c41d3a939be7\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/15192\n\n",
    "closed_at": "2014-01-05T06:54:04Z",
    "created_at": "2013-09-13T13:33:04Z",
    "labels": [
        "component: categories",
        "trivial"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.1",
    "title": "add is_unit() for residue fields",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/15192",
    "user": "https://github.com/saraedum"
}
```
Currently, residue fields have no `is_unit`:

```
sage: K.<z> = CyclotomicField(7)
sage: P = K.factor(17)[0][0]
sage: ff = K.residue_field(P)
sage: a = ff(z)
sage: a.is_unit()
NotImplementedError
```

They are fields but the implementation from `Fields.ElementMethods` is not used because the generic ring class defines a trivial `is_unit()` already. This ticket moves that method to the category of rings which gives a working `is_unit()` for residue fields.

Reviewer: David Roe

Author: Julian Rueth

Branch: u/saraedum/ticket/15192

Commit: 953cf953cf7fa5a75cea895a7607c41d3a939be7

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/15192





---

archive/issue_comments_239943.json:
```json
{
    "body": "Changing branch from \"\" to \"u/saraedum/ticket/15192\"",
    "created_at": "2013-09-13T13:43:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15192",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15192#issuecomment-239943",
    "user": "https://github.com/saraedum"
}
```

Changing branch from "" to "u/saraedum/ticket/15192"



---

archive/issue_comments_239944.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2013-09-13T13:44:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15192",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15192#issuecomment-239944",
    "user": "https://github.com/saraedum"
}
```

Changing status from new to needs_review.



---

archive/issue_events_043860.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2013-09-13T13:45:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15192",
    "milestone": "sage-6.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15192#event-43860"
}
```



---

archive/issue_comments_239945.json:
```json
{
    "body": "Changing author from \"\" to \"Julian Rueth\"",
    "created_at": "2013-09-13T13:45:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15192",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15192#issuecomment-239945",
    "user": "https://github.com/saraedum"
}
```

Changing author from "" to "Julian Rueth"



---

archive/issue_comments_239946.json:
```json
{
    "body": "<a id='comment:4'></a>I think there is a more fundamental problem: in your example,\n\n```\nsage: isinstance(a, FieldElement)\nFalse\n```\nThe class `FieldElement` does have an `is_unit()` method that does the right thing, but evidently it is not used because of this.",
    "created_at": "2013-09-13T16:43:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15192",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15192#issuecomment-239946",
    "user": "https://github.com/pjbruin"
}
```

<a id='comment:4'></a>I think there is a more fundamental problem: in your example,

```
sage: isinstance(a, FieldElement)
False
```
The class `FieldElement` does have an `is_unit()` method that does the right thing, but evidently it is not used because of this.



---

archive/issue_comments_239947.json:
```json
{
    "body": "<a id='comment:5'></a>Actually, it seems that *every* finite field element 'a' has the above problem.  Finite field elements should be made to inherit from field elements!",
    "created_at": "2013-09-13T16:54:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15192",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15192#issuecomment-239947",
    "user": "https://github.com/pjbruin"
}
```

<a id='comment:5'></a>Actually, it seems that *every* finite field element 'a' has the above problem.  Finite field elements should be made to inherit from field elements!



---

archive/issue_comments_239948.json:
```json
{
    "body": "<a id='comment:6'></a>Thanks for having a look at this.\n\nReplying to [pbruin](#comment%3A5):\n> Actually, it seems that *every* finite field element 'a' has the above problem.\n\nThat's scary.\n\n> Finite field elements should be made to inherit from field elements!\n\nI agree. I wonder if there is a reason why they don't?\n\nIn any case, should `Ring` really define `is_unit`? Isn't the idea that elements figure these things out depending on their parent, i.e., whether they are a unit depends on the parent. Putting this into the category framework seems to be the right thing. (Notice that `is_unit` was already there for `Fields()`). So is there any benefit of such an implementation in `Ring`? Don't all rings live in the category of `Rings()` now?",
    "created_at": "2013-09-13T17:00:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15192",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15192#issuecomment-239948",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:6'></a>Thanks for having a look at this.

Replying to [pbruin](#comment%3A5):
> Actually, it seems that *every* finite field element 'a' has the above problem.

That's scary.

> Finite field elements should be made to inherit from field elements!

I agree. I wonder if there is a reason why they don't?

In any case, should `Ring` really define `is_unit`? Isn't the idea that elements figure these things out depending on their parent, i.e., whether they are a unit depends on the parent. Putting this into the category framework seems to be the right thing. (Notice that `is_unit` was already there for `Fields()`). So is there any benefit of such an implementation in `Ring`? Don't all rings live in the category of `Rings()` now?



---

archive/issue_comments_239949.json:
```json
{
    "body": "<a id='comment:7'></a>Replying to [saraedum](#comment%3A6):\n\n> > Finite field elements should be made to inherit from field elements!\n\n> I agree. I wonder if there is a reason why they don't?\n\nProbably because they also derive from `IntegerMod` (for prime fields) and `FinitePolyExtElement`, respectively; I assume one could use multiple inheritance, but it might be tricky.  However, this should certainly be fixed.\n\n> In any case, should `Ring` really define `is_unit`?\n\n\n(I assume you mean `RingElement`...)\n\n> Isn't the idea that elements figure these things out depending on their parent, i.e., whether they are a unit depends on the parent.\n\n\nThat is clear; talking about elements doesn't even make much sense without mentioning parent sets.  But there isn't really any 'figuring out' to be done here: if the element knows that it is a field element, then it knows that being a unit is equivalent to being non-zero.  An element can either know that is is a field element because it inherits from `FieldElement`, or otherwise (e.g. if the parent can only determine at runtime if it is a field) using the `is_field()` method of the parent.\n\n> Putting this into the category framework seems to be the right thing.\n\n\nI don't agree, for the same reason why I wrote the comment at http://trac.sagemath.org/ticket/13441#comment:20.  Parents describe relations between elements; e.g. a ring is a set whose elements satisfy certain axioms.  Categories are on a higher level; they describe relations between objects, and it shouldn't matter too much what the elements of these objects are, or even if the objects are sets at all.\n\nWhether an element is unit depends on relations satisfied *within* its parent, not on  relations between the parent and *other* parents.  In other words, the category its parent is in should not play any role.  It is true that some categories (like the category of rings) *are* in a sense defined by relations between the *elements* of objects, but exploiting this to influence the behaviour of elements is confusing two different levels of abstraction, in my opinion.\n\nHence, I don't see a good mathematical reason for putting methods in `Ring.ElementMethods` instead of `RingElement`.  I can see why one would want a `Category` to have `ObjectMethods` (which are now called `ParentMethods`) and perhaps `MorphismMethods` (which only exist in a doctest).  With `ElementMethods`, however, I somehow get the feeling that the authors of the category framework are aiming for world domination!\n\n> (Notice that `is_unit` was already there for `Fields()`).\n\n\nYes, but `FieldElement` still has its own `is_unit()`.  I would expect that `RingElement` should have one, too, and that these are the correct locations for these methods, rather than `*.ElementMethods`.\n\n> So is there any benefit of such an implementation in `Ring`? Don't all rings live in the category of `Rings()` now?\n\n\nMaybe, but I don't think this is a good argument for using the category framework here.  I think the benefit of putting ring element methods in `RingElement` is that it simply seems the most logical place.\n\nSorry for the long sermon; this might have been more appropriate for a sage-devel discussion...",
    "created_at": "2013-09-13T18:32:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15192",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15192#issuecomment-239949",
    "user": "https://github.com/pjbruin"
}
```

<a id='comment:7'></a>Replying to [saraedum](#comment%3A6):

> > Finite field elements should be made to inherit from field elements!

> I agree. I wonder if there is a reason why they don't?

Probably because they also derive from `IntegerMod` (for prime fields) and `FinitePolyExtElement`, respectively; I assume one could use multiple inheritance, but it might be tricky.  However, this should certainly be fixed.

> In any case, should `Ring` really define `is_unit`?


(I assume you mean `RingElement`...)

> Isn't the idea that elements figure these things out depending on their parent, i.e., whether they are a unit depends on the parent.


That is clear; talking about elements doesn't even make much sense without mentioning parent sets.  But there isn't really any 'figuring out' to be done here: if the element knows that it is a field element, then it knows that being a unit is equivalent to being non-zero.  An element can either know that is is a field element because it inherits from `FieldElement`, or otherwise (e.g. if the parent can only determine at runtime if it is a field) using the `is_field()` method of the parent.

> Putting this into the category framework seems to be the right thing.


I don't agree, for the same reason why I wrote the comment at http://trac.sagemath.org/ticket/13441#comment:20.  Parents describe relations between elements; e.g. a ring is a set whose elements satisfy certain axioms.  Categories are on a higher level; they describe relations between objects, and it shouldn't matter too much what the elements of these objects are, or even if the objects are sets at all.

Whether an element is unit depends on relations satisfied *within* its parent, not on  relations between the parent and *other* parents.  In other words, the category its parent is in should not play any role.  It is true that some categories (like the category of rings) *are* in a sense defined by relations between the *elements* of objects, but exploiting this to influence the behaviour of elements is confusing two different levels of abstraction, in my opinion.

Hence, I don't see a good mathematical reason for putting methods in `Ring.ElementMethods` instead of `RingElement`.  I can see why one would want a `Category` to have `ObjectMethods` (which are now called `ParentMethods`) and perhaps `MorphismMethods` (which only exist in a doctest).  With `ElementMethods`, however, I somehow get the feeling that the authors of the category framework are aiming for world domination!

> (Notice that `is_unit` was already there for `Fields()`).


Yes, but `FieldElement` still has its own `is_unit()`.  I would expect that `RingElement` should have one, too, and that these are the correct locations for these methods, rather than `*.ElementMethods`.

> So is there any benefit of such an implementation in `Ring`? Don't all rings live in the category of `Rings()` now?


Maybe, but I don't think this is a good argument for using the category framework here.  I think the benefit of putting ring element methods in `RingElement` is that it simply seems the most logical place.

Sorry for the long sermon; this might have been more appropriate for a sage-devel discussion...



---

archive/issue_comments_239950.json:
```json
{
    "body": "<a id='comment:8'></a>Does trac send out another email when I edit this? Anyway, I'm confused. I'm actually not sure what `ElementMethods` are good for so I'd rather continue this discussion on sage-devel if you don't mind. Can I just post what you wrote there? Or maybe it is less confusing if you do?",
    "created_at": "2013-09-13T18:40:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15192",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15192#issuecomment-239950",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:8'></a>Does trac send out another email when I edit this? Anyway, I'm confused. I'm actually not sure what `ElementMethods` are good for so I'd rather continue this discussion on sage-devel if you don't mind. Can I just post what you wrote there? Or maybe it is less confusing if you do?



---

archive/issue_comments_239951.json:
```json
{
    "body": "<a id='comment:9'></a>Replying to [saraedum](#comment%3A8):\n> Does trac send out another email when I edit this? Anyway, I'm confused. I'm actually not sure what `ElementMethods` are good for so I'd rather continue this discussion on sage-devel if you don't mind. Can I just post what you wrote there? Or maybe it is less confusing if you do?\n\n\nI edited my comment and trac does not send out an email, so I'm not sure if you noticed. Could you comment on this please? Thanks.",
    "created_at": "2013-09-13T18:55:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15192",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15192#issuecomment-239951",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:9'></a>Replying to [saraedum](#comment%3A8):
> Does trac send out another email when I edit this? Anyway, I'm confused. I'm actually not sure what `ElementMethods` are good for so I'd rather continue this discussion on sage-devel if you don't mind. Can I just post what you wrote there? Or maybe it is less confusing if you do?


I edited my comment and trac does not send out an email, so I'm not sure if you noticed. Could you comment on this please? Thanks.



---

archive/issue_comments_239952.json:
```json
{
    "body": "<a id='comment:10'></a>Replying to [saraedum](#comment%3A8):\n> Does trac send out another email when I edit this?\n\nNo, it doesn't. :-)\n\n> Anyway, I'm confused. I'm actually not sure what `ElementMethods` are good for so I'd rather continue this discussion on sage-devel if you don't mind. Can I just post what you wrote there? Or maybe it is less confusing if you do?\n\nSure, I'd be happy if more people gave their opinion on this, and if someone who knows more about the category framework (maybe one of the authors) could explain the reason for `ElementMethods`.\n\nI can post a message to sage-devel, but won't have a lot of time to write more.",
    "created_at": "2013-09-13T19:03:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15192",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15192#issuecomment-239952",
    "user": "https://github.com/pjbruin"
}
```

<a id='comment:10'></a>Replying to [saraedum](#comment%3A8):
> Does trac send out another email when I edit this?

No, it doesn't. :-)

> Anyway, I'm confused. I'm actually not sure what `ElementMethods` are good for so I'd rather continue this discussion on sage-devel if you don't mind. Can I just post what you wrote there? Or maybe it is less confusing if you do?

Sure, I'd be happy if more people gave their opinion on this, and if someone who knows more about the category framework (maybe one of the authors) could explain the reason for `ElementMethods`.

I can post a message to sage-devel, but won't have a lot of time to write more.



---

archive/issue_comments_239953.json:
```json
{
    "body": "<a id='comment:11'></a>Replying to [pbruin](#comment%3A10):\n> Replying to [saraedum](#comment%3A8):\n> > Does trac send out another email when I edit this?\n\n> No, it doesn't. :-)\n> \n> > Anyway, I'm confused. I'm actually not sure what `ElementMethods` are good for so I'd rather continue this discussion on sage-devel if you don't mind. Can I just post what you wrote there? Or maybe it is less confusing if you do?\n\n> Sure, I'd be happy if more people gave their opinion on this, and if someone who knows more about the category framework (maybe one of the authors) could explain the reason for `ElementMethods`.\n> \n> I can post a message to sage-devel, but won't have a lot of time to write more.\n\n\nThanks. And no worries, it's great that you had a look at this in the first place.",
    "created_at": "2013-09-13T19:05:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15192",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15192#issuecomment-239953",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:11'></a>Replying to [pbruin](#comment%3A10):
> Replying to [saraedum](#comment%3A8):
> > Does trac send out another email when I edit this?

> No, it doesn't. :-)
> 
> > Anyway, I'm confused. I'm actually not sure what `ElementMethods` are good for so I'd rather continue this discussion on sage-devel if you don't mind. Can I just post what you wrote there? Or maybe it is less confusing if you do?

> Sure, I'd be happy if more people gave their opinion on this, and if someone who knows more about the category framework (maybe one of the authors) could explain the reason for `ElementMethods`.
> 
> I can post a message to sage-devel, but won't have a lot of time to write more.


Thanks. And no worries, it's great that you had a look at this in the first place.



---

archive/issue_comments_239954.json:
```json
{
    "body": "Changing work_issues from \"\" to \"waiting for input from sage-devel\"",
    "created_at": "2013-09-13T19:06:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15192",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15192#issuecomment-239954",
    "user": "https://github.com/saraedum"
}
```

Changing work_issues from "" to "waiting for input from sage-devel"



---

archive/issue_comments_239955.json:
```json
{
    "body": "Changing status from needs_review to needs_info.",
    "created_at": "2013-09-13T19:06:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15192",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15192#issuecomment-239955",
    "user": "https://github.com/saraedum"
}
```

Changing status from needs_review to needs_info.



---

archive/issue_comments_239956.json:
```json
{
    "body": "<a id='comment:13'></a>For the record: the discussion is here http://osdir.com/ml/sage-devel/2013-09/msg00086.html",
    "created_at": "2013-09-19T16:09:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15192",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15192#issuecomment-239956",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:13'></a>For the record: the discussion is here http://osdir.com/ml/sage-devel/2013-09/msg00086.html



---

archive/issue_comments_239957.json:
```json
{
    "body": "Changing work_issues from \"waiting for input from sage-devel\" to \"\"",
    "created_at": "2013-09-19T16:09:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15192",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15192#issuecomment-239957",
    "user": "https://github.com/saraedum"
}
```

Changing work_issues from "waiting for input from sage-devel" to ""



---

archive/issue_comments_239958.json:
```json
{
    "body": "Changing status from needs_info to needs_review.",
    "created_at": "2013-09-19T16:09:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15192",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15192#issuecomment-239958",
    "user": "https://github.com/saraedum"
}
```

Changing status from needs_info to needs_review.



---

archive/issue_events_043861.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2013-12-17T18:42:12Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15192",
    "milestone": "sage-6.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15192#event-43861"
}
```



---

archive/issue_events_043862.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2013-12-17T18:42:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15192",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15192#event-43862"
}
```



---

archive/issue_comments_239959.json:
```json
{
    "body": "<a id='comment:15'></a>Looks good.\n\n---\nNew commits:",
    "created_at": "2014-01-03T23:47:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15192",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15192#issuecomment-239959",
    "user": "https://github.com/roed314"
}
```

<a id='comment:15'></a>Looks good.

---
New commits:



---

archive/issue_comments_239960.json:
```json
{
    "body": "Changing commit from \"\" to \"953cf953cf7fa5a75cea895a7607c41d3a939be7\"",
    "created_at": "2014-01-03T23:47:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15192",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15192#issuecomment-239960",
    "user": "https://github.com/roed314"
}
```

Changing commit from "" to "953cf953cf7fa5a75cea895a7607c41d3a939be7"



---

archive/issue_comments_239961.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2014-01-03T23:47:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15192",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15192#issuecomment-239961",
    "user": "https://github.com/roed314"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_239962.json:
```json
{
    "body": "<a id='comment:16'></a>Fill in your names, kids! ;-)",
    "created_at": "2014-01-05T03:00:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15192",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15192#issuecomment-239962",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:16'></a>Fill in your names, kids! ;-)



---

archive/issue_comments_239963.json:
```json
{
    "body": "Changing reviewer from \"\" to \"David Roe\"",
    "created_at": "2014-01-05T03:00:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15192",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15192#issuecomment-239963",
    "user": "https://github.com/vbraun"
}
```

Changing reviewer from "" to "David Roe"



---

archive/issue_events_043863.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-01-05T06:54:04Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/15192",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15192#event-43863"
}
```



---

archive/issue_comments_239964.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2014-01-05T06:54:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15192",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15192#issuecomment-239964",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed
