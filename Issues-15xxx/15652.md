# Issue 15652: Always add -fno-strict-aliasing when compiling Cython code

archive/issues_015415.json:
```json
{
    "assignees": [],
    "body": "As reported for example by leif on #11687, the flag `-fno-strict-aliasing` should always be added when compiling Cython code. Normally, this flag is added but it disappears when the user has manually set `CFLAGS` (even if set to `\"\"`).\n\nCC:  @nexttime @kini\n\nBranch/Commit: **[u/jdemeyer/ticket/15652](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/15652) @ [5b79c2c](https://github.com/sagemath/sagetrac-mirror/commit/5b79c2cfc2f31eee3f9c66a7e8d2a7ccdf4fc5d2)**\n\nReviewer: **Fran\u00e7ois Bissey**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15652_\n\n",
    "closed_at": "2014-01-11T14:56:14Z",
    "created_at": "2014-01-09T09:56:12Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Always add -fno-strict-aliasing when compiling Cython code",
    "type": "issue",
    "updated_at": "2014-01-11T14:56:14Z",
    "url": "https://github.com/sagemath/sage/issues/15652",
    "user": "https://github.com/jdemeyer"
}
```
As reported for example by leif on #11687, the flag `-fno-strict-aliasing` should always be added when compiling Cython code. Normally, this flag is added but it disappears when the user has manually set `CFLAGS` (even if set to `""`).

CC:  @nexttime @kini

Branch/Commit: **[u/jdemeyer/ticket/15652](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/15652) @ [5b79c2c](https://github.com/sagemath/sagetrac-mirror/commit/5b79c2cfc2f31eee3f9c66a7e8d2a7ccdf4fc5d2)**

Reviewer: **François Bissey**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/15652_





---

archive/issue_events_205849.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-01-09T09:56:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15652",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15652#event-205849"
}
```



---

archive/issue_events_205850.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-01-09T09:56:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15652",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15652#event-205850"
}
```



---

archive/issue_events_205851.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-01-09T09:56:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15652",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15652#event-205851"
}
```



---

archive/issue_events_205852.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-01-09T09:56:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15652",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15652#event-205852"
}
```



---

archive/issue_comments_199263.json:
```json
{
    "body": "Branch: **[u/jdemeyer/ticket/15652](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/15652)**",
    "created_at": "2014-01-09T10:23:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15652",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15652#issuecomment-199263",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/ticket/15652](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/15652)**



---

archive/issue_events_205853.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-01-09T10:23:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15652",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15652#event-205853"
}
```



---

archive/issue_comments_199264.json:
```json
{
    "body": "Commit: **[5b79c2c](https://github.com/sagemath/sagetrac-mirror/commit/5b79c2cfc2f31eee3f9c66a7e8d2a7ccdf4fc5d2)**",
    "created_at": "2014-01-09T18:56:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15652",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15652#issuecomment-199264",
    "user": "https://github.com/kiwifb"
}
```

Commit: **[5b79c2c](https://github.com/sagemath/sagetrac-mirror/commit/5b79c2cfc2f31eee3f9c66a7e8d2a7ccdf4fc5d2)**



---

archive/issue_comments_199265.json:
```json
{
    "body": "Reviewer: **Fran\u00e7ois Bissey**",
    "created_at": "2014-01-09T18:56:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15652",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15652#issuecomment-199265",
    "user": "https://github.com/kiwifb"
}
```

Reviewer: **François Bissey**



---

archive/issue_comments_199266.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLooks like one of the oldest thing we have in sage-on-gentoo ebuilds. Except we don't patch setup.py to achieve it. That's one thing I can remove if this goes in. Positive from me.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5b79c2cfc2f31eee3f9c66a7e8d2a7ccdf4fc5d2\">5b79c2c</a></td><td><code>Always add -fno-strict-aliasing when compiling Cython extensions</code></td></tr></table>\n",
    "created_at": "2014-01-09T18:56:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15652",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15652#issuecomment-199266",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:3" align="right">Comment 3</div>

Looks like one of the oldest thing we have in sage-on-gentoo ebuilds. Except we don't patch setup.py to achieve it. That's one thing I can remove if this goes in. Positive from me.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5b79c2cfc2f31eee3f9c66a7e8d2a7ccdf4fc5d2">5b79c2c</a></td><td><code>Always add -fno-strict-aliasing when compiling Cython extensions</code></td></tr></table>




---

archive/issue_events_205854.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2014-01-09T18:56:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15652",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15652#event-205854"
}
```



---

archive/issue_events_205855.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2014-01-09T18:56:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15652",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15652#event-205855"
}
```



---

archive/issue_events_205856.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-01-11T14:56:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15652",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15652#event-205856"
}
```



---

archive/issue_events_205857.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "584a7cdbb207fb8a704092df357c7238782e7068",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-01-11T14:56:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15652",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15652#event-205857"
}
```
