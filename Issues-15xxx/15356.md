# Issue 15356: linear programming enhancements

archive/issues_015119.json:
```json
{
    "assignees": [],
    "body": "I've been using the linear programming for class, and love it!  Thanks!  I noticed a few areas that would make it more user-friendly:\n\n* it would be great if `p.get_variables()` returned the values of all of the variables by default, instead of insisting that you ask for a specific variable\n* Maybe a `p.show_solution()` method could be added that would print out the optimal objective function value and the values of all variables, if the problem was solved.\n* it would be cool if we wrapped the sensitivity analysis of various solvers.  Like http://en.wikibooks.org/wiki/GLPK/Solution_information#Sensitivity_analysis_report for GLPK.\n* the `maximization=False` parameter is a little awkward.  Perhaps that parameter could be renamed to `type=\"maximize\"` or `type=\"minimize\"`.  Or perhaps we could also have a `minimize=None` parameter, and if maximize and minimize are both None, the default is maximize (for compatibility), but if either is set, that is the problem, and if both are set to True, then we throw an error.\n\n\nCC:  @nathanncohen @mforets\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15356_\n\n",
    "created_at": "2013-11-05T21:20:38Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20programming",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "linear programming enhancements",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/15356",
    "user": "https://github.com/jasongrout"
}
```
I've been using the linear programming for class, and love it!  Thanks!  I noticed a few areas that would make it more user-friendly:

* it would be great if `p.get_variables()` returned the values of all of the variables by default, instead of insisting that you ask for a specific variable
* Maybe a `p.show_solution()` method could be added that would print out the optimal objective function value and the values of all variables, if the problem was solved.
* it would be cool if we wrapped the sensitivity analysis of various solvers.  Like http://en.wikibooks.org/wiki/GLPK/Solution_information#Sensitivity_analysis_report for GLPK.
* the `maximization=False` parameter is a little awkward.  Perhaps that parameter could be renamed to `type="maximize"` or `type="minimize"`.  Or perhaps we could also have a `minimize=None` parameter, and if maximize and minimize are both None, the default is maximize (for compatibility), but if either is set, that is the problem, and if both are set to True, then we throw an error.


CC:  @nathanncohen @mforets

_Issue created by migration from https://trac.sagemath.org/ticket/15356_





---

archive/issue_events_201033.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2013-11-05T21:20:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15356",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15356#event-201033"
}
```



---

archive/issue_events_201034.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2013-11-05T21:20:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15356",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20programming",
    "label_color": "0000ff",
    "label_name": "component: linear programming",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15356#event-201034"
}
```



---

archive/issue_events_201035.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2013-11-05T21:20:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15356",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15356#event-201035"
}
```



---

archive/issue_events_201036.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2013-11-05T21:20:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15356",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15356#event-201036"
}
```



---

archive/issue_events_201037.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15356",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15356#event-201037"
}
```



---

archive/issue_events_201038.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15356",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15356#event-201038"
}
```



---

archive/issue_events_201039.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15356",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15356#event-201039"
}
```



---

archive/issue_events_201040.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15356",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15356#event-201040"
}
```



---

archive/issue_comments_192911.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nNote also that #14288 is positively reviewed!",
    "created_at": "2014-06-21T01:34:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15356",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15356#issuecomment-192911",
    "user": "https://github.com/novoselt"
}
```

<div id="comment:3" align="right">Comment 3</div>

Note also that #14288 is positively reviewed!



---

archive/issue_events_201041.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15356",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15356#event-201041"
}
```



---

archive/issue_events_201042.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15356",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15356#event-201042"
}
```



---

archive/issue_events_201043.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15356",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15356#event-201043"
}
```
