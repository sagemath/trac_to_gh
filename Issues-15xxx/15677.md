# Issue 15677: Packages shipping too old libtool fail to build on Cygwin64

archive/issues_015440.json:
```json
{
    "assignees": [],
    "body": "This includes:\n* libpng (running autorconf -fiv updates it correctly),\n* freetype (running ./autogen.sh updates it correctly),\n* gd (running autorconf -fiv updates it correctly).\n\nSome of these packages may also be outdated so an upgrade might solves the issue.\n\nCC:  @kcrisman @dimpase @tscrim\n\nKeywords: **cygwin libtool**\n\nReviewer: **Jean-Pierre Flori**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15677_\n\n",
    "closed_at": "2014-12-04T18:37:46Z",
    "created_at": "2014-01-15T10:00:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Packages shipping too old libtool fail to build on Cygwin64",
    "type": "issue",
    "updated_at": "2014-12-04T18:37:46Z",
    "url": "https://github.com/sagemath/sage/issues/15677",
    "user": "https://github.com/jpflori"
}
```
This includes:
* libpng (running autorconf -fiv updates it correctly),
* freetype (running ./autogen.sh updates it correctly),
* gd (running autorconf -fiv updates it correctly).

Some of these packages may also be outdated so an upgrade might solves the issue.

CC:  @kcrisman @dimpase @tscrim

Keywords: **cygwin libtool**

Reviewer: **Jean-Pierre Flori**

_Issue created by migration from https://trac.sagemath.org/ticket/15677_





---

archive/issue_events_206231.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2014-01-15T10:00:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15677",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15677#event-206231"
}
```



---

archive/issue_events_206232.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2014-01-15T10:00:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15677",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
    "label_color": "000080",
    "label_name": "component: porting: cygwin",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15677#event-206232"
}
```



---

archive/issue_events_206233.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2014-01-15T10:00:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15677",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15677#event-206233"
}
```



---

archive/issue_events_206234.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2014-01-15T10:00:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15677",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15677#event-206234"
}
```



---

archive/issue_events_206235.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15677",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15677#event-206235"
}
```



---

archive/issue_events_206236.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15677",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15677#event-206236"
}
```



---

archive/issue_comments_199691.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nLatest version of libpng is ok.\nSee #16099.",
    "created_at": "2014-04-08T13:21:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15677",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15677#issuecomment-199691",
    "user": "https://github.com/jpflori"
}
```

<div id="comment:2" align="right">Comment 2</div>

Latest version of libpng is ok.
See #16099.



---

archive/issue_comments_199692.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nfreetype has been updated as well and is ok now.\nSee #15561.",
    "created_at": "2014-04-13T19:35:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15677",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15677#issuecomment-199692",
    "user": "https://github.com/jpflori"
}
```

<div id="comment:3" align="right">Comment 3</div>

freetype has been updated as well and is ok now.
See #15561.



---

archive/issue_events_206237.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15677",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15677#event-206237"
}
```



---

archive/issue_events_206238.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15677",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15677#event-206238"
}
```



---

archive/issue_events_206239.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15677",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15677#event-206239"
}
```



---

archive/issue_events_206240.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15677",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15677#event-206240"
}
```



---

archive/issue_comments_199693.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nIt seems latest gd 2.1.0 still ships a too ancient libtool.",
    "created_at": "2014-11-13T14:25:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15677",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15677#issuecomment-199693",
    "user": "https://github.com/jpflori"
}
```

<div id="comment:6" align="right">Comment 6</div>

It seems latest gd 2.1.0 still ships a too ancient libtool.



---

archive/issue_comments_199694.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nEverything is ok now, even gd which we autoreconf ourselves, see #17334.",
    "created_at": "2014-11-25T14:41:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15677",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15677#issuecomment-199694",
    "user": "https://github.com/jpflori"
}
```

<div id="comment:7" align="right">Comment 7</div>

Everything is ok now, even gd which we autoreconf ourselves, see #17334.



---

archive/issue_comments_199695.json:
```json
{
    "body": "Reviewer: **Jean-Pierre Flori**",
    "created_at": "2014-11-25T14:41:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15677",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15677#issuecomment-199695",
    "user": "https://github.com/jpflori"
}
```

Reviewer: **Jean-Pierre Flori**



---

archive/issue_events_206241.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2014-11-25T14:41:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15677",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15677#event-206241"
}
```



---

archive/issue_events_206242.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2014-11-25T14:41:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15677",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15677#event-206242"
}
```



---

archive/issue_events_206243.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2014-11-25T14:41:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15677",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15677#event-206243"
}
```



---

archive/issue_comments_199696.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nwhat do you want to do, is this invalid? Please set milestone then.",
    "created_at": "2014-11-27T22:27:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15677",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15677#issuecomment-199696",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:9" align="right">Comment 9</div>

what do you want to do, is this invalid? Please set milestone then.



---

archive/issue_comments_199697.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nI think this was more of a task/meta-ticket.",
    "created_at": "2014-11-27T23:04:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15677",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15677#issuecomment-199697",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:10" align="right">Comment 10</div>

I think this was more of a task/meta-ticket.



---

archive/issue_events_206244.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-11-27T23:04:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15677",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15677#event-206244"
}
```



---

archive/issue_comments_199698.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\npositive review to what?",
    "created_at": "2014-11-28T18:36:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15677",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15677#issuecomment-199698",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:11" align="right">Comment 11</div>

positive review to what?



---

archive/issue_events_206245.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-11-28T18:36:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15677",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15677#event-206245"
}
```



---

archive/issue_events_206246.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-11-28T18:36:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15677",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15677#event-206246"
}
```



---

archive/issue_events_206247.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2014-12-01T22:28:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15677",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15677#event-206247"
}
```



---

archive/issue_events_206248.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2014-12-01T22:28:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15677",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15677#event-206248"
}
```



---

archive/issue_comments_199699.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nThere is nothing to do here anymore, everything was fixed in other tickets.",
    "created_at": "2014-12-01T22:28:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15677",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15677#issuecomment-199699",
    "user": "https://github.com/jpflori"
}
```

<div id="comment:12" align="right">Comment 12</div>

There is nothing to do here anymore, everything was fixed in other tickets.



---

archive/issue_events_206249.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2014-12-01T22:28:41Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15677",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15677#event-206249"
}
```



---

archive/issue_events_206250.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2014-12-01T22:28:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15677",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15677#event-206250"
}
```



---

archive/issue_events_206251.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2014-12-01T22:28:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15677",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15677#event-206251"
}
```



---

archive/issue_events_206252.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-12-04T18:37:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15677",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15677#event-206252"
}
```



---

archive/issue_events_206253.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-12-04T18:37:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15677",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15677#event-206253"
}
```
