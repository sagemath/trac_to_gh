# Issue 15838: tell user about sage crash before doing dev work

archive/issues_015601.json:
```json
{
    "assignees": [],
    "body": "Due to a missing SPKG Sage did not build, but when I wanted to `sage -dev push` it said\n\n```\nDeveloper interface disabled.\n\nTo develop for Sage you must compile Sage from its git\nrepository. This just amounts to running\n\n    git clone git://github.com/sagemath/sage.git\n    cd sage\n    make\n\nSee http://trac.sagemath.org/wiki/QuickStartSageGit for slightly more\ninformation.\n```\nRather it should give the same `Oops, Sage crashed...` message which it does when I try to start it without options.\n\nKeywords: **crash message**\n\nComponent: **user interface**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15838_\n\n",
    "closed_at": "2015-01-13T01:18:46Z",
    "created_at": "2014-02-20T15:16:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "tell user about sage crash before doing dev work",
    "type": "issue",
    "updated_at": "2015-01-13T01:18:46Z",
    "url": "https://github.com/sagemath/sage/issues/15838",
    "user": "https://github.com/rwst"
}
```
Due to a missing SPKG Sage did not build, but when I wanted to `sage -dev push` it said

```
Developer interface disabled.

To develop for Sage you must compile Sage from its git
repository. This just amounts to running

    git clone git://github.com/sagemath/sage.git
    cd sage
    make

See http://trac.sagemath.org/wiki/QuickStartSageGit for slightly more
information.
```
Rather it should give the same `Oops, Sage crashed...` message which it does when I try to start it without options.

Keywords: **crash message**

Component: **user interface**

_Issue created by migration from https://trac.sagemath.org/ticket/15838_





---

archive/issue_events_226183.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-02-20T15:16:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15838",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15838#event-226183"
}
```



---

archive/issue_events_226184.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-02-20T15:16:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15838",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "000080",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15838#event-226184"
}
```



---

archive/issue_events_226185.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-02-20T15:16:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15838",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15838#event-226185"
}
```



---

archive/issue_events_226186.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-02-20T15:16:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15838",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15838#event-226186"
}
```



---

archive/issue_events_226187.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15838",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15838#event-226187"
}
```



---

archive/issue_events_226188.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15838",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15838#event-226188"
}
```



---

archive/issue_events_226189.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-06-05T15:59:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15838",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15838#event-226189"
}
```



---

archive/issue_events_226190.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-06-05T15:59:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15838",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15838#event-226190"
}
```



---

archive/issue_events_226191.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15838",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15838#event-226191"
}
```



---

archive/issue_events_226192.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15838",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15838#event-226192"
}
```



---

archive/issue_comments_200509.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nwontfix because of removal of dev scripts, see https://groups.google.com/forum/?hl=en#!topic/sage-devel/JtVWAdvw-3A",
    "created_at": "2014-12-27T07:09:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15838",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15838#issuecomment-200509",
    "user": "https://github.com/rwst"
}
```

<div id="comment:4" align="right">comment:4</div>

wontfix because of removal of dev scripts, see https://groups.google.com/forum/?hl=en#!topic/sage-devel/JtVWAdvw-3A



---

archive/issue_events_226193.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-12-27T07:09:33Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15838",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15838#event-226193"
}
```



---

archive/issue_events_226194.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-12-27T07:09:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15838",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15838#event-226194"
}
```



---

archive/issue_events_226195.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-12-27T07:09:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15838",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15838#event-226195"
}
```



---

archive/issue_events_226196.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-12-27T07:09:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15838",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15838#event-226196"
}
```



---

archive/issue_events_226197.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2015-01-09T13:39:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15838",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15838#event-226197"
}
```



---

archive/issue_events_226198.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2015-01-09T13:39:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15838",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15838#event-226198"
}
```



---

archive/issue_events_226199.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-01-13T01:18:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15838",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "a6a6a6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15838#event-226199"
}
```



---

archive/issue_events_226200.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-01-13T01:18:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15838",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15838#event-226200"
}
```



---

archive/issue_events_226201.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-01-13T01:18:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15838",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15838#event-226201"
}
```
