# Issue 15897: UniqueFactory for QuaternionAlgebras

archive/issues_015660.json:
```json
{
    "assignees": [],
    "body": "Quaternion algebras use a generic dictionary to cache objects (instead of\n`UniqueRepresentation` or `UniqueFactory`). This ticket replaces this generic\ncache with a `UniqueFactory` implementation.\n\nBranch/Commit: **[bdaaaff](https://github.com/sagemath/sagetrac-mirror/commit/bdaaaffa4603355c0ef5afad8c43e42766c2dfcf)**\n\nReviewer: **David Roe, Peter Bruin**\n\nAuthor: **Julian Rueth**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15897_\n\n",
    "closed_at": "2014-05-13T07:58:38Z",
    "created_at": "2014-03-06T13:36:59Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "UniqueFactory for QuaternionAlgebras",
    "type": "issue",
    "updated_at": "2014-05-13T07:58:38Z",
    "url": "https://github.com/sagemath/sage/issues/15897",
    "user": "https://github.com/saraedum"
}
```
Quaternion algebras use a generic dictionary to cache objects (instead of
`UniqueRepresentation` or `UniqueFactory`). This ticket replaces this generic
cache with a `UniqueFactory` implementation.

Branch/Commit: **[bdaaaff](https://github.com/sagemath/sagetrac-mirror/commit/bdaaaffa4603355c0ef5afad8c43e42766c2dfcf)**

Reviewer: **David Roe, Peter Bruin**

Author: **Julian Rueth**

_Issue created by migration from https://trac.sagemath.org/ticket/15897_





---

archive/issue_events_209450.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2014-03-06T13:36:59Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15897",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15897#event-209450"
}
```



---

archive/issue_events_209451.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2014-03-06T13:36:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15897",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
    "label_color": "0000ff",
    "label_name": "component: commutative algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15897#event-209451"
}
```



---

archive/issue_events_209452.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2014-03-06T13:36:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15897",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15897#event-209452"
}
```



---

archive/issue_events_209453.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2014-03-06T13:36:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15897",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15897#event-209453"
}
```



---

archive/issue_comments_203856.json:
```json
{
    "body": "Branch: **[u/saraedum/ticket/15897](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/15897)**",
    "created_at": "2014-03-06T13:41:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15897#issuecomment-203856",
    "user": "https://github.com/saraedum"
}
```

Branch: **[u/saraedum/ticket/15897](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/15897)**



---

archive/issue_events_209454.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2014-03-06T13:43:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15897",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15897#event-209454"
}
```



---

archive/issue_comments_203857.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/dba5677ceacb5531b6d1266e489412804c7f3934\">dba5677</a></td><td><code>Use a UniqueFactory to cache instances of QuaternionAlgebra</code></td></tr></table>\n",
    "created_at": "2014-03-06T13:43:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15897#issuecomment-203857",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/dba5677ceacb5531b6d1266e489412804c7f3934">dba5677</a></td><td><code>Use a UniqueFactory to cache instances of QuaternionAlgebra</code></td></tr></table>




---

archive/issue_comments_203858.json:
```json
{
    "body": "Author: **Julian Rueth**",
    "created_at": "2014-03-06T13:43:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15897#issuecomment-203858",
    "user": "https://github.com/saraedum"
}
```

Author: **Julian Rueth**



---

archive/issue_comments_203859.json:
```json
{
    "body": "Commit: **[dba5677](https://github.com/sagemath/sagetrac-mirror/commit/dba5677ceacb5531b6d1266e489412804c7f3934)**",
    "created_at": "2014-03-06T13:43:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15897#issuecomment-203859",
    "user": "https://github.com/saraedum"
}
```

Commit: **[dba5677](https://github.com/sagemath/sagetrac-mirror/commit/dba5677ceacb5531b6d1266e489412804c7f3934)**



---

archive/issue_comments_203860.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nLooks good to me.",
    "created_at": "2014-03-06T21:42:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15897#issuecomment-203860",
    "user": "https://github.com/roed314"
}
```

<a id='comment:3'>Comment 3:</a>
Looks good to me.



---

archive/issue_events_209455.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2014-03-06T21:42:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15897",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15897#event-209455"
}
```



---

archive/issue_events_209456.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2014-03-06T21:42:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15897",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15897#event-209456"
}
```



---

archive/issue_comments_203861.json:
```json
{
    "body": "Reviewer: **David Roe**",
    "created_at": "2014-03-06T21:43:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15897#issuecomment-203861",
    "user": "https://github.com/roed314"
}
```

Reviewer: **David Roe**



---

archive/issue_events_209457.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-03-11T21:17:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15897",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15897#event-209457"
}
```



---

archive/issue_events_209458.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-03-11T21:17:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15897",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15897#event-209458"
}
```



---

archive/issue_comments_203862.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\n\n```\nsage -t src/sage/algebras/quaternion_algebra.py\n**********************************************************************\nFile \"src/sage/algebras/quaternion_algebra.py\", line 12, in sage.algebras.quaternion_algebra.unpickle_QuaternionAlgebra_v0\nFailed example:\n    f, t = Q.__reduce__()\nException raised:\n    Traceback (most recent call last):\n      File \"/home/release/Sage/local/lib/python2.7/site-packages/sage/doctest/forker.py\", line 480, in _run\n        self.execute(example, compiled, test.globs)\n      File \"/home/release/Sage/local/lib/python2.7/site-packages/sage/doctest/forker.py\", line 839, in execute\n        exec compiled in globs\n      File \"<doctest sage.algebras.quaternion_algebra.unpickle_QuaternionAlgebra_v0[1]>\", line 1, in <module>\n        f, t = Q.__reduce__()\n    ValueError: too many values to unpack\n**********************************************************************\nFile \"src/sage/algebras/quaternion_algebra.py\", line 14, in sage.algebras.quaternion_algebra.unpickle_QuaternionAlgebra_v0\nFailed example:\n    sage.algebras.quaternion_algebra.unpickle_QuaternionAlgebra_v0(*t)\nException raised:\n    Traceback (most recent call last):\n      File \"/home/release/Sage/local/lib/python2.7/site-packages/sage/doctest/forker.py\", line 480, in _run\n        self.execute(example, compiled, test.globs)\n      File \"/home/release/Sage/local/lib/python2.7/site-packages/sage/doctest/forker.py\", line 839, in execute\n        exec compiled in globs\n      File \"<doctest sage.algebras.quaternion_algebra.unpickle_QuaternionAlgebra_v0[3]>\", line 1, in <module>\n        sage.algebras.quaternion_algebra.unpickle_QuaternionAlgebra_v0(*t)\n    NameError: name 't' is not defined\n**********************************************************************\n1 item had failures:\n   2 of   5 in sage.algebras.quaternion_algebra.unpickle_QuaternionAlgebra_v0\n    [4 tests, 2 failures, 0.01 s]\n----------------------------------------------------------------------\nsage -t src/sage/algebras/quaternion_algebra.py  # 2 doctests failed\n----------------------------------------------------------------------\n```",
    "created_at": "2014-03-11T21:17:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15897#issuecomment-203862",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:5'>Comment 5:</a>

```
sage -t src/sage/algebras/quaternion_algebra.py
**********************************************************************
File "src/sage/algebras/quaternion_algebra.py", line 12, in sage.algebras.quaternion_algebra.unpickle_QuaternionAlgebra_v0
Failed example:
    f, t = Q.__reduce__()
Exception raised:
    Traceback (most recent call last):
      File "/home/release/Sage/local/lib/python2.7/site-packages/sage/doctest/forker.py", line 480, in _run
        self.execute(example, compiled, test.globs)
      File "/home/release/Sage/local/lib/python2.7/site-packages/sage/doctest/forker.py", line 839, in execute
        exec compiled in globs
      File "<doctest sage.algebras.quaternion_algebra.unpickle_QuaternionAlgebra_v0[1]>", line 1, in <module>
        f, t = Q.__reduce__()
    ValueError: too many values to unpack
**********************************************************************
File "src/sage/algebras/quaternion_algebra.py", line 14, in sage.algebras.quaternion_algebra.unpickle_QuaternionAlgebra_v0
Failed example:
    sage.algebras.quaternion_algebra.unpickle_QuaternionAlgebra_v0(*t)
Exception raised:
    Traceback (most recent call last):
      File "/home/release/Sage/local/lib/python2.7/site-packages/sage/doctest/forker.py", line 480, in _run
        self.execute(example, compiled, test.globs)
      File "/home/release/Sage/local/lib/python2.7/site-packages/sage/doctest/forker.py", line 839, in execute
        exec compiled in globs
      File "<doctest sage.algebras.quaternion_algebra.unpickle_QuaternionAlgebra_v0[3]>", line 1, in <module>
        sage.algebras.quaternion_algebra.unpickle_QuaternionAlgebra_v0(*t)
    NameError: name 't' is not defined
**********************************************************************
1 item had failures:
   2 of   5 in sage.algebras.quaternion_algebra.unpickle_QuaternionAlgebra_v0
    [4 tests, 2 failures, 0.01 s]
----------------------------------------------------------------------
sage -t src/sage/algebras/quaternion_algebra.py  # 2 doctests failed
----------------------------------------------------------------------
```



---

archive/issue_comments_203863.json:
```json
{
    "body": "Changed commit from **[dba5677](https://github.com/sagemath/sagetrac-mirror/commit/dba5677ceacb5531b6d1266e489412804c7f3934)** to **[bdaaaff](https://github.com/sagemath/sagetrac-mirror/commit/bdaaaffa4603355c0ef5afad8c43e42766c2dfcf)**",
    "created_at": "2014-04-09T17:55:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15897#issuecomment-203863",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[dba5677](https://github.com/sagemath/sagetrac-mirror/commit/dba5677ceacb5531b6d1266e489412804c7f3934)** to **[bdaaaff](https://github.com/sagemath/sagetrac-mirror/commit/bdaaaffa4603355c0ef5afad8c43e42766c2dfcf)**



---

archive/issue_comments_203864.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/bdaaaffa4603355c0ef5afad8c43e42766c2dfcf\">bdaaaff</a></td><td><code>Fixed a pickling doctest for QuaternionAlgebras</code></td></tr></table>\n",
    "created_at": "2014-04-09T17:55:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15897#issuecomment-203864",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:6'>Comment 6:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/bdaaaffa4603355c0ef5afad8c43e42766c2dfcf">bdaaaff</a></td><td><code>Fixed a pickling doctest for QuaternionAlgebras</code></td></tr></table>




---

archive/issue_events_209459.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2014-04-09T17:55:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15897",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15897#event-209459"
}
```



---

archive/issue_events_209460.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2014-04-09T17:55:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15897",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15897#event-209460"
}
```



---

archive/issue_events_209461.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15897",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15897#event-209461"
}
```



---

archive/issue_events_209462.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15897",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15897#event-209462"
}
```



---

archive/issue_events_209463.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2014-05-10T01:29:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15897",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
    "label_color": "0000ff",
    "label_name": "component: commutative algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15897#event-209463"
}
```



---

archive/issue_events_209464.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2014-05-10T01:29:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15897",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15897#event-209464"
}
```



---

archive/issue_events_209465.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2014-05-10T01:29:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15897",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15897#event-209465"
}
```



---

archive/issue_events_209466.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2014-05-10T01:29:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15897",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15897#event-209466"
}
```



---

archive/issue_comments_203865.json:
```json
{
    "body": "Changed reviewer from **David Roe** to **David Roe, Peter Bruin**",
    "created_at": "2014-05-10T01:29:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15897#issuecomment-203865",
    "user": "https://github.com/pjbruin"
}
```

Changed reviewer from **David Roe** to **David Roe, Peter Bruin**



---

archive/issue_comments_203866.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nLooks good and the patchbot is happy.  (But quaternion algebras are not commutative!)",
    "created_at": "2014-05-10T01:29:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15897#issuecomment-203866",
    "user": "https://github.com/pjbruin"
}
```

<a id='comment:9'>Comment 9:</a>
Looks good and the patchbot is happy.  (But quaternion algebras are not commutative!)



---

archive/issue_comments_203867.json:
```json
{
    "body": "Changed branch from **[u/saraedum/ticket/15897](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/15897)** to **[bdaaaff](https://github.com/sagemath/sagetrac-mirror/commit/bdaaaffa4603355c0ef5afad8c43e42766c2dfcf)**",
    "created_at": "2014-05-13T07:58:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15897",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15897#issuecomment-203867",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/saraedum/ticket/15897](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/15897)** to **[bdaaaff](https://github.com/sagemath/sagetrac-mirror/commit/bdaaaffa4603355c0ef5afad8c43e42766c2dfcf)**



---

archive/issue_events_209467.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-05-13T07:58:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15897",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15897#event-209467"
}
```



---

archive/issue_events_209468.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "fedf21483e59f57fcec5f85d8c5510cb65eaa03d",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-05-13T07:58:38Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15897",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15897#event-209468"
}
```
