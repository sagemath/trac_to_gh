# Issue 15293: Lazy import root systems, crystals and other combinatorics features to save on startup time.

archive/issues_015056.json:
```json
{
    "body": "The title says it all. See also #14652.\n\nThe attached patch is but a proof of concept to evaluate how much we\ncan hope to gain. And that's a rough 6% which is not so bad :-)\n\nWithout patch:\n\n```\nTotal time (sum over exclusive time): 2871.367ms\nTotal time (sum over exclusive time): 2881.301ms\nTotal time (sum over exclusive time): 2876.823ms\nTotal time (sum over exclusive time): 2859.830ms\nTotal time (sum over exclusive time): 2865.218ms\nTotal time (sum over exclusive time): 2867.298ms\nTotal time (sum over exclusive time): 2864.525ms\nTotal time (sum over exclusive time): 2919.009ms\n```\n\nWith patch:\n\n```\nTotal time (sum over exclusive time): 2685.894ms\nTotal time (sum over exclusive time): 2691.945ms\nTotal time (sum over exclusive time): 2687.476ms\nTotal time (sum over exclusive time): 2681.121ms\nTotal time (sum over exclusive time): 2708.517ms\nTotal time (sum over exclusive time): 2688.468ms\nTotal time (sum over exclusive time): 2684.715ms\n```\n\nI haven't checked yet that all test pass properly, and this patch\nprobably conflicts with other patches. So no rush to get it in, the\ngoal is just to evaluate whether we can make up for the little startup\ntime loss with #10963.\n\nA strange thing is that, from reading the startup report, most of the\nprogress comes from lazy importing the module\nsage/combinat.abstract_tree which does not seem that particular.\n\nThe dependency upon #10963 is probably just syntactic; it might actually work without it.\n\nCC:  sage-combinat simonking @tscrim\n\nKeywords: startup\n\nWork_Issues: Check that all test pass, check it does not conflict with other patches.\n\nAuthor: Nicolas M. Thi\u00e9ry\n\nStatus: new\n\nDependencies: #10963\n\nIssue created by migration from https://trac.sagemath.org/ticket/15293\n\n",
    "created_at": "2013-10-16T16:49:30Z",
    "labels": [
        "component: performance"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.4",
    "title": "Lazy import root systems, crystals and other combinatorics features to save on startup time.",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/15293",
    "user": "https://github.com/nthiery"
}
```
The title says it all. See also #14652.

The attached patch is but a proof of concept to evaluate how much we
can hope to gain. And that's a rough 6% which is not so bad :-)

Without patch:

```
Total time (sum over exclusive time): 2871.367ms
Total time (sum over exclusive time): 2881.301ms
Total time (sum over exclusive time): 2876.823ms
Total time (sum over exclusive time): 2859.830ms
Total time (sum over exclusive time): 2865.218ms
Total time (sum over exclusive time): 2867.298ms
Total time (sum over exclusive time): 2864.525ms
Total time (sum over exclusive time): 2919.009ms
```

With patch:

```
Total time (sum over exclusive time): 2685.894ms
Total time (sum over exclusive time): 2691.945ms
Total time (sum over exclusive time): 2687.476ms
Total time (sum over exclusive time): 2681.121ms
Total time (sum over exclusive time): 2708.517ms
Total time (sum over exclusive time): 2688.468ms
Total time (sum over exclusive time): 2684.715ms
```

I haven't checked yet that all test pass properly, and this patch
probably conflicts with other patches. So no rush to get it in, the
goal is just to evaluate whether we can make up for the little startup
time loss with #10963.

A strange thing is that, from reading the startup report, most of the
progress comes from lazy importing the module
sage/combinat.abstract_tree which does not seem that particular.

The dependency upon #10963 is probably just syntactic; it might actually work without it.

CC:  sage-combinat simonking @tscrim

Keywords: startup

Work_Issues: Check that all test pass, check it does not conflict with other patches.

Author: Nicolas M. Thiéry

Status: new

Dependencies: #10963

Issue created by migration from https://trac.sagemath.org/ticket/15293





---

archive/issue_comments_242256.json:
```json
{
    "body": "Changing keywords from \"\" to \"startup\".",
    "created_at": "2013-10-28T08:30:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15293",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15293#issuecomment-242256",
    "user": "https://github.com/fchapoton"
}
```

Changing keywords from "" to "startup".



---

archive/issue_comments_242257.json:
```json
{
    "body": "<a id='comment:1'></a>Attachment [trac_15293-lazy_import_combinat_stuff-nt.patch](tarball://root/attachments/some-uuid/ticket15293/trac_15293-lazy_import_combinat_stuff-nt.patch) by @fchapoton created at 2013-10-28 08:30:02",
    "created_at": "2013-10-28T08:30:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15293",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15293#issuecomment-242257",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:1'></a>Attachment [trac_15293-lazy_import_combinat_stuff-nt.patch](tarball://root/attachments/some-uuid/ticket15293/trac_15293-lazy_import_combinat_stuff-nt.patch) by @fchapoton created at 2013-10-28 08:30:02



---

archive/issue_events_044282.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15293",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15293#event-44282"
}
```



---

archive/issue_events_044283.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15293",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15293#event-44283"
}
```



---

archive/issue_events_044284.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15293",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15293#event-44284"
}
```



---

archive/issue_events_044285.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15293",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15293#event-44285"
}
```



---

archive/issue_events_044286.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15293",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15293#event-44286"
}
```
