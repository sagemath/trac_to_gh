# Issue 15813: make cunningham_tables-1.0.spkg standard and upgrade it

archive/issues_015576.json:
```json
{
    "assignees": [],
    "body": "The optional spkg cunningham_tables-1.0 has no tickets, and is now available since sage-4.3.1. It is remarkably small.\n\nAccording to #7240, it was voted to be included as standard. The newest version would be http://sage.math.washington.edu/home/roed/cunningham_tables-2.2.spkg\n\nIt however uses the obsolete SAGE_DATA, so a fixed version of the SPKG is attached. Also, in #7240 David Roe uploaded a patch that provides access to the tables. This is the first commit visible here. It however misses and changes some things.\n\nNeeded for #7240.\n\nCC:  @roed314\n\nKeywords: **package, cunningham, factorization**\n\nBranch/Commit: **[u/rws/ticket/15813](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/ticket/15813) @ [7738db3](https://github.com/sagemath/sagetrac-mirror/commit/7738db3a4e19b16e3dd03495e92444443e2c65a4)**\n\nAuthor: **David Roe, Ralf Stephan**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15813_\n\n",
    "closed_at": "2018-05-18T17:16:26Z",
    "created_at": "2014-02-13T09:00:56Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "make cunningham_tables-1.0.spkg standard and upgrade it",
    "type": "issue",
    "updated_at": "2018-05-18T17:16:26Z",
    "url": "https://github.com/sagemath/sage/issues/15813",
    "user": "https://github.com/rwst"
}
```
The optional spkg cunningham_tables-1.0 has no tickets, and is now available since sage-4.3.1. It is remarkably small.

According to #7240, it was voted to be included as standard. The newest version would be http://sage.math.washington.edu/home/roed/cunningham_tables-2.2.spkg

It however uses the obsolete SAGE_DATA, so a fixed version of the SPKG is attached. Also, in #7240 David Roe uploaded a patch that provides access to the tables. This is the first commit visible here. It however misses and changes some things.

Needed for #7240.

CC:  @roed314

Keywords: **package, cunningham, factorization**

Branch/Commit: **[u/rws/ticket/15813](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/ticket/15813) @ [7738db3](https://github.com/sagemath/sagetrac-mirror/commit/7738db3a4e19b16e3dd03495e92444443e2c65a4)**

Author: **David Roe, Ralf Stephan**

_Issue created by migration from https://trac.sagemath.org/ticket/15813_





---

archive/issue_events_208283.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-02-13T09:00:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15813#event-208283"
}
```



---

archive/issue_events_208284.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-02-13T09:00:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15813#event-208284"
}
```



---

archive/issue_events_208285.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-02-13T09:00:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15813#event-208285"
}
```



---

archive/issue_events_208286.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-02-13T09:00:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15813#event-208286"
}
```



---

archive/issue_comments_202191.json:
```json
{
    "body": "Changed keywords from **** to **package, cunningham, factorization**",
    "created_at": "2014-02-13T09:05:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15813#issuecomment-202191",
    "user": "https://github.com/rwst"
}
```

Changed keywords from **** to **package, cunningham, factorization**



---

archive/issue_comments_202192.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,5 +1,5 @@\n The optional spkg cunningham_tables-1.0 has no tickets, and is now available since sage-4.3.1. It is remarkably small.\n \n-According to #7240, the newest version would be http://sage.math.washington.edu/home/roed/cunningham_tables-2.2.spkg\n+According to #7240, it was voted to be included as standard. The newest version would be http://sage.math.washington.edu/home/roed/cunningham_tables-2.2.spkg\n \n Needed for #7240.\n``````\n",
    "created_at": "2014-02-15T16:57:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15813#issuecomment-202192",
    "user": "https://github.com/rwst"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,5 +1,5 @@
 The optional spkg cunningham_tables-1.0 has no tickets, and is now available since sage-4.3.1. It is remarkably small.
 
-According to #7240, the newest version would be http://sage.math.washington.edu/home/roed/cunningham_tables-2.2.spkg
+According to #7240, it was voted to be included as standard. The newest version would be http://sage.math.washington.edu/home/roed/cunningham_tables-2.2.spkg
 
 Needed for #7240.
``````




---

archive/issue_comments_202193.json:
```json
{
    "body": "Attachment: **[cunningham_tables-2.2.spkg.gz](https://github.com/sagemath/sage/files/ticket15813/cunningham_tables-2.2.spkg.gz)**\n\nDavid Roe's spkg with SAGE_DATA fix",
    "created_at": "2014-02-21T09:25:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15813#issuecomment-202193",
    "user": "https://github.com/rwst"
}
```

Attachment: **[cunningham_tables-2.2.spkg.gz](https://github.com/sagemath/sage/files/ticket15813/cunningham_tables-2.2.spkg.gz)**

David Roe's spkg with SAGE_DATA fix



---

archive/issue_comments_202194.json:
```json
{
    "body": "Author: **David Roe, Ralf Stephan**",
    "created_at": "2014-02-21T09:31:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15813#issuecomment-202194",
    "user": "https://github.com/rwst"
}
```

Author: **David Roe, Ralf Stephan**



---

archive/issue_comments_202195.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,4 +2,6 @@\n \n According to #7240, it was voted to be included as standard. The newest version would be http://sage.math.washington.edu/home/roed/cunningham_tables-2.2.spkg\n \n+It however uses the obsolete SAGE_DATA, so a fixed version of the SPKG is attached. Also, in #7240 David Roe uploaded a patch that provides access to the tables. This is the first commit visible here. It however misses and changes some things.\n+\n Needed for #7240.\n``````\n",
    "created_at": "2014-02-21T09:31:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15813#issuecomment-202195",
    "user": "https://github.com/rwst"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,4 +2,6 @@
 
 According to #7240, it was voted to be included as standard. The newest version would be http://sage.math.washington.edu/home/roed/cunningham_tables-2.2.spkg
 
+It however uses the obsolete SAGE_DATA, so a fixed version of the SPKG is attached. Also, in #7240 David Roe uploaded a patch that provides access to the tables. This is the first commit visible here. It however misses and changes some things.
+
 Needed for #7240.
``````




---

archive/issue_comments_202196.json:
```json
{
    "body": "Branch: **[u/rws/ticket/15813](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/ticket/15813)**",
    "created_at": "2014-02-21T09:53:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15813#issuecomment-202196",
    "user": "https://github.com/rwst"
}
```

Branch: **[u/rws/ticket/15813](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/ticket/15813)**



---

archive/issue_comments_202197.json:
```json
{
    "body": "Commit: **[7d1204f](https://github.com/sagemath/sagetrac-mirror/commit/7d1204fcd9a647766d20cdd3dba5ed609203302f)**",
    "created_at": "2014-02-21T09:55:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15813#issuecomment-202197",
    "user": "https://github.com/rwst"
}
```

Commit: **[7d1204f](https://github.com/sagemath/sagetrac-mirror/commit/7d1204fcd9a647766d20cdd3dba5ed609203302f)**



---

archive/issue_comments_202198.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nI reverted David's unrelated changes to `databases/install.py`\n\n@roed314: David, if you want to deprecate `database_install()` please open a separate ticket.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/391d5f5eef7e43fcbe6464d52885f87f50d60f07\">391d5f5</a></td><td><code>Trac #7240: update cunningham_tables package to version 2.2</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7d1204fcd9a647766d20cdd3dba5ed609203302f\">7d1204f</a></td><td><code>Trac #15813: add ref to databases/all.py; revert silent changes to install.py</code></td></tr></table>\n",
    "created_at": "2014-02-21T09:55:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15813#issuecomment-202198",
    "user": "https://github.com/rwst"
}
```

<div id="comment:5" align="right">Comment 5</div>

I reverted David's unrelated changes to `databases/install.py`

@roed314: David, if you want to deprecate `database_install()` please open a separate ticket.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/391d5f5eef7e43fcbe6464d52885f87f50d60f07">391d5f5</a></td><td><code>Trac #7240: update cunningham_tables package to version 2.2</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7d1204fcd9a647766d20cdd3dba5ed609203302f">7d1204f</a></td><td><code>Trac #15813: add ref to databases/all.py; revert silent changes to install.py</code></td></tr></table>




---

archive/issue_events_208287.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-02-21T09:55:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15813#event-208287"
}
```



---

archive/issue_comments_202199.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/670fb1cd18fd8b3a3a3ffd9a7e07b29b1e32241e\">670fb1c</a></td><td><code>Merge branch 'develop' into ticket/15813</code></td></tr></table>\n",
    "created_at": "2014-03-09T14:31:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15813#issuecomment-202199",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:6" align="right">Comment 6</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/670fb1cd18fd8b3a3a3ffd9a7e07b29b1e32241e">670fb1c</a></td><td><code>Merge branch 'develop' into ticket/15813</code></td></tr></table>




---

archive/issue_comments_202200.json:
```json
{
    "body": "Changed commit from **[7d1204f](https://github.com/sagemath/sagetrac-mirror/commit/7d1204fcd9a647766d20cdd3dba5ed609203302f)** to **[670fb1c](https://github.com/sagemath/sagetrac-mirror/commit/670fb1cd18fd8b3a3a3ffd9a7e07b29b1e32241e)**",
    "created_at": "2014-03-09T14:31:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15813#issuecomment-202200",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[7d1204f](https://github.com/sagemath/sagetrac-mirror/commit/7d1204fcd9a647766d20cdd3dba5ed609203302f)** to **[670fb1c](https://github.com/sagemath/sagetrac-mirror/commit/670fb1cd18fd8b3a3a3ffd9a7e07b29b1e32241e)**



---

archive/issue_comments_202201.json:
```json
{
    "body": "Changed commit from **[670fb1c](https://github.com/sagemath/sagetrac-mirror/commit/670fb1cd18fd8b3a3a3ffd9a7e07b29b1e32241e)** to **[7738db3](https://github.com/sagemath/sagetrac-mirror/commit/7738db3a4e19b16e3dd03495e92444443e2c65a4)**",
    "created_at": "2014-03-12T07:56:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15813#issuecomment-202201",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[670fb1c](https://github.com/sagemath/sagetrac-mirror/commit/670fb1cd18fd8b3a3a3ffd9a7e07b29b1e32241e)** to **[7738db3](https://github.com/sagemath/sagetrac-mirror/commit/7738db3a4e19b16e3dd03495e92444443e2c65a4)**



---

archive/issue_comments_202202.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7738db3a4e19b16e3dd03495e92444443e2c65a4\">7738db3</a></td><td><code>Merge branch 'develop' into ticket/15813</code></td></tr></table>\n",
    "created_at": "2014-03-12T07:56:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15813#issuecomment-202202",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:7" align="right">Comment 7</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7738db3a4e19b16e3dd03495e92444443e2c65a4">7738db3</a></td><td><code>Merge branch 'develop' into ticket/15813</code></td></tr></table>




---

archive/issue_comments_202203.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nWe need a plain tarball (gz/bz2) for standard packages. David, can you make one?",
    "created_at": "2014-04-13T22:08:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15813#issuecomment-202203",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:8" align="right">Comment 8</div>

We need a plain tarball (gz/bz2) for standard packages. David, can you make one?



---

archive/issue_events_208288.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15813#event-208288"
}
```



---

archive/issue_events_208289.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15813#event-208289"
}
```



---

archive/issue_comments_202204.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nSome problem when buiding in patchbot. Set flag again when tarball available.",
    "created_at": "2014-05-07T12:45:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15813#issuecomment-202204",
    "user": "https://github.com/rwst"
}
```

<div id="comment:10" align="right">Comment 10</div>

Some problem when buiding in patchbot. Set flag again when tarball available.



---

archive/issue_events_208290.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-05-07T12:45:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15813#event-208290"
}
```



---

archive/issue_events_208291.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2014-05-07T12:45:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15813#event-208291"
}
```



---

archive/issue_events_208292.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15813#event-208292"
}
```



---

archive/issue_events_208293.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15813#event-208293"
}
```



---

archive/issue_events_208294.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2018-01-15T08:16:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15813#event-208294"
}
```



---

archive/issue_events_208295.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2018-01-15T08:16:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15813#event-208295"
}
```



---

archive/issue_events_208296.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2018-01-15T08:16:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15813#event-208296"
}
```



---

archive/issue_events_208297.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2018-01-15T08:16:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15813#event-208297"
}
```



---

archive/issue_events_208298.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2018-01-15T08:16:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15813#event-208298"
}
```



---

archive/issue_comments_202205.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nThe package is now not even experimental. The spkg is no longer available. The issue is superseded by #12133 because the Brent database contained the Cunningham tables.\n\nSo this is a duplicate.",
    "created_at": "2018-01-15T08:16:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15813#issuecomment-202205",
    "user": "https://github.com/rwst"
}
```

<div id="comment:12" align="right">Comment 12</div>

The package is now not even experimental. The spkg is no longer available. The issue is superseded by #12133 because the Brent database contained the Cunningham tables.

So this is a duplicate.



---

archive/issue_events_208299.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-05-18T17:16:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "a6a6a6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15813#event-208299"
}
```



---

archive/issue_comments_202206.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nclosing positively reviewed duplicates",
    "created_at": "2018-05-18T17:16:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15813#issuecomment-202206",
    "user": "https://github.com/videlec"
}
```

<div id="comment:13" align="right">Comment 13</div>

closing positively reviewed duplicates



---

archive/issue_events_208300.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-05-18T17:16:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15813#event-208300"
}
```



---

archive/issue_events_208301.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-05-18T17:16:26Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15813",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15813#event-208301"
}
```
