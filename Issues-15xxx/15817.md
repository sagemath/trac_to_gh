...
```json
{
    "body": "The problem may come from the fact that the GAP function `ConstituentsOfCharacter` does not return the decomposition into irreducible characters, but only the set of irreducible characters that appear in the decomposition. In other words, it forgets the multiplicities.",
...
```json
{
    "body": "Here is a git branch, where I have removed the useless call to  Constituents.\n\ncould you please check that it works in more complicated cases ?\n\n---\nNew commits:",
    "created_at": "2014-09-07T14:42:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15817",
    "type": "issue_comment",
...
```json
{
    "body": "Everything works now. I have taken care of the missing fixed points, that are excluded by `Gap` from the `NaturalCharacter`.",
