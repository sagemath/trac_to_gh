# Issue 15188: quo_rem incorret for p-adic polynomials

archive/issues_014951.json:
```json
{
    "assignees": [],
    "body": "The current implementation of `quo_rem` for p-adic polynomials does not guarantee a remainder with degree less than that of the divisor:\n\n```\nsage: R.<x> = Qp(3)[]\nsage: x.quo_rem(x)\n((1 + O(3^20)), (O(3^20))*x)\n```\nThese leading zero coefficients cause problems, e.g., in #13662.\n\nDepends on #14482\nDepends on #15191\n\nCC:  @roed314\n\nBranch/Commit: **[b5303b7](https://github.com/sagemath/sagetrac-mirror/commit/b5303b77810f0b92d22724ed4cfa909202426771)**\n\nReviewer: **Xavier Caruso**\n\nAuthor: **Julian R\u00fcth**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15188_\n\n",
    "closed_at": "2017-03-12T17:16:30Z",
    "created_at": "2013-09-12T15:57:04Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20padics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/good%20first%20issue"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "quo_rem incorret for p-adic polynomials",
    "type": "issue",
    "updated_at": "2017-03-12T17:16:30Z",
    "url": "https://github.com/sagemath/sage/issues/15188",
    "user": "https://github.com/saraedum"
}
```
The current implementation of `quo_rem` for p-adic polynomials does not guarantee a remainder with degree less than that of the divisor:

```
sage: R.<x> = Qp(3)[]
sage: x.quo_rem(x)
((1 + O(3^20)), (O(3^20))*x)
```
These leading zero coefficients cause problems, e.g., in #13662.

Depends on #14482
Depends on #15191

CC:  @roed314

Branch/Commit: **[b5303b7](https://github.com/sagemath/sagetrac-mirror/commit/b5303b77810f0b92d22724ed4cfa909202426771)**

Reviewer: **Xavier Caruso**

Author: **Julian Rüth**

_Issue created by migration from https://trac.sagemath.org/ticket/15188_





---

archive/issue_events_198392.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2013-09-12T15:57:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "milestone_number": null,
    "milestone_title": "sage-6.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15188#event-198392"
}
```



---

archive/issue_events_198393.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2013-09-12T15:57:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20padics",
    "label_color": "0000ff",
    "label_name": "component: padics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15188#event-198393"
}
```



---

archive/issue_events_198394.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2013-09-12T15:57:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15188#event-198394"
}
```



---

archive/issue_events_198395.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2013-09-12T15:57:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15188#event-198395"
}
```



---

archive/issue_comments_189458.json:
```json
{
    "body": "Branch: **[u/saraedum/ticket/15188](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/15188)**",
    "created_at": "2013-09-13T13:53:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15188#issuecomment-189458",
    "user": "https://github.com/saraedum"
}
```

Branch: **[u/saraedum/ticket/15188](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/15188)**



---

archive/issue_comments_189459.json:
```json
{
    "body": "Dependencies: **#14482, #15190, #15191**",
    "created_at": "2013-09-13T13:53:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15188#issuecomment-189459",
    "user": "https://github.com/saraedum"
}
```

Dependencies: **#14482, #15190, #15191**



---

archive/issue_events_198396.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2013-12-17T18:39:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "milestone_number": null,
    "milestone_title": "sage-6.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15188#event-198396"
}
```



---

archive/issue_events_198397.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2013-12-17T18:39:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15188#event-198397"
}
```



---

archive/issue_events_198398.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15188#event-198398"
}
```



---

archive/issue_events_198399.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15188#event-198399"
}
```



---

archive/issue_events_198400.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15188#event-198400"
}
```



---

archive/issue_events_198401.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15188#event-198401"
}
```



---

archive/issue_events_198402.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15188#event-198402"
}
```



---

archive/issue_events_198403.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15188#event-198403"
}
```



---

archive/issue_comments_189460.json:
```json
{
    "body": "Commit: **[0baae9b](https://github.com/sagemath/sagetrac-mirror/commit/0baae9b7f4191597b3ade15d44100185d07d6331)**",
    "created_at": "2016-07-09T17:24:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15188#issuecomment-189460",
    "user": "https://github.com/sagetrac-jakobkroeker"
}
```

Commit: **[0baae9b](https://github.com/sagemath/sagetrac-mirror/commit/0baae9b7f4191597b3ade15d44100185d07d6331)**



---

archive/issue_comments_189461.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nseems fixed. Add a test and then ask for review?\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/901f95321554c229f00764ae180be7076b513ee9\">901f953</a></td><td><code>Added new base classes for p-adic polynomial elements</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/451a1f4cd0d0a8a71431b5f42b1b7aba4284d457\">451a1f4</a></td><td><code>Merge branch 'u/saraedum/ticket/14482' of http://trac.sagemath.org/sage into ticket/15188</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6cf3729aab2aca6f7d5461be73c887c9d2325ac9\">6cf3729</a></td><td><code>Merge branch 'u/saraedum/ticket/15191' of http://trac.sagemath.org/sage into ticket/15188</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0baae9b7f4191597b3ade15d44100185d07d6331\">0baae9b</a></td><td><code>Merge branch 'u/saraedum/ticket/14482' of http://trac.sagemath.org/sage into ticket/15188</code></td></tr></table>\n",
    "created_at": "2016-07-09T17:24:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15188#issuecomment-189461",
    "user": "https://github.com/sagetrac-jakobkroeker"
}
```

<div id="comment:7" align="right">Comment 7</div>

seems fixed. Add a test and then ask for review?

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/901f95321554c229f00764ae180be7076b513ee9">901f953</a></td><td><code>Added new base classes for p-adic polynomial elements</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/451a1f4cd0d0a8a71431b5f42b1b7aba4284d457">451a1f4</a></td><td><code>Merge branch 'u/saraedum/ticket/14482' of http://trac.sagemath.org/sage into ticket/15188</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6cf3729aab2aca6f7d5461be73c887c9d2325ac9">6cf3729</a></td><td><code>Merge branch 'u/saraedum/ticket/15191' of http://trac.sagemath.org/sage into ticket/15188</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0baae9b7f4191597b3ade15d44100185d07d6331">0baae9b</a></td><td><code>Merge branch 'u/saraedum/ticket/14482' of http://trac.sagemath.org/sage into ticket/15188</code></td></tr></table>




---

archive/issue_events_198404.json:
```json
{
    "actor": "https://github.com/sagetrac-jakobkroeker",
    "created_at": "2016-07-09T17:24:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15188#event-198404"
}
```



---

archive/issue_comments_189462.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nYes. It seems that `degree()` is now aware of leading zero coefficients.",
    "created_at": "2016-07-15T01:00:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15188#issuecomment-189462",
    "user": "https://github.com/saraedum"
}
```

<div id="comment:8" align="right">Comment 8</div>

Yes. It seems that `degree()` is now aware of leading zero coefficients.



---

archive/issue_events_198405.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2016-07-15T01:00:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15188#event-198405"
}
```



---

archive/issue_events_198406.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2016-07-15T01:00:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15188#event-198406"
}
```



---

archive/issue_comments_189463.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:",
    "created_at": "2016-07-15T01:22:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15188#issuecomment-189463",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:9" align="right">Comment 9</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:



---

archive/issue_comments_189464.json:
```json
{
    "body": "Changed commit from **[0baae9b](https://github.com/sagemath/sagetrac-mirror/commit/0baae9b7f4191597b3ade15d44100185d07d6331)** to **[b7cfa42](https://github.com/sagemath/sagetrac-mirror/commit/b7cfa42a5bae4e3e56fab9fd9398aab421430203)**",
    "created_at": "2016-07-15T01:22:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15188#issuecomment-189464",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[0baae9b](https://github.com/sagemath/sagetrac-mirror/commit/0baae9b7f4191597b3ade15d44100185d07d6331)** to **[b7cfa42](https://github.com/sagemath/sagetrac-mirror/commit/b7cfa42a5bae4e3e56fab9fd9398aab421430203)**



---

archive/issue_comments_189465.json:
```json
{
    "body": "Changed commit from **[b7cfa42](https://github.com/sagemath/sagetrac-mirror/commit/b7cfa42a5bae4e3e56fab9fd9398aab421430203)** to **[b5303b7](https://github.com/sagemath/sagetrac-mirror/commit/b5303b77810f0b92d22724ed4cfa909202426771)**",
    "created_at": "2016-07-15T02:17:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15188#issuecomment-189465",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[b7cfa42](https://github.com/sagemath/sagetrac-mirror/commit/b7cfa42a5bae4e3e56fab9fd9398aab421430203)** to **[b5303b7](https://github.com/sagemath/sagetrac-mirror/commit/b5303b77810f0b92d22724ed4cfa909202426771)**



---

archive/issue_comments_189466.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b5303b77810f0b92d22724ed4cfa909202426771\">b5303b7</a></td><td><code>Add doctest for #15188</code></td></tr></table>\n",
    "created_at": "2016-07-15T02:17:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15188#issuecomment-189466",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:10" align="right">Comment 10</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b5303b77810f0b92d22724ed4cfa909202426771">b5303b7</a></td><td><code>Add doctest for #15188</code></td></tr></table>




---

archive/issue_comments_189467.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nIt also seems to work for capped absolute. I have not checked much there since capped absolute has no special implementation of polynomials which is aware of precision issues.",
    "created_at": "2016-07-15T02:24:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15188#issuecomment-189467",
    "user": "https://github.com/saraedum"
}
```

<div id="comment:11" align="right">Comment 11</div>

It also seems to work for capped absolute. I have not checked much there since capped absolute has no special implementation of polynomials which is aware of precision issues.



---

archive/issue_comments_189468.json:
```json
{
    "body": "Author: **Julian R\u00fcth**",
    "created_at": "2016-07-15T02:24:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15188#issuecomment-189468",
    "user": "https://github.com/saraedum"
}
```

Author: **Julian Rüth**



---

archive/issue_events_198407.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2016-07-15T02:24:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15188#event-198407"
}
```



---

archive/issue_events_198408.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2016-07-15T02:24:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15188#event-198408"
}
```



---

archive/issue_events_198409.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2016-07-15T02:24:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "label": "https://github.com/sagemath/sage/labels/good%20first%20issue",
    "label_color": "7057ff",
    "label_name": "good first issue",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15188#event-198409"
}
```



---

archive/issue_comments_189469.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nSeems fine :-).",
    "created_at": "2017-02-28T21:34:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15188#issuecomment-189469",
    "user": "https://github.com/xcaruso"
}
```

<div id="comment:13" align="right">Comment 13</div>

Seems fine :-).



---

archive/issue_events_198410.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2017-02-28T21:34:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15188#event-198410"
}
```



---

archive/issue_events_198411.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2017-02-28T21:34:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15188#event-198411"
}
```



---

archive/issue_events_198412.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-03-01T02:02:32Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15188#event-198412"
}
```



---

archive/issue_events_198413.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-03-01T02:02:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "milestone_number": null,
    "milestone_title": "sage-7.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15188#event-198413"
}
```



---

archive/issue_comments_189470.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nReviewer name",
    "created_at": "2017-03-01T02:02:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15188#issuecomment-189470",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:14" align="right">Comment 14</div>

Reviewer name



---

archive/issue_comments_189471.json:
```json
{
    "body": "Reviewer: **Xavier Caruso**",
    "created_at": "2017-03-01T02:39:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15188#issuecomment-189471",
    "user": "https://github.com/roed314"
}
```

Reviewer: **Xavier Caruso**



---

archive/issue_comments_189472.json:
```json
{
    "body": "Changed dependencies from **#14482, #15190, #15191** to **#14482, #15191**",
    "created_at": "2017-03-11T17:33:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15188#issuecomment-189472",
    "user": "https://github.com/vbraun"
}
```

Changed dependencies from **#14482, #15190, #15191** to **#14482, #15191**



---

archive/issue_events_198414.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-03-12T17:16:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15188#event-198414"
}
```



---

archive/issue_events_198415.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "4d2447fa9f91319e3899868280d640eb1e3f5023",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-03-12T17:16:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15188#event-198415"
}
```



---

archive/issue_comments_189473.json:
```json
{
    "body": "Changed branch from **[u/saraedum/ticket/15188](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/15188)** to **[b5303b7](https://github.com/sagemath/sagetrac-mirror/commit/b5303b77810f0b92d22724ed4cfa909202426771)**",
    "created_at": "2017-03-12T17:16:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15188#issuecomment-189473",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/saraedum/ticket/15188](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/15188)** to **[b5303b7](https://github.com/sagemath/sagetrac-mirror/commit/b5303b77810f0b92d22724ed4cfa909202426771)**
