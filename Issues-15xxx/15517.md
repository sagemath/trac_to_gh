# Issue 15517: Run sage-upgrade *outside* of the Sage shell

archive/issues_015280.json:
```json
{
    "body": "Running `make` inside a Sage shell doesn't really work well, because `sage-env` isn't resourced during the build. This breaks upgrading if new packages are installed in the upgrade which require environment variable changes. Most of the old work-arounds in `spkg/install` removed in #14715 existed precisely for this reason.\n\nNow that nothing can upgrade **to** Sage 6.0, we should make sure that upgrading **from** Sage 6.0 doesn't suffer this problem.\n\nCC:  @vbraun @ohanar\n\nReviewer: Volker Braun, R. Andrew Ohana\n\nAuthor: Jeroen Demeyer\n\nBranch: u/vbraun/ticket/15517\n\nDependencies: #14715\n\nCommit: 1f274351a18de73d96eb67052fe49b2765de54ea\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/15517\n\n",
    "closed_at": "2013-12-16T22:53:31Z",
    "created_at": "2013-12-13T13:30:19Z",
    "labels": [
        "component: build",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.0",
    "title": "Run sage-upgrade *outside* of the Sage shell",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/15517",
    "user": "https://github.com/jdemeyer"
}
```
Running `make` inside a Sage shell doesn't really work well, because `sage-env` isn't resourced during the build. This breaks upgrading if new packages are installed in the upgrade which require environment variable changes. Most of the old work-arounds in `spkg/install` removed in #14715 existed precisely for this reason.

Now that nothing can upgrade **to** Sage 6.0, we should make sure that upgrading **from** Sage 6.0 doesn't suffer this problem.

CC:  @vbraun @ohanar

Reviewer: Volker Braun, R. Andrew Ohana

Author: Jeroen Demeyer

Branch: u/vbraun/ticket/15517

Dependencies: #14715

Commit: 1f274351a18de73d96eb67052fe49b2765de54ea

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/15517





---

archive/issue_comments_247916.json:
```json
{
    "body": "Changing dependencies from \"\" to \"#14715\"",
    "created_at": "2013-12-13T13:30:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247916",
    "user": "https://github.com/jdemeyer"
}
```

Changing dependencies from "" to "#14715"



---

archive/issue_comments_247917.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n-Running `make` inside a Sage shell doesn't really work well, because `sage-env` isn't resourced during the build. This breaks upgrading if new packages are installed in the upgrade which require environment variable changes.\n+Running `make` inside a Sage shell doesn't really work well, because `sage-env` isn't resourced during the build. This breaks upgrading if new packages are installed in the upgrade which require environment variable changes. Most of the old work-arounds in `spkg/install` removed in #14715 existed precisely for this reason.\n \n Now that nothing can upgrade **to** Sage 6.0, we should make sure that upgrading **from** Sage 6.0 doesn't suffer this problem.\n``````\n",
    "created_at": "2013-12-13T13:40:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247917",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
-Running `make` inside a Sage shell doesn't really work well, because `sage-env` isn't resourced during the build. This breaks upgrading if new packages are installed in the upgrade which require environment variable changes.
+Running `make` inside a Sage shell doesn't really work well, because `sage-env` isn't resourced during the build. This breaks upgrading if new packages are installed in the upgrade which require environment variable changes. Most of the old work-arounds in `spkg/install` removed in #14715 existed precisely for this reason.
 
 Now that nothing can upgrade **to** Sage 6.0, we should make sure that upgrading **from** Sage 6.0 doesn't suffer this problem.
``````




---

archive/issue_comments_247918.json:
```json
{
    "body": "Changing branch from \"\" to \"u/jdemeyer/ticket/15517\"",
    "created_at": "2013-12-13T19:01:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247918",
    "user": "https://github.com/jdemeyer"
}
```

Changing branch from "" to "u/jdemeyer/ticket/15517"



---

archive/issue_comments_247919.json:
```json
{
    "body": "<a id='comment:4'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                         |                                                      |\n|---------------------------------------------------------------------------------------------------------|------------------------------------------------------|\n|[bcd9b51](http://git.sagemath.org/sage.git/commit/?id=bcd9b51)|Create sage-update script, split off from sage-upgrade|",
    "created_at": "2013-12-13T19:12:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247919",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:4'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                         |                                                      |
|---------------------------------------------------------------------------------------------------------|------------------------------------------------------|
|[bcd9b51](http://git.sagemath.org/sage.git/commit/?id=bcd9b51)|Create sage-update script, split off from sage-upgrade|



---

archive/issue_comments_247920.json:
```json
{
    "body": "Changing commit from \"\" to \"bcd9b510676560b88a357da12de5880d967f5d5a\"",
    "created_at": "2013-12-13T19:12:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247920",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "" to "bcd9b510676560b88a357da12de5880d967f5d5a"



---

archive/issue_comments_247921.json:
```json
{
    "body": "<a id='comment:5'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                         |                               |\n|---------------------------------------------------------------------------------------------------------|-------------------------------|\n|[6298111](http://git.sagemath.org/sage.git/commit/?id=6298111)|Source sage-env before \"set -e\"|",
    "created_at": "2013-12-13T19:23:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247921",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:5'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                         |                               |
|---------------------------------------------------------------------------------------------------------|-------------------------------|
|[6298111](http://git.sagemath.org/sage.git/commit/?id=6298111)|Source sage-env before "set -e"|



---

archive/issue_comments_247922.json:
```json
{
    "body": "Changing commit from \"bcd9b510676560b88a357da12de5880d967f5d5a\" to \"629811164a6af2727109fac2ce1affe91a812990\"",
    "created_at": "2013-12-13T19:23:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247922",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "bcd9b510676560b88a357da12de5880d967f5d5a" to "629811164a6af2727109fac2ce1affe91a812990"



---

archive/issue_comments_247923.json:
```json
{
    "body": "<a id='comment:6'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                         |                             |\n|---------------------------------------------------------------------------------------------------------|-----------------------------|\n|[3709390](http://git.sagemath.org/sage.git/commit/?id=3709390)|Pass arguments to sage-update|",
    "created_at": "2013-12-13T19:29:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247923",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:6'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                         |                             |
|---------------------------------------------------------------------------------------------------------|-----------------------------|
|[3709390](http://git.sagemath.org/sage.git/commit/?id=3709390)|Pass arguments to sage-update|



---

archive/issue_comments_247924.json:
```json
{
    "body": "Changing commit from \"629811164a6af2727109fac2ce1affe91a812990\" to \"3709390b289870f250de60bb9a78c79ded10838b\"",
    "created_at": "2013-12-13T19:29:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247924",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "629811164a6af2727109fac2ce1affe91a812990" to "3709390b289870f250de60bb9a78c79ded10838b"



---

archive/issue_comments_247925.json:
```json
{
    "body": "<a id='comment:7'></a>\nIn theory, this needs_review, but I don't manage to test upgrading because I don't know what command line to give to `sage -upgrade`.",
    "created_at": "2013-12-13T19:48:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247925",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:7'></a>
In theory, this needs_review, but I don't manage to test upgrading because I don't know what command line to give to `sage -upgrade`.



---

archive/issue_comments_247926.json:
```json
{
    "body": "<a id='comment:8'></a>\nThere is no need to name a temporary branch, you can just work with commits:\n\n```\ngit fetch trac master\ngit merge --ff-only FETCH_HEAD\n```",
    "created_at": "2013-12-13T19:58:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247926",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:8'></a>
There is no need to name a temporary branch, you can just work with commits:

```
git fetch trac master
git merge --ff-only FETCH_HEAD
```



---

archive/issue_comments_247927.json:
```json
{
    "body": "<a id='comment:9'></a>\nThe point is that I want to test the actual `sage -upgrade` command, not emulate it with `git` commands.",
    "created_at": "2013-12-13T20:06:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247927",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:9'></a>
The point is that I want to test the actual `sage -upgrade` command, not emulate it with `git` commands.



---

archive/issue_comments_247928.json:
```json
{
    "body": "<a id='comment:10'></a>\nNo I get that, but I'm saying that the contraption in the `sage-update` script to generate a random string (as branch name) is entirely unnecessary.",
    "created_at": "2013-12-13T20:07:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247928",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:10'></a>
No I get that, but I'm saying that the contraption in the `sage-update` script to generate a random string (as branch name) is entirely unnecessary.



---

archive/issue_comments_247929.json:
```json
{
    "body": "<a id='comment:11'></a>\nReplying to [vbraun](#comment%3A10):\n> No I get that, but I'm saying that the contraption in the `sage-update` script to generate a random string (as branch name) is entirely unnecessary.\n\nPerhaps, but I guess Andrew wrote that, feel free to simplify it.",
    "created_at": "2013-12-13T20:13:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247929",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:11'></a>
Replying to [vbraun](#comment%3A10):
> No I get that, but I'm saying that the contraption in the `sage-update` script to generate a random string (as branch name) is entirely unnecessary.

Perhaps, but I guess Andrew wrote that, feel free to simplify it.



---

archive/issue_comments_247930.json:
```json
{
    "body": "Changing author from \"\" to \"Jeroen Demeyer\"",
    "created_at": "2013-12-15T10:40:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247930",
    "user": "https://github.com/jdemeyer"
}
```

Changing author from "" to "Jeroen Demeyer"



---

archive/issue_comments_247931.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2013-12-15T10:40:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247931",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_247932.json:
```json
{
    "body": "Changing branch from \"u/jdemeyer/ticket/15517\" to \"u/vbraun/ticket/15517\"",
    "created_at": "2013-12-15T20:13:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247932",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/jdemeyer/ticket/15517" to "u/vbraun/ticket/15517"



---

archive/issue_comments_247933.json:
```json
{
    "body": "Changing commit from \"3709390b289870f250de60bb9a78c79ded10838b\" to \"1f274351a18de73d96eb67052fe49b2765de54ea\"",
    "created_at": "2013-12-15T20:22:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247933",
    "user": "https://github.com/vbraun"
}
```

Changing commit from "3709390b289870f250de60bb9a78c79ded10838b" to "1f274351a18de73d96eb67052fe49b2765de54ea"



---

archive/issue_comments_247934.json:
```json
{
    "body": "<a id='comment:14'></a>\nI've simplified the script and added the `--ff-only`.\n\n---\nNew commits:\n|                                                                                                         |                                                         |\n|---------------------------------------------------------------------------------------------------------|---------------------------------------------------------|\n|[1f27435](http://git.sagemath.org/sage.git/commit/?id=1f27435)|Simplify the update script, error out if not fast-forward|",
    "created_at": "2013-12-15T20:22:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247934",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:14'></a>
I've simplified the script and added the `--ff-only`.

---
New commits:
|                                                                                                         |                                                         |
|---------------------------------------------------------------------------------------------------------|---------------------------------------------------------|
|[1f27435](http://git.sagemath.org/sage.git/commit/?id=1f27435)|Simplify the update script, error out if not fast-forward|



---

archive/issue_comments_247935.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Volker Braun\"",
    "created_at": "2013-12-15T20:22:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247935",
    "user": "https://github.com/vbraun"
}
```

Changing reviewer from "" to "Volker Braun"



---

archive/issue_comments_247936.json:
```json
{
    "body": "<a id='comment:15'></a>\nAndrew, can you review this please?",
    "created_at": "2013-12-15T21:29:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247936",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:15'></a>
Andrew, can you review this please?



---

archive/issue_comments_247937.json:
```json
{
    "body": "<a id='comment:16'></a>\nWhy are you always upgrading to the current development version, unless otherwise specified? IMO this is a user feature, developers will just be using git directly, so I would argue that it should update to the latest released version by default (which is more or less what the old one was doing -- it just checked if you were on either a released tag, or the release branch).",
    "created_at": "2013-12-15T22:49:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247937",
    "user": "https://github.com/ohanar"
}
```

<a id='comment:16'></a>
Why are you always upgrading to the current development version, unless otherwise specified? IMO this is a user feature, developers will just be using git directly, so I would argue that it should update to the latest released version by default (which is more or less what the old one was doing -- it just checked if you were on either a released tag, or the release branch).



---

archive/issue_comments_247938.json:
```json
{
    "body": "<a id='comment:17'></a>\nThere is no release branch yet. Has there been any decision on using (master, release) or (beta, master) or some variation of it? \n\nIn any case, all we have to make sure NOW is that we can update 6.0 to a future version that fixes the update script...",
    "created_at": "2013-12-15T22:53:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247938",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:17'></a>
There is no release branch yet. Has there been any decision on using (master, release) or (beta, master) or some variation of it? 

In any case, all we have to make sure NOW is that we can update 6.0 to a future version that fixes the update script...



---

archive/issue_comments_247939.json:
```json
{
    "body": "<a id='comment:18'></a>\nReplying to [vbraun](#comment%3A17):\n> There is no release branch yet. Has there been any decision on using (master, release) or (beta, master) or some variation of it?\n\n\nWhat I was imagining (although you are welcome to do whatever you wish) was that there would be a beta branch (or whatever you would want to call it) -- which would be our release candidate (i.e. only bug fix type things would get merged in here, and when the release manager is happy, it would become the next release). Secondly, when I initially wrote this (like 9 months ago), I was imagining only one release branch, however, I could easily imagine more in the future (e.g. 6.0.x, 6.1.x, etc.).\n\n> \n> In any case, all we have to make sure NOW is that we can update 6.0 to a future version that fixes the update script...\n\n\nSure. One other thing that I just noticed before I can hand off a positive review, is using origin as the remove server name. Why not just specify the actual repository, since we have no idea where origin would point to (if anywhere)? (i.e. `$SAGE_REPO_ANONYMOUS`)",
    "created_at": "2013-12-15T23:18:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247939",
    "user": "https://github.com/ohanar"
}
```

<a id='comment:18'></a>
Replying to [vbraun](#comment%3A17):
> There is no release branch yet. Has there been any decision on using (master, release) or (beta, master) or some variation of it?


What I was imagining (although you are welcome to do whatever you wish) was that there would be a beta branch (or whatever you would want to call it) -- which would be our release candidate (i.e. only bug fix type things would get merged in here, and when the release manager is happy, it would become the next release). Secondly, when I initially wrote this (like 9 months ago), I was imagining only one release branch, however, I could easily imagine more in the future (e.g. 6.0.x, 6.1.x, etc.).

> 
> In any case, all we have to make sure NOW is that we can update 6.0 to a future version that fixes the update script...


Sure. One other thing that I just noticed before I can hand off a positive review, is using origin as the remove server name. Why not just specify the actual repository, since we have no idea where origin would point to (if anywhere)? (i.e. `$SAGE_REPO_ANONYMOUS`)



---

archive/issue_comments_247940.json:
```json
{
    "body": "<a id='comment:19'></a>\nI wanted to use *master* as the release branch since we've been publicizing it for a while now. Checking out *master* should give you something that has a high probability of working ;-) Beta versions will go into *develop*. That also matches the git flow thing so far. There can be more short-lived branches but there is probably no need to publish those.\n\nAbout origin, I thought about it. But if origin does not point at our repository then you must have changed it yourself after checking out Sage. The update script shouldn't try to outsmart the user.",
    "created_at": "2013-12-16T01:27:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247940",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:19'></a>
I wanted to use *master* as the release branch since we've been publicizing it for a while now. Checking out *master* should give you something that has a high probability of working ;-) Beta versions will go into *develop*. That also matches the git flow thing so far. There can be more short-lived branches but there is probably no need to publish those.

About origin, I thought about it. But if origin does not point at our repository then you must have changed it yourself after checking out Sage. The update script shouldn't try to outsmart the user.



---

archive/issue_comments_247941.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2013-12-16T18:04:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247941",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_events_045327.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-12-16T18:04:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15517#event-45327"
}
```



---

archive/issue_comments_247942.json:
```json
{
    "body": "Resolution changed from fixed to ",
    "created_at": "2013-12-16T18:07:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247942",
    "user": "https://github.com/vbraun"
}
```

Resolution changed from fixed to 



---

archive/issue_comments_247943.json:
```json
{
    "body": "Changing status from closed to new.",
    "created_at": "2013-12-16T18:07:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247943",
    "user": "https://github.com/vbraun"
}
```

Changing status from closed to new.



---

archive/issue_events_045328.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-12-16T18:07:34Z",
    "event": "reopened",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15517#event-45328"
}
```



---

archive/issue_comments_247944.json:
```json
{
    "body": "<a id='comment:22'></a>\nSorry wrong button... needs review",
    "created_at": "2013-12-16T18:07:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247944",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:22'></a>
Sorry wrong button... needs review



---

archive/issue_comments_247945.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2013-12-16T18:07:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247945",
    "user": "https://github.com/vbraun"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_247946.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2013-12-16T20:21:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247946",
    "user": "https://github.com/ohanar"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_247947.json:
```json
{
    "body": "Changing reviewer from \"Volker Braun\" to \"Volker Braun, R. Andrew Ohana\"",
    "created_at": "2013-12-16T20:21:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247947",
    "user": "https://github.com/ohanar"
}
```

Changing reviewer from "Volker Braun" to "Volker Braun, R. Andrew Ohana"



---

archive/issue_events_045329.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-12-16T22:53:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15517#event-45329"
}
```



---

archive/issue_comments_247948.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2013-12-16T22:53:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15517",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15517#issuecomment-247948",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed
