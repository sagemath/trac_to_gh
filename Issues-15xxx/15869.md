# Issue 15869: Add the ability to compute quality bounds

archive/issues_015632.json:
```json
{
    "assignees": [],
    "body": "It is often convenient to be able to get bounds on the quality of a projective point, since computing actual quality can be a very expensive operation. The attached branch adds a function that uses trial division to get a reasonable bound on the quality of a point.\n\nBranch/Commit: **[u/ohanar/quality_bounds](https://github.com/sagemath/sagetrac-mirror/tree/u/ohanar/quality_bounds) @ [453d0ff](https://github.com/sagemath/sagetrac-mirror/commit/453d0ff2103de80c834a1d872bcf8159d7bd5f51)**\n\nAuthor: **R. Andrew Ohana**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15869_\n\n",
    "created_at": "2014-02-27T01:21:42Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20number%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/needs%20work"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add the ability to compute quality bounds",
    "type": "issue",
    "updated_at": "2022-12-29T01:33:44Z",
    "url": "https://github.com/sagemath/sage/issues/15869",
    "user": "https://github.com/ohanar"
}
```
It is often convenient to be able to get bounds on the quality of a projective point, since computing actual quality can be a very expensive operation. The attached branch adds a function that uses trial division to get a reasonable bound on the quality of a point.

Branch/Commit: **[u/ohanar/quality_bounds](https://github.com/sagemath/sagetrac-mirror/tree/u/ohanar/quality_bounds) @ [453d0ff](https://github.com/sagemath/sagetrac-mirror/commit/453d0ff2103de80c834a1d872bcf8159d7bd5f51)**

Author: **R. Andrew Ohana**

_Issue created by migration from https://trac.sagemath.org/ticket/15869_





---

archive/issue_events_209082.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2014-02-27T01:21:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15869",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15869#event-209082"
}
```



---

archive/issue_events_209083.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2014-02-27T01:21:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15869",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20theory",
    "label_color": "0000ff",
    "label_name": "component: number theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15869#event-209083"
}
```



---

archive/issue_events_209084.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2014-02-27T01:21:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15869",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15869#event-209084"
}
```



---

archive/issue_events_209085.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2014-02-27T01:21:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15869",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15869#event-209085"
}
```



---

archive/issue_events_209086.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2014-02-27T01:22:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15869",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15869#event-209086"
}
```



---

archive/issue_comments_203389.json:
```json
{
    "body": "Changed commit from **[66e29e4](https://github.com/sagemath/sagetrac-mirror/commit/66e29e4cfea4d73ce68041f4a60135ed0087af6c)** to **[52c3ebb](https://github.com/sagemath/sagetrac-mirror/commit/52c3ebb1ab2f4f2dd21ae679ad583efc4be5fd7d)**",
    "created_at": "2014-02-27T01:27:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15869#issuecomment-203389",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[66e29e4](https://github.com/sagemath/sagetrac-mirror/commit/66e29e4cfea4d73ce68041f4a60135ed0087af6c)** to **[52c3ebb](https://github.com/sagemath/sagetrac-mirror/commit/52c3ebb1ab2f4f2dd21ae679ad583efc4be5fd7d)**



---

archive/issue_comments_203390.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/52c3ebb1ab2f4f2dd21ae679ad583efc4be5fd7d\">52c3ebb</a></td><td><code>quality_bounds: fix endless loop when a coordinate is zero</code></td></tr></table>\n",
    "created_at": "2014-02-27T01:27:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15869#issuecomment-203390",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/52c3ebb1ab2f4f2dd21ae679ad583efc4be5fd7d">52c3ebb</a></td><td><code>quality_bounds: fix endless loop when a coordinate is zero</code></td></tr></table>




---

archive/issue_comments_203391.json:
```json
{
    "body": "Changed commit from **[52c3ebb](https://github.com/sagemath/sagetrac-mirror/commit/52c3ebb1ab2f4f2dd21ae679ad583efc4be5fd7d)** to **[c41ac64](https://github.com/sagemath/sagetrac-mirror/commit/c41ac649c5b1f341ce20557591cdc843de9375f4)**",
    "created_at": "2014-03-11T00:45:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15869#issuecomment-203391",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[52c3ebb](https://github.com/sagemath/sagetrac-mirror/commit/52c3ebb1ab2f4f2dd21ae679ad583efc4be5fd7d)** to **[c41ac64](https://github.com/sagemath/sagetrac-mirror/commit/c41ac649c5b1f341ce20557591cdc843de9375f4)**



---

archive/issue_comments_203392.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3117ef5a5f9a4e6d364eca6186f975b488c22815\">3117ef5</a></td><td><code>Merge remote-tracking branch 'trac/develop' into quality_bounds</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c41ac649c5b1f341ce20557591cdc843de9375f4\">c41ac64</a></td><td><code>refactor quality bounds as a class</code></td></tr></table>\n",
    "created_at": "2014-03-11T00:45:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15869#issuecomment-203392",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3117ef5a5f9a4e6d364eca6186f975b488c22815">3117ef5</a></td><td><code>Merge remote-tracking branch 'trac/develop' into quality_bounds</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c41ac649c5b1f341ce20557591cdc843de9375f4">c41ac64</a></td><td><code>refactor quality bounds as a class</code></td></tr></table>




---

archive/issue_comments_203393.json:
```json
{
    "body": "Changed commit from **[c41ac64](https://github.com/sagemath/sagetrac-mirror/commit/c41ac649c5b1f341ce20557591cdc843de9375f4)** to **[3179dd1](https://github.com/sagemath/sagetrac-mirror/commit/3179dd1d9a58e143dfb93f8997fd6ef5bce0c29e)**",
    "created_at": "2014-03-11T04:51:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15869#issuecomment-203393",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[c41ac64](https://github.com/sagemath/sagetrac-mirror/commit/c41ac649c5b1f341ce20557591cdc843de9375f4)** to **[3179dd1](https://github.com/sagemath/sagetrac-mirror/commit/3179dd1d9a58e143dfb93f8997fd6ef5bce0c29e)**



---

archive/issue_comments_203394.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3179dd1d9a58e143dfb93f8997fd6ef5bce0c29e\">3179dd1</a></td><td><code>fix off by one error</code></td></tr></table>\n",
    "created_at": "2014-03-11T04:51:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15869#issuecomment-203394",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3179dd1d9a58e143dfb93f8997fd6ef5bce0c29e">3179dd1</a></td><td><code>fix off by one error</code></td></tr></table>




---

archive/issue_comments_203395.json:
```json
{
    "body": "Changed commit from **[3179dd1](https://github.com/sagemath/sagetrac-mirror/commit/3179dd1d9a58e143dfb93f8997fd6ef5bce0c29e)** to **[453d0ff](https://github.com/sagemath/sagetrac-mirror/commit/453d0ff2103de80c834a1d872bcf8159d7bd5f51)**",
    "created_at": "2014-03-19T00:10:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15869#issuecomment-203395",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[3179dd1](https://github.com/sagemath/sagetrac-mirror/commit/3179dd1d9a58e143dfb93f8997fd6ef5bce0c29e)** to **[453d0ff](https://github.com/sagemath/sagetrac-mirror/commit/453d0ff2103de80c834a1d872bcf8159d7bd5f51)**



---

archive/issue_comments_203396.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/453d0ff2103de80c834a1d872bcf8159d7bd5f51\">453d0ff</a></td><td><code>QualityBounder: lots of fixes for non-abc triples</code></td></tr></table>\n",
    "created_at": "2014-03-19T00:10:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15869#issuecomment-203396",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:5" align="right">Comment 5</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/453d0ff2103de80c834a1d872bcf8159d7bd5f51">453d0ff</a></td><td><code>QualityBounder: lots of fixes for non-abc triples</code></td></tr></table>




---

archive/issue_events_209087.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15869",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15869#event-209087"
}
```



---

archive/issue_events_209088.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15869",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15869#event-209088"
}
```



---

archive/issue_events_209089.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15869",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15869#event-209089"
}
```



---

archive/issue_events_209090.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15869",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15869#event-209090"
}
```



---

archive/issue_events_209091.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-08-04T14:40:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15869",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15869#event-209091"
}
```



---

archive/issue_events_209092.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-08-04T14:40:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15869",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15869#event-209092"
}
```



---

archive/issue_events_209093.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-08-04T14:40:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15869",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15869#event-209093"
}
```



---

archive/issue_events_209094.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-08-04T14:40:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15869",
    "milestone_number": null,
    "milestone_title": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15869#event-209094"
}
```



---

archive/issue_comments_203397.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nneeds rebase, does not apply",
    "created_at": "2015-08-04T14:40:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15869#issuecomment-203397",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:8" align="right">Comment 8</div>

needs rebase, does not apply



---

archive/issue_events_209095.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:33:44Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15869",
    "milestone_number": null,
    "milestone_title": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15869#event-209095"
}
```
