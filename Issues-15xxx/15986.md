# Issue 15986: Python 3 preparation: Change names of some method attributes

archive/issues_015749.json:
```json
{
    "assignees": [],
    "body": "Only the modern syntax like `method.__func__` is accepted by Python 3.  \n\nChanges according to `lib2to3/fixes/fix_methodattrs.py`:\n\n```\nmethod.im_? -> method.__?__\nfor\n('im_func' | 'im_self' | 'im_class')\n```\n\nThis ticket is tracked as a dependency of meta-ticket ticket:15980.\n\n\nKeywords: **python3**\n\nBranch/Commit: **[addb5a6](https://github.com/sagemath/sagetrac-mirror/commit/addb5a6a75e926deadf1a80209ce39de0d9ca940)**\n\nReviewer: **R. Andrew Ohana**\n\nAuthor: **Wilfried Luebbe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15986_\n\n",
    "closed_at": "2014-04-04T15:55:44Z",
    "created_at": "2014-03-20T15:24:02Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20distribution",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Python 3 preparation: Change names of some method attributes",
    "type": "issue",
    "updated_at": "2014-04-04T15:55:44Z",
    "url": "https://github.com/sagemath/sage/issues/15986",
    "user": "https://github.com/wluebbe"
}
```
Only the modern syntax like `method.__func__` is accepted by Python 3.  

Changes according to `lib2to3/fixes/fix_methodattrs.py`:

```
method.im_? -> method.__?__
for
('im_func' | 'im_self' | 'im_class')
```

This ticket is tracked as a dependency of meta-ticket ticket:15980.


Keywords: **python3**

Branch/Commit: **[addb5a6](https://github.com/sagemath/sagetrac-mirror/commit/addb5a6a75e926deadf1a80209ce39de0d9ca940)**

Reviewer: **R. Andrew Ohana**

Author: **Wilfried Luebbe**

_Issue created by migration from https://trac.sagemath.org/ticket/15986_





---

archive/issue_events_210621.json:
```json
{
    "actor": "https://github.com/wluebbe",
    "created_at": "2014-03-20T15:24:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15986",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15986#event-210621"
}
```



---

archive/issue_events_210622.json:
```json
{
    "actor": "https://github.com/wluebbe",
    "created_at": "2014-03-20T15:24:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15986",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20distribution",
    "label_color": "000080",
    "label_name": "component: distribution",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15986#event-210622"
}
```



---

archive/issue_events_210623.json:
```json
{
    "actor": "https://github.com/wluebbe",
    "created_at": "2014-03-20T15:24:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15986",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15986#event-210623"
}
```



---

archive/issue_events_210624.json:
```json
{
    "actor": "https://github.com/wluebbe",
    "created_at": "2014-03-20T15:24:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15986",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15986#event-210624"
}
```



---

archive/issue_comments_205323.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nFor additional information from http://docs.python.org/2/reference/datamodel.html#index-39\n\n//Changed in version 2.6: For Python 3 forward-compatibility, im_func is also available as !__func!__, and im_self as !__self!__.//",
    "created_at": "2014-03-22T08:55:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15986",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15986#issuecomment-205323",
    "user": "https://github.com/wluebbe"
}
```

<div id="comment:1" align="right">Comment 1</div>

For additional information from http://docs.python.org/2/reference/datamodel.html#index-39

//Changed in version 2.6: For Python 3 forward-compatibility, im_func is also available as !__func!__, and im_self as !__self!__.//



---

archive/issue_events_210625.json:
```json
{
    "actor": "https://github.com/wluebbe",
    "created_at": "2014-03-22T16:01:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15986",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15986#event-210625"
}
```



---

archive/issue_comments_205324.json:
```json
{
    "body": "Commit: **[a9713ec](https://github.com/sagemath/sagetrac-mirror/commit/a9713ece0aee6b025da14e9e3dafdf97ef73e7e0)**",
    "created_at": "2014-03-22T16:01:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15986",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15986#issuecomment-205324",
    "user": "https://github.com/wluebbe"
}
```

Commit: **[a9713ec](https://github.com/sagemath/sagetrac-mirror/commit/a9713ece0aee6b025da14e9e3dafdf97ef73e7e0)**



---

archive/issue_comments_205325.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\n8 modules were changed. Only the case \"im_func -> !__func!__\" was found.\n\nOne occurrence in `sage/structure/element.pyx` had already been handled in ticket:15983.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a9713ece0aee6b025da14e9e3dafdf97ef73e7e0\">a9713ec</a></td><td><code>changed to new method attribute names (im_func -> `__func__`)</code></td></tr></table>\n",
    "created_at": "2014-03-22T16:01:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15986",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15986#issuecomment-205325",
    "user": "https://github.com/wluebbe"
}
```

<div id="comment:2" align="right">Comment 2</div>

8 modules were changed. Only the case "im_func -> !__func!__" was found.

One occurrence in `sage/structure/element.pyx` had already been handled in ticket:15983.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a9713ece0aee6b025da14e9e3dafdf97ef73e7e0">a9713ec</a></td><td><code>changed to new method attribute names (im_func -> `__func__`)</code></td></tr></table>




---

archive/issue_comments_205326.json:
```json
{
    "body": "Branch: **[u/wluebbe/ticket/15986](https://github.com/sagemath/sagetrac-mirror/tree/u/wluebbe/ticket/15986)**",
    "created_at": "2014-03-22T16:01:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15986",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15986#issuecomment-205326",
    "user": "https://github.com/wluebbe"
}
```

Branch: **[u/wluebbe/ticket/15986](https://github.com/sagemath/sagetrac-mirror/tree/u/wluebbe/ticket/15986)**



---

archive/issue_comments_205327.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nneeds work since automerging failed",
    "created_at": "2014-03-26T14:40:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15986",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15986#issuecomment-205327",
    "user": "https://github.com/wluebbe"
}
```

<div id="comment:3" align="right">Comment 3</div>

needs work since automerging failed



---

archive/issue_events_210626.json:
```json
{
    "actor": "https://github.com/wluebbe",
    "created_at": "2014-03-26T14:40:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15986",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15986#event-210626"
}
```



---

archive/issue_events_210627.json:
```json
{
    "actor": "https://github.com/wluebbe",
    "created_at": "2014-03-26T14:40:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15986",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15986#event-210627"
}
```



---

archive/issue_comments_205328.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/addb5a6a75e926deadf1a80209ce39de0d9ca940\">addb5a6</a></td><td><code>changed to new method attribute names (im_func -> `__func__`)</code></td></tr></table>\n",
    "created_at": "2014-03-26T16:09:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15986",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15986#issuecomment-205328",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/addb5a6a75e926deadf1a80209ce39de0d9ca940">addb5a6</a></td><td><code>changed to new method attribute names (im_func -> `__func__`)</code></td></tr></table>




---

archive/issue_comments_205329.json:
```json
{
    "body": "Changed commit from **[a9713ec](https://github.com/sagemath/sagetrac-mirror/commit/a9713ece0aee6b025da14e9e3dafdf97ef73e7e0)** to **[addb5a6](https://github.com/sagemath/sagetrac-mirror/commit/addb5a6a75e926deadf1a80209ce39de0d9ca940)**",
    "created_at": "2014-03-26T16:09:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15986",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15986#issuecomment-205329",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[a9713ec](https://github.com/sagemath/sagetrac-mirror/commit/a9713ece0aee6b025da14e9e3dafdf97ef73e7e0)** to **[addb5a6](https://github.com/sagemath/sagetrac-mirror/commit/addb5a6a75e926deadf1a80209ce39de0d9ca940)**



---

archive/issue_comments_205330.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nResolved automerge conflict by rebasing on 6.2.beta5.\n\n1 file was already fixed in 6.2.beta5. So only 7 files remain.",
    "created_at": "2014-03-26T16:15:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15986",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15986#issuecomment-205330",
    "user": "https://github.com/wluebbe"
}
```

<div id="comment:5" align="right">Comment 5</div>

Resolved automerge conflict by rebasing on 6.2.beta5.

1 file was already fixed in 6.2.beta5. So only 7 files remain.



---

archive/issue_events_210628.json:
```json
{
    "actor": "https://github.com/wluebbe",
    "created_at": "2014-03-27T14:16:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15986",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15986#event-210628"
}
```



---

archive/issue_events_210629.json:
```json
{
    "actor": "https://github.com/wluebbe",
    "created_at": "2014-03-27T14:16:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15986",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15986#event-210629"
}
```



---

archive/issue_comments_205331.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nI did \n\n```\ngit merge u/chapoton/15991 u/wluebbe/ticket/15986 u/wluebbe/ticket/15987\n``` \non top of `u/wluebbe/ticket/15992` (which is rebased on `6.2.beta5`)\n\nTest report:\n\n```\n./sage -t -p --all --long --logfile=logs/ptestlong-alle4.log\n...\nAll tests passed!\n```",
    "created_at": "2014-03-27T14:16:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15986",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15986#issuecomment-205331",
    "user": "https://github.com/wluebbe"
}
```

<div id="comment:6" align="right">Comment 6</div>

I did 

```
git merge u/chapoton/15991 u/wluebbe/ticket/15986 u/wluebbe/ticket/15987
``` 
on top of `u/wluebbe/ticket/15992` (which is rebased on `6.2.beta5`)

Test report:

```
./sage -t -p --all --long --logfile=logs/ptestlong-alle4.log
...
All tests passed!
```



---

archive/issue_comments_205332.json:
```json
{
    "body": "Author: **Wilfried Luebbe**",
    "created_at": "2014-04-02T17:21:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15986",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15986#issuecomment-205332",
    "user": "https://github.com/ohanar"
}
```

Author: **Wilfried Luebbe**



---

archive/issue_events_210630.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2014-04-02T17:21:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15986",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15986#event-210630"
}
```



---

archive/issue_events_210631.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2014-04-02T17:21:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15986",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15986#event-210631"
}
```



---

archive/issue_comments_205333.json:
```json
{
    "body": "Reviewer: **R. Andrew Ohana**",
    "created_at": "2014-04-02T17:21:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15986",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15986#issuecomment-205333",
    "user": "https://github.com/ohanar"
}
```

Reviewer: **R. Andrew Ohana**



---

archive/issue_comments_205334.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nLooks good.",
    "created_at": "2014-04-02T17:21:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15986",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15986#issuecomment-205334",
    "user": "https://github.com/ohanar"
}
```

<div id="comment:7" align="right">Comment 7</div>

Looks good.



---

archive/issue_events_210632.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-04-04T15:55:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15986",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15986#event-210632"
}
```



---

archive/issue_events_210633.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "8f18d90b38f2e4b405a5e36c12493787784281c2",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-04-04T15:55:44Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15986",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15986#event-210633"
}
```



---

archive/issue_comments_205335.json:
```json
{
    "body": "Changed branch from **[u/wluebbe/ticket/15986](https://github.com/sagemath/sagetrac-mirror/tree/u/wluebbe/ticket/15986)** to **[addb5a6](https://github.com/sagemath/sagetrac-mirror/commit/addb5a6a75e926deadf1a80209ce39de0d9ca940)**",
    "created_at": "2014-04-04T15:55:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15986",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15986#issuecomment-205335",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/wluebbe/ticket/15986](https://github.com/sagemath/sagetrac-mirror/tree/u/wluebbe/ticket/15986)** to **[addb5a6](https://github.com/sagemath/sagetrac-mirror/commit/addb5a6a75e926deadf1a80209ce39de0d9ca940)**
