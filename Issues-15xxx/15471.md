# Issue 15471: sage/graphs/generators/intersection.py multiplies a permutation by id

archive/issues_015234.json:
```json
{
    "assignees": [],
    "body": "and no, it's not even a poor man's coercion, because they have the same size already. My bet is someone was confused. The attached patch (split out of #15174) removes this exercise in futility.\n\nCC:  @nathanncohen\n\nKeywords: **permutation, combinat, graphs**\n\nBranch/Commit: **[u/ncohen/15471](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/15471) @ [848ddb7](https://github.com/sagemath/sagetrac-mirror/commit/848ddb7b9cc53d7b39841ef6f2674ba234ff5ad6)**\n\nReviewer: **Nathann Cohen**\n\nAuthor: **Darij Grinberg**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15471_\n\n",
    "closed_at": "2014-02-08T17:04:06Z",
    "created_at": "2013-11-29T23:10:08Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sage/graphs/generators/intersection.py multiplies a permutation by id",
    "type": "issue",
    "updated_at": "2014-02-08T17:04:06Z",
    "url": "https://github.com/sagemath/sage/issues/15471",
    "user": "https://github.com/darijgr"
}
```
and no, it's not even a poor man's coercion, because they have the same size already. My bet is someone was confused. The attached patch (split out of #15174) removes this exercise in futility.

CC:  @nathanncohen

Keywords: **permutation, combinat, graphs**

Branch/Commit: **[u/ncohen/15471](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/15471) @ [848ddb7](https://github.com/sagemath/sagetrac-mirror/commit/848ddb7b9cc53d7b39841ef6f2674ba234ff5ad6)**

Reviewer: **Nathann Cohen**

Author: **Darij Grinberg**

_Issue created by migration from https://trac.sagemath.org/ticket/15471_





---

archive/issue_events_202958.json:
```json
{
    "actor": "https://github.com/darijgr",
    "created_at": "2013-11-29T23:10:08Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15471",
    "milestone_number": null,
    "milestone_title": "sage-5.13",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15471#event-202958"
}
```



---

archive/issue_events_202959.json:
```json
{
    "actor": "https://github.com/darijgr",
    "created_at": "2013-11-29T23:10:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15471",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15471#event-202959"
}
```



---

archive/issue_events_202960.json:
```json
{
    "actor": "https://github.com/darijgr",
    "created_at": "2013-11-29T23:10:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15471",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15471#event-202960"
}
```



---

archive/issue_events_202961.json:
```json
{
    "actor": "https://github.com/darijgr",
    "created_at": "2013-11-29T23:10:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15471",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15471#event-202961"
}
```



---

archive/issue_comments_195262.json:
```json
{
    "body": "Attachment: **[trac_15471-intersection-dg.patch.gz](https://github.com/sagemath/sage/files/ticket15471/trac_15471-intersection-dg.patch.gz)**",
    "created_at": "2013-11-29T23:12:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15471#issuecomment-195262",
    "user": "https://github.com/darijgr"
}
```

Attachment: **[trac_15471-intersection-dg.patch.gz](https://github.com/sagemath/sage/files/ticket15471/trac_15471-intersection-dg.patch.gz)**



---

archive/issue_events_202962.json:
```json
{
    "actor": "https://github.com/darijgr",
    "created_at": "2013-11-29T23:13:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15471",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15471#event-202962"
}
```



---

archive/issue_comments_195263.json:
```json
{
    "body": "Branch: **[u/ncohen/15471](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/15471)**",
    "created_at": "2013-11-30T11:41:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15471#issuecomment-195263",
    "user": "https://github.com/nathanncohen"
}
```

Branch: **[u/ncohen/15471](https://github.com/sagemath/sagetrac-mirror/tree/u/ncohen/15471)**



---

archive/issue_comments_195264.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nHellooooooooooooooooo !!\n\nHMmmmmmm, makes total sense `O_O`\nAnd most probably totally my mistake `O_O`\n\nWhat do you think of this patch ? It just clears a docstring by emphasizing that both list/permutations are accepted as input (and possibly making the instructions clearer). It also uses `graphs.PermutationGraph` instead of importing the function as it was done.\n\nIf you agree with it you can set the patch to `positive_review`, otherwise tell me what's wrong.\n\nThaaaaaaaaaaaaaaaaaaaaaaaaaanks for simplifying that and for the doc ! `:-)`\n\nNathann",
    "created_at": "2013-11-30T11:41:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15471#issuecomment-195264",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:2'>Comment 2:</a>
Hellooooooooooooooooo !!

HMmmmmmm, makes total sense `O_O`
And most probably totally my mistake `O_O`

What do you think of this patch ? It just clears a docstring by emphasizing that both list/permutations are accepted as input (and possibly making the instructions clearer). It also uses `graphs.PermutationGraph` instead of importing the function as it was done.

If you agree with it you can set the patch to `positive_review`, otherwise tell me what's wrong.

Thaaaaaaaaaaaaaaaaaaaaaaaaaanks for simplifying that and for the doc ! `:-)`

Nathann



---

archive/issue_comments_195265.json:
```json
{
    "body": "Reviewer: **Nathann Cohen**",
    "created_at": "2013-11-30T11:41:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15471#issuecomment-195265",
    "user": "https://github.com/nathanncohen"
}
```

Reviewer: **Nathann Cohen**



---

archive/issue_comments_195266.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td>[848ddb7](https://github.com/sagemath/sagetrac-mirror/commit/848ddb7)</td><td><code>trac #15471: reviewer's patch</code></td></tr><tr><td>[2dcbbc1](https://github.com/sagemath/sagetrac-mirror/commit/2dcbbc1)</td><td><code>trac #15471: getting rid of a pointless product with an identity permutation</code></td></tr></table>\n",
    "created_at": "2013-11-30T11:42:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15471#issuecomment-195266",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:3'>Comment 3:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td>[848ddb7](https://github.com/sagemath/sagetrac-mirror/commit/848ddb7)</td><td><code>trac #15471: reviewer's patch</code></td></tr><tr><td>[2dcbbc1](https://github.com/sagemath/sagetrac-mirror/commit/2dcbbc1)</td><td><code>trac #15471: getting rid of a pointless product with an identity permutation</code></td></tr></table>




---

archive/issue_comments_195267.json:
```json
{
    "body": "Commit: **[848ddb7](https://github.com/sagemath/sagetrac-mirror/commit/848ddb7b9cc53d7b39841ef6f2674ba234ff5ad6)**",
    "created_at": "2013-11-30T11:42:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15471#issuecomment-195267",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[848ddb7](https://github.com/sagemath/sagetrac-mirror/commit/848ddb7b9cc53d7b39841ef6f2674ba234ff5ad6)**



---

archive/issue_events_202963.json:
```json
{
    "actor": "https://github.com/darijgr",
    "created_at": "2013-12-01T00:43:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15471",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15471#event-202963"
}
```



---

archive/issue_events_202964.json:
```json
{
    "actor": "https://github.com/darijgr",
    "created_at": "2013-12-01T00:43:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15471",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15471#event-202964"
}
```



---

archive/issue_comments_195268.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nI agree with the changes -- thanks for the review!",
    "created_at": "2013-12-01T00:43:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15471#issuecomment-195268",
    "user": "https://github.com/darijgr"
}
```

<a id='comment:4'>Comment 4:</a>
I agree with the changes -- thanks for the review!



---

archive/issue_events_202965.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-12-02T09:54:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15471",
    "milestone_number": null,
    "milestone_title": "sage-5.13",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15471#event-202965"
}
```



---

archive/issue_events_202966.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-12-02T09:54:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15471",
    "milestone_number": null,
    "milestone_title": "sage-6.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15471#event-202966"
}
```



---

archive/issue_events_202967.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2013-12-17T18:39:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15471",
    "milestone_number": null,
    "milestone_title": "sage-6.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15471#event-202967"
}
```



---

archive/issue_events_202968.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2013-12-17T18:39:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15471",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15471#event-202968"
}
```



---

archive/issue_events_202969.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15471",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15471#event-202969"
}
```



---

archive/issue_events_202970.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15471",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15471#event-202970"
}
```



---

archive/issue_events_202971.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-02-08T17:04:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15471",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15471#event-202971"
}
```



---

archive/issue_events_202972.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "594a4cdae71f2cfe9673f435d3d43ce6fb0c49ce",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-02-08T17:04:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15471",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15471#event-202972"
}
```
