# Issue 15471: sage/graphs/generators/intersection.py multiplies a permutation by id

archive/issues_015234.json:
```json
{
    "body": "and no, it's not even a poor man's coercion, because they have the same size already. My bet is someone was confused. The attached patch (split out of #15174) removes this exercise in futility.\n\nCC:  @nathanncohen\n\nKeywords: permutation, combinat, graphs\n\nReviewer: Nathann Cohen\n\nAuthor: Darij Grinberg\n\nBranch: u/ncohen/15471\n\nCommit: 848ddb7b9cc53d7b39841ef6f2674ba234ff5ad6\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/15471\n\n",
    "closed_at": "2014-02-08T17:04:06Z",
    "created_at": "2013-11-29T23:10:08Z",
    "labels": [
        "component: combinatorics",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.2",
    "title": "sage/graphs/generators/intersection.py multiplies a permutation by id",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/15471",
    "user": "https://github.com/darijgr"
}
```
and no, it's not even a poor man's coercion, because they have the same size already. My bet is someone was confused. The attached patch (split out of #15174) removes this exercise in futility.

CC:  @nathanncohen

Keywords: permutation, combinat, graphs

Reviewer: Nathann Cohen

Author: Darij Grinberg

Branch: u/ncohen/15471

Commit: 848ddb7b9cc53d7b39841ef6f2674ba234ff5ad6

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/15471





---

archive/issue_comments_246805.json:
```json
{
    "body": "Attachment [trac_15471-intersection-dg.patch](tarball://root/attachments/some-uuid/ticket15471/trac_15471-intersection-dg.patch) by @darijgr created at 2013-11-29 23:12:45",
    "created_at": "2013-11-29T23:12:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15471#issuecomment-246805",
    "user": "https://github.com/darijgr"
}
```

Attachment [trac_15471-intersection-dg.patch](tarball://root/attachments/some-uuid/ticket15471/trac_15471-intersection-dg.patch) by @darijgr created at 2013-11-29 23:12:45



---

archive/issue_comments_246806.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2013-11-29T23:13:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15471#issuecomment-246806",
    "user": "https://github.com/darijgr"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_246807.json:
```json
{
    "body": "Changing branch from \"\" to \"u/ncohen/15471\"",
    "created_at": "2013-11-30T11:41:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15471#issuecomment-246807",
    "user": "https://github.com/nathanncohen"
}
```

Changing branch from "" to "u/ncohen/15471"



---

archive/issue_comments_246808.json:
```json
{
    "body": "<a id='comment:2'></a>\nHellooooooooooooooooo !!\n\nHMmmmmmm, makes total sense `O_O`\nAnd most probably totally my mistake `O_O`\n\nWhat do you think of this patch ? It just clears a docstring by emphasizing that both list/permutations are accepted as input (and possibly making the instructions clearer). It also uses `graphs.PermutationGraph` instead of importing the function as it was done.\n\nIf you agree with it you can set the patch to `positive_review`, otherwise tell me what's wrong.\n\nThaaaaaaaaaaaaaaaaaaaaaaaaaanks for simplifying that and for the doc ! `:-)`\n\nNathann",
    "created_at": "2013-11-30T11:41:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15471#issuecomment-246808",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:2'></a>
Hellooooooooooooooooo !!

HMmmmmmm, makes total sense `O_O`
And most probably totally my mistake `O_O`

What do you think of this patch ? It just clears a docstring by emphasizing that both list/permutations are accepted as input (and possibly making the instructions clearer). It also uses `graphs.PermutationGraph` instead of importing the function as it was done.

If you agree with it you can set the patch to `positive_review`, otherwise tell me what's wrong.

Thaaaaaaaaaaaaaaaaaaaaaaaaaanks for simplifying that and for the doc ! `:-)`

Nathann



---

archive/issue_comments_246809.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Nathann Cohen\"",
    "created_at": "2013-11-30T11:41:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15471#issuecomment-246809",
    "user": "https://github.com/nathanncohen"
}
```

Changing reviewer from "" to "Nathann Cohen"



---

archive/issue_comments_246810.json:
```json
{
    "body": "<a id='comment:3'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                         |                             |\n|---------------------------------------------------------------------------------------------------------|-----------------------------|\n|[848ddb7](http://git.sagemath.org/sage.git/commit/?id=848ddb7)|trac #15471: reviewer's patch|\n|[2dcbbc1](http://git.sagemath.org/sage.git/commit/?id=2dcbbc1)|trac #15471: getting rid of a pointless product with an identity permutation|",
    "created_at": "2013-11-30T11:42:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15471#issuecomment-246810",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:3'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                         |                             |
|---------------------------------------------------------------------------------------------------------|-----------------------------|
|[848ddb7](http://git.sagemath.org/sage.git/commit/?id=848ddb7)|trac #15471: reviewer's patch|
|[2dcbbc1](http://git.sagemath.org/sage.git/commit/?id=2dcbbc1)|trac #15471: getting rid of a pointless product with an identity permutation|



---

archive/issue_comments_246811.json:
```json
{
    "body": "Changing commit from \"\" to \"848ddb7b9cc53d7b39841ef6f2674ba234ff5ad6\"",
    "created_at": "2013-11-30T11:42:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15471#issuecomment-246811",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "" to "848ddb7b9cc53d7b39841ef6f2674ba234ff5ad6"



---

archive/issue_comments_246812.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2013-12-01T00:43:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15471#issuecomment-246812",
    "user": "https://github.com/darijgr"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_246813.json:
```json
{
    "body": "<a id='comment:4'></a>\nI agree with the changes -- thanks for the review!",
    "created_at": "2013-12-01T00:43:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15471#issuecomment-246813",
    "user": "https://github.com/darijgr"
}
```

<a id='comment:4'></a>
I agree with the changes -- thanks for the review!



---

archive/issue_events_045129.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-12-02T09:54:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15471",
    "milestone": "sage-6.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15471#event-45129"
}
```



---

archive/issue_events_045130.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2013-12-17T18:39:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15471",
    "milestone": "sage-6.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15471#event-45130"
}
```



---

archive/issue_events_045131.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2013-12-17T18:39:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15471",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15471#event-45131"
}
```



---

archive/issue_events_045132.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15471",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15471#event-45132"
}
```



---

archive/issue_events_045133.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/15471",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15471#event-45133"
}
```



---

archive/issue_events_045134.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-02-08T17:04:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/15471",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15471#event-45134"
}
```



---

archive/issue_comments_246814.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2014-02-08T17:04:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15471",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15471#issuecomment-246814",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed
