# Issue 15604: pynac and polybori shouldn't be dependencies of csage

archive/issues_015367.json:
```json
{
    "assignees": [],
    "body": "The `pb_wrap.h` and `ginac_wrap.h` headers should really belong in the sage library, since csage doesn't include them.\n\nBranch/Commit: **[u/ohanar/fix_csage_deps](https://github.com/sagemath/sagetrac-mirror/tree/u/ohanar/fix_csage_deps) @ [144d006](https://github.com/sagemath/sagetrac-mirror/commit/144d006f9185299c4121eb94bd606ee1fb16d992)**\n\nReviewer: **Jeroen Demeyer**\n\nAuthor: **R. Andrew Ohana**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15604_\n\n",
    "closed_at": "2014-02-03T16:07:52Z",
    "created_at": "2013-12-29T01:35:42Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20c%20lib",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "pynac and polybori shouldn't be dependencies of csage",
    "type": "issue",
    "updated_at": "2014-02-03T16:07:52Z",
    "url": "https://github.com/sagemath/sage/issues/15604",
    "user": "https://github.com/ohanar"
}
```
The `pb_wrap.h` and `ginac_wrap.h` headers should really belong in the sage library, since csage doesn't include them.

Branch/Commit: **[u/ohanar/fix_csage_deps](https://github.com/sagemath/sagetrac-mirror/tree/u/ohanar/fix_csage_deps) @ [144d006](https://github.com/sagemath/sagetrac-mirror/commit/144d006f9185299c4121eb94bd606ee1fb16d992)**

Reviewer: **Jeroen Demeyer**

Author: **R. Andrew Ohana**

_Issue created by migration from https://trac.sagemath.org/ticket/15604_





---

archive/issue_events_205089.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2013-12-29T01:35:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15604",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15604#event-205089"
}
```



---

archive/issue_events_205090.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2013-12-29T01:35:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15604",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20c%20lib",
    "label_color": "000080",
    "label_name": "component: c lib",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15604#event-205090"
}
```



---

archive/issue_events_205091.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2013-12-29T01:35:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15604",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15604#event-205091"
}
```



---

archive/issue_events_205092.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2013-12-29T01:35:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15604",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15604#event-205092"
}
```



---

archive/issue_comments_198170.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nLike #15594, I did this to make testing python3 stuff easier.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/144d006\">144d006</a></td><td><code>csage: remove the psuedo-depedencies polybori and pynac</code></td></tr></table>\n",
    "created_at": "2013-12-29T01:37:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15604",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15604#issuecomment-198170",
    "user": "https://github.com/ohanar"
}
```

<div id="comment:1" align="right">Comment 1</div>

Like #15594, I did this to make testing python3 stuff easier.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/144d006">144d006</a></td><td><code>csage: remove the psuedo-depedencies polybori and pynac</code></td></tr></table>




---

archive/issue_events_205093.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2013-12-29T01:37:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15604",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15604#event-205093"
}
```



---

archive/issue_comments_198171.json:
```json
{
    "body": "Commit: **[144d006](https://github.com/sagemath/sagetrac-mirror/commit/144d006f9185299c4121eb94bd606ee1fb16d992)**",
    "created_at": "2013-12-29T01:37:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15604",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15604#issuecomment-198171",
    "user": "https://github.com/ohanar"
}
```

Commit: **[144d006](https://github.com/sagemath/sagetrac-mirror/commit/144d006f9185299c4121eb94bd606ee1fb16d992)**



---

archive/issue_comments_198172.json:
```json
{
    "body": "Branch: **[u/ohanar/fix_csage_deps](https://github.com/sagemath/sagetrac-mirror/tree/u/ohanar/fix_csage_deps)**",
    "created_at": "2013-12-29T01:37:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15604",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15604#issuecomment-198172",
    "user": "https://github.com/ohanar"
}
```

Branch: **[u/ohanar/fix_csage_deps](https://github.com/sagemath/sagetrac-mirror/tree/u/ohanar/fix_csage_deps)**



---

archive/issue_comments_198173.json:
```json
{
    "body": "Author: **R. Andrew Ohana**",
    "created_at": "2013-12-29T01:37:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15604",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15604#issuecomment-198173",
    "user": "https://github.com/ohanar"
}
```

Author: **R. Andrew Ohana**



---

archive/issue_events_205094.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-01-07T10:57:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15604",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15604#event-205094"
}
```



---

archive/issue_events_205095.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-01-07T10:57:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15604",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15604#event-205095"
}
```



---

archive/issue_events_205096.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-01-07T10:57:23Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/15604",
    "title_is": "pynac and polybori shouldn't be dependencies of csage",
    "title_was": "pynac and polybori are psuedo-dependencies of csage",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15604#event-205096"
}
```



---

archive/issue_comments_198174.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2014-01-07T10:57:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15604",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15604#issuecomment-198174",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_205097.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/15604",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15604#event-205097"
}
```



---

archive/issue_events_205098.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15604",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15604#event-205098"
}
```



---

archive/issue_events_205099.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-02-03T16:07:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15604",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15604#event-205099"
}
```



---

archive/issue_events_205100.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "6bdf018f058ae8f8930917ae9c92f7af787a76af",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-02-03T16:07:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15604",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15604#event-205100"
}
```
