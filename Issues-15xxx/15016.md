# Issue 15016: Update the sagenb spkg

archive/issues_014779.json:
```json
{
    "assignees": [],
    "body": "As a dependency for #14469, we need a sagenb spkg that includes https://github.com/vbraun/sagenb/commit/89c6b6ced13733bcedff7c18ae49ba0dc8db0457. \n\n\nNew spkg - [sagenb-0.10.7.2.spkg](https://googledrive.com/host/0B2kYdLdBVFHIZEd4ekpjNUFBNTQ/sagenb-0.10.7.2.spkg) corresponding to git rev [db0726fea0](https://github.com/sagemath/sagenb/commit/db0726fea0811173cff7b43484ceeec30835b1dd)\n\nCC:  @kini\n\nReviewer: **Volker Braun**\n\nAuthor: **Punarbasu Purkayastha**\n\nMerged: **sage-5.12.beta2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/15016_\n\n",
    "closed_at": "2013-08-16T21:20:26Z",
    "created_at": "2013-08-06T22:06:13Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20notebook",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.12",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Update the sagenb spkg",
    "type": "issue",
    "updated_at": "2013-09-07T08:39:52Z",
    "url": "https://github.com/sagemath/sage/issues/15016",
    "user": "https://github.com/vbraun"
}
```
As a dependency for #14469, we need a sagenb spkg that includes https://github.com/vbraun/sagenb/commit/89c6b6ced13733bcedff7c18ae49ba0dc8db0457. 


New spkg - [sagenb-0.10.7.2.spkg](https://googledrive.com/host/0B2kYdLdBVFHIZEd4ekpjNUFBNTQ/sagenb-0.10.7.2.spkg) corresponding to git rev [db0726fea0](https://github.com/sagemath/sagenb/commit/db0726fea0811173cff7b43484ceeec30835b1dd)

CC:  @kini

Reviewer: **Volker Braun**

Author: **Punarbasu Purkayastha**

Merged: **sage-5.12.beta2**

_Issue created by migration from https://trac.sagemath.org/ticket/15016_





---

archive/issue_events_195765.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-08-06T22:06:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/15016",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15016#event-195765"
}
```



---

archive/issue_events_195766.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-08-06T22:06:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15016",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20notebook",
    "label_color": "000080",
    "label_name": "component: notebook",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15016#event-195766"
}
```



---

archive/issue_events_195767.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-08-06T22:06:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15016",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15016#event-195767"
}
```



---

archive/issue_events_195768.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-08-06T22:06:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15016",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15016#event-195768"
}
```



---

archive/issue_comments_186736.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nPlease try out the new spkg. It depends on #14469. Tested\n\n```\nsage -tp --sagenb\n```\nagainst sage-5.11.rc0.",
    "created_at": "2013-08-08T09:49:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15016",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15016#issuecomment-186736",
    "user": "https://github.com/ppurka"
}
```

<a id='comment:2'>Comment 2:</a>
Please try out the new spkg. It depends on #14469. Tested

```
sage -tp --sagenb
```
against sage-5.11.rc0.



---

archive/issue_comments_186737.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,5 @@\n As a dependency for #14469, we need a sagenb spkg that includes https://github.com/vbraun/sagenb/commit/89c6b6ced13733bcedff7c18ae49ba0dc8db0457. It is merged but not in a release.\n+\n+---\n+\n+New spkg - [sagenb-0.10.7.2.spkg](https://googledrive.com/host/0B2kYdLdBVFHIZEd4ekpjNUFBNTQ/sagenb-0.10.7.2.spkg) corresponding to git rev [db0726fea0](https://github.com/sagemath/sagenb/commit/db0726fea0811173cff7b43484ceeec30835b1dd)\n``````\n",
    "created_at": "2013-08-08T09:49:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15016",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15016#issuecomment-186737",
    "user": "https://github.com/ppurka"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,5 @@
 As a dependency for #14469, we need a sagenb spkg that includes https://github.com/vbraun/sagenb/commit/89c6b6ced13733bcedff7c18ae49ba0dc8db0457. It is merged but not in a release.
+
+---
+
+New spkg - [sagenb-0.10.7.2.spkg](https://googledrive.com/host/0B2kYdLdBVFHIZEd4ekpjNUFBNTQ/sagenb-0.10.7.2.spkg) corresponding to git rev [db0726fea0](https://github.com/sagemath/sagenb/commit/db0726fea0811173cff7b43484ceeec30835b1dd)
``````




---

archive/issue_events_195769.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2013-08-08T09:49:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15016",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15016#event-195769"
}
```



---

archive/issue_comments_186738.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nWorks for me. \n\nThe SPKG.txt hasn't been updated with the git changelog in a while, so I guess there is no point in starting now.",
    "created_at": "2013-08-08T13:22:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15016",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15016#issuecomment-186738",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:3'>Comment 3:</a>
Works for me. 

The SPKG.txt hasn't been updated with the git changelog in a while, so I guess there is no point in starting now.



---

archive/issue_events_195770.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-08-08T13:22:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15016",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15016#event-195770"
}
```



---

archive/issue_events_195771.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-08-08T13:22:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/15016",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15016#event-195771"
}
```



---

archive/issue_comments_186739.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2013-08-08T13:22:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15016",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15016#issuecomment-186739",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_comments_186740.json:
```json
{
    "body": "Author: **Punarbasu Purkayastha**",
    "created_at": "2013-08-08T13:22:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15016",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15016#issuecomment-186740",
    "user": "https://github.com/vbraun"
}
```

Author: **Punarbasu Purkayastha**



---

archive/issue_comments_186741.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,5 +1,4 @@\n-As a dependency for #14469, we need a sagenb spkg that includes https://github.com/vbraun/sagenb/commit/89c6b6ced13733bcedff7c18ae49ba0dc8db0457. It is merged but not in a release.\n+As a dependency for #14469, we need a sagenb spkg that includes https://github.com/vbraun/sagenb/commit/89c6b6ced13733bcedff7c18ae49ba0dc8db0457. \n \n----\n \n New spkg - [sagenb-0.10.7.2.spkg](https://googledrive.com/host/0B2kYdLdBVFHIZEd4ekpjNUFBNTQ/sagenb-0.10.7.2.spkg) corresponding to git rev [db0726fea0](https://github.com/sagemath/sagenb/commit/db0726fea0811173cff7b43484ceeec30835b1dd)\n``````\n",
    "created_at": "2013-08-08T13:22:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15016",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15016#issuecomment-186741",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,5 +1,4 @@
-As a dependency for #14469, we need a sagenb spkg that includes https://github.com/vbraun/sagenb/commit/89c6b6ced13733bcedff7c18ae49ba0dc8db0457. It is merged but not in a release.
+As a dependency for #14469, we need a sagenb spkg that includes https://github.com/vbraun/sagenb/commit/89c6b6ced13733bcedff7c18ae49ba0dc8db0457. 
 
----
 
 New spkg - [sagenb-0.10.7.2.spkg](https://googledrive.com/host/0B2kYdLdBVFHIZEd4ekpjNUFBNTQ/sagenb-0.10.7.2.spkg) corresponding to git rev [db0726fea0](https://github.com/sagemath/sagenb/commit/db0726fea0811173cff7b43484ceeec30835b1dd)
``````




---

archive/issue_comments_186742.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nReplying to [@vbraun](#comment%3A3):\n> The SPKG.txt hasn't been updated with the git changelog in a while, so I guess there is no point in starting now.\n\nWhat is missing from the SPKG changelog? It is not complete, but it is up to date.",
    "created_at": "2013-08-08T15:16:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15016",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15016#issuecomment-186742",
    "user": "https://github.com/ppurka"
}
```

<a id='comment:5'>Comment 5:</a>
Replying to [@vbraun](#comment%3A3):
> The SPKG.txt hasn't been updated with the git changelog in a while, so I guess there is no point in starting now.

What is missing from the SPKG changelog? It is not complete, but it is up to date.



---

archive/issue_comments_186743.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nOops, got the directories mixed up when I looked at the contents. Everything is as it should be.",
    "created_at": "2013-08-08T15:33:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15016",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15016#issuecomment-186743",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:6'>Comment 6:</a>
Oops, got the directories mixed up when I looked at the contents. Everything is as it should be.



---

archive/issue_events_195772.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-16T21:20:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/15016",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15016#event-195772"
}
```



---

archive/issue_events_195773.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-16T21:20:26Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/15016",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/15016#event-195773"
}
```



---

archive/issue_comments_186744.json:
```json
{
    "body": "Merged: **sage-5.12.beta2**",
    "created_at": "2013-08-16T21:20:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15016",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15016#issuecomment-186744",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.12.beta2**



---

archive/issue_comments_186745.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nThis ticket is not fixed in the latest sage-git. I wonder why, because it has the latest sagenb.",
    "created_at": "2013-09-07T08:39:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/15016",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/15016#issuecomment-186745",
    "user": "https://github.com/ppurka"
}
```

<a id='comment:8'>Comment 8:</a>
This ticket is not fixed in the latest sage-git. I wonder why, because it has the latest sagenb.
