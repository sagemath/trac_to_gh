# Issue 15016: Update the sagenb spkg

archive/issues_014779.json:
```json
{
    "body": "As a dependency for #14469, we need a sagenb spkg that includes https://github.com/vbraun/sagenb/commit/89c6b6ced13733bcedff7c18ae49ba0dc8db0457. \n\n\nNew spkg - [sagenb-0.10.7.2.spkg](https://googledrive.com/host/0B2kYdLdBVFHIZEd4ekpjNUFBNTQ/sagenb-0.10.7.2.spkg) corresponding to git rev [db0726fea0](https://github.com/sagemath/sagenb/commit/db0726fea0811173cff7b43484ceeec30835b1dd)\n\nCC:  @kini\n\nReviewer: Volker Braun\n\nAuthor: Punarbasu Purkayastha\n\nMerged: sage-5.12.beta2\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/15016\n\n",
    "closed_at": "2013-08-16T21:20:26Z",
    "created_at": "2013-08-06T22:06:13Z",
    "labels": [
        "component: notebook",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-5.12",
    "title": "Update the sagenb spkg",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/15016",
    "user": "https://github.com/vbraun"
}
```
As a dependency for #14469, we need a sagenb spkg that includes https://github.com/vbraun/sagenb/commit/89c6b6ced13733bcedff7c18ae49ba0dc8db0457. 


New spkg - [sagenb-0.10.7.2.spkg](https://googledrive.com/host/0B2kYdLdBVFHIZEd4ekpjNUFBNTQ/sagenb-0.10.7.2.spkg) corresponding to git rev [db0726fea0](https://github.com/sagemath/sagenb/commit/db0726fea0811173cff7b43484ceeec30835b1dd)

CC:  @kini

Reviewer: Volker Braun

Author: Punarbasu Purkayastha

Merged: sage-5.12.beta2

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/15016





---

archive/issue_comments_236418.json:
```json
{
    "body": "<a id='comment:2'></a>Please try out the new spkg. It depends on #14469. Tested\n\n```\nsage -tp --sagenb\n```\nagainst sage-5.11.rc0.",
    "created_at": "2013-08-08T09:49:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15016",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15016#issuecomment-236418",
    "user": "https://github.com/ppurka"
}
```

<a id='comment:2'></a>Please try out the new spkg. It depends on #14469. Tested

```
sage -tp --sagenb
```
against sage-5.11.rc0.



---

archive/issue_comments_236419.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,5 @@\n As a dependency for #14469, we need a sagenb spkg that includes https://github.com/vbraun/sagenb/commit/89c6b6ced13733bcedff7c18ae49ba0dc8db0457. It is merged but not in a release.\n+\n+---\n+\n+New spkg - [sagenb-0.10.7.2.spkg](https://googledrive.com/host/0B2kYdLdBVFHIZEd4ekpjNUFBNTQ/sagenb-0.10.7.2.spkg) corresponding to git rev [db0726fea0](https://github.com/sagemath/sagenb/commit/db0726fea0811173cff7b43484ceeec30835b1dd)\n``````\n",
    "created_at": "2013-08-08T09:49:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15016",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15016#issuecomment-236419",
    "user": "https://github.com/ppurka"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,5 @@
 As a dependency for #14469, we need a sagenb spkg that includes https://github.com/vbraun/sagenb/commit/89c6b6ced13733bcedff7c18ae49ba0dc8db0457. It is merged but not in a release.
+
+---
+
+New spkg - [sagenb-0.10.7.2.spkg](https://googledrive.com/host/0B2kYdLdBVFHIZEd4ekpjNUFBNTQ/sagenb-0.10.7.2.spkg) corresponding to git rev [db0726fea0](https://github.com/sagemath/sagenb/commit/db0726fea0811173cff7b43484ceeec30835b1dd)
``````




---

archive/issue_comments_236420.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2013-08-08T09:49:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15016",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15016#issuecomment-236420",
    "user": "https://github.com/ppurka"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_236421.json:
```json
{
    "body": "<a id='comment:3'></a>Works for me. \n\nThe SPKG.txt hasn't been updated with the git changelog in a while, so I guess there is no point in starting now.",
    "created_at": "2013-08-08T13:22:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15016",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15016#issuecomment-236421",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:3'></a>Works for me. 

The SPKG.txt hasn't been updated with the git changelog in a while, so I guess there is no point in starting now.



---

archive/issue_comments_236422.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2013-08-08T13:22:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15016",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15016#issuecomment-236422",
    "user": "https://github.com/vbraun"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_236423.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Volker Braun\"",
    "created_at": "2013-08-08T13:22:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15016",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15016#issuecomment-236423",
    "user": "https://github.com/vbraun"
}
```

Changing reviewer from "" to "Volker Braun"



---

archive/issue_comments_236424.json:
```json
{
    "body": "Changing author from \"\" to \"Punarbasu Purkayastha\"",
    "created_at": "2013-08-08T13:22:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15016",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15016#issuecomment-236424",
    "user": "https://github.com/vbraun"
}
```

Changing author from "" to "Punarbasu Purkayastha"



---

archive/issue_comments_236425.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,5 +1,4 @@\n-As a dependency for #14469, we need a sagenb spkg that includes https://github.com/vbraun/sagenb/commit/89c6b6ced13733bcedff7c18ae49ba0dc8db0457. It is merged but not in a release.\n+As a dependency for #14469, we need a sagenb spkg that includes https://github.com/vbraun/sagenb/commit/89c6b6ced13733bcedff7c18ae49ba0dc8db0457. \n \n----\n \n New spkg - [sagenb-0.10.7.2.spkg](https://googledrive.com/host/0B2kYdLdBVFHIZEd4ekpjNUFBNTQ/sagenb-0.10.7.2.spkg) corresponding to git rev [db0726fea0](https://github.com/sagemath/sagenb/commit/db0726fea0811173cff7b43484ceeec30835b1dd)\n``````\n",
    "created_at": "2013-08-08T13:22:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15016",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15016#issuecomment-236425",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,5 +1,4 @@
-As a dependency for #14469, we need a sagenb spkg that includes https://github.com/vbraun/sagenb/commit/89c6b6ced13733bcedff7c18ae49ba0dc8db0457. It is merged but not in a release.
+As a dependency for #14469, we need a sagenb spkg that includes https://github.com/vbraun/sagenb/commit/89c6b6ced13733bcedff7c18ae49ba0dc8db0457. 
 
----
 
 New spkg - [sagenb-0.10.7.2.spkg](https://googledrive.com/host/0B2kYdLdBVFHIZEd4ekpjNUFBNTQ/sagenb-0.10.7.2.spkg) corresponding to git rev [db0726fea0](https://github.com/sagemath/sagenb/commit/db0726fea0811173cff7b43484ceeec30835b1dd)
``````




---

archive/issue_comments_236426.json:
```json
{
    "body": "<a id='comment:5'></a>Replying to [comment:3 vbraun]:\n> The SPKG.txt hasn't been updated with the git changelog in a while, so I guess there is no point in starting now.\n\nWhat is missing from the SPKG changelog? It is not complete, but it is up to date.",
    "created_at": "2013-08-08T15:16:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15016",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15016#issuecomment-236426",
    "user": "https://github.com/ppurka"
}
```

<a id='comment:5'></a>Replying to [comment:3 vbraun]:
> The SPKG.txt hasn't been updated with the git changelog in a while, so I guess there is no point in starting now.

What is missing from the SPKG changelog? It is not complete, but it is up to date.



---

archive/issue_comments_236427.json:
```json
{
    "body": "<a id='comment:6'></a>Oops, got the directories mixed up when I looked at the contents. Everything is as it should be.",
    "created_at": "2013-08-08T15:33:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15016",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15016#issuecomment-236427",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:6'></a>Oops, got the directories mixed up when I looked at the contents. Everything is as it should be.



---

archive/issue_events_043155.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-16T21:20:26Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/15016",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/15016#event-43155"
}
```



---

archive/issue_comments_236428.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2013-08-16T21:20:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15016",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15016#issuecomment-236428",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: fixed



---

archive/issue_comments_236429.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-5.12.beta2\"",
    "created_at": "2013-08-16T21:20:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15016",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15016#issuecomment-236429",
    "user": "https://github.com/jdemeyer"
}
```

Changing merged from "" to "sage-5.12.beta2"



---

archive/issue_comments_236430.json:
```json
{
    "body": "<a id='comment:8'></a>This ticket is not fixed in the latest sage-git. I wonder why, because it has the latest sagenb.",
    "created_at": "2013-09-07T08:39:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15016",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/15016#issuecomment-236430",
    "user": "https://github.com/ppurka"
}
```

<a id='comment:8'></a>This ticket is not fixed in the latest sage-git. I wonder why, because it has the latest sagenb.
