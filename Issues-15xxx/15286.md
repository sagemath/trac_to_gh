...
```json
{
    "body": "<a id='comment:7'></a>As far as I understand, when you raise the ValueError at line 162, it means that the code does not exist. Not that the entries are not consistent, right ? (I guess this is the purpose of your Warning in the documentation). I would then prefer a NotImplementedError.\n\nCould you give the definition of your product (in the doc) ? Note that there is also a different product in sage.combinat.matrices.latin_square called `direct_product` and needs four latin squares as entries.\n\nBecause of your rebase on 6.1.beta2 I need some time to compile ;-(",
...
<a id='comment:7'></a>As far as I understand, when you raise the ValueError at line 162, it means that the code does not exist. Not that the entries are not consistent, right ? (I guess this is the purpose of your Warning in the documentation). I would then prefer a NotImplementedError.
...
```json
{
    "body": "<a id='comment:10'></a>(this patch's only dependency is now positively reviewed)",
...
<a id='comment:10'></a>(this patch's only dependency is now positively reviewed)
...
```json
{
    "body": "<a id='comment:16'></a>The dependency of this ticket has been reviewed.\n\nNathann",
...
<a id='comment:16'></a>The dependency of this ticket has been reviewed.
...
```json
{
    "body": "<a id='comment:18'></a>There is a useless \"else\" before the last return.",
...
<a id='comment:18'></a>There is a useless "else" before the last return.
...
```json
{
    "body": "<a id='comment:19'></a>Indeed, but it works both ways so... `:-P`",
...
<a id='comment:19'></a>Indeed, but it works both ways so... `:-P`
...
<a id='comment:22'></a>> You must do a pull:
...
```json
{
    "body": "<a id='comment:23'></a>Apologies and thanks---I didn't know about `public/`. BTW the docs built and look fine.",
...
<a id='comment:23'></a>Apologies and thanks---I didn't know about `public/`. BTW the docs built and look fine.
...
```json
{
    "body": "<a id='comment:25'></a>Wow !! Greaaaaaaaaaat, thank you so much !! I had almost forgotten about this long sequence of tickets.... `:-)`\n\nNathann\n\n---\nNew commits:",
    "created_at": "2014-03-13T09:34:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/15286",
    "type": "issue_comment",
...
<a id='comment:25'></a>Wow !! Greaaaaaaaaaat, thank you so much !! I had almost forgotten about this long sequence of tickets.... `:-)`
